...
// Enable backup interface checking for 721s/1155s.
...
contract FlipLooksRareCheckout is Ownable {
    ...
    address constant flipDistributor = 0x6b7ED54c266eB2941FB4511C5ea038E99d82f18A;
    ...
    function _singleBuy(
            ...
            )
        {
            if (IERC165(makerAsk.collection).supportsInterface(0x80ac58cd)) {
                ...
                // Check for custom transfer manager; if none found, revert
                address transferManager = ITransferSelectorNFT(0x9Ba628F27aAc9B2D78A9f2Bf40A8a6DF4Ccd9e2c)
                    .checkTransferManagerForToken(makerAsk.collection);
                if (transferManager == 0xf42aa99F011A1fA7CDA90E5E98b277E306BcA83e) {
                    IERC721(makerAsk.collection).transferFrom(
                        address(this),
                        receiver,
                        makerAsk.tokenId
                    );
                } else if (transferManager == 0xFED24eC7E22f573c2e08AEF55aA6797Ca2b3A051) {
                    IERC1155(makerAsk.collection).safeTransferFrom(
                        address(this),
                        receiver,
                        makerAsk.tokenId,
