...
contract UTAutographFactory is Ownable, BlackHolePrevention {
    using Strings  for uint32; 

    uint8       public constant     traitType            = 5;            // Autograph
    bytes32     public constant     TRAIT_REGISTRY_ADMIN = keccak256("TRAIT_REGISTRY_ADMIN");
    ...
    function addTrait(
        ...
        require(
            crAddr != address(0),
            "UTAutographFactory: Invalid community ID"
        );
        ...
        require(
            ...
            "UTAutographFactory: Only community admin may add traits"
        );
        ...
        require(
            ...
            "UTAutographFactory: Trait Registry not installed"
        );
        ...
        if(controllerAddr == address(0)) {
            ...
            controllerCtr = UTAutographController(controllerAddr);
        }

        // Deploy _storage_
        UTAutographStorage storageCtr = new UTAutographStorage(address(traitRegistry), traitId);
...
contract UTAutographStorage {

    uint8       public constant     traitType = 5;    // Autograph Storage
    ...
    modifier onlyAllowed() {
         require(
            ...
            "UTAutographStorage: Not Authorised"
        );
        _;
    }
}
...
contract UTAutographController is UTGenericTimedController {
    ...
    function setValue(uint16[] memory tokenId, storageStruct memory value, address traitStorage) public {
        require(
            !locked,
            "Autograph Controller: contract locked"
        );
        require(
            tokenId.length > 0,
            "Autograph Controller: at least 1 token"
        );
        require(
            getTimestamp() > startTime,
            "Autograph Controller: before start time"
        );
        require(
            getTimestamp() < endTime,
            "Autograph Controller: after end time"
        );
        ...
        for(uint8 i = 0; i < tokenId.length; i++) {
            ...
            if(currentData.state == TRAIT_OPEN_VALUE && value.state == TRAIT_AUTOGRAPH_REQUESTED_VALUE) {
                ...
                require(
                    ...
                    "UT Autograph Controller: not owner of token"
                );
                ...
                require(
                    false,
                    "UT Autograph Controller: invalid state"
                );
            }
        }
    }
