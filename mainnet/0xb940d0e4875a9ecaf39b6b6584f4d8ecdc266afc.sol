...
contract FoxGame is ERC721, Ownable {
    uint256 public constant PRICE = 0.06 * 1e18;
  ...
  /** 
   * mint a token - 85% Rabbit, 15% Foxes
   * The first 22% are free to claim, the remaining cost $CARROTZ
   */
...
function freemint(uint8 amount) public payable {
    ...
    require(amount <= 5, "Can only mint 5 or less");
    require(
        minted + amount <= 200,
        "Sold out!"
    );
  ...
  /** 
   * the first 19% are paid in ETH
   * the next 27% are 100 $CARROTZ
   * the next 40% are 40000 $CARROTZ
   * the final 14% are 80000 $CARROTZ
