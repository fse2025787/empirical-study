...
interface IVault {
    ...
    function refund(address user, uint256 amount) external returns (bool);

    function depositAdmin(uint256 amount) external returns (bool);

    function updateAdmin(address _newAdmin) external returns (bool);

    function updateFundsDepositAddress(address _newFundsDepositAddress)
        external
        returns (bool);

    function updateToken(address _newToken) external returns (bool);
}
