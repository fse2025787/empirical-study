...
contract BullswapERC20 is IBullswapERC20 {
    ...
    string public constant name = "BNBull LPs";
    string public constant symbol = "BNBull-LP";
...
interface IBullswapFactory {
    ...
    function setFeeTo1(address) external;
    function setFeeTo2(address) external;
...
// 
contract BullswapPair is BullswapERC20 {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(int112(-1)) && balance1 <= uint112(int112(-1)), 'Bullswap: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo1 = IBullswapFactory(factory).feeTo1();
        address feeTo2 = IBullswapFactory(factory).feeTo2();
        feeOn = feeTo1 != address(0);
        feeOn = feeTo2 != address(0);
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) _mint(feeTo1, liquidity/2);
                    if (liquidity > 0) _mint(feeTo2, liquidity/2);
                }
            }
        } else if (_kLast != 0) {
            kLast = 0;
