...
contract Chaotic3DMind is Ownable, ERC721A {
    ...
    bool public isWlSaleStart;
    bool public isFreeMintStart;
    uint256 public isSaleStartTime;
    bytes32 public wlMerkleRoot;
    bytes32 public freeMerkleRoot;
    uint256 public fnishTime = 1 hours;
    ...
    constructor() ERC721A("Chaotic 3D Mind", "Chaotic3DMind") {}
    
    function whitelistMint(bytes32[] memory _wlMerkleProof, uint256 amount) public payable {
        require(isWlAndFreeMintStatus(),"Fnished WL Mint");
        require(isWlSaleStart, "Sale Has Not Started Yet");
        require(totalSupply() + amount <= 3333, "Maximum Total Suply");
        require(totalMint[msg.sender] + amount <= maxMintAmount, "Maximum Mintable Amount Error");
        bytes32 _leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_wlMerkleProof, wlMerkleRoot, _leaf), "Not On The WhiteList");

        if(totalWlMint[msg.sender] + amount > maxWlMintAmount){
            uint256 wlMintAmount = maxWlMintAmount - totalWlMint[msg.sender];
            uint256 publicMintAmount = amount - wlMintAmount;
            uint256 payMint = wlMintAmount * wlPrice + publicMintAmount * publicPrice;
            require(msg.value >= payMint, "Insufficient Balance");
            totalWlMint[msg.sender] += wlMintAmount;
            totalMint[msg.sender] += amount;
            _safeMint(msg.sender, amount);

        }else{
            require(totalWlMint[msg.sender] + amount <= maxWlMintAmount, "Maximum Mintable Amount Error");
    ...
    function publicMint(uint256 amount) public payable {
        require(isPublicSaleStart, "Sale Has Not Started Yet");
        require(totalSupply() + amount <= 3333, "Maximum Total Suply");
        require(msg.value >= amount * publicPrice, "Insufficient Balance");
    ...
    function freeMint(bytes32[] memory _freeMerkleProof) public {
        require(isWlAndFreeMintStatus(),"Fnished Free Mint");
        require(isFreeMintStart, "Sale Has Not Started Yet");
    ...
    function sendGift(address _to, uint256 _amount) public onlyOwner{
        require(isPublicSaleStart, "Sale Has Not Started Yet");
        require(totalSupply() + _amount <= 3333, "Maximum Total Suply");
        require(totalGift + _amount <= 333, "Fnished Free Mint");
         _safeMint(_to, _amount);
         totalGift += _amount;
    }

    function isWlAndFreeMintStatus() internal view returns(bool){
        bool statusWlAndFreeMint = isPublicSaleStart && (isSaleStartTime + fnishTime >= block.timestamp) ? true : false;
        return statusWlAndFreeMint;
    }
    ...
    function changeSaleStatus() public onlyOwner {
        isSaleStartTime = block.timestamp;
        isPublicSaleStart = !isPublicSaleStart;
        isWlSaleStart = !isWlSaleStart;
        isFreeMintStart = !isFreeMintStart;
    }
