...
contract SocialMisfits is ERC721 , DefaultOperatorFilterer {
    uint256 public totalSupply;
    uint256 public cost = 0.0016 ether;
    uint256 public maxMints = 100;
    ...
    constructor()
        ERC721("Social Misfits", "SOCIALMISFIT")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 700) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeicxhvzexmdjl2nrtpzdpcxry6pek7irfyh5qfaxtwboss77z7ggte/", Strings.toString(tokenId), ".json")
            );
    }
