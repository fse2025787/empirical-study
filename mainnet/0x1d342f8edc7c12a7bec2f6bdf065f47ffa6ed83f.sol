...
contract BoredSneakerHeads is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("BoredSneakerHeads", "BSH"){}

  function mint(uint256 amt) external payable
  {
    require(totalSupply() + amt < maxSupply + 1,"Sold out!");
    require(msg.value == amt * price,"Exact amount needed.");
    require(msg.sender == tx.origin,"Mismatched origin!");
    require(minting, "Cannot mint rn.");
    require(numberMinted(msg.sender) + amt <= maxPerWallet,"Max reached per wallet!");
    require( amt < maxPerTx + 1, "Max reacehed per transaction!");
