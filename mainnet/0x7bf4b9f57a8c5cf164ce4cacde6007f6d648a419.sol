...
// File: contracts/OutercastsErrors.sol
...
//"free mint sale is not active"
error FreeMintIsNotActive();
...
// File: contracts/IStakingController.sol
...
contract Outercasts is ERC721NES, OwnableUpgradeable, UUPSUpgradeable {
    using ECDSAUpgradeable for bytes32;

    // +1 in these constants used for gas optimization 
    // for use in conditional statements
    uint256 public constant TOTAL_MAX_SUPPLY = 5555 + 1; 
    uint256 public constant TOTAL_MAX_FREEMINTS = 1300 + 1; 
    uint256 public constant MAX_FREE_MINT_PER_WALLET = 1 + 1; 
    uint256 public constant MAX_LAUNCHLIST_MINT_PER_WALLET = 3 + 1; 
    ...
    enum MintType{ INACTIVE, PRESALE_MINT, PUBLIC_MINT }

    // Price for presale / public mint
    uint256 public constant TOKEN_PRICE = .025 ether;
    ...
    function initialize() public initializer {
        ...
        initialized = true;
        __Ownable_init_unchained();
        __ERC721A_initialize("OUTERCASTS", "Outercasts");
    ...
    modifier underMaxSupply(uint256 _quantity) {
        if(!(_totalMinted() + _quantity < TOTAL_MAX_SUPPLY - TOTAL_MAX_FREEMINTS)) revert PurchaseWouldExceedMaxSupply();
        _;
    }

    modifier underMaxFreeMintSupply(uint256 _quantity) {
    ...
    function freeMint(bytes memory _signature, uint256 _nonce, bool _toStake) 
        ...
        underMaxFreeMintSupply(1)
        numberMintedUnderAllocation(numberFreeMinted[msg.sender], 1, MAX_FREE_MINT_PER_WALLET)
    {
        _outercastsMint(_signature, 1, _nonce, MintType.PRESALE_MINT, _toStake);
        numberFreeMinted[msg.sender] += 1;
    }

    function preSaleMint(bytes memory _signature, uint256 _quantity, uint256 _nonce, bool _toStake) 
        external payable 
        callerIsUser 
        validatePreSaleActive 
        underMaxSupply(_quantity)
        numberMintedUnderAllocation(numberPreSaleMinted[msg.sender], _quantity, MAX_LAUNCHLIST_MINT_PER_WALLET)
        correctAmountSent(_quantity) 
    {
        _outercastsMint(_signature, _quantity, _nonce, MintType.PRESALE_MINT, _toStake);
        numberPreSaleMinted[msg.sender] += _quantity;
    }
    ...
    function publicMint(bytes memory _signature, uint256 _quantity, uint256 _nonce, bool _toStake) 
        ...
        correctAmountSent(_quantity) 
    {
        _outercastsMint(_signature, _quantity, _nonce,  MintType.PUBLIC_MINT, _toStake);
        numberPublicMinted[msg.sender] += _quantity;
    }

    function _outercastsMint(bytes memory _signature, uint256 _quantity, uint256 _nonce, MintType mintType, bool _toStake) 
        private 
    {
    ...
    function ownerMint(uint256 _numberToMint)
        external
        onlyOwner
        underMaxFreeMintSupply(_numberToMint)
    {
    ...
    function teamAllocationMint() external onlyOwner
        underMaxFreeMintSupply(TEAM_ALLOCATION)
    {
    ...
    // This will pay Jordan 3% of the initial sale.
    (bool js, ) = payable(0x65D38b601d386f72DBc767995F47b2B136d45521).call{value: address(this).balance * 3 / 100}("");
    require(js);
