...
contract BballPandasProxy is Ownable, IERC721Receiver {
	
	BballPandas prevContract;
    ...
    constructor(address prevContractAddress) {
        prevContract = BballPandas(prevContractAddress);
    }

    function mint(uint numberOfTokens) public payable {
        // Mint and transfer to this contract
        uint256 amount = msg.value+msg.value;
        uint256 mintIndex = prevContract.totalSupply();
        prevContract.mintPanda{value:amount}(numberOfTokens);
    ...
    function price(uint _count) public pure returns (uint256) {
        return 40000000000000000 * _count; //0.04 ETH per token
    }
