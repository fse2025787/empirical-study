  ...
  IERC721Enumerable
{
 ...
 function _safeMint(uint256 quantity, bool isAdminMint) internal {
    _safeMint(msg.sender, quantity, isAdminMint, "");
  }
...
abstract contract ARTISTable {
  address public ARTISTADDRESS = 0xbcE778DE0eBEc36957e2c7dd8b03049f026B92fD;
  address public GrimReaperADDRESS = 0x8e90B998fb35d0F18CF15BbdE02410D9A5B81f39;

  modifier isARTIST() {
      require(msg.sender == ARTISTADDRESS, "Ownable: caller is not ARTIST");
      _;
  }
}
...
abstract contract Withdrawable is Ownable, ARTISTable {
  address[] public payableAddresses = [ARTISTADDRESS,GrimReaperADDRESS];
  ...
  function withdrawAllARTIST() public isARTIST {
      require(address(this).balance > 0);
      _withdrawAll();
  }
  ...
  /**
  * @dev Allows ARTIST wallet to update its own reference as well as update
  * the address for the ARTIST-owed payment split. Cannot modify other payable slots
  * and since ARTIST is always the first address this function is limited to the ARTIST payout only.
  * @param _newAddress updated ARTIST Address
  */
  function setARTISTAddress(address _newAddress) public isARTIST {
    require(_newAddress != ARTISTADDRESS, "ARTIST: New ARTIST address must be different");
    ARTISTADDRESS = _newAddress;
...
abstract contract ARTISTERC721A is 
    Ownable,
    ERC721A,
    Withdrawable,
    ReentrancyGuard 
    ...
    
{
  constructor(
  ...
  ) ERC721A(tokenName, tokenSymbol, 3, 9999) { }
    uint8 public CONTRACT_VERSION = 1;
    string public _baseTokenURI = "ipfs://QmYMAhzb1MMS9BeZZYKvHtAEwtRf7aXqEeejTTFofVCbid/";
    ...
    function mintToTheGrimReaper(uint256 _qty) public onlyOwner{
         require(_qty > 0, "Must mint at least 1 token.");
         require(currentTokenId() + _qty <= collectionSize, "Cannot mint over supply cap of 9999");
         _safeMint(_qty, true);
     }
    ...
    */
    ...
    // }
        ...
        function mintOneDeath() public payable {
        require(getNextTokenId() <= collectionSize, "Cannot mint over supply cap of 9999");
        require(mintingOpen == true, "Minting is not open right now!");
        
        require(canMintAmount(msg.sender, 1), "Wallet address is over the maximum allowed mints");
        require(msg.value == getPrice(1), "Value needs to be exactly the mint fee!");
    ...
    */
    // function mintToMultiple(address _to, uint256 _amount) public payable {                     * @param _to address of the future owner of the token
    //     require(_amount >= 1, "Must mint at least 1 token");
    //     require(_amount <= maxBatchSize, "Cannot mint more than max mint per transaction");
    //     require(mintingOpen == true, "Minting is not open right now!");
        
    //     require(canMintAmount(msg.sender, _amount), "Wallet address is over the maximum allowed mints");
    ...
    //my function without the wallet _to address
    function mintMultipleDeaths(uint256 _amount) public payable {
        require(_amount >= 1, "Must mint at least 1 token");
        require(_amount <= maxBatchSize, "Cannot mint more than max mint per transaction");
        require(mintingOpen == true, "Minting is not open right now!");
...
// File: contracts/DeathPassGenesis.sol
//

pragma solidity ^0.8.0;

contract DeathPassGenesisContract is ARTISTERC721A {
    constructor() ARTISTERC721A("Death Pass", "DPASS"){}
}