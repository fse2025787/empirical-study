...
// File: contracts/nexus/interfaces/IGateway.sol
...
// File: contracts/nexus/interfaces/IPool.sol
...
interface IPool {
    ...
    function sellNXMTokens(uint tokenAmount) external;

    function buyNXM(uint minTokensOut) external payable;
}

// File: contracts/nexus/interfaces/INXMaster.sol
...
// File: contracts/nexus/Distributor.sol
...
contract Distributor is Initializable, ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  string public constant DEFAULT_BASE_URI = "https://brightunion.io/documents/nfts/nexus/cover.png";
  ...
  */
  ...
  returns (uint)
  {
    ...
    uint fee = coverPriceWithFee.sub(coverPrice);
    if (fee > 0) {
  ...
  */
    ...
    returns (uint claimId, uint payoutAmount, address payoutToken)
  {
    ...
    if (payoutToken == ETH) {
      ...
      IERC20Upgradeable(payoutToken).safeTransfer(msg.sender, payoutAmount);
    }
    emit ClaimSubmitted(tokenId, claimId, msg.sender);
  }
