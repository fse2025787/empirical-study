...
// File contracts/notabubble.sol

pragma solidity ^0.8.4;

contract notabubble is ERC721A, AccessControl {
    ...
    function setMaxMintPerTx(uint256 maxMintPerTx) external onlyOperator {
        MAX_MINT_AMOUNT_PER_TX = maxMintPerTx;
    }

    function setSaleActive(bool saleIsActive) external onlyOperator {
    ...
    modifier onlyOperator() {
        if (!hasRole(OPERATOR_ROLE, msg.sender)) revert AccessDenied();
        _;
    }
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        if (_mintAmount <= 0 || _mintAmount > MAX_MINT_AMOUNT_PER_TX)
    ...
    /**
     * @notice Minting 1 is free, 2 and more will cost extra 0.005 ETH each.
     * So if you're minting 2, then the payableAmount is 0.005 and _mintAmount is 2.
     * @dev Call _safeMint with the to address and quantity.
     */
    ...
    {
        if (!IS_SALE_ACTIVE) revert SaleNotActive();
    ...
    function devMint(address _to, uint256 _mintAmount) public onlyOperator {
        if (_totalMinted() + _mintAmount > MAX_SUPPLY)
            revert MaxSupplyExceeded();
    ...
    function withdraw() public onlyOwner {
        (bool success, ) = payable(0x7b5925E6Bc4EB21a56F5413b4C7EB0a6abBa8c3D).call{
