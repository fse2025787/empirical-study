...
contract Baby_Balls is ERC721, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmcLo28cQsqmb695UEnm87Png3c5ZbB4CMXt4Avsspm5Lb/";
    uint256 public Balls_FREE_RESERVE = 1000;
    uint256 public Balls_PUBLIC = 3000;
    uint256 public Balls_MAX = Balls_FREE_RESERVE + Balls_PUBLIC;
    uint256 public Balls_PRICE = 0.01 ether;
    ...
    constructor() ERC721("Baby Balls", "Ball") {}

    function gift(address[] calldata receivers) external onlyOwner {
        require(_tokensMinted.current() + receivers.length <= Balls_MAX, "EXCEED_MAX");
    ...
    function founderMint(uint256 tokenQuantity) external onlyOwner {
        require(_tokensMinted.current() + tokenQuantity <= Balls_MAX, "EXCEED_MAX");
    ...
    function claim(uint256 tokenQuantity) external {
        ...
        require(claimTokensMinted + tokenQuantity <= Balls_FREE_RESERVE, "EXCEED_FREE_RESERVE");
        require(_tokensMinted.current() + tokenQuantity <= Balls_MAX, "EXCEED_MAX");
    ...
    function mint(uint256 tokenQuantity) external payable {
        ...
        require(publicTokensMinted + tokenQuantity <= Balls_PUBLIC, "EXCEED_PUBLIC");
        require(_tokensMinted.current() + tokenQuantity <= Balls_MAX, "EXCEED_MAX");
        require(tokenQuantity <= 15, "EXCEED_PER_MINT");
        require(Balls_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
