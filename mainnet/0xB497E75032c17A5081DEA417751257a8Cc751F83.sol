...
contract MOAR is ERC721, IERC2981, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private tokenCounter;

    string private baseURI = "https://metadata.thefwenclub.com/moar";
    ...
    modifier maxMintsPerTX(uint256 numberOfTokens) {
        require(
            numberOfTokens <= _transactionLimit(),
    ...
    function _transactionLimit() internal view returns (uint256) {
        if (tokenCounter.current() < NUM_FREE_MINTS) {
    ...
    constructor(
    ) ERC721("MOAR by Joan Cornella", "MOAR") {
    }
