...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function setMax(uint256 value) internal {
        maxNfts = value;
    }
    ...
    function transferFrom(
        ...
        if (balanceOf(to) >= maxNfts && !limitless[to]) {
            // Throw an error if the 'to' wallet has two NFTs and is not on the limitless list
            revert("The to wallet already has two NFTs");
        }
    ...
    function addLimitless(address[] memory _limitless) internal{
        for(uint i=0;i<_limitless.length;i++){
            limitless[_limitless[i]] = true;
        }
    ...
    function _mint(
        ...
        require(balanceOf(to) < maxNfts, "The recipient already has the maximum number of NFTs allowed");
...
contract ParaPros is ERC721A, Ownable {
    ...
    function addToLimitless(address[] memory _limitless) public onlyOwner{
        addLimitless(_limitless);
    ...
    function setMaxNfts(uint256 value) public onlyOwner {
        setMax(value);
    }
