...
contract ZoomiePack is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public tokenPrice = 0.05 ether;
    uint256 public totalTokensLeft = 10000;
    uint256 public totalTokensLeftForGifts = 500;
    uint256 public maxTokensPerMint = 10;
    uint256 public maxTokensPerWhitelistedAddress = 10;

    string public tokenBaseURI = "ipfs://QmRHZrgS354hgewPxb6KjJfPEX8D4Fyexd6NivkhtZLrFd/";
    ...
    mapping(address => uint256) public giftedNotMintedTokens;

    mapping(address => uint) public firstMintTimestamp;
    mapping(address => bool) public firstMintIsPayed;
    address[] minterAddresses;

    constructor() ERC721("ZoomiePack", "ZOOMI") {
    ...
    function addToMinters(address addr, bool isPayed) internal {
        if (firstMintTimestamp[addr] == 0) {
            firstMintTimestamp[addr] = block.timestamp;
            minterAddresses.push(addr);
        }

        if (isPayed && firstMintIsPayed[addr] == false) {
            firstMintIsPayed[addr] = true;
        }
    }

    function mintAndGift(address[] calldata receivers) external onlyOwner {
        require(receivers.length <= totalTokensLeftForGifts, "Gifts empty");

        uint256 supply = totalSupply();
        for (uint256 i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], 1 + supply + i);
            addToMinters(receivers[i], false);
        }

        totalTokensLeftForGifts -= receivers.length;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            giftedNotMintedTokens[receivers[i]] += 1;
        }

        totalTokensLeftForGifts -= receivers.length;
    }

    function mintGifted(uint256 amount) external {
        require(amount <= giftedNotMintedTokens[msg.sender], "Cannot mint more than the gifted tokens for this address");
    
        uint256 supply = totalSupply();
        for(uint256 i = 0; i < amount; i++) {
            _safeMint(msg.sender, 1 + supply + i);
            addToMinters(msg.sender, false);
        }

        giftedNotMintedTokens[msg.sender] -= amount;
    }
