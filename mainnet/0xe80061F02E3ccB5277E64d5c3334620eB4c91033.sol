...
// File: contracts/SmartContract_v5_royalty.sol

pragma solidity >=0.7.0 <0.9.0;

contract SmartContract is ERC721Enumerable, IERC2981, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_ROYALTIES_PERCENTAGE = 2000;
    uint256 public royaltiesPercentage = 1000;
    uint256 public campaignNum = 1;
    bool public paused = false;
    bool public revealed = false;
    bool public onlyWhitelisted = true;
    address[] public whitelistedAddresses;

    //this mapping is map with campaign num, so that i can change the num for different presales
    //we don't need to reset the mapping, we just need to set the campaign num with function
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            if(onlyWhitelisted == true) {
                ...
                uint256 ownerMintedCount = addressMintedBalance[campaignNum][msg.sender];
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            addressMintedBalance[campaignNum][msg.sender]++;
    ...
    function setCampaignNum(uint256 _num) public onlyOwner() {
        campaignNum = _num;
    }
