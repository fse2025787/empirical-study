...
contract FarmCerts is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public stakeFee = 5;
    uint256 public unStakeFee = 5;
    uint256 public earlyUnstakeFee = 50;
    uint256 public earlyUnstakeSeconds = 604800; // 1 week.

    address public teamAddress =
        address(0x411718D37ed44f7b85fF6656Cce49D7b7EdF1AcF);
    ...
    function addRewards(uint256 amount) public onlyOwner {
        ...
        if (pool.creationBlock == 0) {
            ...
            pool.perBlockNum = (amount.add(currentBal)).div(
                _farmIntervalBlocks
            );
            ...
            uint256 newPerBlockNum = pool
                .perBlockNum
                .mul(amount.add(_cumulativeOrigTotalRewards))
                .div(_cumulativeOrigTotalRewards);
    ...
    function changeEmissionPerBlock(uint256 newEmissionAmount)
        public
        onlyOwner
    {
    ...
    // harvest on behalf of a user and send to their address.
    ...
    // Emergency unstake on behalf of user and send to their address.
    ...
    function getLastStakableBlock() public view returns (uint256) {
        uint256 _blockToAdd = pool.creationBlock == 0 // no stakers yet
            ? block.number
            : pool.creationBlock;
...
contract RewardVault is Ownable {
  ...
  function withdrawResidual(IERC20 token) public {
    require(authorizedAddresses[msg.sender] == true, 'unauthorized');
