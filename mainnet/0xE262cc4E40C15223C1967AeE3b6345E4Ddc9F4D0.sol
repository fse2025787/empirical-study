...
contract AwooStore is OwnerAdminGuard {
    ...
    constructor(address payable withdrawAddr, IAwooToken awooTokenContract, IAwooClaimingV2 claimingContract){
        require(withdrawAddr != address(0), "Withdraw address cannot be the zero address");
    ...
    function mintWithEth(uint16 itemId, uint256 qty) public payable whenStoreActive nonZeroQuantity(qty) {
        ItemDetail memory item = validateItem(itemId);
        require(!item.NonMintable, "Specified item is not mintable");
        require(item.PmtType == PaymentType.ETHER, "Specified item cannot be purchased with Ether");
        validateRequestedQuantity(itemId, item, qty);
        ensureAvailablity(item, itemId, qty);
        validateEtherValue(item, qty);
    ...
    function mintWithEthAndAwoo(uint16 itemId, uint256 qty, AwooSpendApproval calldata approval, 
        ...
        ItemDetail memory item = validateItem(itemId);
        require(!item.NonMintable, "Specified item is not mintable");
        require(item.PmtType == PaymentType.AWOO_AND_ETHER, "Specified item cannot be purchased with AWOO and Ether");
        validateRequestedQuantity(itemId, item, qty);
        ensureAvailablity(item, itemId, qty);
        validateEtherValue(item, qty);
    ...
    function purchaseWithAwoo(uint16 itemId, uint256 qty, AwooSpendApproval calldata approval, 
        ...
        nonMintedItemPurchaseCounts[itemId] += qty;
    ...
    function purchaseAndApplyWithAwoo(uint16 itemId, AwooSpendApproval calldata approval, 
        ...
        ItemDetail memory item = validateItem(itemId);
        require(item.PmtType == PaymentType.AWOO, "Specified item cannot be purchased with AWOO");
        validateRequestedQuantity(itemId, item, applicationTokenIds.length);
        ensureAvailablity(item, itemId, applicationTokenIds.length);

        claimAwoo(requestedClaims);
        awooContract.spendVirtualAwoo(approval.Hash, approval.Sig, approval.Nonce, _msgSender(), applicationTokenIds.length * item.AWOOPrice);

        applyItem(itemId, applicationTokenIds);
    }
    ...
    function applyOwnedItem(uint16 itemId, uint32[] calldata applicationTokenIds) public whenStoreActive {
        ItemDetail memory item = validateItem(itemId);       
        require(applicationTokenIds.length <= ownedItemCountsByOwner[_msgSender()][itemId], "Requested qty exceeds owned quantity");
        
        for(uint256 i; i < applicationTokenIds.length; i++){
            applyItem(item, itemId, applicationTokenIds[i]);
        }

        if(item.Burnable) {
            ownedItemCountsByOwner[_msgSender()][itemId] -= applicationTokenIds.length;
        }
    }
    ...
    function useOwnedItem(uint16 itemId, uint256 qty) public whenStoreActive nonZeroQuantity(qty) {
        ItemDetail memory item = validateItem(itemId);       
        require(qty <= ownedItemCountsByOwner[_msgSender()][itemId], "Requested qty exceeds owned quantity");
       
        if(item.Burnable) {
            ownedItemCountsByOwner[_msgSender()][itemId] -= qty;
        }

        emit NonMintedItemUsed(itemId, _msgSender(), qty);
    }
    ...
    function applyMintedItem(uint16 itemId, uint32[] calldata applicationTokenIds) public whenStoreActive {
        ItemDetail memory item = validateItem(itemId);
        require(!item.NonMintable, "Specified item is not mintable");
        require(applicationTokenIds.length <= awooMintableCollectionContract.balanceOf(_msgSender(), item.TokenId), "Requested application qty exceeds owned qty");
        ...
        if(item.Burnable) {
            awooMintableCollectionContract.burn(_msgSender(), item.TokenId, applicationTokenIds.length);
        }
    }
    ...
    function updateItem(uint16 itemId, ItemDetail memory newItem) external onlyOwnerOrAdmin {
        ...
        ItemDetail memory existingItem = validateItem(itemId);

        require(existingItem.NonMintable == newItem.NonMintable, "Item mintability cannot change");
        require(newItem.TotalAvailable <= availableQty(existingItem, itemId), "Purchased quantity exceeds new total available quantity");
        
        if(!existingItem.NonMintable) {
            newItem.TokenId = existingItem.TokenId;

            if(
                bytes(newItem.MetadataUri).length != bytes(existingItem.MetadataUri).length || 
                keccak256(abi.encodePacked(newItem.MetadataUri)) != keccak256(abi.encodePacked(existingItem.MetadataUri))
                ) {
                    awooMintableCollectionContract.setTokenUri(existingItem.TokenId, newItem.MetadataUri);
            }

            if(newItem.Active != existingItem.Active) {
                awooMintableCollectionContract.setTokenActive(existingItem.TokenId, newItem.Active);
            }
        }
    ...
    function ensureAvailablity(ItemDetail memory item, uint16 itemId, uint256 requestedQty) private view {
        require(requestedQty <= availableQty(item, itemId), "Exceeds available quantity");
    }

    function availableQty(uint16 itemId) public view returns(uint256){
        ItemDetail memory item = validateItem(itemId);
        return availableQty(item, itemId);
    }
    function availableQty(ItemDetail memory item, uint16 itemId) private view returns(uint256) {
        ...
        if(!item.NonMintable) {
            mintedCount = awooMintableCollectionContract.totalMinted(item.TokenId);
        }
        return item.TotalAvailable-mintedCount-nonMintedItemPurchaseCounts[itemId];
    }
    ...
    function isWithinAddressLimit(uint16 itemId, ItemDetail memory item, uint256 requestedQty, address recipient) public view returns(bool) {
        if(item.PerAddressLimit > 0) {
            uint256 tokenCountByOwner = item.NonMintable
                ...
                : ownedItemCountsByOwner[recipient][itemId] + mintedItemCountsByAddress[recipient][itemId];
    ...
    function checkItemTokenApplicationStatus(uint8 collectionId, uint16 itemId, uint256[] calldata tokenIds
        ...
        for(uint256 i; i < tokenIds.length;i++){
            ...
            applicationCounts[i] = tokenAppliedItemCountsByCollection[collectionId][tokenId][itemId];
        }
    ...
    function setActive(bool active) external onlyOwnerOrAdmin {
        if(active){
            require(address(awooMintableCollectionContract) != address(0), "Awoo collection has not been set");
        }
        storeActive = active;
    }
    ...
    function setWithdrawAddress(address payable addr) external onlyOwnerOrAdmin {
        require(addr != address(0), "Withdraw address cannot be the zero address");
        withdrawAddress = addr;
    }

    function withdraw(uint256 amount) external onlyOwnerOrAdmin {
        require(amount <= address(this).balance, "Amount exceeds balance");
        require(payable(withdrawAddress).send(amount));
    }
    ...
    function rescueArbitraryERC20(IERC20 token) external {
        ...
        require(token.transfer(payable(withdrawAddress), balance), "Transfer failed");
    }
