...
contract ZigZag is Context, IERC20, Ownable {
    ...
    string private constant _name = "Zig.Zag";
    ...
    uint256 public opsBuyFee = 70;
    uint256 private _previousOpsBuyFee = opsBuyFee;
    uint256 public opsSellFee = 70;
    ...
    function okayLetsGo(uint256 blocks) public onlyOwner {
        ...
        maxBuy = _totalSupply.mul(75).div(10000);
        maxSell = _totalSupply.mul(75).div(10000);
        maxWallet = _totalSupply.mul(150).div(10000);
        _swapTokensAtAmount = _totalSupply.mul(5).div(100000);
        tradingOpen = true;
    ...
    function CONFIG_maxBuy(uint256 amount) public onlyOwner {
        require(amount >= (totalSupply().mul(1).div(10000)), "Max buy cannot be lower than 0.01% total supply.");
        maxBuy = amount;
    }

    function CONFIG_maxSell(uint256 amount) public onlyOwner {
        require(amount >= (totalSupply().mul(1).div(10000)), "Max sell cannot be lower than 0.01% total supply.");
        maxSell = amount;
    }
    
    function CONFIG_maxWallet(uint256 amount) public onlyOwner {
        require(amount >= (totalSupply().mul(1).div(1000)), "Max wallet cannot be lower than 0.1% total supply.");
        maxWallet = amount;
    }
    
    function CONFIG_SwapTokensAtAmount(uint256 amount) public onlyOwner {
        require(amount >= (totalSupply().mul(1).div(100000)), "Swap amount cannot be lower than 0.001% total supply.");
        require(amount <= (totalSupply().mul(5).div(1000)), "Swap amount cannot be higher than 0.5% total supply.");
    ...
    function AC_setBlacklisted(address[] memory accounts, bool isBL) public onlyOwner {
        for (uint i = 0; i < accounts.length; i++) _isBlacklisted[accounts[i]] = isBL;
    }

    function UTILS_Unclog() public {
        require(msg.sender == opsWalletAddress, "Unauthorized");
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForETH(contractBalance);
    }
