...
// 
// File: erc721mok/IERC721MintTx.sol


pragma solidity ^0.8.16;

interface IERC721MintTx {
    // 铸造TokenId
    function safeMintTokenIdUri(address to, uint256 tokenId, string memory tokenUri) external;
    // 交易TokenId
    function safeTxTokenId(address from, address to, uint256 tokenId) external;
}
...
contract ERC721 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, EIP712Upgradeable, ERC721VotesUpgradeable, IERC721MintTx{
    event PermanentURI(string _value, uint256 indexed _id);

    // 预售Uri
    string private _preSaleUri = "https://arweave.net/M4cTgZGPngstdS7Zg8sB_jg3bv-HfACW9-iaBkXCk1w";

    IAllowVisit private _allowVisit;

    // 是否正式发布
    bool private _releaseOfficially;

    function initialize(string memory name, string memory symbol, address allowVisit) initializer public {
        ...
        __ERC721Votes_init();
        _allowVisit = IAllowVisit(allowVisit);
    }

    function releaseOfficially(bool flag) public onlyOwner {
        _releaseOfficially = flag;
    }

    function getReleaseOfficially() public onlyOwner view returns (bool flag){
        flag = _releaseOfficially;
    }

    // 设置允许访问的合约
    function setAllowVisit(address allowVisit) public onlyOwner {
        _allowVisit = IAllowVisit(allowVisit);
    }

    // 设置预售token URI
    function setPreSaleURI(string memory preSaleUri) public onlyOwner {
        _preSaleUri = preSaleUri;
    }

    // 获取预售token URI
    function getPreSaleURI() public onlyOwner view returns (string memory) {
        return _preSaleUri;
    }

    function safeTxTokenId(address from, address to, uint256 tokenId) public virtual override {
        require(_allowVisit.isAllowVisit(msg.sender), "address not permissions");
        require(tokenId > 0, "tokenId require > 0");
        _safeTransfer(from, to, tokenId, bytes(""));
    }

    function safeMintTokenIdUri(address to, uint256 tokenId, string memory tokenUri) public virtual override {
        require(_allowVisit.isAllowVisit(msg.sender), "address not permissions");
        require(tokenId > 0, "tokenId require > 0");
        _safeMint(to, tokenId);
    ...
    function safeSetTokenURI(uint256 tokenId) public onlyOwner {
        require(tokenId > 0, "tokenId require > 0");
