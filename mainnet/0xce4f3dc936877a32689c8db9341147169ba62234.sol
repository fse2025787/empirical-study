...
contract TrialXdNFT is ERC721A, Ownable {
    ...
    // 0: mintpass/leatherface

    // 1: whitelist and mintpass/leatherface

    // 2: public and mintpass/leatherface
    ...
    uint256 public maxPassAndLeatherfaceMintsTotal = 10;
    ...
    function mintPassMint(uint256[] memory _mintPasses) public {
        ...
        require(_mintPasses.length > 0, "You must select at least one NFT");

        require(

            currentTotalOfPassAndLeatherfaceMints + _mintPasses.length <=

                maxPassAndLeatherfaceMintsTotal,

            "Max number of NFTs minted using Passes and Companions"

        );



        uint256 mintPassCount = _mintPasses.length;



        require(

            leatherfaceContract.balanceOf(msg.sender) >= mintPassCount,
    ...
    function leatherfaceMint(uint256[] memory _leatherfaces) public {
        ...
        require(_leatherfaces.length > 0, "You must select at least one NFT");

        require(

            currentTotalOfPassAndLeatherfaceMints + _leatherfaces.length <=
        ...
        require(
            ...
            leatherfaceContract.balanceOf(msg.sender) >= leatherfacesCount,
        ...
        _safeMint(msg.sender, leatherfacesCount);
