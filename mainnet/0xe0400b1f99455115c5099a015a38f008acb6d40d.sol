...
contract Bet is Ownable, ReentrancyGuard {
    ...
    mapping(uint16 => uint256) public matadorClaimed;
    uint256 private totalMatadors;
    uint16[14] private matadorIds;
    mapping(uint16 => bool) public isMatador;
    ...
    constructor(address _bullRun, address _topia, address _hub, address payable _randomizer, address _coinFlip, address _arena, uint256 _matadors) {
    ...
    function setIsMatador() internal {
        for(uint i = 0; i < matadorIds.length; i++) {
            isMatador[matadorIds[i]] = true;
        }
    }
    ...
    function _payMatadorTax(uint256 _amount) internal {
        topiaPerMatador += _amount / totalMatadors;
    }
    ...
    function claimMatadorEarnings(uint16[] calldata tokenIds) external nonReentrant notContract() {
        ...
        for(uint i = 0; i < tokenIds.length; i++) {
            ...
            require(msg.sender == tokenOwner && isMatador[tokenIds[i]]);
            owed += topiaPerMatador - matadorClaimed[tokenIds[i]];
            matadorClaimed[tokenIds[i]] = topiaPerMatador;
        }
