...
contract PandaCubz is ERC721A, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    function price() public view returns (uint256) {
        ...
        uint256 _pubSalePrice = token.pubSalePrice;
        if (_currentSupply >= 4000) {
            return _pubSalePrice + 0.02 ether;
        } else if (_currentSupply >= 3000) {
            return _pubSalePrice + 0.015 ether;
        } else if (_currentSupply >= 2000) {
            return _pubSalePrice + 0.01 ether;
        } else if (_currentSupply >= 1000) {
            return _pubSalePrice + 0.005 ether;
        } else {
...
contract PandaFactory is Ownable {
    ...
    function getDeployedNFTs() public view returns (PandaCubz[] memory) {
        return nfts;
    }
    ...
    function deploy(
        ...
        PandaCubz nft = new PandaCubz(_name, _symbol, _uri, payees, shares, msg.sender, token);
