...
contract TomodachiSigner is EIP712Upgradeable {
    string private constant SIGNING_DOMAIN = "Tomodachi";
    ...
    function __TomodachiSigner_init() internal initializer {
        __EIP712_init(SIGNING_DOMAIN, SIGNATURE_VERSION);
    }

    function getSigner(WhiteList memory tomo) internal view returns (address) {
        return _verify(tomo);
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "WhiteList(address userAddress,bool isReserveList)"
                        ),
                        tomo.userAddress,
                        tomo.isReserveList
                    )
    ...
    PausableUpgradeable
{
    ...
    mapping(address => bool) public reserveListSpotBought;
    ...
    function reserveListMint(WhiteList memory _whitelist)
        external
        payable
        onlyEOA
        nonReentrant
    {
        ...
        require(
            !reserveListSpotBought[_whitelist.userAddress],
            "Already Bought ReserveList Spot"
        );
        require(
            reserveListMinted + 1 <= whiteListCap - whiteListMinted,
            "ReserveList Spot Ended"
        );
        require(totalSupply() + 1 <= MAX_SUPPLY, "Max Supply Limit Exceed");
        require(_whitelist.isReserveList, "Is Whitelist");
        require(msg.value == reserveListPriceForEach, "Pay Exact Amount");

        reserveListSpotBought[_whitelist.userAddress] = true;
        reserveListMinted += 1;

        _mint(_whitelist.userAddress, 1);
    }
    ...
    function setReserveListPriceForEach(uint8 _price) external onlyOwner {
