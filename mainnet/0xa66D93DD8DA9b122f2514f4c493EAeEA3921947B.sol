...
contract RandomGame is ERC20Capped, Ownable {
        ...
        ERC20Capped(cap * multiplier) 
    {
        ...
        uint256 tokensForLiquidity = super.cap().div(2); // 50% of cap goes to liquidity
        uint256 tokensForPresalers = tokensForLiquidity.mul(40).div(100); // 40% goes to presalers
        uint256 tokensTotalForUtility = tokensForLiquidity.mul(10).div(100); // 10% goes to utility
        uint256 tokenForUtility = tokensTotalForUtility.div(4);

        _mint(owner(), tokensForLiquidity);
        _mint(circWallet, initialSupply.mul(multiplier).sub(tokensForLiquidity.add(tokensForPresalers).add(tokensTotalForUtility)));
