...
// @author CodedByGoblins
// @contact [email protected]

contract GollumTown is 
    ERC721, 
    Ownable, 
    ReentrancyGuard, 
    PaymentSplitter 
{
    ...
    onlyAccounts
    {
        require(publicM,                        "gollums: PublicSale is OFF");
        require(!paused, "gollums: Contract is paused");
        require(_amount > 0, "gollums: zero amount");

        uint current = _tokenIds.current();

        require(
            current + _amount <= maxSupply,
            "gollums: Max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "gollums: Not enough ethers sent"
        );
