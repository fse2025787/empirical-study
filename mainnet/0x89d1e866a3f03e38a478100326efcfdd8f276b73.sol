...
// File: mewkans/utils/IERC721Batch.sol
...
// File: mewkans/utils/ERC721B.sol
...
// File: mewkans/utils/ERC721EnumerableLite.sol
...
// File: mewkans/utils/Delegated.sol
...
contract MewkansThePrototypes is Delegated, ERC721EnumerableLite, DefaultOperatorFilterer {
    using Strings for uint;

    // General NFT Variables
    uint16 public maxTokens        = 1000;    // maximum number of tokens
    uint16 public maxMint          = 1;       // maximum mint per address per mint phase
    uint16 public phases           = 10;      // total number of mint phases
    uint16 public mintsPerPhase    = 100;     // total number of mints per phase
    uint16 public currentPhase     = 1;       // current mint phase
    uint public price              = 0 ether; // price during public mint
    
    bool public isMintActive;
    bool public isPayWhatYouWant;
    bool public isAllowListActive;

    string private _tokenURIPrefix = 'https://mewkans.s3.amazonaws.com/metadata/';
    string private _tokenURISuffix = '.json';

    event TeamMint(address indexed to, uint tokenId);
    event Mint(address indexed to, uint tokenId);

    
    mapping(address => uint16) public phaseMinted;
    ...
    constructor()
    Delegated()
    ERC721B("Mewkans: The Prototypes", "MKTP", 1){
    }
    ...
    /**
     * @notice Checks merkle proof to determine if address is allowlisted.
     */
    ...
    /**
     * @dev Modifier to disallow other smart contracts from calling a function.
     */
    modifier onlySender {
    ...
    // Public payable
    /**
     * @notice Public mint function. Mint and phase must be active. Addresses can only mint once per phase. Cannot be called by contracts.
     */
    function mint() external payable onlySender {
        address _minter = msg.sender;
        require(!isAllowListActive, "Allowlist is active!");
        require(isMintActive, "Mint is not active!");
        uint _supply = totalSupply();
        require(_supply + 1 <= maxTokens, "Mint exceeds token supply!");
        require(_supply + 1 <= mintsPerPhase * currentPhase, "Mint exceeds phase supply!");
        require(phaseMinted[_minter] < currentPhase, "Address has already minted in this phase!");
        if (!isPayWhatYouWant) {
            require(msg.value == price, "Eth sent is not correct!");
        }
        
        phaseMinted[_minter] = currentPhase; // Update phase minted, so address cannot mint again in current phase
        uint _mintId = _supply;
        _mint(_minter, _mintId);
        emit Mint(_minter, _mintId);
    }

    /**
     * @notice Public mint function for allowlist. Follows same rules as public mint, but requires merkle proof.
     */
    function mintAllowList(bytes32[] memory proof_) external payable onlySender {
        address _minter = msg.sender;
    ...
    /**
     * @notice Mints tokens directly to address(es). Can only be called by delegates.
     */
    function teamMint(uint[] calldata amount_, address[] calldata recipient_) external payable onlyDelegates {
        ...
        for(uint i; i < recipient_.length; ++i){
            for(uint j; j < amount_[i]; ++j){
                ...
                emit TeamMint(msg.sender, _mintId);
            }
        }
    }

    /**
     * @notice Set mint price and move to next mint phase. Can only be called by delegates.
     * @param price_ Price of next mint phase, in wei.
     * @param isPayWhatYouWant_ If set to true, mint price does not have to be respected.
     */
    function nextPhase(uint price_, bool isPayWhatYouWant_, bool isAllowListActive_) external onlyDelegates {
        require(isMintActive, "Sale must be active before moving to next phase!");
        require(currentPhase < phases, "Cannot increment phase beyond max!");
        price = price_;
        isPayWhatYouWant = isPayWhatYouWant_;
        isAllowListActive = isAllowListActive_;
        currentPhase++;
    }

    /**
     * @notice Activate/deactivate mint. Can only be called by delegates.
     */
    ...
    /**
     * @notice Set base metadata URI. Can only be called by delegates.
     */
    ...
    /**
     * @notice Withdraw funds from contract. Can only be called by owner.
     * @param recipient_ Address to send funds to.
     */
    function withdraw(address payable recipient_) external onlyOwner {
