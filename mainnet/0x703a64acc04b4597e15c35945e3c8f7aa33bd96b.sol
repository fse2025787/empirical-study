...
contract SmokeToken is ERC20PresetMinterPauser("Smoke", "SMOKE"), ReentrancyGuard {
    ...
    mapping(uint256 => uint256) private _cooldown;
    ...
    function claim(uint256 tokenId) external nonReentrant returns (uint256){
        require(getTime() + getInterval(3) > _cooldown[tokenId], "Dragon is on cooldown");
        ...
        _cooldown[tokenId] = getTime() + getInterval(3);
        emit Claimed(tokenId);
        return owed;
    }

    function dragonCooldown(uint256 tokenId) external view returns (uint256) {
        return _cooldown[tokenId];
    }
