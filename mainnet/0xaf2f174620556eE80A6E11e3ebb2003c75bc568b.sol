...
contract OnlyFunLab is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    /* DETAILS */
    ...
    function whitelistMint(
        ...
        if (!verifySignature(signature_, "Whitelist"))
            revert InvalidSignature();
        if (_totalMinted() + quantity_ > maxSupply) revert SoldOut();
        if (_numberMinted(msg.sender) + quantity_ > limitPerWallet) revert LimitPerWalletExceeded();
        if (msg.value != quantity_ * whitelistMintPrice) revert IncorrectPrice();
        if (quantity_ > limitPerTxn) revert LimitPerTxnExceeded();
    ...
    function airdropToMany(
        ...
        if (receivers_.length != quantities_.length) revert LengthNotMatch();

        for (uint256 i = 0; i < receivers_.length; i++) {
            address _receiver = receivers_[i];
            uint256 _quantity = quantities_[i];

            _airdrop(_receiver, _quantity);
        }
    }

    function _airdrop(address receiver_, uint256 quantity_) internal {
        if (_totalMinted() + quantity_ > maxSupply) revert SupplyExceeded();
    ...
    function setLimitPerTxn(uint256 limitPerTxn_) public onlyOwner {
        limitPerTxn = limitPerTxn_;
    }
