...
abstract contract PausableControl {
    ...
    modifier whenNotPaused(bytes32 role) {
        require(!paused(role) && !paused(PAUSE_ALL_ROLE), "PausableControl: paused");
        _;
    }

    modifier whenPaused(bytes32 role) {
        require(paused(role) || paused(PAUSE_ALL_ROLE), "PausableControl: not paused");
        _;
    }
...
contract AnyswapTokenAnycallClient is AnycallClientBase {
    ...
    function swapout(
        ...
        if (_underlying != address(0) && IERC20(token).balanceOf(msg.sender) < amount) {
        ...
        bytes memory data = abi.encodeWithSelector(this.anyExecute.selector, token, dstToken, amount, msg.sender, receiver, toChainId);
        IAnycallV6Proxy(callProxy).anyCall{value: msg.value}(clientPeer, data, address(this), toChainId, flags);
