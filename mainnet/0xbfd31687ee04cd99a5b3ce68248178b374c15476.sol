...
contract NekoNode is ERC721Enumerable, Ownable, ControlledAccess{
    using Strings for uint256;
    ...
    function updateNKRIAddress( address _nkriAddress) public onlyOwner{
        NKRI = _nkriAddress;
    }
    ...
    function mint(uint _type) public payable {
        ...
        if (msg.sender != owner()) {
            require(IERC20(NKRI).transferFrom(msg.sender, address(this), price), "Not enough funds");
            IERC20(NKRI).approve(address(uniswapV2Router), _tax);
            if (autosell) {
                swapTokensForEth(_tax);
            }
            IERC20(NKRI).transfer(rewardPool, price-_tax);
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(NKRI).approve(address(uniswapV2Router), tokenAmount);
