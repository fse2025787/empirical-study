...
contract LamboLiquidity is Ownable, Pausable, ERC1155Holder {
    ...
    uint256 migrateFromToken = 0xC0C8D886B92A811E8E41CB6AB5144E44DBBFBFA30000000000018B0000000001;
    ...
    uint256 getSwapPrice = .01 ether;
    uint256 getMaxSwapPrice = .05 ether;
    ...
    function sellPunk(uint256[] memory osTokenIds) external whenNotPaused {
    ...
    function buyPunk(uint256[] memory osTokenIds) external payable whenNotPaused {
        require(msg.value % getBuyPrice == 0 && msg.value / getBuyPrice == osTokenIds.length);
    ...
    function swapPunk(uint256[] memory ownedTokens, uint256[] memory vaultTokens) external payable whenNotPaused {
        require(ownedTokens.length == vaultTokens.length, "uneven swap");
        require(msg.value == min(ownedTokens.length * getSwapPrice, getMaxSwapPrice), "didnt send fee");

        //transfer owned tokens
        for(uint256 i = 0; i < ownedTokens.length; i++) {
            uint256 osTokenId = ownedTokens[i];
            require(osTokenId >= migrateFromToken, "not a valid LEP");
            require(osTokenId <= migrateTilToken, "not a valid LEP");
            os.safeTransferFrom(msg.sender, address(this), osTokenId, 1, '');        
        }
        ...
        for(uint256 i = 0; i < vaultTokens.length; i++) {
            uint256 osTokenId = vaultTokens[i];
            require(osTokenId >= migrateFromToken, "not a valid LEP");
            require(osTokenId <= migrateTilToken, "not a valid LEP");
            os.safeTransferFrom(address(this), msg.sender, osTokenId, 1, '');
        }
