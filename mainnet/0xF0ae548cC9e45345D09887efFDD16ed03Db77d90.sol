...
contract MetaverseCapital is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public treasuryAddress = payable(0x3916888B581DfF7E81E4623BD2E993F6B19e6D83); // Treasury Address

    address payable public liquidityAddress =
        payable(0xc4c3F428C6f128ceaf8C0c6C6C9B947d41352666); // Liquidity Address
                
    address payable public devAddress =
        payable(0x6f11b5A9d2c997ccC7F3359218a32aC47Daf7796); // Buyback Address
    ...
    string private constant _name = "Metaverse Capital";
    string private constant _symbol = "MVC";
    ...
    uint256 public _sellTreasuryFee = 10;
    ...
    uint256 private _treasuryTokensToSwap;
    ...
    constructor() payable {
        ...
        address futureOwner = address(msg.sender);
        
        maxBuyAmount = _tTotal * 5 / 1000; // 0.5% maxTransactionAmountTxn
        maxSellAmount = _tTotal * 5 / 1000; // 0.5% maxTransactionAmountTxn
        maxWallet = _tTotal * 5 / 1000; // 0.5% max txn
    ...
    function updateMaxBuyAmount(uint256 newNum) external onlyOwner { 
        require(newNum >= (_tTotal * 5 / 1000)/1e9, "Cannot set max buy amount lower than 0.5%"); 
    ...
    function updateMaxSellAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 5 / 1000)/1e9, "Cannot set max sell amount lower than 0.5%");
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyTreasuryFee + _buyBuybackFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            } 
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellTreasuryFee + _sellBuybackFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        bool success;
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _devTokensToSwap + _treasuryTokensToSwap;
        ...
        uint256 ethForTreasury = ethBalance.mul(_treasuryTokensToSwap).div(totalTokensToSwap);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _devTokensToSwap += tLiquidity * _buyBuybackFee / _liquidityFee;
            ...
            _devTokensToSwap += tLiquidity * _sellBuybackFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTreasuryFee)
        external
        onlyOwner
    {
        _buyTreasuryFee = buyTreasuryFee;
        require(_buyTreasuryFee <= 20, "Must keep taxes below 20%");
    }

    function setSellFee(uint256 sellTreasuryFee)
        external
        onlyOwner
    {
        _sellTreasuryFee = sellTreasuryFee;
   
        require(_sellTreasuryFee <= 30, "Must keep taxes below 30%");
    }
