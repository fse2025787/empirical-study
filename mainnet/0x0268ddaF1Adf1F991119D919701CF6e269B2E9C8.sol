...
contract MrdrVrsContract is ERC721A, Ownable, PaymentSplitter, DefaultOperatorFilterer, ReentrancyGuard, Referable, Pausable  {
    ...
    function isEligible(
        ...
        if(token.claimIsActive && (_quantity <= (_maxMint - hasClaimed[_address]))) {
                _isEligibleClaim = _hasSupply && _maxMint <= 50 &&
                    MerkleProof.verify(_proof, claimMerkleRoot, keccak256(abi.encode(_address, _maxMint)));
                if (_isEligibleClaim) {
                    require(_value == 0, "Bad ETH");
                    hasClaimed[_address] += _quantity;
                }
        }
        if(!_isEligibleClaim && token.preSaleIsActive && (_quantity <= _maxMint - hasMinted[_address])) {
                ...
                if (_isEligiblePreSale) {
                    require(token.preSalePrice * _quantity <= _value, "Bad ETH");
        ...
        if (!_isEligibleClaim && !_isEligiblePreSale && token.pubSaleIsActive) {
            ...
            if (_isEligiblePubSale) {
                require(token.pubSalePrice * _quantity <= _value, "Bad ETH");
