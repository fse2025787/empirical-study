...
contract ParmeusCareer is ERC721URIStorage {
    ...
    constructor() ERC721("Parmeus Career Match", "PCRRM"){
        _tokenIds.increment(); // token id start from 1
        _contractManager = msg.sender;
    }

    function mint(string memory _cid, uint8 v, bytes32 r, bytes32 s ) public returns(bool success){
     ...
     *  get calller's nft
     */
    function getSelfNfts() public view returns(string[] memory tokenUris, uint256[] memory tokenIds){
     ...
     */
    function approve(address to, uint256 tokenId) public virtual override {
        require(false, 'Parmeus Career Match nft can not be delegated');
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(from == address(0) || to == address(0), 'Parmeus Career Match nft can not be transfered');
    }
