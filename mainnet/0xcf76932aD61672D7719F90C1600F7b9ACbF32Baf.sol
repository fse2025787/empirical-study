...
/*
 ...
 / _, _// /_/ // /_/ // /_/ // / / / / // // /_ / /_/ /
/_/ |_| \____//_.___/ \____//_/ /_/ /_//_/ \__/ \____/ 
...
contract Robomito is ERC721A, Ownable, ReentrancyGuard {
    address private constant creator1 =
        0x0140D50CDb7408882ba92551DE191f3F9a4645fa;
    address private constant creator2 =
        0x02085D99239BeAdcB82bd6c4CC55a5bB79E3f487;
    address private constant creator3 =
        0x768a93fA0C1213aa1411456a8056b7EBEE94bEaF;
    address private constant creator4 =
        0xE2BFf72848B50e2385E63c23681695e990eC42cb;
	...
	uint256 private _preSalePrice = 0.066 ether;

	uint256 private _reservedSupply = 150;
	address private _marketAddress = 0xbF0a44954F37C9DcE6b9B1Bd779088A8324BE8eb;
	uint256 private _marketMintSupply = 100;
    ...
    uint256 private MAX_MINTS_PER_WHITELIST = 10;
    uint256 private MAX_MINTS_PER_TX = 20;
	
    uint256 public MAX_SUPPLY = 3333;
	

    constructor() ERC721A("Robomito", "ROBOMITO") {}
    ...
    function withdrawAll(uint256 leftPercent) external onlyOwner {

		if (leftPercent < 0 || leftPercent > 1000)
            revert("value range must be between 0 - 1000");

		uint256 pricePercent = 1000 - leftPercent;
		if(leftPercent > 999)
			pricePercent = 1000;
		uint256 balance = address(this).balance * pricePercent / 1000;
		uint256 amountToCreator1 = (balance * 310) / 1000;
		uint256 amountToCreator2 = (balance * 310) / 1000;
		uint256 amountToCreator3 = (balance * 305) / 1000;
		uint256 amountToCreator4 = (balance * 75) / 1000;
    ...
    {
        if (!isClaimActive()) revert("Sale not started");
        if (hasMintedClaim(msg.sender)) revert("Amount exceeds claim limit");
        if (totalSupply() + 1 > (MAX_SUPPLY - _reservedSupply)) revert("Amount exceeds supply");
        _mintedClaim[msg.sender] = true;
        _safeMint(msg.sender, 1);
    }

	function marketMint(uint256 quantity)
        external
        nonReentrant
        callerIsUser
    {
		...
		if(msg.sender != _marketAddress)
			revert("only market address can mint");
        if (quantity > _marketMintSupply)
            revert("Amount exceeds mint supply limit");
		if(_marketMintSupply < 1)
			revert("no market supply left");
        if (totalSupply() + quantity > (MAX_SUPPLY - _reservedSupply))
            revert("Amount exceeds supply");
		
		_marketMintSupply -= quantity;
        _reservedSupply -= quantity;
		_safeMint(msg.sender, quantity);
    }

    function presaleMint(bytes32[] calldata merkleProof, uint256 quantity)
        external
        payable
        nonReentrant
        callerIsUser
        verify(msg.sender, merkleProof, _presaleMerkleRoot)
    {
        if (!isPresaleActive()) revert("Sale not started");
        if (quantity > MAX_MINTS_PER_WHITELIST)
            revert("Amount exceeds presale Limit");
        if (_whitelistMints[msg.sender] + quantity > MAX_MINTS_PER_WHITELIST)
            revert("Amount exceeds your total presale  limit");
        if (totalSupply() + quantity > (MAX_SUPPLY - _reservedSupply))
            revert("Amount exceeds supply");
        if (getPresalePrice() * quantity > msg.value)
	...
	function mintReserveToAddress(address to, uint256 quantity) external onlyOwner {
        if (totalSupply() + quantity > MAX_SUPPLY)
            revert("Amount exceeds supply");
		if(quantity > _reservedSupply)
			revert("Amount exceeds reserved supply");
		if(_reservedSupply < 1)
			revert("No reserved supply left");
	...
	function mintedWhiteListLeft(address account) external view returns (uint256){
		return _whitelistMints[account];
	}
