...
pragma solidity ^0.6.12;
    ...
    contract DoggyStyle is Context, IERC20, Ownable {
        ...
        string private _name = 'Doggy Stlye';
        string private _symbol = 'DSTYLE';
        uint8 private _decimals = 9;
        
        // Tax and kennel fees will start at 0 so we don't have a big impact when deploying to Uniswap
        // kennel wallet address is null but the method to set the address is exposed
        uint256 private _taxFee = 10; 
        uint256 private _kennelFee = 10;
        uint256 private _previousTaxFee = _taxFee;
        uint256 private _previouskennelFee = _kennelFee;

        address payable public _kennelWalletAddress;
        ...
        uint256 public _maxTxAmount =  10000000000000000000;  
        uint256 private _numOfTokensToExchangeForkennel = 5000000000000000;
        ...
        constructor (address payable kennelWalletAddress, address payable marketingWalletAddress) public {
            _kennelWalletAddress = kennelWalletAddress;
        ...
        function removeAllFee() private {
            if(_taxFee == 0 && _kennelFee == 0) return;
            
            _previousTaxFee = _taxFee;
            _previouskennelFee = _kennelFee;
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            // also, don't get caught in a circular kennel event.
            ...
            bool overMinTokenBalance = contractTokenBalance >= _numOfTokensToExchangeForkennel;
            if (!inSwap && swapEnabled && overMinTokenBalance && sender != uniswapV2Pair) {
                // We need to swap the current tokens to ETH and send to the kennel wallet
                swapTokensForEth(contractTokenBalance);
                
                uint256 contractETHBalance = address(this).balance;
                if(contractETHBalance > 0) {
                    sendETHTokennel(address(this).balance);
                }
            }
            ...
            //transfer amount, it will take tax and kennel fee
        ...
        function sendETHTokennel(uint256 amount) private {
            _kennelWalletAddress.transfer(amount.div(2));
        ...
        function manualSend() external onlyOwner() {
            uint256 contractETHBalance = address(this).balance;
            sendETHTokennel(contractETHBalance);
        }
        ...
        function _transferStandard(address sender, address recipient, uint256 tAmount) private {
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tkennel) = _getValues(tAmount);
            _rOwned[sender] = _rOwned[sender].sub(rAmount);
        ...
        function _takekennel(uint256 tkennel) private {
            uint256 currentRate =  _getRate();
            uint256 rkennel = tkennel.mul(currentRate);
            _rOwned[address(this)] = _rOwned[address(this)].add(rkennel);
            if(_isExcluded[address(this)])
                _tOwned[address(this)] = _tOwned[address(this)].add(tkennel);
        }
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tkennel) = _getTValues(tAmount, _taxFee, _kennelFee);
            uint256 currentRate =  _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
            return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tkennel);
        }

        function _getTValues(uint256 tAmount, uint256 taxFee, uint256 kennelFee) private pure returns (uint256, uint256, uint256) {
            uint256 tFee = tAmount.mul(taxFee).div(100);
            uint256 tkennel = tAmount.mul(kennelFee).div(100);
            uint256 tTransferAmount = tAmount.sub(tFee).sub(tkennel);
            return (tTransferAmount, tFee, tkennel);
        }
        ...
        function _setkennelFee(uint256 kennelFee) external onlyOwner() {
            require(kennelFee >= 1 && kennelFee <= 11, 'kennelFee should be in 1 - 11');
            _kennelFee = kennelFee;
        }
        
        function _setkennelWallet(address payable kennelWalletAddress) external onlyOwner() {
            _kennelWalletAddress = kennelWalletAddress;
        }
