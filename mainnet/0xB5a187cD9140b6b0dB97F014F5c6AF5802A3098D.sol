 ...
 * S5: token to sell for (_order.signer.token) must be WBTC
 * S6: tokens being sold (_order.sender.amount) and tokens being minted (_otokenAmount) must be the same
 * S7: amount of WBTC being sold for (_order.signer.amount) does not meet the minimum option premium
 * S8: unable to unwrap WBTC to ETH and add liquidity to curve: insufficient ETH
...
contract ShortOTokenActionWithSwapBtc is IAction, AirswapBase, RollOverBase {
    ...
    constructor(
        ...
        // enable vault to take all the wbtc back and re-distribute.
        IERC20(wbtc).safeApprove(_vault, uint256(-1));

        // enable pool contract to pull wbtc from this contract to mint options.
        IERC20(wbtc).safeApprove(controller.pool(), uint256(-1));
    ...
    /**
     * @dev return the net worth of this strategy, in terms of wbtc.
     * if the action has an opened gamma vault, see if there's any short position
     */
    function currentValue() external view override returns (uint256) {
        return wbtc.balanceOf(address(this)).add(lockedAsset);
    ...
    /**
     * @dev owner only function to mint options with "wbtc" and sell otokens in this contract
    ...
    function mintAndSellOToken(
        ...
        require(_order.signer.token == address(wbtc), "S5");
        ...
        // sell options on airswap for wbtc
    ...
    function _isValidStrike(uint256 strikePrice) internal view returns (bool) {
        uint256 spotPrice = oracle.getPrice(address(wbtc));
