...
// Smart Contract -- Coded By ShAj
contract BABYTIKITRIBES is ERC721ABurnable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    // Variables
    string baseURI;
    string public baseExtension = ".json"; // Base extension of metadata file
    uint256 public cost = 0.05 ether; // Cost of each NFT
    uint256 public maxSupply = 1200; // Max amount of NFTs that can be minted in total
    uint256 public maxMintAmount = 22; // Max amount to be minted at once
    bool public paused = false; // Contract paused when deployed
    bool public revealed = false; // NFT collection revealed when deployed
    string public notRevealedUri; // URI for placeholder image
    uint256 public nftPerAddressLimit = 100; // Maximum NFTs mintable on one wallet
