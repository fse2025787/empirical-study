...
contract TheX is ERC721A, Ownable, ReentrancyGuard {
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        bool success;
        (success, ) = payable(0x44084dff2a31c34C7EA4CFceA80aC9A883c51F74).call{value: ((balance * 40) / 100)}("BlocqX");
        require(success, "BlocqX Transaction Unsuccessful");

        (success, ) = payable(0xcea28cD43406a8a83d140dD9A4ABF8A413735fEd).call{value: ((balance * 40) / 100)}("Mode");
        require(success, "Mode Transaction Unsuccessful");

        (success, ) = daoWallet.call{value: ((balance * 10) / 100)}("Dao");
        require(success, "DAO Transaction Unsuccessful");

        (success, ) = payable(0xc259D6a0623411033f2E69384556f7c0F49dFa37).call{value: ((balance * 10) / 100)}("Dan");
