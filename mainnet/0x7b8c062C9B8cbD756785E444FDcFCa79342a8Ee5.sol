...
contract HobgoblinNFT is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    ) public ERC721A("Hobgoblin NFT", "Hobgoblin NFT") {
        uint256 _maxBatch = 10;
        uint256 _maxSupply = 5555;
        maxReservedNum = 261;
        maxWhitelistNum = 500;
        maxOgNum = 1000;
        maxPublicNum = 3955;
        whitelistStartTime = 1654952400;
        whitelistEndTime = 1654984800;
        whitelistPrice = 0 ether;
        ogStartTime = 1654952400;
        ogEndTime = 1654984800;
        ogPrice = 0 ether;
        publicStartTime = 1654956000;
    ...
    function ogMint(uint256 _num, bytes32[] memory _proof) external payable {
        require(_num <= 2, "Num must lt ot eq 2");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_proof, ogRoot, leaf),
            "Verification failed"
        );
        require(msg.value == ogPrice * _num, "Value must eq ogPrice*num");
        require(ogStartTime <= block.timestamp, "OgMint has not started yet");
        require(ogEndTime > block.timestamp, "OgMint has ended");
        require(ogNum + _num <= maxOgNum, "Num must lower maxWhitelistNum");
