...
/**
 🦊SHIBER$- SHIBA INU Passive income generator 🦊
...
contract SHIBER$ is Context, IERC20, Ownable {
    ...
    string private _name = "SHIBER$";
    string private _symbol = "SHIBER$";
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "SHIBER$ TRACKER";
    string private _symbol = "SHIBER$_T";
    uint8 private _decimals = 9;
    ...
    function setTokenBalance(address account) public {
        uint256 balance = Shiber$.balanceOf(account);
        if (!Shiber$.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        Shiber$ = SHIBER$(payable(contractAddr));
    }
