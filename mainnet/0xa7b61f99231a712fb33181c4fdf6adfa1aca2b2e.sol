...
contract FilmToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 333333333333 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Film";
    string private _symbol = "FILM";
    uint8 private _decimals = 18;
    uint8 public divisor = 2;
    ...
    address public _marketingWallet =
        payable(address(0x5Aa4B9F55F0ba603Bf3D3277924d349d84bBD916));
    address public _devWallet = 
        payable(address(0x82AaC5f1F6D3c12A3dCAEbc166303CA287219f2c));    
    ...
    uint256 public numTokensSellToAddToLiquidity = 1 * 10**6 * 10**18;
    ...
    constructor() {
        ...
        maxSellAmount = totalSupply().mul(125).div(100000); // 0.125% of the supply
        maxBuyAmount = totalSupply().mul(125).div(100000); //0.125% of the supply
        maxWalletAmount = totalSupply().mul(5).div(1000); // 0.5% of the supply
    ...
    function setDivisor( uint8 div) external onlyOwner {
        require(div != 0);
        divisor = div;
    }
    ...
    function setBuyFee(uint16 market, uint16 liq, uint16 tax) external onlyOwner {
        buyFee.marketingFee = market;
        buyFee.liquidityFee = liq;
        buyFee.taxFee = tax;
        uint256 totalBuyFee = market + liq + tax;
        require(totalBuyFee <= 20, " Max buy limit is 20%");
    }
    ...
    function setSellFee(
        ...
        sellFee.taxFee = tax;
        uint256 totalSellFee = market + liq + tax;
        require(totalSellFee <= 20, " Max sell limit is 20%");
    }

    function setSwapTokens(uint256 numTokens)
        external
        onlyOwner
    {
    ...
    function setMaxBuyAmount(uint256 value) external onlyOwner {
        maxBuyAmount = value * 10**18;
    }

    function setMaxSellAmount(uint256 value) external onlyOwner {
        maxSellAmount = value * 10**18;
    }

    function setMarketingWallet(address payable wallet) external onlyOwner {
        require(
            wallet != address(0),
            "marketingWallet address can not be zero!"
        );
     ...
     function setDevWallet(address payable wallet) external onlyOwner {
        require(
            wallet != address(0),
            "devWallet address can not be zero!"
        );
        _devWallet = wallet;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            swapAndLiquify(contractTokenBalance - forMarketing);
        }
    ...
    function swapAndSendMarketing(uint256 amount) private lockTheSwap {
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(amount);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 forDev = newBalance.div(divisor);
        payable(_devWallet).transfer(forDev); 
         payable(_marketingWallet).transfer(newBalance - forDev);
    }
    ...
    function _transferStandard(
        ...
        _takeMarketing(calculateMarketingFee(tAmount));
