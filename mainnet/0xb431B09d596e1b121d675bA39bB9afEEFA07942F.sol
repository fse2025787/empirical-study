...
contract InvisibleDudesGenesis is ERC721Delegated, ReentrancyGuard {
  using Counters for Counters.Counter;

  constructor(address baseFactory, string memory customBaseURI_)
    ERC721Delegated(
      baseFactory,
      "Invisible Dudes Genesis",
      "IDGen",
      ConfigSettings({
        royaltyBps: 1000,
  ...
  uint256 public constant PRICE = 900000000000000000;
  ...
  function mint(uint256 count) public payable nonReentrant {
    ...
    require(
      msg.value >= PRICE * count, "Insufficient payment, 0.9 ETH per item"
    );
