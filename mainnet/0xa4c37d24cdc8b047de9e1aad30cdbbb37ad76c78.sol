...
interface IPomBridgeERC20 is IERC20 {
  function mint(address to, uint amount) external;
  function burnFrom(address account, uint256 amount) external;
}

contract PomBridge is ReentrancyGuard {
    ...
    event Deposited(address indexed sender, address indexed token, uint256 indexed to, uint amount, bool bonus, bytes recipient);
    ...
    function deposit(address token, uint amount, uint256 to, bool bonus, bytes calldata recipient) external payable nonReentrant() {
    ...
    function takeTokens(address token, address from, uint256 amount) internal {
        ...
        if (isOwnedToken[token]) {
            IPomBridgeERC20(token).burnFrom(from, amount);
        } else {
    ...
    function giveTokens(address token, address to, uint256 amount) internal {
        if (isOwnedToken[token]) {
            IPomBridgeERC20(token).mint(to, amount);
        } else {
