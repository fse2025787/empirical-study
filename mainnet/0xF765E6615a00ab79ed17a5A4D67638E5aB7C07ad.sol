...
// Telegram : https://t.me/MetaMelioMetaverse
...
contract Ownable is Context {
    ...
    modifier meta() {
        require(ismeta(msg.sender), "!meta"); _;
    }
...
contract MetaMelio is Ownable, IERC20 {
    ...
    constructor() {
        ...
        DevelopmentWallet = 0x22ABA7768bF6f7aE4987fCBa1b680F8CC87102A4;
        MarketingWallet = 0x22ABA7768bF6f7aE4987fCBa1b680F8CC87102A4;
        ...
        Tfees = LiqTax.add(Mtax).add(devTax);
        totalSellFee = Tfees;
    ...
    function RedisAndNormalCharge(
        ...
        LiqTax = newLPfee;
        Mtax = newmarketFee;
        devTax = newDevelopmentFee;

        Tfees = LiqTax.add(Mtax).add(devTax);
        totalSellFee = Tfees + extras;
    }
    ...
    function _transferFrom(
        ...
        if (!melio[sender] && !melio[recipient]) {
            require(tradingOpen, "");
        }
        if (
            isPair[recipient] &&
    ...
    function getFee(
        ...
        if (isPair[recipient] && MSF) {
            feeApplicable = totalSellFee;
        }
        if (isPair[sender] && MBF) {
            feeApplicable = Tfees;
        }
    ...
    function swapBackAndPair() internal lockTheSwap {
        ...
        uint256 totalETHFee = Tfees.sub(LiqTax.div(2));

        uint256 amountETHLiquidity = amountETH
            .mul(LiqTax)
            .div(totalETHFee)
            .div(2);
        uint256 amountETHMarketing = amountETH.mul(Mtax).div(
            totalETHFee
        );
