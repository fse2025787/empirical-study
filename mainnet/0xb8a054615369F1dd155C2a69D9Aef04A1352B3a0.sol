...
contract WizardCreatureNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public PRICE = 4 * 10**16; // 0.04 ETH
    uint256 public PRESALE_PRICE = 3 * 10**16; //0.03 ETH
    ...
    event CreateWizardCreature(uint256 indexed id);

    constructor(string memory baseURI) ERC721("Wizard Creature", "WIZARD") {
        setBaseURI(baseURI);
    ...
    function mint(uint256 _count) public payable saleIsOpen {
        ...
        require(
            total <= MAX_ELEMENTS,
            "All Wizard Creature NFT's are sold out"
        );
    ...
    function presaleMint(uint256 _count) public payable {
        ...
        uint256 total = _totalSupply();
        require(total + _count <= MAX_ELEMENTS, "Max limit");
        require(total <= MAX_ELEMENTS, "All Wizard Creature NFT's are sold out");
