...
/**
 The Dice 
 Gambling is a billion dollar industry where, each day ,hundreds of millions of dollars are played at a big number of games like Slots, Craps, Blackjack, Poker, much more .

 Well, the online casino bussines has been among us for years, but the ideea of an crypto casino with its own crypto token that can be played onto the website has never been promoted that much.
 So, we are comming with the initiative of starting an online casino where our token holders will get a pecentage of the winnings fron the casino.
 After the succes of the first token, we are planning on launching the second token, based on the funding from the taxes accumulated after the first launch.

 Roadmap:
 ■Launching our first token, among renouncing and locking right after
 ■Launching our online casino in the beta version with the taxes accumulated
 ■Starting giveaways
 ■Airdropping the holders of our first token 35% of our casino winnings each week

 ARE YOU IN?
 LET'S ROLL THE DICE


 https://t.me/TheDiceERC
...
contract TheDice is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = "The Dice";
    ...
    uint256 public _maxWalletAmount = 1000 * 1e3 * 1e6; //2%
    uint256 public j_maxtxn = 50 * 1e3 * 1e6; //1%
    uint256 public swapAmount = 5 * 1e2 * 1e6; //.07%
    uint256 private buyPyroUpperLimit = 100 * 1e14; // 0.01

    // fees
    uint256 public j_liqBuy = 1; 
    uint256 public j_burnBuy = 0;
    uint256 public j_pyroBuy = 0;
    ...
    address payable private scorcherWallet = payable(0x6Bf97Dc73C96707da5757e9B4B7d101ca8C29BB6);
    address payable private jeetWallet = payable(0x6Bf97Dc73C96707da5757e9B4B7d101ca8C29BB6);
