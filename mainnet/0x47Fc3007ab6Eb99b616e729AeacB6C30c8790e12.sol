...
contract GakkoLootS1 is ERC721AQueryable, OperatorFilterer, Delegated {
  ...
  function handleClaim(address owner, Token calldata claim) external onlyDelegates{
    require(!tokensClaimed[claim.parentId], "Loot already claimed");
    tokensClaimed[claim.parentId] = true;
    _mint(owner, 1);
  }
  ...
  function mintTo(uint16[] calldata quantity, address[] calldata recipient) external payable onlyDelegates{
    ...
    require( tokenId + totalQuantity <= maxSupply, "mint/order exceeds supply" );
