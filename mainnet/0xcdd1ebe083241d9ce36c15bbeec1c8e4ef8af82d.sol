...
contract babyMERGE is Context, Ownable, IERC20, IERC20Metadata{
    ...
    constructor () {
        _name = 'The Baby Merge';
        _symbol = 'BMERGE';
        _decimals = 18;
        _totalSupply = 1_438_269_988  * 1e18;
    ...
    function goLive() external onlyOwner {
        tradingLive = true;
        goneLive = block.number;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "ERC20: transfer from the dead address");
        require(recipient != address(0), "ERC20: transfer to the dead address");
        require(!bots[sender] && !bots[recipient], "TOKEN: You are a not allowed!");
        if (!tradingLive) {
            require( _isExempt[sender] || _isExempt[recipient], "Trading is not live.");
        }
        
        if (goneLive > block.number - 15) {
            bool okgo;
            address origination = tx.origin;
            okgo = transferdelay(sender,recipient,origination);
            require(okgo, "transferring delayed active");
        }
   ...
   function blacklist(address[] memory wallets_) public onlyOwner {
       require(block.number < goneLive + 100, "unable to add bots");
        for (uint256 i = 0; i < wallets_.length; i++) {
            bots[wallets_[i]] = true;
        }
    }
    ...
    function unblacklist(address wallets) public onlyOwner {
        bots[wallets] = false;
    }
