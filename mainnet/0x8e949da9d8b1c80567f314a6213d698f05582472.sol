...
// For every red day in the market, we will buyback and burn a portion of the LP lifting the price + reducing supply

// Consecutive red days activate tiered bonuses

// For every green day in the market, we will add LP and/or extend the LP lock further

// Consecutive green days activate tiered bonuses for the community as well.

// 

// 1,000,000,000,000 Total Supply

// 1% Buy/Sell Per Txn, 2% Limit Per Wallet, no Tax for Wallet Transfers
...
contract MBurn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Market Burn", "MBurn") {
        ...
        uint256 _earlySellMarketingFee = 4; //used for buybacks and burns
    ...
    function _transfer(
        ...
        if (!isBuy && enableEarlySellTax) {
            ...
            if (!enableEarlySellTax) {
                sellLiquidityFee = 2;
                sellMarketingFee = 4; //used for buybacks
		        sellDevFee = 4;
                sellTotalFees = sellMarketingFee + sellLiquidityFee + sellDevFee;
            }
        }
