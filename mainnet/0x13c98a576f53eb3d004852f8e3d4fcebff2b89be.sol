...
contract Treat is ERC20Permit {
    uint256 internal constant maxSupply = 10**27;
    mapping(address => bool) public taxFreeAccount;
    mapping(address => bool) public taxedReceiverAccount;

    /**
     * @dev Constructor
     */
    constructor() ERC20("Treat", "TREAT") ERC20Permit("Treat") {
        _mint(_msgSender(), 10**24);
        taxFreeAccount[_msgSender()] = true;
    }

    function setTaxedReceiver(address _account) external {
        require(taxFreeAccount[_msgSender()], "Not owner");

        if (taxedReceiverAccount[_account]) {
            delete taxedReceiverAccount[_account];
        } else {
            taxedReceiverAccount[_account] = true;
        }
    }
    ...
    function _transfer(
        ...
        uint256 _burnAmount = (taxFreeAccount[sender] ||
            ...
            ? 0
            : (taxedReceiverAccount[recipient] ? amount / 50 : 0);
