...
contract MoriesEth is Ownable, ERC721('CryptoMories', 'CRYPTOMORIES'), ReentrancyGuard {
    ...
    function onERC721Received(address _operator, address _from, uint256 tokenId, bytes calldata) external nonReentrant returns (bytes4) {
        if (msg.sender == address(this)) {
            ERC721(MORIES).transferFrom(address(this), _from, tokenId);
            emit MorieSwapped2(tokenId);
        }
        if (msg.sender == MORIES) {
            if(minted[tokenId]) {
                this.safeTransferFrom(address(this), _from, tokenId);
            } else {
                minted[tokenId] = true;
    ...
    function retrieveMorie(address morieAddress, uint256 tokenId, address _user) external onlyOwner {
        ERC721(morieAddress).safeTransferFrom(address(this), _user, tokenId);
    }

    function transferFrom(address, address, uint256) public override {
        revert('use safe transfers');
    }
