...
// File: Fanverse/fanverse/contracts/libraries/FNVvoucher.sol
...
// File: Fanverse/fanverse/contracts/PrimaryMarketplace.sol
...
*/
    ...
    */
    ...
    function buyFixBuy(FNVvoucher.mintVoucher memory mintvoucher, FNVvoucher.priListing memory _voucher, address redeemer, uint256 nftamount) public payable{
        ...
        require(!allMinted[mintvoucher.tokenId] && !PriCounterstatus[_voucher.countervalue] , "ERR 1"); //MintVoucher has expired
        require(_voucher.listed == true, "ERR 2"); // NFT should be listed
        require(mintvoucher.amount >= _voucher.amount, "ERR 3"); //Cannot mint less than listed NFTS
        ...
        require(nftamount <= _voucher.amount , "ERR 4"); //Not listed enough NFTs
        require(Mintamountleft[mintvoucher.tokenId] >= nftamount, "ERR 5"); //Owner doesn't have enough NFT balance
        require(priListAmonutLeft[_voucher.countervalue] >= nftamount, "ERR 6"); //Owner doesn't have enough NFT balance
        require((_voucher.unitprice*(nftamount)) == msg.value , "ERR 7"); //pay more
        require(address(mintvoucher.nftAddress) == address(this),"ERR 8"); //invalid NFT address


        address minter = _verifyMinter(mintvoucher);
        require(minter == address(mintvoucher.nftOwner),"ERR 9"); // invalid minter
        address signer = priListingVerify(_voucher);
        require(signer == address(_voucher.nftOwner),"ERR 10"); //invalid signer
        require(signer == minter,"ERR 11"); //invalid user
    ...
    */
    ...
    function distributeAmount(address _receiver) internal {
        ...
        uint256 nftPrice = msg.value - serviceFees;
            ...
            require(success, "ERR 12"); //reedem: Failed to transfer the funds

            (bool success1, ) = payable(
                Admin).call{value: serviceFees}("");
            require(success1, "ERR 13"); // reedem: Failed to transfer the funds
    }
    
    function setMarketFee(uint256 _marketfees) external onlyOwner{
        require(_marketfees != 0, "ERR 14"); // Market fees cannot be zero
