...
// File: contracts\NftExchangeV1.sol
...
contract NftExchangeV1 is Initializable, ERC1155SupplyUpgradeable, EIP712Upgradeable {
    address owner;
    ...
    event NewNftFixedPriceVoucherRedeemed(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NftFixedPriceVoucherRedeemed(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NewNftAuctionVoucherRedeemed(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NftAuctionVoucherRedeemed(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NewNftAuctionWithFixedPriceVoucherRedeemed(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    event NftAuctionWithFixedPriceVoucherRedeemed(
        address seller,
        address buyer,
        uint256 tokenId,
        uint256 price,
    ...
    /**
     * @dev Burns a voucher ID associated with the caller of this method so
     * that voucher cannot be redeemed in the future.
     */
    function burnVoucher(uint256 voucherId) external {
        redeemedVouchers[msg.sender][voucherId] = true;
        emit VoucherBurned(msg.sender, voucherId);
    }
    ...
    function placeOrder(
        ...
        address seller,
        OrderType orderType,
        BuyerVoucher calldata buyerVoucher,
        SellerVoucher calldata sellerVoucher,
        uint256 eventId
    ) external payable {
        ...
        if (
            ...
            if (sellerVoucher.sellVoucherType == SellVoucherType.FIXED_PRICE) {
                if (sellerVoucher.saleMarketType == SaleMarketType.PRIMARY) {
                    emit NewNftFixedPriceVoucherRedeemed(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
                    ...
                    emit NftFixedPriceVoucherRedeemed(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
            ...
            } else if (sellerVoucher.sellVoucherType == SellVoucherType.AUCTION_WITH_FIXED_PRICE) {
                if (sellerVoucher.saleMarketType == SaleMarketType.PRIMARY) {
                    emit NewNftAuctionWithFixedPriceVoucherRedeemed(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
                    ...
                    emit NftAuctionWithFixedPriceVoucherRedeemed(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        sellerVoucher.minPriceWei,
            ...
            if (sellerVoucher.sellVoucherType == SellVoucherType.AUCTION) {
                if (sellerVoucher.saleMarketType == SaleMarketType.PRIMARY) {
                    emit NewNftAuctionVoucherRedeemed(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        buyerVoucher.offerWei,
                    ...
                    emit NftAuctionVoucherRedeemed(
                        seller,
                        buyer,
                        sellerVoucher.tokenId,
                        buyerVoucher.offerWei,
    ...
    /**
     * @dev Verifies the signature for a given BuyerVoucher, returning the
     * address of the signer.
     *
     * Will revert if the signature is invalid.
     *
     * @param voucher A BuyerVoucher describing an offer made by a buyer.
     */
    function _verify(BuyerVoucher calldata voucher) internal view returns (address) {
        bytes32 digest = _hash(voucher);
        return ECDSAUpgradeable.recover(digest, voucher.signature);
    }

    /**
     * @dev Verifies the signature for a given SellerVoucher, returning the
     * address of the signer.
     *
     * Will revert if the signature is invalid.
     *
     * @param voucher A SellerVoucher describing an NFT for sale.
     */
    function _verify(SellerVoucher calldata voucher) internal view returns (address) {
        bytes32 digest = _hash(voucher);
        return ECDSAUpgradeable.recover(digest, voucher.signature);
    }

    /**
     * @dev Returns a hash of the given BuyerVoucher, prepared using
     * EIP712 typed data hashing rules.
     *
     * @param voucher A BuyerVoucher to hash.
     */
    function _hash(BuyerVoucher calldata voucher) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            'BuyerVoucher(uint8 buyVoucherType,uint256 voucherId,uint256 offerVoucherId,uint256 tokenId,uint256 offerWei,address wethSmartContractAddress,bool hasEndDate,uint256 endDate)'
                        ),
                        voucher.buyVoucherType,
                        voucher.voucherId,
                        voucher.offerVoucherId,
                        voucher.tokenId,
                        voucher.offerWei,
    ...
    /**
     * @dev Returns a hash of the given SellerVoucher, prepared using
     * EIP712 typed data hashing rules.
     *
     * @param voucher A SellerVoucher to hash.
     */
    function _hash(SellerVoucher calldata voucher) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            'SellerVoucher(uint8 sellVoucherType,uint8 saleMarketType,uint256 voucherId,uint256 tokenId,bool hasStartDate,uint256 startDate,bool hasEndDate,uint256 endDate,address wethSmartContractAddress,uint256 minPriceWei,string uri,uint8 creatorFee,address creatorFeePayoutWalletAddress)'
                        ),
     ...
     * fixed price. metadataUri and creatorFee are only used for primary
     * market sales.
     */
    function _redeemVoucherForNftFixedPrice(
        ...
        if (saleMarketType == SaleMarketType.PRIMARY) {
            // Make sure that the NFT does not already exist.
            require(totalSupply(params.tokenId) == 0, TOKEN_ALREADY_EXISTS);

            require(params.creatorFee <= 100, CREATOR_FEE_OVER_TEN_PERCENT);
        } else {
            // Make sure that the token already exists.
            require(totalSupply(params.tokenId) == 1, TOKEN_DOES_NOT_EXIST);

            // Make sure that the seller is the current owner of the NFT.
            require(balanceOf(params.seller, params.tokenId) == 1, SELLER_MUST_BE_NFT_OWNER);
        }

        // Make sure that the voucher has not been redeemed yet.
        require(!redeemedVouchers[params.seller][params.voucherId], SELL_VOUCHER_ALREADY_REDEEMED);
        ...
        // cost.
        require(msg.value >= params.minPriceWei, INSUFFICIENT_ETH_BALANCE);
        ...
        uint256 weiToExchange = (params.minPriceWei * exchangeRateNumerator) /
            EXCHANGE_RATE_DENOMINATOR;
        uint256 weiToCreator;
        if (saleMarketType == SaleMarketType.SECONDARY) {
            weiToCreator = (params.minPriceWei * tokenCreatorFee[params.tokenId]) / 1000;
        }
        uint256 weiToSeller = params.minPriceWei - weiToExchange - weiToCreator;
    ...
    function _acceptOfferForSale(
        ...
        if (saleMarketType == SaleMarketType.PRIMARY) {
            // Make sure that the NFT does not already exist.
            require(totalSupply(params.sellVoucherTokenId) == 0, TOKEN_ALREADY_EXISTS);

            require(params.creatorFee <= 100, CREATOR_FEE_OVER_TEN_PERCENT);
        } else {
            // Make sure that the token already exists.
            require(totalSupply(params.sellVoucherTokenId) == 1, TOKEN_DOES_NOT_EXIST);

            // Make sure that the seller is the current owner of the NFT.
            require(
                balanceOf(params.seller, params.sellVoucherTokenId) == 1,
                SELLER_MUST_BE_NFT_OWNER
            );
        }

        // Make sure that the buy and sell vouchers have not been redeemed yet.
        require(!redeemedVouchers[params.buyer][params.buyVoucherId], BUY_VOUCHER_ALREADY_REDEEMED);
        require(
            !redeemedVouchers[params.seller][params.sellVoucherId],
            SELL_VOUCHER_ALREADY_REDEEMED
        );

        // Make sure that the buy voucher is intended for the provided sell
        // voucher.
        require(
            params.buyVoucherAssociatedSellVoucherId == params.sellVoucherId,
            BUY_AND_SELL_VOUCHER_ID_MISMATCH
        );
        ...
        // avoid the possibility of a reentrancy attack.
        uint256 weiToExchange = (params.buyVoucherOfferWei * exchangeRateNumerator) /
            EXCHANGE_RATE_DENOMINATOR;
        uint256 weiToCreator;
        if (saleMarketType == SaleMarketType.SECONDARY) {
            // solium-disable-next-line operator-whitespace
            weiToCreator =
                (params.buyVoucherOfferWei * tokenCreatorFee[params.buyVoucherTokenId]) /
                1000;
        }
        uint256 weiToSeller = params.buyVoucherOfferWei - weiToExchange - weiToCreator;
     ...
     * BuyerVoucher is applicable for OrderType.ACCEPT_BUY_NOW_OFFER when the
     * buyer makes an offer on a fixed price item. BuyVoucher is also
    ...
    struct BuyerVoucher {
        ...
        uint256 voucherId;
        // @dev A unique ID associated with the seller offer voucher that this
        // buy offer is intended for.
        uint256 offerVoucherId;
        // @dev The id of the token to be transacted.
        uint256 tokenId;
        // @dev The offer value in Wei.
        uint256 offerWei;
    ...
    /**
     * @dev Represents an NFT that is listed for sale.
     */
    struct SellerVoucher {
        ...
        address wethSmartContractAddress;
        // @dev The minimum price (in wei) that the NFT creator is willing to
        // accept for the sale of this NFT. This field is used if
    ...
    struct AcceptOfferForSaleParams {
        ...
        uint256 buyVoucherAssociatedSellVoucherId;
        uint256 buyVoucherTokenId;
        address buyVoucherWethSmartContractAddress;
        uint256 buyVoucherOfferWei;
