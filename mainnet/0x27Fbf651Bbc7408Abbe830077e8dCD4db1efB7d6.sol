...
// File: localhost/contract/interface/IYouswapWithdrawPrecisionV1.sol
...
interface IYouswapWithdrawPrecisionV1 {

    function setMintAndInvite(YouswapFactoryV1, IYouswapInviteV1) external;
    
    function withdraw(address, uint256) external view returns (uint256);
    
    function subPower(uint256, address, uint256, address, address) external view returns (bool);

    function getActualEstimateAmount(address, uint256) external view returns (uint256, uint256);
    
    function transferOwnership(address) external;
    
}
// File: localhost/contract/implement/YouswapWithdrawPrecisionV1.sol
...
contract YouswapWithdrawPrecisionV1 is IYouswapWithdrawPrecisionV1 {
    ...
    YouswapFactoryV1 public factory;
    IYouswapInviteV1 public invite;
    
    constructor (YouswapFactoryV1 _factory, IYouswapInviteV1 _invite) {
        owner = msg.sender;
        factory = _factory;
        invite = _invite;
    }
    
    function setMintAndInvite(YouswapFactoryV1 _factory, IYouswapInviteV1 _invite) override external {
        require(owner == msg.sender, 'YouSwap:FORBIDDEN');
        factory = _factory;
        invite = _invite;
    }

    function withdraw(address _user, uint256 _pool) override external view returns (uint256) {
        uint256 min = 0;        
        if (ZERO == _user) {
            return min;
        }
        
        (, , , , uint256 amount, , , uint256 totalPower, , ) = factory.poolInfos(_pool);
        (, uint256 userAmount, uint256 userInvitePower, uint256 userPledgePower, , , ) = factory.pledgeUserInfo(_pool, _user);

        min = amount > userAmount ? userAmount : amount;
        min = min > userPledgePower ? userPledgePower : min;
        min = min > totalPower ? totalPower : min;
        
        (address upper1, address upper2) = invite.inviteUpper2(_user);
        if (ZERO == upper1) {
            return min;
        }
            
        uint256 inviteSelfAmount = userInvitePower.mul(20);
        
        (, , uint256 upper1InvitePower, , , , ) = factory.pledgeUserInfo(_pool, upper1);
        uint256 upper1InviteAmount = upper1InvitePower.mul(100).div(15);
        if (ZERO == upper2) {
            if (inviteSelfAmount != 0) {
                min = min > inviteSelfAmount ? inviteSelfAmount : min;
            }
            min = min > upper1InviteAmount ? upper1InviteAmount : min;
            min = comparePower1(totalPower, min);
            
            return min;
        }
        
        (, , uint256 upper2InvitePower, , , , ) = factory.pledgeUserInfo(_pool, upper2);
        uint256 upper2InviteAmount = upper2InvitePower.mul(10);
        
        if (inviteSelfAmount != 0) {
            min = min > inviteSelfAmount ? inviteSelfAmount : min;
        }
        min = min > upper1InviteAmount ? upper1InviteAmount : min;
        min = min > upper2InviteAmount ? upper2InviteAmount : min;
        min = comparePower2(totalPower, min);
        
        return min;
    }
    
    function subPower(uint256 _pool, address _user, uint256 _amount, address _upper1, address _upper2) override external view returns (bool) {
        (, , , , uint256 amount, , , uint256 totalPower, , ) = factory.poolInfos(_pool);
        (, uint256 userAmount, uint256 userInvitePower, uint256 userPledgePower, , , ) = factory.pledgeUserInfo(_pool, _user);
        ...
        userPledgePower = userPledgePower.sub(_amount);
        totalPower = totalPower.sub(_amount);
        
        if (address(0) != _upper1) {
            uint256 inviteSelfPower = _amount.div(20);
            userInvitePower = userInvitePower.sub(inviteSelfPower);
            totalPower = totalPower.sub(inviteSelfPower);

            
            (uint256 upper1StartBlock, , uint256 upper1InvitePower, , , , ) = factory.pledgeUserInfo(2, _upper1);
            if (0 < upper1StartBlock) {
                amount = _amount.mul(15).div(100);//invite1Power->amount
                upper1InvitePower.sub(amount);//invite1Power->amount
                totalPower = totalPower.sub(amount);//invite1Power->amount

                if (address(0) != _upper2) {
                    (upper1StartBlock, , upper1InvitePower, , , , ) = factory.pledgeUserInfo(_pool, _upper2);//upper2StartBlock->upper1StartBlock,upper2InvitePower->upper1InvitePower
                    if (0 < upper1StartBlock) {
                        amount = _amount.div(10);//invite2Power->amount
                        upper1InvitePower = upper1InvitePower.sub(amount);//invite2Power->amount
                        totalPower = totalPower.sub(amount);//invite2Power->amount
                    }
                }
            }
        }
    ...
    function getActualEstimateAmount(address _user, uint256 _pool) override external view returns (uint256, uint256) {
        (address upper1, address upper2) = invite.inviteUpper2(_user);
        (, uint256 amountActual, , , , , ) = factory.pledgeUserInfo(_pool, _user);
        uint256 amountEstimate = this.withdraw(_user, _pool);
        this.subPower(_pool, _user, amountEstimate, upper1, upper2);
        
        return (amountActual, amountEstimate);
    }
    
    function comparePower1(uint256 _totalPower, uint256 _amount) internal pure returns (uint256) {
        uint256 result = _amount;
        uint256 inviteSelfPower = 0;
        uint256 invite1Power = 0;
        
        while ((result > 0) && (_totalPower > 0)) {
            inviteSelfPower = result.div(20);
            invite1Power = result.mul(15).div(100);
            if (_totalPower >= (result.add(inviteSelfPower).add(invite1Power))) {
                break;
            }
    ...
    function comparePower2(uint256 _totalPower, uint256 _amount) internal pure returns (uint256) {
        uint256 result = _amount;
        uint256 inviteSelfPower = 0;
        uint256 invite1Power = 0;
        uint256 invite2Power = 0;
        
        while ((result > 0) && (_totalPower > 0)) {
            inviteSelfPower = result.div(20);
            invite1Power = result.mul(15).div(100);
            invite2Power = result.div(10);
            if (_totalPower >= (result.add(inviteSelfPower).add(invite1Power).add(invite2Power))) {
                break;
            }
            result = result.sub(10);
        }
