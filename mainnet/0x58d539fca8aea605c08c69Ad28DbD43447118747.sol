...
contract Duality is ERC20, Ownable, RewardManager {
    ...
    address public buybackWallet = owner();
    ...
    constructor() ERC20("DUALITY", "DUALITY") {
    ...
    function allocateRewardPool(address pool, uint256 amount) internal {
        require(pool != address(0));
        _mint(pool, amount);
    }

    function createRewardPool(uint256 amount) public onlyOwner{
    ...
    function distributeRewards(uint256 amount) public onlyOwner{
        uint256 amountToWithdraw = _distReward(amount);
        allocateRewardPool(msg.sender, amountToWithdraw);
    }
