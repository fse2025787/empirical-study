...
contract FishTankNFT is ERC721, Ownable {
    ...
    function mintFreePresale(uint numberOfMints) public payable{
        uint256 supply = _tokenSupply.current() + 1;
        uint256 freeMints = freePresaleWhitelist[msg.sender];
        require(freeMints > 0, "No free mints for this adress");
        require(numberOfMints <= freeMints, "More free mints than allowed");
        require(mintFreePresaleOpen, "Presale needs to be active");
        require(supply.add(numberOfMints) <= supplyLimit, "Payment exceeds total supply");
        freePresaleWhitelist[msg.sender] = freeMints - numberOfMints;
    ...
    function editPresale(address[] calldata presaleAddresses, uint256[] calldata amount) external onlyOwner {
        for(uint256 i; i < presaleAddresses.length; i++){
            freePresaleWhitelist[presaleAddresses[i]] = amount[i];
        }
    }
