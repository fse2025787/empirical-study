...
// File: contracts/MetaAliensNFT.sol
...
contract MetaAliensNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if (onlyWhitelisted == true) {
            require(iswhitelisted(msg.sender), "User is not whielisted");
  ...
  function viewBaseURI() public onlyOwner view returns (string memory ) {
      return baseURI;
  }
}