...
interface SenturiaGalleryOriginals721 {
    function mint(
...
interface SenturiaGalleryEditions1155 {
    function mint(
        string memory ipfsmetadata,
        address from,
        address to,
...
contract SenturiaTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    event DropsSale(address indexed _from, uint256 amount);
    event DropsSaleToken(address indexed _from, uint256 amount);
    using SafeMathUpgradeable for uint256;

    function initialize() public initializer {
        __Ownable_init();
        serviceValue = 0;
        sellervalue = 10000000000000000000;
        deci = 18;
        publicMint = true;
        _tid = 1;
    }
    ...
    address public Senturia721;
    address public Senturia1155;
    uint256 deci;
    uint256 public _tid;
    ...
    function orderPlace(
        ...
        if (
            ...
            if (_type == 721) {
                require(
                    IERC721Upgradeable(_conAddress).ownerOf(tokenId) ==
                        msg.sender,
                    "Not a Owner"
                );
            } else {
    ...
    // fromContract[0] - from fromContract[1]  - contractAddress
    function saleToken(uint256[] memory ids, address[] memory fromContract)
        public
        payable
    {
        require(
            ids[1] == order_place[fromContract[0]][ids[0]].price.mul(ids[2]) &&
                order_place[fromContract[0]][ids[0]].price.mul(ids[2]) > 0,
            "Order Mismatch"
        );
        _saleToken(fromContract, ids, "ETH");
        if (ids[3] == 721) {
            IERC721Upgradeable(fromContract[1]).safeTransferFrom(
                fromContract[0],
                msg.sender,
                ids[0]
            );
            if (order_place[fromContract[0]][ids[0]].price > 0) {
                delete order_place[fromContract[0]][ids[0]];
            }
        } else {
            if (
                IERC1155Upgradeable(fromContract[1])
                    .balanceOf(fromContract[0], ids[0])
                    .sub(ids[2]) == 0
            ) {
                if (order_place[fromContract[0]][ids[0]].price > 0) {
                    delete order_place[fromContract[0]][ids[0]];
            ...
            IERC1155Upgradeable(fromContract[1]).safeTransferFrom(
                fromContract[0],
                msg.sender,
                ids[0],
                ids[2],
    ...
    // fromContract[0] - from fromContract[1]  - contractAddress
    function _saleToken(
        address[] memory fromContract,
        uint256[] memory ids,
        string memory bidtoken
    ) internal // address _conAddr
    {
        uint256[7] memory ldatas;
        ldatas[6] = pERCent(ids[1], serviceValue).add(ids[1]);
        address create;
        if (Senturia721 == fromContract[1] || Senturia1155 == fromContract[1]) {
            if (ids[3] == 721) {
                (create, ldatas[0]) = SenturiaGalleryOriginals721(Senturia721)
                    .getCreatorsAndRoyalty(ids[0]);
            } else {
                (create, ldatas[0]) = SenturiaGalleryEditions1155(Senturia1155)
                    .getCreatorsAndRoyalty(ids[0]);
            }
        }
        require(
        ...
        if (
            ...
            if (ldatas[5] != 0) {
                payable(fromContract[0]).transfer(ldatas[5]);
            }
        } else {
    ...
    function saleWithToken(
        ...
        uint256[] memory ids,
        address[] memory fromContract
    ) public // address _conAddr
    {
        require(
            ids[1] == order_place[fromContract[0]][ids[0]].price.mul(ids[2]),
            "Order is Mismatch"
        );
        _saleToken(fromContract, ids, bidtoken);
        if (ids[3] == 721) {
            IERC721Upgradeable(fromContract[1]).safeTransferFrom(
                fromContract[0],
                msg.sender,
    ...
    function acceptBId(
        ...
        _acceptBId(bidtoken, bidaddr, ids, _conAddr);
        if (ids[3] == 721) {
            ...
            if (
                IERC1155Upgradeable(_conAddr).balanceOf(msg.sender, ids[0]).sub(
                    ids[2]
                ) == 0
            ) {
                if (order_place[msg.sender][ids[0]].price > 0) {
    ...
    function _acceptBId(
        ...
        ldatas[6] = pERCent(ids[1], serviceValue).add(ids[1]);
        address create;
        if (Senturia721 == _conAddr || Senturia1155 == _conAddr) {
            if (ids[3] == 721) {
                (create, ldatas[0]) = SenturiaGalleryOriginals721(Senturia721)
                    .getCreatorsAndRoyalty(ids[0]);
            } else {
        ...
        if (ldatas[3] != 0) {
            t.transferFrom(from, owner(), ldatas[3].div(10**ldatas[1]));
        }
        if (ldatas[4] != 0) {
            for (
                uint256 i = 0;
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            SenturiaGalleryOriginals721(Senturia721).mint(
                ipfsmetadata,
                users[0],
                users[1],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Senturia721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            SenturiaGalleryEditions1155(Senturia1155).mint(
                ipfsmetadata,
                users[0],
                users[1],
                datas[0],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Senturia1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(address _Senturia721, address _Senturia1155)
        public
        onlyOwner
    {
        Senturia721 = _Senturia721;
        Senturia1155 = _Senturia1155;
    }
    ...
    function openUri(bool open) public onlyOwner {
        SenturiaGalleryOriginals721(Senturia721)._openUri(open);
        SenturiaGalleryEditions1155(Senturia1155)._openUri(open);
    }
    ...
    function _transferNFT(
        ...
        SenturiaGalleryEditions1155(_conaddress).TransferNFT(to, tokenid, count);
    }

    function _dropsale(uint256 amount, string memory coinname) public payable {
        if (
            keccak256(abi.encodePacked((coinname))) ==
            keccak256(abi.encodePacked(("ETH")))
        ) {
            payable(owner()).transfer(msg.value);
            emit DropsSale(msg.sender, msg.value);
        } else {
            IERC20Upgradeable t = IERC20Upgradeable(tokentype[coinname]);
            uint256 decimal = deci.sub(t.decimals());
            require(
                amount <= t.allowance(msg.sender, address(this)),
                "You Dont have Enough Allowance"
            );
            t.transferFrom(msg.sender, owner(), amount.div(10**decimal));
            emit DropsSaleToken(msg.sender, amount);
        }
    }
}