...
contract StablzLiquidityMining is StablzStaking, OracleManaged {
    ...
    constructor(
        ...
        uint _apr6Month,
        uint _apr12Month
    ) StablzStaking(_lpToken, _stablz, _totalRewards, _minimumDeposit, _apr1Month, _apr3Month, _apr6Month, _apr12Month) {
        _setOracle(_oracle);
        IUniswapV2Pair lpToken = IUniswapV2Pair(_lpToken);
        require(lpToken.token0() == _stablz || lpToken.token1() == _stablz, "StablzLiquidityMining: _lpToken pair does not contain _stablz");
    }
    ...
    function setLPValue(uint _lpPrice, uint _lifeTimePeriod) external onlyOwnerOrOracle {
        require(_lpPrice > 0, "StablzLiquidityMining: _lpPrice cannot be zero");
        require(_lifeTimePeriod <= MAXIMUM_LIFETIME_PERIOD, "StablzLiquidityMining: _lifeTimePeriod is greater than the maximum lifetime period");
        lpPrice = _lpPrice;
    ...
    function _calculateReward(uint _amount, uint _lockUpPeriodType) internal override view returns (uint reward) {
        ...
        require(_isLPPriceValid(), "StablzLiquidityMining: LP price has expired");
