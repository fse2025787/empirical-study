...
// File: SausageTown.sol

// Mint 3 for Mess Hall
// Mint 10 for Food Fight



contract SausageTownWTF is Ownable, ERC721A, ReentrancyGuard {
    uint256 public immutable maxPerWallet = 40;
    uint256 public immutable maxPerFreeMint = 5;

    uint256 public immutable freeMints = 1500;
    ...
    mapping(address => uint256) freeListAmount;
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        ...
        require(quantity <= maxPerTx, "try minting less, you can only mint 10 at a time");
    ...
    function freeMint(uint256 quantity) external callerIsUser {
        ...
        require(
            ...
            "there are no more freeMints"
        );
        require(
            walletQuantity(msg.sender) + quantity <= maxPerWallet,
            "can not mint this many .. you have too many suasages"
        );
        require(freeMintActive, "can not mint yet .. wait for the moon");
        require(quantity <= maxPerFreeMint, "you can only mint up to 5 per tx");
        require(freeListAmount[msg.sender] + quantity <= maxPerFreeMint, "you are only allowed 5 free suasages");


        freeListAmount[msg.sender] += quantity;
        _safeMint(msg.sender, quantity);
    }
