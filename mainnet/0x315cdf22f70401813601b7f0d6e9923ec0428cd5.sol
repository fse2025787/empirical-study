...
contract BenMezrichProject1 is ERC721, Ownable {
    ...
    constructor (string memory _tokenBaseUri, address _fundWallet) ERC721("Ben Mezrich Project 1", "BENFT1") {
        _setBaseURI(_tokenBaseUri);
        fundWallet = _fundWallet;
    }

    function mintToken(uint256 _howMany) external payable whenPublicSaleActive {
	    require(_howMany > 0, "Minimum 1 tokens need to be minted");
	    require(mintedTokens + _howMany <= MAX_SUPPLY, "Minting would exceed max supply");
		require(itemPrice.mul(_howMany) == msg.value, "Insufficient ETH to mint");
        require(walletMinted[_msgSender()] + _howMany <= walletLimit, "Wallet limit exceeds");
        ...
        walletMinted[_msgSender()] = walletMinted[_msgSender()].add(_howMany);
	}
    ...
    function _mintToken(address _to) private {
        if(mintedTokens == 0) {
            _lastIpfsId = random(1, MAX_SUPPLY, uint256(uint160(address(_msgSender()))) + 1);
    ...
    function modifyItemPrice(uint256 _itemPrice) external onlyOwner {
    ...
    function setFundWallet(address _fund) external onlyOwner {
        fundWallet = _fund;
    }
