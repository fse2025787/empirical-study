...
contract ThoughtsNFT is ERC721Enumerable {
  ...
  mapping(uint256=>string) public wordBubble;
  ...
  function buy(uint256[] memory toBuy) public payable {
    require(saleEnabled, "Sale has ended");
    require(msg.value >= minPrice * toBuy.length, "Not enough ether to buy");
    for (uint256 i = 0; i < toBuy.length; i++) {
      require(toBuy[i] != 0 && toBuy[i] <= MAX_SUPPLY, "Invalid tokenId");
      _mint(msg.sender, toBuy[i]); 
    }
  }
  ...
  function setBubble(string memory newBubble, uint256 tokenId) public {
    require(this.ownerOf(tokenId) == msg.sender, "Only owner can setBubble");
    require(timelock[tokenId] < block.timestamp, "Can't setBubble too soon");
    wordBubble[tokenId] = newBubble;
    timelock[tokenId] = block.timestamp + 1 days;
  }
