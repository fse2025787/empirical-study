...
// File contracts/PsychedelicFlowersSale.sol

// 
pragma solidity ^0.8.9;


contract PsychedelicFlowersSale is Ownable {

    //The contract holding the NFTs.
    PsychedelicFlowers public immutable psychedelicFlowers;

    uint256 public mintLimit = 2; //Free mint per account.

    //Tracks the number of NFT minted per account.
    mapping(address => uint256) public minted;

    constructor(address _psychedelicFlowers) {
        psychedelicFlowers = PsychedelicFlowers(_psychedelicFlowers);
    }

    /**
     * @dev Changes the mint limit.
     * @param _newLimit The new mint limit. 0 means pausing the sale.
     */
    ...
    /**
     * @dev Free mints tokens.
     * @param _to The address that tokens will be minted to.
     * @param _amount The amount of tokens to mint. Must not go exceed mintLimit.
     */    
    function mint(address _to, uint256 _amount) public {
        require(_amount + minted[_to] <= mintLimit, "Too much mints.");

        minted[_to] += _amount;

        psychedelicFlowers.mintTo(_to, _amount);
    }
}