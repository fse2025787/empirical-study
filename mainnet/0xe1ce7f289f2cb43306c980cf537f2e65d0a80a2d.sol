...
contract RisingStackersToken is ERC20, Ownable {

    uint256 public presaleCost = 0.0001 ether;
    uint256 public publicCost = 0.0002 ether;
    uint256 public maxSupply = 12000000000 ether;
    bool public preSale = true;

    constructor(uint256 totalSupply) ERC20("Rising Stackers Token", "STK") {
    ...
    function mint(address _to,uint256 amount) public payable {
        ...
        if(!isOwner(msg.sender)){
            if(preSale){
                require(msg.value >= amount * presaleCost / 1 ether, "Insufficient funds");
            }else{
                require(msg.value >= amount * publicCost / 1 ether, "Insufficient funds");
            }
        }
        _mint(_to,amount);
    }
