...
contract Passport is ERC721BurnableUpgradeable, PassageAccess, UUPSUpgradeable, ERC2771Recipient, Ownable, IPassport {
    ...
    bool public whitelistClaimEnabled;
    ...
    uint256 public whitelistClaimFee; // 0 is no fee
    bytes32 public whitelistRoot;
    mapping(address => bool) public whitelistClaimed; // whitelist address -> claimed
    ...
    function initialize(
        ...
        claimAmount = 1;
        emit PassportInitialized(_msgSender(), address(this), _tokenSymbol, _tokenName, _transferEnabled, _maxSupply);
    }
    ...
    function claimWhitelist(
        ...
        require(whitelistClaimEnabled, "T9");
        if (whitelistClaimFee > 0) require(msg.value == whitelistClaimFee * _claimAmount, "T8");
        ...
        require(!whitelistClaimed[_msgSender()], "T12");
    ...
    function mintPassport(address to) external onlyRole(MINTER_ROLE) returns (uint256) {
        return _mint(to);
    }

    
    
    function mintPassports(address[] calldata _addresses) external onlyRole(MINTER_ROLE) {
    ...
    function lockTransferEnabled() external versionLockRequired onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setClaimOptions(uint256 _claimFee, uint256 _claimAmount) external onlyRole(MANAGER_ROLE) {
        claimFee = _claimFee;
    ...
    function setTransferEnabled(bool _transferEnabled) external onlyRole(MANAGER_ROLE) {
    ...
    function setWhitelistClaimFee(uint256 _claimFee) external onlyRole(MANAGER_ROLE) {
        whitelistClaimFee = _claimFee;
    }

    
    
    function setWhitelistClaimEnabled(bool _claimEnabled) external onlyRole(MANAGER_ROLE) {
        whitelistClaimEnabled = _claimEnabled;
    }

    
    
    
    function setWhitelistRoot(bytes32 _whitelistRoot) external onlyRole(MANAGER_ROLE) {
    ...
    function setWhitelistOptions(uint256 _claimFee, bytes32 _whitelistRoot) external onlyRole(MANAGER_ROLE) {
        whitelistClaimFee = _claimFee;
        whitelistRoot = _whitelistRoot;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require((from == address(0) || ((to == address(0)) || transferEnabled)), "T18");
    ...
    function hasUpgraderRole(address _address) public view override(IPassport, PassageAccess) returns (bool) {
    ...
    function upgradeTo(address newImplementation) external override(UUPSUpgradeable, IPassport) onlyProxy {
        _authorizeUpgrade(newImplementation);
        _upgradeToAndCallUUPS(newImplementation, new bytes(0), false);
    }
