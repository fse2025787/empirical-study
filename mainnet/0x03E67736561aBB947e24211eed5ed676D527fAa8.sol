...
contract DynoStrategy is Ownable {
    ...
    // Strategy should be owner of masterchef
    ...
    function registerMasterchef(address _masterchef) external onlyOwner {
        masterchef = _masterchef;
    }

    function functionCall(address _target, bytes memory _calldata) external onlyOwner returns (bool) {
    ...
    function updateTax(uint256 _buyTax, uint256 _sellTax) external onlyOwner {
        buyTax = _buyTax;
        sellTax = _sellTax;
        require(buyTax < 20 && sellTax < 20, 'set high tax');
    }
