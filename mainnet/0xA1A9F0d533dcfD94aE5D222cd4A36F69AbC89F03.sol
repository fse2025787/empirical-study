...
contract OperWallet is Ownable {
  ...
  function updateShares(address _holder, uint _share) external onlyOwner {
    // remove old share
    totalShares = totalShares.sub(shares[_holder]);
    // update share
    shares[_holder] = _share;
    // add new share
    totalShares = totalShares.add(_share);
  }

  function updatePeriod() private {
    if(block.timestamp >= periodDeadline)
      periodIndex = periodIndex + 1;
  }

  function withdraw() external {
    updatePeriod();
    // check msg.sender share
    uint userShare = shares[msg.sender];
    require(userShare > 0, "Empty shares");
    // check msg.sender claimed
    require(!userRemoved[periodIndex][msg.sender], "Withdrawed");
    // compute withdraw amount
    uint _sharesRemoved = sharesRemoved[periodIndex];
    ...
    sharesRemoved[periodIndex] = sharesRemoved[periodIndex].add(userShare);
