 ...
 */
abstract contract ERC721AQueryable is Ownable, ERC721A, IERC721AQueryable {
    address biggieContract;

    modifier onlyBiggieContractOrOwner {
        require(msg.sender == biggieContract || owner() == msg.sender);
        _;
    }

    function setBiggieContractAddress(address _address) external onlyOwner {
        biggieContract = _address;
    }
     ...
     * Burn token
     */
    function burnToken(uint256 tokenId) external onlyBiggieContractOrOwner {
        _burn(tokenId);
    }
     ...
     * Batch burn token
     */
    function batchBurnToken(uint256[] calldata tokenIds) external onlyOwner {
