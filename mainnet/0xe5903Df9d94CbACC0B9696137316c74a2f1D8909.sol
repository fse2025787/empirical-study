...
contract Aliens is ERC721A, Ownable {
    ...
    */
    function airdrop(uint256 from, uint256 to, address collection) external onlyOwner {
        require(to-from <= maxAirdrop, "maxAirdrop one tx limit have reached!");
        require(!isPublicSaleActive() && !isPrivateSaleActive(), "public and private sales should be not active");
        require(!_freeze, "minting is freeze!");

        for (uint256 i = from; i <= to; i++) {
            _safeMint(IERC721Enumerable(collection).ownerOf(i), 1);
        }
    }
