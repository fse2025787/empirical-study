...
contract BorrowRecipient is Controllable {
  ...
  function deposit(
    ...
    return IUniswapV3Vault(investmentVault).deposit(
      _amount0,
      _amount1,
      _zapFunds,
      _sqrtRatioX96,
  ...
  function withdraw(uint256 _numberOfShares,
    ...
    uint256 _sqrtRatioX96,
    uint256 _tolerance,
    uint256 _amount0OutMin, uint256 _amount1OutMin, uint160 _sqrtPriceLimitX96
  ) external onlyGovernance returns (uint256, uint256) {
    return IUniswapV3Vault(investmentVault).withdraw(_numberOfShares, _token0, _token1, _sqrtRatioX96, _tolerance, _amount0OutMin, _amount1OutMin, _sqrtPriceLimitX96);
  }

  function drain(address token, uint256 _amount) external onlyGovernance {
    if (underlying == token) {
      require(IdleBorrowableStrategy(strategy).borrowedShares() == 0, "Settle loan before taking profit");
    }
