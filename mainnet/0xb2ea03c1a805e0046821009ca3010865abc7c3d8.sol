...
contract SuperCar is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0xC53dFE7f021E33356b0A1Cf43563ABc484d469B3);
    uint256 public ENZO_MAX = 1000;
    uint256 public PRICE = 0.05 ether; // 0.05 ETH

    string private _tokenBaseURI = "";
    bool private _isActive = false;

    Counters.Counter private _publicENZO;

    constructor() ERC721("Enzos Ferrari", "ENZO") {}
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        require(_publicENZO.current() < ENZO_MAX, "Purchase would exceed ENZO_MAX");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicENZO.current();

            if (_publicENZO.current() < ENZO_MAX) {
                _publicENZO.increment();
                _safeMint(to, tokenId);
            }
        }
    }
