...
interface IAvatarComic {
    function existsToken(address _addr, uint _id) external view returns(bool);
}

// File: contracts/IMultipassComic.sol



interface IMultipassComic {
    function hasApplyComic(uint32 multipassId) external view returns(bool);
...
contract Comic is ERC1155, Ownable {
    ...
    {
        nftToken.allowMaxAvatarsId = _id;
    }
    ...
    function claimFromMultipass(uint16[] memory multipassIds) external canMint(uint16(multipassIds.length))
    {
        ...
        {
            if (!IMultipassComic(multipassAddress).hasApplyComic(multipassIds[i]))
            {
                IMultipassComic(multipassAddress).applyComic(multipassIds[i], holder, _currentMintedID);
                _balances[_currentMintedID][holder] += 1;

                emit TransferSingle(_msgSender(), address(0), holder, _currentMintedID, 1);
                emit ClaimFromMultipsas(_currentMintedID, multipassIds[i], holder, block.timestamp);
