...
/**
 *   SpeedWagon The Ultimate Waifu
 *   https://t.me/SpeedWagonToken
 *   https://SpeedWagonInu.com
*/
...
contract SpeedWagonFoundation is Context, IERC20, Ownable {
    ...
    string private constant _name = "SpeedWagonFoundation";
    string private constant _symbol = "SPWF";
    ...
    uint256 public _sellMarketingFee = 14; // these taxes will be set so match the buy taxes after 24 hours 
    
    uint256 public tradingActiveBlock = 0; // 0 means trading is not active
    mapping(address => bool) public boughtEarly; // mapping to track addresses that buy within the first 2 blocks pay a 2x tax for 24 hours to sell
    ...
    uint256 public maxTransactionAmount = (_tTotal * maxTxPercent) / 1000; // .05% maxTransactionAmountTxn
    mapping (address => bool) public _isExcludedMaxTransactionAmount;
    
    uint256 private maxWalletPercent = 30;
    uint256 public maxWalletSize = (_tTotal * maxWalletPercent) / 1000; // 3% maxWalletSize of Total Supply
    
    bool private gasLimitActive = true;
    uint256 private gasPriceLimit = 1000 * 1 gwei; // do not allow over 1000 gwei for launch
    ...
    constructor() payable {
        _rOwned[_msgSender()] = _rTotal / 1000 * 5;
        _rOwned[address(this)] = _rTotal / 1000 * 995;
        
        minimumTokensBeforeSwap = _tTotal * 5 / 10000; // 0.05% swap tokens amount
        
        marketingAddress = payable(0x847927604d59d0D6E07b75F4ba145AE90DD04458); // Marketing Address
        
        devAddress = payable(0x1eb220Df28CD888254322910A8C3d9c793F3D22D); // Dev Address
        ...
        emit Transfer(address(0), _msgSender(), _tTotal * 5 / 1000);
        emit Transfer(address(0), address(this), _tTotal * 995 / 1000);
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external virtual returns (bool) {_approve(
            _msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20: decreased allowance below zero")); return true;}
    ...
    function enableTrading() internal onlyOwner {
        ...
        earlyBuyPenaltyEnd = block.timestamp + 7200 hours;
    }
    ...
    function launch(address[] memory presaleWallets, uint256[] memory amounts) external onlyOwner returns (bool){
        require(!tradingActive, "Trading is already active, cannot relaunch.");
        require(presaleWallets.length < 200, "Can only airdrop 200 wallets per txn due to gas limits"); // allows for airdrop + launch at the same exact time, reducing delays and reducing sniper input.
        for(uint256 i = 0; i < presaleWallets.length; i++){
            address wallet = presaleWallets[i];
    ...
    function setMaxTxPercent(uint256 percent) external onlyOwner {
        maxTransactionAmount = (_tTotal * percent) / 1000;
    }
    
    function setMaxWalletSize(uint256 percent) external onlyOwner {
        maxWalletSize = (_tTotal * percent) / 1000;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            // Sell
                ...
                // higher tax if bought in the same block as trading active for 7200 hours (sniper protect)
                if(boughtEarly[from] && earlyBuyPenaltyEnd > block.timestamp){
                    _taxFee = _taxFee * 1;
                    _sellMarketingFee = _sellMarketingFee * 2;
        ...
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee <= 20, "Must keep buy taxes below 20%");
