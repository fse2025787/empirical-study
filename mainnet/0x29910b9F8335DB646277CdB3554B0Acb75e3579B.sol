...
contract XENEXIS_NFT is ERC721, Ownable {
  using Strings for uint256;
  ...
  mapping (address => bool) public mintedForMaestro;
  mapping (uint256 => mapping (address => bool)) public mintedForOthers;
  ...
  uint256 public costForMaestro = 1.11 ether;
  ...
  constructor() ERC721("XENEXIS", "XNX") {
    setUriPrefix("ipfs://QmedvF9FvKRci67X6VGSou7fL12C2fhDd7GiqMhYRuxFRh/");
  ...
  function mintForMaestro(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(!mintedForMaestro[msg.sender], "Already minted!");

    //_mintLoop(msg.sender, _mintAmount);
    for (uint256 i = 0; i < _mintAmount; i++) {
        require(mintCountForMaestro < maxMintCountForMaestro);
        supply.increment();
        _safeMint(msg.sender, mintCountForMaestro);
        mintCountForMaestro = mintCountForMaestro + 1;
    }
    mintedForMaestro[msg.sender] = true;
  }
  ...
  function mintForOthers(uint256 _collectionNumber, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(!mintedForOthers[_collectionNumber][msg.sender], "Already minted!");

    //_mintLoop(msg.sender, _mintAmount);
    for (uint256 i = 0; i < _mintAmount; i++) {
        require(mintCountForOthers[_collectionNumber] < maxMintCountForOthers[_collectionNumber]);
        supply.increment();
        _safeMint(msg.sender, firstNumberForOthers[_collectionNumber].add(mintCountForOthers[_collectionNumber]));
        mintCountForOthers[_collectionNumber] = mintCountForOthers[_collectionNumber] + 1;
    }
    mintedForOthers[_collectionNumber][msg.sender] = true;
  }
...
//   }
  ...
  function getMintedForMaestro(address _checkAddress) public view returns (bool) {
    return mintedForMaestro[_checkAddress];
  }

  function getMintedForOthers(uint256 _collectionNumber, address _checkAddress) public view returns (bool) {
