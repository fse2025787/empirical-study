...
contract NFTStakingPool is ERC721HolderUpgradeable, ReentrancyGuard,Ownable {
    using SafeMath for uint256;
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;     // How many NFTLP tokens the user has provided.
    ...
    struct PoolInfo {
        IERC721 lpToken;           // Address of nft lp token contract.
        ...
        //use the swap pair to caculate bonustoken swap ratio
        address swapPairAddress;

        uint256 floorPrice; //set the nfts floor price based from oracle service
    }
    ...
    uint256 public decayRatio = 1000;     //default ratio: 0%
    ...
    // Add a new nft lp to the pool. Can only be called by the owner.
    // XXX DO NOT add the same LP token more than once. Rewards will be messed up if you do.
    function add(uint256 _allocPoint, IERC721 _lpToken,bool _enableBonus, address _bonusToken, address _swapPairAddress, uint256 _lockSeconds , uint256 _floorPrice, bool _withUpdate) public onlyAdmin {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if( pool.enableBonus == false ){
            minRet = iToken.mint(address(this), iTokenReward);
        }
        if (minRet) {
    ...
    function lockedToken(uint256 _pid, address _user) external view returns (uint256){
        ...
        for (uint256 i = 0; i < len; i++) {
            if( orders[i].orderTime.add(pool.lockSeconds) > checkTime ){
                lockedAmount = lockedAmount.add( 1 );
            }
        }
    ...
    // Deposit nft LP tokens to Pool for IToken allocation.
    function deposit(uint256 _pid, uint256 _tokenID) public notPause {
        depositIToken(_pid, _tokenID, msg.sender);
    }

    function _isApprovedOrOwner(address _address, IERC721 nftToken, uint _tokenID) internal view returns (bool) {
        return nftToken.getApproved(_tokenID) == _address || nftToken.ownerOf(_tokenID) == _address;
    }

    function depositIToken(uint256 _pid, uint256 _tokenID, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];

        require(_isApprovedOrOwner( msg.sender, pool.lpToken, _tokenID ),"nft is not valid asset");
        ...
        if (_tokenID > 0) {
            IERC721Upgradeable( address(pool.lpToken) ).safeTransferFrom( _msgSender(), address(this), _tokenID );
        ...
        userDepositInfo[_pid][msg.sender].push( DepositOrder({orderTime: block.timestamp, tokenID: _tokenID}));

        emit Deposit(_user, _pid, _tokenID);
    }

    // Withdraw LP tokens from Pool.
    function withdraw(uint256 _pid, uint256 _tokenID) public notPause {
        withdrawIToken(_pid, _tokenID, msg.sender);
    }

    function withdrawIToken(uint256 _pid, uint256 _tokenID, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        if (_tokenID > 0) {
            //need check the nft deposit orders
            DepositOrder[] memory orders = userDepositInfo[_pid][msg.sender];
            uint256 len = orders.length;
            ...
            for (uint256 i = 0; i < len; i++) {
                if( orders[i].orderTime.add(pool.lockSeconds) <= checkTime && orders[i].tokenID == _tokenID){
                    tokenIndex = i;
                    bFind = true;
                    break;
                }
            ...
            require( bFind == true , "no find nft token");

            //pop find order
            for (uint256 i = tokenIndex; i < len - 1 ; i++) {
                userDepositInfo[_pid][msg.sender][i] = userDepositInfo[_pid][msg.sender][i + 1];
            }

            //pop last order
            userDepositInfo[_pid][msg.sender].pop();

            IERC721Upgradeable( address(pool.lpToken) ).safeTransferFrom( address(this), _msgSender(),_tokenID );
    ...
    function emergencyWithdrawIToken(uint256 _pid, address _user) private {
        ...
        for (uint256 i = len -1 ; i >= 0; i--) {
            userDepositInfo[_pid][msg.sender].pop();
            IERC721Upgradeable( address(pool.lpToken) ).safeTransferFrom( address(this), _msgSender(),orders[i].tokenID );
        }
    ...
    // Safe IToken transfer function, just in case if rounding error causes pool to not have enough BXHs.
    function safeITokenTransfer(address _to, uint256 _amount) internal {
        uint256 bxhBal = iToken.balanceOf(address(this));
        if (_amount > bxhBal) {
            iToken.transfer(_to, bxhBal);
        } else {
    ...
    function safeBonusTransfer(uint256 _pid,address _to, uint256 _amount) internal {
        ...
        uint256 bal = IERC20(pool.bonusToken).balanceOf(address(this));
        if (_amount > bal) {
            IERC20(pool.bonusToken).transfer(_to, bal);
        } else {
