...
@title ERC-721 token for Aether
@author archiviste

*/

contract Aether is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public discountOnMint;
    mapping(uint256 => uint256) public discountOnRenewal;
    ...
    function mint() public payable nonReentrant {
        ...
        uint256 _discountTierA = ERC1155(discountAddress).balanceOf(msg.sender, 1);
        uint256 _discountTierB = ERC1155(discountAddress).balanceOf(msg.sender, 2);

    if (msg.sender != owner() ){
        require(saleStarted, "Sale has not started.");
        if (_discountTierB > 0) {
            require(
                msg.value >= tokenPrice.sub(discountOnMint[2]),
                "Not enough ETH sent."
            );
        } else if (_discountTierA > 0) {
            require(
                msg.value >= tokenPrice.sub(discountOnMint[1]),
                "Not enough ETH sent."
            );
        } else {
     ...
     */
    function renewToken(uint256 _tokenId)
        public
        payable
        nonReentrant
    ...
    {
        if (msg.sender != owner()) {
            uint256 _discountTierA = ERC1155(discountAddress).balanceOf(
                msg.sender,
                1
            );
            uint256 _discountTierB = ERC1155(discountAddress).balanceOf(
                msg.sender,
                2
            );
            ...
            if (_discountTierB > 0) {
                require(
                    msg.value >= tokenPrice.sub(discountOnRenewal[2]),
                    "Not enough ETH sent."
                );
            } else if (_discountTierA > 0) {
                require(
                    msg.value >= tokenPrice.sub(discountOnRenewal[1]),
                    "Not enough ETH sent."
                );
            } else {
                require(msg.value >= renewalPrice, "Not enough ETH sent.");
            }
        }
     ...
     * only callable by the owner.
     *
     * @param _updatedRenewalPrice The new token price in units of wei. E.g.
     * 500000000000000000 is 0.50 Ether.
     */
    function updateRenewalPrice(uint256 _updatedRenewalPrice)
        external
        onlyOwner
    {
        require(renewalPrice != _updatedRenewalPrice, "Price has not changed.");
        renewalPrice = _updatedRenewalPrice;
    }
    ...
    function authenticateUser(uint256 _tokenId) public view returns (bool) {
        require(_exists(_tokenId), "Token does not exist.");
        if (!renewalsPaused){
            if(block.timestamp > expiryTime[_tokenId]){
                return false;
            }

            return msg.sender == ownerOf(_tokenId) || msg.sender == owner () ? true : false;
        }
    ...
    /**
     * @notice Function that is used to flip the renewal state of the contract,
     * only callable by the owner.
     */
    function toggleRenewals() public onlyOwner {
        renewalsPaused = !renewalsPaused;
    }
    ...
    /**
     * @notice Function that is used to set the contract used for discounts.
     */
    function setDiscountAddress(address _discountAddress) public onlyOwner {
        discountAddress = _discountAddress;
    }

    /**
     * @notice Function that is used to set the discount rates on mint.
     */
    function setDiscountOnMint(uint256 _id, uint256 _discount)
        public
        onlyOwner
    {
        require(tokenPrice > _discount, "Attempting to pay users to mint.");
        discountOnMint[_id] = _discount;
    }

    /**
     * @notice Function that is used to set the discount rates on renewal.
     */
    function setDiscountOnRenewal(uint256 _id, uint256 _discount)
        public
        onlyOwner
    {
        require(renewalPrice > _discount, "Attempting to pay users to renew.");
        discountOnRenewal[_id] = _discount;
    }
}