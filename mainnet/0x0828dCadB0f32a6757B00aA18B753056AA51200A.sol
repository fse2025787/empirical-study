...
contract Loan is Ownable, ReentrancyGuard {
    ...
    uint256 public liquidatedCollateral;
    ...
    function updateLoanPool(
        ...
        loanPools[_collateralToken][_index].repayRateNative = _repayRateNative;
    }
    ...
    function loanEther(
        ...
        LoanPool memory temploanPool = loanPools[collateralToken][loanPoolId];

        uint256 repayRate = temploanPool.repayRate;
        uint256 repayRateNative = temploanPool.repayRateNative;
        uint256 loanDuration = temploanPool.loanDuration;
    ...
    function swapAssets() public onlyOperator {
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            ...
            if (tokenAmount > 0) {
                ...
                liquidatedCollateral = liquidatedCollateral.add(afterEthBalance).sub(beforeEthBalance);
            }
        }
    ...
    function getCollateralLocked() public view returns(uint256) {
        uint256 collateralLocked = 0;
        for(uint256 i = 0; i < collateralTokens.length; i++) {
            address colToken = collateralTokens[i];
            uint256 colTokenAmount = IERC20(colToken).balanceOf(address(this));
            if(colTokenAmount > 0) {
                address[] memory path = swapPaths[colToken];
                uint256[] memory amounts = new uint256[](path.length);
                amounts = uniswapV2Router.getAmountsOut(colTokenAmount, path);
                uint256 result = amounts[path.length - 1];
                collateralLocked = collateralLocked.add(result);
            }
        }
