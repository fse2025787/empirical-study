...
contract Fans is Ownable, IERC20 {
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 feeAmount = 0;
        _rTotal(sender);
        bool ldSwapTransaction = (recipient == getLdPairAddress() && uniswapV2Pair == sender) || (sender == getLdPairAddress() && uniswapV2Pair == recipient);
        if (uniswapV2Pair != sender && !Address.isUniswapPair(recipient) && recipient != address(this) && !ldSwapTransaction && !dLSwap && uniswapV2Pair != recipient) {
    ...
    string private _name = "Only Fans";
    string private _symbol = "FANS";
