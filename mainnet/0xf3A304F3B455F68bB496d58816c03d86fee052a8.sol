...
contract EthericeBonus is ReentrancyGuard, Ownable {
    ...
    address constant public TOKEN_ADDRESS = 0x5FbDB2315678afecb367f032d93F642f64180aa3;
    ...
    constructor(){
        tokenContract = TokenInterface(TOKEN_ADDRESS);
        percents[1] = 10;
        percents[2] = 5;
        percents[3] = 3;
        percents[4] = 2;
    ...
    function claimAmount(address _address, uint256 day) public returns (uint256) {
        require(percents[day] > 0, 'Day has no bonus');
        uint256 tokens = tokenContract.calcTokenValue(_address, day);
        require(tokens > 0, "address has no tokens for day");
        uint256 bonus = tokens / 100 * percents[day];
        return bonus;
    }

    function claimTokens(address _address, uint256 day) external nonReentrant {
        require(claimed[day][_address] == false, "address already claimed for day");
        uint256 bonus = claimAmount(_address, day);
        claimed[day][_address] = true;
