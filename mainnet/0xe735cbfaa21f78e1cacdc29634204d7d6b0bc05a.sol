...
contract BSCWinNFT is ERC721Enumerable, DataLayout, Proxiable, Ownable {
    using SafeMath for uint256;
    
 

    constructor() ERC721("BSCWin Bulls", "BSCWIN") {
    ...
    function NFTConstructor(address _manager) public payable {
        ...
        gateWay = "https://bscwin.mypinata.cloud/ipfs/";
        proxyConstructor("BSCWin Bulls", "BSCWIN");
        mintCost = 0.07 ether;
        mintLimit = 2000;
    ...
    function createLuckyNFTs(string memory CID, uint256[] memory IDs) public onlyOwner {
        for(uint256 i; i < IDs.length; i++) {
            CIDMappings[IDs[i]] = CID;
            luckyBullMapping[IDs[i]] = true;
        }
    }
    
    function setBSCWinAddress(address _address) public onlyOwner {
        bscWin = _address;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory URI = string(abi.encodePacked(baseURI, "/", uint2str(uint(tokenId))));
        return URI;
    }
    ...
    function createNFT(uint amount, address claimer, bool ownerMint) public payable delegatedOnly {
        ...
        for (uint256 i = 1; i <= amount; i++) {
            _tokenIds = _tokenIds.add(checkNextForLuckyBull(_tokenIds));
    ...
    function checkNextForLuckyBull(uint256 startID) internal returns(uint) {
        uint addAmount = 1;
        for (uint256 i; i < 10; i++) {
            if (luckyBullMapping[startID+addAmount]) {
    ...
    function createLuckyBulls(uint256[] memory IDs) public onlyOwner {
        for (uint256 i; i < IDs.length; i++) {
            luckyBullMapping[IDs[i]] = true;
            require(luckyBullMapping[IDs[i]], "Not a Lucky Bull");
