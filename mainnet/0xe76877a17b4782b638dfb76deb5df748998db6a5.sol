...
contract SuperMatchstickM is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor(address payable _owner) ERC721("Super Matchstick M", "SMM") {
        require(_owner != address(0));
        contract_owner = _owner;

        saleConfig.max_num = 5;
        saleConfig.SMM_total = 4500;
        saleConfig.startTime = 1658192400;
        saleConfig.endTime = 1658365200;
        saleConfig._price = 1*10**15;
    }
    ...
    function mintBatch(address account, uint256 num) external onlyOwner() {
        ...
        require(totalSupply() + num <= saleConfig.SMM_total, "Purchase would exceed max supply of tokens");

        for(uint i = 0; i < num; i++) 
        {
            uint mintIndex = totalSupply();
            if (totalSupply() < saleConfig.SMM_total) 
            {
