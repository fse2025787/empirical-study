...
contract Eggspect is ERC721  {
    ...
    constructor()
        ERC721("Eggspect Pump", "Egg")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 3210) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeics265vsjlbssagc6gwr74v6gxsx4ckmfnpf3ekvthlqxezhevvkm/", Strings.toString(tokenId), ".json")
            );
    }
