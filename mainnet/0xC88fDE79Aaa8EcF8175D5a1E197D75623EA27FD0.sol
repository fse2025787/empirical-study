...
contract RedlionArtdrops is Context, AccessControlEnumerable, Ownable, ERC2981, ERC721, ERC721Enumerable {
    ...
    mapping (address => BitMaps.BitMap) private _claimedArtdrops;
    ...
    function claim(uint[] calldata tokenIds) public {
        for(uint i = 0; i < tokenIds.length; i++) {
            ...
            require(!_claimedArtdrops[msg.sender].get(tokenId), "CLAIM:ALREADY CLAIMED ISSUE");
            require(bytes(artdropToIPFS[gazette.tokenIdToIssue(tokenId)]).length > 0, "CLAIM:ISSUE NOT ARTDROPPED YET");
            require(gazette.ownerOf(tokenId) == msg.sender, "CLAIM:NOT TOKEN OWNER");
            _claimedArtdrops[msg.sender].set(tokenId);
    ...
    function isClaimed(address user, uint issue) public view returns (bool) {
        return _claimedArtdrops[user].get(issue);
    }
