        ...
        if (msg.sender != owner) {
            if(!whitelisted[msg.sender]) {
                if(passContract != address(0)) {
                    uint256 balance = IERC721(passContract).balanceOf(msg.sender);
                    require(balance > 0, "Only whitelisted or pass holders can mint");
                } else {
...
// File: contracts/PRJCTBatchFactory.sol
...
contract PRJCTBatchFactory is Ownable {
    using Counters for Counters.Counter;
    ...
    function createCollection(string memory _name, string memory _symbol, string memory _contractURI, address[] memory whitelistAddresses, string memory _initialBaseURI, address _passContract) public returns (address) {
        if (msg.sender != owner()) {
            require(
                whitelisted[msg.sender],
                "Only whitelisted addresses can create collections"
            );
        }
        PRJCTNFT newCol = new PRJCTNFT(_name, _symbol, msg.sender, _contractURI, whitelistAddresses, _initialBaseURI, _passContract);
