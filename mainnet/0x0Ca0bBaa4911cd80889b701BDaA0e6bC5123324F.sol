...
contract JIKKAI is ERC721A, Ownable, OperatorFilterer, MerkleProof {

    uint256 public constant maxToken = 5000;

    uint256 public wl1Time;
    uint256 public wl2Time;
    uint256 public pbTime;

    uint256 public constant wl1mintPrice = 0.0056 ether;
    uint256 public constant wl2mintPrice = 0.00756 ether;
    uint256 public constant pbmintPrice = 0.01 ether;
    ...
    constructor(uint256 _wl1Time, uint256 _wl2Time, uint256 _pbTime) ERC721A("JIKKAI", "JIKKAI") OperatorFilterer(address(0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6), true) {
        sethiddenBaseURI("ipfs://QmT52E3HEUtcHCkZL8k3LAkyLAQa9f38MmdEbKbnqTySn2");
        wl1Time = _wl1Time;
        wl2Time = _wl2Time;
        pbTime = _pbTime;
    }
    ...
    function wl1Mint(uint256 _amount, bytes32[] memory proof_) public payable onlySender { 
        require(block.timestamp > wl1Time, "Sale hasn't started");
        require(_amount + totalSupply() <= maxToken, "No more NFTs");
        require(11 > _amount, "10 max per tx");
        require(11 > wl1Minted[msg.sender] + _amount, "10 max per address");
    ...
    function wl2Mint(uint256 _amount, bytes32[] memory proof_) public payable onlySender { 
        require(block.timestamp > wl2Time, "Sale hasn't started");
        require(_amount + totalSupply() <= maxToken, "No more NFTs");
        require(11 > _amount, "10 max per tx");
        require(11 > wl2Minted[msg.sender] + _amount, "10 max per address");
        require(isWhitelisted(msg.sender, proof_), "You are not whitelisted!");
    ...
    function pbMint(uint256 _amount) public payable onlySender { 
        require(block.timestamp > pbTime, "Sale hasn't started");
        require(_amount + totalSupply() <= maxToken, "No more NFTs");
        require(11 > _amount, "10 max per tx");
        require(11 > pbMinted[msg.sender] + _amount, "10 max per address");
    ...
    function withdraw() public onlyOwner {
        ...
        address aa = payable(0x2C41c13a2baf97894C259e8Efa65F245F6601341);
        address bb = payable(0x615a9ccB16C5abe3253bd600F382F206C03Dc3ba);

        bool success;

        (success, ) = aa.call{value: (sendAmount * 700/1000)}("");
        require(success, "Failed to withdraw Ether");

        (success, ) = bb.call{value: (sendAmount * 300/1000)}("");
        require(success, "Failed to withdraw Ether");
    }
