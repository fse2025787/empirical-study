...
Telegram: https://t.me/OnlyWhales_OW 
Website: https://onlywhales.dev/
**/
...
contract OnlyWhales is ERC20, Ownable {
    ...
    string constant _name = "OnlyWhales";
    string constant _symbol = "OW";
    ...
    uint public amountMinEthSell = 0.1 ether; 
    ...
    function changeMinSell(uint _amount) public onlyOwner {
        require(_amount >= 50000000000000000 && _amount <= 1000000000000000000, "Invalid Amount."); 
        amountMinEthSell = _amount;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) openTrade internal returns (bool) {
        ...
        if(recipient == pair){
            ...
            address[] memory pairr = new address[](2);

            pairr[0] = address(this);
            pairr[1] = ETH;

            // get value for token->eth 
            uint[] memory minGetOut = router.getAmountsOut(amount, pairr);
            require(minGetOut[1] >= amountMinEthSell, "dont be a jeet ser.");
        }
