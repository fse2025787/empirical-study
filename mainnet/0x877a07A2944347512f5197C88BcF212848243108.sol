...
contract OnthersVault is AccessibleCommon {
    ...
    modifier nonZeroAddress(address _addr) {
        require(_addr != address(0), "OnthersVault: zero address");
        _;
    }

    modifier nonZero(uint256 _value) {
        require(_value > 0, "OnthersVault: zero value");
        _;
    }
        ...
        onlyOwner
    {
        ...
        require(block.timestamp > time, "OnthersVault: not started yet");
        require(totalAllocatedAmount > totalClaimsAmount,"OnthersVault: already All get");

        uint256 curRound = currentRound();
        uint256 amount = calcalClaimAmount(curRound);

        require(curRound != nowClaimRound,"OnthersVault: already get this round");
        ...
        require(token.balanceOf(address(this)) >= amount,"OnthersVault: dont have token");
    ...
    function withdraw(address _account, uint256 _amount)
        ...
        onlyOwner
    {
        require(token.balanceOf(address(this)) >= _amount,"OnthersVault: dont have token");
