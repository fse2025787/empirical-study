...
contract MenaceWarriors is Ownable, ERC721A, PaymentSplitter {
    ...
    mapping(address => uint) public mintedAmountNFTsperWalletVIPSale;

    uint public maxMintAmountPerVIP = 1;
    uint public maxMintAmountPerWhitelist = 3; 

    uint public maxAmountPerTxnPublic = 3; 

    uint private teamLength;

    constructor(address[] memory _team, uint[] memory _teamShares, address _signerAddressVIP, address _signerAddressWL, string memory _baseURI) ERC721A("Menace Warriors", "MenaceWarriors")
    ...
    function publicSaleMint(address _account, uint _quantity) external payable {
        ...
        if(_quantity > maxAmountPerTxnPublic) revert("Max amount per txn is 3");

        if(sellingStep != Step.PublicSale) revert("Public Mint not live.");
        if(totalSupply() + _quantity > (MAX_SUPPLY - MAX_VIP)) revert("Max supply exceeded for public exceeded");
