...
// File: SoundBound_ERC721A_flat.sol
...
contract SoundBound is ERC721A, Ownable, AccessControl {
    ...
    event LogNFTCreatedForDomain(address indexed ownerWalletAddress, string name, uint32 genre);
    event LogNFTCreatedForSubdomain(address indexed ownerWalletAddress, string subdomain, uint32 genre);

    bool public paused = false;

    mapping(string => bool) private purchaseStatus;

    string public kolaSubdomainSuffix = ".kolalabs";
    ...
    function subdomainCheck(string memory subdomain) internal view returns (bool) {
        bytes memory subdomain_bytes = bytes(subdomain);
        for(uint i; i<subdomain_bytes.length; i++){
            if (subdomain_bytes[i] == 0x2E) {
                return true;
            }
        }
        return false;
    ...
    function mintNFTForDomain(string memory name, uint32 genreId) external payable {
        ...
        emit LogNFTCreatedForDomain(msg.sender, name, genreId);
    }

    function mintNFTForSubdomain(string memory subdomain, uint32 genreId) external payable {
        require(!paused, "The contract has been paused");

        require(genreIdIsIn[genreId], "Genre id is invalid.");

        if (isWhitelistPeriod) {
            require(isWhitelisted(msg.sender), "Sorry, you are not being whitelisted; please mint during the public sale.");
        }
        require(!subdomainCheck(subdomain), "Please don't include the character '.' in the subdomain.");

        string memory name = string(abi.encodePacked(subdomain, kolaSubdomainSuffix));

        require(purchaseStatus[name] == false, "This NFT has been purchased.");

        _mint(msg.sender, 1);
        purchaseStatus[name] = true;
        emit LogNFTCreatedForSubdomain(msg.sender, subdomain, genreId);
    }

    function mintNFTByAdmin(string[] memory domainMap, address[] memory addressMap, uint32 genreId) external onlyMember {
        require(!paused, "The contract has been paused");
        require(genreIdIsIn[genreId], "Genre id is invalid.");
        for (uint i=0; i<domainMap.length; i++) {
            string memory domainName = domainMap[i];
            address userAddress = addressMap[i];
            if (subdomainCheck(domainName)) {
                continue;
            }

            string memory name = string(abi.encodePacked(domainName, kolaSubdomainSuffix));
            if(purchaseStatus[name] == true) {
                continue;
            }

            _mint(userAddress, 1);
            purchaseStatus[name] = true;
            emit LogNFTCreatedForSubdomain(msg.sender, domainName, genreId);
        }
    }
    ...
    function setWhitelistPeriod(bool state) public onlyOwner {
