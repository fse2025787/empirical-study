...
contract YPToken is ERC20, ERC20Burnable, Pausable, Ownable, ERC20Permit, ERC20Votes {
    constructor() ERC20("YPToken", "YCHT") ERC20Permit("YPToken") {
...
contract YachtStake is Ownable, IERC721Receiver {
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(ERC721Enumerable _nft, YPToken _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      rewardcalc = 500 ether * (block.timestamp - stakedAt) / 86400 ;
      earned = rewardcalc / 500;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
        timestamp: uint48(block.timestamp)
