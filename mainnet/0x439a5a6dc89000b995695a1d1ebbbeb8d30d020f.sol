...
contract FusionNFT is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string private baseURI = "https://infura-ipfs.io/ipfs/QmNsAd4JDxbZkBaWp6o24mkkubXbWf5ALeg6VacV8AGLAA";
    ...
    uint256 public constant NUM_FREE_MINTS = 333;
    ...
    modifier canMintNFTs(uint256 numberOfTokens) {
        require(totalSupply() + numberOfTokens <= maxSupply, "Not enough mints remaining to mint");
        _;
    }

    modifier freeMintsAvailable() {
        require(totalSupply() <= NUM_FREE_MINTS, "Not enough free mints remain");
        _;
    }
    ...
    constructor() ERC721A("Fusion Tiger", "FusTiger") {}
    ...
    function devMint(uint256 numberOfTokens) external onlyOwner nonReentrant {
        require(numberOfTokens <= 20, "max limit");
        require(devMinted == false, "dev minted");
        require(totalSupply() + numberOfTokens <= maxSupply, "Not enough mints remaining to mint");
        _safeMint(msg.sender, numberOfTokens);
        devMinted = true;
    }
