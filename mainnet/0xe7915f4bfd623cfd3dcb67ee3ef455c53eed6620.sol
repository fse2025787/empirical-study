...
contract NFTVz is Ownable, ERC721A {
    ...
    string public baseURI = "ipfs://bafybeighd5vlaz4v5lnvtkrehjqxvj2vmpq7lh2aifb6yq2a3ellazothm/";
    string public _extension = ".json";
    uint256 public price = 0 ether;
    uint256 public maxSupply = 4444;

    constructor() ERC721A("NFTVz", "NFTV"){}
    
    function mintNFT(uint256 _quantity) public payable {
        require(_quantity > 0 && _quantity <= adoptionLimit, "Wallet full, check max per wallet!");
        require(totalSupply() + _quantity <= maxSupply, "Reached max supply!");
        require(msg.value == price * _quantity, "Needs to send more ETH!");
        require(getMintedCount(msg.sender) + _quantity <= adoptionLimit, "Exceeded max minting amount!");
