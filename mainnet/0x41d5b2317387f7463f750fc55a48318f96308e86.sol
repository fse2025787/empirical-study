...
contract MutantZuki is ERC721, ERC721Enumerable, Ownable {
    ...
    modifier onlyAzukiOwner() {
        uint256 azukiBalance = IERC721Enumerable(CONTRACT_ADDRESS).balanceOf(
            msg.sender
        );
        require(
            azukiBalance > 0,
            "You must own an Azuki to be able to call this function."
        );
        _;
    }
    ...
    function setTotalSupply(uint256 totalSupply_) external onlyOwner {
        require(
            ...
            "The total number of mutantzukis cannot exceed 10,000"
        );
        require(
            TOTAL_SUPPLY <= totalSupply_,
            "You can only increase the total supply."
        );
        TOTAL_SUPPLY = totalSupply_;
    }
        ...
        returns (uint256)
    {
        ...
        for (
            uint256 i = 0;
            i < IERC721Enumerable(CONTRACT_ADDRESS).balanceOf(addr);
            i++
        ) {
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(
            mintingFee * numberOfTokens <= msg.value,
