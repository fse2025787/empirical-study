...
// 
pragma solidity >=0.5.15 >=0.7.6 >=0.6.0 <0.8.0 >=0.7.0 <0.8.0;
...
contract Shelf is Auth, TitleOwned, Math {
    ...
    function depend(bytes32 contractName, address addr) external auth {
        if (contractName == "token") { currency = TokenLike_1(addr); }
        else if (contractName == "title") { title = TitleLike_1(addr); }
        else if (contractName == "pile") { pile = PileLike_2(addr); }
