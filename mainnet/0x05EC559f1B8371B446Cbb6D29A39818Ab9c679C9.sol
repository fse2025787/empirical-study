...
contract nLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    
    address payable payowner = payable(address(0xB97C9B5DE2870ffa00f0B0a9d0A236C03a047f3A));

    // claim times
    mapping(address => uint256) public _claimTimes;
    
    //global token id
    uint256 _globalTokenId = 1;
    ...
    function claim() public payable nonReentrant {
        require (msg.value >= 0.01 ether, "mint fees not enough");
        payowner.transfer(msg.value);
        require(_globalTokenId < 10001, "Token ID invalid");
    ...
    constructor() ERC721("nloot", "nloot") Ownable() {}
}
