...
// File: contracts/StabinolStaker.sol
...
contract StabinolStaker is Ownable, ReentrancyGuard {
    ...
    function getSTOLInLP(address _user) external view returns (uint256){
        if(IERC20(lpAddress).totalSupply() == 0) { return 0; }
        uint256 stolInLP = IERC20(stolAddress).balanceOf(lpAddress);
        return stolInLP.mul(allUsersInfo[_user].lpAmount).div(IERC20(lpAddress).totalSupply());
    }

    function getLPBalance(address _user) external view returns (uint256){
        return allUsersInfo[_user].lpAmount;
    }
    
    function getSTBZBalance(address _user) external view returns (uint256){
        return allUsersInfo[_user].stbzAmount;
    }
    
    function getDepositTime(address _user) external view returns (uint256){
        return allUsersInfo[_user].lastDeposit;
    }
    
    function getWithdrawTime(address _user) external view returns (uint256){
        return allUsersInfo[_user].lastWithdraw;
    }
    ...
    function updateMyBalance() external nonReentrant
    {
        ...
        allUsersInfo[_msgSender()].lastDeposit = now; // Reset the claim time
    ...
    function stakeSTBZ() external nonReentrant
    {
        ...
        require(allUsersInfo[_msgSender()].stbzAmount >= min, "Not enough tokens staked");
    ...
    function stakeSTOLLP(uint256 amount) external nonReentrant
    {
        ...
        IERC20 _token = IERC20(lpAddress); // Trusted token
        uint256 _before = _token.balanceOf(address(this));
        _token.safeTransferFrom(_msgSender(), address(this), amount); // Transfer token to this address
        amount = _token.balanceOf(address(this)).sub(_before);
        
        totalLP = totalLP.add(amount);
        allUsersInfo[_msgSender()].lpAmount = allUsersInfo[_msgSender()].lpAmount.add(amount);
        emit StakedLP(_msgSender(), amount);
    }
    ...
    modifier timelockConditionsMet(uint256 _type) {
        ...
        if(claimerAddress != address(0)){
            ...
            require(now >= _timelockStart + TIMELOCK_DURATION + minLPStakeTime, "Timelock time not met");
        }
        _;
    }
    ...
    function finishClaimerChange() external onlyGovernance timelockConditionsMet(2) {
        claimerAddress = _timelock_address;
    }
    // --------------------
