...
contract Soundtrack is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    struct Art {
  		mapping(uint256 => string) alt;
  	}
    Art art;

    string[] private prefixes = [
      "Symphony", "Suite", "Theme and Variations", "Rondo", "Requiem", "Fantasy", "Rhapsody", "Sonata",
      "Concerto", "Overture", "Prelude","Fugue", "Intermezzo", "Interlude", "Finale", "Nocturne", "Impromptu", "Songs", "Divertimento", "Ballade", "Berceuse", "Elegy",
      "A Polish Requiem", "A German Requiem", "Rondo alla Turca", "Spanish Overture", "Romanian Rhapsody", "Romanian Folk Dances",
      "Chinese Dance", "Hungarian Rhapsodies", "Balkan Miniatures", "Italian Symphony", "Serenata Espanola"
    ];
    ...
    string[] private pitch = ["A","B","C","D","E","F","G"];
    string[] private alt = ['','#','b'];
    ...
    function getScores(uint256 tokenId) public view returns (Detail memory detail) {

      Detail memory newDetail = Detail({
          title:pluck2(tokenId, "TITLE"),
          pitch:pluck(tokenId, "PITCH", pitch),
          index: pluck4(tokenId, "ALT",alt),
          alt: pluck(tokenId, "ALT",alt),
          mode: pluck(tokenId, "MODE", mode),
          genre:pluck(tokenId, "GENRE", genre),
          meter:pluck(tokenId, "METER", meter),
          instrument:pluck3(tokenId, "INSTRUMENT"),
          tempo:pluck(tokenId, "TEMPO",tempo),
    ...
    function pluck3(uint256 tokenId, string memory keyPrefix) internal view returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
            string[2] memory name;
            name[0] = stamp[rand % stamp.length];
            name[1] = instrument[rand % instrument.length];
            string memory output = string(abi.encodePacked( name[0], ' ', name[1]));
        return output;
    }
    function pluck4(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal pure returns (uint256) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        return rand % sourceArray.length;
    }

    function tokenURI(uint256 tokenId) override public view returns (string memory) {

      Detail memory score = getScores(tokenId);
      ...
      string memory output = string(abi.encodePacked(
        ...
        '"',score.title,'"',
        '</text><text text-anchor="middle" x="175" y="140" class="base">in ',
        score.pitch,
        art.alt[score.index],
        '<text text-anchor="middle" x="175" y="160" class="base">',
        score.mode,
        '</text><text text-anchor="middle" x="175" y="180" class="base">',
        score.genre,
        '</text><text text-anchor="middle" x="175" y="200" class="base">',
        score.meter,
        '</text><text text-anchor="middle" x="175" y="220" class="base">',
        score.instrument,
        '</text><text text-anchor="middle" x="175" y="240" class="base">',
        score.tempo,
        '</text></svg>'
      ));

      string memory attributes;
      string[8] memory artItems = [
      score.title,
      score.pitch,
      score.alt,
      score.mode,
      ...
      string[8] memory traitType = ["Title", "Pitch", "Alt", "Mode", "Genre", "Meter", "Tempo", "Instrument"];
      ...
      string memory json = Base64.encode(bytes(
        string(abi.encodePacked(
          '{',
          '"name": "Soundtrack #', toString(tokenId), '",',
          '"description": "Soundtrack is randomized adventurer music generated and stored on chain. Stats, notes, and other functionality are intentionally omitted for others to interpret. Feel free to use Soundtrack in any way you want.",',
    ...
    function claim(uint256 tokenId) public payable nonReentrant {
        ...
        require(msg.value>=price, "Must send minimum value to purchase!");

        _detail[tokenId] = getScores(tokenId);
    ...
    function donate() public payable {
      payable(0x1F9d0F0B739c8809785EFA70Ac5D14E85c299722).transfer(address(this).balance);
    }
