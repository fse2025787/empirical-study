...
contract boughtIdentity is ERC721Enumerable, ReentrancyGuard, Ownable {

    uint BASE_CREDIT_VALUE = 250; // Multiply base credit value by probability of each classes spawn rate and their multiplier then add all of those values to get ~900k. We then 25 from this value and rand % 49 to get a range
    ...
    function getCredits(uint256 tokenId) external view returns (string memory)
    {
        ...
        boughtIdentity dataContract = boughtIdentity(regularMint);
    ...
    function getIntelligence(uint256 tokenId) public view returns (string memory) {
        ...
        boughtIdentity dataContract = boughtIdentity(regularMint);
        output = dataContract.getIntelligence(tokenId);
    ...
    function getGender(uint256 tokenId) public view returns (string memory) {
        ...
        boughtIdentity dataContract = boughtIdentity(regularMint);
        output = dataContract.getGender(tokenId);
    ...
    function claimIdentity(address _to) public nonReentrant {
        require(msg.sender == citizenContract, "Must be citizen contract");
        _safeMint(_to, totalSupply() + startingIndex);
    }

    function setMintContract(address contractAddress) public onlyOwner {
        regularMint = contractAddress;
    }  

    function setCitizenContract(address contractAddress) public onlyOwner {
        citizenContract = contractAddress;
    }
    ...
    constructor() ERC721("Neo Tokyo: Bought Identities", "NEOTBI") Ownable() {
        regularMint = 0x5804ae06600951778CbEA8b7065011272F2f3023;
        startingIndex = 2501;
    }
}