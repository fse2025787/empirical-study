...
contract GranChacoImpactNFT is ERC721Enumerable, Ownable {
    ...
    constructor(string memory tier1Uri, string memory tier2Uri, string memory tier3Uri, string memory tierXUri) ERC721("Gran Chaco Impact", "GCI") {
        COST_WALLET = 0x317C0ba97678876711c254b6F156887B3ECE3040;
        COMMUNITY_WALLET = 0xF5A884fad853488742E0A4A2dcEDAad46BF57cE9;
        NATIVE_ARTISTS_WALLET = 0xE5d7111E49A55798713e2Ccd152B6F2359E0f92F;
        CAMINNOS_WALLET = 0xf22AB6d47EE060fC2ecD3E35072968692c3456d4;
        NFT_ARTISTS_WALLET = [
            0xae38ed000577F9E4f8A9A8521D5ABdaf74Ca07De,
            0x0833A7744bCc0f5cceDF09dEC9724df0E73A26A9,
            0x18272E9166f2295fEeEc29090BA5c393FF9365C5,
            0x7C8b4bbDcD39E75Cc32F493eA3aB548D833C5E43,
            0x5B84b9d45340A3311423551BF3243bBd2f07b4e0,
            0xD4B23220b20cDc6622F0F3FFeFC12E8826E412Fb
            ];
        _tier1BaseURI = tier1Uri;
        _tier2BaseURI = tier2Uri;
        _tier3BaseURI = tier3Uri;
        _tierXBaseURI = tierXUri;
    }

    function setMaxLimit(uint256 oneofoneSupply, uint256 commonSupply) external onlyOwner {
        ONEOFONE_SUPPLY = oneofoneSupply;
        COMMON_SUPPLY = commonSupply;
    }
    ...
    function setCostPercent(uint256 tier1Percent, uint256 tier2Percent, uint256 tier3Percent, uint256 tierXPercent) external onlyOwner {
        TIER1_COST_PERCENT = tier1Percent;
        TIER2_COST_PERCENT = tier2Percent;
        TIER3_COST_PERCENT = tier3Percent;
        TIERX_COST_PERCENT = tierXPercent;
    }
    ...
    function setEarnPercent(uint256 communityPercent, uint256 nativeArtistsPercent, uint256 nftArtistsPercent, uint256 caminnosPercent) external onlyOwner {
        COMMUNITY_EARN_PERCENT = communityPercent;
        NATIVE_ARTISTS_EARN_PERCENT = nativeArtistsPercent;
        NFT_ARTISTS_EARN_PERCENT = nftArtistsPercent;
        CAMINNOS_EARN_PERCENT = caminnosPercent;
    }
    ...
    function mintOneOfOne(address to, uint256 tokenId, uint256 tierNumber) external payable {
        ...
        if (tierNumber == 1) {
            require(TIER1_PRICE <= msg.value,"ETH amount is not sufficient");
            (bool os, ) = payable(COST_WALLET).call{value: msg.value * TIER1_COST_PERCENT / 10000}('');
            require(os);
        } else if (tierNumber == 2) {
            require(TIER2_PRICE <= msg.value,"ETH amount is not sufficient");
            (bool os, ) = payable(COST_WALLET).call{value: msg.value * TIER2_COST_PERCENT / 10000}('');
            require(os);
        } else if (tierNumber == 3) {
            require(TIER3_PRICE <= msg.value,"ETH amount is not sufficient");
            (bool os, ) = payable(COST_WALLET).call{value: msg.value * TIER3_COST_PERCENT / 10000}('');
            require(os);
        } 
    ...
    function mintCommon(address to, uint256 tokenAmount) external payable {
        ...
        require(TIERX_PRICE * tokenAmount <= msg.value,"ETH amount is not sufficient");
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (MAP_TIER[tokenId] == 1) {
            _tokenBaseURI = _tier1BaseURI;
        } else if (MAP_TIER[tokenId] == 2) {
            _tokenBaseURI = _tier2BaseURI;
        } else if (MAP_TIER[tokenId] == 3) {
            _tokenBaseURI = _tier3BaseURI;
        } else if (MAP_TIER[tokenId] == 4) {
            _tokenBaseURI = _tierXBaseURI;
        }
