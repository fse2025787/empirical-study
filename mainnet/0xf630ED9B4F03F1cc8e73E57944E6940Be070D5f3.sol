...
contract Tooblue is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(address _feeReceiver, uint256 starttime)
        ERC721("Tooblue OG NFT PASS", "tooblue")
    {
    ...
    function removeStuckEth(uint256 _amount) external onlyOwner {
        payable(feeReceiver).transfer(_amount);
    }

    function freeNftPass(address account, bool _state) external onlyOwner {
        _isWhiteListed[account] = _state;
    }
    ...
    function changeStartTime(uint256 _start) public onlyOwner {
        startTime = _start;
    }
}