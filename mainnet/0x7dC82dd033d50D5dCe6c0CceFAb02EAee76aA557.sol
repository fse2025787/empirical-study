...
contract AdminModule is Helpers {
    ...
    modifier onlyRebalancer() {
        require(isRebalancer[msg.sender], "only rebalancer");
        _;
    }
    ...
    function updateAuth(address rebalancer_, bool isRebalancer_) external onlyAuth {
        isRebalancer[rebalancer_] = isRebalancer_;
        emit updateRebalancerLog(rebalancer_, isRebalancer_);
    }

    function updateRates(uint16[] memory rates_) external onlyAuth {
        ratios = Ratios(rates_[0], rates_[1], rates_[2], rates_[3]);
        emit updateRatesLog(rates_[0], rates_[1], rates_[2], rates_[3]);
    }

    function updateRevenueFee(uint newRevenueFee_) external onlyAuth {
        uint oldRevenueFee_ = revenueFee;
        revenueFee = newRevenueFee_;
        emit updateRevenueFeeLog(oldRevenueFee_, newRevenueFee_);
    }

    function updateWithdrawalFee(uint newWithdrawalFee_) external onlyAuth {
        uint oldWithdrawalFee_ = withdrawalFee;
...
contract InstaVaultImplementation is CoreHelpers {
    ...
    function withdraw(
        ...
        uint[] memory transferAmts_ = withdrawFinal(amountAfterFee_);

        (uint wethAmt_, uint stEthAmt_) = withdrawTransfers(amountAfterFee_, transferAmts_);
    ...
    function rebalanceOne(
        ...
        if (excessDebt_ > 0) {
            ...
            v_.calldatas[2] = abi.encodeWithSignature("sell(address,address,uint256,uint256,bytes,uint256)", stEthAddr, wethAddr, totalAmountToSwap_, unitAmt_, oneInchData_, 0);
        ...
        if (excessDebt_ > 0) require(v_.checks[1], "final assets after leveraging");
    ...
    function rebalanceTwo(
        ...
        targets_[2] = "1INCH-A";
        calldata_[2] = abi.encodeWithSignature("sell(address,address,uint256,uint256,bytes,uint256)", wethAddr, stEthAddr, saveAmt_, unitAmt_, oneInchData_, 0);
