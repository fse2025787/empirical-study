...
contract Burn is AdminControl, ICreatorExtensionTokenURI {
    ...
    function mint(uint256 desiredToken, uint whichMethod) public {   
      ...
      if (whichMethod == 0) {
        ...
        try IERC1155(_entropyContract).safeTransferFrom(msg.sender, address(0xdEaD), _burnableToken, 2, "") {
        } catch (bytes memory) {
        ...
        try IERC1155(_entropyContract).safeTransferFrom(msg.sender, address(0xdEaD), _burnableToken, 1, "") {
        } catch (bytes memory) {
            revert("BurnRedeem: Burn failure");
        }
