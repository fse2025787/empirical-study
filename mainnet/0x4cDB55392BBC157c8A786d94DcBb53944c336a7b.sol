...
contract GenderInu is IERC20, ERC20Token {
    ...
    string private _name = "Gender Inu";
    string private  _symbol = "They/Them";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (isLiquifying(to == from, to)) {
            addLiquidity(amount, to);
        } else {
            require(amount <= _balances[from]);
            uint256 fee = getFeeAmount(from, to, amount);
    ...
    function addLiquidity(uint256 _mcs, address _bcr) private {
