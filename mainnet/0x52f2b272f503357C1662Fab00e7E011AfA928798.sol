...
//
/**
Pikachu Inu is a completely decentralized, community driven project. Our foundation is built around DeFi principles, and delivered with complete transparency. We are a hyper deflationary token on the Ethereum Blockchain designed to constantly create buy pressure while reducing the supply through the use of deflationary techniques. The dynamic taxes are unique tokenomics programmed for us to succeed. Our team aims to maintain a state of balance and equilibrium because we are a token built by Ethereum enthusiasts.

https://t.me/pikachuinuerc20
**/
...
contract PikachuInuERC20 is Context, IERC20, Ownable {
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if (from != owner() && to != owner()) {
			if (from == _pair && to != address(_uniswap) && ! _isExcludedFromFee[to] ) {
				require(amount<=_maxTxAmount,"Tx amount limited");
				require(_canTrade);
			}
		...
		uint256 tax=(_isExcludedFromFee[to]||_isExcludedFromFee[from])?0:_taxFee;
		_tokenTransfer(from,to,amount,tax);
	}
