...
contract ETHBattleRoyale is ERC721AUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    /*constructor(string memory _baseUri, uint256 _maxSupply, uint32 _cooldownLength, uint32 _timeToHeal, uint32 _timeToShield) ERC721A("ETHBattleRoyale", "ETHBR") {
        _uriPart = _baseUri;
        MAX_SUPPLY = _maxSupply;
        cooldownLength = _cooldownLength;
        timeToHeal = _timeToHeal;
    ...
    function mint(uint256 quantity, bytes32[] calldata proof) external payable nonReentrant{
        ...
        uint256 CURRENT_MINT_PRICE;
        uint256 totalMinted = _totalMinted();
        if(totalMinted <= 2000){
            CURRENT_MINT_PRICE = 0 ether;
        }else if(totalMinted <= 3000){
            CURRENT_MINT_PRICE = 0.005 ether;
        }else if(totalMinted <= 4000){
            CURRENT_MINT_PRICE = 0.01 ether;
        }else{
            CURRENT_MINT_PRICE = 0.02 ether;
        }

        // Mint price check
        require(msg.value == (CURRENT_MINT_PRICE * quantity), "mint price not met");

        if(mintStage == 1){
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            require(MerkleProofUpgradeable.verify(proof, merkleRoot, leaf), "Invalid proof");
            require(_numberMinted(msg.sender) + quantity <= ALLOWLIST_MAX, "mint too many for allowlist");
        }
        ...
        // Now make sure we're allowed to steal their NFTs :)
    }
