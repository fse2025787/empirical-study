...
contract ArttsNFT2 is ERC721 {
    ...
    function createNFT(string memory _tokenURI, uint256 _price)
        ...
        returns (uint256)
    {
        require(_totalSupply < _maxSupply, "NFT max supply already minted");
        require(
            msg.sender == contractCreator,
            "Only contract creator can mint"
        );
    ...
    function priceOf(uint256 tokenId) public view returns (uint256) {
        require(
            _exists(tokenId),
            "Price query for nonexistent token"
        );
    ...
    function setPrice(uint256 tokenId, uint256 price) public {
        require(
            msg.sender == contractCreator,
            "Only contract creator can change price"
        );
