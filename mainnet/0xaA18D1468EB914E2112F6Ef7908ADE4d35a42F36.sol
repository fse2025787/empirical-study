...
contract SimpleERCFund is ISimpleERCFund, Ownable {
    ...
    function setDev(address[] calldata _dev, uint256[] calldata _points) public onlyOwner {
        require(_dev.length == _points.length, "invalid");
        dev = _dev;
        points = _points;
        uint256 _totalPoints = 0;
        for (uint256 i = 0; i < _points.length; i++) {
            _totalPoints = _totalPoints.add(_points[i]);
        }
    ...
    function deposit(
        ...
        if (totalPoints > 0) {
            for (uint256 i = 0; i < points.length; i++) {
                IERC20(token).transfer(dev[i], amount.mul(points[i]).div(totalPoints));
            }
        }
