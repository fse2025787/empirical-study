...
contract LottoPunk is ERC721Enumerable, Ownable {
    ...
    uint public constant MAX_LP = 10000;
	string _baseTokenURI;
    uint256 public constant LP_Price = 30000000000000000; // 0.03  ETH
    ...
    function mint(uint numberOfTokens) public payable saleIsOpen {
        require(!paused, "Sale is Paused");
        require(totalSupply() + numberOfTokens <= LP_Price, "Purchase would exceed max supply of Lotto Punks");
        require(totalSupply() < LP_Price, "Sale end");
        require(numberOfTokens <= 20, "Can only mint 20 tokens at a time");
        require(LP_Price.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
