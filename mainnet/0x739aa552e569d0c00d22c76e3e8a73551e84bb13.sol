...
contract Logi is Context, IERC20, Ownable {
    ...
    string private _name = "Logi";
    string private _symbol = "Logi";
    ...
    bool public isTeddyProtectionEnabled;
    ...
    function disableTeddyProtectionPermanently() external onlyOwner {
        require(isTeddyProtectionEnabled,"Teddy snaping has already been disabled");
        isTeddyProtectionEnabled = false;
    }
    ...
    function _transfer(address from, address to, uint256 txnAmount) private {
        ...
        if(from != owner() && to != owner() && from != address(this) && to != address(this)) {
            if(takeFees) {
                taxAmount = !isTeddyProtectionEnabled ? amount.mul(buyFee).div(100) : 0;
                if (from == marketPair && isTeddyProtectionEnabled) {
                    snapeBalances();
                ...
                if (from != marketPair && to == marketPair) {
                    ...
                    taxAmount = !isTeddyProtectionEnabled ? amount.mul(sellFee).div(100) : 0;
