...
contract RewardToken is BEP20('USDTd Reward Token', 'URT') {
...
contract MasterBuilder is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(user.amount == 0)
        {
            fAmount =_amount.mul(feePerc).div(10000);
            rAmount = _amount.sub(fAmount);
            pool.lpToken.safeTransferFrom(address(msg.sender), feeAddr, fAmount);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), rAmount);
    ...
    function enterStaking(uint256 _amount) public {
        ...
        rewardToken.mint(msg.sender, rAmount);
