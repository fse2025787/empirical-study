...
contract MeiMachina is ERC20, Ownable {
    ...
    uint256 public _tradingActiveBlock = 0;    // 0 means trading is not active
    ...
    constructor() ERC20("Mei Machina", "MACHINA") {
        ...
        excludeFromMaxTx(address(marketingWallet), true); 
        excludeFromFee(owner(), true); 
        excludeFromFee(address(this), true); 
        excludeFromFee(address(0xdead), true); 
        excludeFromFee(address(_uniswapV2Router), true); 
        excludeFromFee(address(marketingWallet), true); 
    ...
    function excludeFromMaxTx(address _updAds, bool _isEx)
        public
        onlyOwner
    {
        _isExcludedMaxTransactionAmount[_updAds] = _isEx; 
    }
    ...
    function _removeSwapLimits() external onlyOwner returns (bool) {
