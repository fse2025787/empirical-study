...
contract Felix is ERC20, Ownable {
    ...
    constructor() ERC20("Felix", "Felix") {
        ...
        TreasuryAddress = address(0x089F6bB6Bb862EbAc6a32D8f58FB6d021fcFC500);
        RewardsAddress = address(0x33d6Fa29a9F81195a0F5281B75EF8CCc24F2Cc14);
    ...
    function multiTransferCall(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
    ...
    function swapBack() private {
        ...
        uint256 ethForLiquidity = usdcBalance;

        uint256 ethForTreasury = usdcBalance * tokensForTreasury / (totalTokensToSwap - (tokensForLiquidity/2));
        uint256 ethForRewards = usdcBalance * tokensForRewards / (totalTokensToSwap - (tokensForLiquidity/2));
        ...
        if (liquidityTokens > 0 && usdcBalance > 0) {
            addLiquidity(tokensForLiquidity, usdcBalance);
        }

        IERC20(USDC).transfer(RewardsAddress, ethForRewards);
        IERC20(USDC).transfer(TreasuryAddress, IERC20(USDC).balanceOf(address(this)));
    }
    ...
    function tax_enabled(bool status) public onlyOwner {
       swapEnabled = status;
    }
