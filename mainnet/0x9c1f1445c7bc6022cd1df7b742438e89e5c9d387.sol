...
contract NWMintPass is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function burn(uint256 tokenId) public {
        require(_msgSender() == _burner, "burn error: invalid burner");
        _burn(tokenId);
    }

    function devMint(address[] memory holders) public onlyOwner {
        uint24 idx = uint24(holders.length);
        while(idx > 0) {
            idx--;
            totalSupply = totalSupply + 1;
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner_ = NWMintPass.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(NWMintPass.ownerOf(tokenId) == from, "ERC721: transfer from incorrect owner");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(NWMintPass.ownerOf(tokenId), to, tokenId);
    }
