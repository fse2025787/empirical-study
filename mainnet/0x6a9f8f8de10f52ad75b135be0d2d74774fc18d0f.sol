...
// Website: https://visualrealityai.com
// Telegram: https://t.me/visualrealityaierc
// 
...
contract VisualRealityAI is ERC20, Ownable {
    ...
    constructor(
        ...
        taxWallets = TaxWallets(_taxWallets[0], _taxWallets[1], _taxWallets[2]);

        buyTax = Taxes(_taxes[0][0], _taxes[0][1], _taxes[0][2]);
        sellTax = Taxes(_taxes[1][0], _taxes[1][1], _taxes[1][2]);
        launchTax = Taxes(_taxes[2][0], _taxes[2][1], _taxes[2][2]);

        totalTaxes = TotalTaxes(
            buyTax.marketing + buyTax.liquidity + buyTax.burn,
            sellTax.marketing + sellTax.liquidity + sellTax.burn,
            launchTax.marketing + launchTax.liquidity + launchTax.burn
        );
    ...
    function createPair() external onlyOwner {
        require(uniswapV2Pair == address(0), "The pair has been created.");
        _createPair();
    }

    function startTrading() external onlyOwner {
        require(tradingActive == false, "The trading has been started.");
        tradingActive = true;
    ...
    function updateSwapEnabled(bool _swapEnabled) external onlyOwner {
        swapEnabled = _swapEnabled;
    }

    function updateGasLimitActive(bool _gasLimitActive) external onlyOwner {
    ...
    function updateBuyTaxes(uint256 _marketing, uint256 _liquidity, uint256 _burn) external onlyOwner {
        buyTax = Taxes(_marketing, _liquidity, _burn);
        totalTaxes.buy = buyTax.marketing + buyTax.liquidity + buyTax.burn;
        require(totalTaxes.buy <= 5, "Must keep fees at 5% or less");
    }

    function updateSellTaxes(uint256 _marketing, uint256 _liquidity, uint256 _burn) external onlyOwner {
        sellTax = Taxes(_marketing, _liquidity, _burn);
        totalTaxes.sell = sellTax.marketing + sellTax.liquidity + sellTax.burn;
        require(totalTaxes.sell <= 5, "Must keep fees at 5% or less");
    }

    function setTaxWallets(address _marketingWallet, address _liquidityWallet, address _burnWallet) external onlyOwner {
        require(_marketingWallet != address(0), "Marketing Address cannot be zero address");
        require(_marketingWallet != deadAddress, "Marketing Address cannot be dead address");
        taxWallets = TaxWallets(_marketingWallet, _liquidityWallet, _burnWallet);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (takeFee) {
            if (
                ...
                fees = amount.mul(totalTaxes.launch).div(100);
                tokensForMarketing += (fees * launchTax.marketing) / totalTaxes.launch;
                tokensForLiquidity += (fees * launchTax.liquidity) / totalTaxes.launch;
                tokensForBurn += (fees * launchTax.burn) / totalTaxes.launch;
            }
            else if (automatedMarketMakerPairs[to] && totalTaxes.sell > 0) {
                ...
                tokensForBurn += (fees * sellTax.burn) / totalTaxes.sell;
            }
                ...
                tokensForBurn += (fees * buyTax.burn) / totalTaxes.buy;
            }
    ...
    function swapBack() private {
        ...
        if(tokensForBurn > 0) {
            super._transfer(address(this), taxWallets.burn, tokensForBurn);
        }
