...
contract Simulacra is ERC721, Ownable, ReentrancyGuard {
    ...
    string constant NAME = "Simulacra";
    string constant SYMBOL = "SIM";

    uint256 internal constant _MAX_SIMULACRA = 3447;
    uint256 public constant MAX_ARTISTS = 50;
    ...
    string contractUrl = "https://ipfs.io/ipfs/QmeR2snLRCEJzhXDLZCuBwR5YUJTxRDrYeHZL1tCZVDvQJ";
    string constant EMPTY_SIMULACRA_IPFS_URL = "https://ipfs.io/ipfs/Qmb6tuZCf2pNwNWPgHeUCk7Y1wZt9zDTXX8uMr57cvV2Jf";

    constructor(uint256 _saleStartTimestamp, uint8 _saleDurationInDays, uint8 _failsafeDurationInDays)
        ERC721(NAME, SYMBOL) {
        
        require(block.timestamp <= _saleStartTimestamp && _saleDurationInDays > 0 && _failsafeDurationInDays > 0,
    ...
    */
    ...
    function artWasRevealed() public view returns(bool){
        return address(metadata) != address(0)          // metadata is not null ... 
    ...
    function MAX_SIMULACRA() public virtual pure returns(uint256){
    ...
    */
    ...
    function getCurrentPrice() public view returns(uint256){
        require(SALE_START <= block.timestamp && block.timestamp <= SALE_END, "Sale is closed");
        require(!artSoldOut(), "No more pieces to sell");
        uint256 nextItemAvailable = totalSupply();
        return getPriceOf(nextItemAvailable);
    }

    function getPriceOf(uint256 _tokenID) public virtual pure returns(uint256){       
        if (_tokenID < 55){
    ...
    function remainingAtCurrentPrice() external virtual view returns(uint256){
        uint256 piecesSold = totalSupply();
        return remainingAt(piecesSold);
    }

    function remainingAt(uint256 _piecesSold) internal virtual view returns(uint256){
        uint256 nextStepAt;
        if (_piecesSold < 55){
            nextStepAt = 55;
    ...
    */
    ...
    function commissionPieces(uint256 _indices, uint256 _number) external payable nonReentrant {
        
        uint256 currentSupply = totalSupply();

        // Check sale is valid (note that call to getCurrentPrice below also makes additional checks)
        require(0 < _number && _number <= remainingAt(currentSupply), "Invalid number");

        // Check the price is right
        require(msg.value == _number * getCurrentPrice(), "Invalid eth amount");

        // Check popularity indices
        require(uint8(_indices) <= MAX_ARTISTS, "1st artist index is not valid");
        require(uint8(_indices >> 8) <= MAX_ARTISTS, "2nd artist index is not valid");
        require(uint8(_indices >> 16) <= MAX_ARTISTS, "3rd artist index is not valid");
    ...
    function setMetadata(address _metadata) external onlyOwner {
        require(SALE_END <= block.timestamp || artSoldOut(), "Sale must have ended");
    ...
    */
    ...
    function getRefund(uint256 _tokenID) external nonReentrant {
        ...
        require(msg.sender == ownerOf(_tokenID), "Only token owner gets a refund");
    ...
    */
    function tokenURI(uint256 _tokenID) public view override returns(string memory){
        ...
        if (artWasRevealed()){
            ...
            return EMPTY_SIMULACRA_IPFS_URL;
        }
    }
    ...
    function setContractURI(string memory _uri) external onlyOwner returns (string memory) {
        contractUrl = _uri;
    }
}