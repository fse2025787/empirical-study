...
contract Club44WhitelistPass is ERC721, ERC721Burnable, AccessControl {
    ...
    constructor() ERC721("44 Club Whitelist Pass", "44ClubPass") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
    }
    
    function _baseURI() internal pure override returns (string memory) {
        return "https://the44.club/api/whitelist/";
    }
    
    function safeMint(address to) public onlyRole(MINTER_ROLE) {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        require(tokenId <= 8888, "44Club: Whitelist spots finished");
        _safeMint(to, tokenId);
    }
