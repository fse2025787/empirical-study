...
contract Anatomy00ts is ERC721A, Ownable, ReentrancyGuard {
    bool public Minting  = false;
    uint256[] public freeMintArray = [3,2,1];
    uint256[] public supplyMintArray = [8000,8500,9000];
    uint256 public price = 3300000000000000;
    string public baseURI;  
    uint256 public maxPerTx = 50;  
    uint256 public maxSupply = 10000;
    uint256 public teamSupply = 100;  
    mapping (address => uint256) public minted;
    constructor() ERC721A("Anatomy00ts", "Anatomy00ts"){}
    ...
    {
        require(Minting , "Anatomy00ts Minting Close !");
        require(qty <= maxPerTx, "Anatomy00ts Max Per Tx !");
        require(totalSupply() + qty <= maxSupply-teamSupply,"Anatomy00ts Soldout !");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal  {
        ...
        if(minted[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * price,"Anatomy00ts Insufficient Funds !");
            minted[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * price,"Anatomy00ts Insufficient Funds !");
            minted[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
