...
contract mAsset is AccessControl, ERC20("Mellow USD", "mUSD") {
  ...
  event Paused(address refineryAddress, bool isPaused);
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "mUSD: Refinery is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "mUSD: Refinery is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"mUSD: Refinery's ceiling was breached.");
    require(!paused[msg.sender], "mUSD: user is currently paused.");
  ...
  /// This function reverts if the caller does not have the admin role.
  function pauseRefinery(address _toPause, bool _state) external onlySentinel {
