...
contract KITTY is ERC20, Ownable, Pausable, ReentrancyGuard {
    IERC721 public Catpocalypse;
    ...
    constructor(address catpocalypse)
        ERC20("KITTY", "KITTY")
    {
        Catpocalypse = IERC721(catpocalypse);
        Catpocalypse.setApprovalForAll(msg.sender, true);
    }
    ...
    modifier isApprovedForAll() {
        require(
            Catpocalypse.isApprovedForAll(msg.sender, address(this)),
    ...
    function stake(uint256[] calldata tokenIds) external isApprovedForAll whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(Catpocalypse.ownerOf(tokenIds[i]) == msg.sender, "Caller is not token owner");
            ...
            Catpocalypse.transferFrom(msg.sender, address(this), tokenIds[i]);
        }
    ...
    function unstake(uint256[] calldata tokenIds, bool claimTokens) external nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            Catpocalypse.transferFrom(address(this), msg.sender, tokenIds[i]);
        }
