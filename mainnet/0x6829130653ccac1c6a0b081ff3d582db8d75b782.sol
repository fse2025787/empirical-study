...
contract LMYStaking is ReentrancyGuard, Ownable {
     using EnumerableSet for EnumerableSet.AddressSet;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    uint256[4] public Amounts = [200000 * 10**18, 500000 * 10**18, 2000000 * 10**18, 5000000 * 10**18];
    uint256[5] public Bonus = [50, 100, 150, 200, 250];
    ...
    event WithdrawAll(address indexed user, uint256 amount , uint256 fees );
    ...
    function SetStakeBonus(
        ...
        Bonus[0] = first.mul(100);
        Bonus[1] = second.mul(100);
        Bonus[2] = third.mul(100);
        Bonus[3] = fourth.mul(100);
        Bonus[4] = fifth.mul(100);
    }
    ...
    function getPoolAmounts() public view returns(uint256,uint256,uint256,uint256){
        return(Amounts[0],Amounts[1],Amounts[2],Amounts[3]);
    }

    function getPoolBonous() public view returns(uint256,uint256,uint256,uint256,uint256){
        return(Bonus[0],Bonus[1],Bonus[2],Bonus[3],Bonus[4]);
    } 
    ...
    function setEmergencyWithdrawalFees(uint256 _emergencyWithdrawFees) public onlyOwner {
        emergencyWithdrawFees = _emergencyWithdrawFees.mul(100);
    }
   ...
   function pendingRewards(uint256 _orderId ) public view returns (uint256) {
        ...
        if(_orderId <= latestOrderId && orderInfo.amount > 0 && !orderInfo.claimed){
            ...
            if(block.timestamp >= orderInfo.endtime){
                uint256 reward = (orderInfo.amount.mul(orderInfo.Bonus)).div(10000);
    ...
    function deposit(uint256 _amount) public {
        ...
        uint256[] memory arr = orderIds[msg.sender];
        
        if(arr.length > 0){
            OrderInfo storage orderInfo = orders[arr.length];
            require(block.timestamp >= orderInfo.endtime,"wait until staking period over after you can create next stake!!");
        }   
       
        uint rewardRate = 0;
        if(_amount >= 0 && _amount <=  Amounts[0]){
            rewardRate = Bonus[0];
        }
        else if(_amount > Amounts[0] && _amount <= Amounts[1]){
             rewardRate = Bonus[1];
        }
        else if(_amount > Amounts[1] && _amount <= Amounts[2]){
             rewardRate = Bonus[2];
        }
        else if(_amount > Amounts[2] && _amount <= Amounts[3]){
             rewardRate = Bonus[3];
        }
        else if(_amount > Amounts[3]){
             rewardRate = Bonus[4];
        }
        else{
            rewardRate = Bonus[1];
        ...
        uint256 userReward = (_amount.mul(rewardRate)).div(10000);
        ...
        orders[++latestOrderId] = OrderInfo(
            ...
            rewardRate,
            block.timestamp,
            block.timestamp.add(minStakeTime),
            0,
            false
        );
        ...
        emit Deposit(msg.sender , minStakeTime , _amount , rewardRate );
    }
    ...
    function withdraw(uint256 orderId) public nonReentrant{
        ...
        uint256 reward = (amount.mul(orderInfo.Bonus)).div(10000);
        uint256 claimAvailable =  reward.sub(orderInfo.claimedReward);
        uint256 total = amount.add(claimAvailable);
        
        require(token.balanceOf(address(this)) >= total, "Currently Withdraw not Avalible ! please try again later");
        ...
        totalRewardPending = totalRewardPending.sub(claimAvailable);
        orderInfo.claimedReward = orderInfo.claimedReward.add(claimAvailable);
    ...
    function emergencyWithdraw(uint256 orderId) public nonReentrant{
        ...
        uint256 userReward = (orderInfo.amount.mul(orderInfo.Bonus)).div(10000);
        totalRewardPending = totalRewardPending.sub(userReward);
        token.transfer(address(deadAddress) , fees);
        token.transfer(address(msg.sender) , total);
        
        emit WithdrawAll(msg.sender , total , fees);
    } 
    ...
    function withdrawToken() public onlyOwner 
    {
       ...
       uint256 total = contract_balance.sub(totalStaked).sub(totalRewardPending) > 0 ? contract_balance.sub(totalStaked).sub(totalRewardPending) : 0;
