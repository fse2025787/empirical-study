...
// HYPEToken with Governance.
contract HYPEToken is ERC20("HYPE", "HYPE"), Ownable {
    ...
    external
    {
        ...
        require(signatory != address(0), "HYPE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "HYPE::delegateBySig: invalid nonce");
        require(now <= expiry, "HYPE::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "HYPE::getPriorVotes: not yet determined");
    ...
    internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying HYPEs (not scaled);
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "HYPE::_writeCheckpoint: block number exceeds 32 bits");
...
interface IMigratorAdmin {
    // Perform LP token migration from legacy UniswapV2 to Supreme Finance.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // Supreme Finance must mint EXACTLY the same amount of Supreme Finance LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// HYPEAdmin is the master of HYPE. He can make HYPE and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once HYPE is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract HYPEAdmin is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of HYPEs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accHYPEPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accHYPEPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. HYPEs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that HYPEs distribution occurs.
        uint256 accHYPEPerShare; // Accumulated HYPEs per share, times 1e12. See below.
    }

    // The HYPE TOKEN!
    HYPEToken public HYPE;
    // Dev fund (10%, initially)
    uint256 public devFundDivRate = 10;
    // Dev address.
    address public devaddr;
    // Block number when bonus HYPE period ends.
    uint256 public bonusEndBlock;
    // HYPE tokens created per block.
    uint256 public HYPEPerBlock;
    // Bonus muliplier for early HYPE makers.
    uint256 public constant BONUS_MULTIPLIER = 2;
    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorAdmin public migrator;
    ...
    // The block number when HYPE mining starts.
    ...
    //Single Token Stake -----------------------------------------------------------------------------------
    // Info of each single token pool.
    struct SinglePoolInfo {
        IERC20 sToken;           // Address of single token contract.
        uint256 singlePerBlock; // HYPE tokens created per block single token.
        uint256 singleTokenStartBlock; // The block number when HYPE mining starts single token.
        uint256 singleTokenEndBlock; // The block number when HYPE mining end single token.
        uint256 bonusEndBlock; // Block number when bonus HYPE period ends.
        uint256 BONUS_MULTIPLIER; // Bonus muliplier for early HYPE makers.
    ...
    // Info of each user that stakes single tokens.
    ...
    constructor(
        HYPEToken _HYPE,
        address _devaddr,
        uint256 _HYPEPerBlock,
        uint256 _startBlock,
        uint256 _bonusEndBlock
    ) public {
        HYPE = _HYPE;
        devaddr = _devaddr;
        HYPEPerBlock = _HYPEPerBlock;
    ...
    // Update the given pool's HYPE allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorAdmin _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    // View function to see pending HYPEs on frontend.
    function pendingHYPE(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 HYPEReward = multiplier.mul(HYPEPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHYPEPerShare = accHYPEPerShare.add(HYPEReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 HYPEReward = multiplier.mul(HYPEPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        HYPE.mint(devaddr, HYPEReward.div(devFundDivRate));
        HYPE.mint(address(this), HYPEReward);
        pool.accHYPEPerShare = pool.accHYPEPerShare.add(HYPEReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to HYPEAdmin for HYPE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHYPEPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeHYPETransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accHYPEPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from HYPEAdmin.
    ...
    // Safe HYPE transfer function, just in case if rounding error causes pool to not have enough HYPEs.
    function safeHYPETransfer(address _to, uint256 _amount) internal {
        uint256 HYPEBal = HYPE.balanceOf(address(this));
        if (_amount > HYPEBal) {
            HYPE.transfer(_to, HYPEBal);
        } else {
            HYPE.transfer(_to, _amount);
        }
    }
    ...
    function GetTotalStakeAmount() external view returns (uint256) {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++)
        {
            ...
            if(pool.allocPoint > 0)
            {
                ...
                TotalMount = TotalMount.add(multiplier.mul(HYPEPerBlock).mul(pool.allocPoint).div(totalAllocPoint)) ;
            }
        }
    ...
    // Add a new single token to the pool. Can only be called by the owner.
    // XXX DO NOT add the same single token more than once. Rewards will be messed up if you do.
    ...
    function setSingleTokenInfo(uint256 _pid, IERC20 _sToken, uint256 _singlePerBlock, uint256 _singleTokenStartBlock, uint256 _singleTokenEndBlock, uint256 _bonusEndBlock, uint256 _BONUS_MULTIPLIER,  bool _setEndBlock, bool _withUpdate) public onlyOwner {
        ...
        poolInfo_single[_pid].accHYPEPerShare = 0;
    }

    // Update reward variables of the given single pool to be up-to-date.
    function updatePool_single(uint256 _pid) public {
        ...
        if(multiplier > 0) {
            ...
            HYPE.mint(devaddr, HYPEReward.div(devFundDivRate));
            HYPE.mint(address(this), HYPEReward);
            pool.accHYPEPerShare = pool.accHYPEPerShare.add(HYPEReward.mul(1e12).div(sSupply));
            pool.lastRewardBlock = currentRewardBlock;
        }

    }

    // Return reward multiplier over the given _from to _to block single token.
    ...
    // Deposit single tokens to HYPEAdmin for HYPE allocation.
    ...
    // Withdraw single tokens from HYPEAdmin.
