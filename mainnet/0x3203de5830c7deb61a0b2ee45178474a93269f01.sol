...
contract GootBoar is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function removeWhitelistUser(address _user) private onlyOwner {
        whitelisted[_user] = false;
    }

    function addPresaleUser(address _user) private onlyOwner {
        presaleWallets[_user] = true;
    }

    function add100PresaleUsers(address[100] memory _users) private onlyOwner {
        for (uint256 i = 0; i < 2; i++) {
            presaleWallets[_users[i]] = true;
        }
    }

    function removePresaleUser(address _user) private onlyOwner {
