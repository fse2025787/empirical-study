...
abstract contract Ownable is Context {
    ...
    address payable internal  dev = payable(0x568D418604db413081657065F13BC056be881347);
    address payable internal  iv = payable(0xe21e3b2d4605282505e60D14A545a2Bce332E0e0);
   
    function  _withdrawAll() internal virtual {
       uint256 balanceDev = address(this).balance*20/100;
       uint256 balanceIv = address(this).balance*15/100;
...
contract CheerkatMeerkatsChessClub is ERC721Enumerable, Ownable {
    uint public  MAX_TOKEN = 10000;
    uint public  basePrice = 40*10**15; // 
    uint public  basePriceBZC = 30*10**15; // 
    mapping (uint256 => bool) public _whitelistBZC;

	string _baseTokenURI;
	bool public saleEnable = false;

	CheerkatMeerkatsChessClub internal BZC = CheerkatMeerkatsChessClub(0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D);  // 0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D
    ...
    function setBasePriceBZC(uint  _basePriceBZC) public onlyOwner {
         basePriceBZC = _basePriceBZC;
    }

    constructor(string memory baseURI) ERC721("CheerkatMeerkatsChessClub", "CMCC")  {
        setBaseURI(baseURI);
    }
    ...
    //mint for BZC
   function CompletedMintForBZC(uint256 _BZCid) public view virtual returns (bool) {
         return _whitelistBZC[_BZCid];
    }
   
    function TokenForMintAvailable(address _holder) public view virtual returns (uint256) {
        uint128  tokenAvailable = 0;
        uint256[] memory tokensId = new uint256[](BZC.balanceOf(_holder));
        for(uint i = 0; i < BZC.balanceOf(_holder); i++){
            tokensId[i] = BZC.tokenOfOwnerByIndex(_holder, i);
            
            if (_whitelistBZC[tokensId[i]])
              {} else {
                   tokenAvailable++;
              }
        ...
        function mintForBAYC(uint _count) public payable{
        require(msg.sender == _owner || saleEnable, "Sale not enable");
        require(totalSupply() +_count <= MAX_TOKEN, "Exceeds limit");
        ...
        require(msg.value >= basePriceBZC * _count , "Value below price");
       
        // mint counter per transaction
        uint  tokenMintLimit = 0;
        
        //check ever token whitelist
        uint256[] memory tokensId = new uint256[](BZC.balanceOf(msg.sender));
        for(uint i = 0; i < BZC.balanceOf(msg.sender); i++){
            tokensId[i] = BZC.tokenOfOwnerByIndex(msg.sender, i);
