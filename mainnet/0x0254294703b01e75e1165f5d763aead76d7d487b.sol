...
// File contracts/common/StringsUtil.sol
...
contract JuraNFT is ERC721Tradable {
    ...
    // block height on token(ID) last received/mint
    mapping(uint256 => uint256) public heightsOnTokenReceived;
    // block height on user(address) last claim power
    mapping(address => uint256) public heightsOnUserClaimPower;
    // total power of user(address)
    mapping(address => uint256) public tPowers;
    // debt/consumed power of user(address): powerClaimable = tPower - dPower
    ...
    function mintByUser(uint256 _amount) public payable {
        ...
        // remain value will auto return to msg sender
    ...
    function claimPower() public {
        ...
        // todo: require claim start time and had unminted jura token
        require(
    ...
    // out
    // get the amount of power that user claimable
    // cPowerPerToken = scaleByTokenID * (currHeight-max(heightOnTokenReceived,heightOnUserClaimPower))*powerPerBlock
    // tPower = tPower + sum(cPowerPerToken)
    // cPower = tPower - dPower
