...
contract AbilityScores is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function random(string memory input) internal pure returns (uint256) {
        return (uint256(keccak256(abi.encodePacked(input))) % 100);
    }
    
    function getbGay(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "%  bGay");
    }

    function pluck(uint256 tokenId, string memory keyPostfix) internal pure returns (string memory) {
        uint256 roll1 = random(string(abi.encodePacked(keyPostfix, toString(tokenId), "1")));
    
        
        string memory output = string(abi.encodePacked(toString(roll1), keyPostfix));
    ...
    function tokenURI(uint256 tokenId) override public pure returns (string memory) {
        ...
        parts[1] = getbGay(tokenId);
        parts[2] = '</text></svg>';

        string memory output = string(abi.encodePacked(parts[0], parts[1]));
        output = string(abi.encodePacked(output, parts[2]));
        
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "bGay #', toString(tokenId), '", "description": "bGay are randomized Tumblr style stats generated and stored on chain. Feel free to use bGay in any way you want. Come on out of the closet, its hip to show your pride !", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
