...
{    
    ...
    uint public _IndexEnding = 40;                                                          // Ending Index Of Sale
    uint public _MintPackPrice = 5 ether;                                                   // Mint Pack Price
    bool public _SaleActive;                                                                // Public Sale State
    bool public _SaleActiveBrightList;                                                      // Brightlist Sale State
    bytes32 _Root = 0x106640d8b3d5aa1e8275b43dd79cb922143f3e1caf88582bddc92e52f67f5bd8;     // Merkle Root
     ...
     */
    function PurchaseMintPack() public payable nonReentrant
    {
        // === Purchases Pack 1-10 ===
     ...
     * @dev Purchases Berlin Collection Artist Mint Pack
     */
    function PurchaseMintPackBrightList(bytes32[] calldata Proof) public payable nonReentrant
    {
        // === Purchases Pack 1-10 ===
        require(msg.value == _MintPackPrice, "Invalid Message Value. Mint Pack Is 5 Ether | 5000000000000000000 WEI");
        require(_SaleActiveBrightList, "Sale Inactive");
        require(viewBrightListAllocation(msg.sender, Proof), "User Is Not On BrightList");
        require(_Index <= _IndexEnding, "Sold Out");
        IERC721(_ArtistMintPass1).transferFrom(_BRTMULTISIG, msg.sender, _Index); 
        IERC721(_ArtistMintPass2).transferFrom(_BRTMULTISIG, msg.sender, _Index);  
        IERC721(_ArtistMintPass3).transferFrom(_BRTMULTISIG, msg.sender, _Index);  
    ...
    /**
     * @dev Withdraws All Ether From Sale To Message Sender
     * note: OnlyOwner
     */
    function __withdrawEther() external onlyOwner { payable(msg.sender).transfer(address(this).balance); }

    /**
     * @dev Withdraws All Ether From Contract To `Recipient`
     * note: OnlyOwner
     */
    ...
    /**
     * @dev Withdraws `Amount` Of Ether, Input As WEI, From Contract To `Recipient` With An Amount
     * note: OnlyOwner
     * note: `Amount` Is Input In WEI. (1 ETH = 1000000000000000000 WEI)
     */
     ...
     */
    function __flipSaleStatePublic() external onlyOwner { _SaleActive = !_SaleActive; }

    /**
     * @dev Flips Sale State BrightList
     */
    function __flipSaleStateBrightList() external onlyOwner { _SaleActiveBrightList = !_SaleActiveBrightList; }

    /**
     * @dev Changes Merkle Root Hash
     */
    function __changeMintPackRoot(bytes32 Root) external onlyOwner { _Root = Root; }

    /**
     * @dev Changes Mint Pack Starting Index
     */
    function __changeMintPackIndex(uint Index) external onlyOwner { _Index = Index; }

    /**
     * @dev Changes Mint Pack Ending Index
     */
    function __changeMintPackIndexEnding(uint Index) external onlyOwner { _IndexEnding = Index; }
