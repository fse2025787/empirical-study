...
Low Effort Pixel Punks ASCII Art
...
/**
 * @title Low Effort Pixel Punks contract.
 * @author The Gutter Punks team.
 *
 */
contract LowEffortPixelPunks is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public constant GP_OFFSET = 8000;
    uint256 public constant MAX_SUPPLY = 4321;
    uint256 public REWARD_AIRDROPS = 0;
    uint256 public ADDITIONAL_PUNKS = 0;
    uint256 public ADDITIONAL_PUNKS_START = 1001;
    ...
    function airdropAdditional(address[] calldata recipients) external { 
        uint256 startingTokenId = ADDITIONAL_PUNKS_START + ADDITIONAL_PUNKS;
        ...
        ADDITIONAL_PUNKS = ADDITIONAL_PUNKS + recipients.length;
    ...
    function mint(uint256 amount) external payable { 
        uint256 startingTokenId = ADDITIONAL_PUNKS_START + ADDITIONAL_PUNKS;
        require(startingTokenId + amount - 1 <= MAX_SUPPLY, "Out of tokens.");
        require(_saleIsActive, "Sale not active");
        require(getCost(amount) <= msg.value, "Insufficient payment");

        _totalSupply = _totalSupply + amount;
        ADDITIONAL_PUNKS = ADDITIONAL_PUNKS + amount;

        for(uint256 i = 0;i < amount; i++) {
            _owners[startingTokenId + i] = msg.sender;
            _mint(msg.sender, startingTokenId + i);
        }
    }
	...
	function parentTokenTransferred(address from, uint256 tokenId) public virtual { 
        ...
        if(tokenId >= 8001 && tokenId <= 9000) {
            tokenId = tokenId - GP_OFFSET; // offset for GP reward batch GP #8001 -> VP #1
