...
contract NFTDigiWork is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;
    using Strings for uint256;

    uint public constant _TOTALSUPPLY = 8888;
    ...
    constructor(string memory baseURI) ERC721("UnIdentified Predators", "UIP") {
        setBaseURI(baseURI);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(bytes(base).length > 0){
            return reveal ? string(abi.encodePacked(base, tokenId.toString(), ".json")) : string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/QmX4SobwijBtSr1nix7EiSD7tu6yGVcm54hZw8iJvQ6XXv"));
        }
        else 
        return "";
    }
    function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "UnIdentified Predators", "description": "Unidentified Predators is a collection of 8,888 avatars consisting of 5,000 aliens and 3,888 predators, battling on the Ethereum ERC-721 blockchain.", "seller_fee_basis_points": 600, "fee_recipient": "0xf4a07925cFCe8F3abF79539f584dC86F4964E512"}'))));
