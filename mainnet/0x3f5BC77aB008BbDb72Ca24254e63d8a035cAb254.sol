...
contract PermissionManagerV2 is Initializable, AccessControlUpgradeable, PermissionManagerStorage {
     ...
     */
    function setPermissionsAdmin(address _permissionsAdmin)
        external
        zeroAddressCheck(_permissionsAdmin)
        onlyDefaultAdmin
    {
     ...
     */
    function setPermissionItems(IPermissionItems _permissionItems)
        external
        zeroAddressCheck(address(_permissionItems))
        onlyDefaultAdmin
        returns (bool)
    {
     ...
     * @param _account The address to assign Tier1.
     */
    function assignItem(address _account, uint256 _itemId) public zeroAddressCheck(_account) onlyPermissionsAdmin {
     ...
     * @param _accounts The addresses to assign Tier1.
     */
    function assignItems(uint256 _itemId, address[] calldata _accounts) external {
        for (uint256 i = 0; i < _accounts.length; i++) {
            assignItem(_accounts[i], _itemId);
        }
    }
     ...
     * @param _account The address to assign Tier1.
     */
    function removeItem(address _account, uint256 _itemId) public onlyPermissionsAdmin {
     ...
     * @param _accounts The addresses to assign Tier1.
     */
    function removeItems(uint256 _itemId, address[] calldata _accounts) external {
        for (uint256 i = 0; i < _accounts.length; i++) {
            removeItem(_accounts[i], _itemId);
        }
    }
    ...
    function _hasItem(address _user, uint256 itemId) private view returns (bool) {
        return permissionItems.balanceOf(_user, itemId) > 0;
    }
}
