...
contract OnlyPass is ERC721A, Administered {
    enum Stage {
        Paused,
        AllowList,
        Public
    ...
    constructor(address[] memory admins, string memory _baseTokenURI)
        ERC721A("OnlyPass", "OP")
        Administered(admins)
    {
    ...
    function updatePrice(uint256 _alPrice, uint256 _publicPrice) public onlyAdmin {
        publicPrice = _publicPrice;
        allowListPrice = _alPrice;
    }

    function setStage(Stage _state) external onlyAdmin {
        stage = _state;
    }
    ...
    function setFreeMintsRemaining(uint256 _freeMintsRemaining) external onlyAdmin {
        freeMintsRemaining = _freeMintsRemaining;
    }

    function setBaseURI(string calldata _baseTokenURI) external onlyAdmin {
        baseTokenURI = _baseTokenURI;
    }
