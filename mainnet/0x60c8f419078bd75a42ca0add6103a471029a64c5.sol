...
contract Pupzland is Ownable, ERC721A  {
    ...
    uint256 public publicSaleCost = 0.0065 ether;
    uint256 public freeAmountTotalSupply= 0;

    uint128 public maxSupply = 5000;
    uint64 public maxFreeMintSupply =  2500;

    uint64 public maxMintAmountPerFreeAccount = 1;
    ...
    constructor() ERC721A("Pupzland", "Pupzland") {}
    ...
    function freeMint( uint64 _mintAmount) public payable mintCompliance(_mintAmount) {
        ...
        require(freeAmountTotalSupply + _mintAmount <= maxFreeMintSupply, "Free mint supply exceeded." );

        require(freeAmountMinted + _mintAmount <= maxMintAmountPerFreeAccount, "Mint limit exceeded." );
        require(msg.value == freeMintCost * _mintAmount, "Insufficient funds!");

        setFreeAmountMinted(msg.sender,freeAmountMinted + _mintAmount);

        _safeMint(msg.sender, _mintAmount);

        freeAmountTotalSupply+=_mintAmount;
        if(freeAmountTotalSupply==maxFreeMintSupply){
    ...
    function getFreeAmountMinted(address _owner) public view returns (uint64) {
        return _getAux(_owner);
    }

    function setFreeAmountMinted(address _owner, uint64 _aux) internal {
        _setAux(_owner, _aux);
    }
    ...
    function setMaxMintAmountPerPublicAccount(uint64 _maxMintAmountPerPublicAccount) public onlyOwner {
        maxMintAmountPerPublicAccount = _maxMintAmountPerPublicAccount;
    }

    function setMaxMintAmountPerFreeAccount(uint64 _maxMintAmountPerFreeAccount) public onlyOwner {
    ...
    function setMaxFreeMintSupply(uint64 _maxFreeMintSupply) public onlyOwner {
