...
contract NFTAuction is INFTAuction, OwnableUpgradeable {
  ...
  function getNftAuctionDetails(address _nftContractAddress, uint256 _tokenId)
    public
    view
  ...
  function flipOnlyOwnerCanList() public onlyOwner {
    onlyOwnerCanlist = !onlyOwnerCanlist;
  }
  ...
  function _isHighestBidderWhitelisted(address _nftContractAddress, uint256 _tokenId)
    internal
    view
    returns (bool)
  {
  ...
  // ===> End of Auction Check Functions
  ...
  function _getBidIncreasePercentage(address _nftContractAddress, uint256 _tokenId)
    internal
    view
    returns (uint32)
  {
  ...
  {
    if (onlyOwnerCanlist) {
      require(msg.sender == owner(), "Only owner can list NFTs at this moment");
    }
  ...
  function _payFeesAndSeller(
    ...
    for (
      uint256 i = 0;
      i < nftContractAuctions[_nftContractAddress][_tokenId].feeRecipients.length;
      i++
    ) {
  ...
  // ===>  END OF TRANSFER NFT & PAY SELLER
