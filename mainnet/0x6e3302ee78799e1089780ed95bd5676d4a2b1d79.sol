...
contract HotDogTimeMachine is ERC721A {
    ...
    mapping(address => mapping(uint8 => uint8)) private partyPeople;
    ...
    function claimableDogs(address _partyPerson) public view returns(uint256) {
        return (partyPhase == 33 ? 1 : partyPeople[_partyPerson][partyPhase]) - balanceOf(_partyPerson);
    }

    function sizzle() external {
        require(partyPhase > 0, "The party hasn't started yet");
        require(
            isInvited(msg.sender, partyPhase) || partyPhase == 33,
            "Looks like we're not ready for you yet"
        );

        uint256 leCount = totalSupply();
        uint256 maxAllowed = claimableDogs(msg.sender);
