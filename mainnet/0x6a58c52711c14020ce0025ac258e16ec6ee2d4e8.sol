...
https://twitter.com/HachiFinance
...
contract CHISRewards is ICHISRewards, Ownable {
  address public CHI;
  IERC721 private xCHI;
  IERC721 private xlCHI;
  ...
  modifier onlyToken() {
    require(
      msg.sender == address(xCHI) || msg.sender == address(xlCHI),
      'must be stake token'
    );
    _;
  }

  constructor(address _CHI, address _dexRouter) {
    CHI = _CHI;
  ...
  function getxCHI() external view returns (address) {
    return address(xCHI);
  }

  function getxlCHI() external view returns (address) {
    return address(xlCHI);
  }
  ...
  function setxCHI(address _xCHI) external onlyOwner {
    xCHI = IERC721(_xCHI);
  }

  function setxlCHI(address _xlCHI) external onlyOwner {
    xlCHI = IERC721(_xlCHI);
  }
}
...
contract CHIStake is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  ICHI internal CHI;
  ICHIVester internal vester;
  ICHISRewards internal rewards;
  ...
  constructor(
    ...
    CHI = ICHI(_CHI);
    vester = ICHIVester(_vester);
    rewards = ICHISRewards(_rewards);
  ...
  function _stake(
    ...
    _amountStaked = _amountStaked == 0
      ...
      : _amountStaked;
    _amountCHIBaseEarn = _amountCHIBaseEarn == 0
      ? _amountStaked
      : _amountCHIBaseEarn;
    require(
      _amountStaked > 0 && _amountCHIBaseEarn > 0,
    ...
    stakes[_ids.current()] = Stake({
      ...
      amountCHIBaseEarn: _amountCHIBaseEarn,
  ...
  function unstake(uint256 _tokenId) public {
    ...
    if (_isUnstakingEarly) {
      ...
      if (address(stakeToken) == address(CHI)) {
        CHI.burn(_tokenStake.amountStaked - _earnedAmount);
      } else {
  ...
  function claimAndVestRewards(uint256 _tokenId) public {
    ...
    require(block.timestamp > lastClaim[_tokenId] + FOUR_DAYS);
    ...
    returns (uint256)
  {
    ...
    return
      (_tokenStake.amountCHIBaseEarn * _tokenStake.apr * _secondsStaked) /
      PERCENT_DENOMENATOR /
      ONE_YEAR;
  }
  ...
  function _afterTokenTransfer(
    ...
    if (_from != address(0)) {
      ...
      rewards.setShare(_from, _tokenStake.amountCHIBaseEarn, true);
    }
    ...
    if (_to != address(0)) {
      ...
      rewards.setShare(_to, _tokenStake.amountCHIBaseEarn, false);
    }

    super._afterTokenTransfer(_from, _to, _tokenId);
  }
}
contract xlCHI is CHIStake {
    ...
    )
  {
    ...
    _addAprLockOption(10000, 0);
    _addAprLockOption(15000, 14 days);
    _addAprLockOption(30000, 120 days);
    _addAprLockOption(45000, 240 days);
    _addAprLockOption(60000, 360 days);
  }
  ...
  function zapAndStakeETHOnly(uint256 _lockOptIndex) external payable {
    ...
    uint256 _CHIBalanceBefore = CHI.balanceOf(address(this));
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);

    // swap half the ETH for CHI
    uint256 _tokensToReceiveNoSlip = _getTokensToReceiveOnBuyNoSlippage(
      msg.value / 2
    );
    address[] memory path = new address[](2);
    ...
    _addLp(CHI.balanceOf(address(this)) - _CHIBalanceBefore, msg.value / 2);
    ...
    _returnExcesxCHI(msg.sender, _CHIBalanceBefore);

    emit ZapETHOnly(msg.sender, msg.value, _lpBalanceToStake);
  }

  function zapAndStakeETHAndCHI(uint256 _amountCHI, uint256 _lockOptIndex)
    external
    payable
  {
    require(msg.value > 0, 'need to provide ETH to zap');
    ...
    uint256 _ethBalBefore = address(this).balance - msg.value;
    uint256 _CHIBalBefore = CHI.balanceOf(address(this));
    CHI.transferFrom(msg.sender, address(this), _amountCHI);
    uint256 _CHIToProcess = CHI.balanceOf(address(this)) - _CHIBalBefore;

    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_CHIToProcess, msg.value);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);

    _returnExcessETH(msg.sender, _ethBalBefore);
    _returnExcesxCHI(msg.sender, _CHIBalBefore);

    emit ZapETHAndCHI(msg.sender, msg.value, _amountCHI, _lpBalanceToStake);
  }

  function zapAndStakeCHIOnly(uint256 _amountCHI, uint256 _lockOptIndex)
    external
  {
    require(
      _aprLockOptions[_lockOptIndex].lockTime > 0,
      'cannot zap and stake CHI only without lockup period'
    );
    ...
    CHI.approve(address(_uniswapV2Router), _CHIToProcess / 2);
    ...
    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_CHIToProcess / 2, address(this).balance - _ethBalBefore);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);
    ...
    emit ZapCHIOnly(msg.sender, _amountCHI, _lpBalanceToStake);
  }

  function _addLp(uint256 tokenAmount, uint256 ethAmount) private {
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);
    CHI.approve(address(_uniswapV2Router), tokenAmount);
  ...
  function _getETHToReceiveOnSellNoSlippage(uint256 _amountCHI)
    internal
    view
    returns (uint256)
  {
    ...
    (uint112 _r0, uint112 _r1, ) = pair.getReserves();
    if (pair.token0() == IUniswapV2Router02(_uniswapRouter).WETH()) {
      return (_amountCHI * _r0) / _r1;
    } else {
      return (_amountCHI * _r1) / _r0;
    }
  }
  ...
  function _stakeLp(
    ...
    uint256 CHIReserves = t0 == address(CHI) ? res0 : res1;
    uint256 singleSideTokenAmount = (_amountStakeToken * CHIReserves) /
  ...
  function _returnExcesxCHI(address _user, uint256 _initialBal) internal {
    uint256 _currentBal = CHI.balanceOf(address(this));
    if (_currentBal > _initialBal) {
      CHI.transfer(_user, _currentBal - _initialBal);
      require(CHI.balanceOf(address(this)) >= _initialBal, 'took too much');
    }
  }
...
contract xCHI is CHIStake {
    ...
    )
  {
    ...
    _addAprLockOption(10000, 14 days);
    _addAprLockOption(20000, 120 days);
    _addAprLockOption(30000, 240 days);
    _addAprLockOption(40000, 360 days);
  }
}
contract CHI is ICHI, ERC20, Ownable {
  uint256 private constant EARLY_SELL_EXPIRATION = 60 days;
  ...
  xCHI private _stakedCHI;
  xlCHI private _stakedCHILP;
  CHIVester private _vester;
  CHISRewards private _rewards;
  ...
  modifier onlyStake() {
    require(
      address(_stakedCHI) == _msgSender() ||
        address(_stakedCHILP) == _msgSender(),
  ...
  constructor() ERC20('HaCHI.Fi', 'CHI') {
    _mint(msg.sender, 888_000_000 * 10**18);
    ...
    _vester = new CHIVester(address(this));
    _rewards = new CHISRewards(address(this), address(_uniswapV2Router));

    _stakedCHI = new xCHI(
      address(this),
      address(_vester),
      address(_rewards),
      'https://api.HaCHI.Fi/xCHI/metadata/'
    );
    _stakedCHI.setPaymentAddress(msg.sender);
    _stakedCHI.setRoyaltyAddress(msg.sender);
    _stakedCHI.transferOwnership(msg.sender);

    _stakedCHILP = new xlCHI(
      uniswapV2Pair,
      address(_uniswapV2Router),
      address(this),
      address(_vester),
      address(_rewards),
      'https://api.HaCHI.Fi/xlCHI/metadata/'
    );
    _stakedCHILP.setPaymentAddress(msg.sender);
    _stakedCHILP.setRoyaltyAddress(msg.sender);
    _stakedCHILP.transferOwnership(msg.sender);

    _vester.addStakingContract(address(_stakedCHI));
    _vester.addStakingContract(address(_stakedCHILP));
    _vester.transferOwnership(msg.sender);

    _rewards.setxCHI(address(_stakedCHI));
    _rewards.setxlCHI(address(_stakedCHILP));
    ...
    isTaxExcluded[address(_stakedCHILP)] = true; // allow zapping without taxes
  ...
  function _transfer(
    ...
    // handles transferring to a fresh wallet or wallet that hasn't bought CHI before
  ...
  function getStakedCHI() external view returns (address) {
    return address(_stakedCHI);
  }

  function getStakedCHILP() external view returns (address) {
    return address(_stakedCHILP);
  }
...
contract CHIVester is Ownable {
  ...
  function withdraw(uint256 _index) external {
    ...
    _vest.withdrawsCompleted = _withdrawsAllowed;
    _CHI.transfer(_user, _tokensPerWithdrawPeriod * _withdrawsToComplete);
    _CHI.addToBuyTracker(
      _user,
  ...
  function getCHI() external view returns (address) {
    return address(_CHI);
  }
