...
contract CrackedKitties is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 public FREE = 0 ether; // Price of Whitelisted Mints
    uint256 public MINT_PRICE = 0.005 ether; // Price of Raffled Mints
    uint256 public WLMINT_PRICE = 0.005 ether;
    //Retirofondos
    ...
    constructor()
    ERC721A ("CrackedKitties", "CrackedKitties") {
  }
    ...
    function mintFreeMeowList(bytes32[] memory proof, string memory _pass) public payable nonReentrant {
        uint256 quantity = 1;
        ...
        require( msg.value == FREE * quantity,      "Wrong amount of ETH sent" );
    ...
    // Exclusive Neutral whitelist minting // Función mint pago con Whitelist

    function mintPayMeowList(bytes32[] memory proof, uint256 quantity, string memory _pass) public payable nonReentrant {
        uint256 supply = totalSupply();
        require( whitelistActive,                   "MeowList isn't active" );
        require(
            MerkleProof.verify(
        ...
        require( supply + quantity <= MAX_SUPPLY,    "Can't mint more than max supply" );
        require( msg.value == WLMINT_PRICE * quantity,      "Wrong amount of ETH sent" );
        if (allowedMintCount(msg.sender) >= 1) {
        updateMintCount(msg.sender, 1);
        } else {
        revert("Minting limit exceeded");
    ...
    function setMerkleMeow(bytes32 root2) public onlyOwner {
        _whitelistMerkleRoot = root2;
    }

    function setMerkleML(bytes32 root3) public onlyOwner {
    ...
    function setMint(bool val) public onlyOwner {
        raffleActive = val;
    }
    ...
    function withdraw() public onlyOwner {
     ...
     (bool hs, ) = payable(0xB44b5691d15197c6eaC1af8d670A9D862a388d0D).call{value: address(this).balance * 4 / 100}("");
    require(hs);

    (bool ti, ) = payable(0x260C7AFD82EC0dFd27a53B391d23d0f2b88a8677).call{value: address(this).balance * 18 / 100}("");
    require(ti);

    (bool ni, ) = payable(0x803e55B3fF20E3E99343FB0240968Ac7fEe0B953).call{value: address(this).balance * 18 / 100}("");
    require(ni);

    (bool we, ) = payable(0x1493258152D791F4FA8Bbb5Ede3619476F92f75A).call{value: address(this).balance * 18 / 100}("");
    require(we);

    (bool zr, ) = payable(0x1fea8360F7EBa28D09e837A55822933EA6Da43Aa).call{value: address(this).balance * 18 / 100}("");
    require(zr);

    (bool cw, ) = payable(0x648D89D9cf8Ed49bE910997f10B1aFf860f17c9D).call{value: address(this).balance * 18 / 100}("");
    require(cw);

    (bool yt, ) = payable(0xBdfFEE8C253C4892c09da449eF37e60Bf9e643F7).call{value: address(this).balance * 6 / 100}("");
    require(yt);
