...
contract EvaToken is Context, IERC20, Ownable {
    ...
    string private _name = "Eva Token";
    ...
    address public MarketingWallet= 0xB26313Adc8Fc5Ec94d19f4AF41dCECa39bD4a8fF;
    ...
    uint256 private numTokensSellToAddToLiquidity = 25000000000 * 10**18;
    uint256 public maxWalletToken = 5000000000000 * (10**18);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Temporarily remove fees to transfer to Marketing  wallet
        _taxFee = 0;
        _liquidityFee = 0;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
         require(liquidityFee <=4,"Cannot set the fee more than 4%");
         _liquidityFee = liquidityFee;
    }
    
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <=4,"Cannot set the fee more than 4%");
        _taxFee = taxFee;
    }

    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <=4,"Cannot set the fee more than 4%");
        _MarketingFee = marketingFee;
    }

    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
        require(burnFee <=4,"Cannot set the fee more than 4%");
        _BurnFee = burnFee;
    }

     function setMaxTxAmount(uint256 maxTx) external onlyOwner() {
       require(maxTx >=25000000000,"Cannot set the max txn less than 0.5%");
        _maxTxAmount = maxTx * 10 ** 18;
    }
  
    function setMaxWalletTokens(uint256 _maxToken) external onlyOwner {
          require(_maxToken >=25000000000,"Cannot set the max wallet less than 0.5%");
          maxWalletToken = _maxToken * 10 ** 18;
  	}

    function NumTokenChange(uint256 numTokenAmount) external onlyOwner() {
        numTokensSellToAddToLiquidity = numTokenAmount * 10 ** 18;
    }
