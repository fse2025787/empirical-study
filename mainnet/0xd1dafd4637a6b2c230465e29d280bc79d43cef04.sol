...
contract evilElon is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Evil Elon";
    string private constant _symbol = "EVON";
    uint8 private constant _decimals = 9;
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee;
    uint256 private constant _tTotal = 10000000 * 1e9;

    uint256 public maxTxn = 300000 * 1e9;
    ...
    uint256 public _marketingFeeOnSell = 2; 
    ...
    address payable private _marketingAddress = payable(0xcFEEDA0f5A4D65E0ad1f3Cd4DaCAb4a6DC66BD52);
    address payable private deploymentWallet = payable(0xcFEEDA0f5A4D65E0ad1f3Cd4DaCAb4a6DC66BD52);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this) && to != address(this)) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                require(balanceOf(to).add(amount) <= maxTxn, "wallet balance after transfer must be less than max wallet amount");
            }
    ...
    function setMaxWalletAmount(uint256 maxWalletAmount) external {
        ...
        maxTxn = maxWalletAmount;
    }

    function setSwaptokensAmount(uint256 _swapAmount) external {
        require(_msgSender() == deploymentWallet);
