...
// File: @openzeppelin\contracts-upgradeable\token\ERC721\extensions\ERC721URIStorageUpgradeable.sol
...
// File: @openzeppelin\contracts-upgradeable\token\ERC721\extensions\ERC721BurnableUpgradeable.sol
...
// File: @openzeppelin\contracts-upgradeable\token\ERC721\extensions\ERC721PausableUpgradeable.sol
...
abstract contract HasRoyaltyFees is ERC165StorageUpgradeable {
    ...
    event RoyaltyFees(uint256 tokenId, address[] recipients, uint[] bps);

    /*
     * bytes4(keccak256('getFeeBps(uint256)')) == 0x0ebd4c7f
     * bytes4(keccak256('getFeeRecipients(uint256)')) == 0xb9c4d9fb
     *
     * => 0x0ebd4c7f XOR 0xb9c4d9fb == 0xb7799584
     */
    ...
    OwnableOperatorRole
{
    ...
    // token id => RoyaltyFees
    mapping (uint256 => RoyaltyFee[]) public royaltyFees;
    ...
    function initialize(
        ...
        __ANIFIE721_init_unchained();

        erc20TransferProxy = _erc20TransferProxy;
        beneficiary = _beneficiary;
    }

    function __ANIFIE721_init_unchained() internal initializer virtual {
    ...
    function mint(string memory _tokenURI, RoyaltyFee[] memory _royaltyFees) 
        public 
        virtual 
        payable
        returns (uint256) 
    {
        require(hasRole(MINTER_ROLE, _msgSender()), "ANIFIE721: must have minter role to mint");
        ...
        address[] memory recipients = new address[](_royaltyFees.length);
        uint[] memory feeBPSs = new uint[](_royaltyFees.length);

        for (uint i = 0; i < _royaltyFees.length; i++) {
            require(_royaltyFees[i].recipient != address(0x0), "Recipient should be present");
            require(_royaltyFees[i].feeBPS != 0, "Fee value should be positive");
            royaltyFees[newItemId].push(_royaltyFees[i]);
            recipients[i] = _royaltyFees[i].recipient;
            feeBPSs[i] = _royaltyFees[i].feeBPS;
        }

        if (_royaltyFees.length > 0) {
            emit RoyaltyFees (newItemId, recipients, feeBPSs);
        }
    ...
    function setERC20TransferProxy(ERC20TransferProxy newERC20TransferProxy) virtual external onlyOwner {
        erc20TransferProxy = newERC20TransferProxy;
    }
    ...
    function setStorageFee(AnifieNFTDomain.StorageFee calldata _storageFee) virtual external onlyOperator {
        storageFee = _storageFee;
    }

    function getFeeRecipients(uint256 id) public override virtual view returns (address payable[] memory) {
        RoyaltyFee[] memory _fees = royaltyFees[id];
    ...
    function pause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "ANIFIE721: must have pauser role to pause");
        _pause();
    }

    function unpause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "ANIFIE721: must have pauser role to unpause");
        _unpause();
    }
