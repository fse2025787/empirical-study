...
/**
 * @title NftTestnet contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract PaperBagBoys is ERC721, Ownable {
    ...
    uint256 public SALE_PRICE = 40000000000000000;
    uint256 public SALE_MINT = 20;
    uint256 public SALE_SUPPLY = 4444;
    // PreSale
    bool public PRESALE_ACTIVE;
    uint256 public PRESALE_MINT = 10;
    uint256 public PRESALE_SUPPLY = 1333;
    uint256 public PRESALE_PRICE  = 30000000000000000;
    mapping(address => bool) public whiteList;
    mapping(address => uint256) public whiteListClaimed;
    // promotion
    bool private PROMOTION_ACTIVE;
    uint256 private PROMOTION_BUY = 4;
    uint256 private PROMOTION_GET = 5;
    // Others
    address private artistAddress = 0x32edd4E815400210e94BAC0ddd9284895992125a;

    constructor() ERC721("Paper Bag Boys", "PBB") {
        string memory _baseURI = "ipfs://QmQXL6m6E2zkhcG7Vfcf1H7pbmkJPh6Bnam7BfuJMTmdYR/";
    ...
    */
    ...
    function bulkGiveaway(address[] calldata addresses, uint256 _numberOfTokens) external  {
        require(msg.sender == owner() || msg.sender == address(artistAddress), "Not team!");
        for (uint256 i = 0; i < addresses.length; i++){
            giveaway(addresses[i], _numberOfTokens);
        }
    }
    ...
    */
    ...
    function addToWhiteList(address[] calldata _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            require(_addresses[i] != address(0), "Null address found");
    ...
    function mintPreSale(uint256 _numberOfTokens) external payable {
        ...
        uint256 ownerMintedCount = whiteListClaimed[msg.sender];
        require(ownerMintedCount + _numberOfTokens <= PRESALE_MINT, "Max mint exceeded");
        require(totalSupply().add(promotionCalcul(_numberOfTokens)) <= PRESALE_SUPPLY, "Purchase would exceed max supply");
    ...
    */
    ...
    function saleState() external view returns (bool, uint256, uint256, uint256, bool, uint256, uint256, uint256, uint256) {
        return (
            PRESALE_ACTIVE,
            PRESALE_PRICE,
            PRESALE_MINT,
    ...
    function saleState(address _address) external view returns (bool, uint256, uint256, uint256, bool, uint256, uint256, uint256, uint256, bool) {
        bool isWhitelist = whiteList[_address];
        return (
            PRESALE_ACTIVE,
            PRESALE_PRICE,
            PRESALE_MINT,
