...
contract KevinwowContract is Ownable, ERC721A, ReentrancyGuard {
 ...
 constructor(
    uint256 maxBatchSize_,
    uint256 TotalToken_
  ) ERC721A("Kevin WOW", "Kevin WOW", maxBatchSize_, TotalToken_) {
    MaxPerTx = maxBatchSize_;
    TotalTokenSize = TotalToken_; 
    claimIsActive = false;
    price =   0;
    MaxClaimToken = 555;
    MaxClaimTokenPerwallet = 2;
  }

  /** SETTERS **/

  function SetMaxClaimTokenPerwallet(uint _MaxClaimTokenPerwallet) external onlyOwner {
    MaxClaimTokenPerwallet = _MaxClaimTokenPerwallet;
  }   
  ...
  function Claim(uint256 mintamount) external payable {
    require(claimIsActive, "Claim is not started");
    require(mintamount > 0 && mintamount <= MaxPerTx, "Can not mint more than 2 token per tx");  
    require(freeMintCountMap[msg.sender] + mintamount <= MaxClaimTokenPerwallet, "Can not mint more than 2 token per wallet");     
