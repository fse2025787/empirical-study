...
abstract contract FixedTokenTimelockUpgradeable is TokenTimelockUpgradeable {
    ...
    function _release(address to, uint256 amount) internal override {
        require(amount <= totalToken(), "balance");
...
contract FixedQuadraticTokenTimelockUpgradeable is Initializable, FixedTokenTimelockUpgradeable {
    ...
    function _proportionAvailable(
        ...
        uint year = 31536000;
        if (elapsed <= year) return elapsed * 1e18 / year * initialBalance / 10 / 1e18; // 10% of the initial balance
        else if (elapsed <= 2 * year) return elapsed * 1e18 / (2 * year) * initialBalance / 4 / 1e18; // 25%
        else if (elapsed <= 3 * year) return elapsed * 1e18 / (3 * year) * initialBalance * 45 / 100 / 1e18; // 45%
        else if (elapsed <= 4 * year) return elapsed * 1e18 / (4 * year) * initialBalance * 7 / 10 / 1e18; // 70 %
        else if (elapsed < 5 * year) return elapsed * 1e18 / (5 * year) * initialBalance / 1e18;
