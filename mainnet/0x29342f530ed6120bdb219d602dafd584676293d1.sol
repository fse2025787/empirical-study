...
// hevm: flattened sources of src/clip.sol
// 
...
interface SpotterLike {
    ...
    function ilks(bytes32) external returns (PipLike_1, uint256);
}
...
contract Clipper {
    ...
    VatLike_2  immutable public vat;   // Core CDP Engine

    DogLike_1     public dog;      // Liquidation module
    ...
    function file(bytes32 what, address data) external auth lock {
        ...
        else if (what == "dog")    dog = DogLike_1(data);
    ...
    function getFeedPrice() internal returns (uint256 feedPrice) {
        (PipLike_1 pip, ) = spotter.ilks(ilk);
    ...
    function upchost() external {
        (,,,, uint256 _dust) = VatLike_2(vat).ilks(ilk);
