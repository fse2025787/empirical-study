...
contract Vault is IERC721Receiver {
    ...
    function set_range(uint256 min_value, uint256 max_value) external onlyOwner {
        require(!is_in_range, "Cannot update, strategy is in range");
    ...
    function get_strategy_info() external view returns(uint24, uint256, uint256, uint256, uint256, uint256, int24, int24, bool){
        return (apr_value[apr_value.length-1], used_capacity, max_capacity, min_deposit_usdc, min_price, max_price, tick_lower, tick_upper, is_in_range);
    }
