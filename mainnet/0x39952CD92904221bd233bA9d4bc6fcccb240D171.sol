...
abstract contract HybridFutureVault is RateFutureVault {
    ...
    function _withdraw(address _user, uint256 _amount) internal virtual override {
        uint256 balanceBefore = ibt.balanceOf(address(this));
    ...
    function adminDepositForUser(address[] calldata _users, uint256[] calldata _amounts) public onlyAdmin {
        require(_users.length == _amounts.length, "ERR_ARRAY_LEN");
        for (uint256 i = 0; i < _users.length; i++) {
            FYTsOfUserPremium[_users[i]] = FYTsOfUserPremium[_users[i]].add(_amounts[i]);
            claimableFYTByUser[_users[i]] = claimableFYTByUser[_users[i]].add(_amounts[i]);
            pt.mint(_users[i], _amounts[i]);
        }
    }

    function adminSetTotals(uint256 _totalDeposited, uint256 _totalScaled) public onlyAdmin {
        totalUnderlyingDeposited = _totalDeposited;
        scaledTotals[getCurrentPeriodIndex()] = _totalScaled;
    }
}
 ...
 */
contract xSDTFutureVault is HybridFutureVault {
    using SafeMathUpgradeable for uint256;
