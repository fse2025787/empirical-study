...
contract RockyDoge is Context, IERC20, Ownable {
    ...
    string private _name = 'RockyDoge';
    string private _symbol = 'RDOGE';
    uint8 private _decimals = 9;

    uint256 private _taxFee = 4; // 4% reflection fee for every holder
    uint256 private _marketingFee = 3; // 3% marketing
    ...
    address payable private _marketingWalletAddress = payable(0x6573d0417953545730d4A478Cf25D80B86c1289d);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && sender != uniswapV2Pair && !inSwapAndLiquify) {
            require(amount <= balanceOf(uniswapV2Pair).mul(50).div(100), "Sorry the amount you tried to sell affects the price by more than 50%");
