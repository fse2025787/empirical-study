...
// File: https://github.com/MetaplasiaTeam/castable-nft/blob/main/constracts/ERC721/IERC721Castable.sol
...
// File: contracts/castable-nft.sol
...
contract CastableNFT is ERC721, Ownable,IERC721Castable {
    ...
    function get(uint256 _tokenId) external view existToken(_tokenId) returns(Collectible memory) {
        return _collectibles[_tokenId];
    }

    function getByOwner(address _owner) external view returns(Collectible[] memory) {
    ...
    * @dev create a collection minted using ERC721 tokens.
    ...
    function mintByERC721(address _addr, uint256 _id, string memory _uri,uint256 _burnTime) external returns (uint256) {
        ...
        require(erc721Token.getApproved(_id) == address(this), "Token not approved!");
    ...
    * @dev create multiple collectibles minted using ERC721 tokens.
    ...
    function mintByERC721Custom(address _addr, string[] memory _uris, uint256[] memory _ids,uint256 _burnTime) external returns (uint256[] memory) {
        ...
        uint256[] memory ids = new uint[](_uris.length);
        for (uint256 i = 0; i < _uris.length; i++) {
            require(erc721Token.getApproved(_ids[i]) == address(this), "Token not approved!");
     ...
     */
    function burn(uint256 _tokenId) existToken(_tokenId) external {
        require(_isApprovedOrOwner(msg.sender, _tokenId), "ERC721: transfer caller is not owner nor approved");
        Collectible memory collectible = _collectibles[_tokenId];
        require(block.timestamp >= collectible.burnTime, "It's not reach time to burn!");
        _burn(_tokenId);
