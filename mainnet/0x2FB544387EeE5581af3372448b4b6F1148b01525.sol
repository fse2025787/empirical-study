...
interface IWhalezAuctionHouse {
    struct Auction {
        // ID for the Whalez (ERC721 token ID)
        uint256 whaleId;
    ...
    event AuctionCreated(uint256 indexed whaleId, uint256 startTime, uint256 endTime);

    event AuctionBid(uint256 indexed whaleId, address sender, uint256 value, bool extended);

    event AuctionExtended(uint256 indexed whaleId, uint256 endTime);

    event AuctionSettled(uint256 indexed whaleId, address winner, uint256 amount);
    ...
    function createBid(uint256 whaleId) external payable;
