...
contract PlayerZeroToken is ERC721A, Ownable, ReentrancyGuard {
    using Strings
    for uint256;
   ...
   //@notice admin wallet
    ...
    //@notice future auction contract, set by dev
    ...
    uint256 public MAX_SUPPLY_ALLOWLIST = 50;
    uint256 public PRICE_PER_TOKEN = 5 ether;
    ...
    constructor() ERC721A("PlayerZeroFounders", "PZFP") {}

    //@notice mint presale, requires added to allowed to mint array
    function mintPresale(uint256 _tokenAmount) external payable nonReentrant {
        uint256 ts = totalSupply();
        require(isAllowlistActive,"Presale is not Active");
        require(isAllowedToMint[msg.sender],"Ineglible to Mint");
        require(msg.value >= PRICE_PER_TOKEN * _tokenAmount, "Ether value sent not correct");
        require(_tokenAmount <= maxMintPerWallet, "Purchase would exceed max tokens per tx in this phase");
        require(ts + _tokenAmount <= MAX_SUPPLY_ALLOWLIST, "Purchase would exceed max tokens");
    ...
    //@notice mint to auction contract
    function mintToken() external  {
        uint256 ts = totalSupply();
        require(isAuctionActive);
        require(tx.origin == auctionContract || msg.sender == auctionContract);
        require(ts + 1 <= MAX_SUPPLY, "Purchase would exceed max tokens");
        _safeMint(auctionContract, 1);
    }

    //@notice airdrop token, only owner
    ...
    //@notice add wallets to allowlist for presale
	function allowlistSet(address[] calldata _addresses) public onlyOwner {
        for(uint256 i; i < _addresses.length; i++){
            isAllowedToMint[_addresses[i]] = true;
        }
	}

    //@notice remove wallets from  allowlist for presale
	function allowlistRemove(address[] calldata _addresses) public onlyOwner {
        for(uint256 i; i < _addresses.length; i++){
            isAllowedToMint[_addresses[i]] = false;
        }
	}

    //@notice set new auction contract
    function setAuctionContract(address _contract) external onlyOwner {
    ...
    //@notice set auction active
    function setAuctionLive(bool _status) external onlyOwner {
        isAuctionActive = _status;
    }
