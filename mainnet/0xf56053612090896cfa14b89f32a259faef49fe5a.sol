...
// File contracts/interfaces/Liquity/ILiquityStakingPool.sol
...
// File contracts/libraries/uni/IUniswapV2Pair.sol
...
// File contracts/adapters/LiquityStakingPoolAdapterV4.sol
...
contract LiquityStakingPoolAdapterV4 is IVaultAdapterV2 {
  ...
  IUniswapV2Pair public wethLusdPair;

  
  IUniswapV2Pair public lqtyWethPair;
  ...
  uint256 public slippage; //divied by 10000

  constructor(ILiquityStakingPool _vault, address _admin, address _sentinel, IUniswapV2Router02 _uniV2Router, IDetailedERC20 _lqtyToken, IDetailedERC20 _lusdToken, IDetailedERC20 _wethToken, IUniswapV2Pair _lqtyWethPair, IUniswapV2Pair _wethLusdPair, address _frontendTag, uint256 _slippage) public {
    vault = _vault;
    admin = _admin;
    sentinel = _sentinel;
    ...
    lqtyWethPair = _lqtyWethPair;
    wethLusdPair = _wethLusdPair;
  ...
  modifier onlyAdmin() {
    require(admin == msg.sender, "LiquityStakingPoolAdapterV4: only admin");
    _;
  }

  modifier onlySentinel() {
    require(sentinel == msg.sender, "LiquityStakingPoolAdapterV4: only sentinel");
    _;
  }
  ...
  function withdraw(address _recipient, uint256 _amount, bool _isHarvest) external override onlyAdmin {
    ...
    if(_isHarvest){
      ...
      if(lqtyToken.balanceOf(address(this)) > 0){
        ...
        uint256 minLusdAmountOut = 0;
        (uint112 reservePartOne0, uint112 reservePartOne1, ) = lqtyWethPair.getReserves();
        (uint112 reservePartTwo0, uint112 reservePartTwo1, ) = wethLusdPair.getReserves();

        if(lqtyWethPair.token0() == address(wETHToken)){

          if(wethLusdPair.token0() == address(wETHToken)){
            minLusdAmountOut = uniV2Router.getAmountOut(uniV2Router.getAmountOut(lqtyToken.balanceOf(address(this)),reservePartOne1,reservePartOne0),reservePartTwo0,reservePartTwo1);
          } else if(wethLusdPair.token1() == address(wETHToken)) {
            minLusdAmountOut = uniV2Router.getAmountOut(uniV2Router.getAmountOut(lqtyToken.balanceOf(address(this)),reservePartOne1,reservePartOne0),reservePartTwo1,reservePartTwo0);
          }

        } else if(lqtyWethPair.token1() == address(wETHToken)) {
          if(wethLusdPair.token0() == address(wETHToken)){
            minLusdAmountOut = uniV2Router.getAmountOut(uniV2Router.getAmountOut(lqtyToken.balanceOf(address(this)),reservePartOne0,reservePartOne1),reservePartTwo0,reservePartTwo1);
          } else if(wethLusdPair.token1() == address(wETHToken)) {
            minLusdAmountOut = uniV2Router.getAmountOut(uniV2Router.getAmountOut(lqtyToken.balanceOf(address(this)),reservePartOne0,reservePartOne1),reservePartTwo1,reservePartTwo0);
          }
        }

        uniV2Router.swapExactTokensForTokens(lqtyToken.balanceOf(address(this)),
                                             minLusdAmountOut.mul(slippage).div(10000),
                                             _pathlqty,
                                             address(this),
                                             block.timestamp+800);
      }
      ...
      if(address(this).balance > 0){
        ...
        (uint112 reserve0, uint112 reserve1, ) = wethLusdPair.getReserves();
        if(wethLusdPair.token0() == address(wETHToken)){
          minAmountOut = uniV2Router.getAmountOut(address(this).balance,reserve0,reserve1);
        } else if(wethLusdPair.token1() == address(wETHToken)) {
          minAmountOut = uniV2Router.getAmountOut(address(this).balance,reserve1,reserve0);
        }
        uniV2Router.swapExactETHForTokens{value:address(this).balance}(minAmountOut.mul(slippage).div(10000),
                                                                       _pathEthLusd,
                                                                       address(this),
                                                                       block.timestamp+800);
      }
  ...
  function updateSlippage(uint256 newSlippage) public onlySentinel{
    slippage = newSlippage;
  }

  function handoverSentinel(address newSentinel) public onlySentinel{
    require(newSentinel != address(0), "LiquityStakingPoolAdapterV4: sentinel address cannot be 0x0.");
