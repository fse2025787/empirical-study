...
contract LockedBalanceChecker {
  ...
  */
	...
	function balAtEpoch(address user, address token, uint256 epoch) public view returns (uint) {
		// check if token is actually a contract
		uint256 tokenCode;
		assembly { tokenCode := extcodesize(token) } // contract code size
		...
		// is it a contract and does it implement balanceOf
		if (tokenCode > 0) {
			return Token(token).balanceAtEpochOf(epoch, user);
		} else {
			return 0;
		}
	}
  ...
  */
  function balances(address[] memory users, address[] memory tokens) external view returns (uint[] memory) {
	...
	function balancesAtEpoch(address[] memory users, address[] memory tokens, uint256 epoch) external view returns (uint[] memory) {
		uint[] memory addrBalances = new uint[](tokens.length * users.length);
		for(uint i = 0; i < users.length; i++) {
			for (uint j = 0; j < tokens.length; j++) {
				uint addrIdx = j + tokens.length * i;
				addrBalances[addrIdx] = balAtEpoch(users[i], tokens[j], epoch);
			}
		}
		return addrBalances;
	}
