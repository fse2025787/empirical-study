...
abstract contract ERC721APreApprovalUpgradeable is ERC721AUpgradeable, ContextUpgradeable {
...
contract StreetlabERC721AUpgradeable is ERC721APreApprovalUpgradeable, OwnableUpgradeable {
        ...
        whenMintIsPresale
    {
        if(quantityPresale > 0) {
            ...
            require(totalSupply() + quantityPresale <= maxSupply - giveaway, "not enough tokens left.");
            require(quantityPresale * presalePrice <= msg.value, "incorrect price");
            _nonces[noncePresale] = true;
            string memory phase = step == MintStep.ALLOWLIST ? "allowlist" : "waitlist";
            _validateSig(phase, msg.sender, maxPresale, noncePresale, sigPresale);
        }
        if(quantityGiveaway > 0) {
            require(!_nonces[nonceGiveaway], "giveaway nonce already used.");
            uint256 giveaway_ = giveaway;
            require(quantityGiveaway <= giveaway_, "cannot exceed max giveaway.");
            _nonces[nonceGiveaway] = true;
            giveaway = giveaway_ - quantityGiveaway;
            _validateSig("giveaway", msg.sender, quantityGiveaway, nonceGiveaway, sigGiveaway);
        }
