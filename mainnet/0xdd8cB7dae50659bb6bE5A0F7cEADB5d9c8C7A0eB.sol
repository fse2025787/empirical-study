...
contract phoenix is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    constructor() ERC721("ashes", "phoenix") {}
    ...
    function setPriceOnConditions() internal {
        ...
        if (IndexOfMint.current() < 5 && cost != 90000000000000000 && !publicSale) {
            cost = 0.009 ether;
        }
        if (IndexOfMint.current() > 4 && cost != 10000000000000000 && !publicSale) {
            cost = 0.01 ether;
        }
        if (publicSale && cost != 15000000000000000) {
            cost = 0.015 ether;
        }
    }
