...
contract CyberManufactureCoNFT is ERC1155 {
    ...
    event CyberManufactureCoTransfer(uint256 id, uint256 amount, address from, address to);
    event CyberManufactureCoSetup(uint32 coolingPeriod_, string creator_);

    // not using default uri scheme
    constructor(uint32 coolingPeriod_, string memory creator_) ERC1155(""){
        coolingPeriod = coolingPeriod_;
        creator = creator_;
        _admin = msg.sender;
        emit CyberManufactureCoSetup(coolingPeriod_, creator_);
    }
    ...
    function mint(uint256 id, uint256 amount, string memory metadataUri, address initialOwner, bytes calldata data) public isAdmin() nonExist(id) {
    ...
    function _beforeTokenTransfer(
        ...
        for (uint i = 0; i < ids.length; i++) {
            emit CyberManufactureCoTransfer(ids[i], amounts[i], from, to);
        }
