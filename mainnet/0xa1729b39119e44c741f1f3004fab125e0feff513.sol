...
// ----------------------------------------------------------------------------
// ERC-721 Helper v0.9.5
//
// https://github.com/bokkypoobah/TokenToolz
//
// Deployed to Mainnet 0xA1729b39119E44c741F1f3004faB125e0feFF513
//
// 
//
// Enjoy.
...
contract ERC721Helper {
    ...
    bytes4 private constant FUNC_SELECTOR_ERC20_TOTALSUPPLY = bytes4(keccak256("totalSupply()"));
    bytes4 private constant FUNC_SELECTOR_ERC20_BALANCEOF = bytes4(keccak256("balanceOf(address)"));
    bytes4 private constant FUNC_SELECTOR_ERC20_SYMBOL = bytes4(keccak256("symbol()"));
    bytes4 private constant FUNC_SELECTOR_ERC20_NAME = bytes4(keccak256("name()"));
    ...
    function hasERC20TotalSupply(address _token) internal view returns (bool) {
        bool success;
        bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR_ERC20_TOTALSUPPLY);
    ...
    function hasERC20BalanceOf(address _token, address _tokenOwner) public view returns (bool) {
        bool success;
        bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR_ERC20_BALANCEOF, _tokenOwner);
        uint256 _gas = gasleft();
        assembly {
            success := staticcall(
                _gas,           // gas remaining
    ...
    function hasERC20Symbol(address _token) public view returns (bool) {
        bool success;
        bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR_ERC20_SYMBOL);
        uint256 _gas = gasleft();
        assembly {
            success := staticcall(
                _gas,           // gas remaining
    ...
    function hasERC20Name(address _token) public view returns (bool) {
        bool success;
        bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR_ERC20_NAME);
        uint256 _gas = gasleft();
        assembly {
            success := staticcall(
                _gas,           // gas remaining
    ...
    function isERC20(address token) internal view returns (bool b) {
        return hasERC20TotalSupply(token) && hasERC20BalanceOf(token, msg.sender);
    }
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            if (token.code.length == 0) {
                ...
                if (isERC721(token)) {
                    ...
                    status |= ISERC20;
                    if (hasERC20Symbol(token)) {
                        symbols[i] = getERC20Symbol(token);
                    }
                    if (hasERC20Name(token)) {
