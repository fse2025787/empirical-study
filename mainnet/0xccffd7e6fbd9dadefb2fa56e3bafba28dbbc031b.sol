...
contract Jelly is ERC721A, Ownable, ReentrancyGuard {

    mapping (address => uint256) public WalletMint;
    bool public MintStartEnabled  = false;
    uint public MintPrice = 0.004 ether; 
    string public baseURI;  
    uint public freeMint = 0;
    uint public maxMintPerTx = 15;  
    uint public maxSupply = 600;

    constructor() ERC721A("Alpha Jelly Cubes", "Alpha Jelly Cubes",69,6969)
    {
        baseURI = "ipfs://QmZ5hwM42R3Eq5LKmvRNMstpw3g1qQxauYw3KQNTuGaHyA/";
    ...
    function mint(uint256 qty) external payable
    {
        ...
        require(totalSupply() + qty <= maxSupply,"Info:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"Info:  Fund not enough");
