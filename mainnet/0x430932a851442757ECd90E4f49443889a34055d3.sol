    ...
    ReentrancyGuard
{
    ...
    constructor(
        ...
        allowMultiMint = _allowMultiMint;
        name = _name;
        symbol = _symbol;
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(super.uri(_tokenId), tokenSuffix[_tokenId])
            );
    }
    ...
    function mint(
        ...
        if (allowMultiMint) {
            require(
                ...
                "ERC1155Invoke: token minted with different uri"
            );
        } else {
            require(
                bytes(tokenSuffix[_id]).length == 0,
