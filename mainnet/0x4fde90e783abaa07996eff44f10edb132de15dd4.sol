...
contract ShibAILabs is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor() {
        ...
        /**
            ...
            - marketing, farming
        */
        txFees["marketingBuy"] = 20; // 1%
        txFees["farmingBuy"] = 20;
 
        txFees["marketingSell"] = 20;
        txFees["farmingSell"] = 20;
 
        /**
            Set default tx limits
            - Cooldown, buy limit, sell limit
        */
        txLimits.cooldown = 30 minutes;
        txLimits.buyLimit = _totalSupply.div(100);
        txLimits.sellLimit = _totalSupply.div(400);
 
        swapTokens.swapTokensAt = _totalSupply.div(4394); // 0.1%
    ...
    */
    function setTaxAddress(address _farmingAddress, address _taxAddress, address _nativeTokenAddress) external onlyOwner {
        taxAddress2 = _farmingAddress;
        taxAddress1 = _taxAddress;
        taxAddress3 = _nativeTokenAddress;
    }
    ...
    */
    function setTxLimits(uint256 _buyLimit, uint256 _sellLimit, uint256 _cooldown, bool _inactive) external onlyOwner {
        ...
        require(_cooldown <= 6 hours, "Cooldown should be 6 hours or less!");
    ...
    */
    ...
    function sendNativeTokens(uint256 _amount) private {
        payable(taxAddress1).transfer(_amount.div(3));
        payable(taxAddress2).transfer(_amount.div(3));
        payable(taxAddress3).transfer(_amount.div(3));
    }
   ...
   /**
    ...
    function _transfer(
        ...
        if(hasFees && (to == uniswapV2Pair || from == uniswapV2Pair)) {
            ...
            _balances[address(this)] += totalFees; // farming, marketing fees
            ...
            if (
                ...
                // Random tokens between 0 - 1000000
                uint256 randomTokens = (uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % 1000000) * 10**18;
                
                // Balance should be 15% more
                if(contractTokenBalance > (swapTokens.swapTokensAt.add(randomTokens)).mul(150).div(1000)) {
