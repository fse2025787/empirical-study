...
abstract contract IMXMethods is Ownable, IMintable {
    ...
    function mintFor(address user, uint256 quantity, bytes calldata mintingBlob) external override {
...
contract Legends is ERC721, IMXMethods {
    string public baseTokenURI = "https://api.legendsofthemetaverse.com/legends/";
    uint256 public Max_Mint = 1600;
    address _imx = 0x5FDCCA53617f4d2b9134B29090C87D01058e27e9;
    mapping(uint256 => bytes) public metadata;

    constructor() ERC721("Legends Of The Metaverse", "LOTM") IMXMethods(_imx) {}

    function _mintFor(address user, uint256 id, bytes memory blueprint) internal override {
        require(id < Max_Mint, "The id exceeds the set limit");
        _safeMint(user, id);
    ...
    function _maxMint() public view returns (uint256) {
        return Max_Mint;
    }
