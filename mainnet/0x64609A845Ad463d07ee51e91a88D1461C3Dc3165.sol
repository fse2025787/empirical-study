...
contract Ridge is IERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize () external initializer {
        string memory name_ = "Ridge" ;                             
        string memory symbol_ = "RIDGE";
        uint256 totalSupply_ = 1000000000000* 10**9;                                // 1000000000000 -> 1 trillion
        address owner_ = 0x0638eaA4FcCbe1b3fCd532D03162D39F9698ac49;                // Owner address.
        address router_ = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;              // Uniswap Router
        address charityAddress_ = 0x8D4FEAA1a03F2136B8C9C9dd736171214a86753d;       // Charity wallet addresss.
        address marketingAddress_ = 0xA60b690237F39997681bd3551998c47F7747fCc6;     // Marketing wallet address.
        uint16 taxFeeBps_ = 100;                                                    // Tax fee.
        uint16 liquidityFeeBps_ = 400;                                              // Liquidity fee.
        uint16 charityFeeBps_ = 100;                                                // Charity fee.
        uint16 marketingFeeBps_ = 600;                                              // Mrketing fee.
        uint16 maxTxBps_ = 400;                                                     // Max transaction.
        ...
        require(marketingFeeBps_ >= 0 && marketingFeeBps_ <= 10**4, "Invalit marketing fee.");
        require(maxTxBps_ > 0 && maxTxBps_ <= 10**4, "Invalid max tx amount.");
        require(taxFeeBps_ + liquidityFeeBps_ + charityFeeBps_  + marketingFeeBps_ <= 10**4, "Total fee is over 100% of transfer amount.");
        ...
        // Exclude Owner, Dead, Contract, Marketing, Charity from max TX amount.
        _isExcludedFromMaxTx[address(this)] = true;
        _isExcludedFromMaxTx[address(0xdead)] = true;
        _isExcludedFromMaxTx[address(0)] = true;
        _isExcludedFromMaxTx[address(_marketingAddress)] = true;
        _isExcludedFromMaxTx[address(_charityAddress)] = true;
    ...
    function deliver(uint256 tAmount) public {
        ...
        (uint256 rAmount,,) = _getRValues(tAmount, tFee, tLiquidity, tCharity, tMarketing, _getRate());
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tMarketing) = _getTValues(tAmount);
            (,uint256 rTransferAmount,) = _getRValues(tAmount, tFee, tLiquidity, tCharity, tMarketing, _getRate());
    ...
    function excludeFromReward(address account) public onlyOwner() {
        // require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'We can not exclude Uniswap router.'); // TODO: check this one up
    ...
    function setMarketingWallet(address account) external onlyOwner() {
        if (account == address(0)) {
            require(_marketingFee == 0, "Cant set both marketing address to address 0 while markeing percent more than 0.");
        }
    ...
    function setCharityWallet(address account) external onlyOwner() {
        if (account == address(0)) {
            require(_charityFee == 0, "Cant set both charity address to address 0 while charity percent more than 0.");
        }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        return (tFee, tLiquidity, tCharityFee, tMarketingFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // If any account belongs to _isExcludedFromFee account, remove the fee.
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Include only swap created ETH and exclude manually transfered ETH.
    ...
    // This method is responsible for taking all fee, if takeFee is set to true.
