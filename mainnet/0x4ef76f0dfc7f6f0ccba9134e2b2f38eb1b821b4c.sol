...
contract Tsukanomics is Context, IERC20, Ownable {
    ...
    uint256 public buyThreshold = _totalSupply;
    uint256 public sellThreshold = _totalSupply;
    uint256 public walletThreshold = _totalSupply;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != ZERO && to != DEAD && !_swapping) {
            ...
            if (from == _uniswapV2Pair && to != address(_uniswapV2Router) && !_isExcludedMaxTransactionAmount[to]) {
                require(amount <= buyThreshold, "TNOMICS: Transfer amount exceeds the buyThreshold.");
                require(balanceOf(to) + amount <= walletThreshold, "TNOMICS: Exceeds maximum wallet token amount.");
            }
            
            if (to == _uniswapV2Pair && from != address(_uniswapV2Router) && !_isExcludedMaxTransactionAmount[from]) {
                require(amount <= sellThreshold, "TNOMICS: Transfer amount exceeds the sellThreshold.");
                shouldSwap = true;
            }
        }
        ...
        if (canSwap && swapEnabled && !_swapping && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (contractTokenBalance == 0 || _tokensForFee == 0) return;
            if (contractTokenBalance > _swapTokensAtAmount * 5) contractTokenBalance = _swapTokensAtAmount * 5;
        ...
        if (!takeFee) {
            ...
            if (fees > 0) _transferStandard(to, address(this), fees);
    ...
    function launch() public onlyOwner {
        ...
        swapEnabled = true;
        buyThreshold = _totalSupply.mul(15).div(1000);
        sellThreshold = _totalSupply.mul(15).div(1000);
        walletThreshold = _totalSupply.mul(15).div(1000);
        _swapTokensAtAmount = _totalSupply.mul(5).div(10000);
        tradingOpen = true;
    }

    function setBuyThreshold(uint256 maxBuy) public onlyOwner {
        require(maxBuy >= (totalSupply().mul(1).div(100)), "TNOMICS: Max buy amount cannot be lower than 1% total supply.");
        buyThreshold = maxBuy;
    }

    function setSellThreshold(uint256 maxSell) public onlyOwner {
        require(maxSell >= (totalSupply().mul(1).div(100)), "TNOMICS: Max sell amount cannot be lower than 1% total supply.");
        sellThreshold = maxSell;
    }
    
    function setWalletThreshold(uint256 maxToken) public onlyOwner {
        require(maxToken >= (totalSupply().mul(1).div(100)), "TNOMICS: Max wallet amount cannot be lower than 1% total supply.");
        walletThreshold = maxToken;
    }
