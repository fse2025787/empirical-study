...
contract JiggyContractFlat is ERC721, Ownable, DateTime {
  using Strings for uint256;
  ...
  ) ERC721("The Jiggys", "JIGGY") {
    setBaseURI(_initBaseURI);
    setMintDate(2022, 1, 26, 19, 0);
    setWMintDate(2022, 1, 25, 19, 0);
    // mintInternal(msg.sender, 20);
  }

  function setInitialTokenId() public onlyOwner {
    require(initialTokenId <= 0, "Starting index is already set");
    initialTokenId = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % maxSupply + 1;
  }
 ...
 //public dates 
  ...
  function mintInternal(address _to, uint256 _mintAmount) internal {
    require(!paused);
    require(tokenId + _mintAmount <= maxSupply);
    for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(_to, (tokenId + i) % maxSupply);
    }
    tokenId += _mintAmount;
  }
  ...
  function burn(uint256 _tokenId) public virtual payable {
