...
contract WCATokensAggregator is Ownable {
	...
	function balanceOf(address _address) external view returns (uint256) {
		return Token.balanceOf(_address) + Restake.getTotalClaimable(_address) + Whitelist.getTotalClaimable(_address) + ICO.getTotalClaimable(_address) + getMundialStakingAvailableRewards(_address) + MundialRewards.getAvailableTokens(_address);
	}

	function stakedBalanceOf(address _address) external view returns (uint256) {
		return Restake.getTotalClaimable(_address) + Whitelist.getTotalClaimable(_address) + ICO.getTotalClaimable(_address) + getMundialStakingAvailableRewards(_address) + MundialRewards.getAvailableTokens(_address);
	}
