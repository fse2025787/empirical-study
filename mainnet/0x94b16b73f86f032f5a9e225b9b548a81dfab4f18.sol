...
OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
    ...
    event BurnByOwner(address to, uint256 amount);

    event BurnArrayByOwner(address msgSender, UserInfo[] array);
    ...
    function burnByOwner(address to, uint256 amount) public onlyOwner {
        _burn(to, amount);
        emit BurnByOwner(to, amount);
    }

    function burnArrayByOwner(UserInfo[] calldata array) public onlyOwner {
        uint256 arrayLen = array.length;
        require(arrayLen > 0, "data error");
        for(uint256 i = 0;i < arrayLen;i++) {
            UserInfo memory userInfo = array[i];
            _burn(userInfo.user, userInfo.amount);
        }
        emit BurnArrayByOwner(msg.sender, array);
    }
