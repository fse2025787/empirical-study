...
contract IndexRouter is IIndexRouter, UUPSUpgradeable {
    ...
    bytes32 internal INDEX_ROLE;
    
    bytes32 internal ASSET_ROLE;
    
    bytes32 internal EXCHANGE_ADMIN_ROLE;
    
    bytes32 internal SKIPPED_ASSET_ROLE;
    
    bytes32 internal EXCHANGE_TARGET_ROLE;
    ...
    function initialize(address _WETH, address _registry) external override initializer {
        ...
        EXCHANGE_ADMIN_ROLE = keccak256("EXCHANGE_ADMIN_ROLE");
        SKIPPED_ASSET_ROLE = keccak256("SKIPPED_ASSET_ROLE");
        EXCHANGE_TARGET_ROLE = keccak256("EXCHANGE_TARGET_ROLE");
        ...
        for (uint i; i < totalAssetsCount; ) {
            ...
            if (inputAssetBalanceBefore > 0 && address(inputAsset) != _params.outputAsset) {
                ...
                // otherwise exchange the asset for the desired output asset
                    ...
                    require(
                        inputAsset.balanceOf(address(this)) == 0 &&
