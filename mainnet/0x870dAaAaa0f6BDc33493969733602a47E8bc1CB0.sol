...
contract TalesOfDorin is ERC721A, Ownable, ReentrancyGuard {
    ...
    // phase 2 allowance data
    mapping(address => bool) public phase2FreeMintAllowed;
    mapping(address => uint256) public phase2PaidMintQuota;
    ...
    function addToWhitelist(MINT_PHASE _mintPhase, address[] memory _whitelistedAddresses) external onlyOwner {
        ...
        if (_mintPhase == MINT_PHASE.FIRST) {
            ...
            for (uint256 i = 0; i < _whitelistedAddresses.length; i++) {
                phase2FreeMintAllowed[_whitelistedAddresses[i]] = true;
                phase2PaidMintQuota[_whitelistedAddresses[i]] = 2;
            }
        }
    }

    // Remove addresses from whitelist for a phase
    //    function addToWhitelist(MINT_PHASE _mintPhase, address[] memory _whitelistedAddresses) external onlyOwner {
    //        require(_mintPhase == MINT_PHASE.FIRST || _mintPhase == MINT_PHASE.SECOND, "Invalid mint phase");
    //
    //        if (_mintPhase == MINT_PHASE.FIRST) {
    //            for (uint256 i = 0; i < _whitelistedAddresses.length; i++) {
    //                phase1FreeMintAllowed[_whitelistedAddresses[i]] = false;
    //                phase1PaidMintQuota[_whitelistedAddresses[i]] = 0;
    //            }
    //        } else if (_mintPhase == MINT_PHASE.SECOND) {
    //            for (uint256 i = 0; i < _whitelistedAddresses.length; i++) {
    //                phase2FreeMintAllowed[_whitelistedAddresses[i]] = false;
    //                phase2PaidMintQuota[_whitelistedAddresses[i]] = 0;
    //            }
    //        }
    //    }
    ...
    function getMintCountAllowedForAddress(address _user) public view returns (MintCountAllowed memory) {
        if (PHASE == MINT_PHASE.FIRST) {
            ...
            return MintCountAllowed(phase2FreeMintAllowed[_user] ? 1 : 0, phase2PaidMintQuota[_user]);
        } else {
    ...
    function _mintPhase2(address user, uint256 amount, uint256 ethPaid) internal {
        require(amount <= (phase2FreeMintAllowed[user] ? 1 : 0) + phase2PaidMintQuota[user],
            "You have reached your max mint quota, please try minting lesser number of tokens");

        uint256 payableTokensCount = amount - (phase2FreeMintAllowed[user] ? 1 : 0);
        uint256 amountPayable = payableTokensCount * PUBLIC_PRICE;
        ...
        phase2FreeMintAllowed[user] = false;
        phase2PaidMintQuota[user] -= payableTokensCount;
    }
