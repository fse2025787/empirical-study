...
contract InfanityDrop2 is ERC721A, DefaultOperatorFilterer, AccessControl {
    ...
    uint256 public mintPriceInWei = 0 ether;
    uint256 public whitelistMintPriceInWei = 0 ether;
    ...
    constructor(address _tokenGated, string memory _uri) ERC721A("President Davo Nation", "PDN") {
    ...
    function whitelistMint(
        ...
        require(
            _numberOfTokenToMint * whitelistMintPriceInWei == msg.value,
    ...
    function publicMint(uint256 _quantity) public payable {
        ...
        uint256 holdings = IERC721(tokenGated).balanceOf(msg.sender);

        require(holdings > 0, "No First Drop NFT Holding.");
        require(_quantity <= holdings, "exceed mint limit");
        require(
            _totalMinted() + _quantity <= MAX_SUPPLY,
            "Drop: Exceeds max supply."
        );
        require(
            publicMintMinted[msg.sender] + _quantity <= holdings, "Drop: max limit per address exceeded"
        );
        require(
    ...
    function updateTokenGatedNFT(address _tokenGated) external onlyHasRole(ADMIN_ROLE) {
        tokenGated = _tokenGated;
    }
    ...
    function updateWhitelistMintPrice(uint256 _whitelistMintPrice) external onlyHasRole(ADMIN_ROLE) {
        whitelistMintPriceInWei = _whitelistMintPrice;
    }
