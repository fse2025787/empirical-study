...
contract NounishPebbles is ERC721A, Ownable, ReentrancyGuard {
    ...
    address private POINTLESS_PEBBLES;
    ...
    function mint(uint256 quantity) external nonReentrant {
        ...
        uint256 balanceOf = pointLessPebbles.balanceOf(msg.sender);
        
        for (uint i = 0; i < balanceOf; i++) {
            uint256 tokenId = pointLessPebbles.tokenOfOwnerByIndex(msg.sender,i);
            if (isClaimed(tokenId)) {
                alreadyClaimed +=1;
            }
        }

        elegibleForRedemption = balanceOf - alreadyClaimed;

        require(quantity<=elegibleForRedemption, "claiming too many");

        for (uint i = alreadyClaimed; i < alreadyClaimed+quantity; i++) {
            uint256 tokenId = pointLessPebbles.tokenOfOwnerByIndex(msg.sender,i);
            _setClaimed(tokenId);
        }
