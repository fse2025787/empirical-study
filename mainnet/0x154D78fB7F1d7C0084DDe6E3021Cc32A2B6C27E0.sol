...
contract atak721 is  ERC721Upgradeable, OwnableUpgradeable, DefaultOperatorFiltererUpgradeable {
    ...
    function mint(uint256[] calldata tokenIds) external hasSN(msg.sender){
        ...
        for(uint i = 0; i < tokenIds.length;){
            ...
            require(IERC721(SNContract).ownerOf(tokenId) == msg.sender,"You don't own at least one of the tokens you input.");

            _mintInternal(msg.sender, tokenId);
            IERC721(SNContract).transferFrom(msg.sender,burnAddress,tokenId);
