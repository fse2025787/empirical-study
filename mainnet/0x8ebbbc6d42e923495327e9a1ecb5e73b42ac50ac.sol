...
contract Chikara is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Chikara", "CHIKARA") {
        ...
        marketingWallet = address(0xE26dCA30094a2F8E4D531f12c59bAd073BF8EcfE);
    ...
    function updateMaxTxnAmount(uint256 newMaxBuy, uint256 newMaxSell) external onlyOwner {
        require(((totalSupply() * newMaxBuy) / 1000) >= (totalSupply() / 5000), "Can't set lower");
        require(((totalSupply() * newMaxSell) / 1000) >= (totalSupply() / 5000), "Can't set lower");
    ...
    function updateMaxWalletAmount(uint256 newPercentage) external onlyOwner {
        require(((totalSupply() * newPercentage) / 1000) >= (totalSupply() / 5000), "Can't set lower");
    ...
    function updateFees(uint256 _marketingFeeBuy, uint256 _liquidityFeeBuy,uint256 _marketingFeeSell, uint256 _liquidityFeeSell) external onlyOwner{
        ...
        _fees.sellTotalFees = _fees.sellMarketingFee + _fees.sellLiquidityFee;
        require(_fees.buyTotalFees <= 8, "8% maximum");   
        require(_fees.sellTotalFees <= 8, "8% maximum");
