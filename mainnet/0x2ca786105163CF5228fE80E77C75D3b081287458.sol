...
contract PalDelegatorClaimer{
    ...
    */
    function initiate(
        ...
        //Delegate governance power : HPAL version
        IhPAL govToken = IhPAL(underlying);
    ...
    */
    function closeLoan(uint _usedAmount, address _currentBorrower) public motherPoolOnly {
        //Claim the reward from the hPal contract and send them to the PalPool
        IhPAL _hPal = IhPAL(underlying);
        _hPal.claim(type(uint256).max);
        IERC20 _pal = IERC20(_hPal.pal());
        _pal.safeTransfer(motherPool, _pal.balanceOf(address(this)));
    ...
    */
    function killLoan(address _killer, uint _killerRatio) public motherPoolOnly {
        //Claim the reward from the hPal contract and send them to the PalPool
        IhPAL _hPal = IhPAL(underlying);
        _hPal.claim(type(uint256).max);
        IERC20 _pal = IERC20(_hPal.pal());
    ...
    * @return bool : Power Delagation success
    */
    function changeDelegatee(address _delegatee) public motherPoolOnly returns(bool){
        delegatee = _delegatee;
