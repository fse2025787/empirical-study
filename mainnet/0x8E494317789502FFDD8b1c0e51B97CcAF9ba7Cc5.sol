...
contract LFWUtils_ETH {
    ...
    address private aave = 0x7Fc66500c84A76Ad7e9c93437bFc5Ac33E2DDaE9;
    ...
    address private xsushi = 0x8798249c2E607446EfB7Ad49eC89dD1865Ff4272;
    ...
    address private earnXSHIB = 0xB4a81261b16b92af0B9F7C4a83f1E885132D81e4;
    address private earnXLEASH = 0xa57D319B3Cf3aD0E4d19770f71E63CF847263A0b;
    address private earnTBONE = 0xf7A0383750feF5AbaCe57cc4C9ff98e3790202b3;
    ...
    function getLidoStakingInfo(
        ...
        stakingInfo[1] = scInterface.sharesOf(_userAddress)*scInterface.getTotalPooledEther()/scInterface.getTotalShares();

        // [2] and [3]: calculate an optional term to calculate APR for backend
        (uint256 postTotalPooledEther, uint256 preTotalPooledEther, uint256 timeElapsed) = scOracle.getLastCompletedReportDelta();
        stakingInfo[2] = (postTotalPooledEther - preTotalPooledEther) * secondsPerYear;
        stakingInfo[3] = preTotalPooledEther * timeElapsed;

        stakingInfo[4] = 1;
        // [0], and [1] are the stake and reward addresses
        stakingData[0] = weth;
        stakingData[1] = steth;
    }
    ...
    function getAaveStakingInfo(
        ...
        stakingInfo[4] = (scInterface.DISTRIBUTION_END() - block.number)*12;
    ...
    function getConvexStakingInfo(
        ...
        if (_scAddress == earnCRV) {
            ...
            stakingInfo[1] = scInterface.balanceOf(_userAddress);
            (stakingInfo[2], stakingInfo[3], stakingInfo[4]) = (0, 0, 0);
            // [0], and [1] are the stake and reward addresses
            stakingData[0] = cvx;
            stakingData[1] = crv;
        } else {
            ...
            // [0] is the user pending reward
            stakingInfo[0] = scInterface.rewards(_userAddress, cvxCRV);
            // [1] is the user staking amount
            (stakingInfo[1], , ) = scInterface.userLocks(_userAddress, 0); 
            (stakingInfo[2], stakingInfo[3], stakingInfo[4]) = (0, 0, 0);           
            // [0], and [1] are the stake and reward addresses
            stakingData[0] = cvx;
            stakingData[1] = cvxCRV;
        }
    }
    ...
    function getShibaStakingInfo(
        ...
        if (_scAddress == earnXSHIB) {
            stakingData[0] = 0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE;
            stakingData[1] = 0xB4a81261b16b92af0B9F7C4a83f1E885132D81e4;
        } else if (_scAddress == earnXLEASH) {
            stakingData[0] = 0x27C70Cd1946795B66be9d954418546998b546634;
            stakingData[1] = 0xa57D319B3Cf3aD0E4d19770f71E63CF847263A0b;
        } else {
            stakingData[0] = 0x9813037ee2218799597d83D4a5B6F3b6778218d9;
            stakingData[1] = 0xf7A0383750feF5AbaCe57cc4C9ff98e3790202b3;
        }
    }
