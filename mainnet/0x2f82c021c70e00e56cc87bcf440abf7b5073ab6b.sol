...
// Angry Potatoes Management was inspired by BoringBananasCo and pays all due respects to the amazing team of Bananas Community!
// We love you guys!
...
contract AngryPotatoes is ERC721, Ownable {

    using SafeMath for uint256;

    string public BANANA_PROVENANCE = ""; // IPFS URL WILL BE ADDED WHEN POTATOES ARE ALL SOLD OUT
    ...
    uint256 public constant MAX_BANANAS = 10000;

    bool public saleIsActive = false;

    mapping(uint => string) public bananaNames;

    // Reserve 125 Potatoes for team - Giveaways/Prizes etc
    ...
    constructor() ERC721("Angry Potatoes Management", "APM") { }
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE A POTATO WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function mintBoringBanana(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Potato");
        require(numberOfTokens > 0 && numberOfTokens <= maxBananaPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_BANANAS, "Purchase would exceed max supply of Potatoes");
    ...
    function changeBananaName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this potato!");
    ...
    function viewBananaName(uint _tokenId) public view returns( string memory ){
        require( _tokenId < totalSupply(), "Choose a potato within range" );
