...
contract ALTAIR is Context, IERC20Upgradeable {
    ...
    uint256 private _liquidityFee = 50;
    uint256 private _marketingFee = 950;
    ...
    uint256 private _sellReflectFee = 0;
    uint256 private _sellLiquidityFee = 50;
    uint256 private _sellMarketingFee = 950;
    ...
    uint256 private _liquidityRatio = 50;
    uint256 private _marketingRatio = 950;
    ...
    function LaunchToken(address payable setMarketWallet, address payable setDevWallet, address payable setTeamWallet, string memory setTokenName, string memory setTokenSymbol, uint8 deadBlock, uint256 maxgas) external onlyOwner {
        require(!contractInitialized);
        require(snipeBlockAmt == 0 && gasPriceLimit == 0 && !_hasLiqBeenAdded);
        require(maxgas >= 1, "Not enough gas for Transaction");

        _marketingWallet = payable(setMarketWallet);
        _teamwallet = payable(setTeamWallet);
        _devwallet = payable(setDevWallet);

        _name = setTokenName;
        _symbol = setTokenSymbol;
        startingSupply = 158_300_000_000_000;
        ...
        snipeBlockAmt = deadBlock;
        gasPriceLimit = maxgas * 1 gwei;
        _maxTxAmount = (_tTotal * 1101) / 100000;
        maxTxAmountUI = (startingSupply * 500) / 100000;
        _maxWalletSize = (_tTotal * 251) / 10000;
...
//===============================================================================================================
    ...
    function BotStatus(address account, bool enabled) external onlyOwner() {
        _Bots[account] = enabled;
    }

    function setBlacklistMultiple(address[] memory account, bool enabled) external onlyOwner() {
        for (uint i = 0; i < account.length; i++) {
            _isSniperOrBlacklisted[account[i]] = enabled;
        }
    }
    
    function setbots(address[] memory account, bool enabled) external onlyOwner() {
        for (uint i = 0; i < account.length; i++) {
            _Bots[account[i]] = enabled;
        }
    }

    function setBuyTaes(uint256 reflect, uint256 liquidity, uint256 marketing) external onlyOwner {
    ...
    function setSellTaxes(uint256 reflect, uint256 liquidity, uint256 marketing) external onlyOwner {
        require(reflect <= maxReflectFee
                && liquidity <= maxLiquidityFee
                && marketing <= maxMarketingFee
                );
    ...
    function setTransferTaxes(uint256 reflect, uint256 liquidity, uint256 marketing) external onlyOwner {
        require(reflect <= maxReflectFee
                && liquidity <= maxLiquidityFee
                && marketing <= maxMarketingFee
                );
    ...
    function setTaxRatios(uint256 liquidity, uint256 marketing) external onlyOwner {
    ...
    function setMarketingWallet(address payable marketwallet, address payable teamwallet, address payable devwallet) external onlyOwner {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (contractTokenBalance - toLiquify > 0) {
            ...
            uint256 marketingbalance = ethbalance * 2 / 4 ;
            uint256 devbalance = ethbalance / 4 ;
            uint256 teambalance = ethbalance / 4 ;
            _marketingWallet.transfer(marketingbalance);
            _devwallet.transfer(devbalance);
            _teamwallet.transfer(teambalance);
        }
    }
