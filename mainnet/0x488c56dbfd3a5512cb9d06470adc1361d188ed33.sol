...
contract FESDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("FES_Dividend_Tracker", "FES_Dividend_Tracker") {
        claimWait = 86400;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 100000000+ tokens
    }
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "FES_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FES_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FES contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FES_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "FES_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FES is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    FESDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 100000000000000 * (10**18);
    ...
    constructor() ERC20("FEStoken", "FES") {
        ETHRewardsFee = 0;
        liquidityFee = 0;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 0;
        marketingWallet = payable(0x8ac2b8bbA1e03c224A557E77e180Bd43E83B9aB1);

        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new FESDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FES: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FES: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FES: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FES: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FES: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FES: Cannot update gasForProcessing to same value");
