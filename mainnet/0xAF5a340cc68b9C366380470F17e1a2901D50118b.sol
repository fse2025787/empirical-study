...
contract moicNFT is ERC721Enumerable, Ownable, RoyaltiesV2Impl, ReentrancyGuard  {
  //To concatenate the URL of an NFT
  using Strings for uint256;

  uint256 public totalsupply;
  // set the maximum supply of the NFTs 
  uint256 public constant MAX_SUPPLY = 18;
  // set the Max mint amount
  uint256 public max_mint_allowed = 18;
  // set the Max NFT per address
  uint256 public max_mint_per_address = 18;
  ...
  constructor( string memory _name, string memory _symbol, string memory _theBaseURI, string memory _notRevealedURI, uint96 _royaltyFeesInBips, address payable _withdrawWallet, address payable _royaltyAddress, address _newOwner ) ERC721(_name, _symbol) {
    newOwner = _newOwner;
  ...
  function contractURI() public view returns (string memory) {
    return "https://ipfs.io/ipfs/QmdnkZXVwjQaUMFB5PzE8zkxUAFtg4vMkn1XokfZNKPmHs/contractMetadata.json";
  }
  ...
  function saleMint(uint256 _ammount) public payable {
    ...
    require(_ammount <= max_mint_allowed, "You can't mint more than 5 tokens");
