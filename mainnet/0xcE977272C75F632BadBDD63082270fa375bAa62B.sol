...
interface IWealthyTowersApartmentsStaking {
  ...
  function updateStakedTokenTimestampsOf(address _owner) external;
}

contract WealthyApeSocialClubStakingV2 is OwnableUpgradeable, UUPSUpgradeable {
  ...
  IWealthyTowersApartmentsStaking
    private wealthyApeSocialClubApartmentsContract;
  ...
  function getPendingWealthOf(address _owner) public view returns (uint256) {
    ...
    IWealthyTowersApartmentsStaking.Token[]
      memory _stakedApartments = wealthyApeSocialClubApartmentsContract
        .getStakedTokensOf(_owner);

    for (uint256 i; i < _stakedApartments.length; ++i) {
      totalBalance += wealthyApeSocialClubApartmentsContract
        .getTokenPendingWealth(_stakedApartments[i]);
    }
  ...
  function claim() external {
    ...
    wealthyApeSocialClubApartmentsContract.updateStakedTokenTimestampsOf(
      _msgSender()
    );
  ...
  function setApartmentsStakingContract(address _apartmentContract)
    external
    onlyOwner
  {
    wealthyApeSocialClubApartmentsContract = IWealthyTowersApartmentsStaking(
      _apartmentContract
    );
  }
}
