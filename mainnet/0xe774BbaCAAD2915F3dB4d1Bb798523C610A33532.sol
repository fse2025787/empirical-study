    ...
    Pausable
{
    ...
    function run(
        ...
        require(
            detail.txDeadline > block.timestamp,
            "transaction deadline reached"
        );
        ...
        if (detail.opcode == OP_COMPLETE_SELL) {
            ...
            require(
                ...
                "intent.kind should be KIND_SELL"
            );
            _newSellDeal(
                detail.id,
                intent.user,
            ...
            _assertSender(detail.caller);
            require(
                intent.kind == KIND_AUCTION,
                "intent.kind should be KIND_AUCTION"
            );
            _bid(
                detail.id,
                intent.user,
    ...
    function cancelBuys(uint256[] calldata ids)
        public
        nonReentrant
        whenNotPaused
    {
    ...
    function emergencyCancelAuction(uint256 id, bool noBundle)
        public
        onlyOwner
    {
    ...
    function swap(Swap memory req, bytes memory signature)
        public
        nonReentrant
        whenNotPaused
    {
    ...
    function send(address to, Pair721[] memory tokens)
        public
        nonReentrant
        whenNotPaused
    {
    ...
    function _cancelBuy(uint256 id) internal {
        Inventory storage inv = inventories[id];
        require(
            inv.buyer == msg.sender || isExpired(id),
            "caller is not buyer"
        );
        _cancelBuyAnyway(id);
    }
    ...
    function _bid(
        ...
        if (isAuction(id)) {
            ...
            require(
                price >=
                    auc.price + ((auc.price * minAuctionIncrement) / RATE_BASE),
                "bid price too low"
            );
    ...
    function _completeAuction(uint256 id, Settlement calldata settlement)
        internal
    {
        require(
    ...
    function _acceptAuction(uint256 id, Settlement calldata settlement)
        internal
    {
    ...
    function _completeTransaction(uint256 id, Settlement calldata settlement)
        internal
    {
        Inventory storage inv = inventories[id];
        require(
        ...
        uint256 buyerAmount = (price * settlement.buyerCashbackRate) /
            RATE_BASE;
    ...
    function _transferBundle(
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            ...
            if (p.kind == TOKEN_MINT) {
                ...
                IERC1155(p.token).safeTransferFrom(
                    from,
                    to,
                    p.tokenId,
                    p.amount,
    ...
    function isBundleApproved(uint256 invId, address owner)
        public
        view
        returns (bool)
    {
...
contract TZKGMinter is Context, Ownable {
