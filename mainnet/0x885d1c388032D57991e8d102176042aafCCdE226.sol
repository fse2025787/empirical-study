...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    IOLDNames oldNames = IOLDNames(address(0x5564886ca2C518d1964E5FCea4f423b41Db9F561));

    event unwrapped(string _name);
    event wrapped(string _name);
    ...
    // Mapping from token ID to approved address
    mapping(uint256 => bytes32) private tokenId2oldId;

    // tokenid => name hash map
    mapping(uint256 => bytes32) private hashForToken;

    // tokenId => name
    mapping(string => uint256) private nameToToken;

    // bytes32(name) => address owner mapping
    mapping(bytes32 => address) private nameOwnerMap;
    ...
    function _safeMint(address to, string[] calldata _names) internal {
        _safeMint(to, _names, '');
    }
    ...
    function _safeMint(
        ...
        for (uint256 i = 0; i < length;){
            if (address(uint160(uint256(oldNames.owner(stringToBytes32(_names[i]))))) == _msgSender()) {
                uint256 retTokenId = nameToToken[_names[i]];
                if(hashForToken[retTokenId] == stringToBytes32(_names[i])){
                    emit Transfer(address(0), to, retTokenId);
                }
            }
            unchecked{i++;}
        }
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            ////////// LCFR - keep track of hash -> tokenID internally for transfers.
            uint256 tokenId = updatedIndex;
            bytes32 tokenHash = stringToBytes32(_names[i]);
            hashForToken[tokenId] = tokenHash;
            nameOwnerMap[stringToBytes32(_names[i])] = _msgSender();
            nameToToken[_names[i]] = tokenId;
            /////////////////////////////////////////////////////////////////////////
            updatedIndex++;
        }
    ...
    function _transfer(
        ...
        ////
        bytes32 hash = hashForToken[tokenId];
        require(nameOwnerMap[hash] == _msgSender(), "not owner of name or name not wrapped");
        require(from == _msgSender(), "not sender");
        // so names which are not wrapped cant be transferred.
        require((address(uint160(uint256(oldNames.owner(hash)))) == address(this)), "contract not OG owner");
        ////
    ...
    // ripped from ethereum stackexchange
    ...
    function _getTokenName(uint256 tokenId) internal view returns (string memory) {
        return bytesToString(bytes.concat(hashForToken[tokenId]));
    }

    function _getTokenHash(uint256 tokenId) internal view returns (bytes32) {
        return hashForToken[tokenId];
    }

    function _getOwnerWrapper(string calldata nameString) internal view returns(address) {
        return nameOwnerMap[stringToBytes32(nameString)];
    }

    function _getNameTokenId(string calldata nameString) internal view returns(uint256 retTokenId) {
        retTokenId = nameToToken[nameString];
        require(hashForToken[retTokenId] == stringToBytes32(nameString), "invalid name");
    }

    function _unwrap(string[] calldata _names) internal virtual {
        uint256 length = _names.length;
        for(uint256 i = 0; i < length;){
            require(_getOwnerWrapper(_names[i]) == _msgSender(), "name not wrapped or owned by sender");
            emit unwrapped(_names[i]);
            nameOwnerMap[stringToBytes32(_names[i])] = address(0);
            oldNames.transfer(stringToBytes32(_names[i]), _msgSender());
            require(address(uint160(uint256(oldNames.owner(stringToBytes32(_names[i]))))) == _msgSender(), "failed to set msg.sender as OG owner.");
            unchecked{i++;}
        }
    }

    function _wrap(string[] calldata _names) internal virtual {
        uint256 length = _names.length;
        for(uint256 i = 0; i < length;){
            require(address(uint160(uint256(oldNames.owner(stringToBytes32(_names[i]))))) == _msgSender(), "not OG owner.");
            emit wrapped(_names[i]);
            //uint256 retTokenId = nameToToken[_names[i]];
            //emit Transfer(address(0), _msgSender(), retTokenId);
            //_transfer(address(0), _msgSender(), retTokenId);
            //nameOwnerMap[stringToBytes32(_names[i])] = _msgSender();
            unchecked{i++;}
        }
         _safeMint(_msgSender(), _names);
    }
...
// Creator: Chiru Labs
// 
// non-optimized hack job added by lcfr.eth
...
// lol
interface IOLDNames {
    function reserve(bytes32 _name) external payable;
    function transfer(bytes32 _name, address newowner) external payable;
    function owner(bytes32 _name) external view returns(bytes32);
}
