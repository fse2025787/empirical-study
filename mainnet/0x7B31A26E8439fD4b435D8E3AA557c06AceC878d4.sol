...
contract GlobalPool_R32 is Lockable, Pausable {
    ...
    function migrateProviderRewards(address[] calldata addresses, uint256[] calldata amounts) public onlyOwner {
        require(addresses.length == amounts.length, "Lengths are not the same");
        for (uint256 i = 0; i < addresses.length; i++) {
            address provider = addresses[i];
            _aETHProviderRewards[provider] = amounts[i];
            delete _etherBalances[provider];
        }
    }
