...
contract Contract is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _initBaseURI) ERC721A("Bored Spirit", "BSP") {
    ...
    /**
    * @dev mints 2 token per whitelisted address
    */
    ...
    function setMax(uint256 _whitelist, uint256 _public) external onlyOwner {
        maxWhitelist = _whitelist;
        maxPublic = _public;
    }

    function setMintPerTx(uint256 quantity) external onlyOwner {
        mintPerTx = quantity;
    }
    ...
    function mintForAddresses(uint256 quantity, address _address) external onlyOwner {
        require(!paused, "The contract is paused!");
