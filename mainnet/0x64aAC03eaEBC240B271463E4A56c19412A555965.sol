...
interface IAitoCreatorNFT {
    ...
    /**
     * @notice Allows token creators to update their token's URI in case of an IPFS gateway failure or similar.
     *
    ...
    /**
     * @notice Allows token creators to change the fee BPS associated with one of their tokens.
     *
    ...
    /**
     * @notice Allows token creators to change the fee recipient address associated with one of their tokens.
     *
    ...
    /**
     * @notice Allows token creators to change the creator address associated with one of their tokens.
     *
...
/**
 * This is almost a direct carbon copy of Openzeppelin's ERC721 standard, this is the only differences:
 * Visibility on _operatorApprovals set to internal to allow approving for all with a signature.
 *
 * Forked from @openzeppelin/[email protected]
 */
 ...
 */
contract AitoCreatorNFT is ERC721, IAitoCreatorNFT {
    ...
    mapping(uint256 => IAitoCreatorNFT.FeeData) internal _feeData;

    /**
     * @dev Constructor calls the ERC721 constructor and initializes the ID counter to 1.
     */
    ...
    function changeURI(uint256 tokenId, string calldata newUri) external override {
        _validateTokenCreator(tokenId, msg.sender);
    ...
    function changeFeeRecipient(uint256 tokenId, address newFeeRecipient) external override {
        _validateTokenCreator(tokenId, msg.sender);
    ...
    function changeCreator(uint256 tokenId, address newCreator) external override {
        _validateTokenCreator(tokenId, msg.sender);
    ...
    function permit(
        address spender,
        uint256 tokenId,
        IAitoCreatorNFT.EIP712Signature calldata sig
    ) external override {
    ...
    function creator(uint256 tokenId) external override view returns (address) {
        return _creator[tokenId];
    }

    
    function feeData(uint256 tokenId) external override view returns (IAitoCreatorNFT.FeeData memory) {
        return _feeData[tokenId];
    }

    
    function feeBps(uint256 tokenId) external override view returns (uint16) {
        return _feeData[tokenId].feeBps;
    }

    
    function feeRecipient(uint256 tokenId) external override view returns (address) {
        return _feeData[tokenId].feeRecipient;
    }

    
    function domainSeparator() external override view returns (bytes32) {
    ...
    function _validateTokenCreator(uint256 tokenId, address creatorCandidate) internal view {
        require(creatorCandidate == _creator[tokenId], Errors.NOT_CREATOR);
    }
    ...
    function _fullMint(
        ...
        _feeData[tokenId] = IAitoCreatorNFT.FeeData(feeRecipient, feeBps);
        _safeMint(to, tokenId);
