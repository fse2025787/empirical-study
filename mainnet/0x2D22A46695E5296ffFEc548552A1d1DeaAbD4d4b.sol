...
// import "C:\VRM\VRM.FarmingStaking.Contracts\node_modules\@openzeppelin\contracts\utils\Context.sol";
/**
...
// Dependency file: contracts\StakingTokenV2.sol
...
contract StakingTokenV2 is IStakingToken, ReentrancyGuard {
    ...
    function canStake(address account) external view override returns (bool) {
        return
            _balances[account].releaseDate == 0 ||
            block.timestamp < _balances[account].releaseDate.sub(stakingDuration).add(1 days);
    }
    ...
    function stake(uint256 amount) external override nonReentrant {
        ...
        if (_balances[msg.sender].amount == 0) {
            ...
            require(block.timestamp < _balances[msg.sender].releaseDate.sub(stakingDuration).add(1 days));
            _balances[msg.sender].amount = _balances[msg.sender].amount.add(amount);
        }
...
// Root file: contracts\StakingFactoryV2.sol
...
// import 'contracts\StakingTokenV2.sol';

contract StakingFactoryV2 is Ownable {
    ...
    function deploy(
        ...
        address deployedContract = address(new StakingTokenV2(stakingToken, duration, baseApy, address(this)));
    ...
    function emergencyWithdraw(address _token) external onlyOwner {
        for (uint256 index = 0; index < stakingTokens.length; index++) {
            ...
            IStakingToken(info.stakingTokenContract).emergencyWithdraw(_token);
        }
        IERC20(_token).transfer(msg.sender, IERC20(_token).balanceOf(address(this)));
    }
}