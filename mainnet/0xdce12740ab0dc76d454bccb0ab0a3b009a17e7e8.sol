...
/*
Token features:
5% redistribution to all holders
10% tax on sell transactions to the dev wallet
*/
...
contract LokiF is Context, IERC20, Ownable {
    ...
    // ********************************* START VARIABLES **********************************
    string private _name = "Loki Floki";                                           // name
    string private _symbol = "LokiF";                                             // symbol
    uint256 private _tTotal = 1000 * 10**9 * 10**uint256(_decimals);             // total supply

    // % to holders
    uint256 public defaultTaxFee = 5;
    uint256 public _taxFee = defaultTaxFee;
    uint256 private _previousTaxFee = _taxFee;

    // % to swap & send to dev wallet
    uint256 public defaultDevFee = 0;
    uint256 public _devFee = defaultDevFee;
    uint256 private _previousDevFee = _devFee;

    uint256 public _devFee4Sellers = 10;

    bool public feesOnSellersAndBuyers = true;

    uint256 public _maxTxAmount = _tTotal.div(1).div(100);                                           // max transaction amount
    uint256 public numTokensToExchangeForDevWallet = _tTotal.div(100).div(100);                     // contract balance to trigger swap & send
    address payable public devWallet = payable(0x193bC9B350eF339407Af6C779956e9Df381f28dE); // dev wallet
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tDev) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tDev, _getRate());
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= numTokensToExchangeForDevWallet;
    ...
    function setdefaultDevFee(uint256 devFee) external onlyOwner() {
        defaultDevFee = devFee;
    }

    function setDevFee4Sellers(uint256 devFee4Sellers) external onlyOwner() {
        _devFee4Sellers = devFee4Sellers;
    }
    ...
    function setnumTokensToExchangeForDevWallet(uint256 _numTokensToExchangeForDevWallet) public onlyOwner() {
        numTokensToExchangeForDevWallet = _numTokensToExchangeForDevWallet;
    }

    function _setdevWallet(address payable wallet) external onlyOwner() {
        devWallet = wallet;
    }
