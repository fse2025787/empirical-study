...
contract ContractRegistry is IContractRegistry, Ownable {
    ...
    ) external view override(IContractRegistry) returns (address) {
        address _addr = contracts[_contractName];

        require(
            _addr != address(0),
            string(
                abi.encodePacked(
                    "ContractRegistry: missing contract - ",
                    _contractName
                )
            )
        );
    ...
    function setByName(
        ...
        emit Set(_contractName, _contractAddress);
    }
}
