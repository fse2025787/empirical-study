...
contract PU is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
    uint256 public SALE_NFT = 22022;
    ...
    constructor() ERC721P('Peace Ukraine NFT', 'PU') {}
	
    function _baseURI() internal view virtual returns (string memory) {
        return baseURI;
    }
	
	function mintGiveawayNFT(address[] calldata to, uint256[] calldata count) public onlyOwner{
	    uint256 totalSupply = totalSupply();
		require(to.length == count.length, "Length MisMatch");
		
		for(uint256 i = 0; i < to.length; i++) {
		   require(
				SALE_MINTED + count[i] <= SALE_NFT, 
				"All NFT Minted"
			);
	...
	function mintGiveawayNFT(address[] calldata to, uint256 count) public onlyOwner{
	    uint256 totalSupply = totalSupply();
