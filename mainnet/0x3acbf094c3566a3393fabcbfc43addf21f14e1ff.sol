...
contract PinkyFriendz is Ownable, ERC721Enumerable {
    using Strings for uint256;
   ...
   function buy(uint _count) public payable{
        ...
        require(totalAmount + _count<= phase, "Not enough tokens remaining in current phase");
        require(totalAmount + _count <= maxSupply, "Not enough tokens left");
        require(msg.value >= tokenPrice * _count, "incorrect ether amount");
 
      
    for(uint i = 0; i < _count; i++)
       { _safeMint(msg.sender, totalAmount + 1);
             totalAmount += 1; 
             addbuyer();
           }
    }
   ...
   function buy_presale(uint _count, bytes32[] calldata merkleProof) public payable{ 
    ...
    function sendGifts(address[] memory _wallets) public onlyOwner{
        ...
        require(totalAmount + _wallets.length <= maxSupply, "not enough tokens left");
        for(uint i = 0; i < _wallets.length; i++)
            _safeMint(_wallets[i], totalAmount + 1 + i);
        totalAmount += _wallets.length;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(0xb6934bC03743a525ED5Fe865fb9202b4eeEaD10e).transfer(_balance * 20 / 100);
        payable(0x33F77035eb6eaAb48AeeF7a789BaCC32Aaf86e69).transfer(_balance * 20 / 100);
