...
// File: contracts/girlesVesting.sol
...
contract GirlesVesting is ReentrancyGuard, Ownable {
    // Allocation distribution of the total supply.
    uint256 private constant LOCKED_ALLOCATION = 1_000_000_000 * 1e6;

    // vesting wallets
    address private constant lockedWallet = address(0x150F9d2d728500cDEB050078647b1CD61d71c25E);
    uint256 private constant VESTING_END_AT = 365 days;  // 12 months
    ...
    function initVestingSchedule() public onlyOwner {
        ...
        _createSchedule(lockedWallet, Schedule({
            ...
            periodic             :   7776000,       //  3 Months ( 3 * 30 * 24 * 60 * 60)   
            lastClaimTime        :  0
        }));
    }
    ...
    /**
     * @dev Allow owner to create the new vesting schedule
     */
    function createSchedule(address _beneficiary, string memory _templateName, uint256 _allocation, 
            uint256 _cliff, uint256 _linear, uint256 _periodic) external onlyOwner {
    ...
    /**
     * @dev Allow owner to update the current vesting schedule
     */
    function updateSchedule(string memory _templateName, uint256 _allocation, 
            uint256 _cliff, uint256 _linear, uint _periodic) external onlyOwner {
        ...
        schedules[index].linear = _linear;
        schedules[index].periodic = _periodic;
    }

    /**
     * @dev Allow owner to update the beneficiary
     */
    function updateBeneficiary(string memory _templateName, address _beneficiary) external onlyOwner {
        uint index = schedulesByName[_templateName];
        require(index >= 0 && index < schedules.length, "Vesting: NOT_SCHEDULE");
        beneficiary[_templateName] = _beneficiary;
    }
    ...
    function pendingTokensByScheduleName(string memory _templateName) public view returns (uint256) {
        ...
        if (block.timestamp - schedules[index].lastClaimTime < schedules[index].periodic)
            return 0;

        if (schedule.startTime + schedule.cliff + schedule.linear <= block.timestamp)
        ...
        return timePeriod * unitPeriodAllocation - schedule.claimedTokens;
    }
