...
$WAGBO
2% buy tax for liquidity building
2% sell tax for buying bears
3% max amount allowed per wallet -- FOR THE PEOPLE NOT THE WHALES

For the Bears, by the Bears. This is your chance to buy $WAGBO 
and make enough profits to buy your own Okay Bears. 

No marketing wallet, no secret wallets, community led project
WE ARE GOING TO BE OKAY

http://twitter.com/WAGBOtoken
...
contract WAGBO is Context, IERC20, Ownable {
    ...
    // limitations
    uint256 private _maxPercentagePerAddress = 3;

    // taxes and fees
    address payable public _treasuryAddress;
    uint256 private _currentBuyTax = 2; // modified depending on context of tx
    uint256 private _currentSellTax = 2; // modified depending on context of tx
    uint256 public _fixedBuyTax = 2; // unchanged save by owner transaction
    uint256 public _fixedSellTax = 2; // unchanged save by owner transaction
    ...
    // We expose this function to modify the max %age per wallet
    ...
    // We allow the owner to set addresses that cannot trade
    ...
    function _tokenTransfer(
        ...
        // implement max wallet percentage check -- this is ugly as fuck.
        // sorry y'all
        if (
        ...
        // only do inline swaps on the sells. buys just accumulate into the contract.
        ...
        // we shouldn't have any balance. but if we do send it to the treasury
