...
contract CyberTama is Context, IERC20, Ownable {
    ...
    string private _name = "Cybertama";
    ...
    address public _marketingWallet =
        payable(address(0xfac820D7fa7267cc7A9213006a477c064b3bFf80));

    address public _devWallet = payable(address(0x58550ec20AA8D496eCdB8Fb8aa9231Da79850c98));    
    ...
    uint8 public constant BLOCKCOUNT = 100; //Blacklist first 100 block buys
    ...
    constructor() {
        ...
        buyFee.marketingFee = 7;
        buyFee.liquidityFee = 2;
        buyFee.taxFee = 2;
        buyFee.burnFee = 1;

        sellFee.liquidityFee = 2;
        sellFee.marketingFee = 7;
        sellFee.taxFee = 2;
        sellFee.burnFee = 1;
    ...
    function setBuyFee(uint16 market, uint16 Liquidity, uint16 tax, uint16 burn) external onlyOwner {
        buyFee.marketingFee = market;
        buyFee.liquidityFee = Liquidity;
        buyFee.taxFee = tax;
        buyFee.burnFee = burn;
        uint256 totalBuyFee = market + Liquidity + tax + burn;
        require(totalBuyFee < 35, "Buy fees should be less than 35%");
    }
    ...
    function setSellFee(
        ...
        uint256 totalSellFee = liq + market + tax + burn;
        require(totalSellFee < 35, "Sell fees should be less than 35%");
    }
    ...
    function setMaxWallet(uint256 value) external onlyOwner {
        require (value > 100000,"No rug pulls");
        ...
        )
    {
        ...
        uint256 tWallet = calculateMarketingFee(tAmount) + calculateBurnFee(tAmount);
    ...
    function _takeLiquidityAndBurn(uint256 tLiquidity, uint256 tBurn) private {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function _transfer(
        ...
        if (
            ...
            if (forMarketing > 0) {
                swapAndSendMarketingAndDev(forMarketing);
            }
    ...
    function swapAndSendMarketingAndDev(uint256 amount) private lockTheSwap {
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(amount);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 devShare = newBalance.div(7);
        payable(_devWallet).transfer(devShare);

        payable(_marketingWallet).transfer(newBalance - devShare );
    }
    ...
    function _transferStandard(
        ...
        _takeLiquidityAndBurn(tLiquidity, calculateBurnFee(tAmount));
