...
contract Music is ERC1155,Ownable{
    ...
    mapping(address => bool) public white_adress;
    ...
    function delWhite(address[] memory _adr) public onlyOwner{
        for(uint256 i = 0;i < _adr.length;i++){
            if(white_adress[_adr[i]]){
                delete white_adress[_adr[i]];
            }
        }

    } 
  
    function addWhite(address[] memory _adr) public onlyOwner{
        for(uint256 i = 0;i < _adr.length;i++){
            white_adress[_adr[i]] = true;
        }
