...
contract Toeken is ERC20, Ownable, ReentrancyGuard {
    ...
    function getPending(address account) internal view returns (uint256) {
        ...
        uint256 timeMultiplier = (block.timestamp -
            (lastUpdate[account] > startTime ? lastUpdate[account] : startTime)) / 1 days;
        uint256 pendingFromTiptoe = TiptoePunks.balanceOf(account) * TIPTOE_BASE_RATE * timeMultiplier;
        uint256 pendingFromFoot = FootPunks.balanceOf(account) * FOOT_BASE_RATE * timeMultiplier;
