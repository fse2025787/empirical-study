...
contract Tamadoge is ERC721, Ownable, ERC721Burnable {
    ...
    // Constructor
    constructor(string memory _name, string memory _symbol, address _owner, string memory _currentBaseURI, string memory _dummyURI) ERC721(_name, _symbol) {
    ...
    */
    ...
    /**
        @dev Set Dummy URI - onlyOwner
    */
    function setDummyURI(string memory _dummyURI) public onlyOwner {
        dummyURI = _dummyURI;
    }
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _requireMinted(tokenId);
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : dummyURI;
    }
