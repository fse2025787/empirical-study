...
contract LuckyGreenWitch is ERC721A, Ownable, ReentrancyGuard, ERC721AQueryable {  
    ...
    constructor() ERC721A("LuckyGreenWitch", "LGW") {
        _safeMint(_msgSender(), 22);  // reserved for owners   
        _safeMint(_msgSender(), 22);  // reserved for team  
        _safeMint(_msgSender(), 22);  // reserved for donors  
    }
    ...
    function tokenURI(uint256 _tokenId) public view override(ERC721A, IERC721A) returns (string memory) {
        ...
        if (hasAddon[uint64(_tokenId)] > 0) {
            string memory ipfsAddress = onIpfs[uint64(_tokenId)];
            if (bytes(ipfsAddress).length > 0) {
                return ipfsAddress;
            } else {
                return bytes(_tokenWithAddonURI).length != 0 ? string(abi.encodePacked(_tokenWithAddonURI, _toString(_tokenId))) : '';
            }
        }
    ...
    function putOnIpfs(uint64 _tokenId, string memory _address) public onlyOwner {
        ...
        require(hasAddon[_tokenId] != 0, "Token has no addon");
    ...
    function random(uint _l) private returns (uint256) {
        ++cntRnd;
        uint rnd = uint(keccak256(abi.encodePacked(cntRnd, block.difficulty, msg.sender)));
        return rnd % _l;
    }
    ...
    function setAddonPrice(uint256 _newPrice) public onlyOwner {
        addonPurchasePrice = _newPrice;
    }

    function decreaseMaxSupply(uint64 _maxSupply) public onlyOwner {
        require(_maxSupply <= 10000, "Cannot increase max supply");
        require(_maxSupply >= _totalMinted() , "Too small");
    ...
    function purchaseAddon(uint64 _tokenId) public payable nonReentrant {
        ...
        require(hasAddon[_tokenId] == 0, "Witch already has addon");
    ...
    function getAddonPrice() public view returns (uint) {
        return addonPurchasePrice;
    }

    function setAddons(uint[] memory _addons) public onlyOwner {
        delete addonsAvailability;
        uint l = _addons.length;
        for (uint i = 0; i < l; i++){
            addonsAvailability.push(uint64(_addons[i]));
        }
    }   
    ...
    function setMerkleReserved(uint64 _maxM) public onlyOwner {
        require((MAX_SUPPLY + _maxM) <= 10000, "Cannot increase");
