...
contract PBGBNFT is Ownable, ERC721Min, ReentrancyGuard {
    ...
    address private treasuryAddress = 0xE6f84d92B4aB5E585c6f33364Aba8A29597D83C6; 
    ...
    constructor() ERC721Min("PUSH BUTTON GET BANANA", "PBGB") {
    ...
    function buyFive(uint256 nftTypeId) external payable {
        require(NftTypes[nftTypeId].saleActive, "SALE_CLOSED");
        require(NftTypes[nftTypeId].priceForFive == msg.value, "INCORRECT_ETH");
        require(NftTypes[nftTypeId].maxMintForFive > NftTypes[nftTypeId].purchaseCount, "EXCEED_MAX_SALE_SUPPLY");
        require(NftTypes[nftTypeId].maxPerAddress == 0 || 
            NftTypes[nftTypeId].PurchasesByAddress[_msgSender()] < NftTypes[nftTypeId].maxPerAddressForFive, "EXCEED_MAX_PER_USER");
        NftIdToNftType[_owners.length] = nftTypeId;
        _mintMin();
        NftIdToNftType[_owners.length] = nftTypeId;
        _mintMin();
        ...
        if (NftTypes[nftTypeId].maxPerAddress > 0) {
            NftTypes[nftTypeId].PurchasesByAddress[_msgSender()] += 5;
        } 
        NftTypes[nftTypeId].purchaseCount += 5;
    }
    ...
    function mintFive(address receiver, uint256 nftTypeId) external onlyProxy {
        NftIdToNftType[_owners.length] = nftTypeId;
        _mintMin2(receiver);
        NftIdToNftType[_owners.length] = nftTypeId;
        _mintMin2(receiver);
        ...
        uint16 _maxPerAddress, bool _saleActive, string calldata _uri) external onlyOwner
    {
        ...
        NftTypes[nftTypeCount].maxMintForFive = _maxMint-1;
        NftTypes[nftTypeCount].maxPerMint = _maxPerMint;
        NftTypes[nftTypeCount].price = _price;
        NftTypes[nftTypeCount].priceForFive = _price * 5;
        NftTypes[nftTypeCount].maxPerAddress = _maxPerAddress > 0 ? _maxPerAddress : NftTypes[nftTypeCount].maxPerAddress;
        NftTypes[nftTypeCount].maxPerAddressForFive = _maxPerAddress > 4 ? _maxPerAddress - 4 : NftTypes[nftTypeCount].maxPerAddress;
    ...
    function setMaxMint(uint256 nftTypeId, uint16 maxMint_) external onlyOwner {
        ...
        NftTypes[nftTypeId].maxMintForFive = maxMint_ > 4 ? maxMint_ - 4 : maxMint_;
    }
    ...
    function setMaxPerAddress(uint256 nftTypeId, uint16 _maxPerAddress) external onlyOwner {
        ...
        NftTypes[nftTypeId].maxPerAddressForFive = _maxPerAddress > 1 ? _maxPerAddress - 4 : NftTypes[nftTypeCount].maxPerAddress;
    }

    function setPrice(uint256 nftTypeId, uint256 _price) external onlyOwner {
        NftTypes[nftTypeId].price = _price;
        NftTypes[nftTypeId].priceForFive = _price * 5;
    }    
