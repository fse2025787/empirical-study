...
contract GodHatesNFTs is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public FreeMintWalletLimit = 2;
...
/** MINT **/
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (supply < maxFreeSupply) {
        uint256 remainingfreemint = FreeMintWalletLimit - freeMintCountMap[msg.sender];
        if (remainingfreemint > 0) {
            if (_mintAmount >= remainingfreemint) {
                price -= remainingfreemint * cost;
                updateFreeMintCount(msg.sender, remainingfreemint);
            } else {
                price -= _mintAmount * cost;
