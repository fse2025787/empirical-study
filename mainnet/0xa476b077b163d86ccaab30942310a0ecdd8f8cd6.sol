...
contract GMsV2 is GMs {
    ...
    WLClaimEvent[] public wlClaimEvents;

    function wlClaim(bytes32[] memory _proof, uint256 _wlClaimEventIndex) public {
        WLClaimEvent memory wlClaimEvent = wlClaimEvents[_wlClaimEventIndex];
        require(wlClaimEvent.startTimestamp > 0, "Claim hasn't started yet!");
        require(wlClaimEvent.startTimestamp + wlClaimEvent.period > block.timestamp, "Claim has ended!");
        require(MerkleProof.verify(_proof, wlClaimEvent.root, keccak256(bytes.concat(keccak256(abi.encode(msg.sender))))), "You are not whitelisted for this gm!");
        require(super.balanceOf(msg.sender, wlClaimEvent.tokenId) == 0, "You already have this gm!");
        super._mint(msg.sender, wlClaimEvent.tokenId, 1, "");
    }
    ...
    function startNewWlClaim(
        uint256 _claimId,
        bytes32 _root,
        uint256 _claimPeriod
    ) public onlyOwner {
        WLClaimEvent memory wlClaimEvent = WLClaimEvent(_claimId, _root, block.timestamp, _claimPeriod);
        wlClaimEvents.push(wlClaimEvent);
    }
}