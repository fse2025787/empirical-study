...
contract NFTBatchTokenEngine {
    event NftTokenCreated(address);
    function setNftTokenValues(
        string memory name,
        address owner,
        ...
        address newNftContract = address(
            new HUMBLBATCHNFT(name, owner, symbol, uri, cap, royaltyAmount, creator, batchQuantity)
        );
        emit NftTokenCreated(newNftContract);
...
contract HUMBLBATCHNFT is ERC721URIStorage, Royalties, Ownable {
    ...
    string nftURI;
    mapping (uint256 => string) private _tokenURIs;
    string init_url = "https://hgapi.humblpay.com/api/v1.0/my-contract-meta-data";
    ...
    ERC721(name, symbol) Royalties(royaltyAmount, creator){
        require(cap >= 0);
        _cap = cap;
        nftOwner = owner;
        nftURI = uri;
        mintBatch(batchQuantity);
    }
    ...
    function mintBatch(uint256 batchQuantity) public {
        for(uint idCounter = 1; idCounter <= batchQuantity ; idCounter++) {
            _tokenIds.increment();
            require(_tokenIds.current() <= _cap);
