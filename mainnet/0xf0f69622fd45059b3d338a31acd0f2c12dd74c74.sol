...
contract EDOToken is ERC20, Ownable {
  ...
  uint256 public TOTAL_MINTED = 0 ether;
	...
	constructor(address _vsamurai, address _newOwner, address _signingKey) ERC20("EDO", "EDO") {
		...
		tokenRates.common = 2 ether;
		tokenRates.rare = 4 ether;
		tokenRates.legendary = 7 ether;
		tokenRates.god = 10 ether;
  ...
  function setTokenRates(uint64 _common, uint64 _rare, uint64 _legendary, uint64 _god) public onlyOwner {
    tokenRates.common = _common;
    tokenRates.rare = _rare;
    tokenRates.legendary = _legendary;
    tokenRates.god = _god;
  }

  function getTokenRates(uint256 tokenId) public view returns(uint64) {
    if(rarities[tokenId] == 1) {
      return tokenRates.common;
    } else if(rarities[tokenId] == 2) {
      return tokenRates.rare;
    } else if(rarities[tokenId] == 3) {
      return tokenRates.legendary;
    } else if(rarities[tokenId] == 4) {
      return tokenRates.god;
    } else {
      return 0;
    }
  }
