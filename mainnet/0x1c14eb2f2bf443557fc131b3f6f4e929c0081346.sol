...
abstract contract Admin is Ownable {
    ...
    uint256[] public intervalDays = [1, 8];
    ...
    constructor(address _owner, address _wethAddress) Ownable(_owner) {
    ...
    function safeWithdrawETH(uint256 amount) external onlyOwner {
        require(
        ...
        // then transfer to owner address
        (bool success, ) = _owner.call{value: amount}("");
        require(success, "SafeWithdrawETH execution failed");

        emit WithdrawDetails(address(WETH), amount, block.timestamp);
    }
...
// File contracts/OROWETHFarm.sol
...
contract OROWETHFarm is Admin {
    ...
    constructor(address _WETHAddress) Admin(_msgSender(), _WETHAddress) {
        poolStartTime = block.timestamp;
    }

    /**
     * @notice do not forget to receive ETH
     */

    receive() external payable {}

    function depositWETH() external payable onlyOwner returns (bool) {
    ...
    function claimRewards(
        ...
        // Reward calculation
        if (interval >= HOURS)
            _rewardCalculationETH(userAddress, stakeId, stakeData, interval);
    }
    ...
    function _rewardCalculationETH(
        ...
        rewardsEarnedETH = noOfDays[1].mul(
            getOneDayReward(
                stakingData[0],
        ...
        // withdraw the WETH
        WETH.withdraw(rewardsEarnedETH);

        if (stakingDetails[userAddress].referrer[stakeId] != address(0)) {
            refEarned = (rewardsEarnedETH.mul(refPercentage)).div(100 ether);
            rewardsEarnedETH = rewardsEarnedETH.sub(refEarned);

            (bool success, ) = stakingDetails[userAddress]
                .referrer[stakeId]
                .call{value: refEarned}("");
