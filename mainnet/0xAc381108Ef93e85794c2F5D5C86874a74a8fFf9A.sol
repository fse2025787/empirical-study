    ...
    ReentrancyGuardUpgradeable
{
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IXanaTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "XanaMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "XanaMarketplace: Owner transfer failed"
            );
        } else {
            ...
            if (price.sub(feeAmount) != 0) {
                RWT.safeTransferFrom(rewardTokenOwner, owner, price.sub(feeAmount).mul(rewardFee).div(10000));
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IXanaMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
