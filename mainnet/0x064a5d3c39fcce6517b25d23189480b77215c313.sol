...
contract PizzaSlice is ERC721M, ERC721Tradable, Ownable {

	uint256 public constant PRICE = 0.0069 ether;
	uint256 public constant MAX_SUPPLY = 4444;
	uint256 public constant MAX_RESERVE = 350;
	uint256 public constant MAX_PUBLIC = 4094; 
	...
	) payable ERC721M("Party Slice", "PARTYSLICE") ERC721Tradable(_openSeaProxyRegistry, _looksRareTransferManager) {
		baseURI = _baseURI;
	}
	function publicMint(uint256 amount) external payable {
		...
		if (_totalSupply < MAX_FREE) {
			require(!hasClaimed[msg.sender], "Already claimed 3 Free Mints!");
