...
contract Twap is CustomOwnable, CustomInitializable, ReentrancyGuard {
    ...
    function acceptFeed (bytes32 topic, IBasicAsset targetToken, uint256 stepIndex, uint256 buyingAmount, bytes32 secretRevealed, IBasicAsset sellingToken, uint256 sellingAmount, bytes memory payload) external onlyOperator ifInitialized nonReentrant {
        ...
        bytes32 actualHash = _buildHash(address(targetToken), stepIndex, buyingAmount, secretRevealed);
        ...
        // Run the swap
        _approveSpenderProxy(sellingToken, sellingAmount);
        ...
        _revokeSpenderProxy(sellingToken);
    ...
    function buildSteps (address targetTokenAddr, uint256[] calldata amounts, bytes32[] calldata secrets) external pure returns (bytes32[] memory) {
        bytes32[] memory steps = new bytes32[](amounts.length);

        for (uint256 stepIndex = 0; stepIndex < amounts.length; stepIndex++) {
            steps[stepIndex] = _buildHash(targetTokenAddr, stepIndex, amounts[stepIndex], secrets[stepIndex]);
        }

        return steps;
    }

    function _buildHash (address targetTokenAddr, uint256 stepIndex, uint256 amount, bytes32 secretRevealed) private pure returns (bytes32) {
    ...
    function buildTopic (address buyingTokenAddr, address sellingTokenAddr, address walletAddr, uint8 side) external pure returns (bytes32) {
        return keccak256(abi.encodePacked(buyingTokenAddr, sellingTokenAddr, walletAddr, side));
    }
