...
error StructERC721_CollectionNotRevealed();
error StructERC721_ProvenanceHashNotSet();
error StructERC721_TokenOffsetDisabled();
error StructERC721_TokenOffsetAlreadySet();
    ...
    OperatorFilterer
{
    ...
    IAccessManager internal immutable _ACCESS_MANAGER;
    IRandomnessManager internal immutable _RANDOMNESS_MANAGER;
    uint256 internal constant _TOKEN_OFFSET_SLOT = uint256(keccak256('TOKEN_OFFSET'));
    ...
    constructor(
        ...
        IRandomnessManager randomnessManager
    ) {
        _ACCESS_MANAGER = accessManager;
        _RANDOMNESS_MANAGER = randomnessManager;
    }

    /**
     * @notice Initialize a new proxy instance of the contract
     * @param _nonce Receipt number encoded in the signature
     * @param _signature Signed receipt of payment
    ...
    function initialize(
        ...
        public
    {
        _ACCESS_MANAGER.validateDeployment(address(this), _nonce, _signature);
        ...
        if (config.revealed) config.offsetEnabled = false;
        if (config.operatorFilterEnabled) _registerForOperatorFiltering();
    }
    ...
    /**
     * @notice Return randomness generated for this collection
     */
    function _randomness() internal view returns (uint256) {
        return _RANDOMNESS_MANAGER.randomness(address(this), _TOKEN_OFFSET_SLOT);
    }
    ...
    function tokenOffset() external view returns (uint256) {
        if (!config.offsetEnabled) revert StructERC721_TokenOffsetDisabled();

        return _randomness() % config.maxSupply;
    }
     ...
     * @dev `maxSupply` and `offsetEnabled` cannot be changed to ensure fairness
     ...
     */
    function setConfig(Config memory _config) external payable lockable onlyOwner {
        _config.revealed = config.revealed;
        _config.maxSupply = config.maxSupply;
        _config.offsetEnabled = config.offsetEnabled;
     ...
     */
    function setProvenanceHash(string calldata _provenanceHash) external payable lockable onlyOwner {
        if (config.offsetEnabled && _RANDOMNESS_MANAGER.isSet(address(this), _TOKEN_OFFSET_SLOT))
            revert StructERC721_TokenOffsetAlreadySet();
     ...
     * @param _uri Placeholder or base token URI depending on revealed state
     */
    function setRevealed(bool _revealed, string calldata _uri) external payable lockable onlyOwner {
        if (config.offsetEnabled) _RANDOMNESS_MANAGER.randomness(address(this), _TOKEN_OFFSET_SLOT);
     ...
     */
    function setOffset(bool useVRF) external payable lockable onlyOwner {
        if (!config.offsetEnabled) revert StructERC721_TokenOffsetDisabled();
        if (bytes(provenanceHash).length == 0) revert StructERC721_ProvenanceHashNotSet();
    ...
    function privateMint(uint256 amount, bytes memory signature) external payable {
        if (msg.value != config.privatePrice * amount) revert StructERC721_InvalidTransactionValue();
        ...
        if (config.signer != ECDSAUpgradeable.recover(
            ECDSAUpgradeable.toEthSignedMessageHash(keccak256(abi.encodePacked(msg.sender))),
            signature
    ...
    function publicMint(uint256 amount) external payable {
        if (msg.value != config.publicPrice * amount) revert StructERC721_InvalidTransactionValue();
    ...
    function _mint(uint256 amount) internal {
        ...
        if (_totalMinted() + amount > config.maxSupply) revert StructERC721_InsufficientSupply();
        if (_numberMinted(msg.sender) + amount > config.maxPerAddress)
    ...
    function commitMetadata() external onlyOwner {
        if (!config.revealed) revert StructERC721_CollectionNotRevealed();
...
interface IRandomnessManager {
    function isSet(address collection, uint256 slot) external view returns (bool);
    function randomness(address collection, uint256 slot) external view returns (uint256);
    function setWithPRNG(address collection, uint256 slot) external;
    function setWithVRF(address collection, uint256 slot) external;
}
