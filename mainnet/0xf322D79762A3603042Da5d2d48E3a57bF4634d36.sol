...
contract CurveGaugeIncentives is OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping (address => address[]) public blacklistedGaugeAddresses; // used to blacklist specific addresses from gauges if they've already done long-term vote deals

    uint public maximumRewardsPerGauge;

    // Blacklisted weight cache
    mapping (address => mapping(uint => uint)) public blacklistedWeightPerPeriod;

    // Blacklist address status
    mapping (address => bool) public blacklistedAddressStatus;
    mapping (address => mapping(address => bool)) public blacklistedGaugeAddressStatus;
    ...
    function initialize(address _feeAddress, uint256 _platformFee, address _gaugeControllerAddress) public initializer {
       ...
       maximumRewardsPerGauge = 10;
    }
    /* ========== END INITIALIZER FUNCTION ========== */ 

    /* ========== EXTERNAL VIEW FUNCTIONS ========== */

    function isBlacklisted(address _blacklistedAddress) public view returns (bool) {
        return blacklistedAddressStatus[_blacklistedAddress];
    }

    function isBlacklistedGaugeAddress(address _gauge, address _blacklistedAddress) public view returns (bool) {
        return blacklistedGaugeAddressStatus[_gauge][_blacklistedAddress];
    }
    ...
    function claimable(address _user, address _gauge, address _token) external view returns (uint _amount) {
        ...
        bool userBlacklisted = isBlacklisted(_user) || isBlacklistedGaugeAddress(_gauge, _user);
        // user can't claim if blacklisted
        if (userBlacklisted) {
            return _amount;
        }
        ...
        if (_currentPeriod > _checkpointedPeriod) {
            ...
            _totalWeight -= blacklistedWeightPerPeriod[_gauge][_currentPeriod];
            ...
            if (last_user_claim[_user][_gauge][_token] < _votingWeek) {
                ...
                _totalWeight -= blacklistedWeightPerPeriod[_gauge][_checkpointedPeriod];
        ...
        // Factor in platform fee
        _amount -= (_amount * platformFee) / DENOMINATOR;
    }
    ...
    // if msg.sender is not user,
    function claimReward(address _user, address _gauge, address _token) external notBlacklisted(_user) notGaugeBlacklisted(_gauge, _user) returns (uint _amount) {
        _amount = _claimReward(_user, _gauge, _token);
        emit Claimed(_user, _gauge, _token, _amount);
    }

    // if msg.sender is not user,
    function claimReward(address _gauge, address _token) external notBlacklisted(msg.sender) notGaugeBlacklisted(_gauge, msg.sender) returns (uint _amount) {
    ...
    function addRewardAmount(address _gauge, address _token, uint _amount, uint _pricePerPercent) external returns (bool) {
        ...
        require(pendingRewardAddresses[_gauge][_token].length < maximumRewardsPerGauge, "Too many rewards on that gauge");
    ...
    function _claimReward(address _user, address _gauge, address _token) internal returns (uint _amount) {
        ...
        if (last_user_claim[_user][_gauge][_token] < _votingWeek) {
            ...
            uint _blacklistedWeight = blacklistedWeightPerPeriod[_gauge][_period];
    ...
    function _updatePeriod(address _gauge, address _token) internal returns (uint _currentPeriod) {
        ...
        if (_currentPeriod > _checkpointedPeriod) {
            ...
            // Calculate blacklisted weight for the current period
            blacklistedWeightPerPeriod[_gauge][_currentPeriod] = _calculateBlacklistedWeight(_gauge, _currentPeriod);
            
            // Update pending rewards
            uint newlyQualifiedRewards = _updatePendingRewards(_gauge, _token, _currentPeriod);
     ...
     */
    function _updatePendingRewards(address _gauge, address _token, uint _period) internal returns (uint _amount) {
        _amount = 0;
        ...
        for (uint i = 0; i < pendingRewardAddressLength; i++) {
            ...
            uint _lrAmount = calculatePendingGaugeAmount(_gauge, _token, _pendingRewardAddress, _period);
    ...
    function calculatePendingGaugeAmount(address _gauge, address _token, address _pendingRewardAddress, uint _period) internal returns (uint _amount) {
        _amount = 0;
        Reward memory pr = pendingPricedRewards[_gauge][_token][_pendingRewardAddress];

        uint _totalWeight = IGaugeController(gaugeControllerAddress).points_weight(_gauge, _period).bias;

        uint totalGaugeWeight = _totalWeight - blacklistedWeightPerPeriod[_gauge][_period];

        uint voterEarnedRewards = _voterEarnedRewards(pr.pricePerPercent, totalGaugeWeight);
    ...
    function _calculateBlacklistedWeight(address _gauge, uint _period) internal view returns (uint) {
        uint _totalWeight = 0;

        // factor out the general blacklisted addresses
        uint length = blacklistedAddresses.length;
        for (uint i = 0; i < length; i++) {
            // Continue if gauge blacklist status false
            if (!blacklistedAddressStatus[blacklistedAddresses[i]]) {
                continue;
            }

            IGaugeController.VotedSlope memory _blacklistedSlope = IGaugeController(gaugeControllerAddress).vote_user_slopes(blacklistedAddresses[i], _gauge);
            uint _timeRemaining = _blacklistedSlope.end - _period;
            uint _blacklistedWeight = _timeRemaining * _blacklistedSlope.slope;
            _totalWeight += _blacklistedWeight;
        }

        // factor out the gauge specific blacklisted addresses
        uint gaugeLength = blacklistedGaugeAddresses[_gauge].length;
        for (uint i = 0; i < gaugeLength; i++) {
            // Continue if gauge blacklist status if false
            if (!blacklistedGaugeAddressStatus[_gauge][blacklistedGaugeAddresses[_gauge][i]]) {
                continue;
            }

            IGaugeController.VotedSlope memory _blacklistedSlope = IGaugeController(gaugeControllerAddress).vote_user_slopes(blacklistedGaugeAddresses[_gauge][i], _gauge);
            uint _timeRemaining = _blacklistedSlope.end - _period;
            uint _blacklistedWeight = _timeRemaining * _blacklistedSlope.slope;
            _totalWeight += _blacklistedWeight;
        }
    ...
    function blacklistAddress(address _blacklistedAddress) external onlyOwner notBlacklisted(_blacklistedAddress) {
        blacklistedAddresses.push(_blacklistedAddress);
        blacklistedAddressStatus[_blacklistedAddress] = true;
    }

    function blacklistGaugeAddress(address _gauge, address _blacklistedAddress) external onlyOwner notGaugeBlacklisted(_gauge, _blacklistedAddress) {
        blacklistedGaugeAddresses[_gauge].push(_blacklistedAddress);
        blacklistedGaugeAddressStatus[_gauge][_blacklistedAddress] = true;
    }

    function unBlacklistAddress(address _blacklistedAddress) external onlyOwner {
        blacklistedAddressStatus[_blacklistedAddress] = false;
    }

    function unBlacklistGaugeAddress(address _gauge, address _blacklistedAddress) external onlyOwner {
        blacklistedGaugeAddressStatus[_gauge][_blacklistedAddress] = false;
    }

    function removeBlacklistAddress(address _blacklistedAddress) external onlyOwner {
        uint blacklistedAddressLength = blacklistedAddresses.length;
        for (uint i = 0; i < blacklistedAddressLength; i++) {
            if (blacklistedAddresses[i] == _blacklistedAddress) {

                // copies last element to current element and pops off the end
                blacklistedAddresses[i] = blacklistedAddresses[blacklistedAddressLength-1];
    ...
    function removeBlacklistGaugeAddress(address _gauge, address _blacklistedAddress) external onlyOwner {
        uint blacklistedAddressLength = blacklistedGaugeAddresses[_gauge].length;
        for (uint i = 0; i < blacklistedAddressLength; i++) {
            if (blacklistedGaugeAddresses[_gauge][i] == _blacklistedAddress) {

                // copies last element to current element and pops off the end
                blacklistedGaugeAddresses[_gauge][i] = blacklistedGaugeAddresses[_gauge][blacklistedAddressLength-1];
                blacklistedGaugeAddresses[_gauge].pop();
                return;
            }
        }
    }
    ...
    function updateMaximumRewardsPerGauge(uint256 _numRewards) public onlyOwner {
        require(_numRewards > 0, "Must accepted at least one reward.");
        maximumRewardsPerGauge = _numRewards;
    }
    /* ========== END OWNER FUNCTIONS ========== */
    ...
    modifier notBlacklisted(address _address) {
        require(blacklistedAddressStatus[_address] == false, "Blacklisted Aggregated Multisig");
        _;
    }

    modifier notGaugeBlacklisted(address _gauge, address _address) {
        require(blacklistedGaugeAddressStatus[_gauge][_address] == false, "Blacklisted Gauge Multisig");
        _;
    }
}
