...
contract MiaocraftMetadata is IMetadata, Initializable, Ownable {
    using Base64 for bytes;
    using DecimalString for uint256;
    using Strings for uint256;

    IMiaocraft public miaocraft;
    IMiaocraftRenderer public renderer;
    ISpatialSystem public spatialSystem;

    uint256 maxWidth = 25;
    uint256 maxHeight = 25;
    ...
    function initialize(
        ...
        renderer = IMiaocraftRenderer(renderer_);
    ...
    function getMetadata(uint256 tokenId) public view returns (string memory) {
        uint256 spins = miaocraft.spinsOf(tokenId);
        ShipInfo memory info = miaocraft.getShipInfo(tokenId);

        string[] memory attributes = new string[](7);

        attributes[0] = FormatMetadata.formatTraitNumber(
            "Spins",
            spins.toDecimalString(1),
            "number"
        );

        {
            uint256 spinsDecay = miaocraft.spinsDecayOf(tokenId);
            uint256 netSpins = spins - spinsDecay;

            attributes[1] = FormatMetadata.formatTraitNumber(
                "Net Spins",
                netSpins.toDecimalString(1),
                "number"
            );
            attributes[2] = FormatMetadata.formatTraitNumber(
                "Health",
                (netSpins * 100) / spins,
                "boost_percentage"
            );
        ...
        {
            (int256 x, int256 y) = spatialSystem.coordinate(
                tokenToEntity(address(miaocraft), tokenId)
            );
            x /= int256(ASTEROID_COORD_PRECISION);
            y /= int256(ASTEROID_COORD_PRECISION);

            attributes[3] = FormatMetadata.formatTraitNumber("x", x, "number");
            attributes[4] = FormatMetadata.formatTraitNumber("y", y, "number");
        }

        attributes[5] = FormatMetadata.formatTraitNumber(
            "Last Service Time",
            info.lastServiceTime,
            "date"
        );

        return
            FormatMetadata.formatMetadata(
                bytes(info.name).length > 0 ? info.name : tokenId.toString(),
                description,
                _getEncodedSvg(Math.max(spins / 1e18, 1), tokenId),
                attributes,
                ""
            );
    }

    function _getEncodedSvg(uint256 spins, uint256 seed)
        internal
        view
    ...
    */
    ...
    function setMiaocraft(address miaocraft_) public onlyOwner {
        miaocraft = IMiaocraft(miaocraft_);
    }
    ...
    function setSpatialSystem(address spatialSystem_) public onlyOwner {
        spatialSystem = ISpatialSystem(spatialSystem_);
    }

    function setMaxWidth(uint256 maxWidth_) public onlyOwner {
        maxWidth = maxWidth_;
    }

    function setMaxHeight(uint256 maxHeight_) public onlyOwner {
...
library DecimalString {
    ...
    function toDecimalString(
        ...
        uint256 decimalsToShift = decimals - precision;
        uint256 shiftedValue = value / (10**decimalsToShift);
        uint256 decimalValue = shiftedValue % (10**precision);
        uint256 integerValue = shiftedValue / (10**precision);
        return
            string(
                abi.encodePacked(
    ...
    function toDecimalString(uint256 value, uint256 precision)
        internal
        pure
        returns (string memory)
    {
        return toDecimalString(value, precision, 18);
    }
}
