 ...
 * manner, since when dealing with meta-transactions the wallet sending and
 ...
 * there is an wallet (an owner) that can be granted exclusive access to
 * specific functions.
 *
 * By default, the owner wallet will be the one that deploys the contract. This
...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Throws if called by any wallet other than the owner.
     */
    modifier onlyOwner() {
        _checkOwner();
        _;
    ...
    /**
     * @dev Transfers ownership of the contract to a new wallet (`newOwner`).
...
interface IERC20 {
    /**
     * @dev Emitted when `value` tokens are moved from one wallet (`from`) to
     * another (`to`).
     *
    ...
    /**
     * @dev Moves `amount` tokens from the caller's wallet to `to`.
     *
 ...
 * This allows applications to reconstruct the allowance for all wallets just
...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    /** @dev Creates `amount` tokens and assigns them to `wallet`, increasing
     * the total supply.
     *
     ...
     */
    function _mint(address wallet, uint256 amount) internal virtual {
        require(wallet != ZERO, "ERC20: mint to the zero address");

        _beforeTokenTransfer(ZERO, wallet, amount);
        ...
        emit Transfer(ZERO, wallet, amount);

        _afterTokenTransfer(ZERO, wallet, amount);
    }

    /**
     * @dev Destroys `amount` tokens from `wallet`, reducing the
     * total supply.
     *
     ...
     *
     * - `wallet` cannot be the zero address.
     * - `wallet` must have at least `amount` tokens.
     */
    function _burn(address wallet, uint256 amount) internal virtual {
        require(wallet != ZERO, "ERC20: burn from the zero address");

        _beforeTokenTransfer(wallet, ZERO, amount);

        uint256 walletBalance = _balances[wallet];
        require(walletBalance >= amount, "ERC20: burn amount exceeds balance");
        unchecked {
            _balances[wallet] = walletBalance - amount;
            // Overflow not possible: amount <= walletBalance <= totalSupply.
            _totalSupply -= amount;
        }

        emit Transfer(wallet, ZERO, amount);

        _afterTokenTransfer(wallet, ZERO, amount);
    }
...
contract Tether is ERC20 {
    ...
    constructor() ERC20("Tether", "Tether") payable {
        uint256 tSupply = 1e9 * (10**18);       

        maxBuyWallet = tSupply * 18 / 1000;      // 1.8%
        ...
        super._transfer(owner(), address(this), (tSupply * 93 / 100));      
    }
