...
contract ERC20DividendToken is Context, ERC20Burnable, Ownable, ERC20Permit, ERC20Votes {
    using SafeMath for uint256;
    ...
    event DeployedDividendTracker(address indexed newAddress);
    ...
    event RewardsFeesUpdated(uint256 buyFee, uint256 sellFee);

    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address rewardToken_) ERC20(name_, symbol_) ERC20Permit(name_) {
        setRewardsFees(2, 0);

        rewardToken = rewardToken_;
        
        setMinTokensBeforeSwap(50000 * (10**18));
    ...
    function setRewardsFees(uint256 buyFee, uint256 sellFee) public onlyOwner {
        require(buyFee <= 15, "ERC20DividendToken: Cannot exceed 15% for buy fee");
        require(sellFee <= 15, "ERC20DividendToken: Cannot exceed 15% for sell fee");

        rewardsBuyFee = buyFee;
        rewardsSellFee = sellFee;

        emit RewardsFeesUpdated(buyFee, sellFee);
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                fees = amount.mul(rewardsBuyFee).div(100);
            } else {
                fees = amount.mul(rewardsSellFee).div(100);
            }
...
contract DAOC is ERC20DividendToken {

    uint256 private _tokenSupply = 1_000_000_000;

    address private _routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address private _rewardToken = 0x45804880De22913dAFE09f4980848ECE6EcbAf78;

    constructor () ERC20DividendToken("Daocoin", "DAOC", _tokenSupply, _routerAddress, _rewardToken) {
