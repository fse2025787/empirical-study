...
contract MarketPlace is OwnableUpgradeable, Payments {
    ...
    // purplePigetToken IERC20 refference
    IERC20 public purplePigetToken;
    ...
    // Mapping from ERC721 contract to mapping of tokenId to sale price using Token PurplePiget
    mapping(address => mapping(uint256 => SalePrice)) private tokenPricesPurplePiget;
    ...
    // Mapping from ERC721 contract to mapping of tokenId to current bid using Token PurplePiget
    mapping(address => mapping(uint256 => ActiveBid)) private tokenCurrentBidsPurplePiget;
    ...
    /////////////////////////////////////////////////////////////////////////
    // setSalePrice For PurplePigetToken
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Set the token for sale. The owner of the token must be the sender and have the marketplace approved.
     * @param _originContract address of the contract storing the token.
     ...
     * @param _amount uint256 wei value that the item is for sale
     */
    function setSalePriceForPurplePiget(
        address _originContract,
        uint256 _tokenId,
        uint256 _amount
    ) external {
        ...
        tokenPricesPurplePiget[_originContract][_tokenId] = SalePrice(msg.sender, _amount);
        emit SetSalePrice(_originContract, _amount, _tokenId);
    }
     ...
     * @param _amount uint256 wei amount expecting to purchase the token for.
     */
    function safeBuyUsingPurplePiget(
        address _originContract,
        uint256 _tokenId,
        uint256 _amount
    ) external payable {
        // Make sure the tokenPrice is the expected amount
        require(
            tokenPricesPurplePiget[_originContract][_tokenId].amount == _amount,
            "safeBuy::Purchase amount must equal expected amount"
        );
        buyUsingPurplePiget(_originContract, _tokenId);
    }
     ...
     * @param _tokenId uint256 ID of the token.
     */
    function buyUsingPurplePiget(address _originContract, uint256 _tokenId) public payable {
        // The owner of the token must have the marketplace approved
        ownerMustHaveMarketplaceApproved(_originContract, _tokenId);
        ...
        // Check that the person who set the price still owns the token.
        require(
            erc721.ownerOf(_tokenId) == tokenPricesPurplePiget[_originContract][_tokenId].seller,
            "buy::Current token owner must be the person to have the latest price."
        );

        SalePrice memory sp = tokenPricesPurplePiget[_originContract][_tokenId];

        // Check that token is for sale.
        require(sp.amount > 0, "buy::Tokens priced at 0 are not for sale.");

        require(
            purplePigetToken.allowance(msg.sender, address(this)) >= sp.amount,
            'Not enough Allowance for PurplePiget Tokens'
        );
        ...
        purplePigetToken.transferFrom(msg.sender, tokenOwner, (sp.amount) );
     ...
     * @return uint256 sale price of the token
     */
    function tokenPriceInPurplePiget(address _originContract, uint256 _tokenId)
        external
        view
        returns (uint256)
    {
        ...
        if (erc721.ownerOf(_tokenId) == tokenPricesPurplePiget[_originContract][_tokenId].seller) {
            return tokenPricesPurplePiget[_originContract][_tokenId].amount;
        }
        return 0;
    }
    ...
    /////////////////////////////////////////////////////////////////////////
    // tokenPricePurplePigetFeeIncluded
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Gets the sale price of the token including the marketplace fee.
     * @param _originContract address of the contract storing the token.
     ...
     * @return uint256 sale price of the token including the fee.
     */
    function tokenPricePurplePigetFeeIncluded(address _originContract, uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        ...
        if (_priceSetterStillOwnsTheToken(_originContract, _tokenId)) {
            return
                tokenPricesPurplePiget[_originContract][_tokenId].amount.add(0);
        }
        return 0;
    }
    ...
    function changePurplePigetToken (address PurplePiget) public onlyOwner returns (address){
        purplePigetToken = IERC20(PurplePiget);
        return address(purplePigetToken);
    }
