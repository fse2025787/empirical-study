...
contract KujiraWhales is ERC721A, Ownable {
    ...
    function whitelistMint(uint256 _nbr) public {
        ...
        require(numberToMint[msg.sender] >= _nbr, "you don't have enough nft to mint");
    ...
    function setOwner(address[] memory _addresses, uint256[] memory _nbr) public onlyOwner {
        require(_addresses.length > 0, "array addresses is empty");
        require(_nbr.length > 0, "array nbr is empty");
        require(_nbr.length == _addresses.length, "not same size");
        for (uint256 i = 0; i < _nbr.length; i++) {
            numberToMint[_addresses[i]] = _nbr[i];
        }
    }
