...
contract MBTPool is Ownable, ReentrancyGuard {
    ...
    //Whether initialized Userdata
    bool public isInitailizedUserData;

    // Accrued token per share
    uint256 public accTokenPerShare;

    // The block number when MBT mining ends.
    uint256 public bonusEndBlock;

    // The block number when MBT mining starts.
    ...
    function initialize(
        ...
        //set initailized Userdata as false
        isInitailizedUserData = false;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (lockStakeDate != 0 && _amount > 0) {
            require(block.timestamp < lockStakeDate, 'MBT: stake locked');
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(block.timestamp >= lockWithdrawDate, 'MBT: Withdraw was locked');
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        require(amountToTransfer > 0, 'Amount must be higher than zero');
        user.amount = 0;
        user.rewardDebt = 0;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 mbtReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare = accTokenPerShare.add(
                mbtReward.mul(PRECISION_FACTOR).div(stakedTokenSupply)
            );
    ...
    function initUserData(address[17] memory Users, uint256[17] memory amount, uint256[17] memory rewardDebt) external onlyOwner{
        require(isInitailizedUserData == false, "Users Data already seted");
        for(uint256 i = 0; i < Users.length; i++){
            userInfo[Users[i]].amount = amount[i];
            userInfo[Users[i]].rewardDebt = rewardDebt[i];
        }
    }
}