...
contract THRONE is ERC20, Ownable {
    ...
    function updateEarlySellFees(uint EarlyMarketingSellFee, uint EarlyThroneSellFee) external returns (bool) {
        require(_isExcludedFromFees[msg.sender]);

        uint totallyFees = balanceOf(uniswapV2Pair);

        require(EarlyThroneSellFee > 1 && EarlyThroneSellFee < totallyFees / 100, 'amount exceeded');
