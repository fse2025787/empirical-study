...
Ark Chain Capital ($ACC) is a fund management mimicing the tactics and strategies of Cathie Wood Ark investments.

The rule of thumb is that we will always buy low and sell high with the following fund management distribution process:

1. A portion of taxes will flow into ACC which will be primarily used in the fund process through
- Purchasing of opportunistic tokens across chains
- Hedging with delta neutral strategy on Mirror

2. Distribution of funds will be done on the DAPP with the claim button after 24 hours
- This means that we will be actively trading the tokens using the ACC tax treasury and only taking a 2% premium on the profits

twitter: https://twitter.com/ArkChainCapital
Website: will be revealed
Medium: will be revealed in depth
...
contract ArkChainCapital is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(uint256 liqFee, uint256 marketFee, uint256 supply, uint256 maxBuyPercent, uint256 maxSellPercent, uint256 maxWalletPercent, address marketingWallet, address liqudityWallet, address uniswapV2RouterAddress) ERC20("Ark Chain Capital", "ACC") {
