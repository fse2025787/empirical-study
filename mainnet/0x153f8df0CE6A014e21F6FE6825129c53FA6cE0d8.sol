...
contract KILLAGEAR is ERC1155, Ownable {
    ...
    address public immutable killabitsAddress;
    mapping(uint256 => bool) public compatibleTraits;
    ...
    constructor(address killabits) ERC1155("") {
        killabitsAddress = killabits;
    }
    ...
    function tokenize(
        ...
        if (msg.sender != killabitsAddress) revert NotAllowed();
        if (!compatibleTraits[typeId]) revert IncompatibleTrait();
    ...
    function toggleCompatibility(uint256[] calldata typeIds, bool compatible)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < typeIds.length; i++) {
            compatibleTraits[typeIds[i]] = compatible;
        }
    }
