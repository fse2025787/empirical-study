...
contract IRamura is Ownable, ERC721Enumerable, Pausable {
    ...
    uint256 private baseTime = 1641016800;  // 6h00m00s 1/1/2022;
    ...
    modifier mintRole() {
        require(whitelisted[msg.sender] && block.timestamp <= deadline, "Only whitelited can mint");
        _;
    }
    ...
    function freeMint() external mintRole {
        require(totalNFT < 4444 && !_isMint[msg.sender], "Full NFT or Have mint");
    ...
    function setDeadlineTime(uint256 time) external onlyOwner {
        deadline = time;
    }

    function addWhitelisted(address[] calldata list) external onlyOwner {
        for(uint i =0; i < list.length ; i ++) {
            whitelisted[list[i]] = true;
        }
    }

    function addWhitelisteDemoncell(address[] calldata list) external onlyOwner {
        for(uint i =0; i < list.length ; i ++) {
