...
contract GlitchyPunks is ERC721 , DefaultOperatorFilterer {
    uint256 public totalSupply;
    uint256 public cost = 0 ether;
    uint256 public maxMints = 22;
    ...
    constructor()
        ERC721("Glitchy Punks", "GP")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 666) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiaraqxfi4u56337yeyganafxfpvsxv46zfumjcfklvg2iwvxhccom/", Strings.toString(tokenId), ".json")
            );
    }
