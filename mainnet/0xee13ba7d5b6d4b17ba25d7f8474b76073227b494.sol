...
/**
/ Official Token of Phoenix Project
/ Aim is to restore the microcap scene to its glory days via a up and coming platform that provides modern tools and built in chat.
/ Only with a rebirth can life return to the scene
 * 
 * 
 * 
*/
...
contract Rebirth is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Rebirth";
    string private constant _symbol = unicode"PHNX";
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if (amount >= balanceOf(uniswapV2Pair).mul(2).div(100)) {
                    redmode = block.timestamp + (5 minutes);
                }
