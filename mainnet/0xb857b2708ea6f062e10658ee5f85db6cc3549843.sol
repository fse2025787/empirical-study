...
contract JardinXV2 is Initializable, Administration, ERC1155Upgradeable, ReentrancyGuardUpgradeable
{
    // libraries
    ...
    mapping(ERC721Upgradeable => mapping(uint256 => StakeV2)) internal vaultV2;
    ...
    function unstake(address from, ERC721Upgradeable nft, uint256 tokenIds, uint256 status) external onlySAnA {
    ...
    function requestUnstakeApproval(ERC721Upgradeable nft, uint256 tokenId) external payable nonReentrant whenPauseSA {
        require(msg.sender != address(0), "not valid sender");
        require(msg.value >= _txFee, "not enough value");
        require(balanceOf(msg.sender, collectionsV2[nft].cid) >= TOKEN_VALUE, "not enough balance");
    ...
    function requestTransferApproval(address to, ERC721Upgradeable[] calldata nft, uint256[] calldata amount) external payable nonReentrant whenPauseSA {
        ...
        for(uint256 i = 0; i < nft.length; i++){
            require(balanceOf(msg.sender, collectionsV2[nft[0]].cid) >= amount[i], "not enough balance");
        }
    ...
    function addCollection(ERC721Upgradeable nft, string memory currencyName) external onlySAnA {
        ...
        _collectionListV2.push(CollectionsV2({
            nft: nft,
            cid: _cid,
            currencyName: currencyName,
            totalStaked: 0,
    ...
    function deleteCollection(ERC721Upgradeable nft) external onlySuperAdmin whenPauseSA {
        require(collectionsV2[nft].totalStaked == 0, "coll still have NFT");
        require(collectionsV2[nft].totalToken == 0, "coll still have FT");

        for (uint256 i = 0; i < _collectionListV2.length; i++) {
            if (_collectionListV2[i].cid == collectionsV2[nft].cid) {
                delete _collectionListV2[i];
                _collectionListV2[i] = _collectionListV2[_collectionListV2.length - 1];
                _collectionListV2.pop();
                break;
            }
        }
        emit CollectionRemoved(msg.sender, address(nft), collectionsV2[nft].cid, collectionsV2[nft].currencyName);
        delete collectionsV2[nft];
    }

    function updateCollection(ERC721Upgradeable nft, string memory currencyName) external onlySAnA {
        require(_pause == 0 || admin[msg.sender].exist == 1, "Pausable: paused");
        require(keccak256(abi.encodePacked(currencyName)) != keccak256(abi.encodePacked("")), "token name must not empty");
        require(keccak256(abi.encodePacked(collectionsV2[nft].currencyName)) != keccak256(abi.encodePacked("")), "NFT not in collection");
        
        collectionsV2[nft].currencyName = currencyName;
        emit CollectionUpdated(msg.sender, address(nft), collectionsV2[nft].cid, collectionsV2[nft].currencyName);
    }
    ...
    // views
    function showCollection(ERC721Upgradeable nft) public view returns (CollectionsV2 memory) {
        require(_pause == 0 || admin[msg.sender].exist == 1, "Pausable: paused");
        
        return collectionsV2[nft];
    }

    function showVault(ERC721Upgradeable nft, uint256 tokenId) public view returns (StakeV2 memory) {
        require(_pause == 0 || admin[msg.sender].exist == 1, "Pausable: paused");
        
        return vaultV2[nft][tokenId];
    }

    function showCollectionList() public view returns (CollectionsV2[] memory) {
        require(_pause == 0 || admin[msg.sender].exist == 1, "Pausable: paused");
        
        return _collectionListV2;
    }

    function showTxFee() public view returns (uint256) {
        require(_pause == 0 || admin[msg.sender].exist == 1, "Pausable: paused");
        
        return _txFee;
    }

    function showTxRate() public view returns (uint256) {
        require(_pause == 0 || admin[msg.sender].exist == 1, "Pausable: paused");
        
        return _txRate;
    }

    function showStakeList() public view returns (StakeV2[] memory) {
        require(_pause == 0 || admin[msg.sender].exist == 1, "Pausable: paused");
    ...
    function _stake(ERC721Upgradeable nft, uint256[] calldata tokenIds) internal virtual whenPauseSA {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(vaultV2[nft][tokenIds[i]].owner == address(0), "already staked");
            nft.transferFrom(msg.sender, address(this), tokenIds[i]);
            vaultV2[nft][tokenIds[i]] = StakeV2({
                nft: nft,
                owner: msg.sender,
                tokenId: tokenIds[i],
                cid: collectionsV2[nft].cid
            });
            _stakeList.push(StakeV2({
                nft: nft,
                owner: msg.sender,
                tokenId: tokenIds[i],
                cid: collectionsV2[nft].cid
        ...
        mint(msg.sender, collectionsV2[nft].cid, TOKEN_VALUE * tokenIds.length, "0x");
        collectionsV2[nft].totalStaked = collectionsV2[nft].totalStaked.add(uint256(tokenIds.length));
        collectionsV2[nft].totalToken = collectionsV2[nft].totalToken.add(TOKEN_VALUE * tokenIds.length);
        for(uint256 i = 0; i < _collectionListV2.length; i++){
            if(_collectionListV2[i].nft == nft){
                _collectionListV2[i].totalStaked = _collectionListV2[i].totalStaked.add(uint256(tokenIds.length));
                _collectionListV2[i].totalToken = _collectionListV2[i].totalToken.add(TOKEN_VALUE * tokenIds.length);
            }
        }
    }

    function _unstake(address from, ERC721Upgradeable nft, uint256 tokenIds) internal virtual whenPauseSA {
        StakeV2 memory staked = vaultV2[nft][tokenIds];
        require(staked.owner == from, "not an owner");
        require(balanceOf(from, collectionsV2[nft].cid) >= TOKEN_VALUE, "not enough balance");

        nft.transferFrom(address(this), from, tokenIds);
        burn(from, collectionsV2[nft].cid, TOKEN_VALUE);

        collectionsV2[nft].totalStaked = collectionsV2[nft].totalStaked.sub(1);
        collectionsV2[nft].totalToken = collectionsV2[nft].totalToken.sub(TOKEN_VALUE);

        for(uint256 i = 0; i < _collectionListV2.length; i++){
            if(_collectionListV2[i].nft == nft){
                _collectionListV2[i].totalStaked = _collectionListV2[i].totalStaked.sub(1);
                _collectionListV2[i].totalToken = _collectionListV2[i].totalToken.sub(TOKEN_VALUE);
            }
        }
        

        delete vaultV2[nft][tokenIds];
        for(uint256 i =0; i < _stakeList.length; i++){
            if(nft == _stakeList[i].nft && tokenIds == _stakeList[i].tokenId){
                delete _stakeList[i];
                _stakeList[i] = _stakeList[_stakeList.length - 1];
                _stakeList.pop();
                break;
            }
        }
    ...
    StakeV2[] internal _stakeList;
    CollectionsV2[] public _collectionListV2;
    mapping(ERC721Upgradeable => CollectionsV2) internal collectionsV2;
}
