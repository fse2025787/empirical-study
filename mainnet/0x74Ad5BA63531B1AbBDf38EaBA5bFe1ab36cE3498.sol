...
contract YogiesCar is ERC721A, Ownable {
    using Strings for uint256;

    // ERC 721
    address public openseaProxyRegistryAddress;
    string public baseURIString = "https://storage.googleapis.com/yogies-assets/metadata/items/car/";
    ...
    constructor(
        ...
        address _staking
    ) ERC721A("Yogies Cars", "Yogies Cars") Ownable() {
    ...
    function _beforeTokenTransfers(
        ...
        if (from != address(0))
            staking.updateAccumulatedReward(from);
        if (to != address(0))
            staking.updateAccumulatedReward(to);
    }
