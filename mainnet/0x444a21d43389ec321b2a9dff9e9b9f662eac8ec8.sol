...
// File: contracts/Dorktownwtf.sol
...
contract Dorktownwtf is ERC721A, ReentrancyGuard, Pausable, AccessControl{

    // public mint variables
    uint256 public maxSupply = 7777;
    uint256 public maxMint = 20;
    uint256 public mintPrice = 0.003 ether; // @dev 10 finney = 0.01 ether
    ...
    function teamMint(address[] calldata _address, uint256 _amount) external nonReentrant {

        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
    ...
    function publicMint(uint256 _quantity) external payable whenNotPaused nonReentrant {
        ...
        if (totalFreeMints[msg.sender] == 0) {
            ...
            require(msg.value >= ((_quantity * mintPrice) - mintPrice), "Not enough ether sent");
    ...
    function togglePublicMint() external nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        publicMintEnabled = !publicMintEnabled;
    }
    function setPrice(uint256 _mintPrice) external nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        mintPrice = _mintPrice;
    }

    function toggleReveal() external nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        revealed = !revealed;
    }

    function setmaxMintAmount(uint256 _maxMint) external nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        maxMint = _maxMint;
    }
  
    function pause() public nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        _pause();
    }

    function unpause() public nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        _unpause();
    }

    function setBaseExtension(string memory _newBaseExtension) external nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        baseExtension = _newBaseExtension;
    }

    function setBaseURI(string memory _newURI) public nonReentrant{
        require(hasRole(TEAM_ROLE, msg.sender), "Caller is not a team member");
        baseURI = _newURI;
    }
    ...
    function withdraw(address _to) external  nonReentrant {
        ...
        payable(0x9faEB44C7A59CA9eED6B3E4ecC894ECeaA19De11).transfer(address(this).balance * 5 / 100);
