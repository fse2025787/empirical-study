...
contract CSMCrossChainRouter is ICSMCrossChainRouter, AccessControl, ReentrancyGuard {
    ...
    function handleReceive(
        ...
        IPoolCrossChain pool = _poolsPerAssets[dstAsset];
        require(dstChainId == pool.chainId(), "WRONG_CHAIN");

        _poolsPerAssets[dstAsset].receiveSwapCrossChain(sender, srcAsset, srcChainId, amount, dstAsset, haircut);

        emit MessageReceived(sender, srcChainId, srcAsset, dstAsset, nonce_, amount, haircut);
    }
