...
contract CrossTowerStorefrontUserToken1155 is ERC1155 {

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    constructor (string memory name, string memory symbol, string memory tokenURIPrefix, address operator) ERC1155 (name, symbol) {
        _owner = msg.sender;
        newItemId = 1;
    ...
    function mint( string memory uri, uint256 supply, uint256 fee) external {
        _mint(msg.sender, newItemId, supply, uri,fee);
