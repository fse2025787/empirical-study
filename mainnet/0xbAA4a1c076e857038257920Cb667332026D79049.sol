...
contract TenkEvin is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        require(
            totalSupply + _mintAmount <= _publicSupply(),
    ...
    function mintPresale(uint256 _mintAmount) external payable {
        ...
        uint256 totalSupply = totalSupply();
        uint256 availableMints = MAX_PRESALE_MINT -
            presaleWhitelistMints[msg.sender];
    ...
    function addToWhitelist(address[] calldata _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            presaleWhitelistMints[_addresses[i]] > 0
                ? presaleWhitelistMints[_addresses[i]]
                : 0;
        }
    }
    ...
    function reserve(address _to, uint256 _reserveAmount) public onlyOwner {
        ...
        require(
            _reserveAmount > 0 &&
                _reserveAmount <= TEAM_RESERVE_AVAILABLE_MINTS,
