...
contract PixelPumpkins is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC721("Pixel Pumpkins", "PUMPK") {
        destination = msg.sender;
    }

    function mint(uint256 numberOfTokens) external payable {
        
    require(isActive == true || presaleActive == true,"Sale is not active");
        
    if (isActive == true){
        _verifyMintPumpkin(numberOfTokens);
        ...
        require(whitelistedUser[msg.sender] == true,"You're not whitelisted.");
        require(amountBought[msg.sender] + numberOfTokens<= 10,"You are buying more than allowed.");
        _verifyMintPumpkin(numberOfTokens);
    ...
    function bulkWhitelist (address[] memory addressList) public onlyOwner{
        uint256 size = addressList.length;
        for (uint i = 0; i<size;i++){
            whitelistedUser[addressList[i]] = true;
        }
    }

    function _verifyMintPumpkin(uint256 numberOfTokens) private {
        require(numberOfTokens <= maxPurchase, "Minting more than 20");
        require(
    ...
    function reservePumpkins() external onlyOwner {
