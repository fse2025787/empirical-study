 ...
 * the owner.
 */
abstract contract Ownable is Context ,BaseRelayRecipient {
 ...
 */
abstract contract BEP20 is Context,BaseRelayRecipient, IBEP20, Ownable {
...
abstract contract Pausable is Context,BaseRelayRecipient {
    /**
...
// Monetas with Governance.
contract Monetas is BaseRelayRecipient, BEP20('Monetas', 'MNTG'),Pausable{
   ...
   mapping(address => mapping(address => uint256)) private _allowances;
    ...
    function mint(address _to, uint256 _amount) whenNotPaused()  public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }
    
    function burn(address _to, uint256 _amount) whenNotPaused()  public onlyOwner {
        _burn(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }
    function mint(uint256 amount) whenNotPaused() public onlyOwner returns (bool) {
        _mint(_msgSender(), amount);
        return true;
    }

   function burn(uint256 amount) whenNotPaused() public onlyOwner returns (bool) {
    ...
    function unpause() public onlyOwner whenPaused {
      ...
      function transfer(address recipient, uint256 amount) whenNotPaused() public override returns (bool) {
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) whenNotPaused() public override returns (bool) {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "MNTG::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MNTG::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "MNTG::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) 
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "MNTG::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying APESs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "MNTG::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function setTrustedForwarder(address forwarder) onlyOwner public {
        trustedForwarder = forwarder;
    }
}
