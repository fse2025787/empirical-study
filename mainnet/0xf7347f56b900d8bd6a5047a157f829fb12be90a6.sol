...
contract HyenaHypeClub is ERC721Enumerable, Ownable {
    ...
    uint256 public HHC_MAX = 9999;
    uint256 private PRESALE_PRICE = 0.15 ether;
    uint256 private PUBLIC_PRICE = 0.15 ether;
    uint256 private REVEAL_DELAY = 0 hours;
    uint256 private PRESALE_HOURS = 0 hours;
    uint256 private PURCHASE_LIMIT = HHC_MAX;
    uint256 private PRESALE_MINT_LIMIT = HHC_MAX;
    uint256 private PUBLIC_MINT_LIMIT = HHC_MAX;
    uint256 private STORAGE_PERCENT = 0;
    ...
    Counters.Counter private _publicHHC;

    constructor() ERC721("Hyena Hype Club", "HHC") {}

    function setPaymentAddress(address paymentAddress, uint256 fee) external onlyOwner {
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicHHC.current();

                if (_publicHHC.current() < HHC_MAX) {
                    _publicHHC.increment();
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(_publicHHC.current() < HHC_MAX,"Purchase would exceed HHC_MAX");
        ...
        if (msg.sender != owner()) {
            ...
            if (STORAGE_PERCENT > 0) {
                uint256 feeAmount = (msg.value * STORAGE_PERCENT) / 100;
