...
contract TRAC is ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
   ...
   */
  function claim(uint16 m0, uint16 m1, uint16 m2, uint16 m3, uint16 m4) external nonReentrant {
    address to = msg.sender;
    require(tx.origin == to, "eos only");
    require(claimActive || to == owner(), "not active");

    uint16 amount = m0 + m1 + m2 + m3 + m4;
    uint256 tokenId = minted + 1;
    uint256 endMint = minted + amount;
    require(endMint <= MAX_SUPPLY, "supply exhausted");

    _minters[uint16(tokenId)] = OwnerTime(to, uint48(block.timestamp));
    _balance[to] += amount;
    minted = uint16(endMint);

    _serums.burnBatch(to, m0, m1, m2, m3, m4);

    for (; tokenId <= endMint; tokenId++) {
      emit Transfer(address(0), to, tokenId);
      require(
