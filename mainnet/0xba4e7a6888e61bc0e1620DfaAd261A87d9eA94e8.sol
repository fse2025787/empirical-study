...
// File contracts/Bridges/Avalanche/FraxLiquidityBridger_AVAX_AnySwap.sol
...
contract FraxLiquidityBridger_AVAX_AnySwap is FraxLiquidityBridger {
    ...
    function _bridgingLogic(uint256 token_type, address address_to_send_to, uint256 token_amount) internal override {
        ...
        if (token_type == 0){
            ...
            // AnySwap Bridge
            TransferHelper.safeTransfer(address(FRAX), address_to_send_to, token_amount);
        }
        else if (token_type == 1) {
            // L1 FXS -> anyFXS
            // Simple dump in / CREATE2
            // AnySwap Bridge
            TransferHelper.safeTransfer(address(FXS), address_to_send_to, token_amount);
        }
        else {
            // L1 USDC -> USDC.e
            // Simple dump in / CREATE2
            // AEB / Official Avalanche Bridge
            TransferHelper.safeTransfer(address(collateral_token), address_to_send_to, token_amount);
        }
    }
