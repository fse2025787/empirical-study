...
/*
  ...
  TwoHoursChill
  - Social media: @TwoHoursChill
  - Telegram: @TwoHoursChillGroup
  - Web: TwoHoursChill.com
  
  Understanding the tokenomics
  
  -> 12% fees on all transfers  (4% burn / 4% reflection / 4% liquidity)
  -> 4% burn actually destroys liquidity tokens which is why you'll see total supply decrease over time.
  -> Cooldown of 2 hours after any transfer. Buy / sell.
  -> Max transaction size of: 3,000,000,000,000 (3Trillion)
        This means that if someone with 30Trillion tokens wants to dump it will take atleast 10 
        transactions to do so. Since we have a cooldown of 2 hours after any transaction it's 
        gonna take atleast 20 hours for the person to dump their 30 trillion.
  
  
  YES. There's been reemoved liquidity from this account.
  Those liquidity was from a failing contract. The community came
  together to launch this new one.
...
contract TwoHoursChill is Context, IERC20, Ownable {
    ...
    string private _name = "TwoHoursChill.com";
    string private _symbol = "THC";
    ...
    uint256 private numTokensSellToAddToLiquidity = 3000000000000 * 10**9;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (sender == uniswapV2Pair && recipient != address(uniswapV2Router) && cooldownEnabled) {
            require(amount <= _maxTxAmount);
            require(cooldown[recipient] < block.timestamp);
            cooldown[recipient] = block.timestamp + (_cooldownSeconds * 1 seconds);
        }
        if (recipient == uniswapV2Pair && sender != address(uniswapV2Router) && cooldownEnabled) {
            require(amount <= _maxTxAmount);
            require(cooldown[sender] < block.timestamp);
            cooldown[sender] = block.timestamp + (_cooldownSeconds * 1 seconds);
        }
