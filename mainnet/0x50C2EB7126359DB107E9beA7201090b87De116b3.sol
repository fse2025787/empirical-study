...
contract TestPass is ERC721URIStorage, Ownable {
    ...
    function isValidUser(CouponData memory meta, Coupon memory coupon)
        ...
        returns (bool)
    {
        return _isValidCoupon(meta, coupon);
    }
        ...
        returns (uint256)
    {
        ...
        // Existing pass check
        require(!whitelistClaimed[msg.sender], "User has pass already");

        require(_isValidCoupon(meta, coupon), "None whitelisted user");

        tokenId.increment();
        uint256 id = tokenId.current();
        _safeMint(msg.sender, id);
        string memory passURI = "https://ipfs.io/ipfs/QmXYnYZgQ6tghHi1oQyygsCJqXSsusea1gsFmC87Zqvg7r";
    ...
    function _isValidCoupon(CouponData memory meta, Coupon memory coupon)
        private
        view
        returns (bool)
    {
        ...
        bytes32 digest = keccak256(abi.encode(2, msg.sender, meta.expire));
        address signer = ecrecover(digest, coupon.v, coupon.r, coupon.s);
        return (signer == _couponOwner);
    }

    function getSigner(CouponData memory meta, Coupon memory coupon)
        public
        view
        onlyOwner
