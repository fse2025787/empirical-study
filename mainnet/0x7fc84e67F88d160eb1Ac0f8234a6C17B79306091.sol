...
library Transfers {
    ...
    function handleTransferWithFees(Data storage data, AntiJeetInuStorage.Data storage _storage, address from, address to, uint256 amount, address referrer) public returns(uint256 fees, uint256 buyerMint, uint256 referrerMint) {
...
library Fees {
    ...
    function init(
        Data storage data,
        AntiJeetInuStorage.Data storage _storage,
    ...
    function updateFeeDestinationPercents(Data storage data, AntiJeetInuStorage.Data storage _storage, uint256 dividendsPercent, uint256 marketingPercent, uint256 mysteryPercent) public {
    ...
    function swapAccumulatedFees(Data storage data, AntiJeetInuStorage.Data storage _storage, uint256 tokenAmount) public {
...
library AntiJeetInuStorage {
    ...
    struct Data {
        ...
        IUniswapV2Pair pair;
        AntiJeetInuDividendTracker dividendTracker;
...
library MaxWalletCalculator {
    function calculateMaxWallet(uint256 totalSupply, uint256 hatchTime) public view returns (uint256) {
        ...
        uint256 antiJeetInuAge = block.timestamp - hatchTime;

        uint256 base = totalSupply * 30 / FACTOR_MAX; // 0.3%
        uint256 increasePerMinute = totalSupply * 10 / FACTOR_MAX; // 0.1%

        uint256 extra = increasePerMinute * antiJeetInuAge / (1 minutes); // up 0.1% per minute
...
contract AntiJeetInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    AntiJeetInu public immutable token;
    ...
    */
    ...
    constructor(address payable owner, address pair, address weth) DividendPayingToken("AntiJeetInuDividendTracker", "AJI_DIVS") {
        token = AntiJeetInu(owner);
    ...
    function _transfer(address, address, uint256) internal override {
        silenceWarning = true;
        require(false, "AntiJeetInuDividendTracker: No transfers allowed");
    }
    ...
    function updateVestingDuration(uint256 newVestingDuration) public onlyOwner {
        require(newVestingDuration <= 2592000, "AntiJeetInuDividendTracker: max vesting duration is 30 days");
    ...
    function updateUnvestedDividendsMarketingFee(uint256 newUnvestedDividendsMarketingFee) public onlyOwner {
        require(newUnvestedDividendsMarketingFee <= 30, "AntiJeetInuDividendTracker: max marketing fee is 30%");
...
contract AntiJeetInu is ERC20, Ownable {
    using SafeMath for uint256;
    using AntiJeetInuStorage for AntiJeetInuStorage.Data;
    using Fees for Fees.Data;
    using BiggestBuyer for BiggestBuyer.Data;
    using Referrals for Referrals.Data;
    using Transfers for Transfers.Data;

    AntiJeetInuStorage.Data private _storage;
    ...
    AntiJeetInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Anti Jeet Inu", "AJI") {
        ...
        _storage.dividendTracker = new AntiJeetInuDividendTracker(payable(address(this)), address(_storage.pair), _storage.router.WETH());

        setupDividendTracker();

        _storage.marketingWallet1 = 0xa8786a101D830cA035C95576e0d3713b023ED10E;
        _storage.marketingWallet2 = 0x46ddc2db443B74fE7Fcf49eEB2acE256c0AB50f5;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        _storage.dividendTracker = AntiJeetInuDividendTracker(payable(newAddress));
