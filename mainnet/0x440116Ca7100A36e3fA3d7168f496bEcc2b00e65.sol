...
contract ShibBurnLottery is Ownable {
    ...
    function activateLottery(Settings calldata _settings) external onlyOwner {
        require(!active, "Lottery already started");
        require(_settings.rewardAmount > 0 && _settings.numOfWinners1 > 0, "Invalid settings");
        require(_settings.rewardAmount <= IERC20(verseAddress).balanceOf(address(this)), "Balance too low");
        active = true;
        lotterySettings[currentLotteryId] = _settings;
    }
