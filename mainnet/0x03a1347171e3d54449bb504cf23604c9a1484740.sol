...
contract ShibaNinjaInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public tradingActiveBlock = block.number;
    // Anti-bot and anti-whale mappings and variables
    mapping(address => uint256) private _holderLastTransferTimestamp; // to hold last Transfers temporarily during launch

    TaxWallets public _taxWallet = TaxWallets ({
        buyback: 0x4865a63a055dB5FbE385b4a64Fd1bD8Ea60108a1,
        marketing: 0xcf987B9eF1402D5483584604B690bFC144cc3437
    });
    ...
    constructor() ERC20("ShibaNinjaInu", "SBINU") {
        ...
        maxSellAmount = totalSupply * 5 / 1000; // 0.5% maxSellAmount
    ...
    function _authorized(address addr) private view returns (bool) {
        return addr == owner() || addr == _taxWallet.marketing || addr == _taxWallet.buyback;
    }
    ...
    function updateMaxAmount(uint256 newBuyAmount, uint256 newSellAmount) external onlyOwner {
        require(newBuyAmount >= (totalSupply() * 5 / 1000)/1e18, "Cannot set maxBuyAmount lower than 0.5%");
        require(newSellAmount >= (totalSupply() * 5 / 1000)/1e18, "Cannot set maxSellAmount lower than 0.5%");
        maxBuyAmount = newBuyAmount * (10**18);
        maxSellAmount = newSellAmount * (10**18);
    }
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _buyBackFee) external authorized {
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _buyBackFee) external authorized {
    ...
    function updateBuyBackWallet(address newWallet) external onlyOwner {
        emit BuyBackWalletUpdated(newWallet, _taxWallet.buyback);
        excludeFromFees(_taxWallet.buyback, false);
        excludeFromMaxTransaction(_taxWallet.buyback, false);
        _taxWallet.buyback = newWallet;
        excludeFromFees(_taxWallet.buyback, true);
        excludeFromMaxTransaction(_taxWallet.buyback, true);
    }
    ...
    function swapBack() private {
        ...
        (success,) = address(_taxWallet.buyback).call{value: address(this).balance}("");
    }
