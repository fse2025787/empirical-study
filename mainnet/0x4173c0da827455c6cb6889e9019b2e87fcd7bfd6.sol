...
contract MainlandPortal {
    ...
    function _emergencyProcessMessageFromChild(bytes memory data, bool reqSuccess, bytes memory signature) external {
        bytes32 hash = keccak256(abi.encodePacked(data, reqSuccess, msg.sender));
        address signer = hash.toEthSignedMessageHash().recover(signature);
        require(signer == admin, "not allowed");
        (address target, bytes[] memory calls ) = abi.decode(data, (address, bytes[]));
        for (uint256 i = 0; i < calls.length; i++) {
            (bool succ, ) = target.call(calls[i]);
            // emit CallMade(target, succ, calls[i]);
    ...
    function _debugValidateAndExtractMessage(bytes memory inputData) external view returns (bytes memory) {
