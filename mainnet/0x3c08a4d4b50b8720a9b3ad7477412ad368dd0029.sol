...
contract loremporium is ERC1155Supply, Ownable {
	...
	public validProof(_proof, _maxAmountKey) {
		...
		walletMints[msg.sender][id] += amount;
	...
	public validProof(_proof, _maxAmountKey) {
		...
		for( uint256 i = 0; i < ids.length; i++ ) {
			...
			require( ids_i < tokenDetails.length, string.concat(UString.uint2str(ids_i), " doesn't exist"));
			require( amounts[i] + totalSupply(ids_i) <= tokenDetails[ids_i].maxSupply, 
				string.concat(UString.uint2str(ids_i), " amount exeeds supply") );
			
			alloc_int = UString.toUint16( _maxAmountKey, ids_i*2 );
			require( amounts[i] + walletMints[msg.sender][ids_i] <= alloc_int, 
				string.concat(UString.uint2str(ids_i), " exceeds allocation") );
	...
	function minterBatch( address _to, uint256[] calldata ids, uint256[] calldata amounts ) public requireManager {
		require( ids.length == amounts.length, "ids and amounts length mismatch" );
		
		for( uint256 i = 0; i < ids.length; i++ ) {
			require( ids[i] < tokenDetails.length, string.concat(UString.uint2str(ids[i]), " doesn't exist"));
			require( amounts[i] + totalSupply(ids[i]) <= tokenDetails[ids[i]].maxSupply, 
				string.concat(UString.uint2str(ids[i]), " amount exeeds supply") );
			
			walletMints[_to][ids[i]] += uint16(amounts[i]);
			
        }
		
		_mintBatch( _to, ids, amounts, "0x" );
	...
	function createToken( uint128 _supply, string calldata _tokenURI ) public requireManager {
		tokenDetails.push();
		...
		tokenDetails[_id].tokenID = uint16(_id);
...
/**
* Flattened contract dependencies for ERC1155 NFT projects with use of Merkle Trees
*
*
...
* 
* 	UString (custom)
* 	OpenZeppelin (on 2022-08-22):
*		MerkleProof.sol (2022-09-15)
* 		ERC1155.sol
* 		IERC1155.sol
...
/**
 * @dev type conversion and other handling utilities
 */
library UString {
