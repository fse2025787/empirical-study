  ...
  AccessControlUpgradeable
{
  ...
  uint256 public FACTOR; // Reward factor 1000 => 10%
  ...
  function placeBid(uint256 _issue) external payable {
    ...
    uint256 minimum = minimumOutbid(_issue);

    require(calculatedBidValue >= issue.reservePrice, 'BELOW_RESERVE_PRICE');
    require(calculatedBidValue >= minimum, 'CANNOT_OUTBID');


    uint256 calculatedReward = 0;

    Bid memory winning = winningBid(_issue);
    if (winning.bidder != msg.sender) {
      if (issue.bidHistory.length == 0)
        calculatedReward = ((calculatedBidValue - issue.reservePrice) * FACTOR) / 10000;
      else
        calculatedReward = ((calculatedBidValue - winning.value) * FACTOR) / 10000;
    }
    ...
    // Bid and Reward will keep the same index
    issue.bidHistory.push(bid);
    issue.rewards.push(reward);
   ...
   * @dev 1000 => 10.00% reward factor
   * @param _factor The reward factor
   */
  function setRewardFactor(uint _factor) external onlyRole(OWNER_ROLE) {
    FACTOR = _factor;
  }
  ...
  function _refund(uint256 _issue, bool _fee) internal {
    ...
    for (uint256 i = 0; i < issue.bids.length; i++) {
      ...
      if (bid.bidder != wBid.bidder) {
        uint totalRefund = issue.bids[i].value + issue.bids[i].fee;
        for (uint z = 0; z < issue.rewards.length; z++) {
          Reward memory reward = issue.rewards[z];
          if (reward.bidder == bid.bidder) totalRefund += reward.value;
        }
        payable(bid.bidder).transfer(totalRefund);
      }
    }
    if (_fee) {
      // Refund entry fee
