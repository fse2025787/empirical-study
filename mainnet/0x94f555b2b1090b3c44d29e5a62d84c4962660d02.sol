...
contract WrappedToken is SecureContract, ERC20
{
    ...
    event TransferBackingTokenTo(address indexed user, uint256 amount);
    event TransferBackingTokenFrom(address indexed user, uint256 amount);

    using SafeERC20 for IERC20;

    IERC20 private backingToken_;
    address private backingTokenAddress_;
    uint8 private decimals_;

    constructor(address backingTokenContract, string memory name, string memory ticker, uint8 tokenDecimals) SecureContract() ERC20(name, ticker)
    {
        require(backingTokenContract != address(0), "WrappedToken: Backing token not configured");
    ...
    function queryToken() public view returns (IERC20)
    {
    ...
    {
        backingToken_.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function unwrap(uint256 amount) public pause
    {
        ...
        backingToken_.safeTransfer(msg.sender, amount);
        emit Unwrap(msg.sender, amount);
    }

    function transferBackingTokenFrom(address user, uint256 amount) public
    {
        require(hasRole(_BRIDGE_TX, msg.sender), "WrappedToken: transferBackingTokenFrom - Permission denied");
        backingToken_.safeTransferFrom(user, address(this), amount);
        emit TransferBackingTokenFrom(user, amount);
    }

    function transferBackingTokenTo(address user, uint256 amount) public
    {
        require(hasRole(_BRIDGE_TX, msg.sender), "WrappedToken: transferBackingTokenTo - Permission denied");
        backingToken_.safeTransfer(user, amount);
        emit TransferBackingTokenTo(user, amount);
    }

    function mintTo(address user, uint amount) public pause
    {
        require(hasRole(_MINTTO, msg.sender), "WrappedToken: mintTo - Permission denied");
        _mint(user, amount);
