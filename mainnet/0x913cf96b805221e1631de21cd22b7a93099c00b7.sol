...
// Dependency file: contracts/interfaces/ITaalRouter01.sol

// pragma solidity >=0.6.2;

interface ITaalRouter01 {
...
// Dependency file: contracts/interfaces/ITaalRouter02.sol

// pragma solidity >=0.6.2;

// import 'contracts/interfaces/ITaalRouter01.sol';

interface ITaalRouter02 is ITaalRouter01 {
...
// Dependency file: taalswap-core/contracts/interfaces/ITaalFactory.sol

// pragma solidity >=0.5.0;

interface ITaalFactory {
...
// Root file: contracts/TaalRouter.sol

pragma solidity =0.6.6;

// import '/Users/peter/Documents/develop/taalswap-periphery/node_modules/@uniswap/lib/contracts/libraries/TransferHelper.sol';
// import 'contracts/interfaces/ITaalRouter02.sol';
// import '/Users/peter/Documents/develop/taalswap-periphery/node_modules/taalswap-core/contracts/interfaces/ITaalFactory.sol';
// import 'contracts/libraries/SafeMath.sol';
// import '/Users/peter/Documents/develop/taalswap-periphery/node_modules/taalswap-core/contracts/interfaces/ITaalPair.sol';
...
contract TaalRouter is ITaalRouter02 {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'TaalRouter: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (ITaalFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            ITaalFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = TaalLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = TaalLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'TaalRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = TaalLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'TaalRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        address pair = TaalLibrary.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = ITaalPair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
        ...
        address pair = TaalLibrary.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = TaalLibrary.pairFor(factory, tokenA, tokenB);
        ITaalPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = ITaalPair(pair).burn(to);
        (address token0,) = TaalLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'TaalRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'TaalRouter: INSUFFICIENT_B_AMOUNT');
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
    ...
    function removeLiquidityWithPermit(
        ...
        ITaalPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'TaalRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, TaalLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'TaalRouter: INVALID_PATH');
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'TaalRouter: INVALID_PATH');
        amounts = TaalLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'TaalRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(TaalLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'TaalRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'TaalRouter: INSUFFICIENT_OUTPUT_AMOUNT');
