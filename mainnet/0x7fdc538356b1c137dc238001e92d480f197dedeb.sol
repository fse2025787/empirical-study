  ...
  IERC721Enumerable
{
  ...
  constructor(
    ...
    require(
      collectionSize_ > 0,
      "ERC721S: collection must have a nonzero supply"
    );
    require(maxBatchSize_ > 0, "ERC721S: max batch size must be nonzero");
    _name = name_;
    _symbol = symbol_;
  ...
  function tokenByIndex(uint256 index) public view override returns (uint256) {
    require(index < totalSupply(), "ERC721S: global index out of bounds");
    return index;
  }
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index)
    ...
    returns (uint256)
  {
    require(index < balanceOf(owner), "ERC721S: owner index out of bounds");
  ...
  function _numberMinted(address owner) internal view returns (uint256) {
    require(
      owner != address(0),
      "ERC721S: number minted query for the zero address"
    );
  ...
  function setApprovalForAll(address operator, bool approved) public override {
    require(operator != _msgSender(), "ERC721S: approve to caller");
  ...
  function _safeMint(
    ...
    require(!_exists(startTokenId), "ERC721S: token already minted");
    require(quantity <= maxBatchSize, "ERC721S: quantity to mint too high");
  ...
  function _setOwnersExplicit(uint256 quantity) internal {
    ...
    for (uint256 i = oldNextOwnerToSet; i <= endIndex; i++) {
      if (_ownerships[i] == address(0)) {
        address ownership = ownershipOf(i);
...
// File: sins_of_shadow_W_final.sol
...
contract SINS is Ownable, ERC721S, ReentrancyGuard {
  using Strings for uint256;

  uint256 thePrice = 110000000000000000;
                      
  mapping (address => uint256) public _whitelist;

  bool public _whitelist_on = true;

  uint256 public current_mint_max = 377;

  uint256 public num_revealed = 0;

  string public _starterbaseTokenURI = "ipfs://Qmd2WCWVFrbsVVybb3MVnKtrQLzQdEgxC3mccS7xnzy9M9/";
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721S("SINS", "SINS", maxBatchSize_, collectionSize_) {
  }


    modifier checkWhitelist(uint256 quantity) {
        if(_whitelist_on){
            require(_whitelist[_msgSender()] >= quantity,"Account needs to whitelist"); 
            _whitelist[_msgSender()] -= quantity;
            }
        _;
    }
  ...
  function mint_multiple_with_nativecoin(uint256 quantity) external payable checkWhitelist(quantity){

    require(totalSupply() + quantity <= current_mint_max, "reached max current mint");

    require(msg.value >= thePrice, "Need to send more Value.");
  ...
  // // metadata URI
  string private _baseTokenURI = "ipfs://Qmd2WCWVFrbsVVybb3MVnKtrQLzQdEgxC3mccS7xnzy9M9/";
  ...
  function setStarterURI(string calldata baseURI) external onlyOwner {
    _starterbaseTokenURI = baseURI;
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (tokenId >= num_revealed){
            string memory temp_uri = _starterbaseTokenURI;
            return bytes(temp_uri).length > 0 ? string(abi.encodePacked(temp_uri, tokenId.toString(),_ending)) : "";
        }
        
        string memory baseURI = _baseTokenURI;
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(), _ending)) : "";
  }
