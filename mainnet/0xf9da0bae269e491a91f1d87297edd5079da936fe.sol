...
contract LootEggs is ERC721Enumerable, ReentrancyGuard, Ownable {
    using Strings for string;
    string public baseURI = "https://api.cyber-hunter.com/factory/";
    uint256 public tokenIdNow = 0;
    IERC721Enumerable public NftAddress = IERC721Enumerable(0xb840EC0DB3b9ab7b920710D6fc21A9D206f994Aa);
    bool public canClaimAirdrop = false;
    mapping(address => bool)  public MinerList;
    mapping(address => bool)  public AirdropList;
    mapping(address => uint256) public AirdropNumList;
    constructor() ERC721("Loot Eggs", "LootEggs") Ownable() {}

    function setBaseURI(string memory _baseURI) public onlyOwner {
        baseURI = _baseURI;
    }

    function setNftAddress(IERC721Enumerable _NftAddress) public onlyOwner {
        NftAddress = _NftAddress;
    }

    function setMinerList(address[] memory _adddressList) public onlyOwner {
        for (uint256 i = 0; i < _adddressList.length; i++) {
            MinerList[_adddressList[i]] = true;
        }
    }

    function removeMinerList(address[] memory _adddressList) public onlyOwner {
        for (uint256 i = 0; i < _adddressList.length; i++) {
            MinerList[_adddressList[i]] = false;
        }
    }

    function setAirdropList(address[] memory _AirdropList, uint256[] memory _NumList) public onlyOwner {
        require(_AirdropList.length == _NumList.length, "k035");
        for (uint256 i = 0; i < _AirdropList.length; i++) {
            AirdropList[_AirdropList[i]] = true;
            AirdropNumList[_AirdropList[i]] = _NumList[i];
        }
    }
    ...
    function disableAirdrop() public onlyOwner {
        canClaimAirdrop = false;
    }

    function claim(address _to, uint256 _num) public nonReentrant {
        require(MinerList[msg.sender] == true || msg.sender == owner(), "k029");
        require(_num > 0, "k030");
    ...
    function getClaimAirdropNum(address _user) public view returns (uint256){
        return AirdropNumList[_user];
    }

    function claimAirdrop() public nonReentrant {
        require(AirdropList[msg.sender] == true, "k031");
        require(canClaimAirdrop == true, "k032");
        uint256 num2 = AirdropNumList[msg.sender];
        require(num2 > 0, "k034");
        for (uint256 i = 0; i < num2; i++) {
            tokenIdNow = tokenIdNow + 1;
            _safeMint(msg.sender, tokenIdNow);
        }
