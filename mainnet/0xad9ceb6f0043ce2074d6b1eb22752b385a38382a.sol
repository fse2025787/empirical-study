...
contract KRLTokenVesting is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    IERC20 public KRL;

    constructor(IERC20 krl) {
        KRL = krl;
    }
 ...
 function lock (LockParams[] calldata _lock_params) external nonReentrant onlyOwner {
    require(_lock_params.length > 0, 'NO PARAMS');
    address _token = address(KRL);
    for (uint256 i = 0; i < _lock_params.length; i++) {
        totalAmount_ += _lock_params[i].tokensDeposited;
        uint256 endtime = _lock_params[i].endEmission;
  ...
  function getLock (uint256 _lockID) external view returns (TokenLock memory) {
   ...
   function withdraw (uint256 _lockID, uint256 _amount) external nonReentrant {
    ...
    TransferHelper.safeTransfer(address(KRL), msg.sender, _amount);
