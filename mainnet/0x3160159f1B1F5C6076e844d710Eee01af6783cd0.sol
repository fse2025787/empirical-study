...
// File: contracts/IHolomateDAO.sol
 ...
 * @title IHolomateDAO 
 */
contract IHolomateDAO {
    
    // HoloMate NFT related 

    function mintNFTWithID(uint256 _tokenId, address toAddress, uint256 paidEthAmount) public;
    function mintNFT(uint32 _numTokens, address toAddress, uint256 paidEthAmount) public;
    function depositNFT(uint256 _tokenId, address fromAddress) public;
    function pickUpNFT(uint256 _tokenId, address toAddress, uint256 paidEthAmount) public;


    // HoloMate Maco related 
    function swapMacoToEth(address account, uint256 ethAmount, uint256 macoAmount) public;
    function swapEthToCash(address account, uint256 ethAmount, uint256 macoAmount) public;
    function swapEthToMaco(address account, uint256 ethAmount, uint256 macoAmount) public;
    function cashOutMaco(address account, uint256 ethAmount, uint256 macoAmount) public;
    function cashOutEth(address account, uint256 ethAmount, uint256 macoAmount) public;
    function depositMaco(address account, uint256 macoAmount) public;

}

// File: contracts/Maco.sol
...
/**
@title Maco: The Mate Coin
@dev ERC20 Token to be used as in-world money for the Holomate.io.
 ...
 * Approves payout to developers for each 10% of all minted mates, monthly infrastructre costs.
 * Bootstrap approved to transfer on creation.
 */
contract Maco is ERC20, ERC20Detailed, ERC20Mintable, ERC20Capped, Ownable, IUniswapV3SwapCallback {
    ...
    IUniswapV3PoolActions macoEthUniswapPool;
    address payable public macoEthUniswapPoolAddress;
    bool macoEthUniswapPoolToken0IsMaco;
    ...
    // DAO contract to execute L2 functions
    IHolomateDAO dao;
    ...
    **/
    constructor(
        ...
        address _daoAddress
    ) public ERC20Detailed("Holomate MateCoin", "MACO", 18) 
        ERC20Capped(_maxSupply)
    { 
        require(_WETHAddress != address(0), "WETH is the zero address");
        require(_bootstrapPayoutAdress != address(0), "bootstrap_payout is the zero address");

        WETH = WETH9_(_WETHAddress);
        dao = IHolomateDAO(_daoAddress);
    ...
    /**
     * @dev ETH to Maco Coin
     */
    function toMacoCoin(uint160 sqrtPriceLimitX96) public payable {
        
        wrap(msg.value);
        WETH.approve(macoEthUniswapPoolAddress, msg.value);

        // → coinswap ETH/MACO
        macoEthUniswapPool.swap(address(this), !macoEthUniswapPoolToken0IsMaco, int256(msg.value), sqrtPriceLimitX96, swapDataToBytes(SwapData(4, msg.sender)));
    }

    /**
     * @dev Maco Coin to ETH
     * @param _amount amount of maco to swap to eth
     */
    function toETH(uint256 _amount, uint160 sqrtPriceLimitX96) public {
        
        require(_amount <= balanceOf(msg.sender), 'low_balance');
        _approve(msg.sender, macoEthUniswapPoolAddress, _amount);

        // → coinswap MACO/ETH
        macoEthUniswapPool.swap(address(this), macoEthUniswapPoolToken0IsMaco, int256(_amount), sqrtPriceLimitX96, swapDataToBytes(SwapData(3, msg.sender)));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        require(msg.sender == macoEthUniswapPoolAddress, 'uni_sender');
        ...
        int256 macoAmount = macoEthUniswapPoolToken0IsMaco ? amount0Delta : amount1Delta;
        int256 ethAmount = macoEthUniswapPoolToken0IsMaco ? amount1Delta : amount0Delta;

        if(macoAmount > 0) {
            // MACO is needed by the pool, means maco to ETH
            require(uint256(macoAmount) <= balanceOf(swapData.account), 'owner_MACO_bal');
            transferFrom(swapData.account, msg.sender, uint256(macoAmount));
            ...
            //emit SwapEvent(3, swapData.account, uint256(ethAmount), uint256(macoAmount));
            dao.swapMacoToEth(swapData.account, uint256(ethAmount), uint256(macoAmount));

        } else if(ethAmount > 0) {
            // ETH is needed, means eth to maco cash (eventType 2) or coin (eventType 4)
            ...
            WETH.transfer(macoEthUniswapPoolAddress, uint256(ethAmount));
            //macoEthUniswapPoolAddress.transfer(uint256(amount0Delta));// ), 'eth_to_uni');

            // pay the owner the MACO he got
            if(swapData.eventType == 2) {
                // inform the cash system that it should mint coins to the owner
                //emit SwapEvent(2, swapData.account, uint256(ethAmount), uint256(-macoAmount));
                dao.swapEthToCash(swapData.account, uint256(ethAmount), uint256(-macoAmount));
                

            } else {
                //emit SwapEvent(4, swapData.account, uint256(ethAmount), uint256(-macoAmount));
                dao.swapEthToMaco(swapData.account, uint256(ethAmount), uint256(-macoAmount));
                _approve(address(this), macoEthUniswapPoolAddress, uint256(-macoAmount));
                transferFrom(address(this), swapData.account,  uint256(-macoAmount));
            }
        } 
    }
     ...
     */
    function settleCashOut(address payable _to, uint256 _amount, bool _toEth, uint160 sqrtPriceLimitX96, uint256 _notMinted) public onlyMinter {
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        if(recipient == address(this)) {
            ...
            dao.depositMaco(msg.sender, amount);
        }
        return true;
    }
    ...
    function setUniswapPool(address payable _poolAddress) public onlyOwner {
        ...
        macoEthUniswapPoolToken0IsMaco = (poolImmu.token0() == address(this));
        macoEthUniswapPoolAddress = _poolAddress;
        macoEthUniswapPool = IUniswapV3PoolActions(macoEthUniswapPoolAddress);
    }

    function setDaoContract(address _daoAddress) public onlyOwner {
        dao = IHolomateDAO(_daoAddress);
    }    
...
// File: contracts/L2AccountRole.sol

pragma solidity ^0.5.1;


contract L2AccountRole {
  using Roles for Roles.Role;

  event L2AccountAdded(address indexed account);
  event L2AccountRemoved(address indexed account);

  Roles.Role private l2accounts;

  constructor() public {
    _addL2Account(msg.sender);
  }

  modifier onlyL2Account() {
    require(isL2Account(msg.sender));
    _;
  }

  function isL2Account(address account) public view returns (bool) {
    return l2accounts.has(account);
  }

  function addL2Account(address account) public onlyL2Account {
    _addL2Account(account);
  }

  function renounceL2Account() public {
    _removeL2Account(msg.sender);
  }

  function _addL2Account(address account) internal {
    l2accounts.add(account);
    emit L2AccountAdded(account);
  }

  function _removeL2Account(address account) internal {
    l2accounts.remove(account);
    emit L2AccountRemoved(account);
  }
}

// File: contracts/Holomate.sol
...
/**
 * @title Holomate Mate ownership NFT
 * Holomate - a contract for mate ownership inside the Holomate.io world as non-fungible ERC721 token.
 ...
 * - Each call from off-chain with additional data to reflect off-chain cash of the Maco currency on the blockchain.
 * - Supports deposit and pickup of tokens on the contract to be handled gasless off-chain.
 */
contract Holomate is ERC721Full, Ownable, L2AccountRole {
    using Strings for string;
    
    // a Maco contract and must be owned by this contract 
    ...
    // how many nft got minted
    uint32 public mintedSupply = 0;

    // public variables 
    uint256 public gasToPickup = 93233;
    uint256 public gasToMint = 182000;
    ...
    ) public ERC721Full("Holomate", "MATE") {
        dao = IHolomateDAO(_daoAddress);
    ...
    /**
     * @dev Emits event to mint a token to the senders adress
     * @param _numTokens address of the future owner of the token
     */
    function mintNFT(uint32 _numTokens) public payable {
        require(tx.gasprice * gasToMint <= msg.value, "min_price");
        ...
        //→ emit event for off-chain listener
        //emit NFTEvent(1, _tokenId, msg.sender, msg.value);
        dao.mintNFT(_numTokens, msg.sender, msg.value);
    }
    /**
     * @dev Mints a token to an address and mints unminted maco cash.
     * @dev Called by server
     * @param _tokenId Id of the token
     * @param _to address of the future owner of the token
     */
    function mintTo(uint256 _tokenId, address _to, uint256 _notMinted) public onlyL2Account {
        require(!_exists(_tokenId), "token_exists");
        _mint(_to, _tokenId);
    ...
    /**
     * @dev Releases an off-chain stored token into the blockchain and mints unminted maco cash.
     * @dev Called by server
     * @param _tokenId address of the future owner of the token
     */
    function release(uint256 _tokenId, address mateOwner, uint256 _notMinted) public onlyL2Account {
        transferFrom(address(this), mateOwner, _tokenId);
        if(_notMinted > 0) {
            maco.mintFromCash(_notMinted);
        }
    }
    ...
    function setMacoContract(address payable _fundTokenAddress) public onlyOwner {
        maco = Maco(_fundTokenAddress);
    }
