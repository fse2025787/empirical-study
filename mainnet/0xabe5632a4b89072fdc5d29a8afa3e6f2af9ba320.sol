...
// File: broke_bears.sol
// Coded by broke people for the broke people <3
...
contract BrokeBearsOfficial is ERC721A, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxMintableForFree = 2222;  
  uint256 public preMinted = 222;

  bool public paused = true;
  mapping(address => uint256) public addressMintBalance;

  constructor(string memory _initBaseURI) ERC721A("BrokeBearsOfficial", "BRKBRSOF") {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (_mintAmount <= 0 || _mintAmount > maxMintAmountPerTx) revert InvalidQuantity();
    if (addressMintBalance[msg.sender] + _mintAmount > maxMintPerWallet) revert InvalidQuantity();
    if (supply + _mintAmount > maxSupply) revert SoldOut();

    if (msg.sender != owner() || supply > maxMintableForFree) {
      if (msg.value < cost * _mintAmount) revert InvalidPrice();
    }
    
    _safeMint(msg.sender, _mintAmount);
    addressMintBalance[msg.sender] += _mintAmount;

    if (supply + _mintAmount >= maxMintableForFree && cost == 0){
        cost = 0.0035 ether;
    }
  ...
  function preMint(uint256 _mintAmount) public onlyOwner {
    uint256 supply = totalSupply();
    if (_mintAmount <= 0) revert InvalidQuantity();
    if (supply + _mintAmount > preMinted ) revert SoldOut();
  ...
  function setmaxMintAmountPerWallet(uint256 _newMaxMintAmountPerWallet) public onlyOwner {
    maxMintPerWallet = _newMaxMintAmountPerWallet;
  }

  function setmaxMintForFree(uint256 _newMaxMintableForFree) public onlyOwner {
    maxMintableForFree = _newMaxMintableForFree;
  }
