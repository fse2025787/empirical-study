...
contract Quik_Marketplace is Ownable {
    ...
    // address nftContract; //contract address for NFT
    ...
    function bidForAuction(address _nft, uint256 _tokenId, uint256 _tokenAmount) public payable{
        ...
        if(act.quikTrade){
          ...
          //return the previous eth to the bidder
          if(currentBidder != address(0))
        ...
        emit BidCreated(_nft, _tokenId, msg.sender, _tokenAmount);
    }
    ...
    function ClaimNFTDomain(address _nft, uint256 _tokenId) public payable{
        ...
        (address minter, uint256 royaltyFee) = ERC721Royalty(_nft).royaltyInfo(_tokenId, act.currentBidPrice);
        uint256 sellerFee = calculateSellerFee(act.currentBidPrice, act.quikTrade);
        ...
        if(act.quikTrade){
          ...
          IERC20(tradeToken).transferFrom(msg.sender, tokenOwner, act.currentBidPrice.sub(sellerFee).sub(royaltyFee));
          if(royaltyFee!=0){
          ...
          //transfer remainings to token owner
          withdrawETH(tokenOwner, act.currentBidPrice.sub(sellerFee).sub(royaltyFee));
          if(royaltyFee!=0){
              //transfer minter commission
