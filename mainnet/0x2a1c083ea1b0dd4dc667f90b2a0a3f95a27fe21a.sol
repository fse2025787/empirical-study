...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Rigel Lottery Ticket", "RLT") {}
    ...
    function multiClaimReward(uint256[] memory _multiTokenId) external onlyOwner {
        for (uint i = 0; i < _multiTokenId.length; i++) {
            claimInfo[_multiTokenId[i]] = true;
        }
    }
...
contract Lottery is LotteryOwnable {
    ...
    // Allocation for first/sencond/third & fourth reward 
    uint8[4] public allocation;
    // The TOKEN to buy lottery
    ERC20Burnable public rigel;
    ...
    // issueId => [totalAmount, firstMatchAmount, secondMatchingAmount, thirdMatchingAmount, fourthMatchAmount]
    ...
    function reset() external onlyAdmin {
        ...
        if(getMatchingRewardAmount(issueIndex-1, 4) == 0) {
            carryOverAmount = carryOverAmount.add(getTotalRewards(issueIndex-1).mul(allocation[0]).div(100));
        }
        
        if(getMatchingRewardAmount(issueIndex-1, 3) == 0) {
            carryOverAmount = carryOverAmount.add(getTotalRewards(issueIndex-1).mul(allocation[1]).div(100));
        }
        
        internalBuy(carryOverAmount, nullTicket);
        
        uint256 burnableAmount = getTotalRewards(issueIndex-1).mul(5).div(100);
        
        if(getMatchingRewardAmount(issueIndex-1, 2) == 0) {
            burnableAmount = burnableAmount.add(getTotalRewards(issueIndex-1).mul(allocation[2]).div(100));
        }
        if(getMatchingRewardAmount(issueIndex-1, 1) == 0) {
           burnableAmount = burnableAmount.add(getTotalRewards(issueIndex-1).mul(allocation[3]).div(100));
        }
        
        rigel.burn(burnableAmount);
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase {
        ...
        rigel.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        rigel.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            rigel.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            rigel.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        rigel.safeTransfer(address(msg.sender), _amount);
    ...
    // Set the maximum Number that a each digit can be in the lottery number combination
