...
interface IERC721Token {
    function transferFrom(address _from, address _to, uint256 tokenId) external returns (bool);
}
...
contract ProxySender {
    ...
    function transferNFT(IERC721Token _token, address _sender, address _receiver, uint256 tokenId) external returns (bool) {
        require(msg.sender == owner, "access denied");
        return _token.transferFrom(_sender, _receiver, tokenId);
    }
}