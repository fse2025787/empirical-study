...
Every one of use has, at some time or another, suffered from adverse Mental Health.  
We understand the stigmatism that is involved with this, especially for young men.  
It is sometimes, wrongly, seen as a sign of weakness when in fact nothing is further from the truth.
We will use this platform to not only raise money for established Mental Health Charities, 
but to also raise awareness and provide a safe space for people to come and share their own experiences.
Take the first step and join us today.

Website -  https://www.mensmentalhealth.io  
Telegram - https://t.me/mensmentalhealthtoken
Twitter -  https://twitter.com/MMHCrypto
...
contract MMH is ERC20, Ownable{
    ...
    address public MMHCharityWallet;
    ...
    event TaxRecipientsUpdated(address newLpRecipient, address newMarketingWallet, address newMMHCharityWallet);
    event FeesUpdated();
    ...
    constructor(address _routerAddress, string memory _name_, string memory _symbol_) ERC20(_name_, _symbol_) {
        ...
        exemptFee[MMHCharityWallet] = true;
        exemptFee[lpRecipient] = true;

        _mint(msg.sender, 1_000_000_000 * 10**18);
    }

    function setTaxRecipients(address _lpRecipient, address _marketingWallet, address _MMHCharityWallet) external onlyOwner{
        ...
        require(_MMHCharityWallet != address(0), "MMHCharityWallet cannot be the zero address");
        lpRecipient = _lpRecipient;
        marketingWallet = _marketingWallet;
        MMHCharityWallet = _MMHCharityWallet;

        exemptFee[MMHCharityWallet] = true;
        exemptFee[marketingWallet] = true;
        exemptFee[lpRecipient] = true;

        emit TaxRecipientsUpdated(_lpRecipient, _marketingWallet, _MMHCharityWallet);
    }
    ...
    function setBuyFees(uint256 _lp, uint256 _marketing, uint256 _MMHCharity) external onlyOwner{
        require(_lp + _marketing + _MMHCharity < 100, "Buy fee must be less than 100");
        buyFees = Fees(_lp, _marketing, _MMHCharity);
        totalBuyFee = _lp + _marketing + _MMHCharity;
        emit FeesUpdated();
    }

    function setSellFees(uint256 _lp, uint256 _marketing, uint256 _MMHCharity) external onlyOwner{
        require(_lp + _marketing + _MMHCharity < 100, "Sell fee must be less than 100");
        sellFees = Fees(_lp, _marketing, _MMHCharity);
        totalSellFee = _lp + _marketing + _MMHCharity;
        emit FeesUpdated();
    }
    ...
    function handle_fees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 MMHCharityAmt = unitBalance * 2 * sellFees.MMHCharity;
            if(MMHCharityAmt > 0){
                payable(MMHCharityWallet).sendValue(MMHCharityAmt);
            }
        }
    }
