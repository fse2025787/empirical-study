...
contract EthSquirrelGame is ERC721URIStorage, Ownable{

    event MintEthSquirrel (address indexed minter, uint256 startWith, uint256 times);
   
    
    uint256 public totalEthSquirrel;
    uint256 public totalCount = 7500; //bruhTotal
    ...
    function MintEthSquirrels(uint256 _times) payable public mintEnabled {
       
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalEthSquirrel + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintEthSquirrel(_msgSender(), totalEthSquirrel+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalEthSquirrel++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalEthSquirrel + _times <= presaleMax, "Mint amount will exceed total presale amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalEthSquirrel + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintEthSquirrel(_addr, totalEthSquirrel+1, 1);
        _mint(_addr, 1 + totalEthSquirrel++);
    } 
