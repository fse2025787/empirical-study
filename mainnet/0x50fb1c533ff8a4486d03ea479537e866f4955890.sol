...
// File contracts/tree-bridge/MasterChef.sol
...
contract MasterChef is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(
                rewardReward.mul(uint256(100).sub(OP_FUND_RATE)).div(100).mul(1e12).div(lpSupply)
            );
        }
    ...
    // op claim reward
    function opClaim(address _to, uint256 _amount) public {
        require(msg.sender == opAddr, "op: wut?");
        opBal = opBal.sub(_amount);
