...
contract DeadDoods is ERC721, Ownable {
  using Strings for uint256;
  ...
  constructor() ERC721("DeadDoods", "DD") {
    setUriPrefix("");
    setHiddenMetadataUri("https://ipfs.io/ipfs/QmSgk3ANYbrwKfjQezd9fRGY2cVRz6kYmwuiQcmCG3dQei/hidden.json");
  }
  ...
  function mint(uint256 _mintAmount, uint256[] memory ids) public payable mintCompliance(_mintAmount) {
    ...
    require(addressMintedBalance[msg.sender] + _mintAmount <= maxMintAmountPerTx, "Mint exceed the wallet limit");
  ...
  function mintForSelf(uint256 _mintAmount, uint256[] memory ids) public mintCompliance(_mintAmount) onlyOwner {
    _mintLoop(msg.sender, _mintAmount, ids);
  }
