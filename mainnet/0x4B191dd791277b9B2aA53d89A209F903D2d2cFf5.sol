...
contract Hunting is IERC721Receiver, OwnableUpgradeable {
     ...
     */
    function grantWishes(address[] calldata _blessed) external onlyKami {
        for (uint256 i = 0; i < _blessed.length; i++) {
            _grantWish(_blessed[i]);
        }
    }
    ...
    function _grantWish(address _blessed) internal {
        wish[_blessed] = true;
    }
