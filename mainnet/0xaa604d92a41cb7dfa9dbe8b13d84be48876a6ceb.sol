...
contract ProofOfGas is IERC721A { 
    ...
    string private constant _name = "ProofOfGas";
    string private constant _symbol = "PoG";
    string private constant _contractURI = "QmbVTsDCS78ATXJa5QMJsTQm7hjnrAgrrKP758WikDv4MC";
	...
	function nextPrime() public returns (uint256){
		uint runs = 10000;
        for (uint num = (lastPrime/3 + lastPrime/2); num < runs; num++) {
    ...
    function render(uint256 _tokenId) internal view returns (string memory) {
        ...
        return string.concat(
            '<svg height="200" width="200" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="-100 -100 200 200">',
            ...
            '<circle cx="0" cy="20" r="70" fill="url(#glow)" />',
            '</svg>'
        );
    }
        ...
        returns (string memory)
    {
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "PoG #', Strings.toString(_tokenId),
                '", "description": "Proof of Gas on Chain", "image": "data:image/svg+xml;base64,',
