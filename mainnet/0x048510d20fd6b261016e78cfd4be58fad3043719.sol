...
contract imaGANation is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public constant COST = 0.03 ether;

    uint256 public constant MAX_MINT = 5;
    uint256 public constant MAX_PRIVATE_SUPPLY = 20;
    uint256 public constant MAX_PUBLIC_SUPPLY = 480;
    ...
    constructor() ERC721("imaGANation", "IGN") {
    ...
    function whitelistMint() external payable {
        ...
        require(
            _whitelistClaimed[msg.sender] + 1 < 2,
            "Whitelist tokens already claimed"
        );
        require(totalSupply() < MAX_SUPPLY, "All tokens minted");
        require(totalPublicSupply < MAX_PUBLIC_SUPPLY, "Over max public limit");
        require(whitelistCost * 1 <= msg.value, "ETH amount is not correct");
        totalPublicSupply += 1;
