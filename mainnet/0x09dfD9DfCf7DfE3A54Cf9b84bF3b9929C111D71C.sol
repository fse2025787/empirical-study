...
contract UniswapV2Converter is Ownable {
    ...
    function _getSwapPath(address source, address destination) internal view returns(address[] memory path) {
    ...
    function swap(
        ...
        address[] memory path = _getSwapPath(source, destination);
    ...
    function previewSwap(address source, address destination, uint256 value) external view returns(uint256) {
        address[] memory path = _getSwapPath(source, destination);
        return router.getAmountsOut(value, path)[path.length - 1];
    }
}
