...
contract ProbablyNothing is ERC721A, Ownable {

    // Minting Variables
    uint256 public mintPrice = 1.0 ether;
    uint256 public maxPurchase = 1;
    uint256 public maxSupply = 10000;

    uint256 public shrugTokenId = 2**256-1;
    ...
    constructor() ERC721A("Probably Nothing", "PN") {
        _safeMint(msg.sender, 1);
    }

    function mint() external payable {
        require(saleIsActive, "SALE_INACTIVE");
        require(balanceOf(msg.sender) + 1 <= maxPurchase , "this would exceed mint max allowance");
    ...
    function ownerOf(uint256 tokenId) public view override returns (address) {
        if(tokenId >= shrugTokenId) { return address(this); }
    ...
    function shrug(uint256 count) external {
        for(uint256 i = 0;i < count;i++) {
            emit Transfer(address(0), msg.sender, shrugTokenId - i);
            emit Transfer(msg.sender, address(this), shrugTokenId - i);
        }
