...
contract Degen is ERC20, Ownable {
    ...
    constructor() ERC20("Degen", "DEGEN") {
        ...
        _fees.buyTotalFees = _fees.buyBankFee + _fees.buyLiquidityFee;

        _fees.sellBankFee = 3;
        _fees.sellLiquidityFee = 1;
        _fees.sellTotalFees = _fees.sellBankFee + _fees.sellLiquidityFee;


        bankWallet = address(0x541f4258018b12a0C856D695Bc7bBA48FfA5acc4); // 
    ...
    function updateBuyFees(uint256 _bankFee, uint256 _liquidityFee) external onlyOwner {
        _fees.buyBankFee = _bankFee;
        _fees.buyLiquidityFee = _liquidityFee;
        _fees.buyTotalFees = _fees.buyBankFee + _fees.buyLiquidityFee;
        require(_fees.buyTotalFees <= 7, "Immutable");
    }

    function updateSellFees(uint256 _bankFee, uint256 _liquidityFee) external onlyOwner {
        _fees.sellBankFee = _bankFee;
        _fees.sellLiquidityFee = _liquidityFee;
        _fees.sellTotalFees = _fees.sellBankFee + _fees.sellLiquidityFee;
        require(_fees.sellTotalFees <= 7, "Immutable");
    }
    ...
    function updateWallets(address newBankWallet) external onlyOwner{
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && _fees.sellTotalFees > 0) {
                ...
                tokensForBank += fees * _fees.sellBankFee / _fees.sellTotalFees;
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && _fees.buyTotalFees > 0) {
                fees = amount.mul(_fees.buyTotalFees).div(100);
                tokensForLiquidity += fees * _fees.buyLiquidityFee / _fees.buyTotalFees;
                tokensForBank += fees * _fees.buyBankFee / _fees.buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForBank ;
        bool success;
        ...
        uint256 ethForBank = ethBalance.mul(tokensForBank).div(totalTokensToSwap);
        uint256 ethForLiquidity = ethBalance - ethForBank;
        ...
        (success,) = address(bankWallet).call{ value: address(this).balance } ("");
    }
