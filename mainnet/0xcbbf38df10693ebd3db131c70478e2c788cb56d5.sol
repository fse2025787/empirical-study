...
contract ShibJamaica is Context, IERC20, Ownable {
    ...
    string private _name = "ShibJamaica";
    string private _symbol = "SJ";
    ...
    uint256 public _marketingAndDevBudget = 8;
    ...
    event SwapAndDonation(
        uint256 tokensSwapped,
        uint256 ethReceived,
        uint256 tokensIntoDonation
    );
    ...
    constructor () public  {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); //(0xD99D1c33F9fC3444f8101754aBC46c52416550D1) 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function setDonationAddress(address payable donation) public onlyOwner {
    ...
    function showDonationaddress() public view returns(address payable) {
    ...
    function SETnumTokensSell(uint256 numTokensSell) external onlyOwner {
        numTokensSellToAddToLiquidity = numTokensSell;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusDonationFee(tAmount);
    ...
    function calculateLiquidityPlusDonationFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _donationFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0 && _donationFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee + _donationFee;
        ...
        // calculate the portions of the liquidity to add to donation
        uint256 donationBalance = newBalance.div(totalLiqFee).mul(_donationFee);
        uint256 donationPortion = otherHalf.div(totalLiqFee).mul(_donationFee);
        ...
        uint256 finalBalance = newBalance.sub(donationBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(donationPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _donationAddress.call{value: donationBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, donationPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, donationPortion);
        } else {
            addLiquidity(donationPortion, donationBalance, _donationAddress);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address donation) private {
