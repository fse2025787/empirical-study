...
contract ERC721A is IERC721A {
    ...
    // Mapping from token ID to address who staked token.
    ...
    /**
     * @dev Approves user to transfer token from contract for unstaking.
     */
    function _approveUnstake(uint256 tokenId) internal {
        address user = _msgSenderERC721A();
        address currentOwner = ownerOf(tokenId);
        require(currentOwner == address(this), "NOT_STAKED");
        require(user == _tokenOwners[tokenId], "NOT_AUTHORISED");
        _tokenApprovals[tokenId].value = user;
        emit Approval(currentOwner, user, tokenId);
    }
  ...
  Ownable
{
  ...
  // MINT CONSTANTS
  uint256 public constant PUBLIC_ALLOWANCE = 10;
  uint256 public constant TEAM_MINT = 49;
  uint256 public constant MINTS_PER_MIGRATION = 2;
  uint256 public constant MIGRATION_MINTS = 594 * MINTS_PER_MIGRATION;
  uint256 public constant MAX_MINT = 5555 - TEAM_MINT - MIGRATION_MINTS;
  uint256 public PRIVATE_PRICE = 0.05 ether;
  uint256 public PUBLIC_PRICE = 0.05 ether;
  address public constant FEE_ADDRESS =
    0x670326f4470d4D2F5347377Ff187717a81aB1318;

  uint256 public _fees = 30.0 ether;
  uint256 public _teamMinted = 0;
  uint256 public _migrationMints = 0;
  address public _signer;
  ...
  uint8[MAX_MINT + TEAM_MINT + MIGRATION_MINTS] private _basePowerLevels; // token id => base power level
  ...
  uint8 private constant DEFAULT_POWER_LEVEL = 5;
  ...
  function setMigrationProxy(address migrationProxy)
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    grantRole(MINTER_ROLE, migrationProxy);
  }

  function setMagicFolkItemsContract(address magicFolkItems, ItemType itemType)
    public
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    if (itemType == ItemType.Mainhand) {
  ...
  function setPublicPrice(uint256 newPrice)
    public
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
  ...
  function setPrivatePrice(uint256 newPrice)
    public
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    PRIVATE_PRICE = newPrice;
  }
  ...
  function migrationMint(address to, uint256 qty)
    external
    onlyRole(MINTER_ROLE)
  {
    _safeMint(to, qty * MINTS_PER_MIGRATION, '');
    _migrationMints += qty;
  }
  ...
  function setGemRate(uint256 newGemRate) public onlyRole(DEFAULT_ADMIN_ROLE) {
    _setGemRate(newGemRate);
  }

  function _soldOut(uint256 qty) internal view returns (bool) {
    return !(_totalMinted() - _teamMinted - _migrationMints + qty <= MAX_MINT);
  }

  function _stakeToken(address user, uint256 tokenId) internal {
    require(_ownsToken(user, tokenId), 'NOT_AUTHORISED');
    require(!_isTokenStaked(tokenId), 'ALREADY_STAKED');
    _setLastClaim(tokenId);
    _tokenOwners[tokenId] = user;
    _stakedTokens[user].push(tokenId);
  ...
  function _ownsToken(address user, uint256 tokenId)
    internal
    view
    returns (bool)
  {
    TokenOwnership memory ownership = _ownershipOf(tokenId);
    return (ownership.addr == user && !ownership.burned);
  }
  ...
  // function _safeMint(address to, uint256 qty) internal override {
  // _safeMint(to, qty, "");
  // }
  ...
  function supportsInterface(bytes4 interfaceID)
    ...
    view
    virtual
    override(ERC721A, IERC165, ERC165, AccessControl)
    returns (bool)
  {
    return
      super.supportsInterface(interfaceID) ||
  ...
  function _unstakeToken(address user, uint256 tokenId) internal {
    ...
    transferFrom(address(this), user, tokenId);
  }
  ...
  function _equip(Item memory _item, uint256 _nftTokenId) internal {
    require(!(_isEquipped(_nftTokenId, _item.itemType)), 'Slot is not empty');
  ...
  function _getBasePowerLevel(uint256 tokenId)
    ...
    returns (uint8)
  {
    uint8 basePowerLevel = _basePowerLevels[tokenId];
    if (basePowerLevel == 0) {
