...
// File: contracts/farkutils/FarkUtilities.sol
...
contract FarkUtilities is ERC721A, Ownable {
    using StringsUpgradeable for uint256;
    event Minted(address indexed _from, uint256 _amount, uint256 _value);

    mapping(address => bool) public extendingContracts;
    ...
    uint16 public maxMintAmountPerWhitelist = 500;
    ...
    constructor(string memory _baseUrl) ERC721A("Fark Utilities", "FUNFT") {
        baseApiURI = _baseUrl;
    }

    //This function will be used to extend the project with more capabilities
    function setExternalContractStatus(address _bAddress, bool _val)
        public
        onlyOwner
    {
        extendingContracts[_bAddress] = _val;
    }
    ...
    function airdrop(address[] memory _airdropAddresses) public onlyOwner {
        require(
            totalSupply() + _airdropAddresses.length <= maxSupply,
            "Exceeds Max Supply"
        );
