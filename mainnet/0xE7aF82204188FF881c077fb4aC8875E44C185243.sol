...
contract StakingRewards is IStakingRewards, Ownable {
    ...
    // DAO used to verify collabland ownership
    address public discordDao; 
    ...
    function balanceOf(address _account) external view returns (uint256 balance) {
        UserStake memory user = userStakes[discordDao][_account];
        balance = user.stakedAmount;
    }

    function setDiscordDao(address _discordDao) external onlyOwner {
        discordDao = _discordDao;
    }
