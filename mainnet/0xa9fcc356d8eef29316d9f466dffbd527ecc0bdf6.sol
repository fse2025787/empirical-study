...
/** 
    ...
    Example burntSHIB distribution:

    n = number of winners = 3
    i = winner position (eg. 1 = 1st place, 2 = 2nd place)
    a = amount of burntSHIB in this lottery / amont of SHIB burned / amount of tickets = 100
...
contract ShibBurnLottery is Ownable {
    ...
    constructor (address _shibAddress, address _burntShibAddress, address _shibBurnContract, address _wethAddress, address _router, uint _minParticipation, uint _maxParticipants) {
    ...
    function withdrawParticipation() external noReentrancy {
        uint256 amount = accountLotteryParticipation[msg.sender][currentLotteryId];
        require(amount > 0, "Nothing to withdraw");

        delete accountLotteryParticipation[msg.sender][currentLotteryId];

        address[] memory participants = lotteryParticipants[currentLotteryId];
        uint256 length = participants.length;

        for ( uint256 i=0; i < length;) {
            if (participants[i] == msg.sender) {
                lotteryParticipants[currentLotteryId][i] = participants[length - 1];
                lotteryParticipants[currentLotteryId].pop();
                break;
            }
            unchecked{ i++; }
        }

        lotteryTotalTickets[currentLotteryId] -= amount;

        IERC20(shibAddress).transfer(msg.sender, amount);
    }

    /* ========== BURN AND DRAW LOTTERY ========== */

    function burnAndDrawWinner(uint _numOfWinners1, uint _numOfWinners2) external onlyOwner { 
        // _numOfWinners1 = winners that receive burntShib
        ...
        uint totalTickets = lotteryTotalTickets[lotteryId];

        // burn SHIB through the shibBurn portal
        IERC20(shibAddress).approve(shibBurnContract, totalTickets);
        IShibBurn(shibBurnContract).burnShib(totalTickets); 

        // draw lottery winners and distribute burntShib
        drawWinners(lotteryId, _numOfWinners1, _numOfWinners2);
    ...
    function drawWinners(uint _lotteryId, uint _numOfWinners1, uint _numOfWinners2) internal {

        address[] memory participants = lotteryParticipants[_lotteryId];
        uint totalTickets = lotteryTotalTickets[_lotteryId];
        uint toPay = totalTickets;
            ...
            for(uint j = 0; j < participants.length; j++) {
                ...
                if(currentNumber >= randomNumber) {
                    ...
                    if(i < _numOfWinners1) {
                        ...
                        IERC20(burntShibAddress).transfer(participants[j], pay);
                    }
