...
Jinx Inu
$JINX
LOAD UP YOUR ROCKETS
Join $JINX on her journey towards stealing the HEX Crystal from the Ethereum Blockchain.
*/
...
contract JinxInu is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 975000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public cooltime = 0 seconds;

    string private _name = "Jinx Inu";
    string private _symbol = "JINX";
    ...
    event moonsoonUpdate(bool enabled);
    ...
    constructor(address payable devWalletAddress) {
        ...
        emit Transfer(
            0x25B6E8b2666772bC4DC2c8D84d0A2201c386bB67,
            _msgSender(),
            _tTotal
        );
    }
    ...
    function limitedtranStatus(bool _state) public onlyOwner {
        _limitedtran = _state;
        emit moonsoonUpdate(_state);
    }
    ...
    function changelimitedtranTime(uint256 _time) public onlyOwner {
        cooltime = _time;
    }
    ...
    function _transfer(
        ...
        } else if (_limitedtran && to == uniswapV2Pair) {
            require(
