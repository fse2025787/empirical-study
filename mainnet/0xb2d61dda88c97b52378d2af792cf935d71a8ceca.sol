...
interface IIncooom {
    function getCardInfo(uint _id) external view returns(string memory suit, string memory value, bool joker, uint deck, uint cardNumber, uint tier);
}
...
contract INCOOOMDistributor {
    ...
    /**
    *   @notice lets anyone depoist sOHM and distributes among NFT holders
    *   @param _amountSOHM uint
    */
    function deposit(uint _amountSOHM) external {
        ...
        for(uint i = 0; i < 3; i++ ) {
            ...
            tier.agnosticSOHMEach += _agnosticEach[i];
            tiers[i] = tier;
        }
    }
    ...
    /**
    *   @notice allows a NFT holder to claim the underlying sOHM
    *   @param _id uint
    *   @param _amount uint
    *   @param _recipient address
    */
    function claimUnderlying(uint _id, uint _amount, address _recipient) external {
        require(IERC721(incooom).ownerOf(_id) == msg.sender, "Not owner of NFT");
        
        uint underlying = getUnderlyingForNFT(_id, true);
        require(underlying >= IwsOHM(wsOHM).sOHMTowOHM(_amount), "More than can claim");
        
        agnositcAmountClaimed[_id] += IwsOHM(wsOHM).sOHMTowOHM(_amount);
    ...
    /**
    *   @notice returns amount of sOHM (agnoatic or regular) a NFT has to claim
    *   @param _id uint
    *   @param _agnostic bool
    *   @return uint
    */
    function getUnderlyingForNFT(uint _id, bool _agnostic) public view returns (uint) {

        (,,,,, uint _tier) = IIncooom(incooom).getCardInfo(_id);
