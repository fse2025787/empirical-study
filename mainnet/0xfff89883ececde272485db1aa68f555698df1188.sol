...
contract WickedAliens is ERC721Enumerable, EIP712, AccessControl, Ownable {
    ...
    // Base URI of WickedAlien NFTs
    string private _wickedBaseURI;

    // Maximum number of WickedAliens that can be minted ever
    uint256 public MAX_RANGE = 8888;

    // Maximum number of WickedAliens that can be redeemed by public
    uint256 public MAX_PUBLIC_RANGE = 8800; // Public range

    // Presale start and end indicator
    bool private _presale;

    // Extended sale indicator
    bool private _extSale;

    // All presale redeemed WickedAliens will have a single CID
    string private _extSaleCid;

    // Maximum number of WickedAliens that can be sold in presale
    uint256 public presaleLimit = 888;

    // Presale + Extended Sale image
    string private _unrevealedURI;

    // Presale counter
    uint256 private _extSaleRange;

    // State variable for storing the latest minted toke id
    uint256 public currentTokenID;

    // withdrawal address were all the funds will be transferred
    address payable withdrawalAddress;

    // Total amount received until withdrawn
    ...
    constructor(address payable minter, address _withdrawalAddress)
        ERC721("Wicked Aliens Space Club", "WASC")
        EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION)
    {
        _unrevealedURI = "QmY2ARgV3pF33sLWHkswtrLSZXDZxqQLr3LkgDvMwuKaks";
        _setupRole(MINTER_ROLE, minter);
        setBaseURI("https://wasc.mypinata.cloud/ipfs/");
    ...
    function extSaleStatus() public view returns (bool) {
        return _extSale;
    }

    function extSaleCid() public view onlyOwner returns (string memory) {
        return _extSaleCid;
    }

    function extSaleRange() public view returns (uint256) {
        return _extSaleRange;
    }
    ...
    function switchExtSale() public onlyOwner {
        require(!presaleStatus(), "Presale has not ended yet");
        require(extSaleStatus(), "Extended presale has already ended");
        _extSale = false;
    }
    ...
    function setExtSaleCid(string memory newCid) public onlyOwner {
        require(
            !presaleStatus() || !extSaleStatus(),
            "Cannot set the Extended Sale CID during the presale and Extended sale"
        );
        _extSaleCid = newCid;
    }
        ...
        returns (string memory)
    {
        ...
        if (presaleStatus() || extSaleStatus()) {
            return string(abi.encodePacked(baseURI(), _unrevealedURI));
        } else if (tokenId <= extSaleRange()) {
    ...
    // Special function for setting the URI of special WickedAliens
    ...
    // special function for minting special WickedAliens
    ...
    function withdraw() public {
        ...
        receiver.transfer(pendingWithdrawals);
    ...
    // Sets the URIs for all the WickedAliens
    function _setURI(uint256 tokenId, string memory cid) internal virtual {
        ...
        if (!presaleStatus() || tokenId > extSaleRange()) {
            _tokenURIs[tokenId] = cid;
        }
    }
        ...
        virtual
    {
        ...
        if (getNextTokenID() > presaleLimit) {
            require(
                !presaleStatus(),
                "Presale range sold out, You cannot buy until the presale time ends"
            );
        }
    ...
    function _incrementExtSaleRange() private {
        require(
            extSaleRange() <= MAX_PUBLIC_RANGE,
            "Max public range sold out"
        );
        _extSaleRange++;
    }
