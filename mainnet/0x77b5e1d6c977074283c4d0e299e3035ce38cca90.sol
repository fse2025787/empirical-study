...
Squuid squuid;
    ...
    constructor(address _squuid) {
        squuid = Squuid(_squuid);
    }
    

    function getTokenIds(address _owner) public view returns (uint256[] memory _tokensOfOwner) {
        _tokensOfOwner = new uint256[](squuid.balanceOf(_owner));
        for (uint256 i;i<squuid.balanceOf(_owner);i++){
            _tokensOfOwner[i] = squuid.tokenOfOwnerByIndex(_owner, i);
        }
  }
}