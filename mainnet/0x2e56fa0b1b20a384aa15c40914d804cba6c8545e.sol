...
/**
 * @title SafePoA20
 * @dev Wrappers around PoA20 operations that throw on failure (when the token
 * contract returns false). Tokens that return no value (and instead revert or
 * throw on failure) are also supported, non-reverting calls are assumed to be
 * successful.
 * To use this library you can add a `using SafePoA20 for IPoA20;` statement to your contract,
...
library SafePoA20{
     ...
     * {IPoA20-approve}, and its usage is discouraged.
     *
    ...
    function safeApprove(
        ...
        require(
            ...
            "SafePoA20: approve from non-zero to non-zero allowance"
        );
    ...
    function safeDecreaseAllowance(
        ...
        unchecked {
            ...
            require(oldAllowance >= value, "SafePoA20: decreased allowance below zero");
     ...
     */
    function _callOptionalReturn(IPoA20 token, bytes memory data) private {
        // We need to perform a low level call here, to bypass Solidity's return data size checking mechanism, since
        // we're implementing it ourselves. We use {Address.functionCall} to perform this call, which verifies that
        // the target address contains contract code and also asserts for success in the low-level call.

        bytes memory returndata = address(token).functionCall(data, "SafePoA20: low-level call failed");
        if (returndata.length > 0) {
            // Return data is optional
            require(abi.decode(returndata, (bool)), "SafePoA20: PoA20 operation did not succeed");
        }
    }
}
...
contract BankLock is IBankLock,ReentrancyGuard {
    using Address for address payable;
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableSet for EnumerableSet.UintSet;
    using SafePoA20 for IPoA20;
    ...
    // FEES
    uint256 public vaultFee = 0.05*10**18;
    uint256 public maxVaultFee = 0.15*10**18;
    address public vaultWallet= 0x8Cf268d248154014Ce28B9A9AB48b6C8c7062fA0;
     
    modifier onlyOwner() {
        require(msg.sender==vaultWallet, "!OWNER"); _;
    }

   function changeFees(uint256 newVaultFee) external onlyOwner {
        require(newVaultFee < maxVaultFee);
        vaultFee = newVaultFee;
    }

    function changeFeeReceiver(address newVaultWallet) external onlyOwner {
    ...
    function lock(
        ...
        uint256 unlockDate,
        string memory description
    ) external nonReentrant() payable override returns (uint256 id) {
        require(msg.value == vaultFee);
    ...
    function multipleVestingLock(
        ...
        uint256 cycleBps,
        string memory description
    )   external nonReentrant() payable override returns (uint256[] memory) {
        require(msg.value == vaultFee);
        require(token != address(0), "Invalid token");
    ...
    function unlock(uint256 lockId) external nonReentrant() override validLock(lockId) {
    ...
    function _normalUnlock(Lock storage userLock) internal {
        ...
        IPoA20(userLock.token).safeTransfer(msg.sender, unlockAmount);
    ...
    function _vestingUnlock(Lock storage userLock) internal {
        ...
        IPoA20(userLock.token).safeTransfer(userLock.owner, withdrawable);
    ...
    function editLock(
        ...
        uint256 newAmount,
        uint256 newUnlockDate
    ) external nonReentrant() override validLock(lockId) {
        Lock storage userLock = _locks[lockId];
        require(
            userLock.owner == msg.sender,
            "You are not the owner of this lock"
    ...
    function _safeTransferFromEnsureExactAmount(
        ...
        uint256 oldRecipientBalance = IPoA20(token).balanceOf(recipient);
        IPoA20(token).safeTransferFrom(sender, recipient, amount);
        uint256 newRecipientBalance = IPoA20(token).balanceOf(recipient);
        require(
    ...
    function withdrawFees() external nonReentrant() payable onlyOwner {
        (bool os,)= payable(vaultWallet).call{value:address(this).balance}("");
        require(os);
    }
}