...
contract DragonlordEggs is ERC721AQueryable, Ownable {
    ...
    SalePhase public salePhase = SalePhase.PAUSED;

    constructor() ERC721A("Dragonlord Eggs", "DLE") {}

    modifier mintPhaseCompliant() {
        if (salePhase == SalePhase.WHITELIST) {
            require(whitelistedAddresses[msg.sender], "You are not whitelisted");
        } else {
            require(salePhase == SalePhase.PUBLIC, "Sale is not public");
        }
        _;
    }

    modifier mintSupplyCompliant(uint256 _mintAmount) {
        require(_mintAmount <= maxMintAmountPerTx, "Invalid mint amount!");
        require(
            _numberMinted(msg.sender) + _mintAmount <= maxMintPerWallet,
    ...
    modifier mintPriceCompliant(uint256 _mintAmount) {
        if (whitelistedAddresses[msg.sender]) {
            uint256 numberMinted = _numberMinted(msg.sender);
            uint256 numberToPayFor = numberMinted > 0 ? _mintAmount : _mintAmount - 1;
            require(msg.value >= cost * numberToPayFor, "insufficient funds");
        } else {
    ...
    function mint(uint256 _mintAmount)
        external
        payable
        mintSupplyCompliant(_mintAmount)
        mintPriceCompliant(_mintAmount)
        mintPhaseCompliant
    {
