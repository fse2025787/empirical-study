...
/* STAKING MECHANICS */
    ...
    function getTokensByOwner(address owner) external view returns(uint256[] memory tokens) {
        uint256 balance = IERC721(stackable).balanceOf(owner);

        if (balance > 0) {
            tokens = new uint256[](balance);
            for (uint256 i = 0; i < balance; i++) {
                tokens[i] = IERC721Enumerable(stackable).tokenOfOwnerByIndex(owner,i);
            }
        } 
