...
contract LootTerritory is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getPopulation(uint256 tokenId) public view returns (string memory) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Sheet #', toString(tokenId), '", "description": "Loot Territory are randomized RPG style Territory generated and stored on chain. Feel free to use Loot Territory in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
