...
contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(
            newOwner != address(0),
            "Ownable: new owner may not be the zero address"
        );
...
// solhint-disable
contract CommemenismToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public devAddress =
        payable(0xA525edB2428D7E9a087A98C0261FF3Fe5e17d85f);

    address payable public liquidityAddress =
        payable(0x828eE4072cDDD8F486569895371E851B05792B6d);

    address payable public treasuryAddress =
        payable(0x8FB776ACa35b705B52F7228002a7910D4129247e);

    address payable public marketingAddress =
        payable(0xC74E65ECa8bD7c809dB0d0Ee7fD0F711Ef905B8b);                
    ...
    uint256 private constant _tTotal = 100_000_000_000 * 1e9; // 100 Billion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Co-MEME-nism Token";
    string private constant _symbol = "COMEME";
    ...
    // Fee Breakdown
    uint256 public _buyTaxFee = 0;              // No Reflections
    uint256 public _buyLiquidityFee = 3;
    uint256 public _buyTeamFee = 7;             // To be distributed to Dev/Marketing and Charity (Disbursed from Treasury)
                                                // per split below

    uint256 public _sellTaxFee = 0;             // No Reflections
    uint256 public _sellLiquidityFee = 3;
    uint256 public _sellTeamFee = 7;            // To be distributed to Dev/Marketing and Charity (Disbursed from Treasury)
                                                // per split below

    // These values are the percentages of the team fees to distribute (add extra 0: 333 = 33.3%)
    // These combined values must be lower than 1000 (100%)
    // Remaining pocket lint goes to marketing address
    uint256 public _percentTeamFundsToDev = 284;
    uint256 public _percentTeamFundsToTreasury = 715;

    uint256 public tradingActiveBlock = 0; // 0 = trading is not active
    ...
    constructor() payable {
        _owner = msg.sender;
        _rOwned[_owner] = (_rTotal / 1000) * 300;
        _rOwned[address(this)] = (_rTotal / 1000) * 700;

        maxTransactionAmount = (_tTotal * 5) / 1000;        // 0.5% maxTransactionAmountTxn
        maxWalletAmount = (_tTotal * 6) / 1000;             // 0.6% maxWalletAmount
        ...
        // Mint
        // Owner Wallet to manage distribution to Dev, Charity and Prize Pool
        emit Transfer(address(0), _owner, (_tTotal * 300) / 1000);
        // Contract token supply to be placed in initial liquidity pool and locked
        emit Transfer(address(0), address(this), (_tTotal * 700) / 1000);        
    }
    ...
    // Call this function after token is stable, allow 30 to 60 minutes
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                // Sniper protection
                // Higher liquidity fee if bought on same block or next block as trading actived
            ...
            // Sell
                ...
                // Sniper protection
                // Higher liquidity fee if bought on same block or next block as trading activated
                if (block.number == tradingActiveBlock) {
                    _taxFee = 0;
                    _liquidityFee = 90;
                } else if (block.number == tradingActiveBlock + 1) {
                ...
                // Normal transfers do not get assessed fees
            } else {
                removeAllFee();
                buyOrSellSwitch = TRANSFER; // Transfer do not pay fees
            }
        }
    ...
    function swapBack() private lockTheSwap {
        ...
        // Divide the liquidity into tokens for liquidity and tokens to swap for ETH
        ...
        // send remainder to the marketing wallet so it does not remain on the contract.
    ...
    function addToBlacklist(address account) external onlyOwner {
        require(account != address(0), "Cannot blacklist the 0 address");
        blacklist[account] = true;
    }

    // Remove from blacklist
    function removeFromBlacklist(address account) external onlyOwner {
        require(account != address(0), "Cannot use the 0 address");
    ...
    function setDeveloperAddress(address _devAddress) external onlyOwner {
        require(_devAddress != address(0), "_devAddress cannot be 0 address");
        devAddress = payable(_devAddress);
        _isExcludedFromFee[devAddress] = true;
    }

    function setTreasuryAddress(address _treasuryAddress) external onlyOwner {
        require(_treasuryAddress != address(0), "_treasuryAddress cannot be 0 address");
