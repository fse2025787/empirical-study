...
contract PutinCards is Ownable, ERC721A, PaymentSplitter {
    ...
    constructor(address[] memory _team, uint[] memory _teamShares, string memory _baseURI) ERC721A("Putin Digital Trading Cards", "PUTINCARDS")
    ...
    function publicSaleMint(uint _quantity) external payable {
        ...
        if(mintedAmountNFTsperWalletPublicSale[msg.sender] + _quantity > maxMintAmountPerPublic) revert("Limit per wallet for public exceeded");
    ...
    function currentState() external view returns (Step, uint, uint, uint) {
        return (sellingStep, publicSalePrice, maxMintAmountPerPublic, maxAmountPerTxnPublic);
    }
