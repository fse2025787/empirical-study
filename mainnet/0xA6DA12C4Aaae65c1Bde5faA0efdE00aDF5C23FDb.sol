...
/* 
A big thanks to Zora for sharing their auction contracts with the world! 
It has provided us with much inspiration and guidance.
*/
...
/**
 * @title An auction house for the HaktDAO project
 */
contract HaktAuctionHouse is IAuctionHouse, ReentrancyGuard, Ownable {
     ...
     * Constructor
     */
    constructor(address _weth, address _dao) {
        wethAddress = _weth;
    ...
    /**
     * @notice Set a new Time Buffer.
     * @dev Only allow the Owner to change it.
     */
    ...
    /**
     * @notice Set a new min bid increment percentage.
     * @dev Only allow the Owner to change it.
     */
    function setMinBidIncrementPercentage(uint8 _minBidIncrementPercentage) public onlyOwner {
        minBidIncrementPercentage = _minBidIncrementPercentage;
    }

    /**
     * @notice Set a new DAO address.
     * @dev Only allow the DAO address to change the DAO address in this contract.
     */
    function setDAOAddress(address _dao) public {
        require(daoAddress == msg.sender, "Can only be called by the DAO Address");
        daoAddress = _dao;
    }
    ...
    function createAuction(
        ...
        require(address(this) == IERC721(tokenContract).getApproved(tokenId), "Caller must be approved or owner for token id");
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        require(
            msg.sender == owner() || msg.sender == auctions[auctionId].bidder,
            "Can only be called by Owner or winner"
        );
        ...
        // DAO address receives the Profit
        _handleOutgoingBid(daoAddress, tokenOwnerProfit, auctions[auctionId].auctionCurrency);
     ...
     */
    function cancelAuction(uint256 auctionId) external override nonReentrant auctionExists(auctionId) onlyOwner {
        require(
            uint256(auctions[auctionId].firstBidTime) == 0,
