...
contract BondDepository is ReentrancyGuard, Pausable, Ownable {
    ...
    address public immutable KOGA; // token given as payment for bond
    ...
    struct Terms {
        ...
        uint256 tokenPrice; // token price so how many KOGA you get for one principle
    }
    ...
    function deposit(
        ...
        uint256 value = ((_amount * ((terms.principlePrice * 1e9) / terms.tokenPrice))/1e9) /10 **(IERC20Deci(principle).decimals()-IERC20Deci(KOGA).decimals());
        ...
        IERC20Mintable(KOGA).mint(address(this), value);

        if (fee != 0) {
            // fee is transferred to dao
            IERC20(KOGA).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(KOGA).safeTransfer(_recipient, _amount); // send payout
        } else {
            IERC20(KOGA).safeIncreaseAllowance(staking, _amount);
    ...
    function principlePriceInUSD() public view returns (uint256 price_) {
        price_ = principlePrice() / (10**IERC20Deci(KOGA).decimals() / 100);
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        price_ = bondPrice() / (10**IERC20Deci(KOGA).decimals() / 100);
    }

    /* ======= AUXILLIARY ======= */

    /**
     *  @notice allow anyone to send lost tokens (excluding principle or KOGA) to the DAO
     *  @return bool
     */
