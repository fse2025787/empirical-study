...
/**
 * @title LiquidityMiningPools
 * Version: 1.4
 */
contract LiquidityMiningPools is MyPausableUpgradeable, IPools {
  ...
  function setCrossChainBridgeERC20LiquidityManager(ICrossChainBridgeERC20LiquidityManager contractAddress) external {
    require(
      ...
      'LiquidityMiningPools: must have MANAGE_LIQUIDITY_MANAGER_ROLE role to execute this function'
    );
  ...
  function setMinStakeAmount(address tokenAddress, uint256 _minStakeAmount) external {
    ...
    require(liquidityMiningPools[tokenAddress].exists, 'LiquidityMiningPools: liquidityMiningPool does not exist');
  ...
  function setBuyBackAndBurnContract(IBuyBackAndBurn _buyBackAndBurnContract) external {
    require(
      ...
      'LiquidityMiningPools: must have MANAGE_COLLECTED_FEES_ROLE role to execute this function'
    );
    require(
  ...
  function stake(address tokenAddress, uint256 amount) external whenNotPaused nonReentrant {
    ...
    require(amount > 0, 'LiquidityMiningPools: staking amount cannot be 0');
    require(liquidityMiningPools[tokenAddress].exists, 'LiquidityMiningPools: liquidityMiningPool does not exist');
    require(amount >= pool.minStakeAmount, 'LiquidityMiningPools: staking amount too small');
    require(
      pool.maxStakeAmount == 0 || staker.balance + amount <= pool.maxStakeAmount,
      'LiquidityMiningPools: staking amount too high'
    );
    require(
      pool.maxPoolSize == 0 || pool.totalStakedAmount + amount <= pool.maxPoolSize,
      'LiquidityMiningPools: max staking pool size exceeded'
    );
  ...
  function unstake(address tokenAddress, uint256 amount) external whenNotPaused nonReentrant {
    ...
    // check input parameters
    require(amount > 0, 'LiquidityMiningPools: amount to be unstaked cannot be 0');
    require(pool.exists, 'LiquidityMiningPools: liquidityMiningPool does not exist');
    require(staker.balance >= amount, 'LiquidityMiningPools: amount exceeds available balance');
    if (staker.balance - amount != 0) {
      // check if remaining balance is above min stake amount
      require(
        staker.balance - amount >= pool.minStakeAmount,
        'LiquidityMiningPools: remaining balance below minimum stake amount'
      );
    }
  ...
  function transferInterestBearingTokenHandler(
    ...
    require(
      ...
      'LiquidityMiningPools: function can only be called by PoolsInterestBearingToken'
    );
    require(
      tokenAddress != address(0) && from != address(0) && to != address(0),
      'LiquidityMiningPools: invalid address provided'
    );
    require(amount > 0, 'LiquidityMiningPools: transfer amount cannot be 0');
    ...
    require(receiverFinalBalance >= pool.minStakeAmount, 'LiquidityMiningPools: transfer amount too small');
    require(
      pool.maxStakeAmount == 0 || receiverFinalBalance <= pool.maxStakeAmount,
      'LiquidityMiningPools: transfer amount too high'
    );
    require(stakerSender.balance >= amount, 'LiquidityMiningPools: insufficient balance for transfer');
  ...
  function addRewards(IERC20 token, uint256 amount) external whenNotPaused nonReentrant {
    // check input parameters
    require(address(token) != address(0), 'LiquidityMiningPools: invalid token address provided');

    // get LP token for the given token
    (, IERC20 lpToken) = liquidityManager.lpTokens(address(token));
    require(address(lpToken) != address(0), 'LiquidityMiningPools: could not find LP token for this token');
  ...
  function _harvest(address tokenAddress, address stakerAddress) private whenNotPaused {
    ...
    require(staker.balance > 0, 'LiquidityMiningPools: Staker has a balance of 0');
