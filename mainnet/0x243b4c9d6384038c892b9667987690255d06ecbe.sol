...
contract TokenPresaleV2 is Ownable {
    ...
    function availableLimit(address beneficiary) public constant returns (uint256){
	   uint256 totalBuy = balance[beneficiary];
	   uint256 tokens = buyLimit-totalBuy;
       return tokens;
    }
	
    function tokenBalance() public constant returns (uint256 balances){
        return Token(tokenAddr).balanceOf(this);
    }
	
	function convertWeiToTokens(uint256 weiAmount) public constant returns (uint256) {
