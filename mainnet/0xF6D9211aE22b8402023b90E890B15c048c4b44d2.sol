...
contract UpgradeableHimeNFT is
    Initializable,
    ERC721Upgradeable,
    ...
    ERC2981Upgradeable
{
    ...
    function initialize(
        ...
        __ERC2981_init();
        setRoyaltyInfo(0x6Cb19608A1551ed8F820B7Abb602F762D6029969, _royaltyFee);
        ...
        notRevealedURI = "ipfs://bafybeidc64plmyfnfdd65cclj45ievnpkrqejzf3mkrjy2oe6yj45iojpy/";
        revealedURI = "ipfs://bafybeihocema66ksjk257hwekvtsbuxjm3thuiad42ureu5j7qxt4tg454/";

        mintCost = 0.1 ether;
        paymentMintMD = (mintCost * 95) / 100;
        maxPerMint = 3;
        ...
        // all time in GMT
        publicSaleStartTime = 1667221200;
        publicSaleEndTime = 1669813200;
        revealTime = 1669813200;
    }
    ...
    function withdrawMD() public payable onlyRole(MD_ROLE) {
        ...
        (bool success, ) = payable(_msgSender()).call{value: tempBalance}("");
        require(success);
    ...
    function mint(uint32 count) public payable preMintChecks(count) {
        require(isPublicSalePeriod(), "Not Open");

        performMint(_msgSender(), count);

        balanceMD += paymentMintMD * count;
        balanceBMD += (mintCost - paymentMintMD) * count;
    }

    function airdrops(address[] memory _recipients)
        public
        onlyRole(OPERATOR_ROLE)
    {
        for (uint256 i = 0; i < _recipients.length; i++) {
            performMint(_recipients[i], 1);
        }
    }

    function performMint(address minter, uint32 count) internal {
        uint256 supply = totalSupply();

        for (uint32 i = 0; i < count; i++) {
            _safeMint(minter, ++supply, "");
        }
    }
        ...
        returns (string memory)
    {
        ...
        return bytes(targetURI).length > 0 ? targetURI : "";
    }
    ...
    modifier preMintChecks(uint32 count) {
        ...
        require(supply + count < maxSupply + 1, "Sold out");
        _;
    }
}
