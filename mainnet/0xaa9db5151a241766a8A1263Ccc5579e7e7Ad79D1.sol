...
interface IPiBundleMarketplace {
 ...
 */
contract PiAuction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    using AddressUpgradeable for address payable;
    using SafeERC20 for IERC20;

    
    event PiAuctionContractDeployed();
        ...
        initializer
    {
        require(
            ...
            "PiAuction: Invalid Platform Fee Recipient"
        );

        platformFeeRecipient = _platformFeeRecipient;
        emit PiAuctionContractDeployed();
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IPiTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
 ...
 */
        ...
        nonReentrant
    {
        ...
        IPiMarketplace marketplace = IPiMarketplace(
        ...
        IPiBundleMarketplace(addressRegistry.bundleMarketplace())
        ...
        emit AuctionResulted(
            ...
            winner,
            auction.payToken,
            IPiMarketplace(addressRegistry.marketplace()).getPrice(
                auction.payToken
            ),
            winningBid
        );
    ...
    /**
     @notice Update PiAddressRegistry contract
     @dev Only admin
     */
