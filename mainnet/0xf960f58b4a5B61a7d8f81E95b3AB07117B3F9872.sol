...
contract ERC1155Token is ERC1155Supply, ReentrancyGuard, Ownable {
...
contract NormalERC1155 is ERC1155Token, ERC2981 {
    ...
    function uri(uint256 id_) public view virtual override returns (string memory)
    {
        return string(abi.encodePacked(super.uri(id_), suffix));
    }
    ...
    function _mint(
        ...
        require(id <= maxId, "over id");
        super._mint(to, id, amount, data);
    }

    function mintAll(uint256 id) external payable {
        MintLimitInfo memory limit = getMintLimit(id);
        require(limit.amount > mintCount[_msgSender()][id], "over limit");
        uint256 amount = limit.amount - mintCount[_msgSender()][id];
        mint(id, amount);
    }
    ...
    function getMintLimit(uint256 id) public view returns (MintLimitInfo memory) {
        MintLimit memory limit = mintLimitList[id];
        if (limit.operator == address(0)) {
            limit = _defaultMintLimit;
        }
        return MintLimitInfo(limit.price, limit.amount, limit.maxAmount, totalSupply(id));
    }

    function setDefaultMintLimit(uint128 price, uint64 amount, uint64 maxAmount) external onlyOwner {
        require(maxAmount >= amount, 'amount over maxAmount');
        _defaultMintLimit = MintLimit(msg.sender, price, amount, maxAmount);
    }

    function deleteDefaultMintLimit() external onlyOwner {
        delete _defaultMintLimit;
    }

    function setMintLimit(uint256 id, uint128 price, uint64 amount, uint64 maxAmount) external onlyOwner {
        require(maxAmount >= amount, 'amount over maxAmount');
        mintLimitList[id] = MintLimit(msg.sender, price, amount, maxAmount);
    }

    function resetMintLimit(uint256 id) external onlyOwner {
        delete mintLimitList[id];
    }
