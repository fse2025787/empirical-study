...
contract SHIBPUP is Context, IERC20, Ownable {
    ...
    string private _name = "Shib Puppy";
    string private _symbol = "SHIBPUP";
    ...
    address public marketingWallet = 0x2564694fD796fD3474Bf81a3f5130418c9205AE3;
    address public developmentWallet = 0x921A79dE9BD10Ac12f8393985bAc51940f2DB513;
    ...
    uint256 public _maxWalletSize = 1000000000000000 * 10**18; 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(sender != owner() && recipient != owner() && sender!= address(this))
        {
            ...
            developmentAmount = calculatedevelopmentFee(amount);
        }


        
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(marketingAmount).sub(developmentAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(marketingAmount).sub(developmentAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(developmentAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(marketingAmount).sub(developmentAmount));
        } else {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(developmentAmount));
        }
        ...
        if(developmentAmount > 0) {
            _transferStandard(sender, developmentWallet, developmentAmount);
        }
