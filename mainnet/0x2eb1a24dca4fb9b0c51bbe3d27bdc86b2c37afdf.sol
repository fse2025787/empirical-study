...
contract WorldCupApesICO is Ownable {
  ...
  modifier onlyBrokerOrOwner {
    require(msg.sender == owner() || msg.sender == broker, "Unauthorized");
    _;
  }

  constructor(address _WCAToken, address _USDT, address _wallet, address _broker) {
  ...
  function buy(address _address, uint256 _value, uint256 _price) external onlyBrokerOrOwner purchaseEnabled {
