...
contract RussianBridge is IERC20, Ownable {
    ...
    uint256 public _totalBuyTaxes;
    uint256 private _previousTotalBuyTaxes = _totalBuyTaxes;

    uint256 public _totalSellTaxes;
    ...
    uint256 public _extraMarketingSell;
    uint256 public _extraDevSell;
    uint256 public _extraLiquiditySell;
    ...
    uint256 public otherPortion;
    uint256 public _preSellDevelopmentFee;
    ...
    uint256 public totalDev;

    uint256 public liqPercent;
    uint256 public liquidityTokenPortion;
    ...
    constructor(
        ...
        _marketingFee = 4;
        _previousmarketingFee = 4;

        _developmentFee = 1;
        _previousDevelopmentFee = 1;

        _totalBuyTaxes = 7;
        _previousTotalBuyTaxes = 7;
        ...
        numTokensSellToAddToLiquidity =  200000000000000; // 1% of an CRO
        ...
        marketingAddress=payable(0xCf678FF6098A661c5fa7e9cB5248244B75Fd3cD9);

        developmentAddress=payable(0xCf678FF6098A661c5fa7e9cB5248244B75Fd3cD9);
        
        stakingAddress=payable(0xCf678FF6098A661c5fa7e9cB5248244B75Fd3cD9);
    ...
    function setSellFees(uint256 extMarkFee, uint256 extraLiqFee, uint256 extraDevFee) public onlyOwner 
    {
    ...
    function addPreSale(address [] memory addrArray, uint256 [] memory amt) external onlyOwner {
        ...
        {
            _preSale[addrArray[x]]=amt[x];
        }
        
    }

    function changeAddresss(address payable sAddress, address payable mAddress, address payable dAddress) external onlyOwner {
    ...
    function setTaxPercents(uint256 liquidityFeeBps, uint256 marketingFeeBps, uint256 developmentFeeBps) external onlyOwner {

        _liquidityFee = liquidityFeeBps;
        _marketingFee = marketingFeeBps;
        _developmentFee = developmentFeeBps;

        _totalBuyTaxes = _liquidityFee + _marketingFee  + _developmentFee;
      
        require(   _totalBuyTaxes >= 0 && _totalBuyTaxes <= 20,"Invalid bps");
    }
        ...
        )
    {
       ...
       (uint256 tLiquidity, uint256 tMarketing, uint256 tDevelopment) = calculateFees(tAmount);
        uint256 tTransferAmount = tAmount-(tLiquidity);
        tTransferAmount=tTransferAmount-(tMarketing)-(tDevelopment);

        return (tTransferAmount,  tLiquidity, tMarketing, tDevelopment);
    }
    ...
    function restoreAllFee() private {
        ...
        _extraMarketingSell=_previousExtraLiquiditySell;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
         ...
         liqPercent = (totalLiquidity*100)/contractTokenBalance;  
        uint256 markPercent = (totalMarketing*100)/contractTokenBalance; 
        uint256 devPercent = (totalDev*100)/contractTokenBalance; 
        
         liquidityTokenPortion = contractTokenBalance/(100)*(liqPercent);
        liquidityTokenPortion = liquidityTokenPortion/(2);

       otherPortion = contractTokenBalance-liquidityTokenPortion;
        ...
        uint256 divisor = markPercent + devPercent  + liqD;
        ...
        uint256 marketingPortion = (newBalanceAfterLiq/(total))*(markPercent);
        payable(marketingAddress).transfer(marketingPortion);

        uint256 developmentPortion = (newBalanceAfterLiq/(total))*(devPercent);
    ...
    function _transferStandard(
        ...
        (
         ...
         if (_isLaunched !=true && _preSale[recipient] !=0) 
        {
            require(balanceOf(recipient)+tAmount<=_preSale[recipient]);
            require(tAmount <= _preSale[recipient]);
            _balance[sender] = _balance[sender]-(tAmount);
            _balance[recipient] = _balance[recipient]+(tTransferAmount);
            emit Transfer(sender, recipient, tTransferAmount);                    
        }

        else if (_isLaunched !=true && recipient !=uniswapV2Pair && sender!=owner() && recipient!=owner()) 
        {
            _balance[sender] = _balance[sender]-(tAmount);
            _balance[recipient] = _balance[recipient]+(tTransferAmount);
            emit Transfer(sender, recipient, tTransferAmount);                    
