...
contract DividendDistributor is IDividendDistributor {
    ...
    ERC20 RWRD = ERC20(0x000000000000000000000000000000000000dEaD);
...
contract ZEROINU is ERC20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Zero Inu";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x4EcbfE9D4194e8f601628cd620E94D2Cc85FFC9F;
    ...
    function clearStuckBalance(uint256 amountPercentage) external authorized {
        uint256 amountERC = address(this).balance;
        payable(marketingFeeReceiver).transfer(amountERC * amountPercentage / 100);
    }

    function clearStuckBalance_sender(uint256 amountPercentage) external authorized {
        uint256 amountERC = address(this).balance;
        payable(msg.sender).transfer(amountERC * amountPercentage / 100);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalERCFee = totalFee.sub(dynamicLiquidityFee.div(2));
       
        uint256 amountERCLiquidity = amountERC.mul(dynamicLiquidityFee).div(totalERCFee).div(2);
        uint256 amountERCReflection = amountERC.mul(reflectionFee).div(totalERCFee);
        uint256 amountERCMarketing = amountERC.mul(marketingFee).div(totalERCFee);
        uint256 amountERCTeam = amountERC.mul(teamFee).div(totalERCFee);
        uint256 amountERCDev = amountERC.mul(devFee).div(totalERCFee);

        try distributor.deposit{value: amountERCReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountERCMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(teamFeeReceiver).call{value: amountERCTeam, gas: 30000}("");
        (tmpSuccess,) = payable(devFeeReceiver).call{value: amountERCDev, gas: 30000}("");
       
        // because life isnt fair
        tmpSuccess = false;

        if(amountToLiquify > 0){
            router.addLiquidityETH{value: amountERCLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
            ...
            emit AutoLiquify(amountERCLiquidity, amountToLiquify);
        }
    }
...
event AutoLiquify(uint256 amountERC, uint256 amountBOG);
