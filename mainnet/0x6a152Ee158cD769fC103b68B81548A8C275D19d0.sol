...
contract Bus is ERC20, Ownable {
    ...
    string constant _name = "Bus";
    string constant _symbol = "$Bus";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 8000000 * (10 ** _decimals);
    uint256 public _maxWalletAmount = _totalSupply.mul(50).div(1000);
    uint256 public _maxTx = _totalSupply.mul(60).div(1000);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 amountReceived = (shouldTakeFees(sender) && shouldTakeFees(recipient)) ? takeFee(sender, recipient, amount) : amount;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == pair && _feeBuy.totalFee != 0) {// Buy
            ...
            feeAmount = amount.mul(_sell).div(feeDenominator);
            _balances[_taxWallet.marketing] = _balances[_taxWallet.marketing].add(feeAmount);
        }
    ...
    function updateTaxWallet(address _marketing, address _addLp) external onlyOwner {
    ...
    function setFeeExempts(address adr, bool _isFeeExempt) external onlyOwner {
        isFeeExempt[adr] = _isFeeExempt;
    }

    function setMultipleFeeExempts(address[] calldata wallets, bool _isFeeExempt) external onlyOwner {
