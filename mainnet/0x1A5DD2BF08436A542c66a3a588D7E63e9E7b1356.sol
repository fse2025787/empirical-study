...
contract copypasta is Context, IERC20, Ownable {
    ...
    uint256 private _buyBuyBackTax = 2;
    uint256 private _sellBuyBackTax = 3;

    uint256 private _totalBuyTax = _buyDevTax + _buyMarketingTax + _buyBuyBackTax;
    uint256 private _totalSellTax = _sellDevTax + _sellMarketingTax + _sellBuyBackTax;
    ...
    address payable private _BuyBackW;
    ...
    constructor(address payable marketingTaxAddress, address payable devfeeAddr, address payable buyback) {
        ...
        _isExcludedFromFee[_BuyBackW] = true;
    ...
    function takeSellFee() private {
        _devTax = _sellDevTax;
        _salesTax = _sellBuyBackTax;
        _marketingTax = _sellMarketingTax;
        _summedTax = _sellBuyBackTax+_sellMarketingTax;
    }
    ...
    function sendETHToFee(uint256 amount) private {
        _Marketingfund.transfer(amount.div(_totalSellTax).mul(_sellMarketingTax));
        _devWalletAddress.transfer(amount.div(_totalSellTax).mul(_sellDevTax));
        _BuyBackW.transfer(amount.div(_totalSellTax).mul(_sellBuyBackTax));
    }
