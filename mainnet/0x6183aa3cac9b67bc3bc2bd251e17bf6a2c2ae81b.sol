...
contract FuglyFairies is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public maxperAddressMint = 10;

    mapping(address => uint256) public addressFreeMintedBalance;

    constructor() ERC721A("FuglyFairies", "FUGLY") {
        setBaseURI("https://fuglyfairies.s3.amazonaws.com/metadata/");
    ...
    function mintFree(uint256 _mintAmount) public payable nonReentrant{
        ...
        require(addressFreeMintedCount + _mintAmount <= maxperAddressFreeLimit, "Max Fugly Fairies per address exceeded");
		require(_mintAmount > 0, "Cannot mint 0 Fugly Fairies" );
		require(s + _mintAmount <= maxFree, "Cannot exceed Fugly Fairies supply" );
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant {
        uint256 s = totalSupply();
        require(_mintAmount > 0, "Cannot mint 0 Fugly Fairies");
        require(_mintAmount <= maxperAddressMint, "Cannot mint more Fugly Fairies" );
        require(s + _mintAmount <= maxSupply, "Cannot exceed Fugly Fairies supply");
    ...
    function gift(uint256[] calldata quantity, address[] calldata recipient)
        ...
        require(s + totalQuantity <= maxSupply, "Too many Fugly Fairies");
    ...
    function setMaxperAddressMint(uint256 _amount) public onlyOwner {
        maxperAddressMint = _amount;
    }
