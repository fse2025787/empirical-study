...
contract WIDEANGLE is ERC721, ReentrancyGuard {
    ...
    constructor(
        ...
        uint256[] memory categoryMaxSupplies,
        uint256[] memory categoryCounters,
        uint256[] memory categoryPrices,
        uint256[] memory categoryPreSalePrices,
        uint256[] memory categoryPrivSalePrices)
        ERC721(name, symbol)
    {
        ...
        WIDEANGLE_ADMIN_WALLET = 0x82e47679037A73CB306bB828beBd076256D79d38;
        _tokenIdCounter += 1;
        _base_URI = baseURI;
        sellIsClose = 1;
        stableorEth = 0;
    ...
    modifier onlyOwner() {
        require(WIDEANGLE_OWNER == _msgSender(), 
        "Ownable: caller is not the owner");
        _;
    }

    modifier privatesaleTime() {
        require(mintFunctionsState[0] == 1, 
        "Private sale is not available.");
        _;
    }

    modifier presaleTime() {
        require(mintFunctionsState[1] == 1, 
        "Presale is not available.");
        _;
    }

    modifier publicsaleTime() {
        require(mintFunctionsState[2]==1,
        "Public sale is not available.");
        _;
    }
    ...
    onlyOwner
    {
        require(categoryId[_category].counter + numberOfTokens <= categoryId[_category].maxSupply, 
    ...
    privatesaleTime
    {
        require(privateSaleAddresses[_msgSender()] == 1,
        "Unidentified wallet address.");
        require(categories.length > 0, 
        "Wrong category.");
        require(categories.length <= 4, 
        "Maximum mint count exceeded.");
        uint256 price = 0;
    ...
    payable
    {
        require(whitelists[_msgSender()] == 1, 
        "You are not whitelisted.");
        require(_categories.length > 0, 
        "Wrong category.");
        require(_categories.length <= 4, 
        "Maximum mint count exceeded.");
        uint256 price = 0;
        if (stableorEth == 0){
            require(msg.value == 0);
    ...
    function changePublicSaleMintPrice(
        uint256 bronze,
        uint256 silver,
        uint256 gold,
        uint256 diamond
    ...
    function changePreSaleMintPrice(
        uint256 bronze,
        uint256 silver,
        uint256 gold,
        uint256 diamond
    ...
    function changePrivateSaleMintPrice(
        uint256 bronze,
        uint256 silver,
        uint256 gold,
        uint256 diamond
    ...
    function setPrivateSaleAddresses(address[] memory addresses) 
    external 
    onlyOwner{
    ...
    function switchMintFunction(uint256 whichFunction)
    external 
    onlyOwner {
        for(uint256 i = 0 ; i < 3 ; i++){
    ...
    function getPrivatesalePrices()
    public
    view 
    returns(uint256, uint256, uint256, uint256){
        return(
            categoryId[0].privateSalePrice,
            categoryId[1].privateSalePrice,
            categoryId[2].privateSalePrice,
            categoryId[3].privateSalePrice);
    }

    function getPresalePrices()
      public
      view 
      returns(uint256,uint256,uint256,uint256){
      return(
          categoryId[0].preSalePrice, 
          categoryId[1].preSalePrice, 
          categoryId[2].preSalePrice, 
          categoryId[3].preSalePrice);
    }

    function getPublicsalePrices() 
    public 
    view 
    returns(uint256,uint256,uint256,uint256){
      return(
          categoryId[0].publicSalePrice, 
          categoryId[1].publicSalePrice, 
          categoryId[2].publicSalePrice, 
          categoryId[3].publicSalePrice);
    }
