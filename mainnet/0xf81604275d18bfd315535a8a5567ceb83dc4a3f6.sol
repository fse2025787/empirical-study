...
contract PROT is IERC20, OWNED {
	mapping(address => uint256) private _balances;
	mapping(address => mapping(address => uint256)) private _allowances;
	uint8 private constant _decimals = 9;
	uint256 private constant _totalSupply = 1_000_000_000 * 10**_decimals;
	string private constant _name = "Protectors of the Wave";
	string private constant _symbol = "PROT";
	...
	uint256 private constant taxMcBracket1 = 8_000_000; // below this MC tax is 6% (2 LP, 2 Okinami LP, 2 Marketing)
	uint256 private constant taxMcBracket2 = 16_000_000; // below this MC tax is 4% (2 OkinamiLP, 2 Marketing)
	uint256 private constant taxMcBracket3 = 100_000_000; // below this MC tax is 2% (only OkinamiLP), above it tax is 0%

	mapping(address => bool) private _excluded;
	address private _marketingWallet = address(0x6f48933F51c30E7e6250fB08c015934e3d59637f);
	
	address private constant _okinami = address(0x1C4853Ec0d55e420002c5eFaBc7Ed8e0bA7A4121); 
	...
	Prot_Sidecar private immutable _sidecarContract;
	...
	constructor() OWNED(msg.sender)  {
		...
		_sidecarContract = new Prot_Sidecar();
	...
	function addressList() external view returns (address owner, address sidecar, address marketing, address okinami, address usdc, address swapRouter, address primaryLP) {
		return (_owner, _sidecarAddress, _marketingWallet, _okinami, _usdc, _swapRouterAddress, _primaryLP);
	}
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		(uint256 okinamiLP, uint256 werLP, uint256 marketing) = _getTaxTokens(from, to, amount);
		uint256 taxTokens = okinamiLP + werLP + marketing;
	...
	function _getTaxRates() private view returns (uint8 okinamiRate, uint8 werRate, uint8 marketingRate) {
		uint8 _okinamiRate; uint8 _werRate; uint8 _marketingRate;
		uint256 currentDilutedUsdMC = _getCurrentDilutedMcUSD();
		if (currentDilutedUsdMC < taxMcBracket1 ) {
			_okinamiRate = 2; _werRate = 2; _marketingRate = 2;
		} else if (currentDilutedUsdMC >= taxMcBracket1 && currentDilutedUsdMC < taxMcBracket2) {
			_okinamiRate = 2; _werRate = 0; _marketingRate = 2;
		} else if (currentDilutedUsdMC >= taxMcBracket2 && currentDilutedUsdMC < taxMcBracket3) {
			_okinamiRate = 2; _werRate = 0; _marketingRate = 0;
		} else { 
			_okinamiRate = 0; _werRate = 0; _marketingRate = 0;
		}
		return (_okinamiRate, _werRate, _marketingRate);
	}
	function _getTaxTokens(address from, address to, uint256 amount) private view returns (uint256 okinamiLP, uint256 werLP, uint256 marketing) {
		uint256 _okinamiLP; uint256 _werLP; uint256 _marketing;
		if ( (_isLP[from] && !_excluded[to]) || (_isLP[to] && !_excluded[from]) ) { 
			(uint8 okinamiRate, uint8 werRate, uint8 marketingRate) = _getTaxRates();
			_okinamiLP = amount * okinamiRate / 100;
		...
		return (_okinamiLP, _werLP, _marketing);
	}  
	...
	function tax() external view returns (uint8 Liquidityokinami, uint8 LiquidityProt, uint8 Marketing) { 
		(uint8 okinamiRate, uint8 werRate, uint8 marketingRate) = _getTaxRates();
		return (okinamiRate, werRate, marketingRate);
	}
	...
	function _processTaxTokens() private lockSwap {
		...
		(uint8 okinamiRate, uint8 werRate, uint8 marketingRate) = _getTaxRates();
		uint8 totalRate = okinamiRate + werRate + marketingRate;
		uint256 swapAmount = _balances[address(this)];
		if (totalRate>0 && swapAmount >= thresholdTokens) {
			swapAmount = thresholdTokens;

			uint256 tokensForokinami = (swapAmount * okinamiRate / totalRate);
			uint256 tokensForWerLP = (swapAmount * werRate / totalRate)/2;
			uint256 tokensForMarketing = swapAmount * marketingRate / totalRate;

			uint256 tokensToSwap = tokensForokinami + tokensForMarketing + tokensForWerLP;
			if (tokensToSwap >= 10**_decimals) {
				uint256 swappedOutputUSDC = _swapTokens(address(this), _usdc, tokensToSwap, true); //swap Prot for USDC, use sidecar contract
				uint256 usdcForWerLP = swappedOutputUSDC * tokensForWerLP / tokensToSwap; //calc USDC for Prot liquidity
				uint256 usdcToSpendOnokinami = (swappedOutputUSDC * tokensForokinami / tokensToSwap) / 2; //calc USDC for okinami liquidity
				uint256 usdcForMarketing = swappedOutputUSDC * tokensForMarketing / tokensToSwap; //calc USDC for marketing

				if (werRate>0) { _addLiquidity(address(this), tokensForWerLP, usdcForWerLP, true); } //add Prot liquidity and burn LP tokens

				if (okinamiRate>0) {
					uint256 okinamiPurchased = _swapTokens(_usdc, _okinami, usdcToSpendOnokinami, false); //purchase okinami for liquidity, sidecar not used
					_addLiquidity(_okinami, okinamiPurchased, usdcToSpendOnokinami, true); //add okinami liquidity and burn LP tokens
				}
