...
/**
 * @title Libertas SpaceWar Series contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract LibertasSpaceWar is ERC721, Ownable {
    ...
    ERC721 private constant LibertasSpectrum = ERC721(0xeD3a2472f9c371F12D43cEE483E687484499d580);
    ...
    function dropNft(address[] calldata dropAddrs) public onlyOwner {        
        ...
        uint supply = totalSupply();
        for (uint i = 0; i < dropAddrs.length; i++) {
    ...
    */
    ...
    /**
    * Mints Libertas SpaceWar Series
    */
    function mint(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint");
        require(numberOfTokens <= maxPurchase && numberOfTokens >= 1, "Can only mint 20 tokens at a time");
        require(msg.value >= price.mul(numberOfTokens), "not enough value for mint");
    ...
    function whitelistMint(uint num) public payable {
        require(saleIsActive, "Sale must be active to mint");
        require(num >= 1, "mint number must be greater than 0");
        require(balanceOf(msg.sender).add(num) <= LibertasSpectrum.balanceOf(msg.sender) || ( whitelist[msg.sender] && balanceOf(msg.sender).add(num) <= 5 ), "sender doesn't hold enough spectrum or whitelist mint more than 5");
        require(totalSupply().add(num) <= MAX_LIBER_NFT, "Purchase would exceed max supply");
