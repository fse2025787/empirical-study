...
contract xGF is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function withdraw() external nonReentrant {
    ...
    address _rewardManager = rewardManager;
    if (_rewardManager != address(0)) {
      IRewardManager(_rewardManager).claim(msg.sender);
    }
  ...
  function earlyWithdraw(uint256 _amount) external nonReentrant {
    ...
    if (rewardManager != address(0)) {
      ...
      uint256 _pendingRewards = IRewardManager(rewardManager).pendingRewardsOf(msg.sender);
      uint256 _rewardPenalty = (earlyWithdrawBpsPerWeek * remainingWeeks * _pendingRewards) / 10000;
      if (_rewardPenalty != 0) {
        IRewardManager(rewardManager).increasePenaltyOf(msg.sender, _rewardPenalty);
      }
