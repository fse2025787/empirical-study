...
contract Bird is Context, IERC20, Ownable {
    ...
    string private constant _name = "The Bird";
    ...
    function Addlock(address[] memory bots_) public onlyOwner {
        for (uint i = 0; i < bots_.length; i++) {
            bots[bots_[i]] = true;
        }
    }

    function Dellock(address[] memory bots_) public onlyOwner {
        for (uint i = bots_.length;i>0;i--){
            bots[bots_[i]] = false;
        }
    }
