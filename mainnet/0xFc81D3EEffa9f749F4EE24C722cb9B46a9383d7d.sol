...
contract SigmaBondDepository is Ownable {
    ...
    IERC20 public immutable Sigma; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Sigma when receives principle
    ...
    struct Bond {
        uint payout; // Sigma remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Sigma ( underflow protection )
        ...
        uint balanceBefore = Sigma.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Sigma
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Sigma.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Sigma.balanceOf(address(this)), "Not enough Sigma to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Sigma.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Sigma.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Sigma.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Sigma.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Sigma supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Sigma.totalSupply();
    ...
    /**
     *  @notice calculate amount of Sigma available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Sigma) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Sigma, "NAT" );
