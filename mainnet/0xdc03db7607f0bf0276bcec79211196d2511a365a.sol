...
contract LogLinearVRGDA {
    ...
    function getVRGDAAmountForPrice(
        ...
        amount = uint256(
            -unsafeWadDiv(
                wadLn(
                    1e18 -
                        unsafeWadDiv(
                            unsafeWadMul(price, decayConstant + 1e18),
                            int256(getVRGDAPrice(timeSinceStart, sold))
                        )
                ),
                decayConstant + 1e18
            )
    ...
    function getVRGDAPrice(
        ...
        price = uint256(
            wadMul(
                ...
                1e18 - wadExp(unsafeWadMul(-amount, decayConstant + 1e18))
            )
        );
    }
    ...
    function getVRGDAPrice(int256 timeSinceStart, int256 sold)
        public
        view
        virtual
        returns (uint256)
    {
        unchecked {
            ...
            return uint256(wadMul(targetPrice, wadExp(unsafeWadMul(decayConstant,
                ...
                timeSinceStart - getTargetSaleTime(sold)
            ))));
        }
    }
...
contract SupermassiveButterHole is
    VRFConsumerBaseV2,
    Initializable,
    LogLinearVRGDA,
    Ownable
{
    uint256 public immutable EMISSION_INTERVAL;
    uint256 public immutable EMISSION_SMOOTHING_INTERVAL;
    uint256 public immutable EMISSION_MULTIPLIER;
    ...
    GenesisGalaxy public galaxy;
    IERC20Resource public butter;

    uint40 public lastEmissionTimestamp;

    int104 public sold;
    uint104 public emissionMA;
    uint40 public lastEmissionMAUpdateTimestamp;

    constructor(
        address vrfCoordinator,
        uint256 emissionInterval,
        uint256 emissionSmoothingInterval,
        uint256 emissionMultiplier
    ) VRFConsumerBaseV2(vrfCoordinator) LogLinearVRGDA(1e15, .5e18, 1e18) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        EMISSION_INTERVAL = emissionInterval;
        EMISSION_SMOOTHING_INTERVAL = emissionSmoothingInterval;
    ...
    function initialize(
        ...
        butter = IERC20Resource(butter_);
        galaxy = GenesisGalaxy(galaxy_);
        keyHash = keyHash_;
        subId = subId_;
        emissionMA = 1e18 * uint104(EMISSION_MULTIPLIER);
        lastEmissionMAUpdateTimestamp = uint40(block.timestamp);
    ...
    function getPrice(uint256 amount) external view returns (uint256) {
        ...
        return ((getVRGDAPrice(
            toDaysWadUnsafe(block.timestamp - START_TIME),
            sold,
            _compound(unsafeWadDiv(int256(amount), int256(balance)))
        ) * balance) / 1e18);
    }

    function getAmountForPrice(uint256 price)
        public
        view
        returns (uint256 amount)
    {
        int256 balance = int256(butter.balanceOf(address(this)));
        return
            uint256(
                wadMul(_getPercentForPrice(int256(price), balance), balance)
            );
    }

    function canEmit() public view returns (bool) {
        return
            block.timestamp / EMISSION_INTERVAL >
            lastEmissionTimestamp / EMISSION_INTERVAL;
    }

    function _getPercentForPrice(int256 price, int256 balance)
        public
        view
        returns (int256 percent)
    {
        return _uncompound(_getCompoundedPercentForPrice(price, balance));
    }

    function _getCompoundedPercentForPrice(int256 price, int256 balance)
        public
        view
        returns (int256 logInvPercent)
    {
        return
            int256(
                getVRGDAAmountForPrice(
                    toDaysWadUnsafe(block.timestamp - START_TIME),
                    sold,
                    unsafeWadDiv(price, balance)
                )
            );
    }

    function _uncompound(int256 value) internal pure returns (int256) {
        return 1e18 - wadExp(-value);
    }

    function _compound(int256 value) internal pure returns (int256) {
        return -wadLn(1e18 - value);
    }

    function mint(address to) public payable {
        int256 balance = int256(butter.balanceOf(address(this)));
        int256 compPercent = _getCompoundedPercentForPrice(
            int256(msg.value),
            balance
        );
        sold += int104(compPercent);
        butter.transfer(to, uint256(wadMul(_uncompound(compPercent), balance)));

        _updateEmissionMA(msg.value);
        if (canEmit()) _emit();
    }

    function forceEmit() public {
        forceEmit(keyHash, subId);
    }

    function forceEmit(bytes32 keyHash_, uint64 subId_) public {
        require(canEmit(), "Cannot emit yet");
        _updateEmissionMA(0);
        _emit(keyHash_, subId_);
    }

    function _updateEmissionMA(uint256 amount) internal {
        uint256 timeSinceLastUpdate = block.timestamp -
            lastEmissionMAUpdateTimestamp;
        lastEmissionMAUpdateTimestamp = uint40(block.timestamp);
        timeSinceLastUpdate = Math.max(timeSinceLastUpdate, 1);

        emissionMA = uint104(
            updateEMA(
                emissionMA,
                (amount * EMISSION_MULTIPLIER * EMISSION_INTERVAL) /
                    timeSinceLastUpdate,
                (timeSinceLastUpdate * 1e18) / EMISSION_SMOOTHING_INTERVAL
            )
        );
    }
    ...
    function _emit(bytes32 keyHash_, uint64 subId_) internal {
        lastEmissionTimestamp = uint40(block.timestamp);
        _requestSeed(keyHash_, subId_);
    }

    function _requestSeed(bytes32 keyHash_, uint64 subId_) internal {
    ...
    function fulfillRandomWords(uint256, uint256[] memory randomWords)
        ...
        override
    {
        galaxy.addEmission(uint128(randomWords[0]), uint128(emissionMA));
    }
    ...
    */
    ...
    function setGalaxy(address galaxy_) external onlyOwner {
        galaxy = GenesisGalaxy(galaxy_);
    }

    function setButter(address butter_) external onlyOwner {
        butter = IERC20Resource(butter_);
    }
...
function updateEMA(
    ...
    weight = 1e18 - uint256(wadExp(-int256(weight)));
    return unsafeUpdateMA(curr, update, weight);
}

function unsafeUpdateMA(
    uint256 curr,
    uint256 update,
    uint256 weight
) pure returns (uint256) {
