...
contract GeneticAndGenome is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last timestamp number that YEL distribution occurs.
        uint256 allocPoint; // How many allocation points assigned to this pool. YEL to distribute per second.
    }
    ...
    uint256 immutable public startTime; // The timestamp when YEL farming starts.
    ...
    function changeEndTime(uint256 addSeconds) external onlyOwner {
        endTime = endTime.add(addSeconds);
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.stakingTokenTotalAmount != 0) {
            ...
            uint256 yelReward =
                multiplier.mul(yelPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accYelPerShare = yelReward.mul(1e12).div(pool.stakingTokenTotalAmount).add(accYelPerShare);
        }
        return user.amount.mul(accYelPerShare).div(1e12).sub(user.rewardDebt).add(user.remainingYelTokenReward);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        require(
            ...
            "Genetic & Genome: block timestamp is less than last reward time"
        );
        ...
        if (pool.stakingTokenTotalAmount != 0) {
            ...
            uint256 yelReward =
                multiplier.mul(yelPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accYelPerShare = 
                yelReward.mul(1e12).div(pool.stakingTokenTotalAmount).add(pool.accYelPerShare);
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accYelPerShare).div(1e12).sub(user.rewardDebt).add(user.remainingYelTokenReward);
        ...
        user.amount = _amount.add(user.amount);
        pool.stakingTokenTotalAmount = _amount.add(pool.stakingTokenTotalAmount);
        user.rewardDebt = user.amount.mul(pool.accYelPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require(
            user.amount >= _amount,
            "Genetic & Genome: you do not have enough tokens to complete this operation"
        );
        updatePool(_pid);
        uint256 pending =
            user.amount.mul(pool.accYelPerShare).div(1e12).sub(user.rewardDebt).add(user.remainingYelTokenReward);
        ...
        require(
            ...
            "Genetic & Genome: you do not have enough staking tokens to complete this operation"
        );
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal returns(uint256) {
        ...
        if (_amount > yelTokenBalance) {
            ...
            return _amount.sub(yelTokenBalance);
        }
