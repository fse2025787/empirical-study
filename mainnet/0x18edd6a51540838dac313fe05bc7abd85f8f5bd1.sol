...
contract Solluks is Ownable, ReentrancyGuard, ERC721A {
    ...
    uint256 constant TOTAL_TOKEN_SUPPLY = 2974;
    uint256 constant MINT_PRICE = 0.02 ether;
    
    mapping(address => uint256) private _num_minted;


    constructor() ERC721A("Solluks", "SLK") {
        
    }

    function mint(uint256 numTokens) payable external nonReentrant {
        require(_halt_mint == false, "solluks: halted");
        require(numTokens > 0, "solluks: >0");
        require(totalSupply() + numTokens < TOTAL_TOKEN_SUPPLY, "solluks: >supply");

        uint256 totalPrice = _computePrice(numTokens, msg.sender);
        require(msg.value == totalPrice, "solluks: incorrect price");
    ...
    function computePrice(uint256 numTokens) public view returns (uint256){
        return _computePrice(numTokens, msg.sender);
    }

    function _computePrice(uint256 numTokens, address addr) public view returns (uint256) {
