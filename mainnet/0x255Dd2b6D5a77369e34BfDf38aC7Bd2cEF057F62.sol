...
contract WalkingMfers is Auth, ERC721 {
  using Strings for uint256;

  
  uint256 public constant SUPPLY_MAX = 500;
  ...
  constructor(string memory newUnrevealedURI) ERC721("Walking Mfers", "Wmfers") {
  ...
  function mint(uint256 amount) external payable {
    ...
    if (msg.sender != owner) {
      ...
      if (supply <= 500) require(msg.value == amount * 0.042069 ether, "Invalid ether amount");
    }
