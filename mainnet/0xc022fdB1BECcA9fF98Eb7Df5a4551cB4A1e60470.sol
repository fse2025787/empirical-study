...
// File: contracts/NFTSPO.sol


pragma solidity ^0.8.16;

//@author BedoSensei
//@title Stoned Pop Originals - Saison 1





contract NFTSPO is Ownable, ERC721A {

    using Strings for uint;

    //URI of the NFTs when revealed or not Revealed
...
//SUPPLY 
    ...
    uint private constant MAX_PRESALE = 3000;
    uint private constant MAX_PUBLIC = 4800;
    ...
    mapping(address => uint) public amountNFTsperWalletPreSale;


    constructor(string memory _baseURI, string memory _notRevealedURI) ERC721A("Stoned Pop Originals - Season 1", "SPO")
    {
        baseURI = _baseURI;
   ...
   //PreSale
    function presaleMint(address _account, uint _quantity) external payable callerIsUser {
        ...
        require(sellingStep == Step.PreSale, "Pre-sale sale is not activated");
        require(amountNFTsperWalletPreSale[msg.sender] + _quantity <= 100, "You can only get 100 NFT on the Pre-sale");
        require(totalSupply() + _quantity <= MAX_PRESALE, "Max supply exceeded");
        require(msg.value >= price * _quantity, "Not enough funds");
        amountNFTsperWalletPreSale[msg.sender] += _quantity;
    ...
    function publicSaleMint(address _account, uint _quantity) external payable callerIsUser {
        ...
        require(totalSupply() + _quantity <= MAX_PRESALE + MAX_PUBLIC, "Max supply exceeded");
