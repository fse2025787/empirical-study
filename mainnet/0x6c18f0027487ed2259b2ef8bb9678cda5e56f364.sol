...
// File: contracts/StakeHolderSplit.sol

pragma solidity ^0.8.7;

// Create by 0xChrisX
// Contract Version : v.1 Banana Split - Split ETH and ERC20 to StakeHolder


contract StakeHolderSplit  {
    ...
    function transferERC20(IERC20 token) public {
        ...
        require(erc20balance > 0, "No Coin left");

        uint256 _paytoW1 = erc20balance*40/100 ; // K.Bass
        uint256 _paytoW2 = erc20balance*18/100 ; // K.Kayy
        uint256 _paytoW3 = erc20balance*18/100 ; // K.Chris
        uint256 _paytoW4 = erc20balance*18/100 ; // K.Yok
        uint256 _paytoW5 = erc20balance*6/100 ; // VAULT    

        token.transfer(wallet1, _paytoW1);
        token.transfer(wallet2, _paytoW2);
        token.transfer(wallet3, _paytoW3);
        token.transfer(wallet4, _paytoW4);
        token.transfer(wallet5, _paytoW5);
        
        emit TransferSent(address(this), wallet1, _paytoW1);
        emit TransferSent(address(this), wallet2, _paytoW2);
        emit TransferSent(address(this), wallet3, _paytoW3);
        emit TransferSent(address(this), wallet4, _paytoW4);
        emit TransferSent(address(this), wallet5, _paytoW5);
