...
contract MySuckingTest is ERC721A, Ownable {
    using Strings for uint256;
    using ECDSA for bytes32;

    // metadata
    string public baseURI = "https://suckit.suckers";
    ...
    uint256 public timestampWL = 1658683800;
    uint256 public timestampPublic = 1658687400; 

    uint256 public aksdhkfewpruoruqowdjasldfhlsdfjhznmxbcmnzxbczxmcz21983879456324341 = 897136281926389123; // TODO remove
    ...
    function mint(uint256 count, bytes calldata signature) external payable {
        ...
        require(block.timestamp >= timestampWL, "Sale not started");

        uint256 expectedPrice;

        if (block.timestamp < timestampPublic) {
            // WL sale
            ...
            expectedPrice = (count + _numberMinted(msg.sender) - PER_WALLET_FREE_IN_WL) * price;
        } else {
            // public sale
