...
// File: contracts/HypeSaints099.sol
...
contract HypeSaints099 is 
   ...
   function lottery(uint256 _round,uint256[] memory _tokenIds) external nonReentrant {
    ...
    function raffle(uint256 _round) external onlyOwner returns (uint256){
        ...
        require(!roundRaffleInfo[_round].raffled,"this round has rewarded");
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        uint256 _round = roundRequestId[requestId];

        require(!roundRaffleInfo[_round].raffled,"this round has rewarded");
    
        uint256 num = randomWords[0]%roundRaffleInfo[_round].raffleAddress.length();
        address winner = roundRaffleInfo[_round].raffleAddress.at(num);
        roundRaffleInfo[_round].roundWinner = winner;

        roundRaffleInfo[_round].raffled = true;
        emit Raffle(winner,_round,roundRaffleInfo[_round].roundBonus);
    }
    ...
    function initRaffle(uint256 _round,uint256[] memory _tokenIds,uint256 _bonus) external onlyOwner {
        ...
        require(raffleInfo.tokenIds.length() == 0,"this round has been inited");
