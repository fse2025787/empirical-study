...
interface IERC721Dispatcher {
  ...
  /**
   * @dev Returns true if `_tokenId` token is available from `_startTime` for `_durationInSecs` seconds.
   */
  function isAvailable(uint256 _startTime, uint256 _durationInSecs, uint256 _tokenId) external view returns (bool available);
  /**
...
abstract contract ERC721Dispatcher is IERC721Dispatcher, IERC721Receiver, ERC721Enumerable, IERC2981, Ownable, ReentrancyGuard {
  ...
  function _processRequest(address _payee, address _to, uint256 _tokenId, bytes memory _requestedTerms) internal returns (bool processed){
    ...
    if(DispatchLib.isPaymentOutstanding(_requestedTerms)){
      ...
      require(isAvailable(block.timestamp, DispatchLib.getDuration(_requestedTerms), _tokenId), "not avail");
  ...
  /**
   * @dev See {IERC721Dispatcher-isAvailable}.
   */
  function isAvailable(uint256 _startTime, uint256 _durationInSecs, uint256 _tokenId) public view virtual returns (bool available) {
  ...
  /**
   * @dev See {IERC721Dispatcher-getNextAvailable}.
   */
  function getNextAvailable(uint256 _tokenId) external view returns (uint256 availableStartingTime) {
...
contract RQDQ is ERC721Dispatcher {
   ...
   * @dev See {IERC721Dispatcher-isAvailable}.
   */
  function isAvailable(uint256 _startTime, uint256 _durationInSecs, uint256 _tokenId) public view virtual override returns (bool available) {
    (bool reserved,) = reservationBook.isReserved(_startTime, _startTime + _durationInSecs - 1, _tokenId);

    if(!reserved && _deposits[_tokenId].nextAvailable <= block.timestamp){
      return true;
    }
  }
}
