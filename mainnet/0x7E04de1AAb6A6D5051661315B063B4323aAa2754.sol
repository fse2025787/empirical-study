...
contract LOOKUSCOIN is
    ERC20Lockable,
    ERC20Burnable,
    ERC20Mintable,
    Freezable
{
    string constant private _name = "LOOKUSCOIN";
    string constant private _symbol = "LUC";
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "LOOKUSCOIN/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "LOOKUSCOIN/transferFrom : Should not send to zero address"
        );
        ...
        _approve(
            ...
            _allowances[from][msg.sender].sub(
                amount,
                "LOOKUSCOIN/transferFrom : Cannot send more than allowance"
            )
        );
        success = true;
    }
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "LOOKUSCOIN/approve : Should not approve zero address"
        );
