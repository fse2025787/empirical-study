...
contract MetaBunniesNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  //   presale price 0.03 ether
  //   public sale price 0.033 ether
  ...
  address[] public fundRecipients = [
    0x638b389C966e29BA91dA3ccEe8326d0Fa54C4B0c,
   0x3097617CbA85A26AdC214A1F87B680bE4b275cD0,
   0x7943C4027Df99dFBAd320784F2FA534d5865a6D5,
   0x181Cb650FB05f91986C89fD8c2c25A8b6f6dB864
  ];
  uint256[] public receivePercentagePt = [5200, 2250, 550, 2000];   //distribution in basis points
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount > 0, "You need to adopt at least 1 bunny");
    uint256 ownerMintedCount = addressesMintedBalance[msg.sender];
    require(_mintAmount <= maxMintAmount, "Max bunny adoptions per transaction exceeded");
    require(supply + _mintAmount <= maxSupply, "Max bunny mint limit exceeded");

    if (msg.sender != owner()) {
        if (onlyWhitelisted == true){
            require(isWhitelisted(msg.sender), "Your address is not whitelisted for this sale.");
            // uint256 ownerMintedCount = addressesMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "Max bunnies for your wallet for this sale would be exceeded");
        }
      require(msg.value >= cost * _mintAmount, "You will need more ETH to adopt your bunnies during this sale.");
    }
