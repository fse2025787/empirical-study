...
contract Reflect is Context, IERC20, Ownable {
    ...
    constructor(
        ...
        require(tTotal_ > 0, "tTotal is 0");
        require(feePercentage_ < 100, "fee >= 100%");
        ...
        _rOwned[teamAccount_] = _rTotal.div(tTotal_).mul(teamPreMinted_); // rTotal/(tTotal/teamPreMinted)
        _rOwned[address(this)] = _rTotal.sub(_rOwned[teamAccount_]);
    ...
    function tOwned(address account) external view returns (uint256) {
        return _tOwned[account];
    }

    function rOwned(address account) external view returns (uint256) {
        return _rOwned[account];
    }

    function rTotal() external view returns (uint256) {
        return _rTotal;
    }
    ...
    {
        uint256 tFee = tAmount.mul(_feePercentage).div(100);
