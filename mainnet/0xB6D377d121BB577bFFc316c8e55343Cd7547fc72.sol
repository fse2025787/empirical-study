...
interface IFragmentMiniGame {
    ...
    event FragmentGroupUnregistered(bytes16 groupId);

    
    event ObjectGroupUnregistered(bytes16 groupId);
    ...
    error ObjectGroupShouldNotExist(bytes16 groupId);

    
    error ObjectGroupShouldExist(bytes16 groupId);

    
    error FragmentGroupShouldNotExist(bytes16 groupId);

    
    error FragmentGroupShouldExist(bytes16 groupId);

    
    error SecretHashDoesNotMatchGroup();
    ...
    function unregisterFragmentGroup(string calldata groupName, bytes32 secretHash) external;

    
    
    function unregisterObjectGroup(string calldata groupName) external;
    ...
    function deconstructTokenId(uint256 tokenId) external pure returns (bytes16 groupId, uint128 tokenIndex);
}
...
library FragmentMiniGameUtils {
    function constructGroupId(string memory group) internal pure returns (bytes16) {
        return bytes16(keccak256(bytes(group)));
    }

    function constructFragmentTokenId(bytes16 groupId, uint128 tokenIndex) internal pure returns (uint256 tokenId) {
        return _constructTokenId(groupId, tokenIndex);
    }

    
    function constructObjectTokenId(bytes16 groupId) internal pure returns (uint256 tokenId) {
        return _constructTokenId(groupId, 0);
    }

    function deconstructTokenId(uint256 tokenId) internal pure returns (bytes16 groupId, uint128 tokenIndex) {
    ...
    function constructFragmentSupply(uint128 supplyLeft, uint128 totalSupply) internal pure returns (bytes32) {
...
library Features {
    uint8 internal constant _CONFIGURING = 0;
    uint8 internal constant _MINTING = 1;
    uint8 internal constant _REVEALING = 2;
    uint8 internal constant _WHITELIST = 3;
}
...
contract ZeeMasks is ERC721, ERC721Enumerable, Ownable, Whitelist {
    ...
    constructor() ERC721("Zee Masks", "ZMK") {
    }
