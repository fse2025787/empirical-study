...
abstract contract LiquidityPool is ILiquidityPool, Authorization, Pausable, Initializable {
    ...
    uint256 public reserveDeviation;
    uint256 public requiredReserves;
    uint256 public maxWithdrawalFee;
    uint256 public minWithdrawalFee;
    uint256 public withdrawalFeeDecreasePeriod;
    ...
    function _initialize(address vault_, InitializationArgs memory args) internal initializer {
        _checkFeeInvariants(args.minWithdrawalFee, args.maxWithdrawalFee);
        require(args.reserveDeviation <= ScaledMath.ONE / 2, Error.INVALID_AMOUNT);
        require(args.requiredReserves <= ScaledMath.ONE, Error.INVALID_AMOUNT);

        vault = IVault(vault_);
        name = args.name;
        maxWithdrawalFee = args.maxWithdrawalFee;
        minWithdrawalFee = args.minWithdrawalFee;
        requiredReserves = args.requiredReserves;
        reserveDeviation = args.reserveDeviation;
    }
