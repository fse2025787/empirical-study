...
abstract contract PackWoodERC721StorageV2 {
    ...
    // checks whether monsterbud is already processed or not
    ...
    PackWoodERC721StorageV2
{
    ...
    function checkTokenId(uint256 tokenId) external view returns (bool) {
    ...
    /**
     * @dev updates processed token Ids.
     *
    ...
    function updateProcessedTokens(
        uint256[] calldata _tokenIds,
        bool _status
    ) external virtual onlyOwner {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            isProcessed[_tokenIds[i]] = _status;
        }
    }
        ...
        returns (uint256)
    {
        ...
        require(
            !isProcessed[_monsterTokenId],
            "PackwoodERC721: Token can not be processed"
        );
