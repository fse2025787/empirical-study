...
// File @openzeppelin/contracts/token/ERC20/ERC2[email protected]
...
// File contracts/aHades.sol

pragma solidity 0.7.5;



contract aHades is ERC20PresetMinterPauser {
  using SafeMath for uint256;
  ...
  constructor(
    ...
    uint256 _startTime,
    uint256 _endTime
  ) ERC20PresetMinterPauser("alphaHADES", "aHADES") {
    require(_OHM != address(0));
    require(_treasury != address(0));
    require(_endTime > _startTime, "endTime must come after startTime");
  ...
  /**
   * @dev Transfer _amount OHM into the contract and mint 0.8 * _amount aHADES for the sender.
   *
   * Will error if called after endTime or before startTime.
   * */
  function buy(uint256 _amount) external {
    require(
      block.timestamp >= startTime,
      "Current block time is earlier than startTime"
    );
    require(block.timestamp < endTime, "Current block is later than endTime");
    require(ERC20(OHM).decimals() == 9, "OHM decimal is not 9");
    IERC20(OHM).transferFrom(msg.sender, address(this), _amount);
    // Mint 0.8 * _amount for sender.
    _mint(msg.sender, _amount.mul(8).div(10));
  }

  /**
   * @dev Transfer all of the OHM owned by the contract to the treasury.
   *
   * Will error if called before endTime.
   * */
  function sendOHMToTreasury() external {
    require(
      block.timestamp >= endTime,
      "Can't send funds to treasury before endTime"
    );
    IERC20(OHM).approve(treasury, IERC20(OHM).balanceOf(address(this)));
    Treasury(treasury).deposit(IERC20(OHM).balanceOf(address(this)), OHM, 0, true);
  }
}