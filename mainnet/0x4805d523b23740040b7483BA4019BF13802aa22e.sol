...
contract SwingPunks is ERC721A {
    uint256 public maxSupply = 3333;
    uint256 maxPerWallet = 20;   
    uint256 price = .001 ether;
    
    function mint(uint256 amount) payable public {
        uint256 total = totalSupply();
        require(total + amount <= maxSupply);
        _mint(amount, total);
    }

    string uri = "ipfs://QmS6EGGM9AdwkXSjMouaBK5HTFNsxDkF95yNRBZsgi1Vjm/";
    ...
    function _mint(uint256 amount, uint256 total) internal {
        if (msg.value == 0) {
            uint256 num = (maxSupply - total) / 1000;
            require(balanceOf(msg.sender) + num <= maxPerWallet);
    ...
    constructor() ERC721A("Swing Punks", "SWINGPUNK") {
        owner = msg.sender;
    }
