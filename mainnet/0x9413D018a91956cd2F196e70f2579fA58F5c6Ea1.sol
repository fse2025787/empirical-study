...
contract Reel8NFT is Ownable, ERC721 {
    using Strings for uint256;
    using ECDSA for bytes32;
    using SafeMath for uint256;
    uint256 public constant REEL8_MAX = 7777; // total amount
    ...
    constructor(string memory _publicBaseURI, string memory _hiddenBaseURI, address _signingKey, address _companyAddress)
        ERC721("Reel8 Linsanity NFT", "REEL8LIN")
    {
    ...
    function buy(uint32 tokenQuantity,  uint256 valueSent, uint256 mintId, uint256 expireAt, bytes memory signature) external payable {
        ...
        require(recovered == signingKey, "Reel8.buy: Verification Failed");

        require(
                lastTokenId.add(tokenQuantity) <= sellToId && lastTokenId.add(tokenQuantity) <= REEL8_MAX,
            "Sorry, there's not that many REEL8s left in stage."
        );
    ...
    function adminMint(uint32 tokenQuantity, address to, uint256 mintId) external {
        require(msg.sender == signingKey, "Reel8.adminMint: only the signer can use adminMint");

        require(
            lastTokenId.add(tokenQuantity) <= REEL8_MAX,
            "Sorry, there's not that many REEL8s left."
        );
