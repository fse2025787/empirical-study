...
/*                                                                                
              ...
              &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&             
                    [email protected]&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&                    
                        *@&&&&&&&&&&&&&&&&&&&&&&&&&&&&&@.                       
                           %@@@@@@@@@@@@@@@@@@@@@@@@@@                          
                        Ma kaynch behal tajine a3chiri lbenna ya salaaaam
                                    Israe <3
...
contract CryptoTajines is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, EIP712 {
    using Counters for Counters.Counter;

    string public tajine = unicode"Δ";
    ///delta i know but it's the only tajine symbol i found :D
    uint256 public lmaxA3chiri = 10000;//10k baby

    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("CryptoTajines", tajine) EIP712("CryptoTajines", "1") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);///ana :D
    ...
    function safeMint(address to, string memory uri) public onlyRole(MINTER_ROLE) {
        ...
        require( tokenId < lmaxA3chiri, "We done, all Tajines have been sold!");
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);///tototo wa lmarib tototo wa lmarib
    }
