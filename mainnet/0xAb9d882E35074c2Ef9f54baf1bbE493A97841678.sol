...
contract WizForest is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant rewardRate = 5*(10**18); // 3 minutes
    uint256 public constant rewardCap = 259200; // 3 days
    ...
    function calculateRewardQuantity(StakeWZRD memory stakedWZRD) internal view returns (uint256) {
        uint256 duration = (block.timestamp - stakedWZRD.start)/180;
        if (rewardCap < duration * rewardRate) {
            return rewardCap;
        }
    ...
    function lockWZRD(uint256 tokenId) public onlyAltars {
        StakeWZRD storage stakedWZRD = stakedWZRDs[tokenId];
        stakedWZRD.locked = true;
    }

    function unlockWZRD(uint256 tokenId) public onlyAltars {
        StakeWZRD storage stakedWZRD = stakedWZRDs[tokenId];
