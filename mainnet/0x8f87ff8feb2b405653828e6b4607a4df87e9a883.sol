...
// File: contracts/DegeneratesClub.sol
...
contract DegeneratesClub is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
  ...
  ) ERC721A("Degenerates Club", "Degens", maxBatchSize_, collectionSize_) {
    require(amountForAuctionAndDev_ <= collectionSize_, "larger collection size needed" );
  }
    
    uint256 pricePer = 0.04 ether;
    uint256 totalCost;
    uint256 maxSupply = 5000;
    uint256 saleIsActive = 0;

    function flipSaleState(uint256 num) public onlyOwner {
        saleIsActive = num;
    }
    function setMintPrice(uint256 price) public onlyOwner {
        pricePer = price;
    }
    function maxSupplyChange(uint256 newMax) public onlyOwner {
        maxSupply = newMax;
    }
     ...
     *
     * 
     * - Degenerates Club members cost 0.04 per mint.
     * - 10 can be minted per transaction. Gas fees are real small, if you want more just send another transaction.
  ...
  function mintMarketing(address toPerson, uint256 quantity) external onlyOwner {
    require(saleIsActive == 1, 'Sale is not active');
    require(quantity <= 10, 'Cant mint more than 10');
    require(totalSupply() + quantity <= maxSupply);
    _safeMint(toPerson, quantity);
  }
