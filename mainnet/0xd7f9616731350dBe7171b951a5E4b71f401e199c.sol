...
contract Nft20Market {
    ...
    function buyAssetsForErc20(
        ...
        // send NFTs to the recipient
            ...
            (success, ) = _fromERC20.call(
                abi.encodeWithSelector(0x456a09c8, _ids, _amounts, recipient)
            );
    ...
    function swapErc721(
        ...
        uint256 length = fromTokenId.length;
        for (uint256 i; i < length; ) {
            INFT20Pair(fromERC20).swap721(
                fromTokenId[i],
                toTokenId[i],
                recipient
            );
