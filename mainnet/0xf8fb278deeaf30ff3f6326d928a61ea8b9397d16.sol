...
contract Strategy is ConvexStable {
    address public constant pool = address(0x43b4FdFD4Ff969587185cDB6f0BD875c5Fc83f8c);
    ...
    constructor(address _vault) public ConvexStable(_vault) {
        ...
        _setPathTarget(2, 0); // alcx path target

        dex = new address[](3);
        dex[0] = sushiswap; // crv
        dex[1] = sushiswap; // cvx
        dex[2] = sushiswap; // alcx
        _approveDex();
    }
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(alcx).approve(dex[2], 0);
        IERC20(alcx).approve(dex[2], type(uint256).max);
    }

    // >>> include other rewards
    function _migrateRewards(address _newStrategy) internal override {
        super._migrateRewards(_newStrategy);
        IERC20(alcx).safeTransfer(_newStrategy, IERC20(alcx).balanceOf(address(this)));
    }
    ...
    function _claimableInETH() internal override view returns (uint256 _claimable) {
        ...
        if (_alcx > 0) {
            ...
            path[0] = alcx;
            path[1] = weth;
            uint256[] memory swap = Uni(dex[2]).getAmountsOut(_alcx, path);
        ...
        )
    {
        ...
        if (_alcx > 0) {
            ...
            Uni(dex[2]).swapExactTokensForTokens(_alcx, uint256(0), path, address(this), now);
        }
