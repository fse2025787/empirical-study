...
contract ChillinChameleonDFS is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress =
        payable(0xfc5c2EBBFED62b01aCAdcCAdE2216030E0A2052e);
    address payable private _DevAddress;

    uint256 public constant CCDFS_MAX = 10000;
    uint256 public PURCHASE_LIMIT = 25;
    uint256 public PRICE = 50_000_000_000_000_000; // 0.05 ETH

    uint256 private _activeDateTime = 1633813200; // October 9, 2021 9:00:00 PM GMT
    ...
    Counters.Counter private _publicCCDFS;

    constructor() ERC721("Chillin Chameleon DFS", "CCDFS") {
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCCDFS.current();

            if (_publicCCDFS.current() < CCDFS_MAX) {
                _publicCCDFS.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicCCDFS.current() < CCDFS_MAX,
            "Purchase would exceed CCDFS_MAX"
        );
