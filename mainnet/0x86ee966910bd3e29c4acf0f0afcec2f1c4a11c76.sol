...
contract NftStaking is ReentrancyGuard {
    ...
    struct Staker {
        ...
        uint256 rewards;
        // Income array index for which rewards were calculated
        uint256 incomeIndex;
    ...
    /**
      Events
    */
    event Staked(address indexed user, uint256[] _tokenIds);
    event Unstaked(address indexed user, uint256[] _tokenIds);
    ...
    constructor(IERC721 _nftCollection, IERC20 _rewardsToken) {
        require(address(_nftCollection) != address(0), "Nft collection address can't be set to 0");
        require(address(_rewardsToken) != address(0), "Rewards token address can't be set to 0");
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        if (stakers[msg.sender].amountStaked > 0) {
            ...
            //stakers[msg.sender].incomeIndex = index;
        }
    ...
    // calculate the rewards and store them in the rewards and for each
    ...
    // claim, add to paid sent amount to the user.
    function claimRewards() external {
        ...
        require(rewardsToken.transfer(msg.sender, availableRewardsAmt),'Transfer failed');
    }

    function addIncome(uint256 _amount) external {
      require(_amount > 0, 'Amount should be greater than 0');
      require(rewardsToken.transferFrom(msg.sender, address(this), _amount), 'Transfer of funds failed');
      incomes.push(Income(_amount,_amount, totalStaked));
    }
    ...
    function availableRewards(address _user) internal view returns (uint256) {
        ...
        return stakers[_user].rewards + rewards - stakers[_user].paid;
    }
    ...
    function calculateRewards(address _staker)
        ...
        returns (uint256)
    {
        uint256 notAppliedRewardsAmountSum;
        if(incomes.length > 0 && stakers[_staker].incomeIndex < incomes.length) {
            for(uint i = stakers[_staker].incomeIndex; i < incomes.length; i++) {
                notAppliedRewardsAmountSum += stakers[_staker].amountStaked * incomes[i].amount / incomes[i].totalStaked;
            }
        }
        return notAppliedRewardsAmountSum;
    }
