...
contract BridgeAdmin is Container {
    ...
    // keccak256("selector")
    bytes32 internal constant SELECTORHASH = 0x4c11fe2a708d5242b13f178422d4088cb270c488d9932765064ea92953422272;
    ...
    function initAdmin(address[] memory _owners, uint _ownerRequired) internal validRequirement(_owners.length, _ownerRequired) {
        ...
        addItemAddress(SELECTORHASH,_owners[0]);
        ...
        classHashArray.push(SELECTORHASH);
...
contract Bridge is BridgeAdmin, Pausable {
    ...
    modifier onlySelectorSetter() {
        require(itemAddressExists(SELECTORHASH, msg.sender), "only selector setter allowed");
        _;
    }
    ...
    returns(bool)
    {
        ...
        require(!logic.taskIsFinshed(taskHash), "taskHash is finished");
        ...
        if (status == logic.TASKPROCESSING()){
            ...
            logic.markTaskAsFinished(taskHash);
    ...
    function setDepositSelector(address token, string memory method, bool _isValueFirst) onlySelectorSetter external{
        depositSelector[token] = assetSelector(method,_isValueFirst);
    }

    function setWithdrawSelector(address token, string memory method, bool _isValueFirst) onlySelectorSetter external{
