 ...
 */
contract ClaimableFortuneTeller is
    ERC1155Upgradeable,
    AccessControlUpgradeable
{
    ...
    function create(string calldata uriHash) external onlyRole(ADMIN) {
        setTokenURIHash(_idCounter.current(), uriHash);
    ...
    /**
     * @notice Sets a token metadata URI hash.
     * @param tokenURIHash The hash to the updated token metadata.
     */
    function setTokenURIHash(uint256 tokenId, string memory tokenURIHash)
        public
        onlyRole(ADMIN)
    {
        _idToUriHash[tokenId] = tokenURIHash;
    }
     ...
     */
    function initialize(address _minter, string calldata contractURIHash)
        public
        virtual
        initializer
    {
