...
// File: contracts/inBetweenersMysteryBox.sol
...
contract MysteryBox is ERC721A, ERC2981, ReentrancyGuard, AccessControl, Ownable {
    bytes32 public constant SUPPORT_ROLE = keccak256('SUPPORT');

    IERC721Enumerable public immutable inBetweeners;
    ...
    constructor(address inBetweeners_) ERC721A("MysteryBox", "IBMB") {
        require(
            IERC721Enumerable(inBetweeners_).supportsInterface(0x780e9d63),
            "TEST address does not support ERC721Enumerable"
        );

        inBetweeners = IERC721Enumerable(inBetweeners_);
    ...
    function claim() external nonReentrant {
        ...
        for (uint256 i = 0; i < inBetweeners.balanceOf(msg.sender); i++) {
            uint256 tokenId = inBetweeners.tokenOfOwnerByIndex(msg.sender, i);
