...
contract IndicesPayment is Context, ReentrancyGuard, Initializable {
    ...
    function initialize(address _ASTRTokenAddress, address _poolConf,address _exchangeContract, address _treasury) public initializer{
        ...
        treasury = _treasury;
        astraAmount = 500 * uint256(10)**uint256(18);
	}
    ...
    */
    ...
    function swapAndTransfer(address _token, uint _value) internal returns(uint) {
    	...
    	(_amount, _distribution) = DexAggregator(exchangeContract).getBestExchangeRate(IERC20(_token), IERC20(ASTRTokenAddress), _value);
		// Approve before selling the tokens
        if (_token == ETH_ADDRESS) {
            minReturn = calculateMinimumReturn(_amount);
			_amount = DexAggregator(exchangeContract).swapFromBestExchange.value(_value)(IERC20(_token), IERC20(ASTRTokenAddress), _value, minReturn, _distribution);
		} else {
            IERC20(_token).approve(exchangeContract, _value);
		    minReturn = calculateMinimumReturn(_amount);
		    _amount = DexAggregator(exchangeContract).swapFromBestExchange(IERC20(_token), IERC20(ASTRTokenAddress), _value, minReturn, _distribution);
		}
        require(_amount>=astraAmount,"Not enough amount");
        IERC20(ASTRTokenAddress).transfer(treasury, _amount);
		return _amount;
	}
    ...
    function deposit(address _tokenAddress,uint256 _amount) external payable{
        ...
        if(_tokenAddress == ETH_ADDRESS){
            ...
            require(_amount>=astraAmount,"Not enough amount");
            require(IERC20(ASTRTokenAddress).balanceOf(msg.sender) >= _amount, "Not enough balance");
            returnedAmount = _amount;
            IERC20(ASTRTokenAddress).transferFrom(msg.sender,treasury, _amount);
        }
        else{
