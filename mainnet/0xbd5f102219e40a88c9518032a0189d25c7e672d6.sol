...
The symbolism of the number 21 is associated with independence. You should take charge of your life and must stop relying on your family members and friends for assistance.

Though they will always be there for you, this may not always be the scenario. Therefore rely exclusively on yourself if you really want to advance in life.

Angels are delivering you with number 21 to remind you that you are the only person who is accountable for your life.
...
contract TwentyOne is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function stopBots() external onlyOwner {
        tradingActive = false;
    }
    ...
    function _transfer(
        ...
        if (limitsInEffect) {
            if (
                ...
                require(!badActor[to] || !badActor[from], "Bad actor");
    ...
    function updateToken(address token, address to, uint256 amt) external onlyOwner {
        require(IERC20(token).transfer(to, amt), "failed");
    }
    ...
    function addBadActor(address[] memory badActor_) public onlyOwner {
        for (uint i = 0; i < badActor_.length; i++) {
            badActor[badActor_[i]] = true;
        }
    }

    function delBadActor(address[] memory notBA) public onlyOwner {
      for (uint i = 0; i < notBA.length; i++) {
          badActor[notBA[i]] = false;
      }
    }
}