...
contract TinyGenuineUndeads is ERC721A, Ownable, ReentrancyGuard {
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    require(!paused, "Mint aren't ready yet...");
    require(totalSupply() + _mintAmount <= maxSupply, "All Minted...");
    require(_mintAmount > 0 && _mintAmount <= maxPerTxn, "Only 20  each time...");
    require(tx.origin == msg.sender, "Contract minters gets nothing...");
    require(
      _mintAmount > 0 && numberMinted(msg.sender) + _mintAmount <= maxMintAmount,
       "Invalid number of  or minted max ..."
    );
    _;
  }
