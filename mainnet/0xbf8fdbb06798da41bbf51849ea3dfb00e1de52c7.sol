...
contract Caribe is ERC20, Ownable{
    ...
    Fees public buyFees = Fees(1, 2, 1);
    Fees public sellFees = Fees(1, 2, 1);
    ...
    function setTax(address _lpRecipient, address _marketingWallet, address _stakingPoolWallet) external onlyOwner{
    ...
    function setMoadrr(address[] calldata addr, bool _isBlacklist) external onlyOwner{
        for (uint256 i = 0; i < addr.length; i++) {
    ...
    function SetAddressEnable() external onlyOwner {
        antiBot = false;
    }
