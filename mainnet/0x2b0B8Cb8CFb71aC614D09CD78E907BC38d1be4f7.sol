...
contract TikiCarving is OwnableUpgradeable {
    ...
    function setup(address tikiToken, address signer) onlyOwner public {
        tikiAddress = tikiToken;
        txnSigner = signer;
    }
    ...
    function upgrade(uint256[] memory upgradeIDs, uint256[] memory sacrificeIDs, uint256 level, uint256 method, uint256 cost, bytes32 nonce, bytes memory signature) external payable {
        ...
        for (uint i; i < upgradeIDs.length; i++) {
            if ( ! ownsToken(operator, upgradeIDs[i])) revert Errors.UnownedToken(upgradeIDs[i]);
            emit TokenUpgraded(upgradeIDs[ i ], level, nonce);
        }

        for (uint i = 0; i < sacrificeIDs.length; i++) {
            if ( ! ownsToken(operator, sacrificeIDs[i])) revert Errors.UnownedToken(sacrificeIDs[i]);
    ...
    function verifySignature(bytes memory signature, bytes32 digestHash) public view returns(bool) {
        return txnSigner == getSigner(signature, digestHash);
    }
    ...
    function ownsToken(address owner, uint256 tokenID) public view returns(bool) {
        return owner == ITiki(tikiAddress).ownerOf(tokenID);
    }
}
