...
contract CozyInvestRariUSDC is ICozyInvest2, ICozyDivest2 {
  ...
  function divest(
    ...
    // Pay back as much of the borrow as possible, excess USDC is refunded to `recipient`
    uint256 _borrowBalance = cozyUsdcPm.borrowBalanceCurrent(address(this));
    uint256 _initialUsdcBalance = usdc.balanceOf(address(this));
    if (_initialUsdcBalance < _borrowBalance && _excessTokens > 0) {
      usdc.safeTransferFrom(msg.sender, address(this), _excessTokens);
    }
    uint256 _usdcBalance = _initialUsdcBalance + _excessTokens; // this contract's current USDC balance
    uint256 _repayAmount = _usdcBalance >= _borrowBalance ? type(uint256).max : _usdcBalance;

    // Approval if required
    if (usdc.allowance(address(this), address(cozyUsdcPm)) < _repayAmount) {
        usdc.approve(address(cozyUsdcPm), _repayAmount);
    }
    require(cozyUsdcPm.repayBorrow(_repayAmount) == 0, "Repay failed");
