...
abstract contract PermitControl is Ownable {
  ...
  */
    modifier hasValidPermit(bytes32 _circumstance, bytes32 _right) {
        require(
            _msgSender() == owner() || hasRightUntil(_msgSender(), _circumstance, _right) > block.timestamp,
  ...
  */
    function setPermit(
        ...
        require(_right != ZERO_RIGHT, "PermitControl: you may not grant the zero right");
  ...
  */
    ...
    {
        require(_managedRight != ZERO_RIGHT, "PermitControl: you may not specify a manager for the zero right");
...
contract Sweepable is PermitControl {
  ...
  */
    function sweep(
        ...
        uint256 _amount,
        address _address
    ) external hasValidPermit(UNIVERSAL, SWEEP) {
        require(!sweepLocked, "MintShop1155: the sweep function is locked");
...
contract Random is Named, Sweepable, VRFConsumerBase {
  ...
  */
    ...
    mapping(address => mapping(uint256 => ChainlinkResponse)) public callerRequests;
  ...
  */
    event ChainlinkUpdated(address indexed updater, Chainlink indexed oldChainlink, Chainlink indexed newChainlink);
  ...
  */
    event RequestCreated(address indexed requester, bytes32 indexed id, bytes32 indexed chainlinkRequestId);
  ...
  */
    event RequestFulfilled(bytes32 indexed chainlinkRequestId, uint256 indexed result);
  ...
  */
    ...
    ) public Named(_name) VRFConsumerBase(_chainlink.coordinator, _chainlink.link) {
    ...
    Return a version number for this contract's interface.
  */
    function version() external pure virtual override(Named, Sweepable) returns (uint256) {
        return 1;
    }
    ...
    @param _chainlink The new Chainlink data to use for connecting to a VRF
      coordinator.
  */
    function setChainlink(Chainlink calldata _chainlink) external hasValidPermit(UNIVERSAL, SET_CHAINLINK) {
  ...
  */
    function random(bytes32 _id) external returns (bytes32) {
        require(
            chainlinkResponses[_id].requester == address(0),
        ...
        require(
            link.balanceOf(_msgSender()) >= chainlink.fee,
        ...
        bytes32 chainlinkRequestId = requestRandomness(chainlink.keyHash, chainlink.fee);
  ...
  */
    function asRange(
        ...
        require(_bound > _origin, "Random: there must be at least one possible value in range");
        require(
            chainlinkResponses[_source].requester != address(0) && !chainlinkResponses[_source].pending,
            "Random: you may only interpret the results of a fulfilled request"
        );

        // Return the interpreted result.
        return chainlinkResponses[_source].result.mod(_bound.sub(_origin)).add(_origin);
    }
}
