...
contract StakeWake is Owned {
     ...
     * transfers WAKE from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(wake).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough WAKE to pay registration fee.");
        //makes sure smart contract transfers WAKE from user
        require(IERC20(wake).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked WAKE amount in the pool
    ...
    function calculateEarnings(address _stakeholder) public view returns(uint) {
        //records the number of minutes between the last payout time and now
        uint activeMinutes = (now.sub(lastClock[_stakeholder])).div(60);
        //returns earnings based on perMinute ROI and active minutes
        return ((stakes[_stakeholder]).mul(perMinuteROI).mul(activeMinutes)).div(100000);
    }
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(wake).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(wake).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient WAKE balance in pool');
        ...
        IERC20(wake).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() public view returns(uint) {
        return (IERC20(wake).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function setWake(address _newWake) external onlyOwner() {
        wake = _newWake;
    }
    ...
    //sets the perMinute ROI
    function setPerMinuteROI(uint _perMinuteROI) external onlyOwner() {
        perMinuteROI = _perMinuteROI;
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(wake).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient WAKE balance in pool');
        //transfers _amount to _address
        IERC20(wake).transfer(msg.sender, _amount);
        //emit event
