...
contract Bananas is ERC20PresetMinterPauser {
    using SafeMath for uint256;

    INFTContract public nftContract;

    uint256 public constant INITIAL_REWARD = 50 ether;
    uint256 public constant REWARD_RATE = 5 ether;
    uint256 public constant SECONDARY_REWARD_RATE = 2 ether;
    // Sunday August 22 2032 00:00:00 GMT-0400 (Eastern Daylight Time)
    uint256 public constant REWARD_END = 1976760000000;
    ...
    event BananasClaimed(address indexed account, uint256 reward);
    event BananasSpent(address indexed account, uint256 amount);

    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");

    constructor(address _nftContract)
        ERC20PresetMinterPauser("Bananas", "NANS")
    {
    ...
    function getReward(address _to) external {
        ...
        if (reward > 0) {
            rewards[_to] = 0;
            _mint(_to, reward);
            emit BananasClaimed(_to, reward);
        }
    }
    ...
    function spend(address _from, uint256 _amount) external {
        ...
        emit BananasSpent(_from, _amount);
    }
}