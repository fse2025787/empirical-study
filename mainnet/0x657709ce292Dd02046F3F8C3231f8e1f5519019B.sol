...
contract EXP is IEXP, ERC20, Ownable {
  ...
  function transferFrom(
      ...
      address recipient,
      uint256 amount
  ) public virtual override(ERC20, IEXP) disallowIfStateIsChanging returns (bool) {
  
    require(admins[_msgSender()] || lastWrite[sender] < block.number , "need approval before transfer");
    ...
    // For instance, someone can't just make a contract and siphon $EXP from every account
  ...
  function balanceOf(address account) public view virtual override disallowIfStateIsChanging returns (uint256) {
    ...
    require(admins[_msgSender()] || lastWrite[account] < block.number, "viewlock");
