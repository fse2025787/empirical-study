...
// File contracts/strategies/MeroBiCvx.sol

/**
 * This is the MeroBiCvx strategy, which is designed to be used by a Mero ERC20 Vault.
 ...
 */
contract MeroBiCvx is ConvexStrategyBase {
    using ScaledMath for uint256;
    using UncheckedMath for uint256;
    using SafeERC20 for IERC20;
        ...
        )
    {
        ...
        address underlying_ = ICurveSwapEth(curvePool_).coins(curveIndex_);
        IERC20(underlying_).safeApprove(curvePool_, type(uint256).max);
    }
    ...
    function _deposit() internal override returns (bool) {
        ...
        curvePool.add_liquidity(amounts, _minLpAccepted(underlyingBalance));
