...
abstract contract ERC721LimitedTransfer is ERC721 {
    ...
    {
        uint size;
        require(msg.sender == tx.origin, "Permitted only sending to non-contract addresses");
...
contract MEXNFT is ERC721URIStorage, ERC721Enumerable, ERC721LimitedTransfer, Ownable {
    constructor() Ownable() ERC721("MostExpensive :: Daniil Medvedev", "DMNFT") {}

    function mint(address _owner, uint256 tokenId, string memory _tokenURI, AccessType _accessType)
        public onlyOwner
    {
        require(!_exists(tokenId),
                "Token should not be already exists");
    ...
    function _burn(uint256 tokenId)
        internal override(ERC721, ERC721LimitedTransfer, ERC721URIStorage)
    {
        return super._burn(tokenId);
    }

    function _beforeTokenTransfer(address _from, address _to, uint256 _tokenId)
        internal virtual override (ERC721, ERC721Enumerable, ERC721LimitedTransfer) 
    {
        super._beforeTokenTransfer(_from, _to, _tokenId);
    }
