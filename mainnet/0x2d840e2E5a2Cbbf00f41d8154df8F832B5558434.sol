...
contract PASS is ERC721A {
    ...
    event setAdmin(address _newAdminAddress);
    ...
    function adminMint(uint256 tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= maxSupply, "Exceed Max");
        require(tokenQuantity <= 50, "Exceed Max Mint");
        _mint(msg.sender, tokenQuantity);
    }
    ...
    //admin
    function setAdminInfo(address _addr) external onlyOwner {
        _owner = _addr;
        emit setAdmin(_addr);
    }
