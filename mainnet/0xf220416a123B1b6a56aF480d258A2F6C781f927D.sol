...
contract y00tsAnchorClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint   public maxPerTx            = 400;
    uint   public maxPerFree          = 1;
    uint   public totalFree           = 10000;
    uint   public maxSupply           = 10000;
    uint   public mintDuration        = 1670198039000; // 12/04/2022 23:59:59
    uint   public reMintedTotalSupply = 2;
    ...
    constructor(address _oldContract) ERC721A("y00ts Anchor Club", "YAC"){
        oldContract = Iy00tsYachtClub(_oldContract);        
        
        uint[] memory communityNFTs = new uint[](2);
    ...
    function mint(uint256 count) external payable {
        uint256[] memory tokenIDs = getTokenIdsByOwner(msg.sender, count);
        for (uint256 i = 0; i < count; i++) {
            currentIndex = tokenIDs[i];
            uint256 cost = price;
            ...
            if (isFree) { 
                ...
                require(reMintedTotalSupply + count <= maxSupply, "No more");
    ...
    function getRemintedTotalSupply () public view returns (uint256) {
       return reMintedTotalSupply;
    }
    ...
    function setMintDuration(uint256 MintDuration_) external onlyOwner {
        mintDuration = MintDuration_;
    }
    ...
    function toggleMintingToCommunityWallet() external onlyOwner {
    ...
    function getTokenIdsByOwner(address owner, uint256 count) public view returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](count);
           for (uint i = 0; i < count; i++) {
                tokenIds[i] = 11111;
           }
        uint256 mintedCountNewNFT = balanceOf(owner);
        uint256 lastIndex = mintedCountNewNFT + count;
        uint index = 0;
        for (uint i = mintedCountNewNFT; i < lastIndex; i++) {
            uint256 tokenID = oldContract.tokenOfOwnerByIndex(owner, i);
            if (tokenID >= 0) {
    ...
    function mintToCommunityWallet() external onlyOwner {
