...
/*
    // DankChain 
    // $Dank / Dankest Blockchain of All Time, Much Dank.
    // https://t.me/dankchain // t.me/dankchain
    // dankchain.org / Made with Love from Cracked
*/
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xf94AaE1f58cC008d50b0Ee001100F74e35aD5172;
...
contract DankChain is Context, IERC20, Ownable { 
    ...
    */
    ...
    address payable private Wallet_Operations = payable(0xf94AaE1f58cC008d50b0Ee001100F74e35aD5172);
    address payable private Wallet_Development = payable(0x5A1b38D6B27E3b8aa5e9458Ba18675BEFbABBdc5);
    ...
    */
    ...
    string private _name = "Dank Chain"; 
    string private _symbol = "Dank";  
    uint8 private _decimals = 18;
    uint256 private _tTotal = 1000000000 * 10**18;
    uint256 private teamDonationMultiplier = 7500;
    uint256 private shareBasisDivisor = 10000;
    ...
    uint256 private maxPossibleFee = 10; 
    ...
    */
    ...
    constructor () {
        ...
        _isExcludedFromFee[Wallet_Operations] = true;
    ...
    */
    ...
    // Enable or disable DevOps
    function toggleDevOps(bool _devOps) public onlyOwner {
        devOps = _devOps;
    }
    ...
    */
    ...
    function Wallet_Update_Operations(address payable wallet) public onlyOwner() {
        Wallet_Operations = wallet;
        _isExcludedFromFee[Wallet_Operations] = true;
    }
    
    // Update development wallet
    function Wallet_Update_Development(address payable wallet) public {
        require(address(_msgSender()) == address(Wallet_Development));
    ...
    */
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        */
        ...
        // Limit wallet total
        if (to != owner() && to != Wallet_Operations && to != Wallet_Development && to != address(this) && to != uniswapV2Pair && to != Wallet_Burn && from != owner()) { 
    ...
    */
    ...
    function split(uint liquidity) public view returns(uint,uint,uint) {
        ...
        uint operationsLiquidity = (liquidity * teamDonationMultiplier) / shareBasisDivisor;
        uint developmentLiquidity = (liquidity - operationsLiquidity);
        uint totalSumOfLiquidity = operationsLiquidity+developmentLiquidity;
        assert(uint(totalSumOfLiquidity)==uint(liquidity));
        return (totalSumOfLiquidity,operationsLiquidity,developmentLiquidity);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        (uint256 tSum, uint256 oSum, uint256 dSum) = split(contractETH);
        if(devOps == true) {
            sendToWallet(Wallet_Operations,oSum);
            sendToWallet(Wallet_Development,dSum);
        } else {
            sendToWallet(Wallet_Operations,tSum);
        }
    }
    ...
    */
    ...
    function _transferTokens(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tOps) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _tOwned[address(this)] = _tOwned[address(this)].add(tOps);   
        emit Transfer(sender, recipient, tTransferAmount);
    }

    // Calculating the fee in tokens
    function _getValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tOps = tAmount*_TotalFee/100;
        uint256 tTransferAmount = tAmount.sub(tOps);
        return (tTransferAmount, tOps);
    }
}