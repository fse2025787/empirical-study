...
// File: contracts/ProofOfWork721.sol
pragma solidity >=0.6.0 <0.8.0;
...
contract ProofOfWork721 is Context, ERC721, Ownable, ERC721Burnable {
    ...
    //This needs to be the same as the maxNumberOfPieces
    bool[50] public isTokenMinted;
    ...
    ) public ERC721("Proof of Work", "POW") {
        pricePerPiece = givenPricePerPiece;
        withdrawalAddress = givenWithdrawalAddress;
        _setBaseURI(givenTokenUriBase);
        maxNumberOfPieces = 50;
        maxTokenIdForSale = 0;
        minTokenIdForSale = 1;
    }
    
    //Update metadata base  
    function setMetadataBase(string memory givenTokenUriBase) public onlyOwner{
        emit MetadataUpdated(givenTokenUriBase);
        _setBaseURI(givenTokenUriBase);
    }
    
    //Specify a range of tokens for current sale
    function setSaleRange(uint256 givenMinTokenIdForSale, uint256 givenMaxTokenIdForSale) public onlyOwner {
         maxTokenIdForSale = givenMaxTokenIdForSale;
         minTokenIdForSale = givenMinTokenIdForSale;
    }
    
    //Dutch auction, price decreases each block until it reaches the minimum
    function startDutchAuction(uint256 givenStartPrice, uint256 givenOfferPriceDecrement, uint256 givenEndPrice) public onlyOwner{
        startBlock = block.number;
        startPrice = givenStartPrice;
        endPrice = givenEndPrice;
        currentPrice = givenStartPrice;
        offerPriceDecrement = givenOfferPriceDecrement;
        dutchAuctionActive = true;
    }
    
    function endDutchAuction() public onlyOwner{
    ...
    //Dutch auction buy function, calculates appropriate price based on blocks elapsed
    function buyDutch( uint256 givenTokenId) public payable{
        require(dutchAuctionActive, "dutch auction must be active"); 
       
        currentPrice = startPrice - (block.number - startBlock) * offerPriceDecrement;
     
        if(currentPrice < endPrice || currentPrice > startPrice){
            currentPrice = endPrice;
        }
        require(msg.value >= currentPrice, "must send in correct amount ");
        mintItem(givenTokenId);
    }
    
    //Standard buying functions are separate
    ...
    function buy( uint256 givenTokenId) public payable {
    ...
    function mintItem(uint256 givenTokenId) private returns (uint256) {
        ...
        require(piecesSold.current() < maxNumberOfPieces, "series is sold out");
        
        emit Purchase(msg.sender, msg.value, givenTokenId);
    
        _mint(msg.sender, givenTokenId);
        
        isTokenMinted[givenTokenId] = true;
        piecesSold.increment();
