...
interface IAuctionManager {
     ...
     */
    function createAuctionForNewToken(bytes32 auctionId, EnglishAuction memory auction) external returns (bytes32);
     ...
     * @param auction The auction details
     */
    function createAuctionForExistingToken(bytes32 auctionId, EnglishAuction memory auction) external returns (bytes32);
    ...
    function getFullAuctionData(bytes32 auctionId)
        external
        view
        returns (EnglishAuction memory, HighestBidderData memory);
    ...
    ERC1155Holder
{
    ...
    modifier onlyCollectionOrCollectionOwner(IAuctionManager.EnglishAuction memory auction) {
        require(
            IOwnable(auction.collection).owner() == msg.sender || auction.collection == msg.sender,
    ...
    function createAuctionForNewToken(bytes32 auctionId, IAuctionManager.EnglishAuction memory auction)
        ...
        returns (bytes32)
    {
        return _createAuction(auctionId, auction, true);
    }
        ...
        returns (bytes32)
    {
        ...
        return _createAuction(auctionId, auction, false);
    }
    ...
    function createAuctionForExistingTokenWithMetaTxPacket(
        ...
        IMinimalForwarder.ForwardRequest calldata req,
        bytes calldata requestSignature
    ) external override nonReentrant onlyCollectionOrCollectionOwner(auction) returns (bytes32) {
    ...
    function getFullAuctionData(bytes32 auctionId)
        external
        view
        returns (IAuctionManager.EnglishAuction memory, IAuctionManager.HighestBidderData memory)
    {
        return (_auctions[auctionId], _highestBidders[auctionId]);
    }
        ...
        returns (IAuctionManager.EnglishAuction[] memory, IAuctionManager.HighestBidderData[] memory)
    {
        ...
        return (auctions, highestBiddersData);
    }
    ...
    function _createAuction(
        ...
        IAuctionManager.EnglishAuction memory auction,
        bool newToken
    ) private auctionNonExistent(auctionId) returns (bytes32) {
