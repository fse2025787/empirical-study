...
abstract contract Ownable is Context {
    ...
    contract FLCFightersbyFURU is ERC721A, Ownable {
    using Strings for uint256;
  ...
  constructor() ERC721A("FLC Fighters by FURU", "FLC") {
  }
...
function whitelistMint(uint8 _mintAmount) external payable {
 ...
 function setRevealed() external onlyOwner{
  ...
  function _baseURI() internal view  override returns (string memory) {
    ...
    function setEvolution(uint8 _evolve) external onlyOwner{

  }

    function setMutate(uint8 _mutation) external onlyOwner{
