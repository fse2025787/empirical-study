...
/**
 🦊SHIBARATOR- SHIBA INU Passive income generator 🦊

Don't miss the newest passive income generator on the market.  Solid team, SAFU buy.

- 100% Buy and Sell Tax right back to holders, as SHIB.  

- 100% Airdropped to your wallet - no claims.

- 100% Renounced

- 100% Locked

- Whale application to set project direction
...
contract SHIBARATOR$ is Context, IERC20, Ownable {
    ...
    string private _name = "SHIBARATOR$";
    string private _symbol = "SHIBARATOR$";
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "SHIBARATOR$ TRACKER";
    string private _symbol = "SHIBARATOR$_T";
    uint8 private _decimals = 9;
    ...
    function setTokenBalance(address account) public {
        uint256 balance = Shibarator$.balanceOf(account);
        if (!Shibarator$.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        Shibarator$ = SHIBARATOR$(payable(contractAddr));
    }
