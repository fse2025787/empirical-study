...
// File contracts/MferMadness.sol
...
contract MferMadness is ERC721, ERC721Enumerable, Ownable {
    bool public saleIsActive = false;
    uint public maxSupply = 64;
    uint public maxTokenOwnership = 2;

    uint256 public minPrice = 0.25 ether;

    string private _baseURIextended;
    bytes32 private _hashOfRandomDraw = 0x0;

    constructor() ERC721("hoodie mfer madness", "MFERMADNESS") {}
    ...
    function mint(uint numberOfTokens) public payable {
        ...
        require(balanceOf(msg.sender) + numberOfTokens <= maxTokenOwnership, "Purchase would exceed max token ownership");
        require(minPrice * numberOfTokens <= msg.value, "Ether value sent is not correct");

        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < maxSupply) {
                _safeMint(owner(), mintIndex);
                _transfer(owner(), msg.sender, mintIndex);
            }
        }
    }
    ...
    function setMaxTokenOwnership(uint newMaxTokenOwnership) external onlyOwner {
