...
contract LilDeadpunks is Ownable, ERC721AQueryable, ERC2981, ReentrancyGuard {
    ...
    mapping(uint256 => bool) claimedPunk;
    ...
    function claimLilPunk(uint256[] calldata punkIds) external {
        // Validation
        ...
        for (uint256 i; i < _amount; ) {
            if (claimedPunk[punkIds[i]]) revert AlreadyClaimed();
            if (deadPunks.ownerOf(punkIds[i]) != msg.sender) revert NotYourPunk();
            claimedPunk[punkIds[i]] = true;
