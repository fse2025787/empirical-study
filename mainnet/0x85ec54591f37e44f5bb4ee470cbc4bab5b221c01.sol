...
contract Shattered is ERC721, ReentrancyGuard, Ownable {
    ...
    function setRandomList(uint256[] memory list) public onlyOwner {
        order = list;
    }
    ...
    function mintPiece(uint256 tokenId) private {
        require(tokenId <= maxSupply, 'Total supply has been minted');

        uint256 blitTokenId = order[tokenId - 1];
        uint256 cornerIndex;
    ...
    function mintMany(uint256 count) public payable nonReentrant {
        require(live == true, 'Minting must be live');
        require(count * PRICE == msg.value, "Mint price is not correct");
        require(count <= 8, "Can only mint 8 max at a time.");

        for (uint256 i = 0; i < count; i++) {
            mintPiece(getNextPieceId());
        }
    }
