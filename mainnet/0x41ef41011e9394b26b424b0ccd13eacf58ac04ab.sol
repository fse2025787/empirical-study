...
contract OTK is ERC721A, Ownable, Pausable 
{
    ...
    address payable private seller = payable(0xAfebd04310a4A3f5Cbe3178B8054FC5C978a49b2); 
    uint256 public currentId = 0;
    uint256 public startingPrice = 10000;
    uint public  discount = 10;
    uint256 private startAt = 1659884400;
    ...
    function mint() external payable callerIsUser {
        require(mintList[msg.sender] + 1 <= walletMintLimit, "Up to 3 mint allowed per wallet");
        require(totalSupply() + 1 < collectionSize, "EXCEED_COL_SIZE");
        uint256 timeElapsed = block.timestamp - startAt;
        require(timeElapsed >= 0, "Auction has not started");
        uint timeMinutes = timeElapsed / 60;
        if(timeMinutes >= discount && startingPrice > 0){
            uint x = timeMinutes / discount;
            for(uint i=0;i<x;i++){
                startingPrice = startingPrice * 80 / 100;
            }
            if(startingPrice < 1){
                startingPrice = 0;
            }
