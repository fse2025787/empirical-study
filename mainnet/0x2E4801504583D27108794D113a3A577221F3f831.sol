...
Globo Inu Socials:
Telegram: https://t.me/globoinu
Twitter: https://twitter.com/globoinu
Website: https://www.globohomo.online

** Secured With FTP Antibot **
** Using FTP LPAdd to recycle 7.00% of ALL transactions back into the liquidity pool. **
...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x30b92A3fBCC0bceAa1895d10c597F073a27018dd), 6020);
		setTaxAlloc(payable(0x7AbB2D375e66A11e379812E01B902ABca8891811), 980);
        m_DidDeploy = true;
    }
...
contract GloboInu is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000000000 * 10**9;
    string private m_Name = "Globo Inu";
    string private m_Symbol = "GLOBO";
	...
	// LP ADD
    IWETH private WETH;
    uint256 private m_LiqAlloc = 7000;
    // MISC
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != address(0) && _recipient != m_UniswapV2Pair && block.timestamp <= m_Launched.add(24 hours);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(1 minutes))
            return TOTAL_SUPPLY.div(20);
        else
            return TOTAL_SUPPLY;
    }
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(45 days), msg.sender, address(m_UniswapV2Router));
