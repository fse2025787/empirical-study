...
contract Airdrop is EIP712, Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        require(_signer != address(0), "Signer is the zero address");
        token = IERC20(_token);
        MAX = _max;
        cSigner = _signer;
    }
    ...
    function changeSigner(address _signer) external onlyOwner {
        require(_signer != address(0), "Signer is the zero address");
        address oldSigner = cSigner;
        cSigner = _signer;
        emit ChangeSigner(oldSigner, cSigner);
    }

    function signMessage(address user, uint256 amount)
        external
        view
        returns (bytes32)
    {
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    ECDSA.toTypedDataHash(
                        _domainSeparatorV4(),
                        keccak256(abi.encode(MINT_CALL_HASH_TYPE, user, amount))
                    )
                )
            );
    }
        ...
        onlyOwner
    {
        ...
        require(balanceToWithdraw != 0, "Nothing to withdraw");
    ...
    function claim(
        ...
        uint256 total = MAX - token.balanceOf(address(this)) + amount;
        require(total <= MAX, "Airdrop: Exceed max supply");
        require(!hasClaimed[msg.sender], "Airdrop: Claimed");
        ...
        require(
            ...
            "Airdrop: Invalid signer"
        );
        hasClaimed[msg.sender] = true;
        if (token.balanceOf(address(this)) <= MAX / 2) {
            amount = amount / 2;
        }
