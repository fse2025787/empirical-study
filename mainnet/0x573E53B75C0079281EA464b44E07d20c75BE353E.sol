...
/**
$BTHE - Baby The Protocol ⚫️

The baby is here to support his daddy The Protocol ($THE) with buybacks and burns. Moreover, the baby earns you reflections in $THE.

Taxes: 3/3
2% to buybacks and burns.
1% reflections in $THE

https://t.me/babytheprot
...
interface IReflectionsContract {
...
contract ReflectionsContract is IReflectionsContract {
    ...
    constructor () {
        ...
        THETOKEN = 0x0cbA60Ca5eF4D42f92A5070A8fEDD13BE93E2861;
    }
    ...
    function deposit() public payable override {
        uint256 balanceBefore = IERC20(THETOKEN).balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(THETOKEN);

        // takes the current balance of WETH on the contract and transfers to THETOKEN
        ...
        uint256 amount = IERC20(THETOKEN).balanceOf(address(this)).sub(balanceBefore);
        // amount = amount of new THETOKEN just converted. add that to the totalDividends
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            IERC20(THETOKEN).transfer(shareholder, amount);
...
contract BabyProtocol is IERC20, Auth {
    using SafeMath for uint256;

    address public THETOKEN = 0x0cbA60Ca5eF4D42f92A5070A8fEDD13BE93E2861; //THETOKEN COIN
    ...
    uint256 public toReflections = 35;
    uint256 public toLiquidity = 0;
    uint256 public toMarketing = 65;
    ...
    address public teamWallet = payable(0x96760c36957C8837ae70daD7DEf24f34507Ed165); // add address here

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    bool public tradingOpen = false;
    
    ReflectionsContract public babytheprotocol;
    uint256 public babyGas = 0;
    ...
    constructor (
        ...
        babytheprotocol = new ReflectionsContract();
    ...
    function setDistributionCriteria(uint256 newMinPeriod, uint256 newMinDistribution) external onlyOwner {
        babytheprotocol.setDistributionCriteria(newMinPeriod, newMinDistribution);        
    }
    ...
    function _setIsDividendExempt(address holder, bool exempt) internal {
        ...
        if(exempt){
            babytheprotocol.setShare(holder, 0);
        }else{
            babytheprotocol.setShare(holder, _balances[holder]);
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]) {
            try babytheprotocol.setShare(sender, _balances[sender]) {} catch {}
        }

        if(!isDividendExempt[recipient]) {
            try babytheprotocol.setShare(recipient, _balances[recipient]) {} catch {} 
        }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 ethForTHETOKEN = totalEthBalance.mul(toReflections).div(100);
        ...
        try babytheprotocol.deposit{value: ethForTHETOKEN}() {} catch {}
    ...
    function manualProcessGas(uint256 manualGas) external onlyOwner {
        babytheprotocol.process(manualGas);
    }

    function checkPendingReflections(address shareholder) external view returns (uint256) {
        return babytheprotocol.getUnpaidEarnings(shareholder);
    }

    function getRewards() external {
        babytheprotocol.getDividends(msg.sender);
    }
}