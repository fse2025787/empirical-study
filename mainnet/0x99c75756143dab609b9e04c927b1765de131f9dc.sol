...
contract DTTDIAPerContract is ERC1155, Ownable {
    struct Token {
        string name;
        string symbol;
        uint256 maxSupply;
    ...
    function setupNFT(string memory tokenName, string memory tokenSymbol, uint256 tokenMaxSupply, string memory tokenURI) external onlyOwner {
    ...
    function airdrop(bytes32 tid, address to, uint256 tokenID) external onlyOwner tidCheck(tid) {
        require(tokenID < numberOfTokens, "Invalid ID");
