...
contract MintingModule is AccessControl {
  ...
  LegacyMintingModule public legacyMintingModule;
  ...
  constructor(
    ...
    address hashingModuleContractAddress,
    address legacyMintingModuleAddress,
    uint256 _fee,
    ...
    legacyMintingModule = LegacyMintingModule(legacyMintingModuleAddress);
  ...
  function mintDecagon() external payable {
    if (!publicMintEnabled) revert PublicMintingDisabled();
    if (msg.value < fee) revert NotEnoughEth();
    _mint();
  }

  function mintAllowlistedDecagon(bytes32[] calldata _merkleProof) external payable {
  ...
  function _mint() internal {
    ...
    if (legacyMintingModule.minted(msg.sender)) revert AddressAlreadyMinted();
  ...
  function setLegacyMintingModule(address legacyMintingModuleAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    legacyMintingModule = LegacyMintingModule(legacyMintingModuleAddress);
    emit LegacyMintingModuleSet(legacyMintingModuleAddress);
  }
