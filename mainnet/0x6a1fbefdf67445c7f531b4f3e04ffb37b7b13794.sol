...
contract WaEthToken is AccessControl, ERC20("Wasabi ETH", "waETH") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "WaETH: YumEthVault is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "WaETH: YumEthVault is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"WaETH: YumEthVault's ceiling was breached.");
    require(!paused[msg.sender], "WaETH: user is currently paused.");
