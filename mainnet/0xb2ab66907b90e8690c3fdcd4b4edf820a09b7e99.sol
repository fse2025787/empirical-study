...
contract BadInfluenceHonoraries is Ownable, ERC721A, DefaultOperatorFilterer {
  ...
  event SetTokenMetadata(uint256[] tokenIds);
  ...
  function setTokenMetadata(
    ...
    require(tokenIdsLength == _tokenMetadata.length, "Array size mismatch");

    for (uint256 i = 0; i < tokenIdsLength; i++) {
      require(_exists(tokenIds[i]), "nonexistent token");
      tokenMetadata[tokenIds[i]] = _tokenMetadata[i];
    }

    emit SetTokenMetadata(tokenIds);
  }
  ...
  function mint(string[] memory _tokenMetadata) external onlyOwner {
    uint256 nextId = _nextTokenId();
    uint256 tokenMetadataLength = _tokenMetadata.length;

    for (uint256 i = 0; i < tokenMetadataLength; i++) {
      tokenMetadata[nextId + i] = _tokenMetadata[i];
    }

    _mint(msg.sender, tokenMetadataLength);
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if (bytes(tokenMetadata[tokenId]).length != 0) {
      return string(tokenMetadata[tokenId]);
    }
