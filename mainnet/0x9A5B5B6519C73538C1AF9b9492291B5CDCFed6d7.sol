...
contract AMMRouterV1 is IAMMRouterV1, AccessControlUpgradeable {
    ...
    function swapExactAmountIn(
        ...
        uint256 _currentTokenAmountIn = _pushFees(_amm, _pairPath, _tokenPath, _tokenAmountIn, _referralRecipient);
    ...
    function _pushFees(
        ...
        if (referralFee != 0) {
            uint256 feeAmount = _tokenAmountIn.sub(_currentAmountIn).mul(referralFee) / UNIT;
            if (feeAmount > 0) {
                tokenIn.safeTransfer(_referralRecipient, feeAmount);
                emit ReferralFeePaid(_referralRecipient, feeAmount);
            }
        }
    }
    ...
    function swapExactAmountOut(
        ...
        uint256 currentAmountIn = getAmountIn(_amm, _pairPath, _tokenPath, _tokenAmountOut);
        uint256 amountIn = _pushFees(_amm, _pairPath, _tokenPath, currentAmountIn, _referralRecipient);
        require(amountIn <= _maxAmountIn, "AMMRouterV1: Max amount in reached");
        tokenAmountIn = _executeSwap(_amm, _pairPath, _tokenPath, amountIn, _tokenAmountOut, _to);
    }
    ...
    function _executeSwap(
        ...
        if (_pairPath.length == 2) {
            ...
            firstSwapAmountOut = getAmountIn(_amm, pairPath, tokenPath, _tokenAmountOut).mul(UNIT - GOVERNANCE_FEE).div(
                UNIT
            );
