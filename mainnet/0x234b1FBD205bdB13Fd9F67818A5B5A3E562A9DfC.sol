...
//   address public nftContractAddress;
  ...
  // contract address => tokenID => ItemID(which is assigned by our market and can be tracked all history of that token id by using this itemID)
  ...
  mapping(uint256 => address []) public biddingaddress;
  mapping(uint256 => uint256 []) public biddingamount;
...
//   }
  ...
  function createMarketItem(
    ...
    tokenIdToItemId[nftContractAddress][tokenId] = itemId;
...
uint256 public divisor=100;
    ...
    //owner can change the market fee percentage
    function changeMarketfee(uint256 _fee) public onlyOwner{
        Marketfee=_fee;
    }
  ...
  function createMarketSale(
   ...
   require(msg.value>0,"enter some eth!");
   // 20% fees for marketplace
   uint256 _mfee=(Marketfee.mul(msg.value) ).div(divisor);  //calculating market fee
    ...
    if(idToRoyalityData[tokenId].royalityPercentage>0 && idToRoyalityData[tokenId].creator!=address(0)){
      royalityAmount = (price*idToRoyalityData[tokenId].royalityPercentage)/TOTAL_PERC; //calculating royality fee
      console.log("royalityAmount",royalityAmount);
        payable(idToRoyalityData[tokenId].creator).transfer(royalityAmount);
    }
    uint256 remaining = price.sub(royalityAmount.add(_mfee));
    console.log("remaining",remaining);
    idToMarketItem[itemId].seller.transfer(remaining); //transfer the reamining to seller
  ...
  function createBidOnItem(
    ...
    biddingaddress[itemId].push(msg.sender);
    biddingamount[itemId].push(msg.value);
  }

//   can claim the  bid items if become the winner
// royality and platform fee will be distributed

  function claimBidItem(uint256 itemId,address nftContractAddress) public payable nonReentrant {
    require(idToMarketItem[itemId].isOnAuction,"Item is not on auction");
   ...
   // 20% fees for marketplace
   uint256 _mfee=(Marketfee.mul(price) ).div(divisor);  //calculating market fee
...
//   function withdrawYourBid(uint256 itemId , address nftContractAddress) public nonReentrant{
//     require(idToMarketItem[itemId].isOnAuction,"Item is not on auction");
//     require(idToMarketItem[itemId].bidEndTime<=block.timestamp,"Bidding is still in progress");
//     uint tokenId = idToMarketItem[itemId].tokenId;
//     require(highestBidderMapping[itemId].bidderAddr==msg.sender,"Only highest bidder can claim the NFT");

//     // _approve(address(0), itemId);
...
//   }
  ...
  function fetchMyNFTs(address add) public view returns (MarketItem[] memory) {
    ...
    for (uint i = 0; i < totalItemCount; i++) {
      if (idToMarketItem[i + 1].owner == add) {
        itemCount += 1;
      }
    }
  ...
  function fetchItemsCreated(address add) public view returns (MarketItem[] memory) {
    uint totalItemCount = _itemIds.current();
    uint itemCount = 0;
    uint currentIndex = 0;
...
//   owner can withdraw the contract's eth
  ...
  //caller can withdraw the amount on UN-successfull bidings
