...
contract NFT2040Crates is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;
    uint256 public constant NFT_PRICE = 100000000000000000;
    uint256 public constant MAX_NFT_PURCHASE = 7;
    ...
    constructor(address _store) ERC721("NFT2040 Crates", "NFT2040Crates") {
        store = _store;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        (bool sent, ) = store.call{value: balance}("");
    ...
    function mint(uint256 count) public payable {
        ...
        require(count > 0, "Number of tokens must be positive");
        require(
            totalSupply().add(count) <= MAX_SUPPLY,
            "Purchase would exceed max supply of crates"
        );
        require(
            count <= MAX_NFT_PURCHASE,
            "Can only mint up to 7 per purchase"
        );
        require(
            NFT_PRICE.mul(count) == msg.value,
