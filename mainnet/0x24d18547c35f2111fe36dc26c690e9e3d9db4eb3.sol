...
contract Nakedz is ERC721A, Ownable {
    ...
    string public hiddenURI="ipfs://bafybeih5jcdvmsiut4v6wmctjp5c2csh3pploohgya7kcarh563ixr7lci/hidden.json";
...
function nakedMint(bytes32[] calldata _merkleProof,uint256 count) external payable {
        uint256 cost = price;
       ...
       bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        ...
        require(nakedList, "Minting is not live yet");
...
function allowListMint(bytes32[] calldata _merkleProof) external payable {
       ...
       bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        ...
        require(allowList, "Minting is not live yet");
...
function publicMint(uint256 count) external payable {
    ...
    returns (string memory)
  {
   ...
   function flipSale(bool status) external onlyOwner {
    ...
    function flipNakedList(bool status) external onlyOwner {
        nakedList = status;
    }
     function flipallowList(bool status) external onlyOwner {
