...
contract BridgeKeepers is ERC721, Ownable {
	...
	function setWhitelisted(address[] memory addressList) external onlyOwner{
		 for (uint8 i = 0; i < addressList.length; i++){
           _whitlisted[addressList[i]] = true;
        }
	}
	
	function removeWhitelisted(address[] memory addressList) external onlyOwner {
		for (uint8 i = 0; i < addressList.length; i++){
           _whitlisted[addressList[i]] = false;
        }
	}
