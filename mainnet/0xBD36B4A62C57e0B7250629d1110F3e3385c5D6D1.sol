...
contract Canopy is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "Canopy";
    ...
    uint256 internal _tokenTotal = 1000000 *10**18; // 1 million totalSupply
    uint256 internal _reflectionTotal = (MAX - (MAX % _tokenTotal));
    mapping(address => bool) isExcludedFromFee;
    mapping(address => bool) internal _isExcluded;
    address[] internal _excluded;

    uint256 public _taxFee = 200; // 200 = 2%
    uint256 public _charityFee = 100; // 100 = 1%
    uint256 public _refferalFee = 300; // 300 = 3%
  
    uint256 public _taxFeeTotal;
    uint256 public _charityFeeTotal;
    uint256 public _refferalFeeTotal;
   
    address public charityAddress = 0x80F4aea6A4390f5265A690B68E6E3f822F7879b2; // TREE Charity Address
    address public refferalAddress = 0x69d96053Ec00Fa5413AAd33A19f9380CC7F75456; // refferalAddress
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        uint256 refferalFee = amount.mul(_refferalFee).div(10000);
        ...
        //@dev refferalFee fee
        if(refferalFee > 0){
            transferAmount = transferAmount.sub(refferalFee);
            _reflectionBalance[refferalAddress] = _reflectionBalance[refferalAddress].add(refferalFee.mul(rate));
            _refferalFeeTotal = _refferalFeeTotal.add(refferalFee);
            emit Transfer(account,refferalAddress,refferalFee);
        }
    ...
    function setRefferalAddress(address _Address) public onlyOwner {
        require(_Address != refferalAddress);
        refferalAddress = _Address;
    }
    
    function setRefferalFee(uint256 fee) public onlyOwner {
        _refferalFee = fee;
    }
