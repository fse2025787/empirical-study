...
pragma solidity ^0.8.0;
contract RealEstateTycoons is Ownable, ERC721A, ReentrancyGuard {

    bool public publicSale = false;
    bool public whitelistSale = false;
    bool public revealed = false;

    uint256 constant public reserve = 777;
    ...
    constructor(string memory _NAME, string memory _SYMBOL)
        ERC721A(_NAME, _SYMBOL, maxPerWallet, maxToken)
    {}
    ...
    function mint(uint256 quantity) external payable callerIsUser {
        ...
        require(numberMinted(msg.sender) + quantity <= maxPerWallet, "PER_WALLET_LIMIT_REACHED");
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory proof) external payable callerIsUser {
        ...
        require(numberMinted(msg.sender) + quantity <= maxPerWalletWL, "WHITELIST_PER_WALLET_LIMIT_REACHED");
        require(totalSupply() + quantity <= maxToken, "NOT_ENOUGH_SUPPLY_TO_MINT_DESIRED_AMOUNT");
        require(msg.value == (price * quantity), "INVALID_ETH");
    ...
    function setMaxPerTxWL(uint256 _maxPerTxWL) external onlyOwner {
        maxPerTxWL = _maxPerTxWL;
    }
