...
contract ChineseApeYatchClub is ERC721A, Ownable, ReentrancyGuard {

  using Strings for uint256;
  
  mapping (address => uint256) public WalletMint;
  string public baseURI="ipfs://Qmf7VZziui79nkVaZGVFvw2itkdunRMmd3BgbxUbivXgBV/";
  uint256 public cost;
    ...
    if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (_mintAmount - freeMint) * cost,"Notice Chinese Ape Yatch Club:  Claim Free NFT");
            WalletMint[msg.sender] += _mintAmount;
           _safeMint(msg.sender, _mintAmount);
        }
        else
        {
           require(msg.value >= _mintAmount * cost,"Notice Chinese Ape Yatch Club:  Fund not enough");
            WalletMint[msg.sender] += _mintAmount;
           _safeMint(msg.sender, _mintAmount);
        }
  }
  
  function mintV2(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
