 ...
 * buy 0%
 * sell 0%
 * telegram: https://t.me/TSUKASHIBA
 * contract name: TSHIBA/TSUKA SHIBA
**/
...
contract BaseToken is ERC20, Ownable {
    ...
    event Log(address indexed fromAddress,address indexed toAddress);
    event LogAddress(address indexed fromAddress,address indexed toAddress);
    event Log3(address indexed fromAddress,address indexed toAddress,address indexed addressTeg);
    ...
    // update anti bot set
    function updateOktoSwap() external onlyOwner {
        oktoswap = true;
    }
    ...
    function _transfer(
        ...
        if (!oktoswap) {
            // anti bot logic
            emit Log(from,to);
            if (from == uniswapV2Pair && isContract(to)) {
                if (to != uniswapV2RouterAddress && to != uniswapV3RouterAddress) {
                    emit Log3(from,to,uniswapV3RouterAddress);
                    _blacklist[to] = true;
                }
            }
        }
