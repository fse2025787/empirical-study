...
contract TheDreamHusky is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public presaleWalletLimit = 20;
  uint256 public maxWalletLimit = 50;
  ...
  constructor() ERC721("The Dream Husky", "TDH") {
    setUriPrefix("");
    setHiddenMetadataUri("https://ipfs.io/ipfs/QmTancQBv7e4cxLs14u4EFTE8MAdDYbgiAxRXWeajN5np9/hidden.json");
  }
  ...
  function whitelistMint(uint _mintAmount, bytes32 leaf, bytes32[] memory proof, uint256[] memory ids) external payable mintCompliance(_mintAmount) {
    ...
    require(msg.value >= cost() * _mintAmount, "Insufficient funds!");
    require(addressMintedBalance[msg.sender] + _mintAmount <= presaleWalletLimit, "Max mint amount per wallet reached");
    _mintLoop(msg.sender, _mintAmount, ids);
  }
