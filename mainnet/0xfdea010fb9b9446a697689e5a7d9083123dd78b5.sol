...
contract CRYPTO_SHARK is ERC721A, Ownable, Pausable {
    string  public baseURI = "ipfs://QmWYA1zgmWXt6cmd9EMMC1LaxU2Y3GgnneZjGLc1XwNFLf/";
    uint256 public _price = 0.001 ether;
    uint256 public Supply = 11416;
    uint256 public _reserved = 50;

    // Public Minting
    uint256  public PM_limit = 4;
    mapping(address => uint) public P_Mint;

    // WhiteList Minting
    bool public _WLpaused = true;
    uint256  public WL_limit = 4;
    mapping(address => uint) public WL_Mint;

    address public fundWallet = 0x3D2b318D2514FBB6b74ef6f193b69516ba7E252d;

    // Our rootHash
    bytes32 public root = 0xeeefd63003e0e702cb41cd0043015a6e26ddb38073cc6ffeb0ba3e808ba8c097;


    constructor() ERC721A("CRYPTO SHARK", "$SHRK", 100) {
      // _unpause();
    }

    function Mint(uint256 num) public payable {
      require(paused() == false, "Public Minting is paused");
      require((_price * num) <= msg.value, "Invalid minting price");
      require( P_Mint[msg.sender] + num <= PM_limit,     "Exceeds Public minting limit" );
      unchecked { require(totalSupply() + num <= Supply - _reserved, "Supply MAX REACHED"); }
      _safeMint(msg.sender, num);
      P_Mint[msg.sender] += num;
    }
    ...
    function _airdrop(address _address, uint256 num) internal {
        require(num <= _reserved, 'Airdrop limit Exceeds');
        unchecked { require(totalSupply() + num <= Supply, 'Supply MAX REACHED'); }
        _safeMint(_address, num);
        _reserved -= num;
    }

    function WHITELIST_MINT(uint256 num, bytes32[] calldata _merkleProof) public payable {
      uint256 supply = totalSupply();
      require( !_WLpaused,                                     "Pre-Sale paused" );
      require( checkValidity(_merkleProof) == true,            "Only WHITELIST can mint" );
      require( supply + num <= Supply - _reserved,             "Exceeds maximum pre-sale supply" );
      require( msg.value >= _price * num,                      "Ether sent is not correct" );
      require( WL_Mint[msg.sender] + num <= WL_limit,          "Exceeds whitelist minting limit" );
    ...
    function checkValidity(bytes32[] calldata _merkleProof) public view returns (bool){
      ...
      require(MerkleProof.verify(_merkleProof, root, leaf), "Incorrect proof, caller not whitelisted");
    ...
    function setNewMerkelHash(bytes32 __hash) public onlyOwner() {
        root = __hash;
    }
    ...
    function setNewWhiteListMintLimit(uint256 __limit) public onlyOwner {
      WL_limit = __limit;
    }

    function setNewPublicMintLimit(uint256 __limit) public onlyOwner {
      PM_limit = __limit;
    }

    function setNewFundWallet(address _fundWallet) public onlyOwner {
