...
contract PCH is Ownable, IERC20 {
    ...
    address payable public devWallet =
        payable(0x994be338093Fda9Bcbe68677359d0c05957643A9);
    address payable public treasuryWallet =
        payable(0x889D07ba77806e4c5F1ed67ef4c749bE81C84EA8);
    ...
    event SendFunds(uint256 tokensSwapped, uint256 amount);
        ...
        returns (bool)
    {
        ...
        require(
            currentAllowance >= subtractedValue,
            "PCH: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            currentAllowance >= amount,
            "PCH: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(
            ...
            "PCH: Token isn't tradable yet"
        );

        require(!isBlacklisted[sender], "PCH: Sender is blacklisted");
        require(!isBlacklisted[recipient], "PCH: Recipient is blacklisted");

        require(sender != address(0), "PCH: transfer from the zero address");
        require(recipient != address(0), "PCH: transfer to the zero address");
        ...
        require(
            senderBalance >= amount,
            "PCH: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "PCH: approve from the zero address");
        require(spender != address(0), "PCH: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        if (address(treasuryWallet) != address(0)) {
            tokensForTreasury = (tokens * treasuryFeeBPS) / totalFeeBPS;
        }

        uint256 tokensForDevelopment;
        if (address(devWallet) != address(0)) {
            tokensForDevelopment = (tokens * devFeeBPS) / totalFeeBPS;
        }

        uint256 tokensForDividends;
        if (dividendTracker.totalSupply() > 0) {
            tokensForDividends = (tokens * dividendFeeBPS) / totalFeeBPS;
        }
        ...
        if (nativeTreasury > 0) {
            ...
            if (success) {
                emit SendFunds(tokensForTreasury, nativeTreasury);
            }
        }

        // Send tokens to dev
        if (nativeDev > 0) {
            (bool success, ) = devWallet.call{
                value: nativeDev
            }("");
            if (success) {
                emit SendFunds(tokensForDevelopment, nativeDev);
            }
        }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "PCH: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "PCH: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "PCH: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function compound() public {
        require(compoundingEnabled, "PCH: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "PCH_DividendTracker";
    string private _symbol = "PCH_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "PCH_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "PCH_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "PCH_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "PCH_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("PCH_DividendTracker: method not implemented");
    }
