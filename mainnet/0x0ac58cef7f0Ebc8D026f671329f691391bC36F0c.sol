        ...
        if (chainlinkPriceUSD[_asset] != address(0)) {
            ...
            if (chainlinkPriceETH[_asset] != address(0)) {
                ...
                if (sushiswapKeeperOracle.observationLength(sushiPair) > 0) {
                    try sushiswapKeeperOracle.current(_asset, 10 ** decimals, weth) returns (uint256 sushiOraclePrice) {
                        price = sushiOraclePrice * wethPrice / 1e18;
                    } catch {
                        return 0;
                    }
                } else { 
                    ...
                    if (uniswapKeeperOracle.observationLength(uniPair) > 0) {
                        try uniswapKeeperOracle.current(_asset, 10 ** decimals, weth) returns (uint256 uniOraclePrice) {
                            price = uniOraclePrice * wethPrice / 1e18;
                        } catch {
                            return 0;
                        }
                    }
