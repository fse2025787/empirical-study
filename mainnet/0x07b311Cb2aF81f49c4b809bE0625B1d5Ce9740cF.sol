...
////////////////////////////////////////////// MAIN //////////////////////////////////////////////
  ...
  function mintFor(address _miner, uint256 _initPrice, uint256 _currentPrice, uint256 _cId, string memory _remarks, address _owner, uint256 _txs) external onlyOwner {
    require(!start);
    uint256 tokenId = uint256(keccak256(abi.encodePacked(_miner, _cId)));
    _mint(_miner, tokenId);
    _buidls[tokenId] = Buidl(_cId, _initPrice, _currentPrice, _txs, _miner, _remarks);
    if (_owner != _miner) {
      _removeTokenFrom(_miner, tokenId);
      _addTokenTo(_owner, tokenId);
      emit Transfer(_miner, _owner, tokenId);
    }
  }
