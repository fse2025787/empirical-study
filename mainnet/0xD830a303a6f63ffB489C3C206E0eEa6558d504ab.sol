...
contract NFTData {
    ...
    bool public usesMerkleProofs = false;
    mapping ( address => bool ) internal allowlist;
}

/**
    Auto Generated NFT Smart Contract
    Developed by DeFi Mark and dappd
    No more malicious developers, artists can use the NFT Generation Tool
    To create their own NFTs, or create a copy from this NFT with the `clone` function

    To launch your NFT using a nice front end interface, visit:
    https://dappd.net
*/
contract DappdGeneratedNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable, Clones, NFTData {
    ...
    function __init__(
        ...
        uint256 maxSupply,
        address mintToken_, // if address(0) use WETH
        uint256 cost_,
        address _owner
    ) external {
        require(
            MAX_SUPPLY == 0 && maxSupply > 0,
            'Already Initialized'
        );
    ...
    function addAllowlist(address[] calldata users) external onlyOwner {
    ...
    function revokeAllowlist(address[] calldata users) external onlyOwner {
        uint len = users.length;
        for (uint i = 0; i < len;) {
    ...
    function setUseMerkleProofs(bool shouldUseMerkleProof) external onlyOwner {
        usesMerkleProofs = shouldUseMerkleProof;
    }
    ...
    function mint(uint256 numTokens, bytes32[] calldata proof) external payable {
        ...
        require(
            cost * numTokens <= msg.value, 
            'Incorrect Value Sent'
        );

        _registerMint(msg.sender, numTokens, proof);
    }

    /** 
     * Mints `numTokens` NFTs To Caller
     */
    function mintWithToken(uint256 numTokens, bytes32[] calldata proof, uint256 tip) external {
        require(
            mintWithEther() == false,
            'Call mint'
        );
        ...
        uint256 received = _transferIn((cost * numTokens) + tip);
        require(
            received >= cost * numTokens,
            'Too Few Tokens Sent In'
        );

        _registerMint(_msgSender(), numTokens, proof);
    }
    ...
    function getIDsByOwner(address user) external view returns (uint256[] memory) {
        ...
        if (balanceOf(user) == 0) return ids;
        uint256 count = 0;
    ...
    */
    ...
    function _registerMint(address to, uint256 numTokens, bytes32[] calldata proof) internal {
        require(
            mintingEnabled,
            'Trading Not Enabled'
        );
        ...
        require(
            _balances[to] <= MAX_MINT,
            'Max Holdings Exceeded'
        );
    }

    function _transferIn(uint256 amount) internal returns (uint256) {
        uint256 balBefore = IERC20(mintToken).balanceOf(address(this));
        IERC20(mintToken).transferFrom(
            _msgSender(),
            address(this),
            amount
        );
        uint256 balAfter = IERC20(mintToken).balanceOf(address(this));
        require(
            balAfter > balBefore,
            'Zero Received'
        );
