...
/**
https://t.me/boosterfinancial
https://Booster.Financial/
...
pragma solidity ^0.6.12;
    ...
    contract BOOSTER is Context, IERC20, Ownable {
        ...
        uint256 public _DonationsFee = 2;
        uint256 private _previousTaxFee = _taxFee;
        uint256 private _previousMarketingFee = _MarketingFee;
        uint256 private _previousDonationsFee = _DonationsFee;

        bool public _isAntiDumpEnabled = false;
        uint256 public antiDumpTime;

        address public _MarketingWalletAddress;
        address public _Donations = 0x050e079536585aD688052d73f35f922d75C6c1c8;
        ...
        uint256 private _numOfTokensToExchangeForSwap = 5 * 10**6 * 10**9;
        ...
        function removeAllFee() private {
            if(_taxFee == 0 && _MarketingFee == 0 && _DonationsFee == 0) return;

            _previousTaxFee = _taxFee;
            _previousMarketingFee = _MarketingFee;
            _previousDonationsFee = _DonationsFee;
        ...
        function restoreAllFee() private {
            ...
            _DonationsFee = _previousDonationsFee;
        }
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            if(_isAntiDumpEnabled == true && sender != owner() && recipient != owner()){
                if(sender == uniswapV2Pair){
                    ...
                    require(timePassed1 > antiDumpTime && timePassed2 > antiDumpTime, 'You must wait between transfers');
                    _antiBot[sender] = block.timestamp;
                    _antiBot[recipient] = block.timestamp;
                }
            }
            ...
            if(_isWhitelist[sender] == true) {
                ...
                if(time_since_purchase < _lockPreSale[sender]){
                    require((balanceOf(sender) - amount) >= _lockedAmount[sender], 'You must wait time from purchase');
                }
                else {
            ...
            bool overMinTokenBalance = contractTokenBalance >= _numOfTokensToExchangeForSwap;
        ...
        function sendETHToFee(uint256 amount) private {
            uint256 swapFee = _MarketingFee.add(_DonationsFee);
            payable(_MarketingWalletAddress).transfer(amount.div(swapFee).mul(_MarketingFee));
            payable(_Donations).transfer(amount.div(swapFee).mul(_DonationsFee));
        }
        ...
        function _takeSwapFee(uint256 tSwap) private {
            uint256 currentRate =  _getRate();
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tSwap) = _getTValues(tAmount, _taxFee, _MarketingFee, _DonationsFee);
            uint256 currentRate =  _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
            return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tSwap);
        }

        function _getTValues(uint256 tAmount, uint256 taxFee, uint256 MarketingFee, uint256 DonationsFee) private pure returns (uint256, uint256, uint256) {
            uint256 tFee = tAmount.mul(taxFee).div(100);
            uint256 tSwap = tAmount.mul(MarketingFee.add(DonationsFee)).div(100);
        ...
        function setDonations(address Donations) external onlyOwner{
