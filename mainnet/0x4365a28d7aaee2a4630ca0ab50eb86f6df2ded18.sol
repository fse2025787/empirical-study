...
// File: contracts/stakingskull.sol
...
contract StakingSkull is Ownable, ReentrancyGuard{
    ...
    function unstakeLand(uint256 _tokenId) external {
        ...
        require(block.timestamp - stakers[landAddress[_tokenId]].stakedLands[_tokenId].timeOfLastStake > 86400, "You should wait a day to unstake your land");
        uint256 index = 0;
    ...
    function unstakeJunior(uint256 _tokenId) external {
        ...
        delete stakers[msg.sender].stakedJuniors[index];
        delete juniorAddress[_tokenId];
        juniorContract.transferFrom(address(this), msg.sender, _tokenId);
    }

    // Raiding
    function raid(uint256[] calldata _tokensId) public payable {
        // Find the index of this token id in the stakedLands array
        for (uint256 i = 0; i < _tokensId.length; i++) {
            require(juniorAddress[i] == msg.sender, "You must own the Junior Kong");
        }
        uint counter = 1;
        uint256 index = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, counter))) % 3165;
        do {
            index = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, counter))) % 3165;
            counter++;
        } while(landAddress[index] == msg.sender || landAddress[index] == address(0)) ;

        emit RaidToken(msg.sender, index);

        if ( randomRaid() > 90 + landResistance[index] - _tokensId.length) {
            ERC20Burnable(tokenContract).burnFrom(msg.sender, raidPrice);
            emit RaidSucces(msg.sender, true);
            // Unstake
            delete stakers[landAddress[index]].stakedLands[index];
            delete landAddress[index];
            // Stake
            stakers[msg.sender].stakedLands.push(StakedToken(msg.sender, index, block.timestamp));
            landAddress[index] = msg.sender;
            // Burn Junior
            if (_tokensId.length > 0 && randomBurn() > 90 ) {
                emit RaidJunior(msg.sender, true);
                ERC721Burnable(juniorContract).burn(_tokensId[0]);
                delete stakers[juniorAddress[_tokensId[0]]].stakedJuniors[index];
                delete juniorAddress[_tokensId[0]];
            }
        } else {
            ERC20Burnable(tokenContract).transferFrom(msg.sender, address(this), raidPrice);
            stakers[landAddress[index]].unclaimedRewards += raidPrice;
