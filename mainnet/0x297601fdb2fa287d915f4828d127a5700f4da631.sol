...
contract TiffBoredApes is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  string private hiddenMetadataUri;
    ...
    constructor() ERC721A("TiffBoredApes", "TBA") {
...
//pack5
    function pack5() external payable {
        uint256 cost = price;
        require(!paused, "The contract is paused!");
        if (msg.sender != owner()) {
            require(msg.value >= 4 * cost, "Please send the exact amount.");
        }

        require(totalSupply() + 5 <= maxSupply, "No more");

        _safeMint(msg.sender, 5);
    }

//pack10
    function pack10() external payable {
        uint256 cost = price;
        require(!paused, "The contract is paused!");
        if (msg.sender != owner()) {
            require(msg.value >= 8 * cost, "Please send the exact amount.");
        }

        require(totalSupply() + 10 <= maxSupply, "No more");
