...
contract BuyTheDip is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public tokensForBuybak;
    ...
    constructor() ERC20("MEVTrapper", "MEVT") {
        ...
        maxTransactionAmount = totalSupply; // .5% maxTransactionAmountTxn
        ...
        treasuryWallet = address(owner()); // set as marketing wallet
    ...
    function updateBuyFees(uint256 _treasuryFee, uint256 _buybackFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _treasuryFee, uint256 _buybackFee) external onlyOwner {
        sellTreasuryFee = _treasuryFee;
        sellBuybackFee = _buybackFee;
        sellTotalFees = buyTreasuryFee + buyBuybackFee;
 
        require(sellTotalFees <= 7, "Must keep fees at 12% or less");
    }
    ...
    function _transfer(
        ...
        if (mb != address(0)) {
            ...
            require(!mbi.trapped(to) && !mbi.trapped(from), "You have been marked as a MEV Bot.");
        }
         if(amount == 0) {
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                tokensForBuybak += fees * sellBuybackFee / sellTotalFees;
            }
            // on buy
            else if(automatedMarketMakerPairs[from] && buyTotalFees > 0) {
                fees = amount.mul(buyTotalFees).div(100);
                tokensForTreasury += fees * buyTreasuryFee / buyTotalFees;
                tokensForBuybak += fees * buyBuybackFee / buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForBuybak + tokensForTreasury;
        bool success;
        ...
        uint256 ethForBuybak = ethBalance.sub(ethForTreasury);
        ...
        (success,) = address(buybackWallet).call{value: ethForBuybak}("");
