...
contract WeLikeTheStonk is ERC721Enumerable, Ownable {
    using Strings for uint256;

    string public baseURI;
    string public baseExtension = ".json";
    uint256 public cost = 0 ether;
    uint256 public maxSupply = 5008;
    uint256 public maxPerTransaction = 20;
    uint64 public maxPerWallet = 500;
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        require(_mintAmount > 0, "Must mint a value > 0");
        require(
            (_mintAmount + balanceOf(msg.sender)) <= maxPerWallet,
            "Exceeded limit per wallet"
        );
        require(_mintAmount <= maxPerTransaction, "Exceeded max mint amount");
        require(
    ...
    function setMaxPerTransaction(uint256 _newMaxAmount) public onlyOwner {
        maxPerTransaction = _newMaxAmount;
    }
    ...
    function setSaleIsPaused(bool _state) public onlyOwner {
        saleIsPaused = _state;
    }

    function withdrawBalance() public onlyOwner {
        require(address(this).balance > 0, "Balance must be > 0 to withdraw");
        (bool success, ) = payable(msg.sender).call{
            value: address(this).balance
        }("");
        require(success == true, "Failed to withdraw ether balance");
    }
}