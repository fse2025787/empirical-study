...
contract XANALand is ProxyBaseStorage, ERC721AQueryable, Ownable, ReentrancyGuard, Storage {
 ...
 using Address for address;
    ...
    function mintNFT(address to,uint256 _mintAmount, string memory rarity, uint256 _size, bool isDiscount) internal {
        ...
        {
            tokenDetails[i] = TokenDetails(_size, rarity);
        }
    ...
    function freeMint(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount, 1, "Common") external{
        string memory rarity = "Common";
        uint256 _size=1;
    ...
    //Only for staking Users
    function claimRooster(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount, 1, "Common") external{
        string memory rarity = "Common";
        uint256 _size=1;
        //To check allowed buy amount from staking per user
        require(checkWhiteList(_mintAmount, proof, 6, true), "XanaLand: Not whitelisted");

        _userBought[6][msg.sender] += _mintAmount;

        
        //Minting Land NFTs as per sent amount above
        mintNFTWithLock(msg.sender,_mintAmount, rarity, _size, false);
       
    }

    function mintNFTWithLock(address to,uint256 _mintAmount, string memory rarity, uint256 _size, bool isDiscount) internal {
        //Minting Land NFTs as per sent amount above
        (uint256 start, uint256 end) =_safeMint(to,_mintAmount);
        rates[_size][rarity].sold+=_mintAmount;
        for (uint256 i = start; i<=end; i++) 
        {
            _nftLockup[i] = block.timestamp + 365 days;
            tokenDetails[i] = TokenDetails(_size, rarity);
        }
        emit _mintLand(to, rarity, _size, start, end, _mintAmount,msg.value,isDiscount);
    }
