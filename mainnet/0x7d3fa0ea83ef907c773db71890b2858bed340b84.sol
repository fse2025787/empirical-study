...
contract BabyZaddyInu is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 1e12 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Baby Zaddy Inu";
    string private _symbol = "BZADDY";
    ...
    function getUserToken1Balance(address account)
        public
        view
        returns (uint256)
    {
    ...
    function setToken1(IERC20 _token1) external onlyOwner {
        token1 = _token1;
    }
    ...
    function calculateBonusReflection(address _user) public override {
        ...
        uint256 userBalance2 = getUserToken1Balance(_user);
        uint256 currentRate = _getRate();
        ...
        if (delegateCall) {
            delegateCall = false;
            token1.calculateBonusReflection(_user);
        }
    }
