...
// File: contracts/ClaimMining.sol
...
contract ClaimMining is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 maxAmountPerUser;   // The maximum amount of deposits per user.
    }
    ...
    constructor(address _mining_token, uint256 _mining_start_block) public {
        ...
        uint256 blockCountOf21day = blockCountPerDay.mul(21);
        phase1StartBlockNumber = _mining_start_block;
        phase1EndBlockNumber = phase1StartBlockNumber.add(blockCountOf21day);
        phase1TokenPerBlock = 75 * 1e18;
    }
    ...
    function add(uint256 _allocPoint, address _lpToken, uint256 _maxAmountPerUser, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        if (phase1StartBlockNumber < _from && _from < phase1EndBlockNumber && _to > phase1EndBlockNumber) {
            return phase1EndBlockNumber.sub(_from).mul(phase1TokenPerBlock);
        }
        return 0;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant onlyWhitelisted {
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function addAddressToWhitelist(address[] memory _addresses) public onlyOwner {
    ...
    function removeAddressFromWhitelist(address[] memory _addresses) public onlyOwner {
        require(_addresses.length > 0, "_addresses is empty");
        for (uint256 i = 0; i < _addresses.length; i++) {
            address addr = _addresses[i];
