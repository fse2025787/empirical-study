...
contract FourteenDaysFixedAPYStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        stakingToken = IERC20(0xE5e326D12A3644FE71E5585047F0664b4036126E);
        rewardToken = IERC20(0xE5e326D12A3644FE71E5585047F0664b4036126E);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = totalTokensStaked;
    ...
    function deposit(uint256 _amount) public nonReentrant {
        if(holderUnlockTime[msg.sender] == 0){
            holderUnlockTime[msg.sender] = block.timestamp + lockTime;
        }
        PoolInfo storage pool = poolInfo[0];
        ...
        if(_amount > 0) {
            ...
            totalTokensStaked += amountTransferred;
        }
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if(holderUnlockTime[msg.sender] >= block.timestamp){
            _amount -= _amount * exitPenalty / 100;
        }
        holderUnlockTime[msg.sender] = 0;
        pool.lpToken.safeTransfer(address(msg.sender), _amount);
        user.amount = 0;
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)) - totalTokensStaked, 'not enough tokens to take out');
    ...
    function calculateNewRewards() public view returns (uint256) {
        ...
        return (((block.timestamp - pool.lastRewardTimestamp) * totalTokensStaked) * APY / 100 / 365 days);
    }

    function rewardsRemaining() public view returns (uint256){
        return rewardToken.balanceOf(address(this)) - totalTokensStaked;
    }
    ...
    function updatelockTime(uint256 newlockTimeInSeconds) external onlyOwner {
        require(newlockTimeInSeconds <= 30 days, "Lock must be lower than 30 days");
        lockTime = newlockTimeInSeconds;
    }

    function updateExitPenalty(uint256 newPenaltyPerc) external onlyOwner {
        require(newPenaltyPerc <= 20, "May not set higher than 20%");
        exitPenalty = newPenaltyPerc;
    }
}