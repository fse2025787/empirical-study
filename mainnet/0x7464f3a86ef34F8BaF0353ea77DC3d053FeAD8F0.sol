...
contract DODO is IExchange, TokenFetcher {
  using SafeMath for uint256;
  using Address for address;

  uint public swapLimitOverhead = 900;
  address public erc20ApproveProxy;
  ...
  constructor(address _erc20ApproveProxy) public {
  ...
  returns (uint256)
  {
    ...
    uint256 receivedAmount = IDODO(exchange).dodoSwapV1{value: msg.value}(
      address(fromToken),
      address(toToken),
      fromAmount,
      toAmount,
