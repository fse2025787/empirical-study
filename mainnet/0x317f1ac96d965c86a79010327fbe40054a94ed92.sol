...
// File: contracts/jpegSeasonThreeContract.sol
...
contract jpegSeasonThreeContract is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxRegularSupply = 110;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(msg.sender, regularSupply + maxOGSupply + i);
      howManyMinted[msg.sender] = totalMinted + i;
    }
  ...
  function mintOG(uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(msg.sender, OGSupply + i);
      howManyMinted[msg.sender] = totalMinted + i;
    }
