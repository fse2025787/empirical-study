...
contract LouisWolfBySaines is ERC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_PER_WL_ADDRESS = 3;
    uint256 public constant MAX_SUPPLY = 888;
    bool public teamSupplyMinted = false;
    bool public revealed = false;
    bool public paused = false;
    bool public presaleOnly = true;
    bool private provenanceSet = false;
    mapping(address => uint256) public whitelistMintedBalance;

    bytes32 private merkleRootHash;

    // TEAM ADDRESSES
    address private constant TEAM = 0x704e594956F7e966fA46a19cE81d9162547FAA0e;
    address private constant DEV = 0x42cD50b383d6098688BaB8a68a2feCC8e8EB7367; // Change this

    constructor(
        string memory _baseURI,
        string memory _preRevealUri,
        bytes32 _merkleRootHash
        ) ERC721("Louis Wolf by Saines", "LWC") {
        setBaseURI(_baseURI);
        setPreRevealUri(_preRevealUri);
        setMerkleRootHash(_merkleRootHash);
    }
        ...
        mintCheck(_mintAmount)
    {
        ...
        require(!presaleOnly, "Presale mint only");
        address minter = _msgSender();
        require(tx.origin == minter, "Contracts can not mint");
        require(_mintAmount > 0, "Must be a positive number");
        require(_mintAmount <= MAX_PER_TX, "More than max per tx");
    ...
    function presaleMint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
        ...
        mintCheck(_mintAmount)
    {
        require(!paused, "the presale is not open yet");
        require(_mintAmount > 0, "Must be a positive number");

        address minter = _msgSender();
        require(tx.origin == minter, "contracts are not allowed to mint");

        uint256 ownerMintedCount = whitelistMintedBalance[msg.sender];
        require(ownerMintedCount + _mintAmount <= MAX_PER_WL_ADDRESS, "Max whitelist mints exceeded");
    ...
    function mintReserved()
        ...
        onlyOwner
    {
        require(teamSupplyMinted != true, "Team supply already minted.");
        _mintLoop(TEAM, 15);
        _mintLoop(DEV, 10);
        ...
        returns (string memory)
    {
        ...
        if (revealed == false) {
            return bytes(preRevealUri).length > 0
                ? string(
                    abi.encodePacked(
                        preRevealUri,
                        _tokenId.toString(),
    ...
    function setPresaleOnly() external onlyOwner {
    ...
    function setProvenace(string memory _provenance) external onlyOwner {
        require(provenanceSet != true, "Provenance hash already set");
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 team = balance * 95 / 100;
        uint256 dev = balance * 5 / 100;
        _withdraw(TEAM, team);
        _withdraw(DEV, dev);
    }
