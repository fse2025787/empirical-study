...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public constant SHIB = address(0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE);
...
contract $SHEN is ERC20, Ownable, ERC20Burnable {
    using SafeMath for uint256;
    ...
    address private deadWallet = address(0x0000000000000000000000000000000000000000);
    ...
    address payable public marketingWallet = payable(address(0x0000000000000000000000000000000000000001));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SHEN: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "SHEN: The new dividend tracker must be owned by the TOKEN token contract"
        );
    ...
    require(newAddress != address(uniswapV2Router), "SHEN: The router already has that address");
    ...
    function claimStuckTokens(address _token) external onlyOwner {
        require(_token != address(this),"SHEN: No rug pulls");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SHEN: Account is already excluded"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setWallets(address payable _marketing, address _burn ) external onlyOwner {
        require(_marketing != address(0), "SHEN: marketing Wallet can't be a zero address");
    ...
    function setBuyFees(
        ...
        totalBuyFee = buyFee.reward + buyFee.marketing + buyFee.burn + buyFee.autoLP;
        require (totalBuyFee <=20, "SHEN: Max buy fees limit is 20 percent");
    }
    ...
    function setSellFees(
        ...
        totalSellFee = sellFee.reward + sellFee.marketing + sellFee.burn + sellFee.autoLP;
        require(totalSellFee <= 20, "SHEN: Max Sell fees limit is 20 percent");
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SHEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SHEN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "SHEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SHEN: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 shibTokens = contractTokenBalance
                .mul(buyFee.reward + sellFee.reward + buyFee.burn + sellFee.burn)
                .div(totalFees);
            swapForSHIB(shibTokens);
        ...
        if (takeFee) {
            uint256 fees;
            require(amount <= maxTxAmount,"SHEN: Amount exceeds transfer per transaction limit");
            if (!automatedMarketMakerPairs[to]) {
                require(
                    balanceOf(to) + amount <= maxWallet,
                    "SHEN: Balance exceeds Max Wallet limit"
                );
            }
    ...
    function setMaxTx (uint256 amount) external onlyOwner {
        require(amount > (totalSupply().mul(10).div(1000)).div(10**18),"SHEN: No rug pull");
        maxTxAmount = amount * 10**18;
    }

    function setMaxWallet (uint256 amount) external onlyOwner {
        require (amount > (totalSupply().mul(50).div(1000)).div(10**18),"SHEN: No rug pull");
    ...
    function swapForSHIB(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeSHIBDividends(newBalance-burnPart);
...
contract TOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("SHEN_Dividend_Tracker", "SHEN_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "SHEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SHEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TOKEN contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "SHEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "SHEN_Dividend_Tracker: Cannot update claimWait to same value"
        );
