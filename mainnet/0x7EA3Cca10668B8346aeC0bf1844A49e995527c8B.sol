...
contract KongzVX is ERC721Namable, Ownable {

	address constant public KONGZ = address(0x57a204AA1042f6E66DD7730813f4024114d74f37);
	uint256 constant public CAP = 15000;
	uint256 constant public MINTABLE = 14000;
	uint256 constant public BUYABLE = 10000;
	uint256 constant public PRICE = 7 ether / 100;

	uint256[MINTABLE] internal availableIds;
	uint256 public sold;
	...
	mapping(uint256 => uint256) public bebeClaimBitMask;


	constructor(address _banana, string memory _name, string memory _symbol, string[] memory _names, uint256[] memory _ids) public ERC721Namable(_name, _symbol, _names, _ids) {
		_setBaseURI("http://kongz.herokuapp.com/api/metadata-vx");
		banana = _banana;
	}
	...
	function isClaimed(uint256 _tokenId) public view returns (bool) {
		...
		return bebeClaimBitMask[wordIndex] & mask == mask;
	}
	...
	function _setClaimed(uint256 _tokenId) internal{
		...
		uint256 bitIndex = _tokenId % 256;
		uint256 mask = 1 << bitIndex;
		bebeClaimBitMask[wordIndex] |= mask;
	}

	function _getNewId(uint256 _totalMinted) internal returns(uint256 value) {
		uint256 remaining = MINTABLE - _totalMinted;
	...
	// 	uint256 currentMinted = sold + minted;
	// 	uint256 bebeIndex = 0;
	// 	for (uint256 i = 0; i < _tokenIds.length; i++) {
	// 		require(IERC721(KONGZ).ownerOf(_tokenIds[i]) == msg.sender, "KongzVX: !owner of token");
	// 		if (_tokenIds[i] < 1004) {
	// 			require(!_exists(_tokenIds[i]), "KongzVX: Genesis claimed already");
	// 			_mint(msg.sender, _tokenIds[i]);
	// 		}
	// 		else {
	// 			require(!isClaimed(_tokenIds[i]), "KongzVX: Bebe claimed already");
	// 			_setClaimed(_tokenIds[i]);
	// 			_mint(msg.sender, _getNewId(currentMinted + bebeIndex));
	// 			bebeIndex++;
	// 		}
	// 	}
	// 	minted += bebeIndex;
	...
	function claim(uint256 _tokenId) external {
		require(block.timestamp >= startTime && startTime != 0, "KongzVX: Claiming not started");
		require(IERC721(KONGZ).ownerOf(_tokenId) == msg.sender, "KongzVX: !owner of token");
		if (_tokenId < 1001) {
			require(!_exists(_tokenId), "KongzVX: Genesis claimed already");
			_mint(msg.sender, _tokenId);
		}
		else {
			uint256 currentMinted = sold + minted;
			require(!isClaimed(_tokenId), "KongzVX: Bebe claimed already");
			_setClaimed(_tokenId);
			_mint(msg.sender, _getNewId(currentMinted));
			minted++;
		}
	}

	function mint(uint256 _amount) external payable {
		require(block.timestamp >= startTime && startTime != 0, "KongzVX: Minting not started");
		require(sold + _amount <= BUYABLE, "KongzVX: Buyable amount has been reached");
		require(_amount < 4, "KongzVX: Buy limit is 3");
		require(msg.value == PRICE * _amount, "KongzVX: incorrect price");
		require(!_isContract(msg.sender), "KongzVX: Caller cannot be contract");

		uint256 currentMinted = sold + minted;
		for (uint256 i = 0; i < _amount; i++)
			_mint(msg.sender, _getNewId(currentMinted + i));
		sold += _amount;
	}
