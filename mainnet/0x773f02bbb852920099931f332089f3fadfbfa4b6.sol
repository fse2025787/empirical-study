...
contract MonaAvatars is ERC721, Ownable, Pausable {
    ...
    // EVENTS
    event UpdateMonaWallet(address indexed newOwner, address indexed sender);
    event UpdateRoyaltyRate(uint256 newRate, address indexed sender);
    event UnPauseCreatorURIUpdates(address indexed sender);
    ...
    event UpdateTokenURI(uint256 tokenId, string oldUri, string newUri);
    ...
    // @param: _monaWallet: The address where Mona royalties should be paid out (also used for withdrawals).
    constructor(
        string memory _name,
        ...
        updateMonaWallet(_monaWallet);
        updateURIPrefix(_uriPrefix);
    }
    ...
    modifier allowedToMint() {
        require(allowedMinters[msg.sender], "caller not allowed minter");
        _;
    }
    ...
    function _tokenURIPrefixed(string memory _tokenURI) internal view {
        ...
        require(
            newURIBytes.length > prefixLength,
            "Incorrect URI prefix length"
        );
    ...
    // @notice: Mints a new token and assigns URI specific variable.
    // @notice: Once a creator claims are URI it cannot be used again.
    ...
    {
        string storage oldTokenUri = _tokenURIs[_tokenId];
        _tokenURIs[_tokenId] = _tokenURI;
        _tokenURIUsed[_tokenURI] = true;
        emit UpdateTokenURI(_tokenId, oldTokenUri, _tokenURI);
    }
    ...
    function updateCreator(uint256 _tokenId, address _newCreator) public {
        ...
        emit UpdateCreator(_tokenId, _newCreator, msg.sender);
    }
    ...
    function updateURIPrefix(string memory _uriPrefix) public onlyOwner {
        ...
        emit UpdateURIPrefix(_uriPrefix, msg.sender);
    }
    ...
    function pauseCreatorURIUpdates() public onlyOwner {
        ...
        emit PauseCreatorURIUpdates(msg.sender);
    }
    ...
    function unpauseCreatorURIUpdates() public onlyOwner {
        ...
        emit UnPauseCreatorURIUpdates(msg.sender);
    }
    ...
    {
        require(_exists(_tokenId), "nonexistant token");
        royaltyAmount = (_salePrice * royaltyRate) / royaltyRateDivisor;
        receiver = monaWallet == address(0) ? _creators[_tokenId] : monaWallet;
    ...
    function updateMonaWallet(address _newMonaWallet) public onlyOwner {
        ...
        emit UpdateMonaWallet(_newMonaWallet, msg.sender);
    }
    ...
    function updateRoyaltyRate(uint256 _rate) public onlyOwner {
        require(_rate <= 10000, "royalty out of range");
        royaltyRate = _rate;
        emit UpdateRoyaltyRate(_rate, msg.sender);
    }

    // @notice: Set
    // @notice: Only the admin can call this function, addresses and allowances must be same length.
    // @param: addresses: list of addresses to update
    // @param: allowances: list of boolean permissions for addresses
    function updateAllowedMinters(
        address[] calldata addresses,
        bool[] calldata allowances
    ) public onlyOwner {
        require(
            addresses.length == allowances.length,
            "addresses and allowances must be same length"
        );
        for (uint256 i = 0; i < addresses.length; i++) {
            allowedMinters[addresses[i]] = allowances[i];
        }
    }

    // WITHDRAWALS //

    // @notice: Withdraw any Eth in this contract to the Mona Wallet.
    function withdrawEth() public onlyOwner {
        (bool success, ) = payable(monaWallet).call{
            value: address(this).balance
        }("");
        require(success);
    }

    // @notice: Withdraw any ERC20 tokens in this contract to the Mona Wallet.
    // @param: amount: The amount of tokens to withdraw.
    // @param: token: The ERC20 token to withdraw.
    function withdrawERC20(uint256 amount, address token) public onlyOwner {
        bool success = IERC20(token).transfer(monaWallet, amount);
        require(success);
    }
}