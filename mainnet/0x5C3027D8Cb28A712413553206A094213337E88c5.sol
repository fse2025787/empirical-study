...
contract Oracle {
    ...
    function setNewTokensInfo(address[] memory tokensA, address[] memory tokensB) external onlyOwner{
        require(tokensA.length == tokensB.length,"ERR_LENGTH_NOT_MATCH");
        for(uint i;i < tokensA.length; i++){
            this.setNewTokenInfo(tokensA[i],tokensB[i]);
        }
    }

    function setNewTokenInfo(address tokena, address tokenb) external onlyOwner {
        (address tokenA, address tokenB) = tokena < tokenb ? (tokena, tokenb) : (tokenb, tokena);
    ...
    function setPERIOD(uint amount) external onlyOwner {
