...
// File: contracts/texturedstones.sol
...
contract TexturedStones is ERC721A, ERC2981, Ownable, ReentrancyGuard {
    

      uint public constant total_tokens = 12;
      uint public price;
      uint public constant presale_price = 0.001 ether;
      uint public constant publicsale_price = 0.002 ether;
   // string public METADATA_PROVENANCE_HASH = "";

      string public baseURI = "https://www.antonuxio.com/collections/texturedstones/json/";
   ...
   // uint private _curIndex = 1;
    ...
    constructor(address royaltyRecipient, uint96 royaltyPoints) ERC721A("Textured Stones","TST")  {
    ...
    function mint(uint256 numTokens) external payable nonReentrant() {
        ...
        require(totalSupply() + numTokens <= total_tokens, "Not enough pieces remain.");
        require(balanceOf(msg.sender) + numTokens <= 2, "Max 2 pieces per wallet.");
        ...
        if (totalSupply() + numTokens >= total_tokens) {
            saleActive = false;
            isPresaleLive = false;
    ...
    function setDiamondList(address[] calldata addresses) external onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
            diamondList[addresses[i]] = 2; // Max 2 mints per wallet 
        }
    }
    ...
    function claimMany(uint256 numTokens) external onlyOwner {
        require(totalSupply() < total_tokens, "Sale has already ended");
        require(totalSupply() + numTokens <= total_tokens, "Not enough available");
        _safeMint(owner(), numTokens);
    }

    function claimAllRemaining() external onlyOwner {
        // send remaining pieces to owner wallet
        _safeMint(owner(), total_tokens - totalSupply());
        saleActive = false;
    }
    ...
    function giveAway(address recipient) external nonReentrant() onlyOwner {
        require(totalSupply() < total_tokens, "No more pieces");
