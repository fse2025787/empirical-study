...
// File: https://github.com/seifislam123456789/merkle/blob/main/merkleProof.sol
...
contract Gobian is ERC721, ERC2981, Ownable {
  ...
  string public uriPrefix = "ipfs://zzzzz/";
  string public uriSuffix = ".json";
  string public contractURI;
  uint256 public maxSupply = 50;
  uint256 public maxMintAmountPerWallet = 1;
  uint256 public cost = 0.03 ether;
  bytes32 public roott;
  bytes32 public root;


  mapping(address => uint256) public mintedWallets;

  bool public isFreeWhitelistPaused = false;
 ...
 constructor(uint96 _royaltyFeesInBips, string memory _contractURI, bytes32 _roott, bytes32 _root) ERC721("asffgssfg", "ss") {
...
//mint functions
  function FreeWhitlistMint(bytes32[] memory prooff) public {
    uint256 _mintAmount = 1;
    require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
    require(!isFreeWhitelistPaused, "Free whitelist mint is not open yet!");
    require(isValidd(prooff, keccak256(abi.encodePacked(msg.sender))), "Not whitlisted");
  ...
  function isValidd(bytes32[] memory prooff, bytes32 leaff) public view returns (bool) {
    return Merkleprooff.verify(prooff, roott, leaff);
  }
  
  function WhitelistMint(uint256 _mintAmount, bytes32[] memory prooff) public payable {
    require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
    require(!isWhitelistMintPaused, "Whitelist mint is not open sucker!");
    require(isValid(prooff, keccak256(abi.encodePacked(msg.sender))), "Not whitlisted");
  ...
  function PublicMint1() public payable {
    uint256 _mintAmount = 1;
    require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
    require(!isPublicMintpaused, " mint is not open sucker!");
    require(msg.value >= cost , "Insufficient funds!");
    _mintLoop(msg.sender, _mintAmount);
  }
  
  function PublicMint2() public payable {
    uint256 _mintAmount = 2;
    require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
    require(!isPublicMintpaused, " mint is not open sucker!");
    require(msg.value >= cost + 0.01 ether , "Insufficient funds!");
    _mintLoop(msg.sender, _mintAmount);
  }
  function PublicMint3() public payable {
    uint256 _mintAmount = 3;
    require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
    require(!isPublicMintpaused, " mint is not open sucker!");
    require(msg.value >= cost + 0.02 ether , "Insufficient funds!");
    _mintLoop(msg.sender, _mintAmount);
  }
...
//
  ...
  function setisFreeWhitelistPaused(bool _state) public onlyOwner {
    isFreeWhitelistPaused = _state;
  }
