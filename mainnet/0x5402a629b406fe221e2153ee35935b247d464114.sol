...
// File: contracts/WagmiNFTBaby.sol
...
/** 
    * @title WagmiNFT
    * @dev Upgrade for SeedsWagmiNFT, featuring extensions
    * @dev Other features include: optional freeze, royalty on-chain, setBeneficiary
 */
contract WagmiNFTBaby is ERC721, ERC721Enumerable, Ownable {
    ...
    function nextSeason(string calldata uri) public onlyOwner {
        baseURI = uri;
    }

    function seedContract() public view returns (ERC721) {
        return seedsTokens;
    }
    function setSeedContract(ERC721 token) public onlyOwner {
        seedsTokens = token;
    }
    ...
    function mint(uint256 _nbTokens, uint256 _roomId) whenSaleStarted public payable virtual {
        uint256 supply = totalSupply();
        require(supply + _nbTokens <= MAX_SUPPLY - reserved, "Not enough Tokens left.");
        require(_nbTokens * price * 3 <= msg.value, "Inconsistent amount sent!");
        require(_roomId <= 5, "UnknownRoom");
     ...
     function growMint(uint256 tokenId, uint256 _roomId) external payable whenSaleStarted {
        uint256 supply = totalSupply();
        require(supply < MAX_SUPPLY - reserved, "Not enough Tokens left.");
        require(price <= msg.value, "Inconsistent amount sent!");
        require(_roomId <= 5, "UnknownRoom");

        seedsTokens.transferFrom(msg.sender, address(this), tokenId);
