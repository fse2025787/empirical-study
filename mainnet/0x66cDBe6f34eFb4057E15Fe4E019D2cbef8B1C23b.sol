...
// added functionnalities by the wukong's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when WUKONG mining ends.
    uint256 public bonusEndBlock;
    // Deposited amount WUKONG in SmartChef
	...
	uint256 lpSupply = depositedWukong;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 wukongReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accWukongPerShare = accWukongPerShare.add(wukongReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accWukongPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Stake WUKONG tokens to TheBushV1
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedWukong = depositedWukong.add(_amount).sub(depositFee).sub(transferTax);
            }else{
                uint256 transferTax = _amount.mul(wukong.transferTaxRate()).div(10000);
                user.amount = user.amount.add(_amount).sub(transferTax);
                depositedWukong = depositedWukong.add(_amount).sub(transferTax);
            }
        }        
    ...
    // Withdraw WUKONG tokens from STAKING.
