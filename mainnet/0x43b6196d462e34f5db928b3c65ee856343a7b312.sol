...
/**
 * @title ImpactVault128
 * @author @DaoDeCyrus @douglasqian
 * @notice This contract implements a more gas-optimized version of the
 *   impact vault primitive by shifting to using uint128 new token vault standard inspired by
 ...
 *   (ex: ETH) and a non-rebasing yield token (ex: rETH).
 */
abstract contract ImpactVault128 is
    ERC20Upgradeable,
    PausableUpgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    event WithdrawYieldAsset(
        uint128 _amountYieldAsset,
        uint128 _amountAsset,
        address _owner,
        address _receiver
    );
    ...
    function withdrawYieldAsset(
        ...
        emit WithdrawYieldAsset(
            uint128(amountYieldAssetToWithdraw),
            uint128(_amountAsset),
            _owner,
            _receiver
        );
        ...
        returns (uint256)
    {
        ...
        yIndex.totalAssetValueAtLastUpdate = uint128(balanceOf(_address)); // just assets

        emit TransferYieldToManager(
            _address,
            uint128(amountToTransferYieldAsset),
            uint128(amountToTransferUSD)
        );
        // slither-disable-next-line unchecked-transfer
    ...
    function _updateYieldIndexSinceLastUpdate(
        ...
        yIndex.accumulatedYield = uint128(_calculateYield(yIndex));

        // caching here saves a load on line 301
        uint128 newAmount = yIndex.amountYieldAssetAtLastUpdate;
        if (_isDeposit) {
            newAmount += uint128(convertToYieldAsset(_amount));
        } else {
            newAmount -= uint128(convertToYieldAsset(_amount));
        }
        yIndex.amountYieldAssetAtLastUpdate = newAmount;
        // Update total value of yield asset (denominated in asset).
        yIndex.totalAssetValueAtLastUpdate = uint128(convertToAsset(newAmount));
    }
        ...
        returns (uint256)
    {
        unchecked {
            uint256 assetValueNow = convertToAsset(
                uint256(_yIndex.amountYieldAssetAtLastUpdate)
            );
            uint256 yieldSinceLastUpdate = (assetValueNow -
                MathUpgradeable.min(
                    assetValueNow,
                    uint256(_yIndex.totalAssetValueAtLastUpdate)
                ));
            return uint256(_yIndex.accumulatedYield) + yieldSinceLastUpdate;
        }
    }
