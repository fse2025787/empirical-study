...
contract EternalVikingsSacrificer is OwnableUpgradeable {
    ...
    IEternalVikingsYielder public yielder;
    ...
    event EnterAuction(address indexed user, uint256 auctionId, uint256 bid);
    event RedeemAuction(address indexed user, uint256 auctionId, uint256 totalBid);
    ...
    function claimBackAuctionGold(uint256 auctionId) external {
        require(msg.sender != ringHighestBidder[auctionId]);
        require(block.timestamp > ringAuctionDeadline[auctionId]);
        uint256 userBid = ringBids[auctionId][msg.sender];
        require(userBid > 0);
        delete ringBids[auctionId][msg.sender];
        yielder.rewardGold(msg.sender, userBid);
        emit RedeemAuction(msg.sender, auctionId, userBid);
    }
    ...
    function enterAuction(uint256 auctionId, uint256 bid) external {
        ...
        emit EnterAuction(msg.sender, auctionId, bid);
    }
    ...
    function setYielder(address yldr) external onlyOwner {
        yielder = IEternalVikingsYielder(yldr);
    }
}
