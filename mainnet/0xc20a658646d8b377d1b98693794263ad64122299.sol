...
/**
    ...
    */
    contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
        ...
        */
        function _safeMint(
            ...
            mintedTotal = mintedTotal + uint128(quantity);
    ...
    contract LyrraAirdrop is ERC721A, SignerRole {
        using Strings for uint256;
        ...
        uint256 public maxMintAmountPerTx = 1; //100
    
        mapping(address => uint256) public addressMintedBalance;

        constructor() ERC721A("LyrraAirdrop", "LYRRAAIRDROP", maxMintAmountPerTx) {     
        }
        ...
        function BatchMint(address[] calldata _tos) public  mintCompliance(maxMintAmountPerTx) onlySigner {
            
            for (uint256 i = 0; i < _tos.length; ++i) {
                require(_tos[i] != address(0), 'ERC721A: transfer to the zero address');
            }
            for (uint256 i = 0; i < _tos.length; ++i) {
            
            _safeMint(_tos[i],maxMintAmountPerTx);
            }
        }
