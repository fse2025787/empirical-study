...
contract Nutmeg is Initializable, Governable, INutmeg {
    ...
    mapping(address => mapping(address => uint)) mulNut4Borrowers; // pool => adapter => uint
    ...
    function _getWithdrawAmount(Pool memory pool, uint tranche, uint amount) internal pure returns (uint) {
        uint availPrincipal = pool.principals[tranche].sub(pool.loans[tranche]);
    ...
    function borrow(address baseToken, address collToken, uint baseAmt, uint borrowAmt)
        ...
        uint nutStaked = minNut4Borrowers[baseToken][pos.adapter];
            nutStaked = nutStaked.add(mulNut4Borrowers[baseToken][pos.adapter].mul(borrowAmt).div(MULTIPLIER));
        if (nutStaked > 0) {
