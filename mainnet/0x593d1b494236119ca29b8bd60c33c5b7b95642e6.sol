...
/**
    Degen Inu, first Inu launched on DegenSwap
    App: https://degenswap.app/#/swap?chain=mainnet
    Telegram: https://t.me/DegenInuPortal
**/
...
contract DegenInu is Context, IERC20, Ownable {
    ...
    string private _name = "DegenInu";
    string private _symbol = "DINU";
    uint8 private _decimals = 9;
     
    uint256 private _taxFee = 1 * 100;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 private _developmentFee = 4 * 100;
    uint256 private _previousDevelopmentFee = _developmentFee;
    uint256 public _totalTax = _taxFee + _developmentFee;
    ...
    constructor (address _feeAddress) {
        ...
        IDEGENSwapPair(degenSwapPair).updateTotalFee(_totalTax);
    ...
    function setmaxTxAmount(uint256 amount) external onlyOwner() {
        require(amount >= 100000, "Please check MaxtxAmount amount, should exceed 0.1% of the supply");
    ...
    function setFees(uint256 _tax, uint256 _developmentTax) public onlyOwner {
        ...
        _totalTax = _taxFee + _developmentFee;
    ...
    function _transfer(
        ...
        if (cooldownEnabled) {
            if (to != address(degenSwapRouter) && to != address(degenSwapPair) && to != owner()) {
                require(_holderLastTransferBlock[tx.origin] < block.number - 1 && _holderLastTransferBlock[to] < block.number - 1, "_transfer: Cooldown enabled.  Try again later.");
                _holderLastTransferBlock[tx.origin] = block.number;
                _holderLastTransferBlock[to] = block.number;
            }
        }
        
        if(from == degenSwapPair && to != owner()) {
