...
// File: contracts/utils/Signers.sol
...
abstract contract Signers is Context {
    ...
    function _containsSigner(address _signer) internal view virtual returns (bool){
        return signers.contains(_signer);
    }

    function _signersLength() view internal returns (uint256){
        return signers.length();
    }

    function _setupSigner(address _signer) internal virtual {
        _addSigner(_signer);
    }

    function _revokeSigner(address _signer) internal virtual {
...
contract BscBridge is Ownable, Signers {
    ...
    constructor(IERC20 _token, address[] memory _signers) Ownable() public {
    ...
    modifier isSigned(
        ...
        bytes32 _hash = keccak256(abi.encodePacked(eth, msg.sender, _txHash, _amount));
    ...
    function isSigners(address[] memory _signers) public view returns (bool){
        if (_signers.length != _signersLength()) {
            return false;
        }
