    ...
    IERC721ReceiverUpgradeable
{
    ...
    uint256 public constant unlockEmissionOne = 200 ether;
    uint256 public constant lockEmissionTwo = 400 ether;
    uint256 public constant unlockEmissionTwo = 100 ether;
    uint256 public constant lockDurationTwo = 86400 * 84;

    mapping(uint256 => uint256) public stakedLockTokensTimestampsTwo;
    ...
    function initialize(address _paperHandsContract, uint256 _startTime)
        public
        initializer
    {
        __Ownable_init();
    ...
    function viewActiveTokens(address _user)
        external
        view
        returns (uint256[] memory activeTokens)
    {
    ...
    function viewLockTokens(address _user)
        external
        view
        returns (uint256[] memory lockTokens)
    {
    ...
    function withdrawActive() external {
        ...
        uint256 arrLen = stakedActiveTokens[msg.sender].length;
    ...
    function stakeLock(address owner, uint256[] memory tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            stakedLockTokensTimestampsTwo[tokenIds[i]] =
                ((block.timestamp + lockDurationTwo) / 86400) *
                86400;
        }
        ...
        if (block.timestamp <= timestamp1155) {
            ...
            if (quantity > 0) {
                IDiamondHandsPass(diamondHandsPassContract).mint(
                    owner,
                    quantity
                );
            }
    ...
    function withdrawLock() external {
        ...
        for (uint256 i; i < _stakelockTokens.length; i++) {
            if (stakedLockTokensTimestampsTwo[_stakelockTokens[i]] != 0) {
                block.timestamp <
                    stakedLockTokensTimestampsTwo[_stakelockTokens[i]]
        ...
        if (lockArrLength == 0) {
            ...
            for (uint256 i; i < _stakelockTokens.length; i++) {
                if (stakedLockTokensTimestampsTwo[_stakelockTokens[i]] != 0) {
                    delete stakedLockTokensTimestampsTwo[_stakelockTokens[i]];
                }
            }
            ...
            for (uint256 i; i < _stakelockTokens.length; i++) {
                if (stakedLockTokensTimestampsTwo[_stakelockTokens[i]] != 0) {
                    if (
                        ...
                        unlockedTokens[
                            unlockArrLengthConst - unlockArrLength
                        ] = (_stakelockTokens[i]);
                        unlockArrLength = unlockArrLength - 1;
                        delete stakedLockTokensTimestampsTwo[
                            _stakelockTokens[i]
                        ];
                    }
                } else {
    ...
    function _getLockPendingReward(address _user)
        ...
        returns (uint256)
    {
        if (stakedLockTokens[_user].length == 0) return 0;
        uint256 pendingReward;
        uint256 lastUpdatesUserLock = lastUpdates[_user].lock;
        uint256[] memory _stakelockTokens = stakedLockTokens[_user];
        for (uint256 i; i < _stakelockTokens.length; i++) {
            if (stakedLockTokensTimestampsTwo[_stakelockTokens[i]] != 0) {
                uint256 rateChangeTimestamp = stakedLockTokensTimestampsTwo[
                    _stakelockTokens[i]
                ];
                if (block.timestamp < rateChangeTimestamp) {
                    pendingReward +=
                        (lockEmissionTwo *
                            (block.timestamp -
                                (
                                    lastUpdatesUserLock >= startTime
                                        ? lastUpdatesUserLock
                                        : startTime
                                ))) /
                        INTERVAL;
                } else if (
                    block.timestamp >= rateChangeTimestamp &&
                    lastUpdatesUserLock >= rateChangeTimestamp
                ) {
                    pendingReward +=
                        (unlockEmissionTwo *
                            (block.timestamp -
                    ...
                    uint256 amountOne = (lockEmissionTwo *
                        ((rateChangeTimestamp - 1) -
                            (
                                lastUpdatesUserLock >= startTime
                                    ? lastUpdatesUserLock
                                    : startTime
                            ))) / INTERVAL;
                    uint256 amountTwo = (unlockEmissionTwo *
                        (block.timestamp - rateChangeTimestamp)) / INTERVAL;
                    pendingReward += amountOne + amountTwo;
                }
            } else {
                uint256 rateChangeTimestamp = stakedLockTokensTimestamps[
                    _stakelockTokens[i]
                ];
                if (block.timestamp < rateChangeTimestamp) {
                    pendingReward +=
                    ...
                    uint256 amountOne = (lockEmission *
                        ((rateChangeTimestamp - 1) -
                            (
                                lastUpdatesUserLock >= startTime
                                    ? lastUpdatesUserLock
                    ...
                    uint256 amountTwo = (unlockEmissionOne *
                        (block.timestamp - rateChangeTimestamp)) / INTERVAL;
                    pendingReward += amountOne + amountTwo;
                }
            }
    ...
    function isUnlocked(address _user, uint256 _tokenId)
        public
        view
        returns (bool)
    {
        ...
        bool exists;
        for (uint256 i; i < _stakelockTokens.length; i++) {
            if (_stakelockTokens[i] == _tokenId) exists = true;
        }
        require(exists, "TOKEN_ID_NOT_STAKED_BY_USER");
        if (stakedLockTokensTimestampsTwo[_tokenId] != 0) {
            return block.timestamp > stakedLockTokensTimestampsTwo[_tokenId];
        } else {
            return block.timestamp > stakedLockTokensTimestamps[_tokenId];
        }
    }

    function getUnlockTimeStamp(address _user, uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        ...
        require(exists, "TOKEN_ID_NOT_STAKED_BY_USER");
        if (stakedLockTokensTimestampsTwo[_tokenId] != 0) {
            return stakedLockTokensTimestampsTwo[_tokenId];
        } else {
            return stakedLockTokensTimestamps[_tokenId];
        }
    }
