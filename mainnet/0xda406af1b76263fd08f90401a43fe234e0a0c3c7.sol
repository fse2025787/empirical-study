...
contract SendMoon is ERC20, AccessControl {
    using SafeMath for uint256;
      mapping(address => bool) public Maxwalletcheck;
      mapping(address => uint256) public AntiBot;
       mapping(address=>uint256) private cooldown;
    uint8 public interval = 5;
    ...
    uint public txFee = 0;
    uint public maxWallet=1000000000000e18; 
    bool public inSwapAndLiquify = false;

    address public uniswapV2Pair;

    address private marketting_address=_msgSender();
    ...
    constructor() ERC20("SendMoon", "SENDOR") {
        ...
        Maxwalletcheck[marketting_address]=true;
        Maxwalletcheck[address(this)]=true;
        Maxwalletcheck[_msgSender()]=true;
    }
    ...
    function UpdateMaxwalletcheck(address _addr,bool _status) external onlyRole(DEFAULT_OWNER) {
        Maxwalletcheck[_addr]=_status;
    }

     function SetInterval(uint8 _num) external onlyRole(DEFAULT_OWNER) {
        interval=_num;
    }
    ...
    function _transfer(
        ...
        if(!Maxwalletcheck[to]) {
            require(maxWallet >=  balanceOf(to).add(amount), "ERC20: maxWallet >= amount");
          require(cooldown[to] <= block.timestamp,"Cooldown not expire");
         cooldown[to]=(block.timestamp).add(interval);
        }else{
           require(cooldown[from] <= block.timestamp,"Cooldown not expire");  
            cooldown[from]=(block.timestamp).add(interval);
        }
    ...
    function _createPair(address _router) private {
        ...
        _setupRole(PAIR_HASH,uniswapV2Pair);
         Maxwalletcheck[uniswapV2Pair]=true;
         Maxwalletcheck[address(uniswapV2Router)]=true;
    }   
