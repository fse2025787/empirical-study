...
/*
The chameleon is the ultimate cloaker. They are born with the ability to change the pattern and color of their skin, just as we are born with the ability to become powerful lizards even though we are young.
Maybe you don't know why chameleons have the unique ability to change color.
Just as you don't know the unique features of our tokens.
Launch in 10mins.
https://twitter.com/1BabyChameleon
*/
...
library MaxWalletCalculator {
    function calculateMaxWallet(uint256 totalSupply, uint256 babyhatchTime) public view returns (uint256) {
        if(babyhatchTime == 0) {
            return totalSupply;
        }
        ...
        uint256 babycmlnAge = block.timestamp - babyhatchTime;

        uint256 base = totalSupply * 50 / FACTOR_MAX; // 0.5%
        uint256 incrasePerMinute = totalSupply * 10 / FACTOR_MAX; // 0.1%

        uint256 extra = incrasePerMinute * babycmlnAge / (1 minutes); // up 0.1% per minute
...
library Fees {
    ...
    function init(
        Data storage data,
        babycmlnStorage.Data storage _storage,
    ...
    function updateFeeDestinationPercents(Data storage data, babycmlnStorage.Data storage _storage, uint256 dividendsPercent, uint256 marketingPercent, uint256 mysteryPercent) public {
    ...
    function swapAccumulatedFees(Data storage data, babycmlnStorage.Data storage _storage, uint256 tokenAmount) public {
...
library Transfers {
    ...
    function handleTransferWithFees(Data storage data, babycmlnStorage.Data storage _storage, address from, address to, uint256 amount, address referrer) public returns(uint256 fees, uint256 buyerMint, uint256 referrerMint) {        
...
contract babycmln is ERC20, Ownable {
    using SafeMath for uint256;
    using babycmlnStorage for babycmlnStorage.Data;
    using Fees for Fees.Data;
    using BiggestBuyer for BiggestBuyer.Data;
    using Referrals for Referrals.Data;
    using Transfers for Transfers.Data;

    babycmlnStorage.Data private _storage;

    uint256 public constant MAX_SUPPLY = 1000000 * (10**18);

    uint256 private babyhatchTime;

    bool private swapping;
    uint256 public liquidityTokensAvailableToBurn;
    uint256 public liquidityBurnTime;

    babycmlnDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BabyChameleon", "BCMLN") {
        ...
        _storage.dividendTracker = new babycmlnDividendTracker(payable(address(this)), address(_storage.pair), _storage.router.WETH());

        setupDividendTracker();

        _storage.marketingWallet1 = 0xdf873721DAb531aE8E5fE913B16cd626F5c2499d;
        _storage.marketingWallet2 = 0xc82889fc3dd61EAD31e36D4Bd05a687691161ca8;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        _storage.dividendTracker = babycmlnDividendTracker(payable(newAddress));
    ...
    function babyhatch() external onlyOwner {
        require(babyhatchTime == 0);
    ...
    function maxWallet() public view returns (uint256) {
        return MaxWalletCalculator.calculateMaxWallet(MAX_SUPPLY, babyhatchTime);
    }
...
library babycmlnStorage {
    ...
    struct Data {
        ...
        IUniswapV2Pair pair;
        babycmlnDividendTracker dividendTracker;
...
contract babycmlnDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    babycmln public immutable token;
    ...
    */
    ...
    constructor(address payable owner, address pair, address weth) DividendPayingToken("babycmlnDividendTracker", "$CMFLG_DIVS") {
        token = babycmln(owner);
    ...
    function _transfer(address, address, uint256) internal override {
        silenceWarning = true;
        require(false, "babycmlnDividendTracker: No transfers allowed");
    }
    ...
    function updateVestingDuration(uint256 newVestingDuration) public onlyOwner {
        require(newVestingDuration <= 2592000, "babycmlnDividendTracker: max vesting duration is 30 days");
