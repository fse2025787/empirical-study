...
contract GalacticApes is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function migrateGenesisGape(uint256 _tokenId) external nonReentrant {
        uint256 id = _getGalacticApeId(_tokenId);
    ...
    function migrateGalacticApe(uint256 _tokenId) external nonReentrant {
        require(
            GapeContract.ownerOf(_tokenId) == msg.sender,
            "Sender is not the token owner"
        );
        ...
        uint256 tokenId = _tokenId + 151 + 1;
