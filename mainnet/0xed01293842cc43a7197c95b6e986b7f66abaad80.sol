...
contract YCSM is ERC721Enumerable, Ownable {
    
    using SafeMath for uint256;
    
    bool public mintingActive;
    uint256 public mintingPrice = 0.05 ether;
    uint256 public mintingPerTxLimit = 1;
    ...
    constructor(uint256 mintingStart, string memory _uri, address poeToken) ERC721("YCSM", "YCSM") {
        mintingActive = false;
    ...
    function mint(uint256 _amount) public payable {
        ...
        require(totalSupply().add(_amount) <= maxSupply, "This tx woud exceed max supply");
        require(msg.value >= mintingPrice.mul(_amount), "Not enough ether sent");
        require(IERC20(poe).balanceOf(msg.sender)  == 1, "Must Authenticate to mint!");
        require(!hasPurchased(msg.sender), "Cannot buy: You've already purchased!");
