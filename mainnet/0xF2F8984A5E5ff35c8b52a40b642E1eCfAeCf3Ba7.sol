...
contract bBetaToken is ERC20("bBeta", "bBETA"), Ownable {
    uint256 public cap = 20000e18;
    address public bBetaMaster;
    mapping(address => uint) public redeemed;

    uint256 public startAtBlock;
    uint256 public NUMBER_BLOCKS_PER_DAY;
    uint256 constant public DATA_PROVIDER_TOTAL_AMOUNT = 2000e18;
    uint256 constant public AIRDROP_TOTAL_AMOUNT = 2000e18;
    uint256 public dataProviderAmount = DATA_PROVIDER_TOTAL_AMOUNT;
    uint256 public farmingAmount = 16000e18;

    constructor(address _sendTo, uint256 _startAtBlock, uint256 _numberBlockPerDay) public {
        startAtBlock = _startAtBlock;
        NUMBER_BLOCKS_PER_DAY = _numberBlockPerDay == 0 ? 6000 : _numberBlockPerDay;
        _mint(_sendTo, AIRDROP_TOTAL_AMOUNT);
    }
    ...
    function setMaster(address _bBetaMaster) public onlyOwner {
        bBetaMaster = _bBetaMaster;
    }

    function mint(address _to, uint256 _amount) public {
        require(msg.sender == bBetaMaster, "bBetaToken: only master farmer can mint");
        if (_amount > farmingAmount) {
    ...
    function burn(uint256 _amount) public {
        require(redeemed[msg.sender] + _amount <= 1e18, "bBetaToken: cannot burn more than 1 bBeta");
    ...
    function mintForDataProvider(address _to) public onlyOwner {
        require(block.number >= startAtBlock + 14 * NUMBER_BLOCKS_PER_DAY, "bBetaToken: Cannot mint at this time");
        require(dataProviderAmount > 0, "bBetaToken: Cannot mint more token for future farming");

        _mint(_to, dataProviderAmount);
        dataProviderAmount = 0;
    }
}

// File: contracts/bBetaMaster.sol
...
contract bBetaMaster is Ownable {
    using SafeMath for uint256;
    ...
    bBetaToken public bBeta;
    
    uint256 public REWARD_PER_BLOCK;
    uint256[] public REWARD_MULTIPLIER = [222, 211, 200, 189, 178, 167, 156, 144, 133, 122, 111, 100, 0];
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(address(_lpToken) != address(0x0), "bBetaMaster::add: wrong lptoken");
        require(poolId1[address(_lpToken)] == 0, "bBetaMaster::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        bBeta.mint(address(this), reward);
    ...
    function safeTransferReward(address _to, uint256 _amount) internal {
        uint256 bal = bBeta.balanceOf(address(this));
        if (_amount > bal) {
            bBeta.transfer(_to, bal);
        } else {
            bBeta.transfer(_to, _amount);
        }
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint) {
        ...
        uint256 amountCanMint = bBeta.farmingAmount();
