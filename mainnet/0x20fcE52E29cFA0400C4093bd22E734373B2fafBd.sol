...
contract Tamagogi is Ownable, ERC721A, ReentrancyGuard, TamagogiDrawer {
    ...
    constructor() ERC721A("Tamagogi", "TMGG") {
        config.price = 20000000000000000;
        config.propMaxSupply = 2000; // 45% (food) 45% (toy) 10% (shield)
        config.petMaxSupply = 1825; // 365 * 5 (4 pets born every day)
    ...
    function merkleHatchEgg(bytes32[] calldata _proof) external payable {
        ...
        require(MerkleProof.verify(_proof, rootHash, leaf), "invalid");
        _hatchEgg();
    }
    ...
    function getPetHungerAndBored(uint tokenId) public view revealOnly() petOnly(tokenId) returns(uint, uint) {
        uint _now = block.timestamp;
        TMGG memory tmgg = TMGGs[tokenId];
        uint hunger = dateTimeContract.diffHours(tmgg.lastFeed, _now);
        uint bored = dateTimeContract.diffHours(tmgg.lastPlay, _now);
    ...
    function _tokenURI(uint256 tokenId) private view validToken(tokenId) returns (string memory) {
        string memory json = tokenId <= config.propMaxSupply ? config.reveal ? _getPropsMetadata(tokenId) : _getPropsUnrevealMetadata(tokenId) : config.reveal ? _getPetsMetadata(tokenId) : _getPetsUnrevealMetadata(tokenId);
