...
contract PriceModuleV7 is ChainlinkService, Initializable {
    ...
    function getUSDPrice(address _tokenAddress) public view returns (uint256) {
        ...
        if (tokens[_tokenAddress].tokenType == 1) {
            ...
            return IConvex(tokens[_tokenAddress].feedAddress).get_virtual_price();  // get USD Price from pool contract
        } 
        else if (tokens[_tokenAddress].tokenType == 13) {
            uint256 priceInEther = IConvex(tokens[_tokenAddress].feedAddress).get_virtual_price();
            uint256 etherToUSD = getUSDPrice(address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE));
            return (priceInEther.mul(etherToUSD)).div(1e18);
        }
        else if (tokens[_tokenAddress].tokenType == 14) {              //similar to type 8
             uint256 lpPriceBTC=IConvex(tokens[_tokenAddress].feedAddress).get_virtual_price();
             uint256 btcToUSD = getUSDPrice(
                address(0xF4030086522a5bEEa4988F8cA5B36dbC97BeE88c) // Address representing BTC.
            );
            return (lpPriceBTC.mul(btcToUSD)).div(1e18);
