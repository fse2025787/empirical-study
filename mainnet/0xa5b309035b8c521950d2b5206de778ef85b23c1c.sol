...
// @title Gangster Sapiens NFT - Official Smart Contract
// @authors Gangster Sapiens Development Team
// @notice This contract provides 6666 Gangster Sapiens NFTs
...
contract GangsterSapiens is ERC721Enumerable, Ownable{
    using Strings for uint256;
    ...
    bool private revealSingle = true;
    string public contractURI;
    bool public paused = true;
    bool public revealed = false;

 
    uint256 public mintingPrice = 0.08 ether;
    ...
    uint256 public airdropSize = 0;
    mapping(address => bool) public airdropAllowed;
    ...
    event mintAirdropCalled(address indexed caller, address indexed receiver, uint256 tokenID);
    ...
    constructor(string memory _newBaseURI, string memory _newRevealURI, string memory _newContractURI) ERC721("Gangster Sapiens", "GSAP") {
        tokenCounter = 0;
        baseURI = _newBaseURI;
    ...
    function mintNFT(address _to, uint256 _mintAmount) public payable {
        ...
        if(msg.sender != owner()) {
            require(msg.value >= mintingPrice * _mintAmount, "Payed Ether is too less for minting.");
        }   
    ...
    function mintWhitelistNFT(address _to, uint256 _mintAmount) public payable {
        ...
        require(whitelistActive == true, "Whitelistminting not activated");
        require(tokenCounter + _mintAmount <= maxSupply, "Maximum of possible NFTs is reached");
        require(_mintAmount > 0, "At least one token must be minted");
        require(whitelist[msg.sender], "You are not whitelisted!");
 
        if(msg.sender != owner()) {
            require(msg.value >= whitelistPrice * _mintAmount, "Payed Ether is too less for minting.");
        }   
        ...
        returns (string memory)
    {
        ...
        if(revealed) {
            ...
            if(revealSingle) {
                return string(abi.encodePacked(revealURI, _tokenId.toString(), baseExtension));
            } else {
    ...
    function switchRevealSingle() public onlyOwner {
        revealSingle = !revealSingle;
    }
    ...
    function whitelistImport(address[] memory accounts) public onlyOwner returns (uint256) {
    ...
    function airdopNFT(address[] memory _list) public onlyOwner {
    ...
    function mintAirdrop() public {
        ...
        require(tokenCounter + 1 <= maxSupply, "Maximum of possible NFTs is reached");
        require(!airdropClaimed[msg.sender], "You have already claimed your Airdrop"); 
        require(airdropAllowed[msg.sender], "Your are not allowed for the Airdrop"); 

        tokenCounter++;
        airdropClaimed[msg.sender] = true;
        _safeMint(msg.sender, tokenCounter);
        emit mintNFTCalled(msg.sender, msg.sender, tokenCounter);
    }


        function airdropImport(address[] memory accounts) public onlyOwner returns (uint256) {
        uint256 taskCounter = 0;
        for (uint256 i = 0; i < accounts.length; i++) {
            if(!airdropAllowed[accounts[i]]) {
                airdropAllowed[accounts[i]] = true;
                airdropSize++;
                taskCounter++;
            }
        }
