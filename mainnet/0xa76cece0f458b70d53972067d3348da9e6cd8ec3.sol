...
contract BlootMoji is ERC721Enumerable, ReentrancyGuard, Ownable {
    uint256 public constant PUBLIC_MINT_PRICE = 10000000000000000; //0.01 ETH
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Moji #',
                        toString(tokenId),
                        '", "description": "Bloot Moji describes an abstract person. Who are you going to be?", "image": "data:image/svg+xml;base64,',
    ...
    function blootOwnerFreeClaim() public nonReentrant {
        require(blootOwnersSupply < 8008, "No more bloot owner claims");
    ...
    function publicClaim() public nonReentrant payable {
        require(publicOwnersSupply < 1950, "No more tokens to mint");
        require(totalSupply() < maxSupply, "No more tokens to mint");
        require(msg.value >= PUBLIC_MINT_PRICE, "Payment too low, try .008");
        uint index = totalSupply();
    ...
    function ownerClaim() public nonReentrant onlyOwner {
        require(creatorSupply < 42, "No more tokens to mint");
        require(totalSupply() < maxSupply, "No more tokens to mint");
        uint index = totalSupply();
        creatorSupply = creatorSupply + 1;
        _safeMint(_msgSender(), index);
    }

    function toString(uint256 value) internal pure returns (string memory) {
        // Inspired by OraclizeAPI's implaementation - MIT license
        // https://github.com/oraclize/ethereum-api/blob/b42146b063c7d6ee1358846c198246239e9360e8/oraclizeAPI_0.4.25.sol
    ...
    constructor() ERC721('Bloot Moji', 'Bloot Moji') Ownable() {}
}
