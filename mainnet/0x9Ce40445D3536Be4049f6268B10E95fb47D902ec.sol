...
library Blueprint {
    // Input format: "name,issueNumber" as hex
    // name,issuenumber must be unique
    function deserializeBlueprint(bytes memory blueprint)
        internal
        pure
        returns (string memory, uint256)
    {
        string[] memory tokenBlueprintParts = StringUtils.split(
            string(blueprint),
            ","
        );
        require(
            tokenBlueprintParts.length == 2,
            "Invalid token blueprint count"
        );
        string memory name = tokenBlueprintParts[0];
        uint256 issueNumber = uint256(
            StringUtils.toUint(tokenBlueprintParts[1])
        );
        return (name, issueNumber);
    }

    function serializeBlueprint(string memory name, uint256 issueNumber)
        internal
        pure
        returns (bytes memory)
    {
    ...
    UUPSUpgradeable
{
    ...
    function safeMint(
        ...
        (string memory name, uint256 issueNumber) = Blueprint
            .deserializeBlueprint(blueprint);
        require(
            tokenBlueprintToTokenId[blueprint] == 0,
            "Token blueprint already minted"
        );
        _safeMint(to, tokenId);
        tokenBlueprintToTokenId[blueprint] = tokenId;
        tokenNames[tokenId] = name;
        tokenIssueNumbers[tokenId] = issueNumber;
    }
    ...
    function burn(uint256 tokenId) public onlyRole(BURNER_ROLE) {
        ...
        bytes memory tokenBlueprint = Blueprint.serializeBlueprint(
            name,
            issueNumber
        );
        require(
