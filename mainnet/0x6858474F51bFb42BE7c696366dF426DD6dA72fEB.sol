...
contract TheVisitors is ERC721A, Ownable, ReentrancyGuard {

  using Strings for uint256;

  

  string public uriPrefix = "ipfs://QmdzApMSJkjzsTVSzzy65pt7w2oLdWBH9odbJkoNYtpLeP/";
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) nonReentrant  {
    ...
    if(totalSupply() > freeSupply)
    {
         require(msg.value >= cost * _mintAmount, "Insufficient funds!");
          require(PaidaddressMintedBalance[msg.sender] + _mintAmount <= nftPerAddressLimit, "Max mint amount per address exceeded!"); 
        PaidaddressMintedBalance[msg.sender] += _mintAmount;
    }
    else
    {
         require(FreeaddressMintedBalance[msg.sender] + _mintAmount <= nftPerAddressLimit, "Max mint amount per address exceeded!"); 
