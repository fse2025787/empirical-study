...
,ERC20Payment
{
	...
	EnumerableMap.UintToAddressMap internal _tokenCreators;
    ...
    function _creators(uint256 tokenId) internal view virtual returns (address) {
    ...
    function _endAuction(uint256 _tokenId) internal virtual {
		...
		address payable creator = payable(_tokenCreators.get(_tokenId));
		...
		uint _royaltyValue    = bid.value * royalty / 1000; // 로얄티 10%
		uint _sellerValue = bid.value - _commissionValue - _royaltyValue;
		...
		if(offer.tokenAddress == address(0)){
			...
			creator.transfer(_royaltyValue);//NFT 창작자에게 로열티 지급
		}else{
			TransferHelper.safeTransfer(
			...
			TransferHelper.safeTransfer(
				offer.tokenAddress, creator, _royaltyValue
			);
		}
...
ERC721Burnable
{
	...
	function _trade(uint _id) internal {
        address payable contractOwner = payable(_owner);
		address payable creator = payable(_creators(_id));
		...
		uint _royaltyValue    = price[_id] * royalty / 1000; // 로얄티 10%
		uint _sellerValue = price[_id] - _commissionValue - _royaltyValue;
		...
		if(saleTokenAddresses[_id] == address(0)){
			...
			creator.transfer(_royaltyValue);
			...
			TransferHelper.safeTransferFrom(
				saleTokenAddresses[_id], msg.sender, creator, _royaltyValue
			);
		}
