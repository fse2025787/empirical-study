...
contract KernelAppIds {
    ...
    */
    bytes32 internal constant KERNEL_CORE_APP_ID =
        0x3b4bf6bf3ad5000ecf0f989d5befde585c6860fea3e574a4fab4c49d1c177d9c;
    bytes32 internal constant KERNEL_DEFAULT_ACL_APP_ID =
        0xe3262375f45a6e2026b7e7b18c2b807434f2508fe1a2a3dfb493c7df8f4aad6a;
    bytes32 internal constant KERNEL_DEFAULT_VAULT_APP_ID =
...
contract AppProxyFactory {
     ...
     */
    function newAppProxy(IKernel _kernel, bytes32 _appId)
        public
        returns (AppProxyUpgradeable)
    {
     ...
     * @return AppProxyUpgradeable
     */
    function newAppProxy(
        IKernel _kernel,
        bytes32 _appId,
        bytes _initializePayload
    ) public returns (AppProxyUpgradeable) {
        AppProxyUpgradeable proxy = new AppProxyUpgradeable(
            _kernel,
            _appId,
            _initializePayload
        );
     ...
     */
    function newAppProxyPinned(IKernel _kernel, bytes32 _appId)
        public
        returns (AppProxyPinned)
    {
     ...
     * @return AppProxyPinned
     */
    function newAppProxyPinned(
        IKernel _kernel,
        bytes32 _appId,
        bytes _initializePayload
    ) public returns (AppProxyPinned) {
        AppProxyPinned proxy = new AppProxyPinned(
            _kernel,
            _appId,
            _initializePayload
        );
...
contract ACL is IACL, TimeHelpers, AragonApp, ACLHelpers {
    ...
    */
    bytes32 public constant EMPTY_PARAM_HASH =
    ...
    modifier onlyPermissionManager(address _app, bytes32 _role) {
        require(
            msg.sender == getPermissionManager(_app, _role),
            ERROR_AUTH_NO_MANAGER
        );
        _;
    }
    ...
    modifier noPermissionManager(address _app, bytes32 _role) {
        ...
        require(
            getPermissionManager(_app, _role) == address(0),
            ERROR_EXISTENT_MANAGER
        );
        _;
    }
    ...
    function grantPermissionP(
        ...
        bytes32 paramsHash = _params.length > 0
            ? _saveParams(_params)
            : EMPTY_PARAM_HASH;
    ...
    function getPermissionParamsLength(
        ...
        return
            permissionParams[permissions[permissionHash(_entity, _app, _role)]]
                .length;
    }
    ...
    {
        Param storage param = permissionParams[
            permissions[permissionHash(_entity, _app, _role)]
        ][_index];
     ...
     */
    function getPermissionManager(address _app, bytes32 _role)
        public
        view
        returns (address)
    {
    ...
    function hasPermission(
        ...
        return
            hasPermission(
                ...
                _where,
                _what,
                ConversionHelpers.dangerouslyCastBytesToUintArray(_how)
            );
    }
    ...
    function hasPermission(
        ...
        if (
            whoParams != NO_PERMISSION &&
            evalParams(whoParams, _who, _where, _what, _how)
        ) {
            return true;
        }

        bytes32 anyParams = permissions[
            permissionHash(ANY_ENTITY, _where, _what)
        ];
        if (
            anyParams != NO_PERMISSION &&
            evalParams(anyParams, ANY_ENTITY, _where, _what, _how)
        ) {
            return true;
        }
    ...
    function _setPermission(
        ...
        bool permissionHasParams = entityHasPermission &&
            _paramsHash != EMPTY_PARAM_HASH;
    ...
    function _evalLogic(
        ...
        if (Op(_param.op) == Op.IF_ELSE) {
            ...
            (conditionParam, successParam, failureParam) = decodeParamsList(
                uint256(_param.value)
            );
            bool result = _evalParam(
                _paramsHash,
                conditionParam,
                _who,
                _where,
            ...
            return
                _evalParam(
                    _paramsHash,
                    result ? successParam : failureParam,
                    _who,
                    _where,
                    _what,
                    _how
    ...
    function roleHash(address _where, bytes32 _what)
        internal
        pure
        returns (bytes32)
    {
    ...
    ACLSyntaxSugar
{
    ...
    */
    bytes32 public constant APP_MANAGER_ROLE =
        0xb6d92708f3d4817afc106147d969e229ced5c46e65e0a5002a0d391287762bd0;

    string private constant ERROR_APP_NOT_CONTRACT = "KERNEL_APP_NOT_CONTRACT";
    string private constant ERROR_INVALID_APP_CHANGE =
    ...
    function setApp(
        ...
        bytes32 _appId,
        address _app
    ) public auth(APP_MANAGER_ROLE, arr(_namespace, _appId)) {
    ...
    function setRecoveryVaultAppId(bytes32 _recoveryVaultAppId)
        ...
        auth(
            APP_MANAGER_ROLE,
            arr(KERNEL_APP_ADDR_NAMESPACE, _recoveryVaultAppId)
        )
    {
     ...
     */
    function getApp(bytes32 _namespace, bytes32 _appId)
        public
        view
        returns (address)
    {
    ...
    function acl() public view returns (IACL) {
        return
            IACL(getApp(KERNEL_APP_ADDR_NAMESPACE, KERNEL_DEFAULT_ACL_APP_ID));
    }
    ...
    function hasPermission(
        ...
        IACL defaultAcl = acl();
        return
            address(defaultAcl) != address(0) && // Poor man's initialization check (saves gas)
    ...
    AragonApp
{
    ...
    */
    bytes32 public constant REGISTRY_ADD_EXECUTOR_ROLE =
    ...
    string private constant ERROR_EXECUTOR_DISABLED =
        "EVMREG_EXECUTOR_DISABLED";
    string private constant ERROR_SCRIPT_LENGTH_TOO_SHORT =
        "EVMREG_SCRIPT_LENGTH_TOO_SHORT";
    ...
    modifier executorExists(uint256 _executorId) {
        require(
            _executorId > 0 && _executorId < executorsNextIndex,
     ...
     */
    function addScriptExecutor(IEVMScriptExecutor _executor)
        external
        auth(REGISTRY_ADD_EXECUTOR_ROLE)
        returns (uint256 id)
    {
     ...
     */
    function getScriptExecutor(bytes _script)
        public
        view
        returns (IEVMScriptExecutor)
    {
        require(
            _script.length >= SCRIPT_START_LOCATION,
            ERROR_SCRIPT_LENGTH_TOO_SHORT
        );
...
contract AppProxyPinned is IsContract, AppProxyBase {
    ...
    bytes32 internal constant PINNED_CODE_POSITION =
