...
contract PaperDAONameResolver is Ownable {
    ...
    function addr(bytes32 node) external returns (address) {
        address stakingOwner = stakingContract.stakedTokens(uint256(node)).owner;

        if (address(stakingOwner) != address(0)) {
            return stakingOwner;
        }
        return tokenContract.ownerOf(uint256(node));
    }

    function _ownerOf(bytes32 node) private returns(address) {
        return tokenContract.ownerOf(uint256(node));
    }
