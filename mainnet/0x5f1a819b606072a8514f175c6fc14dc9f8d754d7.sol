...
contract DogePredator is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 10**21 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    address payable public _marketingAddress =
        payable(address(0x003B550713C5c9DdECEB2dA137711Db6EaF0b400));
    address payable public _devwallet =
        payable(address(0x003B550713C5c9DdECEB2dA137711Db6EaF0b400));

    string private _name = "Doge Predator";
    string private _symbol = "FOM";
    ...
    constructor(uint256 tokenSupply, uint256 sniperBlockNumber) {
        _tTotal = tokenSupply * 10**_decimals; // Put without decimals

        _maxTxAmount = _tTotal.mul(5).div(10**3); // 0.5% Of supply
        _maxWalletSize = _tTotal.mul(10).div(10**3); // 1% Of supply
        numTokensSellToAddToLiquidity = _tTotal.mul(5).div(10**3); // 0.5% Of supply

        guestBlock = sniperBlockNumber; // Amount of block antisniper active
    ...
    function setMaxWalletSizePercent(uint256 maxWalletSize)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (_launchTime == 0 && to == uniswapV2Pair) {
            _launchTime = block.number;
        }
        
        if (block.number < _launchTime.add(guestBlock) && from == uniswapV2Pair) {
            _isBlackListedBot[to] = true;
            _blackListedBots.push(to);
        }
