...
// File: Moontownwtf.sol

//We're just having fun

contract MoonTownWtf is Ownable, ERC721A, ReentrancyGuard {
    uint256 public immutable maxPerWallet = 50;
    uint256 public immutable maxFree = 2000;

    uint256 public immutable devMints;
    uint256 public immutable maxPerTx;
    uint256 public immutable actualCollectionSize;

    uint256 public publicMintPrice = .0069 ether;
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        ...
        uint cost = publicMintPrice;

        if (totalSupply() + quantity < maxFree + 1){
            cost = 0;
        }
        require(
        ...
        require(
            ...
            "can not mint this many .. you have too many goblins"
        );
        require(quantity <= maxPerTx, "you are trying to mint too many, can only mint 5 at a time");
        require(quantity * cost == msg.value, "enter exact amount quantity * mintprice");
        require(publicSaleActive, "can not mint yet .. wait for the moon");
