...
contract SolanaBurn is Context, IERC20, Ownable {
    ...
    string private constant _name = "SolanaBurn"; // 
    string private constant _symbol = unicode"SOLB"; //    
    ...
    function _transfer(
        ...
        if (balanceOf(address(this)) >= TaxSwapAmount && !swapping && from != uniswapV2Pair) {
            swapping = true;
            uint256 checkBalance = balanceOf(address(this));
            swapAndSendToFee(checkBalance);
            swapping = false;
        }
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(0x701383bA12Cb6aEa3cB522f4d0166FC410B17324).transfer(ethToSend); //marketing wallet
    }
