...
contract Bonk is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A(".Bonk Name Service", ".bonk") {
        tokenIDandAddress[_currentIndex]="bonk";
        tokenAddressandID["bonk"]=_currentIndex;
        resolveAddress["bonk"]=msg.sender;
    ...
    function setAddress(string calldata bonk_name, address newresolve) external {
         TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[bonk_name]);
        if (Ownership.addr != msg.sender) revert("Error");
        

    bytes memory result = bytes(primaryAddress[resolveAddress[bonk_name]]);
        if (keccak256(result) == keccak256(bytes(bonk_name))) {
            primaryAddress[resolveAddress[bonk_name]]="";
        }
        resolveAddress[bonk_name]=newresolve;
    }

    function setPrimaryAddress(string calldata bonk_name) external {
        require(resolveAddress[bonk_name]==msg.sender, "Error");
    ...
    function namediff(uint256 tokenId , string calldata new_bonk_name) external onlyOwner {
        tokenIDandAddress[tokenId]=new_bonk_name;
        tokenAddressandID[new_bonk_name]=tokenId;
    }
...
function lastAddresses(uint256 count)
 ...
 function _checkName(string memory _name) public view returns(bool){
    ...
    function withdraw() public onlyOwner nonReentrant {
        uint256 balance = address(this).balance;
        Address.sendValue(payable(0x0c45E0161d6bc315BcaD2F592F72762034Ec3C3D),balance);
        }
