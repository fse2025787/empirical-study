...
contract OMW is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    address deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private _name = "OMW";
    string private _symbol = "OMW";
    uint8 private _decimals = 9;    
    uint256 private initialsupply = 1_000_000_000_000;
    ...
    function setFeePercent(uint256 liquidityFee, uint256 marketingFee, uint256 taxFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
        _marketingFee = marketingFee;
        _taxFee = taxFee;
        require((liquidityFee.add(marketingFee).add(taxFee)) <= 20);
    }    
