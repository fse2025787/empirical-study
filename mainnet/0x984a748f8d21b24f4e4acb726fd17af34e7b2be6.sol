...
contract Evadore is Context, IERC20, Ownable {
	...
	uint256 public airdropBalance = 0;
    ...
    function setWalletfeeInc(address _account, bool _status) public onlyOwner {
    ...
    function addTokenforAirdropBalance(uint256 _amount) public onlyOwner{
        require(_amount>0,"Need amount than 0 bigger");
        IERC20(address(this)).transferFrom(msg.sender,address(this),_amount);
        airdropBalance = airdropBalance.add(_amount);
        airdropStatus = true;
    }

    function refundTokenforAirdropBalance() public onlyOwner{
        require(airdropBalance>0,"Need amount than 0 bigger");
        IERC20(address(this)).transfer(owner(),airdropBalance);
        airdropBalance = 0;
        airdropStatus = false;
    }
    ...
    function sendAirdrop(address[] memory _wallets , uint256 _balance) public onlyOwner{
        require(airdropBalance>0,"no balance for airdrop");
        require(airdropStatus,"Airdrop not Active");

        IERC20 token = IERC20(address(this));

        for(uint a=0; a<_wallets.length; a++){
            if(airdropBalance > _balance){
                token.transfer(_wallets[a],_balance);
            }
            airdropBalance = airdropBalance.sub(_balance);
        }
    }
}