...
/**
TG: https://t.me/BLACKFLOKIportal
Twitter: https://twitter.com/BlackFlokiEth
Every floki needs a color, black is his most mysterious color $BlackFloki
Tax: 2% Burn
*/
// 
...
contract BlackFloki is ERC20, Ownable {
    using SafeMath for uint256;

    string private name_ = "BlackFloki";
    string private symbol_ = "BlackFloki";
    uint256 private totalSupply_ = 100000000;

    uint256 public deadFee = 1;
    uint256 public marketingFee = 1;

    address public marketingWalletAddress = 0x000000000000000000000000000000000000dEaD;
    ...
    function setDeadFee(uint256 fee) public onlyOwner {
        deadFee = fee;
    }
    ...
    function _transfer(
        ...
        uint256 burnAmount = amount.mul(deadFee).div(100);
        uint256 marketingAmount = amount.mul(marketingFee).div(100);
        uint256 transferAmount = amount.sub(burnAmount).sub(marketingAmount);

        if(burnAmount > 0) super._transfer(from, deadWallet, burnAmount);
        if(marketingAmount > 0) super._transfer(from, marketingWalletAddress, marketingAmount);
