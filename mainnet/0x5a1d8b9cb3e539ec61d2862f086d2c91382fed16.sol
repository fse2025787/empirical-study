  ...
  */
  function buyPYE(uint16 tokenId) external payable whenNotPaused nonReentrant returns(uint8) {
    ...
    */
    ...
    if(pyeFilling[1] > 0) { TOPIAToken.mint(msg.sender, pyeFilling[boughtPYE]); }
    vrf.transfer(msg.value);
   ...
   */
    ...
    function getUnclaimedGenesis(uint16 tokenId) external view returns (uint256 owed) {
        ...
        } else if (genesisType[tokenId] == 2 && foodie[tokenId].owner != address(0) && foodie[tokenId].value > 0) {
            return TOPIAPerFoodie - foodie[tokenId].value;
        } else if (genesisType[tokenId] == 3 && shopOwner[tokenId].owner != address(0) && shopOwner[tokenId].value > 0) {
   ...
   */
    ...
    function updatePYECost(uint8 _cost) external onlyOwner {
        PYE_COST = _cost;
    }
