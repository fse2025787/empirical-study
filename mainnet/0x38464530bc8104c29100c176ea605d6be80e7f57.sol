...
contract CosmicFusionVX is ERC721, Ownable {
    ...
    CosmicFusion CosmicFusionContract = CosmicFusion(0x362BA0425Fa90138e9189Ed369b1ea4827Eb43D1);

    mapping (uint256 => bool) public fusionClaimed;

    constructor(string memory _initBaseURI ) ERC721("Cosmic Fusion VX", "FUSIONVX") 
    {
        setBaseURI(_initBaseURI);
    }
    ...
    function claimVX(uint256[] memory tokenIDs) public onlySender 
    {
        for(uint256 i; i<tokenIDs.length; i++)
        {
            if(msg.sender != CosmicFusionContract.ownerOf(tokenIDs[i]))
            {
                revert notAuthorizedToClaim();
            }

            fusionClaimed[tokenIDs[i]] = true;  
    ...
    {
        return address(CosmicFusionContract);
    }
