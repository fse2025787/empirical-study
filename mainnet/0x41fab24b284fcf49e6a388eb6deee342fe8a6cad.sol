...
contract TCERC1155 is Initializable, ERC1155Upgradeable, AccessControlUpgradeable, ERC1155BurnableUpgradeable {
    bytes32 public constant URI_SETTER_ROLE = keccak256("URI_SETTER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant SET_COMMON_ADDRESS_ROLE = keccak256("SET_COMMON_ADDRESS_ROLE");
    string contractMetadataURI;
    address public commonAddressRecv;

    
    constructor() initializer {}

    function initialize(string calldata uri_, string calldata contractMetadataURI_, address admin_, address commonAddressRecv_) initializer public {
        require(admin_!=address(0),"admin cannot be empty!");
        ...
        _setupRole(URI_SETTER_ROLE, admin_);
        _setupRole(MINTER_ROLE, admin_);
        _setupRole(SET_COMMON_ADDRESS_ROLE, admin_);
        
        contractMetadataURI = contractMetadataURI_;
        commonAddressRecv = commonAddressRecv_;
    }

    function setURI(string memory newuri) public onlyRole(URI_SETTER_ROLE) {
        _setURI(newuri);
    }
    function setCommonAddress(address newCommonAddressRecv) public onlyRole(SET_COMMON_ADDRESS_ROLE) {
        commonAddressRecv = newCommonAddressRecv;
    }
    ...
    function mint(address account, uint256 id, uint256 amount, bytes memory data)
        public
    {
        require((account==commonAddressRecv&&account!=address(0))||hasRole(MINTER_ROLE, _msgSender()), "Error: must have minter role to mint");
        ...
        public
    {
        ...
        require((to==commonAddressRecv&&to!=address(0))||hasRole(MINTER_ROLE, _msgSender()), "Error: must have minter role to mint");
