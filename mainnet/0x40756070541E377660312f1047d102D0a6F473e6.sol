    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier onlyAdmin() {
        require(
             hasRole(GOVERNANCE_ROLE, msg.sender) ||
                hasRole(DEFAULT_ADMIN_ROLE, msg.sender),  "only governor"
        );
        _;
    }
...
contract AliveFixedSongMultiUri is AliveC {
...
mintStopTime = _stopTime;
    ...
    function initialize(
        ...
        __ERC721A_init("Dragaan (Never Again)", "AE1");
        _grantRole(DEFAULT_ADMIN_ROLE, _artist);
        _grantRole(GOVERNANCE_ROLE, _artist);
        _grantRole(GOVERNANCE_ROLE, _aliveRoyaltyReceiver);
        _grantRole(PAUSER_ROLE, _artist);
        _grantRole(PAUSER_ROLE, _aliveRoyaltyReceiver);
    ...
    {   require(block.timestamp > startedAt);
        ...
        uint256 feePercent = 20;
        uint256 feePrice = (price * feePercent) / 100;
        payable(aliveRoyaltyReceiver)
            .transfer(feePrice);
      ...
      function setURIExclusive(string memory _uri) public onlyAdmin {
        baseuri2 = _uri;
     }
