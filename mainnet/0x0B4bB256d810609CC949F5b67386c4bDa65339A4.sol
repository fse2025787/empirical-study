...
/*
A DEGENS PRESENT ($PRSNT)

Merry Christmas and Happy Holidays, you degens. $PRSNT is here to reveal a new state of the art Smart Contract on ERC20.
$PRSNT is designed to collect transaction fees in the form of $ETH and use the funds to buyback. But this isn't any normal buyback.
The $PRSTN Smart Contract will be using 100% of its stored value to buyback $PRSNT on the open market, call it a "Present" for all of 
the Degens! The enormous buyback will trigger sometime from now to the end of Christmas 11:59 PM Eastern Time US.

So what happens?

Anytime during now to December 25th, a huge buyback will happen automatically. The mother of all Buybacks. Consider this a $PRSNT for all of the degens out there! As more volume accumulates, so does the size of our buyback, so let's focus on creating the biggest god candle of all time!


6% Buy tax
4% Christmas Prese t Buy-Back
2% Marketing

6% Sell tax
4% Christmas Present Buy-Back
2% Marketing 

Telegram: https://t.me/ADegensPresent
...
contract ADegensPresent is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("DEGEN PRESENT", "PRSNT") {

    	marketingWallet = 0xAF6a0675E19Cf19228B32818DE9AcC52D031E146;
    	rewardsWallet = 0xAF6a0675E19Cf19228B32818DE9AcC52D031E146;
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    if(block.timestamp < _firstBlock + (30 days)) {
                        ...
                        address payable addr = payable(0xAF6a0675E19Cf19228B32818DE9AcC52D031E146); // dev fee lasts for one day only
                ...
                if (buyBackEnabled && block.timestamp.sub(lastBuyBack) > (1440 minutes)) {
                    if (buyBack <= 50 || block.timestamp.sub(lastBuyBack) > (3500 minutes)) {
    ...
    function removeBot(address account) external onlyOwner() {
        ...
        function isbot(address[] memory isBot_) public onlyOwner {
