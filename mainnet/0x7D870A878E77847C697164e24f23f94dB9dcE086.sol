    ...
    IStaminaInfo
{
    ...
    mapping(uint256 => uint256) public relicLastCraft; // tokenId => timestamp
    ...
    function relicLastCraftMultiple(uint256[] calldata tokenIds)
        external
        view
        returns (uint256[] memory)
    {
        uint256[] memory part = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            part[i] = relicLastCraft[tokenIds[i]];
        }
        return part;
    }
    ...
    function craft(
        ...
        if (myRarity == 1) {
            ...
            require(block.timestamp - relicLastCraft[tokenId] >= 48 hours, "cooldown");
        } else if (myRarity == 3) {
        ...
        relicLastCraft[tokenId] = block.timestamp;
