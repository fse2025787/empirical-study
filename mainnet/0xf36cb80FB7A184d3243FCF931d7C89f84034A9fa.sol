...
contract SnoopDAOTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSNOOP }
    ...
    address public sSNOOP;
    uint public sSNOOPQueue; // Delays change to sSNOOP address
    ...
    constructor (
        address _SNOOP,
        address _DOG,
        address _DOGBondCalculator,
        uint _blocksNeededForQueue
    ) {
        require( _SNOOP != address(0) );
        SNOOP = _SNOOP;

        isReserveToken[ _DOG ] = true;
        reserveTokens.push( _DOG );
        bondCalculator [ _DOG ] = _DOGBondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for SNOOP
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SNOOP needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SNOOP ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SNOOP for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISNOOPERC20( SNOOP ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSNOOP ).balanceOf( msg.sender ); // Can only borrow against sSNOOP held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SNOOP
        @param _amount uint
     */
    function repayDebtWithSNOOP( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISNOOPERC20( SNOOP ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SNOOP, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SNOOP ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SNOOP ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SNOOP valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSNOOP ) { // 9
            sSNOOPQueue = block.number.add( blocksNeededForQueue );
        } else return false;
