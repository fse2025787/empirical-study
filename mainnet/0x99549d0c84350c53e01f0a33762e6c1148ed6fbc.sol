...
contract PukePukes is ERC2981, ERC721AQueryable, Ownable {
    using Address for address payable;
    using Strings for uint256;

    event LuckyPuke(address minter, uint32 amount);
    ...
    function mint(uint32 amount) external payable {
        require(msg.sender == tx.origin, "PukePukes: EOA restrictions");
        require(_started, "PukePukes: Sale is not started");

        uint32 publicMinted = _publicMinted();
        uint32 publicSupply = _publicSupply();
        require(amount + publicMinted <= _publicSupply(), "PukePukes: Exceed max supply");

        uint32 minted = uint32(_numberMinted(msg.sender));
        require(amount + minted <= _walletLimit, "PukePukes: Exceed wallet limit");
        ...
        if (minted + amount > instantFreeWalletLimit) {
            ...
            if (quota > 0) {
                ...
                if (randomFreeAmount > 0) {
                    ...
                    emit LuckyPuke(msg.sender, randomFreeAmount);
                }
            }
        }

        uint256 requiredValue = (amount - freeAmount) * _price;
        require(msg.value >= requiredValue, "PukePukes: Insufficient fund");
    ...
    function devMint(address to, uint32 amount) external onlyOwner {
        _teamMinted += amount;
        require(_teamMinted <= _teamSupply, "PukePukes: Exceed max supply");
        _safeMint(to, amount);
    }
