...
error ErrorTheCallerIsAnotherContract();

contract boobpix is ERC721A, Ownable, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    uint32 public immutable _freeWalletLimit;

    bool public _mintStarted;
    string public _metaDataURI = "ipfs://bafybeid75upi4c2dbicrm25jbjvqqcyfpkgn3hrrysn24casxmehwv6vqa/";

    constructor(uint256 mintPrice, uint32 maxSupply, uint32 walletLimit,uint32 freeWalletLimit) ERC721A("boobpix", "BOOB") {
        _mintPrice = mintPrice;
        _maxSupply = maxSupply;
        _walletLimit = walletLimit;
        _freeWalletLimit = freeWalletLimit;
    }
    ...
    function mint(uint32 amount) external payable {
        ...
        uint32 userMinted = uint32(_numberMinted(msg.sender));
        uint32 freeAmount = 0;
        if (userMinted < _freeWalletLimit)
        {
            uint32 anum = _freeWalletLimit - userMinted;
            freeAmount += anum > amount ? amount : anum;
        }
        requiredValue -= freeAmount * _mintPrice;
    ...
    function _state(address minter)
        external
        view
        returns (
            uint256 mintPrice,
            uint32 freeWalletLimit,
            uint32 walletLimit,
            uint32 maxSupply,
            uint32 totalMinted,
            uint32 userMinted,
    ...
    {
        freeWalletLimit = _freeWalletLimit;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_metaDataURI, tokenId.toString(), ".json"));
    }
