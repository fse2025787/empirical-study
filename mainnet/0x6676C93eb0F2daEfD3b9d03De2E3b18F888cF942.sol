...
library UniV3Actions {
    ...
    function kyberSwap(address _router, bytes memory _path, uint256 _amount) internal returns (uint256 amountOut) {
     ...
     // kyber V3 swap
    function kyberSwapSingle(address _router, address _input, address _output, uint24 _fee, uint256 _amount) internal returns (uint256 amountOut) {
        IKyberElastic.ExactInputSingleParams memory swapParams = IKyberElastic.ExactInputSingleParams({
            tokenIn: _input,
            tokenOut: _output, 
            fee: _fee,
            recipient: address(this),
        ...
        return IKyberElastic(_router).swapExactInputSingle(swapParams);
    }
    ...
    function swapV3Single(address _router, address _input, address _output, uint24 _fee, uint256 _amount) internal returns (uint256 amountOut) {
            ...
            return IUniswapRouterV3(_router).exactInputSingle(swapParams);
    }
...
// File: contracts/BIFI/strategies/Balancer/StrategyAuraBalancerMultiRewardGaugeUniV3.sol
