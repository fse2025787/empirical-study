...
contract MyGoodDogNFT is ERC721AQueryable, Initializable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 1555;
    uint256 public constant WALLET_MINT_LIMIT = 50;
    uint256 public constant TX_MINT_LIMIT = 15;
    uint256 public constant MINT_PRICE = 0.0325 ether;
    uint256 public constant MIN_HEEL_TOKEN_AMOUNT = 25e9; // for those without HEEL NFTs v1
    ...
    IERC20 public constant HEEL_TOKEN = IERC20(0xf941D3AAbf2EE0F5589E68Ba6047b8329592B366);
    ...
    function claim(uint256[] memory tokenIds) external nonReentrant {
        ...
        // Also, we do not impose wallet and transaction limits to free claims
    ...
    function mint(uint256 qty) external payable nonReentrant {
        ...
        require(qty <= TX_MINT_LIMIT, "Per-transaction mint limit exceeded!");
        require(
            balanceOf(msg.sender) + qty <= WALLET_MINT_LIMIT,
            "Wallet mint limit exceeded!"
        );
        require(
            HEEL_TOKEN.balanceOf(msg.sender) >= MIN_HEEL_TOKEN_AMOUNT,
            "Must have min 25 HEEL tokens on the wallet"
        );
