...
contract CASPER is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Casper the Ghost";
    string private constant _symbol = unicode"FRIENDLY";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled && contractTokenBalance>_taxSwapThreshold && _buyCount>_preventSwapBefore) {
                swapTokensForEth(_maxTaxSwap>amount?amount:_maxTaxSwap);
