...
// File: contracts/interface/INON.sol
...
// File: contracts/token/TokenNON.sol
...
contract TokenNON is INON, ERC20 {
    bytes32 public DOMAIN_SEPARATOR;
    // keccak256("Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)");
    bytes32 public constant PERMIT_TYPEHASH = 0x6e71edae12b1b97f4d1f60370fef10105fa2faae0126114a169c64845d6126c9;
    mapping(address => uint256) public nonces;

    constructor(address platform) ERC20(platform, "NON Token", "NON", 4 * 10 ** 8) {
        uint chainId;
        assembly {
            chainId := chainid()
        }
    ...
    function transferWithMemo(address to, uint256 amount, string memory memo) external override returns (bool) {
        address owner = _msgSender();
        _transfer(owner, to, amount);
        emit TransferWithMemo(owner, to, amount, memo);
        return true;
    }
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'NON: INVALID_SIGNATURE');
