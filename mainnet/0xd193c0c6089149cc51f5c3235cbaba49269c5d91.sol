...
// File: interfaces/IStaxAuthority.sol


pragma solidity =0.7.5;

interface IStaxAuthority {
    /* ========== EVENTS ========== */
...
// File: types/StaxAccessControlled.sol


pragma solidity >=0.7.5;


abstract contract StaxAccessControlled {

    /* ========== EVENTS ========== */

    event AuthorityUpdated(IStaxAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    IStaxAuthority public authority;


    /* ========== Constructor ========== */

    constructor(IStaxAuthority _authority) {
    ...
    function setAuthority(IStaxAuthority _newAuthority) external onlyGovernor {
...
// File: interfaces/IsStax.sol


pragma solidity >=0.7.5;


interface IsStax is IERC20 {
    function rebase( uint256 StaxProfit_, uint epoch_) external returns (uint256);
...
// File: interfaces/IStax.sol


pragma solidity >=0.7.5;


interface IStax is IERC20 {
...
contract StaxTreasury is StaxAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(
        ...
        uint256 _timelock,
        address _authority
    ) StaxAccessControlled(IStaxAuthority(_authority)) {
        require(_Stax != address(0), "Zero address: Stax");
        Stax = IStax(_Stax);
    ...
    /**
     * @notice allow approved address to deposit an asset for Stax
    ...
    function deposit(
        ...
        // mint Stax needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        Stax.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn Stax for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        Stax.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new Stax using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        Stax.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or Stax from the treasury, using sStax as collateral. This might allow an
     * sStax holder to provide Stax liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(Stax)) {
            require(permissions[STATUS.StaxDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sStax.changeDebt(value, msg.sender, true);
        require(sStax.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(Stax)) {
            Stax.mint(msg.sender, value);
            StaxDebt = StaxDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sStax.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with Stax
     * @param _amount uint256
     */
    function repayDebtWithStax(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.StaxDEBTOR][msg.sender], notApproved);
        Stax.burnFrom(msg.sender, _amount);
        sStax.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        StaxDebt = StaxDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(Stax), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SStax) {
            sStax = IsStax(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SStax) {
            // 9
            sStax = IsStax(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(Stax.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns Stax valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(Stax)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return Stax.totalSupply() - StaxDebt;
    }
}