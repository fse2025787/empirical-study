    ...
    ERC20Permit
{
    ...
    function _lock(
        ...
        _locker.dueTime = safe32((block.timestamp).add(_duration), "due time overflow.");
     ...
     * @return The amount of veDF burned.
     */
    function _unLock(address _caller, address _from) internal returns (uint96) {
        require(
            uint256(lockers[_from].dueTime) < block.timestamp,
            "due time not meeted."
        );
    ...
    /**
     * @notice If not expired, relock Staked DF and update veDF balance.(transfer to _recipient)
     * @dev Update the lockup information and veDF balance.
     * @param _recipient veDF recipient address.
     * @param _amount Staked DF token new amount.
     * @param _duration New duration, in seconds.
     * @param _minted veDF new balance.
     */
    function proExtend(
        address _recipient,
        uint256 _amount,
        ...
        returns (uint96 _minted)
    {
        ...
        _minted = _lock(msg.sender, _recipient, _amount.add(_locker.amount), _duration);
        require(
            _locker.dueTime <= lockers[_recipient].dueTime,
            "due time refuses to pro extend."
        );
    }
    ...
    function calcBalanceReceived(
        ...
        return
            _receiveAmount.add(
                _amount.rmul(
                    _weightedRate(
                        uint256(_locker.dueTime).add(_duration).sub(block.timestamp)
                    )
                )
            );
    }
