...
contract Splitter2 is Ownable, ReentrancyGuard, Pausable {
    ...
    modifier noReceivedRoyalties() {
        require(!findInArray(msg.sender), "This address has already been requested roylties!");
        _;
    }

    function findInArray(address _sender) private view returns (bool) {
        for(uint256 i = 0; i < received_royalties.length; i++) {
            if (received_royalties[i] == _sender) {
                return true;
            }
        }
    ...
    function getRoyalty(bytes32[] calldata merkleProof, uint256 token) 
        ...
        nonReentrant
    {
        (bool status, bytes memory _data) = contract_addr.call(abi.encodeWithSignature("totalSupply()"));
        require(status, "Error");

        uint256 totalSupply = abi.decode(_data, (uint256));
        uint256 amount = totalAmount * token / totalSupply;

        (bool os, ) = payable(msg.sender).call{value: amount}("");
        require(os);

        received_royalties.push(msg.sender);
    }

    function resetAddresses() public onlyOwner {
        delete received_royalties;
    }
