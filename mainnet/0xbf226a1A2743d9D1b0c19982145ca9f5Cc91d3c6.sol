...
contract AMMO is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("AmmoToken", "AMMO");
        __Ownable_init();

        marketingAddress = _marketingAddress;
        lpAddress = _lpAddress;

        _totalSupply = 10e9 * 1e18; // 10 B tokens
        swapTokensAtAmount = 10e6 * 1e18; // 10M = Threshold for swap (0.1%)
        maxWalletHoldings = 0; // Start at zero to disable trading. //30e6 * 1e18; // 0.3% max wallet holdings (mutable)

        marketingFee = 500; // Basis points
        liquidityFee = 250; // Basis points
        launchedAt = 0;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "AMMO: The router already has that address"
        );
    ...
    function swapAndSendDividendsAndLiquidity(uint256 tokens) private {
        uint256 tokensForMarketing = (tokens * marketingFee) / 10000;
        uint256 tokensForLiquify = tokens - tokensForMarketing;
        ...
        require(successMarketing, "Error Sending tokens");
    ...
    function _transfer(
        ...
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(!blacklisted[from], "AMMO: Blocked Transfer");
        
        // prevents premature launch exploit
        require(amount > 0, "AMMO: Amount Must be greater than zero"); 
        ...
        if (takeFee) {
            ...
            uint256 fees = (amount * (marketingFee + liquidityFee)) / (10000);
            amount = amount - (fees);
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "AMMO: The Uniswap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "AMMO: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
