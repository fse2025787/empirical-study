...
contract LBNFT is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
    uint256 public SALE_NFT = 2000;
	uint256 public FIAT_WEBSITE_BATCH_NFT = 5777;
	
	uint256 public MAX_MINT_PRESALE = SALE_NFT;
	uint256 public MAX_MINT_SALE = SALE_NFT;
	
	uint256 public MAX_BY_MINT_IN_TRANSACTION_PRESALE = SALE_NFT;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_SALE = SALE_NFT;
	
	uint256 public PRESALE_PRICE = 0.064 ether;
	uint256 public SALE_PRICE = 0.071 ether;
	
	uint256 public SALE_MINTED;
	uint256 public FIAT_WEBSITE_BATCH_MINTED;
    ...
    constructor() ERC721P('Lost Boys NFT', 'LBNFT') {}
	
    function _baseURI() internal view virtual returns (string memory) {
        return baseURI;
    }
	
	function mintFiatWebsiteBatchNFT(address _to, uint256 _count) public onlyOwner{
	    uint256 totalSupply = totalSupply();
        require(
            FIAT_WEBSITE_BATCH_MINTED + _count <= FIAT_WEBSITE_BATCH_NFT, 
            "Max limit"
        );
		for (uint256 i = 0; i < _count; i++) {
            _safeMint(_to, totalSupply + i);
			FIAT_WEBSITE_BATCH_MINTED++;
        }
    }
	...
	function updateFiatWebsiteBatchSupply(uint256 newSupply) external onlyOwner {
	    require(newSupply >= FIAT_WEBSITE_BATCH_MINTED, "Incorrect value");
        FIAT_WEBSITE_BATCH_NFT = newSupply;
    }
