    ...
    */
    function stakeAll() external virtual {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokensOfOwner[msg.sender].push(tokenIds[i]);
            tokenOwner[tokenIds[i]] = msg.sender;
            tokenStakedTime[tokenIds[i]] = block.timestamp;
            _stake(tokenIds[i]);
        }
    ...
    */
    function stake(uint256[] calldata tokenIds_) external virtual {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            _stake(tokenIds_[i]);
            tokensOfOwner[msg.sender].push(tokenIds_[i]);
            tokenOwner[tokenIds_[i]] = msg.sender;
        }
        _updateUniqueOwnerCount(true);
    }
