...
contract DVDA is ERC721AQueryable, Ownable, ReentrancyGuard {    
    ...
    //WHITELIST MINT (FREE)
  function whitelistMint(bytes32[] memory proof) external nonReentrant {
    ...
    function isAddressWhitelisted(address owner, bytes32[] memory proof) external view returns (bool)  {
        return isWhitelisted(proof, keccak256(abi.encodePacked(owner)));
    }
