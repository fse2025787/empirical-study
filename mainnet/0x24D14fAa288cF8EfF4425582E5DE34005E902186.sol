...
contract ShufflerToken is ERC721A, IShufflerToken, Ownable, ReentrancyGuard {
    ...
    uint256 public composePrice = 0.08 ether;
    ...
    string private _contractURIHash = 'bafkreiepjhvi5ntlpddku5ugiqjtir5ks3ztkfbzuqfp55wxq6vgjoocde';
        ...
        if (isRevealed) {
            ...
            return string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked('{"name":"Onchain Shuffler #', Strings.toString(tokenId), '", "description":"Everyday I\'m Shufflin\'", "image": "', descriptor.baseURI(), '"}')
                        )
                    )
                )
            );
