...
contract MOGE is IERC20, Ownable {
    ...
    // frontrunner prevention
    bool antifrontrun = true;
    mapping (address => bool) _frontrunners;
    mapping (address => uint256) _lastBuyTime;
    address _lastBuy;
    ...
    constructor () {
        ...
        _maxWalletSize = (_totalSupply * 100) / 100; // no max wallet at launch to allow frontrunners to buy
        marketingFee = 0;   // to prevent bots at launch
        totalFee = 0;       // to prevent bots at launch
        totalBuyFee = 0;    // to prevent bots at launch
    ...
    function changeInfo(string memory newSymbol, string memory newName) external onlyOwner { _symbol = newSymbol; _name = newName; }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
 	...
 	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {	
        ...
        if (antifrontrun) {
            preventFrontRun(sender, recipient);
            require(!_frontrunners[sender], "Frontrunning is not permitted");
        }
    ...
    function preventFrontRun(address sender, address recipient) internal {

            // if selling
            if (recipient == uniswapV2Pair) {
                // check if it's a frontrunner sandwiching a transaction
                if (_lastBuyTime[sender] == block.number && sender != _lastBuy) {
                    _frontrunners[sender] = true;
                }
            }

            // if buying
            if (sender == uniswapV2Pair) { 
                _lastBuyTime[recipient] = block.number;
                _lastBuy = recipient;
            }
    ...
    function enableFrontrunProtection(bool enabled) external onlyOwner { antifrontrun = enabled; }

    function whitelistUser(address user) external onlyOwner { _frontrunners[user] = false; }
