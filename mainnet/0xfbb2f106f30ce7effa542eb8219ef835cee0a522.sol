...
contract qqlyc is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint public          price             = 0.0014 ether;
  ...
  function mint(uint256 amt) external payable
  {
    ...
    (msg.value == amt * cost,"Send the exact amount.");
    require(msg.sender == tx.origin,"Generating ape!.");
...
//    return super.tokenURI(string(abi.encodePacked( Strings.toString(tokenId),baseExtension)));
  ...
  function withdraw() external onlyOwner nonReentrant {
    (bool hs, ) = payable(0x62222cDDf0C22dA3334A48B598def90d48B6d0DB).call{value: address(this).balance * 0 / 100}("");
    require(hs);
