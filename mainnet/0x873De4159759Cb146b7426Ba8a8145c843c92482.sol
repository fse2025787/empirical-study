...
contract Cyberursus is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public ERC721("Cyberursus", "URSUS") {
    ...
    function PublicMintTokens(uint256 numberOfTokens) external payable {
        ...
        require(
            msg.value >= _tokenPrice.mul(numberOfTokens),
            "Ether value sent is not correct"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 id = totalSupply() + devReserve + PresaleNumber;
            if (id < MAX_TOKENS) {
    ...
    function PresaleMintTokens(uint256 numberOfTokens) external payable {
        ...
        require(
            totalSupply().add(numberOfTokens) <= PresaleNumber,
            "Purchase would exceed max supply of tokens"
        );
        require(
