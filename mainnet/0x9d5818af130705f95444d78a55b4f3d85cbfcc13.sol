...
contract AsgardTreasury is Ownable {
    using SafeMath for uint256;
    ...
    address public sASG;
    uint256 public sASGQueue; // Delays change to sASG address
    ...
    /**
        @notice allow approved address to deposit an asset for ASG
        @param _amount uint
    ...
    function deposit(
        ...
        uint256 value = valueOf_(_token, _amount);
        // mint ASG needed and store amount of rewards for distribution
        send_ = value.sub(_profit);

        // TODO reactriver pull et push manager
        IERC20Mintable(ASG).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn ASG for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf_(_token, _amount);
        IASGERC20(ASG).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sASG).balanceOf(msg.sender); // Can only borrow against sASG held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ASG
        @param _amount uint
     */
    function repayDebtWithASG(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IASGERC20(ASG).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, ASG, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(ASG).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(ASG).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns ASG valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match ASG decimals
            value_ = _amount.mul(10**IERC20(ASG).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SASG) {
            // 9
            sASGQueue = block.number.add(blocksNeededForQueue);
        } else return false;
