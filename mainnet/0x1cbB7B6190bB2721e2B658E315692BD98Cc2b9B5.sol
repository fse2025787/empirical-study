...
abstract contract Strategy is IStrategy, Pausable {
    ...
    constructor(
        ...
        keepers.push(_msgSender());
        isKeeper[_msgSender()] = true;
    }
    ...
    modifier onlyKeeper() {
        require(isKeeper[_msgSender()], "caller-is-not-keeper");
        _;
    }
     ...
     */
    function addKeeper(address _keeperAddress) external onlyController {
        require(!isKeeper[_keeperAddress], "already-keeper");
        keepers.push(_keeperAddress);
     ...
     */
    function removeKeeper(address _keeperAddress) external onlyController {
        require(isKeeper[_keeperAddress], "!keeper");
        uint256 _len = keepers.length;
        for (uint8 i; i < _len; ++i) {
            if (keepers[i] == _keeperAddress) {
                delete isKeeper[_keeperAddress];
                keepers[i] = keepers[_len - 1];
                keepers.pop();
                break;
            }
        }
     ...
     * @dev Earned interest fee will go to the fee collector. We want fee to be in form of Vesper
