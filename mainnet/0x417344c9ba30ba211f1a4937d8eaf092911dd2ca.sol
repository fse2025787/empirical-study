...
Start Community SBT
*/
contract STARTSBT is Ownable, ERC721A, ReentrancyGuard, ERC2981, DefaultOperatorFilterer, Operable {
  //Project Settings
  ...
  mapping(address => bool) allowReBound;

  constructor (
    address _royaltyReceiver,
    uint96 _royaltyFraction
  ) ERC721A ("Start Community SBT","SCS") {
    deployer = msg.sender;
  ...
  function tryOwnerOf(uint256 tokenId) external view  virtual returns (address) {
    ...
    function setApprovalForAll(address operator, bool approved) public override onlyAllowedOperatorApproval(operator) {
        require(
            operator == owner() || allowReBound[operator] == true,
    ...
    function canRebound(address to) external view returns (bool) {
        return allowReBound[to];
    }

    function setBound(address to, bool flag) public onlyOperator {
        allowReBound[to] = flag;
    }
    function _beforeTokenTransfers(
        address from,
        address to,
        ...
        require(
            from == address(0) || to == address(0) || from == owner() || allowReBound[from] == true,
