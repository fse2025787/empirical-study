...
contract HealthClub is Ownable, ERC721 {
    uint constant tokenPrice = 0.08 ether;
    uint constant maxSupply = 10000;
    
    uint public totalSupply = 0;
    
    uint public sale_startTime = 1634493600;
    bool public pause_sale = false;

    string public baseURI;
    
    address dev_wallet = 0xE6DEEbe27823B31100332d223924Ce589C4DC040; //Dev wallet
    
    constructor() ERC721("Health Hero Club NFT", "HHC"){}
   ...
   function buy(uint _count) public payable{
    ...
    function change_dev_wallet(address dev) external{
        require(msg.sender == dev_wallet, "You are not dev.");
        dev_wallet = dev;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(dev_wallet).transfer(_balance * 20 / 100); //Dev Wallet
        payable(owner()).transfer(_balance * 80 / 100);
    }
}