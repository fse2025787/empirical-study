...
// File: contracts/interfaces/IHelLottery.sol

pragma solidity ^0.8.4;

interface IHelLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in Hel per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in Hel token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInHel: price of a ticket in Hel
 ...
 */
contract HelLottery is ReentrancyGuard, IHelLottery, Ownable {
    ...
    uint256 public maxPriceTicketInHel = 50 ether;
    uint256 public minPriceTicketInHel = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInHel;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] HelPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInHel;
        uint32 finalNumber;
    }
     ...
     * @param _helTokenAddress: address of the Hel token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _helTokenAddress, address _randomGeneratorAddress) {
        helToken = IERC20(_helTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of Hel to this contract
        uint256 amountHelToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInHel,
            _ticketNumbers.length
        );

        // Transfer Hel tokens to this contract
        helToken.safeTransferFrom(address(msg.sender), address(this), amountHelToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInHel += amountHelToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInHelToTransfer
        uint256 rewardInHelToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInHelToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        helToken.safeTransfer(msg.sender, rewardInHelToTransfer);

        emit TicketsClaim(msg.sender, rewardInHelToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInHel) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in Hel for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].HelPerBracket[j] =
                ...
                // A. No Hel to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].HelPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInHel - amountToShareToWinners);

        // Transfer Hel to treasury address
        helToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        helToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInHel += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInHel >= minPriceTicketInHel) && (_priceTicketInHel <= maxPriceTicketInHel),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInHel: _priceTicketInHel,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            HelPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInHel: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(helToken), "Cannot be Hel token");
    ...
    /**
     * @notice Set Hel price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInHel: minimum price of a ticket in Hel
     * @param _maxPriceTicketInHel: maximum price of a ticket in Hel
     */
    function setMinAndMaxTicketPriceInHel(uint256 _minPriceTicketInHel, uint256 _maxPriceTicketInHel)
        external
        onlyOwner
    {
        require(_minPriceTicketInHel <= _maxPriceTicketInHel, "minPrice must be < maxPrice");

        minPriceTicketInHel = _minPriceTicketInHel;
        maxPriceTicketInHel = _maxPriceTicketInHel;
    }
     ...
     * @param _priceTicket price of a ticket (in Hel)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].HelPerBracket[_bracket];
        } else {
            return 0;
        }
    }
