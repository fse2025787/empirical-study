...
contract BondDepositoryCVX is Ownable {
    ...
    IERC20 public immutable BIG; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints BIG when receives principle
    ...
    uint256 public minPayout = 1000;
    uint public scaling = 4;
    ...
    struct Terms {
        ...
        uint minimumPrice; // vs principle value (The minimum price’s unit is in cents and without the decimals.)
    ...
    struct Bond {
        ...
        uint pricePaid; // for front end viewing
    ...
    struct Adjust {
        ...
        uint rate; // increment - step
        uint target; // BCV when adjustment finished
        uint32 buffer; // minimum length (in seconds) between adjustments - time period to 0
    ...
    constructor (
        ...
        principle = IERC20(_principle); // wrapped native token?
    ...
    function setMinPayout(uint256 _newMinPayout) public onlyOwner() {
        minPayout = _newMinPayout;
    }
    ...
    function deposit(
        ...
        uint value = treasury.valueOf(address(principle), _amount); // how much cost (what?)
        uint payout = payoutFor(value); // payout to bonder is computed in BIG
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            BIG.safeTransfer( DAO, fee );
        }
        // FIXME: do we need this require. part2?
        // require(balanceBefore.add(profit) == BIG.balanceOf(address(this)), "Not enough BIG to cover profit");
    ...
    function redeem( address _recipient, bool _stake ) external returns ( uint ) {
        ...
        if ( percentVested >= 10000 ) { // if fully vested
            ...
            bondInfo[ _recipient ] = Bond({
                payout: info.payout.sub( payout ), // 0.000100000 BIG
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            BIG.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                BIG.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                BIG.approve( address(staking), _amount );
     ...
     *  @return uint
     */
    function payoutFor( uint _value ) public view returns ( uint ) { // bondPayout
    ...
    /**
     *  @notice converts bond price to principle value
     *  @return price_ uint
     */
