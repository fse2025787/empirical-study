...
contract CaveShop is Ownable {
    ...
    address public DivideReceiver = 0xd1c2809f12D74E691769A6C865E6f0d531a8a36f;
    ...
    mapping(uint256 => uint256) private commodityUsdtPrice;  
    ...
    mapping(uint256 => bool) private specialTwotoken; 
    ...
    function commonStore(uint256[] memory _tokenId,uint256[] memory _amount) public{
        ...
        for(uint i = 0; i < commodityLength; i++){
            ...
            if(special[_tokenId[i]]){
               specialPrice +=  commodityPrice[_tokenId[i]] * _amount[i];
                if(specialTwotoken[_tokenId[i]]){
                    specialUsdtPrice +=  commodityUsdtPrice[_tokenId[i]] * _amount[i];
                }
        ...
        require(
            tokenC.allowance(msg.sender,address(this)) >=  upgradePrice + specialUsdtPrice,
            "insufficient allowance"
        );
        ...
        require(
            tokenC.balanceOf(msg.sender) >=  upgradePrice + specialUsdtPrice,
            "Not enough Torch send"
        );
        ...
        if(specialUsdtPrice > 0){
            tokenC.transferFrom(msg.sender, specialAddress, specialUsdtPrice);  
        }
    ...
    function setUpCommodityUsdtPrice(uint256[] memory _tokenId,uint256[] memory _amount) public  onlyOwner {
        uint commodityLength = _tokenId.length;
        uint commodityAmount = _amount.length;
        require(
            commodityLength == commodityAmount,
        ...
        for(uint i = 0; i < commodityLength; i++){
            commodityUsdtPrice[_tokenId[i]] = _amount[i];
        }
    }

    function getCommodityUsdtPriceList() public view returns(uint256[] memory,uint256[] memory){
        uint commodityLength = commodityTokenId.length;
        uint SpecialTwotokenLength  =  getSpecialTwotokenNumber();
        uint256[] memory allPrice = new uint[](SpecialTwotokenLength);
        uint256[] memory commodityTokenIdUsdt = new uint[](SpecialTwotokenLength);
        uint counter = 0;
        for(uint i = 0; i < commodityLength;i++){
            if(specialTwotoken[commodityTokenId[i]]){
                allPrice[counter] = commodityUsdtPrice[commodityTokenId[i]];
                commodityTokenIdUsdt[counter] = commodityTokenId[i];
                counter++;
            }
        }
        return (commodityTokenIdUsdt,allPrice);
    }
    ...
    function setSpecialTwotoken(uint256[] memory  _tokenId,bool[] memory  value) public onlyOwner{
        uint specialLength = _tokenId.length;
        uint specialBool = value.length;
        ...
        for(uint i = 0; i < specialLength; i++){
            specialTwotoken[_tokenId[i]] = value[i];
        }
     
    }

    function getSpecialTwotoken(uint256[] memory _tokenId) public view returns(bool[] memory){
        bool[] memory result = new bool[](_tokenId.length);
        uint counter = 0;
        for (uint i = 0; i < _tokenId.length; i++) {
                result[counter] = specialTwotoken[_tokenId[i]];
                counter++;
        }
       return result;
    }
    ...
    function getSpecialTwotokenNumber() public view  returns(uint){
        uint commodityLength = commodityTokenId.length;
        uint counter = 0;
        for(uint i = 0; i < commodityLength;i++){
            if(specialTwotoken[commodityTokenId[i]]){
