...
library SwapDescriptionLibrary {
...
contract GenericRouter is EthReceiver {
    ...
    function swap(
        ...
        SwapDescriptionLibrary.SwapDescription calldata desc,
        bytes calldata permit,
        bytes calldata data
    )
        external
...
contract UnoswapRouter is EthReceiver {
    ...
    address private constant _WETH = 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6;
...
contract HappySwapAggregator is Ownable, OnlyWethReceiver {
    AggregationRouterV5 constant router = AggregationRouterV5(payable(0x1111111254EEB25477B68fb85Ed929f73A960582));

    bool public takeFee = true;
    address public splitFeeTo;
    uint256 public flatFee = .0035 ether;
    uint256 public charityPerc = 6700;
    uint256 public constant FEE_DIVISOR = 10000;
    mapping (address => bool) public activeCharities;
    ...
    constructor(IERC20 weth) OnlyWethReceiver(address(weth))     
    {
        splitFeeTo = msg.sender;
        //Doctors Without Borders
        activeCharities[0xc8D948503722B669A260504585f015a9f1ce05dE] = true;
        //Rainforest Partnership
        activeCharities[0x0e34CcA01700a98864e538Ab5BF2587bf779FE9f] = true;
        //Center for Reproductive Rights
        activeCharities[0x789e4Ffa74201b51B855Db7439Ebd332dE018790] = true;
        //Waterwells of Africa
        activeCharities[0x272dC2c62C31f5A83DDa05875cabea4315d9d5c3] = true;
        //Partnership to End Addiction
        activeCharities[0x115907B21c6674C655d1B0b3fC1f779cfD9B4D53] = true;
        //Feeding America
        activeCharities[0xf5210DBF244c655629Cd40904e6426864F3D780A] = true;
        //The Bee Conservancy
        activeCharities[0xbe4Ad941b13bfEB32383D34A8BfA9Cbb67AD9533] = true;
        //Pathfinders for Autism
        activeCharities[0x88c005e4897aAce687a64fb23d42Bf6e2b3c2013] = true;
        //Disabled American Veterans
        activeCharities[0x52bBf840a899058280C22B98Bd3392Bd2902d236] = true;
        //National Park Rescue
        activeCharities[0x95ecF363687DB1F95909d5bD040156534A98dECD] = true;
        //Give Directly
        activeCharities[0x750EF1D7a0b4Ab1c97B7A623D7917CcEb5ea779C] = true;
        //Human Rights Campaign
        activeCharities[0xFab55b2f91e0A11f74F460d044e77d06506950B5] = true;
        //Give Well
        activeCharities[0x7cF2eBb5Ca55A8bd671A020F8BDbAF07f60F26C1] = true;
        //Save the children
        activeCharities[0xF6736CD5FF15F5cD92Ecd46c45A5b844B3773812] = true;
        //water.org (http://water.org/) 
        activeCharities[0x7a86d441606d05b6AA71ABd2D5FD3224EE767e7D] = true;
        //Girls Who Code
        activeCharities[0x92a3FD46b457be0A98c34A268D3Ff9E294Be9DE2] = true;
        //Code to Inspire
        activeCharities[0xD481f69E79FEE411627C87e20FbD01fd42cb7D6a] = true;
        //One Tree Planted
        activeCharities[0xf502880b4F01a240331a45BC66b6DcCFD834abBa] = true;
        //Care
        activeCharities[0x5c61d57cAF7E0Fd7136c491aDc70BFdD549b669a] = true;
        //World Central Kitchen
        activeCharities[0xA99A4614FAA2635a6501a6680C3ffe53898C8BB7] = true;
        //Outright International 
        activeCharities[0x8c2470A138664827ABB578eaCC5c8eC0E57E4BAA] = true;
        //International Medical Corpos 
        activeCharities[0xD09f1F0435bC1Ee362148E35483FE68321966022] = true;
        //No Kid Hungry
        activeCharities[0x95cc292549f6A54F9E78741cC411E761B1E66Ce4] = true;
        //American Cancer Society 
        activeCharities[0xE740541D8D879A066b83E62b489660fCe786Df45] = true;
    }

    function setFeeSettings(address feeReceiver, bool feesEnabled, uint256 _flatFee, uint256 _charityPerc) external onlyOwner {
        require(feeReceiver != address(0), "Zero Address");
        splitFeeTo = feeReceiver;
        takeFee = feesEnabled;
        require(_flatFee <= .01 ether, "May not set fees above .01 ETH");
        flatFee = _flatFee;
        require(_charityPerc >= 5000, "Must keep charity at or above 50%");
        charityPerc = _charityPerc;
    }

    function setActiveCharities(address[] calldata charityAddresses, bool active) external onlyOwner{
        for(uint256 i = 0; i < charityAddresses.length; i++){
            activeCharities[charityAddresses[i]] = active;
        }
    }

    function takeFeeAmount(address charityAddress, IERC20 srcToken) internal {
        if(takeFee){
            if(isETH(srcToken)){
                require(msg.value >= flatFee, "Must send flat fee to process txn");
                require(activeCharities[charityAddress], "Charity must be active to be selected");
                TransferHelper.safeTransferETH(charityAddress, flatFee * charityPerc / FEE_DIVISOR);
                TransferHelper.safeTransferETH(splitFeeTo, flatFee * (10000 - charityPerc) / FEE_DIVISOR);
            } else {
                require(msg.value == flatFee, "Must send flat fee to process txn");
                require(activeCharities[charityAddress], "Charity must be active to be selected");
                TransferHelper.safeTransferETH(charityAddress, flatFee * charityPerc / FEE_DIVISOR);
    ...
    function transferSourceToken(IERC20 srcToken, uint256 amount) internal {
        if(!isETH(srcToken)){
            uint256 initialAmount = srcToken.balanceOf(address(this));
            TransferHelper.safeTransferFrom(address(srcToken), msg.sender, address(this), amount);
            uint256 deltaAmount = srcToken.balanceOf(address(this)) - initialAmount;
            require(deltaAmount == amount, "Not enough tokens transferred. Tokens with transfer tax not supported.");
            srcToken.approve(address(router), amount);
        }
    }

    // swap functions with charity
    function clipperSwap(
        IClipperExchangeInterface clipperExchange,
        IERC20 srcToken,
        IERC20 dstToken,
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, inputAmount);
        return router.clipperSwapTo{value: msg.value - flatFee}(clipperExchange, payable(msg.sender), srcToken, dstToken, inputAmount, outputAmount, goodUntil, r, vs);
    }
    ...
    function clipperSwapTo(
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, inputAmount);
        return router.clipperSwapTo{value: msg.value - flatFee}(clipperExchange, payable(recipient), srcToken, dstToken, inputAmount, outputAmount, goodUntil, r, vs);
    }
    ...
    function clipperSwapToWithPermit(
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, inputAmount);
        return router.clipperSwapToWithPermit(clipperExchange, recipient, srcToken, dstToken, inputAmount, outputAmount, goodUntil, r, vs, permit);
    }
    ...
    function unoswap(
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, amount);
        return router.unoswapTo{value: msg.value -flatFee}(payable(msg.sender), srcToken, amount, minReturn, pools);
    }
    ...
    function unoswapTo(
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, amount);
        return router.unoswapTo{value: msg.value -flatFee}(recipient, srcToken, amount, minReturn, pools);
    }
    ...
    function unoswapToWithPermit(
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, amount);
        return router.unoswapToWithPermit(recipient, srcToken, amount, minReturn, pools, permit);
    }
    ...
    function uniswapV3(
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, amount);
        return router.uniswapV3SwapTo{value: msg.value - flatFee}(payable(msg.sender), amount, minReturn, pools);
    }
    ...
    function uniswapV3To(
        ...
        takeFeeAmount(charityAddress, srcToken);
        transferSourceToken(srcToken, amount);
        return router.uniswapV3SwapTo{value: msg.value -flatFee}(recipient, amount, minReturn, pools);
    }
    ...
    function swap(
        ...
        returns (
            ...
            takeFeeAmount(charityAddress, desc.srcToken);
            transferSourceToken(desc.srcToken, desc.amount);
