...
/**
  ...
  Inspired by SuperFarm, built for LPI DAO with modified sweep function to NOT allow sweeping of Vested tokens and a bug fix in safeTransfer
...
contract VestStream is Ownable, ReentrancyGuard {
  ...
  */
  ...
  /**
    Sweep all of a particular ERC-20 token from the contract beside Vested Token. 

    @param _token The token to sweep the balance from except Vested
  */
  function sweep(IERC20 _token) external onlyOwner {
    require(_token != token, "Can't sweep Vested Token");
