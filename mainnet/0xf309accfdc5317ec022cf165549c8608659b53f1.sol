 ...
 * - The maximum token ID cannot exceed 2**256 - 1 (max value of uint256).
 */
contract BabyNeo_ERC721A is IERC721A {
    ...
    function owner_mint(address addr, uint256 amount) 
        public 
        onlyOwner {
        require(totalSupply() + amount <= maxSupply);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked("ipfs://QmPUwXg256KDRcfxMwYjph3Cp4yWDdcdgrfdRbEiU1XPkQ/", _toString(tokenId), ".json"));
    }
