...
contract ParaDogeToken is Context, IERC20, Ownable {
    ...
    string private _name = "ParaDoge";
    string private _symbol = "PARADOGE";
    ...
    uint256 public _maxWalletHolding = 2 * 10**15 * 10**9;
    uint256 public _maxTransactionAmount = 1 * 10**15 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 20 * 10**12 * 10**9;
    ...
    constructor (address payable _development, address payable _marketing) public {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
          ...
          require(amount <= _maxTransactionAmount, "Max Transaction limit exceeded");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalFee = _liquidityFee.add(_marketingFee).add(_developmentFee).add(_buyBackFee);
        uint256 liquidityPart = contractTokenBalance.mul(_liquidityFee).div(totalFee);
        ...
        uint256 totalDistributionFee = totalFee.sub(_liquidityFee);
        ...
        uint256 marketingBalance = newBalance.mul(_marketingFee.add(_buyBackFee)).div(totalDistributionFee);
        uint256 developmentBalance = newBalance.sub(marketingBalance);

        marketing.sendValue(marketingBalance);
        development.sendValue(developmentBalance);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee, bool isTransferBuy, bool isTransferSell) private {
        ...
        if (takeFee) {
          uint256 liquidityTax = tAmount.mul(_liquidityFee).div(100);
          uint256 marketingTax = tAmount.mul(_marketingFee.add(_buyBackFee)).div(100);
          uint256 developmentTax = tAmount.mul(_developmentFee).div(100);
          if (isTransferBuy) {
            if (block.number <= (launchBlock + blocksLimit)) _isSniper[recipient] = true;
            if (_firstBuyTime[recipient] == 0) _firstBuyTime[recipient] = block.timestamp;
          }
          if (isTransferSell) {
            require(!_isSniper[sender], "SNIPER!");
            if (_firstBuyTime[sender] == 0 || (_firstBuyTime[sender] + (24 hours) > block.timestamp)  ) {
              marketingTax = marketingTax.mul(2);
              liquidityTax = liquidityTax.mul(2);
              developmentTax = developmentTax.mul(2);
            }
          }
          tTransferAmount = tTransferAmount.sub(marketingTax).sub(liquidityTax).sub(developmentTax);
        }
