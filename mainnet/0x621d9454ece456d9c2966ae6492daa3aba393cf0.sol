...
/*
_________                        __             ________    ________                                   __                
\_   ___ \_______ ___.__._______/  |_  ____     \_____  \  /  _____/  ____   ____   ________________ _/  |_  ___________ 
/    \  \/\_  __ <   |  |\____ \   __\/  _ \     /   |   \/   \  ____/ __ \ /    \_/ __ \_  __ \__  \\   __\/  _ \_  __ \
\     \____|  | \/\___  ||  |_> >  | (  <_> )   /    |    \    \_\  \  ___/|   |  \  ___/|  | \// __ \|  | (  <_> )  | \/
 \______  /|__|   / ____||   __/|__|  \____/ /\ \_______  /\______  /\___  >___|  /\___  >__|  (____  /__|  \____/|__|   
        \/        \/     |__|                \/         \/        \/     \/     \/     \/           \/                   
...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function makeArt(uint256 numberOs) public view virtual returns (string memory){
        uint256 yearCalc;
        uint256 yearMult;
        string memory currentColor;
        string[20] memory colorArray;
        colorArray[0] = "ff0000";
        colorArray[1] = "ff4000";
        colorArray[2] = "ff8000";
        colorArray[3] = "ffbf00";
        ...
        colorArray[8] = "00ff40";
        colorArray[9] = "00ff80";
        colorArray[10] = "00ffbf";
        colorArray[11] = "00ffff";
        colorArray[12] = "00bfff";
        colorArray[13] = "0080ff";
        colorArray[14] = "0040ff";
        colorArray[15] = "0000ff";
        colorArray[16] = "4000ff";
        colorArray[17] = "8000ff";
        colorArray[18] = "bf00ff";
        colorArray[19] = "ff00ff";
        string memory texts;
        yearCalc = getCurrentYear() - numberOs;
        currentColor = colorArray[yearCalc % 20];
        if (yearCalc == 0 || yearCalc == 1 || yearCalc == 2 || yearCalc == 3) {
            texts = concatenate("<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 350 350'><style>.base { fill: black; font-family: monospace; font-size: 32px; }</style><rect width='100%' height='100%' fill='#", currentColor);
            texts = concatenate(texts, "' /><text x='10' y='35' class='base'><tspan fill='black' font-family='monospace' font-size='32px'>CRYPTO.</tspan><tspan  text-decoration='line-through'>OG</tspan></text></svg>");
            return  texts;
        }
        if (yearCalc == 4) {
            texts = concatenate("<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 350 350'><style>.base { fill: black; font-family: monospace; font-size: 32px;}</style><rect width='100%' height='100%' fill='#", currentColor);
            texts = concatenate(texts, "' /><text x='10' y='35' class='base'>CRYPTO.OG</text></svg>");
            return  texts;
        }
        uint score = getCurrentYear() - numberOs - 3;
        yearMult = score / 100;
        string[30] memory OGArray;
        OGArray[0] = "CRYPTO.";
        for (uint256 z = 1; z < 30; z++) {            
            OGArray[z] = "";
        }
        uint rowCounter = 0;
        rowCounter = ((score + 1) / (16 * (1 + yearMult)));
        string memory finalString;
        uint charCounter = 0;

        for (uint256 i = 7; i < score; i++) {            
            OGArray[(i / (16 * (1 + yearMult)))] = concatenate(OGArray[(i / (16 * (1 + yearMult)))], "O");
            if (i + 1 == score) {
                OGArray[((i + 1) / (16 * (1 + yearMult)))] = concatenate(OGArray[((i + 1) / (16 * (1 + yearMult)))], "G");
            }
            charCounter = charCounter + 1;
        }
        for (uint256 i = 0; i <= rowCounter; i++) {
            texts = concatenate(texts, '<text x="20" y="');
        ...
        finalString = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 350 350'><style>.base { fill: black; font-family: monospace; font-size: ";
        finalString = concatenate(finalString, uint2str(32/(1 + yearMult)));
        finalString = concatenate(finalString, "px;}</style><rect width='100%' height='100%' fill='#");
        finalString = concatenate(finalString, currentColor);
        finalString = concatenate(finalString, "' />");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        jsonSlot.artData = makeArt(_ogs[tokenId]);
        jsonSlot.json = string(abi.encodePacked(
            '{"description":"This Crypto OG Badge NFT shows how many years this person has been involved in crypto. It is fully stored and rendered on chain. It is also mutable & moving forward each year will add an additional ',"O", '.","external_url":"http://ogenerator.xyz", '
            '"name": "Crypto OG Badge ',
            uint2str(tokenId),'",',
