...
contract AidiCraftMultiple is ERC1155, Ownable, ERC1155Burnable{
    event Approve(
   ...
   mapping(string => tokenfee) public Tokendetail;
    ...
    constructor(
        ...
        addTokenType("WETH",0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2,2500000000000000000);
        addTokenType("VERSE",0xe18841d7A75866688E291703BdE66c3378bd74A3,2000000000000000000);
    }
    ...
    function addTokenType(string memory _type, address tokenAddress, uint256 fee)
        public
        onlyOwner
    {
    ...
    function _saleToken(
        ...
        if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("ETH")))){   
            ...
            uint256 dyntokfee =  Tokendetail[bidtoken].tokenfee;
            uint256 val = pERCent(amount, dyntokfee).add(amount);
            ERC20 t = ERC20(Tokendetail[bidtoken].tokenAddress);

            uint256 approveValue = t.allowance(msg.sender, address(this));
            require( approveValue >= val, "Approve Value Mismatches");
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], dyntokfee);
            require( approveValue >= _adminfee.add(roy.add(netamount)), "Value difference in the Admin Fee");
