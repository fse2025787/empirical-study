...
contract PaceArtStaking is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accPacePerShare; // Accumulated SUSHIs per share, times 1e12. See below.
    }
    // The PACE TOKEN!
    ...
    // The block number when PACE mining starts.
    ...
    constructor(
        PaceArtToken _pace,
        uint256 _pacePerBlock,
        uint256 _startBlock,
        uint256 _bonusEndBlock
    ) public {
        pace = _pace;
        pacePerBlock = _pacePerBlock;
    ...
    function updateStartBlock(uint256 _startBlock) external onlyOwner {
        require(_startBlock != startBlock, "PaceArtStaking::Start block is exactly the same!");
        startBlock = _startBlock;
    }

    function updatePacePerBlock(uint256 _pacePerBlock) external onlyOwner {
        require(pacePerBlock != _pacePerBlock, "PaceArtStaking::Pace Per block is exactly the same!");
        pacePerBlock = _pacePerBlock;
    }
    ...
    function pendingPace(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accPacePerShare = pool.accPacePerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 paceReward =
                multiplier.mul(pacePerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accPacePerShare = accPacePerShare.add(
                paceReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accPacePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        // pace.mint(devaddr, paceReward.div(10));
        pace.mint(address(this), paceReward);
        pool.accPacePerShare = pool.accPacePerShare.add(
            paceReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accPacePerShare).div(1e12).sub(
                    user.rewardDebt
                );
            safePaceTransfer(msg.sender, pending);
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accPacePerShare).div(1e12);
    ...
    function safePaceTransfer(address _to, uint256 _amount) internal {
        uint256 paceBal = pace.balanceOf(address(this));
        if (_amount > paceBal) {
            pace.transfer(_to, paceBal);
        } else {
            pace.transfer(_to, _amount);
        }
    }
