...
ApEth - Purchase your Stake. 80% of all income will be claimable revenue for all stakeholders.

https://t.me/ApEthSociety/
...
contract ApEthSociety is Ownable, ERC721, ERC721Enumerable {
    ...
    // mint shares
    uint private _mintSharesTeam = 200;
    uint private _mintSharesReflect = 800;
    ...
    function _claimedReflection(address tokenOwner) public view returns (uint) {
        uint[] memory _tokens = _tokensByOwner(tokenOwner);
        if (_tokens.length == 0) {
            return 0;
        }
     ...
     */
    function setteam(address _teamMarketing) external onlyOwner {
        teamMarketing = _teamMarketing;
    }
    function teamPayout() public onlyOwner {
        uint p = _balanceTeam;
        payable(teamMarketing).transfer((p*1000)/1000);
        _balanceTeam = 0;
    }
     ...
     */
    function cData() public view returns (bool, uint, uint, uint, uint, uint, uint, string memory) {
        return (_mintingStarted, _decimalBase, totalSupply(), _maxSupply, _mintPrice, _reflection, _maxMintPerTx, _assetsBaseURI);
    }
    ...
    function cShares() public view returns (uint, uint) {
        return (_mintSharesReflect, _mintSharesTeam);
    }
    ...
    function cBalances() public view returns (uint, uint, uint) {
        return (address(this).balance, _balanceReflections, _balanceTeam);
    }
    ...
    function dustSweep() public onlyOwner {
        require(_mintingStarted == false, "Sale hasn't ended");
        uint amount = address(this).balance;
        payable(teamMarketing).transfer(amount);
