...
contract StakingLandTest is  OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, GlobalsAndUtils {
     ...
     */
    function stakeLand(uint256 tokenId, uint256 size, string calldata rarity, uint256[] calldata genesisTokenIds, bytes32[] calldata proof) whenNotPaused external payable nonReentrant CampaignOnGoing {
       require(size.mul(size) == genesisTokenIds.length, "ERR:1");
        require(isWhitelisted(rarity,size, tokenId, proof), "ERR:2");
        // require(msg.sender == tx.origin, "ERR:3");
...
/**
 ...
 /**
    ...
    function isWhitelisted(string memory rarity,  uint256 size, uint256 tokenId,bytes32[] memory proof) public view returns (bool) {
    ...
    function _getClaimableXTickets(address _stakeOwner, uint256 _stakeId) private view returns(uint256) {
        ...
        uint256 claimableTickets = stakedDays.mul(usrStake.xTickets);
