...
/**
 * @author MetaPlayerOne DAO
 * @title SelectiveCollectionFactory
 */
contract SelectiveCollectionFactory is Pausable {
    ...
    event collectionCreated(string name, string symbol, uint256 limit, uint256 royalty, address token_address, address owner_of);

    function createCollection(string memory name_, string memory symbol_, uint256 limit_, uint256 royalty_) public notPaused {
        SelectiveCollectionToken token = new SelectiveCollectionToken(name_, symbol_, msg.sender, limit_, royalty_);
        address token_address = address(token);
        emit collectionCreated(name_, symbol_, limit_, royalty_, token_address, msg.sender);
