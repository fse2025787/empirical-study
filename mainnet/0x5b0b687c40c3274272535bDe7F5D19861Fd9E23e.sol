...
abstract contract TheCollectorsNFTVaultBaseFacet is Ownable, ReentrancyGuard {
    ...
    */
    function _afterPurchaseNFT(uint256 vaultId, uint256 purchasePrice, bool withEvent, uint256 prevERC1155Amount) internal {
        LibDiamond.AppStorage storage _as = LibDiamond.appStorage();
        address[] memory participants = _as.vaultParticipantsAddresses[vaultId];
        uint256 totalPaid;
        for (uint256 i; i < participants.length; i++) {
        ...
        for (uint256 i; i < participants.length; i++) {
            ...
            // Adding vault to collection users' list
            _as.collectionsParticipantVaults[_as.vaults[vaultId].collection][_as.vaultParticipants[vaultId][i].participant].add(vaultId);
        }
...
contract TheCollectorsNFTVaultDiamond is TheCollectorsNFTVaultBaseFacet {
    ...
    constructor(
        ...
        _as.nftVaultAssetsHolderCreator = _seaportManagerFacetAddress;
    ...
    */
    function createVault(
        ...
        _as.vaultsExtensions[_as.vaultIdTracker.current()] = LibDiamond.VaultExtension(
            !privateVault, privateVault ? 0 : minimumFunding, 0,
            !IERC165(collection).supportsInterface(type(IERC721).interfaceId), false, 0, false
        );
        Address.functionDelegateCall(
            _as.nftVaultAssetsHolderCreator,
            abi.encodeWithSelector(TheCollectorsNFTVaultSeaportManagerFacet.createNFTVaultAssetsHolder.selector, _as.vaultIdTracker.current())
        );
            ...
            if (_as.vaultParticipants[vaultId][i].participant == msg.sender) {
                ...
                if (_as.vaultParticipants[vaultId][i].paid == 0 && _as.vaultsExtensions[vaultId].publicVault) {
                    ...
                    if (_as.vaultParticipants[vaultId][i].collectorOwner == msg.sender) {
                        ...
                        uint256 stakedCollectorTokenId = _as.vaultParticipants[vaultId][i].stakedCollectorTokenId;
...
contract TheCollectorsNFTVaultAssetsManagerFacet is TheCollectorsNFTVaultBaseFacet {
    ...
    */
    function migrate(uint256 vaultId, uint256 tokenId, address[] memory _participants, uint256[] memory payments) external {
        ...
        _afterPurchaseNFT(vaultId, totalPaid, false, 0);
    ...
    */
    function sellNFTToVault(uint256 vaultId, uint256 sellPrice) external nonReentrant {
        ...
        _afterPurchaseNFT(vaultId, sellPrice, true, prevERC1155Amount);
    }
    ...
    */
    function withdrawNFTToOwner(uint256 vaultId) external nonReentrant {
        ...
        for (uint256 i; i < participants.length; i++) {
            if (_as.vaultParticipants[vaultId][i].ownership > 0) {
                ...
                if (_as.vaultParticipants[vaultId][i].partialNFTVaultTokenId != 0) {
                    require(IERC721(address(this)).ownerOf(_as.vaultParticipants[vaultId][i].partialNFTVaultTokenId) == msg.sender, "E3");
                    Address.functionDelegateCall(
                        _as.nftVaultTokenHandler,
                        abi.encodeWithSelector(
                            TheCollectorsNFTVaultTokenManagerFacet.burnFractionalToken.selector,
                            _as.vaultParticipants[vaultId][i].partialNFTVaultTokenId
                        )
                    );
        ...
        if (_as.assetsHolders[vaultId].balance > 0) {
            IAssetsHolderImpl(_as.assetsHolders[vaultId]).sendValue(
                payable(msg.sender), _as.assetsHolders[vaultId].balance
            );
        }
        _as.vaultsExtensions[vaultId].isWithdrawnToOwner = true;
    ...
    */
    function validateSale(uint256 vaultId) public view returns (bool) {
        LibDiamond.AppStorage storage _as = LibDiamond.appStorage();
...
library LibDiamond {
    ...
    struct AppStorage {
        ...
        string baseTokenURI;
        // Collection => Participant => VaultIds
        mapping(address => mapping(address => EnumerableSet.UintSet)) collectionsParticipantVaults;
    }
...
interface INFTVault {
    ...
    function validateSale(uint256 vaultId) external view returns (bool);
}
...
*/
contract TheCollectorsNFTVaultSeaportAssetsHolderImpl is ERC721Holder, ERC1155Holder, Ownable {

    // Must be in the same order as @TheCollectorsNFTVaultSeaportAssetsHolderProxy
    ...
    */
    receive() external payable {
        if (listed) {
            require(INFTVault(owner()).validateSale(vaultId), "Wait ~1 minute between list and sale");
        }
    }
