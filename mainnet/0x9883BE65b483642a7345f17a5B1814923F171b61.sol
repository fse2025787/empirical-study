...
contract Namehash {
  function computeNamehash(bytes memory domain) internal pure returns (bytes32) {
...
abstract contract Resolver {
    ...
    function setAuthorisation(bytes32 node, address target, bool isAuthorised) virtual external;
}
...
contract QuantumPFP is Initializable, UUPSUpgradeable, ERC721Upgradeable, OwnableUpgradeable, Namehash {
  ...
  uint public historicalMemberCount;
  ...
  function initialize(string memory __baseURI, string memory __terminatedURI, address ensRegistrar) initializer public {
  ...
  function addMember(string calldata _initial, address targetAddress, string calldata email, string calldata designation, string calldata specialty, uint tokenId) external onlyOwner {
  ...
  function toggleStatus(uint tokenId, bool status) external onlyOwner {
    // true to terminate
    require(_exists(tokenId), "Nonexistent token");
    if(status) {
      memberList[tokenId].terminated = true;
      memberList[tokenId].isActive = false;
      ens.setSubnodeRecord(computeNamehash(abi.encodePacked(BASEDOMAIN)), keccak256(abi.encodePacked(memberList[tokenId].initial)), address(0), address(0), 0);
    } else {
      memberList[tokenId].terminated = false;
    }
  }
  function updateMemberData(uint tokenId, string memory email, string calldata designation, string memory specialty, address newEnsAddress, bool _isActive) external onlyOwner {
    require(_exists(tokenId), "Nonexistent token");

    bytes32 nodeId = computeNamehash(abi.encodePacked(memberList[tokenId].initial,  ".", BASEDOMAIN));
    Resolver resolver = ens.resolver(nodeId);

    memberList[tokenId].isActive = _isActive;
    if (!_isActive) ens.setSubnodeRecord(computeNamehash(abi.encodePacked(BASEDOMAIN)), keccak256(abi.encodePacked(memberList[tokenId].initial)), address(0), address(0), 0);
    if (bytes(email).length > 0) resolver.setText(nodeId, "email", email);
    if (bytes(designation).length > 0) resolver.setText(nodeId, "designation", designation);
    if (bytes(specialty).length > 0) resolver.setText(nodeId, "specialty", specialty);
    resolver.setAddr(nodeId, newEnsAddress);
  }
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    require(_exists(tokenId), "Nonexistent token");
    if(!memberList[tokenId].terminated) {
      return super.tokenURI(tokenId);
    } else {
      return _terminatedURI;
    }
  }
  function transferFrom(
      address from,
  ...
  function getMemberMetaData(uint tokenId) external view returns(string[4] memory data) {
    require(_exists(tokenId), "Nonexistent token");
    bytes32 nodeId = computeNamehash(abi.encodePacked(memberList[tokenId].initial,  ".", BASEDOMAIN));
    Resolver resolver = ens.resolver(nodeId);
