...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    constructor(string memory uri_) {
        ...
        _uriBase = "https://www.urbanminerproject.org/"; // IPFS base for UrbanMinerContract collection or a hosted domain url  "ipfs://IFPS_NODE_PATH/" or "https://website.com/"
...
// File contracts/@openzeppelin/contracts/utils/ContextMixin.sol
...
// File contracts/UrbanMinerContract.sol



contract UrbanMinerContract is
    ERC1155,
    IERC2981,
    Ownable,
    Pausable,
    ...
    ERC1155Burnable
{
    ...
    // Mapping for user mint counts
    mapping(address => uint256) private _userMintCountMap;
    ...
    constructor() ERC1155("https://www.urbanminerproject.org/") {
        name = "The-Urban-Miner-Project";
        symbol = "URM";
        _recipient = owner();

        // Set Contract Presale Time to first 48 hours of deployment;

        contractPresaleTime = block.timestamp + 2 days;
    ...
    function isPresale() public view returns (bool) {
        return block.timestamp < contractPresaleTime;
    }

    function mint(
    ) public payable whenNotPaused {
        // Check if the account parameter exists
        require(msg.sender != address(0), "ERC1155: mint to the zero address");

        // select tier randomly from 0 to 3
        uint256 tier = getRandomTier();

        require(msg.value >= tiers[0].price, "Not enough ETH to mint");

        // If presale, only allow 2 mints per address
        if (isPresale()) {
            require(
                _userMintCountMap[msg.sender] < 2,
                "Presale: Only 2 mints per address"
            );
        }
        ...
        _userMintCountMap[msg.sender] = _userMintCountMap[msg.sender] + 1;

        _mint(
            msg.sender,
            tiers[tier].startingIndex + tierTotalSuppy,
            1,
            tiers[tier].tierName
        );
    }
    ...
    // For viewing ipfs deployed node, use this link https://cloudflare-ipfs.com/ipfs/NODE_ID_PATH
    function contractURI() public pure returns (string memory) {
        return "https://api.ipfsbrowser.com/ipfs/get.php?hash=QmTRNsysLRWkCi2A7UrNc3NbPzyPuiK1X7hnGdj7HRY4x7"; // Contract-level metadata for UrbanMinerProject
    }
    ...
    // return random from 0 - 3
    function getRandomTier() public view returns (uint256) {
