...
contract PlaneMetadata is IPlaneMetadata, ReentrancyGuard, Ownable {
    ...
    string description = "On-chain generative coded art with on-chain trait intelligence. This is a Dynamic NFT where traits and visual representation may evolve over time.";
    string name = "Sky";
    ...
    function genMetadata(string memory tokenSeed, uint256 tokenId) external view virtual override returns (string memory) {
        ...
        BaseAttributes memory atts = artFactory.calcAttributes(tokenSeed);
...
//    function makeAttributeParts(BaseAttributes memory atts) external view returns (string[15] memory);
    ...
    function calcAttributes(string memory seed) external view returns (BaseAttributes memory);
