    ...
    EIP712Upgradeable
{
    ...
    function migrate(address _user, uint256 _amount) internal {
        _approve(_user, address(this), _amount);
    }
...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    modifier notMigrating() {
        bool _isMigrating = IPolicyBookFabric(policyBookFabricAddress).isMigrating();
        require(!_isMigrating, "PB1: Operation paused while migrating");
        _;
    }

    modifier onlyClaimVoting() {
        require(_msgSender() == address(claimVoting), "PB1: Not a CV");
        _;
    }

    modifier onlyPolicyBookRegistry() {
        require(_msgSender() == policyBookRegistry, "PB1: Not a PBR");
        _;
    }

    modifier onlyPolicyBookAdmin() {
        require(_msgSender() == policyBookAdmin, "PB1: Not a PBA");
        _;
    }
    ...
    modifier onlyLiquidityBridge() {
        require(_msgSender() == liquidityBridgeAddress, "PB1: not authorized");
        _;
    }
    ...
    function extractLiquidity() external override onlyLiquidityBridge {
        uint256 _balance = stblToken.balanceOf(address(this));

        if (_balance > 0) {
            stblToken.safeTransfer(liquidityBridgeAddress, _balance);
        }
    }
    ...
    function getPolicyPrice(uint256 _epochsNumber, uint256 _coverTokens)
        ...
        returns (uint256 totalSeconds, uint256 totalPrice)
    {
        require(_coverTokens >= MINUMUM_COVERAGE, "PB1: Wrong cover");
        require(_epochsNumber > 0 && _epochsNumber <= MAXIMUM_EPOCHS, "PB1: Wrong epoch duration");
    ...
    function _buyPolicy(
        ...
        uint256 _epochsNumber,
        uint256 _coverTokens
    ) internal notMigrating withPremiumsDistribution updateBMICoverStakingReward {
        require(
            !policyRegistry.isPolicyActive(_buyer, address(this)),
            "PB1: The holder already exists"
        );
        require(claimingRegistry.canBuyNewPolicy(_buyer, address(this)), "PB1: Claim is pending");

        updateEpochsInfo();

        uint256 _totalCoverTokens = totalCoverTokens.add(_coverTokens);

        require(totalLiquidity >= _totalCoverTokens, "PB1: Not enough liquidity");
    ...
    function updateEpochsInfo() public override {
        ...
        uint256 b = _lastDistributionEpoch + MAXIMUM_EPOCHS;
        uint256 a = getEpoch(block.timestamp);

        uint256 _newDistributionEpoch = Math.min(a, b);
    ...
    function addLiquidityAndStake(uint256 _liquidityAmount, uint256 _stakeSTBLAmount)
        ...
        override
    {
        require(_stakeSTBLAmount <= _liquidityAmount, "PB1: Wrong staking amount");
        ...
        updateBMICoverStakingReward
    {
        ...
        require(stblLiquidity > 0, "PB1: Liquidity amount is zero");
        ...
        returns (uint256)
    {
        ...
        uint256 a = newTotalLiquidity.sub(newTotalCoverTokens);
        uint256 b = _getUserAvailableSTBL(_userAddr);
        return convertSTBLToBMIX(Math.min(a, b));
    }

    function getUserAvailableSTBL(address _userAddr) external view override returns (uint256) {
        return _getUserAvailableSTBL(_userAddr);
    }
    ...
    function requestWithdrawal(uint256 _tokensToWithdraw)
        ...
        withPremiumsDistribution
    {
        revert("disable when migrating liquidity to v2");
        require(_tokensToWithdraw > 0, "PB1: Amount is zero");
        ...
        require(_availableSTBLBalance >= _stblTokensToWithdraw, "PB1: Wrong announced amount");

        updateEpochsInfo();

        require(
            totalLiquidity >= totalCoverTokens.add(_stblTokensToWithdraw),
            "PB1: Not enough free liquidity"
        );
    ...
    function unlockTokens() external override {
        ...
        require(_lockedAmount > 0, "PB1: Amount is zero");

        this.transfer(_msgSender(), _lockedAmount);
        delete withdrawalsInfo[_msgSender()];
    }

    function getUserBMIXStakeInfo(address _sender)
        external
        view
        override
        returns (uint256 _totalBMX, uint256 _totalSbl)
    {
        uint256 _holderBMIXbalance = balanceOf(_sender);
        uint256 _holderPendingWithdrawlBalance = withdrawalsInfo[_sender].withdrawalAmount;

        _totalBMX = _holderBMIXbalance.add(_holderPendingWithdrawlBalance);
        _totalSbl = convertBMIXToSTBL(_totalBMX);
    }

    function migrateRequestWithdrawal(address _sender, uint256 _bmixAmount)
        external
        override
        onlyLiquidityBridge
        returns (uint256 _stblAmountTether)
    {
        uint256 _holderBMIXbalance = balanceOf(_sender);
        uint256 _holderPendingWithdrawlBalance = withdrawalsInfo[_sender].withdrawalAmount;

        uint256 _holderTotalBMIX = _holderBMIXbalance.add(_holderPendingWithdrawlBalance);

        if (allowance(_sender, address(this)) < _holderTotalBMIX) {
            migrate(_sender, _holderTotalBMIX);
        }

        this.transferFrom(_sender, address(this), balanceOf(_sender));

        delete withdrawalsInfo[_sender];

        _burn(address(this), _holderTotalBMIX);
    }

    function withdrawLiquidity() external override {
        require(
            getWithdrawalStatus(_msgSender()) == WithdrawalStatus.READY,
            "PB1: Withdrawal is not ready"
        );

        _withdrawLiquidity(_msgSender(), _msgSender());
    }

    function _withdrawLiquidity(address _sender, address _destiny)
        internal
        withPremiumsDistribution
        updateBMICoverStakingReward
        returns (uint256 _stblAmountTether)
    {
        ...
        uint256 liquidity = totalLiquidity;
        uint256 _currentWithdrawalAmount = withdrawalsInfo[_sender].withdrawalAmount;
        uint256 _tokensToWithdraw =
            Math.min(_currentWithdrawalAmount, convertSTBLToBMIX(liquidity.sub(totalCoverTokens)));

        uint256 _stblTokensToWithdraw = convertBMIXToSTBL(_tokensToWithdraw);

        _stblAmountTether = DecimalsConverter.convertFrom18(_stblTokensToWithdraw, stblDecimals);
        stblToken.safeTransfer(_destiny, _stblAmountTether);
        ...
        if (_currentWithdrawalAmount == 0) {
            delete withdrawalsInfo[_sender];
            liquidityRegistry.tryToRemovePolicyBook(_sender, address(this));
        } else {
            withdrawalsInfo[_sender].withdrawalAllowed = true;
            withdrawalsInfo[_sender].withdrawalAmount = _currentWithdrawalAmount;
        }

        totalLiquidity = liquidity;

        emit LiquidityWithdrawn(_sender, _stblTokensToWithdraw, liquidity);
    }
