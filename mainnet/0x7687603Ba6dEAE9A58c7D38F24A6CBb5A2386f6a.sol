...
abstract contract ReswapPair is
    ...
    ERC1155Holder
{
    ...
    // 90%, must <= 1 - MAX_PROTOCOL_FEE (set in ReswapPairFactory)
    ...
    /**
        ...
        @param isRouter True if calling from ReswapRouter, false otherwise. Not used for
        ETH pairs.
    ...
    function swapTokenForAnyNFTs(
        ...
        IReswapPairFactoryLike _factory = factory();
        ICurve _bondingCurve = bondingCurve();
        IERC721 _nft = nft();
    ...
    function factory() public pure returns (IReswapPairFactoryLike _factory) {
    ...
    function _calculateBuyInfoAndUpdatePoolParams(
        ...
        IReswapPairFactoryLike _factory
    ...
    /**
        ...
        @param isRouter Whether or not the caller is ReswapRouter
        @param routerCaller If called from ReswapRouter, store the original caller
        @param _factory The ReswapPairFactory which stores ReswapRouter allowlist info
    ...
    /**
        ...
        @dev We send ETH back to the caller even when called from ReswapRouter because we do an aggregate slippage check for certain bulk swaps. (Instead of sending directly back to the router caller) 
        Excess ETH sent for one swap can then be used to help pay for the next swap.
     */
    function _refundTokenToSender(uint256 inputAmount) internal virtual;

    /**
        @notice Sends protocol fee (if it exists) back to the ReswapPairFactory from the pair
     */
    ...
    /**
        ...
        @dev This is used by the ReswapPair's swapNFTForToken function. 
    ...
    function _takeNFTsFromSender(
    ...
    ) internal virtual {
        {
            ...
            if (isRouter) {
                ...
                ReswapRouter router = ReswapRouter(payable(msg.sender));
    ...
    /**
        @dev Used internally to grab pair parameters from calldata, see ReswapPairCloner for technical details
     */
 ...
 */
abstract contract ReswapPairETH is ReswapPair {
    ...
    // @dev see ReswapPairCloner for params length calculation
...
/**
    ...
    @author boredGenius and 0xmons
 */
abstract contract ReswapPairERC20 is ReswapPair {
    using SafeTransferLib for ERC20;
    ...
    /**
        ...
        @dev See ReswapPairCloner for an explanation on how this works
     */
...
interface IReswapPairFactoryLike {
    enum PairVariant {
        ENUMERABLE_ETH,
    ...
    function routerStatus(ReswapRouter router)
        external
        view
...
/**
    ...
    @author boredGenius and 0xmons
 */
abstract contract ReswapPairEnumerable is ReswapPair {
...
/**
    ...
    @author boredGenius and 0xmons
 */
abstract contract ReswapPairMissingEnumerable is ReswapPair {
...
contract ReswapPairFactory is Ownable, IReswapPairFactoryLike {
    using ReswapPairCloner for address;
    using SafeTransferLib for address payable;
    using SafeTransferLib for ERC20;
    ...
    ReswapPairEnumerableETH public immutable enumerableETHTemplate;
    ReswapPairMissingEnumerableETH public immutable missingEnumerableETHTemplate;
    ReswapPairEnumerableERC20 public immutable enumerableERC20Template;
    ReswapPairMissingEnumerableERC20 public immutable missingEnumerableERC20Template;
    ...
    mapping(ReswapRouter => RouterStatus) public override routerStatus;
    ...
    event RouterStatusUpdate(ReswapRouter router, bool isAllowed);

    constructor (
        ReswapPairEnumerableETH _enumerableETHTemplate,
        ReswapPairMissingEnumerableETH _missingEnumerableETHTemplate,
        ReswapPairEnumerableERC20 _enumerableERC20Template,
        ReswapPairMissingEnumerableERC20 _missingEnumerableERC20Template,
    ...
    function createPairETH(
        ...
        ICurve _bondingCurve,
        address payable _assetRecipient,
        ReswapPair.PoolType _poolType,
        uint128 _delta,
        uint96 _fee,
        uint128 _spotPrice,
        uint256[] calldata _initialNFTIDs
    ) external payable returns (ReswapPairETH pair) {
        require(
    ...
    /**
        @notice Checks if an address is a ReswapPair. Uses the fact that the pairs are EIP-1167 minimal proxies.
        ...
        returns (bool)
    {
        if (variant == PairVariant.ENUMERABLE_ERC20) {
            return
                ReswapPairCloner.isERC20PairClone(
                    address(this),
            ...
            return
                ReswapPairCloner.isETHPairClone(
                    address(this),
        ...
        onlyOwner
    {
        ...
        if (isAllowed) {
            require(
                !routerStatus[ReswapRouter(target)].wasEverAllowed,
                "Can't call router"
            );
        }
    ...
    /**
        ...
        @param isAllowed True to whitelist, false to remove from whitelist
     */
    function setRouterAllowed(ReswapRouter _router, bool isAllowed)
        external
        onlyOwner
    {
    ...
    */
    ...
    function depositERC20(
        ...
        if (
            ...
            if (token == ReswapPairERC20(recipient).token()) {
...
contract ReswapRouter {
    ...
    IReswapPairFactoryLike public immutable factory;

    constructor(IReswapPairFactoryLike _factory) {
        factory = _factory;
    }
    ...
    function pairTransferERC20From(
        ...
        address to,
        uint256 amount,
        IReswapPairFactoryLike.PairVariant variant
    ) external {
        ...
        require(
            variant == IReswapPairFactoryLike.PairVariant.ENUMERABLE_ERC20 ||
                variant ==
                IReswapPairFactoryLike.PairVariant.MISSING_ENUMERABLE_ERC20,
            "Not ERC20 pair"
        );
...
library ReswapPairCloner {
    ...
    function cloneETHPair(
        ...
        IReswapPairFactoryLike factory,
        ICurve bondingCurve,
        IERC721 nft,
        uint8 poolType
    ...
    /**
     * @notice Checks if a contract is a clone of a ReswapPairETH.
     * @dev Only checks the runtime bytecode, does not check the extra data.
     * @param factory the factory that deployed the clone
     * @param implementation the ReswapPairETH implementation contract
    ...
    /**
     * @notice Checks if a contract is a clone of a ReswapPairERC20.
     * @dev Only checks the runtime bytecode, does not check the extra data.
     * @param implementation the ReswapPairERC20 implementation contract
     * @param query the contract to check
     * @return result True if the contract is a clone, false otherwise
     */
