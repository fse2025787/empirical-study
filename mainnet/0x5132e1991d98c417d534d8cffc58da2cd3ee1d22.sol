...
contract CryptoPepes is ERC721, Ownable {
    ...
    uint256 public constant frogPrice = 50000000000000000; // 0.05 ETH

    uint public constant maxFrogPurchase = 20;

    uint256 public constant MAX_FROGS = 3333;
    ...
    function mintFrogs(address _to, uint256 _reserveAmount) public onlyOwner {
    ...
    function mintFrog(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= maxFrogPurchase, "Can only mint 20 tokens at a time");
    ...
    function changeFrogName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this frog!");
        require(sha256(bytes(_name)) != sha256(bytes(frogNames[_tokenId])), "New name is same as the current one");
        frogNames[_tokenId] = _name;

        emit frogNameChange(msg.sender, _tokenId, _name);
