...
contract mangaforce is ERC721, PaymentSplitter {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares)
        ERC721("0xMANGAFORCE", "MANGAFORCE")
        PaymentSplitter(_payees, _shares)
    {}

    function mint() external payable {
        if (totalSupply + 1 > 2936) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "ipfs://bafybeiev6egrosqzlaoyqo4lrgddpkdiosoaet5s5yuwix4rot4qbsbdya/",
