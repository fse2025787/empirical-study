	...
	BaseContract
{
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return string(abi.encodePacked(getState().tokenBaseExternalUrl, getState().symbol, "/"));
    }
    ...
    function transferFrom(
        ...
        console.log("approvedAddress: %s ", approvedAddress);
        // The nested ifs save around 20+ gas over a compound boolean condition.
        if (!_isOwnerOrApproved(approvedAddress, from, _msgSenderERC721A()))
        {
            console.log("from: %s , _msgSenderERC721A: %s", from, _msgSenderERC721A());
...
library LibDiamond {
    ...
    function addFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            ...
            // console.log("oldFacetAddress: %s , _facetAddress: %s", oldFacetAddress,  _facetAddress);
...
library console {
	...
	function log(address p0, bytes4 p1, address p2) internal view {
		_sendLogPayload(abi.encodeWithSignature("log(address,bytes4,address)", p0, p1, p2));
	}
