    ...
    ReentrancyGuard
{
   ...
   // uint256 public constant SEED = 0;
    ...
    bool private _active1;
    bytes32 public root;
    mapping(address => uint256) public howmanyList;
    ...
    function mintNewOnes(uint256 _amount) external payable nonReentrant {
        require(_active1, "mint not open");
        require(totalSupply(PASS) + 1 <= maxSupply[PASS], "Exceeded max supply");
        require(price * _amount <= msg.value,"not enough eth sent");
        require(msg.sender == tx.origin);
        require(howmanyNew[msg.sender] + _amount <= maxMint,"mint limit reach");
        _mint(_msgSender(), PASS, _amount, "");
    ...
    function mintWlist(bytes32[] calldata proof) external payable {
        require(_active,"whitelist mint not open");
        require(totalSupply(PASS) + 1 <= maxSupply[PASS], "Exceeded max supply");
        require(isWhiteListed(msg.sender, proof), "Not on the whitelist");
        require(msg.sender == tx.origin);
    	require(howmanyList[msg.sender] + 1 <= 1,"mint limit reach");
        _mint(msg.sender, PASS, 1, "");
        howmanyList[msg.sender] += 1;
    }
    ...
    function toggleActive1() external onlyOwner {
        _active1 = !_active1;
    }

    function setBurnAddress(address _addr) external onlyOwner {
        burnAAddress = _addr;
    }

    function burn(address tokenOwner, uint256 typeId, uint256 amount)
        public nonReentrant override(ERC1155Burnable) {
       //  require(msg.sender == burners[typeId], "Invalid burner address");
       require(msg.sender == burnAAddress,"not allowed to burn");
    ...
    function active1() external view returns (bool) {
        return _active1;
    }
