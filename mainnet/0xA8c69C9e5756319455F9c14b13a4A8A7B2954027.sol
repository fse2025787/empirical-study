...
interface IPool {
    ...
    function initializeVesting(uint256[7] memory _vestingInit) external;
...
interface IPoolManager {
    ...
    function countTotalPay(address[] memory _address)
        external
        view
        returns (uint256);
    ...
    function checkKycAuditService(address[] memory _partnerAddress)
        external
        view
        returns (bool[2] memory status);
...
contract PoolFactory is OwnableUpgradeable {
    ...
    function initialize() external initializer {
        __Ownable_init();
        master = 0xdC63703Cc97b9423ed8F1e3658b5776F14Fa2B3D;
        privatemaster = 0x61788674C92d950bF14a87271dB51B821aa543b1;
        poolManager = 0x7413Ab5769CA76597d51c38633B79A8A82B6cf9C;
        fairmaster = 0x9Eb57fC71dF47b93eE86223f9e92d394403124B5;
        kycPrice = 500000000000000000;
        auditPrice = 2000000000000000000;
        masterPrice = 500000000000000000;
        privatemasterPrice = 500000000000000000;
        fairmasterPrice = 500000000000000000;
        contributeWithdrawFee = 100;
        version = 1;
        IsEnabled = true;
        poolOwner = address(this);
        partnerFee = 1000;
        adminWallet = payable(0x71195aC7A1dAA8e66d7B38C16860F69B7a20e797);
    }
    ...
    function initalizeClone(
        ...
        bool[2] memory check = IPoolManager(poolManager).checkKycAuditService(
            _partnerAddress
        );
    ...
    function createSale(
        ...
        checkfees(_saleInfo[10], _saleInfo[11], _partnerAddress);
        ...
        uint256 payed = IPoolManager(poolManager).countTotalPay(
            _partnerAddress
        );

        uint256 feesCount = payed.mul(partnerFee).div(10000);
        uint256 totalPartnerPay = payed.sub(feesCount);
        IPoolManager(poolManager).payAmaPartner{value: totalPartnerPay}(
            _partnerAddress,
            pair
        );

        uint256 totalPay = msg.value;
        uint256 extraAmount = totalPay.sub(totalPartnerPay);
        (bool success, ) = adminWallet.call{ value: extraAmount }("");
    ...
    function createPrivateSale(
        ...
        checkPrivateSalefees(_saleInfo[10], _saleInfo[9], _partnerAddress);
        bytes32 salt = keccak256(
            abi.encodePacked(_poolDetails, block.timestamp)
        );
    ...
    function initalizeFairClone(
        ...
        address ethAddress = IUniswapV2Router01(_addrs[1]).WAVAX();
    ...
    function getPartnerFees() public view returns (uint256){
        return partnerFee;
    }
}