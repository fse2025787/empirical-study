...
contract SmartWalletUpgradeableV2 is
  SmartWalletUpgradeable  
{  
   ...
   * Internal functions
   */  
  function _transfer(bytes4 interfaceId, address tokenAddress, address from, address to, uint256 amount) internal virtual override(SmartWalletUpgradeable) {
    address contractAddress = address(this);
    ...
    if (interfaceId == INTERFACE_ID_IERC20) {
      ...
      // Deposit
        ...
        IERC20Upgradeable(tokenAddress).safeTransferFrom(from, to, amount);   
      }
    }
    else {
      revert("SmartWalletUpgradeable: token contract interface not supported");
