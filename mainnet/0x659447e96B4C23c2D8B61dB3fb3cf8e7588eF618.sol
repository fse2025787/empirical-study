...
interface EtherWaifu {
    function transfer(address to, uint256 tokenId) external;
    function transferFrom(address from, address to, uint256 tokenId) external;
}

contract WrappedEtherWaifu is ERC721 {

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdTracker;

    EtherWaifu public _etherWaifu = EtherWaifu(0x36697e362Ee7E9CA977b7550B3e4f955fc5BF27d);

    mapping(uint => uint) public _waifuIDToTokenID;
    mapping(uint => uint) public  _tokenIDToWaifuID;
    string private _baseTokenURI;
    address public _owner = 0xD2927a91570146218eD700566DF516d67C5ECFAB;


    event Wrapped(uint indexed WaifuId, uint tokenID);
    event Unwrapped(uint indexed WaifuId, uint tokenID);

    constructor() ERC721("Wrapped EtherWaifu", "WEW") {
        _setBaseURI("https://api.ethrelics.com/etherwaifu/");
    }
    ...
    function contractURI() public view returns (string memory) {
        return "https://api.ethrelics.com/etherwaifu/storefront";
    }
    function wrap(uint256 waifuId) public {
        _etherWaifu.transferFrom(_msgSender(), address(this), waifuId);
        uint tokenIDToAssign = _tokenIdTracker.current();
        _tokenIdTracker.increment();
        _waifuIDToTokenID[waifuId] = tokenIDToAssign;
        _tokenIDToWaifuID[tokenIDToAssign] = waifuId;
        _mint(_msgSender(), tokenIDToAssign);
        emit Wrapped(waifuId, tokenIDToAssign);

    }

    function unwrap(uint256 tokenID) public {
        uint waifuId = _tokenIDToWaifuID[tokenID];
        address owner = ownerOf(tokenID);
        require(owner == _msgSender()); //only owner can unwrap
        _etherWaifu.transfer(owner, waifuId); //transfer back to owner
        _burn(tokenID);
        emit Unwrapped(waifuId, tokenID);
    }
