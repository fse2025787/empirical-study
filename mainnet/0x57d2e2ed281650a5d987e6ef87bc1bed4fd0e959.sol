...
abstract contract CurveVoterProxy is BaseStrategy {
    ...
    function switchDex(uint256 _id, address _dex) external onlyAuthorized {
        require(_dex == uniswap || _dex == sushiswap || _dex == uniswapv3, "!dex");
        dex[_id] = _dex;
        _approveDex();
    }
...
contract Strategy is CurveVoterProxy {
    ...
    constructor(address _vault) public CurveVoterProxy(_vault) {
        ...
        gauge = address(0x4dC4A289a8E33600D8bD4cf5F6313E43a37adec7);
        keepCRV = 1000;

        _approveBasic();
        pathTarget = new address[](2);
        _setPathTarget(0, 0); // crv path target
        _setPathTarget(1, 1); // rsr path target
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(rsr).approve(dex[1], 0);
        IERC20(rsr).approve(dex[1], type(uint256).max);
    }
        ...
        )
    {
        ...
        if (_rsr > 0) {
            address dex = dex[1];
            if (dex == uniswapv3) {
                UniV3(dex).exactInput(UniV3.ExactInputParams(
                    abi.encodePacked(
                        rsr, 
                        uint24(3000), 
                        weth, 
