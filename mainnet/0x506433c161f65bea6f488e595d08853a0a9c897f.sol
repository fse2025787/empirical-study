...
contract Meltdown is Ownable, ERC721A, ReentrancyGuard {
  ...
  string private defaultTokenURI = 'ipfs://QmZv2GHXtYBNnZqUCTk8cbuzF6kPxrz4iM1549yHZmXbAZ';
  ...
  function publicMint(uint256 quantity) external payable {
    require(PHASE == 1, "Public mint is closed. Please check back later and follow us on twitter: @MeltdownNFT");
  ...
  function mint(Coupon memory coupon) external payable {
    require(PHASE > 1, "OG and WL mint hasn't started yet. Please check back later and follow us on Discord or Twitter: @MeltdownNFT");
    require(
    ...
    if (PHASE == 2) { // WL mint
      ...
      bytes32 digestWL = keccak256(
        abi.encode(CouponType.WL, msg.sender)
      );
      require(
        _isVerifiedCoupon(digestWL, coupon), 
        'Invalid WL signature. Maybe you are an OG member? Please wait until OG opens.'
      );
    } else if (PHASE == 3) { // OG mint
      bytes32 digestOG = keccak256(
        abi.encode(CouponType.OG, msg.sender)
      );
      require(
