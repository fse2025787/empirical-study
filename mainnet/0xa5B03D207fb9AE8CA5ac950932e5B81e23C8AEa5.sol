...
// 
//Experimenting with different claiming mechanisms to estimate gas usage
...
contract Villain is ERC721URIStorage, Ownable, ERC721Enumerable {
    ...
    mapping (uint256 => uint256) private _villainSatoshiPairsStorage;

    //Fires on a new mint
    event VillainClaim(address nftOwner);

    address private _wasNFTSmartContract;
    IWASNFT wasNFTContract = IWASNFT(_wasNFTSmartContract);

    constructor(uint256 initSupply, address wasScAddress) ERC721("WAS Villain", "WAS-Villain") {
    ...
    //Return the array of We Are Satoshis that claimed their Villain
    function getAllSatoshiIdsThatClaimedAVillain()
    external
    view
    ...
    function didSatoshiClaimAVillain(uint256 satoshiId)
    public
    view
    returns(bool){
    ...
    //For given villain ID, this method returns satoshi ID it originated from
    function getVillainOriginSatoshi(uint256 villainId)
    public
    view
    returns(uint256){
        return _villainSatoshiPairsStorage[villainId];
    }
    ...
    public
    {
        ...
        //emmit only one event per claim tx instead an event for each Villain claimed
        if (newMints){
            emit VillainClaim(to);
        }
    }
    ...
    private
    {
        ...
        _villainSatoshiPairsStorage[newItemId] = satoshiId;
    }
