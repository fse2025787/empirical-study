...
// File: contracts/StabilizeMigrationPool.sol

pragma solidity =0.6.6;

// This is for locking of STBZ in preparation for Arbitrum

contract StabilizeMigrationPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        migrationEnd = now.add(16 days); // Can migrate tokens up to 16 days from contract launch
    }
    ...
    function lock(uint256 amount) external nonReentrant
    {
        ...
        require(now < migrationEnd, "Too late to lock tokens");
        _totalLocked = _totalLocked.add(amount);
        _balances[_msgSender()] = _balances[_msgSender()].add(amount);
        lockedToken.safeTransferFrom(_msgSender(), address(this), amount);
        emit TokenLocked(_msgSender(), amount);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function removeForTreasuryBurn() external onlyGovernance
    {
        // After migration ends, governance will burn these tokens into the treasury furnace
