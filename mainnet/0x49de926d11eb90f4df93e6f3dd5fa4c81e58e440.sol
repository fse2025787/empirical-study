...
contract Devour is ERC20, Ownable {
	...
	bool private _addToLiquidity = true;
	bool private _canSwapAndLiquify = true;
	bool private _applyPriceVariable = true;
	uint256 public minimumTokensBeforeSwap = initialSupply * 25 / 10000;
	uint256 public gasForProcessing = 500000;

	address public marketingWallet = 0xf254Ac119AcdDE9d64afC9a622c0a86bF6473ff8;
	address public liquidityWallet = 0x2988F8A65E69Ce9203B57E4a82828560c5735c1f;
	address public foundationWallet = 0xEb2c05E949B587D7c2918C5543F567eC302eF2e7;
	address public devWallet = 0x2988F8A65E69Ce9203B57E4a82828560c5735c1f;
	...
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000, "Devour: gasForProcessing must be greater than 200,000");
	...
	function setAddToLiquidity(bool newValue) external onlyOwner {
		_addToLiquidity = newValue;
	}
	function setSwapAndLiquify(bool newValue) external onlyOwner {
		_canSwapAndLiquify = newValue;
	}
	function setPriceVariable(bool newValue) external onlyOwner {
		_applyPriceVariable = newValue;
	}
	...
	function _adjustTaxes(address to, address from, bool isBuyFromLp, bool isSelltoLp, uint256 amount) private {
		...
		if (isSelltoLp) {
			...
			if(_applyPriceVariable && (from != owner() && to != owner())) {
	...
	function _swapAndLiquify() private {
		...
		(success,) = address(foundationWallet).call{value: amountEthFoundation}("");
		(success,) = address(devWallet).call{value: amountEthDev}("");

		if (amountToLiquify > 0 && _addToLiquidity) {
