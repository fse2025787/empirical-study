...
|        \                    |  \              |  \     /  \ /      \                              
 \$$$$$$$$______    _______  _| $$_    __    __ | $$\   /  $$|  $$$$$$\ ______    ______    _______ 
   | $$  |      \  /       \|   $$ \  |  \  |  \| $$$\ /  $$$| $$_  \$$/      \  /      \  /       \
   | $$   \$$$$$$\|  $$$$$$$ \$$$$$$  | $$  | $$| $$$$\  $$$$| $$ \   |  $$$$$$\|  $$$$$$\|  $$$$$$$
   | $$  /      $$ \$$    \   | $$ __ | $$  | $$| $$\$$ $$ $$| $$$$   | $$    $$| $$   \$$ \$$    \ 
   | $$ |  $$$$$$$ _\$$$$$$\  | $$|  \| $$__/ $$| $$ \$$$| $$| $$     | $$$$$$$$| $$       _\$$$$$$\
...
Source code based off ERC721B implementation by GoldenX team
...
abstract contract TM721 is Context, ERC165, IERC721, IERC721Metadata {
  ...
  function ownerOf(uint tokenId) public view override returns( address owner ){
    require(_exists(tokenId), "TM721: query for nonexistent token");
  ...
  function safeTransferFrom(address from, address to, uint tokenId, bytes memory _data) public override {
    require(_isApprovedOrOwner(_msgSender(), tokenId), "TM721: caller is not owner nor approved");
  ...
  function _checkOnERC721Received(address from, address to, uint tokenId, bytes memory _data)private returns ( bool ){
    if (to.isContract()) {
      try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
        ...
        if (reason.length == 0) {
          revert("TM721: transfer to non ERC721Receiver implementer");
        } else {
          assembly {
  ...
  function _safeTransfer(address from, address to, uint tokenId, bytes memory _data) internal{
    _transfer(from, to, tokenId);
    require(_checkOnERC721Received(from, to, tokenId, _data), "TM721: transfer to non ERC721Receiver implementer");
  }

  function _transfer(address from, address to, uint tokenId) internal {
    require(ownerOf(tokenId) == from, "TM721: transfer of token that is not own");
...
abstract contract TM721Enumerable is TM721, IERC721Enumerable {
  function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, TM721) returns( bool isSupported ){
  ...
  function tokenOfOwnerByIndex(address owner, uint index) external view override returns( uint tokenId ){
    ...
    revert( "TM721Enumerable: owner index out of bounds" );
  }

  function tokenByIndex(uint index) external view override returns( uint tokenId ){
    require(index < tokens.length, "TM721Enumerable: query for nonexistent token");
    return index;
  }
}
...
abstract contract TM721Batch is TM721Enumerable, IERC721Batch {
...
contract TastyMfers is TM721Batch, Ownable {
  ...
  constructor() TM721("TastyMfers", "TMF") { }


  //view: IERC721Metadata
  function tokenURI( uint tokenId ) external view override returns( string memory ){
    require(_exists(tokenId), "TastyMfers: query for nonexistent token");
  ...
  function mint( uint quantity) external payable {
    require( quantity <= MAX_ORDER,         "TastyMfers: order too big"             );
    require( msg.value >= PRICE * quantity, "TastyMfers: ether sent is not correct" );
    require( isSaleActive,                  "TastyMfers: sale is not active");

    uint supply = totalSupply();
    require( supply + quantity <= MAX_SUPPLY, "TastyMfers: mint/order exceeds supply" );
  ...
  function teamMint() external onlyOwner{
    require(!teamMinted, "TastyMfers: team already minted");
  ...
  function mintTo(address[] calldata accounts, uint[] calldata quantity ) external payable onlyOwner{
    require(quantity.length == accounts.length, "TastyMfers: must provide equal quantities and accounts" );
    ...
    uint supply = totalSupply();
    require( supply + totalQuantity < MAX_SUPPLY, "TastyMfers: mint/order exceeds supply" );
  ...
  function resurrect( address[] calldata accounts, uint[] calldata tokenIds ) external onlyOwner{
    require(tokenIds.length == accounts.length,   "TastyMfers: must provide equal tokenIds and accounts" );
  ...
  function _burn( address from, uint tokenId ) private {
    require( _exists( tokenId ), "TastyMfers: query for nonexistent token" );
    require( from == tokens[ tokenId ].owner, "TastyMfers: owner mismatch" );
  ...
  function _mint( address to, uint tokenId ) private {
    ...
    if( tokenId < tokens.length ){
      require( !_exists( tokenId ), "TastyMfers: can't resurrect existing token" );
      --burned;
      tokens[ tokenId ].owner = to;
    }
    else{
