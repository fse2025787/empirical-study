...
contract LiquidForge is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    mapping(address => uint256) public _membershipForgeBlocks;
    ...
    function revokeLegendReward(uint256[] calldata tokenIds) public onlyOwner {
      ...
      returns (uint256) 
    {
      ...
      return legendsRate * 
          ...
          (Math.min(block.number, expiration) - 
            Math.max(_legendsForgeBlocks[tokenId],_membershipForgeBlocks[account]) );
        }
      ...
      return tigerRate * 
          ...
          (Math.min(block.number, expiration) - 
            Math.max(_tigerForgeBlocks[tokenId],_membershipForgeBlocks[account]) );
        }
      ...
      return cubsRate * 
          ...
          (Math.min(block.number, expiration) - 
            Math.max(_cubsForgeBlocks[tokenId],_membershipForgeBlocks[account]) );
        }
      ...
      return padRate * 
          ...
          (Math.min(block.number, expiration) - 
            Math.max(_padForgeBlocks[tokenId],_membershipForgeBlocks[account]) );
        }
      ...
      return azukiRate * 
          ...
          (Math.min(block.number, expiration) - 
            Math.max(_azukiForgeBlocks[tokenId],_membershipForgeBlocks[account]) );
        }
        return 0;
    }
    ...
    function Forge(uint256[] calldata tokenIds,address nftaddress) external whenNotPaused {
        ...
        require(forgeSet.length() > 0, "No Membership Forged");
        for (uint256 i; i < tokenIds.length; i++) {             
                require(IERC721(nftaddress).ownerOf(tokenIds[i]) == msg.sender,"you do not own that NFT");         
            }
 ...
 //check forge amount. 
    ...
    function membershipForge(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        _membershipForgeBlocks[msg.sender] = blockCur;
    }
