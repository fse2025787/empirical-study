...
contract SourSopKidz is ERC721Enumerable, Ownable {
    ...
    function mintNFT(uint256 _cnt) public payable {
        ...
        require(!pauseMint, "Paused!");
        require(tokenMinted < MAX_NFT_SUPPLY, "Sale has already ended");
        require(balanceOf(msg.sender).add(_cnt) <= MaxMintPerWallet, "Exceded the Minting Count per Wallet.");
        require(getNFTPrice().mul(_cnt) == msg.value, "ETH value sent is not correct");

        if(!publicSale) {
            require(whitelist[msg.sender], "Not ");
            require(_cnt <= MaxMintPerWallet, "Exceded the Minting Count");
        }
    ...
    function setMaxMint(uint256 _maxmint_) external onlyOwner() {
        MaxMintPerWallet = _maxmint_;
    }
