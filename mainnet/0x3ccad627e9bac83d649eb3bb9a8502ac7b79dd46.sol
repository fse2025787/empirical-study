...
contract CoolApesNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public constant CA_GIFT_VOLUME = 300;
    uint256 public constant CA_PRIVATE_VOLUME = 1500;
    uint256 public constant CA_PUBLIC_VOLUME = 3200;
    uint256 public constant CA_MAX_VOLUME = CA_PUBLIC_VOLUME + CA_PRIVATE_VOLUME + CA_GIFT_VOLUME;
    uint256 public constant CA_PUBLIC_PRICE = 0.03 ether;
    uint256 public constant CA_PRIVATE_PRICE = 0.02 ether;
    uint256 public constant CA_PER_MINT = 5;
    
    mapping(string => bool) private _usedNonces;
    
    string private _contractURI;
    string private _tokenBaseURI = "https://api.coolapes.org/metadata/";
    address private _signerAddress = 0x1d25c29A84dff118C81323C7c5313bb02A867512;
    address private _privateSignerAddress = 0x4049196E94748fCf0fF4F9e6B52b5fD804D4c018 ;
    ...
    constructor() ERC721("Cool Apes", "CAPE") {}
   ...
   function matchAddresSigner(bytes32 hash, bytes memory signature, bool isPrivate) private view returns(bool) {
    ...
    function buy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
      ...
      require(tokenQuantity <= CA_PER_MINT, "EXCEED_CA_PER_MINT");
      require(CA_PUBLIC_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function privateBuy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
      ...
      require(hashTransaction(msg.sender, tokenQuantity, nonce) == hash, "HASH_FAIL");
      require(totalSupply() < CA_MAX_VOLUME, "OUT_OF_STOCK");
      require(privateAmountMinted + tokenQuantity <= CA_PRIVATE_VOLUME, "EXCEED_PRIVATE");
      require(tokenQuantity <= CA_PER_MINT, "EXCEED_CA_PER_MINT");
      require(CA_PRIVATE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        ...
        require(giftedAmount + receivers.length <= CA_GIFT_VOLUME, "AIRDROP_EMPTY");
