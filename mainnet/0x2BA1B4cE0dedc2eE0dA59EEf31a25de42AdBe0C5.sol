 ...
 * @notice It distributes Fraktal tokens with rolling Merkle airdrops.
 */
contract TradingRewardsDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    IERC20 public immutable frakToken;
    ...
    // Total amount claimed by user (in ƒ)
    ...
    /**
     * @notice Constructor
     * @param _frakToken address of the Frak token
     */
    constructor(address _frakToken) {
        frakToken = IERC20(_frakToken);
        _pause();
    }
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        ...
        frakToken.safeTransfer(msg.sender, adjustedAmount);
    ...
    /**
     * @notice Transfer FRAK tokens back to owner
    ...
    function withdrawTokenRewards(uint256 amount) external onlyOwner whenPaused {
        ...
        frakToken.safeTransfer(msg.sender, amount);
