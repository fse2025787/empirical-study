...
contract ATSToken is ERC20Detailed, ERC20, Ownable {
    address foundingTeam = 0xF3BB0BE550656E2fa9eB5dc4e0dC183d5D69c8ed;
    address investors = 0x5970699d6BB23abE1cb12e096AaD5aD6FDD256cA;
    address sales = 0x7482DBEE10EAd9b20D081f64d8540e4BB32461D9;
    ...
    constructor() public ERC20Detailed("ATTLAS", "ATS", 8) {

        totalTokens = 100000000 * 10 ** uint256(decimals());
        _mint(owner(), totalTokens); // total supply fixed at 100 million token

        ERC20.transfer(investors, 10000000 * 10 ** uint256(decimals()));

        ERC20.transfer(sales, 50000000 * 10 ** uint256(decimals()));


        foundingTeamMap[1]=1640995200; // 2022-01-01T00:00:00Z
        foundingTeamMap[2]=1672531200; // 2023-01-01T00:00:00Z
        foundingTeamMap[3]=1704067200; // 2024-01-01T00:00:00Z
        foundingTeamMap[4]=1735689600; // 2025-01-01T00:00:00Z

        for(uint i = 1; i<= 4; i++) {
            transferWithLock(foundingTeam, 10000000 * 10 ** uint256(decimals()), foundingTeamMap[i]);
        }
    }
    ...
    function transferWithLock(address _receiver, uint256 _amount, uint256 _releaseDate) public returns (bool success) {
        require(msg.sender == foundingTeam || msg.sender == investors || msg.sender == sales || msg.sender == owner());
