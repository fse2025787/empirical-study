...
contract MoonMissers is ERC721Enumerable, OpenSea, Ownable, PaymentSplitter {
    ...
    uint256 public price = 5900000000000000; // 0.0059 ETH
    uint256 public constant MAX_SUPPLY = 6666;
    uint256 public maxPerTx = 10;
    uint256 public maxPerAddress = 20;
    uint256 public freePerAddress = 2;

    bool public started = false;

    uint256 private FREE_SUPPLY = 500;
    bool private freeBuy = false;
    ...
    ) ERC721("MoonMissers", "MOONM") PaymentSplitter(_payees, _shares) {
    ...
    function mint(uint256 count) public payable {
        ...
        require(_numberMinted(msg.sender) + count <= maxPerAddress, "Exceed max per wallet");
        ...
        if (!freeBuy){
            if (supply < FREE_SUPPLY && supply + count > FREE_SUPPLY) {
                payCount = supply + count - FREE_SUPPLY;
            }
            else if (supply >= FREE_SUPPLY) {
                payCount = count;
            }
        }
        uint256 freeMintsForAddressLocal = freeMintsForAddress(msg.sender);
        if (freeMintsForAddressLocal>0 && payCount > 0) {
            if (payCount > freeMintsForAddressLocal) {
                payCount = payCount - freeMintsForAddressLocal;
            }
            else{
                payCount = 0;
            }
        ...
        for (uint256 i; i < count; i++) {
            ...
            supply++;
            _addressData[msg.sender].numberMinted++;
        }
    }
    ...
    function setFreeBuy(bool value) external onlyOwner {
        freeBuy = value;
    }
    ...
    function isFreeBuy() public view returns (bool) {
        return bool(freeBuy);
    }

    function freeMintsForAddress(address owner) public view returns (uint256) {
        uint256 numberMintedForAddress = _numberMinted(owner);
        if (numberMintedForAddress < freePerAddress) {
            return freePerAddress - numberMintedForAddress;
        }
        else {
            return 0;
        }
