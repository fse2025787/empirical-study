...
// File: contracts\SpiderDAONest12.sol
...
contract SpiderDAONest12 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public immutable stakingCap;
    uint256 public totalReward;
    uint256 public rewardBalance;
    uint256 public stakedBalance;
    uint256 public constant DEPOSIT_TIME = 365 days;
    uint256 public constant REWARD_PERC = 80; //80 %
    address public rewardAddress = 0x9486a25530fF059aA8Bd6D1AB74E4c65527140fC;
    bool private depositEnabled = true;

    IERC20 immutable SpiderDAOToken;
    ...
    constructor(string memory name_) public {
        name = name_;
        SpiderDAOToken = IERC20(0xbcD4b7dE6fde81025f74426D43165a5b0D790Fdd);
        stakingStarts = 1611162000; //Wednesday, January 20, 2021 5:00:00 PM
        stakingEnds = 1623690000; // Monday, June 14, 2021 5:00:00 PM
        stakingCap = 15000000 ether;
    }
    ...
    function canWithdraw(address _addy) public view returns (bool) {
    ...
    function _withdrawAfterClose(address from, uint256 amount) private {
        ...
        SpiderDAOToken.safeTransferFrom(rewardAddress, from, reward); //transfer Reward
        SpiderDAOToken.safeTransfer(from, amount); //transfer initial stake
        _stakes[from] = _stakes[from].sub(amount);
        stakedTotal = stakedTotal.sub(amount);
        stakedBalance = stakedBalance.sub(amount);
        emit PaidOut(address(SpiderDAOToken), from, amount, reward);
    }
        ...
        _before(stakingEnds)
    {
        ...
        SpiderDAOToken.safeTransferFrom(staker, address(this), remaining);

        if (remaining < amount) {
            // Return the unstaked amount to sender (from allowance)
            uint256 refund = amount.sub(remaining);
            SpiderDAOToken.safeTransferFrom(address(this), staker, refund);
            emit Refunded(address(SpiderDAOToken), staker, refund);
        }
        ...
        emit Staked(address(SpiderDAOToken), staker, amount, remaining);
    }

    function emergencyWithdraw() external {
        SpiderDAOToken.safeTransfer(msg.sender,_stakes[msg.sender]);
         stakedTotal = stakedTotal.sub(_stakes[msg.sender]);
        stakedBalance = stakedBalance.sub(_stakes[msg.sender]);
        _stakes[msg.sender] = 0;

    }

    function emergencyWithdrawAdmin() external onlyOwner {
        SpiderDAOToken.safeTransfer(msg.sender,SpiderDAOToken.balanceOf(address(this)));
