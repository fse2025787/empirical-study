  ...
  ReentrancyGuardUpgradeable
{
   ...
   * @param _pRoundIndex Current round index 0 -> private 1-> whitelist 2 -> public
   */
  function fUpdateRoundIndex(uint8 _pRoundIndex) external onlyOwner {
    roundIndex = _pRoundIndex;
    emit UpdateRoundIndex(_pRoundIndex);
  }
  ...
  /**
   * @dev Buy a presale NFT Ext
   * @param _pTo Destination address
   * @param _pPoolType 0 -> common; 1 -> rare; 2 -> legendary
   * @param _pIds NFT ids
   * Required Statements
   * - MPR:buy01 Invalid ids
   * - MPR:buy02 Pool limit is reached
   */
  function fBuyExt(
    address _pTo,
    uint8 _pPoolType,
    ...
    require(_pIds.length > 0, "MPR:buy01");
    uint256 amount_ = _pIds.length;
    require(soldByType[_pPoolType] + amount_ <= limitByType[_pPoolType], "MPR:buy02");
    soldByType[_pPoolType] += amount_;
    soldByAddress[_pTo][_pPoolType] += amount_;

    for (uint i = 0; i < amount_; i++) {
      IMintableNFT(mintableNFT).fMint(_pTo, _pIds[i]);
      _logs.push(Logs(_pTo, 0, 0, _pPoolType, _pIds[i], _pIds[i], block.timestamp));
    }
  }
