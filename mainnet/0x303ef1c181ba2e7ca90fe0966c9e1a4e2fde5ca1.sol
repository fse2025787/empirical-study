...
contract NFTsThatCanOwnTokensAndEth is ERC721A("", ""), ReentrancyGuard {
  using Strings for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  ...
  modifier cannotSellToken(uint256 tokenId) {
      bool isSellable = sellableAt[tokenId] > 0 && block.timestamp > sellableAt[tokenId];
      require(!isSellable, "can sell, shouldnt be able to");
      _;
  }
  modifier canSellToken(uint256 tokenId) {
    require(sellableAt[tokenId] > 0 && block.timestamp > sellableAt[tokenId], "Cannot send/sell token yet!");
    _;
  }

  function mint(uint256 _count) external {
    require(mintedTokens[msg.sender] + _count < 6, "5 is the max at a time");
  ...
  function sendTokensToNFT(uint256 tokenId, ERC20[] calldata tokens, uint256[] calldata amounts) nonReentrant external payable {
  ...
  function withdrawTokensFromNFT(uint256 tokenId, ERC20[] calldata tokens, uint256[] calldata amounts, uint256 ethAmount, address _receiver) external onlyOwnerOf(tokenId) cannotSellToken(tokenId){
  ...
  function enableSelling(uint256 tokenId) external onlyOwnerOf(tokenId) {
    sellableAt[tokenId] = block.timestamp + 1 hours;
  }
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
      ...
      parts = string(abi.encodePacked(parts, "ETH balance is ", nodecimals.toString(), '.', decimals.toString(), '</text><text x="10" y="', y.toString(), '" class="base">'));
      y += 20;

      for (uint256 i = 0; i < tokensInNFT[tokenId].length(); i++) {
        ERC20 token = ERC20(tokensInNFT[tokenId].at(i));
        uint256 nodecimals = (balances[tokenId][token] / (10 ** token.decimals()));
        uint256 decimals = ((balances[tokenId][token] % (10 ** token.decimals()))/ (10 ** (token.decimals()-1)));
        parts = string(abi.encodePacked(parts, "balance is ", (nodecimals == 0 ? "<1" : nodecimals.toString()), " ", token.symbol(), '</text><text x="10" y="',((i * 20) + 40).toString(),'" class="base">'));
        y += 20;
      }
      if(isSellable) {
