...
contract ERC721AFacet is BaseFacet, ERC721AUpgradeable, IERC2981 {
    ...
    */
    ...
    function toggleStaking(uint256 tokenId) internal {
        ...
        require(msg.sender == tokenOwner || msg.sender == owner(), "Budda Bears: Only token owner");
    ...
    function banFromStaking(uint256 tokenId) external onlyOwner {
        LibDiamond.AppStorage storage _as = LibDiamond.appStorage();
...
library LibDiamond {
    ...
    struct AppStorage {
        ...
        // tokenId to staking start time (0 = not staked).
        mapping(uint256 => uint256) stakingStarted;
        // Cumulative per-token staking, excluding the current period.
        mapping(uint256 => uint256) stakingTotal;
        uint32 stakingTransfer;
        bool stakingOpen;
    }
