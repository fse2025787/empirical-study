...
interface IProxy {
  event userRegistered(address indexed user, address indexed agent);
  event fundsWithdrawn(address indexed user, address indexed pairAddress, address tokenA, address tokenB, uint amountA, uint amountB);

  function withdrawAll(string calldata agentId, address user, address pairAddress) external returns (bool);
  function updateRouterContract(address uniswapRouterAddress) external returns(bool);
}
