...
contract LoccoStore is Pausable, Ownable, ReentrancyGuard {
  ...
  uint256 public _maxBuyAmount = 2;
  uint256 public constant LIST_IDENTIFIER = 1;
  uint256 public _startsAt = 1646049600; //2022-02-28T12:00:00.000Z
  uint256 public _endsAt = 1646071200; //2022-02-28T18:00:00.000Z
  ...
  function mint(uint256 amount, bytes memory signature)
    external
    payable
    whenNotPaused 
    nonReentrant
  {
    require(block.timestamp >= _startsAt && block.timestamp <= _endsAt, "LoccoStore: Not active");
    require(signer.isValidSignatureNow(keccak256(abi.encodePacked(msg.sender, LIST_IDENTIFIER)), signature), "LoccoStore: Invalid signature");
    require(_amountMinted[msg.sender].add(amount) <= _maxBuyAmount, "LoccoStore: User already minted");
    require(msg.value >= _price.mul(amount), "LoccoStore: msg.value is less than total price");
    _amountMinted[msg.sender] = _amountMinted[msg.sender].add(amount);
