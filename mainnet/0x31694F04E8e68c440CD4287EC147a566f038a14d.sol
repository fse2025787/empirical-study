...
contract DaoHallRegistry is IDaoHallRegistry, Operable {
    ...
    function setWhitelistClaimed(bytes32 node, address claimer, uint256 count) external override onlyOperator{
        _whitelistClaims[node][claimer] = count;
    }

    function getWhitelistClaimed(bytes32 node, address claimer) external view override returns (uint256) {
        return _whitelistClaims[node][claimer];
    }
}