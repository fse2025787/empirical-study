    ...
    MultiTokenForwardingRules
{
    ...
    */
    function deployAndHandleBridgedTokens(
        ...
        name = string(abi.encodePacked(name, " on ETH"));
    ...
    */
    ...
    function executeActionOnBridgedTokens(address _token, address _recipient, uint256 _value) internal {
        ...
        uint256 fee = _distributeFee(FOREIGN_TO_HOME_FEE, _token, minPerTx(_token));
        if (fee > 0) {
            emit FeeDistributed(fee, _token, _messageId);
            valueToMint = _value.sub(fee);
        }
    ...
    */
        ...
        internal
    {
        ...
        if (!isRewardAddress(_from)) {
                fee = _distributeFee(HOME_TO_FOREIGN_FEE, _token, minPerTx(_token));
                valueToBridge = _value.sub(fee);
        }
