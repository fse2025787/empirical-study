...
contract PBCitems is AbstractERC1155Factory  {
    ...
    ********************************************************************************/
    ...
    function purchaseEarlyAccess(uint256 collectionId, uint256 amount, bytes32[] calldata merkleProof) external payable {
        ...
        require(purchaseTxs[collectionId][msg.sender] + amount <= collectionsConfigs[collectionId].maxTxEarly , "max tx amount exceeded");
    ...
    function purchase(uint256 collectionId, uint256 amount) external payable {
        ...
        require(totalSupply(collectionId) + amount <= collectionsConfigs[collectionId].maxSupply, "Max supply reached");
        require(purchaseTxs[collectionId][msg.sender] + amount <= collectionsConfigs[collectionId].maxTxPublic , "max tx amount exceeded");
        require(amount > 0 && amount <= collectionsConfigs[collectionId].maxPerTx, "Purchase: amount prohibited");
    ...
    ********************************************************************************/
    ...
    /**
     * change the amount of NFT an address have already mint
     * @param collectionId, Id of token
     * @param addresses, List of addresses to reset
     * @param number, Choose the way : (0 => All purchase reset / 1 or more => All addresses purchase Minus the number)
     */
    function changePurchaseTxs(uint256 collectionId, address[] memory addresses, uint256 number) external onlyOwner {
        require(collectionsConfigs[collectionId].maxSupply != 0, "This collection doesn't Exist");
        if (number == 0) {
            for (uint256 i = 0; i < addresses.length; i++) {
                purchaseTxs[collectionId][addresses[i]] = 0;
            }
        } else {
            for (uint256 i = 0; i < addresses.length; i++) {
                require(purchaseTxs[collectionId][addresses[i]] > 0, "Addresses should have already mint ");
                purchaseTxs[collectionId][addresses[i]] = purchaseTxs[collectionId][addresses[i]] - 1;
            }
        } 
    }
