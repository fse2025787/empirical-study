    ...
    AccessControlUpgradeable
{
    ...
    uint256 private maxPreSaleSupply;

    uint256 public publicSalePrice;

    address payable public autoTransferReceiverAddress;
    ...
    struct Claim {
        ...
        bool active;
        uint256 totalSupply;
        SaleState requiredSaleState;
    }
    mapping(bytes32 => Claim) claims;

    mapping(bytes32 => uint256) claimsTotalMints;
    ...
    event SetAutoTransferAddress(address indexed dest);
    ...
    modifier canReserveTokens(uint256 numberOfTokens) {
        //require(numReservedTokens + numberOfTokens <= MAX_RESERVE_TOKENS, 'Insufficient token reserve');
        require(
    ...
    modifier isValidPrice(uint256 price) {
        require(
            price >= 0.0001 ether,
            'minimal price must be at least 100000000000000 wei (0.0001 ether)'
        );
        _;
    }

    modifier isValidClaim(Claim memory claim, bool add) {
        if (add) {
            require(
                claims[claim.id].id != claim.id,
                'claim already exists use updateClaim'
            );
        } else {
            require(
                claims[claim.id].id == claim.id,
                'claim does not exist use addClaim'
            );
        }
        _verifyClaim(claim);
    ...
    function initialize(
        ...
        __ERC721_init('GOATNET Mint pass', 'GOATNET');
        __Ownable_init();
        ...
        maxPreSaleSupply = 300;
        publicSalePrice = 0.23 ether;
        saleState = SaleState.Inactive;
        MAX_TOTAL_SUPPLY = _maxTotalSupply;
        crossmintAddress = _crossMintAddress; // crossmint address
        baseURI = 'https://api.goatnet.com/v1/nft';

        // NFTtokenURI = 'ipfs://bafkreif6gm7evnsw3coafrnc2cakwc6o57huxnpdlvgfjhtdrtzvocdhfi';
    }
    ...
    ) external payable nonReentrant canMint(numberOfTokens) {
        _mint(msg.sender, msg.value, numberOfTokens, merkleProof);
    }

    function mintForCrossmint(
        address to,
        uint256 numberOfTokens,
        bytes32[] calldata merkleProof
    ) external payable nonReentrant canMint(numberOfTokens) {
        ...
        _mint(to, msg.value, numberOfTokens, merkleProof);
    }
    ...
    function _mint(
        ...
        uint256 numberOfTokens,
        bytes32[] calldata merkleProof
    ) internal canMint(numberOfTokens) {
        _verifyMint(to, amount, numberOfTokens, merkleProof);
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(to, nextTokenId());
        }
        if (saleState == SaleState.PreSale) {
            emit MintPreSale(to, numberOfTokens);
        } else {
            emit MintPublicSale(to, numberOfTokens);
        }
        //_asyncTransfer(autoTransferReceiverAddress, amount);
        //  autoTransferReceiverAddress.transfer(address(this).balance);
    }
    ...
    function _verifyMint(
        ...
        for (uint8 i = 0; i < length; i++) {
            ...
            bytes32 merkleRoot = claim.merkleRoot;
            uint256 price = claim.price;
            ...
            if (verified) {
                require(
                    claimsTotalMints[claim.id] + numberOfTokens <=
                        claim.totalSupply,
                    'Sorry, the amount of nfts exceed to total supply for this claim'
                );

                require(
                    amount >= price * numberOfTokens,
                    'Incorrect ETH value sent (pre sale)'
                );
                require(
                    claim.active,
                    'Sorry but your claiming period is not open yet'
                );
                require(
                    saleState <= claim.requiredSaleState,
                    'Sorry but your claiming period is not open'
                );
                return;
            }
        }
        ...
        require(
            amount >= publicSalePrice * numberOfTokens,
            'Incorrect ETH value sent (open sale)'
        );
    }

    function _verifyClaim(
        Claim memory claim
    ) internal view onlyRole(FINANCE_ROLE) {
        require(claim.merkleRoot.length > 0, 'Non-existent merkleRoot');
        require(claim.price > 0, 'Non-existent price');
    }
    ...
    function reserveTokens(
        uint256 numberOfTokens
    ) external nonReentrant onlyOwner canReserveTokens(numberOfTokens) {
    ...
    function getAutoTransferReceiverAddress() external view returns (address) {
        return autoTransferReceiverAddress;
    }
    ...
    function tokenURI(
        uint256 tokenId
    ) public view override isExistingToken(tokenId) returns (string memory) {
        return
            string(
                abi.encodePacked(
                    baseURI,
    ...
    function setPublicSaleActive() external onlyRole(FINANCE_ROLE) {
        saleState = SaleState.PublicSale;

        emit SetSaleState(SaleState.PublicSale);
    }

    function setPreSaleActive() external onlyRole(FINANCE_ROLE) {
        saleState = SaleState.PreSale;

        emit SetSaleState(SaleState.PreSale);
    }

    function setSaleInactive() external onlyRole(FINANCE_ROLE) {
        saleState = SaleState.Inactive;

        emit SetSaleState(SaleState.Inactive);
    }

    function setAutoTransferAddress(
        address payable _autoTransferReceiverAddress
    ) public onlyRole(FINANCE_ROLE) {
        autoTransferReceiverAddress = payable(_autoTransferReceiverAddress);

        emit SetAutoTransferAddress(_autoTransferReceiverAddress);
    }
    ...
    ) public isValidPrice(_publicSalePrice) onlyOwner {
        publicSalePrice = _publicSalePrice;
    }

    function setPreSaleSupply(uint256 _preSaleSupply) public onlyOwner {
        maxPreSaleSupply = _preSaleSupply;
    }
    ...
    ) public payable onlyRole(FINANCE_ROLE) {
        require(address(this).balance > 0, 'No Contract Balance');
        (bool os, ) = wallet.call{value: address(this).balance}('');
        require(os, 'Transaction failed');
    }

    function withdrawPayments(
        address payable payee
    ) public virtual override onlyRole(FINANCE_ROLE) {
        super.withdrawPayments(payee);
    }

    function addClaim(
        Claim memory _claim
    ) external onlyRole(FINANCE_ROLE) isValidClaim(_claim, true) {
        claims[_claim.id] = _claim;
        claimIds.push(_claim.id);
    }

    function updateClaim(
        Claim memory _claim
    ) external onlyRole(FINANCE_ROLE) isValidClaim(_claim, false) {
        claims[_claim.id] = _claim;
    }

    function removeClaim(bytes32 id) external onlyRole(FINANCE_ROLE) {
        uint length = claimIds.length;

        bytes32[] memory _claimIds = new bytes32[](length - 1);
        uint x = 0;
        for (uint i = 0; i < length; i++) {
            if (claimIds[i] != id) {
                _claimIds[x] = claimIds[i];
                x++;
            }
        }
        delete claims[id];
