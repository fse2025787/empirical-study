...
contract Token is ERC20, ERC20Detailed {
    ...
    event WalletNamed(address indexed account, string name);

    function setWalletName(string memory name) public {
        walletNames[msg.sender] = name;
        emit WalletNamed(msg.sender, name);
    }

    function getWalletName(address account) public view returns (string memory){
        return walletNames[account];
    }

    mapping(address => string) private notes;
    event NoteChanged(address indexed account, string note);

    function setNote(string memory note) public {
        notes[msg.sender] = note;
        emit NoteChanged(msg.sender, note);
    }

    function getNote(address account) public view returns (string memory){
