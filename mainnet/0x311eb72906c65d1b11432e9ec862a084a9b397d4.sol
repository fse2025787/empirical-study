...
contract BoredStackers is ERC721A, Pausable, Ownable {
    ...
    modifier onlyPhase03() {
        require(phase == SalePhase.Phase03, "Invalid phase");
        _;
    }
    ...
    onlyOwner
    onlyPhase03 {
        mintPrice_Phase03_TypeAll = max;
        emit updateMintPricePhase03TypeAll(max, msg.sender);
    }
    ...
    whenNotPaused {
        ...
        if(phase == SalePhase.Phase01) {
            ...
            if(couponType == CouponType.Vip) {
             ...
             require(couponType == CouponType.Vip || couponType == CouponType.Normal || couponType == CouponType.Raffle,
                "Invalid coupon");
            if(couponType == CouponType.Vip) {
                _checkMint(amount, maxMint_TypeVip_PerAddress, mintPrice_TypeVip);
            } else if(couponType == CouponType.Normal) {
                ...
                _checkMint(amount, maxMint_TypeRaffle_PerAddress, mintPrice_TypeRaffle);
            }
        } else {
            require(couponType == CouponType.Vip || couponType == CouponType.Normal
                        || couponType == CouponType.Raffle || couponType == CouponType.Public,
                "Invalid coupon");
            _checkMint(amount, maxMints_PerAddress, mintPrice_Phase03_TypeAll);
        }
