...
contract WrappedERC4626YearnCRV is ERC4626, PricePerTokenMixin {
    ...
    function convertToUSD(uint256 shares) public view returns (uint256) {
        return
            (convertToAssets(shares) * uint256(crvPriceOracle.latestAnswer())) / (10**crvPriceOracle.decimals());
    }
    ...
    function _convertToShares(uint256 assets, Math.Rounding rounding)
        ...
        returns (uint256 shares)
    {
        uint256 ycrvAmount = assets == 0 ? 0 : curveYCRVpool.get_dy(0, 1, assets);
        return ycrvAmount * (10**decimals()) / stycrv.pricePerShare();
    }
    ...
    function _convertToAssets(uint256 shares, Math.Rounding rounding)
        ...
        returns (uint256 assets)
    {
        uint256 ycrvAmount = shares * stycrv.pricePerShare() / 10**decimals();
        return ycrvAmount == 0 ? 0 : curveYCRVpool.get_dy(1, 0, ycrvAmount);
    }
