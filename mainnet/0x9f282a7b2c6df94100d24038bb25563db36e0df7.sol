...
contract DevilOfficial is ERC721A, Ownable {
    ...
    string public baseURI = "ipfs://QmRGG8Ad9wri44wkft3fRYj4DfsD8Ta3HdtuUugKieEeuM/unrevealed.json";
    
    uint32 SALE_START_TIME_EPOCH = 1674489600;
    ...
    constructor(
        ...
        presaleConfig = PresaleConfig({
            whitelistMaxSupply: 666,
            whitelistPrice: 0.0044 ether
        });
    }

    modifier mintCompliance(uint256 _mintAmount) {
        if ((totalSupply() + _mintAmount) > SUPPLY_MAX) revert OutOfSupply();
        if (_numberMinted(msg.sender) + _mintAmount > MAX_PER_WALLET) revert ExceedsTxnLimit();
        if (block.timestamp < SALE_START_TIME_EPOCH) revert MintInactive();
        _;
    }
        ...
        mintCompliance(_mintAmount)
    {
        ...
        unchecked {
            if ((whitelistSupply + _mintAmount) > config_.whitelistMaxSupply) revert OutOfSupply();
    ...
    function setSaleStartTime(uint32 startTime_) external onlyOwner {
        SALE_START_TIME_EPOCH = startTime_;
    }

    function setWhitelistMaxSupply(uint256 _supply) external onlyOwner {
        presaleConfig.whitelistMaxSupply = _supply;
    }
