...
// founder - 40
    ...
    function collectPendingRoyalties() external onlyLandlord nonReentrant {
        ...
        emit RoyaltiesDue(msg.sender, amount);
    }
        ...
        returns (uint256 amount)
    {
        ...
        console.log("amount to pull from collector",amountToPull);
        ...
        console.log("Shares received", amount);
        return amount;
    }
    ...
    function payLanlord(address _landlord, uint256 _amount) internal {
...
contract NFTContract is OwnableUpgradeable, ERC721EnumerableUpgradeable{
    ...
    */
    ...
    ILandlordVault vault;
    mapping(address => mapping(uint => bool)) public burnedWhitelistPhase;
    ...
    function mint(uint256 _qty, address _to) external payable {
        ...
        require(
            mintedPhase[currentPhase] + _qty < maxMintPhase[currentPhase],
            "Maximum reached for phase"
        );

        if (isWhitelist(msg.sender)) {
           require( !burnedWhitelistPhase[msg.sender][currentPhase], "Already Minted Phase");
            yourPrice = (cost / 5) * 4;
            if(_qty > 1){
                yourPrice += (cost * (_qty - 1));
            }
            burnedWhitelistPhase[msg.sender][currentPhase] = true;
        } else {
            yourPrice = cost * _qty;
        }
...
interface ILandlordVault {
   
      function updateDebtOf(uint256 id) external;
