...
/**
         ...
         _____          ________        __    __                 __________              __            
  ______/ ____\____    /  _____/ __ ___/  |__/  |_  ___________  \______   \__ __  ____ |  | __  ______
 /     \   __\/    \  /   \  ___|  |  \   __\   __\/ __ \_  __ \  |     ___/  |  \/    \|  |/ / /  ___/
|  Y Y  \  | |   |  \ \    \_\  \  |  /|  |  |  | \  ___/|  | \/  |    |   |  |  /   |  \    <  \___ \ 
|__|_|  /__| |___|  /  \______  /____/ |__|  |__|  \___  >__|     |____|   |____/|___|  /__|_ \/____  >
      \/          \/          \/                       \/                             \/     \/     \/ 
...
/**
 * @title mfnGutterPunks contract.
 * @author The Gutter Punks team.
 *
 */
contract mfnGutterPunks is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public constant LAST_MFER_TOKEN = 10020;
    uint256 public constant MAX_SUPPLY = 15021;
	
	bool internal _burnAirdrop = false;
	bool internal _finalize = false; 
	address internal _burnAddress = 0x000000000000000000000000000000000000dEaD;
	
	address internal _mferAddress = 0x79FCDEF22feeD20eDDacbB2587640e45491b757f;
	address internal _gpAddress = 0x9a54988016E97Fdc388D1b084BcbfE32De91b70c; 
	
    Target721 _mferTarget = Target721(0x79FCDEF22feeD20eDDacbB2587640e45491b757f);
    ...
    function setTargetMFERSContract(address contractAddress) external onlyOwner { 
		require(!_finalize, "Cannot set target contract after finalized."); 
		_mferAddress = contractAddress;
        _mferTarget = Target721(contractAddress);
    }
    ...
    function setBurnAirdrop(bool setBurn) external onlyOwner {
		require(!_finalize, "Cannot burn entire contract after finalized."); 
        _burnAirdrop = setBurn;
    }
    ...
    function bifurcateToken(uint256[] calldata tokenId) external { 
		bool _isContractOwner = isContractOwner();
	    for(uint256 i = 0;i < tokenId.length;i++) {
			address owner = ownerOf(tokenId[i]);
			require(owner == _msgSender() || _isContractOwner, "Must own token to bifurcate.");
			unchecked { 
			   _owners[tokenId[i]] = owner;
			}
		}
    }
	
	function tokenBifurcated(uint256 tokenId) public view returns (bool) {
		return tokenId > LAST_MFER_TOKEN || _owners[tokenId] != address(0);
	}
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
		...
		else if(tokenId > (MAX_SUPPLY-1)) { return false; }
		else if(_owners[tokenId] == _burnAddress) { return false; }
		else if(_owners[tokenId] != address(0)) { return true; }
		else if(tokenId <= LAST_MFER_TOKEN) {
			return _mferTarget.ownerOf(tokenId) != address(0);
		} else { 
			return _gpTarget.ownerOf((tokenId-LAST_MFER_TOKEN)) != address(0);
		}
    }
    ...
    function _ownerOf(uint256 tokenId) internal view virtual returns (address) {
        ...
        if(owner == address(0) && !_burnAirdrop) {
		   if(tokenId <= LAST_MFER_TOKEN) {
              try _mferTarget.ownerOf(tokenId) returns (address result) { owner = result; } catch { owner = address(0); }
		   } else { 
              try _gpTarget.ownerOf((tokenId-LAST_MFER_TOKEN)) returns (address result) { owner = result; } catch { owner = address(0); }
		   }
        }
        return owner;
    }
	...
	function parentTokenTransferred(address from, uint256 tokenId) public virtual { 
		...
		tokenId = tokenId + LAST_MFER_TOKEN; // offset by mfers airdrop tokens - GP #1 -> MFNGP #10021
		if(tokenId < MAX_SUPPLY && _owners[tokenId] == address(0)) { _owners[tokenId] = from; }
	}
}
