    ...
    ERC721HolderUpgradeable
{
    ...
    IERC20Upgradeable private apeCoin;
    IApeCoinStaking private apeCoinStaking;
    ...
    event ApeCoinClaimed(address indexed to, uint256 indexed tokenId);
    event ApeCoinWithdrew(address indexed to, uint256 indexed tokenId);
    ...
    function initialize(
        ...
        apeCoinStaking = IApeCoinStaking(apeCoinStakingAddress);
        apeCoin = IERC20Upgradeable(apeCoinAddress);
    ...
    function apeCoinStake(uint32 tokenId, uint224 amount) external {
        require(_exists(tokenId), "Wrapped token does not exist");
        require(
            ownerOf(tokenId) == msg.sender,
            "Only wrapped token owner can call this function"
        );
        apeCoin.safeTransferFrom(msg.sender, address(this), amount);
        apeCoin.approve(address(apeCoinStaking), amount);

        IApeCoinStaking.SingleNft[]
            memory params = new IApeCoinStaking.SingleNft[](1);
        params[0] = IApeCoinStaking.SingleNft(tokenId, amount);

        apeCoinStaking.depositBAYC(params);

        emit ApeCoinStaked(msg.sender, tokenId, amount);
    }

    function apeCoinClaim(uint256 tokenId) external {
        require(_exists(tokenId), "Wrapped token does not exist");
        require(
            ownerOf(tokenId) == msg.sender,
            "Only wrapped token owner can call this function"
        ...
        apeCoinStaking.claimBAYC(params, msg.sender);

        emit ApeCoinClaimed(msg.sender, tokenId);
    }

    function apeCoinWithdraw(uint32 tokenId, uint224 amount) external {
        require(_exists(tokenId), "Wrapped token does not exist");
        require(
            ownerOf(tokenId) == msg.sender,
            "Only wrapped token owner can call this function"
        ...
        apeCoinStaking.withdrawBAYC(params, msg.sender);

        emit ApeCoinWithdrew(msg.sender, tokenId);
    }
}
