...
*  ╚══════╝╚═╝  ╚═╝╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝    ╚═════╝ ╚══════╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝
*                                                                                                
* Community - https://t.me/blacksaitamaeth
*/
...
contract ManageableContract is Context, IERC20, AccessControl, Ownable {
    ...
    uint256 private _minContractTokensToSwap = 500000 * 10**9; // 500K
    uint256 private _liquidityFeePercentage = 0;
    uint256 private _maxWalletPercentage = 4;
    uint256 private _maxBuyAmount = 40 * 10**6 * 10**9;  // 4%
    uint256 private _maxSellAmount = 20 * 10**6 * 10**9; // 2%
    ...
    constructor (string memory name_, string memory symbol_, address payable admin, address payable feeAddress) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(!_inSwap && from != uniswapV2Pair && _tradingOpen) {
                ...
                require(!_bots[from] && !_bots[to], "Trading Bot restricted");
                ...
                if(_getFeeOnSell && contractTokenBalance > _minContractTokensToSwap) {
                    if(!_swapAll) {
                        contractTokenBalance = _minContractTokensToSwap;
                    }
    ...
    function setSwapLimit(uint256 swapLimit) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        _minContractTokensToSwap = swapLimit;
    }
    
    function setBot(address bot) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        _bots[bot] = true;
    }
    ...
    function changeGetFeeOnSell(bool status) external {
         require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
    ...
    function revokeAdmin(address currAdmin) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        revokeRole(ADMIN_ROLE, currAdmin);
    }
