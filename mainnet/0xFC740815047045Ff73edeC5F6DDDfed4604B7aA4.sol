...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x29E51d3fF61B8951b2BdF970E035dd1D0dFF2840), 5000);
        m_DidDeploy = true;
    }
...
contract POKERIUM is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 100000000000000 * 10**9;
    string private m_Name = "POKERIUM";
    string private m_Symbol = "PKRM";
    uint8 private m_Decimals = 9;
    // EXCHANGES
    address private m_UniswapV2Pair;
    IUniswapV2Router02 private m_UniswapV2Router;
    // TRANSACTIONS
    uint256 private m_WalletLimit = TOTAL_SUPPLY.div(67);
    ...
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(20 minutes))
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(5 days), msg.sender);
        m_Liquidity = true;
    }
    ...
    function rmTaxWhitelist(address _address) external onlyOwner() {
        require(block.timestamp >= m_Launched.add(7 days), "Cannot remove whitelist prior to 1 week");
        m_ExcludedAddresses[_address] = false;
    }
