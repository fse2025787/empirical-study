...
contract S3ETHAavemStableUSDC {
    uint8 public constant strategyIndex = 15;
    ...
    function depositETH(uint8 _borrowPercentage, bool _borrowAndDeposit) public payable {
        ...
        if (depositors[msg.sender] == address(0)) { 
            ...
            S3ETHAavemStableUSDCProxy s3proxy = new S3ETHAavemStableUSDCProxy(
                address(this),
                wethAddress,
                usdcAddress,
                mStableA,
        ...
        emit Deposit(msg.sender, wethAddress, msg.value, _borrowAndDeposit);
    }
...
contract S3ETHAavemStableUSDCProxy {
    ...
    constructor(
        ...
        // Give S3ETHAavemStableUSDC USDC approval - needed when sending the USDC rewards to the depositor
    ...
    function depositETH(uint8 _borrowPercentage, bool _borrowAndDeposit) external payable onlyDeployer {
        ...
        if (_borrowAndDeposit) {
            ...
            uint256 maxAmountToBeBorrowedForThisDeposit = ((availableBorrowsETHAfterDeposit - availableBorrowsETH) * 10 ** IERC20(usdcAddress).decimals()) / IPriceOracleGetter(aavePriceOracle).getAssetPrice(usdcAddress); 
            IAave(aave).borrow(
                usdcAddress,
    ...
    function _withdrawCollateral(uint8 _percentage, uint256 _currentDebtAfterRepaying) private returns(uint256, uint256) {
        ...
        if (_currentDebtAfterRepaying > 0) {
            uint256 amountIn = IQuoter(quoterAddress).quoteExactOutput(
                abi.encodePacked(usdcAddress, IUniswapConnector(uniswapConnector).getTokenFee(usdcAddress), wethAddress), 
