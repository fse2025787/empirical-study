 ...
 * allowances. See {IERC20-approve}.
 */
contract BlowUp is ERC20, Ownable {
    string private _name = 'Start Up Blow Up';
    string private _symbol = 'BLOWUP';
    uint256 private _supply = 1_000_000 * 10**decimals();
    ...
    constructor() ERC20(_name, _symbol) {
        ...
        maxTxAmount = (_supply * 20) / 1000; // max transaction amount: 2%
        maxWalletAmount = (_supply * 20) / 1000; // max wallet amount: 2%
        _numTokensSellToAddToLiquidity = (_supply * 5) / 1000;
        _numTokensSellToAddToETH = (_supply * 5) / 1000;
    ...
    function _transfer(
        ...
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inSwapAndLiquify) {
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                uint256 transferFee = marketingShare + liquidityShare;
                transferAmount = amount - transferFee;
                _marketingReserves += marketingShare;

                super._transfer(from, address(this), transferFee);
            }
    ...
    function removeCap() public onlyOwner returns (bool) {
        maxTxAmount = _supply;
