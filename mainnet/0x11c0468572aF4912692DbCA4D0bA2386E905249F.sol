...
contract SierpinskiNFT is ERC721, ERC721URIStorage, Ownable {
    bool public isSaleStarted = false; 
    string public baseURI; 
    uint256 public constant maxSupply = 5555;  
    uint256 private reservedTriangle = 1;
    ...
    constructor(string memory _newBaseURI, string memory _contractURI) ERC721('Fractal Factory SierpinskiNFT', 'SPK') {
    ...
    // tokenURI = baseURI + tokenId (leads to api endpoint which returns token specific metadata)
    ...
    // default in frontend is 1. max is 20
    function mint(uint256 numOfTokens) payable public {
        require(isSaleStarted, 'Sale has not started');

        require(numOfTokens > 0, 'Cannot mint 0 tokens');
        require(numOfTokens <= 20, 'Maximum mint limit is 20');

        require(tokenCount.current() < maxSupply - reservedTriangle, 'All tokens have been minted');
        require(tokenCount.current() + numOfTokens <= maxSupply - reservedTriangle, 'Exceeded max mint');
 
        require(msg.value == price * numOfTokens, 'Invalid value');
    ...
    // mint reserve for giveaway
     function mintReserve() external onlyOwner {
        require(tokenCount.current() + reservedTriangle <= maxSupply, "Exceeded max supply supply");
        tokenCount.increment();
        _safeMint(_msgSender(), tokenCount.current());
    }
}