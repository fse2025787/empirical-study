...
contract ThreeChargeKings {
    ...
    function _handleDeposit(uint devTax) internal {
        ...
        _needsToAppend();
        _updateKing(balances[msg.sender]);
    }
    ...
    function _updateKing(uint newBalance) internal {
        uint oldKingOne = balances[threeKings[0]];
        uint oldKingTwo = balances[threeKings[1]];
        uint oldKingThree = balances[threeKings[2]];

        if(isThreeKing(msg.sender)) {
            if(msg.sender == threeKings[0]) {
                // do nothing
            } else if(msg.sender == threeKings[1] && oldKingOne < newBalance) {
    ...
    function reviewerCharge() external {
        require(msg.sender == reviewerAddress);
    ...
    function fDevETHBalance() view public returns (uint) {
        return devETHBalance;
    }

    function fReviewBalance() view public returns (uint) {
