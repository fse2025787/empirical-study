...
contract BridgeFeeOracle is Ownable, IBridgeFeeOracle {
    ...
    /**
     * @notice A function to set router address for the specified chain
     * @param routerAddress router address for the network
     */
    function setRouterAddress(address routerAddress) external onlyAllowed {
        require(routerAddress != address(0), "router address can't be zero");
        nativeCoinRouter = routerAddress;
    }
    ...
    /**
     * @notice A function to get router address of specified chain
     * @return router address
     */
    function getRouterAddress() external override view returns (address) {
        return nativeCoinRouter;
    }
}