...
abstract contract RadArt is OwnerPausable {
    ...
    function _rerollArt(uint256 tokenId, bool allowFromInit) internal returns (uint256 newArt) {
        ...
        bool useInit = allowFromInit && initialArt.totalUsed < secondaryArt.totalUsed;
...
contract Radbro is ERC721ACommon, BaseTokenURI, ReentrancyGuard, Radlist, RadArt, Radmath {
    ...
    bool public radcoinMintingStarted;
    ...
    constructor(
        ...
        ERC721ACommon("Radbro", "RADBRO", payable(msg.sender), 0)
        Radlist(_merkleRoot)
    ...
    /**
    @notice Toggle Radcoin minting.
     */
    function toggleRadcoinMinting() external onlyOwner {
        radcoinMintingStarted = !radcoinMintingStarted;
    }
    ...
    function mintFromRadcoin(
        ...
        require(radcoinMintingStarted, "Radbro: radcoin minting not started");
        require(n > 0, "Radbro: Must mint at least one");
        require(n <= 10, "Radbro: Cannot mint more than 10");
    ...
    function _mintInternal(address to, uint256 n) internal whenNotPaused {
        ...
        for (uint256 i = totalSupply(); i <= totalSupply() + n; i++) {
            radcoin.resetClaimAmount(i);
        }
     ...
     */
    function radrollArt(uint256 tokenId) public returns (uint256 newArt) {
    ...
    function _spendRad(address owner, uint256[] calldata radbroIds, uint256 amount) internal {
        ...
        for (uint256 i = 0; i < radbroIds.length; i++) {
            ...
            if (reward >= amount) {
                radcoin.claimForRadbros(radbroId, amount);
                return;
            } else {
                radcoin.claimForRadbros(radbroId, reward);
                amount -= reward;
            }
        }
