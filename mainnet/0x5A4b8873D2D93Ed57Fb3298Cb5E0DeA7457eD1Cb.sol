...
contract MechaPilotsApes is ERC721Delegated {
    ...
    uint256 public reserveTokenId = 1200;
    address public airdropAddress = 0xB03C221FF41d0111f3059477E526BD4Ede3d84AE;
    bool public sale = false;
    bool public allowProxies = true;

    mapping(address => bool) public projectProxy;
    mapping(address => bool) public allowedContracts;
    mapping(address => mapping(uint256 => bool)) public mappedNfts;
    ...
    function looseAirDrop(address to, uint256[] memory tokensToDrop) public {
        require(msg.sender == airdropAddress);
        for (uint256 i = 0; i < tokensToDrop.length; i++) {
            _mint(to, tokensToDrop[i]);
        }
    }

    function airdropApes(address to, uint256[] memory tokensToDrop, uint256[] memory nftTokenIds, address[] memory contractAddresses) public {
        require(msg.sender == airdropAddress);
        for (uint256 i = 0; i < nftTokenIds.length; i++) {
            require(allowedContracts[contractAddresses[i]], "Contract not allowed to claim.");
            ERC721Upgradeable mechaContract = ERC721Upgradeable(contractAddresses[i]);
            require(mechaContract.ownerOf(nftTokenIds[i]) == msg.sender, "You are not the owner of the nft you are trying to claim.");
            if (mappedNfts[contractAddresses[i]][nftTokenIds[i]]) {
                continue;
            }
            mappedNfts[contractAddresses[i]][nftTokenIds[i]] = true;
        }
    ...
    function claimApes(uint256[] memory nftTokenIds, address[] memory contractAddresses) public {
        require(sale, "Claim not yet open");
        require(reserveTokenId + nftTokenIds.length <= MAX_SUPPLY, "Out of claims");
    ...
    function setAirDropAddress(address airAddress) public onlyOwner {
        airdropAddress = airAddress;
    }
