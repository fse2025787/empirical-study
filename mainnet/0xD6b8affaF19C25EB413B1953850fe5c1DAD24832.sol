...
contract Vault is Ownable, Signers, VerifySigEIP712 {
    ...
    function cBridgeNative(uint256 amount, BridgeDescription calldata bDesc) external payable {
        IBridge(BRIDGE).sendNative{value: amount}(bDesc.receiver, amount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage);

        bytes32 transferId = keccak256(
            abi.encodePacked(address(this), bDesc.receiver, address(NATIVE_ADDRESS), amount, bDesc.dstChainId, bDesc.nonce, uint64(block.chainid))
        );

        BridgeInfo memory tif = transferInfo[transferId];
        require(tif.nonce == 0, " PLEXUS: transferId already exists. Check the nonce.");
        tif.dstToken = address(NATIVE_ADDRESS);
    ...
    function viewNative(IERC20 _address) external view returns (bool isNotNative) {
    ...
    function viewFunction(bytes calldata _data) external view returns (bytes4 selector) {
    ...
    function swapRouter(SwapData calldata _swap, bytes calldata _data) external payable {
        ...
        if (succ) {
            ...
            if (selector == SWAP_CALL_SELECTOR) {
                ...
                (uint256 sentAmount, uint256 returnAmount, ) = abi.decode(data, (uint256, uint256, bytes32));
                require(returnAmount >= swapData.minReturnAmount);
                dstAmount = returnAmount;
            } else {
    ...
    // function swap(SwapData calldata _swap, bytes calldata _data) external payable {
    //     SwapData memory swapData = _swap;
    ...
    //                 }
    //             }
    //             emit Swap(msg.sender, address(swapData.srcToken), address(swapData.dstToken), swapData.amount, returnAmount);
    //         }
    //         // (uint returnAmount, ) = abi.decode(_data, (uint256, uint256, bytes32));
    //     } else {
    //         revert();
    ...
    // function unoSwap(SwapData calldata _swap, bytes calldata _data) external payable {
    //     SwapData memory swapData = _swap;
    ...
    //     (bool succ, bytes memory data) = address(ROUTER).call{value: msg.value}(_data);
    //     if (succ) {
    //         if (selector == UNO_CALL_SELECTOR || selector == UNO_TO_CALL_SELECTOR) {
    //             uint256 returnAmount = abi.decode(data, (uint256));
    //             emit Swap(msg.sender, address(swapData.srcToken), address(swapData.dstToken), swapData.amount, returnAmount);
    //         }
    //         // (uint returnAmount, ) = abi.decode(_data, (uint256, uint256, bytes32));
