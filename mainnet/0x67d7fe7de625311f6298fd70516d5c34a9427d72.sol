...
// 3. Contract gains small amount of unwithdrawable ETH/BNB when swapping so added a 
//    withdrawOverFlowETH function so that the owner can withdraw this periodically.
...
contract initializedERC20 is Context, IERC20, Ownable {
    ...
    function init(string memory name, string memory symbol, uint256 totalsupply, uint8 decimals, uint256 maxtxamount, uint256 taxFee, uint256 liquidityFee, uint256 numtokenstoselltoaddtoliquidity, address extrafeewallet, uint256 extrafeepercent, address newContractOwner) public {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // --> TODO: Kovan or Eth PROD
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // --> BSC Testnet for Pancake!
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); --> PROD address for Pancake?
        
        // NOTE: Auto-lock LP when creating the pair
    ...
    function withdrawOverFlowETH() public onlyOwner {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // this is so that we can capture exactly the amount of ETH/BNB that the
        // swap creates, and not make the liquidity event include any ETH/BNB that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for ETH/BNB
        swapTokensForEth(half); // <- this breaks the ETH/BNB -> TOKEN swap when swap+liquify is triggered
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount).sub(extraFeeAmount); // adds amount to sender after fee and subs extraFeeAmount% for charity
...
// NOTE: This is the factory that will clone the original contract for future deployments
contract TokenFoundry is Ownable, CloneFoundry {

  // This is the address where the Mastercontract initializedERC20 is deployed to
  // address payable public masterContract = 0xEb4c4f701cb50c8dDF7B0CEd051CeB25Aed9f3CA; // master contract for bsc testnet placido factory
  address payable public masterContract = 0x6f2C352149Cd43e1F4421c86473e31B1B69B1923; // PROD ETH MasterContract for kovan.eth testnet jacinto factory
  
  // address to receive fees
  address payable public tokenfoundryboss = 0x19Aa0f155170628566C9f802bd06798511b01D93; // this has to be the owner of contract
  
  address payable public tokenfoundryreferrer;

  //uint256 public fee = 200000000000000000;
  uint256 public fee = 50000000000000000; // ros factory traganiquel
  ...
  function createToken(string memory _name, string memory _symbol,uint256 _totalsupply, uint8 _decimals, uint256 _maxtxamount, uint256 _taxfee, uint256 _liquidityfee, uint256 _numtokenstoselltoaddtoliquidity, address _extrafeewallet, uint256 _extrafeepercent, address payable _tokenfoundryreferrer) public payable {
    ...
    newCloneOwner = msg.sender; // This guy needs to be Owner's Address
    initializedERC20(clone).init(_name, _symbol, _totalsupply, _decimals, _maxtxamount, _taxfee, _liquidityfee, _numtokenstoselltoaddtoliquidity, _extrafeewallet, _extrafeepercent, newCloneOwner);
    emit TokenCreated(clone);

    tokenfoundryreferrer = _tokenfoundryreferrer;
    _forwardFunds(); // Sends fee to referrer
  }
  ...
  function _forwardFunds() internal {
        ...
        tokenfoundryreferrer.transfer(split);
        tokenfoundryboss.transfer(split);
    }
