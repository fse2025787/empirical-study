...
contract MetaMatesNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  bool private whitelistEnforced = false;
  bool private whitelistOver = false;
  mapping (address => bool) public isWhitelisted;
  mapping (address => uint256) public whitelistedMintAmount;

  constructor() ERC721("Meta-Mates", "MM"){
    setBaseURI("");
    setNotRevealedURI("https://gateway.pinata.cloud/ipfs/QmQ1tjEzJ6VdesWEi5NqLU8GgH5mYvuibBcxCa8JssFN7b/hidden.json");
  }
  ...
  function startWhitelist() external onlyOwner {
      require(whitelistOver == false, "Whitelist is over, may not re-enable");
      whitelistEnforced = true;
  }
  
  function startPublicSale() external onlyOwner {
      require(whitelistEnforced == true, "Whitelist must be started first");
      whitelistEnforced = false;
      whitelistOver = true;
  }
  
  function whitelistAddresses(address[] calldata wallets) external onlyOwner {
