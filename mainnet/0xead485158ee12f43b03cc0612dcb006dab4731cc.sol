...
HAM: THE FIRST GREAT APE

https://medium.com/@hamthe1stgr8ape/ham-93aea3ecc187
https://twitter.com/Ham_1stgr8ape
https://t.me/HamTheFirstGreatApe
http://hamthegreatape.com/
...
contract TheFirstGreatApe is Context, IERC20, Ownable {
    ...
    string private _name = "The First Great Ape";
    string private _symbol = "HAM";
    uint8 private _decimals = 9;
    uint256 private _totalSupply = 666_666_666_666_666 * 1e9;
    ...
    constructor() {
        ...
        marketingWallet = address(0xdD5fb3a3cEF95bB13Fc237e52a74530e96eecbb8);
        DevelopmentWallet = address(0x78b7729643570d88D9A1545EeE273564dBa6DA51);
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "HAM: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "HAM: decreased allowance or below zero"
            )
        );
        return true;
    }
    ...
    function setMinTokenToSwap(uint256 _amount) external onlyOwner {
        require(_amount > 0,"HAM: can't be 0");
        minTokenToSwap = _amount;
    }

    function setMaxHoldLimit(uint256 _amount) external onlyOwner {
        require(_amount >= _totalSupply.mul(5).div(percentDivider), "HAM: should be greater than 0.5%");
        maxHoldLimit = _amount;
    }

    function setMaxTxnLimit(uint256 _amount) external onlyOwner {
        require(_amount >= _totalSupply/percentDivider, "HAM: should be greater than 0.1%");
        maxTxnLimit = _amount;
    }
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "HAM: already enabled");
        trading = true;
        feesStatus = true;
    ...
    function _approve(
        ...
        require(owner != address(0), "HAM: approve from the zero address");
        require(spender != address(0), "HAM: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "HAM: transfer from the zero address");
        require(to != address(0), "HAM: transfer to the zero address");
        require(amount > 0, "HAM: Amount must be greater than zero");
        require(!isBot[from], "HAM: Bot detected");

        if (!isExcludedFromMaxTxn[from] && !isExcludedFromMaxTxn[to]) {
            require(amount <= maxTxnLimit, "HAM: max txn limit exceeds");

            // trading disable till launch
            if (!trading) {
                require(
                    dexPair != from && dexPair != to,
                    "HAM: trading is disable"
                );
            }
            // antibot
            if (
    ...
    function _tokenTransfer(
        ...
        if (dexPair == sender && takeFee) {
            ...
            _balances[sender] = _balances[sender].sub(amount,"HAM: insufficient balance");
