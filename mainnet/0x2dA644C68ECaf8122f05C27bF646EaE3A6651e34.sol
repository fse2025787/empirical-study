...
contract MarvinToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xfD0aD54E483Eb4Dc6b0b17BE4fb4cA661D2e8189); // Marketing Address
    ...
    uint256 private _tTotal = 19696657 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Elons Marvin";
    ...
    uint256 public _liquidationThreshold = 200 * 10**9;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && tradingOpen && to == uniswapV2Pair) {
            if (contractTokenBalance > 0) {
                if (contractTokenBalance > _liquidationThreshold) {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 sharePerDivision = contractTokenBalance.div(_marketingFee.add(_liquidityFee));
        uint256 marketingShare = sharePerDivision.mul(_marketingFee);
        uint256 liquidityShare = contractTokenBalance.sub(marketingShare);

        //liquify marketing share
        ...
        )
    {
        ...
        uint256 tMarketing = calcualteMarketingFee(tAmount);
    ...
    function calcualteMarketingFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
    ...
    function setMarketinfFeePercent(uint256 marketingFee) external onlyOwner {
    ...
    function setLiquidationThreshold(uint256 liquidationThreshold)
        external
        onlyOwner
    {
