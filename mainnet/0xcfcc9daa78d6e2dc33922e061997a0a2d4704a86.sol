 ...
 *      And the 73 to 21,000.00 (21k) represents the right to access the Vladimir Club
...
contract CryptoLand is ERC721Enumerable, Ownable, ICryptoLand, ICryptoLandMetadata {
    ...
    uint256 public constant TOKEN_PUBLIC = 20_928; //@dev 60+12+20,928 = 21,000 tokens
    uint256 public constant TOKEN_MAX = 21_000;
    ...
    uint256 public totalPublicSupply = 72; // from 73 to 20,928
    ...
    event tokenPurchased(address indexed _account, uint256 _id, uint256 _priceOfPurchase, string parcelId);
    ...
    function addToWhiteList(address[] calldata addresses) external override onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            */
            ...
            emit whiteList(addresses[i], true);
        }
    }
    ...
    function removeFromWhiteList(address[] calldata addresses) external override onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            emit whiteList(addresses[i], false);
        }
    }
    ...
    */
    ...
    function setPriceConvertion(uint256 _priceConverter) external onlyOwner {
    ...
    function getCalculatedPriceByToken(uint256 tokenNumber) external view returns (uint256) {
        require (tokenNumber <= 60, 'Token Overflow');
        require(priceConverter > 0, 'Price Conversion not set');
    ...
    function purchase(string memory _parcelId) external override payable {
        ...
        if (totalPurchaseSupply < TOKEN_PURCHASE) {
            ...
            //@dev we return the _parcelId to regiter it in the metadada.

            emit tokenPurchased(msg.sender, tokenId, getCalculatedPrice(totalPurchaseSupply + 1), _parcelId);
        }
    }
    ...
    function claimWhiteList() external override {
        ...
        if(totalPublicSupply < TOKEN_PUBLIC) {
    ...
    function claimTeam() external override {
        ...
        for (uint i = 0; i < 12; i++) {
            ...
            if(totalTeamAssignedSupply < TOKEN_TEAM_ASSIGNED) {
