...
contract NFT2021 is ERC721, Ownable {
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE AN NFT WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function changeNftName(uint _tokenId, string memory _name) public {
        ...
        require(sha256(bytes(_name)) != sha256(bytes(nftNames[_tokenId])), "New name is same as the current one");
    ...
    function viewNftName(uint _tokenId) public view returns( string memory ){
        require( _tokenId < totalSupply(), "Choose an NFT within range" );
