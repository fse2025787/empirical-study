...
contract StupidEgg is ERC721A, Ownable{
    using Strings for uint256;

    // Supply
    uint256 public constant MaxSupply = 10000;
    uint256 public constant FreeSupply = 2000;
    ...
    mapping(address => uint256) public totalMaxMint;

    uint256 public constant MaxTeamMint = 200;
    mapping(address => uint256) public totalMaxTeamMint;
    ...
    // Name and symbol
    constructor() ERC721A("Stupid Egg", "SE"){
    ...
    require((totalMaxTeamMint[msg.sender] + _quantity)  <= MaxTeamMint, "Cannot mint beyond team max mint!");
 
     totalMaxTeamMint[msg.sender] += _quantity;
     _safeMint(_to, _quantity);
    }

    function airdrop(address _to, uint _quantity) external onlyOwner {
    require(totalSupply() + _quantity <= 9800, "Reached max Supply");
     _safeMint(_to, _quantity);
    }
