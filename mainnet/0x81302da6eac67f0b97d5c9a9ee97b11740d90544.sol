...
contract EthereumUniswapV2Locker {
    ...
    event newLPadded(uint256 newLPamount, uint256 locknumber);
    event lockExtended(uint256 newTime, uint256 locknumber);
    event ownerUpdated(address newOwner, uint256 locknumber);
    event lpBurnt(uint256 locknumber, uint256 lockamount);
    event lpFastBurnt(address indexed lptoken, uint256 indexed liquidity);
    event newTimeApproved(address indexed _address, uint256 approvedTime);
    event fastBurnApproved(address _address);
    ...
    function lockTokens(
        ...
        uint256 lockNumber = locks.length;
        ownerLocks[_owner].push(lockNumber);
        tokenLocks[token].push(lockNumber);
        ...
        newlock.lockid = lockNumber;

        locks.push(newlock);

        emit lpLocked(token, amount, unlocktime, _owner, lockNumber);
        return lockNumber;
    }
    ...
    function fastLockLpWithTokens(
        ...
        uint256 lockNumber = locks.length;
        ownerLocks[_owner].push(lockNumber);
        tokenLocks[lptoken].push(lockNumber);
        ...
        emit lpLocked(lptoken, liquidity, unlocktime, _owner, lockNumber);
        return lockNumber;
    }
    ...
    function fastLockLPWithETH(
        ...
        uint256 lockNumber = locks.length;
        ownerLocks[fastlocketh.owner].push(lockNumber);
        tokenLocks[lptoken].push(lockNumber);
    ...
    function getLockNumber() external view returns (uint256) {
        return locks.length;
    }
