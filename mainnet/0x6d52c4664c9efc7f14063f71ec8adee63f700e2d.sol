...
contract PuffyCatz is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public ogListTotalMintSupply = 0; // by default - 0
    uint256 public presaleTotalMintSupply = 0; // by default - 0
    uint256 public publicTotalMintSupply = 0; // by default - 0

    uint256 public startTime = 1646565600;
    uint256 public endTime = 1646911200;
    uint256 public presaleMintPrice = 0.01 ether;
    uint256 public presaleSupplyLimit = 3000;
    uint256 public whiteListWalletLimit = 2;
    uint256 public OgWalletLimit = 3;
    ...
    address[] public ogMinterAddresses;
    ...
    function mint(uint256 quantity) public payable {
        ...
        if(block.timestamp >= startTime && block.timestamp < endTime) {
            if(isWhitelisted(msg.sender)) {
                require(msg.value >= presaleMintPrice * quantity, "Insufficient funds");
                require((presaleTotalMintSupply + quantity) <= presaleSupplyLimit, "Max NFT limit exceeded for presale");
                uint256 ownerMintedCount = addressMintedBalance[msg.sender];
                require((ownerMintedCount + quantity) <= whiteListWalletLimit, "Max NFT per address exceeded for whitelist");
                whitelistTotalMintSupply += quantity;
                presaleTotalMintSupply += quantity;
                addressMintedBalance[msg.sender]++;
                _safeMint(msg.sender, quantity);
            } else if(isOglisted(msg.sender)) {
                require(msg.value >= presaleMintPrice * quantity, "Insufficient funds");
                require((presaleTotalMintSupply + quantity) <= presaleSupplyLimit, "Max NFT limit exceeded for presale");
                uint256 ownerMintedCount = addressMintedBalance[msg.sender];
                require((ownerMintedCount + quantity) <= OgWalletLimit, "Max NFT per address exceeded for OG role");
                ogListTotalMintSupply += quantity;
                presaleTotalMintSupply += quantity;
                addressMintedBalance[msg.sender]++;
                _safeMint(msg.sender, quantity);
            } else {
                require(isWhitelisted(msg.sender), "Address is not in the whitelist/OG Role list");
            }
        } else {
            require(block.timestamp > endTime, "Public sale is disabled.");
    ...
    function setWhiteListWalletLimit(uint256 _limit) public onlyOwner {
        whiteListWalletLimit = _limit;
    }

    function setOgWalletLimit(uint256 _limit) public onlyOwner {
        OgWalletLimit = _limit;
    }
    ...
    function setPresaleSupplyLimit(uint256 _newSupply) public onlyOwner {
        require(whitelistTotalMintSupply <= _newSupply, "new limit lower than total supply");
    ...
    function setOnlyPresale(bool _state) public onlyOwner nonReentrant {
        onlyPresale = _state;
    }
    ...
    function setOgMinterAddress(address[] calldata _users) external onlyOwner {
        delete ogMinterAddresses;
        ogMinterAddresses = _users;
    }

    function isOglisted(address _user) public view returns (bool) {
        for (uint i = 0; i < ogMinterAddresses.length; i++) {
            if (ogMinterAddresses[i] == _user) {
                return true;
            }
        }
        return false;
    ...
    function removeOglistUser(address _user) public onlyOwner nonReentrant {
        for (uint i = 0; i < ogMinterAddresses.length; i++) {
            if (ogMinterAddresses[i] == _user) {
                delete ogMinterAddresses[i];
            }
        }
    }

    function configurePresale(uint256 _startTime, uint256 _endTime) public onlyOwner nonReentrant {
