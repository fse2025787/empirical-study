...
contract NFTMarket is ReentrancyGuard, Ownable {
  ...
  uint256 constant public TOTAL_PERC = 100;

  uint256 public listingPercentage = 5; //5%
  ...
  mapping(address=>uint256) public amountToPayMapping;
  ...
  function getListPercenteage() public view returns (uint256) {
    return listingPercentage;
  }

  function setListingPercentage(uint256 _listingPercentage) public onlyOwner {
      require(listingPercentage != _listingPercentage,"New listing percentage cannot be same with previous listing percentage");
      require(_listingPercentage<=20,"Invalid percentage");
      listingPercentage = _listingPercentage;
  }
  ...
  function createMarketItem(
    ...
    require(royalityPerc<=10,"Royality cannot be greater than 10%");
  ...
  function createMarketSale(
    ...
    uint256 listFee = (price * listingPercentage)/TOTAL_PERC;
  ...
  function claimBidItem(uint256 itemId) public nonReentrant {
    require(idToMarketItem[itemId].isOnAuction,"Item is not on auction");
  ...
  function withdrawYourBid(uint256 itemId) public nonReentrant{
    require(idToMarketItem[itemId].isOnAuction,"Item is not on auction");
    require(idToMarketItem[itemId].bidEndTime<=block.timestamp,"Bidding is still in progress");
    uint tokenId = idToMarketItem[itemId].tokenId;
    require(highestBidderMapping[itemId].bidderAddr==msg.sender,"Only highest bidder can claim the NFT");
    require(!IERC721(nftContractAddress).isApprovedForAll(idToMarketItem[itemId].seller,address(this)) &&
      IERC721(nftContractAddress).getApproved(tokenId)!=address(this),"Cannot withdraw your bid");

    uint256 amountToPay = highestBidderMapping[itemId].amount;
    highestBidderMapping[itemId].amount = 0;
    highestBidderMapping[itemId].bidderAddr = address(0);
    totalAmountDue = totalAmountDue.sub(amountToPay);
  ...
  function changeNftContractAddress(address _newNftContract) public onlyOwner{
  ...
  function getDueAmount(address userAddress) public view returns(uint256){
    return amountToPayMapping[userAddress];
  }
  ...
  function withdrawDueAmount() public {
    uint256 amountToPay = amountToPayMapping[msg.sender];
    require(amountToPay>0,"No due amount");
    amountToPayMapping[msg.sender] = 0;
    totalAmountDue = totalAmountDue.sub(amountToPay);
    payable(msg.sender).transfer(amountToPay);
  }
