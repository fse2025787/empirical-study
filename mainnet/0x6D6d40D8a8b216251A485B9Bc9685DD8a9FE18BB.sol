...
contract RoyaltyFeeSetter is Ownable {
    ...
    function updateRoyaltyInfoForCollectionIfSetter(
        ...
        (address currentSetter, , ) = IRoyaltyFeeRegistry(royaltyFeeRegistry).royaltyFeeInfoCollection(
            collection
        );
    ...
    /**
     * @notice Batch update royalty info for collection.
     * @dev Can only be called by contract owner (of this)
     * @param collections address of the NFT contract
     * @param setters address that sets the receiver
     * @param receivers receiver for the royalty fee
     * @param fees fee (500 = 5%, 1,000 = 10%)
     * @param numOfCollections number of collections
     */
    function updateRoyaltyInfoForCollectionBatch(
        address[] memory collections,
        address[] memory setters,
        address[] memory receivers,
        uint256[] memory fees,
        uint256 numOfCollections
    ) external onlyOwner {
        require(
            (collections.length == numOfCollections) &&
                (setters.length == numOfCollections) &&
                (receivers.length == numOfCollections) &&
                (fees.length == numOfCollections),
            "RoyaltyFeeSetter: length must match numOfCollections"
        );

        for (uint256 i = 0; i < numOfCollections; i++) {
            require(
                !IERC165(collections[i]).supportsInterface(INTERFACE_ID_ERC2981),
                "RoyaltyFeeSetter: Must not be ERC2981"
            );

            _updateRoyaltyInfoForCollectionIfOwnerOrAdmin(collections[i], setters[i], receivers[i], fees[i]);
        }
    }
