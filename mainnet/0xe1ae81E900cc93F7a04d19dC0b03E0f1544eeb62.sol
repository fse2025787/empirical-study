...
contract Flokinji is Context, IERC20, Ownable {
    ...
    uint256 private decimalFactor = 1e9;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000 * decimalFactor;
    ...
    string private _name = 'Flokinji';
    string private _symbol = 'FLOKINJI';
    ...
    uint256 public marketingDivisor = 75;    
    
    uint256 public _maxTxAmount = 10000 * decimalFactor;
    uint256 public _maxWalletSize = 20000 * decimalFactor;
    uint256 private minimumTokensBeforeSwap = 500 * decimalFactor;
    uint256 public buyBackSellLimit = 1 ether;
    ...
    function updateBotWallet(address botwallet, bool isBot) external onlyOwner() {
        botWallets[botwallet] = isBot;
    }

    function updateTrading(bool isEnabled) external onlyOwner() {
        canTrade = isEnabled;
    }

    function updateBotTrading(bool isEnabled) external onlyOwner() {
        botscantrade = isEnabled;
    }
    ...
    function buyBackTokens(uint256 amount) private lockTheSwap {
    	if (amount > 0) {
    	    swapETHForTokens(amount.div(_buyBackDivisor));
	    }
    }
    ...
    function _setMaxWallet(uint256 maxWalletSize) external onlyOwner {
