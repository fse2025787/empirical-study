...
contract PACC is ERC721A, Ownable, ReentrancyGuard {
    string public baseURI = "ipfs://bafybeifhqslondqzjkbms4lk3jqqyyfbxby6knndwes7t4nj7mtpwalsam/";
    uint   public price             = 0.0015 ether;
    uint   public maxPerTx          = 20;
    uint   public maxPerFree        = 1;
    uint   public totalFree         = 3999;
    ...
    constructor() ERC721A("Pixel Ape Cyborg Club", "PACC"){}
    ...
    function mint(uint256 count) external payable {
        ...
        require(count <= maxPerTx, "Max per txn reached.");
            if(count >= (maxPerFree - _mintedFreeAmount[msg.sender]))
            {
             require(msg.value >= (count * cost) - ((maxPerFree - _mintedFreeAmount[msg.sender]) * cost), "You've already minted max free allocation or incorrect ETH sent");
