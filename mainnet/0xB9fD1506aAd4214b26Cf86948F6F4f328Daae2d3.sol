...
contract Changer {
  ...
  event ParamChanged(uint256 tokenID, uint256 i);
  ...
  //sets on parameter. Must be an editor with allowCommit access
  function setTokenParam(uint256 tokenID, uint256 i, uint32 v) public requireCommiter(tokenID) virtual {
    deafbeef.setTokenParam(tokenID,i,v);
    emit ParamChanged(tokenID,i);
  }
}