...
// File: contracts/DemonPartySpecials.sol
...
contract DemonPartySpecials is ERC721, Ownable {
  ...
  constructor() ERC721("DemonPartySpecials", "DEMONSSP") {
  }

  function mint(address to, string memory _tokenURI) public onlyOwner {
    tokenURIs[owners.length] = _tokenURI;
    _safeMint(to, owners.length);
  }
  function batchMint(address[] memory tos, string[] memory _tokenURIs) public onlyOwner {
    require(tos.length == _tokenURIs.length, "ARGS_UNEQUAL_LENGTH");
    for(uint i=0; i < tos.length; i++) {
      mint(tos[i],_tokenURIs[i]);
    }
  }
  ...
  function setTokenURI(uint256 id, string memory newUri) public onlyOwner {
    require(_exists(id), "ERC721Metadata: token doesn't exist");
    tokenURIs[id] = newUri;
  }
