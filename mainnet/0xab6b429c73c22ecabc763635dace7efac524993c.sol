...
contract LPStaking is OwnableAndWhitelistble{
    ...
    event SettedBetta(address indexed sender, uint betta);
    ...
    returns (uint value_, uint toDividends_) {
        require(stakes[who_][lpTokenAddress_].startStaking != 0, 'You dont have stake');
    ...
    view
    returns (uint rate_) {
        return ((rate - lowRate) * alpha * ETH) / (pool_ * betta + alpha * ETH) + lowRate;
    }
    ...
    external
    onlyOwner {
        require(alpha_ > 0, 'Alpha is incorrect');
        alpha = alpha_;
        emit SettedAlpha(msg.sender, alpha_);
    }

    function setBetta(uint betta_)
        external
        onlyOwner {
            require(betta_ >= 1, 'Betta is incorrect');
            betta = betta_;
            emit SettedBetta(msg.sender, betta_);
        }
    ...
    onlyOwner {
        lowRate = lowRate_;
        emit SettedLowRate(msg.sender, lowRate_);
    }
