...
contract OQMag3 is ERC721, Ownable, PaymentSplitter {
  ...
  uint256 public constant costWL = 0.015 ether;
  uint256 public currentMaxSupply = 500;
  uint256 public finalMaxSupply = 500;
  uint256 public maxMintAmountPerTx = 50;
  uint256 public maxWLMintCount = 20;
  bytes32 public wlRoot; 
  bool public paused = true;
  bool public revealed = true;
  constructor(address[] memory _payees, uint256[] memory _shares, bytes32 _WLmerkleroot) PaymentSplitter(_payees, _shares) payable ERC721("OQ Magazine", "OQ3")  {
  ...
  function mintWL(uint256 _mintAmount, bytes32[] calldata _proof) public payable mintCompliance(_mintAmount) {
    uint256 mintCount = addressMintCount[msg.sender];
    require(!paused, "The contract is paused!");
    require(msg.value >= costWL * _mintAmount, "Insufficient funds!");
    require(_verify(_leaf(msg.sender), _proof, wlRoot), "Not Whitelisted");
    require(mintCount + _mintAmount <= maxWLMintCount, "Exceeded whitelist allowance.");
    addressMintCount[msg.sender] = mintCount + _mintAmount;
    _mintLoop(msg.sender, _mintAmount);
  }
  ...
  function _leaf(address _account) internal pure returns (bytes32)
    ...
    function _verify(bytes32 leaf, bytes32[] memory _proof, bytes32 _root) pure internal returns (bool)
    {
