...
I can say that this is not a hobby for the poor

However, it is a great thing to learn about history
And to see what came before

And I wish more people to understand this
So I have made a token so that anyone may participate
And join in
Because max transactions are tiny
And everyone can have a chance to understand
Antiques
...
contract Antique is Context, IERC20, Ownable {
    ...
    string private _name = "Collector";
    string private _symbol = "Antique";
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 20, "Tax fee cannot be more than 10%");
        _taxFee = taxFee;
    }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 20000, "Max Tx Amount cannot be less than 69 Million");
        _maxTxAmount = maxTxAmount * 10**9;
    }
    
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount > 20000, "Swap Threshold Amount cannot be less than 69 Million");
