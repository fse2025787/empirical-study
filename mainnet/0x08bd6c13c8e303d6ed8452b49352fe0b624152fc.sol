...
contract TimelockInvestors is Initializable {
    ...
    struct Investor {
        ...
        uint256 amount; // 用户金额
        uint256 released; //释放金额
        bool available;
    }

    struct Strategy {
        uint256 firstReleaseBegin;  //第一次释放开始时间
        uint256 firstReleaseScale;  //第一次释放比例

        uint256 intervalReleaseBegin;   //间隔释放开始时间
        uint256 intervalReleaseScale;   //间隔释放比例
        uint256 intervalReleaseTimes;   //间隔释放次数
        uint256 intervalReleasePeriod;  //间隔释放间隔时间

        uint256 lastReleaseBegin;   //最后一次释放开始时间
        uint256 lastReleaseScale;   //最后一次释放比例
    }
