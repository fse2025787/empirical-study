...
contract CalabasasKidClub is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable public _OwnerAddress =
        payable(0x9B7d506337Dba661837284132FfC8E6072C09F9b);
    address payable public _DevAddress =
        payable(0x7De83438D71cCF4A5121e12259ec4e717E9b7205);

    uint256 public constant CKC_MAX = 5000;
    uint256 public constant PURCHASE_LIMIT = 20;
    uint256 public PRICE = 50_000_000_000_000_000; // 0.05 ETH

    string private _contractURI = "";
    string private _tokenBaseURI = "";
    bool private _isActive = false;

    Counters.Counter private _publicCKC;

    constructor() ERC721("Calabasas Kid Club", "CKC") {}

    function setPaymentAddress(address ownerAddress, address devAddress)
        external
        onlyOwner
    {
        _OwnerAddress = payable(ownerAddress);
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        ...
        require(
            _publicCKC.current() < CKC_MAX,
            "Purchase would exceed CKC_MAX"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCKC.current();

            if (_publicCKC.current() < CKC_MAX) {
                _publicCKC.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        uint256 feeAmount = (msg.value * 3) / 100;
        _DevAddress.transfer(feeAmount);
        _OwnerAddress.transfer(msg.value - feeAmount);
    }
