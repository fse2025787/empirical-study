...
abstract contract PepeMigrationI is Ownable, IERC721 {
...
contract PepeMigration is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    function onERC1155Received(address _sender, address _receiver, uint256 _tokenId, uint256 _amount, bytes memory _data) public override nonReentrant returns (bytes4) {
    ...
    function claim(uint256 _oldId, uint16 _newId) external nonReentrant {
        require(bridgingEnabled, "Bridging is stopped");
        require(osContract.balanceOf(msg.sender, _oldId) > 0, "You don't have ERC1155 Tokens");
        require(!oldIdsBridged[_oldId], "ERC1155 token is already bridged");
        require(!pepeContract.exists(_newId), "ERC721 token already exists");

        totalBridged++;
        idsBridged.push(_newId);
        oldIdsBridged[_oldId] = true;
    ...
    function mint721(uint16 _tokenId, address _to) external onlyOwner {
        ...
        require(!pepeContract.exists(_tokenId), "Token exists");

        if (pepeContract.exists(_tokenId) && pepeContract.ownerOf(_tokenId) == address(this)) {
            pepeContract.safeTransferFrom(address(this), _to, _tokenId);
            return;
        }
    ...
    function transfer721(uint256 _tokenId, address _owner) external onlyOwner {
        ...
        pepeContract.safeTransferFrom(address(this), _owner, _tokenId);
    }

    //Admin only

    function _mint721(uint16 _tokenId, address _owner) private {
        pepeContract.mint(_owner, uint16(_tokenId));
    ...
    function mintOnClaiming(address _sender, uint16 _tokenId) internal returns (bool) {
        ...
        require(!pepeContract.exists(_tokenId), "Token already minted");

        pepeContract.mint(_sender, _tokenId);
        emit Minted721(_sender, _tokenId);
        return true;
    }

    //Settings
    function setpepeContract(address _contract) external onlyOwner {
        require(_contract != address(0), "_contract !address 0");
        pepeContract = PepeMigrationI(_contract);
    }
    ...
    function getPepeContract() external view returns (address) {
        return address(pepeContract);
    }
