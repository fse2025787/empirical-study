...
contract THEFUCKINGBANANA is ERC721, Ownable {
    ...
    constructor(string memory u) ERC721("The Fucking Banana", "FUCK") {
      metaUri=u;
    ...
    function premint(uint256 quantity) public payable {
        require(ispreSale==true, "Public Sale is Not Active");
        require((currentToken + quantity) <= presupply, "Quantity Exceeds Tokens Available");
        require((price * quantity) <= msg.value, "Ether Amount Sent Is Incorrect");
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, currentToken);
            currentToken = currentToken + 1;
