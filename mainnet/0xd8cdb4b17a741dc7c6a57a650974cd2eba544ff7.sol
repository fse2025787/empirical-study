...
contract DoE is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    string public baseURI = "ipfs://QmVoKgyeHD6KLzUyScb72x7nAtUjAqr5PEDoWxdFYCrWer/";
    string private _name = "Dogs of Elon";
    string private _symbol = "DoE"; 
    ...
    uint256 public maxBatch = 20; //Max Mint
    ...
    function claim(uint256 _batchCount) payable public {//Will add in getMintTime check using block.timestamp here tomorrow
        require(canMint, "Claiming not active");
        require(_batchCount > 0 && _batchCount <= maxBatch, "Maximum batch exceeded");
        require((totalSupply() + initialReserve) + _batchCount + burnCount <= totalCount, "Sale over");
        require(msg.value == _batchCount * price, "Insufficient value");
        require(balanceOf(msg.sender) + _batchCount <= maxBatch, "Personal mint amount exceeded");
    ...
    function rescueEtherAmount(uint256 _amount) public onlyOwner {
        (bool sent,) = clientWallet.call{value: _amount}("");
   ...
   */
    ...
    function _walletDistro() internal {
        ...
        (bool sentC,) = clientWallet.call{value: contractBalance}("");
