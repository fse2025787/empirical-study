...
contract ManageLifeInvestorsNFT is ERC721A, Ownable {
    ...
    mapping(uint256 => uint64) public startOfStaking;
    ...
    function updateLifeTokenIssuanceRate(
        ...
        startOfStaking[tokenId] = uint64(block.timestamp);
    ...
    function checkClaimableStakingRewards(
        ...
        return
            (uint64(block.timestamp) - startOfStaking[tokenId]) *
    ...
    function transferNft(
        ...
        bool hasClamaible = checkClaimableStakingRewards(tokenId) > 100;

        
        if (hasClamaible) {
            this.claimStakingRewards(tokenId);
        }
        safeTransferFrom(msg.sender, to, tokenId);
    ...
    function returnNftToML(uint256 tokenId) external onlyInvestor(tokenId) {
        ...
        unlockDate[tokenId] = uint64(block.timestamp);
        safeTransferFrom(msg.sender, owner(), tokenId);
    }
