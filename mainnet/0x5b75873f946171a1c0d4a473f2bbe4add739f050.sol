...
// File: yomiswap/src/OtherPool.sol
...
contract OtherPool is BasePool {
    ...
    //@param totalFTprice: total price of FT price
    uint256 public totalFTprice;
        ...
        returns (uint256 _LP)
    {
        if (totalFTpoint == 0) {
            ...
            _LP = totalFTpoint * _totalFee / (totalFTprice + totalFTfee);
        }
    }
    ...
    function _calcFTfee(address _user)
        ...
        returns (uint256 _userFee)
    {
        uint256 _tmpPrice = ((totalFTprice + totalFTfee) * userInfo[_user].totalFTpoint / totalFTpoint);
        if (_tmpPrice > userInfo[_user].initSellAmount) {
            _userFee = _tmpPrice - userInfo[_user].initSellAmount;
        } else {
            _userFee = 0;
        }
    }
...
// File: yomiswap/src/NativeOPool.sol
...
// File: yomiswap/pool/SingleNativeOPool.sol
...
contract SingleNativeOPool is NativeOPool {
    constructor(
        address _collection,
    ...
    function withdrawFT(
        ...
        uint256[] calldata _tokenIds,
        address _user
    ) external payable onlyRouter returns(uint256 _userFee) {
        uint256 _itemNum = _tokenIds.length;
        uint256 _userSellAmount = userInfo[_user].initSellAmount;
        _userFee = _calcFTfee(_user);
        uint256 _fee;
        ...
        //effect
        totalFTprice -= userInfo[_user].initSellAmount;
        poolInfo.sellNum -= _userSellNum;
    ...
    function withdrawFee(address _user) external payable onlyRouter {
        ...
        if(userInfo[_user].totalFTpoint > 0){
            uint256 _totalStakeFTfee = _calcFTfee(_user);
            uint256 _totalStakeFTpoint = totalFTpoint * _totalStakeFee / (totalFTprice + totalFTfee);
            _totalStakeFee += _totalStakeFTfee;
            _updateFTpointAndFee(_user, _totalStakeFTfee, _totalStakeFTpoint);
        }
        
        if(userInfo[_user].totalNFTpoint > 0){
            uint256 _totalStakeNFTfee = _calcNFTfee(_user);
            uint256 _totalStakeNFTpoint = totalNFTpoint * _totalStakeFee / (totalNFTprice + totalNFTfee);
    ...
    function _updateFTpointAndFee(address _user, uint256 _userFee, uint256 _userPoint) internal {
      userInfo[_user].totalFTpoint -= _userPoint;
      totalFTpoint -= _userPoint;
      totalFTfee -= _userFee;
    }
...
// File: yomiswap/factory/FactorySingleNativeOPool.sol
...
contract FactorySingleNativeOPool is Ownable {
    address public router;
