...
contract PaymentSplitter is Context {
    ...
    function release(address payable account) public virtual {
        ...
        if (block.timestamp - deploymentTime > 31536000) {
            totalReceived = address(this).balance + totalReleased();
        } else {
            totalReceived = address(this).balance + totalReleased() - totalReturnToCommunity;
        }
...
contract LifeUnderwater is ERC721Enumerable, Ownable, PaymentSplitter {
    ...
    constructor() ERC721("Almost Tame Life Underwater", "ATLU") PaymentSplitter(_team, _teamShares) payable {
        categories[Category.FEMALE_COUPLE] = CategoryInfo({startIndex: 1, size: 500, supply: 500, teamReserved: 2});
        categories[Category.MALE_COUPLE] = CategoryInfo({startIndex: 501, size: 500, supply: 500, teamReserved: 2});
        categories[Category.MALE_FEMALE_COUPLE] = CategoryInfo({startIndex: 1001, size: 500, supply: 500, teamReserved: 2});
        categories[Category.FEMALE] = CategoryInfo({startIndex: 1501, size: 500, supply: 500, teamReserved: 2});
        categories[Category.MALE] = CategoryInfo({startIndex: 2001, size: 500, supply: 500, teamReserved: 0});
        categories[Category.KIDS] = CategoryInfo({startIndex: 2501, size: 500, supply: 500, teamReserved: 0});
        categories[Category.CUSTOM] = CategoryInfo({startIndex: 30001, size: 100, supply: 100, teamReserved: 0});
    }

    function setPriceIncrementStep(uint256 val) external onlyOwner {
        priceIncrementStep = val;
    }

    function setPriceChangeOnTokensCount(uint256 val) external onlyOwner {
        priceChangeOnTokensCount = val;
    }
    ...
    function getTokenPrice(uint256 id) public view returns(uint256) {
        return baseTokenPrice + priceIncrementStep * (uint256)(id / priceChangeOnTokensCount);
    }
    ...
    function teamMint() external onlyOwner {
        for (uint256 i = 0; i < _team.length; i++) {
            uint256 start = categories[Category(i)].startIndex;

            for (uint256 j = 0; j < categories[Category(i)].teamReserved; j++) {
                _safeMint(_team[i], start + j);
            }

            categories[Category(i)].supply -= categories[Category(i)].teamReserved;
            categories[Category(i)].teamReserved = 0;
        }
    }
