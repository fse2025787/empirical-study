...
contract Aavee2 is IExchange, AdapterStorage {
  ...
  function getKey() public override pure returns(bytes32) {
      return keccak256(abi.encodePacked("AaveV2", "1.0.1"));
    }
    ...
    private
  {
    ...
    if (address(fromToken) == address(data.aToken)) {
      if (address(toToken) == Utils.ethAddress()) {
        Utils.approve(lData.wethGateway, address(fromToken), fromAmount);
        IWETHGateway(lData.wethGateway).withdrawETH(lData.lendingPool, fromAmount, address(this));
      }
      else {
        Utils.approve(lData.lendingPool, address(fromToken), fromAmount);
        IAaveLendingPool(lData.lendingPool).withdraw(toToken, fromAmount, address(this));
      }
    }
    else if (address(toToken) == address(data.aToken)) {
      if (address(fromToken) == Utils.ethAddress()) {
        IWETHGateway(lData.wethGateway).depositETH{value : fromAmount}(lData.lendingPool, address(this), lData.refCode);
      }
      else {
        Utils.approve(lData.lendingPool, address(fromToken), fromAmount);
        IAaveLendingPool(lData.lendingPool).deposit(fromToken, fromAmount, address(this), lData.refCode);
      }
    }
    else {
