...
// File: contracts\GamingChick.sol
...
contract GamingChick is ERC721Enumerable, OwnableContract{
    ...
    mapping(uint256 => Attribute) private tokenIdToAttributesMap;

    event UpdateAttribute(uint256 tokenId, Attribute attribute);

    constructor() ERC721("GamingChick","GamingChick"){
        
    }

    function updateTokenIdToAttributesMap(uint256 tokenId, Attribute memory attribute) public onlyDev{
        tokenIdToAttributesMap[tokenId] = attribute;
        emit UpdateAttribute(tokenId, attribute);
    }

    function getTokenIdToAttributesMap(uint256 tokenId) public view returns(Attribute memory attribute){
        return tokenIdToAttributesMap[tokenId];
    }

    function mint(address user, uint256 tokenId, Attribute memory attribute) public onlyDev{
        require(tokenId <= maxTotalSupply, "The maximum amount has been exceeded.");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        return string(abi.encodePacked(baseURI, 'tokenId=', tokenId.toString()));
    }
