...
// File contracts/lib/NativeWrap.sol
...
// File contracts/ChainHopWrapper.sol


pragma solidity >=0.8.15;
contract ChainHopWrapper is NativeWrap, ReentrancyGuard, Pauser {
    using SafeERC20 for IERC20;
    uint256 public protocolFee;
    address public chainhopExecutionNode;
    uint256 constant MAX_PERCENT = 1000000;
    address public treasuryAddress;
    event ExecutionNodeUpdated(address chainhopExecutionNode);
    event TreasuryAddressUpdated(address treasuryAddress);
    event ProtocolFeeUpdated(uint256 protocolFee);

    constructor(address _nativeWrap, address _executionNode) NativeWrap(_nativeWrap) {
        protocolFee = 5000; // 0.5% = 5000/1000000
        treasuryAddress = msg.sender;
        chainhopExecutionNode = _executionNode;
    }
    ...
    function srcExecute(
        ...
        (amountIn, ) = _pullFundFromSender(tokenIn, amountIn, nativeIn);
        uint256 feeAmount = (amountIn * protocolFee) / MAX_PERCENT;
        ...
        bool success;
        if (nativeIn) {
            (success, ) = treasuryAddress.call{value: feeAmount}("");
            realAmount = msg.value - feeAmount;
            (success, ) = chainhopExecutionNode.call{value: realAmount}(txData);
        } else {
            IERC20(tokenIn).safeTransfer(treasuryAddress, feeAmount);
            realAmount = amountIn - feeAmount;
            IERC20(tokenIn).safeIncreaseAllowance(chainhopExecutionNode, realAmount);
            (success, ) = chainhopExecutionNode.call{value: msg.value}(txData);
        }

        require(success, "chainhop error");
    }

    function _pullFundFromSender(
        address tokenIn,
        uint256 amountIn,
        bool nativeIn
    ) private returns (uint256 amount, address token) {
        if (nativeIn) {
            require(tokenIn == nativeWrap, "tokenIn not nativeWrap");
            require(msg.value >= amountIn, "insufficient native amount");
    ...
    function setExecutionNode(address _newExecutionNode) external onlyOwner {
        chainhopExecutionNode = _newExecutionNode;
        emit ExecutionNodeUpdated(chainhopExecutionNode);
    }

    function setTresuryAddress(address _newAddress) external onlyOwner {
        treasuryAddress = _newAddress;
        emit TreasuryAddressUpdated(treasuryAddress);
    }

    function setProtocolFee(uint256 _newFeeValue) external onlyOwner {
        require(_newFeeValue < MAX_PERCENT, "overflow");
        protocolFee = _newFeeValue;
