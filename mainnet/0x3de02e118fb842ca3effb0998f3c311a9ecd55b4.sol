...
contract Zinzilla is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = "ZinZilla";
    string private constant _symbol = "Znzl";
    ...
    address payable private _stakingFundsAddress = payable(0x7004Fb8DbFcBF5513Db6E7124EBFD13E470ED3f8); // just add needed addresses
    address payable private _marketingAddress = payable(0x7e3D7c6811202EE21c39385D9D9104752Ad03420);
    ...
    constructor() {
        ...
        UniswapRouter02 = _uniswapRouter;
        UniswapPair = UniswapFactory(_uniswapRouter.factory())
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (canSwap && !inSwap && from != UniswapPair && swapEnabled && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != UniswapPair && to != UniswapPair)) {
            ...
            //Set Fee for Buys
            if(from == UniswapPair && to != address(UniswapRouter02)) {
                _redisFee = _redisFeeOnBuy;
                _taxFee = _taxFeeOnBuy;
            }

            //Set Fee for Sells
            if (to == UniswapPair && from != address(UniswapRouter02)) {
    ...
    function swapTokensForETH(uint256 tokenAmount) private lockTheSwap {
        ...
        _approve(address(this), address(UniswapRouter02), tokenAmount);
