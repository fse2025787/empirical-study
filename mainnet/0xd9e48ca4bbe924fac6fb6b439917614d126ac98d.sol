...
/**
    Tomato Cash - TOMATO
    Website: Tomato.Cash or https://cloudflare-ipfs.com/ipns/tornadocash.eth
...
contract TOMATO is ERC20("TomatoCash", "TOMATO"), ERC20Burnable, ERC20Permit, Pausable, EnsResolve {
  ...
  modifier onlyGovernance() {
    require(_msgSender() == governance, "TOMATO: only governance can perform this action");
    _;
  }

  function changeTransferability(bool decision) public onlyGovernance {
    require(blockTimestamp() > canUnpauseAfter, "TOMATO: cannot change transferability yet");
    if (decision) {
      _unpause();
    } else {
      _pause();
  ...
  function _beforeTokenTransfer(
    ...
    require(!paused() || allowedTransferee[from] || allowedTransferee[to], "TOMATO: paused");
    require(to != address(this), "TOMATO: invalid recipient");
  }
  ...
  function rescueTokens(
    ...
    require(_to != address(0), "TOMATO: can not send to zero address");
    ...
    if (_token == IERC20(0)) {
      ...
      uint256 balance = _balance == 0 ? totalBalance : Math.min(totalBalance, _balance);
      require(balance > 0, "TOMATO: trying to send 0 balance");
