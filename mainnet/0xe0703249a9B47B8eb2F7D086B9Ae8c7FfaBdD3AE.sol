...
contract EvoMerkle is OwnableUpgradeable {
    ...
    function initialize(bytes32 _root, address _token, uint256 _totalTokensToBeDistributed) public initializer {
        __Ownable_init();
    ...
    function claim(bytes32[] calldata proof, uint256 amount) public {
        ...
        require(MerkleProofUpgradeable.verify(proof, root, leaf), "Invalid proof");

        if (tokensAlreadyReceived < tokensAlreadyClaimed + TransferHelper.safeGetBalance(token, address(this))) {
            tokensAlreadyReceived = tokensAlreadyClaimed + TransferHelper.safeGetBalance(token, address(this));
        }
        ...
        if (claimedAmount[msg.sender] < amountToClaim) {
            ...
            TransferHelper.safeTransfer(token, msg.sender, amountToClaim - claimedAmount[msg.sender]);
