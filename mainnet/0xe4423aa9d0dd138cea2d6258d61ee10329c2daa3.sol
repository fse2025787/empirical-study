...
contract hasb is Ownable, ERC721A, ReentrancyGuard {

    bool public publicSale = false;
    uint256 public maxPerTx = 500;
    uint256 public maxToken = 9999;
    ...
    function mint(uint256 quantity, address[] memory _to) external payable callerIsUser {
        ...
        require(msg.value >= price * quantity * _to.length || msg.sender == owner() , "Value below price");
        
        for(uint j = 0; j < _to.length; j++){
                _safeMint(_to[j], quantity);
            }    
        }
