...
/**
 * @title MasterMintV2 contract
 * @author Nil DAO
 * @notice This contract wraps actual minting contracts and performs administrative functions
 */
contract MasterMintV2 is ReentrancyGuard, AccessControl, INOwnerResolver {
     ...
     */
    function addMint(INilPassV2 nilPass, address creator) external onlyOperator {
    ...
    function _createMint(INilPassV2 nilPass, address creator) internal view returns (MintMetadata memory) {
    ...
    function calculateThreshold(INilPassV2 nilPass) public view returns (uint256 artistMiningThreshold) {
    ...
    function mintWithN(
        ...
        metadata.nilPass.mintWithN(msg.sender, tokenIds, msg.value, data);
    }
    ...
    function mint(
        ...
        metadata.nilPass.mint(msg.sender, amount, msg.value, data);
    }
    ...
    function mintTokenIds(
        ...
        require(metadata.status == MintStatus.OPEN, "MasterMint:MINT_NOT_ACTIVE");
        payments[mintId].totalPaidInWei += toUint112(msg.value);
        metadata.nilPass.mintTokenId(msg.sender, tokenIds, msg.value, data);
    }
    ...
    function nUsed(uint256 mintId, uint256 nid) external view returns (bool) {
        MintMetadata storage metadata = mintsMetadata[mintId];
        if (address(metadata.nilPass) == address(0)) return false;
        return metadata.nilPass.nUsed(nid);
    }

    function nsUsed(uint256 mintId, uint256[] calldata nids) external view returns (bool[] memory) {
        bool[] memory result = new bool[](nids.length);
        MintMetadata storage metadata = mintsMetadata[mintId];
        if (address(metadata.nilPass) == address(0)) return result;
        for (uint256 i = 0; i < nids.length; i++) {
            result[i] = metadata.nilPass.nUsed(nids[i]);
        }
        return result;
    }
    ...
    function tokensOwned(uint256 mintId, address owner) external view returns (uint256[] memory) {
        ...
        INilPassV2 nilPass = metadata.nilPass;
...
interface INilPassV2 is IERC721Enumerable, IERC721Metadata, IPricingStrategy {
    struct MintParams {
