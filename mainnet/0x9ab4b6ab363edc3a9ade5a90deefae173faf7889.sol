...
████████                    ████████
 ...
 _                    _                                         __          _                 _ 
|_) _ __  _| _  _    |_| __ _    |\| _ _|_   |/ __  _    __    |_  _  __   |_)   __ __  o __ (_|
...
abstract contract Ownable is Context {
    ...
    contract MergePandas is ERC721A, Ownable {
    using Strings for uint256;
  ...
  constructor() ERC721A("Merge Pandas", "NORUNNING") {
  }

 function burnPanda(uint[] calldata token) external onlyOwner{
  ...
  function mintPanda(uint8 _mintAmount) external payable  {
  ...
  function Panda_Treasury(uint16 _mintAmount, address _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
  ...
  function Yay_ETH2(uint8 _amountPerAddress, address[] calldata addresses) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
...
if ( reveal == false)
{
 ...
 function How_Many_Free_Pandas(uint8 _limit) external onlyOwner{
...
function Pandas_Are_Not_Known_For_Running(uint8 _mintAmount) external payable {
