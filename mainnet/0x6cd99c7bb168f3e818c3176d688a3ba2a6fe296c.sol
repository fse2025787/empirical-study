...
/* GETDRAGONVERSE
 Website: https://getdragonverse.com
 Twitter: https://twitter.com/getdragonverse */
/*
                          _,,,.._       ,_
                        .gMMMMMMMMMp,_    `\
                     .dMMP'       ``^YMb..dP
...
contract DragonVerse is ERC721, Ownable {

    uint public mintPrice = 0.2 ether;
    uint public giveawayMaxItems = 30;
    uint public preMintMaxItems = 4000;
    ...
    bool public preMintNeedsWhitelist = true;
    bool public publicMintPaused = true;
    mapping(address => uint) public whitelist; // Maps address to how many they can premint

    event Mint(address indexed owner, uint indexed tokenId);

    constructor() ERC721("GetDragonVerse", "DRAGON") {}
    ...
    function preMint() external payable {
        ...
        //If whitelist is activated
        if (preMintNeedsWhitelist) {
            require(amount <= whitelist[msg.sender], "preMint: Amount greater than allocation");
            whitelist[msg.sender] -= amount;
        } else {
            require(amount <= maxItemsPerTx, "preMint: Max 2 per tx");
        }
        //
    ...
    function publicMint() external payable {
        ...
        require(amount <= maxItemsPerTx, "publicMint: Max 2 per tx");

        require(totalMinted + amount <= maxItems, "publicMint: Surpasses cap");
        _mintWithoutValidation(msg.sender, amount);
    }

    function _mintWithoutValidation(address to, uint amount) internal {
        require(totalMinted + amount <= maxItems, "mintWithoutValidation: Sold out");
        for (uint i = 0; i < amount; i++) {
            totalMinted += 1;
            _mint(to, totalMinted);
            emit Mint(to, totalMinted);
        }
    }

    function addToWhitelist(address[] memory toAdd) external onlyOwner {
        for(uint i = 0; i < toAdd.length; i++) {
            whitelist[toAdd[i]] = maxItemsPerPreMint;
        }
    }
    ...
    function setPreMintNeedsWhitelist(bool _preMintNeedsWhitelist) external onlyOwner {
        preMintNeedsWhitelist = _preMintNeedsWhitelist;
    }
