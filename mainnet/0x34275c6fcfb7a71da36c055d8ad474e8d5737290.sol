...
contract SoulNfts is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  string public notRevealedUri;
  uint256 public wlcost = 0.006 ether;
  uint256 public pcost = 0.009 ether;
  ...
  function ogmint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyOG == true) {
            require(isog(msg.sender), "user is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max NFT per address exceeded");
        }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
      require(msg.value >= pcost * _mintAmount);
    }
  ...
  function setCostp(uint256 _newCostp) public onlyOwner {
    pcost = _newCostp;
  }
