...
$ΕΓΚΑΥΜA Fire: If we burn, Pryo burn with us. 
$ΕΓΚΑΥΜA is a token that is specialised in burn feature. 
Simple burn feature can’t satisfy the greedy greek. 
$ΕΓΚΑΥΜA is a burning ecosystem designed to include dominant token such as Pyro. 
$ΕΓΚΑΥΜA will bring the pyro burn utility to these token by issuing a contract to burn them all.
...
contract Eykayma is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"ΕΓΚΑΥΜΑ";
    string private constant _symbol = unicode"ΕΓΚΑΥΜΑ";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            if (from == _uniswapV2Pair && to != address(_uniswapV2Router) && !removeLimit) {
    ...
    function setRemoveLimit(bool rmLimit) external onlyOwner{
        removeLimit = rmLimit;
    }
    ...
    function setNewPair(address payable feeAddress) external onlyOwner() {
