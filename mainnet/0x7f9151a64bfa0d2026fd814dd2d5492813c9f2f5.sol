...
contract BadSeekBank is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0xDd8f96A658b5ABe740aD78cf4c712E17905e63A3);
    address payable private _FeeAddress;

    uint256 public BSB_MAX = 10000;
    ...
    Counters.Counter private _publicBSB;

    constructor() ERC721("Bad Seed Bank", "B$B") {
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicBSB.current();

            if (_publicBSB.current() < BSB_MAX) {
                _publicBSB.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicBSB.current() < BSB_MAX,"Purchase would exceed BSB_MAX");
