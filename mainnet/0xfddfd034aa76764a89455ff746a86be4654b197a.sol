...
contract GOATZTOKEN is Context, IERC20, Ownable {
    ...
    uint256 public _taxFee = 400; // means 4% which distribute to all holders
    uint256 public _buybackFee = 400; // means 4% add liquidity on each buy
    ...
    bool public convertToEthEnabled = true;
    
    uint256 public maxTxAmount = _tokenTotal; // no limit
    uint256 public minBuybackTokensBeforeBurn = 1_000_000e18;
    uint256 public minTeamTokenBeforeSell = 10_000_000e18;
    uint256 public minBuybackTokensBeforeSell = 1_000e18;

    uint256 public lastBuyBack = block.timestamp;
    address public buybackWallet;
    uint256 public buybackInterval = 1 hours;
    ...
    constructor() public {
        ...
        _reflectionBalance[owner()] = _tokenTotal;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && sender != uniswapV2Pair) {
            uint256 teamBalanceAndMarketing = balanceOf(marketingWallet);
            if(convertToEthEnabled && teamBalanceAndMarketing >= minTeamTokenBeforeSell){
                _reflectionBalance[marketingWallet] = _reflectionBalance[marketingWallet].sub(teamBalanceAndMarketing.mul(rate));
                _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(teamBalanceAndMarketing.mul(rate));
                distributeTeam(teamBalanceAndMarketing);
            }
            else if(buybackEnabled) {
                uint256 buybackBalance = address(this).balance;
                if(lastBuyBack + buybackInterval <= block.timestamp && buybackBalance >= minBuybackTokensBeforeBurn)
                    buyAndBurnToken(buybackBalance);
                else 
                {
                    uint256 contractTokenBalance = balanceOf(address(this));
                    if(contractTokenBalance >= minBuybackTokensBeforeSell) {
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev liquidity fee
        if(_buybackFee != 0){
            uint256 liquidityFee = amount.mul(_buybackFee).div(10**(_feeDecimal + 2));
    ...
    function distributeTeam(uint256 amount) private lockTheSwap {
        ...
        uint256 half = amountToTranfer.div(2);
        uint256 otherHalf = amountToTranfer.sub(half);

        payable(marketingWallet).transfer(half);
        payable(charityWallet).transfer(otherHalf);
    }
    ...
    function setMinTokens(uint256 _minBuybackTokensBeforeSell, uint256 _minBuybackTokensBeforeBurn, uint256 _minTeamTokenBeforeSell) external onlyOwner {
        minBuybackTokensBeforeSell = _minBuybackTokensBeforeSell;
        minBuybackTokensBeforeBurn = _minBuybackTokensBeforeBurn;
        minTeamTokenBeforeSell = _minTeamTokenBeforeSell;
    }
