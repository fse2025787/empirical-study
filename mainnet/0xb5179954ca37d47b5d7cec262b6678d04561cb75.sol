...
contract flOOr is ERC20, Ownable {
    ...
    address public theflOOrAddress;
    ...
    uint256 public buyflOOrFee;
    uint256 public buyLiquidityFee;
    uint256 public buyTreasuryFee;

    uint256 private originalSellflOOrFee;
    uint256 private originalSellLiquidityFee;
    uint256 private originalSellTreasuryFee;

    uint256 public sellTotalFees;
    uint256 public sellflOOrFee;
    uint256 public sellLiquidityFee;
    uint256 public sellTreasuryFee;

    uint256 public tokensForTheflOOr;
    ...
    event UpdatedTheflOOrAddress(address indexed newWallet);
    ...
    constructor() payable ERC20("flOOr", "flOOr") {
        ...
        buyLiquidityFee = 0;
        buyTreasuryFee = 0;
        buyTotalFees = buyflOOrFee + buyLiquidityFee + buyTreasuryFee;

        originalSellflOOrFee = 3;
        ...
        sellLiquidityFee = 0;
        sellTreasuryFee = 0;
        sellTotalFees = sellflOOrFee + sellLiquidityFee + sellTreasuryFee;

        theflOOrAddress = address(0x78c0487dEEAAc3331fC67fEb13B64a180F091815);
        treasuryAddress = address(0x78c0487dEEAAc3331fC67fEb13B64a180F091815);

        _excludeFromMaxTransaction(newOwner, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(address(theflOOrAddress), true);
        _excludeFromMaxTransaction(address(treasuryAddress), true);

        excludeFromFees(newOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(address(theflOOrAddress), true);
    ...
    function restoreTaxes() external onlyOwner {
        sellflOOrFee = originalSellflOOrFee;
    ...
    function swapBack() private {
        ...
        uint256 ethForTheflOOr = (ethBalance * tokensForTheflOOr) /
            (totalTokensToSwap - (tokensForLiquidity / 2));

        ethForLiquidity -= ethForTheflOOr;
        ...
        (success, ) = address(theflOOrAddress).call{
    ...
    function setTheflOOrAddress(address _theflOOrAddress) external onlyOwner {
        require(
            _theflOOrAddress != address(0),
            "_theflOOrAddress address cannot be 0"
        );
        theflOOrAddress = payable(_theflOOrAddress);
        emit UpdatedTheflOOrAddress(_theflOOrAddress);
    }
