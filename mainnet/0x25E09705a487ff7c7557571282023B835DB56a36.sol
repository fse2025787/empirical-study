...
contract TGUtilities is ERC1155Supply, ERC1155Burnable, Ownable {
    ...
    */
    ...
    function mint(uint256[] memory ids, uint256[] memory amounts) external payable {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(totalSupply(i) + amounts[i]  <= utilities[i].maxSupply, "MAX SUPPLY FOR TOKEN REACHED!");
            calculatedPrice += amounts[i] * utilities[i].price;
        }
    ...
    // OWNER FUNCTIONS
    function sendUtility(uint256 cardID, uint256 amount, address to) external onlyOwner {
