...
contract SudoPunks is ERC721A, Ownable, ReentrancyGuard {
    uint256 private tokenCounter;

    uint256 public maxSupply = 10000; 
    uint256 public maxMintablePerWallet = 100;

    address punksData = address(0x16F5A35647D6F03D5D3da7b35409D65ba03aF3B2);
    ...
    constructor () ERC721A ("SudoPunks","SP")  {
    }
    ...
    function mintCollectibles(uint256 _count) public payable nonReentrant { 
        ...
        require((minted[msg.sender] + _count)< maxMintablePerWallet);
  ...
  function buildMetadata(uint256 _tokenId) internal view returns (string memory) {
      return string(abi.encodePacked(
              'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                          '{"name": "SudoPunk #', Strings.toString(_tokenId), 
                          '", "description":"', 
                          "Not affiliated with SudoSwap or LarvaLabs.",
                          '", "image": "', 
                          IPunkData(punksData).punkImageSvg(uint16(_tokenId)),
                          '"}')))));
  }
