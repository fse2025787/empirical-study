...
contract UpOnly is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private cooldownPeriod = 4 hours;
    mapping(address => uint256) private sellCooldown;  // sell cooldown time - 4 hours
    ...
    constructor() ERC20("Up Only", "Up Only") {
    ...
    function updateCoolDownPeriod(uint256 period) external onlyOwner {
        ...
        if(limitsInEffect){
            if (
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                        ...
                        sellCooldown[to] = block.timestamp + cooldownPeriod;
                }
 
                //when sell
                else if (automatedMarketMakerPairs[to] && !_isExcludedMaxTransactionAmount[from]) {
                        require(amount <= maxTransactionAmount, "Sell transfer amount exceeds the maxTransactionAmount.");
                        require(sellCooldown[from] < block.timestamp, "not able to sell token yet.");
                }
