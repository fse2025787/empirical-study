...
contract Gldm is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 96h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 2500 ether;
    ...
    /**
     * @notice Constructs the GLDM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("GoldMint", "GLDM") {
        // Mints 1 GLDM to contract creator for initial pool setup
    ...
    function _getGldmPrice() internal view returns (uint256 _gldmPrice) {
        try IOracle(gldmOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Gldm: failed to fetch GLDM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _gldmPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_gldmPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGldmOracle(address _gldmOracle) public onlyOperatorOrTaxOffice {
        require(_gldmOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints GLDM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GLDM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGldmPrice = _getGldmPrice();
            currentTaxRate = _updateTaxRate(currentGldmPrice);
            if (currentGldmPrice < burnThreshold) {
                burnTax = true;
            }
        }
