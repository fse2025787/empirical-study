...
contract Lockable is Context {
    ...
    event TimeLocked(address indexed account, uint256 amount, uint256 expiresAt, uint8 index);
     ...
     */
    function _addTimeLock(address account, uint256 amount, uint256 expiresAt) internal {
        ...
        emit TimeLocked(account, amount, expiresAt, uint8(_timeLocks[account].length - 1));
    }
     ...
     */
    function _addVestingLock(address account, uint256 amount, uint256 startsAt, uint256 period, uint256 count) internal {
     ...
     */
    function getVestingLock(address account) public view returns (uint256, uint256, uint256, uint256) {
...
/**
 * @dev Contract for Ripplereum token
 */
contract RPRM is Pausable, Ownable, Supervisable, Burnable, Freezable, Lockable, ERC20 {
    uint256 private constant _initialSupply = 25_000_000_000e18;

    constructor() ERC20("Ripplereum", "RPRM") {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override(ERC20) {
        ...
        require(balanceOf(from) >= amount + getAllLockedAmount(from), "Lockable: insufficient transfer amount");
    }
    ...
    /**
     * @dev unpause all token transfer
     */
     ...
     */
    function addTimeLock(address account, uint256 amount, uint256 expiresAt) public onlyLocker whenNotPaused {
        require(amount <= totalSupply(), "TimeLock: amount exceeds total supply");
    ...
    /**
     * @dev only locker can add custom vesting lock
     */
    function addCustomVestingLock(address account, uint256 amount, uint256 startsAt, uint256 period, uint256 count) external onlyLocker whenNotPaused {
        require(amount <= totalSupply(), "VestingLock: amount exceeds total supply");
