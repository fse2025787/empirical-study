...
interface Royalty is IERC2981Upgradeable {
    struct RoyaltyInfo {
        address recipient;
        uint256 bps;
    }
    ...
    function getRoyaltyInfoForToken(uint256 tokenId)
        external
        view
...
contract LootexCreatorCollection is
    Initializable,
    ...
    IBasicERC721
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("LootexCreatorCollection");
    ...
    {
        return (name(), "LootexCreatorCollection", initialDate);
    }
    ...
    function mintTo(address _to, string memory _uri)
        external
    {
    ...
    function mintToBatch(address[] memory _tos, string[] memory _uris)
        external
    {
        uint256 _tokenId = nextTokenIdToMint;
    ...
    function setUri(uint tokenId, string memory _uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uri[tokenId] = _uri;
    }
    ...
    function collectMistakeCurrency(address currency, address receiver, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        CurrencyTransferLib.transferCurrency(currency, address(this), receiver, amount);
    }
