...
// 
//https://medium.com/@daikoku777
//https://twitter.com/Daikoku_io
...
contract DividendDistributor is IDividendDistributor {
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
            IERC20(DAI).transfer(shareholder, amount);
        }
    }
    ...
    function getDividendsClaimedOf (address shareholder) external view returns (uint256) {
        require (shares[shareholder].amount > 0, "You're not a DaiKoku shareholder!");
        return shares[shareholder].totalClaimed;
    }

    }

contract DaiKoKu is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = unicode"大黒 | God of wealth";
    ...
    address payable public marketingWallet = payable(0x89A4fe91ad7E560838aEe020f40D12cE6C53aD52);
    ...
    function swapBack() internal swapping {
        ...
        swapTokensForDAI(amountToSwap.div(2));

        uint256 dividends = IERC20(DAI).balanceOf(address(this));

        bool success = IERC20(DAI).transfer(address(distributor), dividends);
