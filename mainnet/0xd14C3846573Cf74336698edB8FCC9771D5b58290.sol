...
contract MevRewardsSenseiStake is Ownable, ReentrancyGuard {
    ...
    address payable public senseiRewards;
    ...
    event SenseiRewardsAddressChanged(address indexed new_address);
    ...
    function claim() external nonReentrant {
        ...
        (bool ok_fee, ) = senseiRewards.call{value: fee}("");
        if (!ok_fee) {
            revert ErrorSendingETH();
        }
    ...
    function setSenseiRewardsAddress(address _address) external onlyOwner {
        if (_address == address(0)) {
            revert InvalidAddress();
        }
        senseiRewards = payable(_address);
        emit SenseiRewardsAddressChanged(_address);
    }
}