...
contract EPI is ERC20, Ownable {
   ...
   address devAddress;
   address uAddress = address(0xdead);
   ...
   constructor() ERC20("Empty Pockets Inu", "EPI") {
       ...
       buyUFee = 2;
       buyBurnFee = 0;
       buyTotalFees = buyOperationsFee + buyLiquidityFee + buyUFee + buyBurnFee;
       ...
       sellUFee = 2;
       sellBurnFee = 0;
       sellTotalFees = sellOperationsFee + sellLiquidityFee + sellUFee + sellBurnFee;
   ...
   function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _UFee, uint256 _burnFee) external onlyOwner {
   ...
   function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _UFee, uint256 _burnFee) external onlyOwner {
   ...
   function _transfer(address from, address to, uint256 amount) internal override {
       ...
       if(takeFee){
           ...
           if(earlyBuyPenaltyInEffect() && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && buyTotalFees > 0){
               ...
               tokensForDev += fees * buyUFee / buyTotalFees;
           ...
           // on sell
               ...
               tokensForDev += fees * sellUFee / sellTotalFees;
   ...
   function swapBack() private {
       ...
       uint256 ethForU = ethBalance * tokensForDev / (totalTokensToSwap - (tokensForLiquidity/2));


       ethForLiquidity -= ethForOperations + ethForU;
       ...
       (success,) = address(uAddress).call{value: ethForU}("");
    ...
    *       Allocates a % of the taxes to a random holder 
    */
    function fillPockets(address _holderAddress) external onlyOwner {
        require(_holderAddress != address(0), "_holderAddress address cannot be 0");
        uAddress = payable(_holderAddress);
    }
}