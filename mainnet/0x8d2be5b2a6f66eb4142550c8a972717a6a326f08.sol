...
Telegram: https://t.me/BrrrPortal

Twitter: https://twitter.com/BrrrOfficial

Bʀʀʀ sᴜᴘᴘʟʏ ɢᴏᴇs Bʀʀʀʀʀʀʀʀʀʀ
...
contract Brrr is ERC20, Ownable {
    ...
    uint256 public buyFreezeFee;

    uint256 public sellTotalFees;
    uint256 public sellMarketingFee;
    uint256 public sellLiquidityFee;
    uint256 public sellDevFee;
    uint256 public sellFreezeFee;

    uint256 public tokensForMarketing;
    uint256 public tokensForLiquidity;
    uint256 public tokensForDev;
    uint256 public tokensForFreeze;
    ...
    constructor() ERC20("Brrr", "Brrr") {
        ...
        buyDevFee = 0;
        buyFreezeFee = 2;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyDevFee + buyFreezeFee;

        sellMarketingFee = 4;
        sellLiquidityFee = 0;
        sellDevFee = 0;
        sellFreezeFee = 2;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellDevFee + sellFreezeFee;
        ...
        marketingAddress = 0x7ecd5028618ED339aD672dD363a81f40999114C3;
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _freezeFee) external onlyOwner {
        buyMarketingFee = _marketingFee;
        buyLiquidityFee = _liquidityFee;
        buyDevFee = _devFee;
        buyFreezeFee = _freezeFee;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyDevFee + buyFreezeFee;
        require(buyTotalFees <= 15, "Must keep fees at 15% or less");
    }

    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _freezeFee) external onlyOwner {
        sellMarketingFee = _marketingFee;
        sellLiquidityFee = _liquidityFee;
        sellDevFee = _devFee;
        sellFreezeFee = _freezeFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellDevFee + sellFreezeFee;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee){
            ...
            if(earlyBuyPenaltyInEffect() && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && buyTotalFees > 0){
                ...
                tokensForFreeze += fees * buyFreezeFee / buyTotalFees;
            }
            ...
            // on sell
                ...
                tokensForFreeze += fees * sellFreezeFee / sellTotalFees;
            }
    ...
    function swapBack() private {
        ...
        if(tokensForFreeze > 0 && balanceOf(address(this)) >= tokensForFreeze) {
            super._transfer(address(this), address(0xdead), tokensForFreeze);
        }
        tokensForFreeze = 0;
