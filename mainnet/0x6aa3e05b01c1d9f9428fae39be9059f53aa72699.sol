...
contract PokerLookVIP is ERC721Enumerable, Auth {
    ...
    address adminWallet = 0x76Cfa03a9A5E671aAE6b5fAF866C8BeFA11e2B7c;
    address devWallet = 0x5eacBb8267458B6bAC84D4019Bb64CC1a35b248E;
    ...
    constructor() ERC721("PokerLook VIP", "PKLV") payable Auth(msg.sender) { }
    ...
    function vipMint() public payable {
        ...
        require(balanceOf(msg.sender) < 1, "One token can mint per user!");
        mint();
    }

    function mint() internal {
        require(totalSupply() < maxSupply, "No tokens available for minting!");
        uint256 devAmount = msg.value.mul(devFee).div(1000);
        (bool dev, ) = payable(devWallet).call{value: devAmount}("");
        require(dev);
        uint256 adminAmount = msg.value.sub(devAmount);
        (bool admin, ) = payable(adminWallet).call{value: adminAmount}("");
        require(admin);
        uint256 _mintIndex = useRandomAvailableToken(1, 1);
        _safeMint(msg.sender, _mintIndex);
        emit TokenMint(_msgSender(), _mintIndex);
    }
