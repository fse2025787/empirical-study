...
// File contracts/autocompounder/SS-Strategy/StrategySSBase.sol

pragma solidity ^0.8.0;
//import "hardhat/console.sol";

abstract contract StrategySSBase is StratManager, FeeManager {
    ...
    // Routes
    address[] public rewardToStakedRoute;
    address[] public rewardToProfitRoute;
    ...
    constructor(
        ...
        address _rewardToken, // what the farm emits Token
        address _stakedToken, // what you put in Token
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (stakedBal < _amount) {
            farmWithdraw(_amount - stakedBal);
            stakedBal = stakedToken.balanceOf(address(this));
        }
        ...
        if (tx.origin == owner() || paused()) {
            stakedToken.safeTransfer(vault, stakedBal);
        } else {
            uint256 withdrawalFeeAmount = ((stakedBal * withdrawalFee) / MAXFEE);
            stakedToken.safeTransfer(vault, stakedBal - withdrawalFeeAmount);
        }
    }
    ...
    function chargeFees() internal {
        ...
        if (toProfit > 0)
        {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(toProfit, 0, rewardToProfitRoute, address(this), block.timestamp);
    ...
    function swap() internal {
        uint256 toStaked = IERC20(rewardToken).balanceOf(address(this));
        if (toStaked > 0)
        {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(toStaked, 0, rewardToStakedRoute, address(this), block.timestamp);
        }
    }
...
// File contracts/autocompounder/SS-Strategy/StrategySSMasterChef.sol
