    ...
    IMagpieCore
{
    ...
    mapping(uint8 => mapping(bytes32 => mapping(uint64 => mapping(address => uint256))))
        public deposits;
        ...
        )
    {
        ...
        emit SwappedIn(
            ...
            coreSequence,
            tokenSequence,
            bytes32(uint256(uint160(config.magpieBridgeAddress))),
            msg.sender
        );
    }
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        uint256 depositAmount = payload.bridgeType == IMagpieBridge.BridgeType.Wormhole ? 0 : deposits[payload.senderNetworkId][
            payload.senderAddress
        ][coreSequence][fromAssetAddress];

        // Process bridge if there was no deposit
        ...
        // The storage may changed from external call
        if (amountIn == 0) {
        ...
        swapArgs.routes[0].amountIn =
            swapArgs.routes[0].amountIn + (amountIn > swapArgs.getAmountIn() ? amountIn - swapArgs.getAmountIn() : 0) -
    ...
    function sgReceive(
        ...
        (uint8 networkId, bytes32 senderAddress, uint64 coreSequence) = payload
            .parseSgPayload();
        deposits[networkId][senderAddress][coreSequence][assetAddress] += amount;
        emit DepositReceived(networkId, senderAddress, coreSequence, assetAddress, amount);
    }
}
...
library LibBytes {
    ...
    function parseSgPayload(bytes memory self)
        internal
        pure
        returns (uint8 networkId, bytes32 senderAddress, uint64 coreSequence)
    {
        uint256 i = 0;
