...
// File contracts/MurmurationSale.sol
...
contract MurmurationSale is Ownable, PaymentSplitter {

    Murmuration public immutable murmuration;

    mapping(uint256 => uint256) public prices;

    constructor(
        address _murmuration,
        uint256[] memory _initialPrices,
        address[] memory _team,
        uint256[] memory _teamShares
    ) PaymentSplitter(_team, _teamShares) {
        murmuration = Murmuration(_murmuration);

        for(uint256 i = 0; i < _initialPrices.length; i++){
            prices[i+1] = _initialPrices[i];
        }
    }
    ...
    function mint(address _to, uint256 _id) public payable {
        ...
        require(prices[_id] > 0, "Price not set for this NFT");
        require(msg.value >= prices[_id], "Not enough funds");

        murmuration.mintTo(_to, _id);
