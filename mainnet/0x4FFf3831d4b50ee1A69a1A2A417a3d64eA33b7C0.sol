...
contract Hallowskulldao is ERC721A {
  using Strings for uint256;
  ...
  uint256 public SALE_PRICE = 0.0029 ether;
  uint256 public WL_PRICE = 0.0019 ether;
  uint256 public MAX_PER_TX = 10;
  uint256 public MAX_PER_FREE = 1;
  uint256 public MAX_TX = 10;
  uint256 public MAX_WL_TX = 5;
  uint256 public MAX_WL_SUPPLY = 300;
  ...
  constructor() ERC721A('Hallowskulldao', 'HSD') {
  ...
  function whitelistMint(uint256 _quantity, bytes32[] calldata _merkleProof) external payable {
    ...
    if (msg.sender != owner()) {
      require(balanceOf(msg.sender) + _quantity <= MAX_WL_TX, 'No more!');
      if (!whitelistClaimed[msg.sender]) {
        require(
          msg.value >= (_quantity - MAX_PER_FREE) * WL_PRICE,
          'Please send the exact amount.'
        );
        whitelistClaimed[_msgSender()] = true;
      } else {
        require(msg.value >= _quantity * WL_PRICE, 'Please send the exact amount.');
      }
    }
