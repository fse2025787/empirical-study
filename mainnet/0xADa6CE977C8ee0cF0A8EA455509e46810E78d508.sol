    ...
    IStaminaInfo
{
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _tokenId
    ) public virtual override(ERC721AUpgradeable, IERC721AUpgradeable) onlyAllowedOperator(_from) {
    ...
    function safeTransferFrom(
        ...
        uint256 tokenId,
        bytes memory data
    ) public virtual override(ERC721AUpgradeable, IERC721AUpgradeable) onlyAllowedOperator(from) {
    ...
    function craft(
        ...
        if (myRarity == 1) {
            ...
            require(burnRelicTokenIds.length == 0, "tl3");
        } else {
        ...
        for (uint256 i = 0; i < burnRelicTokenIds.length; i++) {
            ...
            require(
                getRelicRarity(bTokenId) == myRarity,
                "burning relic rarity mismatch"
            );
            _burn(bTokenId);
        }
    ...
    function usePotions(
        ...
        for (uint256 i = 0; i < kubzTokenIds.length; i++) {
            require(kubzContract.ownerOf(kubzTokenIds[i]) == msg.sender, "not owner of kubzTokenIds");
