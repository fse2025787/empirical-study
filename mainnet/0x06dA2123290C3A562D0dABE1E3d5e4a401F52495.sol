...
contract Exchange is Ownable, ReentrancyGuard {
    ...
    function makeOrder(address _tokenGet, uint256 _amountGet, address _tokenGive, uint256 _amountGive) public payable nonReentrant {
        ...
        if(_tokenGive == ETHER || _tokenGive == WETHER) {
            ...
            uint256 ethAmount = countEtherFromToken(_tokenGive, feeAmount);
            ...
            orders[orderCount] = _Order(orderCount, msg.sender, _tokenGet, _amountGet, _tokenGive, _amountGive, ethAmount, block.timestamp);
            
            emit Order(orderCount, msg.sender, _tokenGet, _amountGet, _tokenGive, _amountGive, ethAmount, block.timestamp);
        }
    }
    ...
    function fillOrder(uint256 _id) public payable nonReentrant {
        ...
        tokenBalances[_order.tokenGet][msg.sender] = tokenBalances[_order.tokenGet][msg.sender].sub(_order.amountGet);
