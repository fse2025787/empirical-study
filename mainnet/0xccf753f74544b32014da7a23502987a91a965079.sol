...
interface IBlues {
    ...
    function ownerOf(uint256 tokenId) external view returns (address owner);
    function getLegendaryHoldings(address addr) external view returns (uint256);
}
    ...
    ReentrancyGuardUpgradeable
{
        ...
        updateRewardsForUser
    {
        ...
        function addManyBluesToOceanV2(uint16[] calldata tokenIds)
        external
        nonReentrant
        whenNotPaused
        updateRewardsForUser
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                Blues_V2.ownerOf(tokenIds[i]) == msg.sender,
                "You are not the owner of this blue"
            );
            _adjustUserLegendryWhalesMultiplier(tokenIds[i], true);
            totalBluesStakedPerAddress[msg.sender]++;
            _addBlueToOceanV2(msg.sender, tokenIds[i]);
        }
        ...
        whenNotPaused
    {
     ...
     function _addBlueToOceanV2(address account, uint16 tokenId)
        internal
        whenNotPaused
    {
        ocean[tokenId] = account;
        Blues_V2.safeTransferFrom(msg.sender, address(this), tokenId);
        emit TokenStaked(msg.sender, tokenId);
    }
        ...
        updatePassiveRewardsForUser
    {
      ...
      function claimWhalesWithoutUnstakingV2()
        external
        nonReentrant
        whenNotPaused
        updateRewardsForUser
        updatePassiveRewardsForUserV2
    {
        uint256 rewards = totalWhalesEarnedPerAddress[msg.sender] +
            totalWhalesPassivelyEarned[msg.sender];
        ...
        updateRewardsForUser
    {
        ...
        function claimWhalesAndUnstakeV2(uint16[] calldata tokenIds)
        public
        nonReentrant
        whenNotPaused
        updateRewardsForUser
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                Blues_V2.ownerOf(tokenIds[i]) == address(this),
                "This Blue is not staked"
            );
            require(
                ocean[tokenIds[i]] == msg.sender,
            ...
            delete ocean[tokenIds[i]];
            totalBluesStakedPerAddress[msg.sender]--;
            Blues_V2.safeTransferFrom(address(this), msg.sender, tokenIds[i]);
        }
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            else require(Blues.ownerOf(tokenIds[i]) == msg.sender, "Non owner");
    ...
    modifier updateRewardsForUser() {
        if (userFirstInteracted[msg.sender]) {
            totalWhalesEarnedPerAddress[msg.sender] += (((block.timestamp -
                lastInteractionTimeStamp[msg.sender]) *
                totalBluesStakedPerAddress[msg.sender] *
                userMultiplier[msg.sender]) / 1 days);
     ...
     */
    modifier updatePassiveRewardsForUserV2() {
        uint256 holdings = Blues_V2.balanceOf(msg.sender);
        uint256 legendaryHoldings = Blues_V2.getLegendaryHoldings(msg.sender);

        if ( legendaryHoldings > 0) {
            if (!userTimestampInit[msg.sender]) {
                passiveYeildTimeStamp[msg.sender] = seedTimestamp;
                userTimestampInit[msg.sender] = true;
            }

            _adjustMultiplierForPassiveYeild(holdings , msg.sender);
            ...
            _adjustLegendaryMultiplierForPassiveYeildV2(legendaryHoldings, msg.sender);
   ...
   modifier updatePassiveRewardsForUser() {
     ...
     */
    function _adjustLegendaryMultiplierForPassiveYeildV2(
        uint256 legendaryHoldings,
        address account
    ) internal {
        legendariesUnstaked[account] = legendaryHoldings;
    }
    ...
    function setLegendaryRate(uint256 reward) external onlyOwner {
        DAILY_WHALES_RATE_LEGENDRY = reward;
    }
