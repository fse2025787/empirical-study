...
contract KANCHO is Context, IERC20, ERC20Ownable {
    ...
    uint private _liqDiv = _marTax + _NFTTax + _liqTax;
    uint private _buyMarTax = 5;
    uint private _preBuyMarTax = _buyMarTax;
    uint private _buyNFTTax = 2;
    uint private _preBuyNFTTax = _buyNFTTax;
    ...
    uint private _sellNFTTax = 2;
    uint private _preSellNFTTax = _sellNFTTax;
    ...
    string private constant _nam = "KANCHO";
    string private constant _symb = "KANCHO";
    ...
    constructor() {
        ...
        marketingAddress = payable(0xe48FE6E6359dDe978b0B68332603cdCaB1846fCF);
        NFTAddress = payable(0xF091D895f9B0072713B265D32AA89ceA4126920F);
        minTokensBeforeSwap = _tTotal.mul(5).div(10000);
        _isExcludedFromFee[_msgSender()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[address(marketingAddress)] = true;
        _isExcludedFromFee[address(NFTAddress)] = true;
        ...
        _isMaxWalletExclude[address(NFTAddress)] = true;
    ...
    function taxTokensForNFT() external view returns (uint256) {
        return tokensForNFTToSwap;
    }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        tokensForNFTToSwap += tLiquidity * _NFTTax / _liqDiv;
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marTax + _NFTTax + _liqTax).div(10**2);
    }
    function removeAllFee() private {
        if (_refTax == 0 && _liqTax == 0 && _marTax == 0 && _NFTTax == 0) return;

        _previousRefTax = _refTax;
        _previousLiqTax = _liqTax;
        _previousMarTax = _marTax;
        _previousNFTTax = _NFTTax;
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (_buyLimits == true && from == uniV2Pair) {
			require(amount <= 4000000000 * 10**18, "Limits are in place, please lower buying amount");
		}
        if(_initLaunch == true) {
	...
	function swapTokens() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = tokensForLiquidityToSwap + tokensForMarketingToSwap + tokensForNFTToSwap;
        ...
        uint256 ethForNFT = ethBalance.mul(tokensForNFTToSwap).div(totalTokensToSwap);
        uint256 ethForLiquidity = ethBalance.sub(ethForMarketing).sub(ethForNFT);
        tokensForLiquidityToSwap = 0;
        tokensForMarketingToSwap = 0;
        tokensForNFTToSwap = 0;
        (bool success,) = address(marketingAddress).call{value: ethForMarketing}("");
        (success,) = address(NFTAddress).call{value: ethForNFT}("");
    ...
    function setNFTAddress(address _NFTAddress) public onlyOwner {
        ...
        emit UpdatedNFTAddress(_NFTAddress);
    }
    ...
    function setBF(uint buyMarTax, uint buyNFTTax, uint buyLiqTax, uint buyRefTax) external onlyOwner {
        _buyMarTax = buyMarTax;
        _buyNFTTax = buyNFTTax;
        _buyLiqTax = buyLiqTax;
        _buyRefTax = buyRefTax;
    }
    function setSF(uint sellMarTax, uint sellNFTTax, uint sellLiqTax, uint sellRefTax) external onlyOwner {
        _sellMarTax = sellMarTax;
        _sellNFTTax = sellNFTTax;
        _sellLiqTax = sellLiqTax;
        _sellRefTax = sellRefTax;
    }
    function HappyHour() external onlyOwner {
		_preBuyMarTax = _buyMarTax;
		_preBuyNFTTax = _buyNFTTax;
		_preBuyLiqTax = _buyLiqTax;
		_preBuyRefTax = _buyRefTax;
		_preSellMarTax = _sellMarTax;
		_preSellNFTTax = _sellNFTTax;
