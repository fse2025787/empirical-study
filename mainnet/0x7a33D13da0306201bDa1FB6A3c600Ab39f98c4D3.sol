...
// File: contracts/WagmiStakingV3.sol
...
interface IWagmiTrait {
    ...
    function buyTraits(Transfer[] calldata transfers) external;
}
...
contract WAGMIStakingV3 is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint256 => uint256) public tokenRewardMultiplier;

    IWagmiTrait public traitsCollection;
    ...
    function setTraitCollection(address _traitCollection) external onlyOwner {
        traitsCollection = IWagmiTrait(_traitCollection);
    }
    ...
    function setTokenRewardMultiplier(RewardMultiplier[] calldata _mutlipliers) public onlyOwner { 
        for (uint256 i = 0; i < _mutlipliers.length; i++) {
            tokenRewardMultiplier[_mutlipliers[i].tokenId] = _mutlipliers[i].multiplier;

            if (stakerAddress[_mutlipliers[i].tokenId] != address(0)) {
                stakers[stakerAddress[_mutlipliers[i].tokenId]].rewardMultiplier += _mutlipliers[i].multiplier;
            }
        }
    }
    ...
    function _stake(uint _tokenId) internal {
        ...
        if (tokenRewardMultiplier[_tokenId] > 0) {
            stakers[msg.sender].rewardMultiplier += tokenRewardMultiplier[_tokenId];
        }
    ...
    function glitch(uint _tokenId, uint _vialId) external nonReentrant {
        // Wallet must own the token they are trying to stake
        require(
            nftCollection.ownerOf(_vialId) == msg.sender,
            "You don't own this vial!"
        );

        // Burn The vial
        nftCollection.transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), _vialId);
        emit TroopGlitched(msg.sender, _tokenId, _vialId);
    }
    ...
    function buyTrait(uint256 traitId, uint256 price, uint256 amount) external payable {
        ...
        stakers[msg.sender].timeOfLastUpdate = block.timestamp;
        stakers[msg.sender].unclaimedRewards = rewards - totalPayment;
        IWagmiTrait.Transfer[] memory transfers;
        transfers[0] = IWagmiTrait.Transfer(msg.sender, traitId, amount);
        traitsCollection.buyTraits(transfers);
        emit BuyTrait(msg.sender, traitId, amount);
    }

    function deposit() external payable {
        uint256 amount = msg.value;
        require(amount > 0, "You need put some tokens");
        ...
        stakers[msg.sender].unclaimedRewards += amount;
    ...
    function balance() public view returns (uint256) {
        uint256 balanceV1 = nftCollection.balanceOf(address(stakingV1));
        uint256 balanceV2 = nftCollection.balanceOf(address(stakingV2));
        uint256 balanceV3 = nftCollection.balanceOf(address(this));
        return balanceV1 + balanceV2 + balanceV3;
    }
    ...
    returns (uint256 _rewards)
    {
        ...
        (stakers[_staker].amountStaked + stakers[_staker].rewardMultiplier))
        ) * rewardsPerHour) / 3600);
    }
