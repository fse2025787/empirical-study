...
contract CigarCoin is Context, IERC20, Ownable {
    ...
    constructor (uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address _CharityAddress)  {
        _NAME = "CigarCoin";
        _SYMBOL = "CIGAR";
        _DECIMALS = 9;
        _DECIMALFACTOR = 10 ** _DECIMALS;
        _tTotal = 3000000000 * _DECIMALFACTOR;
        _TAX_FEE = _txFee;
        _BURN_FEE = _burnFee;
    ...
    function setAsFeeAccount(address account) external onlyOwner() {
        FeeAddress = account;
    }
    ...
    function _burn(address account, uint256 _value) internal {
        require(_value <= _tOwned[account]);
        _tOwned[account] = _tOwned[account].sub(_value);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        bool takeFee = true;
        if (CharityAddress == sender || CharityAddress == recipient || FeeAddress == sender || FeeAddress == recipient || _isExcluded[recipient]) {
            takeFee = false;
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _standardTransferContent(sender, recipient, tAmount, tTransferAmount);
        if(tCharity > 0) {
        ...
        _reflectFee(tFee, tBurn, tCharity);
        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _standardTransferContent(address sender, address recipient, uint256 tAmount, uint256 tTransferAmount) private {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tCharity) = _getValues(tAmount);
        _bothTransferContent(sender, recipient, tAmount, tTransferAmount);
        if(tCharity > 0) {
            _sendToCharity(tCharity, sender);
        }
        if(tFee > 0) {
    ...
    function _bothTransferContent(address sender, address recipient, uint256 tAmount, uint256 tTransferAmount) private {
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
    }

    function _reflectFee(uint256 tFee, uint256 tBurn, uint256 tCharity) private {
    ...
    function _sendToDev(uint256 tFee, address sender) private {
