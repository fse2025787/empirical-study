...
| $$      | $$  \ $$| $$    $$  /$$/   | $$        | $$   /$$/\  $$
| $$$$$$$$| $$  | $$|  $$$$$$/ /$$/    | $$$$$$$$ /$$$$$$| $$  \ $$
|________/|__/  |__/ \______/ |__/     |________/|______/|__/  |__/


- Putting ERC712PSI and ERC721A together;
*/
...
error MintExceedsWalletAllowance();
error MintExceedsMaxSupply();
error MintExceedsDevSupply();
error MintQuantityLargerThanMaxPerTX();
...
contract ERC721XUpgradeable is
    Initializable,
    ContextUpgradeable,
    ERC165Upgradeable,
    IERC721Upgradeable,
    ...
    IERC721EnumerableUpgradeable
{
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId);
        if (!_checkOnERC721Received(from, to, tokenId, 1, _data)) revert TransferToNonERC721ReceiverImplementer();
    }
        ...
        if (to.isContract()) {
            ...
            for (uint256 tokenId = startTokenId; tokenId < startTokenId + quantity; tokenId++) {
                try IERC721ReceiverUpgradeable(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (
                    ...
                    r = r && retval == IERC721ReceiverUpgradeable.onERC721Received.selector;
     ...
     */
    function tokenByIndex(uint256 index) public view virtual override returns (uint256 count) {
    ...
    function _beforeTokenTransfers(
        ...
        address to,
        uint256 startTokenId,
        uint256 quantity /* solhint-disable-next-line no-empty-blocks */
    ) internal virtual {}
...
// v3 errors
error DeprecatedMethod();

contract MonolithV4 is
    Initializable,
    OwnableUpgradeable,
    ERC721XUpgradeable,
    UUPSUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    uint32 public whitelistMints;

    uint32 public publicMints;
    ...
    function refundAndMintDutchAuction(address _to) external nonReentrant onlyOwner {
        DAStatus memory status = dutchAuctionStatus;
        ...
        refund(_to, refundAmount);
        status.refunds = status.refunds + bidder.daQuantity;
        dutchAuctionStatus = status;
        // mint
        _safeMint(_to, uint256(bidder.daQuantity));
    }
    ...
    function publicMint(uint256 quantity) external payable nonReentrant {
        ...
        if (publicMints + whitelistMints + devMintSupply + quantity > MAX_SUPPLY) {
            revert MaxSupplyExceeded();
        }
    ...
    // v3
    function whitelistMintV2(uint256 quantity) external payable nonReentrant {
        ...
        if (whitelistMints + quantity > whitelistSupply) {
            revert WhitelistAllowanceExceeded();
        }
        if (msg.value < uint256(status.whitelistPrice) * quantity) {
        ...
        whitelistMints = uint32(whitelistMints + quantity);
    }
}
