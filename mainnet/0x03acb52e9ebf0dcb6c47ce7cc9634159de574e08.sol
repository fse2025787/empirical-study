...
interface IERC721 is IERC165 {
    ...
    event Onsale(uint256 tokenId, bool isSale);
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    mapping(uint256 => mapping (address => bool)) public isOnSale;
...
contract CalypsoMarket is ERC721 {
    ...
    modifier onlySigner() {
        require(signer == msg.sender, "Ownable: caller is not the owner");
        _;
    }
    ...
    function  changeSigner(address payable _signer) public onlySigner{
        require(_signer != address(0),"ERC721: receiver address is zero address");
        emit OwnershipTransferred(signer, _signer);
        signer = _signer;
    }
    ...
    function createCollectible(string memory tokenURI, uint256 fee, bool isSale, Sign memory sign) public payable returns (uint256) {
    ...
    function putOnSale(uint256 _tokenId) public payable {
        require(ownerOf(_tokenId) == msg.sender,"ERC721: caller is not Owner");
        require(_exists(_tokenId),"ERC721: token doesn't exists");
        require(!isOnSale[_tokenId][msg.sender],"ERC721: token already in sale");
        require(msg.value == listingFee,"Listing: value must be equal to ListingFee");
        payable(owner).transfer(msg.value);
        isOnSale[_tokenId][msg.sender] = true;
        emit Onsale(_tokenId, true);
    }
