...
4% Transaction and Wallet limit
*/
...
contract WomenKidsInu is ERC20, Ownable, SafeToken, LockToken {
    ...
    function setFee(uint256 _liquidityFee, uint256 _charityFee) public onlyOwner {
        liquidityFee = _liquidityFee;
        charityFee = _charityFee;

        totalFees = liquidityFee.add(charityFee); // total fee transfer and buy
    }
    ...
    constructor() ERC20("WomenKids Inu", "WIK") {
        
        ETHRewardsFee = 0;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra sell fee
        charityFee = 7;
        charityWallet = payable(0xD3A66bbb9bcB068455699969Be4189bfBD22f5F5);
        totalFees = ETHRewardsFee.add(liquidityFee).add(charityFee); // total fee transfer and buy
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(charityFee);
        charityWallet.transfer(marketingAmount);
