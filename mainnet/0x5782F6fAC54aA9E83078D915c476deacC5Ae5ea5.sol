    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _feelsLucky(
        ...
        unchecked {
            allocations[currLevel] = qty;
            for (uint32 i = currLevel + 1; i < 5; i++) {
                allocations[currLevel] -= allocations[i];
            }
                ...
                for (uint8 i = 0; i < 5; i++) {
                    ...
                    if (k == 0) {
                        ...
                        if (rarityLevel == currLevel) {
                            pools[rarityLevel] = supply;
                        } else if (supply > __reservations[rarityLevel]) {
                            pools[rarityLevel] = supply - __reservations[rarityLevel];
                        }
                    }

                    /* Allocated Too Many, move extra parts to the next level */
                    if (allocations[rarityLevel] > pools[rarityLevel]) {
                        uint8 nextLevel = rarityLevel == 0 ? 4 : (rarityLevel - 1);
                        allocations[nextLevel] += allocations[rarityLevel] - pools[rarityLevel];
                        allocations[rarityLevel] = pools[rarityLevel];
                    }
                }

            // release current level reservations if sword already upgraded
            if (__reservations[currLevel] > 0) {
    ...
    function _tokenAlloc(uint32[5] memory allocations, uint256 tokenId) internal {
        uint8 rand1 = uint8(__randContract.getRand() % 10);
        uint8 rand2 = uint8((block.timestamp + rand1) % 3);
        ...
        unchecked {
            for (uint32 rarityLevel = 0; rarityLevel < 5; rarityLevel++) {
                for (uint32 i = 0; i < allocations[rarityLevel]; i++) {
                    ...
                    if (__upgraded.get(_tokenId)) {
                        if (rarityLevel > 0) {
                            att_restoreRate[_tokenId] = __defaultRestoreRates[rarityLevel] + rand2;
                        }
                        att_strength[_tokenId] = __defaultStrengths[rarityLevel] + rand1;
                    }
    ...
    function mint(uint8 qty) public payable nonReentrant {
    ...
    function ritualOfSummon(
        ...
        _safeMint(addr, totalQty);

        if (isGen0)
            for (uint32 i = 0; i < totalQty; i++) {
                __upgraded.set(_tokenId + i);
            }
    ...
    // Game Hooks
    function setName(uint256[] calldata tokenIds, string[] calldata names) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(ownerOf(tokenIds[i]) == _msgSender(), 'token not owned');
            att_names[tokenIds[i]] = bytes24(abi.encodePacked(names[i]));
        }
    }
    ...
    function upgrade(
        ...
        uint16[] calldata strengths,
        uint8[] calldata restoreRates
    ) public onlyWakumbas {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            __upgraded.set(tokenIds[i]);
            att_strength[tokenIds[i]] = strengths[i];
            att_restoreRate[tokenIds[i]] = restoreRates[i];
        }
    }
}
