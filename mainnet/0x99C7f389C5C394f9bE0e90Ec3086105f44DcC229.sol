...
interface ITimeLock {
    function lock() external returns (uint256 lockId);
...
contract LinkBridgeTimeLock is ITimeLock, Ownable {
    ...
    uint256 fixLockAmount = 500 * (10 ** tokenDecimals);

    mapping(address => bool) adminWallets;

    modifier onlyAdmin() {
        require(adminWallets[_msgSender()], "Admin: caller is not the admin");
        _;
    }
    ...
    function lock() external override returns (uint256 id) {
        address token = lockingToken;
        require(token != address(0), "Invalid token");
        uint256 amount = fixLockAmount;
    ...
    function _lockToken(
        ...
        overallLockedAmount = overallLockedAmount + amount;
    ...
    function _unlock(Lock storage userLock) internal {
        ...
        if (overallLockedAmount <= unlockAmount) {
            overallLockedAmount = 0;
        } else {
            overallLockedAmount = overallLockedAmount - unlockAmount;
        }
    ...
    function updateAdminWallets(address[] memory accounts, bool state) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            adminWallets[accounts[i]] = state;
        }
    }
    ...
    function getLockCountForUser(address user)
        public
        view
        returns (uint256)
    {
    ...
    function getLocksForUser(address user)
        external
        view
        returns (Lock[] memory)
    {
    ...
    function getlockForUserAtIndex(address user, uint256 index)
        external
        view
        returns (Lock memory)
    {
        require(getLockCountForUser(user) > index, "Invalid index");
        return getLockById(_userLockIds[user].at(index));
    }

    function getTotalLockCountForUser(address user)
        external
        view
        returns (uint256)
    {
        return getLockCountForUser(user);
    }
    ...
    function getTotalLockCount()
        ...
        returns (uint256)
    {
        return _tokenToLockIds[lockingToken].length();
    }

    function getTotalLockValue() external view returns(uint256) {
        return overallLockedAmount;
    }

    function getLocks(uint256 start, uint256 end) public view onlyAdmin returns (Lock[] memory) {
        address token = lockingToken;
    ...
    function isAdminWallet(address user) public view returns(bool) {
        return adminWallets[user];
    }
    ...
    function setFixLockAmount(uint256 _fixLockAmount) external onlyOwner {
        fixLockAmount = _fixLockAmount;
    }
