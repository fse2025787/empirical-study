...
contract Nftdns is ERC721, Ownable, ReentrancyGuard {
  ...
  address payable multisig = 0x4e2f98c96e2d595a83AFa35888C4af58Ac343E44;
  ...
  function buyName(string memory s) public payable nonReentrant {
    require(! isNameTaken[s], "Name taken");

    // feeToken or eth payment logic
    if (feeOnly || msg.value == 0) {
      require(feeToken.balanceOf(msg.sender) >= feeAmount, "Need tokens");
      feeToken.safeTransferFrom(msg.sender, address(this), feeAmount);
    }
    else if (!feeOnly) {
      require(msg.value >= ethAmount, "Pay up");
  ...
  function setFeeToken(address a) public onlyOwner {
    feeToken = IERC20(a);
  }

  function setFeeOnly(bool b) public onlyOwner {
    feeOnly = b;
  }

  function setFeeAmount(uint256 a) public onlyOwner {
    feeAmount = a;
  }

  function setEthAmount(uint256 a) public onlyOwner {
    ethAmount = a;
  }
