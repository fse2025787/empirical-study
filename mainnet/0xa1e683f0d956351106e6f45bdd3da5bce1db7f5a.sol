...
contract WePiggyPriceProviderV1 is Ownable {
    ...
    mapping(address => address) public chainLinkTokenBtcPriceFeed;

    address public ethUsdPriceFeedAddress;
    address public btcUsdPriceFeedAddress;
    ...
    function _getUnderlyingPriceInternal(address _pToken, TokenConfig memory tokenConfig, PriceOracle memory priceOracle) internal view returns (uint){
        ...
        } else if (sourceType == PriceOracleType.ChainLinkBtcBase) {
            return _getChainLinkBtcBasePriceInternal(priceOracle, tokenConfig);
        }
    ...
    // base chainlink: token-BTC-USD
    function _getChainLinkBtcBasePriceInternal(PriceOracle memory priceOracle, TokenConfig memory tokenConfig) internal view returns (uint){
        require(tokenConfig.baseUnit > 0, "baseUnit must be greater than zero");

        address token = tokenConfig.underlying;
        AggregatorV3Interface tokenBtcPriceFeed = AggregatorV3Interface(chainLinkTokenBtcPriceFeed[token]);
        (,int tokenBtcPrice,,,) = tokenBtcPriceFeed.latestRoundData();

        AggregatorV3Interface btcUsdPriceFeed = AggregatorV3Interface(btcUsdPriceFeedAddress);
        (,int btcUsdPrice,,,) = btcUsdPriceFeed.latestRoundData();

        if (tokenBtcPrice <= 0) {
            return 0;
        } else {// tokenEthPrice/1e8 * ethUsdPrice/1e8 * 1e36 / baseUnit
            return uint(tokenBtcPrice).mul(uint(btcUsdPrice)).mul(1e20).div(tokenConfig.baseUnit);
        }
    }
    ...
    function setBtcUsdPriceFeedAddress(address feedAddress) public onlyOwner {
        btcUsdPriceFeedAddress = feedAddress;
    }
    ...
    function addOrUpdateChainLinkTokenBtcPriceFeed(address[] memory tokens, address[] memory chainLinkTokenBtcPriceFeeds) public onlyOwner {

        require(tokens.length == chainLinkTokenBtcPriceFeeds.length, "tokens.length must equal than chainLinkTokenBtcPriceFeeds.length");

        for (uint i = 0; i < tokens.length; i++) {
            address token = tokens[i];
            chainLinkTokenBtcPriceFeed[token] = chainLinkTokenBtcPriceFeeds[i];
        }
