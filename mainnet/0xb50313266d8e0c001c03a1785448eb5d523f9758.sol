...
/**
 * @title PunkLoot contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract PunkLoot is ERC721, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    string public PROVENANCE = "";
    uint256 public lootersPrice = 50000000000000000; //0.05 ETH
    ...
    constructor() ERC721("Punk Loot", "PKL") {
    }
    ...
    function emergencyMint(uint[] memory lootIds) public onlyOwner {
        require(!saleIsActive, "Sale must be not active");
        for (uint i=0; i<lootIds.length; i++) {
            require(lootIds[i] > 0 && lootIds[i] < 8001, "Token ID invalid");
            require(!_exists(lootIds[i]), "One of these tokens have already been minted");
            _safeMint(msg.sender, lootIds[i]);
