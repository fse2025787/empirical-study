...
/*
2% InfernSHIB True burn 
2% Pyro burn  through the Burnt IAPE Contract
2% LP 
1% Marketing 
contract by t.me/misamayhem
*/
...
contract InfernTsuka is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = "IT";
    string private constant _symbol = "ITSUKA";
    ...
    uint256 public j_pyroBuy = 2;
    uint256 public j_jeetBuy = 1;

    uint256 public j_liqSell = 2; 
    uint256 public j_burnSell = 2;
    uint256 public j_pyroSell = 2;
    uint256 public j_jeetSell = 14;
    ...
    address payable private infernoWallet = payable(0xFf6848032bEA412Ace4Da546540E302b8c12C2Cc);
    address payable private jeetWallet = payable(0xBc40196582fb11eCF5a10B8Ff89693FF68DA79c6);

    address payable public dead = payable(0x000000000000000000000000000000000000dEaD);
    address payable public BurntLobster = payable(0x8901ceAC9DD796a98DAa32e2fc55dC68fEcDA01A); 
    // Burning Pyro through the Burnt Lobster Contract - the Pyro Utility
    ...
    constructor() {
        ...
        _isExcludedFromFee[infernoWallet] = true;
    ...
    function removeJeetTax(uint256 jeetTax) external {
        require(_msgSender() == infernoWallet);
        j_jeetSell = jeetTax;
        require(jeetTax <= 14);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this) && to != address(this)) {
            ...
            if(tradingActiveBlock + 1 >= block.number && (to == uniswapV2Pair || from == uniswapV2Pair)){
                j_liqFee = 5;
                j_burnFee = 0;
                j_pyroFee = 0;
                j_jeetTax = 75;
    ...
    function sendETHToFee(uint256 amount) private {
        if (!burnMode) {
            infernoWallet.transfer((amount).div(2));
            jeetWallet.transfer((amount).div(2));
        } else if (burnMode) {
            infernoWallet.transfer((amount).div(6));
            jeetWallet.transfer((amount).div(6));
            BurntLobster.transfer((amount).div(6).mul(4));
        }   
    ...
    function setBurntLobsterWallet(address payable _address) external onlyOwner {
        BurntLobster = _address;
    }
