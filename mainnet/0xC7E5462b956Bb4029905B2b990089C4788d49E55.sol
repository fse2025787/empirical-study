...
contract KarensGoneWild is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress;

    uint256 private KARENS_MAX = 5555;
    uint256 private PRESALE_PRICE = 0.08 ether;
    uint256 private PUBLIC_PRICE = 0.08 ether;
    uint256 private REVEAL_DELAY = 0 hours;
    uint256 private PRESALE_HOURS = 0 hours;
    uint256 private PURCHASE_LIMIT = KARENS_MAX;
    uint256 private PRESALE_MINT_LIMIT = KARENS_MAX;
    uint256 private PUBLIC_MINT_LIMIT = KARENS_MAX;
    ...
    Counters.Counter private _publicKARENS;

    constructor() ERC721("Karens Gone Wild", "$KARENS") {}
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicKARENS.current();

                if (_publicKARENS.current() < KARENS_MAX) {
                    _publicKARENS.increment();
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(_publicKARENS.current() < KARENS_MAX,"Purchase would exceed KARENS_MAX");
