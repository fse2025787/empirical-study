...
interface FactoryERC721 {
    ...
    // factory option token transfer, must emit before sale start, usually emit in constructor
    event Transfer(address indexed from,address indexed to,uint256 indexed tokenId);

    // emit on any ERC721 NFT minted
    event FactoryMint(address indexed origin,address indexed to,uint256 indexed optionId,uint256 tokenIdStart,uint256 tokenNum);
     ...
     * callable only by the contract owner or the owner's Element Proxy (later universal login will solve this).
...
abstract contract ERC721Tradable is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    mapping(address => bool) public factoryAddresss;

    uint256 private _currentTokenId = 0;
    uint256 public MAX_TOTAL = 0;
    ...
    function mintTo(address _to) public {
        require(_msgSender() == owner() || factoryAddresss[_msgSender()], "not owner or factory");
        uint256 newTokenId = _getNextTokenId();
        require(newTokenId<=MAX_TOTAL, "over maximum number");
    ...
    /**
     * Override isApprovedForAll to whitelist user's Element proxy accounts to enable gas-less listings.
     */
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        // Whitelist Element proxy contract for easy trading.
    ...
    /**
     * This is used instead of msg.sender as transactions won't be sent by the original token owner, but by Element.
     */
    function _msgSender()
        internal
        override
     ...
     * add factory addrss
     */
    function addFactoryAddress(address factoryAddr) public onlyOwner {
        factoryAddresss[factoryAddr] = true;
    }

    /**
     * remove factory addrss
     */
    function removeFactoryAddress(address factoryAddr) public onlyOwner  {
        delete factoryAddresss[factoryAddr];
    }
