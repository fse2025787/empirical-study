...
contract VoterUpgradeable is Initializable, AccessControlUpgradeable, ERC20Upgradeable, UUPSUpgradeable {
    ...
    modifier isVotingEnded(uint256 _issue) {
        require(
            block.number >= issueMap[_issue].endBlock ||
                (issueMap[_issue].status != ProposalStatus.Executed ||
                    issueMap[_issue].status != ProposalStatus.Cancelled),
            "ERC-1202: Voting has not ended"
        );
        _;
    }
