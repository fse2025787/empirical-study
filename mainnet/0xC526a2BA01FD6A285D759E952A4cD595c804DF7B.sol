...
/*
    ...
    LP LOCK + CONTRACT RENOUNCE
    https://t.me/SonOfKaren
    http://sonofkaren.xyz/
*/
...
contract SonOfKaren is Context, IERC20, Ownable {
    ...
    string private _name = "Son Of $Karen";
    string private _symbol = "SonOfKaren";
    ...
    uint256 public _firstTwoHourSellTax = 20; 

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    
    uint256 public _maxTxAmount = _tTotal.mul(1).div(100);
    uint256 public _maxWallet = _tTotal.mul(3).div(100);
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()){
            require(amount <= _maxTxAmount || excludeMaxTxn[from], "TX Limit Exceeded");
            
            if (to != address(this)  && to != address(DEAD) && to != uniswapV2Pair && to != liquidityWallet){
                uint256 currentBalance = balanceOf(to);
                require(excludeMaxTxn[to] || (currentBalance + amount <= _maxWallet));
            }
        }
        ...
        if(launchAt + 2 hours >= block.timestamp)
            _liquidityFee =  _firstTwoHourSellTax;
