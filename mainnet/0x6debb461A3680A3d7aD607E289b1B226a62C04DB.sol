 ...
 */
abstract contract ContextUtils {
 ...
 * {ERC721Enumerable}.
 */
contract ERC721 is ContextUtils, ERC165, IERC721, IERC721Metadata {
    ...
    function safeTransferFromBatch(
        ...
        _safeTransferBatch(from, to, tokenId, _data);
    }
    ...
    function _safeTransferBatch(
        ...
        for (uint256 i = 0; i < tokenId.length; i++) {
            require(
                _isApprovedOrOwner(_msgSender(), tokenId[i]),
                "ERC721: transfer caller is not owner nor approved"
            );
            _transfer(from, to, tokenId[i]);
        }
        require(
            _checkOnERC721ReceivedBatch(from, to, tokenId, _data),
            "ERC721: transfer to non ERC721Receiver implementer"
        );
    }
    ...
    function _checkOnERC721ReceivedBatch(
        address from,
        address to,
        uint256[] memory tokenId,
        bytes[] memory _data
        ...
        if (to.isContract()) {
            try
                IERC721Receiver(to).onERC721ReceivedBatch(
                    _msgSender(),
                    from,
                    tokenId,
                    _data
            ...
            returns (bytes4 retval) {
                return
                    retval ==
                    IERC721Receiver(to).onERC721ReceivedBatch.selector;
            } catch (bytes memory reason) {
                if (reason.length == 0) {
                    revert(
                        "ERC721: transfer to non ERC721Receiver implementer"
 ...
 */
abstract contract Pausable is ContextUtils {
    /**
 ...
 */
abstract contract AccessControl is ContextUtils, IAccessControl, ERC165 {
    struct RoleData {
