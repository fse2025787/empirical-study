...
// File: contracts/SpaceKidsClub.sol
    ...
    PaymentSplitter
{
    ...
    uint256 public whitelistSupply = preSaleSupply + 1000;
    ...
    uint256 public maxMintPresale = 5;
    ...
    uint256 public salePrice = 0.16 ether;
    ...
    address[] private _team = [0xc4b5909C3F4C6dA2fbeb9c73E9CA273f029Ba887];
     ...
     * @param _maxMintAllowed The number of NFTs that an address can mint
     **/
    function changeMaxMintWhitelist(uint256 _maxMintAllowed) external onlyOwner {
    ...
    function setupPresale() external onlyOwner {
        require(
            ...
            "The collection is not in the right status to set up presale"
        );
        ...
        nonReentrant
    {
        ...
        require(status == Status.Presale, "Sorry, sale has not started yet.");
        //Has user paid enough Ethers
        require(msg.value >= presalePrice * _amount, "Insufficent funds.");
        //The user can only mint max 5 NFTs
        require(
            _amount <= maxMintPresale,
            "You can't mint more tokens"
        );
        require(nftsPerWalletPresale[msg.sender] <= maxMintPresale, "You cannot mint more tokens.");
        //If the user try to mint any non-existent token
        require(
            supply + _amount <= preSaleSupply,
            "Sale is almost done and we don't have enough NFTs left."
        );
        //Add the ammount of NFTs minted by the user to the total he minted
        nftsPerWalletPresale[msg.sender] += _amount;
        ...
        nonReentrant
    {
        ...
        uint256 supply = totalSupply();
        //If Sale didn't start yet
        require(status == Status.WhitelistSale, "Sorry, sale has not started yet.");
        //Has user paid enough Ethers
        require(msg.value >= whitelistPrice * _amount, "Insufficent funds.");
        //The user can only mint max 5 NFTs
        require(
            _amount <= maxMintWhitelist,
            "You can't mint more tokens"
        );
        //If the user try to mint any non-existent token
        require(
        ...
        //Add the ammount of NFTs minted by the user to the total he minted
        nftsPerWalletWhitelist[msg.sender] += _amount;
        //Minting all the account NFTs
        for (uint256 i = 0; i < _amount; i++) {
            _safeMint(msg.sender, nftIdCounter.current());
    ...
    function gift(address _account) external onlyOwner {
        _safeMint(_account, nftIdCounter.current());
        nftIdCounter.increment();
    }
