...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
    ...
    constructor() {
        _setupRole(0x00, 0xF05eA7d9d9AAA317742fA3a637E842D2A5a95756);
    }
...
contract NFKeyz is ERC721, ReentrancyGuard, AccessControl {
    using SafeMath for uint256;
    using SafeMath for uint16;
    using SafeMath for uint8;
    uint16 private _tokenId;

    uint16 public mintLimit = 1000;
    uint16 public presaleLimit = 100;
    ...
    bool public revealStatus = true;
    bool private rarityMinted = false;
    bytes32 private merkleRoot;
    bytes32 public constant ADMIN = keccak256("ADMIN");

    uint public presalePrice = 0.3 ether;  // 0.1
    uint public mintPrice = 0.5 ether;         // 0.12

    string private realBaseURI = "https://nfkeyz.mypinata.cloud/ipfs/QmYajR8qEFqZwY9hqHvVY3QbFZmetK83y1W9UvR8qW5Gj8/";
    string private virtualURI = "https://placeholder.com/";

    uint8 public MAX_PER_WALLET = 2;

    mapping(address => uint8) addressPresaleCountMap;     // Up to 1
    ...
    constructor() ERC721("CroGram NFKeyz", "KEYZ") {
    ...
    function setPresaleStart() external onlyAdmin {
        mintStep = 1;
    }
    ...
    function mintPresale(uint8 _mintCount/*, bytes32[] memory _proof*/) external payable nonReentrant returns (uint256) {
        ...
        require(mintStep == 1 && _mintCount > 0, "Not presale step");
        require(_tokenId + _mintCount <= presaleLimit, "Presale ended");
        require(msg.sender != address(0));
        require(msg.value == (presalePrice * _mintCount));
        require(addressPresaleCountMap[msg.sender] + _mintCount <= MAX_PER_WALLET);
    ...
    function mintPublic(uint8 _mintCount) external payable nonReentrant returns (uint256) {
        ...
        require(_tokenId + _mintCount <= mintLimit);
        require(_mintCount + addressPresaleCountMap[msg.sender] + addressPublicSaleCountMap[msg.sender] <= MAX_PER_WALLET);
    ...
    function withdraw(address _account) external onlyAdmin {
    ...
    function getAccountStatus(address account) external view returns (uint8[] memory) {
        ...
        status[0] = addressPresaleCountMap[account];
    ...
    function changePresaleMintLimit(uint16 _mintLimit) public onlyAdmin {
        presaleLimit = _mintLimit;
    }

    function rarityMint(address account) public onlyAdmin {
        require(rarityMinted == false, "Super Rarity NFTs Already Minted");
        for (uint8 i = 1; i <= 7; i++) {
            _safeMint(account, mintLimit + i);
        }
        rarityMinted = true;
        emit RarityMint(msg.sender, account);
    }
}