...
contract MAM is ERC20, Ownable {
    ...
    string constant _name = "Sergar Momma";
    ...
    address public SERReceiver = 0xc1ecD500D321b576a6A0d296EA8d5ceB28bA1b42;
    address public marketingReceiver = 0xb7ed0727E114d64035e4f3C869414C1589BFc98a;
    ...
    constructor () Ownable(msg.sender) {
        ...
        buyFee.liquidityFee = 0; buyFee.SERFee = 20; buyFee.marketingFee = 10;
        buyFee.total = buyFee.liquidityFee + buyFee.SERFee + buyFee.marketingFee;

        // Set Initial Sell Fees -- Base 1000 Set 10 for 1%
        sellFee.liquidityFee = 20; sellFee.SERFee = 20; sellFee.marketingFee = 10;
        sellFee.total = sellFee.liquidityFee + sellFee.SERFee + sellFee.marketingFee;
    ...
    function ownerSetTargetLiquidity(uint256 target) external onlyOwner {
       ...
       // Use 10 to set 1% -- Base 1000 for easier fine adjust
    function ownerUpdateBuyFees (uint256 _liquidityFee, uint256 _SERFee, uint256 _marketingFee) external onlyOwner {
        buyFee.liquidityFee = _liquidityFee;
        buyFee.SERFee = _SERFee;
        buyFee.marketingFee = _marketingFee;
        buyFee.total = buyFee.liquidityFee + buyFee.SERFee + buyFee.marketingFee;
    }
        // Use 10 to set 1% -- Base 1000 for easier fine adjust
    function ownerUpdateSellFees (uint256 _liquidityFee, uint256 _SERFee, uint256 _marketingFee) external onlyOwner {
        sellFee.liquidityFee = _liquidityFee;
        sellFee.SERFee = _SERFee;
    ...
    function clearStuckToken(address _token) public onlyOwner {
        ...
        payable(SERReceiver).transfer(_contractBalance);
    }
    ///////////////////////////////////////// -- Getter Functions -- /////////////////////////////////////////
    ...
    function _distributeBNB(uint256 remainingBNB) private {
        ...
        uint256 SERFee = (buyFee.SERFee + sellFee.SERFee);
        uint256 totalFee = (marketingFee + SERFee);

        uint256 amountBNBmarketing = remainingBNB * (marketingFee) / (totalFee);
        uint256 amountBNBSER = remainingBNB * (SERFee) / (totalFee);

        if(amountBNBSER > 0){
        (bool SERSuccess, /* bytes memory data */) = payable(SERReceiver).call{value: amountBNBSER, gas: 30000}("");
