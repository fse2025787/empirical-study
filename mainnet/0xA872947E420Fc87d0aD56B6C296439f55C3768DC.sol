...
contract DropspaceSale is IDropspaceSale, ERC721A, Ownable {
    ...
    function airdrop(
        ...
        require(_receivers.length == _numTokens.length, "Airdrop lengths must match");
        for (uint256 i = 0; i < _receivers.length; i++) {
            _safeMint(_receivers[i], _numTokens[i]);
        }
    }

    function viewOwner(address _collection, uint256 _tokenId) public view returns(address) {
        return IERC721(_collection).ownerOf(_tokenId);
    }
