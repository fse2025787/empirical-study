...
// Full fork from:
// Angle Protocol's ANGLE Distributor
// https://github.com/AngleProtocol/angle-core/blob/main/contracts/staking/AngleDistributor.sol
...
/// - AngleDistributor.sol (https://github.com/AngleProtocol/angle-core/blob/main/contracts/staking/AngleDistributor.sol)

contract DfxDistributor is DfxDistributorEvents, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
    ...
    function _distributeReward(address gaugeAddr) internal returns (uint256 weeksElapsed, uint256 rewardTally) {
        ...
        // that need to be bridged) or is associated to an external contract of the DFX Protocol
        if (gaugeType >= 2) {
            ...
            if (delegate != address(0)) {
                ...
                if (isInterfaceKnown[delegate]) {
                    IDfxMiddlemanGauge(delegate).notifyReward(gaugeAddr, rewardTally);
                }
            } else {
    ...
    /// has been called for all gauges in the past weeks. If not, gauges may get an incorrect distribution of DFX rewards
