...
pragma solidity 0.8.17;
    ...
    contract DeadFi is ERC20, Ownable{
    ...
    address public DeadFiLPRecipient;
    address public DeadFiBuyBackWallet;
    address public DeadFiDevWallet;
    ...
    event TaxRecipientsUpdated(address newDeadFiLPRecipient, address newDeadFiBuyBackWallet, address newDeadFiDevWallet);
    event FeesUpdated();
    ...
    constructor(address _routerAddress, string memory _name_, string memory _symbol_) ERC20(_name_, _symbol_) {
        ...
        swapEnabled = true;
        swapThreshold = 800_000 * 10**18;
        maxWalletAmount = 16_000_001 * 10**18;

        exemptFee[msg.sender] = true;
        exemptFee[address(this)] = true;
        exemptFee[DeadFiBuyBackWallet] = true;
        exemptFee[DeadFiDevWallet] = true;
        exemptFee[DeadFiLPRecipient] = true;

        _mint(msg.sender, 800_000_000 * 10**18);
    }

    function setTaxRecipients(address _DeadFiLPRecipient, address _DeadFiBuyBackWallet, address _DeadFiDevWallet) external onlyOwner{
        require(_DeadFiLPRecipient != address(0), "DeadFiLPRecipient cannot be the zero address");
        require(_DeadFiBuyBackWallet != address(0), "DeadFiBuyBackWallet cannot be the zero address");
        require(_DeadFiDevWallet != address(0), "DeadFiDevWallet cannot be the zero address");
        DeadFiLPRecipient = _DeadFiLPRecipient;
        DeadFiBuyBackWallet = _DeadFiBuyBackWallet;
        DeadFiDevWallet = _DeadFiDevWallet;

        exemptFee[DeadFiDevWallet] = true;
        exemptFee[DeadFiBuyBackWallet] = true;
        exemptFee[DeadFiLPRecipient] = true;

        emit TaxRecipientsUpdated(_DeadFiLPRecipient, _DeadFiBuyBackWallet, _DeadFiDevWallet);
    }
    ...
    function setBuyFees(uint256 _DeadFiLP, uint256 _DeadFiBuyBack, uint256 _DeadFiDev) external onlyOwner{
        require(_DeadFiLP + _DeadFiBuyBack + _DeadFiDev < 100, "Buy fee must be less than 100");
        buyFees = Fees(_DeadFiLP, _DeadFiBuyBack, _DeadFiDev);
        totalBuyFee = _DeadFiLP + _DeadFiBuyBack + _DeadFiDev;
        emit FeesUpdated();
    }

    function setSellFees(uint256 _DeadFiLP, uint256 _DeadFiBuyBack, uint256 _DeadFiDev) external onlyOwner{
        require(_DeadFiLP + _DeadFiBuyBack + _DeadFiDev < 100, "Sell fee must be less than 100");
        sellFees = Fees(_DeadFiLP, _DeadFiBuyBack, _DeadFiDev);
        totalSellFee = _DeadFiLP + _DeadFiBuyBack + _DeadFiDev;
        emit FeesUpdated();
    }
    ...
    function isrekt(address account) public view returns(bool) {
        return _isrekt[account];
    }

    function multirektAccount(address[] calldata accounts) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
        _isrekt[accounts[i]] = true;
        }
    }

    function unrektAccount(address account) public onlyOwner {
        _isrekt[account] = false;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(!_isrekt[to], "This address is currently rekt");
        require(!_isrekt[from], "This address is currently rekt");
    ...
    function handle_fees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 tokensToAddLiquidityWith = contractBalance * sellFees.DeadFiLP / denominator;
            ...
            uint256 unitBalance= deltaBalance / (denominator - sellFees.DeadFiLP);
            uint256 ethToAddLiquidityWith = unitBalance * sellFees.DeadFiLP;
            ...
            uint256 DeadFiBuyBackAmt = unitBalance * 2 * sellFees.DeadFiBuyBack;
            if(DeadFiBuyBackAmt > 0){
                payable(DeadFiBuyBackWallet).sendValue(DeadFiBuyBackAmt);
            }

            uint256 DeadFiDevAmt = unitBalance * 2 * sellFees.DeadFiDev;
            if(DeadFiDevAmt > 0){
                payable(DeadFiDevWallet).sendValue(DeadFiDevAmt);
            }
        }
    }
