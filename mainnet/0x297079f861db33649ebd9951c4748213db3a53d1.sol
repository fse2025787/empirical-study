...
contract MB is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
	uint256 public GIVEAWAY_NFT = 50;
	uint256 public PRESALE_NFT = 888;
	uint256 public SALE_NFT = 7950;
	uint256 public MAX_NFT = GIVEAWAY_NFT + PRESALE_NFT + SALE_NFT;
	...
	uint256 public SALE_PRICE = 0.2 ether;
	
    bool public presaleEnable = false;
	bool public saleEnable = false;
	bool public merkleRootStatus = false;
	
	address public partner = 0xA09De897d63fC93405DC796e3c35e0c19EF08AA1;
    ...
    constructor() ERC721P('Majestic Bears', 'MB') {}
	...
	function withdraw() public onlyOwner {
		...
		uint256 partnerValue = (balance*50)/10000;
		uint256 remaning = balance - partnerValue;
		
		payable(partner).transfer(partnerValue);
        payable(msg.sender).transfer(remaning);
    }
	...
	function updateSupply(uint256 giveawaySupply, uint256 preSaleSupply, uint256 saleSupply) external onlyOwner {
	    require(giveawaySupply >= GIVEAWAY_MINTED, "Incorrect value");
		require(preSaleSupply >= PRESALE_MINTED, "Incorrect value");
		require(saleSupply >= SALE_MINTED, "Incorrect value");
		
        GIVEAWAY_NFT = giveawaySupply;
	...
	function setMerkleRootStatus(bool status) public onlyOwner {
        require(merkleRootStatus != status);
