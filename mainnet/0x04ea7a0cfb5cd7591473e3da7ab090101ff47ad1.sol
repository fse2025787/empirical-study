...
// Root file: contracts/TimelockERC20.sol
...
/**
 * @title Timelock Contracts on Ethereum ERC20 tokens.
 *
 * This contract provides a way to create and keep Timelock Contracts for ERC20 tokens.
 *
 *
 * Protocol:
 *
 *  1) newContract(timelock, tokenContract, amount) - a
 *      sender calls this to create a new TLC on a given token (tokenContract)
 *       for a given amount. A 32 byte contract id is returned
 *  2) withdraw() - after timelock has expired the sender / creator of the TLC can get their tokens
 *      back with this function.
 */
contract TimelockERC20 is Ownable {
    event TLCERC20New(
    ...
    event TLCERC20Withdraw(bytes32 indexed contractId);
    ...
    modifier withdrawable(bytes32 _contractId) {
        ...
        require(
            ...
            "withdrawable: timelock not yet passed"
        );
        _;
    }
    ...
    /**
     * @dev Sender / Payer sets up a new time lock contract depositing the
     * funds
     *
     * @param _timelock UNIX epoch seconds time that the lock expires at.
     *                  Withdrawals can be made after this time.
     * @param _tokenContract ERC20 Token contract address.
     * @param _amount Amount of the token to lock up.
     * @return contractId Id of the new TLC. This is needed for subsequent
     *                    calls.
     */
     ...
     *
     * @param _contractId Id of TLC to withdraw from.
        ...
        returns (bool)
    {
        ...
        emit TLCERC20Withdraw(_contractId);
        return true;
    }
        ...
        )
    {
        ...
        LockContract memory c = contracts[_contractId];
        return (c.sender, c.tokenContract, c.amount, c.timelock, c.withdrawn);
    }
