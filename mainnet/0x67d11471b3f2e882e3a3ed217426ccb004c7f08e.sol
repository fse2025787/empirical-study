...
contract BonesRewards is ERC20, ERC20Burnable, Ownable {
    ...
    function initialClaim(ERC721AQueryable _nft) external { 
        Vault memory vaultInfo = vault[address(_nft)];
        uint[] memory ownerTokens = vaultInfo.nft.tokensOfOwner(_msgSender());
        require(ownerTokens.length > 0, "You do now own any tokens");
        uint amount;
        for (uint i = 0; i < ownerTokens.length; i++) {
            uint tokenId = ownerTokens[i];
            for (uint j = 0; j < vaultInfo.specialTokens.length; j++) {
                if (tokenId == vaultInfo.specialTokens[j] && !claimedTokens[address(_nft)][tokenId]) {
                    amount += vaultInfo.specialAllocation;
                    claimedTokens[address(_nft)][tokenId] = true;
                } else if (!claimedTokens[address(_nft)][tokenId]) {
