...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function validTokens() public view returns (uint256[] memory){
        return _allTokens;
    }
}
// 
...
contract ChubbyKaijuDAOInvasion_new is ERC721Enumerable, Ownable{
    ...
    function mutateWithSerum(uint256 typeId, uint256[] memory zombieIds, uint256 amount) external {
        require(isMutate, "Cannot mutate Yet");
        require(typeId==1 || typeId==3, "typeId Should be Serum");
        require(zombieIds.length==amount, "The number of zombies should be same as the amount");
        require(chubbykaijudaocorp.balanceOf(msg.sender, typeId)>=amount,"Must Own the Amount of typeId");
        for(uint i=0; i<zombieIds.length;i++){
            require(_exists(zombieIds[i]),"Not exist zombie");
            require(ownerOf(zombieIds[i])==msg.sender,"Not Owner");
            require(zombieIds[i]<6667, "Only Zombie can Mutate");
            if(typeId == 1) { // SERUM
    ...
    function callWithRadio(uint256 typeId, uint256 amount) external {
        require(isMutate, "Cannot call Yet");
        require(typeId==0 || typeId==2, "typeId Should be Radio");
        require(chubbykaijudaocorp.balanceOf(msg.sender, typeId)>=amount,"Must Own the Amount of typeId");
        for(uint i=0;i<amount;i++){
            if(typeId == 0){ // RADIO
                chubbykaijudaocorp.burnCorpForAddress(typeId, msg.sender);
