 ...
 */
contract ETHPhase2Pool is IETHStakingRewards, Context, AccessControl, RewardDistributionRecipient, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  function totalSupply() public override(IETHStakingRewards) view returns (uint256) {
    return _totalSupply;
  }

  function balanceOf(address account) public override(IETHStakingRewards) view returns (uint256) {
    return _balances[account];
  }

  function lastTimeRewardApplicable() public override(IETHStakingRewards) view returns (uint256) {
    return Math.min(block.timestamp, periodFinish);
  }

  function rewardPerToken() public override(IETHStakingRewards) view returns (uint256) {
    if (totalSupply() == 0) {
  ...
  function earned(address account) public override(IETHStakingRewards) view returns (uint256) {
    return
      balanceOf(account)
  ...
  function getRewardForDuration() external override(IETHStakingRewards) view returns (uint256) {
  ...
  function withdraw(uint256 amount) public override(IETHStakingRewards) updateReward(msg.sender) {
  ...
  function getReward() public virtual override(IETHStakingRewards) updateReward(msg.sender) {
