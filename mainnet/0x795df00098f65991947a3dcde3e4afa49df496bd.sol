...
contract Airdrop is Ownable {
    ...
    IERC20 public token = IERC20(0x8C77057C3343B7DCC97CA21dA274730396162a98);

    function setToke(address _token) public onlyOwner {
        token = IERC20(_token);
    }
        ...
        onlyOwner
    {
        ...
        require(
            token.balanceOf(address(this)) >= SCCC,
            "Not enough tokens in this address"
        );
    ...
    function multiTransfer4EachAirDrop(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            token.transfer(addresses[i], tokens[i]);
        }
    }
