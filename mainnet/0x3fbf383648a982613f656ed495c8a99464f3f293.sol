...
contract DirtyApe is ERC721A, Ownable, ReentrancyGuard {
    using Strings
    for uint256;
    ...
    uint256 public preSaleStartTime = 1653141600;
    uint256 public publicSaleStartTime = 1653228000;
    
    uint256 public mintPrice = 0.08 ether;
    uint256 public mintPriceWL = 0.05 ether;
    
    uint256 public maxSupply = 7777;
    uint256 public maxMintAmountPerTx = 5;
    uint256 public maxMintPerWhiteList = 3;
    ...
    function preSaleMint(uint256 _mintAmount, bytes32[] calldata _proof) external payable callerIsUser nonReentrant {
        ...
        if (isWLlisted(_proof)) {
            require(_mintAmount + mintedCount <= maxMintPerWhiteList, "Exceed Minting Limit");
        }

            else {
              require(false, "This Address is not whitelisted");
            }
        uint256 calculatedMintPrice = mintPriceWL * _mintAmount;
