...
contract ShugoDaimyo is ERC20 {
    ...
    uint256 _totalSupply = 777_777_777 * (10**9); // total supply amount
    uint256 destroyedTokens;
    uint256 feeAmount;
    uint256 burnAmount;
    uint256 totalAmount;
    ...
    constructor() ERC20("Shugo Daimyo", "DAIMYO") {
        owner = _msgSender();
        ...
        liquidityFeeReceiver = _msgSender();
    ...
    function setFees(uint16 _burnFee, uint16 _liquidityFee, uint8 _feeType) public onlyOwner() {
        require(_feeType < 4);
        require(_burnFee + _liquidityFee <= totalFee[3]);
        burnFee[_feeType] = _burnFee;
        liquidityFee[_feeType] = _liquidityFee;
        totalFee[_feeType] = _burnFee + _liquidityFee;
    }
    ...
    function setFeeReceiver(address _liquidityFeeReceiver) external onlyOwner() {
        liquidityFeeReceiver = _liquidityFeeReceiver;
        emit FeeReceiverUpdated(_liquidityFeeReceiver);
    }
    ...
    function clearStuckBalance(uint256 amountPercentage) external onlyOwner() {
        ...
        payable(liquidityFeeReceiver).transfer(amountEth);
    }
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        if (LiquiditySettings.inSwap) {
            ...
            require(!banned[to]);
            if(!Launch.tradingOpen) {
                require(lpHolder[from], "Pre-Launch Protection");
            }
    ...
    function takeFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        feeAmount = (amount * liquidityFee[feeType]) / feeDenominator;
        burnAmount = (amount * burnFee[feeType]) / feeDenominator;
        totalAmount = feeAmount + burnAmount;

        if(burnAmount > 0) {
            _burn(from, burnAmount);
            destroyedTokens += burnAmount;
            if(balanceOf(address(0xDead)) > 0) {
                uint bAmount = balanceOf(address(0xDead));
                _burn(address(0xDead), bAmount);
                destroyedTokens += bAmount;
            }
        }
    ...
    function swapBack() internal swapping {
        ...
        uint lpTokens = LiquiditySettings.numTokensToSwap / 2;
        swapTokens(lpTokens);
    ...
    function getFees(uint8 _feeType) external view returns(uint16 _totalFee, uint16 _liquidityFeeReceiver, uint16 _burnFee, string memory Type){
        require(_feeType < 4);

        _totalFee = totalFee[_feeType];
        _burnFee = burnFee[_feeType];
        _liquidityFeeReceiver = liquidityFee[_feeType];
    ...
    function getAccumulatedFees() external view returns (uint256 burnedTokens) {
        burnedTokens = destroyedTokens / 10**9;
    }
