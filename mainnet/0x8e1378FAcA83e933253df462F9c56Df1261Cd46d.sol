...
contract Animation is Owner {
  ...
  function getBodyArray(TraitOptions memory traitOptions)
    internal
    view
    returns (string[] memory)
  {
    string[] memory bodyArray;

    {
      bodyArray[LayerOrder.BG] = UtilAssets.getAssetBackground(
        traitOptions.background
      );
    }
    {
      bodyArray[LayerOrder.BELLY] = fetchAssetString(
        LayerOrder.BELLY,
        UtilAssets.getAssetBelly(traitOptions.species, traitOptions.belly)
      );
    }
    {
      bodyArray[LayerOrder.ARMS] = fetchAssetString(
        LayerOrder.ARMS,
        UtilAssets.getAssetArms(traitOptions.species)
      );
    }
    {
      bodyArray[LayerOrder.FEET] = fetchAssetString(
        LayerOrder.FEET,
        UtilAssets.getAssetFeet(traitOptions.species)
      );
    }
    {
      bodyArray[LayerOrder.FOOTWEAR] = fetchAssetString(
        LayerOrder.FOOTWEAR,
        UtilAssets.getAssetFootwear(traitOptions.footwear)
      );
    }
    {
      bodyArray[LayerOrder.CLOTHING] = fetchAssetString(
        LayerOrder.CLOTHING,
        UtilAssets.getAssetClothing(traitOptions.clothing)
      );
    }
    {
      bodyArray[LayerOrder.ACCESSORIES] = fetchAssetString(
        LayerOrder.ACCESSORIES,
        UtilAssets.getAssetAccessories(traitOptions.accessories)
      );
    }
    {
      bodyArray[LayerOrder.JEWELRY] = fetchAssetString(
        LayerOrder.JEWELRY,
  ...
  function getHeadLayers(TraitOptions memory traitOptions)
    internal
    view
    returns (bytes memory)
  {
    string[] memory headArray;

    {
      headArray[LayerOrder.HEAD] = fetchAssetString(
        LayerOrder.HEAD,
        UtilAssets.getAssetHead(traitOptions.species, traitOptions.locale)
      );
    }
    {
      headArray[LayerOrder.EYES] = fetchAssetString(
        LayerOrder.EYES,
        UtilAssets.getAssetEyes(traitOptions.eyes)
      );
    }
    {
      headArray[LayerOrder.MOUTH] = fetchAssetString(
        LayerOrder.MOUTH,
        UtilAssets.getAssetMouth(traitOptions.mouth)
      );
    }
    {
      headArray[LayerOrder.NOSE] = fetchAssetString(
        LayerOrder.NOSE,
        UtilAssets.getAssetNose(traitOptions.nose)
      );
    }
    {
      headArray[LayerOrder.HAT] = fetchAssetString(
        LayerOrder.HAT,
        UtilAssets.getAssetHat(traitOptions.hat)
      );
    }
    {
      headArray[LayerOrder.FACE] = fetchAssetString(
        LayerOrder.FACE,
        UtilAssets.getAssetFaceAccessory(traitOptions.faceAccessory)
      );
    }
    ...
    return abi.encodePacked(
      '[',
        headArray[LayerOrder.HEAD], ',',
        headArray[LayerOrder.EYES], ',',
        headArray[LayerOrder.MOUTH], ',',
        headArray[LayerOrder.NOSE], ',',
        headArray[LayerOrder.HAT], ',',
        headArray[LayerOrder.FACE],
      ']'
    );
  }
  ...
  function assetsForDNA(uint256 dna) external view returns (string memory) {
    ...
    string[] memory bodyArray = getBodyArray(traitOptions);
    // string[] memory headArray = getHeadArray(traitOptions);
        ...
        abi.encodePacked(
          ...
          Base64.encode(
            abi.encodePacked(
              "{",
                '"bodyLayers":[',
                    bodyArray[LayerOrder.BG], ',',
                    bodyArray[LayerOrder.BELLY], ',',
                    bodyArray[LayerOrder.ARMS], ',',
                    bodyArray[LayerOrder.FEET], ',',
                    bodyArray[LayerOrder.FOOTWEAR], ',',
                    bodyArray[LayerOrder.CLOTHING], ',',
                    bodyArray[LayerOrder.ACCESSORIES], ',',
                    bodyArray[LayerOrder.JEWELRY],
                '],',
                '"headLayers":', getHeadLayers(traitOptions),
              "}"
            )
          )
        )
