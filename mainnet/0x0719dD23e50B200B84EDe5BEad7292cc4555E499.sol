...
contract PondAcademyPermit is DefaultOperatorFiltererUpgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable {
    // Whitelist state
    // Merkle tree root hash
    bytes32 public rootHash;
    bool public isWhiteListOpen;
    mapping(address => uint256) public whiteListCapTracker;
    uint256 constant private WHITE_LIST_CAP = 3;
    uint256 public whiteListMintValue;

    // Open Mint state
    uint256 public openMintValue;
    bool public isMintOpen;
    // TODO: Check if it's per transaction or in total
    uint256 constant private OPEN_MINT_CAP = 5;
    ...
    function initialize(bytes32 rootHash_, address payable feeReceiver_) initializer public {
        __Ownable_init();
        __ERC721_init("Pond Academy Permit", "PAP");
        ...
        // Whitelist state
        whiteListMintValue = 20000000000000000;
        isWhiteListOpen = false;
        rootHash = rootHash_;

        // Open mint state
        isMintOpen = false;
        openMintValue = 30000000000000000;
    ...
    function mint(uint256 amount) external payable {
        ...
        require(amount > 0 && amount <= OPEN_MINT_CAP, "You can mint between 1 and 5 in one transaction.");

        _baseMint(amount, msg.sender, openMintValue);
    }

    function whiteListMint(uint256 amount, bytes32[] calldata proof) external payable {
        require(isWhiteListOpen, "Whitelist is not active.");
        require(amount > 0 && amount + whiteListCapTracker[msg.sender] <= WHITE_LIST_CAP,
        ...
        whiteListCapTracker[msg.sender] += amount;

        _baseMint(amount, msg.sender, whiteListMintValue);
    }
    ...
    function setOpenMintValue(uint256 newOpenMintValue) public onlyOwner {
        openMintValue = newOpenMintValue;
    }

    function setWhiteListMintValue(uint256 newWhiteListMintValue) public onlyOwner {
        whiteListMintValue = newWhiteListMintValue;
    }
    ...
    // Override for the DefaultOperatorFilterer
