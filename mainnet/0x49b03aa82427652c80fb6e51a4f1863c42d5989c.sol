...
contract LilDunksNFT is ERC721Enumerable, Pausable, Ownable {
    ...
    // Maximum tokens will be 2000 
    uint256 public  _totalSupply = 2000; 
    uint256 public  _price = 70000000000000000; // .07 ETH
    uint256 public _whitelistPrice = 59000000000000000; // 0.059 ETH 
    ...
    uint256 public _whitelistMaxMint = 5; 
    ...
    // Wallet Addresses 
    address public founder1Address = 0x045524425F6371e66324b7728Df5201410f1Eac7; 
    address public founder2Address = 0x71fc301BD42723b8Cc9Ef6af3041047Fbf405AC8; 
    address public devAddress = 0x2Ea23ad853A68c0bD10cA15d5e896273585Ac3E0; 
    address public communityAddress = 0x2DeE5ef42eBa5683332b574381e6DA3a5b820F1c; 
    ...
    event UpdateWhiteListMaxMint(uint256 newMax); 
    event UpdateMaxMint(uint256 newMax); 
    event Withdraw(uint256 amount); 

    constructor() ERC721("Lil Dunks", "LD") {
    }
    ...
    function whiteListMint(uint256 amount) public payable {
        ...
        require(_whitelistMinted[sender] + amount <= _whitelistMaxMint);
        require(msg.value >= amount * _whitelistPrice);
        for (uint256 i = 0; i < amount; i++) {
            _mintItem(sender);
        }
        _whitelistMinted[sender] = _whitelistMinted[sender] + amount; 
    ...
    function mint(uint256 amount) public payable {
        ...
        require(totalMinted < _totalSupply);
        require(totalMinted + amount <= _totalSupply); 
        require(_minted[sender] + amount <= _maxMint); 
        require(msg.value >= amount * _price);
        for (uint256 i = 0; i < amount; i++) {
            _mintItem(msg.sender); 
        }
        _minted[sender] = _minted[sender] + amount; 
    ...
    // State management 
    function updateWhiteListMaxMint(uint256 newMax) public onlyOwner {
        require(newMax > 0); 
        _whitelistMaxMint = newMax; 
        emit UpdateWhiteListMaxMint(newMax); 
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(founder1Address).transfer(balance*20/100); 
        payable(founder2Address).transfer(balance*20/100); 
        payable(devAddress).transfer(balance*20/100); 
        // withdraw remaining to community address
        payable(communityAddress).transfer(address(this).balance);
