...
// File: contracts/MorumelonOG.sol
...
contract MorumelonOG is ERC721A, MultisigOwnable {
  using Address for address;
  ...
  function airdrop(
    ...
    if (receivers.length != numTokens.length) {
      revert InvalidChunk();
    }
    for (uint256 i; i < receivers.length; ++i) {
      _mintWrapper(receivers[i], numTokens[i]);
    }
    processedChunksForAirdrop[chunkNum] = true;
  }

  // Used to claim unclaimed tokens after airdrop/claim phase
  function devClaim(uint256 numTokens) external onlyRealOwner {
    _mintWrapper(msg.sender, numTokens);
  }
