...
contract SubClaim {
    ...
    mapping(address => uint16) public register_count;

    uint16 public count = 5;
    ...
    function register_subdomain() external returns (bool, uint16) {
        ...
        try ens.setSubnodeRecord(rootNode, to_label(count), msg.sender, resolver, 0) { 
    ...
    function admin_claim(uint16 _count, address _to) public onlyOwner returns (bool) {

        try ens.setSubnodeRecord(rootNode, to_label(_count), _to, resolver, 0) {
    ...
    function to_label(uint16 index) internal pure returns (bytes32) {

        string memory label = "";

        if(index < 10) {

            label = string(abi.encodePacked("0", Strings.toString(index)));

        } else {

            label = Strings.toString(index);
