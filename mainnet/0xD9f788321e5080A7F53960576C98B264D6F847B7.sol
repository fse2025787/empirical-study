...
////////////////////////////////////////////////////////////////////
  ...
  function dispose_rewarding_list(uint start, uint ends) public onlyOwner{
    ...
    function numberTokensLocked(address _user) public view returns (uint256){
        return user_db[_user].Total_locked_Nfts;
    }
   ...
   function Rewards_Calculator_per_entry_checker(address user,uint tokenid, uint ot) public view returns(uint) {
        ...
        uint result = (operand1/operand2)*operand3;
         result=result / 1000;
           ...
           calculation+=(totallockingdays-looptorun)*perdaytoken;
            }
        }
        else{
...
//this function is to calculate the rewards per locking record
   ...
   function nft_entry(address user, uint perdaytoken, uint tokenid,
    ...
    function close_nft_entry(address user, uint tokenid) internal
        ...
        for(uint i=0;i<totalentries;i++)
        {   
            if(user_db[user].nft_entries[i].tokenid==tokenid && user_db[user].nft_entries[i].total==0 && user_db[user].nft_entries[i].endtime==0)
            {
  ...
  function check_Locked_ownership(address user, uint[] memory list)public view returns (bool){
    ...
    function Rentrooms(uint256[] memory tokenIds, uint[] memory tms, uint256 id, bytes memory signature) external nonReentrant {
    ...
    function Releaserooms(uint256[] memory tokenIds) external nonReentrant{
        require(!paused,"Contract is paused");
    ...
    function togglePause(bool value) public onlyOwner{
        paused = value;
    }
...
function verifySignature(bytes memory signature, bytes memory message, address expectedAddress) public pure returns (bool) {
    ...
    //Emergancy Exit Code
function withdraw_runes() public onlyOwner{
          rewardToken.transfer(address(msg.sender), rewardToken.balanceOf(address(this)));
    }

function exit_safe(uint256[] memory tokenIds) public onlyOwner {
            uint256 len = tokenIds.length;
            for (uint256 i = 0; i < len; i++) {
               uint256 tokenId = tokenIds[i];
               require(check_ownership(address(this),tokenIds),"Room not available in the contract");
               totalroomsLocked--;
                address own=tokenIdOwners[tokenId];
                  nft.transferFrom(
                        address(this),
                        own,
                        tokenId
