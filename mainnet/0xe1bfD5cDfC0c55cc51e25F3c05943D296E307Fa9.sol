...
contract PLATO is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 34000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tTotalDistributedToken;

    uint256 private maxBuyLimit = 340000000 * (10**18);
    uint256 public maxWallet = _tTotal.div(10000).mul(100);

    string private _name = "PLATO";
    string private _symbol = "PLATO";
    ...
    uint256 private numTokensSellToAddToLiquidity = 4000000 * 10**18;
    ...
    function setPromotionWallet(address account) external onlyOwner() {
    ...
    function manualsend() external onlyOwner() {
        ...
        if(ethMarketing > 0)
            payable(promotionWallet).transfer(ethMarketing);
    }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        if(ethMarketing > 0){
            payable(promotionWallet).transfer(ethmarketers);
