...
contract HappyZorbsRendererV2 is Initializable, OwnableUpgradeable {
    ...
    string[] private zorbTypes;
    uint256 public zorbCount;
    string[] private backgroundColors;
    string[] private frameColors;

    mapping(uint256 => Structs.HappyZorb) internal happyZorbsOptions;
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        string(
                            abi.encodePacked(
                                '{"name": "Happy Zorbs", "description": "',
                                description,
                                '", "image": "ipfs://bafkreieymsk2kjok56nx4j3begorctdf74cdyamwxj4tkrbvqwp4bw7kdy", "seller_fee_basis_points": "0", "seller_fee_recipient": "0xCB43078C32423F5348Cab5885911C3B5faE217F9", "animation_url": "ipfs://bafybeiddpmdlu66ikh2fjfvbhq56euzh75ke5uyrmm3zoymbvmtu43cb2i"}'
                            )
                        )
                    )
                )
    ...
    function getTokenIdSvg(
        ...
        string memory backgroundColor = backgroundColors[0];
        string memory frameColor = frameColors[0];
        if (happyZorbsOptions[tokenId].colorMode >= 1) {
          backgroundColor = backgroundColors[happyZorbsOptions[tokenId].colorMode];
          frameColor = frameColors[happyZorbsOptions[tokenId].colorMode];
        }
            
        return
            string(
                abi.encodePacked(
                    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="2000" height="2000" ><path fill="',
                    backgroundColor,
                    '" d="M100 0H0V100H100V0Z"/><path fill="',
                    frameColor,
                    '" d="M77.3615 16.3496H22.6504V83.6461H77.3615V16.3496Z"/>',
                    zorbs,
                    '</svg>'
                )
            );
    ...
    function buildZorbs(uint256 tokenId) internal view returns (string memory) {
        ...
        // Show full grid even if not enough zorbs
    ...
    function setTokenColorMode(uint256 tokenId, uint256 colorMode) public {
      address ownerAddress = getOwnerOf(tokenId);
      require(msg.sender == ownerAddress, "Rejected: not token owner");

      happyZorbsOptions[tokenId] = Structs.HappyZorb({
        zorbCount: zorbCount,
        zorbType: 0,
        colorMode: colorMode
      });
