...
// File contracts/oblox/Oblox.sol
...
contract Oblox is ERC721Enumerable, Ownable {
   ...
   */
  uint16 public reserve = 50;

  /**
   * @dev Max supply of nfts.
   */
  uint16 public constant MAXSUPPLY = 2999;

  /**
   * @dev Max NFTs that you can mint per transaction.
   */
  uint8 public maxQuantityPerTransaction = 10;
   ...
   * @param _newBaseURI URI base for tokenURI function. Token URI is constructed as _baseTokenURI + tokenId.
   */
  constructor(
  ...
  /**
   * @dev Sets new max quantity per transaction.
   * @param _quantity New quantity.
   */
  function setMaxQtPerTx(
    uint8 _quantity
  )
    external
    onlyOwner
  {
    maxQuantityPerTransaction = _quantity;
  }
   ...
   * @param _chosenAmount Amount of tokens to be bougth.
   */
  function mint(
    ...
    payable
  {
    ...
    require(!isPaused, "Sale is not active at the moment");
    require(_chosenAmount > 0, "Number of tokens can not be less than or equal to 0");
    require(_chosenAmount <= maxQuantityPerTransaction,"Chosen Amount exceeds maxQuantityPerTransaction");
    require(totalSupply() + _chosenAmount + reserve <= MAXSUPPLY, "Purchase would exceed max supply");
    require(price * _chosenAmount == msg.value, "Sent value is incorrect");
