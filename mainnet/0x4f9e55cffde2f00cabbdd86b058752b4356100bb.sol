...
contract Piggee{
    ...
    function depositERC20(uint256 _amount, address _erc20, uint256 _timestamp) external {
        require(_timestamp > unlockTimesERC20[msg.sender][_erc20], "The lock timestamp should be greater than the current one");

        ERC20(_erc20).transferFrom(msg.sender, address(this), _amount);
        balancesERC20[msg.sender][_erc20] += _amount;
        unlockTimesERC20[msg.sender][_erc20] = _timestamp;
    }

    function withdrawERC20(uint256 _amount, address _erc20) external {
        require(balancesERC20[msg.sender][_erc20] >= _amount, "Insufficent funds");
        require(unlockTimesERC20[msg.sender][_erc20] < block.timestamp, "The lock period is not over yet");
