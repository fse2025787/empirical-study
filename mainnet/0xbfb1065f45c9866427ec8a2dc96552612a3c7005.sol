...
contract Oracle{
	...
	uint genesis;
	constructor() public{
		pyramid = Pyramid(0x91683899ed812C1AC49590779cb72DA6BF7971fE);
	...
	function assertOption(uint config, uint option) public{
		if( totalVotes_forEach_configOption[config][option] > totalVotes_forEach_configOption[config][ oracleConfigurations[config] ] ){
			//if(config == RESPONSE_TIME_WINDOW){ option = option>1800?option:(1800); }###
	...
	function fileRequestTicket( uint8 returnType, bool subjective) public payable returns(uint ticketID){
		uint ETH = msg.value;
	...
	function revealVote(uint[] memory tickets, bool[] memory rejected, int[] memory intVotes, bytes[] memory bytesVotes, address[] memory addressVotes, string[] memory passwords) public{
		address sender = msg.sender;
	...
	function finalizeRequests(uint[] memory tickets) public{
		for(uint R; R<tickets.length; R+=1 ){
...
abstract contract Requestor{
	function oracleIntFallback(uint ticketID, bool rejected, uint numberOfOptions, uint[] memory optionWeights, int[] memory intOptions) public virtual;
	function oracleBytesFallback(uint ticketID, bool rejected, uint numberOfOptions, uint[] memory optionWeights, bytes[] memory bytesOptions) public virtual;
	function oracleAddressFallback(uint ticketID, bool rejected, uint numberOfOptions, uint[] memory optionWeights, address[] memory addressOptions) public virtual;
}