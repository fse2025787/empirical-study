...
contract TrippinApeYachtTribe is ERC721A, Ownable, ReentrancyGuard {
   using Strings for uint256;
    ...
    ) ERC721A("TrippinApeYachtTribe", "TAYT") {
    ...
    function freeMint(uint256 _mintAmount) public mintCompliance(_mintAmount) {
        require(!paused && isFreeMintOpen, "TAYT: Free mint phase not open yet!");

        require(totalSupply() + _mintAmount <= maxFreeSupply, "TAYT: Exceeds max free mint supply!");
        require(_mintedFreeAmount[msg.sender] + _mintAmount <= maxFreePerWallet, "TAYT: Exceeds max free mint per wallet!");
    ...
    function publicMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
        require(_numberMinted(msg.sender) + _mintAmount <= 100, "TAYT: Exceeds max mint per wallet!");
