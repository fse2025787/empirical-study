...
function getUserBoughtCount( address _add) public view returns(uint256) {
  return user[_add];
}
function isWhitelisted(address account, bytes32[] calldata proof, uint256 quantity) public view returns (bool) {
        return _verify(_leaf(account, quantity), proof, whiteListRoot[0]);
    }
 ...
 */
  function mint(address to_, uint256 countNFTs_) isValid() public returns(uint256, uint256) {
    require(MaxSupply >= (_currentIndex - _burnCounter) + countNFTs_, "exceeding supply");
    (uint256 start, uint256 end) = _safeMint(to_, countNFTs_);
     
    emit Mint(start, end, countNFTs_, to_);
    return (start, end);
  }
  ...
  function setSeed(uint256 _seed) onlyOwner public {
   ...
   function buyDeemoNFT(bytes32[] calldata proof, uint256 limit, bool isLimit)  payable external {
    uint256 quantity = isLimit ? getUserBoughtCount(msg.sender) + limit : 0;
    // userInfo storage tempUser = user[msg.sender][_roundId];
    require(isWhitelisted(msg.sender, proof, quantity), "not authorize");
    require(status == 1, "sales not started");  
    
    require(MaxSupply >=   (_currentIndex - _burnCounter) + limit, "soldout");
    require(supplyPerRarity[1] + mintedPerRarity[2] >=   (_currentIndex - _burnCounter) + limit, "limit reach");
    uint256 rand;
    ...
    require(price <= depositAmount, "NFT 108");
       ...
       for (uint256 index = from; index <= to; index++) {
        ...
        if(supplyPerRarity[1] <= mintedPerRarity[1] + 1){
          ...
          rand = uint256(keccak256(abi.encode(seed + limit + _currentIndex + _burnCounter,seed + index, block.timestamp, msg.sender, owner()))) % 100;
        ...
        user[msg.sender] =user[msg.sender]+ limit ;
  ...
  function craftDeemo(uint256[] memory tokenIds) public {
    ...
    (uint256 from, uint256 to) = _safeMint(msg.sender, 1);
    tokenRarity[from] = 2;
     mintedPerRarity[3]++;
    emit Mint(from, to, 1, msg.sender);
  }
  ...
  function TransferFromAdmin(uint256 tokenId, address to) isValid() public {
    _transferAdmin(ERC721A.ownerOf(tokenId), to, tokenId);
  ...
  function setWhitelistRoot(bytes32 _root) onlyOwner() public {
    whiteListRoot[0] = _root;
  }
