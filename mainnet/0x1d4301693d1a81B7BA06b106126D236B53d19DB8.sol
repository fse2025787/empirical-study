...
/*
 *  Little Capitalist Asshole
 *  $ASSHOL IS A TOKEN FOR LITTLE CAPITALIST ASSHOLES LIKE YOU!!!
 *  https://twitter.com/Capitalist_Ass
 *  https://t.me/littlecapitalistasshole
 */
...
contract Asshol is IERC20, Ownable {
    ...
    string private _name = "Asshol";
    string private _symbol = "ASSHOL";
    ...
    function airdrop(address[] calldata newholders, uint256[] calldata amounts) external onlyOwner(){
        uint256 iterator = 0;
    ...
    function addBotWallets(address[] calldata _addresses) external onlyOwner() {
        for (uint256 i = 0; i < _addresses.length; i++) {
            botWallets[_addresses[i]] = true;
        }
    }
    
    function removeBotWallet(address _address) external onlyOwner() {
        botWallets[_address] = false;
    }
