 ...
 */
  constructor(string memory name_, string memory symbol_, address owner_) ERC721(name_, symbol_)  public {
      _setOwner(owner_);
  }
 ...
 */
  function mint(address to_, string memory tokenURI_) onlyOwner public returns (uint256) {
      tokenIds.increment();
...
contract CollectionFactory is Ownable {
  ...
  modifier isAllowed() {
    require(_allowedAddress[msg.sender],"not authorized");
    _;
  }
/***
@notice this function is to create collections on user's behalf.
 @param name_ name of collection of NFT
 ...
 */
  function create(string memory name_, string memory symbol_, address owner_) isAllowed public returns(address){
 ...
 */
  ...
  function addAddress(address _address) onlyOwner public {
    _allowedAddress[_address] = true;
  }
  function removeAddress(address _address) onlyOwner public {
    _allowedAddress[_address] = false;
  }
  // events
  event CollectionCreated(address indexed creater, address indexed collection, string name, string symbol);
}