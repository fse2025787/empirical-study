...
contract FortunaDraw {
    uint256 public constant ticketPrice = 0.01 ether;
    uint256 public constant maxTickets = 300; // maximum tickets per round
    uint256 public constant ticketCommission = 0.001 ether; // commition per ticket
    uint256 public constant duration = 30 minutes; // The duration set for the round

    uint256 public expiration; // Timeout in case That the round was not carried out.
    address public roundOperator; // the crator of the round
    uint256 public operatorTotalCommission = 0; // the total commission balance
    address public lastWinner; // the last winner of the round
    uint256 public lastWinnerAmount; // the last winner amount of the round
    ...
    // modifier to check if caller is the round operator
    modifier isOperator() {
        require(
            (msg.sender == roundOperator),
            "Caller is not the round operator"
        );
        _;
    }
    ...
    function RefundAll() public {
        require(block.timestamp >= expiration, "the round has not expired yet");
