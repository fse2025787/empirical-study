...
contract Claimant is Membership {
    ...
    function initialize(uint256 strtd, uint256 trm, uint256 tms) public onlyAdmin {
        started = strtd;
        term = trm;
        times = tms;
    }
    ...
    function claimable() public view returns (uint256){
        ...
        if(sequence>times)
            sequence = times;
        uint256 unlocked = sequence * (totalAmount() / times);
    ...
    function totalAmount() public view returns (uint256) {
        return token.balanceOf(address(this))+claimed;
    }
