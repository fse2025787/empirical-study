...
contract MoreNFTFactoryETH is AccessControlUpgradeable {
    ...
    uint64 public globalTransactionFee;

    uint256[41] private __slackForFutureUsage;

    function version() external pure returns (uint8, uint8, uint8) {
        return (0,9,3);
    }

    function initialize(uint256 _rebid, uint256 _rebidTime, address _royaltiesManager, uint64 _transactionFee, address _priceFeed) external initializer {
        ...
        globalTransactionFee = _transactionFee;
        ethToUSDPriceFeed = AggregatorV3Interface(_priceFeed);
    }
    ...
    function newCollectionSale(
        ...
        if (toMint) {
            ...
            revert("Transfer sales not yet implemented");
        }
    ...
    function bid(bytes32 saleId, uint256 tokenId) external payable {
        ...
        uint256 _realBid = _extractBid(msg.value);
        require(_realBid >= price(saleId, tokenId), "Too low bid");
        
        if (sale.bidders[tokenId] != address(0)) {
            address toRefund = sale.bidders[tokenId];
            uint256 toRefundAmount = sale.bids[tokenId] + transactionFee(sale.bids[tokenId]);
            sale.bidders[tokenId] = _msgSender();
            sale.bids[tokenId] = _realBid;
        ...
        emit Bid(sale.collection, tokenId, _msgSender(), _realBid);
    }
    ...
    function buy(bytes32 saleId, uint256 tokenId) external payable {
        ...
        uint256 overallAmount = currentPrice + transactionFee(currentPrice);
        require(msg.value >= overallAmount, "Price is higher than ETH sent");
        sale.finalized[tokenId] = true;
        if (msg.value - overallAmount > 0) {
            payable(_msgSender()).transfer(msg.value - overallAmount);
        }
    ...
    function transactionFee(uint256 transactionPrice) public view returns(uint256) {
        return transactionPrice*globalTransactionFee/PERCENTAGE_DIVIDER;
    }
    ...
    function _payFeeAndRoyalties(bytes32 saleId, uint256 tokenId, uint256 salePrice) internal {
        CollectionSale storage sale = collectionSales[saleId];
        uint256 factoryFee = salePrice * sale.fee / PERCENTAGE_DIVIDER;
        collectedFees += factoryFee + transactionFee(salePrice);
        // Only interested in the receiver. PERCENTAGE_DIVIDER passed just to not use 0 that some implementation may fail.
    ...
    function _transferNft(bytes32 saleId, uint256 tokenId, address to) internal returns(bool) {
        ...
        if (sale.nftsOwner == address(0)) {
            ...
            try IERC721Upgradeable(sale.collection).safeTransferFrom(sale.nftsOwner, to, tokenId) {
                return true;
            } catch  {
                return false;
            }
    ...
    function _extractBid(uint256 value) internal view returns(uint256) {
        uint256 numerator = value * PERCENTAGE_DIVIDER;
        uint256 denominator = PERCENTAGE_DIVIDER + globalTransactionFee;
    ...
    /**
     * NOTE: transaction fee should be changed with no ongoing auctions. Otherwise
     * some funds loss can be experienced on bids refund. 
     */
    function setTransactionFee(uint64 _newFee) external onlyRole(FEE_MANAGER_ROLE) {
        globalTransactionFee = _newFee;
        emit NewTransactionFee(_newFee);
    }
    ...
    event NewTransactionFee(uint64 indexed fee);
}
