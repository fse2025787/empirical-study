...
/**
 * @title Tokentraxx ERC1155 Token contract based on Openzepplin contracts
 * @author TokenTraxx Team
 * @dev Implementation of multi-token contract.
 */
    ...
    IERC1155Custom
{
    ...
    /**
     * @dev Returns the first owner of a token {_id}
     * @param _id Id of the token which is to be checked.
     */
    ...
    /**
     * @dev Throws if called by any account other than the minter role account.
     */
    modifier onlyMinter() {
    ...
    /**
     * @dev Throws if called by any account other than the maintainer role account.
     */
     ...
     * @param _uri sets up initial contract uri.
     * @param _royaltyBps sets the fees .
     * @param _market sets the market address.
     * Requirements:
     * {_market} address should not be zero address
     * {_royaltyBps} should be less than or equal to Max Bps
     */
    function initialize(
        string memory _uri,
        uint256 _royaltyBps,
        ...
        require(bytes(_uri).length > 0, throwError(INVALID_FUNCTION_PARAMS));
    ...
    /**
     * @dev function to give various roles to accounts.
     * @param _owner address of the owner.
     * @param _maintainer address of the person who gets the maintainer role .
     * Requirements:
     * {_maintainer} and {_owner} address should not be zero address
     */

    function setupRoles(address _owner, address _maintainer)
        external
        onlyAdmin
    {
        require(
            _maintainer != address(0) && _owner != address(0),
            throwError(ZERO_ADDRESS)
        );
        require(_owner == owner, throwError(INVALID_FUNCTION_PARAMS));
     ...
     * @param _royaltyBps sets the royalty fees.
     * Requirements:
     * {_royaltyBps} should be less than Max Bps
     */
     ...
     * @param tokenId tokenId of the asset to be calculated.
     * @param salePrice sale price of that token
     */
    function royaltyInfo(uint256 tokenId, uint256 salePrice)
        external
        view
     ...
     * @param to address of which the tokens are to be transferred.
     */
    function _beforeTokenTransfer(
        address operator,
        address from,
     ...
     * @param _to address of which token will be minted.
     * @param amount number of tokens to be minted.
     * @param _uri uri string for that token
     * @param maxPurchaseLimit Max number of purchase limit of that token
     * Requirement:
     * {_uri} length and {maxPurchaseLimit} should be greater than zero
     */
    function mintTraxx(
        address _to,
        uint256 amount,
        string calldata _uri,
        int256 maxPurchaseLimit
    ) external onlyMinter {
        require(
            amount > 0 && bytes(_uri).length > 0 && maxPurchaseLimit > 0,
        ...
        _primarySalePurchaseLimitPerToken[id] = maxPurchaseLimit;
     ...
     * @param amount number of tokens to be minted.
     * Requirement:
     * {id} length should be greater than zero
     * The function caller should be either admin or first owner
     */
    function addQuantity(
        ...
        require(_to != address(0), throwError(ZERO_ADDRESS));
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
    ...
    function batchMintTraxx(
        ...
        string[] memory uris,
        bytes memory data,
        int256[] calldata maxPurchaseLimit
    ) external onlyMinter {
        require(to != address(0), throwError(ZERO_ADDRESS));
        require(ids.length > 0, throwError(INVALID_FUNCTION_PARAMS));
        require(
            (ids.length == amounts.length) &&
                (amounts.length == uris.length) &&
                (uris.length == maxPurchaseLimit.length),
            throwError(INVALID_FUNCTION_PARAMS)
        );
        ...
        for (uint256 i; i < arrayLen; ++i) {
            ...
            _primarySalePurchaseLimitPerToken[ids[i]] = maxPurchaseLimit[i];
        }
    ...
    /**
     * @dev Used to set the URI for a specific token.
     * @param tokenId id of the token which value to be changed.
     * @param tokenURI uri to be set to that token.
     * Requirement:
     * {tokenId} should be valid
     * {tokenURI} should be greater than zero
     */
    ...
    function setBaseURI(string calldata baseUri)
        ...
        onlyMaintainer
    {
        require(bytes(baseUri).length > 0, throwError(INVALID_FUNCTION_PARAMS));
        _baseURI = baseUri;
    }
     ...
     * @param account address of the account which token to be burned
    ...
    function burnTraxx(
        ...
        require(
            msg.sender == account && (balanceOf(account, id) >= value),
            throwError(ONLY_TOKEN_OWNER)
        );
        _burn(account, id, value);
        ...
        returns (TokenBalance[] memory)
    {
        ...
        for (uint256 i; i < ids.length; ++i) {
            tokenDetails[i] = TokenBalance(ids[i], balanceOf(_owner, ids[i]));
        }
        return tokenDetails;
    }
    ...
    function getPrimarySalePurchaseLimitPerToken(uint256 _tokenId)
        ...
        returns (int256)
    {
        return _primarySalePurchaseLimitPerToken[_tokenId];
    }

    /**
     * @dev maps the number of tokens bought by an user to the token id and user address
     * @param _tokenId Id of token to set the Limit
     * @param maxPurchaseQuantity Number of tokens that can be purchased in primary sale
     * Requirement:
     * {_tokenId} should be valid.
     * {maxPurchaseQuantity} should be > zero.
     */
    function setPrimarySalePurchaseLimitPerToken(
        uint256 _tokenId,
        int256 maxPurchaseQuantity
    ) external onlyMaintainer {
        require(
            _tokenId >= 0 && _tokenId < _tokenIdTracker.current(),
            throwError(TOKEN_ID_NOT_FOUND)
        );

        require(
            maxPurchaseQuantity == -1 || maxPurchaseQuantity > 0,
            throwError(INVALID_FUNCTION_PARAMS)
        );

        _primarySalePurchaseLimitPerToken[_tokenId] = maxPurchaseQuantity;
    }

    /**
     * @dev Returns the primary sale user counts per Token.
     * @param account Address of the account which is to be updated
     * @param _tokenId the id of the collection to update
     */
    function getPrimarySaleUserCountPerToken(address account, uint256 _tokenId)
        public
        view
        ...
        returns (uint256)
    {
        return _primarySaleUserCountPerToken[account][_tokenId];
    }

    /**
     * @dev maps the number of tokens bought by an user to the token id and user address
     * @param account address of which the Sale count should be updated.
     * @param _tokenId id of token which is to be updated.
     * @param quantity Number of tokens to be increased.
     * Requirement:
     * function caller should be market place contract
     */
    function updatePrimarySaleUserCountPerToken(
        address account,
        uint256 _tokenId,
        ...
        _primarySaleUserCountPerToken[account][_tokenId] += quantity;
    ...
    mapping(uint256 => int256) private _primarySalePurchaseLimitPerToken;

    /**
     * @dev maps the number of tokens bought by an user to the token id and user address
     */
    mapping(address => mapping(uint256 => uint256))
        private _primarySaleUserCountPerToken;
}