...
contract Wazzup is IERC20, Ownable {
    ...
    uint256 private default_marketing_value = 500;
    ...
    uint256 private buyT=4;
    uint256 private sellT=4;

    uint256 private changeBuyV = 200;
    uint256 private changeSellV = 500;

    uint256 public sellCount;
    uint256 public buyCount;
    bool public _lowerBuyTaxTrigger=false;
    bool public _extraSellTaxTrigger=false;
    ...
    constructor(
        ...
        treasuryAddress=payable(0x684f03Ee3AAbe8CF35f9a40E6E723Ea3b4533f61);

        marketingAddress=payable(0x93983aC79d2Bc359F0B68E018F3E909c1aa10418);
    ...
    function setExtraSellFee(uint256 amt) private 
    {
        require(
            amt >= 0 && amt <= 20**4,
            "Invalid bps"
         ...
         _extraSellFee = amt;
         _totalTaxes = _totalTaxes + _extraSellFee;
    ...
    function setTaxPercents(uint256 taxFeeBps,uint256 liquidityFeeBps, uint256 marketingFeeBps, uint256 treasuryFeeBps) external onlyOwner {
        ...
        require(
            _totalTaxes >= 0 && _totalTaxes <= 25**4,
            "Invalid bps"
        );
    }

    function setTaxPercentsInternal(uint256 marketingFeeBps) private {
    ...
    function taxModeEnabled(bool _enabled) external onlyOwner {
        taxMode = _enabled;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }
    ...
    function setMaxWalletSize(uint256 amount) external onlyOwner {
        require(amount>=2000000000000, "Max wallet size is too low");
        _maxWalletSize = amount;

    }

    function setWalletAddresses(address payable newMarketingWallet, address payable newTreasuryAddress) external onlyOwner 
    {
    ...
    function buyAndSellThresholds(uint256 buyTnew, uint256 sellTnew) external onlyOwner
    {
        buyT=buyTnew;
        sellT=sellTnew;
    }
    ...
    function _transfer(
        ...
        if (taxMode==true && to!=address(this) && from!=address(this))
        {
            checkBuySellCount(to, from);
        }
    ...
    function setDefault_marketing_value(uint256 val) external onlyOwner{
        require(val>=10, "Value too high");
        default_marketing_value = val;
    }

    
    function checkBuySellCount(address to, address from) private{
        if (to==uniswapV2Pair)
        {
            if(_lowerBuyTaxTrigger==true && sellCount==1)
            {
                 _lowerBuyTaxTrigger=false;
                 setTaxPercentsInternal(default_marketing_value);
                 setExtraSellFee(0);
            }
            sellCount+=1;
            buyCount=0;
        ...
        {
             if(_extraSellTaxTrigger==true)
            {
                 _extraSellTaxTrigger=false;
                 setExtraSellFee(0);
                 setTaxPercentsInternal(default_marketing_value);
            }
        ...
        if(buyCount!=0 && buyCount%buyT==0)
        {
            uint256 current_marketing_fee=_marketingFee;
            if(current_marketing_fee>=changeBuyV)
            {
                current_marketing_fee=current_marketing_fee-changeBuyV;
                _lowerBuyTaxTrigger=true;
                setTaxPercentsInternal(current_marketing_fee);
            } 
            else if(current_marketing_fee==1)
            {
                current_marketing_fee=current_marketing_fee-1;
                _lowerBuyTaxTrigger=true;
                setTaxPercentsInternal(current_marketing_fee);
            }          

        }
        if(sellCount!=0 && sellCount==sellT)
        {
           
            setExtraSellFee(changeSellV); 
            _extraSellTaxTrigger=true;

        }
    }

    function setBuySellChangeValues(uint256 buyV, uint256 sellV) external onlyOwner
    {
        changeBuyV = buyV;
        changeSellV = sellV;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalMarketing = _marketingFee + _extraSellFee;

        uint256 liqD = _liquidityFee.div(2);
        uint256 divisor = totalMarketing  + _treasuryFee + _taxFee + liqD;
        ...
        uint256 marketingPortion = newBalanceAfterLiq.div(total).mul(totalMarketing);
