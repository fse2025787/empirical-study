...
contract ECOInTree is ERC721A, Ownable, ReentrancyGuard {
    ...
    **/
    ...
    function buyNFTWithERC20(uint256 amount) external nonReentrant {
        ...
        uint256 token_amount = priceSale * amount * ethPrice * (10 ** usdt.decimals()) / 1e8 / 1e18;
        usdt.transferFrom(msg.sender, address(this), token_amount);
        addr_num1 = token_amount * addr_rate1 / 100;
        addr_num2 = token_amount * addr_rate2 / 100;
        addr_num3 = token_amount - addr_num1 - addr_num2;
        usdt.transferFrom(address(this), addr_1, addr_num1);
        usdt.transferFrom(address(this), addr_2, addr_num2);
        usdt.transferFrom(address(this), addr_3, addr_num3);

        _safeMint(msg.sender, amount);
    }

    function getCheck(uint256 amount) public view returns(uint256){
        uint256 ethPrice = getLatestPrice(); 
        uint256 token_amount = priceSale * amount * ethPrice * (10 ** usdt.decimals()) / 1e8 / 1e18;
        return token_amount;
    }
