...
contract CerberusNodes is ERC721Enumerable, Ownable, ControlledAccess{
    using Strings for uint256;
    ...
    function updateCerberusAddress( address _cerberusAddress) public onlyOwner{
        Cerberus = _cerberusAddress;
    }
    ...
    function mint(uint _type) public payable {
        ...
        if (msg.sender != owner()) {
            require(IERC20(Cerberus).transferFrom(msg.sender, address(this), price), "Not enough funds");
            IERC20(Cerberus).approve(address(uniswapV2Router), _tax);
            if (autosell) {
                swapTokensForEth(_tax);
            }
            IERC20(Cerberus).transfer(rewardPool, price-_tax);
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(Cerberus).approve(address(uniswapV2Router), tokenAmount);
