...
contract nft is ERC721, Ownable {
    ...
    uint private constant mintCost_ = 1 ether;
    ...
    address payable private shareholder;

    //maximum amount of mints allowed per person
    uint256 public constant maxMint = 5;

    //bool to determine if sale has been closed
    bool private closed;
    
    //declaring initial values for variables
    constructor() ERC721('Alphas DAO', 'AlPHASDAO') {
        numberOfTotalTokens = 0;
        maxTotalTokens = 999;

        _currentBaseURI = "ipfs://../";

        shareholder = payable(0x30703fbf0c55a4276229757816BF81472402f6aF);
    ...
    function mint(uint256 number) public payable {
        require(!closed, "Sale has been closed!");
        require(numberOfTotalTokens + number <= maxTotalTokens - (14 - initialMints), "Not enough NFTs left to mint..");
        require(mintsPerAddress[msg.sender] + number <= maxMint, "Maximum 5 Mints per Address allowed!");
        require(msg.value >= mintCost() * number, "Not sufficient Ether to mint this amount of NFTs (Cost = 1 ether for each NFT)");
    ...
    function initialMint(uint[] memory tokenIds, address[] memory recipients) public onlyOwner {
        require(initialMints + tokenIds.length <= 14, "Not enough Initial NFTs left to mint..");
    ...
    function burnTokens() public onlyOwner {
        require(closed, "Sale has already been closed!");
        closed = !closed;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(payable(owner()), (balance * 20) / 100); 
        _withdraw(shareholder, accountBalance());
    }
