...
contract JuuniRewardTicketV2 is
    ERC1155Upgradeable,
    OwnableUpgradeable,
    ...
    QuestReward
{
    ...
    mapping(address => bool) private _ticketAddresses;
    ...
    function claimForTicket(
        address to,
        uint256 quantity
    ) external nonReentrant {
        if (!_ticketAddresses[msg.sender]) revert InvalidMintSource();
    ...
    function addTicketContract(address ticketAddress) external onlyOwner {
        _ticketAddresses[ticketAddress] = true;
    }

    function removeTicketContract(address ticketAddress) external onlyOwner {
        _ticketAddresses[ticketAddress] = false;
    }
