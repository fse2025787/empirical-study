...
interface INiftyriots is IERC721Enumerable {
...
contract Niftyriots is Context,Ownable, ERC165,INiftyriots, IERC721Metadata {
...
// This is the provenance record of all nekos artwork in existence
    ...
    uint256 public constant MAX_NFT_SUPPLY = 7635;
    uint256 public constant MAX_NFT_ATONCE = 20;

    uint256 public NiftyPrice = 45000000000000000; // 0.045 ETH
    ...
    constructor () {
        ...
        _name = "Niftyriots";
        _symbol = "NRIOTS";
        _baseURI = "https://www.niftyriots.com/api/v1/artworks/";
    ...
    function setPrice(uint256 _newprice) public onlyOwner {
        NiftyPrice = _newprice;
    }



    /**
     * @dev Gets current NiftyRiot Price
     */
    ...
    function superMint(uint256 numberOfNfts)
       ...
       public payable

        {
         ...
         //sale is on
         require(SaleOk, "Sale is not active");

          // the total supply cannot exceed the max nft supply
          require(totalSupply() < MAX_NFT_SUPPLY, "Sale is Sold Out");
          ...
          require(NiftyPrice.mul(numberOfNfts) == msg.value, "Ether value sent is not correct - duhh" );
    ...
    // To ensure
    // Minting 10 at a time

    function reserveGiveaway() public onlyOwner {
        uint currentSupply = totalSupply();
        uint256 numNFTs = 15 ;
        require(totalSupply().add(numNFTs) <= 7600, "Exceeded giveaway supply");
        uint256 index;
    ...
    function MigrateGeneral(address to, uint256 msize) public onlyOwner {

        uint currentSupply = totalSupply();
        require(totalSupply().add(msize) <= 7600, "Exceeded giveaway supply");
        uint256 index;
        for (index = 0; index < msize; index++) {
            _safeMint(to, currentSupply + index);
        }
    }
