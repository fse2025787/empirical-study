...
// File: contracts\Artis.sol


pragma solidity >=0.8.0 <0.9.0;

contract Artis is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  event ChangedSalesTokenExpirationTime(uint256 indexed tokenId, uint256 expire_at);
  ...
  function initialize() public initializer {
    ...
    ERC721Upgradeable.__ERC721_init('ARTIS', 'ARTIS');
    // URL Base to get metadata
    setBaseURI('https://api.artis.market/token?id=');
    // address excluded to pay fees on transfer NFT
    excludedList[owner()] = true;
    // 50 %
    maxAllowedRoyalties = 500000;
    // 2 %
    feeSellTokens = 20000;
    // 5 % min increment for new bids from highest
    minIncrement = 50000;
  }
  ...
  function buy(
    ...
    emit OrderSuccessful(_tokenId, tokenSeller, _tokenMeta[_tokenId].price, msg.sender);
    return _tokenId;
  }
  ...
  function _payTxFee(uint256 _tokenId, bool sellByAuctions) internal returns (uint256) {
    ...
    if (tokenSeller != owner()) {
      uint256 feeSellToken = _computePercent(
        getTokenPrice(_tokenId, sellByAuctions),
        feeSellTokens
      );
    ...
    if (tokenSeller != owner() && tokenSeller != artistOf(_tokenId)) {
      uint256 royaltyFee = _computePercent(
        getTokenPrice(_tokenId, sellByAuctions),
        _tokenMeta[_tokenId].royalty
      );
