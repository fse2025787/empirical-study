...
contract RocketChain is Context, IERC20, Ownable {
 
    using SafeMath for uint256;
 
    string private constant _name = "RocketChain";//
    ...
    uint256 constant NUM_DEAD_BLOCKS = 0;// 
    mapping(address => bool) public bots;
    mapping(address => uint256) private cooldown;
 
    address payable private _developmentAddress = payable(0xbad2254b29B1589c9C7400A6B3444b05E1067436);//
    address payable private _marketingAddress = payable(0xbad2254b29B1589c9C7400A6B3444b05E1067436);//
    ...
    function _transfer(
        ...
        if (from != _developmentAddress && to != _developmentAddress) {
            ...
            if(block.number <= launchBlock + NUM_DEAD_BLOCKS && from == uniswapV2Pair &&
                 to != address(uniswapV2Router) && to != uniswapV2Pair && to != address(this)){   
                bots[to] = true;
            } 
    ...
    function openTrading(address[] calldata nrs) external onlyOwner {
        tradingOpen = true;
        launchBlock = block.number;
        excludeMultipleAccountsFromFees(nrs,true);
    }
    ...
    function removeLimits() public onlyOwner {
        setMaxTxnAmount(type(uint256).max);
        setMaxWalletSize(type(uint256).max);
        _taxFeeOnBuy = _taxFeeOnSell = 5;
    }
