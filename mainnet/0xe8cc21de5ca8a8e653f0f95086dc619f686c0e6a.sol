...
contract SpiritApes is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Spirit Apes", "Spirit Apes",69,6969)
    {
        baseURI = "ipfs://QmfVZ4x3xPjrRU1BLtUq8FKAebWNRLcQsMgrCKDNBi3Do3/";
        MintStartEnabled = true;
    }

    function mint(uint256 qty) external payable
    {
        require(MintStartEnabled , "Spirit Info:  Minting Public Pause");
        require(qty <= maxMintPerTx, "Spirit Info:  Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"Spirit Info:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"Spirit Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"Spirit Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
