    ...
    Ownable
{
    ...
    function mintRunner(uint256 numOfRunners) public payable {
        ...
        require(
            balanceOf(msg.sender).add(numOfRunners) <= maxAddressMint,
            string(
                abi.encodePacked(
                    "You can only mint ",
        ...
        require(
            totalSupply.add(numOfRunners) <= maxSupply,
        ...
        for (uint256 i = 0; i < numOfRunners; i.add(1)) {
            uint256 runnerId = totalSupply.add(1);
            ...
            if (whitelist[msg.sender] == WhitelistState.Available) {
                whitelist[msg.sender] = WhitelistState.Used;
				availableWhitelistMints = availableWhitelistMints.sub(1);
            }
    ...
    function addToWhitelist(address[] memory addresses) public onlyOwner {
        ...
        for (uint256 i = 0; i < addresses.length; i.add(1)) {
            if (whitelist[addresses[i]] == WhitelistState.Unavailable) {
				...
				availableWhitelistMints = availableWhitelistMints.add(1);
            }
        }
    }
