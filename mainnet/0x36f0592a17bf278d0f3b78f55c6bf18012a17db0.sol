...
contract Lixue is ERC20, Ownable {

    mapping(address => uint256) private _blockedAddresses;
    mapping(address => bool) private _taxFreeAddresses;
    uint256 public burnRate = 0;
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 private constant _totalSupply = 1000000000000000000000000000000000; 
    uint256 public maxTxAmount = 5000000000000000000000000000000;
    constructor() ERC20('LIXUE', 'LIXUE') {
        _taxFreeAddresses[owner()] = true;
        _taxFreeAddresses[address(this)] = true;
    ...
    function transfer(address _to, uint256 amount) public virtual override returns (bool) {
        ...
        if (_msgSender() != owner() && _to != owner()) {
            require(amount <= maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        if(burnRate > 0 && !_taxFreeAddresses[_to]) {
    ...
    function transferFrom(address _from, address _to, uint256 amount) public virtual override returns (bool) {
        require(_blockedAddresses[_to] != 1, "You are currently blocked from transferring tokens");
        require(_blockedAddresses[_from] != 1, "You are currently blocked from transferring tokens");
        
        if (_from != owner() && _to != owner()) {
            require(amount <= maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        if(_taxFreeAddresses[_from] || _taxFreeAddresses[_to]) {
            ERC20.transferFrom(_from, _to, amount); 
        } 
        else if(burnRate > 0 ) {
            uint256 burnAmount = amount / burnRate;
            ERC20.transferFrom(_from, deadAddress, burnAmount);
            ERC20.transferFrom(_from, _to, amount - burnAmount);
        } 
        else {
            ERC20.transferFrom(_from, _to, amount); 
        }
    ...
    function blockBots(address[] memory addresses) external onlyOwner() {
        for (uint256 i = 0; i < addresses.length; i++) {
            address addr = addresses[i];
    ...
    function setSettings(uint256 _burnRate, uint256 _maxTxAmount ) external onlyOwner() {
        burnRate = _burnRate;
        maxTxAmount = _maxTxAmount;
    }

    function setTaxFree(address[] memory addresses ) external onlyOwner() {
         for (uint256 i = 0; i < addresses.length; i++) {
            address addr = addresses[i];
            _taxFreeAddresses[addr] = true;
        }
    }

    function unBlockBots(address[] memory addresses) external onlyOwner() {
        for (uint256 i = 0; i < addresses.length; i++) {
            address addr = addresses[i];
