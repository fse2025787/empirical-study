...
contract NFTSchweiz is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;
    using Strings for uint256;

    uint public constant _TOTALSUPPLY = 20222;
    uint public maxQuantity =10;
    uint public maxPerUser=222;
    // uint public reserve=;
    uint256 public price = 0.2022 ether; 
    ...
    constructor(string memory baseURI) ERC721("NFT Schweiz", "NSZ") {
        setBaseURI(baseURI);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(bytes(base).length > 0){
            return reveal ? string(abi.encodePacked(base, tokenId.toString(), ".json")) : string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/QmVBEjtH4VQZbvg7Ww7Equqr4m16oqHiJpV16yKwcU6u1b"));
        }
        else 
        return "";
    }
    function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "NFT Schweiz", "description": "The start of the most unique NFT portfolio in Switzerland. Become a part of it even before the official launch.", "seller_fee_basis_points": 1000, "fee_recipient": "0x556c647EFdF3D03f7eb3b811e3df5e8267BCC595"}'))));
