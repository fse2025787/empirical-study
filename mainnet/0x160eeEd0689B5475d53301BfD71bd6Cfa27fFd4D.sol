...
contract ForefrontVesting is Ownable {
  ...
  uint256 public constant vestingLength = 730 days;
  
  uint256 public constant cliffLength = 365 days;
  ...
  function removeEntry(address wallet) public onlyOwner returns (bool success) {
  ...
  function retrieveFunds() public onlyOwner {
    vestingToken.safeTransfer(treasury, vestingToken.balanceOf(address(this)));
  }
  ...
  function vestableAmount(address wallet) public view returns (uint256) {
    ...
    if (block.timestamp < startTimestamp + cliffLength) {
      ...
      return
        (remainingAmount * (block.timestamp - startTimestamp)) / vestingLength;
    }
  }
