...
contract ShibBurnLottery is Ownable {
    ...
    function drawWinners(uint _numOfWinners1, uint _numOfWinners2, uint _verseAmount) external onlyOwner { 
        // _numOfWinners1 = winners that receive tokens
        // _numOfWinners2 = winners that receive rewards manually

        require(_numOfWinners1 > 0, "_numOfWinners1 cannot be 0");

        uint lotteryId = currentLotteryId;

        // draw lottery winners and distribute verse
        _drawWinners(lotteryId, _numOfWinners1, _numOfWinners2, _verseAmount);
    ...
    function _drawWinners(uint _lotteryId, uint _numOfWinners1, uint _numOfWinners2, uint _verseAmount) internal {

        address[] memory participants = lotteryParticipants[_lotteryId];
        uint totalTickets = lotteryTotalTickets[_lotteryId];
        uint toPay = _verseAmount;

        for(uint i = 0; i < _numOfWinners1 + _numOfWinners2; i++) {
            ...
            for(uint j = 0; j < participants.length; j++) {
                ...
                if(currentNumber >= randomNumber) {
                    ...
                    if(i < _numOfWinners1) {
                        pay = (toPay / (_numOfWinners1 + 1 - i)) * 2;
                        toPay = toPay - pay;
    ...
    function recoverERC20(address _receiver, address _tokenAddress, uint _tokenAmount) external onlyOwner {        
