...
contract DungeonLoot is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function img(uint256 _tokenId) internal view returns (string memory) {
        ...
        string memory svg;
        svg = concat(svg, "<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 512 512' width='512px' height='512px'><rect width='100%' height='100%' fill='");
        ...
        returns (string memory)
    {
        ...
        // level
        itemName = concat(itemName, uint256((determineInt(string(abi.encodePacked("LEVEL", lIdxStr, tokenStr))) % 10) + 1).toString());
