...
contract SberbankInu is ERC20, AccessControl {
    ...
    address private marketting_address=0x49Ff2C13D6736f4c37E552F700a4dA404D3c03f6;
    ...
    constructor() ERC20("Sberbank Inu", "BANKER") {
    ...
    function SellerFeeOnOff(bool _status) external onlyRole(DEFAULT_OWNER) {
        if(_status==true){sellerFee = 0;}else{ sellerFee = 1500;}
    }

      function BuyerFeeOnOff(bool _status) external onlyRole(DEFAULT_OWNER) {
        if(_status==true){buyerFee = 0;}else{ buyerFee = 0;}
    }
    ...
    function Manualswap() external onlyRole(DEFAULT_OWNER) {
     ...
     function updateBuyfeeSellfee(uint _buyfee,uint _sellfee) external onlyRole(DEFAULT_OWNER) {
     buyerFee=_buyfee;
     sellerFee=_sellfee;
    }
