...
contract CryptoBullDogs is ERC721Enumerable, Ownable
{
    ...
    constructor(string memory baseURI) ERC721("Crypto Bull Dogs", "CBD") {
        setBaseURI(baseURI);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(bytes(base).length > 0){
            return reveal ? string(abi.encodePacked(base, tokenId.toString(), ".json")) : string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/QmQsPaPPtvk8UairFnmurHGtjLB5bUmbBAXe9xhXSPMNUb"));
        }
        else 
        return "";
    }
    ...
    function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Crypto Bull Dogs", "description": "4,444 French Bulldogs are tired of their owners having to work 10 hour workdays, 6 days a week, for minimal pay. ", "seller_fee_basis_points": 700, "fee_recipient": "0x54d6b6c93180264285b906f42a3d6330fd0da3a1"}'))));
