...
contract SnowballInu is Context, IERC20, Ownable {
    ...
    uint256 public sellingCoolDown = 0 seconds;
    ...
    function wenmoonStatus(bool _state) public onlyOwner {
        _wenmoon = _state;
    ...
    function changewenmoonTime(uint256 _time) public onlyOwner {
    ...
    function _transfer(
        ...
        } else if (_wenmoon && to == uniswapV2Pair) {
            require(
                block.timestamp >= _lastBuyTime[from] + sellingCoolDown,
                "wait for next sell"
            );
        }
