...
contract USDH is IERC20, ReentrancyGuard {
    ...
    function mint(address _collateral, uint256 _amount, bool _exactOutput) external nonReentrant {
        ...
        uint256 _toMint = _amount;
        (uint256 _toPay, bool _pegStable) = getMintCost(_collateral, _amount, _exactOutput);
        require(_pegStable, "Peg Unstable");
        if (!_exactOutput) {
            _toMint = _toPay;
            _toPay = _amount;
        }
        require(_toPay >= _toMint);

        IERC20 _Collateral = IERC20(_collateral);
        require(_Collateral.balanceOf(msg.sender) >= _toPay);
        require(_Collateral.allowance(msg.sender, address(this)) >= _toPay);
        uint256 _balance = _Collateral.balanceOf(address(this));
        require(_Collateral.transferFrom(msg.sender, address(this), _toPay), "TF");
        require(_Collateral.balanceOf(address(this)) >= _balance + _toPay, "TF");

        uint256 _newSupply = _totalSupply + _toMint;
        require(_newSupply > _totalSupply);
        _totalSupply = _newSupply;
        _balances[msg.sender] = _balances[msg.sender] + _toMint;
        emit Transfer(address(0), msg.sender, _toMint);
        emit Mint(msg.sender, _collateral, _toMint);
    }
    ...
    function getCollateral(address _collateral) external view returns (bool, uint256, address, uint256, uint24) {
        return wasCollateral[_collateral] ? (collateralWhitelist[_collateral], IERC20(_collateral).balanceOf(address(this)), collateralPairings[_collateral], collateralPairingsDecimals[_collateral], collateralFeeTiers[_collateral]) : (false, 0, address(0), 0, 0);
    }

    function getCollateralPrice(address _collateral) public returns (uint256) {
        return collateralPairings[_collateral] != address(0) ? quoter.quoteExactInputSingle(collateralPairings[_collateral], _collateral, collateralFeeTiers[_collateral], 1 ether, 0) : 0;
    }

    function getMintCost(address _collateral, uint256 _amount, bool _exactOutput) public returns (uint256, bool) {
        if (collateralPairings[_collateral] == address(0)) return (0, false);
        uint256 _quote = getCollateralPrice(_collateral);
        uint256 _peg = (1000 * (10 ** collateralPairingsDecimals[_collateral]));
        if (_peg > _quote) _amount = _exactOutput ? (_amount * ((_peg * (1 ether)) / _quote)) / (1 ether) : (_amount * (1 ether)) / ((_peg * (1 ether)) / _quote);
