...
contract Staking is OwnableUpgradeable, ReentrancyGuard{
    ...
    function claimReward(uint256 _id) external nonReentrant{
        ...
        uint256 rewardAmount = idToStake[_id].amount.mul(idToStake[_id].REWARDS_RATE).mul(6500).mul(claimableCycle).div(_currentStakedAmount);
    ...
    function getSomevalues(uint256 _id) public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 claimableCycle = stakingTime.sub(stakingTime.mod(_REWARD_CYCLE)).div(_REWARD_CYCLE);
        uint256 rewardAmount = idToStake[id].amount.mul(idToStake[id].REWARDS_RATE).mul(6500).mul(claimableCycle).div(_currentStakedAmount);
        uint256 temp1 = idToStake[id].amount.mul(idToStake[id].REWARDS_RATE);
        uint256 temp2 = temp1.mul(6500).mul(claimableCycle);
        uint256 temp3 = temp2.div(_currentStakedAmount);
