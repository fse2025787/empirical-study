...
contract TOKEN is MappableToken {
	function __TOKEN_init(address creator_, address factory_, address mine_, address eco_, address team_, address contribution_, address liqudity_, address public_, address airdrop_) external initializer {
        __Context_init_unchained();
		__ERC20_init_unchained("ChainSwap.com Governance Token", "TOKEN");
		__MappableToken_init_unchained(factory_, creator_);
		__TOKEN_init_unchained(mine_, eco_, team_, contribution_, liqudity_, public_, airdrop_);
	}
	
	function __TOKEN_init_unchained(address mine_, address eco_, address team_, address contribution_, address liqudity_, address public_, address airdrop_) public initializer {
...
contract TOKENING is ERC20UpgradeSafe, Configurable {
    using SafeERC20 for IERC20;
	...
	function __TOKENING_init(address governor_) public initializer {
		__Governable_init_unchained(governor_);
        __Context_init_unchained();
		__ERC20_init_unchained("ChainSwap.com TOKEN locking", "TOKENING");
		_setupDecimals(18);
	}
