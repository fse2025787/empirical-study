...
//mainnet
        ...
        function getPopulation() public view returns(uint256 [] memory){
          return population;
        }
        ...
        function getPopulationYield() public view returns(uint256){
          uint256 populationYield = population[0]*20+population[1]*16+population[2]*13+population[3]*10;
          return populationYield;
        }

        function calculateAndDisburse(uint256 dividend) onlyOwner public {
            uint256 totalStonkers = stonkerNFT.totalSupply();
            totalYield = 0;
            uint256 currentAndroidPopulation=0;
            uint256 currentVampirePopulation=0;
            uint256 currentElfPopulation=0;
            uint256 currentHumanPopulation=0;
              for(uint256 i=0;i<totalStonkers;i++){
                address stonkerOwner = stonkerNFT.ownerOf(i);
                (uint256 id, uint256 yieldEffectivity, uint256 mintedTimeStamp, uint256 mintBatch) = stonkerNFT.stonkers(i);
                if(isExcluded[stonkerOwner])continue;
                if(mintedTimeStamp>lastDisbursementTime)continue;
                totalYield += yieldEffectivity;
                if(yieldEffectivity==10)currentHumanPopulation++;
                else if (yieldEffectivity==13)currentElfPopulation++;
                else if (yieldEffectivity==16)currentVampirePopulation++;
                else if (yieldEffectivity==20)currentAndroidPopulation++;
              }
              
              population[0]=currentAndroidPopulation;
              population[1]=currentVampirePopulation;
              population[2]=currentElfPopulation;
              population[3]=currentHumanPopulation;
              uint256 dividendPerYield = dividend/totalYield;
              
              for(uint256 j=0;j<totalStonkers;j++){
                
                (uint256 id, uint256 yieldEffectivity, uint256 mintedTimeStamp, uint256 mintBatch) = stonkerNFT.stonkers(j);
                address stonkerOwner = stonkerNFT.ownerOf(j);
                if(isExcluded[stonkerOwner])continue;
                if(mintedTimeStamp>lastDisbursementTime)continue;
                dividends[stonkerOwner]+=yieldEffectivity*dividendPerYield;
                dividendDetail memory dd;
                dd.stonkerId = j;
                dd.distributionTime = block.timestamp;
                dd.distributionAmount = yieldEffectivity*dividendPerYield;
                distributedDividends[stonkerOwner].push(dd);
              } 
