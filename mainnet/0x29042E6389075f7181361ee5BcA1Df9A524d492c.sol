...
contract LendFlareVault is OwnableUpgradeable, ReentrancyGuardUpgradeable, ILendFlareVault {
    ...
    function initialize(
        ...
        require(_lendFlareCRV != address(0), "LendingBooster: zero acrv address");
    ...
    function withdraw(
        ...
        // 2. withdraw lp token
        UserInfo storage _userInfo = userInfo[_pid][msg.sender];
        require(_amount <= _userInfo.totalUnderlying, "LendingBooster: _amount not enough");
    ...
    function addPool(uint256 _lendingMarketPid) public onlyOwner {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            require(poolInfo[i].convexPoolId != _lendingMarketPool.convexPid, "LendingBooster: duplicate pool");
        }
