...
contract Kishimoto is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x5766f4f998Be2751E64d05f31f12886e9C6841Df);
    address payable private _DevAddress;

    uint256 public KISHI_MAX = 10000;
    ...
    uint256 private _activeDateTime = 1636742701;
    ...
    Counters.Counter private _publicKISHI;

    constructor() ERC721("Kishimoto", "KISHI") {
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicKISHI.current();

            if (_publicKISHI.current() < KISHI_MAX) {
                _publicKISHI.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicKISHI.current() < KISHI_MAX,"Purchase would exceed KISHI_MAX");
