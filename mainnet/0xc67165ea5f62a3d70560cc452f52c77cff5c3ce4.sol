...
contract Jeremy is Ownable, ERC721A, ReentrancyGuard, ERC721ABurnable {
  using Strings for uint256;

  uint256 public  PRICE = 0.075 ether;
  uint256 private constant TotalCollectionSize_ = 100000; // total number of nfts
  string private _baseTokenURI;
  uint public status = 2; //0-pause 2-public
  bool singleURI = true;

  constructor() ERC721A("Jeremy Knows","JK") {
    _baseTokenURI = "ipfs://QmdDoX1g8YFRGmes8aJFqtPxqZKqAFxZQtpRdzWWLVwa8j";
  }
  ...
  function mint(uint256 quantity) external payable callerIsUser {
   ...
   function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if(singleURI)
    return
      bytes(baseURI).length > 0 ? baseURI : "";
    else
    return
  ...
  function setsingleURI(bool s) public onlyOwner{
      singleURI = s;
  }
}