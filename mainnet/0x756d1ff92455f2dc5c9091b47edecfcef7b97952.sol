...
contract ValhallaKids is
    ERC721A,
    ERC2981,
    Ownable,
    DefaultOperatorFilterer
{
    string public baseURI = "https://ipfs-storage.s3.amazonaws.com/valhallakids/";

    uint256 public tokenPrice = 40000000000000000; //0.001 ETH

    uint256 public whitelistTokenPrice = 40000000000000000; //0.0005 ETH
    ...
    constructor() ERC721A("Valhalla Kids", "VAK") {
        // 500 = RoyaltyFee 5%
        ...
        returns (uint256)
    {
        if (numberMinted(minter) > 0) {
            ...
            return
                (numberOfTokens - maxTokensFreePerTxWW) * whitelistTokenPrice;
        } else if (
            whitelistMintRemains > 0 && numberOfTokens <= maxTokensFreePerTxWW
        ) {
            return 0;
    ...
    function mintToken(uint256 numberOfTokens) public payable {
        ...
        if (whitelistMintRemains > 0 && numberMinted(msg.sender) <= 0) {
            require(
                numberOfTokens >= minTokensRequiredPerTxWW,
            ...
            if (numberOfTokens > maxTokensFreePerTxWW) {
                require(
                    msg.value >=
                        (numberOfTokens - maxTokensFreePerTxWW) *
                            whitelistTokenPrice,
                    "Not enough ether"
                );
            }
    ...
    function setminTokensRequiredPerTxWW(uint256 _minTokensRequiredPerTxWW)
        public
        onlyOwner
    {
        minTokensRequiredPerTxWW = _minTokensRequiredPerTxWW;
