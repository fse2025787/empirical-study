...
contract BloodPact is ERC721Enumerable, Ownable {  
    using Address for address;
    ...
    constructor (string memory newBaseURI) ERC721 ("Blood Pact", "BP") {
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(a1).send(percent * 35)); // 35% for Noisy
        require(payable(a2).send(percent * 35)); // 35% for GB
        require(payable(a3).send(percent * 30)); // 30% for NFT Forge
    }
}
