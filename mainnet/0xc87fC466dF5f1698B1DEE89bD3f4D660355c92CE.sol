 ...
 */
contract VaultToken is Context, AccessControl, ERC721Burnable, ERC721Pausable {
    ...
    function vaultAmount() public view returns ( uint256 ){
    ...
    function burnFromRole(uint256 tokenId) public virtual {
        require(hasRole(BURNER_ROLE, _msgSender()), "must have burn role to mint");
        _burn(tokenId);
    }

    function mintFromRole(address to) public virtual returns( uint id ){
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");
...
contract Chick is Context, AccessControl, ERC20Burnable, ERC20Pausable {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURN_ROLE");
     ...
     * `amount`.
     */
    function burnFromOperator(address account, uint256 amount, address operator ) public virtual  {
        require(hasRole(BURNER_ROLE, _msgSender()), "must have burn role to burn");
        uint256 decreasedAllowance = allowance(account, operator ).sub(amount, "ERC20: burn amount exceeds allowance");

        _approve(account, operator, decreasedAllowance);
        _burn(account, amount);
    }
