...
contract TiddyTwister is ERC20, Ownable {
    using SafeMath for uint256;
...
TiddyTwisterDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Tiddy Twister", "TWIST") {
        marketingWallet = payable(0xB594787653a12852db0338E0D9E6Be2288E94f7b);
        devWallet = payable(0xb6CF5aA9455943eACEE7971226441d2D556f95ce);
        ...
        dividendTracker = new TiddyTwisterDividendTracker(payable(this), router, 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48,
                                                   "Tiddy TwisterTRACKER", "TWISTTRACKER");
...
contract TiddyTwisterDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    TiddyTwister public TiddyTwisterContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        TiddyTwisterContract = TiddyTwister(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main TiddyTwister contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TiddyTwisterContract);

            uint256 prevBalance = TiddyTwisterContract.balanceOf(address(this));
            ...
            {
                uint256 received = TiddyTwisterContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TiddyTwisterContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
