...
contract SuperMarx is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public whitelistMaxMint = 1000;
  uint256 public maxMintAmountPerTx = 10;
  uint256 public maxMintWalletLimit = 10;
  ...
  constructor() ERC721("Super Marx", "SM") {
      setHiddenMetadataUri("https://ipfs.io/ipfs/QmNn6aJT2WVckXPiFPE6wadYBkkhHRqDY72FQnjWFYmcXn/hidden.json");
  }
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(addressMintedBalance[msg.sender] + _mintAmount <= maxMintWalletLimit, "Max mint amount per wallet reached");
  ...
  function whitelistMint(uint _mintAmount, bytes32 leaf, bytes32[] memory proof) external payable mintCompliance(_mintAmount) {
    ...
    require(supply.current() + _mintAmount <= whitelistMaxMint, "Max whitelist mint amount reached");
  ...
  function setWhitelistMaxMint(uint256 _amount) public onlyOwner {
      whitelistMaxMint = _amount;
  }
  ...
  function setMaxMintAmountPerWallet(uint256 _maxMintAmountPerWallet) public onlyOwner {
    maxMintWalletLimit = _maxMintAmountPerWallet;
  }
