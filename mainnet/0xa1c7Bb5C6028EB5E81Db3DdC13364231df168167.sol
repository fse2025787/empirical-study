...
contract FETH_R12 is Ownable, IERC20, Lockable {
    ...
    function lockShares(uint256 shares) external {
        ...
        _shares[spender] = _shares[spender].add(shares);
        require(IERC20(_aEthContract).transferFrom(spender, address(this), shares), "can't transfer");
        emit Locked(spender, shares);
    }

    function lockShares(address spender, uint256 shares) external onlyGlobalPool {
    ...
    function lockSharesFor(address account, uint256 shares) external onlyGlobalPool {
        address spender = msg.sender;
