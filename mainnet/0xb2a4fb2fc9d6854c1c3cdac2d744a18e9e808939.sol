...
contract UniswapV2Exchange is OwnableUpgradeable, IExchangeAdapter {
    ...
    function __UniswapV2Exchange_init(address _router, address _wethAddress) external initializer {
        __Ownable_init();
    ...
    function swapExactInput(
        ...
        _transferAndApprove(_tokenIn, _amountIn);
        address[] memory _pairs = _defineSwapPath(_tokenIn, _viaPath, _tokenOut);
        return
            uniswapRouter.swapExactTokensForTokens(
                _amountIn,
                _amountOutMinimum,
    ...
    function _defineSwapPath(
        ...
        address[] memory _viaPath,
        address _tokenOut
    ) internal pure returns (address[] memory _pairs) {
