...
contract DigiMinersLottery is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function startRoundbyManager(uint256 startTime, uint256 endTime) public onlyAuth {
        require(block.timestamp > RoundDetails[currentRound()].endTime, "DIGI: Last Round has not ended");
        LotteryRounds++;
        RoundDetails[LotteryRounds] = LotteryRound(LotteryRounds, startTime, endTime, 0,0);
        setKey();
    ...
    function getTicketKey(uint256 ticketId) private view returns(uint256){
        return uint(keccak256(abi.encodePacked(msg.sender, ticketId, block.timestamp, block.difficulty, block.number))) % 10**7;
    }
    ...
    function getTicketHolding(address wallet, uint256 roundId) public view returns(TicketClaimed[] memory){
        ...
        for(uint256 i = 1; i< RoundDetails[roundId].ticketsSold;i++){
            address holder = TicketKey[roundId][i].holder;
            if(holder == wallet){
                tickets[index] = TicketClaimed(i, TicketKey[roundId][i].holder,TicketKey[roundId][i].winningOre, claimed[roundId][i] );
                index++;
            }
        }
        return tickets;
    ...
    function claimAllWinning(uint256 roundId, uint256[] calldata ticketIds) public nonReentrant callerIsWallet {
        _claim(roundId, ticketIds);            
    }

    function bulkClaim(BulkClaim[] calldata data) public nonReentrant callerIsWallet {
