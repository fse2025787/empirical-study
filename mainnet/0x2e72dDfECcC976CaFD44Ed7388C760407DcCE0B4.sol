...
contract MintersPass is IMintersPass, ERC721, Ownable {
    ...
    modifier onlyWithdrawalWallet() {
        require(address(withdrawalWallet) == _msgSender(), 
            "MintersSale: caller is not the withdrawal wallet");
        _;
    }
    ...
    function buy(uint256 _amount) external override payable {
        require(saleActive, "Minters::buy: Sale is not active.");
        require(_amount <= mintLimit, "Minters::buy: Too many tokens for one transaction.");
        require(msg.value >= mintPrice.mul(_amount), "Minters::buy: Insufficient payment.");

        if (!smartContractAllowance) {
            require(tx.origin == _msgSender(), "Minters::buy: Smart contracts are not allowed to buy.");
        }
    ...
    function setTicketAddress(address _ticketAddress) external onlyOwner override {
        require(_ticketAddress != address(0), "MintersSale::setTicketAddress: Invalid address.");
        ticketAddress = _ticketAddress;
        emit TicketAddressChanged(_ticketAddress);
    }

    function presaleBuy(uint256 _ticketId, uint _amount) external payable override {
        require(presaleActive, "Minters::presaleBuy: Presale is not Active.");
        require(ERC721(ticketAddress).ownerOf(_ticketId) == _msgSender(), "Minters::presaleBuy: invalid ticket.");
        require(!usedTickets[_ticketId], "Minters::presaleBuy: Ticket already used.");
        require(_amount <= mintLimit, "Minters::presaleBuy: Too many tokens for one transaction.");
        require(msg.value >= mintPrice.mul(_amount), "Minters::presaleBuy: Insufficient payment.");
    ...
    function whitelistBuy(uint256 _amount) external payable override {
        require(whitelistSaleActive, "MintersSale::whitelistBuy: Whitelist Buy is not Active.");
        require(_amount <= mintLimit, "Minters::presaleBuy: Too many tokens for one transaction.");
        require(msg.value >= mintPrice.mul(_amount), "Minters::presaleBuy: Insufficient payment.");
        require(whitelist[_msgSender()], "Minters::whitelistBuy: You are not whitelisted."); 
    ...
    function clearTicket(uint256 _ticketId) external override onlyOwner{
        require(usedTickets[_ticketId], "Minters::clearTicket: Ticket is not used");
