...
contract ABBLegacy is MintpassValidator, ERC721A, ERC2981, Ownable {
    ...
    address internal fiatGateway = 0x1eF724143Dd535aF2a6C62a4ffAd8Bd9CcA36064;
    ...
    function withdraw() external onlyBeneficiary {
        require(shares[msg.sender] > 0, "ABB Legacy: 0 Balance");
    ...
    /**
     * @dev Fiat Gateway Mint
     *
     * @param minter user that should receive the token
     * @param quantity amount of tokens to be minted
     *
     * Requirements:
     * - caller is fiat gateway
     * - tokens are not sold out
     */
    function fiatMint(address minter, uint256 quantity) public {
        require(
            TOKEN_LIMIT >= totalSupply() + quantity,
            "ABB Legacy: Token Limit"
        );
        require(fiatGateway == msg.sender, "ABB Legacy: Not allowed");
    ...
    function redeemBottle(uint256 tokenId) public payable {
        require(redeemingEnabled == true, "ABB Legacy: Not enabled");
    ...
    function allowlistMint(
        ...
        require(allowlistMintEnabled == true, "ABB Legacy: Not Enabled");

        uint256 finalMintPrice = (mintPrice() * quantity * 199) / 200; // 0.5% treshold due to conversion rate swings

        if (mintpass.tier == 2) {
            finalMintPrice = (discountMintPrice() * quantity * 199) / 200; // 0.5% treshold due to conversion rate swings
        }

        require(msg.value >= finalMintPrice, "ABB Legacy: Insufficient Amount");
