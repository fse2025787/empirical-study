...
// ----------------------------------------------------------------------------------------------------------
// 'ByDzyne Dollar A fully dencetralized and autonomous parity coin for USD liquidity and Merchant adaptation'
//
...
// Decimals    : 4
// Website     : https://paritydollar.xyz ; https://sustainabilitycreative.solutions
// 
...
//*******************************************************************//
//---------- Contract to Manage ByDzyne Dollar Ownership ------------//
//*******************************************************************//
...
contract ByDzyneDollar is owned {
    ...
    string constant private _name = "ByDzyne Blockchain Dollar";
    string constant private _symbol = "USBD";
     ...
     */
    function issueByDzyneDollar(address StablecoinAddress, uint256 AmountOfByDzyneDollars)  external returns(string memory) {
        ...
        require(AmountOfByDzyneDollars <= maxTransactionAmount, 'max Transaction Amount');
        ...
        uint256 StablecoinAmount = AmountOfByDzyneDollars * (10**StablecoinDecimals) / (10**_decimals);
        IERC20(StablecoinAddress).transferFrom(msgSender, address(this), StablecoinAmount);
        
        //issue DBC tokens
        _balanceOf[msgSender] = _balanceOf[msgSender].add(AmountOfByDzyneDollars);
        _totalSupply = _totalSupply.add(AmountOfByDzyneDollars);
        emit Transfer(address(0), msgSender, AmountOfByDzyneDollars);
        emit Mint(msgSender, AmountOfByDzyneDollars, DARSA_UAC, DARSA_UTC);
        
        return "ByDzyne Dollar minted successfully";
    }
     ...
     */
    function destroyByDzyneDollar(address StablecoinAddress, uint256 AmountOfByDzyneDollars) external returns (string memory) {
        ...
        _balanceOf[msgSender] = _balanceOf[msgSender].sub(AmountOfByDzyneDollars);  // Subtract from the sender
        _totalSupply = _totalSupply.sub(AmountOfByDzyneDollars);                      // Updates totalSupply
        emit Burn(msgSender, AmountOfByDzyneDollars, DARSA_UAC, DARSA_UTC);
        emit Transfer(msgSender, address(0), AmountOfByDzyneDollars);
        
        //send receiving tokens
        uint256 receivingStablecoinDecimals = IERC20(StablecoinAddress).decimals();
        uint256 receivingStablecoinAmount = AmountOfByDzyneDollars * (10**receivingStablecoinDecimals) / (10**_decimals);
        IERC20(StablecoinAddress).transfer(msgSender, receivingStablecoinAmount);
        
        return "ByDzyne Dollar Supply was destroyed successfully; Specified Balance as exchanged with Stablecoin  ";
    }
