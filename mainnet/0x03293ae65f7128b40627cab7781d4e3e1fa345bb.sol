...
contract DoodleMfers is ERC721A, Ownable{
	using Strings for uint256;
    ...
    address mferAddr = 0x79FCDEF22feeD20eDDacbB2587640e45491b757f;      //MferAddress

    mapping(address => bool) public mintlist;

    constructor() ERC721A("DoodleMfers", "dodmfers") {}
    ...
    function isHolder(address _wallet) public view returns (bool) {
        ...
        ERC721A mferToken = ERC721A(mferAddr);
        uint256 _mferBalance = mferToken.balanceOf(_wallet);

        return (_mferBalance > 0);
  }
	...
	function mint(uint256 _mintAmount) public payable{
        ...
        if(minted + _mintAmount > freeSupply){
		    require(msg.value >= cost * _mintAmount, "Not enough ETH");
            require(_mintAmount <= maxMint, "Too many" );
        }else if(minted + _mintAmount < freeSupply){
            require(_mintAmount <= maxFreeMint, "Too many" );
        }
	...
	function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
	    ...
	    return bytes(currentBaseURI).length > 0	? string(abi.encodePacked(currentBaseURI, (tokenId + 1).toString(), baseExtension)) : "";
	}

    //setters
    function setMferAddress(address _mferAddr) external onlyOwner {
        mferAddr = _mferAddr;
    }
