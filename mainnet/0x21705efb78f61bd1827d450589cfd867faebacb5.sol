...
contract CosmicLabsShop is ERC1155Burnable, Ownable {
    ...
    constructor(string memory _initBaseURI, address _yield) ERC1155(_initBaseURI)
    {
        ...
        for(uint256 i=1; i<4; i++)
        {
            ...
            _mint(0x83e02c9f0ec019f75d3B7E6Ac193109c9e7224EA, i, 1, "");
            _mint(0xb93329bB93F52362E97Fd5F953f98Ebed1438Cd8, i, 1, "");
            _mint(0x7b977283DE834A7e82d4f4F81bD8cc9267960459, i, 1, "");
            _mint(0x8147c04cb5c13b482820064e2BdD8A41Ab9A4B51, i, 1, "");
        }
    }   
...
contract CosmicPassRefund is  ERC1155Holder, Ownable {
   
    CosmicLabsShop public cPass = CosmicLabsShop(0x2E8349fe39326833DE048E63664F570a2A9C8904);

    CosmicUtilityToken public cut = CosmicUtilityToken(0x4e712d0a352a5cFc59B4e63A4abaB8c3C63a61ec);

    uint256 constant babyPassRefund = 500 ether;
    uint256 constant cosmicPassRefund = 1250 ether;
    uint256 constant genesisPassRefund = 2500 ether;

    function refundCosmicPass() public
    {
        require(cPass.balanceOf(msg.sender, 1) > 0 || cPass.balanceOf(msg.sender, 2) > 0 || cPass.balanceOf(msg.sender, 3) > 0);
        uint256 totalBabyPasses = cPass.balanceOf(msg.sender, 1);
        uint256 totalCosmicPasses = cPass.balanceOf(msg.sender, 2);
        uint256 totalGenesisPasses = cPass.balanceOf(msg.sender, 3);
        uint256 totalToRefund = 0;


        if(totalBabyPasses > 0)
        {
            totalToRefund += (babyPassRefund * totalBabyPasses);
            cPass.burn(msg.sender, 1, totalBabyPasses);
        }   
        if(totalCosmicPasses > 0)
        {
            totalToRefund += (cosmicPassRefund * totalCosmicPasses);
            cPass.burn(msg.sender, 2, totalCosmicPasses);
        }
        if(totalGenesisPasses > 0)
        {
            totalToRefund += (genesisPassRefund * totalGenesisPasses);
            cPass.burn(msg.sender, 3, totalGenesisPasses);
        }

        cut.transfer(msg.sender,totalToRefund);

    }

    function returnAllCut() public onlyOwner
    {
        cut.transfer(msg.sender, cut.balanceOf(address(this)));
    }
