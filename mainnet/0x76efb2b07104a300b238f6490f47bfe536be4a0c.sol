...
//import "../../3rdparty/openzeppelin-solidity/contracts/lifecycle/Pausable.sol";
...
contract TokenReceiver {
    ...
    function tokenApproveFallback( address from, uint256 value ) public;
}


contract CFX is IERC20, ERC20Pausable, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant MAXIMUM_TOTAL_SUPPLY = 1000000000 * 1e18; 
    ...
    function approve(address spender, uint256 value) public whenNotPaused  returns (bool) {
        _approve(msg.sender, spender, value);
        if( _trusted[spender] ){
                TokenReceiver(spender).tokenApproveFallback(msg.sender, value);
        }
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public whenNotPaused returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].add(addedValue));
        if( _trusted[spender] ){
                TokenReceiver(spender).tokenApproveFallback(msg.sender, _allowances[msg.sender][spender].add(addedValue));
        }
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public whenNotPaused returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].sub(subtractedValue));
        if( _trusted[spender] ){
                TokenReceiver(spender).tokenApproveFallback(msg.sender, _allowances[msg.sender][spender].sub(subtractedValue));
        }
        return true;
    }
     ...
     */
    function _mint(address account, uint256 amount) internal {
        require(amount > 0 && _totalSupply.add(amount) <= MAXIMUM_TOTAL_SUPPLY, "mint not allowed");
        require(account != address(0), "ERC20: mint to the zero address");
    ...
    function getTest() public pure returns (bool){
        return true;
    }

    function initialize() public {
        require(!_initialized, "Already initialized");
        address _owner = address(0x6aF20d9A1847D52C20639518819D51A456225aeB);
        initOwnership(_owner);
        _addPauser(_owner);
        _name = "DeFi Balanced ";
        _decimals = 18;
        _symbol = "DBA";
        _initialized = true;
        _mint(_owner, 300 * (10 ** uint256(decimals())));
    }
    ...
    function reclaimToken(IERC20 token, address _to) public onlyOwner returns(bool) {
