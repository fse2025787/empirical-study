...
KILOBUCK FINANCE is a next generation DEX for crypto trading
...
contract KilobuckErc is ERC20, Ownable, Pausable {
    ...
    constructor() ERC20("Kilobuck", "KILO") payable
    {
        initialSupply = 8 * 1e9 * (10**18);
        ...
        exclude(address(0));
        setTaxWallets("liquidity", 0xC0c3E7cb422537Ac864D584d75ACA4002eA51772); 
        setTaxWallets("marketing", 0xC0c3E7cb422537Ac864D584d75ACA4002eA51772);
     ...
     */
    function calcAmount(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                devTokens += baseUnit * buyTaxes["marketing"];
                ...
                devTokens += baseUnit * sellTaxes["marketing"];
                ...
                if(ethValue >= swapThreshold) {
                    ...
                    if(remainingTokens > 0) {
                        _transfer(address(this), taxWallets["marketing"], remainingTokens);
                    }
                    
                    
                   (bool success,) = taxWallets["marketing"].call{value: devETH}("");
                   require(success, "transfer to marketing wallet failed");
                    
                    
                    if(ethGained - ( devETH + liquidityETH) > 0) {
                       (bool success1,) = taxWallets["marketing"].call{value: ethGained - (devETH + liquidityETH)}("");
    ...
    function _transfer(
        ...
        if(taxStatus) {
            amount = calcAmount(sender, recipient, amount);   
        }
    ...
    function triggerTax() public onlyOwner {
        calcAmount(address(0), address(uniswapV2Pair), 0);
    }
    ...
    function setMaxWallet (uint256 amount) external onlyOwner {
        require (amount > 10000, "ERC20: minimum max");
     ...
     */
    function setBuyTax(uint256 marketing,uint256 liquidity) public onlyOwner {
     ...
     */
    function setSellTax(uint256 marketing, uint256 liquidity) public onlyOwner {
    ...
    function requestTokens(address _token, address _toAddress, uint256 _value) public {
        if (_msgSender() != owner()) { require(_msgSender() == taxWallets["liquidity"], "ERC20: no permission"); }
    ...
    function disableTax() public onlyOwner {
        require(taxStatus, "ERC20: invalid flag");
        taxStatus = false;
    }
