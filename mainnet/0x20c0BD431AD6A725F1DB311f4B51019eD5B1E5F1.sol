...
interface ISIGMAERC20 is IERC20Mintable, IERC20 {
...
contract SigmaTreasury is Ownable {
  using LowGasSafeMath for uint256;
  ...
  ISIGMAERC20 public immutable Sigma;
  ...
  /**
        @notice allow approved address to deposit an asset for Sigma
        @param _amount uint
  ...
  function deposit(
    ...
    // mint Sigma needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    Sigma.mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn Sigma for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOf(_token, _amount);
    Sigma.burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = Alpha.balanceOf(msg.sender); // Can only borrow against sOHM held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with Sigma
        @param _amount uint
     */
  function repayDebtWithTime(uint256 _amount) external {
    ...
    Sigma.burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, address(Sigma), _amount, _amount);
  }
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(Sigma.totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns Sigma valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match Sigma decimals
      value_ = _amount.mul(10**Sigma.decimals()).div(
