...
contract BlurBulls is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;

  mapping(address => bool) private _approvedMarketplaces;

  uint256 public cost = 0.002 ether;
  uint256 public max = 5555;
  ...
  modifier apesCompliance(uint256 _mintAmount) {
    require(!paused, "nft season has not started.");
    require(_mintAmount > 0 && _mintAmount <= txnMax, "Maximum of 10 nft per txn!");
    require(totalSupply() + _mintAmount <= max, "No nft lefts!");
    require(tx.origin == msg.sender, "No smart contract minting.");
    require(
      _mintAmount > 0 && numberMinted(msg.sender) + _mintAmount <= maxMintAmount,
       "You may have minted max number of nfts!"
    );
    _;
  }

  modifier apesPriceCompliance(uint256 _mintAmount) {
    uint256 realCost = 0;
  ...
  function mint(uint256 _mintAmount) public payable apesCompliance(_mintAmount) apesPriceCompliance(_mintAmount) {
    _safeMint(_msgSender(), _mintAmount);
  }
  
  function mintForAddress(uint256 _mintAmount, address _receiver) public onlyOwner {
    require(totalSupply() + _mintAmount <= max, "Max supply exceeded!");
