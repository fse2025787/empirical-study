...
contract TWAPBoundedUPUNKS1221 {
    using SafeMath for uint256;
    ...
    IERC20 internal constant UPUNKS = IERC20(
        0x37a572b95d3FB5007a3519e73D4e9D6e0fc9De50
    );
...
contract UPUNKS1221Farming is TWAPBoundedUPUNKS1221, UniHelper, YamSubGoverned {
    ...
    function _repayAndWithdraw() internal {
        UPUNKS.approve(address(minter), uint256(-1));
        SynthMinter.PositionData memory position = minter.positions(
            address(this)
        );
        uint256 upunksBalance = UPUNKS.balanceOf(address(this));
    ...
    function enter() public timeBoundsCheck {
        ...
        _mintLPToken(uniswap_pair, UPUNKS, WETH, mint_amount, RESERVES);
    ...
    function exit() public timeBoundsCheck {
        ...
        uint256 upunksBalance = UPUNKS.balanceOf(address(this));
        if (upunksBalance > 0) {
            UPUNKS.transfer(RESERVES, upunksBalance);
        }
        completed = true;
    }
