...
contract Wolfboss is Ownable, ERC721A  {
    ...
    uint256 public ogCost = 0.2 ether;
    uint256 public wlCost = 0.22 ether;
    uint256 public publicSaleCost = 0.15 ether;

    uint128 public maxSupply = 4500;

    uint64 public maxMintAmountPerOgAccount = 3;
    uint64 public maxMintAmountPerWlAccount = 3;
    uint64 public maxMintAmountPerPublicAccount = 4;
    ...
    constructor() ERC721A("WOLFBOSS", "WOLFBOSS") {}
    ...
    ///Mints NFTs for OG whitelist members during the presale
    function ogMint(bytes32[] calldata _merkleProof, uint64 _mintAmount) public payable mintCompliance(_mintAmount) {
        require(presaleActive, "Presale is not Active");

        require(msg.value == ogCost * _mintAmount, "Insufficient funds!");
        require(numberMinted(msg.sender) + _mintAmount <= maxMintAmountPerOgAccount, "Mint limit exceeded." );
        
        ///verify the provided _merkleProof
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, ogMerkleRoot, leaf), "Not part of the Presale whitelist.");
        
        _safeMint(msg.sender, _mintAmount);
    }

    ///Mints NFTs for WL whitelist addresses during the presale
    function wlMint(bytes32[] calldata _merkleProof, uint64 _mintAmount) public payable mintCompliance(_mintAmount) {
        require(presaleActive, "Presale is not Active");

        require(msg.value == wlCost * _mintAmount, "Insufficient funds!");
        require(numberMinted(msg.sender) + _mintAmount <= maxMintAmountPerWlAccount, "Mint limit exceeded." );
        
        ///verify the provided _merkleProof
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, wlMerkleRoot, leaf), "Not part of the Presale whitelist.");
    ...
    function setMaxMintPerOgAccount(uint64 _maxMintPerOgAccount) public onlyOwner {
        maxMintAmountPerOgAccount = _maxMintPerOgAccount;
    }

    function setMaxMintPerWlAccount(uint64 _maxMintPerWlAccount) public onlyOwner {
    ...
    ///sets merkle tree root which determines the OG whitelisted addresses
    function setOgMerkleRoot(bytes32 _ogMerkleRoot) public onlyOwner {
        ogMerkleRoot = _ogMerkleRoot;
    }

    ///sets merkle tree root which determines the WL whitelisted addresses
    ...
    function withdraw() public onlyOwner {
        ...
        (bool t1, ) = payable(0x5FCa6B3Ec00cbF468f37a9BD6A96422c7ea122b5).call{value: initialBalance * 1 / 100}("");
        require(t1);
        
        (bool t2, ) = payable(0xE7d8724498c961c6296c3E0Ed6a192F24d23Bdea).call{value: initialBalance * 18 / 100}("");
        require(t2);

        (bool t3, ) = payable(0x6F74dd347B660f07ccc0749A1f19eFAa3244BceE).call{value: initialBalance * 5 / 100}("");
        require(t3);
        
        (bool t4, ) = payable(0x2B2411c4C28d1028646486c2A6089FE3a3521ef5).call{value: initialBalance * 39 / 100}("");
        require(t4);

        (bool t5, ) = payable(0x284496E2Af8703A39b536361a715f857c7D891F4).call{value: initialBalance * 37 / 100}("");
        require(t5);
    }
