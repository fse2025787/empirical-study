...
/** Exchange rate between LyfeETH and BlocETH floats, you can convert your BlocETH for more LyfeETH over time.
    Exchange rate increases as the LYFE msig mints new LyfeETH corresponding to the staking yield and drops it into the vault (BlocETH contract).
    ...
    Someone must call syncRewards, which queues any new LyfeETH in the contract to be added to the redeemable amount.
    BlocETH adheres to ERC-4626 vault specs 
    Mint vs Deposit
    mint() - deposit targeting a specific number of BlocETH out
    deposit() - deposit knowing a specific number of LyfeETH in */
contract BlocETH is xERC4626, ReentrancyGuard {
    ...
    constructor(ERC20 _underlying, uint32 _rewardsCycleLength)
        ERC4626(_underlying, "Staked Lyfe Ether", "BlocETH")
