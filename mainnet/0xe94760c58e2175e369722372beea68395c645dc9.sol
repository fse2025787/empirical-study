...
contract FLOV is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public maxWalletLimit = true;
    ...
    constructor() ERC20("Valentine Floki", "FLOV") {

        address newOwner = address(0x88061b88e6CB9d03805783371c4B6F359BD62D6d);
    	...
    	marketingWallet = address(0xc53BE9b3356775EF96503B39c53B388D5BC290D1); // set as marketing wallet
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
        ...
        require(swapTokensAtAmount >= totalSupply() * 1 / 100000, "Swap amount cannot be lower than 0.001% total supply.");
  	    require(swapTokensAtAmount <= totalSupply() * 5 / 1000, "Swap amount cannot be higher than 0.5% total supply.");
  	    return true;
  	}
    
    function setMaxWallet(uint256 newAmount) external onlyOwner returns (bool) {
        maxWallet = newAmount *(10**18);
        require(maxWallet >= totalSupply() * 5 / 1000, "Swap amount cannot be lower than 0.5% total supply.");
  	    require(maxWallet <= totalSupply() * 2 / 100, "Swap amount cannot be higher than 2% total supply.");
        return true;
    }

    function enableMaxWalletLimit() external onlyOwner {
        require(maxWalletLimit == false, "Already max wallet limit is enabled");
        maxWalletLimit = true;
    }

    function disableMaxWalletLimit() external onlyOwner {
        require(maxWalletLimit == true, "Already max wallet limit is disabled");
        maxWalletLimit = false;
    }
    
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        maxTransactionAmount = newNum * (10**18);
        require(maxTransactionAmount >= (totalSupply() * 5 / 1000), "Cannot set maxTransactionAmount lower than 0.5%");
    }
