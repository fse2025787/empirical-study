...
contract PresaleSettings is Ownable {
    ...
    address public force = 0x8E5a07DF28ec36877407aD373582a44E5d4Bf87b;
    
    constructor() public {
        SETTINGS.BASE_FEE = 20; // 2.0%
        SETTINGS.TOKEN_FEE = 20; // 2.0%
        SETTINGS.REFERRAL_FEE = 200; // 20%
        SETTINGS.ETH_CREATION_FEE = 5e16; // 0.05 eth
        SETTINGS.ETH_FEE_ADDRESS = msg.sender;
        SETTINGS.TOKEN_FEE_ADDRESS = msg.sender;
        SETTINGS.ROUND1_LENGTH = 10; // 552 blocks = 2 hours
        SETTINGS.MAX_PRESALE_LENGTH = 92736; // 2 weeks
    }
    ...
    function setForceAddress(address _address) external onlyOwner {
        force = _address;
    }
    ...
    function userHoldsSufficientRound1Token (address _user) external view returns (bool) {
        ...
        for (uint i = 0; i < earlyAccessTokensLength(); i++) {
          ...
          if(token == force) {
              if (IToken(token).getOwnedBalance(_user) >= amountHold) {
                return true;
            }
          } else {
