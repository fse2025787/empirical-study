...
contract Ownable is Context 
{
    ...
    constructor () {
        address msgSender = 0x6D8c4706569e90ab79dF4f39D67B1fB04D907F68;
        _owner = msgSender;
...
contract HydraToken is Context, IBEP20, LockToken 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x0C4691b2aA78dD7aaFC11754f2a30DB796f4F684);
    address payable public developmentAddress = payable(0x2138B6d4748A028A7F072802C8E0C1c3cCb8c49A);
    ...
    uint256 _saleDevelopmentFee = 40;
    uint256 _saleMarketingFee = 40;
    ...
    {
        uint256 __totalFee = _marketingFee.add(_developmentFee);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(contractTokenBalance); 
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 ethForMarketing = newBalance.mul(_marketingFee).div(__totalFee);
        uint256 ethForDevelopment = newBalance.sub(ethForMarketing);
        marketingAddress.transfer(ethForMarketing);
        developmentAddress.transfer(ethForDevelopment);
    ...
    {
        return _amount.mul(_developmentFee+_marketingFee).div(1000);
    }
    ...
    {
        _developmentFee = _saleDevelopmentFee;
        _marketingFee = _saleMarketingFee;
    }
    

    event updateBuyFee(uint256 totalFee, uint256 timestamp);
    function setAllBuyFeePercentages(uint256 developmentFee, uint256 marketingFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _developmentFee.add(_marketingFee);
    ...
    function setAllSaleFeePercentages(uint256 developmentFee, uint256 marketingFee) 
    external onlyOwner()
    {
        _saleDevelopmentFee = developmentFee;
    ...
    uint256 public _walletHoldingMaxLimit =  2_000_000 * 10**18;
