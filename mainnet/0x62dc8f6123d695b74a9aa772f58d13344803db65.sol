...
// mPOODL with Governance.
contract mPOODL is BEP20('Miniature Poodl', 'mPOODL') {
    uint256 public supplyCap = 100_000_000e18;
    
    constructor() public {
        _mint(address(0xbaC4e63Cd995B6F53f51433587384EE7a30e6722),10_000_000e18);
    }
    
    function mint(address _to, uint256 _amount) public onlyOwner {
        if(totalSupply().add(_amount) <= supplyCap){
            _mint(_to, _amount);
            _moveDelegates(address(0), _delegates[_to], _amount);
        }else if(totalSupply() < supplyCap) {
            _mint(_to, supplyCap.sub(totalSupply()));
            _moveDelegates(address(0), _delegates[_to], _amount);
        }
    }
