...
/**
  Base on ERC721Psi with some modification
  - Optimize transfers by making approval clearing implicit instead of emitting an event. (https://github.com/OpenZeppelin/openzeppelin-contracts/pull/3481)
...
contract ERC721PsiMod is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
        ...
        returns (uint) 
    {
        ...
        uint count;
        for( uint i = _startTokenId(); i < _currentIndex; ++i ){
            if(_exists(i)){
                if( owner == ownerOf(i)){
                    ++count;
                }
    ...
    function _safeMint(
        ...
        uint256 startTransferTokenId = _currentIndex;
        _mint(to, quantity);
        require(
            _checkOnERC721Received(address(0), to, startTransferTokenId, quantity, _data),
            "ERC721Psi: transfer to non ERC721Receiver implementer"
        );
    }
    ...
    function _mint(
        ...
        uint256 tokenIdBatchHead = _currentIndex;
     ...
     * @param startTransferTokenId uint256 the first ID of the tokens to be transferred
    ...
    function _checkOnERC721Received(
        address from,
        address to,
        uint256 startTransferTokenId,
        uint256 quantity,
        bytes memory _data
    ) private returns (bool r) {
        if (to.isContract()) {
            r = true;
            for(uint256 tokenId = startTransferTokenId; tokenId < startTransferTokenId + quantity; tokenId++){
     ...
     *
     * startTransferTokenId - the first token id to be transferred
