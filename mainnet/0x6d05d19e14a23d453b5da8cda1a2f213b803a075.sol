...
/**
 * Modification: remove token id -> index mapping as those are the same for our purpose
 *
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            require(tokenId == _tokensMinted);
            _tokensMinted++;
        } else if (from != to) {
            _removeTokenFromOwnerEnumeration(from, tokenId);
        }
        require(to != address(0)); // burning no longer supported
        if (to != from) {
