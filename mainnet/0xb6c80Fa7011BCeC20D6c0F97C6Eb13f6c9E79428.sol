...
/*
    GALAXY POOL contract of the Kosmos Staking Platform
    www.kosmos.marsxtoken.com

    MarsX - the first multiplanetary crypto
    wwww.marsxtoken.com
    
*/

contract GalaxyPool is Pausable, Ownable{
    
    using SafeMath for uint256;
    
    IBigBang public BigBang;
    address private bigBangAddress;
    ...
    constructor(
        ...
        BigBang = IBigBang(_bigBang);
        bigBangAddress = _bigBang;
        BigBang.addPool(
            _stakingContract,
            _rewardsContract,
            _tier1Fee,
    ...
    function addRewardsToThePool(uint256 _amount) external {
        IBigBang(BigBang).addRewardsToThePool(pid, address(this), _amount);
    }
    

    function stake(uint256 _amount, uint256 _tier) external {
        IBigBang(BigBang).stake(pid, msg.sender, address(this), _amount, _tier);
   
    }
    
    function unstake(address _user) external{
        uint256 totalDeposited = IBigBang(BigBang).getState(pid, _user)[0];
        ERC20(stakingContract).approve(bigBangAddress, totalDeposited);
        IBigBang(BigBang).unstake(pid, _user, totalDeposited);
    }
    
    function claimRewards() external{
        uint256 claimableRewards = IBigBang(BigBang).getState(pid, msg.sender)[2];
        ERC20(rewardsContract).approve(bigBangAddress, claimableRewards);
        IBigBang(BigBang).claimRewards(pid, msg.sender);
        this.unstake(msg.sender);
    }
    
    function getTotalValueStaked() external view returns(uint256){
        return ERC20(stakingContract).balanceOf(address(this));
    }
    
    function getStateByUser(address _user) external view returns(uint256[] memory){
        return  IBigBang(BigBang).getState(pid, _user);
    }

    function getTotalRewardsInThePool() external view returns(uint256) {
        return  IBigBang(BigBang).totalRewardsInThePool(pid, address(this));
    }
