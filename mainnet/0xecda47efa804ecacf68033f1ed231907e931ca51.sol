...
// Dependency file: contracts/handlers/ERC721/ERC721Handler.sol
...
contract ERC721Handler is IHandler, IERC721Receiver, ERC721HandlerStorage, Initializable, Ownable {
    ...
    function withdraw(address recipient, address tokenContract, uint256 tokenId) external override virtual onlyOwner {
...
// Root file: contracts/handlers/ERC721/OldERC721Handler.sol


pragma solidity 0.7.3;

// import "contracts/handlers/ERC721/ERC721Handler.sol";

interface OldERC721 {
    function transfer(address recipient, uint256 tokenId) external;
}

contract OldERC721Handler is ERC721Handler {
    using EnumerableSet for EnumerableSet.UintSet;

    constructor(address owner) ERC721Handler(owner) {}

    function withdraw(address recipient, address tokenContract, uint256 tokenId) external override onlyOwner {
        require(ownerOf(tokenContract, tokenId) == recipient, "OldERC721Handler: recipient address is not the owner of the token");
        OldERC721(tokenContract).transfer(recipient, tokenId);
        delete _tokenOwners[tokenContract][tokenId];
        _ownedTokens[tokenContract][recipient].remove(tokenId);
        delete _depositTimestamp[tokenContract][tokenId];
    }
