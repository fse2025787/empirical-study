...
contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, WhitelistChecker, ERC721EnumerableUpgradeable, IERC721ReceiverUpgradeable {
	...
	function setMultipleGen2Bonus(uint256[] memory values) external onlyOwner {
		doubleRewardNumerator = values[0];
		doubleRewardDenominator = values[1];
		tripleRewardNumerator = values[2];
		tripleRewardDenominator = values[3];
	}

	function editGen2BagIds(uint256 bagId, uint256 val) external onlyOwner {
		gen2BagIds[bagId] = val;
	}

	function editGenBagIds(uint256 bagId, uint256 val) external onlyOwner {
		genBagIds[bagId] = val;
	}

	function editGen2InBag(uint256 bagId, uint256[] memory val) external onlyOwner {
		uint256[] memory _val = val;
		bags[bagId].gen2TokenIds = _val;
	}
