...
contract MoonMferContract is Ownable, ERC721A, ReentrancyGuard {
  bool public  bSalesStart= true; 
  uint256 public  CollectionSize;
  uint256 public  ETHprice =   9000000000000000; 
  uint public MaxMintPerTx; 

 constructor(
    uint256 maxBatchSize_,
    uint256 TotalToken_
  ) ERC721A("MoonMfer", "MoonMfer", maxBatchSize_, TotalToken_) {
    MaxMintPerTx = maxBatchSize_;
    CollectionSize = TotalToken_; 
  }
  
  function MintTo(address _to,uint256 mintamount) external onlyOwner {        
    require(totalSupply() + mintamount <= CollectionSize, "Can not mint more than CollectionSize");
  ...
  function Mint(uint256 mintamount) external payable {
    ...
    require(totalSupply() + mintamount <= CollectionSize, "Can not mint more than CollectionSize");    
    require(mintamount > 0 && mintamount <= MaxMintPerTx, "Can not mint more than Max Mint Per Tx");
    require(msg.value >= ETHprice*mintamount, "Not paid enough ETH.");

    _safeMint(msg.sender, mintamount);
  } 

  function setCollectionSize(uint _TotalToken) external onlyOwner {
    CollectionSize = _TotalToken;
  }
