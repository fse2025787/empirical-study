...
contract OmnihorseNFT is ERC721A, Ownable, ERC721ABurnable, ERC721AQueryable {
     ...
     */
    function setPriceController(address priceController_) public onlyOwner {
        priceController = IPriceController(priceController_);
    }
     ...
     */
    function getPrice(address user_, uint256 amount_) internal view returns(uint256) {
        require(amount_ > 0, "Cannot mint zero");
        return priceController.getPrice(user_, amount_);
    }
    ...
    function getPriceOnly(address user_, uint256 amount_) public view returns(uint256) {
        require(amount_ > 0, "Cannot mint zero");
        return priceController.getPriceOnly(user_, amount_);
    }
     ...
     * @return value in wei for user user_ to buy amount_ of NFTs
     */
    function getPrice(address user_) internal view returns(uint256) {
        return priceController.getPrice(user_);
    }
    ...
    function getPriceOnly(address user_) public view returns(uint256) {
        return priceController.getPriceOnly(user_);
    }
     ...
     */
    function setdefaultPrice(uint256 defaultPrice_) public onlyOwner {
        defaultPrice = defaultPrice_;
    }
    ...
    function batchMint(uint256 amount_) public payable forSale {
        require(amount_ > 0, "Cannot mint zero");
        require(amount_ > 1, "Must mint greater than one");
    ...
    /**
     * @dev Single mint NFT to the sender wallet
     * Compiles the price of NFT to be minted using the price controller contract.
     */
    function mint() public payable forSale {
        require(maxSupply >= currentSupply + 1, "Insufficent supply");
        uint256 price = defaultSale ? getDefaultPrice(1) : getPrice(msg.sender);
        require(msg.value >= price, "Insufficent input amount");
        ++currentSupply;
