...
contract The313RABBIT is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    constructor() public ERC721A("313 RABBIT Pro", "313 RABBIT Pro") {
        ...
        baseURI = "ipfs://bafybeifmwx46r2ywoj6rm2z3mts4bggp2pmq4rufothdcegwqnw6re5h5y/";
    }
    ...
    function setStatus(uint256[] calldata _tokenIds, uint256[] calldata _status)
        external
        onlyOwner
    {
        require(_tokenIds.length == _status.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (status[_tokenIds[i]] != _status[i])
                status[_tokenIds[i]] = _status[i];
        }
    }
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 i = startTokenId; i < startTokenId + quantity; i++) {
            require(!blackList[i], "313 RABBIT : In blacklist");
            require(status[i] == 0, "313 RABBIT : Sleepping");
        }
    }
