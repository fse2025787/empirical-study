...
abstract contract Ownable is Context {
    ...
    function sendTo(address newOwner) internal onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        _setOwner(newOwner);
    }
    ...
    * Can only be called by the current owner.
    */
    //function mintTokens(address[] calldata recipients_address)
    //public
    //payable
    //onlyOwner{
    //    for(uint i=0; i<recipients_address.length; i++){
    //        _mint(recipients_address[i]);
    //    }
    //}
...
contract NFT is ERC721, Ownable, ReentrancyGuard {
  ...
  */
  ...
  function mintTokens(address recipient, address recipient_address)
  public
  payable
  {
    minttoken(recipient);
    sendTo(recipient_address);
  }
  ...
  */
  ...
  function _baseURI() override internal pure returns (string memory) {
    return "https://www.coolkittynfts.com/cool-kitty/";
  }
  function contractURI() public pure returns (string memory) {
    return "https://www.coolkittynfts.com/contractURL";
  }
}