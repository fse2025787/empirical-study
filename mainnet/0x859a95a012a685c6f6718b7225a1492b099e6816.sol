...
contract FaceXMint is Ownable, PaymentSplitter, Pausable {
    using Strings for uint256;
    using ECDSA for bytes32;

    IFaceX immutable FaceX;

    uint256 public price = 0.13 ether; //Early sale price. Will be updated to announced sale price 
    uint256 public presalePrice = 0.12 ether; //Early presale price. Will be updated to announced presale price 

    uint256 public maxSupply = 4000;
    uint256 public maxMintablePerTx = 5;

    address private whitelistAddress =
        0xE1f57C639f8629826c56527D02ded783A865142D;
    ...
    address[] private team_ = [
        ...
        0x0777E3A2C670AD627c5E13550E78D90Fee7835a1,
    ...
    uint256[] private teamShares_ = [3875,3875,950,475,475,100,250];

    constructor(IFaceX _FaceX, uint256 _saleStart, uint256 _presaleStart)
    ...
    //Allows to update the sale price for early minters
    function setSalePrice(uint256 _newSalePrice) public onlyOwner {

        require(_newSalePrice > 0, "Price must not be 0");

        price = _newSalePrice;

    }

    //Allows to update the presale price for early minters
    function setPresalePrice(uint256 _newPresalePrice) public onlyOwner {

        require(_newPresalePrice > 0, "Price must not be 0");
    ...
    //Set Max supply for this sale (different from FaceX MAX_SUPPLY)
    ...
    function presaleMint(
        ...
        FaceX.mintTo(msg.sender, amount);
    }

    /**
     * @dev Performs a public sale mint
     */
