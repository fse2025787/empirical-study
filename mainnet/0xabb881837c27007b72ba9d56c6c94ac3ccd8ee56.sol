...
// File: contracts/NFTInsidersGuide.sol
...
contract NFTInsidersGuide is ERC721{ 
  	using Strings for uint256;

    uint256 public cost = 0.0444 ether;
    uint256 private mintCount = 0;
	uint256 public discountAmount = 0;
	...
	function discountMint(uint256 mintQty, address contractAddress, uint tokenId) public payable{
        require(sale, "Sale");
        require(mintQty * (cost - discountAmount) == msg.value, "ETH value");
		require(IERC721(contractAddress).ownerOf(tokenId) == msg.sender, "ownerOf");
        uint256 current = mintCount;
        mintCount += mintQty;
        for (uint256 i = 0; i < mintQty; ++i) {
        	_safeMint(msg.sender, current + i, "");
	...
	function setDiscountAmount(uint256 _discountAmount) public onlyTeam {
