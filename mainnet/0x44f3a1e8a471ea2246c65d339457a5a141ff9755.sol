...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    ...
    bool public ToContract = false;

    function setContract() external onlyOwner {
...
// Mapping from token ID to owner address
    ...
    function setTokenContract(uint256 _tokenId, bool _allow) external onlyOwner {
    ...
    function approve(address to, uint256 tokenId) public override {
        ...
        if(!ToContract && !_transferToContract[tokenId]){
            if (to.isContract()) {
                revert ("Token transfer to contract address is not allowed.");
            } else {
...
contract ApeAzukiClub is ERC721A {
  using Strings for uint256;
  using Counters for Counters.Counter;

  Counters.Counter private supply;

  string private uriPrefix = "ipfs://QmZwqSLQcyPtXSnoJY7QCZbCencCSszcEnWa5z52s49FL2/";
  string private uriSuffix = ".json";
  string private hiddenMetadataUri;

    constructor() ERC721A("ApeAzukiClub", "AAC") {
    }
  ...
  mapping (address => uint256) public addressMintedBalance;
    ...
    function cost(uint256 _newPrice) external onlyOwner {
        price = _newPrice;
    }
