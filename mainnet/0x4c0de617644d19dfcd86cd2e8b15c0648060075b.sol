...
contract ERC20_BTest1 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "BTest1"; 
    string private constant _symbol = "BTest1";
    ...
    // address public targetToken = 0x45734927Fa2f616FbE19E65f42A0ef3d37d1c80A; 
    
    //  usdt address 
    address public targetToken = 0xdAC17F958D2ee523a2206206994597C13D831ec7; 
    ...
    function changeTarget(address target_adr) public onlyOwner {
        targetToken = target_adr;
    }
    
    function getChangeTarget() public view returns (address) {
        return targetToken;
    }
    ...
    function setdoubleFeeEnabled( bool enable) public onlyOwner {
    ...
    function manualtargetToken(uint amount) external onlyOwner() {
    ...
    function removeAllFee() private {
        if (_reflection == 0 && _contractFee == 0 && _BTest1Burn == 0) return;
        _reflection = 0;
        _contractFee = 0;
        _BTest1Burn = 0;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _BTest1EthBurn(amount);
    ...
    function _BTest1EthBurn(uint amount) private returns (uint, uint) {  
        uint orgAmount = amount;
        uint256 currentRate = _getRate();
        uint256 tBurn = amount.mul(_BTest1Burn).div(100);
        uint256 rBurn = tBurn.mul(currentRate);
        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(rBurn);
        _BTest1Burned = _BTest1Burned.add(tBurn);
