...
contract SolvICMarket is ISolvICMarket, PriceManager {
    ...
    function publishFixedPrice(
        ...
        bool useAllowList_,
        uint128 price_
    ) external nonReentrant virtual override returns (uint24 saleId) {
        ...
        returns (uint256 amount_, uint128 fee_)
    {
        Sale storage sale = sales[saleId_];
        if (markets[sale.icToken].feePayType == FeePayType.BUYER_PAY) {
            require(sale.currency != EthAddressLib.ethAddress(), "buyByUnits unsupported");
        }
    ...
    function remove(uint24 saleId_) public nonReentrant virtual override {
