...
// File: contracts/REDACTEDBondDepositoryRewardBased.sol
...
contract REDACTEDBondDepositoryRewardBased is Ownable {
  ...
  struct Bond {
    uint256 payout; // BTRFLY remaining to be paid
  ...
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(BTRFLY).totalSupply();
  ...
  function setOLYMPUSTreasury(address _newTreasury) external {
    require(
      msg.sender == OLYMPUSDAO || msg.sender == DAO,
      "UNAUTHORISED : YOU'RE NOT OLYMPUS OR REDACTED"
    );
