...
contract SOSVengers is ERC721Enumerable, Ownable {
  using SafeMath for uint256;

  uint constant public maxTokens = 5555;
  
	uint256 constant public mintPrice = 15555555 ether; 

  bool public mintStarted = false;
  uint public tokensMinted = 0;
  uint public batchLimit = 5;
  string public baseURI = "ipfs://QmXHrX2JVKqs29fQqyghBPzDvsaxgVmWYXQxsLTT3LvYWk/";

  IERC20 public sosToken;

  constructor(address sosTokenAddress) ERC721("SOSvengers", "SOSVGR") {
  ...
  function mint(uint tokensToMint) public payable {
    ...
    require(tokensMinted + tokensToMint <= maxTokens, "Too much tokens to mint");

    uint256 cost = tokensToMint.mul(mintPrice);
    ...
    for(uint16 i = 0; i < tokensToMint; i++) {
      uint256 id = tokensMinted + 1;
      _mint(msg.sender, id);
      tokensMinted += 1;
    }
  }
