...
contract Amaterasu is Context, ERC721Enumerable, Ownable {

    using SafeMath for uint256;

    uint256 public PRICE = 0 ether;

    uint256 public MAX_TOKENS = 0;

    bool public saleIsActive = false;

    constructor () ERC721("Amaterasu Society", "Amaterasu") {}

    function chageMaxTokens(uint256 max) public onlyOwner {
        MAX_TOKENS = max;
    }
    ...
    function reserve(uint256 numberOfTokens) public onlyOwner {
        if (MAX_TOKENS != 0) {  
            ...
            require(saleIsActive == false, "Impossible to reserve when the sale is active");
    ...
    function getTokenUriPage(uint256 from, uint256 to) public view returns(string[] memory) {
        string[] memory ret;
        if (to >= from) {
