...
// File contracts/PIFContractBase.sol
...
abstract contract PIFContractBase is Ownable, ReentrancyGuard, IERC721Receiver {
    enum SellMode {
        FixedPrice,
        Auction
    }
    ...
    constructor(address _owner, INftWhiteList _nftWhiteList, IPIFConfig _mrConfig) {
...
// File contracts/PIFMarketPlace.sol
...
contract PIFMarketPlace is PIFContractBase {
    constructor(
        address _owner,
        INftWhiteList _nftWhiteList,
        IPIFConfig _mrConfig
    ) PIFContractBase(_owner, _nftWhiteList, _mrConfig) {}
    ...
    function _processFee(uint256 price) internal override {
        uint256 fee = (price * feeRate) / FEE_RATE_BASE;
        payable(mrConfig.getRevenueAddress()).transfer(fee);
        revenueAmount += fee;
    }
