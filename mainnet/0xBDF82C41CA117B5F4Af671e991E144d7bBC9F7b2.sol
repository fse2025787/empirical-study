...
contract Splitter is IERC777Recipient, Initializable, Ownable {
    /*
    ...
    */
    ...
    function tokensReceived(
        ...
        require(msg.sender == dustContractAddress, "20");
        _splitDust(__amount);
    }
    ...
    function _splitDust(uint256 _amount) private {
        uint256 companyPayment = (companyShares * _amount) / 10000;
        _getDustContract().send(companyAddress, companyPayment, "");

        uint256 remaining = _amount - companyPayment;
        for (uint256 i = 0; i < wallets.length; i++) {
            _getDustContract().send(wallets[i], (remaining * shares[i]) / 10000, "");
        }
    }
