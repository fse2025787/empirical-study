...
abstract contract CurveVoterProxy is BaseStrategy {
    ...
    function switchDex(uint256 _id, address _dex) external onlyAuthorized {
        require(_dex == uniswap || _dex == sushiswap, "!dex");
        dex[_id] = _dex;
        _approveDex();
    }
...
contract Strategy is CurveVoterProxy {
    ...
    constructor(address _vault) public CurveVoterProxy(_vault) {
        ...
        gauge = address(0x6955a55416a06839309018A8B0cB72c4DDC11f15);
        keepCRV = 1000;

        _approveBasic();
        pathTarget = new address[](1);
    ...
    // >>> approve the target token on curve
