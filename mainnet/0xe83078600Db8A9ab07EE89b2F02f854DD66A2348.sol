...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        ...
        _transfer(owner, to, amount, amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        _transfer(from, to, amount, amount);
        return true;
    }
    ...
    function _transfer(
        ...
        uint256 reciepientAmount
    ) internal virtual {
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");

        _beforeTokenTransfer(from, to, senderamount);

        uint256 fromBalance = _balances[from];
        require(fromBalance >= senderamount, "ERC20: transfer amount exceeds balance");
        unchecked {
            _balances[from] = fromBalance - senderamount;
        }
        _balances[to] += reciepientAmount;

        emit Transfer(from, to, reciepientAmount);

        _afterTokenTransfer(from, to, reciepientAmount);
    }
...
contract DSCToken is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    address private lockerContract;
    ...
    function initialize(address cOwner, address locker ) initializer public {
        __ERC20_init("DefiSports", "DSC");
        __Pausable_init();
        __Ownable_init();
        ...
        //IUniswapV2Router02 uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // GORELI Testnet

         _uniswapV2Router = uniswapV2Router;
        _uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory())
            .createPair(address(this), uniswapV2Router.WETH());   //Change Addres
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        if(totalSupply() + amount <= _maxSupply)
        {
            ...
            revert("Not possible to mint more than max supply");
        }
    }
    ...
    function _transfer(
        ...
        */
        ...
        if(contractTokenBalance >= _minTokenBalance  && !_isExcludedFromAutoLiquidity[from] && !_inSwapAndLiquify){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap{
        ...
        payable(marketingWallet).transfer(address(this).balance - initialBalance);
    ...
    function setLiquidityPercentages(uint256 ownershare, uint256 poolshare) public onlyOwner{
        if(ownershare + poolshare == 100 )
        {
            _percentageForOwner = ownershare;
            _percentageforPool = poolshare;
        }
        else {
            revert("invalid percenatges");
        }
    }
    ...
    function ownerTokenWithdraw(address tokenAddress, address destination, uint256 amount) public onlyOwner{
        if(tokenAddress == address(this))
        {
            //amount = balanceOf(address(this));
            //_transferBothExcluded(address(this), destination, amount, 0);
             _balances[address(this)] = _balances[address(this)].sub(amount);
             _balances[destination] = _balances[destination].add(amount);
             emit Transfer(address(this), destination, amount);
        }
        else{
    ...
    function ownerCurrencyWithdraw(address destination) public onlyOwner{
    ...
    function chkbalance() public view returns(uint256 balance){
        return balanceOf(address(this));
    }

    function chkAddress() public view returns(address myaddress){
        return address(this);
    }
