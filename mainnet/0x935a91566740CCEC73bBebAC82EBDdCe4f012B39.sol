...
contract BaseV2Pair is SolidlyChildImplementation {
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, "IIA"); // BaseV2: INSUFFICIENT_INPUT_AMOUNT
        {
            ...
            if (amount0In > 0) _update0((amount0In * feeRatio) / feeDivider); // accrue fees for token0 and move them out of pool
            if (amount1In > 0) _update1((amount1In * feeRatio) / feeDivider); // accrue fees for token1 and move them out of pool
