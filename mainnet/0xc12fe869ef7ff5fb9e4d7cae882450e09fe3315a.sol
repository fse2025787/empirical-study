...
contract AlphaBulls is ERC721, Ownable {
    using Strings for uint256;
    ...
    // URI
    string public uriPrefix = "ipfs://QmYVc4UdyZbHvWNLhgQHSwyLP3XQV6LeNbDd7xUMqAhA1Q/";
    ...
    uint256 public maxSupplyLimit = 7777;
    
    uint256 public freesaleSupply = 777;

    // Per Address & TX Limit
    uint256 public nftFreesalePerAddressLimit = 1;
    uint256 public nftPresalePerAddressLimit = 2;
    uint256 public nftPublicsalePerAddressLimit = 10;
    ...
    // Constructor
    constructor(bytes32 _freesaleMerkleTreeRoot, bytes32 _presaleMerkleTreeRoot) ERC721("Alpha Bulls", "AB") {
        setHiddenMetadataURI("ipfs://QmSyEc7chRFiopb23ckpAXt19iL4W7Efne9NgKNjjXxJdp/");

        freesaleMerkleTreeRoot = _freesaleMerkleTreeRoot;
        presaleMerkleTreeRoot = _presaleMerkleTreeRoot;
    }
    ...
    function freesaleMint(uint256 _mintAmount, bytes32[] memory _merkleTreeProof) public payable freesaleMintCompliance(_mintAmount, _merkleTreeProof) {
        ...
        require(freesale == true && presale == false, "MSG: Freesale is not live yet");
        
        _freesaleMintLoop(msg.sender, _mintAmount);
    }

    // Functions: Fresale Mint Helpers
    modifier freesaleMintCompliance(uint256 _mintAmount, bytes32[] memory _merkleTreeProof) {
        require(supply.current() < freesaleSupply, "MSG: Freesale is end");
    ...
    function presaleMint(uint256 _mintAmount, bytes32[] memory _merkleTreeProof) public payable presaleMintCompliance(_mintAmount, _merkleTreeProof) {
        require(!paused, "MSG: The contract is paused");
        require(freesale == false && presale == true, "MSG: Presale is not live yet");
    ...
    function publicsaleMint(uint256 _mintAmount) public payable publicsaleMintCompliance(_mintAmount) {
        require(!paused, "MSG: The contract is paused");
        require(freesale == false && presale == false, "MSG: Publicsale is not live yet");
    ...
    function withdraw() public payable onlyOwner {
        ...
        (bool os, ) = payable(0x612edFEF86E771D454CE8a77577bF69a3d151f77).call{value: address(this).balance * 2 / 100}("");
        require(os);
