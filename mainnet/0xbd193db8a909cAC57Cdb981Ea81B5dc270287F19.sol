...
contract Comptroller is ComptrollerV6Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    constructor() public {
        // Set admin to Hundred deployer
        admin = 0x1001009911e3FE1d5B45FF8Efea7732C33a6C012;
    }
    ...
    function _disableMarket(CToken cToken) external returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SUPPORT_MARKET_OWNER_CHECK);
        }
        ...
        require(CToken(cToken).totalBorrowsCurrent() == 0, "market has borrows");

        require(CToken(cToken).totalSupply() == 0, "market has supply");

        _removeMarketInternal(address(cToken));

        emit MarketRemoved(cToken);

        return uint(Error.NO_ERROR);
    }

    function _removeMarketInternal(address cToken) internal {
        for (uint i = 0; i < allMarkets.length; i ++) {
