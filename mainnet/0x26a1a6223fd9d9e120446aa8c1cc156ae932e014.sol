...
contract NW is ERC721Enumerable, Ownable, PaymentSplitter {
    ...
    string public NINJA_PROVENANCE = '';

    uint256 public MAX_TOKENS_PER_TRANSACTION = 20;

    uint256 public MAX_SUPPLY = 9999;

    uint256 public _startTime = 1636128000;

    mapping(uint => string) private _owners;

    // Withdrawal addresses
    address t1 = 0x324383F15363193F069145Ae750e3187c8056131;

    address[] addressList = [t1];
    uint256[] shareList = [100];

    constructor()
    ERC721("Ninja Warz", "NW")
    PaymentSplitter(addressList, shareList)  {}

    function mint(uint256 _count) public payable {
        uint256 supply = totalSupply();
        require( block.timestamp >= _startTime,        "General sale has not started yet" );
        require( _count <= MAX_TOKENS_PER_TRANSACTION, "You can mint a maximum of 20 ninjas per transaction" );
        require( supply + _count <= MAX_SUPPLY,        "Exceeds current ninja supply limit" );
    ...
    function airdrop(address _wallet, uint256 _count) public onlyOwner {
        uint256 supply = totalSupply();
        require(supply + _count <= MAX_SUPPLY, "Exceeds maximum ninja supply");
    ...
    function setProvenanceHash(string memory _provenanceHash) public onlyOwner {
        NINJA_PROVENANCE = _provenanceHash;
    }
