...
contract YWND is ERC721A, ReentrancyGuard, Ownable {
    ...
    function AlphaMint(uint256 _count) external onlyOwner payable nonReentrant {
	require( totalSupply() + _count <= MAX_SUPPLY, "Exceeds max supply" );
    ...
    function AlphaTransfer(address[] calldata _tos, uint256[] calldata _ids) external onlyOwner nonReentrant {
	require(_tos.length == _ids.length, "_tos and _ids lengths not equal");
	for (uint256 i = 0; i < _tos.length; i++) {
	    safeTransferFrom(msg.sender, _tos[i], _ids[i]);
	}
    }
