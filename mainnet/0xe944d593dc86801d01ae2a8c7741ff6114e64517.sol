...
(   )   \(''")/   \ |(''")/    \(   )(   ) | /    \(   )(   )/  _  \(   )   \ /    \   /  _  \  /  _  \   
 ...
 | |      | || '  | | | ||  '(   ) |  ; '| || '  | || |  ; '| |  `\ || |  | ||  .'.-. | |  `\ || |  `\ |  
 | |      | |' `-'  / | |'  `-' |' `-'  /| |'  `-' /' `-'  /; '._,' '| |  | |'  `-' / ; '._,' '; '._,' '  
(___)    (___)`.__,' (___)`.__,'  '.__.'(___)`.__.'  '.__.'  '.___.'(___)(___)`.__.'   '.___.'  '.___.'   
...
contract Ridiculousness is Ownable, ERC721A, DefaultOperatorFilterer  {

    using Strings for uint256;

    string private baseTokenURI;

    uint64 public publicMaxSupply = 222;
    uint64 public publicTotalSupply = 0;
    uint64 public presaleMaxSupply = 222;
    uint64 public presaleTotalSupply = 0;
    uint64 public maxMintPerAccount = 1;
    uint64 public maxSupply = 222;
    bool public presaleActive = false;
    bool public publicSaleActive = false;
    bytes32 private merkleRoot;


    constructor() ERC721A("Ye Be Trippin by Ridiculousness", "BADTRIP"){}
    ...
    function presaleMint(uint64 _mintAmount, bytes32[] memory _merkleProof) public payable mintCompliance(_mintAmount) {
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid proof!');
        require(presaleActive, "Presale is not Active");        
        require(numberMinted(msg.sender) + _mintAmount <= maxMintPerAccount, "Mint limit exceeded." );
