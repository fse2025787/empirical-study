...
contract MasterChef is Ownable {
    ...
    function updateBonusEndBlock(uint256 _block) public onlyOwner {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 xlessReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        xless.transfer(devaddr, xlessReward.div(10));
...
library EnumerableSet {
     ...
     * already present.
     */
    function add(Bytes32Set storage _set, bytes32 _value)
        internal
        returns (bool)
    {
        return _add(_set._inner, _value);
    }
...
contract xLessToken is ERC20, Ownable {
    constructor() public ERC20("xLessToken", "xLESS") {
        _mint(msg.sender, 56000000 * (10**uint256(decimals()))); // xLess Token Initial Supply 56m
    }
    ...
    /**
     * @dev notice Delegate votes from `msg.sender` to `delegatee`
    ...
    /**
     * @dev notice Delegates votes from signatory to `delegatee`
     * @param delegatee The address to delegate votes to
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "xLESS::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "xLESS::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "xLESS::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }

    /**
     * @dev notice Gets the current votes balance for `account`
    ...
    /**
     * @dev notice Determine the prior number of votes for an account as of a block number
        ...
        returns (uint256)
    {
        require(
            ...
            "xLESS::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying xLESSs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "xLESS::_writeCheckpoint: block number exceeds 32 bits"
        );
