...
contract FeeBoxETH is Verifier, AdapterBase {
    ...
    mapping(address => uint256) public wethBlance;
    ...
    function paymentCheck(address account, uint256 consumedAmount)
        ...
        onlySigner
    {
        require(wethBlance[account] >= consumedAmount);
        wethBlance[account] -= consumedAmount;
        ...
        onlyAdapterManager
    {
        ...
        require(
            wethBlance[account] + amount + msg.value >= consumedAmount,
            "Insolvent!"
        );
        ...
        onlyAdapterManager
    {
        ...
        require(wethBlance[account] >= consumedAmount + amount, "Insolvent!");
        wethBlance[account] = wethBlance[account] - amount - consumedAmount;
        if (isNative) {
