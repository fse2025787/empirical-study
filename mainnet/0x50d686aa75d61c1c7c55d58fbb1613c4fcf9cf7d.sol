...
contract  LIVINGKINGZ  is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;


    uint public constant _TOTALSUPPLY =1616;
    ...
    uint public reserve = 66;
    uint private startReserve=1551;
    uint private tokenId=1;
    address public developerWallet = 0xb40185164121A79A638A877b22042af58825cE0b;    
    address public communityWallet = 0xD72BCfD28d4285bE1d32C1DAE0c8F678146c2847;
    address public partnerWallet = 0x969B9a6707f7F81C560c4C28Fba45c1fb2501e8D;

    constructor(string memory baseURI) ERC721("Living Kingz Club", "LKC")  {
        setBaseURI(baseURI);
    }
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 ownerBalance = totalbalance * 60 / 100;
        uint256 partnerBalance = totalbalance * 30 / 100;
        uint256 developerBalance = totalbalance * 5 / 100;
        uint256 communityBalance = totalbalance * 5 / 100;
        payable(msg.sender).transfer(ownerBalance);
        payable(developerWallet).transfer(developerBalance);
        payable(communityWallet).transfer(communityBalance);
        payable(partnerWallet).transfer(partnerBalance);
    }
