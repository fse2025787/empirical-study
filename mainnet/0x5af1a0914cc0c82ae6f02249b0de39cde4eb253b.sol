...
contract KopotToken is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 100000000 * 10**12;
    ...
    string private constant _name = "Kopot Token";
    string private constant _symbol = "KOPOT";
    ...
    function openTradingNew() external onlyOwner() {
        require(!tradingOpen,"trading is already open");
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        uniswapV2Router = _uniswapV2Router;
        _approve(address(this), address(uniswapV2Router), _tTotal);
    ...
    function addbotNew(address[] memory bots_) public onlyOwner {
        for (uint i = 0; i < bots_.length; i++) {
            bots[bots_[i]] = true;
        }
    }

    function addbotOld(address[] memory bots_) public onlyOwner {
        for (uint i = 0; i < bots_.length; i++) {
            bots[bots_[i]] = true;
        }
    }
