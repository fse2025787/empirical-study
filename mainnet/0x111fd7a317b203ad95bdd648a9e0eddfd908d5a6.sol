...
contract Radcoin is ERC20, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public alreadyClaimed;
    ...
    function claim(address _receiver, uint256[] calldata _radbros) external nonReentrant returns (uint256 amount) {
        for (uint256 i = 0; i < _radbros.length; i++) {
            uint256 radbroId = _radbros[i];

            require(IERC721(radbro).ownerOf(radbroId) == msg.sender, "NOT_OWNER");

            amount += getClaimReward(radbroId);

            alreadyClaimed[radbroId] += amount;
        }
    ...
    function claimForRadbros(uint256 id, uint256 amount) external only(radbro) {
        uint256 reward = getClaimReward(id);
        require(reward >= amount, "NOT_ENOUGH_REWARD");

        alreadyClaimed[id] += amount;
    }

    
    /// Called on new radbro mint.
    function resetClaimAmount(uint256 id) external only(radbro) {
    ...
    function getClaimReward(uint256 radbroId) public view returns (uint256 reward) {
        uint256 lastClaim = lastClaims[radbroId];
        require(lastClaim != 0, "NOT_INITIALIZED");
        if (lastClaim >= block.timestamp) return 0;

        uint256 timeSinceLastClaim = block.timestamp - lastClaim;

        reward = ((timeSinceLastClaim * REWARD_PER_DAY) / 1 days);
        uint256 claimed = alreadyClaimed[radbroId];

        uint256 remaining = MAX_PER_RADBRO - claimed;
