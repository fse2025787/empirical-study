...
contract RubbishFriends is ERC2981, ERC721AQueryable, Ownable {
    ...
    uint32 public _maxFreeSupply = 10000;
    uint32 public _maxSupply = 10000;
    string _metadataURI = "ipfs://QmTDyGa3g8Fw4QLtYq2KNXArCDvpVysuJjS89QMdUczePJ/";
    ...
    constructor() ERC721A("RubbishFriends", "RF") {
        _setDefaultRoyalty(owner(), 500);
    }

    function mint(uint32 amount) external payable {
        require(amount > 0, "No 0 mints");
        require(amount + _totalMinted() <= _maxSupply, "Exceeds max supply");
        require(amount <= _txLimit, "Exceeds transaction limit");

        uint256 minted = _numberMinted(msg.sender);
        if (minted > 2 || _totalMinted() >= _maxFreeSupply) {
            require(msg.value >= amount * _price, "Insufficient funds provided");
        } else {
            require(msg.value >= (amount + minted - 2) * _price, "Insufficient funds provided");
        }
    ...
    function setMaxFreeSupply(uint32 maxFreeSupply) external onlyOwner {
        _maxFreeSupply = maxFreeSupply;
    }

    function cutSupply(uint32 maxSupply) external onlyOwner {
