...
contract CustomRender is Ownable {
    ...
    )
  {
    ...
    // console.log('themeMatch', themeMatch);
    uint8 skippedModels = 0;
    // uint8 matchedTheme = 0;
    uint256 firstNumber;
    ...
    for (uint256 i = 9; i <= 16; i++) {
      ...
      if (themeApplication < 80) {
        ...
        // if (selection[i] == themeMatch) {
        //   matchedTheme = matchedTheme + 1;
        //   // console.log('matchedTheme + 1', i, matchedTheme);
        // }
      } else {
        selection[i] = 999;
      }
      ...
      // console.log('themeset', i, selection[i]);
      if (selection[i] < 100) {
        themeUsed[selection[i]] = themeUsed[selection[i]] + 1;
        // console.log('themeUsed[selection[i]]', themeUsed[selection[i]]);
      }
    }
    ...
    return (selection, skippedModels, max);
  }
  ...
  function traits(uint256 tokenId) public view returns (string memory) {
    ...
    uint8 matchedTheme;
    uint256[18] memory selection;
    (selection, skippedModels, matchedTheme) = sel(tokenId);
    ...
    string memory firstHalf = string(
      abi.encodePacked(
        '{"Body": "',
    ...
    string memory secondHalf = string(
      abi.encodePacked(
        firstHalf,
        '", "Shoes": "',
        selection[8] > 100 ? none : shoesNames[selection[8]],
        '", "FloorTheme": "',
        selection[9] > 100 ? none : toString(selection[9]),
        '", "GlassesTheme": "',
        selection[10] > 100 ? none : toString(selection[10]),
        '", "GroundTheme": "',
        selection[11] > 100 ? none : toString(selection[11]),
        '", "HandTheme": "',
        selection[12] > 100 ? none : toString(selection[12])
      )
    );

    string memory thirdHalf = string(
      abi.encodePacked(
        secondHalf,
        '", "HeadTheme": "',
        selection[13] > 100 ? none : toString(selection[13]),
        '", "NeckTheme": "',
        selection[14] > 100 ? none : toString(selection[14]),
        '", "ShirtTheme": "',
        selection[15] > 100 ? none : toString(selection[15])
      )
    );
    ...
    return
      string(
        abi.encodePacked(
          thirdHalf,
          '", "ShoesTheme": "',
          selection[16] > 100 ? none : toString(selection[16]),
          '", "Sky": "',
          selection[17] > 100 ? none : toString(selection[17]),
          '", "Clean": "',
          toString(skippedModels),
          '", "Matching": "',
          toString(matchedTheme),
          '"}'
        )
      );
  }
  ...
  function tokenURI(uint256 tokenId) public view returns (string memory) {
    ...
    // current we've moved this out of tokenURI for the read gas limit.
    // we can revert back to that at any time if we need to
    // if (showHtml == true) {
