...
contract DontKwon is Context, IERC20, Ownable {
    ...
    string private constant _name = "Don't Kwon";           
    string private constant _symbol = "DONT";           

    uint8 private constant _decimals = 18;           

    uint256 private constant _totalSupply = 1e9 * (10**_decimals);           

    uint256 public maxBuyAmount = _totalSupply.mul(16).div(1000);           
    uint256 public maxSellAmount = _totalSupply.mul(16).div(1000);           
    uint256 public maxWalletAmount = _totalSupply.mul(16).div(1000);           

    uint256 public constant FEE_DIVISOR = 100;           

    uint256 public opsBuyFee = 6;           
    uint256 private _previousOpsBuyFee = opsBuyFee;           
    uint256 public opsSellFee = 6;           
    ...
    function _takeFees(address sender, uint256 amount, bool isSell) private returns (uint256) {
        ...
        if (_totalFees > 0) {
            ...
            _tokensForOps += fees * _totalFees / _totalFees;           
        }           
    ...
    function CONFIG_init() public onlyOwner {
    ...
    function CONFIG_MaxBuyAmount(uint256 maxBuy) public onlyOwner {
        require(maxBuy >= (totalSupply().mul(1).div(1000)), "Max buy amount cannot be lower than 0.1% total supply.");           
        maxBuyAmount = maxBuy;           
    }           

    function CONFIG_MaxSellAmount(uint256 maxSell) public onlyOwner {
        require(maxSell >= (totalSupply().mul(1).div(1000)), "Max sell amount cannot be lower than 0.1% total supply.");           
    ...
    function CONFIG_BuyFee(uint256 newOpsBuyFee) public onlyOwner {
        require(newOpsBuyFee <= 10, "Must keep buy taxes below 10%");           
        opsBuyFee = newOpsBuyFee;           
    }           

    function CONFIG_SellFee(uint256 newOpsSellFee) public onlyOwner {
        require(newOpsSellFee <= 10, "Must keep sell taxes below 10%");           
