...
contract Fiat24UsdcTopUp is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function topUpUSDCWithERC20(address tokenIn, uint256 amount) external returns(uint256) {
        ...
        TransferHelper.safeTransfer(USDC_ADDRESS, usdcTreasuryAddress, usdcAmount);
        emit UsdcTopUp(msg.sender, tokenIn, block.number, usdcAmount);
        return usdcAmount;
    }
    ...
    function topUpUSDCWithETH() external payable returns(uint256) {
       if(paused()) {
        ...
        emit UsdcTopUp(msg.sender, WETH_ADDRESS, block.number, usdcAmount);
        return usdcAmount;
    }
