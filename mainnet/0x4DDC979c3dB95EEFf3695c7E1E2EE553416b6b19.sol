...
contract YTYC is ERC721A, Ownable, ReentrancyGuard {
    string public baseURI = "ipfs://bafybeih3csonl2lzahpnan5dt3uce7qgn2f3y4sxjeghenlki2tfqhzkqq/";
    ...
    constructor() ERC721A("y00ts t00b Yacht Club", "YTYC"){}
    ...
    function mint(uint256 count) external payable {
        ...
        bool isFree = ((totalFreeMinted + count < totalFree + 1) &&
            (FreeMintAmount[msg.sender] < maxPerFree));
        ...
        if (isFree) { 
            ...
            if(count >= (maxPerFree - FreeMintAmount[msg.sender]))
            {
             require(msg.value >= (count * cost) - ((maxPerFree - FreeMintAmount[msg.sender]) * cost), "Please send the exact ETH amount");
             FreeMintAmount[msg.sender] = maxPerFree;
             totalFreeMinted += maxPerFree;
            }
            else if(count < (maxPerFree - FreeMintAmount[msg.sender]))
            {
             require(msg.value >= 0, "Please send the exact ETH amount");
             FreeMintAmount[msg.sender] += count;
