...
abstract contract StakeBaseUpgradeable is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    */
    function unstake(uint256[] calldata tokenIds_) external virtual {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            (uint256 index, bool owned) = indexOf(tokenIds_[i]);
            require(owned, "Stake: not owned by sender");
            _removeTokenOfOwnerAtIndex(msg.sender, index);
    ...
    */
    ...
    /**
    @notice Fetch the index of a tokenId owned that belongs to a user
    @param tokenId_ The tokenId to search for
    @return uint256 index
            bool owned by the user
    */
    function indexOf(uint256 tokenId_) internal view returns (uint256, bool) {
        for (uint256 i = 0; i < tokensOfOwner[msg.sender].length; i++) {
            if (tokenId_ == tokensOfOwner[msg.sender][i]) {
                return (i, true);
            }
        }
        return (0, false);
    ...
    /**
    @notice Removes the token of a user at an index
    @param owner_ Address of the token's owner
    @param index_ Index of the token to be removed
    */
    function _removeTokenOfOwnerAtIndex(address owner_, uint256 index_) private {
        require(owner_ != address(0), "Stake: owner cannot be zero address");
        tokensOfOwner[owner_][index_] = tokensOfOwner[owner_][tokensOfOwner[owner_].length - 1];
        tokensOfOwner[owner_].pop();
    }
    ...
    */
    function _unstake(uint256 tokenId_) private {
        require(tokenOwner[tokenId_] == msg.sender, "Stake: token not owned by sender");
        tokenOwner[tokenId_] = address(0);
        tokenContract.safeTransferFrom(stakingContract, msg.sender, tokenId_);
