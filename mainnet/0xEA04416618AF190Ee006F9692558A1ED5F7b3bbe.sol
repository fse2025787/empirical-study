...
contract BXHBridge is  Ownable, Pausable,ReentrancyGuard,EIP712 {
    ...
    bytes32 constant public LOCK_CALL_HASH_TYPE = keccak256("lock(address _tokenAddr,uint _toChainID, address _toAddr, uint256 _tokenAmount, uint256 _gasAmount)");

    /**
     * @dev Constructor.
     */
    constructor(address _signer) EIP712("BXHBridge", "1"){
        cSigner = _signer;
    }

    function lock(address _tokenAddr,uint _toChainID, address _toAddr, uint256 _tokenAmount, uint256 _gasAmount, uint8 v, bytes32 r, bytes32 s) public payable nonReentrant{
        require(_toAddr!=address(0x0),"address error");

        bytes32 digest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", 
            ECDSA.toTypedDataHash(_domainSeparatorV4(),
                keccak256(abi.encode(LOCK_CALL_HASH_TYPE, _tokenAddr, _toChainID,_toAddr,_tokenAmount,_gasAmount))
        )));
        require(ecrecover(digest, v, r, s) == cSigner, "BXHBridge: Invalid signer");

        require( msg.value == _gasAmount, "invalid gas amount");

        require(ERC20(_tokenAddr).balanceOf(msg.sender) >= _tokenAmount,"Not Enough token Balance");
        
        TransferHelper.safeTransferFrom(_tokenAddr,msg.sender,address(this), _tokenAmount);

        emit LockRequest(msg.sender,_tokenAddr,_toChainID,_toAddr, _tokenAmount);
    ...
    function pay(address _tokenAddr, address _toAddr, uint256 _amount) public onlyGovernor {
        require(IERC20(_tokenAddr).balanceOf(address(this)) > _amount, "Insufficient contract balance");
        require(maxPays[_tokenAddr] >= _amount,"max pay per tx quota exceed");

        TransferHelper.safeTransfer(_tokenAddr, _toAddr, _amount);
    }
