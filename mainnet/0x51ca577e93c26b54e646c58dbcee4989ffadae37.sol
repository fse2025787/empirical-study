...
//
//  TAX 0%
//  MAX WALLET 3% 30000000
//  LP Lock
//  CA Renounce
//
//  @MammonCommunity - free user for MAMMON ARMY
//
//  https://mammon.world/
//
...
//
//      "Mammon will watch over his loyal disciples"
//
//
...
contract FallenAngel is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "Fallen Angel";
    ...
    address payable private _developmentAddress = payable(0x23cED507Dd5450Ea448934A2f27f76766d98791c);
    ...
    uint256 public _maxTxAmount = 3e7 * 10**9; 
    uint256 public _maxWalletSize = 3e7 * 10**9; 
    uint256 public _swapTokensAtAmount = 5e5 * 10**9;
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                if (_buyMap[from] != 0 && (_buyMap[from] + (4 hours) >= block.timestamp)) {
                    _redisFee = 0;
                    _taxFee = 0;
                } else {
