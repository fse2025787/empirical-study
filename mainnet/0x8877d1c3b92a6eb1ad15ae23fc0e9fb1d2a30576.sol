...
contract MyToken is ERC20Interface, Owned, SafeMath {
    ...
    function () public payable{
        ...
        require(buyRate > 0);
        require(msg.value >= 0.05 ether && msg.value <= 1000 ether);
    ...
    // Constructor
    // ------------------------------------------------------------------------
    function MyToken(uint _buyRate,string _symbo1,string _name,uint _startTime,uint _endTime) public payable {
        require(_buyRate > 0);
    ...
    // set rate
    function setConfig(uint _buyRate,string _symbol,string _name,uint _startTime,uint _endTime) public onlyOwner {
        require(_buyRate > 0);        
        if(_buyRate > 0){
            buyRate = _buyRate;
        }
