...
contract DrOpium is IMerkleDistributor {
    ...
    // DrOpium
    uint256 public constant MAX_BONUS = 0.5e18;
    ...
    constructor(address token_, bytes32 merkleRoot_, uint256 _totalClaims, uint256 _initialPoolSize, uint256 _bonusStart, uint256 _bonusEnd, uint256 _emergencyTimeout, address _deployer) public {
        token = token_;
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override {
        ...
        require(IERC20(token).transfer(account, adjustedAmount), 'MerkleDistributor: Transfer failed.');
    ...
    function getBonus() public view returns (uint256) {
        ...
        uint256 timeRemaining = now > bonusEnd ? 0 : bonusEnd.sub(now);
        // bonus = maxBonus * timeRemaining / (bonusEnd - bonusStart)
        return MAX_BONUS.mul(timeRemaining).div(bonusEnd.sub(bonusStart));
    }

    function calculateAdjustedAmount(uint256 amount) public view returns (uint256 adjustedAmount, uint256 bonus, uint256 bonusPart) {
        ...
        uint256 adjustedPercentage = amount.mul(PERCENTAGE_BASE).div(initialPoolSize).mul(percentageIndex).div(PERCENTAGE_BASE);
    ...
    function _applyAdjustment(uint256 amount) private returns (uint256) {
        ...
        percentageIndex = percentageIndex.mul(currentPoolSize.mul(PERCENTAGE_BASE).div(newPoolSize)).div(PERCENTAGE_BASE);
    ...
    function emergencyWithdrawal() public {
        require(now > emergencyTimeout, 'TIMEOUT_NOT_EXPIRED');
