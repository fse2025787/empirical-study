...
// File: contracts/LiquidityMiningManagerKeeper.sol
...
contract LiquidityMiningManager is TokenSaver,KeeperCompatibleInterface  {
    using SafeERC20 for IERC20;
    ...
    function setLastDistribution(uint setTimestamp) external onlyGov {
        lastDistribution = setTimestamp;
    }
    
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        upkeepNeeded = (block.timestamp - lastDistribution) > 3 days;
    }

    function performUpkeep(bytes calldata) external override {
        if ((block.timestamp - lastDistribution) > 3 days ) {
            distributeRewards();
        }
    }
