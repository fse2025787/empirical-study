...
contract EtherOrcs is ERC721 {
    ...
    function _claim(uint256 id) internal noCheaters {
        ...
        if (action.action == Actions.TRAINING) {
            uint256 progress = (timeDiff * 3000 / 1 days ) + orcs[id].lvlProgress;
    ...
    function pillage(uint256 id, Places place, bool tryHelm, bool tryMainhand, bool tryOffhand) public ownerOfOrc(id) noCheaters {
        ...
        if (pool.cost > 0) {
            require(block.timestamp - startingTime > 16 days);
    ...
    function adjustTimestamp() external onlyOwner {
        activities[3189].timestamp = 1634537278;
        activities[4211].timestamp = 1634553323;
        activities[4263].timestamp = 1634542710;
        activities[4307].timestamp = 1634527428;
        activities[4323].timestamp = 1634544664; 
        activities[4335].timestamp = 1634532623; 
        activities[4338].timestamp = 1634525118;
        activities[4341].timestamp = 1634560388;
    }
    
    function manuallyAdjustOrc(uint256 id, uint8 body, uint8 helm, uint8 mainhand, uint8 offhand, uint16 level, uint16 lvlProgress) external {
        ...
        uint16 zugModifier_ = _tier(helm) + _tier(mainhand) + _tier(offhand);

        orcs[id].zugModifier = zugModifier_;
        
    }
    
    function burnZugFor(address user, uint256 amount) external {
        require(msg.sender == admin || auth[msg.sender], "not authorized");
        zug.burn(user, amount);
    }
    
    function mintZugFor(address user, uint256 amount) external {
        require(msg.sender == admin || auth[msg.sender], "not authorized");
        zug.mint(user, amount);
    }
