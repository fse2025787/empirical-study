...
/**
 * @title Loot Familiars (For Adventurers) contract
...
contract LootFamiliars is ERC721, ReentrancyGuard, Ownable {
    ...
    uint256 public publicPrice = 300000000000000000; //0.3 ETH
    uint256 public publicSaleOwnerRewards = 200000000000000000; //0.2 ETH
    ...
    constructor() ERC721("Familiars (for Adventurers)", "LootFamiliars") {
    }
    ...
    function mintWithLoot(uint lootId) external nonReentrant {
    ...
    function multiMintWithLoot(uint[] memory lootIds) external nonReentrant {
        require(privateSale, "Private sale minting is over");
        require(saleIsActive, "Sale must be active to mint");
    ...
    // Will mint all familiars of a owner available
    function claimAllForOwner() external nonReentrant {
        require(privateSale, "Private sale minting is over");
        require(saleIsActive, "Sale must be active to mint");
        
        uint256 tokenBalanceOwner = lootContract.balanceOf(msg.sender);
        require(tokenBalanceOwner > 0, "No tokens owned");

        // Will iterate over loots owned and mint available ones
        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            uint256 lootId = lootContract.tokenOfOwnerByIndex(msg.sender, i);
    ...
    function isAnythingClaimable(address owner) view external returns (bool) {
        uint256 tokenBalanceOwner = lootContract.balanceOf(owner);
        if (tokenBalanceOwner == 0) {
            // Nothing to mint
            return false;
        }
        
        // Will iterate over loots owned and check if can mint familiar
        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            uint256 lootId = lootContract.tokenOfOwnerByIndex(owner, i);
            if (!_exists(lootId)) {
                // Found a claimable familiar
                return true;
            }
        }
    ...
    // Anyone can airdrop familiars to their respective owners while the sale is private
    function airdrop(uint[] memory lootIds) external nonReentrant {
        require(privateSale, "Private sale minting is over");
        require(saleIsActive, "Sale must be active to mint");
        
        for (uint i=0; i<lootIds.length; i++) {
            address lootOwner = lootContract.ownerOf(lootIds[i]);
            require(!_exists(lootIds[i]), "One of these tokens has already been minted");
            _safeMint(lootOwner, lootIds[i]);
        }
    }

    //Public sale minting where 66% of the revenue goes to current loot owner
    function mint(uint lootId) external payable nonReentrant {
        ...
        // Mint familiar to msg.sender
        _safeMint(msg.sender, lootId);
        
        // 66% of revenue goes to current loot owner
        address payable lootOwner = payable(lootContract.ownerOf(lootId));
        lootOwner.transfer(publicSaleOwnerRewards);
    }
    
    //Public sale multi loot minting where 66% of the revenue goes to current loot owner
    function multiMint(uint[] memory lootIds) external payable nonReentrant {
        ...
        for (uint i=0; i<lootIds.length; i++) {
            ...
            // 66% of revenue goes to current loot owner
            address payable lootOwner = payable(lootContract.ownerOf(lootIds[i]));
            lootOwner.transfer(publicSaleOwnerRewards);
        }
    }
}