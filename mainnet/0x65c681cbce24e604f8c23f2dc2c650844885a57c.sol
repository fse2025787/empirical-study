...
contract Moar is ERC721A, Ownable {

    uint256 constant public MAX_MINT = 5555;
    uint256 constant public MAX_MINT_PER_TX = 2;
    uint256 public constant MAX_DIAMONDHANDMAX_SUPPLY = 1644;
    ...
    constructor() ERC721A("MOAR by Joan Cornella", "MOAR") {}

    function dutchAuctionMint(uint256 quantity) external payable {
    ...
    function diamondHandMint(uint256 quantity) external payable {
        require(!paused, "Sale is currently paused.");
        require(quantity > 0, "Mint quantity less than 0.");
        require(totalSupply() + quantity <= MAX_MINT, "Cannot exceed max mint amount.");
        require(quantity <= MAX_MINT_PER_TX, "Cannot exeeds 5 quantity per tx.");
    ...
    function whitelistMint(uint256 quantity) external onlyOwner {
        require(totalSupply() + quantity <= MAX_MINT, "Cannot exceed max mint amount");
