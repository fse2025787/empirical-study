...
contract StarNFT is ERC165, IERC1155, IERC1155MetadataURI, IStarNFT {
    ...
    modifier onlyGalaxyCommunity() {
        require(msg.sender == galaxyCommunity, "must be galaxy community");
        _;
    }
    /**
     * Only minter.
    ...
    // Contract owner, can be transferred to DAO afterwards
    address public owner;

    // Galaxy community address
    address public galaxyCommunity;
     ...
     */
    function initialize(address _owner, address _galaxyCommunity) external {
        require(!initialized, "Contract already initialized");
        require(_owner != address(0), "Owner must not be null address");
        require(_galaxyCommunity != address(0), "galaxyCommunity must not be null address");
        owner = _owner;
        galaxyCommunity = _galaxyCommunity;
    ...
    /**
      ...
      */
    //    function updatePowah(address _owner, uint256 id, uint256 powah) external onlyOperator override {
    //        require(isOwnerOf(_owner, id), "Must be owner");
    //
    //        emit PowahUpdated(id, _stars[id].powah, powah);
    //        _doSafePowahUpdatedAcceptanceCheck(_owner, id, _stars[id].powah, powah);
    //
    ...
    function transferGalaxyCommunity(address newGalaxyCommunity) external onlyGalaxyCommunity {
        ...
        galaxyCommunity = newGalaxyCommunity;
        emit GalaxyCommunityTransferred(galaxyCommunity, newGalaxyCommunity);
    }
    ...
    private
    {
        if (_owner.isContract() && _owner.supportsERC165()) {
            if (
                ...
                IStarUpdateListener(_owner).onQuasarUpdated(_id, _oldAmount, _newAmount);
            }
        }
    }
