...
contract EVMScriptRegistryConstants {
    ...
    */
    bytes32 internal constant EVMSCRIPT_REGISTRY_APP_ID =
...
interface IForwarder {
    ...
    function canForward(address sender, bytes evmCallScript)
        public
        view
        returns (bool);
...
contract Voting is IForwarder, AragonApp {
    ...
    bytes32 public constant CREATE_VOTES_ROLE =
        0xe7dcd7275292e064d090fbc5f3bd7995be23b502c1fed5cd94cfddbbdcd32bbc; //keccak256("CREATE_VOTES_ROLE");
    bytes32 public constant MODIFY_SUPPORT_ROLE =
        0xda3972983e62bdf826c4b807c4c9c2b8a941e1f83dfa76d53d6aeac11e1be650; //keccak256("MODIFY_SUPPORT_ROLE");
    bytes32 public constant MODIFY_QUORUM_ROLE =
        0xad15e7261800b4bb73f1b69d3864565ffb1fd00cb93cf14fe48da8f1f2149f39; //keccak256("MODIFY_QUORUM_ROLE");

    bytes32 public constant SET_MIN_BALANCE_ROLE =
        0xb1f3f26f63ad27cd630737a426f990492f5c674208299d6fb23bb2b0733d3d66; //keccak256("SET_MIN_BALANCE_ROLE")
    bytes32 public constant SET_MIN_TIME_ROLE =
        0xe7ab0252519cd959720b328191bed7fe61b8e25f77613877be7070646d12daf0; //keccak256("SET_MIN_TIME_ROLE")

    bytes32 public constant ENABLE_VOTE_CREATION =
        0xecb50dc3e77ba8a59697a3cc090a29b4cbd3c1f2b6b3aea524e0d166969592b9; //keccak256("ENABLE_VOTE_CREATION")
    bytes32 public constant DISABLE_VOTE_CREATION =
        0x40b01f8b31b51596de2eeab8c325ff77cc3695c1c1875d66ff31176e7148d2a1; //keccack256("DISABLE_VOTE_CREATION")

    uint64 public constant PCT_BASE = 10**18; // 0% = 0; 1% = 10^16; 100% = 10^18

    string private constant ERROR_NO_VOTE = "VOTING_NO_VOTE";
    string private constant ERROR_INIT_PCTS = "VOTING_INIT_PCTS";
    string private constant ERROR_CHANGE_SUPPORT_PCTS =
        "VOTING_CHANGE_SUPPORT_PCTS";
    string private constant ERROR_CHANGE_QUORUM_PCTS =
        "VOTING_CHANGE_QUORUM_PCTS";
    string private constant ERROR_INIT_SUPPORT_TOO_BIG =
        "VOTING_INIT_SUPPORT_TOO_BIG";
    string private constant ERROR_CHANGE_SUPPORT_TOO_BIG =
        "VOTING_CHANGE_SUPP_TOO_BIG";
    string private constant ERROR_CAN_NOT_VOTE = "VOTING_CAN_NOT_VOTE";
    string private constant ERROR_MALFORMED_CONTINUOUS_VOTE =
    ...
    modifier minBalanceCheck(uint256 _minBalance) {
        ...
        require(
            _minBalance >= minBalanceLowerLimit &&
                _minBalance <= minBalanceUpperLimit,
            "Min balance should be within initialization hardcoded limits"
        );
        _;
    }
    ...
    modifier minTimeCheck(uint256 _minTime) {
        require(
            _minTime >= minTimeLowerLimit && _minTime <= minTimeUpperLimit,
            "Min time should be within initialization hardcoded limits"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(
            _minBalance >= _minBalanceLowerLimit &&
                _minBalance <= _minBalanceUpperLimit
        );
        require(
            _minTime >= _minTimeLowerLimit && _minTime <= _minTimeUpperLimit
        );
    ...
    function changeSupportRequiredPct(uint64 _supportRequiredPct)
        ...
        authP(
            MODIFY_SUPPORT_ROLE,
            arr(uint256(_supportRequiredPct), uint256(supportRequiredPct))
        )
    {
        require(
            minAcceptQuorumPct <= _supportRequiredPct,
            ERROR_CHANGE_SUPPORT_PCTS
        );
    ...
    function changeMinAcceptQuorumPct(uint64 _minAcceptQuorumPct)
        ...
        authP(
            MODIFY_QUORUM_ROLE,
            arr(uint256(_minAcceptQuorumPct), uint256(minAcceptQuorumPct))
        )
    {
        require(
            _minAcceptQuorumPct <= supportRequiredPct,
            ERROR_CHANGE_QUORUM_PCTS
        );
        minAcceptQuorumPct = _minAcceptQuorumPct;
    ...
    function setMinBalance(uint256 _minBalance)
        external
        auth(SET_MIN_BALANCE_ROLE)
        minBalanceCheck(_minBalance)
    {
    ...
    function setMinTime(uint256 _minTime)
        external
        auth(SET_MIN_TIME_ROLE)
        minTimeCheck(_minTime)
    {
    ...
    function vote(
        ...
        bool _supports,
        bool _executesIfDecided
    ) external voteExists(_decodeData(_voteData, 0, MAX_UINT_128)) {
        ...
        if (yeaPct == 0 && nayPct == 0) {
            ...
            require(
                !_supports && yeaPct.add(nayPct) <= PCT_BASE,
                ERROR_MALFORMED_CONTINUOUS_VOTE
            );
        }
    ...
    function canForward(address _sender, bytes) public view returns (bool) {
        ...
        return
            canPerform(_sender, CREATE_VOTES_ROLE, arr()) &&
            canCreateNewVote(_sender);
    }
     ...
     */
    function canExecute(uint256 _voteId)
        public
        view
        voteExists(_voteId)
        returns (bool)
    ...
    function canCreateNewVote(address _sender) public view returns (bool) {
        return
            enableVoteCreation &&
            token.balanceOf(_sender) >= minBalance &&
            block.timestamp.sub(minTime) >= lastCreateVoteTimes[_sender];
    }
    ...
    function _newVote(
        ...
        require(canCreateNewVote(msg.sender), "NOT AUTHORIZED");
    ...
    function _vote(
        ...
        uint256 voterStake = uint256(2)
            .mul(balance)
            .mul(vote_.startDate.add(voteTime).sub(getTimestamp64()))
            .div(voteTime);
        ...
        vote_.voters[_voter] = yea == nay ? VoterState.Even : yea > nay
            ? VoterState.Yea
            : VoterState.Nay;
        ...
        returns (bool)
    {
        ...
        return
            _isVoteOpen(vote_) &&
            token.balanceOfAt(_voter, vote_.snapshotBlock) > 0;
    }
...
contract MiniMeToken is Controlled {
    ...
    function doTransfer(
        ...
        if (isContract(controller)) {
            ...
            require(
                ITokenController(controller).onTransfer(_from, _to, _amount) ==
                    true
            );
        }
        ...
        returns (bool success)
    {
        ...
        if (isContract(controller)) {
            // Adding the ` == true` makes the linter shut up so...
            require(
                ITokenController(controller).onApprove(
                    msg.sender,
                    _spender,
                    _amount
                ) == true
    ...
    function() external payable {
        ...
        // Adding the ` == true` makes the linter shut up so...
        require(
            ITokenController(controller).proxyPayment.value(msg.value)(
                msg.sender
            ) == true
        );
    }
...
library SafeERC20 {
    ...
    string private constant ERROR_TOKEN_BALANCE_REVERTED =
        "SAFE_ERC_20_BALANCE_REVERTED";
    string private constant ERROR_TOKEN_ALLOWANCE_REVERTED =
        "SAFE_ERC_20_ALLOWANCE_REVERTED";
    ...
    function staticTotalSupply(ERC20 _token) internal view returns (uint256) {
        bytes memory totalSupplyCallData = abi.encodeWithSelector(
            _token.totalSupply.selector
        );

        (bool success, uint256 totalSupply) = staticInvoke(
            _token,
            totalSupplyCallData
        );
        require(success, ERROR_TOKEN_ALLOWANCE_REVERTED);
