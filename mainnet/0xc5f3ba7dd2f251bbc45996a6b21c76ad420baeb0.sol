...
contract APECULT is ERC20, Ownable {
	IRouter public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "APE CULT";
	string private constant _symbol = "APE CULT";
	uint8 private constant _decimals = 18;

	APECULTDividendTracker public dividendTracker;

	bool public isTradingEnabled;
	uint256 private _tradingPausedTimestamp;

	// initialSupply
	uint256 constant initialSupply = 10000000000000 * (10**18);
	...
	constructor() ERC20(_name, _symbol) {
		...
		dividendTracker = new APECULTDividendTracker();
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "APECULT: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "APECULT: Account is already the value of 'excluded'");
	...
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "APECULT: Account is already the value of 'excluded'");
		_isExcludedFromMaxWalletLimit[account] = excluded;
		emit ExcludeFromMaxWalletChange(account, excluded);
	}
	function excludeFromTransactionTimingLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromTransactionTimingLimit[account] != excluded, "APECULT: Account is already the value of 'excluded'");
	...
	function setWallets(address newLiquidityWallet, address newDevWallet, address newBuyBackWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "APECULT: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(devWallet != newDevWallet) {
			require(newDevWallet != address(0), "APECULT: The devWallet cannot be 0");
			emit WalletChange('devWallet', newDevWallet, devWallet);
			devWallet = newDevWallet;
		}
		if(buyBackWallet != newBuyBackWallet) {
			require(newBuyBackWallet != address(0), "APECULT: The buyBackWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "APECULT: The router already has that address");
		emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
		uniswapV2Router = IRouter(newAddress);
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "APECULT: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "APECULT: Cannot update minimumTokensBeforeSwap to same value");
	...
	function _transfer(
		...
		uint256 amount
		    ...
		    if(from != owner() && to != owner()) {
				require(isTradingEnabled, "APECULT: Trading is currently disabled.");
				if (!_isExcludedFromMaxWalletLimit[to]) {
					require((balanceOf(to) + amount) <= maxWalletAmount, "APECULT: Expected wallet amount exceeds the maxWalletAmount.");
				}
				if (isBuyFromLp && !_isExcludedFromTransactionTimingLimit[to]) {
					require((block.timestamp - _lastTransactions[to]) > 120, "APECULT: Must wait 2mins after buying");
				}
				else if (isSelltoLp && !_isExcludedFromTransactionTimingLimit[from]) {
					require((block.timestamp - _lastTransactions[from]) > 120, "APECULT: Must wait 2mins after selling");
				}
				else if (!isSelltoLp && !isBuyFromLp && !_isExcludedFromTransactionTimingLimit[from]) {
					require((block.timestamp - _lastTransactions[from]) > 120, "APECULT: Must wait 2mins after transferring");
				}
				else if (!isSelltoLp && !isBuyFromLp && !_isExcludedFromTransactionTimingLimit[to]) {
					require((block.timestamp - _lastTransactions[to]) > 120, "APECULT: Must wait 2mins after transferring");
				}
			}
			...
			_lastTransactions[to] = block.timestamp;
			_lastTransactions[from] = block.timestamp;
...
contract APECULTDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("APECULT_Dividend_Tracker", "APECULT_Dividend_Tracker") {
		claimWait = 3600;
		minimumTokenBalanceForDividends = 0 * (10**18);
	}
	function _transfer(address, address, uint256) internal pure override {
		require(false, "APECULT_Dividend_Tracker: No transfers allowed");
	}
	function excludeFromDividends(address account) external onlyOwner {
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "APECULT_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "APECULT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "APECULT_Dividend_Tracker: Cannot update claimWait to same value");
