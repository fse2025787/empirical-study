...
// File: contracts/interfaces/IMasterBuilder.sol
...
// File: contracts/DexTokenVault.sol

pragma solidity 0.6.12;


contract DexTokenVault is Ownable, Pausable {
    ...
    constructor(
        ...
        treasury = _treasury;
        // Infinite approve
        IERC20(_token).safeApprove(address(_masterBuilder), uint256(-1));
    }
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.dexTokenAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    function calculateHarvestDexTokenRewards() external view returns (uint256) {
        uint256 amount = IMasterBuilder(masterBuilder).pendingDexToken(0, address(this));
     ...
     */
    function calculateTotalPendingDexTokenRewards() external view returns (uint256) {
        uint256 amount = IMasterBuilder(masterBuilder).pendingDexToken(0, address(this));
        amount = amount.add(available());
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IMasterBuilder(masterBuilder).userInfo(0, address(this));
...
contract VaultOwner is Ownable {
    ...
    DexTokenVault public immutable dexTokenVault;

    /**
     * @notice Constructor
     * @param _dexTokenVaultAddress: DexTokenVault contract address
     */
    constructor(address _dexTokenVaultAddress) public {
        dexTokenVault = DexTokenVault(_dexTokenVaultAddress);
    }
    ...
    function setAdmin() external onlyOwner {
        dexTokenVault.setAdmin(address(this));
    }
     ...
     */
    function setTreasury(address _treasury) external onlyOwner {
        dexTokenVault.setTreasury(_treasury);
    }
    ...
    function setPerformanceFee(uint256 _performanceFee) external onlyOwner {
        dexTokenVault.setPerformanceFee(_performanceFee);
    }
    ...
    function setCallFee(uint256 _callFee) external onlyOwner {
        dexTokenVault.setCallFee(_callFee);
    }
    ...
    function setWithdrawFee(uint256 _withdrawFee) external onlyOwner {
        dexTokenVault.setWithdrawFee(_withdrawFee);
    }
    ...
    function setWithdrawFeePeriod(uint256 _withdrawFeePeriod) external onlyOwner {
        dexTokenVault.setWithdrawFeePeriod(_withdrawFeePeriod);
    }
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        dexTokenVault.inCaseTokensGetStuck(_token);
        uint256 amount = IERC20(_token).balanceOf(address(this));
        IERC20(_token).safeTransfer(msg.sender, amount);
    }
