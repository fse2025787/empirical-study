...
// File: contracts/DID/logic_new.sol
...
contract dotbit is ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function initialize( address _owner, address _manager ) initializer public {
    ...
    function recycle(address uuid) external {
        ...
        require(expires[tokenId] + GRACE_PERIOD > block.timestamp, EXPIRES_MESS);
        address _ownerOf = ERC721Upgradeable.ownerOf(tokenId);
        require(_ownerOf == msg.sender, ONLY_OWNER_MESS);
        _burn(tokenId);
        expires[tokenId] = 0;
    ...
    function setContractOwner(address _contractOwner) external onlyRole(MANAGER_ROLE) {
