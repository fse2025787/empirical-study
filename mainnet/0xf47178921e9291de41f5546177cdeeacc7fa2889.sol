...
contract STPEstakeV1 is Ownable {

    using SafeMath for uint256;

    bool public paused = false;
    uint256 public MAX_WALLET_STAKED = 2500;
    uint256 public EMISSIONS_RATE = 1_000_000_000;
    uint256 public CLAIM_END_TIME = block.timestamp + 365 days;
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        require(
            stakerToTokenIds[msg.sender].length + tokenIds.length <=
                MAX_WALLET_STAKED,
            "Must have < required stake amount!"
        );
    ...
    function unstakeAll() public {
        ...
        if(totalRewards > 0)
            payable(address(msg.sender)).transfer(totalRewards);
    }
    ...
    function clearStuckEth() external onlyOwner{
        require(!(address(this).balance <= 0),"Nothing to clear");
    ...
    function claimByTokenId(uint256 tokenId) public {
        ...
        payable(address(msg.sender)).transfer(((block.timestamp - tokenIdToTimeStamp[tokenId]) *
                    EMISSIONS_RATE));
