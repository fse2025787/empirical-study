...
contract FreeBirdYachtClub is ERC721A, Ownable, ReentrancyGuard {
  ...
  ) ERC721A(_tokeName, _tokenSymbol) {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    require(_mintAmount <= maxPerAddressLimit, "max mint amount per session exceeded");
    require(_mintAmount+ _mintAmount <= maxSupply, "max NFT limit exceeded");
...
// OWNER MINT
  ...
  function withdraw() public onlyOwner nonReentrant {
    (bool hs, ) = payable(0x625735fC457099C5C99B44d2D92d294906781D5a).call{value: address(this).balance * 100 / 100}('');
    require(hs);
    }
}