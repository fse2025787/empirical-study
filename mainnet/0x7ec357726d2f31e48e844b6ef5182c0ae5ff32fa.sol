...
/*  
    Breadsticks might be unlimited but you need a way to buy appetizers
...
contract Breadsticks is Context, Ownable, ERC20 {
    // NFOG contract is available at https://etherscan.io/address/0xfee84e716845ec7423c863fc72881804f8bd8441
    address public oliveGardenContractAddress = 0xfEe84e716845ec7423c863fc72881804F8BD8441;
    IERC721 public oliveGardenContract;

    // Give out 10,000 Breadsticks for every Franchise that a user holds
    uint256 public breadsticksPerTokenId = 10000 * (10**decimals());

    // tokenIdStart of 1
    uint256 public tokenIdStart = 0;

    // tokenIdEnd of 4196
    uint256 public tokenIdEnd = 879;
    ...
    constructor() Ownable() ERC20("Breadstick", "BSTICK") {
        oliveGardenContract = IERC721(oliveGardenContractAddress);

        // We are a regional manager and are in charge of 10 franchises
        _mint(0x04AF00E4B004950DD569AdE00A4232af33C8E9CF, 100000 * (10**decimals()));
    }
    ...
    /// relevant in the event that NFOG migrates to a new contract.
    
    function daoSetOliveGardenContractAddress(address oliveGardenContractAddress_)
        external
        onlyOwner
    {
        oliveGardenContractAddress = oliveGardenContractAddress_;
        oliveGardenContract = IERC721(oliveGardenContractAddress);
    }
    ...
    /// want to mint 100 Breadsticks, you should enter "100" rather than the value of
    /// 100 * 10^18.
    function daoSetBreadsticksPerTokenId(uint256 breadsticksDisplayValue)
        public
        onlyOwner
    {
        breadsticksPerTokenId = breadsticksDisplayValue * (10**decimals());
    }

    
    /// in one transaction. This ensures that there is not a gap where a user
    /// can claim more Breadsticks than others
    ...
    /// breadsticks variables directly. It is better practice for security,
    ...
    function daoSetSeasonAndBreadsticksPerTokenID(
        uint256 season_,
        uint256 breadsticksDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetBreadsticksPerTokenId(breadsticksDisplayValue);
    }
}