...
abstract contract Plus is ERC20Upgradeable, IPlus {
    ...
    /**
     * @dev Moves tokens `amount` from `sender` to `recipient`.
     * Note: We do not rebase in the transfer as the rebase hook might cause error.
     * For example, if the rebase hook includes a call to sync() to Uniswap v2 pair,
     * it will trigger reentrancy protection on Uniswap v2 pair.
     */
