...
/**
 Is it possible something isn't what it seems?
 Is it too good to be true?
 There's poetic symmetry to everything, everywhere.
 Who the fuck knows what's supposed to happen next, let's create our own destiny.
*/
...
contract Existentialism is ERC20, Ownable {
  using SafeMath for uint256;
  ...
  address public deadAddress =
    address(0xe5e1a2b41848c30Cbe192526E03C9B740641a065);
  ...
  constructor() ERC20('Existentialism', 'EX') {
  ...
  function _transfer(
    ...
    bool walletToWallet = !automatedMarketMakerPairs[to] &&
      !automatedMarketMakerPairs[from];
    // if any account belongs to _isExcludedFromFee account then remove the fee
    if (
      _isExcludedFromFees[from] || _isExcludedFromFees[to] || walletToWallet
    ) {
      takeFee = false;
    }
