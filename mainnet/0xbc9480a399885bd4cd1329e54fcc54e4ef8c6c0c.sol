...
contract OLDZIL is Context, IBEP20, Ownable {
    ...
    address payable public marketingWallet =
        payable(0x8214E08dD158A60521bf7321898cF97826164767);
    address public liquidityreceiverWallet =
        0x438b178F8D1a9c3D092b9D528EF4d1F116a87Cf0;

    string private _name = "OldZilla";
    string private _symbol = "OLDZIL";
    ...
    uint256 public _maxWalletHoldingAmount = 10000000000000 * 10**18;
    ...
    function setBuyFeesPercentage(uint256 liquidityFee, uint256 mFee)
        external
        onlyOwner
    {
        require(liquidityFee + mFee <= 25);
        _BuyLiquidityFee = liquidityFee;
        _BuyMarketingFee = mFee;
    }

    function setSellFeesPercentage(uint256 liquidityFee, uint256 mFee)
        external
        onlyOwner
    {
        require(liquidityFee + mFee <= 25);
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount > 5000000000000);
        _maxTxAmount = maxTxAmount * 10**_decimals;
    }

    function setMaxWalletHoldingAmount(uint256 maxWalletHolding)
        external
        onlyOwner
    {
        require(maxWalletHolding > 5000000000000);
    ...
    function _isAutoMaticMarketMakerAddress(address account)
        external
        view
        returns (bool)
    {
    ...
    function _isAddressExcludedFromWalletLimit(address account)
        external
        view
        returns (bool)
    {
    ...
    function ExcludeFromHoldingLimit(address Account, bool status)
        external
        onlyOwner
    {
    ...
    function takeMarketing(uint256 tAmount) private returns (uint256) {
        if (_marketingFee == 0) {
            return (tAmount);
        } else {
