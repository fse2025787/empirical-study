...
contract Widdles is Ownable, ERC721A {

    uint256 public adoptionLimit = 30;
    uint256 public adoptionLimitPresale = 15;
    ...
    string private baseURI = "ipfs://QmSwkBHsSPuW81dZwGQ4YyTxAH5edhNNS7GfuuCMfozxi5/";
    string public _extension = ".json";
    string public hiddenMetadataUri = "ipfs://QmZgenSQShTA5yL74NSzn65PqErg3nCU82GBwJc685nqtZ/";
    uint256 public price = 0.025 ether;
    uint256 public WL_Price = 0.015 ether;
    uint256 public maxSupply = 5555;

    bool public revealed = false;    
    constructor() ERC721A("Widdles", "WW"){}
    ...
    function mintNFT(uint256 _quantity) public payable {
        ...
        _safeMint(msg.sender, _quantity + _quantity);

    }

    function mintPresale(uint256 _quantity) public payable {
        require(totalSupply() + _quantity < maxSupply, "Reaching max supply");
        ...
        require(_quantity > 0 && _quantity <= 5, "Wrong Quantity.");


        _safeMint(msg.sender, _quantity + _quantity + _quantity);

    }
    function freeClaim() public payable {
        require(totalSupply() + 3 < maxSupply, "Reaching max supply");
        require(isFreeMintList(msg.sender), "You are not allowed to claim for free.");
    ...
    function addFreeMintList(address[] memory _wallets) public onlyOwner{
        for(uint i; i < _wallets.length; i++)
            freeMintList[_wallets[i]] = true;
    }
    
    function isFreeMintList(address _sender) public view returns(bool){
        return freeMintList[_sender];
    }
    ...
    function sendMany(address _reciever, uint256 _count) external onlyOwner{
    ...
    function updateHiddenMetadata(string memory _newBaseURI) onlyOwner public {
