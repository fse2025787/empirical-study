...
contract StakingWallet is Initializable, ContextUpgradeSafe, OwnableUpgradeSafe {
    ...
    mapping(address => StakingBalance[]) public userStakingBalances;
    ...
    function userStakingBalancesLength(address _user) public view returns (uint256) {
        return userStakingBalances[_user].length;
    }
    ...
    function staking(address _beneficiary, uint256 _amount, uint256 _bonus, uint256 _durationDays) public onlyOwner {
        ...
        userStakingBalances[_beneficiary].push(StakingBalance(_amount, _bonus, now, now.add(_durationDays.mul(86400)), 0, true));

        emit Staked(_beneficiary, userStakingBalances[_beneficiary].length - 1, _amount);
    }
  
    function release(address _beneficiary, uint256 _index) public onlyOwner {
        require(_beneficiary != address(0), 'invalid beneficiary address');
        require(userStakingBalances[_beneficiary][_index].isActive, 'staking balance not active');
        require(userStakingBalances[_beneficiary][_index].endDate <= now, 'still on staking period');
        
        uint256 balanceReleased = userStakingBalances[_beneficiary][_index].amount;
        _releaseStakingBalance(_beneficiary, balanceReleased);
        totalStakingBalance = totalStakingBalance.sub(balanceReleased);
        emit Released(_beneficiary, _index, balanceReleased);
    }
  

    function transferBonus(address _beneficiary, uint256 _index, uint256 _bonusAmountReleased) public onlyOwner {
        require(userStakingBalances[_beneficiary][_index].totalBonusReleased.add(_bonusAmountReleased) <= userStakingBalances[_beneficiary][_index].bonusAmount, 'Exceed bonus amount');
        _transferBonus(_beneficiary, _bonusAmountReleased);
        userStakingBalances[_beneficiary][_index].totalBonusReleased = userStakingBalances[_beneficiary][_index].totalBonusReleased.add(_bonusAmountReleased);
        emit DailyBonused(_beneficiary, _index, _bonusAmountReleased);
    }
...
contract RajaCoin is Initializable, ContextUpgradeSafe, OwnableUpgradeSafe, ERC20UpgradeSafe, StakingWallet {
    using SafeMath for uint256;
    using Address for address;
    ...
    mapping(address => bool) public isBalancesMigrated;
    ...
    function __RajaToken_init(uint256 _cap) public initializer {
        __Context_init_unchained();
    ...
    function __RajaToken_init_unchained(uint256 _cap) internal initializer {

        require(_cap > 0, "RajaCoin: cap is 0");
        cap = _cap;
        _mint(_msgSender(), cap);
    }

    function _balanceOf(address account) internal view override returns (uint256) {
        if (isBalancesMigrated[account] || account == owner() || oldContract == address(0)) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (!isBalancesMigrated[sender] && sender != owner() && oldContract != address(0)) {
        ...
        if (!isBalancesMigrated[recipient] && recipient != owner() && oldContract != address(0)) {
     ...
     */
    function rajaTransfer(address sender, address recipient, uint256 amount, uint256 fee) public onlyOwner returns (bool) {
        require(feeReceiver != address(0), 'fee receiver address not registed');
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from != address(0)) { // When minting tokens
            require(balanceOf(from) >= amount, "RajaCoin: insufficient sender balance");
        }
    }
