...
contract CombMasterV3 is Initializable, OwnableUpgradeable {
    ...
    function setUserAdmin(
        ...
        updatePool(_pid);
        uint256 accCombPerShare = poolInfo[_pid].accCombPerShare;
        uint256 length = _users.length;
        for (uint256 i = 0; i < length; i++) {
            userInfo[_pid][_users[i]].amount = _amounts[i];
            userInfo[_pid][_users[i]].rewardDebt =
                (_amounts[i] * accCombPerShare) /
                1e12;
        }
    }
}
