...
contract ArityClaimV2_1 {
     ...
     * ----------------------------------------------------------------------------------------- Setters
     */
    function setAddresses(address[] memory _addresses, uint32[] memory _silverTokens, uint32[] memory _goldTokens) external onlyOwner {
        mappingVersion+=1;
        mappingLength = _addresses.length;
        for(uint32 i = 0; i < _addresses.length; i++){
            allUsersInfo2[mappingVersion][i] = UserInfo2(_addresses[i],_silverTokens[i],_goldTokens[i]);
        }
    }
     ...
     * ----------------------------------------------------------------------------------------- Getters
     */
    function getAlllUsersInfo() public view returns(UserInfo2[] memory){
        UserInfo2[] memory allInfo = new UserInfo2[](mappingLength);

        for (uint32 i = 0; i < mappingLength; i++) {
            allInfo[i] = allUsersInfo2[mappingVersion][i];
        }
