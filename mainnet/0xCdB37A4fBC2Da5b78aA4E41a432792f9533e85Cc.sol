 ...
 */
contract ERC20 is Context, IERC20, IHoldingTime, IBurnable {
    mapping (address => uint256) private _balances;

    mapping (address => mapping (address => uint256)) private _allowances;

    mapping (address => uint256) private _holdingTimes;
    ...
    /**
     * @dev See {IHoldingTime-balanceOf}.
     */
    function holdTimeOf(address account) external view returns (uint256) {
        return _holdingTimes[account];
    }
     ...
     *
     * Set `recipient` holding time (First reception only).
     * Set a new holding time for the `sender`.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_balances[recipient] == 0) {
            _holdingTimes[recipient] = block.timestamp;
        }
        _holdingTimes[sender] = block.timestamp;
