...
/**
 * @title Represents a factory of transparent proxies (ERC-1967)
 * @dev This contract can be used to deploy proxies at a predictable address in a self serve manner.
 */
contract TransparentProxyFactory is Ownable {
    /**
    ...
    /**
     * @notice Deploys a new proxy at a predictable address.
     * @dev The proxy is an ERC-1967. The salts are required for CREATE2.
     * @param adminSalt The salt for deploying the Proxy Admin.
     * @param proxySalt The salt for deploying the Transparent Proxy.
     * @param implementationAddr The implementation address (logic).
     * @param proxyOwnerAddr The owner of the Proxy Admin.
     * @param initData The initialization call, if any.
     */
    function deployProxy (bytes32 adminSalt, bytes32 proxySalt, address implementationAddr, address proxyOwnerAddr, bytes memory initData) external onlyOwner {
        ...
        require(implementationAddr != address(0) && implementationAddr != address(this) && implementationAddr != proxyOwnerAddr, "Invalid logic address");
        ...
        // Make sure the implementation was deployed to the blockchain
        require(implementationAddr.code.length > 0, "Implementation not found");

        // Deploy the proxy admin at a predictable address
        ...
        // Deploy the transparent proxy at a predictable address
        ...
        // Transfer ownership of the Proxy Admin to the expected party
    ...
    /**
     * @notice Gets the predictable address for deploying a transparent proxy.
     * @param adminSalt The salt for deploying the Proxy Admin.
     * @param proxySalt The salt for deploying the Transparent Proxy.
     * @param implementationAddr The implementation address (logic).
     * @param initData The initialization call, if any.
     * @return adminContractAddr The address of the Proxy Admin contract.
    ...
    function getDeploymentAddress (bytes32 adminSalt, bytes32 proxySalt, address implementationAddr, bytes memory initData) public view returns (address adminContractAddr, address proxyContractAddr) {
        // Calculate the address of the Proxy Admin
        ...
        // Calculate the address of the transparent proxy
