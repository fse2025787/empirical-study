...
/*
   Mover UniBright staking contract
   - allows users to stake their UBT token
   - would be used as aggregated UBT stake in baseledger
*/       	
contract MoverUBTStakePoolV2 is AccessControlUpgradeable, IMoverUBTStakePoolV2 {
    ...
    // user balances (this is NOT UBT, but portion in shares)
    ...
    function initialize(address _baseAsset) public initializer {
        ...
        // pool has virtual 0.01 unit of base asset to avoid
        // division by zero and reasonable starting share value calculation
        // UBT has 8 decimal points, so small 1e6 as a starting point should be enough
    ...
    function withdraw(uint256 _amount) public override {
        ...
        // NOTE: currently all staked funds should reside on this contract, no reclaim mechanics needed
    ...
    function performWithdraw(
        ...
        // we checked this regarding base asset (UBT) amount, just in case check for share amount
        require(
        ...
        // only perform this after all other withdraw flow complete to recalculate pool state
    ...
    ///////////////////////////////////////////////////////////////////////////
    // V2 upgrade: single validator node stake management/rebalancing support
    ///////////////////////////////////////////////////////////////////////////
    ...
    // set status, can disable / restrict invest proxy methods
    ...
    // UBT staking node invest/divest methods
    ...
    // used to get funds from UBT invest proxy for withdrawal (if current amount to withdraw is insufficient)
    function retrieveFunds(uint256 _amount) internal returns (uint256) {
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (
                ...
                // great, this node contract can provide funds
        ...
        // no single Node contract has enough safe amount to get funds from, sort and pick largest ones to save gas
        // as a reasonable empryric, number of active Nodes would be less than 10, so use reverse insertion sort
        ...
        // reclaim funds one by one (from sorted Node contract list)
