...
// File: contracts/CryptoYachts.sol
...
contract CryptoYachts is ERC721A, Ownable {
    ...
    */
    ...
    mapping(uint256 => mapping(address => bool)) public stagesPresaleWhitelist;
    ...
    constructor() ERC721A("CryptoYachts", "YACHT") {
        ...
        stagesTokenPrice[4] = 0.1 ether;
        stagesTokenPrice[5] = 0.1 ether;
        stagesTokenPrice[6] = 0.15 ether;

        stagesMaxTokensPerAddress[1] = 1;
        stagesMaxTokensPerAddress[2] = 10;
        stagesMaxTokensPerAddress[3] = 5;
        stagesMaxTokensPerAddress[4] = 3;
        stagesMaxTokensPerAddress[5] = 2;
        stagesMaxTokensPerAddress[6] = 99999; // unlimited
    ...
    function addToPresaleWhitelist(uint256 stage, address[] calldata entries) external onlyOwner {
        require(stage != 0, "No whitelist for before presale");
        require(stage < stageCount - 1, "No whitelist for public sale");
        for(uint256 i = 0; i < entries.length; i++) {
            stagesPresaleWhitelist[stage][entries[i]] = true;
        }   
    }

    function removeFromPresaleWhitelist(uint256 stage, address[] calldata entries) external onlyOwner {
        require(stage != 0);
        require(stage < stageCount - 1);
        for(uint256 i = 0; i < entries.length; i++) {
            stagesPresaleWhitelist[stage][entries[i]] = true;
        }   
    }
    ...
    function getMaxTokensAllowed(address purchaser) public view returns (uint256) {
        uint256 maxAllowedTokens = 0;

        if (isPublicSale()) {
            maxAllowedTokens = stagesMaxTokensPerAddress[currentStage];
        } else {
            for (uint256 i = 0; i <= currentStage; i++) {
                if (stagesPresaleWhitelist[i][purchaser]) {
                    if (stagesMaxTokensPerAddress[i] > maxAllowedTokens) {
                        maxAllowedTokens = stagesMaxTokensPerAddress[i];
                    }
                }
            }
        }

        uint256 purchasedTokens = purchased[purchaser];
        if (purchasedTokens >= maxAllowedTokens) {
            return 0;
        }
        uint256 maxTokensAllowedForAddress = maxAllowedTokens - purchasedTokens;
        maxAllowedTokens = min(maxAllowedTokens, maxTokensAllowedForAddress);

        if (soldAmount >= FOR_SALE_TOKENS) {
            return 0;
        }
        uint256 publicSaleTokensLeft = FOR_SALE_TOKENS - soldAmount;
    ...
    function isWhitelisted(address purchaser) public view returns (bool) {
        return stagesPresaleWhitelist[currentStage][purchaser];
    }
