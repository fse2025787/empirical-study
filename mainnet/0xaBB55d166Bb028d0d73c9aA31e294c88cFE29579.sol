...
contract GovernorAlpha {
    ...
    string public constant name = "BaconCoin Governor Alpha";

    
    function quorumVotes() public view returns (uint) { 
        uint256 baconTotalSupply = baconCoin.totalSupply();
        return baconTotalSupply.div(4); 
    } //= 25% of Bacon

    
    function proposalThreshold() public view returns (uint) { 
        uint256 baconTotalSupply = baconCoin.totalSupply();
        return baconTotalSupply.div(200); 
    } // = .5% of Bacon
    ...
    BaconCoinInterface public baconCoin;
    ...
    constructor(address timelock_, address baconCoin_, address guardian_) {
        timelock = TimelockInterface(timelock_);
        baconCoin = BaconCoinInterface(baconCoin_);
        guardian = guardian_;
    }

    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(baconCoin.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        require(proposalState == ProposalState.Active || proposalState == ProposalState.Pending, "GovernorAlpha: can only cancel Active or Pending Proposal");

        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || baconCoin.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = baconCoin.getPriorVotes(voter, proposal.startBlock);
