...
// File contracts/ArcMasterChef.sol

//** Arc MasterChef Contract */
//** Author [email protected] : Arc Finance 2022.02 */
//** Author [email protected] : Arc Finance 2022.02 */
//** Version 2.0.0 */
...
contract ArcMasterChef is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    ...
    // Arcs tokens created per block. 0.5 Arc per block. 10% to arc charity ( address )
    ...
    // The block number when Arcs mining starts.
    ...
    function initialize(
        address _arc, // art token address
        address _feeAddress, // ?
        address _rewardHolder, // ?
        uint256 _startBlock, // ?
        uint256 _arcPerBlock // ? reward per block
    ) public initializer {
        arc = IERC20(_arc);
    ...
    // Update the given pool's Arcs allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function pendingArc(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 arcReward = multiplier
                .mul(arcPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accArcPerShare = accArcPerShare.add(
                arcReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accArcPerShare).div(1e12).sub(
            user.rewardDebt
        );
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest Arcs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accArcPerShare = pool.accArcPerShare.add(
            arcReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Arcs allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);

        payOrLockupPendingArc(_pid);
        if (_amount > 0) {
    ...
    // Pay or lockup pending Arcs.
    function payOrLockupPendingArc(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeArcTransferFrom(rewardHolder, msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    function safeArcTransferFrom(
        address _from,
        address _to,
        uint256 _amount
    ) internal {
        uint256 arcBal = arc.balanceOf(rewardHolder);
        if (_amount > arcBal) {
            revert("Not enough balance");
        } else {
            arc.transferFrom(_from, _to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _arcPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, arcPerBlock, _arcPerBlock);
        arcPerBlock = _arcPerBlock;
    }
}