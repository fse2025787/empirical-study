...
// File: contracts/PoiPuppyLei.sol

pragma solidity 0.8.6;

contract PoiPuppyLei is ERC721, Ownable, ReentrancyGuard {
    bool public isActive;
    uint256 public constant MAX_SUPPLY = 1000;
    uint256 public constant PER_ORDER_LIMIT = 10;

    string public constant BASE_URI =
        "ipfs://QmepuZG5nNNgEjWBNBPJ4DXqsey1smPukU8Ht323DkKn83/";

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC721("Poi Puppy Lei", "PPL") {
        isActive = false;
    }

    function mint(uint256 qty) public payable nonReentrant {
        require(isActive, "Contract is not active");
        require(qty <= PER_ORDER_LIMIT, "Cannot purchase more than the per-order limit");
        require((totalSold() + qty) <= MAX_SUPPLY, "Requested quantity would exceed total supply");
    ...
    function salePrice() public pure returns (uint256) {
        return 2000000000000000 wei; // 0.0020 ether
    }
