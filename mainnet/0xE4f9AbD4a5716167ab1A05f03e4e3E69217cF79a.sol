...
// File: finncotton-hold-to-earn.sol
...
contract FinnCottonHoldToEarn is Context, ReentrancyGuard, IRoyaltySplitter,Ownable {
  ...
  function releaseBatch(uint256[] memory tokenIds) public virtual { 
    ...
    if (!COLLECTION.isOwnsAll(_msgSender(), tokenIds)) revert InvalidCall();
  ...
  function _pendingPayment(
    ...
    uint256 amount = totalReceived / 10000;
    if (amount < alreadyReleased) return 0;
    return amount - alreadyReleased;
  }
  
  function withdraw(address _to,uint256 amount) external onlyOwner nonReentrant { 
      payable(_to).transfer(amount);
  }  
 
  function withdraw(IERC20 token,address _to,uint256 amount) external onlyOwner nonReentrant { 
      SafeERC20.safeTransfer(token, payable(_to), amount);
  }    
