...
// File: contracts/AbstractERC1155.sol
...
contract Workwares is AbstractERC1155 {
    ...
    // presale mint
    function presalePurchase() external payable {
        require(presaleActive == 1, "Presale closed");
        require(whitelist[msg.sender] == 1, "Address not on whitelist");
        require(msg.value == presalePrice, "Incorrect payment");

        uint256 nonce = 0;
        uint256 mintIndex = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % 30;
        whitelist[msg.sender] = 0;
        numMints[msg.sender]++;
        
        if (mintIndex == 0 && numGoldMints < MAX_SUPPLY_GOLD) {
            numGoldMints++;
    ...
    function purchase() external payable {
        require(saleActive == 1, "Sale currently closed");
        require(numMints[msg.sender] < 10 , "max tx amount exceeded");
        require(numWhiteMints < MAX_SUPPLY_WHITE || numGoldMints < MAX_SUPPLY_GOLD, "Transaction would exceed maximum supply");
        require(msg.value == mintPrice, "Incorrect payment");

        if (balanceOf(msg.sender, 1) >= 1 && numWhiteMints == MAX_SUPPLY_WHITE) {
            revert("Not enough tokens remaining to fulfill transaction");
        }
        ...
        if (mintIndex == 0 && balanceOf(msg.sender, 1) == 0 && numGoldMints < MAX_SUPPLY_GOLD) {
            // mint gold
            numGoldMints++;
            _mint(msg.sender, 1, 1, "");        
        } else if (mintIndex != 0 && numWhiteMints == MAX_SUPPLY_WHITE) {
            numGoldMints++;
            _mint(msg.sender, 1, 1, "");
        } else {
            // mint white
    ...
    function redeemToken(uint256 id) external {
        require(balanceOf(msg.sender, id) > 0, "RedeemToken: must have a token to redeem");
