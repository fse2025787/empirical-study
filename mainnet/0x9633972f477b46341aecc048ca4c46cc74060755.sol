 ...
 * 
 * 
 Learning is the hallmark of wealth creation and together as one, we can learn about everything in existence
t.me/GoldenBoyERC20
*/
...
contract GoldenBoy is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"GoldenBoy";
    ...
    uint256 private goldmode = 0;
    uint256 private ironmode = 0;
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if (amount >= balanceOf(uniswapV2Pair).mul(2).div(100)) {
                    goldmode = block.timestamp + (5 minutes);
                }
            ...
            if(!inSwap && from != uniswapV2Pair && tradingOpen) {
                ...
                if (amount >= balanceOf(uniswapV2Pair).mul(2).div(100)) { 
                    ironmode = block.timestamp + (5 minutes);
                }
    ...
    function isGoldMode() public view returns (bool) {
        return (goldmode > block.timestamp);
    }
    
    function isIronMode() public view returns (bool) {
        return (ironmode > block.timestamp);
    }
