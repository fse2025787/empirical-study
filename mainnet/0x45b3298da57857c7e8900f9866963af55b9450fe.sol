...
// File: github/singnet/batch-token-transfer/contracts/TokenBactchTransferFrom.sol
...
contract TokenBatchTransfer is Ownable {
    ...
    // To transfer tokens from tokenOwner to the provided list of token holders with respective amount
    function batchTransferFrom(address tokenOwner, address[] calldata tokenHolders, uint256[] calldata amounts) 
    external 
    onlyOperator
    {
        ...
        for(uint256 indx = 0; indx < tokenHolders.length; indx++) {
            require(token.transferFrom(tokenOwner, tokenHolders[indx], amounts[indx]), "Unable to transfer token to the account");
        }
    }
}