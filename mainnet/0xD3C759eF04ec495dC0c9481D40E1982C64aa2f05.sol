...
contract LFGGateKeeper is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _MultisigAddress = payable(0x3F08C9D03F9c6274FA249662f2D6755EC735F639); // 80%
    address payable private _TeamAddress = payable(0xBC719AAC21aBF284A1f1A901EFc17C0cb1947782); // 13.5%
    address payable private _CharityAddress = payable(0x1266957a6b841ae26110633E8eEFC90b1D87BCbc); // 5.5%
    address payable private _Team1Address = payable(0x3130C73051A3Be878A73cc3c8da45b657A1354Ef); // 0.5%
    address payable private _DeveloperAddress = payable(0x7De83438D71cCF4A5121e12259ec4e717E9b7205); // 0.5%

    uint256 private _MultisigPercent = 800;
    uint256 private _TeamPercent = 135;
    uint256 private _CharityPercent = 55;
    uint256 private _Team1Percent = 5;
    uint256 private _DeveloperPercent = 5;

    uint256 private LFGGK_GOLD = 10;
    uint256 private LFGGK_SILVER = 50;
    uint256 private LFGGK_BRONZE = 9940;
    uint256 public LFGGK_MAX = LFGGK_GOLD + LFGGK_SILVER + LFGGK_BRONZE;
    uint256 private PRESALE_PRICE = 0.2 ether;
    uint256 private PUBLIC_PRICE = 0.2 ether;
    uint256 private REVEAL_DELAY = 30 days;
    ...
    uint256 private _activeDateTime = 1646064000; // (GMT): Monday, February 28, 2022 4:00:00 PM
    
    string private _tokenBaseURI = "";
    string private _revealURI = "";
    
    Counters.Counter private _publicLFGGK;

    constructor() ERC721("LFG GateKeeper", "LFGGK") {}

    function setPaymentAddress(address multisigAddress,address teamAddress,address charityAddress,address team1Address) external onlyOwner {
        _MultisigAddress = payable(multisigAddress);
        _TeamAddress = payable(teamAddress);
        _CharityAddress = payable(charityAddress);
        _Team1Address = payable(team1Address);
    }

    function setPaymentPercent(uint256 multisigPercent,uint256 teamPercent,uint256 charityPercent,uint256 team1Percent) external onlyOwner {
        _MultisigPercent = multisigPercent;
        _TeamPercent = teamPercent;
        _CharityPercent = charityPercent;
        _Team1Percent = team1Percent;
    }
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicLFGGK.current();

                if (_publicLFGGK.current() < LFGGK_MAX) {
                    _publicLFGGK.increment();
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(_publicLFGGK.current() < LFGGK_MAX,"Purchase would exceed LFGGK_MAX");
        ...
        if (msg.sender != owner()) {
            ...
            uint256 developerAmount = msg.value * _DeveloperPercent / 1000;
            uint256 charityAmount = msg.value * _CharityPercent / 1000;
            uint256 teamAmount = msg.value * _TeamPercent / 1000;
            uint256 team1Amount = msg.value * _Team1Percent / 1000;
            uint256 multisigAmount = msg.value - developerAmount - charityAmount - teamAmount - team1Amount;

            _DeveloperAddress.transfer(developerAmount);
            _CharityAddress.transfer(charityAmount);
            _TeamAddress.transfer(teamAmount);
            _Team1Address.transfer(team1Amount);
            _MultisigAddress.transfer(multisigAmount);
        }
