...
interface IERC20 {
     ...
     *
     * Returns a boolean value indicating whether the giveaway succeeded.
     *
...
contract FORYOUPAGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public giveawaysSellFee;
    uint256 public liquiditySellFee;
    
    uint256 public totalBuyFees;
    uint256 public rewardsBuyFee;
    uint256 public giveawaysBuyFee;
    uint256 public liquidityBuyFee;
    
    uint256 public tokensForRewards;
    uint256 public tokensForGiveaways;
    ...
    event GiveawaysWalletUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("FYP INU", "FYP") {
        ...
        totalBuyFees = rewardsBuyFee + giveawaysBuyFee + liquidityBuyFee;
        
        rewardsSellFee = 100;
        giveawaysSellFee = 150;
        liquiditySellFee = 0;
        totalSellFees = rewardsSellFee + giveawaysSellFee + liquiditySellFee;

    	dividendTracker = new DividendTracker();
    	
    	giveawaysWallet = address(msg.sender); // set as giveaway wallet
    ...
    function updateBuyFees(uint256 _giveawaysFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        giveawaysBuyFee = _giveawaysFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        totalBuyFees = giveawaysBuyFee + rewardsBuyFee + liquidityBuyFee;
        require(totalBuyFees <= 100, "Must keep fees at 10% or less");
    }
    
    function updateSellFees(uint256 _giveawaysFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        giveawaysSellFee = _giveawaysFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        totalSellFees = giveawaysSellFee + rewardsSellFee + liquiditySellFee;
    ...
    function updateGiveawaysWallet(address newGiveawaysWallet) external onlyOwner {
        require(newGiveawaysWallet != address(0), "may not set to 0 address");
        excludeFromFees(newGiveawaysWallet, true);
        emit GiveawaysWalletUpdated(newGiveawaysWallet, giveawaysWallet);
        giveawaysWallet = newGiveawaysWallet;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            // on sell
                ...
                tokensForGiveaways += fees * giveawaysSellFee / totalSellFees;
            }
            ...
            // on buy
                ...
                tokensForGiveaways += fees * giveawaysBuyFee / totalBuyFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForGiveaways + tokensForRewards;
        ...
        uint256 ethForGiveaways = ethBalance.mul(tokensForGiveaways).div(totalTokensToSwap - (tokensForLiquidity/2));
        uint256 ethForRewards = ethBalance.mul(tokensForRewards).div(totalTokensToSwap - (tokensForLiquidity/2));
        
        uint256 ethForLiquidity = ethBalance - ethForGiveaways - ethForRewards;
        
        tokensForLiquidity = 0;
        tokensForGiveaways = 0;
        tokensForRewards = 0;
        ...
        (success,) = address(giveawaysWallet).call{value: address(this).balance}("");
    }
