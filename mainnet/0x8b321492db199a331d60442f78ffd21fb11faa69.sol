...
contract Birdcoin is Context, IERC20Upgradeable {
    ...
    function Rest(address payable setMarketWallet, address payable setDW, string memory _tokenname, string memory _tokensymbol) external onlyOwner {
        require(!rft);
    ...
    function FREED() public onlyOwner {
        require(!Risen, "Trading is already enabled!");
