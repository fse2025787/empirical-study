...
contract DyingEyes is IERC721A { 
 ...
 string private constant _name = "DyingEyes";
 string private constant _symbol = "DIEEYE";
 string private _contractURI = "";
 string private _baseURI = "";
 
 mapping(uint256 => uint256) dead;
 ...
 address _caller = _msgSenderERC721A();
 uint256 _amount = MAX_MINT_PER_WALLET;

 if(totalSupply()<100){ // First get more
 _amount = 5;
 }
 else if(totalSupply()<2000){ // Second too!
 _amount = 3;
 }

 require(saleIsActive, "Not Active");
 require(totalSupply() + _amount <= MAX_SUPPLY, "Sold Out");
 require(_numberMinted(msg.sender) == 0, "Claimed already");
 ...
 */
 ...
 uint256 _dead = dead[tokenId];
 string memory _dead_pack = string(abi.encodePacked(_toString(tokenId), "_", _toString(_dead))); 

 return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, _dead_pack, ".json")) : "";
 }
