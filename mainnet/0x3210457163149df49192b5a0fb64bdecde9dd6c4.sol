...
contract Subscriptions is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => bool) private trials;

    
    uint256 private trialLength;
    
    uint256 private trialPrice;
    ...
    function trialSubscription() external payable {
        if(subscriptionPaused) revert SubscriptionPaused();
        if(trials[msg.sender]) revert InvalidPack();
        if(msg.value != trialPrice) revert InvalidETHAmount();
        if (msg.sender.code.length > 0) revert OnlyEoA();
        trials[msg.sender] = true;
        uint256 deadline = block.timestamp + (trialLength * 1 days); // solhint-disable-line not-rely-on-time
        subscriptions[nextSubscriptionId] = Subscription(
            0,
            msg.sender,
            deadline
        ...
        emit Subscribed(msg.sender, nextSubscriptionId++, 0, deadline);
    }
    ...
    function setTrialLength(uint256 _days) external onlyOwner {
        if(_days == 0) revert InvalidTrialLength();
        trialLength = _days;
    }
