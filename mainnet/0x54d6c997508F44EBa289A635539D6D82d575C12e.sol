...
contract LinearVestingFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable{
        ...
        nonReentrant
    {
        ...
        uint256 _earned = earned(address(msg.sender));
        require(
            _earned <= totalUserRewards[address(msg.sender)],
            "Earned is more than reward!"
        );
        require(
            _earned > totalUserPayouts[address(msg.sender)],
            "Earned is less or equal to already paid!"
        );

        uint256 rewardAmount = _earned
            .sub(totalUserPayouts[address(msg.sender)]);

        if (rewardAmount > 0) {
            totalUserPayouts[address(msg.sender)] = _earned;
