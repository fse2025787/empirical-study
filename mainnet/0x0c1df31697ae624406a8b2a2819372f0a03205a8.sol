...
interface IMergePool {
    function mergenodes(uint256) external view returns (Types.MergeNode memory);
}

interface INFT is IERC721 {
    // Returns the amount of sPOOL due to the NFT
    function sPOOL(uint256) external returns (uint256);
}
...
contract safePOOL is ERC20, ERC20Permit, ERC20Votes, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;

    IMergePool public immutable mergeNode;
    INFT public nfts;

    mapping(address => mapping(uint256 => uint256)) public lastMintedFromMerge;
    mapping(Types.MergedProduct => uint256) public totalAmountToMintPerProduct;
    ...
    address public mintFeeRecipient = 0x42D26C5cC1fefD4f37Dfe9DD2500Ef2eF8A0E32F;
    address public claimFeeRecipient = 0x42D26C5cC1fefD4f37Dfe9DD2500Ef2eF8A0E32F;
    ...
    constructor(IMergePool _mergeNode, uint256 _lastPot) ERC20("sPOOL", "sPOOL") ERC20Permit("sPOOL") {
        mergeNode = _mergeNode;
        lastPot = _lastPot;

        totalAmountToMintPerProduct[Types.MergedProduct.OneYear] = 515e15;
        totalAmountToMintPerProduct[Types.MergedProduct.ThreeYears] = 1565e15;
        totalAmountToMintPerProduct[Types.MergedProduct.FiveYears] = 4120e15;

        address ops = 0x0BC88F44498e4ae2F921C08b51b37ED0796c3d35;
        address dev = 0xCF94Eb523d9AaEDc3bB58126D762F389af604499;
    ...
    function mint(uint256[] calldata _ids) external payable nonReentrant {
        ...
        for (uint256 i = 0; i < length; ) {
            Types.MergeNode memory mergenode = mergeNode.mergenodes(_ids[i]);

            require(mergenode.minter == msg.sender, "Invalid ownership");
            require(mergenode.expires > block.timestamp, "MergeNode expired");
            require(mergenode.fuseProduct != Types.MergedProduct.None, "Must be fused");

            uint256 lastMintedFromThisMergeNode = lastMintedFromMerge[msg.sender][_ids[i]];

            // If they have fused again since last minting sPOOL, we need to reset lastMintedFromMerge
            if (mergenode.fused >= lastMintedFromThisMergeNode) {
                lastMintedFromMerge[msg.sender][_ids[i]] = 0;
            }

            owed += getMintAmount(mergenode, lastMintedFromThisMergeNode);

            lastMintedFromMerge[msg.sender][_ids[i]] = block.timestamp;
    ...
    function getMintAmount(Types.MergeNode memory _mergenode, uint256 _lastMintedFromThisMergeNode)
        public
        view
        returns (uint256)
    {
        uint256 end = (block.timestamp > _mergenode.unlocks) ? _mergenode.unlocks : block.timestamp;
        uint256 start = (_lastMintedFromThisMergeNode == 0) ? _mergenode.fused : _lastMintedFromThisMergeNode;

        uint256 numerator = end - start;
        uint256 denominator = _mergenode.unlocks - _mergenode.fused;

        return (totalAmountToMintPerProduct[_mergenode.fuseProduct] * numerator) / denominator;
    }

    function getClaimAmount(uint256 _blockNumber) public view returns (uint256) {
        return
            (potPerPeriod[_blockNumber] * getPastVotes(msg.sender, _blockNumber)) /
    ...
    function sPOOLOwed(uint256[] calldata _ids, address _minter) external view returns (uint256) {
        uint256 owed = 0;
        uint256 length = _ids.length;
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            owed += getMintAmount(mergenode, lastMintedFromMerge[_minter][_ids[i]]);
    ...
    function ETHOwed(uint256[] memory _blockNumbers, address _claimant) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _blockNumbers.length; ) {
            ...
            if (!hasClaimedPeriod[_claimant][blockNumber]) {
                owed +=
                    (potPerPeriod[blockNumber] * getPastVotes(_claimant, blockNumber)) /
    ...
    function setTotalAmountToMintPerProduct(Types.MergedProduct _fuseProduct, uint256 _amount) external onlyOwner {
