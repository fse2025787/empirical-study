...
contract Atombolts is ERC721Enumerable, ERC721URIStorage, Ownable {
    string _baseTokenURI = 'https://api.atomblots.com';

    uint256 private _maxPurchase = 20;
    uint256 private _reserved = 100;
    uint256 private _price = 0.02 ether;
    uint256 public constant MAX_COLLECTION = 10000;

    uint256 public saleState = 1; // 0 = paused, 1 = presale, 2 = live
 
    mapping(address => uint256) private _preSaleWhitelist;

    address t1 = 0x83255aAea1E82B5C3A07f1A8210B886a1320EB30; // update owner

    constructor() ERC721("Atombolts", "ATB") {} 
    ...
    function preSaleNfts(uint256 numberOfTokens) public payable {
        uint256 supply = totalSupply();
        ...
        require(
            supply + numberOfTokens <= MAX_COLLECTION - _reserved,
            'Purchase would exceed max supply of Atombolts.'
        );
        require(
    ...
    function mintAtombolts(uint256 numberOfTokens) public payable {
        uint256 supply = totalSupply();
        ...
        if (msg.sender != owner()) {
            require(
                ...
                'You can only mint 20 tokens at a time.'
            );
        }
        require(
            supply + numberOfTokens <= MAX_COLLECTION - _reserved,
            'Purchase would exceed max supply of this collection.'
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= _price * numberOfTokens,
