...
contract PRE is ERC721A, Ownable, ReentrancyGuard {

    string public baseURI = "ipfs://QmW6ZDXzXF7dtd34Hi2tnmPXCnrnbW3uB38Yr34aoXxred/";
    string public baseExtension = ".json";
  uint public          price             = 0.005 ether;
  uint public          maxPerTx          = 20;
  uint public          maxTxForFreeMints    = 2;
  uint public          totalFree         = 200;
  uint public          maxSupply         = 1080;
  uint public          nextOwnerToExplicitlySet;
  bool public          mintEnabled = true;

  constructor() ERC721A("Psychedelic R-Evolution", "PRE"){
...
function tokenURI(uint256 _tokenId)
  ...
  function mint(uint256 amt) external payable
  {
    ...
    if (cost == 0) {
        require( amt <= maxTxForFreeMints, "Max per TX reached.");
    } else {
        require( amt <= maxPerTx, "Max per TX reached.");
    }
