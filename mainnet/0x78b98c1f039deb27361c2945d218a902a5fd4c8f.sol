...
contract BuilderShop {
   ...
   //snowDaq registry is hard coded
   address snowDaqRegistryContract = 0x8FC05c79D7D725D425e0c9Fe2EE6dE48E57AE792;

   modifier onlyValidSender() {
       SnowDaqRegistry nftg_registry = SnowDaqRegistry(snowDaqRegistryContract);
       bool is_valid = nftg_registry.isValidSnowDaqSender(msg.sender);
   ...
   function isValidBuilderShop(address builder_shop) public view returns (bool isValid) {
       //public function, allowing anyone to check if a contract address is a valid snowDaq contract
   ...
   function createNewBuilderInstance(
       ...
       uint num_nifties,
       uint[] memory snowDaq_quantities,
       string memory token_base_uri,
       string memory creator_name)
   public onlyValidSender returns (SnowDaqBuilderInstance tokenAddress) {

       contractId = contractId + 1;

       SnowDaqBuilderInstance new_contract = new SnowDaqBuilderInstance(
           _name,
           _symbol,
           contractId,
           num_nifties,
...
contract SnowDaqBuilderInstance is ERC721Full {
...
//   string public creatorName;
   ...
   address public masterBuilderContract = 0x5eea5Cd7D911cB64C2Ca97C7B4257BB8278BEb47;
   ...
   mapping (uint => Counters.Counter) public _numSnowDaqMinted;
   mapping (uint => uint) public _numSnowDaqPermitted;
   mapping (uint => uint) public _snowDaqPrice;
   mapping (uint => string) public _snowDaqIPFSHashes;
   mapping (uint => bool) public _IPFSHashHasBeenSet;

   //EVENTS

   //purchase + creation events
   event SnowDaqPurchased(address _buyer, uint256 _amount, uint _tokenId);
   event SnowDaqCreated(address new_owner, uint _snowDaqType, uint _tokenId);
   ...
   constructor(
       ...
       //offset starts at 1 - there is no snowDaqType of 0
       for (uint i=0; i<(num_nifties); i++) {
           _numSnowDaqPermitted[i+1] = snowDaq_quantities[i];
       }
   }
   
   function setSnowDaqIPFSHash(uint snowDaqType, 
                            string memory ipfs_hash) onlyValidSender public {
        //can only be set once
        if (_IPFSHashHasBeenSet[snowDaqType] == true) {
            revert("Can only be set once");
        } else {
            _snowDaqIPFSHashes[snowDaqType] = ipfs_hash;
            _IPFSHashHasBeenSet[snowDaqType]  = true;
        }
    }

   function isSnowDaqSoldOut(uint snowDaqType) public view returns (bool) {
       if (snowDaqType > numNiftiesCurrentlyInContract) {
           return true;
       }
       if (_numSnowDaqMinted[snowDaqType].current() > _numSnowDaqPermitted[snowDaqType]) {
           return (true);
       } else {
           return (false);
       }
   ...
   function giftSnowDaq(address collector_address, 
                      uint snowDaqType) onlyValidSender public {
       //master for static calls
       BuilderMaster bm = BuilderMaster(masterBuilderContract);
       _numSnowDaqMinted[snowDaqType].increment();
       //check if this snowDaq is sold out
       if (isSnowDaqSoldOut(snowDaqType)==true) {
           revert("SnowDaq sold out!");
       }
       //mint a snowDaq
       uint specificTokenId = _numSnowDaqMinted[snowDaqType].current();
       uint tokenId = bm.encodeTokenId(contractId, snowDaqType, specificTokenId);
       string memory tokenIdStr = bm.uint2str(tokenId);
       string memory tokenURI = bm.strConcat(baseURI, tokenIdStr);
       string memory ipfsHash = _snowDaqIPFSHashes[snowDaqType];
       //mint token
       _mint(collector_address, tokenId);
       _setTokenURI(tokenId, tokenURI);
       _setTokenIPFSHash(tokenId, ipfsHash);
       //do events
       emit SnowDaqCreated(collector_address, snowDaqType, tokenId);
   }

}

contract SnowDaqRegistry {
   function isValidSnowDaqSender(address sending_key) public view returns (bool);
...
contract BuilderMaster {
   ...
   function encodeTokenId(uint contractId, uint snowDaqType, uint specificSnowDaqNum) public view returns (uint);
...
/**
* Contracts and libraries below are from OpenZeppelin, except snowDaq builder instance
**/
