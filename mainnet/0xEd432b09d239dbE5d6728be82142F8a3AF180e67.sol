...
contract BridgeFee is Ownable, IBridgeFee {
    ...
    function transfer(address dcrmAddress) external payable {
        require(dcrmAddress != address(0), "invalid dcrm address");
        ...
        (bool bridgeAmountSent, ) = payable(dcrmAddress).call{value: bridgeAmount}("");
        require(bridgeAmountSent, "bridge failed");

        (bool feeAmountSent, ) = payable(feeAddress).call{value: feeAmount}("");
        require(feeAmountSent, "fee transfer failed");

        emit BridgeDone(_msgSender(), dcrmAddress, address(0), bridgeAmount, feeAmount);
    }
    ...
    function getFeeAmounts(
        ...
        require(min > 0 && max > 0, "unavailable");
