...
contract ERC1155BaseURI is ERC1155Upgradeable {
    ...
    */
    ...
    function _tokenURI(uint256 tokenId) internal view virtual returns (string memory) {
        string memory _currTokenURI = _tokenURIs[tokenId];
        string memory base = baseURI();

        // If there is no base URI, return the token URI.
        if (bytes(base).length == 0) {
            return _currTokenURI;
        }
        // If both are set, concatenate the baseURI and tokenURI (via abi.encodePacked).
        if (bytes(_currTokenURI).length > 0) {
            return string(abi.encodePacked(base, _currTokenURI));
        }
...
abstract contract HasContractURI is Initializable, ERC165Upgradeable {
    ...
    function __HasContractURI_init_unchained(string memory _contractURI) public initializer {
...
library LibMint1155 {
    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH = keccak256("Mint1155(uint256 tokenId,uint256 supply,string tokenURI,address[] creators,Part[] royalties)Part(address account,uint256 value)");
