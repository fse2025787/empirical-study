...
contract Mayas is ERC721A, Ownable {
    //@dev Attributes for NFT configuration
    string internal baseURI;
    uint256 public cost = 0.003 ether;
    uint256 public maxSupply = 3333;
    uint public MAX_FREE_SUPPLY = 1333;
    ...
    constructor(string memory _uri) ERC721A("Maya's Godzz", "MG") {
        unrevealedURI = _uri;
    }
    ...
    modifier priceCheck(uint _amount) {
        ...
        if(freeMints < MAX_FREE_SUPPLY)
            eligibleFreemint = 1;
        console.log(_amount, eligibleFreemint);
    ...
    function setMaxFreeSupply(uint _maxFreesupply) external onlyOwner{
        MAX_FREE_SUPPLY = _maxFreesupply;
    }
