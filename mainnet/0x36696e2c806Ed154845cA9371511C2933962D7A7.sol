...
contract AbodeCollection is ERC721, ERC721URIStorage,
    Ownable  {
    ...
    event amountPayed(uint256 indexed ammount);
    ...
    constructor(
        ...
        pausedTransfers = _pausedTransfer;
    }

    function pauseTransfers(bool value) public onlyOwner {
    ...
    modifier isOnlyByJurisdiction(address from, address _receiver) {
        require((onlyByJurisdiction == false) || msg.sender == this.owner() || (onlyByJurisdiction &&
         keccak256(bytes(whitelistAddresses[from].country)) == keccak256(bytes(whitelistAddresses[_receiver].country))), "Only users of the same jurisdiction can transfer!");
         _;
    }   
    
    modifier transferToken(address from, address to, uint256 tokenId){
        require(pausedTransfers == false || msg.sender == this.owner(), "transfers are not allowed by now");
    ...
    function safeMint() public payable isPausedMint{
        uint256 quantity = 1;
        ...
        for (uint256 i = 0; i < quantity; i++) {
            if(arrayTokensBurned.length > 0){
                tokenId = arrayTokensBurned[arrayTokensBurned.length-1];
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId)
        public
        override
        isOnlyByJurisdiction(from, to) transferToken(from, to, tokenId)
    {
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public virtual override isOnlyByJurisdiction(from, to) transferToken(from, to, tokenId){
        _transfer(from, to, tokenId);
        transferOwnerTokens(from, to, tokenId);
    }
    ...
    function rollback(address payable tokenOwner, uint256 _refundAmount) external onlyOwner returns(bool) {
      ...
      for(uint i = ownerTokens[tokenOwner].length; i > 0 ; i--){
        ...
        (bool sent, ) = payableTo.call{value: _refundAmount}("");
        return sent;
    }
    ...
    function setBaseExtension(string memory value) public onlyOwner {
        baseExtension = value;
    }
