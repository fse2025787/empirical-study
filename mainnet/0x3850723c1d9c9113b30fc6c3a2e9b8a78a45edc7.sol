...
contract MasterDev is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt;     // Reward debt. See explanation below.
        //
        // We do some fancy math here. Basically, any point in time, the amount of DEVSs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDevsPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDevsPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DEVSs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DEVSs distribution occurs.
        uint256 accDevsPerShare;   // Accumulated DEVSs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The devs TOKEN!
    DevsToken public devs;
    // Dev address.
    address public devaddr;
    // devs tokens created per block.
    uint256 public devsPerBlock;
    // Bonus muliplier for early devs makers.
    ...
    event UpdateEmissionRate(address indexed user, uint256 devsPerBlock);
    ...
    // Update the given pool's devs allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending DEVSs on frontend.
    function pendingDevs(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 devsReward = multiplier.mul(devsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accDevsPerShare = accDevsPerShare.add(devsReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accDevsPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 devsReward = multiplier.mul(devsPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        devs.mint(devaddr, devsReward.div(12));
        devs.mint(address(this), devsReward);
        pool.accDevsPerShare = pool.accDevsPerShare.add(devsReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for devs allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDevsPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeDevsTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDevsPerShare).div(1e12);
    ...
    // Safe devs transfer function, just in case if rounding error causes pool to not have enough DEVSs.
    function safeDevsTransfer(address _to, uint256 _amount) internal {
        uint256 devsBal = devs.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > devsBal) {
            transferSuccess = devs.transfer(_to, devsBal);
        } else {
            transferSuccess = devs.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDevsTransfer: transfer failed");
    }
