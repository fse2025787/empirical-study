	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if(takeFee) 
		{
            ...
            if(tokenDiscountEnable) {
                ...
                bool isGold = IOceidonNFT(OBLOXNFT).balanceOf(from, 2) >= 1 || IOceidonNFT(OBLOXNFT).balanceOf(to, 2) >= 1;
                bool isSilver = IOceidonNFT(OBLOXNFT).balanceOf(from, 3) >= 1 || IOceidonNFT(OBLOXNFT).balanceOf(to, 3) >= 1;
                
                if(isGold && isSilver) {
                    uint256 sumDevDiscount = token2DevDiscount + token3DevDiscount;
                    uint256 sumLiquidityDiscount = token2LiquidityDiscount + token3LiquidityDiscount;
                    devDiscount = sumDevDiscount > 10000 ? 10000 : sumDevDiscount;
                    liquidityDiscount = sumLiquidityDiscount > 10000 ? 10000 : sumLiquidityDiscount;
                }
                else if(isGold) {
