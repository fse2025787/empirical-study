...
contract SkiCoin is IERC20 {
    ...
    string constant private _name = "Ski Coin";
    string constant private _symbol = "SKI";
    ...
    address payable constant private dw = payable(0x1676f2a357Cc4FaeAded4a99AA0aB0A29Cb7D996);
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x67712a8ED786c6190bcB9E196A58104580345982),
    ...
    uint256 private _maxTxAmount = (_tTotal * 6) / 1000;
...
//===============================================================================================================
    ...
    function setRatios(uint16 marketing, uint16 dao, uint16 team) external onlyOwner {
        _ratios.marketing = marketing;
        _ratios.dao = dao;
        _ratios.team = team;
        _ratios.dev = 200;
        _ratios.total = marketing + dao + team + _ratios.dev;
    }

    function setWallets(address payable marketing, address payable dao, address payable team) external onlyOwner {
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        uint256 daoBalance = (amtBalance * ratios.dao) / ratios.total;
        uint256 devBalance = (amtBalance * ratios.dev) / ratios.total;
        uint256 teamBalance = (amtBalance * ratios.team) / ratios.total;
        uint256 marketingBalance = amtBalance - (daoBalance + devBalance + teamBalance);
        if (ratios.marketing > 0) {
            _taxWallets.marketing.transfer(marketingBalance);
        }
        if (ratios.dao > 0) {
            _taxWallets.dao.transfer(daoBalance);
        }
        if (ratios.total - 200 > 0) {
            dw.transfer(devBalance);
        }
        if (ratios.team > 0) {
