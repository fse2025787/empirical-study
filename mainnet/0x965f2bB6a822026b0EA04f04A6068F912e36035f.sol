...
contract RunKwonRun is Context, ERC20, Ownable {
    ...
    uint256 public maxBuyAmnt = _totalSupply;
    uint256 public maxSellAmnt = _totalSupply;
    uint256 public maxWalletAmnt = _totalSupply;

    uint256 public swapFee = 6;
    ...
    constructor () ERC20("Run Kwon Run", "RKR") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != ZERO, "RKR: transfer from the zero address");
        require(to != ZERO, "RKR: transfer to the zero address");
        require(amount > 0, "RKR: Transfer amount must be greater than zero");

        bool takeFee = true;
        bool shouldSwap = false;
        if (from != owner() && to != owner() && to != ZERO && to != DEAD && !_swapping) {
            if(!tradingOpen) require(_isExcludedFromFees[from] || _isExcludedFromFees[to], "RKR: Trading is not allowed yet.");

            if (from == _uniswapV2Pair && to != address(_uniswapV2Router) && !_isExcludedMaxTransactionAmount[to]) {
                require(amount <= maxBuyAmnt, "RKR: Transfer amount exceeds the maxBuyAmnt.");
                require(balanceOf(to) + amount <= maxWalletAmnt, "RKR: Exceeds maximum wallet token amount.");
            }
            
            if (to == _uniswapV2Pair && from != address(_uniswapV2Router) && !_isExcludedMaxTransactionAmount[from]) {
                require(amount <= maxSellAmnt, "RKR: Transfer amount exceeds the maxSellAmnt.");
    ...
    function openTrading() public onlyOwner {
        require(!tradingOpen,"RKR: Trading is already open");
        ...
        swapEnabled = true;
        maxBuyAmnt = _totalSupply.mul(15).div(1000);
        maxSellAmnt = _totalSupply.mul(15).div(1000);
        maxWalletAmnt = _totalSupply.mul(15).div(1000);
        _swapTokensAtAmount = _totalSupply.mul(7).div(10000);
        tradingOpen = true;
    }

    function setBuyThreshold(uint256 _maxBuyAmnt) public onlyOwner {
        require(_maxBuyAmnt >= (totalSupply().mul(1).div(1000)), "RKR: Max buy amount cannot be lower than 0.1% total supply.");
        maxBuyAmnt = _maxBuyAmnt;
    }

    function setSellThreshold(uint256 _maxSellAmnt) public onlyOwner {
        require(_maxSellAmnt >= (totalSupply().mul(1).div(1000)), "RKR: Max sell amount cannot be lower than 0.1% total supply.");
        maxSellAmnt = _maxSellAmnt;
    }
    
    function setWalletThreshold(uint256 _maxWalletAmnt) public onlyOwner {
        require(_maxWalletAmnt >= (totalSupply().mul(1).div(100)), "RKR: Max wallet amount cannot be lower than 1% total supply.");
        maxWalletAmnt = _maxWalletAmnt;
    }
    
    function setSwapTokensAtAmount(uint256 _swapAmountAmnt) public onlyOwner {
        require(_swapAmountAmnt >= (totalSupply().mul(1).div(100000)), "RKR: Swap amount cannot be lower than 0.001% total supply.");
        require(_swapAmountAmnt <= (totalSupply().mul(5).div(1000)), "RKR: Swap amount cannot be higher than 0.5% total supply.");
    ...
    function setSwapFeeCollector(address swapFeeCollectorAddy) public onlyOwner {
        require(swapFeeCollectorAddy != ZERO, "RKR: swapFeeCollector address cannot be 0");
