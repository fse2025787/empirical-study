...
contract slamdunkFactory is Ownable, ReentrancyGuard {
    ...
    address public liquidityWallet = 0x71a95Dcb3aF3ec22A7C2D83825da1fb24F560773;
    ...
    function _transferPerformanceFee() internal {
        ...
        payable(liquidityWallet).transfer(performanceFee);
     ...
     */
    function depositDunkFactoryToken(uint _amount) external onlyOwner nonReentrant {
    ...
    function emergencyFactory(uint256 _amount) external onlyOwner {
    ...
    function setServiceInfo(address _liquidityWallet, uint256 _fee) external {
        require(msg.sender == liquidityWallet, "setServiceInfo: FORBIDDEN");
        require(_liquidityWallet != address(0x0), "Invalid address");
        require(_fee < 0.05 ether, "fee cannot exceed 0.05 ether");

        liquidityWallet = _liquidityWallet;
        performanceFee = _fee;

        emit ServiceInfoUpadted(_liquidityWallet, _fee);
    }
