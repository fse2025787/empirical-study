...
contract RedDeadRedemption is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // Once disabled can never be used again
    ...
    mapping(address => bool) public _isBlackListedBot;  // To store and blacklist addresses of botters
    ...
    constructor() ERC20("Red Dead Redemption", "RED") {
        ...
        // Marketing / Redemption / Dev Fee are Sent to a contract for further distribution to save on transfer gas fees

        uint256 _buyMarketingFee = 85;
        uint256 _buyLiquidityFee = 50;  
        uint256 _buyDevFee = 15;

        uint256 _sellMarketingFee = 85;
        ...
        maxTransactionAmount = totalSupply * 50 / 10000; // 0.5% maxTransactionAmountTxn | 4,999,999,999
        ...
        lpWallet = address(owner());

        addToBlacklistEnabled = true;
    ...
    function updateDevWallet(address newWalletAddress) external onlyOwner {
        emit devWalletUpdated(newWalletAddress, devWallet);
        devWallet = newWalletAddress;
    }

    function updateMarketingWallet(address newWalletAddress) external onlyOwner {
        emit marketingWalletUpdated(newWalletAddress, marketingWallet);
        marketingWallet = newWalletAddress;
    }

    // Change where auto liq provisioning tokens end up

    function updateLpWallet(address newWallet) external onlyOwner {
        emit lpWalletUpdated(newWallet, lpWallet);
        lpWallet = newWallet;
    }
    ...
    // A function to remove added bots from the blacklist 
