...
contract SPURDO is Context,IERC20, Ownable{
    ...
    address _owner;
    address payable public marketingAddress = payable(0xCaaA80EBF9E552eC3002ab69736c3DD83DB0383D);
    ...
    uint256 SPURDODaycooldown = 0;
    bool private tradeEnabled = false;
    uint256 private totalBuyTax = buyliqFee+buymktFee;
    ...
    uint256 private totalSellTax = sellliqFee+sellmktFee;
    ...
    function SPURDODay() public onlyOwner{
         require(block.timestamp > SPURDODaycooldown, "You cant call SPURDODay more than once per day");
    ...
    function SPURDODayOver() public onlyOwner{
         buyliqFee = buyprevLiqFee;
         buymktFee = buyPrevmktFee;
         SPURDODaycooldown = block.timestamp + 86400;
    }
    ...
    function updatecurrentbuyliqFee(uint256 newAmount) public onlyOwner{
        require(buymktFee+newAmount<7,"SPURDO: Tax cant go above 7%");
        buyliqFee = newAmount;
    }
    function updatecurrentbuymktfee(uint256 newAmount) public onlyOwner{
        require(buyliqFee+newAmount<7,"SPURDO: Tax cant go above 7%");
        buymktFee= newAmount;
    }

    function updatecurrentsellLiqFee(uint256 newAmount) public onlyOwner{
        require(sellmktFee+newAmount<7,"SPURDO: Tax cant go above 7%");
        sellliqFee= newAmount;
    }
    function updatecurrentsellmktfee(uint256 newAmount)public onlyOwner{
        require(sellliqFee+newAmount<7,"SPURDO: Tax cant go above 7%");
