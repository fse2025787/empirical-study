...
// File: contracts/pools/RainiLPStakingPool.sol


pragma solidity ^0.8.3;


contract RainiLpStakingPool is AccessControl, ReentrancyGuard {
  ...
  function mint(address[] calldata _addresses, uint256[] calldata _points) 
    external onlyMinter {
      require(_addresses.length == _points.length, "RainiLpStakingPool: Arrays not equal");
  ...
  function buyUnicorn(uint256 _amount) 
    external payable {
      require(_amount > 0, "RainiLpStakingPool: _amount is zero");
      require(msg.value.mul(unicornToEth) >= _amount, "RainiLpStakingPool: not enougth eth");

      balances[_msgSender()] = balances[_msgSender()].add(_amount);

      uint256 refund = msg.value.sub(_amount.div(unicornToEth));
      if(refund > 0) {
