...
contract  FancyFruitFanclubBananas is Ownable, ERC721 {
    uint constant tokenPrice = 0.06 ether;
    uint constant presale_Price = 0.05 ether;
    uint constant maxSupply = 4000;
    
    uint public totalSupply = 0;
    uint public presale_limit = 1000;
    
    uint public sale_startTime = block.timestamp;
    uint public presale_startTime = block.timestamp;
    bool public pause_sale = false;

    string public baseURI = "https://fancyfruitfanclub.com/api/nft/";
   ...
   function buy_Presale(uint _count) public payable{
        require(_count > 0, "mint at least one token");
        require(_count <= 15, "Max 15 Allowed.");
        require(totalSupply + _count <= maxSupply, "Not enough tokens left");
        ...
        require(pause_sale == false, "Sale is Paused.");
        require(checkPresale(msg.sender), "You are not whitelisted.");
        require(totalSupply <= presale_limit, "Presale Limit Reached.");
    ...
    function checkPresale(address _to) public view returns(bool){
        return presaleList[_to];
    }
