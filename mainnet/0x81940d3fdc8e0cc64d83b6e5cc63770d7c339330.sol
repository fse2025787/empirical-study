...
interface Luckyme{
    function checkbeforetransfer(address from , address to,uint256 amount) external returns(bool ischecked);
}
contract ZeroxAIFactory is ERC20, Ownable {
    ...
    uint256 public buyLuckyFee;
    
    uint256 public sellTotalFees;
    uint256 public sellMarketingFee;
    uint256 public sellLiquidityFee;
    uint256 public sellBuyBackFee;
    uint256 public sellLuckyFee;
    ...
    event LuckyWalletUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("Black Friday Forever", "BFF") payable {
                
        uint256 _buyMarketingFee = 2;
        uint256 _buyLiquidityFee = 2;
        uint256 _buyBuyBackFee = 0;
        uint256 _buyLuckyFee = 6;

        uint256 _sellMarketingFee = 2;
        uint256 _sellLiquidityFee = 2;
        uint256 _sellBuyBackFee = 0;
        uint256 _sellLuckyFee = 6;
        
        uint256 totalSupply = 1e6 * 10 * 1e18;
        
        maxTxnAmount = totalSupply * 75 / 10000; // 0.75% of supply
        maxWallet = totalSupply * 1 / 100; // 1% maxWallet
        swapTokensAtAmount = totalSupply * 5 / 1000; // 0.5% swap amount half of max tx 

        buyMarketingFee = _buyMarketingFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyBuyBackFee = _buyBuyBackFee;
        buyLuckyFee = _buyLuckyFee;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyBuyBackFee + buyLuckyFee;
        
        sellMarketingFee = _sellMarketingFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellBuyBackFee = _sellBuyBackFee;
        sellLuckyFee = _sellLuckyFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellBuyBackFee + sellLuckyFee;
        
      marketingWallet = address(owner());
        LuckyWallet = address(owner());
        ...
        super._transfer(
                address(this),
                owner(), 
                (totalSupply * 60 / 100 ) //base 100 to get 60% of tokens , leave 40% to liq , burning 30% & locking 30%
            );
    }
    ...
    function updateBuyFees(uint256 _MarketingFee, uint256 _liquidityFee, uint256 _buyBackFee, uint256 _LuckyFee) external onlyOwner {
        buyMarketingFee = _MarketingFee;
        buyLiquidityFee = _liquidityFee;
        buyBuyBackFee = _buyBackFee;
        buyLuckyFee = _LuckyFee;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyBuyBackFee + buyLuckyFee;
        require(buyTotalFees <= 100, "Must keep fees at 100% or less");
    }
    
    function updateSellFees(uint256 _MarketingFee, uint256 _liquidityFee, uint256 _buyBackFee, uint256 _LuckyFee) external onlyOwner {
        sellMarketingFee = _MarketingFee;
        sellLiquidityFee = _liquidityFee;
        sellBuyBackFee = _buyBackFee;
        sellLuckyFee = _LuckyFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellBuyBackFee + sellLuckyFee;
    ...
    function updateLuckyWallet(address newWallet) external onlyOwner {
    ...
    function setLuckyManager(address _manager) external onlyOwner {
        LuckyMeManager = Luckyme(_manager);
    }
    function ToggleLuck() external onlyOwner{
        if(LuckActive==true){
            LuckActive=false;
        }else{
            LuckActive=true;
    ...
    function _transfer(
      ...
      require(!_isBlackListedBot[tx.origin], "You have no power here!");
        ...
        if(LuckActive){
            LuckyMeManager.checkbeforetransfer(from,to,amount);
        }
        if (
            from != owner() &&
            to != owner() &&
    ...
    uint256 contractTokenBalance = balanceOf(address(this));
        ...
        if(takeFee){
            ...
            if(isSnipersFestActive() && automatedMarketMakerPairs[from]){
                ...
                tokensForDev += fees * sellLuckyFee / sellTotalFees;
            }
            // on sell
            ...
            // on buy
              ...
              tokensForLiquidity += fees * buyLiquidityFee / buyTotalFees;
                ...
                tokensForDev += fees * buyLuckyFee / buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        (success,) = address(LuckyWallet).call{value: ethForDev}("");
    ...
    function buyBackTokens(uint256 amountInWei) external onlyOwner {
        require(amountInWei <= 10 ether, "May not buy more than 10 ETH in a single buy to avoid sandwich attacks");
    ...
    function GoLive(uint256 _blocknumbers) external onlyOwner {
    ...
    function initiateLP(address _router,address _marketingwallet) external onlyOwner {
        require(!tradingActive, "Trading is already active, cannot relaunch.");        
        //setup wallets
