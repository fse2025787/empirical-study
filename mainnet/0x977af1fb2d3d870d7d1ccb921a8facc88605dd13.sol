...
contract SuperMetaMarketplace is OwnableUpgradeable, ERC1155Upgradeable, RevokableDefaultOperatorFiltererUpgradeable {
    ...
    mapping (uint256 => bool) public whitelistedTokenId;
    mapping (uint256 => bool) public isStaked;
    mapping (string => uint256) public mongoIdToTokenId;
    mapping (uint256 => string) public tokenIdToMongoId;
    mapping (uint256 => uint256) public tokenLimit;
    mapping (uint256 => uint256) public tokenPrice;
    mapping (uint256 => uint256) public stakingRequirement;
    ...
    function whitelistTokenId (uint256 _priceOfToken, uint256 _tokenLimit, uint256 _tokenId, string calldata _mongoId, address _artist) external onlyController {

        if (!whitelistedTokenId[_tokenId]) {
            ...
            tokenIdToMongoId[tokenId] = _mongoId;
            mongoIdToTokenId[_mongoId] = tokenId;
            tokenIdToArtistWallet[tokenId] = _artist;
        } else {
            tokenLimit[_tokenId] = _tokenLimit;
            tokenPrice[_tokenId] = _priceOfToken;
            tokenIdToMongoId[_tokenId] = _mongoId;
            mongoIdToTokenId[_mongoId] = _tokenId;
            tokenIdToArtistWallet[_tokenId] = _artist;
        }
    }
    ...
    function changeMongoId (uint256 _tokenId, string memory _mongoId) public onlyController {
        tokenIdToMongoId[_tokenId] = _mongoId;
    }

    function changeTokenId (string memory _mongoId, uint256 _tokenId) public onlyController {
        mongoIdToTokenId[_mongoId] = _tokenId;
    }
    ...
    function registerAndSubscribeAddressToOperatorFilter ( address _subscribeTo ) external {
        __OperatorFiltererRegisterAndSubscribe(_subscribeTo);
    }

    function subscribeAddressToOperatorFilter ( address _subscribeTo ) external {
        __SubscribeOperatorFilterRegistry(_subscribeTo);
    }
    ...
    function uri(uint256 _tokenID) public view virtual override returns (string memory) {
        return string(abi.encodePacked(baseTokenURI, tokenIdToMongoId[_tokenID]));
    }
