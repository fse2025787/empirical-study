...
contract Trade {
    ...
    function getFees(Order calldata order, bool isImport) public view returns(Fee memory){
        address tokenCreator;
        uint platformFee;
        uint royaltyFee;
        uint assetFee;
        ...
        if(order.nftType == BuyingAssetType.ERC721 && !isImport) {
        ...
        if(order.nftType == BuyingAssetType.ERC1155 && !isImport) {
    ...
    function buyAsset(Order calldata order, bool isImport, Sign calldata sign) external returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        Fee memory fee = getFees(order, isImport);
    ...
    function executeBid(Order calldata order, bool isImport, Sign calldata sign) external returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        Fee memory fee = getFees(order, isImport);
