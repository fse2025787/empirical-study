...
contract Yashav3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("YASHA", "YASHA") {
        ...
        address _lpPair = IUniswapV2Factory(_dexRouter.factory())
            .createPair(address(this), _dexRouter.WETH());

        dexRouter = _dexRouter;
        lpPair = _lpPair;

        _setAutomatedMarketMakerPair(_lpPair, true);
    ...
    function setAutoLPBurnSettings(uint256 _frequencyInSeconds, uint256 _percent, bool _Enabled) external onlyOwner {
        require(_frequencyInSeconds >= 100, "cannot set buyback more often than every 10 minutes");
        require(_percent <= 2000 && _percent >= 0, "Must set auto LP burn percent between 1% and 20%");
