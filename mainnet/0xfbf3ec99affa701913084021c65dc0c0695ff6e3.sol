...
/**
 * Copyright 2021 Nubiru LOOT randomized adventurer gear generated and stored on chain.
/**
...
contract NubiruLOOT is ERC721Enumerable, ReentrancyGuard, Ownable {
        ...
        uint256 public characterPrice = 15000000000000000; //0.015 ETH
        ...
        CharacterInterface public characterContract = CharacterInterface(characterAddress);
    ...
    function pluckLocation(uint256 tokenId) internal view returns (string memory, uint256) {
        uint256 rarity = 0;
    ...
    function pluckCreature(uint256 tokenId, uint256 yValue) internal view returns (string memory, uint256) {
        uint256 rarity = 0;
        ...
        string memory prefix = string(abi.encodePacked('<text x="30" y="', toString(yValue), '" class="base">'));
        string memory suffix = '</text>';
        ...
        {
            rarity = rarity + 15;
            creature = string(abi.encodePacked(creatureDescriptors[rand2 % creatureDescriptors.length], " ", creature));
        }
    ...
    function getRarityLevel(uint256 rarity) internal view returns (string memory) {
        if(rarity < 20)
        {
            return("Common");
        }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        (parts[1], rarity) = pluckLocation(tokenId); //location

        (parts[2], rarityHolder) = pluckCreature(tokenId+12345, 40); //creature 1
        rarity = rarity + rarityHolder;

        (parts[3], rarityHolder) = pluckCreature(tokenId+12345+12345, 60); //creature 2
        rarity = rarity + rarityHolder;
        
         if(numCreatures >= 3)
        {
            (parts[4], rarityHolder) = pluckCreature(tokenId+12345+12345+12345, 80); //creature 3
            rarity = rarity + rarityHolder;
        }
        else
        {
         ...
         if(numCreatures >= 4)
        {
            (parts[5], rarityHolder) = pluckCreature(tokenId+12345+12345+12345+12345, 100); //creature 4
            rarity = rarity + rarityHolder;
        }
        else
        {
         ...
         if(numCreatures >= 5)
        {
            (parts[6], rarityHolder) = pluckCreature(tokenId+12345+12345+12345+12345+12345, 120); //creature 5
            rarity = rarity + rarityHolder;
        }
        else
        {
        ...
        if(tokenId == uniqueId)
        {
            rarityLevel = "1 of 1";
            rarity = 180;
            parts[1] = '<tspan fill="gold">The Edge of Time and Space</tspan></text>';
            parts[2] = '<text x="30" y="40" class="base"><tspan fill="gold">The Great World Eater</tspan></text>';
            parts[3] = '';
            parts[4] = '';
            parts[5] = '';
            parts[6] = '';
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "NubiruLOOT #', toString(tokenId), '", "description": "NubiruLOOT are randomized adventure encounters generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use NubiruLOOT in any way you want. NubiruLOOT is inspired by and compatible with Loot (for Adventurers) and Characters (for Adventurers)", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '","attributes":[{"trait_type":"Rarity", "value": "', rarityLevel, '"},{"trait_type": "Rarity Value","value":', toString(rarity), '}]','}'))));
    ...
    function multiMint(uint256 number) public payable nonReentrant {
        require((price * number) <= msg.value, "Ether value sent is not correct");
        require(currentId + number < 12000, "Mint would exceed maximum token amount");
    ...
    function mintWithCharacterOrLoot() public payable nonReentrant {
        require(characterContract.balanceOf(msg.sender) > 0 || lootContract.balanceOf(msg.sender) > 0, "You need either a Loot or Character NFT for discounted minting");
        require(currentId < 12000, "No more tokens to mint");
    ...
    function multiMintWithCharacterOrLoot(uint256 number) public payable nonReentrant {
        require(characterContract.balanceOf(msg.sender) > 0 || lootContract.balanceOf(msg.sender) > 0, "You need either a Loot or Character NFT for discounted minting");
        require((characterPrice * number) <= msg.value, "Ether value sent is not correct");
        require(currentId + number < 12000, "Mint would exceed maximum token amount");
        require(number <= 20, "Cannot mint more than 20 tokens at once");
        for (uint256 i = 0; i < number; i++) {
            _safeMint(msg.sender, currentId);
    ...
    function mintWithLoot(uint256 lootId) public payable nonReentrant {
        ...
        require(claimedWithLoot[lootId] == false, "Already claimed with this loot!");
    ...
    function multiMintWithLoot(uint256[] memory lootIds) public payable nonReentrant {
        for (uint256 i = 0; i < lootIds.length; i++) {
            ...
            require(claimedWithLoot[lootIds[i]] == false, "Already claimed with this loot!");
    ...
    function withdraw() public onlyOwner {
        payable(0x4210138dBEDd7E6700Cbdd652c5cCDcFF6B22Cfc).transfer(address(this).balance);
    }
