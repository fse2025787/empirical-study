...
contract slotContract is Ownable, VRFConsumerBaseV2{
   ...
   function getReservesHel() public view returns (uint256, uint256){
    ...
    function random1(uint256 i, uint256 min, uint256 max) private view returns (uint256) {
        return min + uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, s_randomWords1[i%numWords], counter))) % (max - min + 1);
    }

    function random2(uint256 i, uint256 min, uint256 max) private view returns (uint256) {
        return min + uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, s_randomWords2[i%numWords], counter))) % (max - min + 1);
    }
    ...
    function spin(uint256 _type) public payable{
      ...
      for(uint i = 0; i<spins; i++){
        randomNumber = random(i, 1, 9);
        randomNumber1 = random1(i, 1, 9);
        randomNumber2 = random2(i, 1, 9);
        if(i%numWords == 0){
