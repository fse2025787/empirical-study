...
/**
 * @title AfricaRare NFT Contract
 * @author Beau Williams (@beauwilliams)
 * @dev Smart contract for Africarare's Norman Catherine Self-Portait Drop
 */

contract NCRARE is ERC721, Ownable, Pausable {
    ...
    // TODO: With Mic
    string public normanCatherineMetadata = "https://bafkreihixaahkrkvbe735onhs27hc2kurpwua66dtzb6eajaebqtjxesbm.ipfs.nftstorage.link/";
    ...
    //Constructor
    constructor() ERC721("Norman Catherine Self Portrait", "NCRARE") {
    }
    ...
    */
    function mintSingleNFT(string memory URI) internal onlyOwner {
    ...
    */
    function mintToAddressSingleNFT(string memory URI, address receiverAddress) internal onlyOwner {
        uint256 tokenId = numMinted;
        require(!_exists(tokenId), "That token ID has already been minted. Please try again.");
        ++numMinted;
    ...
    */
    function mintBatchOfNFT(uint256 numToMint) external onlyOwner {
        for (uint i=0; i < numToMint; i++) {
            mintSingleNFT(normanCatherineMetadata);
        }
    }
    ...
    */
    function mintToAddressBatchOfNFT(uint256 numToMint, address receiverAddress) external onlyOwner {
        for (uint i=0; i < numToMint; i++) {
            mintToAddressSingleNFT(normanCatherineMetadata, receiverAddress);
        }
    }
