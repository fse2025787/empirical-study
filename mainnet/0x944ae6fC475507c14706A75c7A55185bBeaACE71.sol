...
contract StakingLand is  OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, GlobalsAndUtils {
    ...
    function setLandAndGenesis() onlyOwner public {
        land = IERC721(LAND_ADDRESS);
        genesis = IERC721(GENESIS_ADDRESS);
    }
     ...
     */
    function stakeLand(uint256 tokenId, uint256 size, string calldata rarity, uint256[] calldata genesisTokenIds, bytes32[] calldata proof) whenNotPaused external nonReentrant CampaignOnGoing {
       require(size.mul(size) == genesisTokenIds.length, "ERR:1");
        require(isWhitelisted(rarity,size, tokenId, proof), "ERR:2");
       
       unchecked {
        latestStakeId += 1;
        stakes[msg.sender][latestStakeId].stakedAt = block.timestamp;
        stakes[msg.sender][latestStakeId].tokenId = tokenId;
        stakes[msg.sender][latestStakeId].rarity = rarity;
        stakes[msg.sender][latestStakeId].size = size;
        stakes[msg.sender][latestStakeId].genesisTokenIds = genesisTokenIds;
        userStakeIds[msg.sender].push(latestStakeId);
       }
        land.transferFrom(msg.sender,address(this), tokenId);
        //  for (uint256 index = 0; index < genesisTokenIds.length; index++) {
        //     genesis.transferFrom(msg.sender, address(this),  genesisTokenIds[index]);
        // }
        genesis.bulkTransfer(genesisTokenIds, address(this), msg.sender);
    ...
    function claimUnstake(uint256 _stakeId) whenNotPaused nonReentrant public {
        ...
        uint256 unixStakedTime =  block.timestamp.sub(usrStake.appliedAt);

           // Transfer payout amount to stake owner
        require(unixStakedTime >= MIN_STAKE_DAYS, "can not claim");
        
        land.transferFrom(address(this), msg.sender, usrStake.tokenId);
        //  for (uint256 index = 0; index < usrStake.genesisTokenIds.length; index++) {
        //     genesis.transferFrom(msg.sender, address(this),  usrStake.genesisTokenIds[index]);
        // }
        genesis.bulkTransfer(usrStake.genesisTokenIds,  msg.sender, address(this));
...
/**
    ...
    function userClaimable(address stakeOwner, uint256 stakeId) public view returns(uint256) {
        ...
        uint256 usrStakedAmount =  XticketEarnPerDay[usrStake.rarity][usrStake.size];
 ...
 /**
    ...
    function joinRuffle() whenNotPaused external ClaimXTicketAllowed returns (uint256 newClaimedTickets) {
        ...
        for (uint256 x=0; x < userStakeIds[msg.sender].length; x++) {
          ...
          totalClaimableTickets += newClaimedTickets;
        totalUserXTickets[msg.sender][latestCampaignId] =  newClaimedTickets;
    ...
    function isWhitelisted(string calldata rarity,  uint256 size, uint256 tokenId,bytes32[] calldata proof) public view returns (bool) {
        return _verify(_leaf(tokenId, rarity, size), proof, landRootHash);
    }
    function _leaf(uint256 tokenId, string calldata rarity, uint256 size) public pure returns (bytes32) {
     ...
     * @return Number of tickets claimed against stake
     */
    function _getClaimableXTickets(address _stakeOwner, uint256 _stakeId) private view returns(uint256) {
        Stake memory usrStake = stakes[_stakeOwner][_stakeId];
        uint256 stakedDays;
        if (
            campaigns[latestCampaignId].endTime == 0 ||
        ...
        uint256 claimableTickets = stakedDays.mul(XticketEarnPerDay[usrStake.rarity][usrStake.size]);
...
interface IERC721 is IERC165 {
    ...
    function bulkTransfer(uint256[] calldata tokenIds, address _to, address from) external;
}
