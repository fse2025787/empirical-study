    ...
    TokensRecoverable
{
    ...
    mapping(uint32 => uint256) public BLLPoints;
    ...
    function balanceOfNFT(address account) external view returns (uint256) {
        uint32[] memory idsStaked = tokenIdsStaked(account);
        return idsStaked.length;
    }
    ...
    {
        return _tokenIdsStaked[account];
    }

    function syncPointsForTokenIDs(uint32[] memory nftIDs)
        external
        returns (bool)
    {
        for (uint256 i; i < nftIDs.length; i = unchecked_inc(i)) {
            BLLPoints[nftIDs[i]] = BLLContract
                .getPointsForTokenID(nftIDs[i])
                .mul(1e18);
        }
        return true;
    }

    function syncPointsForTokenIDsRange(uint32 startNFTID, uint32 endNFTID)
        external
        returns (bool)
    {
    ...
    function stake(uint32[] memory tokenIds)
        external
        nonReentrant
        whenNotPaused
    ...
    function _withdrawAll() internal updateReward(_msgSender()) {
        ...
        for (uint256 i; i < tokenIDarr.length; i = unchecked_inc(i)) {
            uint32 tokenId = tokenIDarr[i];
            totalValue = totalValue.add(BLLPoints[tokenId]);
