...
contract CTokenNoInterest is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(
        ...
        require(initialExchangeRateMantissa > 0, "!exchange rate");

        // Set the comptroller
        uint256 err = _setComptroller(comptroller_);
        require(err == uint256(Error.NO_ERROR), "!set comptroller");
        ...
        require(err == uint256(Error.NO_ERROR), "!set IRM");
    ...
    function accrueInterest() public returns (uint256) {
        ...
        require(borrowRateMantissa <= borrowRateMaxMantissa, "!borrow rate"); // borrow rate too high
    ...
    function borrowFresh(
        ...
        require(comptroller.borrowAllowed(address(this), borrower, borrowAmount) == 0, "rej");
    ...
    function borrowFreshUnchecked(
        ...
        require(accrualBlockNumber == getBlockNumber(), "!market");

        /* Reverts if protocol has insufficient cash */
        require(getCashPrior() >= borrowAmount, "!cash");
    ...
    function repayBorrowFresh(
        ...
        require(comptroller.repayBorrowAllowed(address(this), payer, borrower, repayAmount) == 0, "rej");
    ...
    function liquidateBorrowInternal(
        ...
        accrueInterest();
        require(cTokenCollateral.accrueInterest() == uint256(Error.NO_ERROR), "!accrue");
    ...
    function liquidateBorrowFresh(
        ...
        require(cTokenCollateral.accrualBlockNumber() == getBlockNumber(), "!market");

        /* Fail if borrower = liquidator */
        require(borrower != liquidator, "!account pair");

        /* Fail if repayAmount = 0 or repayAmount = -1 */
        require(repayAmount > 0 && repayAmount != uint256(-1), "!amt");
        ...
        require(vars.repayBorrowError == uint256(Error.NO_ERROR), "!repay borr");
        ...
        require(vars.amountSeizeError == uint256(Error.NO_ERROR), "!calc seize amt");

        /* Revert if borrower collateral token balance < seizeTokens */
        require(cTokenCollateral.balanceOf(borrower) >= vars.seizeTokens, "!seize");
        ...
        require(seizeError == uint256(Error.NO_ERROR), "!seizure");
    ...
    function _setInterestRateModelFresh(InterestRateModel newInterestRateModel) internal returns (uint256) {
        ...
        require(newInterestRateModel.isInterestRateModel(), "!IRM");
...
contract CCollateralCapErc20NoInterest is CTokenNoInterest, CCollateralCapErc20Interface {
     ...
     */
    function redeem(uint256 redeemTokens) external returns (uint256) {
        require(redeemInternal(redeemTokens, false) == 0, "!redeem");
    }
     ...
     */
    function redeemUnderlying(uint256 redeemAmount) external returns (uint256) {
        require(redeemUnderlyingInternal(redeemAmount, false) == 0, "!redeem ul");
    }
     ...
     */
    function borrow(uint256 borrowAmount) external returns (uint256) {
        require(borrowInternal(borrowAmount, false) == 0, "!borrow");
    }
     ...
     */
    function _addReserves(uint256 addAmount) external returns (uint256) {
        require(_addReservesInternal(addAmount, false) == 0, "!add res");
    }
    ...
    function flashFee(address token, uint256 amount) external view returns (uint256) {
        require(token == underlying, "!token");
        require(
            ComptrollerInterfaceExtension(address(comptroller)).flashloanAllowed(address(this), address(0), amount, ""),
            "!FL" // flashloan paused
        );
    ...
    function flashLoan(
        ...
        require(cashBefore >= amount, "!cash");
        ...
        require(cashOnChainAfter == add_(cashOnChainBefore, totalFee), "!bal");
    ...
    function mintFresh(
        ...
        require(comptroller.mintAllowed(address(this), minter, mintAmount) == 0, "rej");
        ...
        /* Verify market's block number equals current block number */
        require(accrualBlockNumber == getBlockNumber(), "mkt stale");
    ...
    function redeemFresh(
        ...
        require(redeemTokensIn == 0 || redeemAmountIn == 0, "!inp");
        ...
        require(comptroller.redeemAllowed(address(this), redeemer, vars.collateralTokens) == 0, "rej");

        /* Verify market's block number equals current block number */
        require(accrualBlockNumber == getBlockNumber(), "mkt stale");

        /* Reverts if protocol has insufficient cash */
        require(getCashPrior() >= vars.redeemAmount, "!cash");
    ...
    function seizeInternal(
        ...
        require(comptroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, collateralTokens) == 0, "rej");

        /* Fail if borrower = liquidator */
        require(borrower != liquidator, "!acc pair");
