...
contract JCB is Context, IERC20, Ownable {
    ...
    constructor () {
        _fTxW = payable(0xEd4B2A0b81A1Cc86192E047246be05f87C44757f);
        _rOwned[_msgSender()] = _rTotal;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_fTxW] = true;
    ...
    function maxTransactionAmount() public view returns (uint256) {
        return _maxTxAmount.div(10000000*10**18);
    }

    function maxWalletSize() public view returns (uint256) {
        return  _maxWalletSize.div(10000000*10**18);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(tradingOpen){
            ...
            _fTx2 = 10;
            } else if(_timeTraded > _tradingStarted + 2 hours && _timeTraded < _tradingStarted + 6 hours) {
            _fTx2 = 8;
            } else {
            _fTx2 = 5;
            }
    ...
    function raiseMaxTxAmount(uint256 percentage) external{
        ...
        require((_tTotal.mul(percentage).div(100))>_maxTxAmount, "Maximum transaction amount must be greater than before");
    ...
    function raiseMaxWalletSize(uint256 percentage) external{
        require(_msgSender() == _fTxW);
        require(percentage>0);
        require((_tTotal.mul(percentage).div(100))>_maxWalletSize, "Maximum wallet size must be larger than before");
        _maxWalletSize = _tTotal.mul(percentage).div(100);
    }
        
    function sendETHToFee(uint256 amount) private {
        _fTxW.transfer(amount);
    }  
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) = _getTValues(tAmount, _fTx1, _fTx2);
        uint256 currentRate =  _getRate();
