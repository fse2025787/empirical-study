...
// The DMEX Custody Contract
contract DMEX_Custody {
    ...
    address public AMBMediatorContract; // AMB token bridge
    address public DMEX_XDAI_CONTRACT;
    address public WETH9_ADDRESS;
    ...
    function deposit() public payable {
        ...
        // Wrap ETH
        WETH9(WETH9_ADDRESS).deposit{value: msg.value};
        //sendDepositInstructionToAMBBridge(msg.sender, WETH9_ADDRESS, msg.value);
    }
    ...
    function sendDepositInstructionToAMBBridge(address user, address token, uint256 amount) internal
    {
        ...
        Token(token).approve(AMBMediatorContract, amount);
        ...
        GnosisMediator(AMBMediatorContract).relayTokensAndCall(token, DMEX_XDAI_CONTRACT, amount, data);
        //bytes32 msgId = IAMB(AMBBridgeContract).requireToPassMessage(DMEX_XDAI_CONTRACT, data, gas);

        emit Deposit(token, user, amount); // fires the deposit event
    }    
    ...
    function withdrawForUser(
        ...
        emit Withdraw(token, user, amount); // fires the withdraw event
    }
    ...
    modifier onlyAMBBridge {
        if (msg.sender != AMBMediatorContract) revert();
        _;
    }
    ...
    constructor(uint256 inactivityReleasePeriod_, address DMEX_XDAI_CONTRACT_, address AMBMediatorContract_, address WETH9_ADDRESS_) {
        owner = msg.sender;
        inactivityReleasePeriod = inactivityReleasePeriod_;
        DMEX_XDAI_CONTRACT = DMEX_XDAI_CONTRACT_;
        AMBMediatorContract = AMBMediatorContract_;
        WETH9_ADDRESS = WETH9_ADDRESS_;
    }
