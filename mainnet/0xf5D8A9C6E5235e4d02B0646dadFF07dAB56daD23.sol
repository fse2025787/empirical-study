...
contract Deepfake is ERC1155Supply, AccessControlEnumerable, Ownable {
    ...
    event MintStateActivated(uint256 tokenId);
    event MintStateDeactivated(uint256 tokenId);
    event PriceUpdated(uint256 tokenId);
    ...
    {
        require(tokenData[_tokenId].approved, "startClaim: claim already live");
        require(
            tokenData[_tokenId].mintState == MintState.OFF,
            "mint: mint must be off"
        );
        tokenData[_tokenId].mintState = MintState.ACTIVE;
        emit MintStateActivated(_tokenId);
    }
    ...
    function setMintStateOff(uint256 _tokenId) external onlyRole(MANAGER_ROLE) {
        require(tokenData[_tokenId].approved, "pauseClaim: claim not live");
        require(
            tokenData[_tokenId].mintState == MintState.ACTIVE,
            "mint: mint must be active"
        );
        tokenData[_tokenId].mintState = MintState.OFF;
        emit MintStateDeactivated(_tokenId);
    }
