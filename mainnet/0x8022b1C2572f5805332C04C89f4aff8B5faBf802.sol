...
contract HippoBaby is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    event CreateHippoBaby(uint256 indexed id);
    event AttributeChanged(uint256 indexed _tokenId, string _key, string _value);

    constructor(string memory baseURI) ERC721("HypeHippos Baby Edition", "HypeHippoBaby") {
        setBaseURI(baseURI);
        pause(true);
        presale = false;
        sale = false;
    ...
    modifier onlyBreeding {
        require(!paused(), "Pausable: paused");
        require(_msgSender() == breedingContract, "Invalid breeding permission");
        _;
    }
    ...
    function burn(uint256 tokenId) public virtual override {
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721Burnable: caller is not owner nor approved");
        _burn(tokenId);
        // call burn baby of breeding contract
        IBreeding(breedingContract).burnBaby(tokenId);
    }
    ...
    function breedingMint(address _to) public payable onlyBreeding returns(uint) {
        _mintAnElement(_to);
        return _tokenIdTracker.current();
    }
    ...
    function setBreedingContract(address breeding) external onlyOwner {
    ...
    */
    ...
    function changeAttribute(uint256 tokenId, string memory key, string memory value) public payable {
        address owner = ERC721.ownerOf(tokenId);
        require(_msgSender() == owner, "This is not your Hippos.");
...
interface IBreeding {
    function burnBaby(uint256 babyNftId) external;
}
