...
contract Kills is ReentrancyGuard, ERC20 {
    ...
    struct JoinLuckyChange {
        address addressLuckyChange;
        string prizeLuckyChange;
    }
    
    mapping(string => JoinLuckyChange[]) public LuckyChange;
    ...
    uint256 public LuckyChangePrice;
    bool public isRaffle; 
    bool public isLuckyChange; 
    ...
    function onLuckyChange(uint256 amount,string memory luckyChangeName) external nonReentrant returns(string memory) {
        require(isLuckyChange, "not Active");
        require(amount >= LuckyChangePrice, "Not Enough $Kills");
        require(MechaStake[msg.sender].deposit >= amount, "Not Enough $Kills");
        uint256 burn = amount;
        uint256 random = generateRandomNumberLuckyChange(luckyChangeName);
        if(keccak256(abi.encodePacked(LuckyChange[luckyChangeName][random].prizeLuckyChange)) != keccak256(abi.encodePacked(Empty)))
        {
            if(LuckyChange[luckyChangeName][random].addressLuckyChange == address(0))
            {
                LuckyChange[luckyChangeName][random].addressLuckyChange = msg.sender;
                return LuckyChange[luckyChangeName][random].prizeLuckyChange;
            }
            else{
    ...
    function setIsFeature(bool _isDeposit,bool _isFeature , bool _isLuckyChange)public onlyOwner {
        isRaffle =_isDeposit; 
        isFeature =_isFeature; 
        isLuckyChange =_isLuckyChange; 
    }
    
    function setPrizeLucky(string memory LuckyChangeName,string[] memory prizeLuckyChange) public onlyOwner {
        for (uint256 i = 0; i < prizeLuckyChange.length; ++i) {
            JoinLuckyChange memory newStruct = JoinLuckyChange(address(0),prizeLuckyChange[i]);
            LuckyChange[LuckyChangeName].push(newStruct);
        }
    }  
    ...
    function generateRandomNumberLuckyChange(string memory raffleName) internal view returns (uint) {
        return uint256(keccak256(abi.encodePacked(block.timestamp))) % (LuckyChange[raffleName].length);
    }
    
    function generateRandomNumberCoin() internal view returns (uint) {
        return uint256(keccak256(abi.encodePacked(block.timestamp))) % (LuckyChangePrice);
    }
    ...
    function setAddressCollection(IERC721 _mechaApe,IERC721 _mechaHound) public onlyOwner {
        IMechaApe = IERC721(_mechaApe);
        IMechaHound = IERC721(_mechaHound);
    }
    ...
    function Stake(uint256[] calldata _idMechaApe , uint256[] calldata _idMechaHound) external nonReentrant {
        ...
        for (uint256 i; i < lenMechaHound; ++i) {
            ...
            MechaHoundStakeToken[msg.sender].push(_idMechaApe[i]);
        }
    ...
    function setLuckyChangePrice(uint256 _luckyChangePrice) public onlyOwner {
        LuckyChangePrice = _luckyChangePrice;
    }

    function setRafflePrice(string memory raffleName, uint256 _raffleChangePrice) public onlyOwner {
        RafflePrice[raffleName] = _raffleChangePrice;
    }

    function setEmptyLuckyChange(string memory emptyName) public onlyOwner {
        Empty = emptyName;
    }
