...
// File: contracts\interfaces\IPancakeRouterSpec.sol

pragma solidity >=0.6.2;

interface IPancakeRouterSpec {
...
contract PancakeRouterSpec is IPancakeRouterSpec {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IPancakePair(PancakeLibrary.pairFor(factory, input, output)).swap(
                amount0Out, amount1Out, to, new bytes(1)
            );
        }
    }
    function swapExactTokensForTokens(
...
// helper methods for interacting with ERC20 tokens and sending ETH that that do not consistently return true/false
