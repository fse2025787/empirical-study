...
/*
PUZLPunks by  
.______    __    __   ________   __      ____    __    ____  ______   .______       __       _______  
|   _  \  |  |  |  | |       /  |  |     \   \  /  \  /   / /  __  \  |   _  \     |  |     |       \ 
|  |_)  | |  |  |  | `---/  /   |  |      \   \/    \/   / |  |  |  | |  |_)  |    |  |     |  .--.  |
|   ___/  |  |  |  |    /  /    |  |       \            /  |  |  |  | |      /     |  |     |  |  |  |
|  |      |  `--'  |   /  /----.|  `----.   \    /\    /   |  `--'  | |  |\  \----.|  `----.|  '--'  |
| _|       \______/   /________||_______|    \__/  \__/     \______/  | _| `._____||_______||_______/ 
...
contract PuzlPunkCollection is ERC721Enum, Ownable, ReentrancyGuard {
  ...
  // internal
   ...
   function updatePuzlBuilder(address _puzlbuilder) public onlyOwner {
        PUZL_BUILDER = _puzlbuilder;
    }
  ...
  function seedInitialContractState(address[] memory tokenOwners, uint256[] memory tokens) public onlyOwner {
    ...
    function mintPunk(address recipient)  external returns (uint256) {
        require(msg.sender == PUZL_BUILDER, "Operator bad.");
        uint256 total = totalSupply();
        require(total <= MAX_NFT, "Max limit");
        _safeMint(recipient, total + 1);
        return (total + 1);
    }
