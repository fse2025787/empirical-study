...
contract pandaBallers is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    string internal baseuri = "ipfs://QmRovsBajngcVQsXapaMdc595EqpjLFhnk3tLiMiNPWQiD/";
    string internal notRevealeduri = ""; //not reveal uri not provided yet
    bool public revealed = true;
    bool public whiteListsSale = true;

    mapping(address => bool) whitelist; // mapping for whitelisted addresses

    
    constructor() ERC721("Panda Ballers NFT", "PB") {
        NFTPrice = 0.08 ether;                    
        NFTMaxSupply = 1000;
        mint(10);
        _pause();
    ...
    returns (string memory)
  {
    ...
    //function for whitelisted addresses to mint NFT

    function whitelistSale(uint256 _countNfts) public payable {
        require (whiteListsSale == true, "white list sales period ended");
        require (whitelist[msg.sender] == true, "not a whitelisted address");
        uint NFTsPrice = NFTPrice * _countNfts;
        address to = msg.sender; 
        require(totalSupply() + _countNfts <= 500, "NFT: Max Supply in whitelist reached"); //max limit of mint during whitelist sale is 500
        require(_countNfts > 0, "NFT: No of NFTs must be greater then zero");
        require(msg.value == NFTsPrice || msg.sender == owner(), "Please enter correct price");
    ...
    // starting or ending whitelist sale

    function setWhitelistSale() external onlyOwner {
        whiteListsSale = !whiteListsSale;
    }
