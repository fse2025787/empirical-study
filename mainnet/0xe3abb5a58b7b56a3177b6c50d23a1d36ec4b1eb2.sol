...
contract VoltApeNFTClub is ERC721Enumerable, Ownable {

    using Strings for uint256;

    string _baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmQyeedo2NrwcoxpgjXtPpYhggTDAPQiTF6JnrbrCch5yk";
    uint256 private _price = 0.05 ether;
    bool public _paused = false;
    bool public _WLpaused = true;
    uint256 public limit = 2000;
    uint256 public maxPerTx = 10;
    
    mapping(address => bool) public WL; 
    mapping(address => uint) public WL_Mint;
    
    address public wallet1 = 0xcCF5416b4cc5b936Ed1cee4A77DdE6eC71e93338;   // 80%
    address public wallet2 = 0x521C45D2903B4d523FA169d58C10A6BdBEFd1Ea9;   // 20%


    constructor() ERC721("Volt-Ape-NFT-Club", "VANC") {}

    function Mint(uint256 num) public payable {
        uint256 supply = totalSupply();
        require( !_paused,                     "Sale paused" );
        require( _WLpaused == true,            "Whitelist minting must be paused." );
        require( num <= maxPerTx,              "You exceeds mint limit per transaction." );
        require( msg.value >= _price * num,    "Ether sent is not correct" );
        require( supply + num <= limit,        "Exceeds maximum supply" );
    ...
    function minto (address _to, uint256 _amount) private {
        ...
        require( supply + _amount <= limit,             "Exceeds maximum supply" );
    ...
    function setWallet1(address _address) public onlyOwner {
        wallet1 = _address;
    }

    function setWallet2(address _address) public onlyOwner {
        wallet2 = _address;
    }
    ...
    function withdraw() public payable onlyOwner {
        uint256 _mars1 = (address(this).balance * 800) / 1000;  // 50%  
        uint256 _mars2 = (address(this).balance * 200) / 1000;  // 50%
 

        if(_WLpaused == false){
            payable(wallet1).transfer(address(this).balance);
