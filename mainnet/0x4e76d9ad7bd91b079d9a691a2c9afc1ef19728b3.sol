...
contract pubSale is ReentrancyGuard {
    ...
    uint public tokensPerETH = 17500; // token per ETH
    uint public ending; // sale end time
    bool public presaleStarted = false; //started or not
    address public deadWallet = 0x000000000000000000000000000000000000dEaD; 
    uint public cooldownTime = 7 days; // time between withdrawals of token
   ...
   //token insertion can only happen 1 time
    ...
    function startSale(uint _presaleTime) public onlyOwner {
    ...
    function invest() public payable nonReentrant {
        ...
        require(invested[msg.sender] >= 0.1 ether, "Your investment should be more than 0.1 ETH.");
        require(invested[msg.sender] <= 2 ether, "Your investment cannot exceed 2 ETH.");
