...
// File contracts/adapters/LiquityStakingPoolAdapterV5.sol
...
contract LiquityStakingPoolAdapterV5 is IVaultAdapterV2 {
  ...
  ICurveMetaPool public lusd3crvPool;
  ...
  constructor(ILiquityStakingPool _vault, address _admin, address _sentinel, IUniswapV3Router _uniV3Router, ICurveMetaPool _lusd3crvPool, IDetailedERC20 _lqtyToken,IDetailedERC20 _wethToken, IDetailedERC20 _lusdToken,IDetailedERC20 _daiToken,  address _frontendTag, uint256 _slippage) public {
    vault = _vault;
    admin = _admin;
    sentinel = _sentinel;
    uniV3Router = _uniV3Router;
    lusd3crvPool = _lusd3crvPool;
  ...
  modifier onlyAdmin() {
    require(admin == msg.sender, "LiquityStakingPoolAdapterV5: only admin");
    _;
  }

  modifier onlySentinel() {
    require(sentinel == msg.sender, "LiquityStakingPoolAdapterV5: only sentinel");
    _;
  }
  ...
  function withdraw(address _recipient, uint256 _amount, bool _isHarvest) external override onlyAdmin {
    ...
    if(_isHarvest){
      ...
      if(lqtyToken.balanceOf(address(this)) > 0){
        ...
        IUniswapV3Router.ExactInputSingleParams memory params0 = IUniswapV3Router.ExactInputSingleParams(
          address(lqtyToken),
          address(wETHToken),
          3000,
          address(this),
          block.timestamp+800,
          lqtyToken.balanceOf(address(this)),
          0,
          0);


        uniV3Router.exactInputSingle(params0);
        ...
        uniV3Router.exactInputSingle(params1);


        lusd3crvPool.exchange_underlying(1,
                                         0,
      ...
      if(address(this).balance > 0){
        ...
        uniV3Router.exactInputSingle{value:address(this).balance}(params2);
  ...
  function updateApproval() public {
    ...
    // lqty -> uniV3
    lqtyToken.safeApprove(address(uniV3Router), uint256(-1));
    wETHToken.safeApprove(address(uniV3Router), uint256(-1));
    // dai -> crvPool
    daiToken.safeApprove(address(lusd3crvPool), uint256(-1));
  ...
  function handoverSentinel(address newSentinel) public onlySentinel{
    require(newSentinel != address(0), "LiquityStakingPoolAdapterV5: sentinel address cannot be 0x0.");
