...
contract TsangnyonLPStaking is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event PoolUpdated(
        uint256 _blocksRewarded,
        uint256 _amountRewarded,
        uint256 _time
    );
    ...
    IERC20 public spiritToken;

    address public rewardPool;
    address public owner;

    uint256 public rewardPerBlock;
    uint256 public accSpiritPerShare;
    uint256 public lastRewardBlock;
    uint256 public totalStaked;

    mapping(address => User) public userDetails;

    // Constructor will set the address of spirit/ETH LP token
    constructor(
        address _LPToken,
        address _SpiritToken,
        address _rewardPool,
        uint256 _rewardPerBlock,
        uint256 _blocksToWait
    ) {
    ...
    // Sets spirit reward for each block
    ...
    function getUserBalance(address _staker)
        external
        view
    ...
    function pendingRewards(address _staker) external view returns (uint256) {
        ...
        uint256 _accSpiritPerShare = accSpiritPerShare;

        if (block.number > lastRewardBlock && totalStaked != 0) {
            uint256 blocksToReward = block.number.sub(lastRewardBlock);
            uint256 spiritReward = blocksToReward.mul(rewardPerBlock);
            _accSpiritPerShare = _accSpiritPerShare.add(
                spiritReward.mul(1e18).div(totalStaked)
            );
        }

        return
            user._LPDeposited.mul(_accSpiritPerShare).div(1e18).sub(
                user._rewardDebt
            );
    }

    // Function that updates Tsangnyon/eth LP pool
    function updatePool() public returns (bool) {
        ...
        spiritToken.safeTransferFrom(rewardPool, address(this), spiritReward);

        emit PoolUpdated(blocksToReward, spiritReward, block.timestamp);

        return true;
    }

    // Function that lets user stake Tsangnyon/eth LP
    function stakeLP(uint256 _amount) external nonReentrant returns (bool) {
        ...
        if (user._LPDeposited > 0) {
            uint256 _pendingRewards = user
                ._LPDeposited
                .mul(accSpiritPerShare)
                .div(1e18)
                .sub(user._rewardDebt);

            if (_pendingRewards > 0) {
                spiritToken.safeTransfer(msg.sender, _pendingRewards);
                emit RewardsClaimed(
                    msg.sender,
                    _pendingRewards,
                    block.timestamp
        ...
        user._rewardDebt = user._LPDeposited.mul(accSpiritPerShare).div(1e18);
    ...
    // Function that lets user unstake Tsangnyon/eth LP in system
