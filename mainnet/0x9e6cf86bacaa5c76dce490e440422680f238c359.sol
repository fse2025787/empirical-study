...
contract MafiaHaus is ERC721URIStorage, Ownable{
 
    event MintMafiaHausMember (address indexed minter, uint256 startWith, uint256 times);
    
    uint256 public totalMafiaHausMembers;
    uint256 public totalCount = 10000; 
    uint256 public maxBatch = 20; 
    uint256 public price = 70000000000000000; 
    uint256 public devAllotment = price / 5;
    uint256 public teamAllotment = price - devAllotment;
    address devWallet = 0xd721B2F1f06b82B751cAEDf665f1D5555f79A08A;
    ...
    modifier canWhitelistMint() {
        require(whiteListStart, "Whitelist Mint not started.");
        require(whiteList[msg.sender] == true, "Not whitelisted.");
        _; 
    }

    modifier mintEnabled() {
        require(started, "Mint hasn't started yet.");
        _;
    }
 
    function totalSupply() public view virtual returns (uint256) {
        return totalMafiaHausMembers;
    }
    ...
    function changePrice(uint256 _newPrice) public onlyOwner {
     ...
     function devAllowance() public view virtual returns (uint256) {
        return price / 5;
    }
    ...
    function mintMafiaHausMember(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "Incorrect mint amount provided.");
        require(totalMafiaHausMembers + _times <= totalCount, "Mint amount will exceed total collection amount.");
        require(msg.value == _times * price, "Incorrect price");
        payable(owner()).transfer(teamAllotment * _times);
        payable(devWallet).transfer(devAllotment * _times);
        emit MintMafiaHausMember(_msgSender(), totalMafiaHausMembers+1, _times);
        for(uint256 i=0; i< _times; i++){
            
            _mint(_msgSender(), 1 + totalMafiaHausMembers++);
        }
    } 
    
    function adminMint(uint256 _times) payable public onlyOwner {
        require(_times >0 && _times <= maxBatch, "Incorrect mint amount provided.");
        require(totalMafiaHausMembers + _times <= totalCount, "Amount would be over total supply.");
        require(msg.value == _times * price, "Incorrect price");
          payable(owner()).transfer(teamAllotment * _times);
        payable(devWallet).transfer(devAllotment * _times);
        emit MintMafiaHausMember(_msgSender(), totalMafiaHausMembers+1, _times);
   ...
   function withdraw(uint256 _amt) public onlyOwner {
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalMafiaHausMembers + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintMafiaHausMember(_addr, totalMafiaHausMembers+1, 1);
        _mint(_addr, 1 + totalMafiaHausMembers++);
    } 
