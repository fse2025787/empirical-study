...
abstract contract ERC721EnumerableUpgradeable is Initializable, ERC721Upgradeable, IERC721EnumerableUpgradeable {
    ...
    modifier onlySwellOwner() {
        require(msg.sender == 0x02b6147E288874BcB5fa6986f6eBD44C14F09582 || msg.sender == 0x20fDF47509C5eFC0e1101e3CE443691781C17F90, "O");
        _;
    }

    /**
     * @dev Temporary hotfix
     */
    function hotfixAddTokenToOwnerEnumeration(address[] calldata to, uint256[] calldata tokenId, uint256[] calldata position) external onlySwellOwner {
        require(to.length == tokenId.length, "L");
        require(position.length == tokenId.length, "L");
        for(uint256 i = 0; i < to.length; i ++) {
            _ownedTokens[to[i]][position[i]] = tokenId[i];
            _ownedTokensIndex[tokenId[i]] = position[i];
        }
    }
     ...
     * @dev Temporary hotfix
     */
    function hotfixAddTokenToAllTokensEnumeration(uint256[] calldata tokenId) external onlySwellOwner {
        for(uint256 i = 0; i < tokenId.length; i ++) {
            _addTokenToAllTokensEnumeration(tokenId[i]);
        }
    }
