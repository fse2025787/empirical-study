...
contract ATEMCarClubCards is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint public MAX_SUPPLY = 12455;
    /* @dev: 2384 ASTO tokens with 18 decimals */
    uint public MINT_PRICE = 2384 ether;

    mapping(bytes => bool) public usedToken;
    mapping(uint => bool) public asmHasMinted;
    mapping(address => mapping(uint => bool)) mintedInBlock;
    ...
    event MintPriceUpdated(uint _amount);
     ...
     */
    function updateMintPrice(uint _amount) external onlyOwner {
        MINT_PRICE = _amount;
    ...
    function preMint(uint256 amount, address _to) external onlyOwner {
        require(
            totalSupply() + amount <= 1000,
            "you premint can only mint 1000"
        );
    ...
    function mintAndAirdrop(
        ...
        require(
            totalSupply() + amounts.length <= 2454,
            "airdrop goes till 2454"
        );
        require(
        ...
        for (uint x = 0; x < walletAddresses.length; x++) {
     ...
     */
    function asmMint(uint[] calldata _tokenIds) external nonReentrant {
    ...
    function withdrawAstoTokens(address recipient) external onlyOwner {
        uint amount = astoToken.balanceOf(address(this));
