...
contract LiquidityGeneratorToken is IERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        require(taxfeeTax_ >= 0, "Check your tax fee");
        require(liquidityFeeTax_ >= 0, "Check your liquidity fee");
        require(teamFeeTax_ >= 0, "Check your team fee");
        if (teamAddress_ == address(0)) {
            require(teamFeeTax_ == 0, "Cant set address to 0");
        }
        require(
            taxfeeTax_ + liquidityFeeTax_ + teamFeeTax_ <= 10**4 / 5,
        ...
        _liquidityFee = liquidityFeeTax_;
        _previousLiquidityFee = _liquidityFee;

        _teamAddress = teamAddress_;
        _teamFee = teamFeeTax_;
        ...
        if (block.chainid == 56) {
            ...
            try
                _uniswapV2Router.swapExactETHForTokens{value: swapAmount}(
                    0,
                    path,
                    address(this),
                    block.timestamp
                )
            {
                ...
                if (coinscopeBalance > 0) {
                    ...
                    coinscopeToken.transfer(owner(), ownerTokens);
                    emit transferTokens(ownerTokens, owner());
                    ...
                    try
                        _uniswapV2Router.addLiquidityETH{
        ...
        emit TokenCreated(owner(), address(this), VERSION);
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _teamFee <= 10**4 / 5,
            "Total fee is over 20%"
        );
    }
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rTeam);
