    ...
    ERC721HolderUpgradeable
{
    ...
    address private sewerPassAddress;
    address private sewerPassClaimAddress;
    ...
    event MintedSewerPass(
        address indexed from,
        uint256 indexed tokenId,
        uint64 indexed sewerPassId
    );
    ...
    function setSewerPassAddress(address _sewerPassAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        sewerPassAddress = _sewerPassAddress;
    }

    function setSewerPassClaimAddress(address _sewerPassClaimAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        sewerPassClaimAddress = _sewerPassClaimAddress;
    }

    function mintSewerPass(uint256 tokenId) external {
        require(_exists(tokenId), "Wrapped token does not exist");
        require(
            ownerOf(tokenId) == msg.sender,
            "Only wrapped token owner can call this function"
        ...
        ISewerPassClaim sewerPassClaim = ISewerPassClaim(sewerPassClaimAddress);
        require(
            sewerPassClaim.checkClaimed(0, tokenId) == false,
            "Sewer Pass already claimed for this token"
        );
        IBAYCSewerPass sewerPass = IBAYCSewerPass(sewerPassAddress);
        uint64 sewerPassTokenId = sewerPass.mintIndex();
        require(
            sewerPass.exists(sewerPassTokenId) == false,
            "Sewer pass exists"
        );
        sewerPassClaim.claimBayc(tokenId);
        sewerPass.safeTransferFrom(address(this), msg.sender, sewerPassTokenId);

        emit MintedSewerPass(msg.sender, tokenId, sewerPassTokenId);
    }
}
...
interface ISewerPassClaim {
     ...
     */
    function checkClaimed(uint8 collectionId, uint256 tokenId)
        external
        view
        returns (bool);
}