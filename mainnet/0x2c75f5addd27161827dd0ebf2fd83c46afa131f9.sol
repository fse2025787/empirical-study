...
contract POPCORN is Context, IERC20, Ownable { 
    ...
    uint private constant _totalSupply = 10000000 * 10**decimals;

    string public constant name = unicode"POPCORN INU"; 
    ...
    function _transfer(address from, address to, uint amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if((_launchedAt + (0 seconds)) > block.timestamp) {
                ...
                if((_launchedAt + (0 seconds)) > block.timestamp) {
                    ...
                    require(cooldown[to].buy < block.timestamp + (0 seconds), "Your buy cooldown has not expired.");
                }
            ...
            if(!_inSwap && _tradingOpen && from != uniswapV2Pair) {
                require(cooldown[from].buy < block.timestamp + (0 seconds), "Your sell cooldown has not expired.");
    ...
    function openTrading() external onlyOwner() {
        ...
        _maxBuyAmount = 300001 * 10**decimals;
        _maxHeldTokens = 300001 * 10**decimals;
    }
