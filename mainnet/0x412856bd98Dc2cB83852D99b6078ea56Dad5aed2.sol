 ...
 * _Available since v3.1._
 */
contract ERC1155X is Context, ERC165, ERC2981, IERC1155, IERC1155MetadataURI {
    using Address for address;
    event BlacklistUpdate(address indexed addy, bool is_blacklisted);
    event NonReselableTokenUpdate(uint256 indexed tokenId, bool reselable);
    ...
    mapping(uint256 => bool) public non_resalable_tokens;
    ...
    function safeTransferFrom(
        ...
        if (from != _msgSender()) {
            /* This transfer was initiated by an operator (not the owner), aka  marketplace.
            *  We can block the non-reselable tokens here
            */
            require(!non_resalable_tokens[id], "This token is non-reselable.");
        }
        ...
        if (from != _msgSender()) {
            ...
            */
            for (uint256 i; i < ids.length; ++i) {
                require(!non_resalable_tokens[ids[i]], "One or more tokens are non-reselable.");
            }
        }        
    ...
    function _updateNonReselableTokens(uint256 tokenId, bool non_reselable) internal virtual {
        non_resalable_tokens[tokenId]=non_reselable;
        emit NonReselableTokenUpdate(tokenId, non_reselable);
    }    
 ...
 * _Available since v3.1._
 */
abstract contract ERC1155PausableX is ERC1155X, Pausable {
    /**
 ...
 * _Available since v3.1._
 */
abstract contract ERC1155BurnableX is ERC1155X {
    function burn(
        address account,
        uint256 id,
        uint256 value
