...
// File: contracts/SaconiStakingAgent.sol
...
contract SaconiStakingAgent is Ownable {
    ...
    // Saconi Generative Staking Vault
    address constant public SACONI_VAULT = 0x997b27eE3F9042698d25F8762b74263b809360C1;
    
    bool public canDepositNFT;
    bool public canWithdrawNFT;
    ...
    // Contract needs to be approved for all before transfering!
    function depositNFT(address tokenContract, uint256 tokenID) external {
        ...
        ITokenContract.safeTransferFrom(msg.sender, SACONI_VAULT, tokenID);
    ...
    function withdrawNFT(address tokenContract, uint256 tokenID) external {
        ...
        IERC721Enumerable(tokenContract).safeTransferFrom(SACONI_VAULT, msg.sender, tokenID);
    ...
    function withdrawMultipleNFT(address tokenContract, uint256 amount, uint256[] calldata tokenIDList) external {
        ...
        for (uint256 i=0; i<amount; i++) {
            tokenID = tokenIDList[i];
            require(NFTStakers[tokenContract][tokenID] == msg.sender, "Token not owned");
            ITokenContract.safeTransferFrom(SACONI_VAULT, msg.sender, tokenID);
            delete NFTStakers[tokenContract][tokenID];
            emit WithdrawNFT(msg.sender, tokenContract, tokenID);
        }
    }
