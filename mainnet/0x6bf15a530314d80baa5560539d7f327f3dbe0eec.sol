...
contract DELTA_Deep_Farming_Vault is DELTA_Deep_Farming_Storage, IDeepFarmingVault, Initializable {
    ...
    IERC20 constant public RLP = IERC20(0xfcfC434ee5BfF924222e084a8876Eee74Ea7cfbA);
    address constant public WITHDRAWAL_CONTRACT_MASTERCOPY = 0x251bbe8C7ABc2a1Ca8d0b25FC1149ABE6160A943;
    IPROXY_FACTORY constant public WITHDRAWAL_PROXY_FACTORY = IPROXY_FACTORY(0x370F101Ad006b10F3050ED2621924E72d1D77436);
    ...
    // Known addresses moved to constants
    ...
    function addPermanentCredits(address person, uint256 amount) override public {
        ...
        adjustBooster(personMemory, personStorage, newBooster);
    ...
    // WARNING 
    // This reads the personMemory for lastBooster
    // And assumes its old aka not updated anywhere else
    function adjustBooster(UserInformationDFV memory personMemory, UserInformationDFV storage personStorage, uint256 newBooster) internal {
        require(newBooster < 11, "DELTA_Deep_Farming_Vault: Booster too high");
        if(personMemory.lastBooster != newBooster) { // No change in booster so we dont need to write
            personStorage.lastBooster = newBooster;

            // New one is different and last booster isn't 0
            if(personMemory.lastBooster != 0) { // we don't set it here since 0 is a change to 1
                                                // And this happens every new deposit
                                                // If its 1 then 1 == 1 so it doesn't trigger
                                                // There is a case its 0 when compounding
                                                // But we trick it by setting it to max booster +1 there
                // How much time has passed since lastBooster deposit?
                // elvis because we are allowing people to boost if they went up without backwards penalty
                // Since they already got penalized by recycling
                uint256 secondsSinceLastBoosterDeposit = personMemory.lastBooster == BOOST_MAX + 1 ? 0 : block.timestamp - personMemory.lastBoosterDepositTimestamp;
                // We take the remainder of the week so people cant cheat this
                // elvis because it should be 0 even if lastBoosterDepositTimestamp is 0 aka never set
                uint256 weekRemainder = personMemory.lastBoosterDepositTimestamp > 0 ? secondsSinceLastBoosterDeposit % 1 weeks : 0;
                // if its 1.9 since last booster deposit then it will be 1 here as solidity always rounds down
                personStorage.lastBoosterDepositTimestamp = block.timestamp - weekRemainder;
            }
        }
    }
    ...
    function _deposit(UserInformationDFV memory recieverMemory, address walletReciever, uint256 amountRLP, uint256 amountDELTA, bool isBurn) internal {
        ...
        adjustBooster(recieverMemory, recieverStorage, newBooster);
    ...
    function withdrawRLP(uint256 amount) override public {
        ...
        adjustBooster(withdrawerMemory, withdrawerStorage, newBooster);
    ...
    function compoundFarmedAndHandleDELTADeposit(UserInformationDFV memory personMemory, UserInformationDFV storage personStorage, uint256 depositDELTA, uint256 farmedDELTA, uint256 calculatedBooster, bool isBurn) internal returns (uint256) { //  return new booster 
        ...
        if(burningDeposit || compondingBurn) { // We had a half and half
            ...
            if(toBurn.mul(20) >= personMemory.totalDelta && block.timestamp >= personMemory.lastBoosterDepositTimestamp + 7 days) {
                // This is nessessary because of possible case :
                // User has 10 , waits 1 week and then re-deposits
                // lastBooster is 10, new is 10, and it would not get written in the update
                // This is why we write to personMemory here so it changes
                // BOOST_MAX + 1 is impossible so its always a change!
                personMemory.lastBooster = BOOST_MAX + 1; // way to force update in adjustBooster()
            ...
            // Note we read frm memory here so updating storage above is still valid since memory will be 0
            if(personMemory.lastBoosterDepositTimestamp == 0) { // This check is still valid
