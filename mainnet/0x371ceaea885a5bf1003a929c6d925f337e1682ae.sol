...
contract CAX is ERC20 {
    ...
    CAXDividendTracker public dividendTracker;

    address public liquidityWallet;
    address payable public charityWallet = 0x7C54192429b853099229dabe0FC711b5C32fB58f;
    uint256 public swapTokensAtAmount = 10000 * (10**18); 
    uint256 public maxBuyTranscationAmount = 500000 * (10**18);
    uint256 public maxSellTransactionAmount = 75000 * (10**18);
    ...
    uint256 public immutable CharityWalletFee;
    ...
    constructor() public ERC20("Calix", "CAX") {
        uint256 _ETHRewardsFee = 4;
        uint256 _liquidityFee = 2;
        uint256 _CharityWalletFee = 3;

        ETHRewardsFee = _ETHRewardsFee;
        liquidityFee = _liquidityFee;
        CharityWalletFee = _CharityWalletFee;
        totalFees = _ETHRewardsFee.add(_liquidityFee).add(_CharityWalletFee);


    	dividendTracker = new CAXDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        CAXDividendTracker newDividendTracker = CAXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the CAX token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CAX: Account is already the value of 'excluded'");
    ...
    function changeCharityWallet(address payable newAddress) external onlyOwner {
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CAX: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue != gasForProcessing, "CAX: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(!blackList[from],"Bots are not allowed");
        require(!blackList[to],"Bots are not allowed");
        ...
        if(
            ...
            uint256 devWalletTokens = contractTokenBalance.mul(CharityWalletFee).div(totalFees);
...
contract CAXDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("CAX_Dividend_Tracker", "CAX_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "CAX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "CAX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CAX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CAX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CAX_Dividend_Tracker: Cannot update claimWait to same value");
