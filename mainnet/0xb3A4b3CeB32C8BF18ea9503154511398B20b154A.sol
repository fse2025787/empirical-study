...
interface IVaultRebalancer {
  function unload(address _pair, uint _amount) external;
...
contract Vault is TransferHelper, ReentrancyGuard, ERC20("X", "X", 18) {
  ...
  function deposit(address _account, uint _amount) external nonReentrant {
  ...
  function withdrawFromETH(address _source, uint _amount) external nonReentrant {
    _checkEthVault();
    _withdrawFrom(_source, _amount);
    _wethWithdrawTo(msg.sender, _amount);
  }
  ...
  function pushToken(address _token, uint _amount) external onlyRebalancer {
  ...
  function rebalancer() public view returns(IVaultRebalancer) {
    return IVaultRebalancer(vaultController.rebalancer());
  }
  ...
  function _withdrawFrom(address _source, uint _amount) internal {
    ...
    rebalancer().unload(_source, _amount - selfBalance);
    _withdraw(msg.sender, _amount);
  }
  ...
  function _checkDepositLimit() internal view {
    require(vaultController.depositsEnabled(), "Vault: deposits disabled");

    uint depositLimit = vaultController.depositLimit(address(this));

    if (depositLimit > 0) {
      require(totalSupply <= depositLimit, "LendingPair: deposit limit reached");
    }
  }
}