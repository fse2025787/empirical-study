...
contract LockContract is Ownable {
    ...
    uint256 public requiredPayment = 10000000000000000;
    uint256 public id = 0;
    ...
    function lockFunds(address othertoken, uint256 otherAmount, uint256 lockTime) public payable {
        require(msg.value >= requiredPayment, 'invalid eth amount paid');
        require(otherAmount > 0, 'invalid amount');
        payable(treasury).transfer(msg.value);

        // transfer LPs in smart contract
        IBEP20(othertoken).safeTransferFrom(msg.sender, address(this), otherAmount);

        // create locker
        lockedToken[id].tokenAddress = othertoken;
        lockedToken[id].withdrawalAddress = msg.sender;
        lockedToken[id].unlockTime = lockTime;
        lockedToken[id].tokenAmount = otherAmount;
        lockedToken[id].withdrawn = false;
        userIds[msg.sender].push(id);
        id = id.add(1);

        emit LockCreated(id.sub(1), msg.sender, othertoken);
    ...
    function withdrawFunds(uint256 _id) public {
        ...
        IBEP20(lockedToken[_id].tokenAddress).safeTransfer(msg.sender, lockedToken[_id].tokenAmount);
        lockedToken[_id].withdrawn = true;

        emit LockWithdraw(_id, msg.sender, lockedToken[_id].tokenAddress);
