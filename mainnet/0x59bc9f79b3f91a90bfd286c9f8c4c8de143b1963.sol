...
interface INFTVault {
    ...
    function claimExpiredInsuranceNFT(uint256 _nftIndex, address _recipient) external;
...
contract Liquidator is Ownable {
    ...
    mapping(INFTVault => bool) public isTrusted;

    constructor(IERC20 _pusd) {
        pusd = _pusd;
    }
    ...
    function liquidate(uint256[] memory _toLiquidate, INFTVault _nftVault)
        external
    {
        require(isTrusted[_nftVault], "NOT_TRUSTED");
        ...
        for (uint256 i; i < length; ++i) {
            ...
            try _nftVault.liquidate(nftIndex, owner()) {} catch Error(
    ...
    function claimExpiredInsuranceNFT(
        ...
        for (uint256 i; i < length; ++i) {
            ...
            try _nftVault.claimExpiredInsuranceNFT(nftIndex, owner()) {
    ...
    function setTrustedNFTVault(INFTVault _nftVault, bool _trusted)
        external
        onlyOwner
    {
        isTrusted[_nftVault] = _trusted;
    }
