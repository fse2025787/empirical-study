...
abstract contract EIP712Upgradeable is Initializable {
    ...
    bytes32 private constant _TYPE_HASH = keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");
		address private _VERIFYING_CONTRACT;
     ...
     * contract upgrade].
     */
    function __EIP712_init(string memory name, string memory version, address distributionAddress) internal onlyInitializing {
        __EIP712_init_unchained(name, version, distributionAddress);
    }

    function __EIP712_init_unchained(string memory name, string memory version, address distributionAddress) internal onlyInitializing {
        bytes32 hashedName = keccak256(bytes(name));
        bytes32 hashedVersion = keccak256(bytes(version));
        _HASHED_NAME = hashedName;
        _HASHED_VERSION = hashedVersion;
				_VERIFYING_CONTRACT = distributionAddress;
    }
    ...
    function _buildDomainSeparator(
        ...
        return keccak256(abi.encode(typeHash, nameHash, versionHash, block.chainid, _VERIFYING_CONTRACT));
    }
