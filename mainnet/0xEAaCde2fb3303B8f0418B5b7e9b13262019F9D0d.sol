...
contract VestingFactory is Ownable {
    ...
    mapping(address=>address[]) public beneficiaryVsVestings;
    
    function getVestingsForBeneficiary(address beneficiary) external view returns(address[] memory) {
        return beneficiaryVsVestings[beneficiary];
    }

    function deployVestings(
        address token,
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            ...
            beneficiaryVsVestings[beneficiaries[i]].push(address(vesting));
            IERC20(token).transferFrom(msg.sender, address(vesting), amounts[i]);
            vesting.transferOwnership(vestingOwner);
        }
        
    }

    function pullRemaingingTokens(address token) external onlyOwner {
