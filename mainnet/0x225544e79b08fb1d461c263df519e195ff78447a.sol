...
contract LPStakingV4 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function withdraw(
        ...
        require(
            _verifyWithdraw(msg.sender, details.NFTId, _dId, _stake, _ethUSD, _mpwrUSD, _totalLpStaked, _signature),
            "invalidated"
        );
    ...
    function _verifyWithdraw(
        ...
        bytes32 signedHash = keccak256(
            abi.encodePacked(_staker, NAME_HASH, _tokenId, _dId, _amountUSD, _ETHUSD, _mpwrUSD, _totalLpStaked)
        );
        bytes32 messageHash = signedHash.toEthSignedMessageHash();
        address messageSender = messageHash.recover(_signature);
        return messageSender == validator;
