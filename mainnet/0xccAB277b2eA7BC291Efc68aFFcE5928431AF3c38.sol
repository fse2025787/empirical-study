...
contract FantasyWorldCup is IERC20 {
    ...
    string constant private _name = "Fantasy World Cup";
    string constant private _symbol = "$FWC";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xD1004317f95a75f0fF7ae1Cbede24992714f1569),
        team: payable(0x50502D0744009835B1684A7679a6ee395fB13C3D),
        prize: payable(0xF098aaEA400d04BaE0Ed0A31C3914024151AcD83)
    });
    ...
    CallGas public _callGas = CallGas({
        marketing: 30000,
        team: 30000,
        prize: 55000
        });
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 team, uint16 prize) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.team = team;
        _ratios.prize = prize;
        _ratios.totalSwap = liquidity + marketing + team + prize;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable team, address payable prize) external onlyOwner {
        ...
        _taxWallets.prize = payable(prize);
    }

    function setCallGas(uint32 marketing, uint32 team, uint32 prize) external onlyOwner {
        require(marketing >= 25_000
                && marketing <= 100_000
                && team >= 25_000
                && team <= 100_000
                && prize >= 25_000
                && prize <= 150_000,
                "Values cannot be out of bounding range.");
        _callGas.marketing = marketing;
        _callGas.team = team;
        _callGas.prize = prize;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        CallGas memory callGas = _callGas;
        ...
        uint256 prizeBalance = (amtBalance * ratios.prize) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - (teamBalance + prizeBalance);
        if (ratios.team > 0) {
            (success,) = _taxWallets.team.call{value: teamBalance, gas: callGas.team}("");
        }
        if (ratios.prize > 0) {
            (success,) = _taxWallets.prize.call{value: prizeBalance, gas: callGas.prize}("");
        }
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: callGas.marketing}("");
        }
    }
