...
contract Moon is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyAmount = _tTotal.mul(3).div(100);
    uint256 public maxSellAmount = _tTotal.mul(3).div(100);
    uint256 public swapTokensAtAmount = 50000000000 * 10**_decimals;
    ...
    //BNB
    address public marketingAddress = 0xC2cAc95654f701931FF9d59Ed39b82aa4AFc3838;


    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "MoonTube";
    string private constant _symbol = "MTE";
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burn, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burn,uint256 _liquidity, uint256 _buyback) external onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rmarketing ,to_return.rburn, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tburn = tAmount*sellFeeRates.burn/1000;
            s.tLiquidity = tAmount*sellFeeRates.liquidity/1000;
            s.tBuyback = tAmount*sellFeeRates.buyback/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tmarketing-s.tburn-s.tLiquidity-s.tBuyback;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rmarketing,uint256 rburn, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rTransferAmount =  rAmount-rRfi-rmarketing-rburn-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rmarketing,rburn, rLiquidity, rBuyback);
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.marketing ) * 2;
