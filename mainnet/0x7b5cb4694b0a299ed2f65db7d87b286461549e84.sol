...
contract Strategy is ConvexBTC {
    address public constant pnt = address(0x89Ab32156e46F46D02ade3FEcbe5Fc4243B9AAeD);

    constructor(address _vault) public ConvexBTC(_vault) {
        curve = address(0x11F419AdAbbFF8d595E7d5b223eee3863Bb3902C);
        id = 18;
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(pnt).approve(uniswap, 0);
        IERC20(pnt).approve(uniswap, type(uint256).max);
    }

    // >>> include other rewards
    function _migrateRewards(address _newStrategy) internal override {
        super._migrateRewards(_newStrategy);
        IERC20(pnt).safeTransfer(_newStrategy, IERC20(pnt).balanceOf(address(this)));
    }
    ...
    function _claimableInETH() internal override view returns (uint256 _claimable) {
        ...
        uint256 _pnt = IERC20(pnt).balanceOf(address(this));
        if (_pnt > 0) {
            address[] memory path = new address[](3);
            path[0] = pnt;
            path[1] = weth;
            path[2] = wbtc;
            uint256[] memory swap = Uni(uniswap).getAmountsOut(_pnt, path);
        ...
        )
    {
        ...
        if (_pnt > 0) {
            ...
            Uni(uniswap).swapExactTokensForTokens(_pnt, uint256(0), path, address(this), now);
        }
