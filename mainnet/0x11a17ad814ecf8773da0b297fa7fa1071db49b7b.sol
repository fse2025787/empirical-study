...
// File: contracts\interfaces\IQubeSwapRouter01.sol

pragma solidity >=0.6.2;

interface IQubeSwapRouter01 {
...
// File: contracts\interfaces\IQubeSwapRouter02.sol

pragma solidity >=0.6.2;

interface IQubeSwapRouter02 is IQubeSwapRouter01 {
...
// File: contracts\interfaces\IQubeSwapFactory.sol
...
// File: contracts\interfaces\IQubeSwapPair.sol
...
// File: contracts\libraries\QubeSwapLibrary.sol
...
library QubeSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'f7024d842b72553d27d6b10054bf99d531bd9c85a3c759be2a1c64244363c16a' // init code hash
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IQubeSwapPair(pairFor(factory, tokenA, tokenB)).getReserves();
...
// File: contracts\QubeSwapRouter.sol
...
contract QubeSwapRouter is IQubeSwapRouter02 {
    using SafeMath for uint;
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = QubeSwapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = QubeSwapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'QubeSwapRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = QubeSwapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquidityETH(
        ...
        address pair = QubeSwapLibrary.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IQubeSwapPair(pair).burn(to);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IQubeSwapPair(QubeSwapLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, QubeSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'QubeSwapRouter: INVALID_PATH');
        amounts = QubeSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'QubeSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(QubeSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'QubeSwapRouter: INVALID_PATH');
        amounts = QubeSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'QubeSwapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, QubeSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, QubeSwapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(QubeSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
