 ...
 * 
 * @dev Operation Rights for land single plot 
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function _beforeTokenTransfer(
        ...
        if( from != address(0x0) && tiles[tokenId].expire < block.timestamp )
        {  
            if( to != address(0x0) ){
...
contract MetaRights is ERC721Metadata {
   
    using SafeMath for uint256;
    using Strings for uint256;

    address public landNft =  address(0x0);
    uint256 public LeaseMax = 31536000;   // Maximum 1 years
    ...
    event SetExpire(uint256 tokenId, uint256 exptimestamp);
    event CloseRights(uint256 tokenId, address target);

    constructor( address _address, string memory name, string memory symb, address landnft) ERC721(name, symb) 
    {
        _setLandContract(landnft);
        _setGovernance(_address);
        _setProjectname(name, symb);
        _setBaseURI(string(abi.encodePacked("https://pfp.allimeta.world/lands/rights/", symb, "/")));
    }

    function _setLandContract(address _landnft) internal {
        landNft = _landnft;
    }
   ...
   function onERC721Received(address operator, address from, uint256 tokenId, bytes memory data) public returns (bytes4) 
    ...
    function lease_mint(address to, uint256 tokenId ) internal returns (bool) 
    {
        ...
        (int128 t,,bool exists) =  r.tiles(tokenId);
        tiles[tokenId] = Tile(t,0,exists);
    ...
    function lease_burn(uint256 tokenId) internal {
        ...
        if(tiles[tokenId].exists){
             emit LeasePlotBurn(tokenId);
        }
        delete tiles[tokenId];
    }

    function setExpiration(uint256 tokenId, uint256 tm_seconds, uint256 amount) external 
    {
        require( amount >= currentTokenCost, "token amount not enough!");
        if( tiles[tokenId].expire == 0 )
        {
            require(
        ...
        else if( tiles[tokenId].expire < block.timestamp )
        {
            require( msg.sender == land_owner[tokenId] , "Invalid Land owner");
            close_lease( tokenId );
            land_owner[tokenId] = msg.sender;
            emit LeasePlotReNew(tokenId, msg.sender);
        }
        else 
            return;

        require( tm_seconds > block.timestamp, "timestamp invalid" );
        require( tm_seconds < block.timestamp+LeaseMax, "timestamp over max. LeaseLimition" );

        tiles[tokenId].expire = tm_seconds;
        ...
        emit SetExpire(tokenId, tm_seconds);
    }
    ...
    function setLeaseMaxTime(uint256 max_timestamp) public onlyGovernance {
        LeaseMax = max_timestamp;
    }
    ...
    function close_lease(uint256 tokenId) internal 
    {
        for(uint256 i=0;i<totalLandServices();i++)
        {
            address transMsg = LandServiceByIndex(i);
            (bool flag, ) = transMsg.call(abi.encodeWithSelector(bytes4(keccak256("CloseRights(uint256)")),tokenId));
            if( flag )
                emit CloseRights(tokenId, transMsg);
        }
    }
...
contract RightsFactory is Governance {
    event deployAddress(string _name, string _symbol, address _address);
   ...
   function deployCollection(
    ...
    public payable
    {
        ...
        MetaRights childContract = new MetaRights(address(msg.sender),_projectname,_projectsymbol,_meta_address);
        newnftaddr[hash] = address(childContract);

        if(ErcToken != address(0x0) && currentTokenCost > 0)
        {
            LERC20Token _ercx =  LERC20Token(ErcToken);
           _ercx.transferFrom(msg.sender, address(this), amount);
        }

        allprj.push( RightsInfo(msg.sender,_projectname,block.number) );
        emit deployAddress(_projectname, _projectsymbol, address(childContract));
    }
