...
abstract contract ERC20Lockable is ERC20, Ownable {
    ...
    function unlockAll(address from) external returns (bool success) {
        for(uint256 i = 1; i <= _locks[from].length;i++){
            //i++;
...
contract SLD is
    ...
    Freezable
{
    string constant private _name = "Sector of Land";
    string constant private _symbol = "SLD";
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "SLD/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "SLD/transferFrom : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "SLD/approve : Should not approve zero address"
        );
