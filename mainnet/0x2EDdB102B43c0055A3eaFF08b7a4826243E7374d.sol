...
// File: contracts/update/NFTBase/NFTBaseUpgradeable.sol
...
contract NFTBaseUpgradeable is ERC721URIStorageUpgradeable,MarketConfigUpgradeable{    
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter internal _tokenIds;
    using AddressUpgradeable for address;
    address internal _marketAddress;
    mapping(uint256 => address) internal _createTokenByOwers;
    /** 
       注意后面状态变量从下面开始 要按照顺序
     */
    
    // constructor(address marketAddress) ERC721Upgradeable("melon", "melon"){
    //      setMarketAccess(marketAddress, true);
    // }
    function initialize(address marketAddress) public initializer {
         __ERC721_init("melon", "melon");
         __Ownable_init();
         setMarketAccess(marketAddress, true);
        
    }
    
    function createNFT(address author,string memory tokenURI)  public returns (uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(author, newItemId);
        _setTokenURI(newItemId, tokenURI);
        _createTokenByOwers[newItemId]=author;
        return newItemId;
    }
    ...
    /**
     owner token 所有者 这个方法必须是已经授权的市场合约调用
     */
    function setApprovalAllToMarket (
        address owner,
        bool approved
    ) public marketAccessRole {
       _setApprovalForAll(owner,_msgSender(),approved);
    }
    function testUpdate() pure public returns (uint256){
        return 2;
    }
...
// File: contracts/update/LocalTransparentUpgradeableProxy.sol


pragma solidity ^0.8.0;


abstract contract LocalTransparentUpgradeableProxy is TransparentUpgradeableProxy {
    constructor(
        address _logic,
        address admin_,
        bytes memory _data
...
// File: contracts/update/NFTBase/NFTBaseProxy.sol


pragma solidity ^0.8.0;


contract NFTBaseProxy is LocalTransparentUpgradeableProxy{

  constructor( address _logic,
        address admin_,address _market
        )LocalTransparentUpgradeableProxy(_logic,admin_,abi.encodeWithSelector(NFTBaseUpgradeable.initialize.selector,_market)){
  } 
