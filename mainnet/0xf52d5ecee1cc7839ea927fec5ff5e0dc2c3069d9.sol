...
contract nft is ERC721, Ownable {
    ...
    uint private mintCost_ = 0.2605 ether;
    ...
    //addresses to where the mint funds will be sent
    address payable private communityWallet = payable(0x5E525DA32Ae4af742BEFe6bA098eD9832D8cea67);
    address payable private marketingWallet = payable(0x2d764Cc0541056f3B2cc3fA666E465fAA71Ccd71);
    ...
    uint256 internal saleLaunchTime;
    ...
    //to see if NFTs have been revealed or not
    bool public reveal;
    
    //declaring initial values for variables
    constructor() ERC721('Wolf of Crypto', 'WOC') {
        numberOfTotalTokens = 0;
        maxTotalTokens = 5000;

        unrevealedURI = "ipfs://QmeswHUsUskjA2zJNsfounZJdxumziv8ErHCRHi7uKkZQ1/";
        reveal = false;
    }
    ...
    //only three addresses allowed to access these functions
    modifier onlyApproved() {
        require(msg.sender == owner() || msg.sender == communityWallet || msg.sender == marketingWallet);
        _;
    }
    ...
    */
    ...
    function mint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(mintsPerAddress[msg.sender] + number <= maxMint, "That is more than you are allowed to mint!");
        require(msg.value >= mintCost() * number, "Your wallet doesn't have enough ETH!");
    ...
    function reservedMintGeneral(uint256 number) public onlyApproved {
        require(_reservedMints + number <= maxReservedMints, "Not enough NFTs left!");
        for (uint256 i = 0; i < number; i++) {
            uint256 tid = tokenId();
            _safeMint(msg.sender, tid);
            mintsPerAddress[msg.sender] += 1;
    ...
    function withdraw() public onlyApproved {
        uint256 balance = accountBalance();
        require(balance > 0, 'No Funds to withdraw, Balance is 0');
        
        _withdraw(communityWallet, (balance * 75) / 100); 
        _withdraw(marketingWallet, accountBalance());  //to avoid dust eth
    }
    ...
    function toggleReveal() public onlyOwner {
        require(!reveal, 'Can only reveal when current state is unrevealed!');
        reveal = !reveal;
    ...
    //change the limit of mints allowed per address
    function changeMaxMint(uint maxMint_) public onlyOwner {
        maxMint = maxMint_;
    }
