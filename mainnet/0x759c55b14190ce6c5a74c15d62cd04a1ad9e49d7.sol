...
contract SHARKBOYFIGHTCLUB is ERC721Enumerable, Ownable
{
    using Strings for string;

    uint public constant MAX_TOKENS = 8888;
    uint public constant NUMBER_RESERVED_TOKENS = 188;
    uint256 public constant PRICE = 5*10**16; //0.05
    uint256 public constant PRE_SALE_PRICE = 3*10**16; //0.03
    ...
    constructor() ERC721("Shark Boy Fight Club", "SBFC") {}

    function mintToken(uint256 amount) external payable
    {
        require(msg.sender == tx.origin, "No transaction from smart contracts!");
        require(saleIsActive, "Sale must be active to mint");
        require(amount > 0 && amount <= 20, "Max 20 Sharks per transaction");
