...
contract Lockdown is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    PaperToken public paper;
    address internal paperLP;
    ...
    constructor(
        ...
        IERC20(paper).approve(router, approveAmount);
    }

    function getPaperAmount(uint256 balance) public view returns(uint256 amount) {
        amount = IERC20(paper).balanceOf(paperLP)
        .div(IERC20(WETH).balanceOf(paperLP))
        .mul(balance);
    }

    receive() external payable {}

    function addLiquidity(uint256 s) external onlyOwner payable returns(uint256 paperAmount){
        paperAmount = getPaperAmount(msg.value);
        require(
            paperAmount > 0,
            "Address: insufficient balance"
        );
        ...
        IUniswapV2Router02(router).addLiquidityETH{ value: msg.value }(
            address(paper),
            paperAmount,
            paperAmount.mul(100 - s).div(100),
            msg.value.mul(100 - s).div(100),
            developers,
            now + 1200
        );
        emit NewLock(msg.value, paperAmount);
    }

    function releaseBalances() external onlyOwner {
        payable(developers).transfer(address(this).balance);
        IERC20(paper).transfer(developers, IERC20(paper).balanceOf(address(this)));
    }
}