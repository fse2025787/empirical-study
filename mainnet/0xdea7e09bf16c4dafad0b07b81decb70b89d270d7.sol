...
// File contracts/B.Protocol/Oracle.sol
...
contract BAMM is CropJoinAdapter, PriceFormula, Ownable {
    ...
    ARTHOracle public immutable priceAggregator;
    OneOracle public immutable lusd2UsdPriceAggregator;    
    ...
    {
        priceAggregator = ARTHOracle(_priceAggregator);
        lusd2UsdPriceAggregator = OneOracle(_lusd2UsdPriceAggregator);
        LUSD = IERC20(_LUSD);
    ...
    function compensateForLusdDeviation(uint ethAmount) public returns(uint newEthAmount) {
        uint chainlinkDecimals;
        uint chainlinkLatestAnswer;
    ...
    function getSwapEthAmount(uint lusdQty) public returns(uint ethAmount, uint feeLusdAmount) {
