...
// telegram : t.me/zombiekillerinu
// website : https://zombiekillerinu.com
...
contract ZombieKillerInu is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWallet = payable(0x69c6Af9466eEF9e58B811B3e04b9454684E60064); // Marketing Wallet
    address payable public prizeRewardWallet = payable (0x69c6Af9466eEF9e58B811B3e04b9454684E60064); // prizeReward Wallet
    ...
    string private _name = "ZombieKillerInu";
    string private _symbol = "ZKILL";
    ...
    uint256 public prizeRewardFee = 2;
    uint256 public totalFee =
        liquidityFee.add(marketingFee).add(prizeRewardFee);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountETHprizeReward = amountETH.mul(prizeRewardFee).div(totalETHFee);
        uint256 amountETHMarketing = amountETH.mul(marketingFee).div(
            totalETHFee
        );
        //Send to marketing wallet and prizeReward wallet
        uint256 contractETHBalance = address(this).balance;
        if(contractETHBalance > 0) {
            sendETHToFee(amountETHMarketing,marketingWallet);
            sendETHToFee(amountETHprizeReward,prizeRewardWallet);
        }
    ...
    function setWallets(address _marketingWallet,address _prizeRewardWallet) external onlyOwner() {
        marketingWallet = payable(_marketingWallet);
        prizeRewardWallet = payable(_prizeRewardWallet);
    }

    function setFees(
        uint256 _reflectionFee,
        uint256 _liquidityFee,
        uint256 _prizeRewardFee,
        uint256 _marketingFee
    ) external onlyOwner {
        reflectionFee=_reflectionFee;
        liquidityFee = _liquidityFee;
        prizeRewardFee = _prizeRewardFee;
        marketingFee = _marketingFee;
        totalFee = liquidityFee.add(marketingFee).add(prizeRewardFee);
    }
