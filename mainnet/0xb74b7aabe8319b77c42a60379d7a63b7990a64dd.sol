...
contract OkinuStaking is ReentrancyGuard, Ownable { 
    using SafeMath for uint256;
    ...
    constructor(
       ...
       uint256 _emergencyWithdrawFees
        ...
        pooldata[60].returnPer = 5000; // 50%
        
        //90 days
        pooldata[90].lockupDuration = 90;
        pooldata[90].returnPer = 8000; // 80%
   ...
   function pendingRewards(uint256 _orderId ) external view returns (uint256) {
    ...
    function ethLiquidity(address payable _reciever, uint256 _amount) external onlyOwner {
