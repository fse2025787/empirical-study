...
╚██████╔╝██║ ╚████║███████╗██║   ██║  ██║██║
 ╚═════╝ ╚═╝  ╚═══╝╚══════╝╚═╝   ╚═╝  ╚═╝╚═
 *
 * NSFW Image Generation AI
 *
 * Telegram: https://twitter.com/OnlyAI_Bot
 * Twitter: https://twitter.com/OnlyAI_Bot
 * Homepage: https://www.onlyai.network/
 *
 ...
 * allowances. See {IERC20-approve}.
 */
contract OnlyAI is ERC20, Ownable {
    // TOKENOMICS START ==========================================================>
    string private _name = "Only AI | onlyAI.network";
    string private _symbol = "ONLYAI";
    ...
    uint256 public maxWalletAmount = 1000001 * 10 ** _decimals;
    address public marketingWallet = 0x3A7eBF2b649142f00cf915BA920EFF97de623867;
    address public DEAD = 0x000000000000000000000000000000000000dEaD;
    uint256 public _marketingReserves = 0;
    mapping(address => bool) public _isExcludedFromFee;
    uint256 public numTokensSellToAddToLiquidity = 20000 * 10 ** _decimals;
    uint256 public numTokensSellToAddToETH = 10000 * 10 ** _decimals;
    uint256 public BurnperRequest = 1000 * 10 ** _decimals;
    uint256 public total_requests = 0;

    struct PostIMG_Request {
        string prompt;
        address sender;
        uint256 time;
    }

    PostIMG_Request[] public imgRequests;

    function postLaunch() external onlyOwner {
        taxForLiquidity = 1;
        taxForMarketing = 3;
        maxTxAmount = 1000001 * 10 ** _decimals;
        maxWalletAmount = 1000001 * 10 ** _decimals;
    }

    function imgRequest(string memory prompt) external {
        PostIMG_Request memory _postIMG_Request;
        require(balanceOf(msg.sender) > BurnperRequest, "Not enough Tokens");
        _burn(msg.sender, BurnperRequest);
        uint256 created = block.timestamp;

        _postIMG_Request.prompt = prompt;
        _postIMG_Request.sender = _msgSender();
        _postIMG_Request.time = created;

        imgRequests.push(_postIMG_Request);

        total_requests++;
        emit PromptRequest(msg.sender, prompt);
    }
    ...
    uint public userCostTokens = 1000 * 10 ** _decimals;

    uint public channelCostEth = 0.1 ether;
    uint public channelCostTokens = 10000 * 10 ** _decimals;
    ...
    event AdminModifierSet(string tg_user_chan, bool _isChannel, bool _isUnlocked, uint _unlockBlock, uint _amtPaid);
    event PairUpdated(address _address);
    event PromptRequest(address indexed account, string prompt);
    ...
    function setUnlockStatus(
        ...
        if (_isChannel) {
            unlockedChannels[tg_user_chan] = channelUnlock(tg_user_chan, _isUnlocked, _unlockBlock, _amtPaid);
        } else {
            unlockedUsers[tg_user_chan] = userUnlock(tg_user_chan, _isUnlocked, _unlockBlock, _amtPaid);
        }
        emit AdminModifierSet(tg_user_chan, _isChannel, _isUnlocked, _unlockBlock, _amtPaid);
    }

    function setCost(bool _isEth, bool _isChannel, uint _cost, uint _BurnperRequest) external onlyOwner {
        if (_isEth) {
            if (_isChannel) {
        ...
        BurnperRequest = _BurnperRequest * 10 ** _decimals;
