...
contract Strategy is CurveVoterProxy {
    ...
    constructor(address _vault) public CurveVoterProxy(_vault) {
        dex = sushiswap;
        curve = address(0x61E10659fe3aa93d036d099405224E4Ac24996d0);
        gauge = address(0xAEA6c312f4b3E04D752946d329693F7293bC2e6D);
        keepCRV = 1000;
    ...
    function _setPath(uint _id, bool _isReward) internal {
        if (_id == 0) {
    ...
    function setPath(uint _id, bool _isReward) external onlyAuthorized {
        ...
        )
    {
        ...
        IVoterProxy(proxy).claimRewards(gauge, dfd);
        uint256 _dfd = IERC20(dfd).balanceOf(address(this));
        if (_dfd > 0) {
            IERC20(dfd).safeApprove(uniswap, 0);
            IERC20(dfd).safeApprove(uniswap, _dfd);

            Uni(uniswap).swapExactTokensForTokens(_dfd, uint256(0), pathReward, address(this), now);
        }
