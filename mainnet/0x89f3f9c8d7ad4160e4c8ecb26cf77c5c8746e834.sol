...
// File: contracts/files/ERC721Base.sol
...
/**
 * @title ERC721Base
 * ERC721Base - ERC721 contract that whitelists a trading address
 */
abstract contract ERC721Base is ERC721, ContextMixin, NativeMetaTransaction, Ownable {

    /**
     * Proxy registry address (OpenSea)
     */
    ...
    function removeProxyRegistryAddress() public onlyOwner {
    ...
    /**
     * Override isApprovedForAll to whitelist proxy (registry) for gas-less listings.
     */
...
// File: contracts/files/MintPass.sol
 ...
 *  Genesis Pass
 */
contract GenesisPass is ERC721Base, ReentrancyGuard {
    ...
    {
        PUBLIC_PRICE = params[0];
        WHITELIST_PRICE = params[1];
        MAX_SUPPLY = params[2];
        MAX_PER_ADDRESS = params[3];
        MAX_MULTIMINT = params[4];
    ...
    function mint(uint256 count) public payable nonReentrant {
        ...
        require(count - 1 < allowedMintCount(msg.sender), "Minting limit exceeded");
        require(msg.value >= PUBLIC_PRICE * count, "Insufficient payment");

        updateMintCount(msg.sender, count);

        _mint(count);

        payable(_paymentSplitter).transfer(msg.value);
    }
    ...
    function _mint(uint256 count) internal {
        for (uint256 i = 0; i < count; i++) {
            uint256 currentTokenId = _supplyCounter.current();
            _supplyCounter.increment();
    ...
    function allowedMintCount(address minter) public view returns (uint256) {
        return MAX_PER_ADDRESS - mintCountMap[minter];
    }
    ...
    bytes32 private _whitelistMerkleRoot = 0xae16b27f1fd4a81a49ebad089e57fce5f821a6714556c8312e66f308cc0a9b69;

    function setWhitelistMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
        _whitelistMerkleRoot = _merkleRoot;
    }

    function whitelistMint(uint256 count, bytes32[] calldata merkleProof) public payable nonReentrant {
        require(whitelistSaleIsActive, "Whitelist sale is not active");
        require(totalSupply() + count - 1 < MAX_SUPPLY, "Exceeds max supply");
        require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
        require(count - 1 < allowedMintCount(msg.sender), "Minting limit exceeded");
        require(msg.value >= WHITELIST_PRICE * count, "Insufficient payment");
        require(
            MerkleProof.verify(merkleProof, _whitelistMerkleRoot, keccak256(abi.encodePacked(msg.sender))), 
    ...
    function release(address payable account) public onlyOwner {
        _paymentSplitter.release(account);
    }
}