...
contract MonstrStake is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public rewardPerBlock = 8101851*10**9;
    uint256 public minDepositAmount = 2000000000000000000;
    uint256 public maxDepositAmount = uint256(-1);


    PoolInfo[] public poolInfo;
    mapping(uint256 => mapping(address => UserInfo)) public userInfo;
    uint256 public totalAllocPoint = 1000000000;
    ...
    function setAllocation(uint pid, uint _allocation) external onlyOwner {
        require(_allocation <= totalAllocPoint, "invalid value");
        PoolInfo storage pool = poolInfo[pid];
        pool.allocPoint = _allocation;
    }
    function setTotalAllocation( uint _totalAllocation) external onlyOwner {
        totalAllocPoint=_totalAllocation;
    }
    ...
    function remaining(uint256 pid, address _user) external view returns (uint256){
         PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][_user];
        return pool.lockupDuration-(block.timestamp-user.depositedAt);
    }
    function canWithdraw(uint256 pid, address _user) external view returns (bool){
         PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][_user];
            return block.timestamp >= user.depositedAt + pool.lockupDuration;
    }
