 ...
 * {ERC721Enumerable}.
 */
contract GERC721Upgradeable is
    Initializable,
    ContextUpgradeable,
    ERC165Upgradeable,
    IERC721Upgradeable,
    IERC721MetadataUpgradeable
{
    ...
    function __ERC721_init(
        ...
        __ERC721_init_unchained(name_, symbol_, maxSupply_);
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        require(_mintedSupply < _maxSupply);
    ...
    function _preGhostMintCollection(
        address to,
        uint256 start,
        uint256 end
    ) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
        require(_mintedSupply + (end - start + 1) <= _maxSupply, "MS");
        require(end >= start, "EST");
        uint256 i = 0;
        _mintedSupply += end - start + 1;
        for (i = start; i <= end; ) {
            _owners[i] = to;
            //emit Transfer(address(0), to, i); nope, premint is so we can keep gas prices stable for the next year
            unchecked {
                i++;
            }
        }
    ...
    function _transfer(
        ...
        require(GERC721Upgradeable.ownerOf(tokenId) == from, "ERC721: transfer from incorrect owner");
        require(to != address(0), "ERC721: transfer to the zero address");

        _beforeTokenTransfer(from, to, tokenId);

        // Clear approvals from the previouss owner
        _approve(address(0), tokenId);
    ...
    function _altarGhostMint(address to, uint256 tokenId) internal virtual {
        require(_owners[tokenId] == msg.sender, "ERC721: transfer from incorrect owner");
        require(to != address(0), "ERC721: transfer to the zero address");
    ...
    function _addTotalSupplyAndBalance(address balanceReceiver, uint256 supply) internal virtual {
        _totalSupply += supply;
        _balances[balanceReceiver] += supply;
    }
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(GERC721Upgradeable.ownerOf(tokenId), to, tokenId);
    }
 ...
 */
interface INSGERC721Upgradeable {
    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    function altarEmitMint(address to_, uint256[] calldata ids) external {
        ...
        uint256 i = 0;
        _addTotalSupplyAndBalance(to_, idsLength);
        for (; i < idsLength; ) {
    ...
    function adminAltarEmitMint(address to_, uint256[] calldata ids) external onlyRole(NIGHTSHADE_ADMIN) {
        uint256 idsLength = ids.length;
        uint256 i = 0;
        _addTotalSupplyAndBalance(to_, idsLength);
        for (; i < idsLength; ) {
    ...
    function adjustTotalSupply(uint256 supply) external onlyRole(NIGHTSHADE_ADMIN) {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://meta.etch.sh/api/meta/ns/";
    }
