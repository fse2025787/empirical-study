...
contract MasterChefAPE is Ownable {
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(totalAPEStaked.sub(_amount) >= totalAPEUsedForPurchase, "Insufficient fund");
        updatePool();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accVEMPPerShare).div(1e12).sub(user.rewardDebt);
            safeVEMPTransfer(msg.sender, pending);
    ...
    function emergencyWithdraw() public {
        ...
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
        require(totalAPEStaked.sub(user.amount) >= totalAPEUsedForPurchase, "Insufficient fund");
