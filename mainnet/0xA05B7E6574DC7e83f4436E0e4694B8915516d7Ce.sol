...
contract MonasteryTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, ZEN }
    ...
    address public ZEN;
    uint public ZENQueue; // Delays change to ZEN address
    ...
    constructor (
        ...
        isLiquidityToken[ _MONKDAI ] = true;
        liquidityTokens.push( _MONKDAI );
        bondCalculator[_MONKDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for MONK
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint MONK needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( MONK ).mint( msg.sender, send_ );
    ...
    function depositICO( uint _amount, address _token) external {
    ...
    /**
        @notice allow approved address to burn MONK for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IMONKERC20( MONK ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( ZEN ).balanceOf( msg.sender ); // Can only borrow against ZEN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with MONK
        @param _amount uint
     */
    function repayDebtWithMONK( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IMONKERC20( MONK ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, MONK, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( MONK ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( MONK ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns MONK valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match MONK decimals
            value_ = _amount.mul( 10 ** IERC20( MONK ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.ZEN ) { // 9
            ZENQueue = block.number.add( blocksNeededForQueue );
        } else return false;
