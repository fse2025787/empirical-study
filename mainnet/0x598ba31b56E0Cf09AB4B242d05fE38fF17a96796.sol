...
contract Orderer is IOrderer, UUPSUpgradeable, ERC165Upgradeable {
    ...
    function externalSwap(ExternalSwap calldata _info) external override onlyRole(KEEPER_JOB_ROLE) {
        ...
        uint _shares = Math.min(_info.maxSellShares, orderSellAsset.shares);
        uint amountInBase = _details.sellVToken.assetBalanceForShares(_shares).mulDiv(
            FixedPoint112.Q112,
            _details.priceOracle.refreshedAssetPerBaseInUQ(_details.sellAsset)
        );

        if (orderSellAsset.shares == _details.sellVToken.balanceOf(_info.account) && amountInBase < MIN_SWAP_AMOUNT) {
            _details.sellVToken.transferFrom(_info.account, address(_details.sellVToken), _shares);
            _details.sellVToken.burnFor(address(_details.sellVToken));

            emit CompleteOrder(lastOrderId, _details.sellAsset, _shares, _details.buyAsset, 0);
        } else {
            ...
            require(_sellShares != 0, "Orderer: ZERO_SHARES");
    ...
    function _calculateExternalSwapShares(
        ...
        sellShares = uint248(_sellOrderShares);
    }
