...
contract Treasury is Ownable{
    ...
    function end() public view returns (uint256){
        IStaking stakingContract = IStaking(_staking);
        return stakingContract.start() + stakingContract.period();
    }

    function withdraw(uint256 amount, address to) public onlyOwner{
        require(block.timestamp>end(),"Campaign still active");
        require(balance()>=amount,"Insufficient balance for transaction");
