...
contract Hunter is ERC20, Ownable {
    ...
    string constant _name = "Hunter X Hunter";
    string constant _symbol = "HUNTER";
    ...
    TaxWallets public _taxWallet = TaxWallets ({
        marketing: 0xE535b7f5c3EB5f193c6eE699327286c7348324C7,
        addLp: 0xE535b7f5c3EB5f193c6eE699327286c7348324C7,
        burn :0xE535b7f5c3EB5f193c6eE699327286c7348324C7
    });
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = 0;
        if (sender == pair && _feeBuy.totalFee != 0) {
        ...
        } else if (recipient == pair && _feeSell.totalFee != 0) {
            ...
            _balances[_taxWallet.burn] = _balances[_taxWallet.addLp].add(feeAmount.mul(_feeSell.burn).div(_feeSell.totalFee));
        }
        return amount.sub(feeAmount);
    }

    function setFeeSell(uint256 _marketing, uint256 _burn, uint256 _addLp) external onlyOwner{
        _feeSell.marketing = _marketing; 
        _feeSell.addLp = _addLp;
        _feeSell.burn = _burn;
        _feeSell.totalFee = _marketing.add(_burn).add(_addLp);
    }

    function setFeeBuy(uint256 _marketing, uint256 _burn, uint256 _addLp) external onlyOwner{
        _feeBuy.marketing = _marketing; 
        _feeBuy.addLp = _addLp;
        _feeBuy.burn = _burn;
        _feeBuy.totalFee = _marketing.add(_burn).add(_addLp);
    }       

    function updateTaxWallets(address _marketing, address _burn, address _addLp) external onlyOwner{
