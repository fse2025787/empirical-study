...
contract PayPhone is ERC721, Ownable {
    ...
    struct tokenStruct {
        uint256 dialId; //indentifies which dial, index into dials[] id;
        string m; //optionally store the message in contract storage
        string ipfs; //optional IPFS hash for backup of metadata
    }

    mapping (uint256 => dialStruct) public dials;
    mapping (uint256 => tokenStruct) public tokens;

    constructor() ERC721("PayPhone", "PAYPHONE") {
        inTheOffice = false;
        callsRemaining = 0;
        callRateLimit = 60;
        setBaseURI("https://payphone.deafbeef.com/");
        dialCost = 25000000000; //calls cost 25 Gwei
        maxMsgLength=10000;
    }
    ...
    function dial(string calldata m) payable public {
        ...
        dials[dial_index].from = msg.sender;
        dials[dial_index].ts = last_dial_ts;
    ...
    function setTokenIPFS(uint256 id, string memory h) public onlyOwner {
        require(id < _tokenIdCounter.current(), "TokenID out of range.");
        
        tokens[id].ipfs = h;
    }

    function setTokenDetails(uint256 id, uint256 dialId, string calldata m) public onlyOwner {
        require(id < _tokenIdCounter.current(), "TokenID out of range.");
        
        tokens[id].dialId = dialId;
        tokens[id].m = m;
    }
    
    function storeDialMessage(uint256 dialId, string calldata m) public onlyOwner {
        require(dialId < dial_index, "Dial Index out of range.");
        dials[dialId].m = m;
    }

    function setBaseURI(string memory b) public onlyOwner {
        _baseURI = b;
    }
    function setMaxMsgLength(uint256 a) public onlyOwner {
        maxMsgLength=a;
    }
    function setDialCost(uint256 a) public onlyOwner {
        dialCost = a;
    }
    ...
    function safeMint(address to) public onlyOwner {
        require(!isSealed, "contract sealed");
