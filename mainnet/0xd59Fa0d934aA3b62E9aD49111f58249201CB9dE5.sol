...
contract FlashLoanSDCRVCRV is IFlashLoanRecipient, Ownable {
    ...
    address private constant YCRV = address(0xFCc5c47bE19d06BF83eB04298b026F81069ff65b);
    ...
    address private constant YCRV_VAULT = address(0x453D92C7d4263201C69aACfaf589Ed14202d83a4);

    mapping(uint256 => Node) public mapNodes;
    uint256 private nodesLength;
    ...
    constructor() {
        ...
        IERC20(CRV).approve(YCRV_VAULT, type(uint256).max);
        ...
        IERC20(YCRV).approve(SDCRV_VAULT_BOOSTER_ADDRESS, type(uint256).max);
        IERC20(YCRV).approve(YCRV_VAULT, type(uint256).max);
    }
    ...
    function makeFlashLoan(
        ...
        vault.flashLoan(this, tokens, amounts, "0");
    }
    ...
    function approve(address _token, address _vault) external onlyOwner {
    ...
    function receiveFlashLoan(
        ...
        for(; i < nodesLength;) {
            ...
            _swap(_amount, mapNodes[i]);

            if(i < nodesLength - 1) {
                _amount = IERC20(mapNodes[i].receiveTokenAddress).balanceOf(address(this));
            }
        ...
        tokens[0].transfer(BALANCER_VAULT_ADDRESS, amounts[0] + feeAmounts[0]);

        uint256 crvBalance = IERC20(CRV).balanceOf(address(this));
        IERC20(CRV).transfer(owner(), crvBalance);
    }
