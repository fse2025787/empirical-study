...
contract Underland is ERC721URIStorage, Ownable {
    ...
    string internal s_themeTokenUris;
    uint256 internal i_mintFee;
    uint8[] internal tokenArray = [
        0,
        1,
        2,
        3,
    ...
    constructor(string memory themeTokenUris, uint256 mintFee) ERC721("Underland", "ULND") {
    ...
    function fulfillRandomness() internal {
        //sets tokenId, increments counter
        uint256 newTokenId = s_tokenCounter;
        s_tokenCounter++;

        //determines the tokenURI by combining the ipfs directory provided on deployment, the randomly chosen index/file name, and the type
        if (tokenArray.length > 0) {
            uint256 tokenIndex = uint256(keccak256(abi.encodePacked(block.timestamp))) %
                tokenArray.length;
            string memory tokenIndexString = Strings.toString(tokenArray[tokenIndex]);
            string memory currentTokenUri = string(
                abi.encodePacked(s_themeTokenUris, tokenIndexString, ".json")
            );
            ...
            //removes tokenURI from possible indexes
            delete tokenArray[tokenIndex];
            tokenArray[tokenIndex] = tokenArray[tokenArray.length - 1];
            tokenArray.pop();
