...
contract HuslAppNow is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "Husl App Now Collection";

    // Token symbol
    string private constant _symbol = "HSLAPPNOW";

    // Founder Card Token
    address private constant FCNFT = 0xBbeb904272FA9888C50DA37D16b9099FcaE78244;
    ...
    uint256 public MAX_MINT = 10**18; // default is set to no limit, more or less

    // cost for minting NFT
    uint256 public cost = 10**18;
    uint256 public discountedCost = 10**17;
    ...
    function toggleWhitelist(bool whitelistEnabled_) external onlyOwner {
        whitelistEnabled = whitelistEnabled_;
    }

    function setWhitelist(address[] calldata users, bool isWhitelisted_) external onlyOwner {
        uint len = users.length;
        for (uint i = 0; i < len;) {
            mintInfo[users[i]].isWhitelisted = isWhitelisted_;
            unchecked { ++i; }
        }
    }
    ...
    function mint(uint256 numberOfMints) external payable {
        ...
        if (whitelistEnabled) {
            require(
                mintInfo[msg.sender].isWhitelisted,
        ...
        // get cost for mint
        (uint256 mintCost, uint nDiscounts) = estimateCost(msg.sender, numberOfMints);

        // if discounts were used
        if (nDiscounts > 0) {
            
            // increment number of discounts used
            unchecked {
                mintInfo[msg.sender].discountsUsed += nDiscounts;
            }
        }
        ...
        // update number of NFTs minted for user to enforce maximum
        unchecked {
            mintInfo[msg.sender].nMinted += numberOfMints;
        }

        // mint `numberOfMints` NFTs to sender
        for (uint i = 0; i < numberOfMints;) {
            _safeMint(msg.sender, _totalSupply);
            unchecked { ++i; }
        }

        // enforce maximum mint per user
        require(
            mintInfo[msg.sender].nMinted <= MAX_MINT,
    ...
    */
    ...
    function estimateCost(address user, uint256 numberOfMints) public view returns (uint256, uint256) {
        uint256 balance = IERC721(FCNFT).balanceOf(user);
        uint256 nUsed = mintInfo[user].discountsUsed;
        uint256 nDiscounts = balance > nUsed ? balance - nUsed : 0;
        if (nDiscounts > numberOfMints) {
            nDiscounts = numberOfMints;
        }
        uint discount = discountedCost * nDiscounts;
        return (( numberOfMints * cost ) - discount, nDiscounts);
    }
