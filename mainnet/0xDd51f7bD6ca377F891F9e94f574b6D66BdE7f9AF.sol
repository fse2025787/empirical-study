...
contract UltraWomen is Ownable, ERC721A {
    ...
    function _beforeTokenTransfers(address from, address to, uint256 startTokenId, uint256 quantity) internal virtual override (ERC721A) {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            hodlStartTime[startTokenId + i] = block.timestamp;
        }
    }

    function getHodlTime(uint256 tokenId) external view returns (uint256) {
        return block.timestamp - hodlStartTime[tokenId];
    }
    ...
    function mintReserved(uint256 amount) external onlyOwner {
        require(amount + teamMintedTokensCount <= teamReservedTokens, "Cannot mint more than the max tokens per mint");
    ...
    function giveawayToAddresses(uint256 amountPerAddress, address[] calldata addresses) external onlyOwner {
        ...
        require(totalAmount + teamMintedTokensCount <= teamReservedTokens, "Cannot mint more than the max tokens per mint");
    ...
    function publicMint(uint256 amount) external {
        ...
        require(mintedTokensCount + amount <= tokenSupplyForMinting, "No tokens left for minting");
  ...
  uint _balance = address(this).balance;
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721A) returns (string memory) {
        return bytes(tokenBaseURI).length > 0 ? string(abi.encodePacked(super.tokenURI(tokenId), tokenURIExtension)) : '';
    }
}