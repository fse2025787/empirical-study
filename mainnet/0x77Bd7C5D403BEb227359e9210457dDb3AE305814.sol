...
///         Relayers are expected to call the forwarder contract `execute`, which deploys a smart Wallet if necessary first.
...
contract AvoWallet is VariablesV1, SelfUpgradeable, EIP712Upgradeable, ERC721HolderUpgradeable {
    ...
    bytes32 public constant CAST_TYPE_HASH =
        keccak256(
            "Cast(Action[] actions,uint256 validUntil,uint256 gas,address source,bytes metadata,uint256 avoSafeNonce)Action(address target,bytes data,uint256 value)"
        );
    ...
    function cast(
        ...
        bool isSenderForwarder_ = msg.sender == avoForwarder;
        ...
        (success_, result_) = address(this).call{ value: msg.value }(abi.encodeCall(AvoWallet._callTargets, actions_));
        ...
        for (uint256 i; i < actionsLength_; ) {
            ...
            if (action_.value != 0) {
                if (address(this).balance < action_.value) {
                    revert(string.concat(i.toString(), "_AVO__INSUFFICIENT_VALUE"));
                }
            }
