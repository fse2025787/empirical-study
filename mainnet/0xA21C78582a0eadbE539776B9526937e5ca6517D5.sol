...
contract Cryptoraffle is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;
  string public baseURI;
  string public uriPrefix = "";
  string public uriSuffix = ".json";
  string public hiddenMetadataUri;
  uint256[] private tokenIdsMinted;
  ...
  constructor() 
    ERC721("Crypto Raffle ", "CR")
    RandomlyAssigned(6000,1)
    {
        setBaseURI("ipfs://QmPwRNC9Qe8SyMN4Je3yZauuPDox9a9XGwXYDo7LsMxKMt/");
    }
      ...
      payable
  {
      ...
      require( tokenCount() + mintAmount <= totalSupply(), "YOU CAN'T MINT MORE THAN MAXIMUM SUPPLY");
      require(mintAmount + _allowList[msg.sender] <= maxMintAmountPerTx ,"You can not buy more NFTs");
      ...
      _allowList[msg.sender] =_allowList[msg.sender] + mintAmount;
    ...
    returns (uint256[] memory)
  {
    ...
    for (uint256 i = 0; i < tokenIdsMinted.length; i++) {
        if(ownerOf(tokenIdsMinted[i]) == address(_owner))
            tokenIds[index++] = tokenIdsMinted[i];
    }
    return tokenIds;
  }
  ...
  function pause(bool _state) public onlyOwner {
    paused = _state;
     ...
     function givewayTo(address _user) public onlyOwner
    {
        uint256 id = nextToken();
        tokenIdsMinted.push(id);
        _mint(_user, id);
