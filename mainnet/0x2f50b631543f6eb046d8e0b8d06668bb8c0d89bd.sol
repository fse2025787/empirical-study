...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function transferOwnership(address newAdmin) external onlyAdmin {
        require(newAdmin != address(0), "ERC721: new admin is the zero address");
        require(newAdmin != _admin, "ERC721: same admin");
        _setAdmin(newAdmin);
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        require(_exists(tokenId), "ERC721: approve for nonexistent token");
...
contract NFTFactory {
    ...
    function createNewNFT(
        ...
        NFT nft = new NFT(owner, baseURI, name, symbol);
        emit NFTCreated(address(nft), name, symbol);
        return address(nft);
    }

    function transferOwnership(address newAdmin) external onlyAdmin {
        require(newAdmin != address(0), "Factory: new admin is the zero address");
        require(newAdmin != admin, "NFTFactory: same admin");
        _setAdmin(newAdmin);
    }
