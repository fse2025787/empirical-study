...
contract Furafic_Fark is ERC721A, Ownable {
    ...
    constructor(string memory baseURI, address _founderAddress, bytes32 root) ERC721A("Furafic Fark", "FFK") {
    ...
    function Airdrop(uint256 quantity, address wallet) external payable onlyOwner {
    require(totalNFTs + quantity <= maxNFTs, "Exceeded max NFTs amount");
    require(quantity <= 150, "Exceeded max transaction amount");
    ...
    function PresaleMint(uint256 quantity, bytes32[] calldata _merkleProof) external payable whenPresaleStarted {
    ...
    function widthdrawSeparated(address wallet) public onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0, "Insufficent balance");
        _widthdraw(wallet, balance.div(100).mul(20));
        _widthdraw(wallet, balance.div(100).mul(30));
        _widthdraw(founderAddress, balance.div(100).mul(50));
    }
