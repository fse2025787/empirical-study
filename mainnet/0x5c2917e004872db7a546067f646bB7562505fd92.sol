...
contract S3ETHAaveVesperFinanceDAI {
    ...
    function withdraw(uint8 _percentage, uint256 _amountInMaximum, uint256 wethAmountOut, address _feeToken) external {
        require(depositors[msg.sender] != address(0), "ERR: INVALID_DEPOSITOR"); 

        uint256 ethAmountToBeWithdrawn = IS3Proxy(depositors[msg.sender]).withdraw(_percentage, _amountInMaximum); 
        ethAmountToBeWithdrawn += _swapYieldProfitTo(depositors[msg.sender], wethAmountOut); 
...
contract S3ETHAaveVesperFinanceDAIProxy {
    ...
    function _withdrawCollateral(uint8 _percentage, uint256 _currentDebtAfterRepaying, uint256 _amountInMaximum) private returns(uint256) {
        ...
        if (_currentDebtAfterRepaying > 0) {
            ...
            IUniswapConnector(uniswapConnector).swapTokenForTokenV3ExactOutput{value: _amountInMaximum}(
                wethAddress, 
                daiAddress,
