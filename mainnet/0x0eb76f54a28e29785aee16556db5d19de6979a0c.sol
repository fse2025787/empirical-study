...
contract KeyPoolInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the smart chef factory
    address public KEY_POOL_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == KEY_POOL_FACTORY, "Not factory");
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (pending > 0) {
            uint256 rewardTransferred = safeRewardTransfer(msg.sender, pending);
            user.pending = pending.sub(rewardTransferred);
        }
        ...
        returns (uint256)
    {
        ...
        // If staking token is same as reward token
        if (address(stakedToken) == address(rewardToken)) {
            rewardBalance = rewardBalance > stakedSupply
                ? rewardBalance.sub(stakedSupply)
                : 0;
        }

        if (_amount > rewardBalance) {
            _amount = rewardBalance;
        }
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        uint256 pending = user
            ...
            .sub(user.rewardDebt);
        return user.pending.add(pending);
    }
...
contract KeyPoolFactory is Ownable {
    event NewKeyPoolContract(address indexed smartChef);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(KeyPoolInitializable).creationCode;
        ...
        KeyPoolInitializable(smartChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewKeyPoolContract(smartChefAddress);
    }
}