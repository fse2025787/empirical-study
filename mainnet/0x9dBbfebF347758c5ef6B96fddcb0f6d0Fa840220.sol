...
interface IRLPToken {
    function initialize(string memory, string memory, uint256, address) external; // Initialized RLP token with, symbol, name, initial supply, baseLPToken
}
...
contract CoreVault is OwnableUpgradeSafe {
    ...
    IRLPFactory public immutable RLP_FACTORY;

    constructor(address rlpFactory) public {
        RLP_FACTORY = IRLPFactory(rlpFactory);
    }

    // === end upgrade 1 state ===

    // == start upgrade 1 functions ==

    // Switches all RLP for LP inside the vault
    function swapAllRLPForRLP() public onlyOwner {
        string memory rlpSymbol = "coreDEX RLP";
        string [3] memory symbols = [rlpSymbol,rlpSymbol,rlpSymbol];
        string [3] memory names = ["CORE/WETH RLP", "CORE/WBTC RLP", "CORE/DAI RLP"];
        // Migration steps 
        // - Create RLP tokens
        // - Initialize RLP tokens
        // - Send LP tokens for migration of liquidity pools
        // - Set RLP token as the new token for that pool
        // - Make sure the balance of RLP is the same as balance of LP was.
        uint256 len = poolInfo.length;
        require(len == 3, "Invalid number of pools");

        for (uint256 pid = 0; pid < len; ++pid) {
            IERC20 currentToken = poolInfo[pid].token;
            uint256 balToken = currentToken.balanceOf(address(this));
            IRLPToken rlpToken = IRLPToken(RLP_FACTORY.createRLPToken());
            rlpToken.initialize(names[pid], symbols[pid], balToken, address(currentToken)); // name, symbol initial supply (sent to initializer), addres of the token being consumed
            currentToken.safeTransfer(devaddr, balToken); // CORE multisig gets all LP to proceed with migration of floor to DAI
            require(IERC20(address(rlpToken)).balanceOf(address(this)) == balToken, "Did not get enough or RLP");
            poolInfo[pid].token = IERC20(address(rlpToken)); // We set the new token as RLP so everyone is migrated to the new one.
        }

    }


    // == end upgrade 1 functions ==
