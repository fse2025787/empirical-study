...
// File: contracts/ANTISQUATCH.sol
...
contract ANTISQUATCH is ERC721A, Ownable {
  
    uint256 constant _maxSupply = 300;
    uint256 constant  maxMintAmountPerTx = 2;
    ...
    require(!paused);
    require(balanceOf(msg.sender) < maxMintAmountPerWallet, "You have 10 Yetis.");
    ...
    function setbaseURL(string memory uri) public onlyOwner{
        ...
        function setRevealed(bool _reveal) public onlyOwner{
        revealed = _reveal;
    }
    ...
    function hasParents(address sender) public view returns(bool){
        ...
        super._beforeTokenTransfers(from, to, tokenId, number);
        if (from == address(0)) {
