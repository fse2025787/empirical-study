...
contract NftQuant is ERC721, Pausable, Ownable, ReentrancyGuard {
  using Counters for Counters.Counter;
  using Strings for uint256;

  // for developers and marketers
  uint256 NUM_INTERNAL_TOKENS = 7;

  string private _baseTokenURI = "https://token-api.nftquant.ai/tokens/";
  uint256 private _price = 0.99 ether;
  uint256 private _presalePrice = 0.9 ether;
  uint256 private _presaleAllowedAmount = 2;
  
  address private _internalTokenWallet;
  address private _paymentWallet;

  uint256 private _totalSupply = 777;
  ...
  ) ERC721("NFTQuant Membership", "QUANT") {
  ...
  function _mintPresale(uint256 amount, bytes32[] calldata proof) private {
    ...
    require(claimedWhitelist[msg.sender] + amount <= _presaleAllowedAmount, "Already claimed");
