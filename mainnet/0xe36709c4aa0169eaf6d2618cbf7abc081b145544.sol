...
// File: contracts/LeprechaunTown_WTF_Staking.sol
...
contract LeprechaunTown_WTF_Staking is ERC20, ERC721Holder, ERC1155Holder, Ownable{
    ...
    constructor(address _parentNFT_A) ERC20("$GOLD", "$GOLD") {
    ...
    /**
     * @dev User can stake an NFT they own to earn rewards over time.
     ...
     */
    function stake(uint256 _tokenID, bool _contract_A) public {
        if(_contract_A){
     ...
     */
    function estimateRewards(uint256 _tokenID, bool _contract_A) public view returns (uint256) {
        uint256 timeElapsed;
    ...
    /**
     * @dev User can unstake an NFT to earn the rewards gained over time.
     * Note: User must have a NFT already staked in order to unstake and gain rewards.
     * This function only unstakes a single NFT ID that they currently have staked.
     * Rewards are calculated based on the Emission_Rate.
     */
    function unstake(uint256 _tokenID, bool _contract_A) public {
        if(_contract_A){
            require(tokenOwnerOf_A[_tokenID] == msg.sender, "YOU CANNOT UNSTAKE A TOKEN");
            _mint(msg.sender, estimateRewards(_tokenID, _contract_A)); // Minting the reward tokens gained for staking A
            ...
            require(tokenOwnerOf_B[_tokenID] == msg.sender, "YOU CANNOT UNSTAKE B TOKEN");
            _mint(msg.sender, estimateRewards(_tokenID, _contract_A)); // Minting the reward tokens gained for staking B
