 ...
 *
 * The admin role for all roles is `ADMIN_ARYZE`, which means
 * that only accounts with this role will be able to grant or revoke other
 * roles. More complex role relationships can be created by using
 * {_setRoleAdmin}.
 *
 * WARNING: The `ADMIN_ARYZE` is also its own admin: it has permission to
    ...
    ERC165Upgradeable
{
    ...
    bytes32 public constant ADMIN_ARYZE = 0x00;
...
/**
 * @dev ARYZE_RYZE token
 */

contract ARYZE_RYZE is
    Initializable,
    ERC20Upgradeable,
    ...
    WhitelistUpgradeable
{
     ...
     */
        _grantRole(ADMIN_ARYZE, msg.sender);
        _grantRole(ADMIN_ARYZE, multiSigTreasury);
        ...
        setMinimumFee(0);
        _mint(multiSigTreasury, 515707537000000000000000000);
    }
     ...
     */
    function addToWhitelist(address account) public onlyRole(ADMIN_ARYZE) {
     ...
     */
    function removeFromWhitelist(address account) public onlyRole(ADMIN_ARYZE) {
     ...
     * @dev Triggers stopped state.
     */
    function bridgePause() public onlyRole(ADMIN_ARYZE) {
        require(_bridgePaused == false, "Bridge paused");
        _bridgePaused = true;
     ...
     * @dev Returns to normal state.
     */
    function bridgeUnpause() public onlyRole(ADMIN_ARYZE) {
     ...
     */
    function setMinimumFee(uint256 fee) public onlyRole(ADMIN_ARYZE) {
     ...
     */
    function setBridgeOwner(address payable account) public onlyRole(ADMIN_ARYZE) {
