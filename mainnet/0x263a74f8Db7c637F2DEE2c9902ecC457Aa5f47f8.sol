...
contract GuessWinner is ReentrancyGuard, Ownable {
    ...
    uint256 public amountMax = 1000 * 10**6;

    uint256 public amountMin = 10 * 10**6;
    ...
    function withdrawal() public nonReentrant {
        ...
        uint256 reward = (totalAmount / 10**6) * share;
    ...
    function getUserFee(uint256 amount) public view returns (uint256) {
        uint256 percentage = mulDiv(1 ether, poolFee * 10**6, 100 * 10**6);

        uint256 fee = (amount / 10**6) * percentage;
    ...
    function getRelationReward(uint256 amount) public view returns (uint256) {
        ...
        uint256 percentage = mulDiv(1 ether, relationFee * 10**6, 100 * 10**6);

        uint256 reward = (amount / 10**6) * percentage;
