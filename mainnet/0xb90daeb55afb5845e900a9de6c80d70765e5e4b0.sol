...
contract PaydeceEscrow is  ReentrancyGuard , Ownable  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function createEscrow(uint _orderId, address payable _buyer, address payable _seller, uint256 _value,IERC20 _currency) external virtual   {
        ...
        uint256 _amountFeeBuyer = weiToEther(_value  * (feeBuyer * 10**_decimals) / (100 * 10**_decimals)); 
        
        //Valida el Allowance
        require( _currency.allowance( _buyer, address(this)) >= (_value+_amountFeeBuyer),"Seller approve to Escrow first");
    
        //TODO: validar la cantidad que envia el comprador, deberia ser el valor del asset + el buyerFee

        //Transfer USDT to contract
        _currency.safeTransferFrom(_buyer, address(this) , (_value+_amountFeeBuyer) );
                
        escrows[_orderId] = Escrow(_buyer, _seller, _value, feeSeller, feeBuyer, _currency,EscrowStatus.Funded);
    ...
    function _releaseEscrow (uint _orderId) private nonReentrant{
        ...
        //Obtiene el monto a transferir desde el comprador al contrato        
        uint256 _amountFeeBuyer = weiToEther(escrows[_orderId].value  * (feeBuyer * 10**_decimals) / (100 * 10**_decimals));
        uint256 _amountFeeSeller = weiToEther(escrows[_orderId].value  * (feeSeller * 10**_decimals) / (100 * 10**_decimals));

        //feesAvailable += _amountFeeBuyer + _amountFeeSeller;
        feesAvailable[escrows[_orderId].currency] += _amountFeeBuyer + _amountFeeSeller;
    ...
    function refundBuyer(uint _orderId) external nonReentrant onlyOwner {       
        ...
        escrows[_orderId].currency.safeTransfer(escrows[_orderId].buyer, escrows[_orderId].value);
    ...
    */
    ...
    //TODO: Guardar los feets en array para despues poder hacer el retiro como Owner
    function withdrawFees(IERC20 _currency) external onlyOwner {        
        ...
        _amount = _currency.balanceOf(owner()); 
    ...
    function weiToEther(uint valueWei) private pure returns (uint)
    {
