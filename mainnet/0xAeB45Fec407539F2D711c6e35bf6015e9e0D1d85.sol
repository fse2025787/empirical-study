...
contract LOGCreatorAccess is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    mapping(address => bool) internal creatorAccessClaim;
    ...
    /**
     * @dev  mint funvtion
     * @dev every address only get 1 NFT
     */
    function mint(bytes32[] calldata merkleProof) public mintCompliance(1) {
        require(enableMint, "Whitelist sale is not enabled!");
        require(
            _isOnList(msg.sender, merkleProof, merkleRoot),
            "Invalid proof"
        );
        require(!creatorAccessClaim[_msgSender()], "Address already claimed");

        creatorAccessClaim[_msgSender()] = true;
    ...
    function setMintingStatus(bool state) public onlyOwner {
        enableMint = state;
    }
