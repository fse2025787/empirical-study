    ...
    UUPSUpgradeable
{
    ...
    function addWhiteList(
        ...
        for (uint16 i = 0; i < list.length; i++) {
            whiteList[round][list[i]] = whiteList[round][list[i]] + count;
        }
    }
    ...
    function mint(string[] memory uris) external {
        require(
            uris.length <= whiteList[currentRound][_msgSender()],
            "invalid address"
        );
        ...
        for (uint16 i = 0; i < uris.length; i++) {
            ...
            emit UserMint(_msgSender(), tokenId, uris[i]);
        }
        whiteList[currentRound][_msgSender()] =
            whiteList[currentRound][_msgSender()] -
            uint8(uris.length);
    }
}
