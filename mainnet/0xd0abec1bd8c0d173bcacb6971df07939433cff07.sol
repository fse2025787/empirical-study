...
// File: contracts/strategies/StabilizeStrategyXUSDArbV2.sol
...
contract StabilizeStrategyXUSDArbV2 is Ownable {
    using SafeMath for uint256;
    ...
    // All the pools go through 3pool to trade between each other
    struct TokenInfo {
    ...
    address constant THREE_POOL_ADDRESS = address(0x6c3F90f043a72FA612cbac8115EE7e52BDe6E490);
    ...
    function setupWithdrawTokens() internal {
        // USDN, MUSD, DUSD, USDP, ALUSD
        // 3pool is the intermediary token at position 1, usdc at position 2 for underlying
        ...
        tokenList.push(
            TokenInfo({
                token: _token,
                decimals: _token.decimals(),
                curvePoolAddress: address(0x43b4FdFD4Ff969587185cDB6f0BD875c5Fc83f8c),
                active: true
            })
        );
    }
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        ...
        if(_idOut == tokenList.length){
            ...
            // Convert to USDC
            _amount = pool.get_dy_underlying(0, 2, _amount);
            ...
            // We'll go through 3pool as it uses less gas
            // First get 3pool
            _amount = pool.get_dy(0, 1, _amount);
            
            // Then convert to target token
            pool = CurvePool(tokenList[_idOut].curvePoolAddress);
            _amount = pool.get_dy(1, 0, _amount);
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        ...
        if(_idOut == tokenList.length){
            ...
            // We will use 2 curve pools to find the prices
            // First get 3pool as it uses less gas
            uint256 _before = IERC20(THREE_POOL_ADDRESS).balanceOf(address(this));
            pool.exchange(0, 1, _amount, 1);
            _amount = IERC20(THREE_POOL_ADDRESS).balanceOf(address(this)).sub(_before); // Get 3POOL amount
            
            // Then convert to target token
            pool = CurvePool(tokenList[_idOut].curvePoolAddress);
            IERC20(THREE_POOL_ADDRESS).safeApprove(address(pool), 0);
            IERC20(THREE_POOL_ADDRESS).safeApprove(address(pool), _amount);
            pool.exchange(1, 0, _amount, 1);
            return;
        }
    }
    ...
    function checkAndSwapTokens(address _executor, uint256 _sellID, uint256 _buyID) internal {
        ...
        if(tokenList[_sellID].active == false || tokenList[_buyID].active == false){ return; }
    ...
    // --------------------
    function startAddNewStableToken(address _tokenAddress, address _curvePool) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 8;
