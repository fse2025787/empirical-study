...
/**
 * @title BadgerPriceOracleV2
 * @notice Returns prices for bDIGG, bBADGER, ibBTC, and bveCVX.
 * @dev Implements `PriceOracle` and `BasePriceOracle`.
 * @author David Lucid <[email protected]> (https://github.com/davidlucid)
 */
contract BadgerPriceOracleV2 is PriceOracle, BasePriceOracle {
    ...
    /**
     * @dev bveCVX ERC20 token contract.
     */
    ISettV4h constant public bveCVX = ISettV4h(0xfd05D3C7fe2924020620A8bE4961bBaA747e6305);

    /**
     * @dev CVX ERC20 token contract.
     */
    ...
    function _price(address token) internal view returns (uint) {
        ...
        } else if (token == address(bveCVX)) {
            return BasePriceOracle(msg.sender).price(CVX).mul(bveCVX.getPricePerFullShare()).div(1e18);
