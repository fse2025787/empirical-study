...
// File contracts/base/interface/curve/Gauge.sol
...
// File contracts/base/interface/curve/ICurveGUSDDeposit.sol
...
// File contracts/strategies/curve/gusd/CRVStrategyGUSD.sol
...
* to produce mixToken.
*/
contract CRVStrategyGUSD is StrategyBaseClaimable {
  ...
  address public weth;
  address public dai;
  address public curveDepositGUSD;
  ...
  constructor(
    ...
    address _weth,
    address _dai,
    address _curveDepositGUSD,
    address _uniswap
  )
  StrategyBaseClaimable(_storage, _underlying, _vault, _crv, _crv, _uniswap) public {
    require(IVault(_vault).underlying() == _underlying, "vault does not support gusdCRV");
    pool = _gauge;
    mintr = _mintr;
    crv = _crv;
    weth = _weth;
    dai = _dai;
    curveDepositGUSD = _curveDepositGUSD;
    uni = _uniswap;
  ...
  */
  function gusdCRVFromDai() internal {
    uint256 daiBalance = IERC20(dai).balanceOf(address(this));
    if (daiBalance > 0) {
      IERC20(dai).safeApprove(curveDepositGUSD, 0);
      IERC20(dai).safeApprove(curveDepositGUSD, daiBalance);
...
// File contracts/strategies/curve/gusd/CRVStrategyGUSDMainnet.sol
...
* to produce crvGUSD.
*/
contract CRVStrategyGUSDMainnet is CRVStrategyGUSD {
  ...
  constructor(
    ...
    _storage,
    _vault,
    address(0xD2967f45c4f384DEEa880F807Be904762a3DeA07), // crvGUSD underlying
    address(0xC5cfaDA84E902aD92DD40194f0883ad49639b023), // _gauge
    ...
    address(0x64448B78561690B70E17CBE8029a3e5c1bB7136e), // depositGUSD
