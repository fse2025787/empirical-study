...
contract FTX is ERC721, Ownable, ReentrancyGuard{
    using Strings for uint256;
    ...
    Counters.Counter private totalTokensIDs;
    constructor(
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) nonReentrant {
    ...
    for(uint i = 0; i<_mintAmount;i++){
        totalTokensIDs.increment();
        uint current = totalTokensIDs.current();
        _safeMint(_msgSender(), current);
