...
contract  TheWinnersGames  is ERC721Enumerable, Ownable
{
    ...
    uint public status = 1; //1-Free 2-Public


    constructor() ERC721("The Winners Games", "TWG")  {
        setBaseURI("https://gateway.pinata.cloud/ipfs/QmQb2FNrvKRicfaQxcpURvzfZCTMMDcuJscUFDEpoTqSES");
    ...
    function mint(uint chosenAmount) public payable isSaleOpen{
        ...
        if( status == 1 && totalSupply() < 500) {
            require(balanceOf(msg.sender)+chosenAmount <= maxQuantity,"Chosen Amount exceeds MaxQuantity");
    ...
    function tokensOfOwner(address _owner) public view returns (uint256[] memory)
    {
     ...
     function airDrop(address a, uint amount)public onlyOwner{
        require(totalSupply() + amount <= _TOTALSUPPLY, "reached max supply");
