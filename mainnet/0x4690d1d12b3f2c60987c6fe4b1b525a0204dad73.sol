...
// Interface to represent UBT reward splitter
interface IUBTRewardSplitter {
...
/*
    ...
    V2 update: added claiming of rewards
*/
contract MoverUBTStakeNodeV2 is AccessControlUpgradeable, IUBTStakingNode {
    ...
    // callable by admin to set UBT rewards splitter contract
    function setRewardSplitAddress(address _rewardSplitAddress) public {
		require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "admin only");
        rewardSplitAddress = _rewardSplitAddress;
    }

    // claim reward, callable by anyone
    // get rewards from distributor (it transfers to current contract balance)
    // then send part to staked node operator's EOA
    // and the rest to main pool as profits
    function claimRewardFromSplitter() public {
        IUBTRewardSplitter(rewardSplitAddress).release();
