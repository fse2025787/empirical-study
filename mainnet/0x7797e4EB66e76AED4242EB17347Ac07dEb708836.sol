...
contract bl4ck0ut is ERC721A, Ownable, ReentrancyGuard {
  using Address for address;
  using Strings for uint;

  string  public  baseTokenURI = "ipfs://QmP3zisTHrwL2FNo285SUmyywPFEjpKe4WXjCAo8XgVLmw/";
  
  uint256 public  maxSupply = 369;
  uint256 public  maxMintPerTx = 10;
  uint256 public  publicSalePrice = .0066 ether;
  uint256 public  allowlistPrice = 0 ether;
  uint256 public  numFreeMints = 0;
  ...
  constructor() ERC721A("bl4ck0ut", "bl40") {
 ...
 function setpublicSalePrice (uint256 _publicSalePrice) public onlyOwner {
  ...
  function setallowlistPrice (uint256 _allowlistPrice) public onlyOwner {
   allowlistPrice = _allowlistPrice;
    ...
    // require only the allowlist mint
    // Add publicMint and allowListMintOpen Variables
    function allowListMint (uint256 numberOfTokens) 
        external
        payable 
        ...
        require(totalSupply() + numberOfTokens < maxSupply + 1, "SOLD OUT");
        require((allowlistPrice * numberOfTokens) <= msg.value,"MAX MINTS EXCEEDED");
        require(numberOfTokens <= maxMintPerTx,"MAX MINTS EXCEEDED");
  ...
  function mint(uint256 numberOfTokens)
      ...
      payable
  {
    require(PublicSaleActive, "MINT CLOSED");
    require(totalSupply() + numberOfTokens < maxSupply + 1, "SOLD OUT");

    if(freeAlreadyMinted + numberOfTokens > numFreeMints){
        require(
        ...
        require(
            numberOfTokens <= maxMintPerTx,
            "MAX MINTS EXCEEDED"
        );
        } else {
            require(
                numberOfTokens <= maxFreePerWallet,
                "MAX FREE MINTS EXCEEDED"
            );
