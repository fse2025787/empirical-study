...
contract ApeGold is Context, Ownable, ERC20 {
    IERC721Enumerable public apeContract;

    // Give out 10,000 Ape Gold for every Ape that a user holds
    uint256 public apeGoldPerTokenId = 10000 * (10**decimals());
    ...
    constructor(address ape, uint256 maxTokens, address transferOwner) Ownable() ERC20("Ape Gold", "APEG") {
        // Transfer ownership to the Ape DAO
        ...
        tokenIdEnd = maxTokens;
        transferOwnership(transferOwner);
        apeContract = IERC721Enumerable(ape);
    }
    ...
    function claimById(uint256 tokenId) external {
        ...
        require(
            _msgSender() == apeContract.ownerOf(tokenId),
            "MUST_OWN_TOKEN_ID"
        );
    ...
    /// this is a concern, you should use claimRangeForOwner and claim Ape
    /// Gold in batches.
    function claimAllForOwner() external {
        uint256 tokenBalanceOwner = apeContract.balanceOf(_msgSender());
        ...
        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            ...
            // the _claim() function
            _claim(
                apeContract.tokenOfOwnerByIndex(_msgSender(), i),
                _msgSender()
            );
        }
    }
    ...
    /// once or if you want to leave some Ape unclaimed. If you leave Ape
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        // Check that Ape Gold have not already been claimed this season
        // for a given tokenId
        require(
        ...
        // Mark that Ape Gold has been claimed for this season for the
        // given tokenId
        seasonClaimedByTokenId[season][tokenId] = true;

        // Interactions

        // Send Ape Gold to the owner of the token ID
        _mint(tokenOwner, apeGoldPerTokenId);
    }
    ...
    /// 100 Ape, you should enter "100" rather than the value of 100 * 10^18.
    function daoMint(uint256 amountDisplayValue) external onlyOwner {
        _mint(owner(), amountDisplayValue * (10**decimals()));
    }

    
    /// relevant in the event that Ape migrates to a new contract.
    
    function daoSetApeContractAddress(address apeContractAddress_)
    external
    onlyOwner
    {
        apeContract = IERC721Enumerable(apeContractAddress_);
    }
    ...
    /// want to mint 100 Ape, you should enter "100" rather than the value of
    /// 100 * 10^18.
    function daoSetApeGoldPerTokenId(uint256 apeGoldDisplayValue)
    public
    onlyOwner
    {
        apeGoldPerTokenId = apeGoldDisplayValue * (10**decimals());
    }

    
    /// in one transaction. This ensures that there is not a gap where a user
    /// can claim more Ape Gold than others
    ...
    /// apeGold variables directly. It is better practice for security,
    ...
    function daoSetSeasonAndApeGoldPerTokenID(
        uint256 season_,
        uint256 apeGoldDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetApeGoldPerTokenId(apeGoldDisplayValue);
    }
}