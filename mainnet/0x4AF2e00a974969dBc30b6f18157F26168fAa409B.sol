    ...
    Ownable
{
    ...
    function safeTransferFrom(
        ...
        require(
            _isOwnerOrProxy(_msgSender()) || ownerOf(tokenId) == from,
...
contract AssetContract is ERC721Tradable, ReentrancyGuard {
    ...
    constructor(
        ...
        _tokenTypeProduce[_tokentype] = PRODUCE_TOKEN_MAX;
    ...
    function tokenTypeNumberOf(string memory _tokenType, uint256 numberOfTokens)
        public
        onlyOwnerOrProxy
    {
    ...
    function setTokenPrice(uint256 _tokenPrice) public onlyOwnerOrProxy {
        tokenPrice = _tokenPrice;
    }

    function tokenPriceOf() public view returns (uint256) {
        return tokenPrice;
    }
    ...
    function tokenTypeBuckle(string memory tokenType, uint256 numberOfTokens)
        private
    {
        require(
            tokenTypeOf(tokenType).sub(numberOfTokens) >= 0,
            "TokenTypeBuckle Purchase would exceed max supply of tokenType"
        );
        _tokenTypeProduce[tokenType] =
            _tokenTypeProduce[tokenType] -
            numberOfTokens;
    }

    function exitTokenType(string memory _tokenType)
        public
        view
        returns (bool)
    {
        return _tokenTypeProduce[_tokenType] != 0;
    }

    function putSell(string memory _tokenType, uint256 numberOfTokens)
        public
        onlyOwnerOrProxy
    {
        if (!exitTokenType(_tokenType)) {
            _tokenTypeProduce[_tokenType] = numberOfTokens;
        }
    ...
    function _mint(
        ...
        require(
            tokenTypeOf(_tokenType).sub(numberOfTokens) >= 0,
            " Mint Purchase would exceed max supply of tokenType"
        );
        for (uint256 i = 0; i < numberOfTokens; i++) {
            mintTo(_msgSender(), _data);
        }
        tokenTypeBuckle(_tokenType, numberOfTokens);
    }
    ...
    function _ownsTokenAmount(address _from, uint256 _id)
        ...
        returns (bool)
    {
        return _isOwnerOrProxy(_from) ? true : ownerOf(_id) == _from;
    }
...
contract Commodity is AssetContract {
    constructor()
        AssetContract(
            ...
            10 * 10**18,
            "hotsales",
            "https://nft721-1305307836.cos.ap-nanjing.myqcloud.com/teing/"
        )
    {}
}