...
// File operator-filter-registry/src/upgradeable/[email protected]
// License-Identifier: MIT
pragma solidity ^0.8.13;
...
// File contracts/MistfitsNFTV3.sol
// License-Identifier: MIT
...
abstract contract MistfitsNFTV1Storage {
    ...
    address[] internal reserveAddresses;
}

contract MistfitsNFTV3 is
    ERC721AQueryableUpgradeable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable,
    MistfitsNFTV1Storage,
    ...
    ERC2981Upgradeable
{
    ...
    mapping(string => bool) public upgraded;

    mapping(address => bool) public freeMinted;
    uint256 public maxMintAmountPerTxFree;

    modifier mintComplianceFree(uint256 _mintAmount) {
        require(publicMintEnabled, "Not in public mint!");
        require(tx.origin == _msgSender(), "Contract address is not allowed!");
        require(!freeMinted[_msgSender()], "Address already free minted!");
        require(
            _mintAmount > 0 && _mintAmount <= maxMintAmountPerTxFree,
            "Invalid mint amount!"
        );
        require(
    ...
    modifier mintCompliancePublic(uint256 _mintAmount) {
        ...
        if (freeLeft() > 0) {
            require(_mintAmount <= freeLeft(), "Free mint limit exceeded!");
        }
        require(
    ...
    function initialize(
        ...
        uriSuffix = ".json";
        upgraded[version] = true;
    }

    function upgradeToV2() external onlyOwner {
        require(!upgraded[version], "Already upgraded!");
        __ERC721AQueryable_upgrade();
        __ERC2981_upgrade();
        __OperatorFilterer_upgrade(CANONICAL_CORI_SUBSCRIPTION, true);
        uriSuffix = ".json";
        upgraded[version] = true;
    }

    function __ERC721AQueryable_upgrade() private {}

    function __ERC2981_upgrade() private {}

    function __OperatorFilterer_upgrade(
    ...
    function setMaxMintAmountPerTxFree(
        uint256 _maxMintAmountPerTx
    ) external onlyOwner {
        maxMintAmountPerTxFree = _maxMintAmountPerTx;
    }
    ...
    function freeMint(
        ...
        whenNotPaused
        nonReentrant
        mintComplianceFree(_mintAmount)
    {
        freeMinted[_msgSender()] = true;
        _safeMint(_msgSender(), _mintAmount);
        if (walletAddress != address(0) && address(this).balance > 0) {
    ...
    function freeLeft() public view returns (uint256) {
        if (freeSupply + whitelistMinted > totalSupply()) {
            return freeSupply + whitelistMinted - totalSupply();
        }
        return 0;
    }
