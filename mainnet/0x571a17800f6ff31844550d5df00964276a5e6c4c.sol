...
contract PopArtCats is ERC721Enumerable, Ownable {
    ...
    function claimSpecial(uint256 count) public payable {
        ...
        if (msg.sender != owner()) {
            require(validateFreeMint(_msgSender()), "Address cannot claim a free mint.");
            require(count == MAX_FREE_MINTS, "You only get one free mint.");
            require(addressToMinted[_msgSender()] == 0, "Free mint has already been claimed.");
        }
    ...
    function setFreeMints(address[] calldata _addresses) public onlyOwner {
        delete freeMints;
        freeMints = _addresses;
    }
