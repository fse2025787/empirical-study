...
// File: contracts/RabbitCollegeClub.sol
 ...
 * Version: 1.2.0
 */
contract RabbitCollegeClub is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public rabbitPrice = 20000000000000000; // 0.02 ETH
    uint256 public maxRabbitPurchase = 40;
    uint256 public maxRabbits = 10000;

    bool public saleIsActive = false;
    bool public reserved = false;

    event RabbitPriceChanged(uint256 price);
    event MaxTokenAmountChanged(uint256 value);
    event MaxPurchaseChanged(uint256 value);
    event RabbitsReserved();
    ...
    function withdraw() public onlyOwner {
        ...
        Address.sendValue(_msgSender(), balance);
    }

    function reserveRabbits() public onlyOwner onReserve {
    ...
    function mintRabbits(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= maxRabbitPurchase, "Exceeds max number of Rabbits in one transaction");
        require(totalSupply().add(numberOfTokens) <= maxRabbits, "Purchase would exceed max supply of Rabbits");
    ...
    function setPrice(uint256 _price) external onlyOwner {
        ...
        rabbitPrice = _price;
        emit RabbitPriceChanged(_price);
    }

    function setMaxTokenAmount(uint256 _value) external onlyOwner {
        require(
