...
contract GummiesStaking is ERC20("GUM", "GUM"), Ownable{
    using SafeMath for uint256;
    bool paused = false;
	IERC721 nft; 
    
    uint256 private STAKINGREWARD = 100 ether ;
    uint256 private LEGENDARYREWARD  = 500 ether;
    ...
    mapping(address => uint256) public numberLegendaryCount;
    mapping(address => uint256) private _balances;
    ...
    function initLegendaries(uint256[] calldata tokenIds, bool state) public onlyOwner{
            unchecked{
                uint256 len = tokenIds.length;
                for (uint256 i = 0; i < len; i++) {
                    uint256 tokenId = tokenIds[i];
                    legendaries[tokenId] = state;
                }
            }
    }
    ...
    function setLegendaryreward(uint256 val) public onlyOwner{
        LEGENDARYREWARD = val;
    }
    ...
    function getRewards(address account) internal view returns(uint256) {
            uint256 _cReward = (numberTokensStaked[account] - numberLegendaryCount[account]) * STAKINGREWARD;
            uint256 _legendaryReward = numberLegendaryCount[account] * LEGENDARYREWARD;
    ...
    function claimRewards() updateReward(msg.sender) external {
        ...
        if(numberTokensStaked[msg.sender] > 4){
    ...
    function viewRewardWithBonus(address account) public view returns(uint256){
    ...
    function calculateBonus(uint256 reward, address account) internal  view returns(uint256){
        uint256 _numStaked = numberTokensStaked[account];
        if(_numStaked >= 15){
            return reward.mul(120).div(100);
        }else if(_numStaked >= 10){
            return reward.mul(110).div(100);
        }else if(_numStaked >= 5){
            return reward.mul(105).div(100);
        }else{
            return reward;
        }
    }
    ...
    function stake(uint256[] calldata tokenIds) updateReward(msg.sender) external {
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < len; i++) {
                uint256 tokenId = tokenIds[i];
                if(legendaries[tokenId]) {
                    numberLegendaryCount[msg.sender]++;
                }
    ...
    function unstake(uint256[] calldata tokenIds) updateReward(msg.sender) external {
        unchecked {
            uint256 len = tokenIds.length;
