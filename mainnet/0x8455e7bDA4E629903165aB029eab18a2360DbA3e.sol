...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV1, GovernorBravoEvents {
    ...
    string public constant name = "Anoncat Governor Bravo";

    
    uint public constant MIN_PROPOSAL_THRESHOLD = 1; // 1 Anoncat = 1% of Anoncats

    
    uint public constant MAX_PROPOSAL_THRESHOLD = 100; // 100 Anoncats = 100 % of Anoncats
    ...
    uint public constant quorumVotes = 3; // 3 = 3% of Anoncats
    ...
    /**
      ...
      */
    function getActions(uint proposalId) external view returns (address[] memory targets, uint[] memory valuesExecutor, uint[] memory valuesTimelock, string[] memory signatures, bytes[] memory calldatas) {
        Proposal storage p = proposals[proposalId];
        return (p.targets, p.valuesExecutor, p.valuesTimelock, p.signatures, p.calldatas);
    }
    ...
    function _initiate(address governorAlpha) external {
        ...
        // if migrating from governorAlpha
        if (governorAlpha != address(0)) {
          proposalCount = GovernorAlpha(governorAlpha).proposalCount();
          initialProposalId = proposalCount;
          timelock.acceptAdmin();
        } else {
        // if not migrating from governorAlpha
          proposalCount = 1;
          initialProposalId = 1;
        }
    }
