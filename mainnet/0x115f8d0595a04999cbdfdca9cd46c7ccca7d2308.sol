...
contract SpikyDJ is ERC721A, Ownable, ReentrancyGuard {

    string public baseURI;

    address public split;
    uint256 private maxSupply = 3309;
    ...
    mapping (uint256 => uint256) private quantityToPrice;
    mapping (uint256 => bool) private quantityHasDiscount;
    ...
    constructor() ERC721A("SpikyDJ", "SPIKY") {
        addQuantityDiscount(5, 0.09 ether);
        addQuantityDiscount(15, 0.085 ether);
        addQuantityDiscount(30, 0.08 ether);
    } 
    ...
    function addQuantityDiscount(uint256 _quantity, uint256 _price) public onlyOwner {
        quantityToPrice[_quantity] = _price;
    ...
    function checkPrice(uint256 quantity) public view returns (uint256) {
        ...
        if(quantityHasDiscount[quantity]) {
            price = quantityToPrice[quantity];
        }
