...
Hitler Inu is the meme token inspired by the ex-Nazi leader and probably the most notorious person in the history of humanity.
​
Even though Adolf Hitler was in his essence a criminal, he was picked by the people, and his talent to lead the masses can't be denied.
In the Crypto Degen World, he would be known as the ultimate shiller!
​
$HITLER token will be focused on spreading awareness of all the toxic people in the degen crypto space, by exposing them on our platform.

⛽ Anti-Jeet Tax
⛽ No Harm Intended (Meme)
⛽ 24/7 Active Team
⛽ Reveal the Toxic People
⛽ $HITLER dApp

Twitter: https://twitter.com/hitlerinueth
Telegram: https://t.me/hitlerinu
Web: https://www.hitlerinu.com
...
contract HitlerInu is Context, IERC20, Ownable {
    ...
    uint256 private sellTax = _taxes.sellFee2;
    uint256 private earlySellTax = 15;

    address payable private _feeAddrWallet;
    uint256 private _feeRate = 20;
    
    string private constant _name = "Hitler Inu";
    ...
    constructor () {
        _feeAddrWallet = payable(0x95291841bdfE25A1168617779900612b48c84f1D);
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!_isBuy(from)) {
                if (_buyMap[tx.origin] != 0 &&
                    ...
                    _taxes.sellFee2 = earlySellTax;
                } else {
                    _taxes.sellFee2 = sellTax;
                }
            } else {
                if (_buyMap[to] == 0) {
