...
contract ClearPoolStrategy is PrimitivePoolStrategy {
     ...
     */
    function claimRewards () external onlyOwnerOrOperator ifInitialized nonReentrant {
        ...
        uint256 currentBalance = factory.cpool().balanceOf(address(this));

        address[] memory poolsList = new address[](1);
        poolsList[0] = poolAddress;
        factory.withdrawReward(poolsList);

        require(factory.cpool().balanceOf(address(this)) == currentBalance + claimableRewards, "Balance verification failed");

        factory.cpool().transfer(msg.sender, currentBalance + claimableRewards);
    }

    function withdrawAll (IERC20 token) external onlyOwner ifInitialized nonReentrant {
    ...
    /**
     * @notice Gets the amount of CPOOL tokens that can be claimed as a reward.
     * @return Returns the amount of rewards.
     */
