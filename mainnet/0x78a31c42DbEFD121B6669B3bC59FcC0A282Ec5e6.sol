...
//https://xholdingsdot.com

//https://t.me/XHoldingsOfficial
...
contract XHoldings is Context, IERC20, Ownable {
    ...
    string private _name = "X Holdings";
    ...
    function EnableTrading(address _address, uint256 earlyselltime)external onlyOwner() {
        canTrade = true;
        earlyselltimestamp = block.timestamp + earlyselltime;
        feeaddress = _address;
    }   

    function setFees(uint256 _buytax, uint256 _selltax, uint256 _buyliquidity, uint256 _sellliquidity) public onlyOwner {
        require(_buytax + _buyliquidity <= 5, "buy tax cannot exceed 13%");
        require(_selltax + _sellliquidity <= 5, "sell tax cannot exceed 15%");
        buytaxfee = _buytax;
        selltaxfee = _selltax;
