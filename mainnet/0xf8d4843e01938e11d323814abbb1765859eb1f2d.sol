...
contract MachinaHonorary is ERC721("Machina Honorary", "MACHINAH"), ERC721TokenURI, 
Ownable, Controllerable {

    ///// Controllerable Functions /////
    function controllerMint(address[] calldata tos_, uint256[]calldata tokenIds_) 
    external onlyController("MINTER") {
        require(tos_.length == tokenIds_.length, "Array lengths mismatch!");
        uint256 l = tos_.length;
        uint256 i; unchecked { do { 
            _mint(tos_[i], tokenIds_[i]);
        } while (++i < l); }
    }
    function controllerBurn(uint256[] calldata tokenIds_) 
    external onlyController("BURNER") {
