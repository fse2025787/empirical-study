...
// File: contracts/bsanyc_gen2.sol
...
contract BSANYC2 is Ownable, ERC721A {
    ...
    BSANYC private _bsanycContract;
    ...
    constructor(
        ...
        uint256 collectionSize_,
        uint256 amountForDevs_, 
        address bsanycAddress_
    ) ERC721A("BSANYC2", "bsanyc2") {
        require(
            maxBatchSize_ < collectionSize_,
            "MaxBarchSize should be smaller than collectionSize"
        );
        ...
        _bsanycContract = BSANYC(bsanycAddress_);
    ...
    function reborn(uint256 tokenId1, uint256 tokenId2)
    external 
    callerIsUser
    {
        require(isRebornTimeOn(), "Reborn has not begun yet");
        ...
        require(_msgSender() == _bsanycContract.ownerOf(tokenId1), 'TokenId1 not owned by currentSender');
        require(_msgSender() == _bsanycContract.ownerOf(tokenId2), 'TokenId2 not owned by currentSender');

        _bsanycContract.burnFromAltar(tokenId1);
        _bsanycContract.burnFromAltar(tokenId2);
    ...
    function isRebornTimeOn() public view returns(bool) {
        require(
            saleConfig.rebornStartTime != 0,
            "Reborn Time is TBD."
        );

        return block.timestamp >= saleConfig.rebornStartTime;
    }
