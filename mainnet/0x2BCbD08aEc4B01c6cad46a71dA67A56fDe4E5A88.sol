...
contract CHIP is Context, IERC20, Ownable {
    ...
    uint256 private gasPriceLimit = 300 * 1 gwei; // do not allow over 300 gwei for launch
    ...
    event SetBuyFee(uint256 marketingFee);
    
    event SetSellFee(uint256 marketingFee);
    ...
    constructor() payable {
        ...
        marketingAddress = payable(0x5eF345e543c08E7026d49983bA4D2D82ACA7CD8F); // Marketing Address
    ...
    function intializeContract() external onlyOwner {
        ...
        _decimals = 18;
         ...
         _tTotal = 21 * 10**6 * 1e18;
         _rTotal = (MAX - (MAX % _tTotal));
        
        _rOwned[_msgSender()] = _rTotal;
       
        maxTransactionAmount = _tTotal * 1 / 100; // 1% maxTransactionAmountTxn
        minimumTokensBeforeSwap = _tTotal * 1 / 1000; // 0.1% swap tokens amount
    ...
    function includeInReward(address account) private onlyOwner {
    ...
    function _transfer(
        ...
        uint256 totalTokensToSwap = _marketingTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyMarketingFee;
                buyOrSellSwitch = BUY;
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellMarketingFee;
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            _marketingTokensToSwap += _buyMarketingFee;
        } else if(buyOrSellSwitch == SELL){
            _marketingTokensToSwap += _sellMarketingFee;
        }
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function setBuyFee(uint256 buyMarketingFee)
        external
        onlyOwner
    {

        _buyMarketingFee = buyMarketingFee;
        require(_buyMarketingFee <= 15, "Must keep buy taxes below 15%");
        emit SetBuyFee(buyMarketingFee);
    }

    function setSellFee(uint256 sellMarketingFee)
        external
        onlyOwner
    {
        _sellMarketingFee = sellMarketingFee;
        require(_sellMarketingFee <= 15, "Must keep sell taxes below 15%");
        emit SetSellFee(sellMarketingFee);
    }
    
     function setMaxTransactionAmount(uint256 percent, uint256 divisor) external onlyOwner {
