...
contract RushBondingCalculator is IBondingCalculator {
    ...
    address public immutable RUSH;

    constructor( address _RUSH ) {
        require( _RUSH != address(0) );
        RUSH = _RUSH;
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == RUSH ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( RUSH ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}
