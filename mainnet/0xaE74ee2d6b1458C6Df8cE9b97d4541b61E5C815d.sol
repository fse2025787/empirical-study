...
//https://t.me/ForeverLoveCoin
//https://www.gracieroze.com/products/forever-love-coins-for-coin-shels

//Low Liquidity into a fearsome marvel!
//Initial Supply: 1,000,000,000 Tokens
//To assist with liquidity stabilisation and aggressive marketing plan
//First 24hrs 12% Buy/Sell (8% LP 4% Marketing)
//Reverts to 9% Buy/Sell (6% LP 3% Marketing)
...
contract ForeverLoveCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private gasPriceLimit = 540 * 1 gwei; // do not allow over x gwei for launch
    ...
    constructor() ERC20("Forever Love Coin", "FLC") {
        ...
        if(airDropLimitInEffect){ // Check if Limit is in effect
            if(airDropLimitLiftDate <= block.timestamp){
                ...
                if(_isAirdoppedWallet[from] && senderBalance.sub(amount) < _airDropTokensRemaining[from]){
                    ...
                    require(_airDropAddressNextSellDate[from] <= block.timestamp && block.timestamp >= airDropLimitLiftDate.sub(10 days), "_transfer:: Please read the contract for your next sale date.");
