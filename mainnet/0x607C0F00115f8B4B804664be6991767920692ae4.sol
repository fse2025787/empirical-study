...
contract AngryApeArmyTreasury is Ownable {
    ...
    function withdrawErc20(address tokenAddress) external {
        ...
        require(token.transfer(teamMemberA, teamMemberAAmount), "Withdraw Failed To Member A.");

        require(token.transfer(teamMemberB, teamMemberBAmount), "Withdraw Failed To Member B.");

        require(token.transfer(teamMemberC, teamMemberCAmount), "Withdraw Failed To Member C.");

        require(token.transfer(teamMemberD, teamMemberDAmount), "Withdraw Failed To Member D.");
        
        require(token.transfer(teamMemberE, restAmount), "Withdraw Failed To Member E.");
    }
