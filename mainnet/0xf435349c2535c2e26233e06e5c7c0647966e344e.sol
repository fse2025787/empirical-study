...
contract Comptroller {
    ...
    // Get clients id from primary address
    mapping(address => uint) primaryId;
    // Get advisor id from advisor address
    mapping(address => uint) advisorToId;
        ...
        returns (uint256)
    {
        ...
        clientId++;
        _registerClient(clientId,_clientSafes,_primaryAddress,_backup,_advisorId);
    ...
    function changeClientPrimaryAddress(
        ...
        primaryId[_primaryAddress] = clientId;
    ...
    function changeClientBackupAddress(
        uint clientId,
    ...
    function isClientPrimaryAddress(address _address) external view returns (bool isClient) {
    ...
    function getClientIdFromSafeAddress(address _address) public view returns(uint){
        return addressId[_address];
    }

    function getClientIdFromPrimaryAddress(address _address) public view returns(uint){
        return primaryId[_address];
    }

    function getPrimaryAddressFromClientId(uint clientId) external view returns (address){
        return clients[clientId].primaryAddress;
    }

    function getBackupAddressFromClientId(uint clientId) external view returns (address){
        return clients[clientId].backupAddress;
    }
    ...
    function getSafesByAdvisor(address advisorAddress) external view returns (address[][] memory safesList) {
        ...
        uint[] memory clientIds = advisors[advisorId].clientIds.values();
        uint len = clientIds.length;
        safesList = new address[][](len);
        for (uint i =0; i< len; i++){
            address[] memory safes = getClientAddresses(clientIds[i]);
            safesList[i] = safes;
        }
    }

    
    /// It takes the client Safe address as input, and returns:
    ...
    function getAllClientDataFromPrimary(
        address primaryAddress
    ) 
        external
        view
        returns(address[] memory safes, string[] memory safeTypes, address[][] memory linkedAddresses, uint[][] memory roles)
    {  
        safes = clients[primaryId[primaryAddress]].clientSet.values();
        uint len = safes.length;
        linkedAddresses = new address[][](len*8);
        roles = new uint[][](len*8);
        safeTypes = new string[](len);
        ...
        returns(string memory safeType, address[] memory linkedAddresses, uint[] memory roles)
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (clients[clientId].primaryAddress==_owners[i]){
                roles[i] = 1;
            } else if (clients[clientId].backupAddress ==_owners[i]){
                roles[i] = 2;
    ...
    function getClientAddresses(address clientSafeAddr)
        public
        view
        returns (address[] memory)
    {
        uint clientId = addressId[clientSafeAddr];
        return clients[clientId].clientSet.values();
    }

    
    function getClientAddressesFromPrimary(address clientPrimaryAddr)
        public
        view
        returns (address[] memory)
    {
        uint clientId = primaryId[clientPrimaryAddr];
        return clients[clientId].clientSet.values();
    }
        ...
        internal 
    {
        ...
        advData.clientIds.add(clientId);
