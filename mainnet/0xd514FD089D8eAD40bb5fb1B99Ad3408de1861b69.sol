...
** Using FTP LPAdd to recycle 10.00% of ALL transactions back into the liquidity pool. **
...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x49b50Dc217865AECb7A7E3e07d96D1C2f06856bd), 3400);
		setTaxAlloc(payable(0x91Bb0b4b539e9A156D4A5d1580CC717dB248d9fF), 1600);
        m_DidDeploy = true;
    }
...
contract GhostinShadow is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000 * 10**9;
    string private m_Name = "Ghost in Shadow";
    string private m_Symbol = "GITS";
    uint8 private m_Decimals = 9;
    // EXCHANGES
    address private m_UniswapV2Pair;
    IUniswapV2Router02 private m_UniswapV2Router;
    // TRANSACTIONS
    uint256 private m_WalletLimit = TOTAL_SUPPLY.div(33);
    bool private m_Liquidity = false;
    event NewTaxAlloc(address Address, uint256 Allocation);
    event SetTxLimit(uint TxLimit);
	// LP ADD
    IWETH private WETH;
    uint256 private m_LiqAlloc = 10000;
    // MISC
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != m_UniswapV2Pair && _recipient != address(m_UniswapV2Router) && block.timestamp <= m_Launched.add(0 hours);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(2 minutes))
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(1 days), msg.sender);
