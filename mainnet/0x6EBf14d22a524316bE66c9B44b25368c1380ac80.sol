...
contract CapsuleToy is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public tokenPrice = 0.06 ether;  //0.06 ETH
    
    uint256 public constant MAX_CAPS = 9999;
    uint256 public constant MAX_GRANT_MINTS = 500;
    uint256 public constant MAX_GROUP_MINTS = 300;
    uint256 public constant MAX_SALE_MINTS = MAX_CAPS - MAX_GRANT_MINTS - MAX_GROUP_MINTS;
    
    //uint256 public grantMintedNum = 0;
    uint256 public saleMintedNum = 0;
    uint256 public groupMintedNum = 0;
    uint256 public multiNum = 0;
    ...
    function withdraw() public onlyOwner {
        ...
        require(payable(wt).send(balance));
    }

    function OnSaleState() public onlyOwner {
        publicSale = !publicSale;
    }
    
    /*
    function OnCommunityState() public onlyOwner {
    ...
    function mint(uint nums) external payable {
        ...
        require(totalSupply() < MAX_CAPS, "capsuletoy sold out.");
    ...
    function mintMulti(address[] memory dests) public onlyOwner {
        require(totalSupply() < MAX_CAPS, "capsuletoy sold out.");
        
        for(uint i = 0; i < dests.length; i++){
            uint id = nextIndex();
            _safeMint(dests[i], id);
            multiNum = multiNum.add(1);
        }
    }

    function ownerMint(uint nums, address recipient)  public onlyOwner {
        require(totalSupply().add(nums) <= MAX_CAPS, "not enough capsuletoy left.");
