...
// File: contracts/BulkMamStake.sol
...
contract BulkMamStake is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint16 public mamStaked;
    uint16 public mutantStaked;
    ...
    function mamStake(uint16 quantity, uint16[] memory tokenIds) external {
    ...
    function mutantStake(uint16 quantity, uint16[] memory tokenIds) external {
    ...
    function unstakeMam(address _owner) external nonReentrant {
        ...
        for(uint8 i; i < mamstaker[_owner].ids.length; i++) {
            mam.transferFrom(address(this), msg.sender, mamstaker[_owner].ids[i]);
        }
    ...
    function unstakeMutant(address _owner) external {
        ...
        for(uint8 i; i < mutantstaker[_owner].ids.length; i++) {
            mutant.transferFrom(address(this), msg.sender, mutantstaker[_owner].ids[i]);
        }
  ...
  function enforceLockingPeriod(bool _state) external onlyOwner {
    ...
    function totalStaked() external view returns (uint16){
