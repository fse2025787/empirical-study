...
contract PelxpMarketplace is OwnableUpgradeable, SignerImplementation {
       ...
       function calculateFees(uint paymentAmt, address nftAddress, uint tokenId, bool isroyalty) internal  view returns(Fee memory){
                ...
                if(isroyalty){
                (tokenCreator, royaltyFee) = INft(nftAddress).royaltyInfo(tokenId, paymentAmt);
                assetFee = paymentAmt - royaltyFee - platformFee;}
                else{
                assetFee=paymentAmt-platformFee;}
        ...
        function transferAsset(Signer memory signer, Fee memory fee,bool is721) internal virtual {
                if (signer.inEth){
                        ...
                        if(!is721)
                        IERC1155Upgradeable(signer._nftAddress).safeTransferFrom(signer._user, signer.buyer, signer.tokenId, 1, "");
                        else
                        IERC721Upgradeable(signer._nftAddress).safeTransferFrom(signer._user,signer.buyer,signer.tokenId);
                } else {
