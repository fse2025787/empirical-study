...
// File: src/contracts/RORAGold.sol
...
contract RORAGold is ERC20, Pausable, AccessControl {
    ...
    function mint(address to, uint256 amount) external whenNotPaused onlyRole(MINT_ADMIN_ROLE) {
        require(totalSupply() + amount <= maxTotalSupply, "CANNOT MINT MORE THAN MAX TOTAL SUPPLY");
        _mint(to, amount);
    }

    // The abilty to burn from any address required because of regulatory requirements.
