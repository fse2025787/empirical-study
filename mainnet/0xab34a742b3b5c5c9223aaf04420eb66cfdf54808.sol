...
contract HTChamber is Ownable, ReentrancyGuard {
   ...
   // uint256 rewardDuration = 36 hours (2160mins);
    ...
    constructor(){
        rewardToken = IERC20(0xa359734C45B5D87109e9cE0906Be0F64a13ABF8D);
        stakedToken = IERC20(0x638A3D66e4a6A6dB13FaE6050b36f7067CcAAcf9);
        stakedLP = IERC20(0x94EAa998953d47623BDbf068c76C0c764b02aaCB);
    }
    ...
    function withdraw(IERC20 _target) public nonReentrant{
         ...
         if (_target == stakedToken && addressToUserPosition[msg.sender].amountToken>0){
             uint256 current = addressToUserPosition[msg.sender].amountTokenReceived;
        ...
        if (_target == stakedLP && addressToUserPosition[msg.sender].amountLP>0){
             uint256 current = addressToUserPosition[msg.sender].amountLPReceived;
    ...
    function nextRewardTime(address _user) public view returns (uint256){
         uint256 elapsedTime = block.timestamp - addressToUserPosition[_user].lastRewardTimestamp;
         uint256 next = addressToUserPosition[_user].lastRewardTimestamp + rewardDuration;
         if (elapsedTime >= rewardDuration){
             uint256 durationUnits = elapsedTime.div(rewardDuration).add(1);
             next = addressToUserPosition[_user].lastRewardTimestamp.add(durationUnits.mul(rewardDuration));
         }
         return next;
    }
    ...
    function claimablePacks(address _user) public view returns (uint256){
        ...
        if (elapsedTime >= rewardDuration && farming==true){
            ...
            if(addressToUserPosition[_user].amountToken>0) {multipleOfTokenQuantums = addressToUserPosition[_user].amountToken.div(quantumStakedTokens);}
            uint256 multipleOfLPQuantums = 0;
            if(addressToUserPosition[_user].amountLP>0) {multipleOfLPQuantums = addressToUserPosition[_user].amountLP.div(quantumLPTokens);}
            claimable = durationUnits.mul(rewardForTokenPerDuration.mul(multipleOfTokenQuantums) + rewardForLPPerDuration.mul(multipleOfLPQuantums));
        }
        return claimable;
    }
