...
contract GalacticBunny is Context, IERC20, Ownable {
    ...
    string private _name = "GALACTIC BUNNY";
    string private _symbol = "GBTK";
    ...
    address public devWallet = 0x8D3c47ba0DA7D0dAEeB92fD40b3BF5Eb5CA1DA42;
    address public marketingWallet = 0x85BBc0cd8438E54E38Aa310d395303312f089D0F;

    uint256 public _marketingFee = 8;
    uint256 public _botIncreaseFee = 3;
    uint256 private _previousMarketingFee = _marketingFee;
    uint256 private _previousDevFee = _devFee;
    uint256 public totalFeesTax = _taxFee.add(_liquidityFee).add(_devFee).add(_marketingFee);
    IUniswapV2Router02 public  uniswapV2Router;
    address public  uniswapV2Pair;
    bool public antibotSystemEnable = true;
    uint public maxTransferAmountRate = 500;
    ...
    function _transfer(
        ...
        if (
            ...
            //add liquidity
            uint256 totalTaxForSwap = _liquidityFee.add(_marketingFee).add(_devFee);
         
            if (_marketingFee > 0) {
                uint256 marketingTokens = contractTokenBalance.mul(_marketingFee).div(totalTaxForSwap);
                swapAndSendMarketing(marketingTokens);
            }
            if(_devFee > 0) {
                uint256 tokensForDev = contractTokenBalance.mul(_devFee).div(totalTaxForSwap);
                swapAndSendDev(tokensForDev);
            }
            swapAndLiquify(contractTokenBalance.mul(_liquidityFee).div(totalTaxForSwap));
    ...
    function swapAndSendMarketing(uint256 tokens) internal {
      ...
      function swapAndSendDev(uint256 tokens) internal {
        uint256 initialBalance = address(this).balance;
        // swap tokens for ETH
        swapTokensForEth(tokens); 
        uint256 newBalance = address(this).balance.sub(initialBalance);
        (bool success,) =  address(devWallet).call{value: newBalance}("");
        if (success) {
    ...
    function setBotFees() private {
        ...
        _previousDevFee = _devFee;
        _previousMarketingFee = _marketingFee;
        _taxFee = _previousTaxFee * _botIncreaseFee;
        _marketingFee = _previousMarketingFee * _botIncreaseFee;
        _liquidityFee = _previousLiquidityFee * _botIncreaseFee;
        _devFee = _previousDevFee * _botIncreaseFee;
    ...
    function setDevFee(uint256 devFee) external onlyOwner() {
        _devFee = devFee;
        totalFeesTax = _taxFee.add(_devFee).add(_marketingFee).add(_liquidityFee);
        require(totalFeesTax <= 20, "max 20%");
    }
    ...
    function setMarketingFee(uint256 marketingF) external onlyOwner {
