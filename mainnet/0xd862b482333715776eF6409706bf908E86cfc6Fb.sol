...
contract RichDadVSPoorDad is ERC20, Ownable {
    ...
    modifier isStakingEnabled() {
        require(stakingEnabled, "Pool is not enabled.");
        _;
    }

    constructor() ERC20("RichDad PoorDad", "RichDad") payable {
        ...
        apr.push(62); // pool apr is 62%, it's fixed value
    ...
    */
    function updateFees(uint256 marketingFee, uint256 liquidityFee, uint256 protocolFee) external onlyOwner {
        ...
        require(totalFees <= 12, "Must keep fees at 10% or less");
    }

    function addStakingTokenDistribution(uint256 _apr) external onlyOwner {
        require (_apr != 0, "can not update apr to zero!");
        apr.push(_apr);
    }

    function decreaseTokenDistribution() external onlyOwner {
        apr.pop();
    }
    /**
    ...
    */
    function amountOfPools() public view returns (uint256) {
    ...
    */
    function setPoolState(bool onoff) external teamOROwner {
