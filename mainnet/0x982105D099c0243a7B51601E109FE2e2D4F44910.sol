...
contract HVOGsamling is ERC721, Ownable {
    ...
    uint public MINT_PRICE = 0.015 ether;
    ...
    constructor() ERC721("HVOGsamling", "HV"){}
    
    
    function contractURI() public pure returns (string memory) {
        return "https://zerocodenft.azurewebsites.net/api/marketplacecollections/ee0c8b2f-9f5c-45aa-3a5c-08da798fa290";
    }
    ...
    function calcTotal(uint count) public view returns(uint) {
        require(saleStatus != SaleStatus.PAUSED, "HVOGsamling: Sales are off");
    ...
    function mint(uint count) external payable {
        require(saleStatus != SaleStatus.PAUSED, "HVOGsamling: Sales are off");
        require(_tokenIds.current() + count <= COLLECTION_SIZE, "HVOGsamling: Number of requested tokens will exceed collection size");
        require(count <= TOKENS_PER_TRAN_LIMIT, "HVOGsamling: Number of requested tokens exceeds allowance (10)");
        require(_mintedCount[msg.sender] + count <= TOKENS_PER_PERSON_PUB_LIMIT, "HVOGsamling: Number of requested tokens exceeds allowance (100)");
        require(msg.value >= calcTotal(count), "HVOGsamling: Ether value sent is not sufficient");
