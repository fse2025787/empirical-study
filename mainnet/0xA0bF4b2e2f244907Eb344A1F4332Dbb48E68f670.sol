...
contract stakingContract is OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function calcHeadReward(uint256 tokenId) public view blockExternalContracts returns (uint256 owed) {
    uint256 headRate = DailyHeadEmitRate + _calcBoost(_msgSender()); 
  ...
  function calculateReward(uint16[] calldata tokenIds) external view blockExternalContracts returns (uint256 owed) {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isHead(tokenIds[i]))
        owed += calcHeadReward(tokenIds[i]);
      else
