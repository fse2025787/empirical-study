...
contract MetaGreetingNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    function safePublicMint(uint256 tokenQuantity) public payable {
        ...
        require(tokenQuantity <= maxMint, "exceed max quantity");
        require(
            tokenQuantity * mintPrice <= msg.value,
            "Not enough ether to mint"
        );

        _safePublicMint(tokenQuantity);
    }

    //send card as a gift.public function.
    function safeGift(address to, uint256 tokenId, string memory mess) public {
        require(isGiftActive, "Gift function not active!");
        require(
            _msgSender() ==ERC721.ownerOf(tokenId),
            "Only token owner can gift card."
        );
        _safeGift(_msgSender(), to, tokenId, mess);
    }
    ...
    function setMessagePublic(uint256 tokenId, bool _state) public {
        ...
        require(
            _msgSender() ==ERC721.ownerOf(tokenId),
            "Only token owner can change message public flag."
        );
        _messagesPublicFlag[tokenId] = _state;
    }

    //set reverse data
    function setReverseString(uint256 tokenId, string memory _data) public {
        require(isReverseActive, "Greeting tree not active!");
        require(bytes(_data).length <= 64, "Reverse data must less than 32 bytes.");
        require(
            _msgSender() ==ERC721.ownerOf(tokenId),
    ...
    function _safePublicMint(uint256 tokenQuantity) internal {
        for (uint256 i = 0; i < tokenQuantity; i++) {
            uint256 tokenId = _tokenIdCounter.current()+1;   //token id start from 1
            _tokenIdCounter.increment();
            if (totalSupply() < MAX_SUPPLY) {
                _safeMint(msg.sender, tokenId);
                _setBlessMessage("", tokenId);
                _messagesPublicFlag[tokenId] = false;
                _reverseString[tokenId] = "";    //reverse
            }
        }
    }
    ...
    function _safeGift(address from, address to, uint256 tokenId, string memory mess) internal {
        ...
        require(bytes(mess).length <= 64, "Send message must less than 32 bytes.");
        _transfer(from, to, tokenId);
        ...
        returns (string memory)
    {
        ...
        require(
            _msgSender() == ERC721.ownerOf(tokenId) || _messagesPublicFlag[tokenId],
            "Only token owner can read messages."
        );
        ...
        returns (bool)
    {
        require(isGiftActive, "Gift and message function not active!");
        require(_exists(tokenId), "Tokenid not exist.");
        return _messagesPublicFlag[tokenId];
    }
    ...
    function getReverseString(uint256 tokenId)
        ...
        returns (string memory)
    {
        require(isReverseActive, "reverse string not active!");
        require(_exists(tokenId), "Tokenid not exist.");
        return _reverseString[tokenId];
    }
    ...
    function flipSaleActive() public onlyOwner {
        require(manageFlag, "manageFlag must be active");
    ...
    function closeManageFunction(bool _flag) public onlyOwner {   //set true not change,set false close
        require(manageFlag, "manageFlag must be active");
        manageFlag = _flag;
    }

    function closeSetBaseURIFlag(bool _flag) public onlyOwner {   //close setBaseURI function.set true not change,set false close
        require(setBaseURIFlag, "setbaseURI must be active");
        setBaseURIFlag = _flag;
    }

    function setBaseURI(string memory _newBaseURI) public onlyOwner {
        require(setBaseURIFlag, "setbaseURI flag must be active");
        baseURI = _newBaseURI;
    }
