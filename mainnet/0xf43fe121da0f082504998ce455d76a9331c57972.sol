...
interface LootContract is IERC721 { }
...
contract LootApartment is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  uint private currentPrice = 10000000000000000;
  ...
  string[] private planNames = [
      "Pacific", "Mediterranean", "Caribbean", "Sun", 
      "Mercury", "Venus", "Moon", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune", "Pluto", "Bacelona",
      "Milan", "Paris", "London", "Berlin", "Moscow", "Beijing", "Tokyo", "New York", "Bordeaux", "Venice",
      "Vancouver", "Bangkok", "Seoul", "Algiers", "Cape Town", "Djibouti", "Tripoli", "Nairobi", "Kinshasa",
      "Buenos Aires",  "San Paolo", "Rio", "Wellington", "Seattle", "Miami", "Austin", "Las Vegas", "Madrid",
      "Oslo", "Stockholm", "Riga", "Rome", "Jakarta", "Manila", "Osaka", "Melbourne", "Prague",
      "Warsaw", "Cairo", "Mumbai", "Delhi", "Shanghai", "TOI 700 d", "Proxima Centauri b", "Kepler-1649c",
      "Wolf 1061c", "Tau Ceti f", "Kapteyn b", "K2-72e", "Luyten b"
  ];
  
  constructor(address _lootContractAddress) ERC721("Loot Apartments", "LAP") Ownable() {
  ...
  function claim(uint256 _tokenId) public nonReentrant {
    ...
    require(_msgSender() == ownerOfLoot(_tokenId), "You need to be LOOT owner!");
    require(claimed[_msgSender()] == false, "You already claimed one apartment!");
    _safeMint(_msgSender(), _tokenId);
    claimed[_msgSender()] = true;
  }

  function buy(uint256 _tokenId) external payable costs(currentPrice) nonReentrant() {
    require(_tokenId > 0 && _tokenId < 8001, "Token ID invalid");
  ...
  function getHOABalance() public view returns(uint) {
  ...
  function getNumBedrooms(uint256 _tokenId) public pure returns (uint256) {
    ...
    uint256 rand = random(string(abi.encodePacked("bed", toString(_tokenId))));
    return uint8((rand % 8) + 1);
  }

  function getPlanName(uint256 _tokenId) public view returns (string memory) {
    require(_tokenId > 0 && _tokenId < 8009, "Token ID invalid");
    uint256 rand = random(string(abi.encodePacked("plan", toString(_tokenId))));
  ...
  function getBedrooms(uint256 _tokenId) public pure returns (string memory) {
    ...
    string memory output = string(abi.encodePacked(toString(numBedrooms), " Bedroom"));
    
    return plural(output, numBedrooms);
  }

  function plural(string memory _output, uint256 _quantity) internal pure returns (string memory) {
    if (_quantity > 1) {
  ...
  function getBathrooms(uint256 _tokenId) public pure returns (string memory) {
    require(_tokenId > 0 && _tokenId < 8009, "Token ID invalid");
    uint256 numBathrooms = getNumBathrooms(_tokenId);
    string memory output = string(abi.encodePacked(toString(numBathrooms), " Bath"));
  ...
  function getNumBathrooms(uint256 _tokenId) public pure returns (uint256) {
    require(_tokenId > 0 && _tokenId < 8009, "Token ID invalid");
    uint256 rand = random(string(abi.encodePacked("bath", toString(_tokenId))));
  ...
  function hasBalcony(uint256 _tokenId) public pure returns (bool) {
    require(_tokenId > 0 && _tokenId < 8009, "Token ID invalid");
    uint256 rand = random(string(abi.encodePacked("balcony", toString(_tokenId))));
  ...
  function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
        ...
        if (greatness > 14) {
            output = string(abi.encodePacked('The ', titles[rand % titles.length], ' ', output));
        }
        return output;
  }
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Apartment #', toString(tokenId), '", "description": "Loot Apartments are randomized apartments for storing LOOT. In addition to the apartment, you also get a sidekick (all hail Devastating Parrot)! Images and other functionality are intentionally omitted for others to interpret. Feel free to use LootApartments in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
