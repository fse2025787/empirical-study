...
contract SakeOfPeace is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "Sake of Peace";
    ...
    uint256 private _bought = 100000000 * 10**_decimals;
    uint256 private _held = 100000000 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 175000 * 10**_decimals; 
    ...
    constructor () {
        ...
        teamWallet = payable(address(0xdEA2F019e620e157C33cAD6599Af7215f5c9F9A8));
        marketingWallet = payable(address(0xdEA2F019e620e157C33cAD6599Af7215f5c9F9A8));
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!_isTxExempt[sender] && !_isTxExempt[recipient], "Transfer amount must from another address");
        if(inSwapAndLiquify)
        { 
        ...
        else
        {
            if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {
                require(amount <= _bought, "Transfer amount exceeds the maxTxAmount.");
            }            
            ...
            if(checkWalletLimit && !isWalletLimitExempt[recipient])
                require(balanceOf(recipient).add(finalAmount) <= _held);
