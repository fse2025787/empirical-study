...
contract MinmiToken is Context, IERC20, Ownable {
    ...
    string private _name = "Minmi Token";
    string private _symbol = "MINMI";
    uint8 private _decimals = 6;
    
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public marketingWallet = payable(0xF5DDa28bFEaB3356d9c9E9A5996d1097c1c19F54);
    ...
    uint256 public _saleLiquidityFee = 5;
    uint256 public _saleMarketingFee = 8;
    uint256 public _saleBurnFee = 1;
    ...
    uint256 public minimumTokensBeforeSwap = 30000000 * 10**6;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        if(recipient==uniswapV2Pair){
            setAllFees(_saleTaxFee, _saleLiquidityFee, _saleMarketingFee, _saleBurnFee);
        }
    ...
    function setAllFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) private {
        require(taxFee.add(_liquidityFee).add(marketingFee).add(burnFee) <= 16, "tax too high");
        _taxFee = taxFee;
    ...
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) 
    external onlyOwner() {
        require(taxFee.add(_liquidityFee).add(marketingFee).add(burnFee) <= 16, "tax too high");
        setAllFees(taxFee, liquidityFee, marketingFee, burnFee);
    ...
    function setSaleFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) 
    external onlyOwner() {
        require(taxFee.add(_liquidityFee).add(marketingFee).add(burnFee) <= 16, "tax too high");
        _saleTaxFee = taxFee;
