...
contract SploogeZone is ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(uint => uint) public amountOfMintPerTeer;
        ...
        notBots
    {
        ...
        require(amountOfMintPerTeer[0] <= baseSplge);
        require(
        ...
        notBots
    {
        ...
        require(amountOfMintPerTeer[1] <= JrSplge);
        require(
    ...
    {
        require(count <= config.maxSplge, "Exceeds max");
        require(amountOfMintPerTeer[2] <= LMBLSplge);
        require(
            mintedInWallet[msg.sender] + count <= config.maxSplge,
            "Exceeds max"
        );
    ...
    {
        require(count <= config.maxSplge, "Exceeds max");
        require(amountOfMintPerTeer[4] <= AYPNDSplge);
        require(
            mintedInWallet[msg.sender] + count <= config.maxSplge,
            "Exceeds max"
        );
