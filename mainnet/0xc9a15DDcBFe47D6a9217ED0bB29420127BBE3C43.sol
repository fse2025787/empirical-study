...
*/
contract kabosu is ERC721, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    uint256 public nftPrice = .075 ether;
    uint public maxNftPurchase = 10;
    uint public maxNftWallet = 10;
    uint256 public maxNfts;
    bool public saleIsActive = false;
    mapping (address => bool) private _isWhitelisted;
    mapping (address => bool) private _isTokenAdmin;

    constructor() ERC721("Kabosu Inu", "Kabosu Inu") {
        maxNfts = 777;
    }
    ...
    function addToWhitelist(address[] calldata addresses) external OnlyTokenAdmin {
        for (uint256 i = 0; i < addresses.length; i++) {
            _isWhitelisted[addresses[i]] = true;
        }
    }

    function removeFromWhitelist(address[] calldata addresses) external OnlyTokenAdmin {
        for (uint256 i = 0; i < addresses.length; i++) {
    ...
    function mint(uint numberOfTokens) isHuman nonReentrant external payable {
        require(saleIsActive || _isWhitelisted[_msgSender()], "Sale must be active or you must be whitelisted to mint");
        require(balanceOf(_msgSender()).add(numberOfTokens) <= maxNftWallet, "Can only mint 10 per wallet");
        require(numberOfTokens <= maxNftPurchase, "Can only mint 10 at a time");
        require(totalSupply().add(numberOfTokens) <= maxNfts, "Purchase would exceed max supply");
