...
/*
* Intermediary contract to easily gather an Array of all token ids owned
* by an account with a contract that has no implementation.
*/
...
contract EvaIds {
    ...
    {
        uint256 tokenCount = eva.balanceOf(_owner);
        if (tokenCount == 0) {
            return new uint256[](0);
        } else {
            uint256[] memory result = new uint256[](tokenCount);
            for (uint256 index; index < tokenCount; index++) {
                result[index] = eva.tokenOfOwnerByIndex(_owner, index);
            }
            return result;
        }
    }
