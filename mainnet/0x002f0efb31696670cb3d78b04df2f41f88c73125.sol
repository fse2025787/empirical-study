...
// File: PunkRockApeClub.sol


// pragma solidity >=0.4.21 <8.10.0;
// pragma solidity 0.8.9;

pragma solidity >=0.4.21 <8.9.0;




contract PunkRockApeClub is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant OG_SALE_MAX_TOKENS = 35;
    uint256 public constant WL_SALE_MAX_TOKENS = 3000;
    ...
    uint256 public wlSaleSupply = 0;
    string private baseTokenURI;
    string private notRevealedURI;

    mapping(address => uint8) private whiteList;
    mapping(address => uint8) private ogList;

    constructor(
        string memory _initBaseURI,
        string memory _initNotRevealedURI
    ) ERC721("Punk Rock Ape Club", "PRAC") {
        _setBaseURI(_initBaseURI);
        setNotRevealedURI(_initNotRevealedURI);
        mintReservedTokens(25);
    }
    ...
    function setOgList(address[] calldata addresses, uint8 numAllowedToMint)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            ogList[addresses[i]] = numAllowedToMint;
        }
    }

    function mintToken(uint8 amount) external payable {
        require(
    ...
    function mintTokenOgSale(uint8 amount) external payable {
        require(
            msg.sender == tx.origin,
            "No transaction from smart contracts!"
        );
        require(ogSaleIsActive, "OG-sale must be active to mint");
        require(amount <= ogList[msg.sender], "Exceeded max available to purchase");
        require(
            ogSaleSupply + amount <= OG_SALE_MAX_TOKENS,
            "Purchase would exceed max supply for OG sale"
        );

        ogList[msg.sender] -= amount;
        for (uint256 i = 0; i < amount; i++) {
            _safeMint(msg.sender, supply);
            supply++;
            ogSaleSupply++;
    ...
    function mintTokenWlSale(uint8 amount) external payable {
        require(
            msg.sender == tx.origin,
            "No transaction from smart contracts!"
        );
        require(wlSaleIsActive, "OG-sale must be active to mint");
        require(amount <= whiteList[msg.sender], "Exceeded max available to purchase");
        require(msg.value >= PRICE * amount, "Not enough ETH for transaction");
        require(
            wlSaleSupply + amount <= WL_SALE_MAX_TOKENS,
            "Purchase would exceed max supply for OG sale"
        );
    ...
    function mintReservedTokens(uint8 amount) public onlyOwner {
        require(
