...
Tokenomics: 1% on to reflections on Buy/Sell, The real tokenomics are 1% buy and 10% sell

Contract taxes are based off of rTotal instead of the delivered tTotal. 
Slippage is maintained at 1% while real tax taken on transfer from a non-excluded address(user -> uniswap LP, user -> user) is 10%(sell 100 tokens, 
taxed 10% with 1% slippage making it received funds of 90 tokens sold). 
The 10% is reserved to only be on sells, on buys you will not be taxed as it’s from an excluded address(uniswap LP -> user). 
The reserved tokens for marketing tax are sent to the contract to then be swap and liquified. 
The benefit of this method and transparency around the tax allows us to take advantage of mev bots volume and taxation. 
Just like users paying taxes, they will and since the slippage is low, 
they will not realize they are being taxed at 8% on sells until it has already occurred and profits are missed. 
This highly beneficial for the protocol to grow and use the volume to garner new heights.
*/
