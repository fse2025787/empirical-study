...
contract Dogenetics is Context, IERC20, Ownable {
    ...
    address public _marketingWallet =
        0xd7BbAaE07dd91D51D5c226e560950594e3a12BB7;

    address public _buyBackWallet = 0x50dA231A838Fa564d6D5b406eb4F235be413Ac9A;
    ...
    string private _name = "Dogenetics";
    string private _symbol = "$DGSN";
    ...
    constructor() {
        ...
        address deployer = 0xa6e463Bc686716FF6e5112D68Fb459264421a962;
    ...
    function updateBuyFees(
        ...
        _buyAutoBurn = autoBurn;
        _buyBuyBackFee = buyBackFee;

        require(
            (rewardFee + liquidityFee + marketingFee + autoBurn + buyBackFee) <=
                25,
            "Fees can to grater than 25%"
        );
    }
    ...
    function updateSwapPercentages(
        ...
        totalSwap = liquidity.add(marketing).add(buyBack);
    }
    ...
    function _transfer(
        ...
        */
        ...
        if (from != _uniswapV2Pair && to != _uniswapV2Pair) takeFee = false;
    ...
    function swapAndSendBnb(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // tokens to auto burn
        uint256 burnTokens = contractTokenBalance.mul(autoBurnSwap).div(
            totalSwap
        );

        _tokenTransfer(address(this), DEAD, burnTokens, false);
        uint256 tokensToSwap = contractTokenBalance
            .sub(tokensToAddLiquidity)
            .sub(burnTokens);
        // swap tokens in to bnb
        swapTokensForBnb(tokensToSwap);

        uint256 swappedBnbFee = totalSwap.sub(liquiditySwap).sub(autoBurnSwap);
