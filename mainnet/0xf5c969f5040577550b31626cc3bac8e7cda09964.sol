...
contract SquareZ is ERC721 {
    ...
    string public name = 'SquareZ';
    string public symbol = 'SQRZ';
    ...
    event Mint(address user, uint id, uint8 r, uint8 g, uint8 b);
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        ...
        infoUrl = strConcat('https://squarez.fun/v/', uint2str(_tokenId));
        return infoUrl;
    }
    ...
    function random() internal returns (uint8) {
        seed += uint256(keccak256(abi.encodePacked(block.timestamp + block.difficulty))) + uint256(keccak256(abi.encodePacked(msg.sender)));
        return uint8(uint256(keccak256(abi.encodePacked(block.timestamp + block.difficulty + seed))) % 256);
    }
    ...
    function mint() external {
        ...
        uint8 g = random();
        uint8 b = random();
        squares.push(Square(r, g, b));
        emit Mint(msg.sender, idCounter, r, g, b);
