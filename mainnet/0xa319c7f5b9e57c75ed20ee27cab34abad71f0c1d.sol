...
contract Evolve_Meijin is ERC721Enumerable, Ownable {
  ...
  uint256 public publicCost = 0.04369 ether;
  ...
  function publicMint(uint256 _mintAmount) public payable {
    ...
    if(supply + _mintAmount <= 369 || supply + _mintAmount >= 6369){
  ...
  function MeditationPeriod(uint256 tokenId)
    ...
    uint256 private MeditationTransfer = 1;

    function safeTransferWhileMeditation(
        address from,
        address to,
        uint256 tokenId
    ) external {
        require(ownerOf(tokenId) == _msgSender(), "Evolve: Meijin - Only owner");
        MeditationTransfer = 2;
        safeTransferFrom(from, to, tokenId);
        MeditationTransfer = 1;
    }
 event Meditated(uint256 indexed tokenId);
 event UnMeditated(uint256 indexed tokenId);
 event Fired(uint256 indexed tokenId);

 bool public MeditationOpen = false;
