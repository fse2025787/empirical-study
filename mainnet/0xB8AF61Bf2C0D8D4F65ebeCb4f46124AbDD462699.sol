...
contract Foes is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getFoes(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "FOE", foes);
    }
    
    function getStatusFoe(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "STATUS", statusFoe);
    }
    ...
    function getSpecialPower(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "SPECIAL", specialPower);
    }
    
    function getArtifact(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "ARTIFACT", artifact);
    }
    ...
    function getSecret(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "SECRET", secret);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[1] = getFoes(tokenId);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = getStatusFoe(tokenId);
        ...
        parts[7] = getTerrain(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getSpecialPower(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getArtifact(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Foe #',
                        toString(tokenId),
                        '", "description": "Every adventurer needs a foe. Randomly generated and stored on chain. Building the @lootproject metaverse.", "image": "data:image/svg+xml;base64,',
    ...
    function mint(uint256 tokenId) public payable nonReentrant {
        require(tokenId > 8000 && tokenId <= 20000, "Token ID invalid");
    ...
    function multiMint(uint256[] memory tokenIds) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] > 8000 && tokenIds[i] < 20000, "Token ID invalid");
    ...
    function withdraw() public onlyOwner {
        payable(0xbE4b30C43eeAB3B8e5a693464ed98e6Bdb5aD87F).transfer(address(this).balance);
    }
    ...
    constructor() ERC721("Foes", "FOE") Ownable() {}
}
