...
contract Metasnacks is ReentrancyGuard, Pausable, ERC721Enumerable, Ownable, IERC721Receiver {
    ...
    uint256 private PRESALE_MINT_LIMIT = MAX_SUPPLY;
    uint256 private PUBLIC_MINT_LIMIT = MAX_SUPPLY;
    uint256 private POOL_PERCENT = 0;
    
    bytes32 public _whitelistRoot;

    mapping(address => uint256) public _mappingPresaleMintCount;
    mapping(address => uint256) public _mappingPublicMintCount;

    bool public _presaleEnabled = false;
    bool public _publicEnabled = false;
    ...
    /* ------------------------ NFT Staking ------------------------- */
    bool public _enableRestake = false;
    bool public _enableHarvest = false;
    uint256 private _enableHarvestDateTime;
    ...
    mapping(address => uint256) private userClaimDateTime;
    ...
    constructor() ERC721("Metasnacks NFT", "METASNACK") {}
    ...
    function setPresaleEnabled(bool bEnable) external onlyOwner {
        _presaleEnabled = bEnable;
    }
    function setPublicEnabled(bool bEnable) external onlyOwner {
        _publicEnabled = bEnable;
    }
    ...
    function setEnableRestake(bool bRestake) external onlyOwner {
        _enableRestake = bRestake;
    }

    function setEnableHarvest(bool bHarvest) external onlyOwner {
        _enableHarvest = bHarvest;
        if (bHarvest == true) _enableHarvestDateTime = block.timestamp;
    }
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            ...
            // UserInfo storage user = userInfo[airdropAddress[k]];
            // user.rewards = earned(airdropAddress[k]);
            // user.lastUpdated = block.timestamp;

            addStakerList(airdropAddress[k]);
            ...
            for (uint256 i = 0; i < numberOfTokens; i++) {
                ...
                if (_publicCounter.current() < MAX_SUPPLY) {
                    ...
                    if (!_exists(tokenId)) {
                        ...
                        userBlanaces[airdropAddress[k]].add(tokenId);
                    }
                }
            }
        }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        if (_msgSender() != owner()) {
            ...
            require(_publicEnabled == true,"Public Mint is not activated.");

            require(PUBLIC_PRICE * numberOfTokens <= msg.value,"ETH amount is not sufficient");

            _mappingPublicMintCount[_msgSender()] = _mappingPublicMintCount[_msgSender()] + numberOfTokens;
            
            require(_mappingPublicMintCount[_msgSender()] <= PUBLIC_MINT_LIMIT,"Overflow for PUBLIC_MINT_LIMIT");

            _PaymentAddress.transfer(msg.value * (100 - POOL_PERCENT) / 100);
        }
    ...
    function presale(uint256 numberOfTokens, uint256 index, uint256 amount, bytes32[] calldata proof) external payable {
        ...
        if (_msgSender() != owner()) {
            ...
            require(isWhiteListed(toLeaf(_msgSender(), index, amount), proof), "Invalid proof");

            require(_presaleEnabled == true,"Presale Round is not activated.");

            require(PRESALE_PRICE * numberOfTokens <= msg.value,"ETH amount is not sufficient");

            _mappingPresaleMintCount[_msgSender()] = _mappingPresaleMintCount[_msgSender()] + numberOfTokens;
            
            require(_mappingPresaleMintCount[_msgSender()] <= PRESALE_MINT_LIMIT,"Overflow for PRESALE_MINT_LIMIT");
    ...
    //     uint256 amount = blockTime.sub(user.lastUpdated).mul(userStakedNFTCount(account)).mul(100 ether).div(1 days);
    ...
    function totalStakedCountForUnclaimed() public view returns (uint256) {
        uint256 totalCount = 0;
        for (uint256 i = 0; i < stakerList.length; i++) {
            if (isHarvested(stakerList[i]) ==  false) totalCount += userStakedNFTCount(stakerList[i]);
        }
        return totalCount;
    }

    function totalStakedMembersForUnclaimed() public view returns (uint256) {
        uint256 totalMembers = 0;
        for (uint256 i = 0; i < stakerList.length; i++) {
            if (userStakedNFTCount(stakerList[i]) > 0 && isHarvested(stakerList[i]) ==  false) totalMembers += 1;
        }
        return totalMembers;
    }

    function stake( uint256[] memory  tokenIdList) public nonReentrant whenNotPaused {
        require(_enableRestake == true, "NFT Staking is not activated");

        require(isApprovedForAll(_msgSender(),address(this)),"Not approve nft to staker address");
    ...
    function isHarvested(address _owner) public view returns (bool) {
        if (_enableHarvest == false || _enableHarvestDateTime == 0) return false;

        if (_enableHarvestDateTime < userClaimDateTime[_owner]) return true;
    ...
    function harvest() public nonReentrant {
        ...
        require(userStakedNFTCount(_msgSender()) > 0, "Nothing Staked NFTs");
        require(isHarvested(_msgSender()) == false, "You already harvested");

        // UserInfo storage user = userInfo[_msgSender()];
        // user.rewards = earned(_msgSender());
        // user.lastUpdated = block.timestamp;

        uint256 harvestAmount = address(this).balance.mul(userStakedNFTCount(_msgSender())).div(totalStakedCountForUnclaimed());
        payable(_msgSender()).transfer(harvestAmount);
        userClaimDateTime[_msgSender()] = block.timestamp;
