...
contract Penguins is ERC721A, Ownable {
    uint256 public MaxPerTxn = 30;
    uint256 public MaxFreePerWallet = 2;
    bool public mintEnabled = false;
    uint256 public maxSupply = 7331;
    uint256 public price = 0.002 ether;
    uint256 public totalFree = 2000;
    string public baseURI =
        "ipfs://QmWM5QrHByxWwE5DWEeSK19aU3qkBZJsptXrzu3zYxudfG/";
    mapping(address => bool) public wlAddress;

    constructor(address[] memory _address) ERC721A("1337Penguins", "1337PEN") {
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(quantity <= MaxPerTxn, "too many");
        uint256 cost = price;
        if (
            totalSupply() + quantity <= totalFree &&
            numberMinted(msg.sender) + quantity <= MaxFreePerWallet
        ) {
            cost = 0;
        }

        require(msg.value >= quantity * cost, "more money!");
    ...
    function setTotalFree(uint256 amount) external onlyOwner {
        totalFree = amount;
    }
