...
// File: contracts/adapters/WBTCCompoundAdapter.sol
...
contract WBTC_Compound_Adapter is ISaffronAdapter {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  address public governance;                    // Governance address
  ISaffronPool public saffron_pool;             // SaffronPool that owns this adapter
  IERC20 public WBTC;                           // Base asset (WBTC)
  ICErc20 public cWBTC;                         // cWBTC redeemable for WBTC
  string public constant platform = "Compound"; // Platform name
  string public constant name = "WBTC/Compound"; // Adapter name
  ...
  constructor(address _saffron_pool, address CErc20_contract_address, address base_asset_address) {
    ...
    cWBTC        = ICErc20(CErc20_contract_address);
    WBTC         = IERC20(base_asset_address);
  ...
  function deploy_capital(uint256 amount) external override {
    ...
    WBTC.safeApprove(address(cWBTC), amount); // Approve the transfer
    uint mint_result = cWBTC.mint(amount);   // Mint the cTokens and assert there is no error
  ...
  function return_capital(uint256 base_asset_amount, address to) external override {
    require(msg.sender == address(saffron_pool), "must be pool");
    uint256 redeem_result = cWBTC.redeemUnderlying(base_asset_amount);
    ...
    WBTC.safeTransfer(to, base_asset_amount);
    emit ReturnCapital(cWBTC.balanceOf(address(this)), base_asset_amount);
  }
  ...
  function sweep(address _to) public {
    ...
    IERC20 tkn = IERC20(address(cWBTC));
    uint256 cBal = tkn.balanceOf(address(this));
    tkn.transfer(_to, cBal);

    uint256 dBal = WBTC.balanceOf(address(this));
    WBTC.transfer(_to, dBal);
  ...
  function erc_sweep(address _token, address _to) public {
    require(msg.sender == governance, "must be governance");
    require(_token != address(WBTC) && _token != address(cWBTC), "cannot sweep adapter assets");
  ...
  function get_exchange_rate() public returns(uint256) {
    uint256 rate = cWBTC.exchangeRateCurrent();
    emit GetExchangeRate(rate, rate.div(10**18));
    return rate;
  }

  // WARNING: holdings expressed in 18 decimals (cWBTC only has 8 decimals)
  event GetHoldings(uint256 holdings);
  function get_holdings() external override returns(uint256) {
    uint256 holdings = cWBTC.balanceOf(address(this)).mul(cWBTC.exchangeRateCurrent()).div(10**18);
  ...
  function get_ctoken_balance() public view returns(uint256) {
    return cWBTC.balanceOf(address(this));
  }
