...
contract OlympusV1BondDepository is OlympusAccessControlled {
    ...
    IERC20 public immutable OHM; // token given as payment for bond
    address public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints OHM when receives principle
    address public immutable DAO; // receives profit share from bond
    IStaking public immutable staking; // to stake payout
    ...
    function deposit( 
        ...
        uint value = treasury.tokenValue(principle, _amount);
        ...
        treasury.deposit( _amount, principle, profit );
        
        if ( fee != 0 ) { // fee is transferred to dao 
            OHM.safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            ...
            staking.stake(_recipient, _amount, false, true);
        }
        return _amount;
    }
