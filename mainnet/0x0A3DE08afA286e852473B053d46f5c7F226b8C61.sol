...
contract MintingRouter is EIP712Whitelisting, ReentrancyGuard, Ownable {
  ...
  mapping(uint256 => mapping(address => uint256)) public mintedAmountPerAddress;
  ...
  function allowedTokenCount(address minter) public view returns (uint256) {
    ...
    uint256 allowedWalletCount = saleRound.limitAmountPerWallet != UNLIMITED_AMOUNT
      ? (saleRound.limitAmountPerWallet > mintedAmountPerAddress[currentSaleIndex][minter] ? saleRound.limitAmountPerWallet - mintedAmountPerAddress[currentSaleIndex][minter] : 0)
  ...
  function _mint(
    ...
    if (saleRound.limitAmountPerWallet != UNLIMITED_AMOUNT) {
      uint256 mintedAmountSoFar = mintedAmountPerAddress[currentSaleIndex][recipient];
    ...
    mintedAmountPerAddress[currentSaleIndex][recipient] += quantity;
