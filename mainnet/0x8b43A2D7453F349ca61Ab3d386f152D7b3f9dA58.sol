...
contract XeroTax is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Xero Tax";
    string private constant _symbol = "XEROT";
    uint8 private constant _decimals = 9;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private constant totalTokens = 100 * 1e6 * 1e9; 
    uint256 public _maxWalletAmount = 4 * 1e6 * 1e9;
    
    uint256 private _previousLiqFee = liquididtyFee;
    uint256 private _previousProjectFee = projectTax;
    uint256 private liquididtyFee;
    ...
    address payable private marketingWallet = payable(0x22e8bbAc53b0F81e4fcBE6a4e7b708bC549c763C);
    ...
    function removeAllFee() private {
        if (projectTax == 0 && liquididtyFee == 0) return;
        _previousProjectFee = projectTax;
        _previousLiqFee = liquididtyFee;
        projectTax = 0;
        liquididtyFee = 0;
    }
    
    function restoreAllFee() private {
        liquididtyFee = _previousLiqFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 autoLPamount = liquididtyFee.mul(contractTokenBalance).div(projectTax.add(liquididtyFee));
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        ...
        fees.tLiquidity = amount.mul(liquididtyFee).div(100);
    ...
    function blacklistmany(address[] memory bots_) external onlyOwner {
