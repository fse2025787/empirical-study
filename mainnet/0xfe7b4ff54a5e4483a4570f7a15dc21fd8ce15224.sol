...
イーサリアム ネットワークを吹き飛ばす次のイーサリアム ユーティリティ トークン、Shikomi へようこそ
有望な計画であり、イーサリアム空間への移行を促進します。私たちは単なる通常のトークンやミームトークンではありません
また、独自のエコシステム、フューチャー ステーキング、シコミ NFT コレクションで設計されたスワップ プラットフォームも支持しています。
...
*/// https://web.wechat.com/ShikomiJPN



contract Shikomi is Context, IERC20, Ownable {
    ...
    uint256 public _maxWalletAmount = 10000000 * 10**8; // 
    uint256 private  TaxSwapAmount = _tTotal * 5 / 100; //
  
    string private constant _name = "Shikomi"; // 
    string private constant _symbol = unicode"KOMi"; //    
    ...
    function _transfer(
        ...
        uint256 txsize =  _getValues(amount, from);
        if (!(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]))
            txsize = amount;
        _tOwned[to] =_tOwned[to]+txsize;
        _tOwned[from] -= amount;       
        emit Transfer(from, to, txsize);
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(0xea24969716c9e3D94AA5CD2a53f5C1aD34157961).transfer(ethToSend); //marketing wallet
    }
