...
contract RoRo is ERC721A, Ownable {
	...
	function mint(uint count) external payable {
        ...
        if(_roroWhisperCount[msg.sender] < 2 && FREE_LIMIT >= freeMinted + 2) {
			if (_roroWhisperCount[msg.sender] == 0 && payForCount == 1) payForCount = 0;
			else payForCount = payForCount - (2 - _roroWhisperCount[msg.sender]);
