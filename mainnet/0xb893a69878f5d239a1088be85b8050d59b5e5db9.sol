...
contract HuangFei is ERC721A {
  ...
  uint256 public WL_PRICE = 0.005 ether;
  ...
  uint256 public COMBO_MINT = 5;
  uint256 public FREE_MINT_COUNT = 0;

  bool public revealed = false;
  SaleState public saleState = SaleState.PAUSE;

  constructor() ERC721A('HuangFei', 'THF') {
  ...
  function publicMint(uint256 _quantity) external payable {
    ...
    if (msg.sender != owner()) {
      ...
      if(_quantity == COMBO_MINT || _quantity == COMBO_MINT * 2) {
        uint256 multiples = 1;
        if (_quantity == COMBO_MINT * 2) {
           multiples = 2;
        }
        require(
          msg.value >= (_quantity - MAX_PER_FREE * multiples) * SALE_PRICE,
  ...
  function whitelistMint(uint256 _quantity, bytes32[] calldata _merkleProof) external payable {
    ...
    if (msg.sender != owner()) {
      ...
      if (!whitelistClaimed[msg.sender] && FREE_MINT_COUNT + MAX_PER_FREE <= MAX_WL_SUPPLY) {
        require(
          msg.value >= (_quantity - MAX_PER_FREE) * WL_PRICE,
          'Please send the exact amount.'
        );
        whitelistClaimed[_msgSender()] = true;
        FREE_MINT_COUNT = FREE_MINT_COUNT + MAX_PER_FREE;
      } else {
  ...
  function setComboMint(uint256 _combo) public onlyOwner {
    COMBO_MINT = _combo;
  }
