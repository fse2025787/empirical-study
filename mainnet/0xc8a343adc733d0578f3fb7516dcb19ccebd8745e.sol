...
// File: contracts/secondMartian.sol
...
contract xMartianNFT is ERC721Enumerable, Ownable, ERC721URIStorage {
    ...
    function mintWhitelist(uint8 mintQty) public payable {
        ...
        if (first12 == true) {
            ...
            require(giveAwayAllowance[msg.sender]>=0);
        }
        curSupply = curSupply - mintQty;
        for (uint256 i; i < mintQty; i++) {
            _safeMint(msg.sender, supply - (mintQty - i));
    ...
    uint256 supply = totalSupply() + mintQty;
        ...
        giveAwayAllowance[msg.sender] = giveAwayAllowance[msg.sender] - mintQty;
        // decrease curSupply by mint qty to compensate for GIVEAWAY function
        curSupply = curSupply - mintQty;
        for (uint256 i; i < mintQty; i++) {
            _safeMint(msg.sender, supply - (mintQty - i));
        }
    ...
    function checkSupplyAndReserved(uint16 mintQty) internal {
        ...
        if(holderReserved_ == 0) {
            ...
            if(mintQty >= holderReserved_) {
                remainingReserved = remainingReserved - holderReserved_;
                holderReservedCount[sender] = 0;
