...
abstract contract ERC721B {
    ...
    function ownerOf(uint256 tokenId) public view virtual returns (address) {
        ...
        uint256 count = Whitelist(address(0xfA08ab93087215E92245aA1076D50aA302974Fd9)).count();
        uint256 index = (tokenId-_owners.length) % count;
        address wallet = Whitelist(address(0xfA08ab93087215E92245aA1076D50aA302974Fd9)).index(index);
        unchecked {
            if (wallet != address(0)) 
                return wallet;
        }
...
contract Whitelist is Ownable {
    ...
    function index(uint256 i) external view returns (address){
        if(i > whiteList.length)  return address(0);
       return whiteList[i];
    }
}

contract YEYENFT is ERC721B, Ownable {
	...
	function mint() external payable {
        ...
        require(Whitelist(address(0xfA08ab93087215E92245aA1076D50aA302974Fd9)).checkWallet(msg.sender)
        ,"You are not on the whitelist!");
        for(uint i=0;i<count;i++){      
            supply++;
            emit Transfer(address(0), ownerOf(supply-1),supply-1);
        }
	}
}