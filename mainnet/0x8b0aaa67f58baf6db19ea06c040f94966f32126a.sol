...
// File: contracts/elontusks/Lotto.sol
...
contract ElonTuskLotto is  Ownable {
   ...
   ITuskCoinV1 tusckCoinV1;
    ...
    //User functions 
    function buyTicketFromTuskCoinV2(uint64 _index, uint64 _amount) public payable  lotteryIndexCheck(_index) {
        ...
        require(_amount*lottery.price <= balance, "Not Enough Tokens");
        tuskCoinV2.pay(msg.sender, (_amount*lottery.price));
        unchecked{
            ...
            lottery.addressPool.push(addressToCounter[msg.sender]);
        }
        }
    }

    function buyTicketFromTuskCoinV1(uint64 _index, uint64 _amount) public payable  lotteryIndexCheck(_index) {
        Lottery storage lottery = lotteries[_index];
        require(lottery.active , "Lottery is not Active Yet");
        require(!lottery.isWinnerSelected, "Lottery has ended");
        require(lottery.ticketsPurchased + _amount <= lottery.ticketMaxSupply, "Purchasing Tickets Exceeds max supply");
        tusckCoinV1.burn(msg.sender, (_amount*lottery.price));
        unchecked{
            lottery.ticketsPurchased += _amount;
             if(addressToCounter[msg.sender] == 0){
                addressToCounter[msg.sender] = ++_lottoAddressCounter;
    ...
    function drawLottery(uint64 _index) public  onlyOwner lotteryIndexCheck(_index)  {
         ...
         uint256 winnerCounter = lottery.addressPool[winnerIndex];
         lottery.winner = counterToAddress[winnerCounter];
    ...
    function getLotteryAddressPool(uint64 _index) public view  lotteryIndexCheck(_index) returns (uint256[] memory)  {
         Lottery storage lottery = lotteries[_index];
