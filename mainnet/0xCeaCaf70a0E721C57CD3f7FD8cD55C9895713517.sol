...
contract WrappedKevins is ERC721, ERC721Holder, Ownable {
    ...
    // fallback incase something happens to Arweave (very unlikely)
    string public _kevinBaseUri;
        ...
        returns (string memory)
    {
        ...
        string memory baseURI = _kevinBaseUri;
                            ...
                            bytes(
                                string(
                                    abi.encodePacked(
                                        '{"name": "Kevin #',
                                        tokenId.toString(),
                                        '", "description": "Wrapped Kevins are original Kevins preserved on a new contract. The underlying metadata is fully on-chain and no royalties are collected. Kevin has been set free.", "image":"',
                                        kevinImage,
    ...
    // fallback - most likely wont be used
    function setKevinBaseUri(string memory uri) external onlyOwner {
        _kevinBaseUri = uri;
    }
}
