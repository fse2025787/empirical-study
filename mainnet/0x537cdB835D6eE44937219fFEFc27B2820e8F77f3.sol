...
contract EURB is AccessControl, ERC20Burnable, Ownable, Pausable {
    ...
    mapping(address => bool) public isSenderExcludedFromFee;
    mapping(address => bool) public isFrozen;
    mapping(address => bool) public isReceiverExcludedFromFee;
    ...
    function excludeSenderFromFee(address addr_) public onlyOwner{
        isSenderExcludedFromFee[addr_] = true;
    }

    function includeSenderInFee(address addr_) public onlyOwner{
        isSenderExcludedFromFee[addr_] = false;
    }

    function excludeReceiverFromFee(address addr_) public onlyOwner{
        isReceiverExcludedFromFee[addr_] = true;
    }

    function includeReceiverInFee(address addr_) public onlyOwner{
        isReceiverExcludedFromFee[addr_] = false;
    }

    function isTransactionExcludedFromFee(address from_, address to_) public view returns (bool){
        if(isSenderExcludedFromFee[from_] || isReceiverExcludedFromFee[to_] || from_ == owner() || from_ == _feeReceiver || to_ == owner() || to_ == _feeReceiver) {
            return true;
        }
        else {
            return false;
    ...
    function getTransactionFee(address from_, address to_, uint256 amount_) public view returns(uint256 fee) {
        if(!isTransactionExcludedFromFee(from_, to_)) {
            fee = amount_ * _feePercentage / 100000;
        } else {
            fee = 0;
        }
    }
    ...
    function transferFee(address sender, address recipient, uint256 amount) internal {
        uint256 txFee = getTransactionFee(sender, recipient, amount);
        if(txFee > 0) {
