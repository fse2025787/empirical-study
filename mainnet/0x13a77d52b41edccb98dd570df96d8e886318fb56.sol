...
/**
The TryEx Token Vesting Contract - 2,000,000 Team Tokens Locked for 6 months
...
function setVesting(uint256 _timePeriodInSeconds) public onlyOwner timestampNotSet {
timestampSet = true;
initialTimestamp = block.timestamp;
timePeriod = initialTimestamp.add(_timePeriodInSeconds);
}

function VestTryEx(address recipient, uint256 amount) public onlyOwner timestampIsSet incomingDepositsStillAllowed {
require(recipient != address(0), "ERC20: transfer to the zero address");
balances[recipient] = balances[recipient].add(amount);
emit AllocationPerformed(recipient, amount);
}

function ClaimTryEx(IERC20 token, address to, uint256 amount) public timestampIsSet noReentrant {
...
function WithdrawAccidentalETH(IERC20 token, uint256 amount) public onlyOwner noReentrant {
