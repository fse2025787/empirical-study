...
contract CrowdsaleToken is MintedCrowdsale, PausableCrowdsale, FinalizableCrowdsale {

    /** 
    *
    * @dev Creates a new crowdsale contracts to sell a `token`
    * representing a pyshical asset that
    *
    * - Will be payed at the defined `rate` to `wallet`.
    * - Will open at block `openingTime` and close at block `closingTime`.
    * - Mintable, so that tokens are directly minted during the crowdsale.
    *
    */
   ...
   /**
     * @dev Overrides finalization of FinalizableCrowdsale. It stores the remaining tokens to
            `wallet` and renounces minter role. `Finalize()` function should be called once the crowdsale
            is finalized to recover the tokens that weren't sold. Even if it is not called, the crowdsale
            enters state of finalization and adquistions are no longer possible.
   */
