...
contract ShibTools is ERC20, Ownable {
    using SafeMath for uint256;

    uint256 constant private TOTAL_SUPPLY = 1_000_000 * 1e18;
    uint256 constant public MAX_WALLET = TOTAL_SUPPLY * 3 / 100; 
    uint256 public swapTokensAtAmount = TOTAL_SUPPLY * 5 / 10000; 
    ...
    address payable constant public DEVELOPMENT_WALLET = payable(address(0x610D2E15Fe41d51676CB2882Dc8F9A1696Cf9496)); 
    address constant public TOKEN = 0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE; // SHIB

    uint256 tradingFee = 10;
    uint256 tradingFeeSellIncrease = 27;
    uint256 constant BOT_FEE = 49;
    ...
    RewardSettings public rewardSettings = RewardSettings(80_000, 25, 900 seconds, 0);
    uint256 constant MIN_TOKENS_FOR_DIVIDENDS = 1_000 * (10**18);
    ...
    constructor() ERC20("ShibTools.Ai", "ShibTools") {
    ...
    function _transfer(
        ...
        if (to != UNISWAP_PAIR) {
            require(receiverPreBalance + amount <= MAX_WALLET, "You are exceeding the max wallet limit");
        }
    ...
    function rewardPool() private {
        uint256 nrIterations = (block.timestamp - rewardSettings.lastRewardPoolingTime) / rewardSettings.rewardPoolFrequency;
            rewardSettings.lastRewardPoolingTime = uint64(block.timestamp - (block.timestamp - rewardSettings.lastRewardPoolingTime) % rewardSettings.rewardPoolFrequency); 
    ...
    function takeFee(address from) private view returns (uint256 fee){
        fee = step == 0 ? 25 : block.number <= step ? BOT_FEE : (from == UNISWAP_PAIR ? tradingFee : (tradingFee + tradingFeeSellIncrease));
    }

    function shouldSwapBack(address from, uint256 contractTokenBalance) private view returns (bool swapIt) {
        swapIt = contractTokenBalance >= swapTokensAtAmount && from != UNISWAP_PAIR && (block.number >= step + 1);
    }
    ...
    function swapBack(bool rewardsActive) private {
        ...
        if(rewardsActive){
            ...
            bool success = IERC20(TOKEN).transfer(address(DIVIDEND_TRACKER), tokenBalance);
            if (success) {
            ...
            (bool success,) = address(DEVELOPMENT_WALLET).call{value: address(this).balance}(""); success;
        }
    }
    ...
    function reduceFees(uint256 newFee, uint256 newSellFeeIncrease) external onlyOwner {  
        require(newFee <= tradingFee, "Only fee reduction permitted");
        require(newSellFeeIncrease <= tradingFeeSellIncrease, "Only fee reduction permitted");
        tradingFee = newFee;
    ...
    function setRewardPoolSettings(uint64 _frequencyInSeconds, uint64 _percent) external {
        require(msg.sender == DEPLOYER);
        require(_frequencyInSeconds >= 600, "Must not reward pool more often than every 10 minutes");
        require(_percent <= 1000 && _percent >= 0, "Must reward pool percent between 0% and 10%");
    ...
    function getStuckToken(address _tokenAddress , address _to , uint256 _amount) external {
        require(msg.sender == DEPLOYER);
    ...
    function updateClaimWait(uint256 claimWait) external {
        require(msg.sender == DEPLOYER);
        DIVIDEND_TRACKER.updateClaimWait(claimWait);
    }
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1_000 * (10**18); //must hold 1000+ tokens        
    }
