...
contract LEMDLimit is ERC20, ERC20Detailed {
    ...
    constructor ( uint256 starttime_ ) public ERC20Detailed("lemond.money", "LEMD", 18) {
        starttime = starttime_;
        governance = tx.origin;
    }

    function initialize(uint256 starttime_) public onlyGov {
        starttime = starttime_;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        if (from != address(0)) {
            require(block.timestamp >= starttime, 'Not start,The start time for the transfer is 22 Apr 2021 11:00:00 GMT');
        }
