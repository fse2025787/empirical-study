...
// File: contracts/BaseCrossBridge/interface/ICrossBridgeAdmin.sol

pragma solidity ^0.8.7;

interface ICrossBridgeAdmin {
    ...
    function tokens(uint8 id, uint64 searchBlockIndex)
        external
        view
        returns (
            address[] memory tokens,
    ...
    function charges()
        ...
        returns (
            ...
            address[][] memory tokens,
            uint256[][] memory charges
        );

    function setToken(address token, address crossToken) external;
    ...
    function setTokenFullInfo(
        address token,
        uint256 minAmount,
        uint256 chargePercent
    ) external;

    function setTokenMinAmount(address token, uint256 minAmount) external;

    function setTokenChargePercent(address token, uint256 chargePercent)
        external;
...
// File: contracts/BaseCrossBridge/base/CrossBridgeAdminUpgradeable.sol

pragma solidity ^0.8.7;

contract CrossBridgeAdminUpgradeable is
    Initializable,
    OwnableUpgradeable,
    ICrossBridgeAdmin
{
    ...
    function owner()
        public
        view
        override(OwnableUpgradeable, ICrossBridgeAdmin)
        returns (address)
    {
        ...
        )
    {
        ...
        (tokens_, crossTokens, minAmounts, chargePercents, tokenTypes) = bts
            .tokens(tf, id, futureBlock, searchBlockIndex);
    }
        ...
        )
    {
        ...
        TokensChargesInfo memory info = BridgeUtils.getCollectedChargesCN(
            bts,
            network.id
        );
        return (info.ids, info.tokens, info.charges);
    }

    function setToken(address token, address crossToken_)
        external
        virtual
        override
        onlyOwner
    {
        ...
        bts.resetConnection(token, crossToken_);
    }
    ...
    function setTokenConnection(
        ...
        bts.setConnection(token, amount, percent, crossToken_, name, symbol);
    }
    ...
    function setTokenFullInfo(
        ...
        bts.setInfo(token, amount, percent);
    }

    function setTokenMinAmount(address token, uint256 amount)
        external
        virtual
        override
        onlyOwner
    {
        require(bts.mapToken(token), "TXE");
        bts.setAmount(token, amount);
    }

    function setTokenChargePercent(address token, uint256 percent)
        external
        virtual
        override
        onlyOwner
...
// File: contracts/Net-Ethereum/Bridge/EthereumBridgeAdmin.sol

pragma solidity ^0.8.7;

contract EthereumBridgeAdmin is CrossBridgeAdminUpgradeable {
    function initialize(
        address tokenFactory,
        address bridgeStorage,
        address bridgeTokenStorage
