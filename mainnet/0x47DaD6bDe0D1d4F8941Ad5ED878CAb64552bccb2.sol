...
// 
// Base: ERC721A from Chiru Labs
// Modified: Adorable Nerds 
// Modify Description: Enable to have reservavle tokens in the begining of the contract, with Lazy mint
 ...
 */
contract ERC721AReservable is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // (Reservable Addition) The reserved tokens in the contract.
    uint256 internal _totalReserved;       

    // (Reservable Addition) The reserved tokenId of the next token to be minted.
    ...
    constructor(
        ...
        uint256 totalReserved_ //(Reservable Addition)
        ) {
            _name = name_;
            _symbol = symbol_;
            _totalReserved = totalReserved_; //(Reservable Addition)
    ...
    /**
     * (Reservable Addition)
     * To change the reserved starting tokenId, please override this function.
     */
    ...
    function _startTokenId() internal view virtual returns (uint256) {
        return _startReservedTokenId() + _totalReserved;
    }

    /**
     * (Reservable Addition)
     * Returns the total amount of tokens reserved minted in the contract.
     */
    function _totalReservedMinted() internal view returns (uint256) {
        unchecked {
            return _currentReservedIndex - _startReservedTokenId();
        }
    } 
    ...
    function totalSupply() public view returns (uint256) {
        ...
        unchecked {
            return (_currentIndex - _burnCounter - _startTokenId()) + _totalReservedMinted();
        }
    }
    ...
    function _totalMinted() internal view returns (uint256) {
        ...
        unchecked {
            return (_currentIndex - _startTokenId()) + _totalReservedMinted();
        }
    }
    ...
    function _ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        ...
        unchecked {
            if (_startReservedTokenId() <= curr && curr < _currentIndex) {
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721AReservable.ownerOf(tokenId);
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        uint256 startToken = _startTokenId();
        uint256 currentIndex = _currentIndex;

        // it means, the token is from reserved slot
        // so we should check if exist on the slot
        if(tokenId < startToken) {
            startToken = _startReservedTokenId();
            currentIndex = _currentReservedIndex;
        }           

        return startToken <= tokenId && tokenId < currentIndex && !_ownerships[tokenId].burned;
    }
     ...
     * (Reservable Addition)
     */
    function _safeMintReserved(address to, uint256 quantity) internal {
        _safeMintReserved(to, quantity, '');
    }
    ...
    function _safeMint(
        ...
        _mint(to, quantity, _data, true, false);
    }

    /**
     * (Reservable Addition)
     * @dev Safely mints `quantity` tokens from reserved and transfers them to `to`.
     *
     * Requirements:
     *
     * - If `to` refers to a smart contract, it must implement {IERC721Receiver-onERC721Received}, which is called for each safe transfer.
     ...
     * Emits a {Transfer} event.
     */
    function _safeMintReserved(
        address to, 
        uint256 quantity,
        bytes memory _data
    ) internal {
        _mint(to, quantity, _data, true, true);
    }    
    ...
    function _mint(
        ...
        if (reserved) {
            startTokenId = _currentReservedIndex;
            index = _currentReservedIndex;
        }
        ...
        unchecked {
            ...
            if (reserved) {
                _currentReservedIndex = updatedIndex;
            } else {
    ...
    function _transfer(
        ...
        // Get index accordingly with slot (reserved or not)
        uint256 index = _currentIndex;
