...
contract Meme is Context, IERC20, Ownable {
    ...
    string private _name = "Memeverse";
    ...
    address payable public marketingWallet = payable(0xB336acCA495Fc7a57221F50a68a7990b9037973e);
    address payable  public devWallet = payable(0x4A6b3E0965FaBb527d7F12a69c4bcf8AEf27c43C);
    ...
    uint256 public numTokensSellToAddToLiquidity = 15000 * 10**18;
    uint256 public _maxTxAmount = 200000 * 10**18;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _burnFee==0 && _devFee==0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 combineFees = _liquidityFee.add(_marketingFee).add(_devFee); // marketing + dev + liquidity fees
        
        
        
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(combineFees); 
        uint256 tokendev = contractTokenBalance.mul(_devFee).div(combineFees);
        
        uint256 tokensForMarketing = contractTokenBalance.sub(tokensForLiquidity).sub(tokendev);
        uint256 tokensFordev = contractTokenBalance.sub(tokensForLiquidity).sub(tokensForMarketing);
        ...
        //Send to dev address
        transferToAddressETH(devWallet, transferredBBalance);
    }
    ...
    function setTaxFeePercent(uint256 taxFee,uint256 liquidityFee,uint256 MarketingFee,uint256 burnFee, uint devFee) external onlyOwner() {
        _taxFee = taxFee;
        _pretaxfee = _taxFee;
    ...
    function setsellTaxFeePercent(uint256 taxsellFee,uint256 liquiditysellFee,uint256 MarketingsellFee,uint256 burnsellFee, uint256 devsellFee) external onlyOwner() {
