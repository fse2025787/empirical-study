...
contract ERC721AContract is ERC721A, ReentrancyGuard, Ownable {
    ...
    function setPubSaleState(bool _isActive) public onlyOwner {
        // merkleRook is not required since pre-sale is not required
        // preSalePrice and pubSalePrice can be zero since free mints are allowed
        require(maxSupply > 0, "maxSupply is not set.");
        require(maxPerWallet > 0, "maxPerWallet is not set.");
        require(maxPerTransaction > 0, "maxPerTransaction is not set.");
        require(multisigAddress != address(0), "multisigAddress is not set.");
