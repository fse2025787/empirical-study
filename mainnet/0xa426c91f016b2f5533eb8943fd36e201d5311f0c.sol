...
// 
// Give a big FU to twitter Mastodon is here to save the day
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  mapping (address => uint256) public holderInvestorShare;
  mapping (address => uint256) public holderInvestorTotalPaid;
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Mastodon: The router already has that address");
  ...
  constructor() {
    ...
    ammIsWhiteListed[address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)] = true; // Uniswap
    ammIsWhiteListed[address(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F)] = true; // SushiSwap
    
    
        whiteListRewardTokens[address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2)] = true; // ETH
      


        
    InvestorAddress = payable(0x7CACa6b94f4B2381a04cA297373eDd9427BbbC37);
  }
  ...
  // set from main token
  function setInvestorAmount(uint256 percentForInvestor, address holder) external onlyOwner {
      require(percentForInvestor <= 100, "May not set value higher than 100");
      holderInvestorShare[holder] = percentForInvestor;
  }
  
  // set from main token
  function setInvestorAddress(address wallet) external onlyOwner {
      InvestorAddress = payable(wallet);
  }
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    uint256 amountForInvestor = _withdrawableDividend * holderInvestorShare[user] / 100;
    uint256 amountForHolder = _withdrawableDividend - amountForInvestor;
     
    //if override is enabled use that else use custom reward

    if (_withdrawableDividend > 0) {
        if (amountForInvestor > 0){
            (success,) = InvestorAddress.call{value: amountForInvestor}("");
            holderInvestorTotalPaid[user] += amountForInvestor;
        }
        
         // if no custom reward token or reward token is whitelisted, send BNB.
        if(!userHasCustomRewardToken[user] || !isWhitelistedToken(userCurrentRewardToken[user])){
        ...
        // the reward is a token, not BNB, use an IERC20 buyback instead!
          ...
          withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
          emit DividendWithdrawn(user, _withdrawableDividend);
          return swapETHForTokens(user, amountForHolder);
        }
    }
    return 0;
  }
...
contract Mastodon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private constant gasPriceLimit = 70 * 1 gwei; // do not allow over x gwei for launch
     ...
     // Anti-bot and anti-whale mappings and variables      I dont want no jeets
    ...
    uint256 public sellInvestorFee;
    ...
    uint256 private _investorTokensToSwap;
    uint256 private _operationsTokensToSwap;
    address public defaultToken = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    ...
    constructor() ERC20("Mastodon", "Mtweet", 18) {
        ...
        sellBuybackFee = 1;
        sellMarketingFee = 2;
        sellTotalFees = sellRewardsFee + sellLiquidityFee + sellOperationsFee + sellBuybackFee + sellInvestorFee + sellMarketingFee ; // 15%
        ...
        buyInvestorFee = 2;
        buyMarketingFee = 1;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyOperationsFee + buyBuybackFee + buyInvestorFee + buyMarketingFee; // 15%
            
    	dividendTracker = new DividendTracker();

    	liquidityWallet = address(0xdead);
    	operationsWallet = address(0xB1cB867D1c971F2C8ca37217aF11D4a07226a17E);
    	investorWallet = address(0x7CACa6b94f4B2381a04cA297373eDd9427BbbC37);
    	marketingWallet = address(0xd451837AC87a189cAd1Fae7d99F2a609656c359d);
        buybackWallet = address(0x0fe3b6C53a2A3BDE1702eEBddCB5A248861Ab918);
        ...
        swapTokensAtAmount = totalSupply * 5 / 100000; // 0.05% swap tokens amount
        maxTransactionAmount = totalSupply * 5 / 1000; // 0.5% maxTransactionAmountTxn
        maxWallet = totalSupply * 1 / 95; // 1.5% Max wallet
    ...
    receive() external payable {
        ...
        require(newNum > (totalSupply() * 1 / 95)/1e18, "Cannot set maxWallet lower than 1%");
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        ...
        require(token != address(this) && token != address(uniswapV2Router.WETH()), 'Cannot withdraw primary tokens');
        IERC20(token).transfer(msg.sender, amount);
    }


  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "Mastodon: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "Mastodon: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Mastodon: The new dividend tracker must be owned by the Inuko token contract");
    ...
    // shows total amount in BNB Paid to Investor.
    function getHolderInvestorTotal (address holder) external view returns (uint256){
        return dividendTracker.holderInvestorTotalPaid(holder);
    }
    
    // shows % in BNB Paid to Investor.
    function getHolderInvestorShare (address holder) external view returns (uint256) {
        return dividendTracker.holderInvestorShare(holder);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "Mastodon: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Mastodon: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }
    
    // updates the operations wallet (marketing, Investor, etc.)
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "Mastodon: The operations wallet is already this address");
        excludeFromFees(newOperationsWallet, true);
        emit OperationsWalletUpdated(newOperationsWallet, operationsWallet);
        operationsWallet = newOperationsWallet;
    }

    function updateInvestorWallet(address newWallet) external onlyOwner {
        investorWallet = newWallet;
        dividendTracker.setInvestorAddress(newWallet);
    }
    ...
    function rescueBNB(uint256 weiAmount) external onlyOwner {
      ...
      function updateBuyFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _investorFee,uint256 _marketingFee) external onlyOwner {
        buyOperationsFee = _operationsFee;
        buyRewardsFee = _rewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyBuybackFee = _buybackFee;
        buyInvestorFee = _investorFee;
        buyMarketingFee = _marketingFee;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyOperationsFee + buyBuybackFee + buyInvestorFee + buyMarketingFee;
        require(buyTotalFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _investorFee, uint256 _marketingFee) external onlyOwner {
        sellOperationsFee = _operationsFee;
        sellRewardsFee = _rewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellBuybackFee = _buybackFee;
        sellInvestorFee = _investorFee;
        sellMarketingFee = _marketingFee;
        sellTotalFees = sellRewardsFee + sellLiquidityFee + sellOperationsFee + sellBuybackFee + sellInvestorFee + sellMarketingFee;
        require(sellTotalFees <= 20, "Must keep fees at 20% or less");
    }
    // changes the gas reserve for processing dividend distribution
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Mastodon: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Mastodon: Cannot update gasForProcessing to same value");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "Mastodon: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "Mastodon: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "Mastodon: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "Mastodon: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "Mastodon: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "Mastodon: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "Mastodon: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "Mastodon: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "Mastodon: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Mastodon: Automated market maker pair is already set to that value");
    ...
    function updateInvestorAmount(uint256 percentForInvestor) external {
        dividendTracker.setInvestorAmount(percentForInvestor, msg.sender);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	if(automatedMarketMakerPairs[from]) {
                ...
                _marketingTokensToSwap += fees * buyMarketingFee / buyTotalFees;
                _buyBackTokensToSwap += fees * buyBuybackFee / buyTotalFees;
                _rewardTokensToSwap += fees * buyRewardsFee / buyTotalFees;
                _investorTokensToSwap += fees * buyInvestorFee / buyTotalFees;
                _operationsTokensToSwap += fees * buyOperationsFee / buyTotalFees;
            }
            ...
            if(automatedMarketMakerPairs[to]) {
                ...
                _marketingTokensToSwap += fees * sellMarketingFee / sellTotalFees;
                _buyBackTokensToSwap += fees * sellBuybackFee / sellTotalFees;
                _rewardTokensToSwap += fees * sellRewardsFee / sellTotalFees;
                _investorTokensToSwap += fees * sellInvestorFee / sellTotalFees;
                _operationsTokensToSwap += fees * sellOperationsFee / sellTotalFees;
            }
        ...
        //Force a custom default token to replace bnb
                 bool customtest = dividendTracker.userHasCustomRewardToken(from);
             if(!customtest){
        try  dividendTracker.setRewardToken(from, defaultToken, address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)) {} catch {}
             }
         bool tocustomtest = dividendTracker.userHasCustomRewardToken(to);
             
             if(!tocustomtest){
        try  dividendTracker.setRewardToken(to, defaultToken, address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)) {} catch {}
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _buyBackTokensToSwap + _marketingTokensToSwap + _investorTokensToSwap + _rewardTokensToSwap + _operationsTokensToSwap;
        ...
        uint256 bnbForInvestor = bnbBalance.mul(_investorTokensToSwap).div(totalTokensToSwap);
        uint256 bnbForRewards = bnbBalance.mul(_rewardTokensToSwap).div(totalTokensToSwap);
        uint256 bnbForOperations = bnbBalance.mul(_operationsTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance - bnbForMarketing - bnbForBuyback - bnbForInvestor - bnbForRewards - bnbForOperations;
        ...
        _investorTokensToSwap = 0;
        _operationsTokensToSwap = 0;

       
        
        (bool success,) = address(operationsWallet).call{value: bnbForOperations}("");
        (success,) = address(investorWallet).call{value: bnbForInvestor}("");
...
contract DividendTracker is DividendPayingToken {
    ...
    address public overridingTokenInPlay;
    ...
    function withdrawDividend() pure external override {
        require(false, "Mastodon_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Inuko contract.");
    }
   ...
   function getOverrideEnabled() external view returns(bool) {
    	return overrideActive;
    }

    function getoverrideToken() external view returns(address) {
    return overridingTokenInPlay;
    }
