...
contract WorldCupApesRestake is Ownable {
    ...
    bool public restakingLive = false;

    IERC721Enumerable private WCANFT;
    ...
    modifier restakingEnabled {
      require(restakingLive && block.timestamp < startUnstakeTimestamp, "Restaking not live");
      _;
    }

    constructor(address _WCANFT, address _WCAToken) {
    ...
    function toggleRestakingLive() external onlyOwner {
      restakingLive = !restakingLive;
    }
    ...
    function restake(address _address) external restakingEnabled {
      uint256 balance = WCAToken.balanceOf(_address);

      require(balance > 0, "You have no $WCA");

      require(WCAToken.allowance(_address, address(this)) >= balance, "First approve to restake");

      WCAToken.transferFrom(_address, owner(), balance);

      stakerToTokens[_address] = balance.mul(2);
      stakerToInitialTokens[_address] = balance.mul(2);
    }
    ...
    function claim(address _address) external canUnstake {
      ...
      uint256 rewards = block.timestamp.sub(fromTimestamp).mul(stakerToInitialTokens[_address]).mul(emissionRate).div(divider).div(86400);
