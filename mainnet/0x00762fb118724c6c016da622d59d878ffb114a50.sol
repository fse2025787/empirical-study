...
// File: contracts/DisciplesLair.sol
...
//The Disciples Lair is the staking contract for CULT-DISCIPLES NFT
pragma solidity ^0.8.4;
...
contract DisciplesLair is Ownable, IERC721Receiver {

  uint256 public totalStaked;
  uint256 public rewardRate = 1000 ether;
...
// rewardmath = 1000 ether .... (This gives 10 tokens per day per NFT staked to the staker)
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      rewardmath = rewardRate * (block.timestamp - stakedAt) / 86400 ;
      earned = rewardmath;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
