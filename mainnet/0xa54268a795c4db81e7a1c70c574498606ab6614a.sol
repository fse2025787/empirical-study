...
contract HirokaHighSchool is ERC721A, Ownable, DefaultOperatorFilterer {
  ...
  mapping(uint256 => bool) anEndTokenHasMinted;


  constructor(address anEndAddress) ERC721A("Hiroko High School", "HHS") {
    _mint(msg.sender, 1);
    AnEnd = IERC721A(anEndAddress);
  }

  function holderMint(uint256[] calldata anEndTokenIdsHeld) public payable {
    if (!holderSaleActive) revert MintInactive();
    if (msg.sender != tx.origin) revert ContractCaller();
    uint256 length = anEndTokenIdsHeld.length;
    unchecked {
      if (length + totalSupply() > MAX_SUPPLY) revert ExceedsSupply();
      for (uint256 i = 0; i < length; i++) {
        if (anEndTokenHasMinted[anEndTokenIdsHeld[i]]) revert TokenHasMinted(anEndTokenIdsHeld[i]);
        if (AnEnd.ownerOf(anEndTokenIdsHeld[i]) != msg.sender) revert NotTokenOwner();
        anEndTokenHasMinted[anEndTokenIdsHeld[i]] = true;
      }
    }
