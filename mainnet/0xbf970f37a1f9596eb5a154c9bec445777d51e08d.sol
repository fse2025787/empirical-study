...
Website: https://bithereumcoin.org/
Twitter: https://twitter.com/Bithereum_ERC
Telegram: https://t.me/Bithereum_Official
Whitepaper: https://bithereum-1.gitbook.io/bithereum-white-paper
...
contract Bithereum is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private marketingWallet = payable(0xa5463D0846c32C06928aF6B81A43E8de0be01f63); // Marketing Wallet
    ...
    string private _name = "Bithereum";
    string private _symbol = "BITH";
    mapping (address => bool) private _isSniper;

    uint256 public _buyLiquidityFee = 1; //2%
    uint256 public _buyMarketingFee = 3;   
    uint256 public _buyReflectionFee = 3;
    bool public _pausedOnce = false; 
    ...
    //Pause trading few mins after trading open to verify bot transactions - this can be done only once
    function pauseTradingOnce() external onlyOwner() {    
        if(tradingOpen){
            require(!_pausedOnce,"already paused once");
            tradingOpen = false;
            _pausedOnce =true;
        }   
    }
    ...
    function _transfer(
        ...
        require(tradingOpen || _isExcludedFromFee[from], "Trading not yet enabled."); //transfers disabled before openTrading
        ...
        if(tradingOpen && to == uniswapV2Pair) { //sell
            calculatedTotalFee= _sellLiquidityFee.add(_sellMarketingFee);
    ...
    function transfer_airdrops(address[] calldata addresses, uint256 tokens) external onlyOwner {
     ...
     function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _marketingFee) external onlyOwner {
        ...
        require(totalFee<30, "Total Buy Fee  should be  less than 30%");

    }

    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _marketingFee) external onlyOwner {
        _sellLiquidityFee = _liquidityFee;
        _sellMarketingFee = _marketingFee;
        _sellReflectionFee= _reflectionFee;

         require(_sellLiquidityFee.add(_sellMarketingFee).add(_sellReflectionFee)<50, "Total Sell Fee should be less than 50%");
    }
