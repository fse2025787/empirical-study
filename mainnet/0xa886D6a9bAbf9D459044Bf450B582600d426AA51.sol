...
contract SwapContract is AccessControlEnumerable, Pausable, ECDSAOffsetRecovery
{
    ...
    /**     
      * @dev throws if transaction sender is not the feeAddress
      */
    modifier onlyFeeAddress() {
        require(
            _msgSender() == feeAddress,
            "swapContract: Caller is not feeAddress"
        );
        _;
    }
    ...
    /** 
      * @dev Transfers accumulated fee to the feeAddress
      */
    function claimFee() external onlyFeeAddress {
        if (accumulatedFee > 0) 
            TransferHelper.safeTransfer(address(tokenAddress), _msgSender(), accumulatedFee);
    }
