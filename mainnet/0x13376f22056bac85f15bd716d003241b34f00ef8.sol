...
contract LepaStrategicBucket is Pausable,Ownable {
    ...
    uint256 public maxLimit =  39 * (10**6) * 10**18;
    ...
    function GrantAllocation(address allocationAdd, uint256 amount) public onlyOwner whenNotPaused {
        ...
        require(amount > users[allocationAdd].allocation, "Amount cannot be less then already allocated amount");
        require(allocatedSum.sub(users[allocationAdd].allocation).add(amount) <= maxLimit, "Limit exceeded");
    ...
    function ProcessClaim() public payable whenNotPaused {
