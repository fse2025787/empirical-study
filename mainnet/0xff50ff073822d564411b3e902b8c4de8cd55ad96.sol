...
contract SatoshiViralStake is Ownable {
    using SafeMath for uint256;

    // stakeToken address (SATs)
    // 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984 : UNI
    IERC20 public stakeToken;
    // rewardToken address (WBTC)
    // 0x110a13FC3efE6A245B50102D2d79B3E76125Ae83 : USDT
    IERC20 public rewardToken;

    // maxStakableToken : available stakable token maximan amount
    // first totalSupply, second can be set
    uint256 public maxStakeableToken;
    // minStakableToken : available stakable token minimam amount
    // first 1e18, second can be set
    uint256 public minimumStakeToken;

    // total amount of unstaked token
    uint256 public totalUnStakedToken;
    // total amount of staked token
    uint256 public totalStakedToken;
    // total amount of claimed reward token
    uint256 public totalClaimedRewardToken;
    // total stakers : user array
    uint256 public totalStakers;

    // percent divider : proportion
    uint256 public percentDivider;
    // duration : lock time
    uint256 public Duration = 3600 * 24 * 7 seconds;
    // bonus : how much reward can be get
    ...
    struct Stake {
        ...
        uint256 reward; // amount of ward (total)
    ...
    struct User {
        ...
        bool alreadyExists; // is this user already exist?
    }

    // user array : addreess => user
    mapping(address => User) public Stakers;
    // staker id array : id => address
    mapping(uint256 => address) public StakersID;
    // record of stakers : address => id => Stake
    ...
    // constructor
    constructor(address stakingtoken, address rewardingtoken) {
        // stakingtoken: staking token address (SATs)
        // rewardtoken: reward token address (WBTC)

        // get IERC20 stakeToken
        stakeToken = IERC20(stakingtoken);
        // get IERC20 rewardToken
        rewardToken = IERC20(rewardingtoken);
        // available maximum amount of stakeToken
        maxStakeableToken = stakeToken.totalSupply();
        // available minimum amount of stakeToken
        minimumStakeToken = 0;
        // percent divider: proportion
        percentDivider = 1e2;
    }
    ...
    function stake(uint256 amount) public {
        ...
        // transfer stakeToken (SATs) to smart contract
        stakeToken.transferFrom(msg.sender, address(this), amount);

        // get stakeCount of staker
        uint256 index = Stakers[msg.sender].stakeCount;
        // add amount to totalStakedTokenUser of staker
        Stakers[msg.sender].totalStakedTokenUser = Stakers[msg.sender]
            .totalStakedTokenUser
            .add(amount);

        // add amount to totalStakedToken => smart contract totalStakedToken
        totalStakedToken = totalStakedToken.add(amount);

        // create new record with staketime as block.timestamp
        stakersRecord[msg.sender][index].staketime = block.timestamp;
        // set unstake time of new record
        stakersRecord[msg.sender][index].unstaketime = block.timestamp.add(
            Duration
        );
        // set amount => staking amount
        stakersRecord[msg.sender][index].amount = amount;
        // set reward => claiming reward
        // amount * Bonus / percentDivider
        // percentDivider is constant
        stakersRecord[msg.sender][index].reward = GetRewardOf(amount);

        stakersRecord[msg.sender][index].availableUnstakedAmount = amount;
        stakersRecord[msg.sender][index].availableUnclaimedReward = GetRewardOf(
            amount
        );
    ...
    function unstake(uint256 amount) public {
        // check the index of stake token that unstaked before
        // require(
        //     amount > GetMaxUnstakeAmount(),
        ...
        for (uint256 index; index < Stakers[msg.sender].stakeCount; index++) {
            if (stakersRecord[msg.sender][index].unstaked || amount == 0) {
                continue;
            }
            if (
    ...
    function claim(uint256 amount) public {
        // check the index of stake token that unstaked before
        // require(
        //     amount > GetMaxUnclaimedAmount(),
        //     "amount must be less than available max unclaimed amount"
        // );

        uint256 sendReward = amount;

        for (uint256 index; index < Stakers[msg.sender].stakeCount; index++) {
            if (stakersRecord[msg.sender][index].withdrawn || amount == 0) {
                continue;
            }
            if (
                stakersRecord[msg.sender][index].unstaketime >= block.timestamp
        ...
        // give rewardToken to staker => send rewardToken from smart contrac to staker
        rewardToken.transfer(msg.sender, sendReward);
        // add totalRewardToken amount
        totalClaimedRewardToken = totalClaimedRewardToken.add(sendReward);
        // add totalClaimUser of Staker amount
        Stakers[msg.sender].totalClaimedRewardTokenUser = Stakers[msg.sender]
            .totalClaimedRewardTokenUser
            .add(sendReward);

        // emit CLAIM
        emit CLAIM(msg.sender, sendReward);
    }

    // SetStakeLimits with min value and max value
    ...
    // set StakeBonus
    function SetStakeBonus(uint256 newBonus) external onlyOwner {
        Bonus = newBonus;
    }

    // set PercentDivider
    function SetPercentDivider(uint256 newPercentDivider) external onlyOwner {
        percentDivider = newPercentDivider;
    }

    // get StakeToken balance
    function GetStakeTokenBalanceOf(address account)
        public
        view
        returns (uint256)
    {
        uint256 stakeTokenBalance = stakeToken.balanceOf(account);
        return stakeTokenBalance;
    }

    // get RewardToken balance
    function GetRewardTokenBalanceOf(address account)
        public
        view
        returns (uint256)
    {
        uint256 rewardTokenBalance = rewardToken.balanceOf(account);
        return rewardTokenBalance;
    }

    // get Staking List of Staker
    function GetStakingListOf(address user)
        public
        view
        returns (Stake[] memory)
    {
        uint256 stakeCount = Stakers[user].stakeCount;
        Stake[] memory stakingList = new Stake[](stakeCount);
        for (uint256 i = 0; i < stakeCount; i++) {
            Stake storage staking = stakersRecord[user][i];
    ...
    // get Staking List of Staker
    function GetRewardOf(uint256 amount) public view returns (uint256 reward) {
        return amount.div(totalStakedToken).mul(1e2).mul(1e9);
    }

    // get Available Unstake Token Amount
    function GetMaxUnstakeAmount(address user)
        public
        view
        ...
        for (uint256 index; index < Stakers[user].stakeCount; index++) {
            if (!stakersRecord[user][index].unstaked) {
                if (stakersRecord[user][index].unstaketime < block.timestamp) {
                    maxUnstakeAmount = maxUnstakeAmount.add(
                        stakersRecord[user][index].availableUnstakedAmount
                    );
                }
            }
        }
    ...
    // get Available Unstake Token Amount
    function GetMaxUnclaimedAmount(address user)
        public
        view
        returns (uint256 reward)
    {
        ...
        for (uint256 index; index < Stakers[user].stakeCount; index++) {
            if (!stakersRecord[user][index].withdrawn) {
                if (stakersRecord[user][index].unstaketime < block.timestamp) {
                    maxUnclaimedReward = maxUnclaimedReward.add(
                        stakersRecord[user][index].availableUnclaimedReward
                    );
                }
            }
        }
