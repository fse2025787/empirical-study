...
contract Casablanca is ERC20, Ownable {
    ...
    constructor () ERC20("Casablanca", "CASA"){
        setSwapRouter();
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        uint256 amountReceived = (takeFee) ? calTaxes(sender, recipient, amount) : amount;
        super._transfer(sender, recipient, amountReceived);
        takeTaxes(sender, recipient);
    }

    function calTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = marketingFee + liquidityFee;
        return amount - feeAmount;
    }

    function takeTaxes(address from, address to) internal{
        // Take liquidity fee from sender because fee amount already contain count it.
        tokensForAutoLiquidity += liquidityFee;
        super._transfer(from, address(this), liquidityFee);
        // Take marketing fee from sender because fee amount already contain count it.
        tokensForMarketing += marketingFee;
        super._transfer(from, address(this), marketingFee);
        // Take marketing fee from receiver because fee amount not contain count it.
        super._burn(to, burnFee);
    }
