...
Mystery has been solved! Welcome to Shibmas! Spread the Christmas spirit!

Tg: https://t.me/ShibmasInu
Website: https://shibmasinu.com/
Whitepapaer: https://shibmasinu.gitbook.io/
...
contract ShibmasInu is ERC20, Auth {
    ...
    string constant _name = "Shibmas Inu";
    string constant _symbol = "Shibmas";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**5 * 10**_decimals;

    uint256 public _maxTxAmount = _totalSupply  / 125; //0.8%
    uint256 public _maxWalletToken = _totalSupply / 83; //1.2%
   ...
   // block number of opened trading
    ...
    uint256 public secretsantaFee      = 1;
    uint256 public teamFee         = 0;
    uint256 public burnFee         = 0;
    uint256 public totalFee        = teamFee + marketingFee + liquidityFee + secretsantaFee + burnFee;
    uint256 public feeDenominator  = 100;

    uint256 public sellMultiplier  = 333;
    ...
    address public secretsantaFeeReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x919a5e8AC6dA7a98a80815a55cd148EE1Af8e12c;
        secretsantaFeeReceiver = 0xb9402626cDCf360F1eE018b209EC6dff4b4c6Ef6;
        teamFeeReceiver = 0xCc568822B2d45bC8842DAe4c305809d24AF03540;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (block.number < (launchedAt + 1) && recipient != pair && recipient != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D) && recipient != address(DEAD)) { 
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHBuyback = amountETH.mul(secretsantaFee).div(totalETHFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountETHMarketing}("");
        (tmpSuccess,) = payable(secretsantaFeeReceiver).call{value: amountETHBuyback}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _teamFee, uint256 _marketingFee, uint256 _secretsantaFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        teamFee = _teamFee;
        marketingFee = _marketingFee;
        secretsantaFee = _secretsantaFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_teamFee).add(_marketingFee).add(_secretsantaFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _secretsantaFeeReceiver, address _burnFeeReceiver, address _teamFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        secretsantaFeeReceiver = _secretsantaFeeReceiver;
