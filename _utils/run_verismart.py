...
def parse(file_path,address,version):
    ...
    try:
        cmd_export = "export LD_LIBRARY_PATH=./pkgs/z3rel/build"
        
        cmd = "./main.native -input {0} -solv {1} -verify_timeout 7200 -z3timeout 7200".format(file_path, version,)
        
        report_txt_file_path = os.path.join("./verismart/result",str(address)+".txt")
        ...
        output = subprocess.run(f"{cmd_export} && {cmd}",stdout = subprocess.PIPE,stderr = subprocess.PIPE,shell=True,check=False,timeout=7200,text=True, cwd="./VeriSmart-public/")
...
if __name__=="__main__":
    ...
    try:
        ...
        num = 0
        data = pd.read_csv("./smart-contract-sanctuary-ethereum_contracts_mainnet_contracts_deduplicate_compile_success.csv")
