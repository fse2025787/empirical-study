...
def parse(file_path,path_output_result,version):
    global error
    cmd = "myth analyze {0} --execution-timeout 7200 --solv {1} -t 3".format(file_path,version)
    ...
    try:
        ...
        output = subprocess.run(cmd,stdout = subprocess.PIPE,stderr = subprocess.PIPE,shell=True,check=True,timeout=7200,text=True)
...
if __name__=="__main__":
    try:
        ...
        path_output_result = r"./mainnet_test_result/mythril/"
        num = 1
        data = pd.read_csv("/pre-processing/smart-contract-sanctuary-ethereum_contracts_mainnet_contracts_deduplicate_compile_success.csv")
