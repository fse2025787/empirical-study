 ...
 yieldOfPeriod[currentPeriodIndex] = convertIBTToUnderlying( remainingYield.mul(UNIT).div(totalUnderlyingDeposited) );
 uint256 collectedYield = remainingYield.mul(collectedFYTSByPeriod[currentPeriodIndex]).div( totalUnderlyingDeposited );
 ...
 scaledTotals[getCurrentPeriodIndex()] = currScaledTotals.add( convertUnderlyingtoIBT(getPTPerAmountDeposited(_amount)) );
 }
 emit FundsDeposited(_user, _amount);
 }
 ...
 uint256 scaledAmount = APWineMaths.getScaledInput( _amount, scaledTotals[getCurrentPeriodIndex()], ibt.balanceOf(address(this)) );
