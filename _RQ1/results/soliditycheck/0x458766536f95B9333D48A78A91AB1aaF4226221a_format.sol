 ...
 function getPilotAmountForTokens( address token0, address token1, uint256 amount0, uint256 amount1, address oracle0, address oracle1 ) external view returns (uint256 total);
 function getPilotAmountWethPair( address tokenAlt, uint256 altAmount, uint256 wethAmount, address altOracle ) external view returns (uint256 amount);
 function getPilotAmount( address token, uint256 amount, address pool ) external view returns (uint256 pilotAmount);
 function assetToEth( address token, address pool, uint256 amountIn ) external view returns (uint256 ethAmountOut);
 function ethToAsset( address tokenOut, address pool, uint256 amountIn ) external view returns (uint256 amountOut);
 function getPrice( address tokenA, address tokenB, address pool, uint256 _amountIn ) external view returns (uint256 amountOut);
 }
 ...
 constructor( address _uniswapFactory, address _weth, address _pilot, address _pilotWethPair, address _strategy, address _governance ) {
 (uniswapFactory, weth, pilot, pilotWethPair, strategy, governance) = ( _uniswapFactory, _weth, _pilot, _pilotWethPair, _strategy, _governance );
 }
 function setUniStrategy(address _strategy) external onlyGovernance {
 require(_strategy != address(0), "IUS");
 emit UniStrategyUpdated(strategy, _strategy);
 strategy = _strategy;
 }
 ...
 function getPoolDetails(address pool) private view returns ( address token0, address token1, uint24 fee, uint16 poolCardinality, uint128 liquidity, uint160 sqrtPriceX96, int24 currentTick, int24 tickSpacing ) {
 ...
 function getPilotAmountForTokens( address token0, address token1, uint256 amount0, uint256 amount1, address oracle0, address oracle1 ) external view override returns (uint256 total) {
 amount0 = getPilotAmount(token0, amount0, oracle0);
 amount1 = getPilotAmount(token1, amount1, oracle1);
 total = amount0.add(amount1);
 }
 function getPilotAmountWethPair( address tokenAlt, uint256 altAmount, uint256 wethAmount, address altOracle ) external view override returns (uint256 amount) {
 ...
 function getPilotAmount( address token, uint256 amount, address pool ) public view override returns (uint256 pilotAmount) {
 uint256 ethAmount = assetToEth(token, pool, amount);
 pilotAmount = ethToAsset(pilot, pilotWethPair, ethAmount);
 }
 function assetToEth( address token, address pool, uint256 amountIn ) public view override returns (uint256 ethAmountOut) {
 return getPrice(token, weth, pool, amountIn);
 }
 function ethToAsset( address tokenOut, address pool, uint256 amountIn ) public view override returns (uint256 amountOut) {
 return getPrice(weth, tokenOut, pool, amountIn);
 }
 function getPrice( address tokenA, address tokenB, address poolAddress, uint256 _amountIn ) public view override returns (uint256 amountOut) {
 int256 twapTick = IUniStrategy(strategy).getTwap(poolAddress);
 return OracleLibrary.getQuoteAtTick( int24(twapTick), SafeUint128.toUint128(_amountIn), tokenA, tokenB );
 }
 }
 pragma solidity =0.7.6;
 library SafeUint128 {
 ...
 function getTicks(address _pool) external returns ( int24 baseLower, int24 baseUpper, int24 bidLower, int24 bidUpper, int24 askLower, int24 askUpper );
 function getReadjustThreshold(address _pool) external view returns (int24 readjustThreshold);
