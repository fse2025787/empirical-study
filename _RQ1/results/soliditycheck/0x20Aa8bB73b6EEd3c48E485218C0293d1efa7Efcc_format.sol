 ...
 loanParamsIds[uint256(keccak256(abi.encodePacked( loanParamsList[i].collateralToken, areTorqueLoans )))] = loanParamsIdList[i];
 }
 }
 ...
 function getInterestRate( uint256 _U, uint256 _a, uint256 _b ) external pure returns (uint256 interestRate);
 ...
 function getTWAI( address pool) external view returns ( uint256 benchmarkRate );
 ...
 function getLoanPrincipal(bytes32 loanId) external view returns (uint256 principal);
 function getLoanInterestOutstanding(bytes32 loanId) external view returns (uint256 interest);
