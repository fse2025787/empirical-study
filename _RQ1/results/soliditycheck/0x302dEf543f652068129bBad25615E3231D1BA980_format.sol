 ...
 proposalTime: block.timestamp - 1, iOOKIWeight: _calcIOOKIWeight(), lpOOKIBalance: 0, lpTotalSupply: 0 }
 );
 }
 function _calcIOOKIWeight() internal view returns (uint256) {
 uint256 total = IERC20(iOOKI).totalSupply();
 if(total != 0) return IERC20(OOKI).balanceOf(iOOKI).mul(1e50).div(total);
 return 0;
 }
 ...
 totalVotes = vestedBalanceForAmount( _vOOKIBalance, _vestingLastSync, proposal.proposalTime ).add(totalVotes);
 }
 ...
 userAltRewardsPerShare[msg.sender][SUSHI] = IStakingV2.AltRewardsUserInfo({
 ...
 userAltRewardsPerShare[msg.sender][SUSHI] = IStakingV2.AltRewardsUserInfo({
 rewardsPerShare: altRewardsPerShare[SUSHI], pendingRewards: _pendingSushiRewards(msg.sender)}
 );
 }
 ...
 userAltRewardsPerShare[_user][SUSHI] = IStakingV2.AltRewardsUserInfo({
 ...
 emit Stake( account, OOKI, account, amount );
 }
 modifier updateRewards(address account) {
 uint256 _ookiPerTokenStored = ookiPerTokenStored;
 uint256 _stableCoinPerTokenStored = stableCoinPerTokenStored;
 (uint256 ookiRewardsEarned, uint256 stableCoinRewardsEarned, uint256 ookiRewardsVesting, uint256 stableCoinRewardsVesting) = _earned( account, _ookiPerTokenStored, _stableCoinPerTokenStored );
 ...
 function earned(address account) external returns ( uint256 ookiRewardsEarned, uint256 stableCoinRewardsEarned, uint256 ookiRewardsVesting, uint256 stableCoinRewardsVesting, uint256 sushiRewardsEarned ) {
 ...
 uint256 pendingSushi = IMasterChefSushi(SUSHI_MASTERCHEF).pendingSushi(OOKI_ETH_SUSHI_MASTERCHEF_PID, address(this));
 sushiRewardsEarned = _pendingAltRewards( SUSHI, account, balanceOfByAsset(OOKI_ETH_LP, account), (_totalSupplyPerToken[OOKI_ETH_LP] != 0) ? pendingSushi.mul(1e12).div(_totalSupplyPerToken[OOKI_ETH_LP]) : 0 );
 }
 function _earned( address account, uint256 _ookiPerToken, uint256 _stableCoinPerToken ) internal returns ( uint256 ookiRewardsEarned, uint256 stableCoinRewardsEarned, uint256 ookiRewardsVesting, uint256 stableCoinRewardsVesting ) {
 ...
 function _syncVesting( address account, uint256 ookiRewardsEarned, uint256 stableCoinRewardsEarned, uint256 ookiRewardsVesting, uint256 stableCoinRewardsVesting ) internal view returns (uint256, uint256) {
 ...
 if (vBZRXBalance != 0) {
 rewardsVested = vBZRXBalance.mul(multiplier) .div(1e35);
 ookiRewardsEarned += rewardsVested;
 }
 }
 ...
 if (balance != 0) {
 vestingBalance = balance.mul(vBZRXWeightStored) .div(1e17);
 }
 ...
 id: proposalId, proposer: msg.sender, eta: 0, targets: targets, values: values, signatures: signatures, calldatas: calldatas, startBlock: startBlock, endBlock: endBlock, forVotes: 0, againstVotes: 0, abstainVotes: 0, canceled: false, executed: false }
 );
 ...
 uint256 totalSupply = IERC20(0x0De05F6447ab4D22c8827449EE4bA2D5C288379B) .totalSupply();
