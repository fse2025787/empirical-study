 ...
 require(_msgSender() == owner || ERC721Upgradeable.isApprovedForAll(owner, _msgSender()), "ERC721: approve caller is not owner nor approved for all" );
 _approve(to, tokenId);
 }
 ...
 bytes memory returndata = to.functionCall(abi.encodeWithSelector( IERC721ReceiverUpgradeable(to).onERC721Received.selector, _msgSender(), from, tokenId, _data ), "ERC721: transfer to non ERC721Receiver implementer");
 ...
 interface IVNFT {
 event TransferUnits( address indexed from, address indexed to, uint256 indexed tokenId, uint256 targetTokenId, uint256 transferUnits );
 ...
 event ApprovalUnits( address indexed approval, uint256 indexed tokenId, uint256 allowance );
 ...
 function tokensInSlot(uint256 slot) external view returns (uint256 tokenCount);
 ...
 function transferFrom( address from, address to, uint256 tokenId, uint256 units ) external returns (uint256 newTokenId);
 function safeTransferFrom( address from, address to, uint256 tokenId, uint256 units, bytes calldata data ) external returns (uint256 newTokenId);
 function transferFrom( address from, address to, uint256 tokenId, uint256 targetTokenId, uint256 units ) external;
 function safeTransferFrom( address from, address to, uint256 tokenId, uint256 targetTokenId, uint256 units, bytes calldata data ) external;
 }
 pragma solidity 0.7.6;
 interface IVNFTMetadata {
 ...
 function _initialize( string memory name_, string memory symbol_, uint8 unitDecimals_ ) internal virtual {
 ERC721Upgradeable.__ERC721_init(name_, symbol_);
 ERC165Upgradeable._registerInterface(type(IVNFT).interfaceId);
 _unitDecimals = unitDecimals_;
 }
 function _safeTransferUnitsFrom( address from_, address to_, uint256 tokenId_, uint256 targetTokenId_, uint256 transferUnits_, bytes memory data_ ) internal virtual {
 _transferUnitsFrom( from_, to_, tokenId_, targetTokenId_, transferUnits_ );
 require( _checkOnVNFTReceived( from_, to_, targetTokenId_, transferUnits_, data_ ), "to non VNFTReceiver implementer" );
 }
 function _transferUnitsFrom( address from_, address to_, uint256 tokenId_, uint256 targetTokenId_, uint256 transferUnits_ ) internal virtual {
 require(from_ == ownerOf(tokenId_), "source token owner mismatch");
 require(to_ != address(0), "transfer to the zero address");
 _beforeTransferUnits( from_, to_, tokenId_, targetTokenId_, transferUnits_ );
 if (_msgSender() != from_ && !isApprovedForAll(from_, _msgSender())) {
 _tokenApprovalUnits[tokenId_].allowances[ _msgSender() ] = _tokenApprovalUnits[tokenId_].allowances[_msgSender()].sub( transferUnits_, "transfer units exceeds allowance" );
 }
 _units[tokenId_] = _units[tokenId_].sub( transferUnits_, "transfer excess units" );
 ...
 emit TransferUnits( from_, to_, tokenId_, targetTokenId_, transferUnits_ );
 }
 ...
 function _split( uint256 tokenId_, uint256 newTokenId_, uint256 splitUnits_ ) internal virtual {
 require( _isApprovedOrOwner(_msgSender(), tokenId_), "VNFT: not owner nor approved" );
 ...
 address owner = ownerOf(tokenId_);
 _units[tokenId_] = _units[tokenId_].sub( burnUnits_, "burn excess units" );
 emit TransferUnits(owner, address(0), tokenId_, 0, burnUnits_);
 return _units[tokenId_];
 }
 function approve( address to_, uint256 tokenId_, uint256 allowance_ ) public virtual override {
 require(_msgSender() == ownerOf(tokenId_), "VNFT: only owner");
 _approveUnits(to_, tokenId_, allowance_);
 }
 function allowance(uint256 tokenId_, address spender_) public view virtual override returns (uint256) {
 return _tokenApprovalUnits[tokenId_].allowances[spender_];
 }
 function _approveUnits( address to_, uint256 tokenId_, uint256 allowance_ ) internal virtual {
 ...
 function unitsInSlot(uint256 slot_) public view override returns (uint256 units_) {
 ...
 function tokensInSlot(uint256 slot_) public view override returns (uint256) {
 return _slotTokens[slot_].length();
 }
 function tokenOfSlotByIndex(uint256 slot_, uint256 index_) public view override returns (uint256) {
 ...
 function _beforeTransferUnits( address from_, address to_, uint256 tokenId_, uint256 targetTokenId_, uint256 transferUnits_ ) internal virtual {
 }
 function _checkOnVNFTReceived( address from_, address to_, uint256 tokenId_, uint256 units_, bytes memory _data ) internal returns (bool) {
 if (!to_.isContract()) {
 return true;
 }
 bytes memory returndata = to_.functionCall( abi.encodeWithSelector( IVNFTReceiver(to_).onVNFTReceived.selector, _msgSender(), from_, tokenId_, units_, _data ), "non VNFTReceiver implementer" );
 bytes4 retval = abi.decode(returndata, (bytes4));
 ...
 function _initialize( string memory name_, string memory symbol_, uint8 unitDecimals_ ) internal override {
 ...
 function split(uint256 tokenId_, uint256[] calldata splitUnits_) public virtual override returns (uint256[] memory newTokenIds) {
 ...
 function merge(uint256[] calldata tokenIds_, uint256 targetTokenId_) public virtual override {
 ...
 function transferFrom( address from_, address to_, uint256 tokenId_, uint256 transferUnits_ ) public virtual override returns (uint256 newTokenId) {
 newTokenId = _generateTokenId();
 _transferUnitsFrom(from_, to_, tokenId_, newTokenId, transferUnits_);
 }
 function transferFrom( address from_, address to_, uint256 tokenId_, uint256 targetTokenId_, uint256 transferUnits_ ) public virtual override {
 require(_exists(targetTokenId_), "target token not exists");
 _transferUnitsFrom( from_, to_, tokenId_, targetTokenId_, transferUnits_ );
 }
 function safeTransferFrom( address from_, address to_, uint256 tokenId_, uint256 transferUnits_, bytes memory data_ ) public virtual override returns (uint256 newTokenId) {
 newTokenId = transferFrom(from_, to_, tokenId_, transferUnits_);
 require( _checkOnVNFTReceived(from_, to_, newTokenId, transferUnits_, data_), "to non VNFTReceiver" );
 return newTokenId;
 }
 function safeTransferFrom( address from_, address to_, uint256 tokenId_, uint256 targetTokenId_, uint256 transferUnits_, bytes memory data_ ) public virtual override {
 transferFrom(from_, to_, tokenId_, targetTokenId_, transferUnits_);
 require( _checkOnVNFTReceived( from_, to_, targetTokenId_, transferUnits_, data_ ), "to non VNFTReceiver" );
 }
 function _transferUnitsFrom( address from_, address to_, uint256 tokenId_, uint256 targetTokenId_, uint256 transferUnits_ ) internal virtual override {
 VNFTCoreV2._transferUnitsFrom( from_, to_, tokenId_, targetTokenId_, transferUnits_ );
 voucherSlotMapping[targetTokenId_] = voucherSlotMapping[tokenId_];
 }
 function _mint( address minter_, uint256 slot_, uint256 units_ ) internal virtual returns (uint256 tokenId) {
 ...
 function _slotOf(uint256 tokenId_) internal view virtual override returns (uint256) {
 ...
 event CreateSlot( uint256 indexed slot, address indexed issuer, address fundCurrency, uint128 lowestPrice, uint128 highestPrice, uint64 effectiveTime, uint64 maturity, CollateralType collateralType );
 event Mint( address indexed minter, uint256 indexed slot, uint256 totalValue );
 event Refund(uint256 indexed slot, address sender, uint256 refundAmount);
 event Withdraw( uint256 indexed slot, address sender, uint256 redeemCurrencyAmount, uint256 redeemUnderlyingTokenAmount );
 event SettlePrice(uint256 indexed slot, uint128 settlePrice);
 function mintWithUnderlyingToken( address minter_, uint256 slot_, uint256 tokenInAmount_ ) external returns (uint256 totalValue);
 function refund(uint256 slot_) external;
 function withdraw(uint256 slot_) external returns (uint256, uint256);
 function claim( uint256 slot_, address to_, uint256 claimValue_ ) external returns (uint256, uint256);
 ...
 event Claim ( uint256 indexed tokenId, address indexed to, uint256 claimUnits, uint256 claimCurrencyAmount, uint256 claimTokenAmount );
 function mint( address issuer_, address fundCurrency_, uint128 lowestPrice_, uint128 highestPrice_, uint64 effectiveTime_, uint64 maturity_, uint256 tokenInAmount_ ) external returns (uint256 slot, uint256 tokenId);
 ...
 function getSlot( address issuer_, address fundCurrency_, uint128 lowestPrice_, uint128 highestPrice_, uint64 effectiveTime_, uint64 maturity_, uint8 collateralType_ ) external view returns (uint256 slot);
 function getSlotDetail(uint256 slot_) external view returns (IConvertiblePool.SlotDetail memory);
 function getIssuerSlots(address issuer_) external view returns (uint256[] memory slots);
 function getSnapshot(uint256 tokenId_) external view returns (ConvertibleVoucherSnapshot memory);
 ...
 function doTransferOut( address underlying, address to, uint256 tokenId, uint256 units ) internal returns (uint256 newTokenId) {
 VNFTInterface token = VNFTInterface(underlying);
 newTokenId = token.safeTransferFrom( address(this), to, tokenId, units, "" );
 }
 function doTransferOut( address underlying, address to, uint256 tokenId, uint256 targetTokenId, uint256 units ) internal {
 VNFTInterface token = VNFTInterface(underlying);
 token.safeTransferFrom( address(this), to, tokenId, targetTokenId, units, "" );
 }
 }
 ...
 enum VoucherType {
 STANDARD_VESTING, FLEXIBLE_DATE_VESTING, BOUNDING }
 ...
 (uint256 year, uint256 month, uint256 day, uint256 hour, uint256 minute, uint256 second) = BokkyPooBahsDateTimeLibrary.timestampToDateTime(timestamp);
 return string( abi.encodePacked( year.toString(), '/', month < 10 ? '0' : '', month.toString(), '/', day < 10 ? '0' : '', day.toString(), ' ', hour < 10 ? '0' : '', hour.toString(), ':', minute < 10 ? '0' : '', minute.toString(), ':', second < 10 ? '0' : '', second.toString() ) );
 }
 ...
 return string( abi.encodePacked( year.toString(), '/', month < 10 ? '0' : '', month.toString(), '/', day < 10 ? '0' : '', day.toString() ) );
 }
 ...
 pragma solidity 0.7.6;
 interface ISolver {
 event SetOperationPaused ( address product, string operation, bool setPaused );
 ...
 function onVNFTReceived( address operator, address from, uint256 tokenId, uint256 units, bytes calldata data ) external returns (bytes4);
 }
 pragma solidity 0.7.6;
 ...
 pragma solidity 0.7.6;
 contract ConvertiblePool is IConvertiblePool, AdminControl, ReentrancyGuardUpgradeable {
 using SafeMathUpgradeable for uint256;
 using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;
 ...
 function initialize( address underlyingToken_, address oracle_, uint8 priceDecimals_, uint8 valueDecimals_ ) external initializer {
 AdminControl.__AdminControl_init(_msgSender());
 ...
 function createSlot( address issuer_, address fundCurrency_, uint128 lowestPrice_, uint128 highestPrice_, uint64 effectiveTime_, uint64 maturity_, uint8 collateralType_ ) external onlyVoucher returns (uint256 slot) {
 validateSlotParams( issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, collateralType_ );
 slot = getSlot( issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, collateralType_ );
 ...
 emit CreateSlot( slot, issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, CollateralType(collateralType_) );
 }
 function validateSlotParams( address issuer_, address fundCurrency_, uint128 lowestPrice_, uint128 highestPrice_, uint64 effectiveTime_, uint64 maturity_, uint8 collateralType_ ) public view {
 ...
 require( lowestPrice_ > 0 && lowestPrice_ < highestPrice_, "invalid price bounds" );
 require( effectiveTime_ > 0 && effectiveTime_ < maturity_, "invalid time setting" );
 }
 function mintWithUnderlyingToken( address minter_, uint256 slot_, uint256 tokenInAmount_ ) external override nonReentrant onlyVoucher returns (uint256 totalValue) {
 ...
 slotBalances[slot_][underlyingToken] = slotBalances[slot_][ underlyingToken ].add(tokenInAmount_);
 ERC20TransferHelper.doTransferIn( underlyingToken, minter_, tokenInAmount_ );
 ...
 slotBalances[slot_][slotDetail.fundCurrency] = slotBalances[slot_][ slotDetail.fundCurrency ].add(currencyAmount);
 ERC20TransferHelper.doTransferIn( slotDetail.fundCurrency, _msgSender(), currencyAmount );
 emit Refund(slot_, _msgSender(), currencyAmount);
 }
 function getWithdrawableAmount(uint256 slot_) public view returns (uint256 withdrawCurrencyAmount, uint256 withdrawTokenAmount) {
 SlotDetail storage slotDetail = _slotDetails[slot_];
 if ( block.timestamp >= slotDetail.maturity && !slotDetail.isIssuerWithdrawn ) {
 ...
 withdrawCurrencyAmount = slotDetail .totalValue .mul(10**currencyDecimals) .div(10**valueDecimals);
 ...
 withdrawTokenAmount = slotDetail .totalValue .div(slotDetail.lowestPrice) .sub(slotDetail.totalValue.div(settlePrice));
 }
 ...
 function withdraw(uint256 slot_) external override nonReentrant returns (uint256 withdrawCurrencyAmount, uint256 withdrawTokenAmount) {
 require(_issuerSlots[_msgSender()].contains(slot_), "only issuer");
 SlotDetail storage slotDetail = _slotDetails[slot_];
 ...
 (withdrawCurrencyAmount, withdrawTokenAmount) = getWithdrawableAmount( slot_ );
 slotDetail.isIssuerWithdrawn = true;
 if (withdrawCurrencyAmount > 0) {
 slotBalances[slot_][slotDetail.fundCurrency] = slotBalances[slot_][ slotDetail.fundCurrency ].sub(withdrawCurrencyAmount);
 ERC20TransferHelper.doTransferOut( slotDetail.fundCurrency, _msgSender(), withdrawCurrencyAmount );
 }
 if (withdrawTokenAmount > 0) {
 slotBalances[slot_][underlyingToken] = slotBalances[slot_][ underlyingToken ].sub(withdrawTokenAmount);
 ERC20TransferHelper.doTransferOut( underlyingToken, _msgSender(), withdrawTokenAmount );
 }
 emit Withdraw( slot_, _msgSender(), withdrawCurrencyAmount, withdrawTokenAmount );
 }
 function claim( uint256 slot_, address to_, uint256 claimValue_ ) external override onlyVoucher nonReentrant returns (uint256 claimCurrencyAmount, uint256 claimTokenAmount) {
 SlotDetail storage slotDetail = _slotDetails[slot_];
 require(slotDetail.isValid, "invalid slot");
 uint128 settlePrice = slotDetail.settlePrice;
 if (settlePrice == 0) {
 ...
 uint256 reservedCurrencyAmount = slotBalances[slot_][ slotDetail.fundCurrency ];
 claimCurrencyAmount = claimValue_ .mul(10 ** ERC20Upgradeable(slotDetail.fundCurrency).decimals()) .div(10 ** valueDecimals);
 ...
 function getSettlePrice(uint256 slot_) public view override returns (uint128) {
 SlotDetail storage slotDetail = _slotDetails[slot_];
 ...
 function getSlot( address issuer_, address fundCurrency_, uint128 lowestPrice_, uint128 highestPrice_, uint64 effectiveTime_, uint64 maturity_, uint8 collateralType_ ) public view returns (uint256) {
 return uint256( keccak256( abi.encode( underlyingToken, underlyingVestingVoucher, issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, collateralType_ ) ) );
 }
 ...
 function setUnderlyingVestingVoucher(address underlyingVestingVoucher_) external onlyAdmin {
 underlyingVestingVoucher = underlyingVestingVoucher_;
 }
 function setFundCurrency(address fundCurrency_, bool enable_) external onlyAdmin {
 ...
 function initialize( address convertiblePool_, address voucherDescriptor_, address solver_, uint8 unitDecimals_, string calldata name_, string calldata symbol_ ) external initializer {
 ...
 function mint( address issuer_, address fundCurrency_, uint128 lowestPrice_, uint128 highestPrice_, uint64 effectiveTime_, uint64 maturity_, uint256 tokenInAmount_ ) external override nonReentrant returns (uint256 slot, uint256 tokenId) {
 uint256 err = solver.operationAllowed( "mint", abi.encode( _msgSender(), issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, tokenInAmount_ ) );
 require(err == 0, "Solver: not allowed");
 slot = getSlot( issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, 0 );
 if (!getSlotDetail(slot).isValid) {
 convertiblePool.createSlot( issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, 0 );
 }
 ...
 solver.operationVerify( "mint", abi.encode(_msgSender(), issuer_, slot, tokenId, units) );
 }
 ...
 uint256 err = solver.operationAllowed( "claim", abi.encode(_msgSender(), tokenId_, to_, claimUnits_) );
 require(err == 0, "Solver: not allowed");
 (uint256 claimCurrencyAmount, uint256 claimTokenAmount) = convertiblePool.claim(voucherSlotMapping[tokenId_], to_, claimUnits_);
 ...
 solver.operationVerify( "claim", abi.encode(_msgSender(), tokenId_, to_, claimUnits_) );
 emit Claim(tokenId_, to_, claimUnits_, claimCurrencyAmount, claimTokenAmount);
 }
 function getSlot( address issuer_, address fundCurrency_, uint128 lowestPrice_, uint128 highestPrice_, uint64 effectiveTime_, uint64 maturity_, uint8 collateralType_ ) public view override returns (uint256) {
 return convertiblePool.getSlot( issuer_, fundCurrency_, lowestPrice_, highestPrice_, effectiveTime_, maturity_, collateralType_ );
 }
 ...
 function getSnapshot(uint256 tokenId_) public override view returns (ConvertibleVoucherSnapshot memory snapshot) {
 ...
 function getPriceOfTokenId(address voucher_, uint256 tokenId_) external view returns (int256 price_);
 function getPriceOfMaturity(address voucher_, uint64 maturity_) external view returns (int256 price_);
 }
 pragma solidity 0.7.6;
 ...
 pragma solidity 0.7.6;
 interface IICToken {
 function mint( uint64 term, uint256 amount, uint64[] calldata maturities, uint32[] calldata percentages, string memory originalInvestor ) external returns (uint256 slot, uint256 tokenId);
 ...
 constructor( string[] memory linearBgColors_, string[] memory onetimeBgColors_, string[] memory stagedBgColors_ ) {
 __AdminControl_init(_msgSender());
 setVoucherBgColors(address(0), linearBgColors_, onetimeBgColors_, stagedBgColors_);
 }
 function setVoucherBgColors( address voucher_, string[] memory linearBgColors_, string[] memory onetimeBgColors_, string[] memory stagedBgColors_ ) public onlyAdmin {
 ...
 function generateSVG(address voucher_, uint256 tokenId_) external virtual override view returns (string memory) {
 ...
 function _generateSVG(SVGParams memory params) internal virtual view returns (string memory) {
 return string( abi.encodePacked( '<svg width="600px" height="400px" viewBox="0 0 600 400" xmlns="http: _generateDefs(params), '<g stroke-width="1" fill="none" fill-rule="evenodd" font-family="Arial">', _generateBackground(), _generateTitle(params), _generateMaturity(params), _generatePriceRange(params), '</g>', '</svg>' ) );
 }
 function _generateDefs(SVGParams memory params) internal virtual view returns (string memory) {
 string memory color0 = voucherBgColors[params.voucher][1].length > 0 ? voucherBgColors[params.voucher][1][0] : voucherBgColors[address(0)][1][0];
 string memory color1 = voucherBgColors[params.voucher][1].length > 1 ? voucherBgColors[params.voucher][1][1] : voucherBgColors[address(0)][1][1];
 ...
 return string( abi.encodePacked( '<text font-size="12" fill="#FFFFFF">', '<tspan x="40" y="365">Maturity Date: ', uint256(params.maturity).dateToString(), '</tspan>', '</text>' ) );
 }
 ...
 bytes memory minAmountStr = minTokenAmount < (10 ** (params.underlyingTokenDecimals + 6)) ? _formatValue(minTokenAmount, params.underlyingTokenDecimals) : abi.encodePacked( _formatValue(minTokenAmount, params.underlyingTokenDecimals + 6), "M" );
 bytes memory maxAmountStr = maxTokenAmount < (10 ** (params.underlyingTokenDecimals + 6)) ? _formatValue(maxTokenAmount, params.underlyingTokenDecimals) : abi.encodePacked( _formatValue(maxTokenAmount, params.underlyingTokenDecimals + 6), "M" );
