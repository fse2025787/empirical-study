 ...
 function initialize( string memory _name, string memory _symbol, string memory _baseTokenURI, address _owner ) public initializer {
 ...
 if ( (uint8(bDid[bDid.length - 7]) != 46) || (uint8(bDid[bDid.length - 6]) != 101) || (uint8(bDid[bDid.length - 5]) != 116) || (uint8(bDid[bDid.length - 4]) != 104) || (uint8(bDid[bDid.length - 3]) != 46) || (uint8(bDid[bDid.length - 2]) != 113) || (uint8(bDid[bDid.length - 1]) != 112) ) {
 return false;
 }
 return true;
 }
