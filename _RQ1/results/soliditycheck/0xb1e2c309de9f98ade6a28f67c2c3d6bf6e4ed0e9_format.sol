 ...
 function getProxiesAndCheckDSGuard(address _user) public view returns (ProxyData[] memory proxies) {
 (address mcdProxy, address[] memory otherProxies) = registry.getAllProxies(_user);
 proxies = new ProxyData[](otherProxies.length + 1);
 proxies[0] = ProxyData({
 proxy: mcdProxy, correctDSGuardOwner: checkDSGuardOwner(mcdProxy), isUserOwner: isUserProxyOwner(_user, mcdProxy) }
 );
 for (uint256 i = 0; i < otherProxies.length; ++i) {
 proxies[i + 1] = ProxyData({
 proxy: otherProxies[i], correctDSGuardOwner: checkDSGuardOwner(otherProxies[i]), isUserOwner: isUserProxyOwner(_user, otherProxies[i]) }
 );
 }
 }
 }
