 ...
 event Minted( uint256 tokenId, address owner, address property, uint256 amount, uint256 price );
 event Updated( uint256 tokenId, uint256 amount, uint256 price, uint256 cumulativeReward, uint256 pendingReward );
 function initialize(address _config) external;
 function mint( address _owner, address _property, uint256 _amount, uint256 _price ) external returns (uint256);
 function update( uint256 _tokenId, uint256 _amount, uint256 _price, uint256 _cumulativeReward, uint256 _pendingReward ) external returns (bool);
 function positions(uint256 _tokenId) external view returns ( address, uint256, uint256, uint256, uint256 );
 function rewards(uint256 _tokenId) external view returns ( uint256, uint256, uint256 );
 function positionsOfProperty(address _property) external view returns (uint256[] memory);
 function positionsOfOwner(address _owner) external view returns (uint256[] memory);
 }
 pragma solidity 0.5.17;
 library Decimals {
 using SafeMath for uint256;
 uint120 private constant BASIS_VAKUE = 1000000000000000000;
 function outOf(uint256 _a, uint256 _b) internal pure returns (uint256 result) {
 if (_a == 0) {
 return 0;
 }
 ...
 function setStorageValue( address _property, address _sender, uint256 _value ) internal {
 ...
 function getStorageValueKey(address _property, address _sender) private pure returns (bytes32) {
 return keccak256(abi.encodePacked("_value", _property, _sender));
 }
 function setStoragePropertyValue(address _property, uint256 _value) internal {
 ...
 function getStoragePropertyValueKey(address _property) private pure returns (bytes32) {
 return keccak256(abi.encodePacked("_propertyValue", _property));
 }
 function setStorageInterestPrice(address _property, uint256 _value) internal {
 ...
 function getStorageInterestPriceKey(address _property) private pure returns (bytes32) {
 return keccak256(abi.encodePacked("_interestTotals", _property));
 }
 function setStorageLastInterestPrice( address _property, address _user, uint256 _value ) internal {
 eternalStorage().setUint( getStorageLastInterestPriceKey(_property, _user), _value );
 }
 function getStorageLastInterestPrice(address _property, address _user) public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastInterestPriceKey(_property, _user) );
 }
 function getStorageLastInterestPriceKey(address _property, address _user) private pure returns (bytes32) {
 return keccak256( abi.encodePacked("_lastLastInterestPrice", _property, _user) );
 }
 function setStorageLastSameRewardsAmountAndBlock( uint256 _amount, uint256 _block ) internal {
 uint256 record = _amount.mul(BASIS).add(_block);
 eternalStorage().setUint( getStorageLastSameRewardsAmountAndBlockKey(), record );
 }
 function getStorageLastSameRewardsAmountAndBlock() public view returns (uint256 _amount, uint256 _block) {
 uint256 record = eternalStorage().getUint( getStorageLastSameRewardsAmountAndBlockKey() );
 uint256 amount = record.div(BASIS);
 uint256 blockNumber = record.sub(amount.mul(BASIS));
 return (amount, blockNumber);
 }
 function getStorageLastSameRewardsAmountAndBlockKey() private pure returns (bytes32) {
 ...
 eternalStorage().setUint( getStorageCumulativeGlobalRewardsKey(), _value );
 }
 ...
 function getStorageCumulativeGlobalRewardsKey() private pure returns (bytes32) {
 return keccak256(abi.encodePacked("_cumulativeGlobalRewards"));
 }
 function setStoragePendingInterestWithdrawal( address _property, address _user, uint256 _value ) internal {
 eternalStorage().setUint( getStoragePendingInterestWithdrawalKey(_property, _user), _value );
 }
 function getStoragePendingInterestWithdrawal( address _property, address _user ) public view returns (uint256) {
 return eternalStorage().getUint( getStoragePendingInterestWithdrawalKey(_property, _user) );
 }
 function getStoragePendingInterestWithdrawalKey( address _property, address _user ) private pure returns (bytes32) {
 return keccak256( abi.encodePacked("_pendingInterestWithdrawal", _property, _user) );
 }
 ...
 function setStorageLastStakedInterestPrice( address _property, address _user, uint256 _value ) internal {
 eternalStorage().setUint( getStorageLastStakedInterestPriceKey(_property, _user), _value );
 }
 function getStorageLastStakedInterestPrice(address _property, address _user) public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastStakedInterestPriceKey(_property, _user) );
 }
 function getStorageLastStakedInterestPriceKey( address _property, address _user ) private pure returns (bytes32) {
 return keccak256( abi.encodePacked("_lastStakedInterestPrice", _property, _user) );
 }
 function setStorageLastStakesChangedCumulativeReward(uint256 _value) internal {
 eternalStorage().setUint( getStorageLastStakesChangedCumulativeRewardKey(), _value );
 }
 function getStorageLastStakesChangedCumulativeReward() public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastStakesChangedCumulativeRewardKey() );
 }
 function getStorageLastStakesChangedCumulativeRewardKey() private pure returns (bytes32) {
 return keccak256(abi.encodePacked("_lastStakesChangedCumulativeReward"));
 }
 function setStorageLastCumulativeHoldersRewardPrice(uint256 _holders) internal {
 eternalStorage().setUint( getStorageLastCumulativeHoldersRewardPriceKey(), _holders );
 }
 function getStorageLastCumulativeHoldersRewardPrice() public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastCumulativeHoldersRewardPriceKey() );
 }
 function getStorageLastCumulativeHoldersRewardPriceKey() private pure returns (bytes32) {
 ...
 eternalStorage().setUint( getStorageLastCumulativeInterestPriceKey(), _interest );
 }
 function getStorageLastCumulativeInterestPrice() public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastCumulativeInterestPriceKey() );
 }
 function getStorageLastCumulativeInterestPriceKey() private pure returns (bytes32) {
 return keccak256(abi.encodePacked("0lastCumulativeInterestPrice"));
 }
 function setStorageLastCumulativeHoldersRewardAmountPerProperty( address _property, uint256 _value ) internal {
 eternalStorage().setUint( getStorageLastCumulativeHoldersRewardAmountPerPropertyKey( _property ), _value );
 }
 function getStorageLastCumulativeHoldersRewardAmountPerProperty( address _property ) public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastCumulativeHoldersRewardAmountPerPropertyKey( _property ) );
 }
 function getStorageLastCumulativeHoldersRewardAmountPerPropertyKey( address _property ) private pure returns (bytes32) {
 return keccak256( abi.encodePacked( "0lastCumulativeHoldersRewardAmountPerProperty", _property ) );
 }
 function setStorageLastCumulativeHoldersRewardPricePerProperty( address _property, uint256 _price ) internal {
 eternalStorage().setUint( getStorageLastCumulativeHoldersRewardPricePerPropertyKey(_property), _price );
 }
 function getStorageLastCumulativeHoldersRewardPricePerProperty( address _property ) public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastCumulativeHoldersRewardPricePerPropertyKey( _property ) );
 }
 function getStorageLastCumulativeHoldersRewardPricePerPropertyKey( address _property ) private pure returns (bytes32) {
 return keccak256( abi.encodePacked( "0lastCumulativeHoldersRewardPricePerProperty", _property ) );
 }
 ...
 eternalStorage().setUint( getStorageCumulativeHoldersRewardCapKey(), _value );
 }
 function getStorageCumulativeHoldersRewardCap() public view returns (uint256) {
 return eternalStorage().getUint(getStorageCumulativeHoldersRewardCapKey());
 }
 function getStorageCumulativeHoldersRewardCapKey() private pure returns (bytes32) {
 ...
 eternalStorage().setUint( getStorageLastCumulativeHoldersPriceCapKey(), _value );
 }
 function getStorageLastCumulativeHoldersPriceCap() public view returns (uint256) {
 return eternalStorage().getUint( getStorageLastCumulativeHoldersPriceCapKey() );
 }
 function getStorageLastCumulativeHoldersPriceCapKey() private pure returns (bytes32) {
 return keccak256(abi.encodePacked("_lastCumulativeHoldersPriceCap"));
 }
 function setStorageInitialCumulativeHoldersRewardCap( address _property, uint256 _value ) internal {
 eternalStorage().setUint( getStorageInitialCumulativeHoldersRewardCapKey(_property), _value );
 }
 function getStorageInitialCumulativeHoldersRewardCap(address _property) public view returns (uint256) {
 return eternalStorage().getUint( getStorageInitialCumulativeHoldersRewardCapKey(_property) );
 }
 function getStorageInitialCumulativeHoldersRewardCapKey(address _property) private pure returns (bytes32) {
 return keccak256( abi.encodePacked( "_initialCumulativeHoldersRewardCap", _property ) );
 }
 function setStorageFallbackInitialCumulativeHoldersRewardCap(uint256 _value) internal {
 eternalStorage().setUint( getStorageFallbackInitialCumulativeHoldersRewardCapKey(), _value );
 }
 function getStorageFallbackInitialCumulativeHoldersRewardCap() public view returns (uint256) {
 return eternalStorage().getUint( getStorageFallbackInitialCumulativeHoldersRewardCapKey() );
 }
 function getStorageFallbackInitialCumulativeHoldersRewardCapKey() private pure returns (bytes32) {
 return keccak256( abi.encodePacked("_fallbackInitialCumulativeHoldersRewardCap") );
 }
 }
 pragma solidity >=0.5.17;
 interface IDev {
 ...
 function calculateCumulativeRewardPrices() external view returns ( uint256 _reward, uint256 _holders, uint256 _interest, uint256 _holdersCap );
 ...
 constructor( address _config, address _devMinter, address _sTokensManager ) public UsingConfig(_config) {
 ...
 require( IERC721(sTokensManager).ownerOf(_tokenId) == msg.sender, "illegal sender" );
 _;
 }
 function depositToProperty(address _property, uint256 _amount) external onlyAuthenticatedProperty(_property) returns (uint256) {
 ...
 updateValues( true, _property, _amount, RewardPrices(reward, holders, interest, holdersCap) );
 ...
 function depositToPosition(uint256 _tokenId, uint256 _amount) external onlyPositionOwner(_tokenId) returns (bool) {
 require(_amount != 0, "illegal deposit amount");
 ISTokensManager sTokenManagerInstance = ISTokensManager(sTokensManager);
 ( address property, uint256 amount, uint256 price, uint256 cumulativeReward, uint256 pendingReward ) = sTokenManagerInstance.positions(_tokenId);
 ( uint256 withdrawable, RewardPrices memory prices ) = _calculateWithdrawableInterestAmount( property, amount, price, pendingReward );
 ...
 bool result = sTokenManagerInstance.update( _tokenId, amount.add(_amount), prices.interest, cumulativeReward.add(withdrawable), pendingReward.add(withdrawable) );
 require(result, "failed to update");
 emit Lockedup(msg.sender, property, _amount);
 return true;
 }
 function lockup( address _from, address _property, uint256 _value ) external onlyAuthenticatedProperty(_property) {
 require(msg.sender == config().token(), "this is illegal address");
 require(_value != 0, "illegal lockup value");
 RewardPrices memory prices = updatePendingInterestWithdrawal( _property, _from );
 updateValues4Legacy(true, _from, _property, _value, prices);
 emit Lockedup(_from, _property, _value);
 }
 function withdrawByPosition(uint256 _tokenId, uint256 _amount) external onlyPositionOwner(_tokenId) returns (bool) {
 ISTokensManager sTokenManagerInstance = ISTokensManager(sTokensManager);
 ( address property, uint256 amount, uint256 price, uint256 cumulativeReward, uint256 pendingReward ) = sTokenManagerInstance.positions(_tokenId);
 require(amount >= _amount, "insufficient tokens staked");
 (uint256 value, RewardPrices memory prices) = _withdrawInterest( property, amount, price, pendingReward );
 if (_amount != 0) {
 ...
 return sTokenManagerInstance.update( _tokenId, amount.sub(_amount), prices.interest, cumulative, 0 );
 }
 function withdraw(address _property, uint256 _amount) external {
 require( hasValue(_property, msg.sender, _amount), "insufficient tokens staked" );
 ...
 ( , uint256 holdersPrice, , uint256 cCap ) = calculateCumulativeRewardPrices();
 setStorageCumulativeHoldersRewardCap(cCap);
 setStorageLastCumulativeHoldersPriceCap(holdersPrice);
 setStorageCap(_cap);
 emit UpdateCap(_cap);
 }
 function _calculateLatestCap(uint256 _holdersPrice) private view returns (uint256) {
 uint256 cCap = getStorageCumulativeHoldersRewardCap();
 uint256 lastHoldersPrice = getStorageLastCumulativeHoldersPriceCap();
 uint256 additionalCap = _holdersPrice.sub(lastHoldersPrice).mul( getStorageCap() );
 return cCap.add(additionalCap);
 }
 function beforeStakesChanged(address _property, RewardPrices memory _prices) private {
 uint256 cHoldersReward = _calculateCumulativeHoldersRewardAmount( _prices.holders, _property );
 if ( getStorageLastCumulativeHoldersRewardPricePerProperty(_property) == 0 && getStorageInitialCumulativeHoldersRewardCap(_property) == 0 && getStoragePropertyValue(_property) == 0 ) {
 setStorageInitialCumulativeHoldersRewardCap( _property, _prices.holdersCap );
 }
 ...
 setStorageLastCumulativeHoldersRewardAmountPerProperty( _property, cHoldersReward );
 setStorageLastCumulativeHoldersRewardPricePerProperty( _property, _prices.holders );
 ...
 uint256 price = allStakes > 0 ? mReward.sub(lastReward).div(allStakes) : 0;
 uint256 holdersShare = IPolicy(config().policy()).holdersShare( price, allStakes );
 ...
 function _calculateCumulativeHoldersRewardAmount( uint256 _holdersPrice, address _property ) private view returns (uint256) {
 (uint256 cHoldersReward, uint256 lastReward) = ( getStorageLastCumulativeHoldersRewardAmountPerProperty(_property), getStorageLastCumulativeHoldersRewardPricePerProperty(_property) );
 uint256 additionalHoldersReward = _holdersPrice.sub(lastReward).mul( getStoragePropertyValue(_property) );
 ...
 function calculateRewardAmount(address _property) external view returns (uint256, uint256) {
 ( , uint256 holders, , uint256 holdersCap ) = calculateCumulativeRewardPrices();
 uint256 initialCap = _getInitialCap(_property);
 uint256 capValue = holdersCap.sub(initialCap);
 return ( _calculateCumulativeHoldersRewardAmount(holders, _property), capValue );
 }
 function _getInitialCap(address _property) private view returns (uint256) {
 uint256 initialCap = getStorageInitialCumulativeHoldersRewardCap( _property );
 if (initialCap > 0) {
 return initialCap;
 }
 if ( getStorageLastCumulativeHoldersRewardPricePerProperty(_property) > 0 || getStoragePropertyValue(_property) > 0 ) {
 ...
 function dry() private view returns (uint256 _nextRewards, uint256 _amount) {
 uint256 rewardsAmount = IAllocator(config().allocator()) .calculateMaxRewardsPerBlock();
 ( uint256 lastAmount, uint256 lastBlock ) = getStorageLastSameRewardsAmountAndBlock();
 uint256 lastMaxRewards = lastAmount == rewardsAmount ? rewardsAmount : lastAmount;
 uint256 blocks = lastBlock > 0 ? block.number.sub(lastBlock) : 0;
 uint256 additionalRewards = lastMaxRewards.mul(blocks);
 uint256 nextRewards = getStorageCumulativeGlobalRewards().add( additionalRewards );
 return (nextRewards, rewardsAmount);
 }
 function _calculateInterestAmount(uint256 _amount, uint256 _price) private view returns ( uint256 amount_, uint256 interestPrice_, RewardPrices memory prices_ ) {
 ( uint256 reward, uint256 holders, uint256 interest, uint256 holdersCap ) = calculateCumulativeRewardPrices();
 uint256 result = interest >= _price ? interest.sub(_price).mul(_amount).divBasis() : 0;
 return ( result, interest, RewardPrices(reward, holders, interest, holdersCap) );
 }
 function _calculateInterestAmount4Legacy(address _property, address _user) private view returns ( uint256 _amount, uint256 _interestPrice, RewardPrices memory _prices ) {
 uint256 lockedUpPerAccount = getStorageValue(_property, _user);
 uint256 lastInterest = getStorageLastStakedInterestPrice( _property, _user );
 ( uint256 reward, uint256 holders, uint256 interest, uint256 holdersCap ) = calculateCumulativeRewardPrices();
 uint256 result = interest >= lastInterest ? interest.sub(lastInterest).mul(lockedUpPerAccount).divBasis() : 0;
 return ( result, interest, RewardPrices(reward, holders, interest, holdersCap) );
 }
 function _calculateWithdrawableInterestAmount( address _property, uint256 _amount, uint256 _price, uint256 _pendingReward ) private view returns (uint256 amount_, RewardPrices memory prices_) {
 if ( IMetricsGroup(config().metricsGroup()).hasAssets(_property) == false ) {
 return (0, RewardPrices(0, 0, 0, 0));
 }
 ( uint256 amount, , RewardPrices memory prices ) = _calculateInterestAmount(_amount, _price);
 uint256 withdrawableAmount = amount.add(_pendingReward);
 return (withdrawableAmount, prices);
 }
 function _calculateWithdrawableInterestAmount4Legacy( address _property, address _user ) private view returns (uint256 _amount, RewardPrices memory _prices) {
 if ( IMetricsGroup(config().metricsGroup()).hasAssets(_property) == false ) {
 return (0, RewardPrices(0, 0, 0, 0));
 }
 ...
 ( uint256 amount, , RewardPrices memory prices ) = _calculateInterestAmount4Legacy(_property, _user);
 uint256 withdrawableAmount = amount.add(pending).add(legacy);
 return (withdrawableAmount, prices);
 }
 function calculateWithdrawableInterestAmount( address _property, address _user ) external view returns (uint256) {
 (uint256 amount, ) = _calculateWithdrawableInterestAmount4Legacy( _property, _user );
 return amount;
 }
 function calculateWithdrawableInterestAmountByPosition(uint256 _tokenId) external view returns (uint256) {
 ISTokensManager sTokenManagerInstance = ISTokensManager(sTokensManager);
 ( address property, uint256 amount, uint256 price, , uint256 pendingReward ) = sTokenManagerInstance.positions(_tokenId);
 (uint256 result, ) = _calculateWithdrawableInterestAmount( property, amount, price, pendingReward );
 return result;
 }
 function _withdrawInterest( address _property, uint256 _amount, uint256 _price, uint256 _pendingReward ) private returns (uint256 value_, RewardPrices memory prices_) {
 ( uint256 value, RewardPrices memory prices ) = _calculateWithdrawableInterestAmount( _property, _amount, _price, _pendingReward );
 require( IDevMinter(devMinter).mint(msg.sender, value), "dev mint failed" );
 update();
 return (value, prices);
 }
 function _withdrawInterest4Legacy(address _property) private returns (RewardPrices memory _prices) {
 ( uint256 value, RewardPrices memory prices ) = _calculateWithdrawableInterestAmount4Legacy(_property, msg.sender);
 setStoragePendingInterestWithdrawal(_property, msg.sender, 0);
 setStorageLastStakedInterestPrice( _property, msg.sender, prices.interest );
 __updateLegacyWithdrawableInterestAmount(_property, msg.sender);
 require( IDevMinter(devMinter).mint(msg.sender, value), "dev mint failed" );
 update();
 return prices;
 }
 function updateValues4Legacy( bool _addition, address _account, address _property, uint256 _value, RewardPrices memory _prices ) private {
 ...
 function updateValues( bool _addition, address _property, uint256 _value, RewardPrices memory _prices ) private {
 ...
 function subValue( address _property, address _sender, uint256 _value ) private {
 uint256 value = getStorageValue(_property, _sender);
 value = value.sub(_value);
 setStorageValue(_property, _sender, value);
 }
 function hasValue( address _property, address _sender, uint256 _amount ) private view returns (bool) {
 uint256 value = getStorageValue(_property, _sender);
 ...
 function updatePendingInterestWithdrawal(address _property, address _user) private returns (RewardPrices memory _prices) {
 ( uint256 withdrawableAmount, RewardPrices memory prices ) = _calculateWithdrawableInterestAmount4Legacy(_property, _user);
 ...
 function __legacyWithdrawableInterestAmount( address _property, address _user ) private view returns (uint256) {
 ...
 function __updateLegacyWithdrawableInterestAmount( address _property, address _user ) private {
 ...
 function ___setFallbackInitialCumulativeHoldersRewardCap(uint256 _value) external onlyOwner {
 ...
 uint256 price = getStorageLastStakedInterestPrice( _property, msg.sender );
 uint256 pending = getStoragePendingInterestWithdrawal( _property, msg.sender );
 setStoragePendingInterestWithdrawal(_property, msg.sender, 0);
 setStorageValue(_property, msg.sender, 0);
 ISTokensManager sTokenManagerInstance = ISTokensManager(sTokensManager);
 uint256 tokenId = sTokenManagerInstance.mint( msg.sender, _property, amount, price );
 bool result = sTokenManagerInstance.update( tokenId, amount, price, 0, pending );
 require(result, "failed to update");
 return tokenId;
 }
 }
