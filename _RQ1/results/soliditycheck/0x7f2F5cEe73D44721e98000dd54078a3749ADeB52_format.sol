 ...
 _registerInterface( ERC1155ReceiverUpgradeable(address(0)).onERC1155Received.selector ^ ERC1155ReceiverUpgradeable(address(0)).onERC1155BatchReceived.selector );
 }
 uint256[50] private __gap;
 }
 pragma solidity >=0.6.0 <0.8.0;
 ...
 function mint( address to, uint64 _ammId, uint64 _periodIndex, uint32 _pairId, uint256 amount, bytes memory data ) external returns (uint256 id);
 ...
 EnumerableSetUpgradeable.UintSet internal activePools;
 mapping(uint256 => mapping(uint256 => uint256)) internal poolToPeriodId;
 ...
 function initialize( address _apw, address _lpToken, uint256 _apwPerBlock, uint256 _startBlock ) external initializer {
 ...
 function add( uint256 _allocPoint, uint256 _lpTokenId, IRewarder _rewarder, bool _withUpdate ) external onlyOwner {
 _add(_allocPoint, _lpTokenId, _rewarder, _withUpdate);
 }
 function _add( uint256 _allocPoint, uint256 _lpTokenId, IRewarder _rewarder, bool _withUpdate ) internal {
 uint64 ammId = lpToken.getAMMId(_lpTokenId);
 uint256 pairId = lpToken.getPairId(_lpTokenId);
 ...
 allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accAPWPerShare: 0, ammId: ammId, pairId: pairId }
 );
 activePools.add(_lpTokenId);
 poolToPeriodId[ammId][pairId] = periodId;
 }
 function set( uint256 _lpTokenId, uint256 _allocPoint, IRewarder _rewarder, bool overwrite, bool _withUpdate ) external onlyOwner {
 _set(_lpTokenId, _allocPoint, _rewarder, overwrite, _withUpdate);
 }
 function _set( uint256 _lpTokenId, uint256 _allocPoint, IRewarder _rewarder, bool overwrite, bool _withUpdate ) internal validPool(_lpTokenId) {
 if (_withUpdate) {
 massUpdatePools();
 }
 ...
 uint256 apwReward = (block.number.sub(pool.lastRewardBlock)).mul(apwPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
 ...
 uint256 apwReward = totalAllocPoint == 0 ? 0 : (block.number.sub(pool.lastRewardBlock)).mul(apwPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
 ...
 _set(previousLpTokenId, 0, IRewarder(0x0), false, false);
 uint256 newLpTokenId = IAMM(ammAddress).getLPTokenId(_ammId, lastPeriodId, _pairId);
 activePools.remove(previousLpTokenId);
 IRewarder rewarder = rewarders[previousLpTokenId];
 _add(nextUpgradeAllocPoint[previousLpTokenId], newLpTokenId, rewarder, false);
 ...
 uint256 accAPWPerShare = (user.periodId != 0 && user.periodId < periodOfToken) ? poolInfo[lastUserLpTokenId].accAPWPerShare : pool.accAPWPerShare;
 ...
 function getExpiredTokensInfo(address _user, uint256 _lpTokenId) external view returns ( uint256, uint256, uint256 );
 ...
 function swapExactAmountOut( uint256 _pairID, uint256 _tokenIn, uint256 _maxAmountIn, uint256 _tokenOut, uint256 _tokenAmountOut, address _to ) external returns (uint256 tokenAmountIn, uint256 spotPriceAfter);
 function createLiquidity(uint256 _pairID, uint256[2] memory _tokenAmounts) external;
 function addLiquidity( uint256 _pairID, uint256 _poolAmountOut, uint256[2] memory _maxAmountsIn ) external;
 function removeLiquidity( uint256 _pairID, uint256 _poolAmountIn, uint256[2] memory _minAmountsOut ) external;
 function joinSwapExternAmountIn( uint256 _pairID, uint256 _tokenIn, uint256 _tokenAmountIn, uint256 _minPoolAmountOut ) external returns (uint256 poolAmountOut);
 function joinSwapPoolAmountOut( uint256 _pairID, uint256 _tokenIn, uint256 _poolAmountOut, uint256 _maxAmountIn ) external returns (uint256 tokenAmountIn);
 function exitSwapPoolAmountIn( uint256 _pairID, uint256 _tokenOut, uint256 _poolAmountIn, uint256 _minAmountOut ) external returns (uint256 tokenAmountOut);
 function exitSwapExternAmountOut( uint256 _pairID, uint256 _tokenOut, uint256 _tokenAmountOut, uint256 _maxPoolAmountIn ) external returns (uint256 poolAmountIn);
 function setSwappingFees(uint256 _swapFee) external;
 function calcOutAndSpotGivenIn( uint256 _pairID, uint256 _tokenIn, uint256 _tokenAmountIn, uint256 _tokenOut, uint256 _minAmountOut ) external view returns (uint256 tokenAmountOut, uint256 spotPriceAfter);
 function calcInAndSpotGivenOut( uint256 _pairID, uint256 _tokenIn, uint256 _maxAmountIn, uint256 _tokenOut, uint256 _tokenAmountOut ) external view returns (uint256 tokenAmountIn, uint256 spotPriceAfter);
 function getSpotPrice( uint256 _pairID, uint256 _tokenIn, uint256 _tokenOut ) external view returns (uint256);
 ...
 function getLPTokenId( uint256 _ammId, uint256 _periodIndex, uint256 _pairID ) external pure returns (uint256);
 ...
 function createFYTDelegationTo( address _delegator, address _receiver, uint256 _amount ) external;
 function withdrawFYTDelegationFrom( address _delegator, address _receiver, uint256 _amount ) external;
 ...
 function getUserEarlyUnlockablePremium(address _user) external view returns (uint256 premiumLocked, uint256 amountRequired);
 ...
 pragma solidity 0.7.6;
 interface IRewarder {
 function onAPWReward( uint256 pid, address user, address recipient, uint256 apwAmount ) external;
