 ...
 pragma solidity >=0.8.6;
 contract METAV_STAKING is Ownable, IERC721Receiver {
 ...
 uint256 public lockupPeriod = 604800;
 IERC721 public NFT;
 ...
 require( stakedTokens[msg.sender].contains(tokenIds[i]), "ClaimRewards: token not staked" );
 require( stakes[tokenIds[i]].lockupExpires < block.timestamp, "ClaimRewards: lockup period not expired" );
 ...
 from = rewardChanges[i].block >= lastClaimedBlock ? rewardChanges[i].block : lastClaimedBlock;
 last = hasNext ? (rewardChanges[i+1].block >= lastClaimedBlock ? rewardChanges[i+1].block : from ) : block.number;
 ...
 for (uint256 i; i < tokenIds.length; i++) {
 require( stakedTokens[msg.sender].contains(tokenIds[i]), "Unstake: token not staked" );
