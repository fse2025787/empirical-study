 ...
 assembly {
 owner := and(owner, _BITMASK_ADDRESS) result := or(owner, or(shl(_BITPOS_START_TIMESTAMP, timestamp()), flags)) }
 }
 ...
 assembly {
 owner := and(owner, _BITMASK_ADDRESS) msgSender := and(msgSender, _BITMASK_ADDRESS) result := or(eq(msgSender, owner), eq(msgSender, approvedAddress)) }
 }
 ...
 assembly {
 approvedAddressSlot := tokenApproval.slot approvedAddress := sload(approvedAddressSlot) }
 }
 ...
 assembly {
 toMasked := and(to, _BITMASK_ADDRESS) log4( 0, 0, _TRANSFER_EVENT_SIGNATURE, 0, toMasked, startTokenId ) for {
 ...
 assembly {
 str := add(mload(0x40), 0x80) mstore(0x40, str) let end := str for {
 let temp := value }
 1 {
 }
 {
 str := sub(str, 1) mstore8(str, add(48, mod(temp, 10))) temp := div(temp, 10) if iszero(temp) {
 break }
 }
 let length := sub(end, str) str := sub(str, 0x20) mstore(str, length) }
 }
 }
 ...
 function explicitOwnershipsOf(uint256[] calldata tokenIds) external view virtual override returns (TokenOwnership[] memory) {
 unchecked {
 ...
 function tokensOfOwnerIn( address owner, uint256 start, uint256 stop ) external view virtual override returns (uint256[] memory) {
 unchecked {
 ...
 littelMamiPASS = IERC721Enumerable( 0x6F555695B057c081F0A0f7c1d1a854EF7e2FEAa2 );
 ...
 function adminMintBatch( address[] calldata _address, uint256[] calldata _num ) external onlyOwner {
 ...
 function setSleep(uint256 _sleepReturnTime, uint256 _returnUSDTAmount) external onlyOwner {
 ...
 function setContract(IERC721Enumerable _littelMamiPASS, IERC20 _USDT) external onlyOwner {
 littelMamiPASS = _littelMamiPASS;
 USDT = _USDT;
 }
 function setMaxMint(uint256 _maxPublicMint, uint256 _maxWlMint) external onlyOwner {
 ...
 function setMaxSupply(uint256 _maxPASSSupply, uint256 _maxWlSupply) external onlyOwner {
 maxPASSSupply = _maxPASSSupply;
 maxWlSupply = _maxWlSupply;
 }
 function setTime( uint256 _PASSStartTime, uint256 _PASSEndTime, uint256 _wlStartTime, uint256 _wlEndTime, uint256 _publicStartTime, uint256 _publicEndTime ) external onlyOwner {
 ...
 function setPrice( uint256 _publicPrice, uint256 _wlPrice, uint256 _PASSPrice, uint256 _partnerPrice ) external onlyOwner {
 ...
 require( ownerOf(_tokenId) == _msgSender(), "313 RABBIT : Not the owner" );
 ...
 if ( block.timestamp - status[_tokenId] >= sleepReturnTime && !returned[_tokenId] ) {
 ...
 require( block.timestamp >= PASSStartTime && block.timestamp <= PASSEndTime, "313 RABBIT : Not at mint time" );
 require( msg.value >= PASSPrice * _num, "313 RABBIT : Too little ether sent" );
 uint256 holdNum = littelMamiPASS.balanceOf(_msgSender());
 require(holdNum > 0, "313 RABBIT : HoldNum is zero");
 require( _num > 0 && _num <= holdNum, "313 RABBIT : Num must greater than 0 and lower than holdNum" );
 uint256 mintNum = 0;
 for (uint256 i = 0; i < holdNum; i++) {
 if (mintNum == _num) break;
 uint256 tokenId = littelMamiPASS.tokenOfOwnerByIndex( _msgSender(), i );
 if (!passUsed[tokenId]) {
 passUsed[tokenId] = true;
 mintNum++;
 }
 }
 require( mintNum == _num, "313 RABBIT : Exceeds the maximum eligible PASS mint" );
 require( PASSNum + mintNum <= maxPASSSupply, "313 RABBIT : Exceeds the maximum PASS supply number" );
 mint(_msgSender(), mintNum);
 PASSNum += mintNum;
 }
 function partnerMint(uint256 _num, bytes32[] memory _proof) external payable {
 require( block.timestamp >= wlStartTime && block.timestamp <= wlEndTime, "313 RABBIT : Not at mint time" );
 require( msg.value >= partnerPrice * _num, "313 RABBIT : Too little ether sent" );
 require( wlMined[_msgSender()] + _num <= maxWlMint, "313 RABBIT : Mint exceeds the maximum wl number" );
 require( _num > 0 && _num <= maxWlMint, "313 RABBIT : Num must greater than 0 and lower than maxWlMint" );
 require( wlNum + _num <= maxWlSupply, "313 RABBIT : Exceeds the maximum PASS supply number" );
 bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
 require( MerkleProof.verify(_proof, partnerRoot, leaf), "313 RABBIT : Merkle verification failed" );
 ...
 require( block.timestamp >= wlStartTime && block.timestamp <= wlEndTime, "313 RABBIT : Not at mint time" );
 require( msg.value >= wlPrice * _num, "313 RABBIT : Too little ether sent" );
 require( wlMined[_msgSender()] + _num <= maxWlMint, "313 RABBIT : Mint exceeds the maximum wl number" );
 require( _num > 0 && _num <= maxWlMint, "313 RABBIT : Num must greater than 0 and lower than maxWlMint" );
 require( wlNum + _num <= maxWlSupply, "313 RABBIT : Exceeds the maximum PASS supply number" );
 bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
 require( MerkleProof.verify(_proof, wlRoot, leaf), "313 RABBIT : Merkle verification failed" );
 mint(_msgSender(), _num);
 wlNum += _num;
 wlMined[_msgSender()] += _num;
 }
 function publicMint(uint256 _num) external payable {
 require( block.timestamp >= publicStartTime && block.timestamp <= publicEndTime, "313 RABBIT : Not at mint time" );
 require( publicMined[_msgSender()] + _num <= maxPublicMint, "313 RABBIT : Mint exceeds the maximum public number" );
 require( _num > 0 && _num <= maxPublicMint, "313 RABBIT : Num must greater than 0 and lower than maxPublicMint" );
 require( msg.value >= publicPrice * _num, "313 RABBIT : Too little ether sent" );
 mint(_msgSender(), _num);
 publicMined[_msgSender()] += _num;
 }
 function mint(address _address, uint256 _num) internal nonReentrant {
 require( totalSupply() + _num <= maxSupply, "313 RABBIT : Exceeds the maximum supply number" );
 _mint(_address, _num);
 }
 ...
 string memory imageURI = bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _tokenId.toString(), ".json")) : defaultURI;
 return imageURI;
 }
 }
 pragma solidity ^0.8.0;
 ...
 assembly {
 mstore(0x00, a) mstore(0x20, b) value := keccak256(0x00, 0x40) }
 }
 }
 pragma solidity ^0.8.4;
