 ...
 function getProgram() external view returns ( uint256, uint256, uint256, uint256 );
 ...
 constructor( IERC20 stakeToken, IERC20 rewardToken, uint256 startTime, uint256 endTime, uint256 rewardRate ) {
 ...
 function getProgram() external view override returns ( uint256, uint256, uint256, uint256 ) {
 ...
 require( address(token) != address(_stakeToken) || amount <= token.balanceOf(address(this)).sub(_totalStaked), "INVALID_AMOUNT" );
 ...
 return _rewardPerTokenStored.add( stakingEndTime.sub(stakingStartTime).mul(_rewardRate).mul(RATE_FACTOR).div(_totalStaked) );
 }
 function _pendingRewards(address account) private view returns (uint256) {
 return _stakes[account].mul(_rewardPerToken().sub(_stakerRewardPerTokenPaid[account])).div(RATE_FACTOR).add( _rewards[account] );
 }
