 ...
 require( hasRole(AccessControl.DEFAULT_ADMIN_ROLE, msg.sender), "grantMarketplaceAccess::Must be admin to call method" );
 ...
 function getMarketplaceFeePercentage() external view override returns (uint8) {
 ...
 function calculateMarketplaceFee(uint256 _amount) external view override returns (uint256) {
 return _amount.mul(marketplaceFeePercentage).div(100);
 }
 function getERC1155ContractPrimarySaleFeePercentage() external view override returns (uint8) {
 ...
 function setERC1155ContractPrimarySaleFeePercentage( uint8 _percentage ) external onlyOwner {
 require( _percentage <= 100, "setERC1155ContractPrimarySaleFeePercentage::_percentage must be <= 100" );
 primarySaleFees = _percentage;
 }
 function calculatePrimarySaleFee(uint256 _amount) external view override returns (uint256) {
 return _amount.mul(primarySaleFees).div(100);
 }
 function hasTokenSold(uint256 _tokenId) external view override returns (bool) {
 return soldTokens[_tokenId];
 }
 function markERC1155Token( uint256 _tokenId, bool _hasSold ) external override {
 require( hasRole(TOKEN_MARK_ROLE, msg.sender), "markERC1155Token::Must have TOKEN_MARK_ROLE role to call method" );
 soldTokens[_tokenId] = _hasSold;
 }
 function markTokensAsSold( uint256[] calldata _tokenIds ) external {
 require( hasRole(TOKEN_MARK_ROLE, msg.sender), "markERC1155Token::Must have TOKEN_MARK_ROLE role to call method" );
 require( _tokenIds.length <= 2000, "markTokensAsSold::Attempted to mark more than 2000 tokens as sold" );
