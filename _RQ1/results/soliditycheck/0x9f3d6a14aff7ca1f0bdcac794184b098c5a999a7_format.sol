 ...
 enum GameStatus {
 INITIAL, PENDING, RESOLVED, REJECTED }
 struct Game {
 uint8 status;
 uint8 wheelResult;
 address player;
 ...
 function queryGameHistory(uint gameId) external view returns( address player, uint256 targetBlock, uint8 status, uint8 wheelResult, bytes32 betTypes, bytes32 first16, bytes32 second16 ) {
