 ...
 string public _roomBaseUrl = "https: mapping(uint256 => string) private _roomMessage;
 ...
 if (randMsg % 17 == 3) return "LFG!";
 else if (randMsg % 7 == 3) return "WAGMI!";
 else return "gm!";
 }
 else {
 ...
 if (rand % 99 < 70) return rand % (tokenId - 1) + 1;
 else return 0;
 }
 else {
 return 0;
 ...
 if (rand % 99 < 50) return link2Id;
 else return 0;
 }
 }
 function getAssetLinks(uint256 tokenId) private pure returns (string memory) {
 string memory traitTypeJson = ', {
 "trait_type": "Linked", "value": "';
 if (getAssetLink1(tokenId) < 1) return '';
 ...
 if (starSeed % 47 == 1) return string(abi.encodePacked(traitTypeJson, 'Sirius"}
 '));
 if (starSeed % 11 == 1) return string(abi.encodePacked(traitTypeJson, 'Vega"}
 '));
 return '';
 }
 function renderStar(uint256 tokenId) private pure returns (string memory) {
 string memory starFirstPart = '<defs><linearGradient id="star" x1="100%" y1="100%"><stop offset="0%" stop-color="black" stop-opacity=".5"><animate attributeName="stop-color" values="black;
 black;
 black;
 black;
 gray;
 ';
 string memory starLastPart = ';
 gray;
 black;
 black;
 black;
 black" dur="3s" repeatCount="indefinite" /></stop></linearGradient></defs><g style="transform:translate(130px,244px)"><g style="transform:scale(0.1,0.1)"><path fill="url(#star)" d="M189.413,84c-36.913,0-37.328,38.157-37.328,38.157c0-33.181-36.498-38.157-36.498-38.157 c37.328,0,36.498-38.157,36.498-38.157C152.085,84,189.413,84,189.413,84z" /></g></g>';
 uint256 starSeed = random("star", tokenId);
 if (starSeed % 47 == 1) return string(abi.encodePacked(starFirstPart, 'aqua', starLastPart));
 if (starSeed % 11 == 1) return string(abi.encodePacked(starFirstPart, 'white', starLastPart));
 return '';
 }
 ...
 if (keySeed % 301 == 1) return string(abi.encodePacked(traitTypeJson, 'Rainbow Key"}
 '));
 if (keySeed % 161 == 1) return string(abi.encodePacked(traitTypeJson, 'Crystal Key"}
 '));
 if (keySeed % 59 == 1) return string(abi.encodePacked(traitTypeJson, 'Gold Key"}
 '));
 if (keySeed % 31 == 1) return string(abi.encodePacked(traitTypeJson, 'Silver Key"}
 '));
 if (keySeed % 11 == 1) return string(abi.encodePacked(traitTypeJson, 'Jade Key"}
 '));
 ...
 uint256 keySeed = random("key", tokenId);
 if (keySeed % 301 == 1) return string(abi.encodePacked('<defs><linearGradient id="rainbow" x1="100%" y1="100%"><stop offset="0%" stop-color="white" stop-opacity=".9"><animate attributeName="stop-color" values="white;
 red;
 orange;
 yellow;
 green;
 ...
 lightpurple;
 white;
 " dur="7s" repeatCount="indefinite" /></stop></linearGradient></defs>', keyFirstPart, 'url(#rainbow)', keyLastPart));
 if (keySeed % 161 == 1) return string(abi.encodePacked(keyFirstPart, '#afcfff', keyLastPart));
 if (keySeed % 59 == 1) return string(abi.encodePacked(keyFirstPart, '#ffff33', keyLastPart));
 if (keySeed % 31 == 1) return string(abi.encodePacked(keyFirstPart, '#dddddd', keyLastPart));
 if (keySeed % 11 == 1) return string(abi.encodePacked(keyFirstPart, '#66ff66', keyLastPart));
 ...
 if (link2Id > link1Id) return string(abi.encodePacked(description0, description1, link1description,' and ',link2description, '.'));
 else return string(abi.encodePacked(description0, description1, link2description,' and ',link1description, '.'));
 }
 else {
 ...
 black;
 ";
 if ( colorSeed % 7 == 2) return "black;
 green;
 black;
 ";
 if ( colorSeed % 7 == 1) return "black;
 blue;
 black;
 ";
 if ( colorSeed % 7 == 4) return "black;
 lightblue;
 black;
 ";
 if ( colorSeed % 7 == 5) return "black;
 red;
 purple;
 blue;
 black;
 ";
 if ( colorSeed % 7 == 6) return "black;
 blue;
 purple;
 blue;
 black;
 ...
 string memory traitTypeJson = ', {
 "trait_type": "Laser", "value": "';
 if (laserSeed % 251 == 2) return string(abi.encodePacked(traitTypeJson, 'Dual Green Lasers"}
 '));
 if (laserSeed % 167 == 2) return string(abi.encodePacked(traitTypeJson, 'Dual Red Lasers"}
 '));
 if (laserSeed % 71 == 2) return string(abi.encodePacked(traitTypeJson, 'Green Laser"}
 '));
 if (laserSeed % 31 == 2) return string(abi.encodePacked(traitTypeJson, 'Red Laser"}
 '));
 return '';
 }
 ...
 if (singleLaser) attribPyramidLasers = string(abi.encodePacked(attribPyramidLasersFirstPart, attribPyramidLasersEndingPart));
 if (dualLasers) attribPyramidLasers = string(abi.encodePacked(attribPyramidLasersFirstPart, attribPyramidLasersDoublePart, attribPyramidLasersEndingPart));
 return string(abi.encodePacked('<g clip-path="url(#b)"><path fill="000000" d="M0 0h290v500H0z" /><path fill="url(#backgroundGradient)" d="M0 0h290v500H0z" /><g style="filter:url(#d);
 transform:scale(2.9);
 ...
 string memory traitTypeJson = '{
 "trait_type": "';
 return string(abi.encodePacked(string(abi.encodePacked(traitTypeJson, 'Room Type", "value": "', getRoomType(tokenId), '"}
 , ')), string(abi.encodePacked(traitTypeJson, 'Room Theme", "value": "', getRoomTheme(tokenId), '"}
 ')), getAssetLinks(tokenId)));
 }
 ...
 -300" dur="15s" repeatCount="indefinite" /></text></g>'));
 ...
 string memory output = string(abi.encodePacked('data:application/json;
 base64,', Base64.encode(bytes(string(abi.encodePacked('{
 ', basicInfo, attributes,', "image": "data:image/svg+xml;
 base64,', Base64.encode(bytes(outputSVG)),'"}
 '))))));
 return output;
 }
