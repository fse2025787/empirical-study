 ...
 function initialize( address _armorMaster ) external override {
 ...
 function updatePlan(address[] calldata _protocols, uint256[] calldata _coverAmounts) external override checkExpiry(msg.sender) {
 ...
 function coverageLeft(address _protocol) public override view returns (uint256) {
 (, uint256 allowed) = _checkBuyerAllowed(_protocol);
 return allowed;
 }
 function _checkBuyerAllowed(address _protocol) internal view returns (uint256, uint256) {
 ...
 function checkCoverage(address _user, address _protocol, uint256 _hackTime, uint256 _amount) external view override returns(uint256 index, bool check) {
 ...
 function changePrice(address _protocol, uint256 _newPrice) external override onlyModule("STAKE") {
 nftCoverPrice[_protocol] = _newPrice;
 }
 function updateExpireTime(address _user, uint256 _expiry) external override onlyModule("BALANCE") {
 ...
 function _hashKey(address _user, uint256 _planIndex, uint256 _protoIndex) internal pure returns (bytes32) {
 return keccak256(abi.encodePacked("ARMORFI.PLAN.", _user, _planIndex, _protoIndex));
 }
 function adjustMarkup(uint256 _newMarkup) external onlyOwner {
 require(_newMarkup >= 100, "Markup must be at least 0 (100%).");
 markup = _newMarkup;
 }
 function adjustPercents(uint256 _newCorePercent, uint256 _newArShieldPercent, uint256 _newArShieldPlusPercent) external onlyOwner {
 ...
 function adjustShields(address[] calldata _shieldAddress, uint256[] calldata _shieldType) external onlyOwner {
