 ...
 constructor( IERC20 _acceptedToken, address _storeFeeAddresses, address[] memory _collectionAddresses, address[] memory _saleBeneficiaryAddresses, uint256[] memory _collectionFee, uint256[][] memory _collectionOptionIds, uint256[][] memory _collectionAvailableQtys, uint256[][] memory _collectionPrices ) public {
 ...
 require( acceptedToken.transferFrom(msg.sender, collection.saleBeneficiary, finalPrice-fee), "Transfering finalPrice to sale beneficiary failed" );
 require( acceptedToken.transferFrom(msg.sender, storeFeeAddresses, fee), "Transfering fee failed" );
 ...
 (bool success, bytes memory data) = address(_collectionAddress).staticcall( abi.encodeWithSelector( IERC721Collection(_collectionAddress).wearables.selector, _optionId ) );
 require(success, "Invalid wearable");
 return abi.decode(data, (string));
 }
 function collectionData(address _collectionAddress, uint256 _optionId) external view returns ( uint256 availableQty, uint256 price ) {
 ...
 function setCollectionData( address _collectionAddress, address _saleBeneficiaryAddress, uint256 _collectionFee, uint256[] calldata _collectionOptionIds, uint256[] calldata _collectionAvailableQtys, uint256[] calldata _collectionPrices ) external onlyOwner {
 ...
 function _setCollectionData( address _collectionAddress, address _saleBeneficiaryAddress, uint256 _collectionFee, uint256[] memory _collectionOptionIds, uint256[] memory _collectionAvailableQtys, uint256[] memory _collectionPrices ) internal {
 CollectionData storage collection = collectionsData[_collectionAddress];
 ...
 require( acceptedToken.balanceOf(_user) >= _price, "Insufficient funds" );
 require( acceptedToken.allowance(_user, address(this)) >= _price, "The contract is not authorized to use the accepted token on sender behalf" );
 }
 }
 pragma solidity >=0.6.0 <0.8.0;
 library SafeMath {
