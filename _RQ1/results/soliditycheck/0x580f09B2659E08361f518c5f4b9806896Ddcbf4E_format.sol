 ...
 bytes32 internal constant _DECIMALS_SLOT = 0x15b9fa1072bc4b2cdb762a49a2c7917b8b3af02283e37ffd41d0fccd4eef0d48;
 bytes32 internal constant _FUND_MANAGER_SLOT = 0x670552e214026020a9e6caa820519c7f879b21bd75b5571387d6a9cf8f94bd18;
 bytes32 internal constant _RELAYER_SLOT = 0x84e8c6b8f2281d51d9f683d351409724c3caa7848051aeb9d92c106ab36cc24c;
 bytes32 internal constant _PLATFORM_REWARDS_SLOT = 0x92260bfe68dd0f8a9f5439b75466781ba1ce44523ed1a3026a73eada49072e65;
 bytes32 internal constant _CHANGE_DELAY_SLOT = 0x0391715d0dd26b729c4ba34639ad5bdb0a7feb89f59a1e38f38485ea7f5a1583;
 ...
 bytes32 internal constant _NEXT_IMPLEMENTATION_SLOT = 0xa7ae0fa763ec3009113ccc5eb9089e1f0028607f5b8198c52cd42366c1ddb17b;
 bytes32 internal constant _NEXT_IMPLEMENTATION_TIMESTAMP_SLOT = 0x5e1f7083e1d90c44893f97806d0ec517436a58b85860b28247fd6fd56f5dc897;
 constructor() public {
 ...
 assert( _DECIMALS_SLOT == bytes32( uint256( keccak256("eip1967.mesh.finance.fundStorage.decimals") ) - 1 ) );
 assert( _FUND_MANAGER_SLOT == bytes32( uint256( keccak256( "eip1967.mesh.finance.fundStorage.fundManager" ) ) - 1 ) );
 assert( _RELAYER_SLOT == bytes32( uint256( keccak256("eip1967.mesh.finance.fundStorage.relayer") ) - 1 ) );
 assert( _PLATFORM_REWARDS_SLOT == bytes32( uint256( keccak256( "eip1967.mesh.finance.fundStorage.platformRewards" ) ) - 1 ) );
 assert( _CHANGE_DELAY_SLOT == bytes32( uint256( keccak256( "eip1967.mesh.finance.fundStorage.changeDelay" ) ) - 1 ) );
 ...
 assert( _NEXT_IMPLEMENTATION_SLOT == bytes32( uint256( keccak256( "eip1967.mesh.finance.fundStorage.nextImplementation" ) ) - 1 ) );
 assert( _NEXT_IMPLEMENTATION_TIMESTAMP_SLOT == bytes32( uint256( keccak256( "eip1967.mesh.finance.fundStorage.nextImplementationTimestamp" ) ) - 1 ) );
 }
 function initializeFundStorage( address _underlying, uint256 _underlyingUnit, uint8 _decimals, address _fundManager, address _relayer, address _platformRewards, uint256 _changeDelay ) public initializer {
 ...
 _setPlatformFee(0);
 _setMaxInvestmentInStrategies(9500);
 ...
 pragma solidity 0.6.12;
 contract Fund is ERC20Upgradeable, ReentrancyGuardUpgradeable, IFund, Governable, FundStorage {
 ...
 event StrategyRewards( address indexed strategy, uint256 profit, uint256 strategyCreatorFee );
 ...
 event StrategyAdded( address indexed strategy, uint256 weightage, uint256 performanceFeeStrategy );
 event StrategyWeightageUpdated( address indexed strategy, uint256 newWeightage );
 event StrategyPerformanceFeeUpdated( address indexed strategy, uint256 newPerformanceFeeStrategy );
 event StrategyRemoved(address indexed strategy);
 address internal constant ZERO_ADDRESS = address(0);
 uint256 internal constant MAX_BPS = 10000;
 uint256 internal constant SECS_PER_YEAR = 31556952;
 uint256 internal constant MAX_PLATFORM_FEE = 500;
 uint256 internal constant MAX_PERFORMANCE_FEE_FUND = 1000;
 uint256 internal constant MAX_PERFORMANCE_FEE_STRATEGY = 1000;
 uint256 internal constant MAX_ACTIVE_STRATEGIES = 10;
 ...
 function initializeFund( address _governance, address _underlying, string memory _name, string memory _symbol ) external initializer {
 ...
 FundStorage.initializeFundStorage( _underlying, _underlyingUnit, _decimals, _governance, _governance, _governance, _changeDelay );
 }
 ...
 require( (_governance() == msg.sender) || (_fundManager() == msg.sender), "Not governance or fund manager" );
 _;
 }
 modifier onlyFundManagerOrRelayer() {
 require( (_fundManager() == msg.sender) || (_relayer() == msg.sender), "Not fund manager or relayer" );
 _;
 }
 ...
 return totalSupply() == 0 ? _underlyingUnit() : _underlyingUnit().mul(underlyingBalanceWithInvestment()).div( totalSupply() );
 }
 ...
 function underlyingFromShares(uint256 _numShares) external view returns (uint256) {
 return _underlyingFromShares(_numShares);
 }
 function _underlyingFromShares(uint256 numShares) internal view returns (uint256) {
 return underlyingBalanceWithInvestment().mul(numShares).div(totalSupply());
 }
 ...
 function addStrategy( address newStrategy, uint256 weightage, uint256 performanceFeeStrategy ) external onlyFundManager {
 ...
 require( _getStrategyCount() + 1 <= MAX_ACTIVE_STRATEGIES, "Can not add more strategies" );
 require(weightage > 0, "The weightage should be greater than 0");
 uint256 totalWeightInStrategies = _totalWeightInStrategies().add(weightage);
 require( totalWeightInStrategies <= _maxInvestmentInStrategies(), "Total investment can't be above max allowed" );
 ...
 function updateStrategyWeightage( address activeStrategy, uint256 newWeightage ) external onlyFundManager {
 require( activeStrategy != ZERO_ADDRESS, "current strategy cannot be empty" );
 require( isActiveStrategy(activeStrategy), "This strategy is not active in this fund" );
 require(newWeightage > 0, "The weightage should be greater than 0");
 uint256 totalWeightInStrategies = _totalWeightInStrategies() .sub(strategies[activeStrategy].weightage) .add(newWeightage);
 require( totalWeightInStrategies <= _maxInvestmentInStrategies(), "Total investment can't be above max allowed" );
 _setTotalWeightInStrategies(totalWeightInStrategies);
 ...
 function updateStrategyPerformanceFee( address activeStrategy, uint256 newPerformanceFeeStrategy ) external onlyFundManager {
 require( activeStrategy != ZERO_ADDRESS, "current strategy cannot be empty" );
 require( isActiveStrategy(activeStrategy), "This strategy is not active in this fund" );
 ...
 emit StrategyPerformanceFeeUpdated( activeStrategy, newPerformanceFeeStrategy );
 }
 ...
 if ( IStrategy(strategy).investedUnderlyingBalance() > strategies[strategy].lastBalance ) {
 profit = IStrategy(strategy).investedUnderlyingBalance() - strategies[strategy].lastBalance;
 ...
 uint256 timeSinceLastHardwork = block.timestamp.sub(_lastHardworkTimestamp());
 uint256 totalInvested = _totalInvested();
 uint256 platformFee = totalInvested.mul(timeSinceLastHardwork).mul(_platformFee()).div( MAX_BPS * SECS_PER_YEAR );
 totalFee = totalFee.add(platformFee);
 uint256 totalFeeInShares = (totalFee == 0 || totalSupply() == 0) ? totalFee : totalFee.mul(totalSupply()).div( underlyingBalanceWithInvestment() );
 if (totalFeeInShares > 0) {
 _mint(address(this), totalFeeInShares);
 }
 for (uint256 i; i < totalStrategies; i++) {
 if (strategyCreatorFees[i] > 0) {
 uint256 strategyCreatorFeeInShares = totalFeeInShares.mul(strategyCreatorFees[i]).div(totalFee);
 if (strategyCreatorFeeInShares > 0) {
 address strategy = strategyList[i];
 IERC20(address(this)).safeTransfer( IStrategy(strategy).creator(), strategyCreatorFeeInShares );
 emit StrategyRewards( strategy, strategyProfits[i], strategyCreatorFeeInShares );
 }
 }
 }
 if (fundManagerFee > 0) {
 uint256 fundManagerFeeInShares = totalFeeInShares.mul(fundManagerFee).div(totalFee);
 if (fundManagerFeeInShares > 0) {
 address fundManagerRewards = (_fundManager() == _governance()) ? _platformRewards() : _fundManager();
 IERC20(address(this)).safeTransfer( fundManagerRewards, fundManagerFeeInShares );
 emit FundManagerRewards(profitToFund, fundManagerFeeInShares);
 }
 }
 if (platformFee > 0) {
 uint256 platformFeeInShares = totalFeeInShares.mul(platformFee).div(totalFee);
 emit PlatformRewards( totalInvested, timeSinceLastHardwork, platformFeeInShares );
 }
 ...
 function doHardWork() external nonReentrant whenStrategyDefined onlyFundManagerOrRelayer {
 ...
 uint256 totalInvested = _totalInvested();
 uint256 lastReserve = totalAccounted > 0 ? totalAccounted.sub(totalInvested) : 0;
 ...
 uint256 toMint = totalSupply() == 0 ? amount : amount.mul(totalSupply()).div( underlyingBalanceWithInvestment() );
 ...
 uint256 underlyingAmountToWithdraw = _underlyingFromShares(numberOfShares);
 ...
 uint256 missingforStrategy = (missing.mul(weightage).div(_totalWeightInStrategies())) .add(missingCarryOver);
 IStrategy(strategyList[i]).withdrawToFund( missingforStrategy );
 missingCarryOver = missingforStrategy .add(balanceBefore) .sub(underlyingBalanceInFund());
 }
 }
 ...
 require( newImplementation != ZERO_ADDRESS, "new implementation address can not be zero address" );
 require( newImplementation != IFundProxy(address(this)).implementation(), "new implementation address should not be same as current address" );
 _setNextImplementation(newImplementation);
 _setNextImplementationTimestamp(block.timestamp.add(_changeDelay()));
 }
 function shouldUpgrade() external view returns (bool, address) {
 return ( _nextImplementationTimestamp() != 0 && block.timestamp > _nextImplementationTimestamp() && _nextImplementation() != ZERO_ADDRESS, _nextImplementation() );
 }
 ...
 function setMaxInvestmentInStrategies(uint256 value) external onlyFundManager {
 ...
 require( _depositLimitTxMin() == 0 || limit > _depositLimitTxMin(), "Max limit greater than min limit" );
 ...
 require( _depositLimitTxMax() == 0 || limit < _depositLimitTxMax(), "Min limit greater than max limit" );
