 ...
 require(salesPeriods[salesPeriod.period].pay == address(0) || salesPeriods[salesPeriod.period].startTime >= block.timestamp, 'already active');
 ...
 require(salesPeriod.startTime <= block.timestamp && salesPeriod.endTime >= block.timestamp, 'not active');
