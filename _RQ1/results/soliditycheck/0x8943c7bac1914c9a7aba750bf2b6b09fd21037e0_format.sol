 ...
 function reserveLions(address _to, uint256 _reserveAmount) public onlyOwner {
 uint256 supply = totalSupply();
 require( _reserveAmount > 0 && _reserveAmount <= lionReserve, "Not enough reserve left for team" );
 ...
 require( numberOfTokens > 0 && numberOfTokens <= maxLionPurchase, "Can only mint 20 tokens at a time" );
 require( totalSupply().add(numberOfTokens) <= MAX_LIONS, "Purchase would exceed max supply of Lions" );
 require( msg.value >= lionPrice.mul(numberOfTokens), "Ether value sent is not correct" );
