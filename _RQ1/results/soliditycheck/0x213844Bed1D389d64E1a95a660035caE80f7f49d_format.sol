 ...
 if (lIndex > 0) {
 require( upperLimits[lIndex] > upperLimits[lIndex - 1], 'setAlphaTiers: upperLimits should be strictly increasing' );
 }
 else {
 ...
 if (0 < index) {
 require( tiers[index - 1] < upperLimit, 'updateAlphaTier: new upper limit must be more than the previous tier' );
 }
 if (index < tierCount.sub(1)) {
 require( upperLimit < tiers[index + 1], 'updateAlphaTier: new upper limit must be less than the next tier' );
 }
