 ...
 function updateMeta( address _tokenAddress, address _routerAddress, string memory _imageUrl ) internal {
 if (_tokenAddress != address(0)) {
 tokenMeta[_tokenAddress] = TokenMetadata({
 routerAddress: _routerAddress, imageUrl: _imageUrl, isAdded: true }
 );
 }
 }
 function updateMetaURL(address _tokenAddress, string memory _imageUrl) internal {
 ...
 (token, , start, end, cliffDuration, exchangeIdentifier, owner) = abi .decode( _encodedData, (IERC20, address, uint256, uint256, uint256, uint256, address) );
 address token0;
 address token1;
 ...
 address routerAddress;
 ( , , , , , , , token0URL, token1URL, inputTokenUrl, routerAddress, token0, token1 ) = abi.decode( _encodedData, ( IERC20, address, uint256, uint256, uint256, uint256, address, string, string, string, address, address, address ) );
 require( address(token) != address(0), "VestingContract::constructor: Invalid token" );
 ...
 require( end >= start.add(cliffDuration), "VestingContract::constructor: Start must be before end" );
 initialized = true;
 }
 function updateTokenURL(address _tokenAddress, string memory _tokenURL) external onlyOwner {
 ...
 TransferHelper.safeTransfer( address(_token), _recipient, _token.balanceOf(address(this)) );
 }
 function createVestingSchedules( address[] calldata _beneficiaries, uint256[] calldata _amounts ) external onlyOwner returns (bool) {
 ...
 function vestingScheduleForBeneficiary(address _beneficiary) external view returns ( uint256 _amount, uint256 _totalDrawn, uint256 _lastDrawnAt, uint256 _remainingBalance ) {
 return ( vestedAmount[_beneficiary], totalDrawn[_beneficiary], lastDrawnAt[_beneficiary], vestedAmount[_beneficiary].sub(totalDrawn[_beneficiary]) );
 }
 ...
 uint256 drawDownRate = (vestedAmount[_beneficiary].mul(1e18)).div( end.sub(start) );
