 ...
 event LogRebase( uint256 epoch, uint256 baseRate, uint256 exchangeRate, uint256 targetRate, int256 supplyDelta, uint256 time );
 event GetRebaseValues( uint256 exchangeRate, uint256 targetRate, int256 supplyDelta );
 ...
 emit GetRebaseValues( exchangeRate, targetRate, supplyDelta.div(dampener) );
 return ( exchangeRate, targetRate, supplyDelta.div(dampener) );
 }
 ...
 if(supplyDelta == 0){
 emit LogRebase( epoch, baseRate, exchangeRate, baseRate, supplyDelta, block.timestamp );
 return _totalSupply;
 }
 if(supplyDelta < 0){
 ...
 emit LogRebase( epoch, baseRate, exchangeRate, targetRate, supplyDelta, block.timestamp );
 oracle.sync();
 return _totalSupply;
 }
