 ...
 constructor( address _lp, address _sarco ) public {
 lp = ERC20(_lp);
 sarco = ERC20(_sarco);
 }
 function deposit( uint256 _totalRewards, uint256 _startTime, uint256 _endTime ) public onlyOwner {
 require( startTime == 0, "LiquidityFarming::deposit: already received deposit" );
 require( _startTime >= block.timestamp, "LiquidityFarming::deposit: start time must be in future" );
 require( _endTime > _startTime, "LiquidityFarming::deposit: end time must after start time" );
 require( sarco.balanceOf(address(this)) == _totalRewards, "LiquidityFarming::deposit: contract balance does not equal expected _totalRewards" );
 ...
 require( block.timestamp >= startTime, "LiquidityFarming::stake: staking isn't live yet" );
 require( sarco.balanceOf(address(this)) > 0, "LiquidityFarming::stake: no sarco balance" );
 if (firstStakeTime == 0) {
 firstStakeTime = block.timestamp;
 }
 else {
 require( block.timestamp < endTime, "LiquidityFarming::stake: staking is over" );
 }
 ...
 function withdraw(address to) public update nonReentrant returns (uint256 lpOut, uint256 reward) {
 ...
 function payout(address to) public update nonReentrant returns (uint256 reward) {
 require( block.timestamp < endTime, "LiquidityFarming::payout: withdraw instead" );
 ...
 function _applyReward(address account) private returns (uint256 lpOut, uint256 reward) {
 require( userStakeLp[account] > 0, "LiquidityFarming::_applyReward: no LP staked" );
 lpOut = userStakeLp[account];
 reward = lpOut .mul(_totalWeight.sub(_userWeighted[account])) .div(10**18) .add(_userAccumulated[account]);
 totalStakeLp = totalStakeLp.sub(lpOut);
 userStakeLp[account] = 0;
 _userAccumulated[account] = 0;
 }
 function rescueTokens( address tokenToRescue, address to, uint256 amount ) public onlyOwner nonReentrant {
 if (tokenToRescue == address(lp)) {
 require( amount <= lp.balanceOf(address(this)).sub(totalStakeLp), "LiquidityFarming::rescueTokens: that LP belongs to stakers" );
 }
 else if (tokenToRescue == address(sarco)) {
 if (totalStakers > 0) {
 require( amount <= sarco.balanceOf(address(this)).sub( totalRewards.sub(totalClaimedRewards) ), "LiquidityFarming::rescueTokens: that sarco belongs to stakers" );
 }
 }
