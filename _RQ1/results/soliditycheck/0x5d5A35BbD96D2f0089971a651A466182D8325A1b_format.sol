 ...
 constructor( address _owner, address _stakingToken, address _reflectionToken ) public Owned(_owner) {
 ...
 require( rewardData[_rewardsToken].rewardsDuration == 0, "Token was already added" );
 ...
 return rewardData[_rewardsToken].rewardPerTokenStored.add( lastTimeRewardApplicable(_rewardsToken) .sub(rewardData[_rewardsToken].lastUpdateTime) .mul(rewardData[_rewardsToken].rewardRate) .mul(1e18) .div(_totalSupply) );
 }
 ...
 return _balances[account] .mul( _reflectionPerToken.sub( _userReflectionPerTokenPaid[account] ) ) .div(1e32);
 }
 ...
 require( reward.div(rewardData[_rewardsToken].rewardsDuration) < MAXUINT256.div(1e18), "Reward too large, would lock" );
 ...
 if (reflection > 0) {
 _digits.claim();
 _reflectionPerToken = reflection.mul(1e32).div(_totalSupply).add( _reflectionPerToken );
 }
