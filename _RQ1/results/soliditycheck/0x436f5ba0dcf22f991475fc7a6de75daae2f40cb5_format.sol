 ...
 function exitTokens( address sender, address rootToken, bytes calldata logRLPList ) external;
 }
 pragma solidity >=0.7.6 <0.8.0;
 library RLPReader {
 ...
 else if (byte0 < _LIST_SHORT_START) return byte0 - (_STRING_LONG_START - 1) + 1;
 ...
 require( bytes32(logTopicRLPList[0].toUint()) == WITHDRAWN_EVENT_SIG, "Predicate: invalid signature" );
 ...
 function lockTokens( address depositor, address depositReceiver, address rootToken, bytes calldata depositData ) external override {
