 ...
 require( _acl.isConfigurator(msg.sender), Errors.ACL_CALLER_NOT_CONFIGURATOR );
 _;
 }
 function pause() external {
 require( _acl.isPausableAdmin(msg.sender), Errors.ACL_CALLER_NOT_PAUSABLE_ADMIN );
 _pause();
 }
 function unpause() external {
 require( _acl.isUnpausableAdmin(msg.sender), Errors.ACL_CALLER_NOT_PAUSABLE_ADMIN );
 _unpause();
 }
 }
 pragma solidity ^0.7.4;
 interface IAppCreditManager {
 function openCreditAccount( uint256 amount, address onBehalfOf, uint256 leverageFactor, uint256 referralCode ) external;
 function closeCreditAccount(address to, DataTypes.Exchange[] calldata paths) external;
 ...
 address public constant ethToUsdPriceFeed = 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419;
 ...
 require( contractsRegister.isCreditManager(creditManager), Errors.REGISTERED_CREDIT_ACCOUNT_MANAGERS_ONLY );
 _;
 }
 ...
 function bestUniPath( uint256 swapInterface, address router, uint256 swapType, address from, address to, uint256 amount, address[] memory tokens ) public returns (TradePath memory) {
 if (amount == 0) {
 return TradePath({
 path: new address[](3), rate: 0, expectedAmount: 0}
 );
 }
 address[] memory path = new address[](2);
 path[0] = from;
 path[1] = to;
 (uint256 bestAmount, bool best) = _getAmountsUni( swapInterface, router, swapType, path, amount, swapType == Constants.EXACT_INPUT ? 0 : Constants.MAX_INT );
 ...
 path[1] = tokens[i];
 (expectedAmount, best) = _getAmountsUni( swapInterface, router, swapType, path, amount, bestAmount );
 if (best) {
 ...
 bestRate = swapType == Constants.EXACT_INPUT ? Constants.WAD.mul(amount).div(bestAmount) : Constants.WAD.mul(bestAmount).div(amount);
 }
 return TradePath({
 rate: bestRate, path: bestPath, expectedAmount: bestAmount }
 );
 }
 function _getAmountsUni( uint256 swapInterface, address router, uint256 swapType, address[] memory path, uint256 amount, uint256 bestAmount ) internal returns (uint256, bool) {
 return swapInterface == Constants.UNISWAP_V2 ? _getAmountsV2( IUniswapV2Router02(router), swapType, path, amount, bestAmount ) : _getAmountsV3( IQuoter(router), swapType, path, amount, bestAmount );
 }
 function _getAmountsV2( IUniswapV2Router02 router, uint256 swapType, address[] memory path, uint256 amount, uint256 bestAmount ) internal view returns (uint256, bool) {
 uint256 expectedAmount;
 ...
 try router.getAmountsIn(amount, path) returns ( uint256[] memory amountsIn ) {
 ...
 if ( (swapType == Constants.EXACT_INPUT && expectedAmount > bestAmount) || (swapType == Constants.EXACT_OUTPUT && expectedAmount < bestAmount) ) {
 return (expectedAmount, true);
 }
 return (bestAmount, false);
 }
 function _getAmountsV3( IQuoter quoter, uint256 swapType, address[] memory path, uint256 amount, uint256 bestAmount ) internal returns (uint256, bool) {
 uint256 expectedAmount;
 if (swapType == Constants.EXACT_INPUT) {
 try quoter.quoteExactInput( convertPathToPathV3(path, swapType), amount ) returns (uint256 amountOut) {
 ...
 try quoter.quoteExactOutput( convertPathToPathV3(path, swapType), amount ) returns (uint256 amountIn) {
 ...
 function convertPathToPathV3(address[] memory path, uint256 swapType) public pure returns (bytes memory result) {
 uint24 fee = 3000;
 if (swapType == Constants.EXACT_INPUT) {
 ...
 function getClosurePaths( address router, address _creditManager, address borrower, address[] memory connectorTokens ) external registeredCreditManagerOnly(_creditManager) returns (TradePath[] memory result) {
 ICreditFilter creditFilter = ICreditFilter( ICreditManager(_creditManager).creditFilter() );
 result = new TradePath[](creditFilter.allowedTokensCount());
 address creditAccount = ICreditManager(_creditManager) .getCreditAccountOrRevert(borrower);
 address underlyingToken = creditFilter.underlyingToken();
 for (uint256 i = 0; i < creditFilter.allowedTokensCount(); i++) {
 (address token, uint256 balance, , ) = creditFilter .getCreditAccountTokenById(creditAccount, i);
 if (i == 0) {
 result[0] = TradePath({
 path: new address[](3), rate: Constants.WAD, expectedAmount: balance }
 );
 }
 else {
 result[i] = bestUniPath( Constants.UNISWAP_V2, router, Constants.EXACT_INPUT, token, underlyingToken, balance, connectorTokens );
 }
 }
 }
 function getPrices(address[] calldata tokens) external view returns (uint256[] memory prices) {
 ( , int256 ethPrice, , , ) = AggregatorV3Interface(ethToUsdPriceFeed).latestRoundData();
 prices = new uint256[](tokens.length);
 for (uint256 i = 0; i < tokens.length; i++) {
 uint256 decimals = ERC20(tokens[i]).decimals();
 prices[i] = priceOracle .convert(10**decimals, tokens[i], wethToken) .mul(uint256(ethPrice)) .div(Constants.WAD);
 }
 }
 }
 ...
 function removeLiquidity( address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, address to, uint deadline ) external override returns (uint amountA, uint amountB);
 ...
 function swapTokensForExactTokens( uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline ) external override returns (uint[] memory amounts);
 ...
 assembly {
 tempBytes := mload(0x40) let length := mload(_preBytes) mstore(tempBytes, length) let mc := add(tempBytes, 0x20) let end := add(mc, length) for {
 let cc := add(_preBytes, 0x20) }
 lt(mc, end) {
 mc := add(mc, 0x20) cc := add(cc, 0x20) }
 {
 mstore(mc, mload(cc)) }
 length := mload(_postBytes) mstore(tempBytes, add(length, mload(tempBytes))) mc := end end := add(mc, length) for {
 ...
 uint256 constant FEE_INTEREST = 1000;
 ...
 return _getAddress(ACL);
 }
 function setACL(address _address) external onlyOwner {
 _setAddress(ACL, _address);
 }
 function getContractsRegister() external view returns (address) {
 return _getAddress(CONTRACTS_REGISTER);
 }
 function setContractsRegister(address _address) external onlyOwner {
 _setAddress(CONTRACTS_REGISTER, _address);
 }
 function getPriceOracle() external view override returns (address) {
 return _getAddress(PRICE_ORACLE);
 }
 function setPriceOracle(address _address) external onlyOwner {
 _setAddress(PRICE_ORACLE, _address);
 }
 function getAccountFactory() external view returns (address) {
 return _getAddress(ACCOUNT_FACTORY);
 }
 function setAccountFactory(address _address) external onlyOwner {
 _setAddress(ACCOUNT_FACTORY, _address);
 }
 function getDataCompressor() external view override returns (address) {
 return _getAddress(DATA_COMPRESSOR);
 }
 function setDataCompressor(address _address) external onlyOwner {
 _setAddress(DATA_COMPRESSOR, _address);
 }
 function getTreasuryContract() external view returns (address) {
 return _getAddress(TREASURY_CONTRACT);
 }
 function setTreasuryContract(address _address) external onlyOwner {
 _setAddress(TREASURY_CONTRACT, _address);
 }
 function getGearToken() external view override returns (address) {
 return _getAddress(GEAR_TOKEN);
 }
 function setGearToken(address _address) external onlyOwner {
 _setAddress(GEAR_TOKEN, _address);
 }
 function getWethToken() external view override returns (address) {
 return _getAddress(WETH_TOKEN);
 }
 function setWethToken(address _address) external onlyOwner {
 _setAddress(WETH_TOKEN, _address);
 }
 function getWETHGateway() external view override returns (address) {
 return _getAddress(WETH_GATEWAY);
 }
 function setWETHGateway(address _address) external onlyOwner {
 _setAddress(WETH_GATEWAY, _address);
 }
 function getLeveragedActions() external view override returns (address) {
 return _getAddress(LEVERAGED_ACTIONS);
 }
 function setLeveragedActions(address _address) external onlyOwner {
 _setAddress(LEVERAGED_ACTIONS, _address);
 }
 function _getAddress(bytes32 key) internal view returns (address) {
 address result = addresses[key];
 require(result != address(0), Errors.AS_ADDRESS_NOT_FOUND);
 return result;
 }
 ...
 function addPool(address newPoolAddress) external configuratorOnly {
 require( newPoolAddress != address(0), Errors.ZERO_ADDRESS_IS_NOT_ALLOWED );
 require(!isPool[newPoolAddress], Errors.CR_POOL_ALREADY_ADDED);
 pools.push(newPoolAddress);
 isPool[newPoolAddress] = true;
 ...
 function addCreditManager(address newCreditManager) external configuratorOnly {
 require( newCreditManager != address(0), Errors.ZERO_ADDRESS_IS_NOT_ALLOWED );
 require( !isCreditManager[newCreditManager], Errors.CR_CREDIT_MANAGER_ALREADY_ADDED );
 creditManagers.push(newCreditManager);
 isCreditManager[newCreditManager] = true;
 ...
 function openCreditAccount( uint256 amount, address onBehalfOf, uint256 leverageFactor, uint256 referralCode ) external override;
 function closeCreditAccount(address to, DataTypes.Exchange[] calldata paths) external override;
 ...
 function addCollateral( address onBehalfOf, address token, uint256 amount ) external override;
 function hasOpenedCreditAccount(address borrower) external view override returns (bool);
 function calcRepayAmount(address borrower, bool isLiquidated) external view override returns (uint256);
 ...
 function getCreditAccountOrRevert(address borrower) external view override returns (address);
 ...
 function checkCollateralChange( address creditAccount, address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut ) external;
 function checkMultiTokenCollateral( address creditAccount, uint256[] memory amountIn, uint256[] memory amountOut, address[] memory tokenIn, address[] memory tokenOut ) external;
 ...
 function underlyingToken() external view returns (address);
 function getCreditAccountTokenById(address creditAccount, uint256 id) external view returns ( address token, uint256 balance, uint256 tv, uint256 twv );
 ...
 require( decimals <= 18, Errors.PO_TOKENS_WITH_DECIMALS_MORE_18_ISNT_ALLOWED );
 require( AggregatorV3Interface(priceFeed).decimals() == 18, Errors.PO_AGGREGATOR_DECIMALS_SHOULD_BE_18 );
 decimalsMultipliers[token] = 10**(18 - decimals);
 decimalsDividers[token] = 10**(36 - decimals);
 emit NewPriceFeed(token, priceFeed);
 }
 function convert( uint256 amount, address tokenFrom, address tokenTo ) external view override returns (uint256) {
 return amount .mul(decimalsMultipliers[tokenFrom]) .mul(getLastPrice(tokenFrom, tokenTo)) .div(decimalsDividers[tokenTo]);
 }
 function getLastPrice(address tokenFrom, address tokenTo) public view override returns (uint256) {
 ...
 require( priceFeeds[token] != address(0), Errors.PO_PRICE_FEED_DOESNT_EXIST );
