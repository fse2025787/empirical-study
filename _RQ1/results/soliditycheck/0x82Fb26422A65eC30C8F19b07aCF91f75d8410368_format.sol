 ...
 function deposit( uint256 deposit0, uint256 deposit1, address to, address from, address pos ) external returns (uint256 shares) {
 require(positions[pos].version != 0, 'not added');
 if (twapCheck || positions[pos].twapOverride) {
 checkPriceChange( pos, (positions[pos].twapOverride ? positions[pos].twapInterval : twapInterval), (positions[pos].twapOverride ? positions[pos].priceThreshold : priceThreshold) );
 }
 ...
 return (FullMath.mulDiv(depositRatio, deltaScale, hypeRatio) < depositDelta && FullMath.mulDiv(hypeRatio, deltaScale, depositRatio) < depositDelta);
 }
 return true;
 }
 function getDepositAmount(address pos, address token, uint256 deposit) public view returns (uint256 amountStart, uint256 amountEnd) {
 ...
 if (price.mul(100).div(priceBefore) > _priceThreshold || priceBefore.mul(100).div(price) > _priceThreshold) revert("Price change Overflow");
 }
 ...
 function customDeposit( address pos, uint256 deposit0Max, uint256 deposit1Max, uint256 maxTotalSupply ) external onlyOwner {
 ...
 function rebalance( int24 _baseLower, int24 _baseUpper, int24 _limitLower, int24 _limitUpper, address _feeRecipient, int256 swapQuantity ) external;
