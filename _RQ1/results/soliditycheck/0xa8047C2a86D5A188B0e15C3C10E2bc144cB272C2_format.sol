 ...
 enum ProposalState {
 WarmUp, Active, Canceled, Failed, Accepted, Queued, Grace, Expired, Executed, Abrogated }
 struct Receipt {
 bool hasVoted;
 uint256 votes;
 bool support;
 ...
 function propose( address[] memory targets, uint256[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description, string memory title ) public returns (uint256) {
 if (!isActive) {
 require(supernova.xyzStaked() >= ACTIVATION_THRESHOLD, "DAO not yet active");
 isActive = true;
 }
 require( supernova.votingPowerAtTs(msg.sender, block.timestamp - 1) >= _getCreationThreshold(), "Creation threshold not met" );
 ...
 require( state(proposalId) == ProposalState.Queued && abrogationProposal.createTime != 0, "Abrogation Proposal not active" );
 ...
 if ((proposal.forVotes + proposal.againstVotes) < _getQuorum(proposal) || (proposal.forVotes < _getMinForVotes(proposal))) {
 ...
 Proposal storage proposal = proposals[proposalId];
 if (msg.sender == proposal.proposer || supernova.votingPower(proposal.proposer) < _getCreationThreshold() ) {
 return true;
 }
 return false;
 }
 ...
 ProposalState s = state(proposalId);
 return s == ProposalState.WarmUp || s == ProposalState.Active || s == ProposalState.Accepted || s == ProposalState.Queued || s == ProposalState.Grace;
 }
