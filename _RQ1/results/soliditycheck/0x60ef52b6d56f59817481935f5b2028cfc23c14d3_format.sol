 ...
 function rebindByStrategyAdd( address token, uint256 balance, uint256 denorm, uint256 deposit ) external;
 function rebindByStrategyRemove( address token, uint256 balance, uint256 denorm ) external;
 function bindByStrategy( address token, uint256 balance, uint256 denorm ) external;
 ...
 function calc_withdraw_one_coin( address _pool, uint256 _tokenAmount, int128 _index ) external view returns (uint256);
 function remove_liquidity_one_coin( address _pool, uint256 _token_amount, int128 _i, uint256 _min_amount ) external;
 }
 pragma solidity 0.6.12;
 ...
 function calc_token_amount( address _pool, uint256[2] memory _amounts, bool _deposit ) external view returns (uint256);
 }
 pragma solidity 0.6.12;
 ...
 function calc_token_amount( address _pool, uint256[3] memory _amounts, bool _deposit ) external view returns (uint256);
 }
 pragma solidity 0.6.12;
 ...
 function calc_token_amount( address _pool, uint256[4] memory _amounts, bool _deposit ) external view returns (uint256);
 }
 pragma solidity 0.6.12;
 ...
 event UpdatePoolWeights( address indexed pool, uint256 indexed timestamp, address[] tokens, uint256[3][] weightsChange, uint256[] newTokenValues );
 ...
 function _computeWeightsChangeWithEvent( PowerIndexPoolInterface _pool, address[] memory _tokens, address[] memory _piTokens, uint256 _minWPS, uint256 fromTimestamp, uint256 toTimestamp ) internal returns ( uint256[3][] memory weightsChange, uint256 lenToPush, uint256[] memory newTokensValues ) {
 (weightsChange, lenToPush, newTokensValues, ) = computeWeightsChange( _pool, _tokens, _piTokens, _minWPS, fromTimestamp, toTimestamp );
 emit UpdatePoolWeights(address(_pool), block.timestamp, _tokens, weightsChange, newTokensValues);
 }
 function computeWeightsChange( PowerIndexPoolInterface _pool, address[] memory _tokens, address[] memory _piTokens, uint256 _minWPS, uint256 fromTimestamp, uint256 toTimestamp ) public view returns ( uint256[3][] memory weightsChange, uint256 lenToPush, uint256[] memory newTokenValues, uint256 newTokenValueSum ) {
 ...
 function getValueChangeRate( address _token, uint256 oldTokenValue, uint256 newTokenValue ) public view returns (uint256 lastChangeRate, uint256 newChangeRate) {
 ...
 event PullLiquidity( address indexed vaultToken, address crvToken, uint256 vaultAmount, uint256 crvAmountExpected, uint256 crvAmountActual, uint256 usdcAmount, uint256 vaultReserve );
 event PushLiquidity( address indexed vaultToken, address crvToken, uint256 vaultAmount, uint256 crvAmount, uint256 usdcAmount );
 event SetPoolController(address indexed poolController);
 event SetCurvePoolRegistry(address curvePoolRegistry);
 event SetVaultConfig( address indexed vault, address indexed depositor, uint8 depositorType, uint8 depositorTokenLength, int8 usdcIndex );
 ...
 function initialize( address _powerPoke, address _curvePoolRegistry, address _poolController, uint256 _maxWithdrawalLoss, StrategyConstraints memory _constraints ) external initializer {
 __Ownable_init();
 ...
 return ICurvePoolRegistry(curvePoolRegistry).get_virtual_price_from_lp_token(IYearnVaultV2(_token).token()).mul( _amount ) / 1e18;
 }
 function getVaultUsdcEstimation( address _token, address _crvToken, uint256 _amount ) public returns (uint256) {
 ...
 function setVaultConfig( address _vault, address _depositor, uint8 _depositorType, uint8 _depositorTokenLength, int8 _usdcIndex ) external onlyOwner {
 ...
 function pokeFromReporter(uint256 _reporterId, bytes calldata _rewardOpts) external onlyReporter(_reporterId, _rewardOpts) onlyEOA {
 _poke(false);
 }
 function pokeFromSlasher(uint256 _reporterId, bytes calldata _rewardOpts) external onlyNonReporter(_reporterId, _rewardOpts) onlyEOA {
 _poke(true);
 }
 ...
 function _vaultToUsdc( address _token, address _crvToken, VaultConfig memory _vc ) internal returns ( uint256 crvBalance, uint256 crvReceived, uint256 usdcBefore ) {
 ...
 function _usdcToVault( address _token, VaultConfig memory _vc, uint256 _usdcAmount ) internal returns ( uint256 crvBalance, uint256 vaultBalance, address crvToken ) {
 ...
 PowerIndexPoolControllerInterface(poolController_).rebindByStrategyRemove( cfg.token, cfg.newBalance, cfg.newWeight );
 ...
 emit PullLiquidity( cfg.token, mem.crvToken, mem.yDiff, mem.crvExpected, mem.crvActual, USDC.balanceOf(address(this)) - mem.usdcBefore, mem.vaultReserve );
 }
 else {
 ...
 (uint256 crvBalance, uint256 vaultBalance, address crvToken) = _usdcToVault(cfg.token, vaultConfigs[si], usdcAmount);
 uint256 newBalance;
 ...
 PowerIndexPoolControllerInterface(poolController_).rebindByStrategyAdd( cfg.token, newBalance, cfg.newWeight, vaultBalance );
 }
 ...
 function fetchRebindConfigs( PowerIndexPoolInterface _pool, address[] memory _tokens, bool _allowNotBound ) internal returns (RebindConfig[] memory configs) {
 uint256 len = _tokens.length;
 (uint256[] memory oldBalances, uint256[] memory poolUSDCBalances, uint256 totalUSDCPool) = getRebindConfigBalances(_pool, _tokens);
 (uint256[3][] memory weightsChange, , uint256[] memory newTokenValuesUSDC, uint256 totalValueUSDC) = computeWeightsChange(_pool, _tokens, new address[](0), 0, block.timestamp, block.timestamp + 1);
 configs = new RebindConfig[](len);
 for (uint256 si = 0; si < len; si++) {
 uint256[3] memory wc = weightsChange[si];
 require(wc[1] != 0 || _allowNotBound, "TOKEN_NOT_BOUND");
 configs[si] = RebindConfig( _tokens[wc[0]], wc[2], oldBalances[wc[0]], getNewTokenBalance(_tokens, wc, poolUSDCBalances, newTokenValuesUSDC, totalUSDCPool, totalValueUSDC) );
 }
 _updatePoolByPoke(pool, _tokens, newTokenValuesUSDC);
 }
 function getNewTokenBalance( address[] memory _tokens, uint256[3] memory wc, uint256[] memory poolUSDCBalances, uint256[] memory newTokenValuesUSDC, uint256 totalUSDCPool, uint256 totalValueUSDC ) internal view returns (uint256) {
 return bdiv( bdiv(bmul(wc[2], totalUSDCPool), totalWeight), bdiv(poolUSDCBalances[wc[0]], IERC20(_tokens[wc[0]]).totalSupply()) ) * 1e12;
 }
 function getRebindConfigBalances(PowerIndexPoolInterface _pool, address[] memory _tokens) internal returns ( uint256[] memory oldBalances, uint256[] memory poolUSDCBalances, uint256 totalUSDCPool ) {
 uint256 len = _tokens.length;
 ...
 totalUSDCPool = totalUSDCPool.add( getVaultUsdcEstimation(_tokens[oi], IYearnVaultV2(_tokens[oi]).token(), oldBalances[oi]) );
 }
 catch {
 oldBalances[oi] = 0;
 }
