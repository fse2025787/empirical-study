 ...
 contract PiBundleMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
 ...
 function getListing(address _owner, string memory _bundleID) external view returns ( address[] memory nfts, uint256[] memory tokenIds, uint256[] memory quantities, uint256 price, uint256 startingTime ) {
 ...
 require( owners[bundleID] == address(0) || (owners[bundleID] == _msgSender() && listings[_msgSender()][bundleID].price == 0), "already listed" );
 address tokenRegistry = addressRegistry.tokenRegistry();
 require( _payToken == address(0) || (tokenRegistry != address(0) && IPiTokenRegistry(tokenRegistry).enabled(_payToken)), "invalid pay token" );
 ...
 else if ( _supportsInterface(_nftAddresses[i], INTERFACE_ID_ERC1155) ) {
 IERC1155 nft = IERC1155(_nftAddresses[i]);
 _check1155Owning( _nftAddresses[i], _tokenIds[i], _quantities[i], _msgSender() );
 require( nft.isApprovedForAll(_msgSender(), address(this)), "item not approved" );
 ...
 _check1155Owning( listing.nfts[i], listing.tokenIds[i], listing.quantities[i], owner );
 }
 }
 ...
 (bool feeTransferSuccess, ) = feeReceipient.call{
 value: feeAmount}
 ( "" );
 require( feeTransferSuccess, "PiMarketplace: Fee transfer failed" );
 (bool ownerTransferSuccess, ) = owner.call{
 value: price.sub(feeAmount) }
 ("");
 require( ownerTransferSuccess, "PiMarketplace: Owner transfer failed" );
 }
 else {
 ...
 IPiMarketplace(addressRegistry.marketplace()).validateItemSold( listing.nfts[i], listing.tokenIds[i], owner, _msgSender() );
 }
 ...
 emit ItemSold( owner, _msgSender(), _bundleID, _payToken, IPiMarketplace(addressRegistry.marketplace()).getPrice(_payToken), price );
 ...
 IPiMarketplace(addressRegistry.marketplace()).validateItemSold( listing.nfts[i], listing.tokenIds[i], owners[bundleID], _creator );
 }
 ...
 emit ItemSold( _msgSender(), _creator, _bundleID, address(offer.payToken), IPiMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)), offer.price );
 ...
 function _check1155Owning( address _nft, uint256 _tokenId, uint256 _quantity, address _owner ) internal view {
 require( IERC1155(_nft).balanceOf(_owner, _tokenId) >= _quantity, "not owning item" );
 }
