 ...
 event UnStake( uint256 depositId, address indexed from, address indexed receiver );
 ...
 constructor( address _depositToken, address _provider, uint256 _rewardRate, uint256 _startTime, uint256 _totalReward, uint256 _maxLockDuration ) {
 ...
 function stake(uint256 amount, uint256 duration) external nonReentrant notifyUpdateReward(msg.sender) {
 require(amount > 0, "AM0");
 duration = Math.max( Math.min(duration, maxLockDuration), MIN_LOCK_DURATION );
 depositToken.safeTransferFrom(msg.sender, address(this), amount);
 uint256 shares = getMultiper( amount, block.timestamp, block.timestamp.add(duration) );
 _userShare[msg.sender] = shares;
 _totalShare = _totalShare.add(_userShare[msg.sender]);
 depositsOf[msg.sender].push( DepositInfo({
 amount: amount, begin: uint64(block.timestamp), until: uint64(block.timestamp) + uint64(duration) }
 ) );
 emit Stake(amount, duration, msg.sender);
 }
 function unstake(uint256 depositId, address receiver) external nonReentrant notifyUpdateReward(msg.sender) {
 ...
 depositsOf[msg.sender][depositId] = depositsOf[msg.sender][ depositOfLength - 1 ];
 depositsOf[msg.sender].pop();
 uint256 shares = getMultiper( userDeposit.amount, userDeposit.begin, userDeposit.until );
 ...
 function claimReward() external nonReentrant notifyUpdateReward(msg.sender) {
 uint256 reward = Math.min( rewardDebt[msg.sender], totalReward.sub(accruedReward) );
 if (reward > 0) {
 ...
 function pendingReward(address account, uint256 depositId) external view returns (uint256) {
 ...
 return rewardPerShareStored.add( lastTimeRewardApplicable() .sub(lastUpdateTime) .mul(rewardRate) .mul(3e18) .div(_totalShare) );
 }
 function earned(address account) public view returns (uint256) {
 return _userShare[account] .mul(rewardPerShare().sub(userRewardPerSharePaid[account])) .div(5e18) .add(rewardDebt[account]);
 }
 function lastTimeRewardApplicable() public view returns (uint256) {
 return Math.min(block.timestamp, periodFinish);
 }
 function getTotalDeposit(address account) public view returns (uint256 totalAmount) {
 for (uint256 idx; idx < depositsOf[account].length; idx++) {
 totalAmount += depositsOf[account][idx].amount;
 }
 }
 function getDepositsOf(address account) public view returns (DepositInfo[] memory) {
 return depositsOf[account];
 }
 function getDepositsOfLength(address account) public view returns (uint256) {
 return depositsOf[account].length;
 }
 function getMultiper( uint256 amount, uint256 _from, uint256 _to ) public view returns (uint256) {
 require(_to >= _from, "NEG");
 uint256 duration = _to.sub(_from);
 require( duration >= MIN_LOCK_DURATION && duration <= maxLockDuration, "DURATION" );
 return amount .mul(duration.mul(1e18).div(1 weeks).mul(2).div(100)) .div(1e18) .add(amount);
 }
