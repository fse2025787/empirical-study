 ...
 function depositFor( uint256[2] calldata maxAmounts, uint256[2] calldata minAmounts, address target ) external payable returns (uint256 fairSupply, uint256[2] memory receivedAmounts);
 }
 contract Mooniswap_ZapIn_V1 is ZapBaseV1 {
 constructor(uint256 _goodwill, uint256 _affiliateSplit) public ZapBaseV1(_goodwill, _affiliateSplit) {
 }
 event zapIn(address sender, address pool, uint256 tokensRec);
 function ZapIn( address fromToken, address toPool, uint256 minPoolTokens, uint256[] calldata fromTokenAmounts, address[] calldata swapTargets, bytes[] calldata swapData, address affiliate ) external payable stopInEmergency returns (uint256 lpReceived) {
 uint256[2] memory toInvest = _pullTokens( fromToken, fromTokenAmounts, affiliate );
 ...
 amounts[0] = _fillQuote( fromToken, tokens[0], toInvest[0], swapTargets[0], swapData[0] );
 }
 if (fromToken == tokens[1]) {
 amounts[1] = toInvest[1];
 }
 else {
 amounts[1] = _fillQuote( fromToken, tokens[1], toInvest[1], swapTargets[1], swapData[1] );
 }
 lpReceived = _inchDeposit(tokens, amounts, toPool);
 require(lpReceived >= minPoolTokens, "ERR: High Slippage");
 }
 function _inchDeposit( address[] memory tokens, uint256[] memory amounts, address toPool ) internal returns (uint256 lpReceived) {
 uint256[2] memory minAmounts = [ amounts[0].mul(90).div(100), amounts[1].mul(90).div(100) ];
 ...
 (lpReceived, receivedAmounts) = IMooniswap(toPool).depositFor.value( amounts[0] )([amounts[0], amounts[1]], minAmounts, msg.sender);
 }
 else {
 IERC20(tokens[0]).safeApprove(toPool, 0);
 IERC20(tokens[0]).safeApprove(toPool, amounts[0]);
 (lpReceived, receivedAmounts) = IMooniswap(toPool).depositFor( [amounts[0], amounts[1]], minAmounts, msg.sender );
 }
 ...
 function _fillQuote( address fromTokenAddress, address toToken, uint256 amount, address swapTarget, bytes memory swapCallData ) internal returns (uint256 amtBought) {
 uint256 valueToSend;
 ...
 function _pullTokens( address fromToken, uint256[] memory fromTokenAmounts, address affiliate ) internal returns (uint256[2] memory toInvest) {
 if (fromToken == address(0)) {
 require(msg.value > 0, "No eth sent");
 require( fromTokenAmounts[0].add(fromTokenAmounts[1]) == msg.value, "msg.value != fromTokenAmounts" );
 }
 else {
 require(msg.value == 0, "Eth sent with token");
 IERC20(fromToken).safeTransferFrom( msg.sender, address(this), fromTokenAmounts[0].add(fromTokenAmounts[1]) );
 }
 toInvest[0] = fromTokenAmounts[0].sub( _subtractGoodwill(fromToken, fromTokenAmounts[0], affiliate) );
 toInvest[1] = fromTokenAmounts[1].sub( _subtractGoodwill(fromToken, fromTokenAmounts[1], affiliate) );
 }
 function _subtractGoodwill( address token, uint256 amount, address affiliate ) internal returns (uint256 totalGoodwillPortion) {
