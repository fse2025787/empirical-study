 ...
 ret;
 assembly {
 let lookup := 0x3031323334353637383961626364656600000000000000000000000000000000 for {
 let i := 40 }
 gt(i, 0) {
 }
 {
 i := sub(i, 1) mstore8(i, byte(and(addr, 0xf), lookup)) addr := div(addr, 0x10) i := sub(i, 1) mstore8(i, byte(and(addr, 0xf), lookup)) addr := div(addr, 0x10) }
 ret := keccak256(0, 40) }
 }
 function toChecksumString( address account ) private pure returns (string memory asciiString) {
 ...
 function toChecksumCapsFlags(address account) private pure returns ( bool[40] memory characterCapitalized ) {
 ...
 function getAsciiOffset( uint8 nibble, bool caps ) private pure returns (uint8 offset) {
 if (nibble < 10) {
 offset = 48;
 }
 else if (caps) {
 ...
 function toAsciiString( bytes20 data ) private pure returns (string memory asciiString) {
 bytes memory asciiBytes = new bytes(40);
 uint8 b;
 uint8 leftNibble;
 uint8 rightNibble;
