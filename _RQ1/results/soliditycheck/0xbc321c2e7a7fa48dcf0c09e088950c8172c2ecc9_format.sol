 ...
 function deploy( address masterContract, bytes calldata data, bool useCreate2 ) public payable returns (address cloneAddress) {
 require(masterContract != address(0), "BoringFactory: No masterContract");
 bytes20 targetBytes = bytes20(masterContract);
 if (useCreate2) {
 bytes32 salt = keccak256(data);
 assembly {
 let clone := mload(0x40) mstore(clone, 0x3d602d80600a3d3981f3363d3d373d3d3d363d73000000000000000000000000) mstore(add(clone, 0x14), targetBytes) mstore(add(clone, 0x28), 0x5af43d82803e903d91602b57fd5bf30000000000000000000000000000000000) cloneAddress := create2(0, clone, 0x37, salt) }
 }
 else {
 assembly {
 let clone := mload(0x40) mstore(clone, 0x3d602d80600a3d3981f3363d3d373d3d3d363d73000000000000000000000000) mstore(add(clone, 0x14), targetBytes) mstore(add(clone, 0x28), 0x5af43d82803e903d91602b57fd5bf30000000000000000000000000000000000) cloneAddress := create(0, clone, 0x37) }
 }
 masterContractOf[cloneAddress] = masterContract;
 IMasterContract(cloneAddress).init{
 value: msg.value}
 (data);
 ...
 bytes32 private constant APPROVAL_SIGNATURE_HASH = keccak256("SetMasterContractApproval(string warning,address user,address masterContract,bool approved,uint256 nonce)");
 ...
 return keccak256(abi.encode(DOMAIN_SEPARATOR_SIGNATURE_HASH, keccak256("CoffinBox V1"), chainId, address(this)));
 }
 ...
 bytes32 digest = keccak256( abi.encodePacked( EIP191_PREFIX_FOR_EIP712_STRUCTURED_DATA, DOMAIN_SEPARATOR(), keccak256( abi.encode( APPROVAL_SIGNATURE_HASH, approved ? keccak256("Give FULL access to funds in (and approved to) CoffinBox?") : keccak256("Revoke access to CoffinBox?"), user, masterContract, approved, nonces[user]++ ) ) ) );
 ...
 contract CoffinBox is MasterContractManager, BoringBatchable {
 using BoringMath for uint256;
 using BoringMath128 for uint128;
 ...
 require(masterContract != address(0), "CoffinBox: no masterContract");
 require(masterContractApproved[masterContract][from], "CoffinBox: Transfer not approved");
 }
 _;
 }
 ...
 function deposit( IERC20 token_, address from, address to, uint256 amount, uint256 share ) public payable allowed(from) returns (uint256 amountOut, uint256 shareOut) {
 require(to != address(0), "CoffinBox: to not set");
 IERC20 token = token_ == USE_ETHEREUM ? wethToken : token_;
 Rebase memory total = totals[token];
 require(total.elastic != 0 || token.totalSupply() > 0, "CoffinBox: No tokens");
 if (share == 0) {
 ...
 require( from != address(this) || token_ == USE_ETHEREUM || amount <= _tokenBalanceOf(token).sub(total.elastic), "CoffinBox: Skim too much" );
 balanceOf[token][to] = balanceOf[token][to].add(share);
 total.base = total.base.add(share.to128());
 total.elastic = total.elastic.add(amount.to128());
 totals[token] = total;
 if (token_ == USE_ETHEREUM) {
 IWETH(address(wethToken)).deposit{
 value: amount}
 ();
 }
 ...
 function withdraw( IERC20 token_, address from, address to, uint256 amount, uint256 share ) public allowed(from) returns (uint256 amountOut, uint256 shareOut) {
 require(to != address(0), "CoffinBox: to not set");
 IERC20 token = token_ == USE_ETHEREUM ? wethToken : token_;
 Rebase memory total = totals[token];
 if (share == 0) {
 ...
 require(total.base >= MINIMUM_SHARE_BALANCE || total.base == 0, "CoffinBox: cannot empty");
 totals[token] = total;
 if (token_ == USE_ETHEREUM) {
 ...
 require(tos[0] != address(0), "CoffinBox: to[0] not set");
 uint256 totalAmount;
 ...
 require(_tokenBalanceOf(token) >= totals[token].addElastic(fee.to128()), "CoffinBox: Wrong amount");
 emit LogFlashLoan(address(borrower), token, amount, fee, receiver);
 }
 function batchFlashLoan( IBatchFlashBorrower borrower, address[] calldata receivers, IERC20[] calldata tokens, uint256[] calldata amounts, bytes calldata data ) public {
 ...
 require(_tokenBalanceOf(token) >= totals[token].addElastic(fees[i].to128()), "CoffinBox: Wrong amount");
