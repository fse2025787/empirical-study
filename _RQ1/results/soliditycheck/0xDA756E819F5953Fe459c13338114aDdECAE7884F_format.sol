 ...
 contract Convert {
 using SafeMath for uint256;
 function convertTokenAmount( address _fromToken, address _toToken, uint256 _fromAmount ) public view returns (uint256 toAmount) {
 ...
 function stake(uint256 amount) public nonReentrant runPhasedDistribution() updateAccount(msg.sender) checkIfNewUser() {
 uint256 tokenAmount = convertTokenAmount(friesToken, token, amount);
 amount = convertTokenAmount(token, friesToken, tokenAmount);
 require(tokenAmount > 0, 'The amount is too small');
 address sender = msg.sender;
 ...
 function forceExchange(address toExchange) public nonReentrant runPhasedDistribution() updateAccount(msg.sender) updateAccount(toExchange) {
 address sender = msg.sender;
 ...
 function userInfo(address user) public view returns ( uint256 depositedToken, uint256 pendingdivs, uint256 inbucket, uint256 realised ) {
 ...
 function upgradeSetting( address _upgradeAddress, uint256 _upgradeTime, uint256 _upgradeAmount ) external requireImpl {
 ...
 require( upgradeAddress != address(0) && upgradeAmount > 0 && block.timestamp > upgradeTime && upgradeTime > 0, '!upgrade' );
