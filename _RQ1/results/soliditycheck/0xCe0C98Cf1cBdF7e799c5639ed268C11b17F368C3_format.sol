 ...
 assembly {
 let succeeded := delegatecall(sub(gas(), 5000), _target, add(_data, 0x20), mload(_data), 0, 0) let size := returndatasize() response := mload(0x40) mstore(0x40, add(response, and(add(add(size, 0x20), 0x1f), not(0x1f)))) mstore(response, size) returndatacopy(add(response, 0x20), 0, size) switch iszero(succeeded) case 1 {
 ...
 function withdrawV3( address _gauge, uint256 _tokenId, address[] calldata _rewardTokens ) external returns (uint256);
 function withdrawV2( address _gauge, address _token, bytes32 _kek_id, address[] calldata _rewardTokens ) external returns (uint256);
 ...
 function withdrawAllV3( address _gauge, address _token, address[] calldata _rewardTokens ) external returns (uint256 amount);
 function withdrawAllV2( address _gauge, address _token, address[] calldata _rewardTokens ) external returns (uint256 amount);
 function harvest(address _gauge, address[] calldata _tokens) external;
 function depositV2( address _gauge, address _token, uint256 _secs ) external;
 function depositV3( address _gauge, uint256 _tokenId, uint256 _secs ) external;
