 ...
 function setAddresses( address _borrowerOperationsAddress, address _troveManagerAddress, address _activePoolAddress, address _lusdTokenAddress, address _sortedTrovesAddress, address _priceFeedAddress, address _communityIssuanceAddress ) external;
 ...
 IPriceFeed internal constant priceFeed = IPriceFeed(0x4c517D4e2C851CA76d7eC94B805269Df0f2201De);
 ISwapRouter internal constant router = ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);
 IStableSwapExchange internal constant curvePool = IStableSwapExchange(0xEd279fDD11cA84bEef15AF5D39BB4d4bEE23F0cA);
 ...
 function setConvertDAItoLUSDonCurve(bool _convertDAItoLUSDonCurve) external onlyEmergencyAuthorized {
 convertDAItoLUSDonCurve = _convertDAItoLUSDonCurve;
 }
 function setSwapFees( uint24 _lqtyToEthFee, uint24 _ethToDaiFee, uint24 _daiToLusdFee ) external onlyEmergencyAuthorized {
 lqtyToEthFee = _lqtyToEthFee;
 ethToDaiFee = _ethToDaiFee;
 daiToLusdFee = _daiToLusdFee;
 }
 function setMinExpectedSwapPercentage(uint256 _minExpectedSwapPercentage) external onlyEmergencyAuthorized {
 minExpectedSwapPercentage = _minExpectedSwapPercentage;
 }
 function setTendAmounts( uint256 _maxEthPercent, uint256 _maxEthAmount, uint256 _maxEthToSell, uint256 _maxTendBaseFee ) external onlyEmergencyAuthorized {
 ...
 function setHarvestTriggerMin( uint256 _harvestProfitMin ) external onlyEmergencyAuthorized {
 ...
 return totalLUSDBalance().add(daiToWant).add( totalETHBalance().mul(priceFeed.lastGoodPrice()).div(1e18) );
 }
 ...
 stabilityPool.withdrawFromSP( stabilityPool.getCompoundedLUSDDeposit(address(this)) );
 uint256 daiBalance = DAI.balanceOf(address(this));
 if (daiBalance > 0) {
 ...
 return balanceOfWant().add( stabilityPool.getCompoundedLUSDDeposit(address(this)) );
 }
 function totalLQTYBalance() public view returns (uint256) {
 return LQTY.balanceOf(address(this)).add( stabilityPool.getDepositorLQTYGain(address(this)) );
 }
 function totalETHBalance() public view returns (uint256) {
 return address(this).balance.add( stabilityPool.getDepositorETHGain(address(this)) );
 }
 function _checkAllowance( address _contract, IERC20 _token, uint256 _amount ) internal {
 ...
 bytes memory path = abi.encodePacked( address(LQTY), lqtyToEthFee, address(WETH), ethToDaiFee, address(DAI) );
 router.exactInput( ISwapRouter.ExactInputParams( path, address(this), now, LQTY.balanceOf(address(this)), 0 ) );
 }
 function _sellETHforDAI(uint256 ethBalance) internal {
 uint256 ethUSD = priceFeed.fetchPrice();
 uint256 minExpected = ethBalance .mul(ethUSD) .mul(minExpectedSwapPercentage) .div(MAX_BPS) .div(1e18);
 ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams( address(WETH), address(DAI), ethToDaiFee, address(this), now, ethBalance, minExpected, 0 );
 ...
 curvePool.exchange_underlying( 1, 0, daiBalance, daiBalance.mul(minExpectedSwapPercentage).div(MAX_BPS) );
 }
 function _sellDAIAmountForLUSDonUniswap(uint256 daiBalance) internal {
 _checkAllowance(address(router), DAI, daiBalance);
 ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams( address(DAI), address(want), daiToLusdFee, address(this), now, daiBalance, daiBalance.mul(minExpectedSwapPercentage).div(MAX_BPS), 0 );
 ...
 curvePool.exchange_underlying( 1, 0, _amount, minOut );
 }
 }
 ...
 uint256 baseFee;
 try IBaseFeeOracle(0xf8d0Ec04e94296773cE20eFbeeA82e76220cD549) .basefee_global() returns (uint256 currentBaseFee) {
