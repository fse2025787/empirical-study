 ...
 function execute( uint256 SEURtoTrade, uint256 minEURrate, uint256 minSEURout ) external onlyOwner {
 uint256 initialSeurBalance = IERC20(SEUR).balanceOf(address(this));
 ICurve(curveEURS).exchange(1, 0, SEURtoTrade, 0);
 ...
 ICurve(curveSUSD).exchange(1, 3, IERC20(USDC).balanceOf(address(this)), 0);
 ...
 function remainingIssuableSynths(address issuer) external view returns ( uint maxIssuable, uint alreadyIssued, uint totalSystemDebt );
 ...
 function exchangeWithTracking( bytes32 sourceCurrencyKey, uint sourceAmount, bytes32 destinationCurrencyKey, address originator, bytes32 trackingCode ) external returns (uint amountReceived);
 function exchangeOnBehalfWithTracking( address exchangeForAddress, bytes32 sourceCurrencyKey, uint sourceAmount, bytes32 destinationCurrencyKey, address originator, bytes32 trackingCode ) external returns (uint amountReceived);
 function exchangeWithVirtual( bytes32 sourceCurrencyKey, uint sourceAmount, bytes32 destinationCurrencyKey, bytes32 trackingCode ) external returns (uint amountReceived, IVirtualSynth vSynth);
