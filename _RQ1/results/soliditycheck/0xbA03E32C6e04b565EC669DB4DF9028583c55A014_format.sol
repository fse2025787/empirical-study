 ...
 uint public DIST_START_BLOCK;
 uint public constant NUM_EPOCH = 15;
 uint public BLOCKS_PER_EPOCH;
 uint public constant DIST_START_AMOUNT = 250000 ether;
 uint public constant DIST_MIN_AMOUNT = 18750 ether;
 ...
 mapping(address => uint[15]) public totalVtbMap;
 mapping(address => uint[15]) public totalNutMap;
 mapping(address => mapping( address => uint[15] ) ) public vtbMap;
 ...
 uint current_epoch = block.number.sub(DIST_START_BLOCK).div(BLOCKS_PER_EPOCH).min(NUM_EPOCH - 1);
 ...
 return i < futureVtbEpoch[pool][lender] ? vtbMap[pool][lender][i] : futureVtbMap[pool][lender];
 }
 function getTotalVtb(address pool, uint i) public view returns (uint) {
 require(i < NUM_EPOCH, 'totalVtb idx err');
 return i < futureTotalVtbEpoch[pool] ? totalVtbMap[pool][i] : futureTotalVtbMap[pool];
 }
