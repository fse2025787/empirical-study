 ...
 require( _tokenAddress == address(0) || IERC20(_tokenAddress).totalSupply() > 0, 'INVALID_TOKEN' );
 }
 function getBalance( address _tokenAddress, address _account ) public view returns (uint) {
 ...
 function _transfer( address _tokenAddress, address _to, uint _amount ) internal {
 if(_amount > 0) {
 if(_tokenAddress == address(0)) {
 ...
 pragma solidity 0.5.17;
 contract MixinLockCore is IERC721Enumerable, MixinLockManagerRole, MixinFunds, MixinDisable {
 using Address for address;
 event Withdrawal( address indexed sender, address indexed tokenAddress, address indexed beneficiary, uint amount );
 event PricingChanged( uint oldKeyPrice, uint keyPrice, address oldTokenAddress, address tokenAddress );
 ...
 require( isLockManager(msg.sender) || msg.sender == beneficiary, 'ONLY_LOCK_MANAGER_OR_BENEFICIARY' );
 _;
 }
 function _initializeMixinLockCore( address _beneficiary, uint _expirationDuration, uint _keyPrice, uint _maxNumberOfKeys ) internal {
 ...
 function updateKeyPricing( uint _keyPrice, address _tokenAddress ) external onlyLockManager onlyIfAlive {
 ...
 function setEventHooks( address _onKeyPurchaseHook, address _onKeyCancelHook ) external onlyLockManager() {
 ...
 function approveBeneficiary( address _spender, uint _amount ) public onlyLockManagerOrBeneficiary returns (bool) {
 ...
 require( _isKeyManager(_tokenId, msg.sender) || _isApproved(_tokenId, msg.sender) || isApprovedForAll(_ownerOf[_tokenId], msg.sender), 'ONLY_KEY_MANAGER_OR_APPROVED' );
 _;
 }
 modifier ownsOrHasOwnedKey( address _keyOwner ) {
 require( keyByOwner[_keyOwner].expirationTimestamp > 0, 'HAS_NEVER_OWNED_KEY' );
 _;
 }
 ...
 function getOwnersByPage(uint _page, uint _pageSize) public view returns (address[] memory) {
 ...
 function isKeyOwner( uint _tokenId, address _keyOwner ) public view returns (bool) {
 return _ownerOf[_tokenId] == _keyOwner;
 }
 function keyExpirationTimestampFor( address _keyOwner ) public view returns (uint) {
 ...
 function setKeyManagerOf( uint _tokenId, address _keyManager ) public isKey(_tokenId) {
 require( _isKeyManager(_tokenId, msg.sender) || isLockManager(msg.sender), 'UNAUTHORIZED_KEY_MANAGER_UPDATE' );
 ...
 function approve( address _approved, uint _tokenId ) public onlyIfAlive onlyKeyManagerOrApproved(_tokenId) {
 require(msg.sender != _approved, 'APPROVE_SELF');
 approved[_tokenId] = _approved;
 emit Approval(_ownerOf[_tokenId], _approved, _tokenId);
 }
 function getApproved( uint _tokenId ) public view isKey(_tokenId) returns (address) {
 ...
 function _isKeyManager( uint _tokenId, address _keyManager ) internal view returns (bool) {
 if(keyManagerOf[_tokenId] == _keyManager || (keyManagerOf[_tokenId] == address(0) && isKeyOwner(_tokenId, _keyManager))) {
 return true;
 }
 else {
 return false;
 ...
 function _timeMachine( uint _tokenId, uint256 _deltaT, bool _addTime ) internal {
 ...
 function _isApproved( uint _tokenId, address _user ) internal view returns (bool) {
 ...
 modifier consumeOffchainApproval( bytes32 _hash, address _keyManager, uint8 _v, bytes32 _r, bytes32 _s ) {
 require( ecrecover( ECDSA.toEthSignedMessageHash(_hash), _v, _r, _s ) == _keyManager, 'INVALID_SIGNATURE' );
 ...
 function initialize( address _lockCreator, uint _expirationDuration, address _tokenAddress, uint _keyPrice, uint _maxNumberOfKeys, string calldata _lockName ) external;
 function() external payable;
 function initialize() external;
 function publicLockVersion() public pure returns (uint);
 function getBalance( address _tokenAddress, address _account ) external view returns (uint);
 function disableLock() external;
 function withdraw( address _tokenAddress, uint _amount ) external;
 function approveBeneficiary( address _spender, uint _amount ) external returns (bool);
 ...
 function isKeyOwner( uint _tokenId, address _keyOwner ) external view returns (bool);
 function keyExpirationTimestampFor( address _keyOwner ) external view returns (uint timestamp);
 ...
 function grantKeys( address[] calldata _recipients, uint[] calldata _expirationTimestamps, address[] calldata _keyManagers ) external;
 function purchase( uint256 _value, address _recipient, address _referrer, bytes calldata _data ) external payable;
 function purchasePriceFor( address _recipient, address _referrer, bytes calldata _data ) external view returns (uint);
 function updateTransferFee( uint _transferFeeBasisPoints ) external;
 function getTransferFee( address _keyOwner, uint _time ) external view returns (uint);
 function expireAndRefundFor( address _keyOwner, uint amount ) external;
 function cancelAndRefund(uint _tokenId) external;
 function cancelAndRefundFor( address _keyManager, uint8 _v, bytes32 _r, bytes32 _s, uint _tokenId ) external;
 function invalidateOffchainApproval( uint _nextAvailableNonce ) external;
 function updateRefundPenalty( uint _freeTrialLength, uint _refundPenaltyBasisPoints ) external;
 function getCancelAndRefundValueFor( address _keyOwner ) external view returns (uint refund);
 ...
 pragma solidity 0.5.17;
 contract MixinERC721Enumerable is IERC721Enumerable, ERC165, MixinLockCore, MixinKeys {
 ...
 function tokenOfOwnerByIndex( address _keyOwner, uint256 _index ) public view returns (uint256) {
 ...
 function grantKeys( address[] calldata _recipients, uint[] calldata _expirationTimestamps, address[] calldata _keyManagers ) external onlyKeyGranterOrManager {
 ...
 emit Transfer( address(0), recipient, idTo );
 }
 }
 }
 pragma solidity 0.5.17;
 contract MixinLockMetadata is IERC721Enumerable, ERC165, MixinLockManagerRole, MixinLockCore, MixinKeys {
 ...
 function updateLockSymbol( string calldata _lockSymbol ) external onlyLockManager {
 ...
 function setBaseTokenURI( string calldata _baseTokenURI ) external onlyLockManager {
 ...
 function purchase( uint256 _value, address _recipient, address _referrer, bytes calldata _data ) external payable onlyIfAlive notSoldOut {
 ...
 emit Transfer( address(0), _recipient, idTo );
 }
 ...
 function purchasePriceFor( address _recipient, address _referrer, bytes calldata _data ) external view returns (uint minKeyPrice) {
 (minKeyPrice, , ) = _purchasePriceFor(_recipient, _referrer, _data);
 }
 function _purchasePriceFor( address _recipient, address _referrer, bytes memory _data ) internal view returns (uint minKeyPrice, uint unlockDiscount, uint unlockTokens) {
 if(address(onKeyPurchaseHook) != address(0)) {
 ...
 pragma solidity 0.5.17;
 contract MixinRefunds is MixinLockManagerRole, MixinSignatures, MixinFunds, MixinLockCore, MixinKeys {
 using SafeMath for uint;
 ...
 event CancelKey( uint indexed tokenId, address indexed owner, address indexed sendTo, uint refund );
 event RefundPenaltyChanged( uint freeTrialLength, uint refundPenaltyBasisPoints );
 function _initializeMixinRefunds() internal {
 refundPenaltyBasisPoints = 1000;
 }
 function expireAndRefundFor( address _keyOwner, uint amount ) external onlyLockManager hasValidKey(_keyOwner) {
 _cancelAndRefund(_keyOwner, amount);
 }
 function cancelAndRefund(uint _tokenId) external onlyKeyManagerOrApproved(_tokenId) {
 address keyOwner = ownerOf(_tokenId);
 uint refund = _getCancelAndRefundValue(keyOwner);
 _cancelAndRefund(keyOwner, refund);
 }
 function cancelAndRefundFor( address _keyManager, uint8 _v, bytes32 _r, bytes32 _s, uint _tokenId ) external consumeOffchainApproval( getCancelAndRefundApprovalHash(_keyManager, msg.sender), _keyManager, _v, _r, _s ) {
 address keyOwner = ownerOf(_tokenId);
 uint refund = _getCancelAndRefundValue(keyOwner);
 _cancelAndRefund(keyOwner, refund);
 }
 function updateRefundPenalty( uint _freeTrialLength, uint _refundPenaltyBasisPoints ) external onlyLockManager {
 ...
 function getCancelAndRefundApprovalHash( address _keyManager, address _txSender ) public view returns (bytes32 approvalHash) {
 return keccak256( abi.encodePacked( address(this), CANCEL_TYPEHASH, keyManagerToNonce[_keyManager], _txSender ) );
 }
 ...
 function _getCancelAndRefundValue( address _keyOwner ) private view hasValidKey(_keyOwner) returns (uint refund) {
 Key storage key = keyByOwner[_keyOwner];
 ...
 pragma solidity 0.5.17;
 contract MixinTransfer is MixinLockManagerRole, MixinFunds, MixinLockCore, MixinKeys {
 using SafeMath for uint;
 using Address for address;
 event TransferFeeChanged( uint transferFeeBasisPoints );
 bytes4 private constant _ERC721_RECEIVED = 0x150b7a02;
 uint public transferFeeBasisPoints;
 function shareKey( address _to, uint _tokenId, uint _timeShared ) public onlyIfAlive onlyKeyManagerOrApproved(_tokenId) {
 ...
 emit Transfer( address(0), _to, idTo );
 }
 else if (toKey.expirationTimestamp <= block.timestamp) {
 _setKeyManagerOf(idTo, address(0));
 }
 ...
 toKey.expirationTimestamp = fromKey .expirationTimestamp.add(previousExpiration - block.timestamp);
 }
 fromKey.expirationTimestamp = block.timestamp;
 fromKey.tokenId = 0;
 ...
 function updateTransferFee( uint _transferFeeBasisPoints ) external onlyLockManager {
 emit TransferFeeChanged( _transferFeeBasisPoints );
 transferFeeBasisPoints = _transferFeeBasisPoints;
 }
 function getTransferFee( address _keyOwner, uint _time ) public view returns (uint) {
 ...
 pragma solidity 0.5.17;
 contract PublicLock is IPublicLock, Initializable, ERC165, MixinLockManagerRole, MixinKeyGranterRole, MixinSignatures, MixinFunds, MixinDisable, MixinLockCore, MixinKeys, MixinLockMetadata, MixinERC721Enumerable, MixinGrantKeys, MixinPurchase, MixinTransfer, MixinRefunds {
 function initialize( address _lockCreator, uint _expirationDuration, address _tokenAddress, uint _keyPrice, uint _maxNumberOfKeys, string memory _lockName ) public initializer() {
 ...
 function createLock( uint _expirationDuration, address _tokenAddress, uint _keyPrice, uint _maxNumberOfKeys, string calldata _lockName, bytes12 _salt ) external returns(address);
 function recordKeyPurchase( uint _value, address _referrer ) external;
 function recordConsumedDiscount( uint _discount, uint _tokens ) external;
 function computeAvailableDiscountFor( address _purchaser, uint _keyPrice ) external view returns(uint discount, uint tokens);
 ...
 function configUnlock( address _udt, address _weth, uint _estimatedGasForPurchase, string calldata _symbol, string calldata _URI, uint _chainId ) external;
 function setLockTemplate( address payable _publicLockAddress ) external;
 function resetTrackedValue( uint _grossNetworkProduct, uint _totalDiscountGranted ) external;
