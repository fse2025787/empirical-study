 ...
 uint256 compensationAmount = _calculateCompensating( msg.sender, peakPriceInUsdc );
 require(compensationAmount == 0, "have compensation");
 _;
 }
 constructor( address payable _fundAddr, address _peakTokenAddr, address _sharesTokenAddr, address _uniswapOracle ) public {
 ...
 function _lostFundAmount(address _investor) internal view returns (uint256 lostFundAmount) {
 ...
 function _calculateCompensating(address _investor, uint256 _peakPriceInUsdc) internal view returns (uint256) {
 uint256 totalFundsAtManagePhaseStart = fund .totalFundsAtManagePhaseStart();
 ...
 if ( fund.cyclePhase() != IPeakDeFiFund.CyclePhase.Intermission || managePhaseStartTime < _lastClaimTimestamp[_investor] || managePhaseStartTime < startProtectTimestamp[_investor] || mintedPeakTokens >= peakMintCap || peaks[_investor] == 0 || lostFundAmount == 0 || totalShares == 0 || _peakPriceInUsdc == 0 || sharesAmount == 0 ) {
 return 0;
 }
 uint256 sharesInUsdcAmount = sharesAmount .mul(totalFundsAtManagePhaseStart) .div(totalShares);
 uint256 peaksInUsdcAmount = peaks[_investor].mul(_peakPriceInUsdc).div( PEAK_PRECISION );
 uint256 protectedPercent = PERCENTS_DECIMALS;
 if (peaksInUsdcAmount < sharesInUsdcAmount) {
 protectedPercent = peaksInUsdcAmount.mul(PERCENTS_DECIMALS).div( sharesInUsdcAmount );
 }
 uint256 ownLostFundInUsd = lostFundAmount.mul(sharesAmount).div( totalShares );
 uint256 compensationInUSDC = ownLostFundInUsd.mul(protectedPercent).div( PERCENTS_DECIMALS );
 uint256 compensationInPeak = compensationInUSDC.mul(PEAK_PRECISION).div( _peakPriceInUsdc );
 ...
 function calculateCompensating(address _investor, uint256 _peakPriceInUsdc) public view returns (uint256) {
 ...
 function claimCompensation() external during(IPeakDeFiFund.CyclePhase.Intermission) nonReentrant {
 uint256 peakPriceInUsdc = _getPeakPriceInUsdc();
 uint256 compensationAmount = _calculateCompensating( msg.sender, peakPriceInUsdc );
 ...
 require( mintedPeakTokens <= peakMintCap, "ProtectionStaking: reached cap" );
 updateLastClaimAmount();
 emit ClaimCompensation(msg.sender, compensationAmount, block.timestamp);
 }
 function requestProtection(uint256 _amount) external during(IPeakDeFiFund.CyclePhase.Intermission) nonReentrant ifNoCompensation {
 ...
 require( peaks[msg.sender] >= _amount, "insufficient fund in Peak Token" );
 require(_amount > 0, "amount is 0");
 ...
 function protectShares(uint256 _amount) external nonReentrant during(IPeakDeFiFund.CyclePhase.Intermission) ifNoCompensation {
 require(_amount > 0, "amount is 0");
 IERC20(sharesToken).safeTransferFrom( msg.sender, address(this), _amount );
 startProtectTimestamp[msg.sender] = block.timestamp;
 shares[msg.sender] = shares[msg.sender].add(_amount);
 updateLastClaimAmount();
 emit ProtectShares(msg.sender, _amount, block.timestamp);
 }
 function withdrawShares(uint256 _amount) external nonReentrant ifNoCompensation {
 require( shares[msg.sender] >= _amount, "insufficient fund in Share Token" );
 require(_amount > 0, "amount is 0");
 ...
 uint256 priceInUSDC = uniswapOracle.consult( address(peakToken), PEAK_PRECISION );
 if (priceInUSDC == 0) {
 ...
 constructor( address _usdcAddr, address payable _kyberAddr, address payable _oneInchAddr ) public {
 USDC_ADDR = _usdcAddr;
 ...
 function __kyberTrade( ERC20Detailed _srcToken, uint256 _srcAmount, ERC20Detailed _destToken ) internal returns ( uint256 _destPriceInSrc, uint256 _srcPriceInDest, uint256 _actualDestAmount, uint256 _actualSrcAmount ) {
 ...
 _actualDestAmount = kyber.tradeWithHint.value(msgValue)( _srcToken, _srcAmount, _destToken, toPayableAddr(address(this)), MAX_QTY, 1, address(0), PERM_HINT );
 ...
 function __oneInchTrade( ERC20Detailed _srcToken, uint256 _srcAmount, ERC20Detailed _destToken, bytes memory _calldata ) internal returns ( uint256 _destPriceInSrc, uint256 _srcPriceInDest, uint256 _actualDestAmount, uint256 _actualSrcAmount ) {
 require(_srcToken != _destToken);
 uint256 beforeSrcBalance = getBalance(_srcToken, address(this));
 ...
 enum Subchunk {
 Propose, Vote }
 function initParams( address payable _devFundingAccount, uint256[2] calldata _phaseLengths, uint256 _devFundingRate, address payable _previousVersion, address _usdcAddr, address payable _kyberAddr, address _compoundFactoryAddr, address _peakdefiLogic, address _peakdefiLogic2, address _peakdefiLogic3, uint256 _startCycleNumber, address payable _oneInchAddr, address _peakRewardAddr, address _peakStakingAddr ) external;
 ...
 function initInternalTokens( address _repAddr, address _sTokenAddr, address _peakReferralTokenAddr ) external;
 function initRegistration( uint256 _newManagerRepToken, uint256 _maxNewManagersPerCycle, uint256 _reptokenPrice, uint256 _peakManagerStakeRequired, bool _isPermissioned ) external;
 function initTokenListings( address[] calldata _kyberTokens, address[] calldata _compoundTokens ) external;
 ...
 function investmentsCount(address _userAddr) external view returns (uint256 _count);
 function nextVersion() external view returns (address payable);
 function transferOwnership(address newOwner) external;
 function compoundOrdersCount(address _userAddr) external view returns (uint256 _count);
 function getPhaseLengths() external view returns (uint256[2] memory _phaseLengths);
 function commissionBalanceOf(address _manager) external returns (uint256 _commission, uint256 _penalty);
 function commissionOfAt(address _manager, uint256 _cycle) external returns (uint256 _commission, uint256 _penalty);
 ...
 function depositEtherAdvanced( bool _useKyber, bytes calldata _calldata, address _referrer ) external payable;
 function depositUSDC(uint256 _usdcAmount, address _referrer) external;
 function depositToken( address _tokenAddr, uint256 _tokenAmount, address _referrer ) external;
 function depositTokenAdvanced( address _tokenAddr, uint256 _tokenAmount, bool _useKyber, bytes calldata _calldata, address _referrer ) external;
 function withdrawEther(uint256 _amountInUSDC) external;
 function withdrawEtherAdvanced( uint256 _amountInUSDC, bool _useKyber, bytes calldata _calldata ) external;
 ...
 function withdrawTokenAdvanced( address _tokenAddr, uint256 _amountInUSDC, bool _useKyber, bytes calldata _calldata ) external;
 ...
 function sellLeftoverToken(address _tokenAddr, bytes calldata _calldata) external;
 function sellLeftoverCompoundOrder(address payable _orderAddress) external;
 function burnDeadman(address _deadman) external;
 function createInvestment( address _tokenAddress, uint256 _stake, uint256 _maxPrice ) external;
 function createInvestmentV2( address _sender, address _tokenAddress, uint256 _stake, uint256 _maxPrice, bytes calldata _calldata, bool _useKyber ) external;
 function sellInvestmentAsset( uint256 _investmentId, uint256 _tokenAmount, uint256 _minPrice ) external;
 function sellInvestmentAssetV2( address _sender, uint256 _investmentId, uint256 _tokenAmount, uint256 _minPrice, bytes calldata _calldata, bool _useKyber ) external;
 function createCompoundOrder( address _sender, bool _orderType, address _tokenAddress, uint256 _stake, uint256 _minPrice, uint256 _maxPrice ) external;
 function sellCompoundOrder( address _sender, uint256 _orderId, uint256 _minPrice, uint256 _maxPrice ) external;
 function repayCompoundOrder( address _sender, uint256 _orderId, uint256 _repayAmountInUSDC ) external;
 function emergencyExitCompoundTokens( address _sender, uint256 _orderId, address _tokenAddr, address _receiver ) external;
 ...
 function getExpectedRate( ERC20Detailed src, ERC20Detailed dest, uint256 srcQty ) external view returns (uint256 expectedRate, uint256 slippageRate);
 function tradeWithHint( ERC20Detailed src, uint256 srcAmount, ERC20Detailed dest, address payable destAddress, uint256 maxDestAmount, uint256 minConversionRate, address walletId, bytes calldata hint ) external payable returns (uint256);
 }
