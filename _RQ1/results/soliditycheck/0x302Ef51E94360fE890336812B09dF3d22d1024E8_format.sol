 ...
 admin = msg.sender;
 treasury = _treasury;
 require( IEthFundManager(_fundManager).token() == ETH, "fund manager token != ETH" );
 ...
 require( IEthFundManager(_fundManager).token() == ETH, "new fund manager token != ETH" );
 fundManager = IEthFundManager(_fundManager);
 ...
 BaseRewardPool private constant REWARD = BaseRewardPool(0x0A760466E1B4621579a82a39CB56Dda2F4E70f03);
 bool public shouldClaimExtras = true;
 StableSwapStEth private constant CURVE_POOL = StableSwapStEth(0xDC24316b9AE028F1497c275EB9192a3Ea0f67022);
 IERC20 private constant CURVE_LP = IERC20(0x06325440D014e39736583c165C2963BA99fAf14E);
 uint public slip = 100;
 uint private constant SLIP_MAX = 10000;
 uint private constant INDEX = 0;
 constructor(address _fundManager, address _treasury) StrategyEth(_fundManager, _treasury) {
 ...
 uint available = _withdraw(_amount);
 uint repaid = fundManager.repay{
 value: available}
 (available);
 require(repaid >= _min, "repaid < min");
 emit Repay(_amount, repaid);
 }
 function _swap( address _dex, address _tokenIn, uint _amount ) private {
 address[] memory path = new address[](2);
 path[0] = _tokenIn;
 path[1] = WETH;
 UniswapV2Router(_dex).swapExactTokensForETH( _amount, 1, path, address(this), block.timestamp );
 }
