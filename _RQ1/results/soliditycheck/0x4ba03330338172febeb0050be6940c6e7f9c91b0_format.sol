 ...
 function get_dy( int128 from, int128 to, uint256 _from_amount ) external view returns (uint);
 }
 ...
 ICurveFi(ypool).remove_liquidity(_amnt, [ ICurveFi(ypool).balances(0).mul(_aux).div(_t), ICurveFi(ypool).balances(1).mul(_aux).div(_t), ICurveFi(ypool).balances(2).mul(_aux).div(_t), ICurveFi(ypool).balances(3).mul(_aux).div(_t)]);
