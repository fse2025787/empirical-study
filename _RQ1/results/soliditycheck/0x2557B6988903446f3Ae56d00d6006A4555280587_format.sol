 ...
 enum Proposal {
 NOTHING, PENDING, ACCEPTED, DECLINED }
 Proposal proposalStatus;
 enum Status {
 NOTHING, TOGETHER, PAUSED, SEPARATED }
 ...
 event ProposalResponded( address indexed to, address indexed from, uint256 indexed _status );
 event ProposalCancelled(address indexed to, address indexed from);
 event GotUnited( address indexed from, address indexed to, uint256 indexed _timestamp, uint256 _registrationNumber );
 event UnionStatusUpdated( address indexed from, address indexed to, uint256 _status, uint256 indexed _timestamp );
 ...
 function getTokenIDS(address _add) external view returns (uint256[] memory) {
