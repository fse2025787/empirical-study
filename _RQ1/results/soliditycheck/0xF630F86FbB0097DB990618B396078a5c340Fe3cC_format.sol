 ...
 m_Index : iCard, m_Duration: duration, m_Level : level, m_DP : dp, m_DPK : dpk, m_SP : sp, m_IP : ip, m_Parts : parts }
 );
 }
 ...
 m_Index : iChip, m_Level : lv, m_LimitNum : limit, m_Part : part, m_UsedNum : 0 }
 );
 ...
 if (bonus == 0) return acc.transfer(bonus);
 }
 ...
 function GenChipIndex( uint seed, uint8 level, uint[] extWeight3, uint[] extWeight4, uint[] extWeight5 ) internal returns(uint32,uint) {
 uint random;
 ...
 (uint[] memory ext3, uint[] memory ext4, uint[] memory ext5 ) = GenExtWeightList(costA,costB,costC);
 ...
 return ( expireList, allChips[0], allChips[1], allChips[2] );
 }
