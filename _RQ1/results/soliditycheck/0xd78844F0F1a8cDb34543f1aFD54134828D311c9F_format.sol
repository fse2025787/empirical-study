 ...
 event SwappedOnTheOther( IERC20 indexed fromToken, IERC20 indexed destToken, uint256 fromTokenAmount, uint256 destTokenAmount, uint256 minReturn, uint256[] distribution );
 event SwappedOnUniswapV3( IERC20 indexed fromToken, IERC20 indexed toToken, uint256 fromTokenAmount, uint256 destTokenAmount, uint256 minReturn );
 event SwappedOnKyberDMM( IERC20 indexed fromToken, IERC20 indexed toToken, uint256 fromTokenAmount, uint256 destTokenAmount, uint256 minReturn );
 event SingleSwappedOnBalancerV2( IERC20 indexed fromToken, IERC20 indexed toToken, uint256 fromTokenAmount, uint256 destTokenAmount, uint256 minReturn );
 event BatchSwappedOnBalancerV2( IERC20 indexed fromToken, IERC20 indexed toToken, uint256 fromTokenAmount, uint256 destTokenAmount );
 uint256 internal constant DEXES_COUNT = 4;
 constructor( ) {
 }
 receive() external payable {
 }
 function swapOnTheOther( IERC20[][] calldata path, uint256 amount, uint256 minReturn, uint256[] memory distribution ) public payable nonReentrant returns (uint256 returnAmount) {
 function(IERC20[] calldata, uint256)[DEXES_COUNT] memory reserves = [ _swapOnUniswapV2, _swapOnSushiswap, _swapOnHelioswap, _swapOnCroDefiSwap ];
 require( distribution.length <= reserves.length, 'TOKordinator: distribution array should not exceed reserves array size.' );
 uint256 parts = 0;
 ...
 function getKyberDMMAmountsOut( uint256 amountIn, IERC20[] memory path, address[] memory poolsPath ) public view returns (uint256[] memory) {
 IERC20[] memory realPath = formatPath(path);
 ...
 address to = i < realPath.length - 2 ? address(uniswapV2.getPair(IERC20(output), realPath[i + 2])) : address(this);
 ...
 uint256 amountOut = amounts[i + 1];
 (uint256 amount0Out, uint256 amount1Out) = input == token0 ? (uint256(0), amountOut) : (amountOut, uint256(0));
 address to = i < realPath.length - 2 ? address(sushiswap.getPair(IERC20(output), realPath[i + 2])) : address(this);
 ...
 function _swapOnHelioswapInternal( IERC20 fromToken, IERC20 destToken, uint256 amount ) internal {
 ...
 pool.swap{
 value: amount}
 (fromToken, destToken, amount, returnAmount);
 }
 }
 ...
 function swapOnUniswapV3( IERC20 tokenIn, IERC20 tokenOut, bytes memory path, uint256 deadline, uint256 amountIn, uint256 amountOutMinimum ) public payable nonReentrant returns (uint256 returnAmount) {
 if (address(tokenIn) == address(0)) {
 require(msg.value >= amountIn, 'TOKordinator: value does not enough');
 }
 ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams( path, address(this), deadline, amountIn, amountOutMinimum );
 if (address(tokenIn) == address(0)) {
 returnAmount = swapRouter.exactInput{
 value: amountIn}
 (params);
 ...
 uint256 amountOut = amounts[i + 1];
 (uint256 amount0Out, uint256 amount1Out) = input == token0 ? (uint256(0), amountOut) : (amountOut, uint256(0));
 address to = i < realPath.length - 2 ? address(croDefiSwap.getPair(IERC20(output), realPath[i + 2])) : address(this);
 ...
 function swapOnKyberDMM( IERC20[] calldata path, address[] calldata poolsPath, uint256 amount, uint256 minReturn ) public payable nonReentrant returns (uint256 returnAmount) {
 if (address(path[0]) == address(0)) {
 ...
 function singleSwapOnBalancerV2( bytes32 poolId, IAsset assetIn, IAsset assetOut, uint256 amountIn, uint256 amountOutMinimum, uint256 deadline ) public payable nonReentrant returns (uint256 returnAmount) {
 if (address(assetIn) == address(0)) {
 require(msg.value >= amountIn, 'TOKordinator: value does not enough');
 }
 IBalancerV2Vault.SingleSwap memory singleSwap = IBalancerV2Vault.SingleSwap( poolId, IBalancerV2Vault.SwapKind.GIVEN_IN, assetIn, assetOut, amountIn, '0x' );
 IBalancerV2Vault.FundManagement memory funds = IBalancerV2Vault.FundManagement( address(this), false, address(this), false );
 if (address(assetIn) == address(0)) {
 returnAmount = balancer.swap{
 value: amountIn}
 ( singleSwap, funds, amountOutMinimum, deadline );
 }
 else {
 ...
 returnAmount = balancer.swap( singleSwap, funds, amountOutMinimum, deadline );
 }
 ...
 function batchSwapOnBalancerV2( IERC20 tokenIn, IERC20 tokenOut, IBalancerV2Vault.BatchSwapStep[] memory swaps, IAsset[] memory assets, int256[] memory limits, uint256 amountOutMinimum, uint256 deadline ) public payable nonReentrant returns (uint256 returnAmount) {
 ...
 value: amountIn}
 ( IBalancerV2Vault.SwapKind.GIVEN_IN, swaps, assets, funds, limits, deadline );
 }
 else {
 TransferHelper.safeTransferFrom(address(tokenIn), msg.sender, address(this), amountIn);
 ...
 returnAmounts = balancer.batchSwap( IBalancerV2Vault.SwapKind.GIVEN_IN, swaps, assets, funds, limits, deadline );
 }
 ...
 function pools(IERC20 token1, IERC20 token2) external view returns ( IHelioswap, uint256, uint256, uint256, uint256[2] memory, uint256[2] memory );
 }
 library HelioswapLibrary {
 function getReturns( IHelioswapFactory helioswapFactory, uint256 amountIn, IERC20[] memory path ) internal view returns (uint256[] memory amounts) {
 require(path.length >= 2, 'TOKordinator: invalid path');
 ...
 function isPool( IERC20 token0, IERC20 token1, IERC20 pool ) external view returns (bool);
 }
 ...
 (reserveA, reserveB, vReserveA, vReserveB) = address(tokenA) == token0 ? (reserve0, reserve1, vReserve0, vReserve1) : (reserve1, reserve0, vReserve1, vReserve0);
 }
 ...
 amounts[0] = amountIn;
 for (uint256 i; i < path.length - 1; i++) {
 ( uint256 reserveIn, uint256 reserveOut, uint256 vReserveIn, uint256 vReserveOut, uint256 feeInPrecision ) = getTradeInfo(poolsPath[i], path[i], path[i + 1]);
 amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, vReserveIn, vReserveOut, feeInPrecision);
 }
 }
 function verifyPoolsPathSwap( IKyberDMMFactory kyber, IERC20[] memory poolsPath, IERC20[] memory path ) internal view {
 require(path.length >= 2, 'TOKordinator: invalid path');
