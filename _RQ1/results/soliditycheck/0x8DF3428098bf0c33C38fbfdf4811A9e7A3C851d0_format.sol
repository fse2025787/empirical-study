 ...
 function withdrawToken( IERC20 token, uint256 amount, address sendTo ) external onlyAdmin {
 ...
 function distributeToOne( address payable winner, IERC20 token, uint256 amount, GasToken gasToken ) public onlyOperator {
 ...
 function distributeToMany( address[] memory winners, IERC20 token, uint256 amount, GasToken gasToken ) public onlyOperator {
 for (uint256 i = 0; i < winners.length; i++) {
 distributeToOne(payable(winners[i]), token, amount, gasToken);
 }
 }
 function _distributeWithGasToken( address payable winner, IERC20 token, uint256 amount, GasToken gasToken ) private discount(gasToken) {
 _distribute(winner, token, amount);
 }
 function _distributeWithoutGasToken( address payable winner, IERC20 token, uint256 amount ) private {
 _distribute(winner, token, amount);
 }
 function _distribute( address payable winner, IERC20 token, uint256 amount ) private {
