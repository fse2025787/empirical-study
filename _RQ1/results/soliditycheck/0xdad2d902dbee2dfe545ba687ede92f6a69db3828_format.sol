 ...
 interface IMultiOwned is IMultiOwnedEvents, IMultiOwnedState, IMultiOwnedActions {
 }
 ...
 for (uint i = 1; i < length; ++i) if (pendingOf[i].yetNeeded != 0) delete pendingOf[i];
 nextPendingTxId = 1;
 }
 }
 ...
 function txsOf(uint txId) external view returns( address to, uint value, bytes memory data );
 ...
 constructor(address[] memory _owners, uint _required) MultiOwned(_owners, _required) {
 }
 ...
 uint length = nextPendingTxId;
 for (uint i = 1; i < length; ++i) if (txsOf[i].to != address(0)) delete txsOf[i];
 super.clearPending();
 }
 }
