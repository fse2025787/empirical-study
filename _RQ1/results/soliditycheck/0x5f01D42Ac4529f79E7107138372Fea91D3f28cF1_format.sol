 ...
 constructor( address _baseToken, address _depositor, address _governor ) {
 ...
 constructor( address _baseToken, address _depositor, address _governor ) VaultBase(_baseToken, _depositor, _governor) {
 }
 ...
 return _balance.mul(rewardsPerShareStored[_index].sub(userRewardPerSharePaid[_account][_index])).div(PRECISION).add( rewards[_account][_index] );
 }
 ...
 uint256 public pid;
 constructor( address _baseToken, address _depositor, address _governor, address _booster, uint256 _pid ) MultipleRewardsVaultBase(_baseToken, _depositor, _governor) {
 ...
 constructor(address _depositor, address _governor) BaseConvexVault( address(0xc4AD29ba4B3c580e6D59105FFf484999997675Ff), _depositor, _governor, address(0xF403C135812408BFbE8713b5A23a04b3D48AAE31), 38 ) {
 ...
 function currentEpoch() external view returns ( uint64 epochNumber, uint64 startBlock, uint64 nextBlock, uint64 epochLength );
 ...
 function getAccountRewardShareSince( uint256 _epoch, address _user, address _vault ) external view returns (uint256[] memory);
