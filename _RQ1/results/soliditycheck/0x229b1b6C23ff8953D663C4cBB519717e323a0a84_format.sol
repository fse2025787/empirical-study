 ...
 constructor( address signer_, address token_, uint256 startDate_, uint256 vestingDuration_, uint256 totalAllocatedAmount_ ) {
 require(signer_ != address(0), "Invalid signer address");
 require(token_ != address(0), "Invalid reward token address");
 require( startDate_ > block.timestamp, "TGE timestamp can't be less than block timestamp" );
 require(vestingDuration_ > 0, "The vesting duration cannot be 0");
 require( totalAllocatedAmount_ > 0, "The number of tokens for distribution cannot be 0" );
 token = IERC20(token_);
 ...
 function changeSignerList(address signer, bool permission) public onlyOwner {
 changePermission(signer, permission);
 }
 function emergencyTokenWithdraw(address tokenAddress_, uint256 amount) external onlyOwner {
 require(block.timestamp > vestingTimeEnd, "Vesting is still running");
 IERC20 tokenAddress = IERC20(tokenAddress_);
 require( tokenAddress.balanceOf(address(this)) >= amount, "Insufficient tokens balance" );
 tokenAddress.safeTransfer(msg.sender, amount);
 }
 function isValidData( address addr, uint256 portionLP, uint256 portionNative, uint256 deadline, uint8 v, bytes32 r, bytes32 s ) public returns (bool) {
 require( uint256(s) <= 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0, "ECDSA: invalid signature 's' value" );
 require(v == 27 || v == 28, "ECDSA: invalid signature 'v' value");
 bytes32 message = keccak256( abi.encodePacked( address(this), addr, portionLP, portionNative, nonces[addr].current(), deadline ) );
 ...
 function withdrawReward( uint256 portionLP, uint256 portionNative, uint256 deadline, uint8 v, bytes32 r, bytes32 s ) public {
 require( portionLP <= PERCENTAGE && portionNative <= PERCENTAGE, "The percentage cannot be greater than 100" );
 require(deadline >= block.timestamp, "Expired");
 bool access = isValidData( msg.sender, portionLP, portionNative, deadline, v, r, s );
 require(access, "Permission not granted");
 _withdrawReward(msg.sender, portionLP, portionNative);
 }
 function getRewardBalance( address beneficiary, uint256 percentageLP, uint256 percentageNative ) public view returns (uint256 amount) {
 ...
 function _withdrawReward( address beneficiary, uint256 percentageLP, uint256 percentageNative ) private {
 uint256 reward = _getRewardBalance(percentageLP, percentageNative);
 Investor storage investor = investorInfo[beneficiary];
 uint256 balance = token.balanceOf(address(this));
 ...
 function _getRewardBalance(uint256 lpPercentage, uint256 nativePercentage) private view returns (uint256) {
 uint256 vestingAvailablePercentage = _calculateAvailablePercentage();
 uint256 amountAvailableForLP = tokensForLP .mul(vestingAvailablePercentage) .div(PERCENTAGE);
 uint256 amountAvailableForNative = tokensForNative .mul(vestingAvailablePercentage) .div(PERCENTAGE);
 uint256 rewardToPayLP = amountAvailableForLP.mul(lpPercentage).div( PERCENTAGE );
 uint256 rewardToPayNative = amountAvailableForNative .mul(nativePercentage) .div(PERCENTAGE);
 return rewardToPayLP.add(rewardToPayNative);
 }
 function _calculateAvailablePercentage() internal view virtual returns (uint256) {
 ...
 constructor( address signer_, address token_, uint256 startDate_, uint256 vestingDuration_, uint256 totalAllocatedAmount_ ) BaseVesting( signer_, token_, startDate_, vestingDuration_, totalAllocatedAmount_ ) {
 }
 }
 pragma solidity >=0.6.0 <0.8.0;
 library SafeMath {
