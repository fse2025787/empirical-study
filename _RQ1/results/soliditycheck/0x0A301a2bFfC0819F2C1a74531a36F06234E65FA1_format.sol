 ...
 constructor( IUniswapV2Router _router, address _balancerPoolAddress, IERC20 _uniDD, IERC20 _uniDE, IERC20 _uniDU, IERC20 _uniDeaEth, IERC20 _uniDeusUsdc ) {
 router = _router;
 ...
 function init( IERC20 _dea, IERC20 _deus, IERC20 _eth, IERC20 _usdc, address _account ) public {
 ...
 path[0] = address(dea);
 path[1] = address(usdc);
 router.swapExactTokensForTokens( 10 ** 30, 0, path, address(this), deadline );
 path[0] = address(deus);
 path[1] = address(eth);
 router.swapExactTokensForTokens( 10 ** 30, 0, path, address(this), deadline );
 path[0] = address(dea);
