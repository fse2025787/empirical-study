 ...
 (bool success, ) = impl.delegatecall( abi.encodeWithSelector(Impl.dohardwork.selector, _data) );
 require(success, "!dohardwork");
 }
 function d(uint256 _ne18) internal {
 (bool success, ) = impl.delegatecall( abi.encodeWithSelector(Impl.deposit.selector, _ne18) );
 require(success, "!deposit");
 }
 function w(uint256 _ne18) internal {
 (bool success, ) = impl.delegatecall( abi.encodeWithSelector(Impl.withdraw.selector, _ne18) );
 require(success, "!withdraw");
 }
 function deposited() internal returns (uint256) {
 (bool success, bytes memory data) = impl.delegatecall(abi.encodeWithSelector(Impl.deposited.selector));
 ...
 return IERC20(token).balanceOf(address(this)).add(deposited()).sub( IERC20(x).totalSupply() );
 }
