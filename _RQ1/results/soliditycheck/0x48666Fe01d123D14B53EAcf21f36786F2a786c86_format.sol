 ...
 string public website = "https: string public telegram = "https: struct CoinTypeInfo {
 address coinAddress;
 address[] routerPath;
 }
 ...
 if ( swapAndWithdrawEnabled == true && _inSwapAndWithdraw == false && address(uniswapRouter) != address(0) && uniswapPair != address(0) && _sender != uniswapPair && _sender != address(uniswapRouter) && _sender != owner() && _sender != address(this) ) {
 swapAndWithdraw();
 }
 ...
 uniswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens( tokenAmount, 0, path, address(this), block.timestamp + 1 days );
 }
 ...
 coinAddress: _coinAddr, routerPath: _path }
 ));
 }
