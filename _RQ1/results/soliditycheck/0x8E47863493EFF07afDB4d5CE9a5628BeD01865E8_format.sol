 ...
 function _beforeConsecutiveTokenTransfer( address from, address to, uint256, uint96 size ) internal virtual {
 if (from != address(0)) {
 _balances[from] -= size;
 }
 if (to != address(0)) {
 ...
 function _afterConsecutiveTokenTransfer( address, address, uint256, uint96 ) internal virtual {
 }
 ...
 function _beforeConsecutiveTokenTransfer( address, address, uint256, uint96 size ) internal virtual override {
 if (size > 0) {
 ...
 function numbersDrawn( uint256 _randomNumber ) external {
 randomNumber = _randomNumber;
 isRandomNumber = true;
 emit setRandomNumber();
 }
 function initialize( address _randomNumberAddress, address _adminAddress ) public initializer {
 __Ownable_init();
 ...
 modifier onlySOM() {
 require( adminAddress == msg.sender || ownerAddress == msg.sender || SomAddress == msg.sender, "RNG: Caller is not the SOM address" );
 _;
 }
 function setJoinItems(uint256 _tokenId, uint256 _state) external onlySOM {
 joinItems.push(SOM({
 tokenId: _tokenId, state: _state }
 ));
 }
 ...
 Coward.push(SOM({
 tokenId: CowardTokenIDs[i], state: 15 }
 ));
 ...
 event randomNumberArrived( bool arrived, uint256 randomNumber, bytes32 batchID );
 modifier onlySOM() {
 ...
 require( LINK.balanceOf(address(this)) >= fee, "RandomNumberConsumer: Not enough LINK - fill contract with faucet" );
 ...
 CowardGambit(CowardAddress).numbersDrawn( _randomness );
 emit randomNumberArrived(true, _randomness, requestID);
 }
 function getVerifiedRandomNumber(bytes32 _reqeustId) public view onlySOM returns (uint256) {
 require( hasReturned[_reqeustId] == true, "RandomGenerator: Random number is not arrived yet" );
 return requestToRandom[_reqeustId];
 }
 function setSOMAddress(address _SOMAddr) public onlyOwner {
 require( _SOMAddr.isContract() == true, "RandomGenerator: This is not a Contract Address" );
 SOMAddress = _SOMAddr;
 }
 function setCowardAddress(address _address) public onlyOwner {
 require( _address.isContract() == true, "RandomGenerator: This is not a Contract Address" );
 ...
 event SOMMinted();
 function initialize( string memory baseTokenURI_, string memory unrevealedURI_, address _adminAddress, address _BankAddress, address _devAddress, uint256[] memory randomorder_ ) public initializer {
 ...
 modifier onlySOM() {
 require( adminAddress == msg.sender || owner() == msg.sender || CowardAddress == msg.sender, "RNG: Caller is not the SOM address" );
 _;
 }
 ...
 return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI, Strings.toString((Randomorder[tokenId - 1] - 1) * 19 + soms[tokenId].state), "")) : "";
 }
