 ...
 function depositToken( uint256 _amount, address _module, address _inputToken, address payable _owner, address _witness, bytes calldata _data, bytes32 _secret ) external;
 }
 ...
 function encodeTokenOrder( address _module, IERC20 _inputToken, address payable _owner, address _witness, bytes calldata _data, bytes32 _secret, uint256 _amount ) external view returns (bytes memory);
 function encodeEthOrder( address _module, address _inputToken, address payable _owner, address _witness, bytes calldata _data, bytes32 _secret ) external pure returns (bytes memory);
 function depositEth(bytes calldata _data) external payable;
 function cancelOrder( address _module, IERC20 _inputToken, address payable _owner, address _witness, bytes calldata _data ) external;
 function existOrder( address _module, IERC20 _inputToken, address payable _owner, address _witness, bytes calldata _data ) external view returns (bool);
 function executeOrder( address _module, address _inputToken, address payable _owner, bytes calldata _data, bytes calldata _signature, bytes calldata _auxData ) external;
 function canExecuteOrder( address _module, IERC20 _inputToken, address payable _owner, address _witness, bytes calldata _data, bytes calldata _auxData ) external view returns (bool);
 function vaultOfOrder( address _module, address _inputToken, address payable _owner, address _witness, bytes calldata _data ) external view returns (address);
 }
 pragma solidity ^0.6.0;
 ...
 constructor( address _gelatoPine, address _module, address _erc20OrderRouter ) public {
 ...
 function placeLimitOrder( uint256 value, address module, address inToken, address payable owner, address witness, bytes calldata limitOrderData, bytes32 secret ) external payable {
 value = _getBalance(inToken, value);
 if (inToken == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) {
 try IGelatoPineCore(GELATO_PINE).depositEth{
 value: value}
 ( IGelatoPineCore(GELATO_PINE).encodeEthOrder( GELATO_LIMIT_ORDER_MODULE, inToken, payable(_getSender()), witness, limitOrderData, secret ) ) {
 }
 ...
 try IERC20OrderRouter(ERC20_ORDER_ROUTER).depositToken( value, GELATO_LIMIT_ORDER_MODULE, inToken, owner, witness, limitOrderData, secret ) {
 }
 catch Error(string memory reason) {
 _revertMsg("placeLimitOrder", reason);
 }
