 ...
 event Disabled();
 modifier ownerOnly() {
 require(msg.sender == creator, "Sorry, you're not the owner of this contract");
 _;
 }
 modifier nilBalance() {
 ...
 require(contributionTime > 0, "No contributions have been received");
 require(now > (contributionTime + HOLD_TIME), "Payments are on hold");
 _;
 }
 modifier enabled() {
 require(!isDisabled, "This contract has been disabled");
 _;
 }
