 ...
 event RoyaltiesDefined( uint256 indexed id, address indexed recipient, uint256 value );
 event RoyaltiesReceived( uint256 indexed id, address indexed recipient, uint256 value );
 ...
 require( maxAllowedRoyalty <= 10000, 'Royalties: max royalty can not be more than 100%' );
 _maxRoyalty = maxAllowedRoyalty;
 }
 function _setRoyalties( uint256 id, address recipient, uint256 value ) internal {
 require( recipient != address(0), 'Royalties: Royalties recipient can not be null address' );
 require( value <= _maxRoyalty, 'Royalties: Royalties can not be more than the defined max royalty' );
 ...
 function requireOperatorSignature( bytes32 message, uint8 v, bytes32 r, bytes32 s ) public view {
 ...
 function _setNnextGroup( string memory currentGroupNewURI, string memory nextGroupBaseURI ) internal {
 ...
 event ConfigurationURI( uint256 indexed tokenId, address indexed owner, string configurationURI );
 mapping(uint256 => mapping(address => string)) private _interactiveConfURIs;
 function _setInteractiveConfURI( uint256 tokenId, address owner, string calldata interactiveConfURI_ ) internal virtual {
 _interactiveConfURIs[tokenId][owner] = interactiveConfURI_;
 emit ConfigurationURI(tokenId, owner, interactiveConfURI_);
 }
 function interactiveConfURI(uint256 tokenId, address owner) public view virtual returns (string memory) {
 ...
 function __ERC1155WithMetadata_init(string memory uri_) internal initializer {
 __ERC1155_init_unchained(uri_);
 }
 ...
 function _setMetadata( uint256 id, string memory tokenURI, address _creator ) internal {
 ...
 function onRoyaltiesReceived(uint256 id) external payable override returns (bytes4) {
 ...
 function royaltyInfo( uint256 tokenId, uint256 value, bytes calldata ) external view returns ( address receiver, uint256 royaltyAmount, bytes memory royaltyPaymentData ) {
 ...
 pragma solidity >=0.6.0 <0.8.0;
 contract BeyondNFT1155V2 is OwnerOperatorControlWithSignature, ERC1155Configurable, ERC1155WithRoyalties, ERC1155WithMetadata, GroupedURI {
 receive() external payable {
 revert('No value accepted');
 }
 function mint( uint256 id, uint256 supply, string memory uri_, uint8 v, bytes32 r, bytes32 s, uint256 royalties, address royaltiesRecipient ) external {
 require(!minted(id), 'ERC1155: Already minted');
 address sender = _msgSender();
 requireOperatorSignature( prepareMessage(sender, id, supply, uri_), v, r, s );
 ...
 require( owner == _msgSender() || isApprovedForAll(owner, _msgSender()), 'ERC1155: caller is not owner nor approved' );
 ...
 function safeBatchTransferIdFrom( address from, address[] memory tos, uint256 id, uint256[] memory amounts, bytes memory data ) public virtual {
 require(tos.length == amounts.length, 'ERC1155: length mismatch');
 for (uint256 i = 0; i < tos.length; i++) {
 safeTransferFrom(from, tos[i], id, amounts[i], data);
 }
 }
 function setInteractiveConfURI( uint256 tokenId, address owner, string calldata interactiveConfURI ) public {
 require( owner == _msgSender() || isApprovedForAll(owner, _msgSender()), 'ERC1155: caller is not owner nor approved' );
 _setInteractiveConfURI(tokenId, owner, interactiveConfURI);
 }
 function prepareMessage( address sender, uint256 id, uint256 supply, string memory uri_ ) public pure returns (bytes32) {
 return keccak256(abi.encode(sender, id, supply, uri_));
 }
 function setBatchTokenMetadata( uint256[] memory ids, string[] memory uris, address[] memory creators ) external onlyOwner {
 for (uint256 i; i < ids.length; i++) {
 ...
 function setNextGroup( string memory currentGroupNewURI, string memory nextGroupBaseURI ) external onlyOwner {
