 ...
 constructor( uint256 givenPricePerPiece, address payable givenWithdrawalAddress ) ERC721("Colour Time", "CT") {
 ...
 parts[0] = '<svg xmlns="http: parts[1] = tokenDataMap[tokenId].colours[0];
 parts[2] = '" dur="';
 ...
 "name": "Colour Time ', tokenDataMap[tokenId].title, '", "description": "Slowly shifting colours, records of time past. Jonathan Chomko, 2021", "image": "data:image/svg+xml;
 base64,',Base64.encode(bytes(output)),'"}
 '))));
 output = string(abi.encodePacked('data:application/json;
 base64,', json));
 return output;
 }
 ...
 {
 i := add(i, 3) let input := and(mload(add(data, i)), 0xffffff) let out := mload(add(tablePtr, and(shr(18, input), 0x3F))) out := shl(8, out) out := add(out, and(mload(add(tablePtr, and(shr(12, input), 0x3F))), 0xFF)) out := shl(8, out) out := add(out, and(mload(add(tablePtr, and(shr(6, input), 0x3F))), 0xFF)) out := shl(8, out) out := add(out, and(mload(add(tablePtr, and(input, 0x3F))), 0xFF)) out := shl(224, out) mstore(resultPtr, out) resultPtr := add(resultPtr, 4) }
 switch mod(len, 3) case 1 {
