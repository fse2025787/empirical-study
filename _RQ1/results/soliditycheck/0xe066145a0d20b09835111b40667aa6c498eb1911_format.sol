 ...
 require( calculateProfit(_betSize, _playerNumber) < maxProfit && _betSize >= minBet && _playerNumber > minNumber && _playerNumber < maxNumber);
 _;
 }
 ...
 function _playerRollDice(uint _rollUnder, TKN _tkn) private gameIsActive betIsValid(_tkn.value, _rollUnder) {
 ...
 function finishBet() public gameIsActive returns (uint) {
 ...
 function ownerUpdateContractBalance(uint newContractBalance) public onlyOwner {
 ...
 function ownerTransferZTH(address sendTo, uint amount) public onlyOwner {
 ...
 function ownerSetBankroll(address newBankroll) public onlyOwner {
