 ...
 event FundCreated( address indexed manager, address indexed token, address indexed fund );
 ...
 function init( address token0, address token1, uint24 fee, int24 tickLower, int24 tickUpper, uint amount ) external;
 function add( uint poolIndex, uint positionIndex, uint amount, bool collect ) external;
 function sub( uint poolIndex, uint positionIndex, uint proportionX128 ) external;
 function move( uint poolIndex, uint subIndex, uint addIndex, uint proportionX128 ) external;
 }
 interface IManagerActions {
 function setPath( address fund, address distToken, bytes memory path ) external;
 function init( address fund, address token0, address token1, uint24 fee, int24 tickLower, int24 tickUpper, uint amount ) external;
 function add( address fund, uint poolIndex, uint positionIndex, uint amount, bool collect ) external;
 function sub( address fund, uint poolIndex, uint positionIndex, uint proportionX128 ) external;
 function move( address fund, uint poolIndex, uint subIndex, uint addIndex, uint proportionX128 ) external;
 }
 ...
 function parameters() external view returns ( address weth9, address uniV3Factory, address uniswapV3Router, address controller, address manager, address token, bytes32 descriptor );
 }
 ...
 assembly {
 switch iszero(_length) case 0 {
 tempBytes := mload(0x40) let lengthmod := and(_length, 31) let mc := add(add(tempBytes, lengthmod), mul(0x20, iszero(lengthmod))) let end := add(mc, _length) for {
 let cc := add(add(add(_bytes, lengthmod), mul(0x20, iszero(lengthmod))), _start) }
 lt(mc, end) {
 mc := add(mc, 0x20) cc := add(cc, 0x20) }
 {
 mstore(mc, mload(cc)) }
 mstore(tempBytes, _length) mstore(0x40, and(add(mc, 31), not(31))) }
 default {
 tempBytes := mload(0x40) mstore(tempBytes, 0) mstore(0x40, add(tempBytes, 0x20)) }
 }
 return tempBytes;
 }
 ...
 function positions(uint poolIndex, uint positionIndex) external view returns( bool isEmpty, int24 tickLower, int24 tickUpper );
 ...
 interface IHotPotV2Fund is IHotPotV2FundERC20, IHotPotV2FundEvents, IHotPotV2FundState, IHotPotV2FundUserActions, IHotPotV2FundManagerActions {
 }
 ...
 function getSqrtPriceX96( bytes memory path, address uniV3Factory, bool isCurrentPrice ) internal view returns (uint160 sqrtPriceX96){
 ...
 sqrtPriceX96 = uint160( tokenIn > tokenOut ? FullMath.mulDiv(sqrtPriceX96, FixedPoint96.Q96, _sqrtPriceX96) : FullMath.mulDiv(sqrtPriceX96, _sqrtPriceX96, FixedPoint96.Q96) );
 if (path.hasMultiplePools()) path = path.skipToken();
 else return sqrtPriceX96;
 }
 }
 }
 library Position {
 ...
 function getAmountsForAmount0( uint160 sqrtPriceX96, uint160 sqrtPriceL96, uint160 sqrtPriceU96, uint deltaX ) internal pure returns(uint amount0, uint amount1){
 ...
 if(deltaX > amount0){
 amount1 = FullMath.mulDiv( deltaX.sub(amount0), FullMath.mulDiv(sqrtPriceX96, sqrtPriceX96, FixedPoint96.Q96), FixedPoint96.Q96 );
 }
 }
 struct SwapParams{
 uint amount;
 ...
 function computeSwapAmounts( SwapParams memory params, mapping(address => bytes) storage buyPath ) internal returns(uint amount0Max, uint amount1Max) {
 uint equalAmount0;
 uint160 buy0Price;
 if(params.amount > 0){
 ...
 equalAmount0 = params.amount0.add((FullMath.mulDiv( params.amount, FullMath.mulDiv(buy0Price, buy0Price, FixedPoint96.Q96), FixedPoint96.Q96 )));
 }
 }
 else equalAmount0 = params.amount0;
 if(params.amount1 > 0){
 equalAmount0 = equalAmount0.add((FullMath.mulDiv( params.amount1, FixedPoint96.Q96, FullMath.mulDiv(params.sqrtPriceX96, params.sqrtPriceX96, FixedPoint96.Q96) )));
 }
 ...
 fundToT0 = FullMath.mulDiv( amount0Max - params.amount0, FixedPoint96.Q96, FullMath.mulDiv(buy0Price, buy0Price, FixedPoint96.Q96) );
 if(fundToT0 > params.amount) fundToT0 = params.amount;
 if(fundToT0 > 0) {
 amount0Max = params.amount0.add(ISwapRouter(params.uniV3Router).exactInput(ISwapRouter.ExactInputParams({
 path: buyPath[params.token0], recipient: address(this), deadline: block.timestamp, amountIn: fundToT0, amountOutMinimum: 0 }
 )));
 }
 ...
 path: buyPath[params.token1], recipient: address(this), deadline: block.timestamp, amountIn: params.amount.sub(fundToT0), amountOutMinimum: 0 }
 )));
 }
 ...
 amount0Max = params.amount0.add(ISwapRouter(params.uniV3Router).exactInput(ISwapRouter.ExactInputParams({
 path: abi.encodePacked(params.token1, params.fee, params.token0), recipient: address(this), deadline: block.timestamp, amountIn: params.amount1.sub(amount1Max), amountOutMinimum: 0 }
 )));
 }
 else amount0Max = params.amount0;
 if (params.amount > 0){
 ...
 path: buyPath[params.token0], recipient: address(this), deadline: block.timestamp, amountIn: params.amount, amountOutMinimum: 0 }
 )));
 }
 }
 }
 ...
 if(amount0Max < params.amount0){
 amount1Max = params.amount1.add(ISwapRouter(params.uniV3Router).exactInput(ISwapRouter.ExactInputParams({
 path: abi.encodePacked(params.token0, params.fee, params.token1), recipient: address(this), deadline: block.timestamp, amountIn: params.amount0.sub(amount0Max), amountOutMinimum: 0 }
 )));
 }
 else amount1Max = params.amount1;
 if(params.amount > 0){
 ...
 path: buyPath[params.token1], recipient: address(this), deadline: block.timestamp, amountIn: params.amount, amountOutMinimum: 0 }
 )));
 }
 }
 }
 ...
 function addLiquidity( Info storage self, AddParams memory params, mapping(address => bytes) storage sellPath, mapping(address => bytes) storage buyPath ) public {
 ...
 (uint amount0, uint amount1) = IUniswapV3Pool(params.pool).mint( address(this), tickLower, tickUpper, liquidity, abi.encode(params.poolIndex) );
 ...
 path: sellPath[swapParams.token0], recipient: address(this), deadline: block.timestamp, amountIn: params.amount0Max - amount0, amountOutMinimum: 0 }
 ));
 }
 }
 ...
 ISwapRouter(params.uniV3Router).exactInput(ISwapRouter.ExactInputParams({
 path: sellPath[swapParams.token1], recipient: address(this), deadline: block.timestamp, amountIn: params.amount1Max - amount1, amountOutMinimum: 0 }
 ));
 }
 }
 if(self.isEmpty) self.isEmpty = false;
 }
 function burnAndCollect( Info storage self, address pool, uint proportionX128 ) public returns(uint amount0, uint amount1) {
 ...
 function subLiquidity ( Info storage self, SubParams memory params, mapping(address => bytes) storage sellPath ) public returns(uint amount) {
 ...
 path: sellPath[token0], recipient: address(this), deadline: block.timestamp, amountIn: amount0, amountOutMinimum: 0 }
 ));
 }
 if(token1 != params.token && amount1 > 0){
 amount = amount.add(ISwapRouter(params.uniV3Router).exactInput(ISwapRouter.ExactInputParams({
 path: sellPath[token1], recipient: address(this), deadline: block.timestamp, amountIn: amount1, amountOutMinimum: 0 }
 )));
 }
 }
 struct AssetsParams {
 ...
 function assetsOfPool( Info[] storage self, address pool, address token, mapping(address => bytes) storage sellPath, address uniV3Factory ) public view returns (uint amount, uint[] memory) {
 ...
 (uint256 _amount0, uint256 _amount1) = getAssetsOfSinglePosition( AssetsOfSinglePosition({
 pool: pool, positionKey: positionKey, tickLower: position.tickLower, tickUpper: position.tickUpper, tickCurrent: params.tick, sqrtPriceX96: params.sqrtPriceX96, feeGrowthGlobal0X128: params.feeGrowthGlobal0X128, feeGrowthGlobal1X128: params.feeGrowthGlobal1X128 }
 ) );
 uint _amount;
 if(params.token0 != token){
 _amount = FullMath.mulDiv( _amount0, FullMath.mulDiv(params.price0, params.price0, FixedPoint96.Q96), FixedPoint96.Q96);
 }
 else _amount = _amount0;
 if(params.token1 != token){
 _amount = _amount.add(FullMath.mulDiv( _amount1, FullMath.mulDiv(params.price1, params.price1, FixedPoint96.Q96), FixedPoint96.Q96));
 }
 else _amount = _amount.add(_amount1);
 amounts[i] = _amount;
 amount = amount.add(_amount);
 }
 return(amount, amounts);
 }
 function assets( Info storage self, address pool, address token, mapping(address => bytes) storage sellPath, address uniV3Factory ) public view returns (uint amount) {
 if(self.isEmpty) return 0;
 (uint160 sqrtPriceX96, int24 tick, , , , , ) = IUniswapV3Pool(pool).slot0();
 bytes32 positionKey = keccak256(abi.encodePacked(address(this), self.tickLower, self.tickUpper));
 (uint256 amount0, uint256 amount1) = getAssetsOfSinglePosition( AssetsOfSinglePosition({
 pool: pool, positionKey: positionKey, tickLower: self.tickLower, tickUpper: self.tickUpper, tickCurrent: tick, sqrtPriceX96: sqrtPriceX96, feeGrowthGlobal0X128: IUniswapV3Pool(pool).feeGrowthGlobal0X128(), feeGrowthGlobal1X128: IUniswapV3Pool(pool).feeGrowthGlobal1X128() }
 ) );
 if(amount0 > 0){
 address token0 = IUniswapV3Pool(pool).token0();
 if(token0 != token){
 uint160 price0 = PathPrice.getSqrtPriceX96(sellPath[token0], uniV3Factory, false);
 amount = FullMath.mulDiv( amount0, FullMath.mulDiv(price0, price0, FixedPoint96.Q96), FixedPoint96.Q96);
 }
 else amount = amount0;
 }
 if(amount1 > 0){
 ...
 amount = amount.add(FullMath.mulDiv( amount1, FullMath.mulDiv(price1, price1, FixedPoint96.Q96), FixedPoint96.Q96));
 }
 ...
 function getAssetsOfSinglePosition(AssetsOfSinglePosition memory params) internal view returns (uint256 amount0, uint256 amount1) {
 ( uint128 liquidity, uint256 feeGrowthInside0LastX128, uint256 feeGrowthInside1LastX128, uint128 tokensOwed0, uint128 tokensOwed1 ) = IUniswapV3Pool(params.pool).positions(params.positionKey);
 (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128) = getFeeGrowthInside( FeeGrowthInsideParams({
 pool: params.pool, tickLower: params.tickLower, tickUpper: params.tickUpper, tickCurrent: params.tickCurrent, feeGrowthGlobal0X128: params.feeGrowthGlobal0X128, feeGrowthGlobal1X128: params.feeGrowthGlobal1X128 }
 ) );
 amount0 = uint256( FullMath.mulDiv( feeGrowthInside0X128 - feeGrowthInside0LastX128, liquidity, FixedPoint128.Q128 ) );
 amount1 = uint256( FullMath.mulDiv( feeGrowthInside1X128 - feeGrowthInside1LastX128, liquidity, FixedPoint128.Q128 ) );
 amount0 = amount0.add(tokensOwed0);
 amount1 = amount1.add(tokensOwed1);
 if (params.tickCurrent < params.tickLower) {
 amount0 = amount0.add(uint256( -SqrtPriceMath.getAmount0Delta( TickMath.getSqrtRatioAtTick(params.tickLower), TickMath.getSqrtRatioAtTick(params.tickUpper), -int256(liquidity).toInt128() ) ));
 }
 else if (params.tickCurrent < params.tickUpper) {
 amount0 = amount0.add(uint256( -SqrtPriceMath.getAmount0Delta( params.sqrtPriceX96, TickMath.getSqrtRatioAtTick(params.tickUpper), -int256(liquidity).toInt128() ) ));
 ...
 amount1 = amount1.add(uint256( -SqrtPriceMath.getAmount1Delta( TickMath.getSqrtRatioAtTick(params.tickLower), TickMath.getSqrtRatioAtTick(params.tickUpper), -int256(liquidity).toInt128() ) ));
 }
 }
 ...
 function getFeeGrowthInside(FeeGrowthInsideParams memory params) internal view returns (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128) {
 IUniswapV3Pool _pool = IUniswapV3Pool (params.pool);
 uint256 lower_feeGrowthOutside0X128;
 uint256 lower_feeGrowthOutside1X128;
 ( , , lower_feeGrowthOutside0X128, lower_feeGrowthOutside1X128, , , ,) = _pool.ticks(params.tickLower);
 ...
 ( , , upper_feeGrowthOutside0X128, upper_feeGrowthOutside1X128, , , , ) = _pool.ticks(params.tickUpper);
 ...
 library Array2D {
 function max(uint[][] memory self) internal pure returns( uint index1, uint index2, uint value ) {
 for(uint i = 0; i < self.length; i++){
 ...
 if(totalSupply == 0) share = amount;
 else share = FullMath.mulDiv(amount, totalSupply, total_assets);
 ...
 proportionX128: FullMath.mulDiv(remainingAmount, DIVISOR, desirableAmount), pool: pools[poolIndex], token: fToken, uniV3Router: uniV3Router }
 ), sellPath);
 break;
 }
 else {
 positions[poolIndex][positionIndex].subLiquidity(Position.SubParams({
 proportionX128: DIVISOR, pool: pools[poolIndex], token: fToken, uniV3Router: uniV3Router }
 ), sellPath);
 ...
 else if(totalSupply == share) amount = value;
 }
 ...
 function setPath( address distToken, bytes memory buy, bytes memory sell ) external override onlyController{
 ...
 function init( address token0, address token1, uint24 fee, int24 tickLower, int24 tickUpper, uint amount ) external override onlyController{
 ...
 if(positions[i][positionIndex].tickLower == tickLower && positions[i][positionIndex].tickUpper == tickUpper) revert();
 }
 break;
 }
 }
 ...
 isEmpty: true, tickLower: tickLower, tickUpper: tickUpper }
 ));
 if(amount > 0){
 address fToken = token;
 ...
 poolIndex: poolIndex, pool: pool, amount: amount, amount0Max: 0, amount1Max: 0, token: fToken, uniV3Router: uniV3Router, uniV3Factory: uniV3Factory }
 ), sellPath, buyPath);
 }
 }
 function add( uint poolIndex, uint positionIndex, uint amount, bool collect ) external override onlyController {
 ...
 position.addLiquidity(Position.AddParams({
 poolIndex: poolIndex, pool: pool, amount: amount, amount0Max: amount0Max, amount1Max: amount1Max, token: token, uniV3Router: uniV3Router, uniV3Factory: uniV3Factory }
 ), sellPath, buyPath);
 }
 function sub( uint poolIndex, uint positionIndex, uint proportionX128 ) external override onlyController{
 require(poolIndex < pools.length, "IPL");
 require(positionIndex < positions[poolIndex].length, "IPS");
 positions[poolIndex][positionIndex].subLiquidity(Position.SubParams({
 proportionX128: proportionX128, pool: pools[poolIndex], token: token, uniV3Router: uniV3Router }
 ), sellPath);
 }
 function move( uint poolIndex, uint subIndex, uint addIndex, uint proportionX128 ) external override onlyController {
 require(poolIndex < pools.length, "IPL");
 require(subIndex < positions[poolIndex].length, "ISI");
 require(addIndex < positions[poolIndex].length, "IAI");
 (uint amount0Max, uint amount1Max) = positions[poolIndex][subIndex] .burnAndCollect(pools[poolIndex], proportionX128);
 positions[poolIndex][addIndex].addLiquidity(Position.AddParams({
 poolIndex: poolIndex, pool: pools[poolIndex], amount: 0, amount0Max: amount0Max, amount1Max: amount1Max, token: token, uniV3Router: uniV3Router, uniV3Factory: uniV3Factory }
 ), sellPath, buyPath);
 }
 ...
 function deploy( address WETH9, address uniswapV3Factory, address uniswapV3Router, address controller, address manager, address token, bytes32 descriptor ) internal returns (address fund) {
 parameters = Parameters({
 WETH9: WETH9, uniswapV3Factory: uniswapV3Factory, uniswapV3Router: uniswapV3Router, controller: controller, manager: manager, token: token, descriptor: descriptor }
 );
 fund = address(new HotPotV2Fund{
 salt: keccak256(abi.encode(manager, token))}
 ());
 delete parameters;
 }
 }
 ...
 constructor( address _controller, address _weth9, address _uniV3Factory, address _uniV3Router ){
