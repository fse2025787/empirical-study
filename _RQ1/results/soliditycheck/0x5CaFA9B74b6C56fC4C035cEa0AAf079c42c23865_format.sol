 ...
 event SendSatoruFiTokenReward( address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount );
 ...
 FINISH_BONUS_AT_BLOCK = _halvingAfterBlock .mul(REWARD_MULTIPLIER.length) .add(_startBlock);
 ...
 lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accGovTokenPerShare: 0 }
 ) );
 }
 ...
 ( GovTokenForDev, GovTokenForFarmer, GovTokenForLP, GovTokenForCom, GovTokenForFounders ) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
 ...
 function pendingReward(uint256 _pid, address _user) external view validatePoolByPid(_pid) returns(uint256) {
 PoolInfo storage pool = poolInfo[_pid];
 ...
 function deposit( uint256 _pid, uint256 _amount, address _ref ) external nonReentrant validatePoolByPid(_pid) {
 require( _amount > 0, "MasterChef::deposit: amount must be greater than 0" );
 PoolInfo storage pool = poolInfo[_pid];
 UserInfo storage user = userInfo[_pid][msg.sender];
 ...
 current.globalAmount = current.globalAmount + _amount - _amount.mul(userDepFee).div(10000);
 updatePool(_pid);
 _harvest(_pid);
 ...
 function withdraw( uint256 _pid, uint256 _amount, address _ref ) external nonReentrant validatePoolByPid(_pid) {
 PoolInfo storage pool = poolInfo[_pid];
 UserInfo storage user = userInfo[_pid][msg.sender];
 UserGlobalInfo storage refer = userGlobalInfo[_ref];
 UserGlobalInfo storage current = userGlobalInfo[msg.sender];
 ...
 else if ( user.blockdelta > blockDeltaStartStage[4] ) {
 ...
 return multiplier .mul(REWARD_PER_BLOCK) .mul(poolInfo[pid1].allocPoint) .div(totalAllocPoint);
 }
 ...
 function reviseWithdraw( uint256 _pid, address _user, uint256 _block ) external onlyAuthorized() validatePoolByPid(_pid) {
 UserInfo storage user = userInfo[_pid][_user];
 user.lastWithdrawBlock = _block;
 emit WithdrawRevised(_user, _block);
 }
 function reviseDeposit( uint256 _pid, address _user, uint256 _block ) external onlyAuthorized() validatePoolByPid(_pid) {
 UserInfo storage user = userInfo[_pid][_user];
 user.firstDepositBlock = _block;
 emit DepositRevised(_user, _block);
 }
 function setStageStarts(uint256[] memory _blockStarts) external onlyAuthorized() {
 ...
 function setUserFeeStage(uint256[] memory _userFees) external onlyAuthorized() {
 ...
 constructor( string memory _name, string memory _symbol, uint256 cap_, uint256 _manualMintLimit, uint256 _lockFromBlock, uint256 _lockToBlock ) public ERC20(_name, _symbol) {
 _cap = cap_;
 ...
 require( signatory != address(0), "SatoruFiToken::delegateBySig: invalid signature" );
 require( nonce == nonces[signatory]++, "SatoruFiToken::delegateBySig: invalid nonce" );
 ...
 require( blockNumber < block.number, "SatoruFiToken::getPriorVotes: not yet determined" );
 uint32 nCheckpoints = numCheckpoints[account];
 if (nCheckpoints == 0) {
 return 0;
 }
 ...
 uint32 blockNumber = safe32( block.number, "SatoruFiToken::_writeCheckpoint: block number exceeds 32 bits" );
