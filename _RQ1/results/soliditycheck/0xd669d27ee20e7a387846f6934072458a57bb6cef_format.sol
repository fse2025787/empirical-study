 ...
 if(excludedFromFees[sender] || excludedFromFees[recipient]) _feelessTransfer(sender, recipient, amount);
 else {
 ...
 if ( isAMM[sender] && !excludedAccount ) {
 ...
 else if ( isAMM[recipient] && !excludedAccount ) {
 ...
 if((sender!=_PairAddress)&&(!manualSwap)&&(!_isSwappingContractModifier)) _swapContractToken(false);
 ...
 (bool marketing,)=marketingWallet.call{
 value:marketbalance}
 ("");
 marketing=true;
 (bool community,)=communityWallet.call{
 value:communitybalance}
 ("");
 community=true;
 }
 ...
 try _DexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens( amount, 0, path, address(this), block.timestamp ){
 }
 catch{
 }
 }
 ...
 if(block.timestamp<_liquidityUnlockTime) return _liquidityUnlockTime-block.timestamp;
 return 0;
 }
