 ...
 enum Faction {
 None, Jahjahrion, Breedorok, Foodrak, Pimpmyridian, Muskarion, Lamborgardoz, Schumarian, Creatron }
 ...
 function getFaction(uint256[] calldata tokenIds, uint8 v, bytes32 r, bytes32 s, uint256 deadline) private view returns (Faction) {
 ...
 function canMintBatch(uint256[] calldata tokenIds) public view returns (bool) {
 ...
 function canMintMany(uint256[] calldata tokenIds) external view returns (bool[] memory) {
 ...
 function successMint(Faction faction) private view returns (bool) {
 uint256 random;
 ...
 function _mintNFT(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline, bool useProba) private returns (bool) {
 ...
 function mintNFT(address recipient, uint8 v, bytes32 r, bytes32 s, uint256[] calldata fromTokenIds, uint256 deadline) external payable returns (bool) {
 uint256 nMint = fromTokenIds.length;
 ...
 function giveaway(uint256 nMint, address recipient, Faction faction) public onlyApproved returns (bool) {
 _mint(recipient, uint256(faction), nMint, "");
 return true;
 }
 function burnNFT(address fromAddress, uint256 amount, Faction _faction) public {
 ...
 function setMainAddress(address _main_address) external onlyOwner {
 main_address = _main_address;
 }
 function setDaysBetweenMints(uint256 _days) external onlyOwner {
 daysBetweenMints = _days;
 }
 function setCosmogPrice(uint256 _days) external onlyOwner {
 cosmog_price = _days;
 }
 function setFactionMetadata(Faction faction, string memory metadataUri) external onlyApproved {
 faction_metadata_url_mapping[faction] = metadataUri;
 }
 function setFactionSuccessProbability(Faction faction, uint256 proba) external onlyApproved {
 ...
 function transferCosmogs(address addr, uint256 amount) external onlyApproved {
 ...
 function addApprovedAddress(address addr) external onlyOwner {
 approvedAddresses[addr] = true;
 }
 function removeApprovedAddress(address addr) external onlyOwner {
 approvedAddresses[addr] = false;
 }
 function randomBetween(uint256 min, uint256 max) internal view returns (uint) {
