 ...
 uint maxClose = mul_ScalarTruncate(Exp({
 mantissa: closeFactorMantissa}
 ), borrowBalance);
 ...
 Exp memory numerator = mul_(Exp({
 mantissa: liquidationIncentiveMantissa}
 ), Exp({
 mantissa: priceBorrowedMantissa}
 ));
 Exp memory denominator = mul_(Exp({
 mantissa: priceCollateralMantissa}
 ), Exp({
 mantissa: exchangeRateMantissa}
 ));
 ...
 Exp memory highLimit = Exp({
 mantissa: collateralFactorMaxMantissa}
 );
 ...
 isListed: true, collateralFactorMantissa: 0, version: version}
 );
