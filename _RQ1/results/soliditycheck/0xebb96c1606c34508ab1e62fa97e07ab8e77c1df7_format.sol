 ...
 function setMins(uint256 _minForNewTopic, uint256 _minForExistingTopic) onlyOwner public {
 require(_minForNewTopic > 0, "The _minForNewTopic should be > 0.");
 require(_minForExistingTopic > 0, "The _minForExistingTopic should be > 0.");
 ...
 function getSupportersForTopic(string topic) public view returns (address[], uint256[]) {
 ...
 require(bytes(topic).length > 0, "Please specify a topic.");
 require(bytes(topic).length <= 500, "The topic is too long (max 500 characters).");
 SupporterList storage supporterList = topicToSupporterList[topic];
 if(supporterList.length == 0) {
 require(msg.value >= minForNewTopic, "Please send at least 'minForNewTopic' to request a new topic.");
 allTopics.idToTopic[allTopics.length++] = topic;
 emit NewTopic(topic, msg.sender, msg.value);
 }
 else {
 require(msg.value >= minForExistingTopic, "Please send at least 'minForExistingTopic' to add support to an existing topic.");
 emit ContributeToTopic(topic, msg.sender, msg.value);
 }
 supporterList.idToSupporter[supporterList.length++] = Supporter(msg.sender, msg.value);
 }
 ...
 for(uint i = 0; i < supporterList.length; i++) {
 totalValue += supporterList.idToSupporter[i].value;
 supporterList.idToSupporter[i].addr.transfer( supporterList.idToSupporter[i].value);
 }
 _removeTopic(topic);
