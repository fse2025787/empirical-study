 ...
 function calcualteReleased(uint amount, uint releaseStart, uint releasePeriod) private view returns(uint) {
 ...
 uint releasedInstitutional = calcualteReleased( user.institutionalSale, INSTITUTIONAL, INSTITUTIONAL_PERIOD);
 ...
 Lock memory user = db.locked[who];
 return user.strategicSale.add(user.privateSale).add(user.institutionalSale) .add(user.secondary).sub(user.claimed).sub(availableToWithdraw(who));
 }
 function assignStrategic(address[] calldata tos, uint[] calldata amounts) external onlyOwner {
 uint len = tos.length;
 ...
 function assignPrivate(address[] calldata tos, uint[] calldata amounts) external onlyOwner {
 uint len = tos.length;
 require(len == amounts.length, 'Invalid input');
 uint total = 0;
 for (uint i = 0; i < len; i++) {
 ...
 function assignInstitutional(address[] calldata tos, uint[] calldata amounts) external onlyOwner {
 uint len = tos.length;
 require(len == amounts.length, 'Invalid input');
 uint total = 0;
 for (uint i = 0; i < len; i++) {
 address to = tos[i];
 uint amount = amounts[i];
 db.locked[to].institutionalSale = db.locked[to].institutionalSale.add(amount).toUInt128();
 total = total.add(amount);
 ...
 uint timeLeft = till(readyAt).mul(requested).add( amount.mul(WITHDRAW_TIMEOUT)).div(newRequested);
 ...
 uint unclaimed = userData.strategicSale.add(userData.privateSale) .add(userData.institutionalSale).sub(userData.claimed);
 ...
 if (token == BULK) {
 require(BULK.balanceOf(address(this)).sub(db.totalBalance) >= amount, 'Not enough to recover');
 }
