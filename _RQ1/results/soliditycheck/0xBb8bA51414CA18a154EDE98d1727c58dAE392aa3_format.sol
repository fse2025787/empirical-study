 ...
 event createAsks( uint256 indexed id, uint256 indexed token_id, uint256 num, uint256 price, address seller, uint256 end_time );
 event upAsks( uint256 indexed id, uint256 price );
 event upAsksTime( uint256 indexed id, uint256 end_time );
 event orderAsks( uint256 indexed id, uint256 indexed token_id, uint256 price, uint256 num, address buyer );
 event canceleAsks( uint256 indexed id, address indexed seller );
 constructor() ERC1155("https: _brokerage = 2;
 _royalty = 8;
 max_num = 100;
 _ask_total = 0;
 }
 function mint( uint256 amount, string memory _name, string memory _author ) external {
 require( amount <= max_num, " num must < max_num" );
 _token2create[_total] = Musics({
 creater:msg.sender, name:_name, author:_author }
 );
 ...
 function createAsk( uint256 token_id, uint256 price, uint256 num, uint256 end_time ) external {
 require( num > 0, " num must > 0" );
 require( balanceOf(msg.sender,token_id) >= (num+user_sell[string(abi.encodePacked(msg.sender, token_id.toString()))]), "token_id num is engouh" );
 ...
 id: _ask_total, token_id: token_id, num: num, price: price, end_time: end_time, seller: msg.sender, is_set : true }
 );
 ...
 emit createAsks({
 id : _ask_total, token_id: token_id, num: num, price: price, seller: msg.sender, end_time:end_time }
 );
 _ask_total ++;
 }
 function cancelAsk(uint256 sell_id) external {
 ...
 emit canceleAsks({
 id: sell_id, seller: msg.sender}
 );
 }
 function upAsk(uint256 sell_id,uint256 price) external {
 require(_id2ask[sell_id].seller == msg.sender, "is not you");
 require(_id2ask[sell_id].is_set == true, "is used");
 require( balanceOf(msg.sender,_id2ask[sell_id].token_id) >= _id2ask[sell_id].num, "token_id num is engouh" );
 require(price > 1000, "price too low");
 _id2ask[sell_id].price = price;
 emit upAsks({
 id: sell_id,price :price}
 );
 }
 function upAskTime(uint256 sell_id,uint256 end_time) external {
 require(_id2ask[sell_id].seller == msg.sender, "is not you");
 require(_id2ask[sell_id].is_set == true, "is used");
 require( balanceOf(msg.sender,_id2ask[sell_id].token_id) >= _id2ask[sell_id].num, "token_id num is not engouh" );
 if(end_time > 0){
 require(end_time > block.timestamp, "end_time is must > now");
 }
 else{
 ...
 emit upAsksTime({
 id: sell_id,end_time :end_time}
 );
 }
 function orderAsk(uint256 sell_id,uint256 buy_num) external payable {
 require(_id2ask[sell_id].is_set, "is selled");
 require( buy_num <= _id2ask[sell_id].num, "token_id num is not engouh" );
 require( balanceOf(_id2ask[sell_id].seller,_id2ask[sell_id].token_id) >= buy_num, "token_id num is not engouh" );
 ...
 emit orderAsks({
 id: sell_id,token_id :_id2ask[sell_id].token_id,price:sell_price,num:buy_num,buyer:msg.sender}
 );
 }
 ...
 require( num > 0, " num must > 0" );
 require( balanceOf(msg.sender,my_token_id) >= num, "token_id num is not engouh" );
