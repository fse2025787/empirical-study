 ...
 require( comptroller.liquidateBorrowAllowed( address(this), address(cTokenCollateral), liquidator, borrower, repayAmount ) == 0, "rejected" );
 require(accrualBlockNumber == getBlockNumber(), "market is stale");
 ...
 function seizeInternal( address seizerToken, address liquidator, address borrower, uint256 seizeTokens ) internal returns (uint256) {
 require( comptroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens) == 0, "rejected" );
 if (seizeTokens == 0) {
 return uint256(Error.NO_ERROR);
 }
 require(borrower != liquidator, "invalid account pair");
 ...
 constructor( address admin_, address registry_, address reference_ ) public {
 admin = admin_;
 ...
 price = mul_(price, Exp({
 mantissa: ethUsdPrice}
 ));
 }
 ...
 function _setAggregators( address[] calldata tokenAddresses, address[] calldata bases, address[] calldata quotes ) external {
 ...
 base: bases[i], quote: quotes[i], isUsed: isUsed}
 );
 ...
 references[tokenAddresses[i]] = ReferenceInfo({
 symbol: symbols[i], isUsed: isUsed}
 );
