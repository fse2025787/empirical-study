 ...
 tokContract.transfer( escrow, toks1, data );
 tokContract.transfer( escrow, toks2, data2 );
 }
 }
 ...
 assembly {
 mstore(0x60,self_slot) for {
 let i := 0 }
 lt(i, sload(self_slot)) {
 i := add(i, 1) }
 {
 ...
 assembly {
 mstore(0x60,self_slot) maxValue := sload(sha3(0x60,0x20)) for {
 let i := 0 }
 lt(i, sload(self_slot)) {
 i := add(i, 1) }
 {
 switch gt(sload(add(sha3(0x60,0x20),i)), maxValue) case 1 {
 ...
 assembly {
 mstore(0x60,self_slot) minValue := sload(sha3(0x60,0x20)) for {
 let i := 0 }
 lt(i, sload(self_slot)) {
 i := add(i, 1) }
 {
 switch gt(sload(add(sha3(0x60,0x20),i)), minValue) case 0 {
 ...
 assembly{
 mstore(0x60,self_slot) switch isSorted case 1 {
 let high := sub(sload(self_slot),1) let mid := 0 let low := 0 for {
 }
 iszero(gt(low, high)) {
 }
 {
 mid := div(add(low,high),2) switch lt(sload(add(sha3(0x60,0x20),mid)),value) case 1 {
 low := add(mid,1) }
 case 0 {
 switch gt(sload(add(sha3(0x60,0x20),mid)),value) case 1 {
 high := sub(mid,1) }
 case 0 {
 found := 1 index := mid low := add(high,1) }
 }
 }
 }
 case 0 {
 for { let low := 0 } lt(low, sload(self_slot)) {
 low := add(low, 1) }
 {
 switch eq(sload(add(sha3(0x60,0x20),low)), value) case 1 {
 found := 1 index := low low := sload(self_slot) }
 }
 }
 }
 }
 ...
 rChild = lChild + 1;
 swap = root;
 if(self[swap] < self[lChild]) swap = lChild;
 if((rChild <= end) && (self[swap]<self[rChild])) swap = rChild;
 if(swap == root) lChild = end+1;
 else {
 temp = self[swap];
 self[swap] = self[root];
 self[root] = temp;
 ...
 assembly{
 size := mload(add(_input,_offst)) let chunk_count := add(div(size,32),1) if gt(mod(size,32),0) {
 ...
 uint size = 32;
 assembly {
 let loop_index:= 0 let chunk_count size := mload(add(_input,_offst)) chunk_count := add(div(size,32),1) if gt(mod(size,32),0) {
 chunk_count := add(chunk_count,1) }
 loop: mstore(add(_output,mul(loop_index,32)),mload(add(_input,_offst))) _offst := sub(_offst,32) loop_index := add(loop_index,1) jumpi(loop , lt(loop_index , chunk_count)) }
 }
 function bytesToBytes32(uint _offst, bytes memory _input, bytes32 _output) internal pure {
 assembly {
 mstore(_output , add(_input, _offst)) mstore(add(_output,32) , add(add(_input, _offst),32)) }
 }
 ...
 assembly {
 mstore(add(_output, _offst), _input) mstore(add(add(_output, _offst),32), add(_input,32)) }
 }
 ...
 assembly {
 let index := 0 stack_size := add(stack_size,1) loop: mstore(add(_output, _offst), mload(add(_input,mul(index,32)))) _offst := sub(_offst , 32) index := add(index ,1) jumpi(loop , lt(index,stack_size)) }
 }
 ...
 require( msg.sender == transactions[transaction_id].to_address && !transactions[transaction_id].executed && transactions[transaction_id].valid );
 ...
 require(msg.sender == owner);
 require(!transactions[transaction_id].executed && transactions[transaction_id].valid && transactions[transaction_id].time > newtime);
