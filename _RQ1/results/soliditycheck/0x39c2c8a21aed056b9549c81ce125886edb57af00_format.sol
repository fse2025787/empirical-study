 ...
 function getSqrtPriceX96( bytes memory path, address uniV3Factory ) internal view returns (uint sqrtPriceX96){
 ...
 sqrtPriceX96 = tokenIn > tokenOut ? FullMath.mulDiv(sqrtPriceX96, FixedPoint96.Q96, _nextSqrtPriceX96) : FullMath.mulDiv(sqrtPriceX96, _nextSqrtPriceX96, FixedPoint96.Q96);
 if (path.hasMultiplePools()) path = path.skipToken();
 else break;
 }
 }
 function getSqrtPriceX96Last( bytes memory path, address uniV3Factory ) internal view returns (uint sqrtPriceX96Last){
 require(path.length > 0, "IPL");
 ...
 sqrtPriceX96Last = tokenIn > tokenOut ? FullMath.mulDiv(sqrtPriceX96Last, FixedPoint96.Q96, _nextSqrtPriceX96) : FullMath.mulDiv(sqrtPriceX96Last, _nextSqrtPriceX96, FixedPoint96.Q96);
 if (path.hasMultiplePools()) path = path.skipToken();
 else break;
 }
 }
 function verifySlippage( bytes memory path, address uniV3Factory, uint32 maxSqrtSlippage ) internal view returns(uint) {
 ...
 if(deltaX > amount0){
 amount1 = FullMath.mulDiv( deltaX.sub(amount0), FullMath.mulDiv(sqrtPriceX96, sqrtPriceX96, FixedPoint64.Q64), FixedPoint128.Q128 );
 }
 }
 function getAmountOutMin( uint curSqrtPirceX96, uint maxPriceImpact, uint amountIn ) internal pure returns(uint amountOutMin){
 amountOutMin = FullMath.mulDiv( FullMath.mulDiv(amountIn, FullMath.mulDiv(curSqrtPirceX96, curSqrtPirceX96, FixedPoint64.Q64), FixedPoint128.Q128), 1e4 - maxPriceImpact, 1e4);
 }
 struct SwapParams{
 uint amount;
 uint amount0;
 ...
 equalAmount0 = params.amount0.add((FullMath.mulDiv( params.amount, FullMath.mulDiv(buy0SqrtPriceX96, buy0SqrtPriceX96, FixedPoint64.Q64), FixedPoint128.Q128 )));
 }
 }
 else equalAmount0 = params.amount0;
 if(params.amount1 > 0){
 equalAmount0 = equalAmount0.add((FullMath.mulDiv( params.amount1, FixedPoint128.Q128, FullMath.mulDiv(params.sqrtPriceX96, params.sqrtPriceX96, FixedPoint64.Q64) )));
 }
 ...
 amountIn = FullMath.mulDiv( amount0Max - params.amount0, FixedPoint128.Q128, FullMath.mulDiv(buy0SqrtPriceX96, buy0SqrtPriceX96, FixedPoint64.Q64) );
 if(amountIn > params.amount) amountIn = params.amount;
 if(amountIn > 0) {
 ...
 path: buy0Path, recipient: address(this), deadline: block.timestamp, amountIn: amountIn, amountOutMinimum: amountOutMin }
 )));
 }
 ...
 path: buy1Path, recipient: address(this), deadline: block.timestamp, amountIn: amountIn, amountOutMinimum: amountOutMin }
 )));
 }
 ...
 amount0Max = params.amount0.add(ISwapRouter(params.uniV3Router).exactInput(ISwapRouter.ExactInputParams({
 path: buy0Path, recipient: address(this), deadline: block.timestamp, amountIn: params.amount, amountOutMinimum: amountOutMin }
 )));
 }
 }
 else amount0Max = params.amount0;
 ...
 if(lastSqrtPriceX96 > buy0SqrtPriceX96) require(buy0SqrtPriceX96 > params.maxSqrtSlippage * lastSqrtPriceX96 / 1e4, "VS");
 uint amountOutMin = getAmountOutMin(buy0SqrtPriceX96, params.maxPriceImpact, amountIn);
 ...
 amount1Max = params.amount1.add(ISwapRouter(params.uniV3Router).exactInput(ISwapRouter.ExactInputParams({
 path: buy1Path, recipient: address(this), deadline: block.timestamp, amountIn: params.amount, amountOutMinimum: amountOutMin }
 )));
 }
 }
 else amount1Max = params.amount1;
 ...
 if(lastSqrtPriceX96 > buy1SqrtPriceX96) require(buy1SqrtPriceX96 > params.maxSqrtSlippage * lastSqrtPriceX96 / 1e4, "VS");
 uint amountOutMin = getAmountOutMin(buy1SqrtPriceX96, params.maxPriceImpact, amountIn);
 ...
 function addLiquidity( Info storage self, AddParams memory params, mapping(address => bytes) storage sellPath, mapping(address => bytes) storage buyPath ) public returns(uint128 liquidity) {
 ...
 if(sqrtPriceX96Last > sqrtPriceX96) require(sqrtPriceX96 > params.maxSqrtSlippage * sqrtPriceX96Last / 1e4, "VS");
 ...
 path: path, recipient: address(this), deadline: block.timestamp, amountIn: amount0, amountOutMinimum: amountOutMin }
 ));
 }
 }
 ...
 path: path, recipient: address(this), deadline: block.timestamp, amountIn: amount1, amountOutMinimum: amountOutMin }
 )));
 }
 }
 }
 ...
 uint _amount;
 if(params.token0 != token){
 _amount = FullMath.mulDiv( _amount0, FullMath.mulDiv(params.sqrt0, params.sqrt0, FixedPoint64.Q64), FixedPoint128.Q128);
 }
 else _amount = _amount0;
 if(params.token1 != token){
 _amount = _amount.add(FullMath.mulDiv( _amount1, FullMath.mulDiv(params.sqrt1, params.sqrt1, FixedPoint64.Q64), FixedPoint128.Q128));
 }
 ...
 amount = FullMath.mulDiv( amount0, FullMath.mulDiv(sqrt0, sqrt0, FixedPoint64.Q64), FixedPoint128.Q128);
 }
 else amount = amount0;
 }
 if(amount1 > 0){
 ...
 amount = amount.add(FullMath.mulDiv( amount1, FullMath.mulDiv(sqrt1, sqrt1, FixedPoint64.Q64), FixedPoint128.Q128));
 }
