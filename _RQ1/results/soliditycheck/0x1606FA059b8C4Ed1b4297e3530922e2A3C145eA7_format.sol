 ...
 constructor( address _registry, address _eurPriceFeed, address _smt, address _xTokenWrapper, address _usdcAddress ) {
 ...
 function calculateAmount(address _asset, uint256 _assetAmountIn) public view onlyValidAsset(_asset) returns (uint256) {
 ...
 uint256 amountUsdToGetEur18 = uint256(amountUsdToGetEur).mul( 10**(eurPriceFeedDecimals.sub(eurUsdDecimals)) );
 ...
 currentPrice = getAvgAmountFromPools( xTokenWrapper.tokenToXToken(smtTokenAddress), xTokenWrapper.tokenToXToken(USDC_ADDRESS), ONE_BASE18 );
 emit PriceComputed(msg.sender, currentPrice);
 }
 function getAvgAmountFromPools( address _assetIn, address _assetOut, uint256 _assetAmountIn ) internal view returns (uint256) {
 ...
 function calcOutGivenIn( address poolAddress, address _assetIn, address _assetOut, uint256 _assetAmountIn ) internal view returns (uint256) {
 ...
 uint256 amount = pool.calcOutGivenIn( tokenBalanceIn, tokenWeightIn, tokenBalanceOut, tokenWeightOut, _assetAmountIn, 0 );
 return amount;
 }
 }
 }
 ...
 function getBestPoolsWithLimit( address fromToken, address destToken, uint256 limit ) external view returns (address[] memory);
 }
 pragma solidity ^0.7.0;
