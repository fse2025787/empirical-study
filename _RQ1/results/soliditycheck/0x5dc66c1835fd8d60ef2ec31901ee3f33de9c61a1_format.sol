 ...
 function timenow() public view returns(uint32) {
 ...
 enum Status {
 NOTFOUND, PLAYING, PROCESSING, PAYING, CANCELING }
 struct Game {
 string nameLottery;
 ...
 function getLotteryByID(uint32 _id) public view returns ( string nameLottery, uint32 countCombinations, uint32 dateStopBuy, uint32 gameID, uint32 teamAID, uint32 teamBID, uint32 minStake, uint32 winCombination, uint32 betsCount, uint256 betsSumIn, uint256 feeValue, Status status, bool isFreezing ){
 ...
 function getTokenByID(uint256 _id) public view returns ( uint256 price, uint256 payment, uint32 combination, uint32 dateBuy, uint32 idLottery, address ownerToken, bool payout, uint256 sameComboAmount, uint256 tokenID ){
 ...
 function getStatLotteries() public view returns ( uint32 countAll, uint32 countPlaying, uint32 countProcessing, string listPlaying, string listProcessing ){
 ...
 if (curtime < game[i].dateStopBuy) {
 countPlaying++;
 listPlaying = strConcat( listPlaying, ",", uint2str(i) );
 }
 if (curtime >= game[i].dateStopBuy) {
 countProcessing++;
 listProcessing = strConcat( listProcessing, ",", uint2str(i) );
 }
 }
 }
 ...
 if ( tmpCombination > curGame.countCombinations ) {
 tmpCombination = 0;
 error = "callback_result_limit";
 }
 ...
 tmpQuery = strConcat( "json(https: uint32 delay;
