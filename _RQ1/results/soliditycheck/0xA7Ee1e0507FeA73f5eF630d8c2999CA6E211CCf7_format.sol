 ...
 event AuctionCreated( uint256 indexed tokenId, uint256 auctionStart, uint256 duration, uint256 reservePrice, string paymentType, address Creator );
 event AuctionBid( uint256 indexed tokenId, address nftContractAddress, address sender, uint256 value );
 event AuctionCanceled( uint256 indexed tokenId, address nftContractAddress, address Creator );
 event AuctionEnded( uint256 indexed tokenId, address nftContractAddress, address Creator, address winner, uint256 amount, address nftCreator );
 event CreatorFeePercentTransfer( uint256 indexed tokenId, address Creator, uint256 amount );
 event Paused(address account);
 event Unpaused(address account);
 event Purchase( address indexed previousOwner, address indexed newOwner, uint256 price, uint256 nftID );
 ...
 require( adminRecoveryAddress == msg.sender && adminRecoveryEnabled(), "Caller does not have admin privileges" );
 _;
 }
 modifier onlyCreator(uint256 tokenId) {
 require( auctions[tokenId].Creator == msg.sender, "Can only be called by auction Creator" );
 _;
 }
 modifier onlyCreatorOrWinner(uint256 tokenId) {
 require( auctions[tokenId].Creator == msg.sender || auctions[tokenId].bidder == msg.sender, "Can only be called by auction Creator" );
 _;
 }
 ...
 require( auctions[tokenId].firstBidTime == 0 || block.timestamp < auctionEnds(tokenId), "Auction expired" );
 _;
 }
 modifier auctionComplete(uint256 tokenId) {
 require( auctions[tokenId].firstBidTime > 0 && block.timestamp >= auctionEnds(tokenId), "Auction hasn't completed" );
 _;
 }
 constructor( address nftContract_, address adminRecoveryAddress_, address ethPaymentAddress_, address ethTokenAddress_, address developmentAddress_ ) {
 require( IERC165(nftContract_).supportsInterface(ERC721_INTERFACE_ID), "Contract at nftContract_ address does not support NFT interface" );
 ...
 function addCreatorMap( uint256[] memory _newtokenIds, address[] memory _creators, uint256[] memory _prices, address[] memory _owners, uint256[] memory _royalties, bool[] memory _listedMap ) external onlyOwner {
 ...
 function openTrade( uint256 _id, uint256 _price, string memory paymentType ) public {
 ...
 attemptETHTransfer( payable(adminRecoveryAddress), _commissionValue.mul(3).div(5) );
 attemptETHTransfer( payable(developmentAddress), _commissionValue.mul(2).div(5) );
 }
 else {
 ...
 require( IERC20(tokenAddressMap[paymentType]).transferFrom( msg.sender, adminRecoveryAddress, _commissionValue.div(5).mul(3) ) );
 require( IERC20(tokenAddressMap[paymentType]).transferFrom( msg.sender, developmentAddress, _commissionValue.div(5).mul(2) ) );
 }
 IERC721(nftContract).transferFrom(address(this), msg.sender, _id);
 ...
 function createAuction( uint256 tokenId, uint256 duration, uint256 reservePrice, string memory paymentType, address Creator ) external nonReentrant whenNotPaused auctionNonExistant(tokenId) {
 require(Creator != address(0));
 ownerMap[tokenId] = msg.sender;
 openTrade(tokenId, reservePrice, paymentType);
 uint256 auctionStart = block.timestamp;
 auctions[tokenId] = Auction({
 duration: duration, reservePrice: reservePrice, paymentType: paymentType, CreatorFeePercent: 50, Creator: Creator, fundsRecipient: payable(adminRecoveryAddress), amount: 0, firstBidTime: auctionStart, bidder: payable(address(0)) }
 );
 emit AuctionCreated( tokenId, auctionStart, duration, reservePrice, paymentType, Creator );
 }
 function createBid( uint256 tokenId, string memory paymentType, uint256 amount ) external payable nonReentrant whenNotPaused auctionExists(tokenId) auctionNotExpired(tokenId) {
 ...
 require( amount >= auctions[tokenId].reservePrice, "Must bid reservePrice or more" );
 }
 else {
 require( amount >= auctions[tokenId].amount.add( auctions[tokenId] .amount .mul(MIN_BID_INCREMENT_PERCENT) .div(100) ), "Must bid more than last bid by MIN_BID_INCREMENT_PERCENT amount" );
 if ( keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("ETH"))) ) {
 ...
 auctions[tokenId].duration += block.timestamp.add(TIME_BUFFER).sub( auctionEnds(tokenId) );
 }
 emit AuctionBid(tokenId, nftContract, msg.sender, amount);
 }
 function endAuction(uint256 tokenId) external nonReentrant whenNotPaused auctionComplete(tokenId) onlyCreatorOrWinner(tokenId) {
 ...
 require( IERC20(tokenAddressMap[paymentType]).transfer( adminRecoveryAddress, _commissionValue.mul(3).div(5) ) );
 require( IERC20(developmentAddress).transfer( payoutAddressMap[paymentType], _commissionValue.mul(2).div(5) ) );
 }
 ownerMap[tokenId] = winner;
 }
 listedMap[tokenId] = false;
 emit AuctionEnded( tokenId, nftContract, Creator, winner, amount, creatorMap[tokenId] );
 }
 function cancelAuction(uint256 tokenId) external nonReentrant auctionExists(tokenId) onlyCreator(tokenId) {
 require( uint256(auctions[tokenId].amount) == 0, "Auction already started" );
 address Creator = auctions[tokenId].Creator;
 delete auctions[tokenId];
 IERC721(nftContract).transferFrom(address(this), Creator, tokenId);
 listedMap[tokenId] = false;
 ...
 IERC721(nftContract).transferFrom( address(this), adminRecoveryAddress, tokenId );
 }
 function recoverETH(uint256 amount) external onlyAdminRecovery returns (bool success) {
 ...
 function attemptETHTransfer(address payable to, uint256 value) private returns (bool) {
 (bool success, ) = to.call{
 value: value, gas: 30000}
 ("");
 return success;
 }
 ...
 uint256 token_bal = IERC20(tokenAddressMap[_tokenName]).balanceOf( address(this) );
 ...
 function balanceOf(address _owner) public view override(ERC721) returns (uint256) {
 ...
 function ownerOf(uint256 _id) public view override(ERC721) returns (address) {
