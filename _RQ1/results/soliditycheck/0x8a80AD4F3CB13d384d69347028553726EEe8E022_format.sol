 ...
 function initialize( address _controller, address _ibt, uint256 _periodDuration, string memory _platformName, address _deployerAddress, address _admin ) public virtual initializer {
 ...
 bytes memory payload = abi.encodeWithSignature( "initialize(string,string,uint8,address)", ibtSymbol, ibtSymbol, ibt.decimals(), address(this) );
 apwibt = IAPWineIBT( IProxyFactory(registry.getProxyFactoryAddress()).deployMinimal(registry.getAPWineIBTLogicAddress(), payload) );
 ...
 startIndex: getNextPeriodIndex(), scaledBalance: _initialScaledBalance}
 );
 }
 ...
 if (treasuryFee > 0) {
 ibt.transferFrom( address(futureVault), IRegistry(controller.getRegistryAddress()).getTreasuryAddress(), treasuryFee );
 }
 ...
 bytes memory payload = abi.encodeWithSignature( "initialize(string,string,uint8,uint256,address)", tokenDenomination, tokenDenomination, ibt.decimals(), _internalPeriodID, address(this) );
 IFutureYieldToken newToken = IFutureYieldToken( IProxyFactory(registry.getProxyFactoryAddress()).deployMinimal(registry.getFYTLogicAddress(), payload) );
 fyts.push(newToken);
 ...
 if ( _periodID >= getNextPeriodIndex() || registrations[_user].startIndex > _periodID || lastPeriodClaimed[_user] >= _periodID ) {
 return 0;
 }
 else {
 ...
 require( (hasRole(ADMIN_ROLE, msg.sender) || hasRole(CONTROLLER_ROLE, msg.sender)), "Caller is not allowed to pause future" );
 PAUSED = true;
 emit PeriodsPaused();
 }
 function resumePeriods() public {
 require( (hasRole(ADMIN_ROLE, msg.sender) || hasRole(CONTROLLER_ROLE, msg.sender)), "Caller is not allowed to resume future" );
 PAUSED = false;
 emit PeriodsResumed();
 }
 ...
 function initialize( address _controller, address _ibt, uint256 _periodDuration, string memory _platformName, address _deployerAddress, address _admin ) public virtual override initializer {
 ...
 apwibt.mint( address(this), registrationsTotals[nextPeriodID].mul(IBTRates[nextPeriodID]).div(10**ibt.decimals()) );
 ...
 uint256 initialTotalUserDeposit = (apwibt.balanceOf(_user)).mul(10**ibt.decimals()).div(IBTRates[getNextPeriodIndex() - 1]);
 ...
 function getScaledInput( uint256 _actualValue, uint256 _initialSum, uint256 _actualSum ) external pure returns (uint256);
 function getActualOutput( uint256 _scaledOutput, uint256 _initialSum, uint256 _actualSum ) external pure returns (uint256);
 }
 pragma solidity 0.7.6;
 ...
 function getFutureIBTSymbol( string memory _ibtSymbol, string memory _platform, uint256 _periodDuration ) external pure returns (string memory);
 ...
 function addFuturePlatform( address _futureFactory, string memory _futurePlatformName, address _future, address _futureWallet, address _futureVault ) external;
