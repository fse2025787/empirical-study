 ...
 event Creators( uint256 indexed tokenId, LibPart.Part[] creators );
 ...
 pragma solidity >=0.6.2 <0.8.0;
 contract EcoFiCommunityERC1155 is ERC1155Base, ERC1155Image {
 event CreateEcoFiERC1155(address owner, string name, string symbol);
 function __EcoFiCommunityERC1155_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address _address) external initializer {
 __Ownable_init_unchained();
 ...
 _setBaseURI(baseURI);
 _setDefaultApproval(_address, true);
 ...
 return keccak256(abi.encode( MINT_AND_TRANSFER_TYPEHASH, data.tokenId, data.supply, keccak256(bytes(data.tokenURI)), keccak256(abi.encodePacked(creatorsBytes)), keccak256(abi.encodePacked(royaltiesBytes)) ));
 }
 }
 pragma solidity 0.7.6;
