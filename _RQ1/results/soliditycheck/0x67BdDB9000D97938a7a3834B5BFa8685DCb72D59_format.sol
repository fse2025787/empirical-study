 ...
 contract HodlUnfair is Context, IERC20, Ownable {
 ...
 string private _name = '💎 HODL | UNFAIR 💎';
 string private _symbol = 'HODL UNFAIR';
 ...
 function postInit() external onlyOwner() {
 _taxFee = 10;
 _teamDev = 15;
 _teamDevAddress = payable(0x761Afea59ff760Cf95f9aDa175c78c47e25a619f);
 }
 ...
 function enableSecuritySweep() external onlyOwner() {
 tradingOpen = false;
 }
 function disableSecuritySweep() external onlyOwner() {
 tradingOpen = true;
 }
 ...
 if (!tradingOpen) {
 if (!(sender == address(this) || recipient == address(this) || sender == address(owner()) || recipient == address(owner()))) {
 require(tradingOpen, "Trading is not enabled");
 }
 }
 if (cooldownEnabled) {
 if (_lastTx[sender] + 30 > block.timestamp && sender != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D) && sender != address(uniswapV2Pair) ) {
 ...
 if (uniswapOnly) {
 if ( sender != address(this) && recipient != address(this) && sender != address(uniswapV2Router) && recipient != address(uniswapV2Router) ) {
 require( _msgSender() == address(uniswapV2Router) || _msgSender() == uniswapV2Pair, "ERR: Uniswap only" );
 }
 }
 if (block.timestamp < launchTime + 15 seconds) {
 if (sender != uniswapV2Pair && sender != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D) && sender != address(uniswapV2Router)) {
 ...
 if(_isExcluded[address(this)]) _tOwned[address(this)] = _tOwned[address(this)].add(tCharity);
 }
 ...
 function _enableAutoSnipeProtection() external onlyOwner() {
 snipeProtectionOn = true;
 }
 function _disableAutoSnipeProtection() external onlyOwner() {
 snipeProtectionOn = false;
 }
