 ...
 event tokenReceiverAdded( address indexed sender, address indexed token, address indexed receiver );
 event tokenReceiverRemoved( address indexed sender, address indexed token, address indexed receiver );
 function execTransfer( address _token, address _receiver, uint256 _amount ) external whenNotPaused {
 address _sender = _msgSender();
 require(senderToTokens[_sender].contains(_token), "Not allowed token");
 require( tokenToReceivers[_sender][_token].contains(_receiver), "Not allowed address" );
 IERC20(_token).safeTransferFrom(_sender, _receiver, _amount);
 }
 function addTokenReceiver(address _token, address _receiver) external whenNotPaused {
 address _sender = _msgSender();
 ...
 function removeTokenReceiver(address _token, address _receiver) external whenNotPaused {
 address _sender = _msgSender();
 ...
 function getAllowedTokensBySender(address _sender) public view returns (address[] memory) {
 ...
 function getAllowedReceiversByToken(address _sender, address _token) public view returns (address[] memory) {
 bytes32[] memory store = tokenToReceivers[_sender][_token] ._inner ._values;
 address[] memory result;
 assembly {
 result := store }
 return result;
