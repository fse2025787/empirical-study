 ...
 if( from != uniswapV2Pair && canSwap && !swapping ) {
 swapping = true;
 swapBack(totalTokensForFee);
 swapping = false;
 }
 else if( from == uniswapV2Pair && to != uniswapV2Pair && block.number < launchBlock + 2 && !isExcludedFromFees[to] ) {
 ...
 value: ethAmount}
 ( address(this), tokenAmount, 0, 0, deployer, block.timestamp );
 }
