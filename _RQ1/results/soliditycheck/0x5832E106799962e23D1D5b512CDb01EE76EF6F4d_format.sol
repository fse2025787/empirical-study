 ...
 event Withdrawn(address indexed _user, uint256 _amountTransferred, uint256 _amountUnstaked, uint256 _shareDeducted, uint256 _rewardsDeducted, uint256 _feeRewardsDeducted);
 ...
 uint256 rewardDistributed = (rewardByWeek[_weekCount][0]) .add(rewardByWeek[_weekCount][1]) .add(rewardByWeek[_weekCount][2]);
 ...
 (rewardByWeek[_weekCount][0], rewardByWeek[_weekCount][1], rewardByWeek[_weekCount][2]) = calculateRewardsByWeekCount(_weekCount);
 }
 ...
 uint256 _weekCount = energy.weekCount();
 userInfo[msg.sender].rewards = _user.rewards .add(getUserRewardsByWeekNo(msg.sender, _weekCount));
 ...
 if(getUserStartedWeek(_who) <= feeRewardData[i].weekGiven && getUserLastDepositTime(_who) < feeRewardData[i].timeStamp && !feeRewardData[i].isClaimed[_who]) {
 ...
 (thisWeekReward[0], thisWeekReward[1], thisWeekReward[2]) = calculateRewardsByWeekCount(_weekCount);
 uint256 userShareAtThatWeek = getUserPercentageShareByWeekNo(_who, _weekCount);
 return getUserRewardsByWeekNo(_who, _weekCount) .add(_calculateRewardByUserShare(thisWeekReward, userShareAtThatWeek)) .add(getUserRewards(_who));
 }
 ...
 return (((rewardAtThatWeek[0] .add(rewardAtThatWeek[1]) .add(rewardAtThatWeek[2])) .mul(userShareAtThatWeek)) .div(percentageDivider.mul(percentageDivider)));
 }
 ...
 if(getUserStatus(_who)) {
 return (_userShareByWeekNo(_who, _weekCount) > 0 || _weekCount == 0) ? _userShareByWeekNo(_who, _weekCount) : getUserShareByWeekNo(_who, _weekCount.sub(1));
 }
 ...
 return (feeRewardData[_index].value, feeRewardData[_index].timeStamp, feeRewardData[_index].totalStakersAtThatTime, feeRewardData[_index].weekGiven);
 }
