 ...
 pragma solidity ^0.8.7;
 contract StakefishTransactionFeePool is IStakefishTransactionFeePool, StakefishTransactionStorage, Initializable, UUPSUpgradeable, ReentrancyGuard {
 ...
 uint256 balanceDiffSinceLastUpdate = address(this).balance - cache.totalUncollectedCommission - cache.totalUncollectedUserBalance - cache.totalUnsentUserRewards;
 ...
 function joinPool( bytes calldata validatorPubKey, address depositor, uint256 joinTime ) external override nonReentrant operatorOnly {
 _joinPool(validatorPubKey, depositor, joinTime);
 emit ValidatorJoined(validatorPubKey, depositor, joinTime);
 }
 function _joinPool( bytes calldata validatorPubKey, address depositor, uint256 joinTime ) internal {
 require( validatorsInPool[validatorPubKey] == address(0), "Validator already in pool" );
 ...
 function partPool( bytes calldata validatorPubKey, uint256 leaveTime ) external override nonReentrant operatorOnly {
 address depositor = _partPool(validatorPubKey, leaveTime);
 emit ValidatorParted(validatorPubKey, depositor, leaveTime);
 }
 function _partPool( bytes calldata validatorPubKey, uint256 leaveTime ) internal returns (address depositorAddress) {
 ...
 function bulkJoinPool( bytes calldata validatorPubkeyArray, address[] calldata depositorAddresses, uint256 ts ) external override nonReentrant operatorOnly {
 ...
 function bulkPartPool( bytes calldata validatorPubkeyArray, uint256 ts ) external override nonReentrant operatorOnly {
 ...
 uint256 totalValidatorUptime = cache.totalValidatorUptime + (block.timestamp - cache.lastCacheUpdateTime) * validatorCount;
 uint256 payoutAmount = 0;
 if (totalValidatorUptime > 0) {
 uint256 payoutUptime = block.timestamp * users[depositorAddress].validatorCount - users[depositorAddress].totalStartTimestamps;
 payoutAmount = uncollectedUserBalance * payoutUptime / totalValidatorUptime;
 }
 return ( payoutAmount + users[depositorAddress].pendingReward, users[depositorAddress].collectedReward );
 }
 else {
 return (users[depositorAddress].pendingReward, users[depositorAddress].collectedReward);
 }
 }
 function _collectReward( address depositorAddress, address payable beneficiary, uint256 amountRequested ) internal {
 ...
 uint256 payoutUptime = block.timestamp * userValidatorCount - users[depositorAddress].totalStartTimestamps;
 uint256 payoutAmount = computePayout(payoutUptime);
 cache.totalValidatorUptime -= payoutUptime;
 cache.totalUncollectedUserBalance -= payoutAmount;
 cache.totalUnsentUserRewards += payoutAmount;
 ...
 function collectPoolCommission(address payable beneficiary, uint256 amountRequested) external override nonReentrant adminOnly {
 updateComputationCache();
 ...
 function emergencyWithdraw ( address[] calldata depositorAddresses, address[] calldata beneficiaries, uint256 maxAmount ) external override nonReentrant adminOnly {
 ...
 require( msg.sender == operatorAddress, "Only stakefish operator allowed" );
 _;
 }
 modifier adminOnly() {
 require( msg.sender == adminAddress, "Only stakefish admin allowed" );
 _;
 }
 modifier devOnly() {
 require( msg.sender == developerAddress, "Only stakefish dev allowed" );
 _;
 }
 ...
 pragma solidity ^0.8.7;
 contract StakefishTransactionFeePoolMigrator is StakefishTransactionFeePool, IMigrator {
 using Address for address payable;
