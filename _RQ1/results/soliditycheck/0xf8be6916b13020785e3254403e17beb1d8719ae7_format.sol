 ...
 function hasPool(address[] calldata tokens, uint256[] calldata denorms) external view returns (bool exist, bytes32 sig) {
 ...
 function updateFarm(address[] calldata pools, bool isFarm) external onlyCore {
 require(pools.length > 0 && pools.length <= 30, "ERR_BATCH_COUNT");
 for (uint256 i = 0; i < pools.length; i++) {
 require(Address.isContract(pools[i]), "ERR_NOT_CONTRACT");
 IXPool pool = IXPool(pools[i]);
