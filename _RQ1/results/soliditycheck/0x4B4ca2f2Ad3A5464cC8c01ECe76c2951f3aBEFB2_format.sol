 ...
 require(log.topicZeroSig == PACKET_SIGNATURE, "ProofLib: packet not recognized");
 ...
 if (functionType == 1) {
 ( , uint256 srcPoolId, uint256 dstPoolId, uint256 dstGasForCall, IStargate.CreditObj memory c, IStargate.SwapObj memory s, bytes memory toAddressBytes, bytes memory contractCallPayload ) = abi.decode(_payload, (uint8, uint256, uint256, uint256, IStargate.CreditObj, IStargate.SwapObj, bytes, bytes));
 ...
 uint64 nonce;
 assembly {
 dstChainId := mload(add(data, 32)) size := mload(add(data, 96)) nonce := mload(add(data, 104)) dstAddress := mload(add(data, sub(add(128, sizeOfSrcAddress), 4))) }
 ...
 assembly {
 let ptr := mload(0x40) mstore(buf, ptr) mstore(ptr, 0) mstore(0x40, add(32, add(ptr, capacity))) }
 return buf;
 }
 ...
 uint src;
 assembly {
 let bufptr := mload(buf) let buflen := mload(bufptr) dest := add(add(bufptr, 32), off) if gt(add(len, off), buflen) {
 ...
 uint len = item.len;
 assembly {
 memPtr := add(memPtr, offset) byte0 := byte(0, mload(memPtr)) }
 if (len >0 && byte0 < LIST_SHORT_START) {
 assembly {
 memPtr := add(memPtr, 0x01) }
 len -= 1;
 ...
 assembly {
 result := mload(memPtr) if lt(len, 32) {
 ...
 assembly {
 let byteLen := sub(byte0, 0xb7) memPtr := add(memPtr, 1) let dataLen := div(mload(memPtr), exp(256, sub(32, byteLen))) itemLen := add(dataLen, add(byteLen, 1)) }
 }
 ...
 assembly {
 let byteLen := sub(byte0, 0xf7) memPtr := add(memPtr, 1) let dataLen := div(mload(memPtr), exp(256, sub(32, byteLen))) itemLen := add(dataLen, add(byteLen, 1)) }
 }
 return itemLen;
 }
