 ...
 function buyForStrategy( uint256 amount, address rewardToken, address recipient ) external;
 }
 ...
 require( msg.sender == owner() || msg.sender == operator, "!owner" );
 _;
 }
 constructor( address _vault, address _controller ) public{
 ...
 uint256 ethWithdrawed = withdrawAmountToken.mul( ETHPerToken ).div(1 ether);
 ...
 if(rewards > 0){
 controller.buyForStrategy( rewards, vault.getRewardToken(), address(vault) );
 }
 }
 function withdrawFromYearn(uint256 ethAmount) private returns(uint256){
 uint256 yShares = yweth.balanceOf(address(this));
 uint256 sharesToWithdraw = ethAmount.div( yweth.pricePerShare() ).mul(1 ether);
 ...
 uint256 yShares = yweth.balanceOf(address(this));
 uint256 yETHBalance = yShares.mul( yweth.pricePerShare() ).div(1 ether);
