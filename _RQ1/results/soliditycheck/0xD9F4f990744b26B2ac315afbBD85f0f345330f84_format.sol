 ...
 function setCongressAndMaintainers( address _hordCongress, address _maintainersRegistry ) internal {
 hordCongress = _hordCongress;
 maintainersRegistry = IMaintainersRegistry(_maintainersRegistry);
 }
 function setMaintainersRegistry( address _maintainersRegistry ) public onlyHordCongress {
 maintainersRegistry = IMaintainersRegistry(_maintainersRegistry);
 }
 }
 ...
 event TokensStaked( address user, uint amountStaked, uint inFavorOfTokenId, uint numberOfTicketsReserved, uint unlockingTime );
 event NFTsClaimed( address beneficiary, uint256 amountUnstaked, uint256 amountTicketsClaimed, uint tokenId );
 function initialize( address _hordCongress, address _maintainersRegistry, address _stakingToken, uint256 _minTimeToStake, uint256 _minAmountToStake ) public initializer {
 ...
 function setMinTimeToStake( uint256 _minimalTimeToStake ) onlyHordCongress external {
 minTimeToStake = _minimalTimeToStake;
 }
 function setMinAmountToStake( uint256 _minimalAmountToStake ) onlyHordCongress external {
 ...
 function stakeAndReserveNFTs( uint tokenId, uint numberOfTickets ) public {
 uint256 numberOfTicketsReserved = tokenIdToNumberOfTicketsReserved[tokenId];
 require(numberOfTicketsReserved.add(numberOfTickets)<= hordTicketFactory.getTokenSupply(tokenId), "Not enough tickets to sell.");
 uint amountOfTokensToStake = minAmountToStake.mul(numberOfTickets);
 stakingToken.transferFrom( msg.sender, address(this), amountOfTokensToStake );
 UserStake memory userStake = UserStake({
 amountStaked: amountOfTokensToStake, amountOfTicketsGetting: numberOfTickets, unlockingTime: minTimeToStake.add(block.timestamp), isWithdrawn: false }
 );
 ...
 emit TokensStaked( msg.sender, amountOfTokensToStake, tokenId, numberOfTickets, userStake.unlockingTime );
 }
 ...
 hordTicketFactory.safeTransferFrom( address(this), msg.sender, tokenId, ticketsToWithdraw, "0x0" );
 emit NFTsClaimed( msg.sender, totalStakeToWithdraw, ticketsToWithdraw, tokenId );
 }
 }
 function getAmountOfTokensClaimed(uint tokenId) external view returns (uint256) {
 ...
 function getAmountOfTicketsReserved( uint tokenId ) external view returns (uint256) {
 return tokenIdToNumberOfTicketsReserved[tokenId];
 }
 function getUserStakesForTokenId( address account, uint tokenId ) external view returns ( uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory ) {
 ...
 function getCurrentAmountStakedForTokenId( address account, uint tokenId ) external view returns (uint256) {
 UserStake [] memory userStakes = addressToTokenIdToStakes[account][tokenId];
 uint numberOfStakes = userStakes.length;
 ...
 function getChampionTokenIds( uint championId ) external view returns (uint[] memory) {
 return championIdToMintedTokensIds[championId];
 }
 function getNumberOfStakesForUserAndToken( address user, uint tokenId ) external view returns (uint256) {
