 ...
 event Unlocked( address ethToken, uint256 amount, address recipient, bytes32 receiptId );
 ...
 function lockTokenFor( address userAddr, uint256 amount, address recipient ) public onlyWallet {
 require( recipient != address(0), "EthManager/recipient is a zero address" );
 require(amount > 0, "EthManager/zero token locked");
 uint256 _balanceBefore = link_.balanceOf(userAddr);
 require( link_.transferFrom(userAddr, address(this), amount), "EthManager/lock failed" );
