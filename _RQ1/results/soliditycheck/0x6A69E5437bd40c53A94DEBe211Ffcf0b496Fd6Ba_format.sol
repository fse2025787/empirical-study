 ...
 event CreateSlot( uint256 indexed slot, address indexed issuer, uint64 effectiveTime, uint64 maturity );
 event Mint( address indexed minter, uint256 indexed slot, uint256 value );
 event Refund(uint256 indexed slot, address sender, uint256 refundAmount);
 function mint( address minter_, uint256 slot_, uint256 mintValue_ ) external;
 function claim( uint256 slot_, address to_, uint256 claimValue_ ) external returns (uint256);
 ...
 function doTransferIn( address underlying, address from, uint256 amount ) internal returns (uint256) {
 ...
 uint256 balanceBefore = ERC20Interface(underlying).balanceOf( address(this) );
 (bool success, bytes memory data) = underlying.call( abi.encodeWithSelector( ERC20Interface.transferFrom.selector, from, address(this), amount ) );
 require( success && (data.length == 0 || abi.decode(data, (bool))), "STF" );
 uint256 balanceAfter = ERC20Interface(underlying).balanceOf( address(this) );
 ...
 (bool success, bytes memory data) = underlying.call( abi.encodeWithSelector( ERC20Interface.transfer.selector, to, amount ) );
 ...
 library Constants {
 enum ClaimType {
 LINEAR, ONE_TIME, STAGED }
 enum VoucherType {
 STANDARD_VESTING, FLEXIBLE_DATE_VESTING, BOUNDING, DREAM }
 ...
 function createSlot( address issuer_, uint64 effectiveTime_, uint64 maturity_ ) external onlyVoucher returns (uint256 slot) {
 ...
 function validateSlotParams( address issuer_, uint64 effectiveTime_, uint64 maturity_ ) public pure {
 require(issuer_ != address(0), "issuer cannot be 0 address");
 require(effectiveTime_ > 0 && effectiveTime_ < maturity_, "invalid time");
 }
 function mint( address minter_, uint256 slot_, uint256 mintValue_ ) external override nonReentrant onlyVoucher {
 ...
 require( !slotDetail.isIssuerRefunded && block.timestamp < slotDetail.maturity, "non-mintable slot" );
 ...
 function claim(uint256 slot_, address to_, uint256 claimValue_) external override onlyVoucher nonReentrant returns (uint256 claimCurrencyAmount) {
 SlotDetail storage slotDetail = _slotDetails[slot_];
 require(slotDetail.isValid, "invalid slot");
 require(slotDetail.isIssuerRefunded, "not refunded");
 claimCurrencyAmount = claimValue_ .mul(10 ** ERC20Upgradeable(fundCurrency).decimals()) .div(10 ** valueDecimals);
 ...
 function getSlot( address issuer_, uint64 effectiveTime_, uint64 maturity_ ) public view returns (uint256) {
 return uint256( keccak256( abi.encode( voucher, issuer_, effectiveTime_, maturity_ ) ) );
 }
