 ...
 uint256[] memory amounts = IUniswapV2Router02(ROUTER).swapExactTokensForTokens(inUSDC, 0, pathToETH, address(this), block.timestamp);
 require(inUSDC == amounts[0], "leftover USDC");
 outETH = amounts[1];
 }
 function _poolSwapExactETHForUSDC(uint256 inETH) internal returns (uint256 outUSDC) {
 if (inETH == 0) return 0;
 uint256[] memory amounts = IUniswapV2Router02(ROUTER).swapExactTokensForTokens( inETH, 0, pathToUSDC, address(this), block.timestamp );
 require(inETH == amounts[0], "leftover ETH");
 outUSDC = amounts[1];
 }
 function _poolAddLiquidityAndStake(uint256 amountETH, uint256 deadline) internal returns ( uint256 addedUSDC, uint256 addedETH, uint256 liquidity ) {
 ...
 (addedETH, addedUSDC, liquidity) = IUniswapV2Router02(ROUTER).addLiquidity( WETH, USDC, amountETH, quotedUSDC, amountETH, 0, address(this), deadline );
 require(addedETH == amountETH, "leftover ETH");
 IMasterChef(MASTERCHEF).deposit(POOL_ID, liquidity);
 }
 function _poolUnstakeAndRemoveLiquidity(uint256 liquidity, uint256 deadline) internal returns (uint256 removedETH, uint256 removedUSDC) {
 if (liquidity == 0) return (0, 0);
 IMasterChef(MASTERCHEF).withdraw(POOL_ID, liquidity);
 (removedETH, removedUSDC) = IUniswapV2Router02(ROUTER).removeLiquidity( WETH, USDC, liquidity, 0, 0, address(this), deadline );
 }
 ...
 using SafeMath for uint256;
 using SafeERC20 for IERC20;
 function applyRebalance( uint256 removedUSDC, uint256 removedETH, uint256 entryUSDC, uint256 entryETH ) internal returns (uint256 exitUSDC, uint256 exitETH) {
 ...
 event Pair( address indexed sender, address indexed minter, uint256 pairedUSDC, uint256 pairedETH, uint256 liquidity );
 ...
 function addLiquidity( address beneficiary, uint256 amountETH, uint256 deadline ) external nonReentrant whenNotPaused {
 IERC20(WETH).safeTransferFrom(msg.sender, address(this), amountETH);
 _deposit(beneficiary, amountETH, deadline);
 }
 function removeLiquidityETH( address payable beneficiary, uint256 shares, uint256 deadline ) external nonReentrant returns (uint256 exitETH) {
 exitETH = _withdraw(msg.sender, beneficiary, shares, deadline);
 IWETH(WETH).withdraw(exitETH);
 Address.sendValue(beneficiary, exitETH);
 }
 function removeLiquidity( address beneficiary, uint256 shares, uint256 deadline ) external nonReentrant returns (uint256 exitETH) {
 exitETH = _withdraw(msg.sender, beneficiary, shares, deadline);
 IERC20(WETH).safeTransfer(beneficiary, exitETH);
 }
 function removeAllLiquidityETH(address payable beneficiary, uint256 deadline) external nonReentrant returns (uint256 exitETH) {
 ...
 function compoundProfits(uint256 amountETH, uint256 capitalProviderRewardPercentmil) external nonReentrant onlyGovernance returns ( uint256 pairedUSDC, uint256 pairedETH, uint256 liquidity ) {
 IERC20(WETH).safeTransferFrom(msg.sender, address(this), amountETH);
 ...
 function _deposit( address beneficiary, uint256 amountETH, uint256 deadline ) private {
 ...
 function _pair( address minterAddress, Minter storage minter, uint256 amountETH, uint256 deadline ) private returns (uint256 shares) {
 ...
 function _withdraw( address sender, address beneficiary, uint256 shares, uint256 deadline ) private returns (uint256 exitETH) {
 ...
 function _unpair( address minterAddress, Minter storage minter, uint256 shares, uint256 deadline ) private {
