 ...
 function giveaway( address to, uint256 animalId, uint8 effectiveness ) external;
 }
 contract Ownable {
 ...
 event NewAirdrop( address to, uint256 animalId );
 event NewGiveway( address to, uint256 animalId, uint8 effectiveness );
 ...
 Dna[] private dnas;
 constructor( uint256 _pendingGivewayCardCount, uint256 _airdropEndTimestamp ) public {
 pendingGivewayCardCount = _pendingGivewayCardCount;
 airdropEndTimestamp = _airdropEndTimestamp;
 }
 function getDna(uint dnaId) external view returns ( uint256 animalId, address ownerAddress, uint8 effectiveness, uint256 id ) {
 Dna storage dna = dnas[dnaId];
 return ( dna.animalId, dnaIdToOwnerAddress[dnaId], dna.effectiveness, dnaId );
 }
 function migrateData( address to, uint256 animalId, uint8 effectiveness ) external onlyOwner {
 ...
 function giveaway( address to, uint256 animalId, uint8 effectiveness ) external contractIsActive {
 ...
 function donateDnaFromContract( address to, uint256 animalId, uint8 effectiveness ) private contractIsActive {
 require(migrated);
 require(addressIsDonator[msg.sender], "donator not registered");
 donateDna(to, animalId, effectiveness);
 }
 function donateDna( address to, uint256 animalId, uint8 effectiveness ) private {
