 ...
 function claimRewards( address[] calldata stakedContracts, address[] calldata sponsors, address[] calldata tokenAddresses ) external returns (uint[] memory tokensRewarded);
 }
 pragma solidity ^0.6.6;
 ...
 function initialize( address _wNxm, address _arNxm, address _nxm, address _nxmMaster, address _rewardManager ) public {
 ...
 function deposit(uint256 _nAmount, address _referrer, bool _isNxm) external oncePerTx {
 ...
 withdrawals[msg.sender] = WithdrawalRequest( uint48(block.timestamp), prevWithdrawal.nAmount + uint104(nAmount), prevWithdrawal.arAmount + uint104(_arAmount) );
 emit WithdrawRequested(msg.sender, _arAmount, nAmount, block.timestamp, block.timestamp.add(withdrawDelay));
 }
 }
 function withdrawFinalize() external oncePerTx {
 ...
 function getShieldMiningRewards(address _shieldMining, address[] calldata _protocols, address[] calldata _sponsors, address[] calldata _tokens) external notContract {
 IShieldMining(_shieldMining).claimRewards(_protocols, _sponsors, _tokens);
 }
 function arNxmValue(uint256 _nAmount) public view returns (uint256 arAmount) {
 ...
 function aum() public view returns (uint256 aumTotal) {
 ...
 function _getRewardsNxm() internal returns (uint256 finalReward) {
 IPooledStaking pool = IPooledStaking( _getPool() );
 ...
 function withdrawNxm() external onlyOwner {
 _withdrawNxm();
 }
 ...
 function _withdrawNxm() internal returns (uint256 amount) {
 IPooledStaking pool = IPooledStaking( _getPool() );
 amount = pool.stakerMaxWithdrawable( address(this) );
 pool.withdraw(amount);
 }
 function _stakeNxm(address[] memory _protocols, uint256[] memory _stakeAmounts) internal returns (uint256 toStake) {
 IPooledStaking pool = IPooledStaking( _getPool() );
 uint256 balance = nxm.balanceOf( address(this) );
 ...
 function _unstakeNxm(uint256 _lastId, address[] memory _protocols, uint256[] memory _amounts) internal {
 IPooledStaking pool = IPooledStaking( _getPool() );
 pool.requestUnstake(_protocols, _amounts, _lastId);
 }
 function _protocolUnstakeable(address _protocol, uint256 _unstakeAmount) internal view returns (uint256) {
 IPooledStaking pool = IPooledStaking( _getPool() );
 ...
 function pullNXM(address _from, uint256 _amount, address _to) external onlyOwner {
 ...
 function submitVote(uint256 _proposalId, uint256 _solutionChosen) external onlyOwner {
