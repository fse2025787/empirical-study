 ...
 function initialize( address _ownerAddress, address _zoneToken, address _governorTimelock, bool[] memory _typeEnables, uint16[] memory _lockDays, uint256[] memory _rewardRates ) public initializer {
 require(_ownerAddress != address(0), "Owner address is invalid");
 stakeLimit = 2500000e18;
 minStakeAmount = 1e18;
 ...
 function addTypes( bool[] memory _enables, uint16[] memory _lockDays, uint256[] memory _rewardRates ) external onlyOwner() {
 _addTypes(_enables, _lockDays, _rewardRates);
 }
 function _addTypes( bool[] memory _enables, uint16[] memory _lockDays, uint256[] memory _rewardRates ) internal {
 require( _lockDays.length == _rewardRates.length && _lockDays.length == _enables.length, "Mismatched data" );
 ...
 enabled: _enables[i], lockDay: _lockDays[i], rewardRate: _rewardRates[i], stakedAmount: 0 }
 );
 types.push(_type);
 emit AddType (_type.enabled, _type.lockDay, _type.rewardRate);
 }
 }
 function changeType( uint8 _typeIndex, bool _enable, uint16 _lockDay, uint256 _rewardRate ) external onlyOwnerOrCommunity() {
 ...
 stakes[staker] = Stake({
 exist: true, typeIndex: typeIndex, stakedTs: block.timestamp, unstakedTs: 0, stakedAmount: amount, rewardAmount: 0 }
 );
 ...
 function _calcReward( uint256 stakedTs, uint256 stakedAmount, uint8 typeIndex ) internal view returns (uint256 claimIn, uint256 rewardAmount) {
 ...
 function getStakeInfo( address staker ) external view returns (uint256 stakedAmount, uint8 typeIndex, uint256 claimIn, uint256 rewardAmount, uint256 capacity) {
