 ...
 buf: new bytes(n), len: 0 }
 );
 }
 function writeString( StringBuilder memory stringBuilder, string memory s ) internal pure {
 writeBytes(stringBuilder, bytes(s));
 }
 function writeBytes( StringBuilder memory stringBuilder, bytes memory b ) internal pure {
 ...
 function writeChar( StringBuilder memory stringBuilder, string memory s ) internal pure {
 stringBuilder.buf[stringBuilder.len] = bytes(s)[0];
 stringBuilder.len++;
 }
 function writeFixed( StringBuilder memory stringBuilder, uint256 value ) internal pure {
 unchecked {
 ...
 function toBytes( StringBuilder memory stringBuilder ) internal pure returns (bytes memory) {
 bytes memory buf = stringBuilder.buf;
 uint256 len = stringBuilder.len;
 assembly {
 mstore(buf, len) }
 ...
 function toString( StringBuilder memory stringBuilder ) internal pure returns (string memory) {
 return string(toBytes(stringBuilder));
 }
 }
 pragma solidity ^0.8.4;
 library PathLib {
 function decodePath( StringBuilderLib.StringBuilder memory stringBuilder, bytes memory path ) internal pure {
 uint256 i = 0;
 while (i < path.length) {
 ...
 assembly {
 let tablePtr := add(table, 1) let resultPtr := add(result, 32) for {
 let dataPtr := data let endPtr := add(data, mload(data)) }
 lt(dataPtr, endPtr) {
 }
 {
 dataPtr := add(dataPtr, 3) let input := mload(dataPtr) mstore8(resultPtr, mload(add(tablePtr, and(shr(18, input), 0x3F)))) resultPtr := add(resultPtr, 1) mstore8(resultPtr, mload(add(tablePtr, and(shr(12, input), 0x3F)))) resultPtr := add(resultPtr, 1) mstore8(resultPtr, mload(add(tablePtr, and(shr(6, input), 0x3F)))) resultPtr := add(resultPtr, 1) mstore8(resultPtr, mload(add(tablePtr, and(input, 0x3F)))) resultPtr := add(resultPtr, 1) }
 switch mod(mload(data), 3) case 1 {
 mstore8(sub(resultPtr, 1), 0x3d) mstore8(sub(resultPtr, 2), 0x3d) }
 case 2 {
 ...
 function setColors( uint8[] calldata ids, string[] calldata names, string[] calldata values ) external onlyOwner {
 ...
 function setBodies( uint8[] calldata ids, string[] calldata names, bytes[] calldata paths, uint256[] calldata destlens ) external onlyOwner {
 for (uint i = 0; i < ids.length; i++) {
 bodies[ids[i]] = Body({
 name: names[i], path: paths[i], destlen: destlens[i] }
 );
 }
 }
 function setFaces( uint8[] calldata ids, string[] calldata names, bytes[] calldata paths, uint256[] calldata destlens ) external onlyOwner {
 for (uint i = 0; i < ids.length; i++) {
 faces[ids[i]] = Face({
 name: names[i], path: paths[i], destlen: destlens[i] }
 );
 ...
 base64,", Base64.encode(abi.encodePacked( "{
 \"image\":\"data:image/svg+xml;
 base64,", Base64.encode(svg), "\",\"name\":\"Buddy #", tokenIdString, "\",\"external_url\":\"", externalUrlPrefix, tokenIdString, "\",\"attributes\":[{
 ...
 function generateSvg( StringBuilderLib.StringBuilder memory stringBuilder, string memory _color, bytes memory _body, bytes memory _face ) private pure {
 StringBuilderLib.writeString(stringBuilder, "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?><svg width=\"1e3\" height=\"1e3\" viewBox=\"0 0 1e3 1e3\" version=\"1.1\" id=\"svg115\" xml:space=\"preserve\" xmlns=\"http: writeSvgPath(stringBuilder, "body", _color, _body);
 writeSvgPath(stringBuilder, "face", "000000", _face);
 StringBuilderLib.writeString(stringBuilder, "</svg>");
 }
 function writeSvgPath( StringBuilderLib.StringBuilder memory stringBuilder, string memory id, string memory color, bytes memory path ) private pure {
