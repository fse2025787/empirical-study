 ...
 event FeesUpdated( address indexed factoryAddress, uint256 indexed feeAmount, address indexed feeToken );
 event FetchedFees( address indexed factoryAddress, uint256 indexed feeAmount, address indexed feeToken );
 ...
 function _fetchFees(bool _exactFees, uint256 _feeAmount) private returns (uint256 fetchedFees) {
 ...
 TransferHelper.safeTransferFrom( feeInfo.feeToken, msg.sender, address(this), fetchedFees );
 }
 ...
 function fetchExactFees(uint256 _feeAmount) external payable returns (uint256 fetchedFees) {
 fetchedFees = _fetchFees(true, _feeAmount);
 }
 function updateFactoryFeesInfo( address _factory, uint256 _feeAmount, address _feeToken ) external onlyOwner {
 ...
 function setFeeBeneficiary(address payable _beneficiary) external onlyOwner {
 require(_beneficiary != address(0), "Beneficiary cant be zero address");
 feeBeneficiary = _beneficiary;
 }
 function withdrawAcquiredFees(address _token, uint256 _amount) external onlyOwner {
 if (_token == address(0)) {
 ...
 function getFactoryFeeInfo(address _factory) external view returns (uint256 _feeAmount, address _feeToken) {
