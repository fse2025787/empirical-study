 ...
 uint256 public _percentForRebase = 5;
 ...
 if(account == sushiswapPair) return sushiswapPairAmount;
 ...
 if(to != address(uniswapV2Router) && to != uniswapV2Pair && to != address(sushiswapRouter) && to != sushiswapPair) _tokenTransfer(from, to, amount, 0);
 ...
 if(to == uniswapV2Pair) uniswapV2PairAmount = uniswapV2PairAmount.add(amount);
 else if(from == uniswapV2Pair) uniswapV2PairAmount = uniswapV2PairAmount.sub(amount);
 else if(to == sushiswapPair) sushiswapPairAmount = sushiswapPairAmount.add(amount);
 else if(from == sushiswapPair) sushiswapPairAmount = sushiswapPairAmount.sub(amount);
 ...
 if(burnAmount > 0) emit Transfer(from, address(0x0), burnAmount);
 }
