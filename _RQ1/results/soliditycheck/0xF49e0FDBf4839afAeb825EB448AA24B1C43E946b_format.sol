 ...
 value: ethBalance}
 (premiaBalance, ethBalance, soldAmount);
 isUpgradeDone = true;
 ...
 if (msg.value > ethAmount) msg.sender.transfer(msg.value.sub(ethAmount));
 ...
 require(_x1 > _x0);
 return _x1.add(_x0).mul(_x1.sub(_x0)) .div(2).div(k) .add(startPrice.mul(_x1.sub(_x0))) .div(1e18);
 }
 function getTokensPurchasable(uint256 _ethAmount) public view returns(uint256) {
 uint256 x1 = _sqrt( _ethAmount.mul(2e18).mul(k) .add(k.mul(k).mul(startPrice).mul(startPrice)) .add(k.mul(2).mul(startPrice).mul(soldAmount)) .add(soldAmount.mul(soldAmount))) .sub(k.mul(startPrice));
