 ...
 function add_liquidity( uint256[3] calldata amounts, uint256 min_mint_amount, bool isUnderLying ) external;
 ...
 constructor( ICurveRegistry _curveRegistry, uint16 _goodwill, uint16 _affiliateSplit ) public {
 curveReg = _curveRegistry;
 goodwill = _goodwill;
 affiliateSplit = _affiliateSplit;
 }
 event addLiquidity(address sender, address pool, uint256 crvTokens);
 function AddLiquidity( address _fromTokenAddress, address _toTokenAddress, address _swapAddress, uint256 _incomingTokenQty, uint256 _minPoolTokens, address _swapTarget, bytes calldata _swapCallData, address affiliate ) external payable stopInEmergency nonReentrant returns (uint256 crvTokensBought) {
 uint256 toInvest = _pullTokens( _fromTokenAddress, _incomingTokenQty, affiliate );
 if (_fromTokenAddress == address(0)) {
 _fromTokenAddress = ETHAddress;
 }
 crvTokensBought = _performAddLiquidity( _fromTokenAddress, _toTokenAddress, _swapAddress, toInvest, _swapTarget, _swapCallData );
 ...
 function _performAddLiquidity( address _fromTokenAddress, address _toTokenAddress, address _swapAddress, uint256 toInvest, address _swapTarget, bytes memory _swapCallData ) internal returns (uint256 crvTokensBought) {
 (bool isUnderlying, uint8 underlyingIndex) = curveReg.isUnderlyingToken( _swapAddress, _fromTokenAddress );
 if (isUnderlying) {
 crvTokensBought = _enterCurve( _swapAddress, toInvest, underlyingIndex );
 }
 else {
 uint256 tokensBought = _fillQuote( _fromTokenAddress, _toTokenAddress, toInvest, _swapTarget, _swapCallData );
 if (_toTokenAddress == address(0)) _toTokenAddress = ETHAddress;
 (isUnderlying, underlyingIndex) = curveReg.isUnderlyingToken( _swapAddress, _toTokenAddress );
 if (isUnderlying) {
 crvTokensBought = _enterCurve( _swapAddress, tokensBought, underlyingIndex );
 }
 else {
 (uint256 tokens, uint8 metaIndex) = _enterMetaPool( _swapAddress, _toTokenAddress, tokensBought );
 crvTokensBought = _enterCurve(_swapAddress, tokens, metaIndex);
 }
 }
 }
 function _pullTokens( address token, uint256 amount, address affiliate ) internal returns (uint256) {
 uint256 totalGoodwillPortion;
 if (token == address(0)) {
 require(msg.value > 0, "No eth sent");
 totalGoodwillPortion = _subtractGoodwill( ETHAddress, msg.value, affiliate );
 ...
 function _enterMetaPool( address _swapAddress, address _toTokenAddress, uint256 swapTokens ) internal returns (uint256 tokensBought, uint8 index) {
 ...
 tokensBought = _enterCurve( intermediateSwapAddress, swapTokens, index );
 return (tokensBought, i);
 }
 }
 }
 function _fillQuote( address _fromTokenAddress, address _toTokenAddress, uint256 _amount, address _swapTarget, bytes memory _swapCallData ) internal returns (uint256 amountBought) {
 uint256 valueToSend;
 ...
 uint256 initialBalance = _toTokenAddress == address(0) ? address(this).balance : IERC20(_toTokenAddress).balanceOf(address(this));
 (bool success, ) = _swapTarget.call.value(valueToSend)(_swapCallData);
 require(success, "Error Swapping Tokens");
 amountBought = _toTokenAddress == address(0) ? (address(this).balance).sub(initialBalance) : IERC20(_toTokenAddress).balanceOf(address(this)).sub(initialBalance);
