 ...
 function sub(uint256 x, uint256 y) internal pure virtual returns (uint256 z) {
 z = SafeMath.sub(x, y);
 }
 ...
 MemoryInterface internal constant stakeAllMemory = MemoryInterface(0x0A25F019be4C4aAa0B04C0d43dff519dc720D275);
 ...
 IGraphProtocolInterface public constant graphProxy = IGraphProtocolInterface(0xF55041E37E12cD407ad00CE2910B8269B01263b9);
 TokenInterface public constant grtTokenAddress = TokenInterface(0xc944E90C64B2c07662A292be6244BDf05Cda44a7);
 }
 pragma solidity ^0.7.6;
 contract GraphProtocolStaking is Helpers {
 string public constant name = "GraphProtocol-v1";
 using SafeMath for uint256;
 function delegate( address indexer, uint256 amount, uint256 getId ) external {
 ...
 function delegateMultiple( address[] memory indexers, uint256 amount, uint256[] memory portions, uint256 getId ) external payable {
 require(amount > 0, "Invalid amount");
 require( portions.length == indexers.length, "Indexer and Portion length doesnt match" );
 uint256 delegationAmount = getUint(getId, amount);
 ...
 indexersAmount[position] = portions[position] .mul(delegationAmount) .div(PORTIONS_SUM);
 ...
 function undelegateMultiple( address[] memory _indexers, uint256[] memory _shares ) external payable {
 ...
 function withdrawDelegated(address _indexer, address _delegateToIndexer) external payable {
 require(_indexer != address(0), "Invalid indexer address");
 graphProxy.withdrawDelegated(_indexer, _delegateToIndexer);
 }
 function withdrawMultipleDelegate( address[] memory _indexers, address[] memory _delegateToIndexers ) external payable {
 uint256 indexersSize = _indexers.length;
 for (uint256 i = 0; i < indexersSize; i++) {
 require(_indexers[i] != address(0), "Invalid indexer");
 ...
 function delegate(address _indexer, uint256 _tokens) external payable returns (uint256 shares_);
 function undelegate(address _indexer, uint256 _shares) external returns (uint256 tokens_);
 function withdrawDelegated(address _indexer, address _delegateToIndexer) external returns (uint256 tokens_);
 }
