 ...
 function mint(address account, uint256 powah) external override onlyMinter returns (uint256) {
 _starCount++;
 uint256 sID = _starCount;
 _mint(account, sID);
 return sID;
 }
 function mintBatch( address account, uint256 amount, uint256[] calldata powahArr ) external override onlyMinter returns (uint256[] memory) {
