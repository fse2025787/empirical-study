 ...
 IRocketPool internal constant rocketStorage = IRocketPool(0x1d8f8f00cfa6758d7bE78336684788Fb0ee0Fa46);
 ...
 uint256 depositDelay = rocketStorage.getUint( keccak256( abi.encodePacked( keccak256("dao.protocol.setting.network"), "network.reth.deposit.delay" ) ) );
 ...
 function rEthCanAcceptDeposit(uint256 _ethAmount) public view returns (bool) {
 ...
 IRocketPool rocketDAOProtocolSettingsDeposit = IRocketPool(getRPLContract("rocketDAOProtocolSettingsDeposit"));
 IRocketPool rocketDepositPool = IRocketPool(getRPLContract("rocketDepositPool"));
 uint256 maxDeposit = rocketDAOProtocolSettingsDeposit.getMaximumDepositPoolSize().sub( rocketDepositPool.getBalance() );
 return maxDeposit;
 }
 function getRocketDepositPoolAddress() public view returns (address) {
 return getRPLContract("rocketDepositPool");
 }
 function getRPLContract(string memory _contractName) internal view returns (address) {
