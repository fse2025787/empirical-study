 ...
 constructor( string memory _name, string memory _symbol, string memory _initBaseURI, uint256 _maxAzukiMfers ) ERC721(_name, _symbol) {
 ...
 require( tokenCounter.current() + numberOfTokens <= maxAzukiMfers, "Not enough azuki mfers remaining to mint" );
 _;
 }
 ...
 function mintToTeam(uint256 numberOfTokens, address recipient) external onlyOwner totalNotExceeded(numberOfTokens) {
 for (uint256 i = 0; i < numberOfTokens; i++) {
 _safeMint(recipient, nextTokenId());
 }
 }
 function mint(uint256 numberOfTokens) external payable nonReentrant mintActive isCorrectPayment(mintPrice, numberOfTokens) totalNotExceeded(numberOfTokens) {
