 ...
 function swapExactAmountIn( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _tokenAmountIn, uint256 _minAmountOut, address _to, uint256 _deadline, address _referralRecipient ) external returns (uint256 tokenAmountOut);
 function swapExactAmountOut( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _maxAmountIn, uint256 _tokenAmountOut, address _to, uint256 _deadline, address _referralRecipient ) external returns (uint256 tokenAmountIn);
 function getSpotPrice( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath ) external returns (uint256 spotPrice);
 function getAmountIn( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _tokenAmountOut ) external returns (uint256 tokenAmountIn);
 function getAmountOut( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _tokenAmountIn ) external returns (uint256 tokenAmountOut);
 }
 pragma solidity >=0.6.0 <0.8.0;
 ...
 function swapExactAmountIn( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _tokenAmountIn, uint256 _minAmountOut, address _to, uint256 _deadline, address _referralRecipient ) public override ensure(_deadline) returns (uint256 tokenAmountOut) {
 ...
 (_currentTokenAmountIn, ) = _amm.swapExactAmountIn( _pairPath[0], _tokenPath[0], _currentTokenAmountIn, _tokenPath[1], 0, _pairPathMaxIndex == 1 ? _to : address(this) );
 if (_pairPathMaxIndex == 2) {
 (_currentTokenAmountIn, ) = _amm.swapExactAmountIn( _pairPath[1], _tokenPath[2], _currentTokenAmountIn, _tokenPath[3], 0, _to );
 }
 require(_currentTokenAmountIn >= _minAmountOut, "AMMRouterV1: Min amount not reached");
 tokenAmountOut = _currentTokenAmountIn;
 }
 function _pushFeesCalculation(uint256 _tokenAmountIn, bool _isSwappingOut) internal returns ( uint256 _currentAmountIn, uint256 _feeAmount, uint256 _pushAmount ) {
 if (_isSwappingOut) {
 ...
 function _pushFees( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _tokenAmountIn, address _referralRecipient, bool _isSwappingOut ) internal returns (uint256 _currentAmountIn) {
 uint256 _feeAmount;
 uint256 _pushAmount;
 IERC20 tokenIn = _tokenPath[0] == 0 ? IERC20(_amm.getPTAddress()) : IERC20(_amm.getPairWithID(_pairPath[0]).tokenAddress);
 ...
 function swapExactAmountOut( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _maxAmountIn, uint256 _tokenAmountOut, address _to, uint256 _deadline, address _referralRecipient ) external override ensure(_deadline) returns (uint256 tokenAmountIn) {
 uint256 _pairPathMaxIndex = _pairPath.length;
 require(_pairPathMaxIndex <= 2 && _pairPathMaxIndex > 0, "AMMRouterswapExactAmountOutV1: invalid path length");
 uint256 currentAmountInWithoutGovernance = _getAmountInWithoutGovernance(_amm, _pairPath, _tokenPath, _tokenAmountOut);
 ...
 function _executeSwap( IAMM _amm, uint256[] memory _pairPath, uint256[] memory _tokenPath, uint256 amountIn, uint256 _tokenAmountOut, address _to ) private returns (uint256 tokenAmountIn) {
 ...
 (tokenAmountIn, ) = _amm.swapExactAmountOut( _pairPath[0], _tokenPath[0], amountIn, _tokenPath[1], firstSwapAmountOut, address(this) );
 }
 else {
 ...
 _amm.swapExactAmountOut( _pairPath.length == 2 ? _pairPath[1] : _pairPath[0], tokenPath[0], firstSwapAmountOut, tokenPath[1], _tokenAmountOut, _to );
 }
 function getSpotPrice( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath ) external view override returns (uint256 spotPrice) {
 uint256 _pairPathMaxIndex = _pairPath.length;
 ...
 function _getAmountInWithoutGovernance( IAMM _amm, uint256[] memory _pairPath, uint256[] memory _tokenPath, uint256 _tokenAmountOut ) internal view returns (uint256 _currentTokenAmountInWithoutGovernance) {
 _currentTokenAmountInWithoutGovernance = _tokenAmountOut;
 uint256 _pairPathMaxIndex = _pairPath.length;
 for (uint256 i = _pairPathMaxIndex; i > 0; i--) {
 (_currentTokenAmountInWithoutGovernance, ) = _amm.calcInAndSpotGivenOut( _pairPath[i - 1], _tokenPath[2 * i - 2], MAX_UINT256, _tokenPath[2 * i - 1], _currentTokenAmountInWithoutGovernance );
 }
 }
 function getAmountIn( IAMM _amm, uint256[] memory _pairPath, uint256[] memory _tokenPath, uint256 _tokenAmountOut ) public view override returns (uint256 tokenAmountIn) {
 ...
 function getAmountOut( IAMM _amm, uint256[] calldata _pairPath, uint256[] calldata _tokenPath, uint256 _tokenAmountIn ) external view override returns (uint256 tokenAmountOut) {
 uint256 _currentTokenAmountOut = _tokenAmountIn;
 uint256 _pairPathMaxIndex = _pairPath.length;
 for (uint256 i; i < _pairPathMaxIndex; i++) {
 (_currentTokenAmountOut, ) = _amm.calcOutAndSpotGivenIn( _pairPath[i], _tokenPath[2 * i], _currentTokenAmountOut, _tokenPath[2 * i + 1], 0 );
 }
