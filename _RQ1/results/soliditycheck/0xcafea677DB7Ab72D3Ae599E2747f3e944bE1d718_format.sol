 ...
 , address , uint , uint16 , CoverType , bytes calldata data ) external view returns (uint coverPrice) {
 this;
 ( coverPrice, , , , , , ) = abi.decode(data, (uint, uint, uint, uint, uint8, bytes32, bytes32));
 }
 function buyCover ( address contractAddress, address coverAsset, uint sumAssured, uint16 coverPeriod, CoverType coverType, bytes calldata data ) external payable onlyMember whenNotPaused returns (uint) {
 ...
 {
 ( uint[] memory coverDetails, uint8 _v, bytes32 _r, bytes32 _s ) = convertToLegacyQuote(sumAssured, data, coverAsset);
 {
 ...
 quotation.createCover( msg.sender, contractAddress, getCurrencyFromAssetAddress(coverAsset), coverDetails, coverPeriod, _v, _r, _s );
 }
 ...
 function claimTokens(uint coverId, uint incidentId, uint coveredTokenAmount, address coveredToken) external returns (uint claimId, uint payoutAmount, address payoutToken) {
 ...
 function getPayoutOutcome(uint claimId) external view returns (ClaimStatus status, uint amountPaid, address coverAsset) {
 (, uint coverId) = claimsData.getClaimCoverId(claimId);
 ...
 function getCover(uint coverId) public view returns ( uint8 status, uint sumAssured, uint16 coverPeriod, uint validUntil, address contractAddress, address coverAsset, uint premiumInNXM, address memberAddress ) {
 bytes4 currency;
 ...
 function executeCoverAction(uint , uint8 , bytes calldata ) external payable returns (bytes memory, uint) {
 revert("Gateway: Unsupported action");
 }
 function convertToLegacyQuote(uint sumAssured, bytes memory data, address asset) internal view returns (uint[] memory coverDetails, uint8, bytes32, bytes32) {
 ( uint coverPrice, uint coverPriceNXM, uint expiresAt, uint generatedAt, uint8 v, bytes32 r, bytes32 s ) = abi.decode(data, (uint, uint, uint, uint, uint8, bytes32, bytes32));
