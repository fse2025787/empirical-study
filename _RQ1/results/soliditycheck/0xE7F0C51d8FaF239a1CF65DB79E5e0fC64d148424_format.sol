 ...
 if ( int_price > 1e18 || (int_price == 1e18 && podRate.lessThanOrEqualTo(C.getOptimalPodRate())) ) {
 caseId += 4;
 }
 ...
 uint256 liquidity;
 uint256 ethAdded;
 (beans, ethAdded, liquidity) = _addLiquidity( msg.value.sub(ethSold), beans, al.minEthAmount, al.minBeanAmount );
 if (al.beanAmount > beans) IBean(ds.bean).transfer(msg.sender, al.beanAmount.sub(beans));
