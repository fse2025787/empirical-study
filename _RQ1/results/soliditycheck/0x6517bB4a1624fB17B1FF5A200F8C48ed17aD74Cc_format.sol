 ...
 function initialize( address rewards_token, address governance_contract_address ) public initializer {
 ...
 return acc_reward_per_voting_token_.add( block.timestamp.sub(last_update_time_).mul(uint256(reward_rate_)).mul(1e18).div(total_supply_) );
 }
 }
 function getPoolRewards(address pool_address) override public view returns (uint256) {
 return uint256(pool_states_[pool_address].votes) .mul(getRewardPerVotingToken().sub(pool_states_[pool_address].last_acc_reward_per_voting_token)) .add(pool_states_[pool_address].acc_reward);
 }
