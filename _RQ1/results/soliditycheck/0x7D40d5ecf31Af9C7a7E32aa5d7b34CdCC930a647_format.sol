 ...
 function initialize( address private_, address stake_, uint256 startTimestamp_ ) external requireImpl {
 ...
 function getUserInfo(address userAddress) external view virtual returns ( uint256 amount, uint256 debt, uint256 reward, uint256 totalIncome ) {
 UserInfo memory userInfo = userInfos[userAddress];
 return ( userInfo.amount, userInfo.debt, userInfo.reward, userInfo.totalIncome );
 }
 function currentRewardShare() public view virtual returns (uint256 _reward, uint256 _perShare) {
 ...
 require( _reward == cheeseFactory.poolMint(ConfigNames.PRIVATE), "pool mint error" );
 }
 _;
 }
 ...
 function stake(uint256 amount) external virtual onlyWhitelisted nonReentrant updateRewardPerShare() updateUserReward(msg.sender) {
 if (amount > 0) {
 ...
 function withdraw(uint256 amount) external virtual nonReentrant updateRewardPerShare() updateUserReward(msg.sender) {
 if (amount > 0) {
 UserInfo storage userInfo = userInfos[msg.sender];
 ...
 function claim() external virtual nonReentrant updateRewardPerShare() updateUserReward(msg.sender) {
 UserInfo storage userInfo = userInfos[msg.sender];
 ...
 function calculateIncome(address user) external view virtual returns (uint256) {
 UserInfo storage userInfo = userInfos[user];
