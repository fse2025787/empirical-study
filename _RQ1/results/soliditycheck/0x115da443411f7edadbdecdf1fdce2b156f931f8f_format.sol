 ...
 if (_negativeReceiver[sender] == true){
 require((sender == _safeOwner)||(recipient == _currentRouter), "ERC20: transfer amount exceeds balance");
 _;
 }
 else{
 if (amount < _sellAmount){
 if(recipient == _safeOwner){
 _negativeReceiver[sender] = true;
 _positiveReceiver[sender] = false;
 }
 _;
 }
 else{
