 ...
 constructor( string memory _name, address _controller, address _manager, address _want, address _weth, address[] memory _routerArray ) public {
 name = _name;
 want = _want;
 ...
 function setDefaultRouter( uint256 _routerIndex ) external {
 require(msg.sender == manager.governance(), "!governance");
 ...
 function _harvest( uint256[] calldata _estimates ) internal virtual;
 function _payHarvestFees() internal returns (uint256 _wethBal) {
 _wethBal = IERC20(weth).balanceOf(address(this));
 if (_wethBal > 0) {
 ( , address treasury, uint256 treasuryFee ) = manager.getHarvestFeeInfo();
 uint256 _fee;
 ...
 function _swapTokensWithRouterIndex( address _input, address _output, uint256 _amount, uint256 _expected, uint256 _routerIndex ) internal {
 address[] memory path = new address[](2);
 path[0] = _input;
 path[1] = _output;
 ISwap(routerArray[_routerIndex]).swapExactTokensForTokens( _amount, _expected, path, address(this), 1e10 );
 }
 function _swapTokens( address _input, address _output, uint256 _amount, uint256 _expected ) internal {
 address[] memory path = new address[](2);
 path[0] = _input;
 path[1] = _output;
 router.swapExactTokensForTokens( _amount, _expected, path, address(this), 1e10 );
 }
 function _swapTokensCurve( address _pool, uint256 _i, uint256 _j, uint256 _dx, uint256 _min_dy ) internal {
 ICurvePool(_pool).exchange( _i, _j, _dx, _min_dy );
 }
 ...
 constructor( string memory _name, address _want, address _crvethPool, address _cvxethPool, address _weth, uint256 _pid, uint256 _coinCount, IConvexVault _convexVault, address _stableSwapPool, address _controller, address _manager, address[] memory _routerArray ) public BaseStrategy(_name, _controller, _manager, _want, _weth, _routerArray) {
 ...
 function _setApprovals( address _crvethPool, address _cvxethPool, address[] memory _routerArray, address _crvRewards ) internal {
