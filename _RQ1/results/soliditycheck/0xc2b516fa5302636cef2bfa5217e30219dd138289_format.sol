 ...
 event LogConvert( address indexed server, address indexed token0, address indexed token1, uint256 amount0, uint256 amount1, uint256 amountHOS );
 constructor( address _factory, address _bar, address _hos, address _weth ) public {
 ...
 require( token != HOS && token != weth && token != bridge, "HorusMaker: Invalid bridge" );
 ...
 function _convertStep( address token0, address token1, uint256 amount0, uint256 amount1 ) internal returns (uint256 hosOut) {
 if (token0 == token1) {
 ...
 hosOut = _toHOS( weth, _swap(token1, weth, amount1, address(this)).add(amount0) );
 }
 else if (token1 == weth) {
 hosOut = _toHOS( weth, _swap(token0, weth, amount0, address(this)).add(amount1) );
 }
 else {
 address bridge0 = bridgeFor(token0);
 address bridge1 = bridgeFor(token1);
 if (bridge0 == token1) {
 hosOut = _convertStep( bridge0, token1, _swap(token0, bridge0, amount0, address(this)), amount1 );
 }
 else if (bridge1 == token0) {
 hosOut = _convertStep( token0, bridge1, amount0, _swap(token1, bridge1, amount1, address(this)) );
 }
 else {
 hosOut = _convertStep( bridge0, bridge1, _swap(token0, bridge0, amount0, address(this)), _swap(token1, bridge1, amount1, address(this)) );
 }
 }
 }
 function _swap( address fromToken, address toToken, uint256 amountIn, address to ) internal returns (uint256 amountOut) {
 IHorusPair pair = IHorusPair(factory.getPair(fromToken, toToken));
 ...
 function _toHOS(address token, uint256 amountIn) internal returns (uint256 amountOut) {
