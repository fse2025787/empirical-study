 ...
 function _safeM1nt( address to, uint256 quantity, bytes memory _data ) internal {
 _m1nt(to, quantity, _data, true);
 }
 function _burn0( uint256 quantity ) internal {
 _mintZero(quantity);
 }
 ...
 function _m1nt( address to, uint256 quantity, bytes memory _data, bool safe ) internal {
 uint256 startTokenId = _currentIndex;
 if (to == address(0)) revert MintToZeroAddress();
 ...
 string public uriPrefix = "ipfs: uint256 public immutable mintPrice = 0.001 ether;
 ...
 constructor() ERC721A ("Yea I am Shocked", "Shocked") {
 }
