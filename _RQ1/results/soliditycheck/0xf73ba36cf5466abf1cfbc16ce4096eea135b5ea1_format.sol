 ...
 ICCPair(pairFor(input, output)).swap( amount0Out, amount1Out, to, new bytes(0) );
 }
 }
 ...
 require( IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin, 'CCRouter: INSUFFICIENT_OUTPUT_AMOUNT' );
 }
