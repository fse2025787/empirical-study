 ...
 using SafeMath for uint256;
 event Leverage( uint256 amountRequested, uint256 amountUsed, uint256 requiredDAI, uint256 amountToCloseLTVGap, bool deficit, address flashLoan );
 ...
 function doFlashMint( bool deficit, uint256 amountDesired, address token, uint256 collatRatioDAI, uint256 depositToCloseLTVGap ) public returns (uint256 amount) {
 if (amountDesired == 0) {
 return 0;
 }
 ...
 uint256 requiredDAI;
 {
 requiredDAI = _toDAI(amount, token).mul(COLLAT_RATIO_PRECISION).div( collatRatioDAI );
 ...
 amount = _fromDAI( requiredDAI.sub(requiredDAIToCloseLTVGap), token ) .mul(collatRatioDAI) .div(COLLAT_RATIO_PRECISION);
 }
 }
 ...
 if (isDai) {
 if (deficit) {
 lp.deposit( dai, amountFlashmint.sub(amount), address(this), referral );
 lp.repay( dai, IERC20(dai).balanceOf(address(this)), 2, address(this) );
 lp.withdraw(dai, amountFlashmint, address(this));
 }
 else {
 lp.deposit( dai, IERC20(dai).balanceOf(address(this)), address(this), referral );
 ...
 lp.deposit( want, IERC20(want).balanceOf(address(this)), address(this), referral );
 }
 lp.withdraw(dai, amountFlashmint, address(this));
 }
 ...
 function _toDAI(uint256 _amount, address asset) internal view returns (uint256) {
 address dai = DAI;
 if ( _amount == 0 || _amount == type(uint256).max || asset == dai ) {
 return _amount;
 }
 if (asset == WETH) {
 return _amount .mul(uint256(10)**uint256(IOptionalERC20(dai).decimals())) .div(_priceOracle().getAssetPrice(dai));
 }
 address[] memory tokens = new address[](2);
 tokens[0] = asset;
 tokens[1] = dai;
 uint256[] memory prices = _priceOracle().getAssetsPrices(tokens);
 uint256 ethPrice = _amount.mul(prices[0]).div( uint256(10)**uint256(IOptionalERC20(asset).decimals()) );
 return ethPrice.mul(DAI_DECIMALS).div(prices[1]);
 }
 function _fromDAI(uint256 _amount, address asset) internal view returns (uint256) {
 address dai = DAI;
 if ( _amount == 0 || _amount == type(uint256).max || asset == dai ) {
 return _amount;
 }
 if (asset == WETH) {
 return _amount.mul(_priceOracle().getAssetPrice(dai)).div( uint256(10)**uint256(IOptionalERC20(dai).decimals()) );
 }
 address[] memory tokens = new address[](2);
 tokens[0] = asset;
 tokens[1] = dai;
 ...
 return ethPrice .mul(uint256(10)**uint256(IOptionalERC20(asset).decimals())) .div(prices[0]);
 }
 ...
 enum CooldownStatus {
 None, Claim, Initiated}
 IUni private constant UNI_V2_ROUTER = IUni(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
 IUni private constant SUSHI_V2_ROUTER = IUni(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F);
 ISwapRouter private constant UNI_V3_ROUTER = ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);
 ...
 function initialize( address _vault, address _strategist, address _rewards, address _keeper ) external {
 ...
 function setCollateralTargets( uint256 _targetCollatRatio, uint256 _maxCollatRatio, uint256 _maxBorrowCollatRatio, uint256 _daiBorrowCollatRatio ) external onlyVaultManagers {
 (uint256 ltv, uint256 liquidationThreshold) = getProtocolCollatRatios(address(want));
 (uint256 daiLtv, ) = getProtocolCollatRatios(dai);
 ...
 function setIsFlashMintActive(bool _isFlashMintActive) external onlyVaultManagers {
 ...
 function setMinsAndMaxs( uint256 _minWant, uint256 _minRatio, uint8 _maxIterations ) external onlyVaultManagers {
 ...
 function setRewardBehavior( SwapRouter _swapRouter, bool _sellStkAave, bool _cooldownStkAave, uint256 _minRewardToSell, uint256 _maxStkAavePriceImpactBps, uint24 _stkAaveToAaveSwapFee, uint24 _aaveToWethSwapFee, uint24 _wethToWantSwapFee ) external onlyVaultManagers {
 require( _swapRouter == SwapRouter.UniV2 || _swapRouter == SwapRouter.SushiV2 || _swapRouter == SwapRouter.UniV3 );
 ...
 uint256 combinedStkAave = pendingRewards.add(stkAaveBalance).mul(stkAaveDiscountFactor).div( MAX_BPS );
 ...
 uint256 minAAVEOut = stkAaveBalance.mul(MAX_BPS.sub(maxStkAavePriceImpactBps)).div( MAX_BPS );
 ...
 (uint256 deposits, uint256 borrows) = getCurrentPosition();
 uint256 depositsToMeetLtv = getDepositFromBorrow(borrows, maxBorrowCollatRatio);
 ...
 IUni router = swapRouter == SwapRouter.SushiV2 ? SUSHI_V2_ROUTER : UNI_V2_ROUTER;
 ...
 if ( block.timestamp > nextClaimStartTimestamp && block.timestamp <= nextClaimStartTimestamp.add(UNSTAKE_WINDOW) ) {
 ...
 function getTokenOutPathV3(address _token_in, address _token_out) internal view returns (bytes memory _path) {
 if (address(want) == weth) {
 _path = abi.encodePacked( address(aave), aaveToWethSwapFee, address(weth) );
 }
 else {
 _path = abi.encodePacked( address(aave), aaveToWethSwapFee, address(weth), wethToWantSwapFee, address(want) );
 }
 }
 ...
 UNI_V3_ROUTER.exactInput( ISwapRouter.ExactInputParams( getTokenOutPathV3(address(aave), address(want)), address(this), now, amountIn, minOut ) );
 }
 else {
 IUni router = swapRouter == SwapRouter.UniV2 ? UNI_V2_ROUTER : SUSHI_V2_ROUTER;
 ...
 UNI_V3_ROUTER.exactInputSingle( ISwapRouter.ExactInputSingleParams( address(stkAave), address(aave), stkAaveToAaveSwapFee, address(this), now, amountIn, minOut, 0 ) );
 }
 ...
 function cloneLevAave(address _vault) external returns (address payable newStrategy) {
 ...
 Strategy(newStrategy).initialize( _vault, msg.sender, msg.sender, msg.sender );
