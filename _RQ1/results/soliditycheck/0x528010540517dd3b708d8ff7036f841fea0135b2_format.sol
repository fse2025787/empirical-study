 ...
 function marketApproval(uint256 _agree, uint256 _opposite) external view returns (bool);
 function policyApproval(uint256 _agree, uint256 _opposite) external view returns (bool);
 ...
 constructor( address _config, address _own, string memory _name, string memory _symbol ) public UsingConfig(_config) {
 require( msg.sender == config().propertyFactory(), "this is illegal address" );
 __author = _own;
 __name = _name;
 __symbol = _symbol;
 ...
 IPropertyFactory(config().propertyFactory()).createChangeAuthorEvent( __author, _nextAuthor );
 __author = _nextAuthor;
 }
 function changeName(string calldata _name) external onlyAuthor {
 IPropertyFactory(config().propertyFactory()).createChangeNameEvent( __name, _name );
 __name = _name;
 }
 function changeSymbol(string calldata _symbol) external onlyAuthor {
 IPropertyFactory(config().propertyFactory()).createChangeSymbolEvent( __symbol, _symbol );
 __symbol = _symbol;
 }
