 ...
 function _setProxyRegistryAddress(address _openSeaProxyRegistryAddress) internal {
 openSeaProxyRegistryAddress = _openSeaProxyRegistryAddress;
 }
 function isApprovedForAll(address owner, address operator) public view virtual returns (bool) {
 OpenSeaProxyRegistry openSeaProxyRegistry = OpenSeaProxyRegistry( openSeaProxyRegistryAddress );
 ...
 constructor( string memory _initialBaseURI, address _openSeaProxyRegistryAddress ) ERC721('PSS MintPass', 'PSSMP') {
 baseURI = _initialBaseURI;
 OpenSeaTradableNFT._setProxyRegistryAddress( _openSeaProxyRegistryAddress );
 }
 ...
 function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
 require( _exists(tokenId), 'ERC721Metadata: URI query for nonexistent token' );
 return bytes(baseURI).length > 0 ? string(abi.encodePacked(_baseURI(), 'metadata.json')) : '';
 }
