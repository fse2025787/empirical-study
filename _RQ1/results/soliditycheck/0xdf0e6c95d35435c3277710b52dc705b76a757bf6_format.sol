 ...
 event AddLpInfo( IERC20MetaData _lpToken, IERC20MetaData _rewardToken0, IERC20MetaData _rewardToken1 );
 ...
 function addLPInfo( IERC20MetaData _lpToken, IERC20MetaData _rewardToken0, IERC20MetaData _rewardToken1 ) public isOperator {
 if (!lpStatus[address(_lpToken)]) {
 uint256 currentIndex = poolLength();
 poolInfo.push( PoolInfo({
 lpToken: _lpToken, accPerShare0: 0, accPerShare1: 0, rewardToken0: _rewardToken0, rewardToken1: _rewardToken1 }
 ) );
 ...
 function addrewardtoken( address _lp, address token, uint256 amount ) public {
 uint256 _pid = lpIndex[_lp];
 PoolInfo storage pool = poolInfo[_pid];
 uint256 lpSupply = pool.lpToken.balanceOf(address(this));
 if (lpSupply == 0) {
 IERC20MetaData(token).transferFrom( msg.sender, feeto, amount );
 return;
 }
 if (amount > 0) {
 if (token == address(pool.rewardToken0)) {
 pool.rewardToken0.transferFrom( msg.sender, address(this), amount );
 pool.accPerShare0 = pool.accPerShare0.add( amount.mul(1e12).div(lpSupply) );
 }
 else if (token == address(pool.rewardToken1)) {
 pool.rewardToken1.transferFrom( msg.sender, address(this), amount );
 pool.accPerShare1 = pool.accPerShare1.add( amount.mul(1e12).div(lpSupply) );
 }
 }
 emit AddReward(address(pool.lpToken), token, address(0x000), amount, 0);
 }
 function addReward( address _lp, address token0, uint256 amount0, uint256 amount1 ) public {
 uint256 _pid = lpIndex[_lp];
 PoolInfo storage pool = poolInfo[_pid];
 uint256 lpSupply = pool.lpToken.balanceOf(address(this));
 uint256 reward0;
 ...
 pool.accPerShare0 = pool.accPerShare0.add( reward0.mul(1e12).div(lpSupply) );
 }
 if (reward1 > 0) {
 pool.rewardToken1.transferFrom(msg.sender, address(this), reward1);
 pool.accPerShare1 = pool.accPerShare1.add( reward1.mul(1e12).div(lpSupply) );
 }
 ...
 if (user.amount > 0) {
 pendingReward0 = user.amount.mul(pool.accPerShare0).div(1e12).sub( user.rewardDebt0 );
 safeRewardTransfer(pool.rewardToken0, _userAddr, pendingReward0);
 pendingReward1 = user.amount.mul(pool.accPerShare1).div(1e12).sub( user.rewardDebt1 );
 ...
 function safeRewardTransfer( IERC20MetaData _reward, address _to, uint256 _amount ) internal {
