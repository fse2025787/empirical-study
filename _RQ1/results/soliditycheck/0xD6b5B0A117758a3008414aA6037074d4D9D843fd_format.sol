 ...
 interface IVault {
 function deposit( uint256, uint256, address ) external returns (uint256, uint256);
 ...
 function deposit( uint256 deposit0, uint256 deposit1, address to ) external override nonReentrant returns (uint256 amount0, uint256 amount1) {
 require(to != address(0), "to");
 if (totalSupply() == 0) {
 uint128 shares = _liquidityForAmounts(baseLower, baseUpper, deposit0, deposit1);
 (amount0, amount1) = _mintLiquidity( baseLower, baseUpper, _uint128Safe(shares), msg.sender );
 _mint(to, shares);
 ...
 (int256 amount0Delta, int256 amount1Delta) = pool.swap( address(this), zeroForOneTerm > 0, zeroForOneTerm > 0 ? int256(token1Exchanged).mul(-1) : int256(token1Exchanged), zeroForOneTerm > 0 ? TickMath.MIN_SQRT_RATIO + 1 : TickMath.MAX_SQRT_RATIO - 1, abi.encode(address(this)) );
 ...
 (uint256 base0, uint256 base1) = _mintLiquidity(baseLower, baseUpper, baseLiquidity, address(this));
 (uint256 limit0, uint256 limit1) = _mintLiquidity(limitLower, limitUpper, limitLiquidity, address(this));
 {
 ...
 function withdraw( uint256 shares, address to, address from ) external override nonReentrant returns (uint256 amount0, uint256 amount1) {
 ...
 function getBaseFees() public view returns ( uint256 fees0, uint256 fees1 ) {
 bytes32 positionKey = keccak256(abi.encodePacked(address(this), baseLower, baseUpper));
 (, , , fees0, fees1) = pool.positions(positionKey);
 }
 function getLimitFees() public view returns ( uint256 fees0, uint256 fees1 ) {
