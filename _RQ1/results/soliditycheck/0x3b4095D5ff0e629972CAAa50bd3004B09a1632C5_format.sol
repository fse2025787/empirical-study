 ...
 function openShort( uint256 _vaultId, uint256 _powerPerpAmount, uint256 _uniNftId, ISwapRouter.ExactInputSingleParams memory _exactInputParams ) external payable nonReentrant {
 if (_vaultId != 0) require(shortPowerPerp.ownerOf(_vaultId) == msg.sender, "Not allowed");
 require( _exactInputParams.tokenOut == address(weth) && _exactInputParams.tokenIn == wPowerPerp, "Wrong swap tokens" );
 (uint256 vaultId, uint256 wPowerPerpAmount) = controller.mintPowerPerpAmount{
 value: msg.value}
 ( _vaultId, _powerPerpAmount, _uniNftId );
 ...
 function closeShort( uint256 _vaultId, uint256 _wPowerPerpAmount, uint256 _withdrawAmount, ISwapRouter.ExactOutputSingleParams memory _exactOutputParams ) external payable nonReentrant {
 require(shortPowerPerp.ownerOf(_vaultId) == msg.sender, "Not allowed");
 require( _exactOutputParams.tokenOut == wPowerPerp && _exactOutputParams.tokenIn == address(weth), "Wrong swap tokens" );
 weth.deposit{
 value: msg.value}
 ();
 ...
 function getFee( uint256 _vaultId, uint256 _wPowerPerpAmount, uint256 _collateralAmount ) external view returns (uint256);
 ...
 function mintPowerPerpAmount( uint256 _vaultId, uint256 _powerPerpAmount, uint256 _uniTokenId ) external payable returns (uint256 vaultId, uint256 wPowerPerpAmount);
 function mintWPowerPerpAmount( uint256 _vaultId, uint256 _wPowerPerpAmount, uint256 _uniTokenId ) external payable returns (uint256 vaultId);
 function deposit(uint256 _vaultId) external payable;
 function withdraw(uint256 _vaultId, uint256 _amount) external payable;
 function burnWPowerPerpAmount( uint256 _vaultId, uint256 _wPowerPerpAmount, uint256 _withdrawAmount ) external;
 function burnOnPowerPerpAmount( uint256 _vaultId, uint256 _powerPerpAmount, uint256 _withdrawAmount ) external returns (uint256 wPowerPerpAmount);
 ...
 function getVaultStatus( Vault memory _vault, address _positionManager, uint256 _normalizationFactor, uint256 _ethQuoteCurrencyPrice, uint256 _minCollateral, int24 _wsqueethPoolTick, bool _isWethToken0 ) internal view returns (bool, bool) {
 ...
 uint256 totalCollateral = _getEffectiveCollateral( _vault, _positionManager, _normalizationFactor, _ethQuoteCurrencyPrice, _wsqueethPoolTick, _isWethToken0 );
 ...
 function _getEffectiveCollateral( Vault memory _vault, address _positionManager, uint256 _normalizationFactor, uint256 _ethQuoteCurrencyPrice, int24 _wsqueethPoolTick, bool _isWethToken0 ) internal view returns (uint256) {
 if (_vault.NftCollateralId == 0) return _vault.collateralAmount;
 (uint256 nftEthAmount, uint256 nftWsqueethAmount) = _getUniPositionBalances( _positionManager, _vault.NftCollateralId, _wsqueethPoolTick, _isWethToken0 );
 uint256 wSqueethIndexValueInEth = nftWsqueethAmount.mul(_normalizationFactor).mul(_ethQuoteCurrencyPrice).div( ONE_ONE );
 return nftEthAmount.add(wSqueethIndexValueInEth).add(_vault.collateralAmount);
 }
 function _getUniPositionBalances( address _positionManager, uint256 _tokenId, int24 _wPowerPerpPoolTick, bool _isWethToken0 ) internal view returns (uint256 ethAmount, uint256 wPowerPerpAmount) {
 ( int24 tickLower, int24 tickUpper, uint128 liquidity, uint128 tokensOwed0, uint128 tokensOwed1 ) = _getUniswapPositionInfo(_positionManager, _tokenId);
 (uint256 amount0, uint256 amount1) = _getToken0Token1Balances( tickLower, tickUpper, _wPowerPerpPoolTick, liquidity );
 return _isWethToken0 ? (amount0 + tokensOwed0, amount1 + tokensOwed1) : (amount1 + tokensOwed1, amount0 + tokensOwed0);
 }
 function _getUniswapPositionInfo(address _positionManager, uint256 _tokenId) internal view returns ( int24, int24, uint128, uint128, uint128 ) {
 INonfungiblePositionManager positionManager = INonfungiblePositionManager(_positionManager);
 ( , , , , , int24 tickLower, int24 tickUpper, uint128 liquidity, , , uint128 tokensOwed0, uint128 tokensOwed1 ) = positionManager.positions(_tokenId);
 return (tickLower, tickUpper, liquidity, tokensOwed0, tokensOwed1);
 }
 function _getToken0Token1Balances( int24 _tickLower, int24 _tickUpper, int24 _tick, uint128 _liquidity ) internal pure returns (uint256 amount0, uint256 amount1) {
 uint160 sqrtPriceX96 = TickMathExternal.getSqrtRatioAtTick(_tick);
 if (_tick < _tickLower) {
 amount0 = SqrtPriceMathPartial.getAmount0Delta( TickMathExternal.getSqrtRatioAtTick(_tickLower), TickMathExternal.getSqrtRatioAtTick(_tickUpper), _liquidity, true );
 }
 else if (_tick < _tickUpper) {
 amount0 = SqrtPriceMathPartial.getAmount0Delta( sqrtPriceX96, TickMathExternal.getSqrtRatioAtTick(_tickUpper), _liquidity, true );
 amount1 = SqrtPriceMathPartial.getAmount1Delta( TickMathExternal.getSqrtRatioAtTick(_tickLower), sqrtPriceX96, _liquidity, true );
 }
 else {
 amount1 = SqrtPriceMathPartial.getAmount1Delta( TickMathExternal.getSqrtRatioAtTick(_tickLower), TickMathExternal.getSqrtRatioAtTick(_tickUpper), _liquidity, true );
 }
 }
 }
 pragma solidity >=0.7.5;
