 ...
 constructor( TotemToken _totemToken, uint256 _totalAmount, uint256 _withdrawInterval, uint256 _releasePeriods, uint256 _lockPeriods ) {
 ...
 function vested(address beneficiary) public view virtual returns (uint256 _amountVested) {
 VestingSchedule memory _vestingSchedule = recipients[beneficiary];
 if ( !isStartTimeSet || (_vestingSchedule.totalAmount == 0) || (lockPeriods == 0 && releasePeriods == 0) || (block.timestamp < startTime) ) {
 return 0;
 }
 ...
 uint256 period = block.timestamp.sub(startTime).div(withdrawInterval) + 1;
 ...
 function withdrawable(address beneficiary) public view returns (uint256 amount) {
 ...
 constructor(TotemToken _totemToken) TotemVesting( _totemToken, TOTAL_AMOUNT, WITHDRAW_INTERVAL, RELEASE_PERIODS, LOCK_PERIODS ) {
 }
 }
 pragma solidity ^0.7.0;
 library SafeMath {
 ...
 uint256 public constant COMMUNITY_DEVELOPMENT = 1000000 * (10**uint256(DECIMALS));
 ...
 uint256 public constant LAUNCH_POOL = 5882352941 * (10**uint256(DECIMALS - 5));
 uint256 public constant PUBLIC_SALE = 450000 * (10**uint256(DECIMALS)) + LAUNCH_POOL;
 uint256 public constant STRATEGIC_ROUND = 1500000 * (10**uint256(DECIMALS)) - LAUNCH_POOL;
 ...
 function setDistributionTeamsAddresses( address _CommunityDevelopmentAddr, address _StakingRewardsAddr, address _LiquidityPoolAddr, address _PublicSaleAddr, address _AdvisorsAddr, address _SeedInvestmentAddr, address _PrivateSaleAddr, address _TeamAllocationAddr, address _StrategicRoundAddr ) public onlyOwner {
 ...
 require((!_isDistributionComplete));
 _transfer( address(this), CommunityDevelopmentAddr, COMMUNITY_DEVELOPMENT );
 ...
 !taxExempt[_msgSender()] ? _transferWithTax(_msgSender(), recipient, amount) : _transfer(_msgSender(), recipient, amount);
 return true;
 }
 function transferFrom( address sender, address recipient, uint256 amount ) public override returns (bool) {
 !taxExempt[sender] ? _transferWithTax(sender, recipient, amount) : _transfer(sender, recipient, amount);
 approve( _msgSender(), allowance(sender, _msgSender()).sub( amount, "Transfer amount exceeds allowance" ) );
 return true;
 }
 }
 pragma solidity 0.7.6;
