 ...
 return _calcTotalAmountPerEpoch(epochId) .mul(_getUserBalancePerEpoch(account, epochId)) .div(epochs[epochId]);
 }
 ...
 return _calcTotalAmountPerEpoch(epochId) .mul(_getUserBalancePerEpoch(msg.sender, epochId)) .div(epochs[epochId]);
 }
