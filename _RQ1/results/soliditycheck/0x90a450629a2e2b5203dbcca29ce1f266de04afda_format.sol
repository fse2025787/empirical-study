 ...
 string public baseURI = "https: uint256 private range1Start = 1;
 ...
 constructor( string memory _name, string memory _symbol, uint256 _amount, address payable _creator ) ERC721(_name, _symbol) Royalties(_amount, _creator) {
 }
 function mint( address _to, uint256 _mintAmount ) public onlyMinter returns (bool) {
 require(_mintAmount > 0, "WoofpackNFTToken: mintAmount should be > 0");
 for (uint256 i = 1; i <= _mintAmount; i++) {
 tokenCounter++;
 if (tokenCounter >= range1Start && tokenCounter <= range1End) tokenCounter = range1End + 1;
 _safeMint(_to, tokenCounter);
 emit Mint(_to, tokenCounter);
 }
 return true;
 }
 function mintHoldToken(address _to, uint256 _tokenId) public onlyMinter returns (bool) {
 require( isHold(_tokenId), 'WoofpackNFTToken: This token is not reserved' );
 _safeMint(_to, _tokenId);
 emit Mint(_to, _tokenId);
 return true;
 }
 function mintHoldTokens(address _to, uint256[] memory _tokenIds) public onlyMinter returns (bool) {
 require(_tokenIds.length > 0, 'MintHoldTokens: Please provide token id');
 for (uint256 x = 0; x < _tokenIds.length; x++) {
 require( isHold(_tokenIds[x]), 'MintHoldTokens: Some tokens are not reserved' );
 }
 ...
 function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
 require( _exists(tokenId), "WoofpackNFTToken: URI query for nonexistent token" );
