 ...
 library SVG721 {
 function metadata( string memory tokenName, string memory tokenDescription, string memory svgString, string memory attributes ) internal pure returns (string memory) {
 string memory json = string(abi.encodePacked('{
 "name":"', tokenName, '","description":"', tokenDescription, '","image": "data:image/svg+xml;
 base64,', Base64.encode(bytes(svgString)),'",',attributes,'}
 '));
 return string(abi.encodePacked("data:application/json;
 base64,", Base64.encode(bytes(json))));
 }
 }
 pragma solidity ^0.8.0;
 ...
 constructor ( string memory _name, string memory _symbol, uint8[] memory _maxRedeemPerWallet, bytes32[] memory _merkleRoots, string memory _contractMetaDataURI, address _VRFCoordinator, address _LinkToken, bytes32 _keyHash ) VRFConsumerBase(_VRFCoordinator, _LinkToken) ERC721(_name, _symbol) {
 VRFCoordinator = _VRFCoordinator;
 ...
 function editRedemptionWindow( uint8 _windowID, bytes32 _merkleRoot, bool _open, uint8 _maxPerWallet ) external override onlyOwner {
 ...
 "trait_type": "',traitName,'" , "value": "',value.toString(),'"}
 '));
 }
