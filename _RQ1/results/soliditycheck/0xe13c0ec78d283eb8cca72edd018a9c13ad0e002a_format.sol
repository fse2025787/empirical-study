 ...
 accRewardPerShare = accRewardPerShare.add( _getPoolRewardsSinceLastUpdate(_pid).mul(precision).div(pool.totalStaked) );
 }
 ...
 require( poolToken[address(_token)] == false, 'MasterChefMod: Stake token has already been added' );
 totalAllocPoint = totalAllocPoint.add(_allocPoint);
 poolInfo.push( PoolInfo({
 token: _token, allocPoint: _allocPoint, lastUpdateTime: block.timestamp, accRewardPerShare: 0, totalStaked: 0, accUndistributedReward: 0 }
 ) );
 ...
 pool.accRewardPerShare = pool.accRewardPerShare.add( poolRewards.mul(precision).div(pool.totalStaked) );
 }
