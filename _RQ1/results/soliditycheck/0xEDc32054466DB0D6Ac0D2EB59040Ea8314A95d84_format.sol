 ...
 function setup_constructor(string calldata name,string calldata symbol, address _owner, address _coo, uint256 _value, uint256 _value1, string calldata _uri_prefix ) external ;
 }
 interface ITMFactory {
 function newTopMinter(string calldata name,string calldata symbol, uint256 _value, uint256 _value1 ) external ;
 ...
 function supportsInterface(bytes4 interfaceId) external view returns (bool) {
 return (interfaceId==_INTERFACE_ID_ERC165) ||(interfaceId==_ERC721_RECEIVED) ||(interfaceId==_INTERFACE_ID_ERC721) ||(interfaceId==_INTERFACE_ID_ERC721_ENUMERABLE) ||(interfaceId==_INTERFACE_ID_ERC721_METADATA) ;
 }
 ...
 function normalMintPOW(address account,uint256 amount,uint256 pow_index) external payable {
 require(POWMint>0,"Mustcall normalMint");
 require(verifyPOW(pow_index,amount),"verifyPOW fail");
 _normalMint(account, amount);
 }
 function normalMint(address account,uint256 amount)nonReentrant external payable {
 require(POWMint==0,"Mustcall normalMintPOW");
 _normalMint(account, amount);
 }
 function _normalMint(address account,uint256 amount) internal {
 ...
 function setWhiteList(bytes32 _mr, uint256 _nftWLNumber, uint256 _nftWLPriceinETH, uint256 _nftWLMintStartTime, uint256 _nftWLMintEndTime, uint256 _WLsingleAddressLimit, uint256 _WLsingleTimeLimit) external onlyOwner {
 merkleRoot=_mr;
