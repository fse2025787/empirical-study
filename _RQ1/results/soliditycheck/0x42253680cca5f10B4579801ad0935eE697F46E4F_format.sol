 ...
 constructor( address _uniswapV3Factory, address _weth, uint24 _defaultFee ) {
 uniswapV3Factory = _uniswapV3Factory;
 weth = _weth;
 defaultFee = _defaultFee;
 }
 function assetToEth( address _tokenIn, uint256 _amountIn, uint32 _twapPeriod ) public view returns (uint256 ethAmountOut) {
 return _fetchTwap(_tokenIn, weth, defaultFee, _twapPeriod, _amountIn);
 }
 function ethToAsset( uint256 _ethAmountIn, address _tokenOut, uint32 _twapPeriod ) public view returns (uint256 amountOut) {
 return _fetchTwap(weth, _tokenOut, defaultFee, _twapPeriod, _ethAmountIn);
 }
 function assetToAsset( address _tokenIn, uint256 _amountIn, address _tokenOut, uint32 _twapPeriod ) public view returns (uint256 amountOut) {
 if (_tokenIn == weth) {
 ...
 function assetToAssetThruRoute( address _tokenIn, uint256 _amountIn, address _tokenOut, uint32 _twapPeriod, address _routeThruToken, uint24[2] memory _poolFees ) public view returns (uint256 amountOut) {
 ...
 function _fetchTwap( address _tokenIn, address _tokenOut, uint24 _poolFee, uint32 _twapPeriod, uint256 _amountIn ) internal view returns (uint256 amountOut) {
 address pool = PoolAddress.computeAddress(uniswapV3Factory, PoolAddress.getPoolKey(_tokenIn, _tokenOut, _poolFee));
 int256 twapTick = OracleLibrary.consult(pool, _twapPeriod);
 return OracleLibrary.getQuoteAtTick( int24(twapTick), SafeUint128.toUint128(_amountIn), _tokenIn, _tokenOut );
 }
 }
