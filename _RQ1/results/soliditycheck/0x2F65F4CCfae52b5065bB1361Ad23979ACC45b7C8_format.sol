 ...
 string public baseURI = "https: uint public price = 0.003 ether;
 ...
 return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI,Strings.toString(_tokenId+1),".json")) : "";
 }
 ...
 uint256 cost = price;
 bool isFree = ((totalFreeMinted + count < totalFree + 1) && (_mintedFreeAmount[msg.sender] < maxPerFree));
 if (isFree) {
 ...
 function CommunityWallet(uint quantity, address user) public onlyOwner {
