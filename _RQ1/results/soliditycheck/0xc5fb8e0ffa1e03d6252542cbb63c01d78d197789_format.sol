 ...
 contract OneEther {
 event onOpenNewBet( uint256 indexed bID, address owner, uint256 check, uint256 unit, uint256 recordTime );
 event onEditBet( uint256 indexed bID, address owner, uint256 check, uint256 unit, uint256 recordTime );
 event onOpenNewRound( uint256 indexed bID, uint256 indexed rID, uint256 total, uint256 current, uint256 ethAmount, uint256 recordTime );
 event RoundMask( uint256 rID, bytes32 hashmask );
 event onReveal( uint256 indexed rID, address winner, uint256 reward, uint256 teamFee, uint256 scretNumber, uint256 randomNumber, uint256 recordTime );
 event onBuyBet( uint256 indexed bID, uint256 indexed rID, address playerAddress, uint256 amount, uint256 key, uint256 playerCode, uint256 invator, uint256 recordTime );
 event onRoundUpdate( uint256 indexed bID, uint256 indexed rID, uint256 totalKey, uint256 currentKey, uint256 lastUpdate );
 event onRoundEnd( uint256 indexed bID, uint256 indexed rID, uint256 lastUpdate );
 event onWithdraw ( uint256 indexed playerID, address indexed playerAddress, uint256 ethOut, uint256 recordTime );
 event onRegistLink ( uint256 indexed playerID, address indexed playerAddress, uint256 recordTime );
 event onBuyFailed ( uint256 indexed playerID, uint256 indexed rID, uint256 ethIn, uint256 recordTime );
 using SafeMath for *;
 ...
 modifier isAdmin(){
 require(msg.sender == admin,"its can only be call by admin");
 _;
 }
 ...
 require(activated_ == false,"the game is running");
 activated_ = true;
 }
 function close() isAdmin() isActivated() public{
 activated_ = false;
 }
 ...
 function openNewBet(address _owner,uint256 _check,uint256 _unit) isAdmin() isActivated() public {
 ...
 function openFirstRound(uint256 _bID,bytes32 _maskHash) public isbetActivated(_bID) {
 ...
 function closeBet(uint256 _bID) public {
 address addr = msg.sender;
 require(bIDBet_[bID].bID != 0,"cant find this bet");
 require(bIDBet_[bID].owner == addr || bIDBet_[bID].owner == admin,"Permission denied");
 bIDBet_[_bID].isActivated = false;
 }
 function openBet(uint256 _bID) public {
 address addr = msg.sender;
 require(bIDBet_[bID].bID != 0,"cant find this bet");
 require(bIDBet_[bID].owner == addr || bIDBet_[bID].owner == admin,"Permission denied");
 ...
 function editBet(uint256 _bID,uint256 _check,uint256 _unit) public {
 require((_check >= MIN_BUY) && (_check <= MAX_BUY),"out of range");
 address addr = msg.sender;
 ...
 function withdrawFee() isAdmin() public {
 uint256 temp = teamFee_;
 teamFee_ = 0;
 msg.sender.transfer(temp);
 }
 function buySome(uint256 _rID,uint256 _key,uint256 _playerCode,uint256 _linkPID) public payable {
 ...
 function buyWithBalance(uint256 _rID,uint256 _key,uint256 _playerCode) public payable {
 ...
 function reveal(uint256 _rID,uint256 _scretKey,bytes32 _maskHash) public {
 require(rIDBet_[_rID].rID != 0,"cant find this round");
 uint256 _bID = rIDBet_[_rID].bID;
 require(bIDBet_[_bID].bID != 0,"cant find this bet");
 ...
 function getPlayerByAddr(address _addr) public view returns(uint256,uint256,bool) {
 uint256 _pID = pIDAddr_[_addr];
 return (_pID,pIDPlayer_[_pID].balance,pIDPlayer_[_pID].VIP);
 }
 function getRoundInfoByID(uint256 _rID) public view returns(uint256,uint256,uint256,uint256,uint256,bytes32,uint256) {
 return ( rIDBet_[_rID].rID, rIDBet_[_rID].bID, rIDBet_[_rID].total, rIDBet_[_rID].current, rIDBet_[_rID].ethAmount, rIDBet_[_rID].maskHash, rIDBet_[_rID].playerCode );
 }
 function getBetInfoByID(uint256 _bID) public view returns(uint256,uint256,address,uint256,uint256,bool) {
 return ( bIDrID_[_bID], bIDBet_[_bID].bID, bIDBet_[_bID].owner, bIDBet_[_bID].check, bIDBet_[_bID].unit, bIDBet_[_bID].isActivated );
 }
 function getBIDList() public view returns(uint256[]) {
 return(bIDList_);
 }
 function withdraw() isActivated() public {
 uint256 _now = now;
 uint256 _pID = pIDAddr_[msg.sender];
 uint256 _eth;
 if(_pID != 0) {
 ...
 function manageKey(uint256 _pID,uint256 _rID,uint256 _key) private {
 ...
 function manageLink(uint256 _pID,uint256 _value) private returns(uint256) {
 ...
 function managePID(uint256 _linkPID) private returns (uint256) {
 uint256 _pID = pIDAddr_[msg.sender];
 if(_pID == 0) {
 pID++;
 ...
 function newRound(uint256 _bID,bytes32 _maskHash) private {
