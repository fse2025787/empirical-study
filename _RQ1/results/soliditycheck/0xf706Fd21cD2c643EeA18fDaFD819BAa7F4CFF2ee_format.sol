 ...
 contract LendFlareVotingEscrow is Initializable, ReentrancyGuard, ILendFlareVotingEscrow {
 ...
 enum DepositTypes {
 DEPOSIT_FOR_TYPE, CREATE_LOCK_TYPE, INCREASE_LOCK_AMOUNT, INCREASE_UNLOCK_TIME }
 struct Point {
 uint256 bias;
 uint256 slope;
 uint256 ts;
 ...
 function _checkpoint(address _sender, LockedBalance storage _newLocked) internal {
 ...
 function _depositFor( address _sender, uint256 _amount, uint256 _unlockTime, LockedBalance storage _locked, DepositTypes _depositTypes ) internal {
 ...
 require( availableTime > block.timestamp, "can only lock until time in the future" );
 require( availableTime <= block.timestamp + MAXTIME, "voting lock can be 4 years max" );
 _depositFor( msg.sender, _amount, availableTime, locked, DepositTypes.CREATE_LOCK_TYPE );
 }
 ...
 require( availableTime <= block.timestamp + MAXTIME, "Voting lock can be 4 years max" );
 _depositFor( msg.sender, 0, availableTime, locked, DepositTypes.INCREASE_UNLOCK_TIME );
 }
