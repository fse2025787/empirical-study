 ...
 function setTierData( uint256 _index, uint256 _priceInCenter, uint256 _priceInOuter) public onlyGlobalAdmin() {
 ...
 constructor () public {
 innerScope = InnerScope( 12, 11, 67, 11, 12, 34, 67, 34 );
 }
 ...
 function mint( address to, uint x, uint y, uint sizeA, uint sizeB, uint256 totalPrice, uint256 actualPrice ) public onlyPlatform() returns (uint256) {
 ...
 event ArtWorkChangeRequestMade( address indexed _fromUser, uint256 _tokenId, uint256 _changeId, string _meta, uint256 _platformTransfer, uint256 _timestamp);
 event ArtWorkChangeRequestApproved( address indexed _fromUser, uint256 _tokenId, uint256 _changeId, string _meta, uint256 _timestamp);
 event ArtWorkChangeRequestDeclined( address indexed _fromUser, uint256 _tokenId, uint256 _changeId, string _meta, uint256 _timestamp);
 ...
 constructor( address _platformWallet, address _token, address _erc20token, address _pricingStrategy, address _signer ) public {
 ...
 tiers.push( Tier( 400, 600, 1 ether, 0.4 ether, 0, 0, false, false ) );
 tiers.push( Tier( 400, 600, 1.2 ether, 0.6 ether, 0, 0, false, false ) );
 tiers.push( Tier( 400, 600, 1.4 ether, 0.8 ether, 0, 0, false, false ) );
 tiers.push( Tier( 144, 288, 1.6 ether, 1.0 ether, 0, 0, false, false ) );
 }
 ...
 function makeOffer( uint256 _tokenId, uint8 _v, bytes32 _r, bytes32 _s ) public notHalted() payable {
 address recoveredSigner = verify(keccak256(msg.sender), _v, _r, _s);
 require(signers[recoveredSigner] == true);
 ...
 function artWorkChangeRequest(uint256 _tokenId, string _meta, uint8 _v, bytes32 _r, bytes32 _s) public payable returns (uint256) {
 ...
 artWorkChangeRequests.push( ArtWorkChangeRequest(msg.sender, _tokenId, changeRequestCounter, _meta, now, false) );
 ...
 emit ArtWorkChangeRequestApproved( request.fromUser, request.tokenId, request.changeId, request.meta, now );
 }
 else {
 emit ArtWorkChangeRequestDeclined( request.fromUser, request.tokenId, request.changeId, request.meta, now );
 }
 }
 ...
 emit ArtWorkChangeRequestApproved( msg.sender, _tokenId, _changeId, _meta, now );
 }
 ...
 (platformShare, forPrevOwner) = pricingStrategy.calculateSharesInTheRevenue( soldPrice, localOffer.offeredPrice);
