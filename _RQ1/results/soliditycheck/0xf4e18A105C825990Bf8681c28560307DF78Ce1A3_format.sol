 ...
 pair = address(uint(keccak256(abi.encodePacked( hex'ff', factory, keccak256(abi.encodePacked(token0, token1)), hex'9caea71a4e9798d7bbdf720c7f8b2d9b63e1f0522376b899ba0c8f6c9737c731' ))));
 }
 ...
 (amountToken, amountETH) = _addLiquidity( token, NUS_WETH, amountTokenDesired, msg.value, amountTokenMin, amountETHMin );
 address pair = NimbusLibrary.pairFor(factory, token, NUS_WETH);
 TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
 INUS_WETH(NUS_WETH).deposit{
 value: amountETH}
 ();
 ...
 (amountToken, amountETH) = removeLiquidity( token, NUS_WETH, liquidity, amountTokenMin, amountETHMin, address(this), deadline );
 ...
 (, amountETH) = removeLiquidity( token, NUS_WETH, liquidity, amountTokenMin, amountETHMin, address(this), deadline );
 ...
 INimbusPair(NimbusLibrary.pairFor(factory, input, output)).swap( amount0Out, amount1Out, to, new bytes(0) );
 }
 }
 ...
 TransferHelper.safeTransferFrom( path[0], msg.sender, NimbusLibrary.pairFor(factory, path[0], path[1]), amounts[0] );
 ...
 TransferHelper.safeTransferFrom( path[0], msg.sender, NimbusLibrary.pairFor(factory, path[0], path[1]), amountIn );
 uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
 _swapSupportingFeeOnTransferTokens(path, to);
 require( IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin, 'NimbusRouter: INSUFFICIENT_OUTPUT_AMOUNT' );
 }
