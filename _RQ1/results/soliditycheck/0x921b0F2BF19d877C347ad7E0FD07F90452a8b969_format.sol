 ...
 pragma solidity ^0.8.0;
 contract InspirationRedBorder is ERC721, Ownable {
 modifier contractIsNotFrozen() {
 require(isFrozen == false, "This function can not be called anymore");
 _;
 }
 uint16 public totalTokens = 75;
 uint16 public totalSupply = 0;
 string private baseURI = "https: bool private isFrozen = false;
 mapping(uint16 => uint16) private tokenMatrix;
 constructor() ERC721("InspirationRedBorder", "IRB") {
 }
 function setBaseTokenURI(string memory _uri) external onlyOwner contractIsNotFrozen {
 baseURI = _uri;
 }
 ...
 require( getAvailableTokens() >= _addresses.length, "No tokens left to be minted" );
 ...
 function setTotalTokens(uint16 _totalTokens) external onlyOwner contractIsNotFrozen {
 ...
 tokenMatrix[maxIndex - 1] == 0 ? tokenMatrix[random] = maxIndex - 1 : tokenMatrix[random] = tokenMatrix[maxIndex - 1];
 return tokenId;
 }
 function _getRandomNumber(uint16 _upper, uint16 _totalMintedTokens) private view returns (uint16) {
 uint16 random = uint16( uint256( keccak256( abi.encodePacked( _totalMintedTokens, blockhash(block.number - 1), block.coinbase, block.difficulty, msg.sender ) ) ) );
