 ...
 contract GenArt is Ownable, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
 ...
 uint256 MEMBERSHIP_PRICE = 100000000000000000;
 uint256 MEMBERSHIP_GOLD_PRICE = 500000000000000000;
 ...
 constructor( string memory name_, string memory symbol_, string memory uri_standard_, string memory uri_gold_, uint256 max_members_ ) public {
 _name = name_;
 _symbol = symbol_;
 ...
 require( _tokenId <= MAX_MEMBERS + MAX_MEMBERS_GOLD, "mint would exceed totalSupply" );
 ...
 uint256 _totalSupplyGold = totalGoldOwners();
 uint256 _totalSupply = totalSupply() - _totalSupplyGold;
 require( (_totalSupply + _amount) <= MAX_MEMBERS, "mint would exceed totalSupply" );
 ...
 uint256 _totalSupply = totalGoldOwners();
 require( _totalSupply < MAX_MEMBERS_GOLD, "mint exceeds Gold totalSupply" );
 _mintGold(_to);
 _reservedTokensGold = _reservedTokensGold - 1;
 }
 function mintReserved(address _to, uint256 _amount) public onlyOwner {
 require( _amount <= _reservedTokens, "reserved token mint exceeds limit" );
 uint256 _totalSupply = totalSupply();
 require( (_totalSupply + _amount) <= MAX_MEMBERS, "mint exceeds totalSupply" );
 ...
 require( sender == owner || GenArt.isApprovedForAll(owner, sender), "approve caller is not owner nor approved for all" );
 _approve(to, tokenId);
 }
 ...
 address sender = _msgSender();
 require( _isApprovedOrOwner(sender, tokenId), "transfer caller is not owner nor approved" );
 ...
 address owner = GenArt.ownerOf(tokenId);
 return (spender == owner || getApproved(tokenId) == spender || GenArt.isApprovedForAll(owner, spender));
 }
 ...
 require( GenArt.ownerOf(tokenId) == from, "transfer of token that is not own" );
 ...
 bytes memory returndata = to.functionCall( abi.encodeWithSelector( IERC721Receiver(to).onERC721Received.selector, _msgSender(), from, tokenId, _data ), "transfer to non ERC721Receiver implementer" );
 bytes4 retval = abi.decode(returndata, (bytes4));
 return (retval == _ERC721_RECEIVED);
 }
 function _approve(address to, uint256 tokenId) internal virtual {
 _tokenApprovals[tokenId] = to;
 emit Approval(GenArt.ownerOf(tokenId), to, tokenId);
 }
 }
