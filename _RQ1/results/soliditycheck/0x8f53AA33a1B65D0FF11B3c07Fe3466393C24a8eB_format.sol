 ...
 constructor( string memory initBaseURI, string memory initNotRevealedUri, address _airdropuser, bytes32 _whiteListRootHash, uint256 _currentSupply ) ERC721("ClubFanX", "CFX") {
 ...
 require( totalSupply() + tokenQuantity <= currentSupply, "Sale would exceed max current supply" );
 ...
 function whitelistMint( uint256 tokenQuantity, bytes32[] calldata _merkleProof ) public {
 require(!whitelistClaimed[msg.sender], "address claimed");
 require(_isWLSaleActive, "Whitelist Sale must be active to mint");
 require( balanceOf(msg.sender) + tokenQuantity <= maxWLBalance, "Sale would exceed max Whitelist balance" );
 require( totalSupply() + tokenQuantity <= currentSupply, "Sale would exceed max current supply" );
 bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
 require( MerkleProof.verify(_merkleProof, whiteListRootHash, leaf), "Invalid Proof" );
 ...
 function airdrop(uint256 tokenQuantity, address owner) public payable onlyOwner {
 require( totalSupply() + tokenQuantity <= MAX_SUPPLY, "Sale would exceed max supply" );
 require( balanceOf(msg.sender) + tokenQuantity <= maxBalance, "Sale would exceed max balance" );
 require( tokenQuantity * mintPrice <= msg.value, "Not enough ether sent" );
 require(tokenQuantity <= maxMint, "Can only mint 1 tokens at a time");
