 ...
 event TokenPurchase( address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount, uint256 now );
 ...
 emit TokenPurchase( msg.sender, _beneficiary, weiAmount, tokens, now );
 ...
 function AirbonCrowdsale(uint256 _starttime, uint256 _endTime, uint256 _rate, address _wallet,ERC20 _token) TimedCrowdsale(_starttime,_endTime)Crowdsale(_rate, _wallet,_token)CappedCrowdsale(cap) {
 }
 function () external payable {
 buyTokens(msg.sender);
 }
