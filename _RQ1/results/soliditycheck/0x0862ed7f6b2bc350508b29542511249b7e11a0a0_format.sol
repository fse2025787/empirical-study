 ...
 function viewStakingDetails(address _user) external view virtual returns ( address[] memory, bool[] memory, uint256[] memory, uint256[] memory, uint256[] memory );
 }
 pragma solidity ^0.7.6;
 ...
 event TokenDetails( address indexed tokenAddress, uint256 userStakeimit, uint256 totalStakeLimit, uint256 Time );
 event WithdrawDetails( address indexed tokenAddress, uint256 withdrawalAmount, uint256 time );
 constructor() Ownable(msg.sender) {
 }
 function setDailyDistribution( address[] memory stakedToken, address[] memory rewardToken, uint256[] memory dailyDistribution ) public onlyOwner {
 require( stakedToken.length == rewardToken.length && rewardToken.length == dailyDistribution.length, "Invalid Input" );
 for (uint8 i = 0; i < stakedToken.length; i++) {
 tokenDailyDistribution[stakedToken[i]][ rewardToken[i] ] = dailyDistribution[i];
 }
 }
 function updateSequence( address stakedToken, address[] memory rewardTokenSequence ) public onlyOwner {
 tokensSequenceList[stakedToken] = new address[](0);
 for (uint8 i = 0; i < rewardTokenSequence.length; i++) {
 require( rewardTokenSequence.length <= intervalDays.length, " Invalid Index" );
 ...
 require( IERC20(tokenAddress).balanceOf(address(this)) >= amount, "SAFEWITHDRAW: Insufficient Balance" );
 require( IERC20(tokenAddress).transfer(_owner, amount) == true, "SAFEWITHDRAW: Transfer failed" );
 ...
 function setLegacyU1Addresses(address u1_, address u1Proxy_) external onlyOwner returns (bool) {
 u1 = UnifarmV1(u1_);
 ...
 event UnStake( address indexed userAddress, address indexed unStakedtokenAddress, uint256 unStakedAmount, uint256 time, uint256 stakeID );
 ...
 require( IERC20(tokenAddress).balanceOf(address(this)) >= amount, "SEND : Insufficient Balance" );
 ...
 uint256 actualUnStaking = u1UpgradeTotalUnStaking[tokenAddress] .add(u1Proxy.totalUnStakingB(tokenAddress)) .add(u1Proxy.totalUnStakingA(tokenAddress));
 ...
 ( address[] memory tokenAddress, bool[] memory activeStatus, , uint256[] memory stakedAmount, uint256[] memory startTime ) = (u1.viewStakingDetails(user));
 bool isAlreadyUnstaked = u1Proxy.unstakeStatus(user, stakeId);
 if ( u1UnstakeStatus[user][stakeId] == false && activeStatus[stakeId] == true && isAlreadyUnstaked == false ) u1UnstakeStatus[user][stakeId] = true;
 else revert("UNSTAKE : Unstaked Already");
 uint256 actualStaking = totalStaking(tokenAddress[stakeId]);
 u1UpgradeTotalUnStaking[tokenAddress[stakeId]] = u1UpgradeTotalUnStaking[ tokenAddress[stakeId] ] .add(stakedAmount[stakeId]);
 require( IERC20(tokenAddress[stakeId]).balanceOf(address(this)) >= stakedAmount[stakeId], "UNSTAKE : Insufficient Balance" );
 IERC20(tokenAddress[stakeId]).transfer(user, stakedAmount[stakeId]);
 if (startTime[stakeId] < poolStartTime.add(u1.stakeDuration())) {
 claimRewards( user, startTime[stakeId], stakedAmount[stakeId], tokenAddress[stakeId], actualStaking );
 }
 emit UnStake( user, tokenAddress[stakeId], stakedAmount[stakeId], block.timestamp, stakeId );
 }
 ...
 if (interval >= DAYS) _rewardCalculation(user, stakedAmount, interval, stakedToken, totalStake);
 }
 ...
 rewardsEarned = noOfDays.mul( getOneDayReward(stakedAmount, stakedToken, stakedToken, totalStake) );
 ...
 if ( rewardToken != stakedToken && u1.tokenBlockedStatus(stakedToken, rewardToken) == false ) {
 rewardsEarned = balDays.mul( getOneDayReward(stakedAmount, stakedToken, rewardToken, totalStake) );
 ...
 ( address[] memory tokenAddress, bool[] memory activeStatus, , uint256[] memory stakedAmount, ) = (u1.viewStakingDetails(userAddress));
 bool isAlreadyUnstaked = u1Proxy.unstakeStatus(userAddress, stakeId);
 if ( u1UnstakeStatus[userAddress][stakeId] == false && isAlreadyUnstaked == false && activeStatus[stakeId] == true ) u1UnstakeStatus[userAddress][stakeId] = true;
 else revert("EMERGENCY: Unstaked Already");
 require( IERC20(tokenAddress[stakeId]).balanceOf(address(this)) >= stakedAmount[stakeId], "EMERGENCY : Insufficient Balance" );
 IERC20(tokenAddress[stakeId]).transfer(userAddress, stakedAmount[stakeId]);
 for (uint256 i; i < rewardtokens.length; i++) {
 require( IERC20(rewardtokens[i]).balanceOf(address(this)) >= amount[i], "EMERGENCY : Insufficient Reward Balance" );
 sendToken(userAddress, tokenAddress[stakeId], rewardtokens[i], amount[i]);
 }
 u1UpgradeTotalUnStaking[tokenAddress[stakeId]] = u1UpgradeTotalUnStaking[ tokenAddress[stakeId] ] .add(stakedAmount[stakeId]);
 emit UnStake( userAddress, tokenAddress[stakeId], stakeId, stakedAmount[stakeId], block.timestamp );
 }
