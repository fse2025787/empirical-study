 ...
 assembly {
 let loc := add(result, 0x20) return(loc, size) }
 }
 receive() external payable {
 require(Address.isContract(msg.sender), "Not allowed from EOA");
 }
 function batchExec( address[] calldata tos, bytes32[] calldata configs, bytes[] memory datas ) external payable override isNotHalted isNotBanned {
 _preProcess();
 _execs(tos, configs, datas);
 _postProcess();
 }
 function execs( address[] calldata tos, bytes32[] calldata configs, bytes[] memory datas ) external payable override isNotHalted isNotBanned isInitialized {
 ...
 assembly {
 let loc := add(add(data, 0x20), offset) let m := mload(loc) if iszero(iszero(m)) {
 let p := mul(m, ref) if iszero(eq(div(p, m), ref)) {
 revert(0, 0) }
 ref := div(p, base) }
 mstore(loc, ref) }
 }
 }
 function _parse( bytes32[256] memory localStack, bytes memory ret, uint256 index ) internal pure returns (uint256 newIndex) {
 uint256 len = ret.length;
 require(len % 32 == 0, "illegal length for _parse");
 newIndex = index + len / 32;
 require(newIndex <= 256, "stack overflow");
 assembly {
 let offset := shl(5, index) for {
 let i := 0 }
 lt(i, len) {
 i := add(i, 0x20) }
 {
 ...
 _addCubeCounter();
 assembly {
 let succeeded := delegatecall( sub(gas(), 5000), _to, add(_data, 0x20), mload(_data), 0, 0 ) let size := returndatasize() result := mload(0x40) mstore( 0x40, add(result, and(add(add(size, 0x20), 0x1f), not(0x1f))) ) mstore(result, size) returndatacopy(add(result, 0x20), 0, size) switch iszero(succeeded) case 1 {
