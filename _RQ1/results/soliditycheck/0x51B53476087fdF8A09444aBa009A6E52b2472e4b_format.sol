 ...
 n = uint(keccak256(abi.encodePacked( tx.origin, blockhash(block.number - 1), block.timestamp, MAGIC_NUMBER, seed )));
 ...
 uint g = (v >> 8) & 0x00ff;
 uint b = v & 0xff;
 string memory styles = string(abi.encodePacked( '<style>.color{
 fill:rgb(', r.toString(), ' ', g.toString(), ' ', b.toString(), ');
 }
 </style>' ));
 return string(abi.encodePacked( '<svg xmlns="http: styles, image, "</svg>" ));
 }
 function compileAttributes(uint256 tokenId) public view returns (string memory) {
 return string(abi.encodePacked( '[', '{
 "trait_type":"Alpha",', '"value":', alphas[tokenId].toString(), '}
 ]' ));
 }
 function tokenURI(uint256 tokenId) public override view returns (string memory) {
 require(_exists(tokenId), "Nonexistent people");
 string memory metadata = string(abi.encodePacked( '{
 "name": "', 'PEOPLE #', tokenId.toString(), '", "description": "We the people, for the people.", "image": "data:image/svg+xml;
 base64,', base64(bytes(drawSVG(tokenId))), '", "attributes":', compileAttributes(tokenId), "}
 " ));
 ...
 assembly {
 mstore(result, encodedLen) let tablePtr := add(table, 1) let dataPtr := data let endPtr := add(dataPtr, mload(data)) let resultPtr := add(result, 32) for {
 }
 lt(dataPtr, endPtr) {
 }
 {
 dataPtr := add(dataPtr, 3) let input := mload(dataPtr) mstore(resultPtr, shl(248, mload(add(tablePtr, and(shr(18, input), 0x3F))))) resultPtr := add(resultPtr, 1) mstore(resultPtr, shl(248, mload(add(tablePtr, and(shr(12, input), 0x3F))))) resultPtr := add(resultPtr, 1) mstore(resultPtr, shl(248, mload(add(tablePtr, and(shr( 6, input), 0x3F))))) resultPtr := add(resultPtr, 1) mstore(resultPtr, shl(248, mload(add(tablePtr, and( input, 0x3F))))) resultPtr := add(resultPtr, 1) }
