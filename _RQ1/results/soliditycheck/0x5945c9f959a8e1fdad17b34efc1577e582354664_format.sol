 ...
 address payable public treasuryWallet = payable(0x505184CFBf416016e4EF0d2291C5097a48722fF0);
 ...
 event SwapAndLiquify( uint256 tokensSwappedForEth, uint256 ethAddedForLp, uint256 tokensAddedForLp );
 ...
 if ( balanceOf(address(this)) >= swapAtAmount && !_inSwapAndLiquify && from != uniswapV2Pair && swapAndTreasureEnabled ) {
 ...
 if ( to != owner() && to != uniswapV2Pair && to != address(uniswapV2Router) ) {
 ...
 function setTreasuryFeePercent(uint256 _newFeeOnBuy, uint256 _newFeeOnSell) external onlyOwner {
 require( _newFeeOnBuy <= 100 && _newFeeOnSell <= 100, "fee cannot exceed 100%" );
