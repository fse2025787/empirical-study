 ...
 function getReplay(List storage ds,uint256 idx) internal view returns( uint256 , uint256, uint256[], uint, uint256, uint256, uint256 ) {
 uint256 ansHash = ds.hashIds[idx];
 uint256 sellPrice = ds.sellPriceMap[ansHash];
 Data storage d= ds.map[ansHash];
 uint256 vt = getVisibleType(ds,ansHash);
 return ( d.aCount, d.bCount, vt!=VISABLE_NONE ? d.answer : new uint256[](0), now-d.replyAt, vt, sellPrice, vt!=VISABLE_NONE ? ansHash : 0 );
 }
 function listBestScore(List storage ds) internal view returns( uint256 aCount , uint256 bCount , uint256 bestCount ){
 uint256 sorce = 0;
 for(uint256 i=0; i<ds.size; i++){
 Data storage d = get(ds,i);
