 ...
 function initialize( address _ownerAddress, address _priceOracle, uint256 _zonePerMinute, uint256 _minDepositAmountInEth, address[] memory _nftAddresses, uint256[] memory _nftModels, uint256[] memory _nftPrices ) public initializer {
 ...
 function setRewardNfts( address[] memory _contractAddresses, uint256[] memory _modelIds, uint256[] memory _pricesInEth ) external onlyOwner() {
 _setRewardNfts(_contractAddresses, _modelIds, _pricesInEth);
 }
 function _setRewardNfts( address[] memory _contractAddresses, uint256[] memory _modelIds, uint256[] memory _pricesInEth ) internal {
 require( _contractAddresses.length == _modelIds.length && _contractAddresses.length == _pricesInEth.length, "Mismatched data" );
 ...
 lpTokenAddress: _lpTokenAddress, lpSupply: 0, poolWeight: _poolWeight, lastRewardTime: lastRewardTime, accZONEPerLP: 0, pid: poolLength() }
 );
 pool.push(newPool_);
 ...
 function getStakeInfo(uint256 _pid, address _account) external view returns ( uint256 stakedAmount, uint256 claimIn, uint256 rewardAmount, address rewardNftAddress, uint256 rewardNftModel, uint256 rewardNftPrice ) {
 User storage user_ = user[_pid][_account];
 if (user_.lpAmount == 0) {
 ...
 function initialize( address _nymLib, address _priceOracle, address _ownerAddress, string memory _name, string memory _symbol, string[] memory _metafileUris, uint256 _capacity, uint256 _price, bool _nameChangeable, bool _colorChangeable, bytes4[] memory _color ) external;
