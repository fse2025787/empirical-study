 ...
 pragma solidity ^0.8.0;
 contract maria is Context, IERC20, Ownable {
 ...
 uint256 public maxTxAmountBuy = _tTotal / 1;
 uint256 public maxTxAmountSell = _tTotal / 1;
 uint256 public maxWalletAmount = _tTotal / 1;
 ...
 string private constant _name = "Maria Shibazawa";
 string private constant _symbol = "SHIBZAWA";
 ...
 rfi: 0, dev: 4, treasury: 4, lp: 2, toSwap: 10 }
 );
 ...
 uint256 ETHToAddLiquidityWith = (deltaBalance * appliedRates.lp) / (denominator - appliedRates.lp);
 addLiquidity(tokensToAddLiquidityWith, ETHToAddLiquidityWith);
 uint256 remainingBalance = address(this).balance;
 uint256 devFee = (remainingBalance * appliedRates.dev) / (denominator - appliedRates.dev);
 uint256 treasuryFee = (remainingBalance * appliedRates.treasury) / (denominator - appliedRates.treasury);
 ...
 value: ETHAmount}
 ( address(this), tokenAmount, 0, 0, devAddress, block.timestamp );
 ...
 function setBuyFees( uint8 _rfi, uint8 _treasury, uint8 _dev, uint8 _lp ) external onlyOwner {
 buyRates.rfi = _rfi;
 ...
 function setSellFees( uint8 _rfi, uint8 _treasury, uint8 _dev, uint8 _lp ) external onlyOwner {
 sellRates.rfi = _rfi;
 ...
 function setMaxTransactionAmount( uint256 _maxTxAmountBuyPct, uint256 _maxTxAmountSellPct ) external onlyOwner {
